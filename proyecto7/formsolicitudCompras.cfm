...
<cfloop query="workload">
    ...
    <cfif rsExisteNRP.cantidad eq 0>
    	<cfif workload.ProcessInstanceId eq 0> 
        	 <cfset hayrechazo = true>
        </cfif>
...
<cfloop query="SolicitudCNRP">
    	<cfif SolicitudCNRP.ProcessInstanceId eq 0> 
        	 <cfset hayrechazo = true>
        </cfif>
		<cfif soy_responsable or request.lista_actividades_superuser>
            <cfif SolicitudCNRP.State EQ "INACTIVE" or State EQ "SUSPENDED">
                <cfset botonstart= true>
            <cfelseif SolicitudCNRP.State EQ "ACTIVE">
                <cfset botonfinish= true>
            <cfelseif SolicitudCNRP.State EQ "COMPLETED">
                <cfinvoke component="sif.Componentes.Workflow.Management" 
                          method="getAllowedTransitions" 
                          returnvariable="rsTrans">
                    <cfinvokeargument name="ActivityInstanceId" value="#SolicitudCNRP.ActivityInstanceId#">
                </cfinvoke>
                <cfset workload_ProcessInstanceId = SolicitudCNRP.ProcessInstanceId>
                <cfset workload_ActivityInstanceId = SolicitudCNRP.ActivityInstanceId>
                <cfloop query="rsTrans">
                    <cfif findNoCase("ACEPTA",name) OR findNoCase("APROB",name) OR findNoCase("APRUEB",name) OR findNoCase("APLICA",name) OR findNoCase("AUTORI",name)
                       OR findNoCase("RATIFIC",name) OR findNoCase("CONCEN",name) OR findNoCase("POSITIV",name) OR findNoCase("CONFIRM",name)
                       OR name EQ "OK" OR findNoCase("ACCEPT",name) OR findNoCase("APPROV",name) OR findNoCase("AGREE",name)  OR findNoCase("PASS",name)>
	...
	<table border="0" cellspacing="0" cellpadding="3" width="100%">
		...
		<cfoutput>
			...
			<tr class="tituloListas">
                ...
                <cfif hayrechazo>
                	<td width="40"><strong><font style="font-size:10px">NRP</font></strong></td>
                </cfif>                
				<cfif botonstart>
                ...
                <cfif botonaprobar or SolicitudCNRP.recordcount gt 0>
        ...
        <cfif rsExisteNRP.cantidad eq 0>
			...
			<tr <cfif CurrentRow MOD 2>class="listaNon"<cfelse>class="listaPar"</cfif>
				...
				<td nowrap valign="top" align="left" ><font style="font-size:10px">
				<cfif workload.ProcessInstanceId neq 0> 
                	#workload.ProcessInstanceId# 
                <cfelse>
                    #workload.ActivityId#
                </cfif>
                </font>
				...
				<td nowrap valign="top" align="left" >
					<font style="font-size:10px">
                    <cfif len(trim(workload.StartTime)) gt 0> 
                    ...
                    <cfelse>
                       <a href="##" tabindex="-1">
                        <img src="/cfmx/sif/imagenes/Page_Load.gif" 
                            ...
                            border="0" 
                            align="absmiddle" 
                            onClick='javascript: detalleRechazo(#workload.ActivityId#,#workload.ProcessId#);'>	
                        </a>
                    </cfif>	   
				</td>				
				<td valign="top">
                	<cfif workload.ProcessInstanceId neq 0> 
                     ...
                     <cfelseif workload.ProcessInstanceId eq 0>
                       <a href="##" tabindex="-1">
                        <img src="/cfmx/sif/imagenes/next_year.gif"
                            title="#BTN_Seguimiento#" 
                            name="imagen" 
                            border="0" 
                            align="absmiddle" 
                            onClick='javascript: seguimientoRechazo(#workload.ActivityInstanceId#);'>	
                        </a>
					</cfif>	
                ...
                <cfelseif hayrechazo>
                  ...
                  <cfif workload.ProcessInstanceId eq 0> 
                    	<cfif botonstart>
                         ...
                         <td>
                            <a href="##" tabindex="-1">
							<img src="/cfmx/proyecto7/imagenes/aprobar.png" 
								title="APLICAR" 
								name="imagen" 
								border="0" 
								align="absmiddle" 
								onClick='javascript: aplicar(#workload.ProcessId#);'>	
							</a>
                         </td>
                         <cfif botonrechazar>
                         	<td></td>
        ...
        <cfquery name="SolicitudCNRP" dbtype="query">
				select * 
				  from SolicitudCNRP
				 where 1=1
				<cfif form.F_numero NEQ "">
					and cast(SolicitudCNRP.ProcessInstanceId as varchar) = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.F_numero#">
				</cfif>
				<cfif form.F_tipo NEQ "">
					and ( UPPER(SolicitudCNRP.ProcessDescription) like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#ucase(form.F_tipo)#%">
					<cfif SolicitudCNRP.Ecodigo neq session.Ecodigo or true>
					   OR UPPER(SolicitudCNRP.Edescripcion) like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#ucase(form.F_tipo)#%">
					</cfif>
					    )
				</cfif>
				<cfif form.F_asignado NEQ "">
					and SolicitudCNRP.StartTime between <cfqueryparam cfsqltype="cf_sql_date" value="#LSParseDateTime(form.F_asignado)#">
											   and <cfqueryparam cfsqltype="cf_sql_date" value="#dateAdd('d',1,LSParseDateTime(form.F_asignado))#">
				</cfif>
			</cfquery>
            
            <cfoutput query="SolicitudCNRP">
			<!--- Esto es para guardar el id de la actividad y en cual línea está --->
			<tr <cfif CurrentRow MOD 2>class="listaNon"<cfelse>class="listaPar"</cfif>
				onMouseMove="style.backgroundColor='##E4E8F3';" 
				onMouseOut="style.backgroundColor='<cfif CurrentRow MOD 2>##FFFFFF<cfelse>##FAFAFA</cfif>';">
				...
				<td nowrap valign="top" align="left" ><font style="font-size:10px">
				<cfif SolicitudCNRP.ProcessInstanceId neq 0> 
                	#SolicitudCNRP.ProcessInstanceId# 
                <cfelse>
                    #SolicitudCNRP.ActivityId#
                </cfif>
                </font>
                </td>
				<td nowrap valign="top" align="left" ><font style="font-size:10px">#SolicitudCNRP.ProcessDescription#
					<cfif SolicitudCNRP.Ecodigo neq session.Ecodigo or true> <br> #SolicitudCNRP.Edescripcion#</cfif></font>
				</td>
				<td valign="top" align="left" >
					<font style="font-size:10px">
					#SolicitudCNRP.ProcessInstanceDescription#<br>
					#SolicitudCNRP.ActivityDescription#
					</font>
				</td>
				<td nowrap valign="top" align="left" >
					<font style="font-size:10px">
                    <cfif len(trim(SolicitudCNRP.StartTime)) gt 0> 
					#LSDateFormat(SolicitudCNRP.StartTime,'dd/mm/yyyy')# #LSTimeFormat(SolicitudCNRP.StartTime, 'hh:mmtt')#
                    </cfif>
					</font>
				</td>
				<cfinvoke component="sif.Componentes.Translate"
                     ...
                     <cfif SolicitudCNRP.ProcessInstanceId neq 0>     
                        <cfquery datasource="#session.dsn#" name="hdr">
                            ...
                            from WfxProcess xp
                                join WfProcess p
                                    on p.ProcessId = xp.ProcessId
                            where xp.ProcessInstanceId = <cfqueryparam cfsqltype="cf_sql_numeric" value="#SolicitudCNRP.ProcessInstanceId#">
                        </cfquery>
                    </cfif>    
				<td valign="top">
					<cfif SolicitudCNRP.ProcessInstanceId neq 0> 
                    ...
                    <cfelse>
                       <a href="##" tabindex="-1">
                        <img src="/cfmx/sif/imagenes/Page_Load.gif" 
                            ...
                            border="0" 
                            align="absmiddle" 
                            onClick='javascript: detalleRechazo(#SolicitudCNRP.ActivityId#,#SolicitudCNRP.ProcessId#);'>	
                        </a>
                    </cfif>	   
				</td>				
				<td valign="top">
                	<cfif SolicitudCNRP.ProcessInstanceId neq 0> 
                        ...
                        <img src="/cfmx/sif/imagenes/next_year.gif"
                            ...
                            border="0" 
                            align="absmiddle" 
                            onClick='javascript: seguimiento(#SolicitudCNRP.ProcessInstanceId#,#SolicitudCNRP.ActivityInstanceId#);'>	
                        </a>	
                     <cfelseif SolicitudCNRP.ProcessInstanceId eq 0>
                       <a href="##" tabindex="-1">
                        <img src="/cfmx/sif/imagenes/next_year.gif"
                            title="#BTN_Seguimiento#" 
                            name="imagen" 
                            border="0" 
                            align="absmiddle" 
                            onClick='javascript: seguimientoRechazo(#SolicitudCNRP.ActivityInstanceId#);'>	
                        </a>
					</cfif>	
                    	...
                    	<cfif botonstart>
                         ...
                         <td>
							...
							<img src="/cfmx/proyecto7/imagenes/aprobar.png" 
								...
								border="0" 
								align="absmiddle" 
								onClick='javascript: aplicar(#SolicitudCNRP.ProcessId#);'>	
							</a>
                         </td>
                         <cfif botonrechazar>
                         	<td></td>
					...
					<cfif soy_responsable or request.lista_actividades_superuser>                         
						...
						<td valign="top">
							...
							<img src="/cfmx/sif/imagenes/stop2.gif" 
								...
								border="0" 
								align="absmiddle" 
								onClick='javascript: set_state(#SolicitudCNRP.ProcessInstanceId#,#SolicitudCNRP.ActivityInstanceId#,"start");'>	
							</a>	&nbsp;						
						...
						<td valign="top">	
							...
							<img src="/cfmx/sif/imagenes/stop.gif" 
								...
								border="0" 
								align="absmiddle" 
								onClick='javascript: set_state(#SolicitudCNRP.ProcessInstanceId#,#SolicitudCNRP.ActivityInstanceId#,"finish");'>	
							</a>	&nbsp;	
						...
						<cfelseif SolicitudCNRP.State EQ "COMPLETED">
							...
							<cfset SolicitudCNRP_ProcessInstanceId = SolicitudCNRP.ProcessInstanceId>
							<cfset SolicitudCNRP_ActivityInstanceId = SolicitudCNRP.ActivityInstanceId>
							<cfloop query="rsTrans">
							<cfset imagen ="">
							<cfif findNoCase("ACEPTA",name)
							   OR findNoCase("APROB",name)
                            ...
                            <a href="##" tabindex="-1">
							<img src="#imagen#"  title="#Ucase(Name)#" title="#Ucase(Name)#"  name="imagen" border="0" align="absmiddle" 
							 onClick='javascript: if (confirm("#MSG_Actual# #SolicitudCNRP.ActivityDescription#\n\n#MSG_Accion#:\t#Ucase(Name)#\n#MSG_Enviar#:\t#toName#<cfif toName NEQ toDescription> - #toDescription#</cfif>")) do_transition(#SolicitudCNRP_ProcessInstanceId#,#SolicitudCNRP_ActivityInstanceId#,#rsTrans.TransitionId#); else return false;'>
                            </a>
                            </td>
							</cfloop>
						</cfif>
        ...
        <!---                            AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA                            --->
		<cfoutput>
			<cfif  not workload.RecordCount GT 0 and not SolicitudCNRP.RecordCount GT 0>
				<tr>
...
<form action="../sif/cm/operacion/solicitudes-sql.cfm" name="fsolicitud" method="post">
	<input type="hidden" name="action" id="action" value="/cfmx/proyecto7/solicitudCompras.cfm" />
    <input type="hidden" name="btnAplicar" id="btnAplicar" value="btnAplicar" />
...
<cfoutput>
	...
	<script language="JavaScript1.2">
		...
		function seguimientoRechazo(ProcessInstanceId) {
			fnLightBoxSetURL_Seguimiento("#LvarAprobacionDetalle#?from=#CurrentPageForURL#&ProcessInstanceId="+ProcessInstanceId);
			fnLightBoxOpen_Seguimiento();
		}
	
		function detalle(ProcessInstanceId,ProcessId) {
				fnLightBoxSetURL_DetaSolicitudCom("detalleTramiteSolicitud.cfm?ProcessId="+ProcessId+"&ProcessInstanceId=" +ProcessInstanceId);
				fnLightBoxOpen_DetaSolicitudCom();
		}
		function detalleRechazo(ProcessInstanceId,ProcessId) {
				fnLightBoxSetURL_DetaSolicitudCom("detalleTramiteSolicitud.cfm?ESidsolicitud="+ProcessId+"&ProcessInstanceId=" +ProcessInstanceId+"&SC=SC");
				fnLightBoxOpen_DetaSolicitudCom();
		}
		function aplicar(ProcessId){
				document.fsolicitud.chk.value =  ProcessId;
				document.fsolicitud.submit();
		}
	</script>
</cfoutput>
