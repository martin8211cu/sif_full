...
<cfquery name="rsLista" datasource="#session.DSN#">
	select a.Nivel, b.EOidorden, b.EOnumero, a.CMAid
	from CMAutorizaOrdenes a
    	inner join EOrdenCM b
        on a.EOidorden=b.EOidorden
	 	and b.EOestado in (-7,-8,-9)
	where b.EOidorden = #url.EOidorden#
    	and b.EOnumero = #url.EOnumero#
...
<table style="width:inherit" align="center"  border="0" cellspacing="0" cellpadding="2">
	<!---<tr>
	...
	<tr>
		<td colspan="4"><strong>Tipo de Solicitud:&nbsp; #CMTOdescripcion#</strong></td>
	</tr>
  <tr>
...
<cfloop query="rsLista">
    ...
    <table>
    	<tr> <cfoutput>
        	<td>  <a id="btnAprobar" class="btnAprobar" style="cursor:pointer;float:none" title="Aprobar" onclick="funcAprobar('#rsLista.EOidorden#','#rsLista.Nivel#','#rsLista.CMAid#')"></a>
            </td>
       		<td> <a class="btnRechazar" style="cursor:pointer;float:none" title="Rechazar" onclick="funcRechazar('#rsLista.EOidorden#','#rsLista.Nivel#','#rsLista.CMAid#')"></a>
            </td>
        </tr> </cfoutput>
     </table>
    </cfif>
...
<script type="text/javascript" language="javascript1.2" >
	function funcAprobar(id,Nivel,CMAid){
		if(confirm('¿Esta seguro de que desea aprobar el proceso?')){
			window.parent.document.formX.action="autorizaOrden-sql.cfm?btnAprobar=btnAprobar";
			window.parent.document.formX.Nivel.value = Nivel;
			window.parent.document.formX.CMAid.value = CMAid;
			window.parent.document.formX.EOidorden.value = id;
			window.parent.document.formX.submit();
		}
	}
	function fncReinici(id,Nivel){
		if(confirm('¿Esta seguro de que desea reiniciar el proceso?')){
			window.parent.document.formX.action="autorizaOrden-sql.cfm?btnReiniciar=btnReiniciar";
			window.parent.document.formX.Nivel.value = Nivel;
			window.parent.document.formX.EOidorden.value = id;
			window.parent.document.formX.submit();
		}
	...
	function funcRechazar(id,Nivel,CMAid){
		window.parent.fnLightBoxSetURL_JustRechazo("/cfmx/proyecto7/justifiRechazo.cfm?Nivel="+Nivel+"&EOidorden="+id+"&CMAid="+CMAid);
