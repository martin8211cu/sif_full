...
<cfquery name="rsTES_U_CF" datasource="#session.dsn#">
	...
	<cfelseif session.Tesoreria.CFid_subordinados EQ 0>
		Select cf.CFid, cf.CFcodigo, cf.CFdescripcion #LvarCNCT# ' (Oficina: ' #LvarCNCT# o.Oficodigo #LvarCNCT# ')' as CFdescripcion, cf.CFpath
...
<cfif not Attributes.soloInicializar>
	...
	<cfif not attributes.readonly>
		<cfif Attributes.aprobacionSP AND session.Tesoreria.CFid_subordinados EQ 0>
			<option value="-1" <cfif -1 EQ session.Tesoreria.CFid>selected<cfset LvarSelected = true></cfif>>(Todos los Centros Funcionales)</option>
			<option value="0" <cfif 0 EQ session.Tesoreria.CFid>selected<cfset LvarSelected = true></cfif>>(Sin Centro Funcional)</option>
