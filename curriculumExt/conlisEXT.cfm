...
--->
<cfif not isdefined("request.IFramesStatus.#Attributes.form#")><CF_IFramesStatusExt form="#Attributes.form#" action="initForm"></cfif>
<cfsilent>
...
<cfif Attributes.traerInicial>
	<cfif Attributes.values NEQ "" OR fnExistsArray("valuesArray")>
		<cf_throwEXT message="ERROR EN LA DEFINICION DEL CF_CONLIS: Los Siguientes Parámetros son excluyentes: values/valuesarray, traerInicial." errorcode="3005">
	<cfelseif len(trim(Attributes.traerFiltro)) EQ 0>
		<cf_throwEXT message="ERROR EN LA DEFINICION DEL CF_CONLIS: Los Siguientes Parámetros deben definirse juntos: traerInicial, traerFiltro." errorcode="3010">
	</cfif>
<cfelse>
	...
	<cfif Lvar_nModificables NEQ 0>
		<cfif Lvar_nRequeridos EQ 1>
			<cfif Lvar_nModificables GT 1>
				<cf_throwEXT message="ERROR EN LA DEFINICION DEL CF_CONLIS: Si hay una columna requerida '#Attributes.desplegararray[LvarRequeridoIdx]#', sólo puede haber un único campo modificable que corresponda a dicha columna." errorcode="3015">
			</cfif>
			...
			<cfif LvarCampoIdx EQ 0 OR Attributes.desplegablesArray[LvarCampoIdx]&Attributes.modificablesArray[LvarCampoIdx] NEQ "SS">
				<cf_throwEXT message="ERROR EN LA DEFINICION DEL CF_CONLIS: Si hay una columna requerida '#Attributes.desplegararray[LvarRequeridoIdx]#', debe corresponder al único campo modificable." errorcode="3020">
			</cfif>
		<cfelseif Lvar_nModificables NEQ 0>
			<cf_throwEXT message="ERROR EN LA DEFINICION DEL CF_CONLIS: Si hay mas de una columna requerida, no puede haber campos modificables." errorcode="3025">
		</cfif>
	</cfif>
</cfif>
...
<cfif LvarCamposArrayN 	lt arraylen(Attributes.desplegablesarray)
	...
	<cf_throwEXT message="ERROR EN LA DEFINICION DEL CF_CONLIS: Los Siguientes Parámetros deben concordar en longitud de items con campos: desplegables, size, values." errorcode="3030">
</cfif>
...
<cfelse>
	<cf_throwEXT message="ERROR EN LA EJECUCION DEL CF_CONLIS: No se permiten más de #MAXCONLISES# conlis en el Request" errorcode="3035">
</cfif>
				...
				conlisArgs = conlisArgs &
							"	if (!document.#Attributes.form#.#name#){
								...
								if (document.#Attributes.form#.#name#.value==''){
									...
									alert('El valor de ' + description + ' es requerido para realizar esta accción.');
									limpia#LvarFuncion1#();
									return;
								}
...
<cfoutput>
	...
	<script language="JavaScript">
		...
		function popUpWindowEXT#LvarFuncion1#(URLStr, left, top, width, height)
		{
		...
		function doConlis#LvarFuncion1#(x)
		{
			var conlisArgs = '';
			#conlisArgs#
			popUpWindowEXT#LvarFuncion1#('../UtilesExt/ConlisPopUpEXT.cfm?c=#Session.Conlises.Identity#'+conlisArgs,#Attributes.left#,#Attributes.top#,#Attributes.width#,#Attributes.height#);
			...
			<cfif Attributes.modificablesarray[item] EQ "S">
				function trae#LvarFuncionItem#(pValue, doTraer)
				{
					...
					if (pValue.replace(/\s+/,"") != "")	
					{
						...
						document.getElementById('frame#LvarFuncion1#').src = '../UtilesExt/ConlisPopUpEXT.cfm?query=1&c=#Session.Conlises.Identity#&CampoOnTrae=#Trim(Attributes.camposarray[item])#'+conlisArgs;
					} 
					else 
					{ 
		...
		<tr>
			...
			<td>
				...
				<a href="javascript:doConlis#LvarFuncion1#();" tabindex="-1" id="img#LvarFuncion1#">
					<img src="/imagenes/Description.gif"
	...
	<cfif Attributes.traerInicial>
		...
		<cfif trim(conlisArgs) NEQ "">
			var conlisArgs = '';
			#conlisArgs#
			document.getElementById('frame#LvarFuncion1#').src = '../UtilesExt/ConlisPopUpEXT.cfm?query=1&c=#Session.Conlises.Identity#&CampoOnTrae=*traerInicial*'+conlisArgs;
		<cfelse>
			document.getElementById('frame#LvarFuncion1#').src = '../UtilesExt/ConlisPopUpEXT.cfm?query=1&c=#Session.Conlises.Identity#&CampoOnTrae=*traerInicial*';
		</cfif>
		</script>
	</cfif>
</cfoutput>
