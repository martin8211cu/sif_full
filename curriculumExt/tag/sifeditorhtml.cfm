...
<script type="text/javascript">
	...
	window.onload = function()
  	{
		var oFCKeditor = new FCKeditor( '#attributes.name#' ) ;
		oFCKeditor.ToolbarSet = '#attributes.toolbarset#';
		oFCKeditor.ReplaceTextarea() ;
  	}
	</cfoutput>
</script>
<cfoutput><textarea id="#attributes.name#" name="#attributes.name#" onFocus="this.select();" ><cfif len(trim(attributes.value))>#attributes.value#</cfif></textarea></cfoutput>
--->
...
<!---
Para setear un valor en el editor en forma dinamica hay que hacer lo siguiente

document.form1.<nombre del campo>.value = <valor a setear>;
document.getElementById("<nombre del campo>___Frame").src=<nombre del objeto fckeditor creado>.BasePath+"editor/fckeditor.html?InstanceName=<nombre del campo>&Toolbar="+<nombre del objeto fckeditor creado>.ToolbarSet;
