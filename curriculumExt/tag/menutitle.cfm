...
<cfoutput>
	...
	<cfif Attributes.tipo EQ 1>
		<cfset img = "/imagenes/menues/Gen_Opciones">
	<cfelseif Attributes.tipo EQ 2>
		<cfset img = "/imagenes/menues/Gen_Opciones2">
	<cfelseif Attributes.tipo EQ 3>
		<cfset img = "/imagenes/menues/Gen1_EstructuraOrg">
	</cfif>
	...
	.menuTitleStyle#Attributes.index#2 {
		color: #Attributes.color#;
		font-weight: bold;
		font-family: #Attributes.fontFamily#;
		font-size: #Attributes.fontSize#;
		background-image: url('/imagenes/menues/Gen2_EstructuraOrg_cen.gif');
		<cfif Len(Trim(Attributes.link)) NEQ 0>
		cursor: pointer;
		</cfif>
	}
	...
	<cfif Attributes.tipo EQ 3>
		<script language="javascript" type="text/javascript">
			function changeStyle(i, mouserOverEvent) {
				...
				if (mouserOverEvent) {
					if (izq) izq.src = '/imagenes/menues/Gen2_EstructuraOrg_izq.gif';
					if (cen) cen.className = 'menuTitleStyle#Attributes.index#2';
					if (der) der.src = '/imagenes/menues/Gen2_EstructuraOrg_der.gif';
				} else {
