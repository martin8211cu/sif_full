...
<CFIF LISTFIRST(SERVER.COLDFUSION.PRODUCTVERSION) LT 6>
	<CFTHROW MESSAGE="<H3>CF_FORWARD works only in CFMX and above.</H3>">
	<CFABORT>
</CFIF>

<CFTRY>
	<CFIF NOT ISDEFINED("attributes.URL")>
		<CFTHROW MESSAGE="You must provide a URL">
	</CFIF>
	...
	<CFIF ISDEFINED("attributes.localvarscopied")>
		<CFSWITCH EXPRESSION="#attributes.localvarscopied#">
			...
			<CFDEFAULTCASE>
				<CFTHROW MESSAGE='The value of the attribute LocalVarsCopied, which is currently "#attributes.localvarscopied#", is invalid.'>
			</CFDEFAULTCASE>
		</CFSWITCH>
	</CFIF>
	...
	<CFIF ISDEFINED("attributes.FORMURLvarscopied")>
		<CFSWITCH EXPRESSION="#attributes.FORMURLvarscopied#">
			...
			<CFDEFAULTCASE>
				<CFTHROW MESSAGE='The value of the attribute formurlvarscopied, which is currently "#attributes.formurlvarscopied#", is invalid.'>
			</CFDEFAULTCASE>
		</CFSWITCH>
	</CFIF>
...
<CFTRY>
	...
	<CFIF CGI.REQUEST_METHOD IS "post">
		<CFTHROW MESSAGE="Error processing CF_FORWARD." DETAIL="Because of a bug in CFMX, you cannot call CF_FORWARD from a page that is a form action page.<br>The underlying getpagecontext().forward() method fails.<br>This is a bug in CFMX which, until it's resolved, restricts use of this tag on form action pages.">
	</CFIF>
	<CFCATCH>
		<CFOUTPUT>
