<!--- 
	...
	Debe invocarse primero el <cf_jdbcqueryEXT_open>, y luego el <cf_jdbcqueryEXT_close>
	...
	Ejemplo de uso:
			...
			<cftry>
				<cf_jdbcqueryEXT_open datasource="sifcontrol" name="myrs">
					select name from systypes order by 1
				</cf_jdbcqueryEXT_open>
...
<cfif Not ThisTag.HasEndTag>
	<cfthrow message="cf_jdbcquery debe tener un tag de cierre">
</cfif>
	...
	<cfscript>
		if(Len(Attributes.datasource)){
			...
			if (not isdefined("request.jdbcquery"))
			{
				...
				request.jdbcquery.conn 	= DataSourceFactory.getInstance().getDataSource(Attributes.datasource).getConnection();
				request.jdbcquery.stmt 	= request.jdbcquery.conn.createStatement();
			}
