...
function doConlisPoliza<cfoutput>#Attributes.EPDid#</cfoutput>() {
	...
	popUpWindow("/UtilesExt/ConlisPoliza.cfm"+params,250,200,650,400);
}
...
function TraePoliza<cfoutput>#Attributes.EPDnumero#</cfoutput>(valor) {
	...
	if (valor!="") {
		document.all["<cfoutput>#Attributes.frame#</cfoutput>"].src="/UtilesExt/sifPolizaquery.cfm?valor="+valor+"&formulario="+'<cfoutput>#Attributes.form#</cfoutput>'+params;
	}
	else{
	...
	<tr>
		...
		<td width="98%">&nbsp;
			<cfif Attributes.conlis>
				<a href="##" tabindex="-1">
					<img id="Almimagen" src="/imagenes/Description.gif" alt="Lista de P&oacute;lizas" name="Polimagen" width="18" height="14" border="0" align="absmiddle" onClick='javascript:doConlisPoliza#Attributes.EPDid#();'>
				</a>
			<cfelse>
				&nbsp;			
			</cfif>
