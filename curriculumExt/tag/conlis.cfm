...
<cfif Attributes.traerInicial>
	<cfif Attributes.values NEQ "" OR fnExistsArray("valuesArray")>
		<cfthrow message="ERROR EN LA DEFINICION DEL CF_CONLIS: Los Siguientes Parámetros son excluyentes: values/valuesarray, traerInicial.">
	<cfelseif len(trim(Attributes.traerFiltro)) EQ 0>
		<cfthrow message="ERROR EN LA DEFINICION DEL CF_CONLIS: Los Siguientes Parámetros deben definirse juntos: traerInicial, traerFiltro.">
	</cfif>
<cfelse>
	...
	<cfif Lvar_nModificables NEQ 0>
		<cfif Lvar_nRequeridos EQ 1>
			<cfif Lvar_nModificables GT 1>
				<cfthrow message="ERROR EN LA DEFINICION DEL CF_CONLIS: Si hay una columna requerida '#Attributes.desplegararray[LvarRequeridoIdx]#', sólo puede haber un único campo modificable que corresponda a dicha columna.">
			</cfif>
			...
			<cfif LvarCampoIdx EQ 0 OR Attributes.desplegablesArray[LvarCampoIdx]&Attributes.modificablesArray[LvarCampoIdx] NEQ "SS">
				<cfthrow message="ERROR EN LA DEFINICION DEL CF_CONLIS: Si hay una columna requerida '#Attributes.desplegararray[LvarRequeridoIdx]#', debe corresponder al único campo modificable.">
			</cfif>
		<cfelseif Lvar_nModificables NEQ 0>
			<cfthrow message="ERROR EN LA DEFINICION DEL CF_CONLIS: Si hay mas de una columna requerida, no puede haber campos modificables.">
		</cfif>
	</cfif>
</cfif>
...
<cfif LvarCamposArrayN 	lt arraylen(Attributes.desplegablesarray)
	...
	<cfthrow message="ERROR EN LA DEFINICION DEL CF_CONLIS: Los Siguientes Parámetros deben concordar en longitud de items con campos: desplegables, size, values.">
</cfif>
...
<cfelse>
	<cfthrow message="ERROR EN LA EJECUCION DEL CF_CONLIS: No se permiten más de #MAXCONLISES# conlis en el Request">
</cfif>
...
<cfoutput>
	...
	<script language="JavaScript">
		...
		function doConlis#LvarFuncion1#(x)
		{
			var conlisArgs = '';
			#conlisArgs#
			popUpWindow#LvarFuncion1#('../UtilesExt/ConlisPopUpEXT.cfm?c=#Session.Conlises.Identity#'+conlisArgs,#Attributes.left#,#Attributes.top#,#Attributes.width#,#Attributes.height#);
