...
<!---
	...
	*
	* Utilización:
	*	<CF_jdbcqueryEXT_param cfsqltype="XX" value="XX" [scale="XX" maxlength="XX" null="XX" returnVariable="XX"]>
	*
--->
...
<cfelseif listFind("cf_sql_blob",Attributes.cfsqltype)>
	<cfif attributes.datasource EQ "">
		<cfthrow message="jdbcqueryEXT_param: Si tipo es cf_sql_blob debe especificar datasource">
	<cfelse>
		<cfthrow message="jdbcqueryEXT_param: No se ha implementado tipo cf_sql_blob">
	</cfif>
<cfelse>
	<cfthrow message="jdbcqueryEXT_param: Sólo se han implementado los siguientes tipos: #LvarTypes#">
</cfif>
...
<cffunction name="fnEsNumerico" access="private" output="false" returntype="string">
	...
	<cfif not isnumeric(Attributes.value)>
		<cfthrow message="El valor '#Attributes.value#' no es numerico">
	</cfif>
...
<cffunction name="sbEsTiempo" access="private" output="false" returntype="void">
	<cfif not isdate(Attributes.value)>
		<cfthrow message="El valor '#Attributes.value#' no es tiempo">
	</cfif>
</cffunction>
</cfsilent>
