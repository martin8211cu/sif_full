...
<!---
	...
	Para traducciones basadas en sifcontrol.VSidioma, utilice TranslateEXTDB.cfc
	Modificado, danim, 2006-09-11 para guardar los XML en Request y no estarlos
	leyendo cada vez
--->

	<cffunction name="TranslateEXT" output="false" returntype="string" access="public">
		...
		<cftry>
			...
			<cfelse>
				...
				<cfreturn TranslateEXT(arguments.key, arguments.Default, arguments.XmlFile, LvarIdioma) >
			</cfif>
		<cfcatch type="any">
			<cflog file="translate" text="TranslateEXT (File=#Arguments.XmlFile#, Key=#Arguments.Key#): #cfcatch.Message# #cfcatch.Detail#">
			<cfreturn Arguments.Default>
		</cfcatch>
		</cftry>
	</cffunction>
	...
	<cffunction name="AgregarRecordatorio" output="false" returntype="void">
		...
		<cfquery datasource="#session.dsn#" name="AgregarRecordatorio_Q">
			select count(1) as cantidad
			from VSnuevo
			where ruta = <cfqueryparam cfsqltype="cf_sql_varchar" value="#ruta#">
			  and entrada = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.Key#">
		</cfquery>
