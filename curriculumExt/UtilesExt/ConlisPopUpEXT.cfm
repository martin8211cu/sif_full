...
<cfquery name="conlisQuery" datasource="#Session.Conlises.Conlis[url.c].conexion#">
	...
	<cf_dbrowcount1EXT rows="#Session.Conlises.Conlis[url.c].maxrowsquery#" datasource="#Session.Conlises.Conlis[url.c].conexion#">
	</cfif>
			...
			<cfif len(mx[i].tip) gt 0 and len(mx[i].val) gt 0>
				<cfswitch expression="cf_sql_#mx[i].tip#">
					<cfcase value="cf_sql_date,cf_sql_timestamp">
						<cf_jdbcqueryEXT_param cfsqltype="cf_sql_#mx[i].tip#" value="#LSParseDateTime(mx[i].val)#">
					</cfcase>
					<cfcase value="cf_sql_decimal,cf_sql_double,cf_sql_float,cf_sql_money,cf_sql_money4,cf_sql_real">
						<cf_jdbcqueryEXT_param cfsqltype="cf_sql_#mx[i].tip#" value="#Replace(trim(mx[i].val), ',', '','all')#">
					</cfcase>
					<cfdefaultcase>
						<cf_jdbcqueryEXT_param cfsqltype="cf_sql_#mx[i].tip#" value="#(Trim(mx[i].val))#">
					</cfdefaultcase>
				</cfswitch>
			</cfif>
		</cfloop>
	...
	<cfsavecontent variable="LvarFiltro">
				...
				<cfif len(trim(temp_value)) and len(temp_value) gt 0>
					...
					<cfif Arraylen(arFormatos)>
						<cfswitch expression="#trim(arFormatos[i])#">
							<cfcase value="D,DI">
								and <cf_dbfunctionExt name="to_datechar" args="#PreserveSingleQuotes(x)#" datasource="#Session.Conlises.Conlis[url.c].conexion#"> <cfif isdefined("Form.Filtro_FechasMayores") or isdefined("Url.Filtro_FechasMayores")>>=<cfelse>=</cfif> <cf_jdbcqueryEXT_param cfsqltype="cf_sql_timestamp" value="#LSParseDateTime(temp_value)#">
							</cfcase>
							<cfcase value="I">
								and #PreserveSingleQuotes(x)# = <cf_jdbcqueryEXT_param cfsqltype="cf_sql_money" value="#Replace(trim(temp_value), ',', '','all')#">
							</cfcase>
							...
							<cfdefaultcase>
								<cfif isdefined("url.query")>
									...
									= <cf_jdbcqueryEXT_param cfsqltype="cf_sql_char" value="#temp_value#">
								<cfelse>
									and upper(rtrim(<cf_dbfunctionExt name="to_char" args="#PreserveSingleQuotes(x)#" datasource="#Session.Conlises.Conlis[url.c].conexion#"> ))
									like  <cf_jdbcqueryEXT_param cfsqltype="cf_sql_char" value="%#Ucase(temp_value)#%">
								</cfif>
							</cfdefaultcase>
						</cfswitch>
					<cfelse>
						...
						<cfelse>
							and upper(rtrim(<cf_dbfunctionExt name="to_char" args="#PreserveSingleQuotes(x)#" datasource="#Session.Conlises.Conlis[url.c].conexion#"> ))
							like  <cf_jdbcqueryEXT_param cfsqltype="cf_sql_char" value="%#temp_value#%">
						</cfif>
					</cfif>
		...
		<cfelse>
			...
			<cfif #Session.Conlises.Conlis[url.c].maxrowsquery# GT 0>
			<cf_dbrowcount2EXT_a rows="#Session.Conlises.Conlis[url.c].maxrowsquery#" datasource="#Session.Conlises.Conlis[url.c].conexion#">
			#PreserveSingleQuotes(LvarPostWhere)#
			<cf_dbrowcount2EXT_b rows="#Session.Conlises.Conlis[url.c].maxrowsquery#" datasource="#Session.Conlises.Conlis[url.c].conexion#">
			</cfif>
		</cfif>
	</cfsavecontent>
...
<cfif find('c',CGI.QUERY_STRING,1) eq 0>
	<cfthrow message="Error en Conlis. Argumentos Requeridos Son Incorrectos!">
</cfif>
...
<script language="javascript">
	...
	<cfelse>
		...
		function funcFiltrar(){
			...
			document.lista.action="ConlisPopUpEXT.cfm?#QueryString_lista#";
			return true;
		}
	</cfif>
...
<cfinvoke 
	...
	ira="ConlisPopUpEXT.cfm?#QueryString_lista#"
	funcion="asignar"
