...
<cffunction name="pListaRH" access="public" returntype="string" output="true">
	...
	<!--- FILTRO AUTOMATICO --->
		...
		<cfquery name="rsLista" datasource="#Conexion#">
			...
			<cf_dbrowcount1EXT rows="#Arguments.MaxRowsQuery#" datasource="#Conexion#">
			</cfif>
    		...
    		<cfif Arguments.MaxRowsQuery GT 0>
			<cf_dbrowcount2EXT_a rows="#Arguments.MaxRowsQuery#" datasource="#Conexion#">
			</cfif>
						...
						<cfswitch expression="#LvarFormato#">
							<cfcase value="C">
								<cfswitch expression="#LvarFormatoCombo#">
									<cfcase value="D,DI">
										and <cf_dbfunctionExt name="to_datechar" args="#PreserveSingleQuotes(x)#" datasource="#Conexion#"> = <cfqueryparam cfsqltype="cf_sql_timestamp" value="#LSParseDateTime(temp_value)#">
									</cfcase>
									<cfcase value="M,P,R,N,F,I">
										and #PreserveSingleQuotes(x)# = <cfqueryparam cfsqltype="cf_sql_money" value="#Replace(trim(temp_value), ',', '','all')#">
									</cfcase>
									<cfdefaultcase>
										and upper(rtrim(<cf_dbfunctionExt name="to_char" args="#PreserveSingleQuotes(x)#" datasource="#Conexion#">)) =  <cfqueryparam cfsqltype="cf_sql_char" value="#Ucase(Trim(temp_value))#">
									</cfdefaultcase>
								</cfswitch>
							</cfcase>
							<cfcase value="D,DI">
								and <cf_dbfunctionExt name="to_datechar" args="#PreserveSingleQuotes(x)#" datasource="#Conexion#"> <cfif isdefined("Form.Filtro_FechasMayores") or isdefined("Url.Filtro_FechasMayores")>>=<cfelse>=</cfif> <cfqueryparam cfsqltype="cf_sql_timestamp" value="#LSParseDateTime(temp_value)#">
							</cfcase>
							<cfcase value="I">
								and #PreserveSingleQuotes(x)# >= <cfqueryparam cfsqltype="cf_sql_money" value="#Replace(trim(temp_value), ',', '','all')#">
							</cfcase>
							...
							<cfdefaultcase>
								and upper(rtrim(<cf_dbfunctionExt name="to_char" args="#PreserveSingleQuotes(x)#" datasource="#Conexion#">)) like  <cfqueryparam cfsqltype="cf_sql_char" value="%#Ucase(Trim(temp_value))#%">
							</cfdefaultcase>
						</cfswitch>
					<cfelse>
						and upper(rtrim(<cf_dbfunctionExt name="to_char" args="#PreserveSingleQuotes(x)#" datasource="#Conexion#">)) like  <cfqueryparam cfsqltype="cf_sql_char" value="%#Ucase(Trim(temp_value))#%">
			...
			<cfif Arguments.MaxRowsQuery GT 0>
			<cf_dbrowcount2EXT_b rows="#Arguments.MaxRowsQuery#" datasource="#Conexion#">
			</cfif>
					...
					<cfif col EQ 1>
						<td align="left" width="18" height="18" nowrap <cfif Len(Trim(funcion))> onclick="javascript: #invocacion_de_funcion#"<cfelseif showLink> onclick="javascript: return Procesar('#Arguments.query.CurrentRow#','#ArrayLen(columnas)#','#formName#'<cfif not incluyeForm>, '#irA#'</cfif>);"</cfif>>
							<cfif comparenocase(datosBD,datosPost) EQ 0 and not isdefined("Nuevo")>
								<img src="/imagenes/addressGo.gif" width="18" height="18"> 
							</cfif>
						</td>
					</cfif>
		  ...
		  <tr>
			<td width="99%" align="center">
				<cfif PageNum_lista GT 1>
				  <a href="#CurrentPage#?PageNum_lista#PageIndex#=1#QueryString_lista#" tabindex="-1"><img src="/imagenes/First.gif" border=0></a> 
				</cfif>
				<cfif PageNum_lista GT 1>
				  <a href="#CurrentPage#?PageNum_lista#PageIndex#=#Max(DecrementValue(PageNum_lista),1)##QueryString_lista#" tabindex="-1"><img src="/imagenes/Previous.gif" border=0></a> 
				</cfif>
				<cfif PageNum_lista LT TotalPages_lista>
				  <a href="#CurrentPage#?PageNum_lista#PageIndex#=#Min(IncrementValue(PageNum_lista),TotalPages_lista)##QueryString_lista#" tabindex="-1"><img src="/imagenes/Next.gif" border=0></a> 
				</cfif>
				<cfif PageNum_lista LT TotalPages_lista>
				  <a href="#CurrentPage#?PageNum_lista#PageIndex#=#TotalPages_lista##QueryString_lista#" tabindex="-1"><img src="/imagenes/Last.gif" border=0></a> 
				</cfif> 
		  </td>
		  <!--- LINEA DE MAXROWSQUERY QUE INDICA AL USUARIO QUE NO ESTA VIENDO TODOS LOS REGISTROS DE LA BASE DE DATOS  --->
		    ...
		    <cftry>
			  <cfoutput>
				  ...
				  <cfif isdefined("Arguments.fontsize") and len(trim(Arguments.fontsize))>style="font-size:#Arguments.fontsize#px"</cfif>>
					<cf_translate XmlFile="generales.xml" key="LB_VistaActual">Vista Actual</cf_translate> #Desde# - #Hasta# <cf_translate XmlFile="generales.xml" key="LB_de">de</cf_translate> #query.recordcount# <cfif Arguments.maxrowsquery gt 0 and (Desde LTE Hasta)> (* <cf_translate  XmlFile="generales.xml" key="LB_ListaLimitada">Lista Limitada</cf_translate>) </cfif>
				  </td>
			  </cfoutput>
			  <cfcatch>
			  </cfcatch>
