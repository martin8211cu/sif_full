...
<cfcomponent extends="ajax">
	...
	<cffunction name="doQuery" output="no" access="remote">
		...
		<cfset fnPreparaDatos(Arguments.IDs, Arguments.PageNum)>
	...
	<cffunction name="fnPreparaHTML1" access="private" output="no"  returntype="boolean" hint="Prepara las instrucciones HTML para el manejo de Listas usando AJAX. Funcion No. 1">
			...
			<cfif len(trim(params.etiquetas)) GT 0>
				...
				<cfset write_to_buffer(' <tr> ')>
					<cfif ucase(params.checkboxes) EQ "S" or ucase(params.radios) EQ "S" or ucase(params.chkcortes) EQ "S">
						<cfset write_to_buffer('<td class="tituloListas" align="left" width="1%">')>
					...
					<cfloop index="i" from="1" to=#ArrayLen(arEtiquetas)#>
						<cfset write_to_buffer('<td class="tituloListas" align="#Trim(alin[i])#" valign="bottom">')>
						...
						<cfset write_to_buffer(' <td class="tituloListas" align="#Trim(alin[i])#"> ')>
							...
							<cfswitch expression="#LvarFormato#">
								...
								<cfcase value="M,P,R,N,F,I">
                                    ...
                                    <cfsavecontent variable="_LvarinputNumber"><cf_inputNumber name="filtros_#HTMLEditFormat(Trim(arDesplegar[i]))#" value="#replace(temp_value,',','','all')#" form="#params.formName#" decimales="#decimales#" style="width:100%"></cfsavecontent>
			...
			<cfloop query="rsLista" startrow="#StartRow_lista#" endrow="#StartRow_lista + MaxRows_lista-1#">
					...
					<cfif lCortes GT 0>
						<cfif DatosCorte[i] NEQ #Evaluate('rsLista.#Trim(ColsCorte[i])#')#>
							...
							<cfif ucase(params.chkcortes) EQ "S">
								<cfset write_to_buffer('<input type="checkbox" name="chkPadre" value="#Trim(Evaluate('rsLista.#Trim(params.keyCorte)#'))#" onclick="javascript: funcChkPadre(this); UpdChkAll(this);">')>
							</cfif>
				...
				<cfif ucase(params.checkboxes) EQ "S" or ucase(params.radios) EQ "S">
					...
					<cfif Len(Trim(params.inactiveCol)) NEQ 0 and Evaluate('rsLista.#Trim(params.inactiveCol)#') EQ Lista>
				...
				<cfloop index="cName" list="#params.desplegar#">
					...
					<cfif ListFind(params.totales, dName)>
						<cfset valor_actual = rsLista[dName]>
					...
					<cfelseif Len(Trim(params.formatos)) EQ 0>
						<cfset write_to_buffer('#Evaluate('rsLista.#dName#')#')>
					<cfelseif trim(Evaluate('rsLista.#dName#')) EQ "">
					...
					<cfelse>
						...
						<cfswitch expression="#LvarFormato#">
							<cfcase value="P,UP">
								<cfset write_to_buffer('#LsCurrencyFormat(Evaluate('rsLista.#dName#'),"none")#%')>
							</cfcase>
							<cfcase value="M,UM">
								<cfset write_to_buffer('#LsCurrencyFormat(Evaluate('rsLista.#dName#'),"none")#')>
							</cfcase>
							<cfcase value="R,UR">
								<cfif evaluate(mid(Trim(fmt[col]),LvarPto,100))>
									<cfset write_to_buffer('<font color="##FF0000">#LsCurrencyFormat(Evaluate('rsLista.#dName#'),"none")#</font>')>
								<cfelse>
									<cfset write_to_buffer('#LsCurrencyFormat(Evaluate('rsLista.#dName#'),"none")#')>
								</cfif>
							</cfcase>
							<cfcase value="D,UD">
								<cfif Len(Trim(Evaluate('rsLista.#dName#')))>
									<cfset write_to_buffer('#LSDateFormat(Evaluate('rsLista.#dName#'),"dd/mm/yyyy")#')>
								</cfif>
							</cfcase>
							<!--- IDem cfcase value ="D" --->
							<cfcase value="DI,UDI">
							...
							<cfcase value="N,F,UN,UF">
								<cfset write_to_buffer('#LSNumberFormat(Evaluate('rsLista.#dName#'),"-__________.____")#')>
							</cfcase>
							<cfcase value="H">
								<cfif Len(Trim(Evaluate('rsLista.#dName#')))>
									<cfset write_to_buffer('#LSTimeFormat(Evaluate('rsLista.#dName#'),"hh:mm tt")#')>
								</cfif>
							</cfcase>
							<!--- Formato de Fecha con Hora --->
							<cfcase value="DT">
								<cfif Len(Trim(Evaluate('rsLista.#dName#')))>
									<cfset write_to_buffer('#LSDateFormat(Evaluate('rsLista.#dName#'),"dd/mm/yyyy")# #LSTimeFormat(Evaluate('rsLista.#dName#'),"hh:mm tt")#')>
								</cfif>
							</cfcase>
							<cfdefaultcase>
								<cfset write_to_buffer('#Evaluate('rsLista.#dName#')#')>
                    ...
                    <cfif ListFind(params.totalgenerales, dName)>
                        ...
                        <cfelse>
                            ...
                            <cfswitch expression="LvarFormato">
                                ...
                                <cfdefaultcase>
                                    <cfset write_to_buffer('#NumberFormat(valor_actual, ",9.00")#')>
                                </cfdefaultcase>
                            </cfswitch>
                        </cfif>
                        <cfset write_to_buffer('</strong>')>
	...
	<cffunction name="fnPreparaHTMLFin" access="private" output="no" hint="Prepara el final de la pagina ( Area de Botones y Gráficos">
		...
		<cfelse>
			<cfif PageNum_lista GT 1>
				<cfset write_to_buffer('<a href="#CurrentPage#?PageNum_lista#params.PageIndex#=1#QueryString_lista#" tabindex="-1"><img src="/cfmx/sif/imagenes/First.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="18" border=0> ')>
			</cfif>
			<cfif PageNum_lista GT 1>
				<cfset write_to_buffer('<a href="#CurrentPage#?PageNum_lista#params.PageIndex#=#Max(DecrementValue(PageNum_lista),1)##QueryString_lista#" tabindex="-1"><img src="/cfmx/sif/imagenes/Previous.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="14" border=0> ')>
			</cfif>
			<cfif PageNum_lista LT TotalPages_lista>
				<cfset write_to_buffer('<a href="#CurrentPage#?PageNum_lista#params.PageIndex#=#Min(IncrementValue(PageNum_lista),TotalPages_lista)##QueryString_lista#" tabindex="-1"><img src="/cfmx/sif/imagenes/Next.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="14" border=0> ')>
			</cfif>
			<cfif PageNum_lista LT TotalPages_lista>
				<cfset write_to_buffer('<a href="#CurrentPage#?PageNum_lista#params.PageIndex#=#TotalPages_lista##QueryString_lista#" tabindex="-1"><img src="/cfmx/sif/imagenes/Last.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="18" border=0> ')>
			</cfif> 
		</cfif>
	...
	<cffunction name="fnPreparaDatos" output="no" access="private">
		...
		<cfif params.filtro_nuevo eq  'true'>
