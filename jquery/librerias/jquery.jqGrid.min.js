...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			ignoreCase : false,
			cmTemplate : {}
		}, $.jgrid.defaults, pin || {},{recordtext:$.jgrid.defaults.recordtext});
		var grid={
			headers:[],
			cols:[],
			footers: [],
		...
		addCell = function(rowId,cell,pos,irow, srvr) {
			...
			<!--Se coloca un Span al Inicio en las Columnas 0 (La primera cuando no hay group) y en la 1 (Primera cuando hay group)-->
			var htmlInner = "<td role=\"gridcell\" "+prp+">";
			if(pos == 0 || pos == 1) 
				htmlInner  += "<span class=\"grid-Row-Space-Left\">";
			htmlInner  += v+"</td>";
			if(pos == 0 || pos == 1) 
				htmlInner  += "</span>";
			return htmlInner;
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
				...
				altr = rcnt === 1 ? 0 : rcnt;
				cn1 = (altr+i)%2 == 1 ? cn : '';
				ListParNon = i%2 == 0 ? 'listaPar':'listNon';
				rowData.push("<tr"+hiderow+" id=\""+ idr +"\" tabindex=\"-1\" role=\"row\" class= \"ui-widget-content jqgrow ui-row-"+ts.p.direction+cn1+" "+ListParNon+"\">");
				if(ts.p.rownumbers===true) {
		...
		if(isMSIE) { if(String(ts.p.height).toLowerCase() === "auto") { ts.p.height = "auto"; } }
		$(grid.bDiv)
		...
		$(grid.cDiv).append(arf).append("<span class='tabla-header-icon'></span><span class='ui-jqgrid-title"+(dir=="rtl" ? "-rtl" :"" )+"'>"+ts.p.caption+"</span><div class='ui-jqgrid-busqueda' id='gs_Busqueda_"+ts.p.id+"'></div>")
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
				...
				$.each($t.p.colModel,function(i,n){
					...
					switch (this.stype) {
						case 'select' :
							...
							v = $("#gs_"+$.jgrid.jqID(this.name)+"_"+$t.p.id,$t.grid.hDiv).val();
							if(v) {
								sdata[nm] = v;
								sopt[nm] = so;
								j++;
			...
			var clearToolbar = function(trigger){
				...
				$.each($t.p.colModel,function(i,n){
					...
					switch (this.stype) {
						case 'select' :
							var v1;
							$("#gs_"+$.jgrid.jqID(nm)+"_"+$t.p.id+" option",$t.grid.hDiv).each(function (i){
							...
							break;
						case 'text':
							$("#gs_"+$.jgrid.jqID(nm)+"_"+$t.p.id,$t.grid.hDiv).val(v);
							if(v) {
								sdata[nm] = v;
								j++;
							} else {
			...
			$.each($t.p.colModel,function(i,n){
				...
				thd = $("<div style='width:100%;position:relative;height:100%;padding-right:0em;'></div>");
				...
				if(this.search){
						...
						if(surl) {
							...
							$.ajax($.extend({
								...
								complete: function(res,status) {
									...
									$("select",self).attr({name:(cm.index || cm.name)+"_"+$t.p.id, id: "gs_"+cm.name+"_"+$t.p.id});
							...
							if (oSv) {	
								...
								$(elem).attr({name:(cm.index || cm.name)+"_"+$t.p.id, id: "gs_"+cm.name+"_"+$t.p.id});
								var so, sv, ov;
					...
					case 'text':
						...
						$(thd).append("<input class='idleFieldGrid' type='text' style='width:95%;padding:0px;' name='"+(cm.index || cm.name)+"_"+$t.p.id+"' id='gs_"+cm.name+"_"+$t.p.id+"' value='"+df+"'/>");
