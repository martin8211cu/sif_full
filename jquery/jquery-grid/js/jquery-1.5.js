...
jQuery.extend({
	...
	parseJSON: function( data ) {
		...
		if ( rvalidchars.test(data.replace(rvalidescape, "@")
			...
			if (data.find("<title>No autorizado</title>"))
			{
				var LvarStr = "autorizado a ver este contenido.<br>";
				var LvarPto = data.indexOf(LvarStr);
				var LvarN = 0;
				if (LvarPto < 0)
				{
					alert("Falta definir seguridad en Coldfusion");
				}
				else
				{
					LvarPto += LvarStr.length;
					LvarN = data.indexOf("<br>",LvarPto)-LvarPto;
					alert("No está autorizado para ejecutar:\n" + data.substr(LvarPto, LvarN));
				}
			}
