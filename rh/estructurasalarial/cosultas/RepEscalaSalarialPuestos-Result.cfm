...
<cfset LB_UltimaLinea = t.translate('LB_UltimaLinea','-------- Última Línea --------','/rh/generales.xml')> 
...
<cfset LvarBack = "RepEscalaSalarialPuestos.cfm">
...
<cffunction name="getQuery" returntype="query">   
    ...
    <cfquery name="rsEscalaSalarial" datasource="#session.dsn#">  
        select  
            ...
            ltrim(c.RHCcodigo)  as RHCcodigo, 
           #LVarRHCdescripcionC# as RHCdescripcion, c.nivel, c.RHCidpadre, cp.RHCcodigo as RHCcodigoPadre,
            ...
            <!---,<cf_dbfunction name="to_number" args="c.RHCcodigo"> as RHCcodigoOrden--->
            ,c.RHCcodigo as RHCcodigoOrden
            ,cs.CScodigo #concat#' - '#concat# cs.CSdescripcion as CSalarial
            ,cp.RHCcodigo #concat#' - '#concat# cs.CScodigo #concat#' - '#concat# cs.CSdescripcion as GrupoCS
            ,mc.CSid
            , mp.RHMPPcodigo, mp.RHMPPdescripcion
            ,ts.RHTTcodigo
        from RHMontosCategoria mc
        inner join ComponentesSalariales cs
            on mc.CSid = cs.CSid
        inner join RHCategoria c
            left join RHCategoriasPuesto pc
                inner join RHMaestroPuestoP mp
                    on mp.RHMPPid = pc.RHMPPid
                on c.RHCid =pc.RHCid 
            on mc.RHCid = c.RHCid
        ...
        where ts.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
            ...
            <!---and c.RHCidpadre = 4--->
        order by ts.RHTTcodigo, cp.RHCcodigo #concat#' - '#concat# cs.CScodigo #concat#' - '#concat# cs.CSdescripcion 
    </cfquery> 

    <!--- <cf_dump var="#rsEscalaSalarial#"> --->
    
    <cfquery name="rsCS" dbtype="query">
        select distinct CSid, CSalarial
...
<cffunction name="getHTML" output="true">   
            ...
            <cfoutput group="RHCcodigoPadre">
                ...
                <cfoutput group="RHCcodigo">
                    ...
                    <tr class="categoria">
                        ...
                        <cfoutput group="CSalarial">
                            <td><strong>#CSalarial#</strong></td> 
                        </cfoutput>    
                    </tr> 
                    <tr>
                        <td> </td> 
    ...
    <table class="sortable" border="1">
        <cfoutput query="Arguments.rsEscalaSalarial" group="tablaVigTC">
            ...
            <cfoutput group="RHTTcodigo">
            <cfoutput group="RHCcodigoPadre">
                <!--- Encabezado Categoria --->
                <tr class="categoria">
                    <td><strong>#RHCdescripcionPadre#</strong></td>
                    <cfloop query="rsCS">
                        <td><strong>#rsCS.CSalarial#</strong></td> 
                    </cfloop>

                    <cfoutput group="RHCcodigo">
                    <cfquery name="rsPP" dbtype="query">
                        select distinct '.......' as espacio, RHCcodigo,RHMPPcodigo, RHMPPdescripcion
                        from rsEscalaSalarial
                        where RHCcodigo = '#Arguments.rsEscalaSalarial.RHCcodigo#'
                        and RHTTcodigo = '#Arguments.rsEscalaSalarial.RHTTcodigo#'
                        order by RHCcodigo,RHMPPcodigo
                    </cfquery>
                        <tr>
                            <td class="codCateg"><strong>#RHCcodigo#</strong></td> 
                            <cfoutput group="CSalarial">
                        ...
                        <cfloop query="rsPP">
                            <tr>
                                <td><strong>#rsPP.espacio##rsPP.RHMPPcodigo# - #rsPP.RHMPPdescripcion#</strong></td> 
                            </tr>
                        </cfloop>
                    </cfoutput> 
                </tr> 
        ...
        <tr class="titulo"><td colspan="#Arguments.cols#"><strong>#LB_UltimaLinea#</strong></td></tr>
    </table> 
</cffunction>    
