...
<cfif rsPermisoGenMarca.recordCount GT 0 or rsPermisoGenIncidencia.recordCount GT 0>
	...
	<cfif modo EQ "CAMBIO">
		...
		<!---Verificar si ya se han generado inconsistencias--->
		<cfquery name="rsVerificaIncidencias" datasource="#session.DSN#">
			select count(1) as cantInsidencias
	...
	<cfinvoke component="sif.Componentes.Translate"
		method="Translate"
		Key="LB_HA_Horas_Adicionales"
		Default="HA"	
		returnvariable="LB_HA_Horas_Adicionales"/>
	<cfinvoke component="sif.Componentes.Translate"
		method="Translate"
		Key="LB_HR_Horas_Rebajar"
		Default="HR"	
		returnvariable="LB_HR_Horas_Rebajar"/>
	<cfinvoke component="sif.Componentes.Translate"
		method="Translate"
		Key="LB_I_Inconsistencias_Positivas"
		Default="I(+)"	
		returnvariable="LB_I_Inconsistencias_Positivas"/>
	<cfinvoke component="sif.Componentes.Translate"
		method="Translate"
		Key="LB_I_Inconsistencias_Negativas"
		Default="I(-)"	
		returnvariable="LB_I_Inconsistencias_Negativas"/>
	<cfinvoke component="sif.Componentes.Translate"
		method="Translate"
		...
		returnvariable="MSG_NoEliminarInconsistencia"/>	
	<cfinvoke component="sif.Componentes.Translate"
		method="Translate"
		...
		returnvariable="MSG_TieneIncidenciasAsociadas"/>
	<cfinvoke component="sif.Componentes.Translate"
		method="Translate"
	...
	<cfinvoke component="sif.Componentes.Translate"
		method="Translate"
		Key="MSG_YaSeHanGeneradoIncidenciasParaLaMarcaDeseaPerderLosDatosYGenerarlosDeNuevo"
		Default="Ya se han generado incidencias para la marca. ¿Desea perder los datos y generarlos de nuevo?"	
		returnvariable="MSG_YaGeneraronIncidencias"/>
	...
	<cfif isdefined("Form.RHPMid") and len(trim(Form.RHPMid)) gt 0 >
		<cfset navegacion = navegacion & Iif(Len(Trim(navegacion)) NEQ 0, DE("&"), DE("")) & "RHPMid=" & Form.RHPMid>
	</cfif>

	<form name="goIncidencia" method="post" action="Marcas.cfm" style="margin: 0; ">
				  ...
				  <tr>
					<td width="60%" valign="top" style="padding-right: 10px; ">
						...
						<cfquery name="rsLista" datasource="#session.DSN#">
							select a.RHCMfcapturada as Fecha, 
								   a.RHCMid, 
								   a.DEid, 
								   a.RHCMhoraentradac as HoraEntrada, 
								   a.RHCMhorasalidac as HoraSalida, 
	 							   b.RHJdescripcion,
								   a.RHPMid,
								   a.RHCMhorasadicautor as ha,
								   a.RHCMhorasrebajar as hr,
								   
								  coalesce((	select sum( di.RHDMhorasautor )  
												...
												and ci.CInegativo=1
								     			...
								     			where a.RHCMid=di.RHCMid
												   and a.RHPMid=di.RHPMid ), 0 ) as imas,
								  ...
								  coalesce((  	select sum( di.RHDMhorasautor )  
											    ...
											    where a.RHCMid=di.RHCMid
												  and a.RHPMid=di.RHPMid ), 0 )  as imenos,
						...
						<cfinvoke 
						 ...
						 returnvariable="pListaEmpl">
							...
							<cfinvokeargument name="desplegar" value="Fecha, HoraEntrada, HoraSalida, RHJdescripcion, Inconsistencia, GeneraIncidencias, ha, hr, imas, imenos"/>
							<cfinvokeargument name="etiquetas" value="#LB_Fecha#, #LB_Entrada#, #LB_Salida#, #LB_Jornada#, #LB_Inconsistencia#, #LB_Incidencias#, #LB_HA_Horas_Adicionales#, #LB_HR_Horas_Rebajar#, #LB_I_Inconsistencias_Positivas#, #LB_I_Inconsistencias_Negativas#"/>
							<cfinvokeargument name="formatos" value="D, H, H, V, V, V, M, M, M, M"/>
							<cfinvokeargument name="formName" value="listaMarcas"/>	
							<cfinvokeargument name="align" value="center, center, center, left, center, center, right, right, right, right"/>
						...
						<cfinvoke 
						 ...
						 method="pListaRH"
						 returnvariable="pListaEmpl">
							<cfinvokeargument name="tabla" value="RHControlMarcas a, RHJornadas b"/>
							<cfinvokeargument name="columnas" value="a.RHCMfcapturada as Fecha, a.RHCMid, a.DEid, a.RHCMhoraentradac as HoraEntrada, a.RHCMhorasalidac as HoraSalida, 
																	b.RHJdescripcion,a.RHPMid,
																	'<img border=''0'' src=''/cfmx/rh/imagenes/' + (case RHCMinconsistencia when 0 then 'un' else null end) + 'checked.gif''>' as Inconsistencia, 
																	case when a.RHCMinconsistencia = 0 and (a.RHCMhorasadicautor > 0 or a.RHCMhorasrebajar > 0)then '<a href=''javascript: ProcesarMarca(' || convert(varchar, a.RHPMid) || ', ' || convert(varchar, a.RHCMid) || ', ' || convert(varchar, a.DEid) || ', ' || convert(varchar, a.RHCMhorasadicautor) || ', ' || convert(varchar, a.RHCMhorasrebajar) || ');''><img src=''/cfmx/rh/imagenes/MasterDetail.gif'' border=''0''></a>' else '&nbsp;' end as GeneraIncidencias
																	"/>
							<cfinvokeargument name="desplegar" value="Fecha, HoraEntrada, HoraSalida, RHJdescripcion, Inconsistencia, GeneraIncidencias"/>
							<cfinvokeargument name="etiquetas" value="Fecha, Hora Entrada, Hora Salida, Jornada, Inconsistencia, Incidencias"/>
					...
					<td width="40%" valign="top">
					  <cfif rsPermisoGenMarca.recordCount GT 0>
							<form name="form1" method="post" action="Marcas-SQL.cfm" onSubmit="javascript: return validarInconsistencia(this);">								
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="2">
								  <tr>
									<td  colspan="2" class="tituloAlterno"  align="center" nowrap>
										...
										<cf_translate key="LB_Marca">Marca</cf_translate>
									</td>
								  </tr>
								  <tr>
									...
									<td colspan="2">
										<table width="100%">
											<tr>
												...
												<td align="right" class="fileLabel"><cfif modo EQ 'CAMBIO'><cf_translate key="LB_Inconsistencias">Inconsistencias:</cf_translate><cfelse>&nbsp;</cfif></td>
												<td nowrap valign="middle" style="color:##000099"><cfif modo EQ 'CAMBIO'  and rsMarca.RHCMinconsistencia EQ 1 ><a href='javascript: verInconsistencias(<cfoutput>#Form.RHCMid#</cfoutput>);'>[ver]</a> </cfif></td>
								  ...
								  <tr>
									...
									<td  nowrap>
										<cfif modo EQ 'CAMBIO' and Len(Trim(rsMarca.RHCMjustificacion))>
											<textarea name="RHCMjustificacion" cols="30" rows="6" id="RHCMjustificacion">#rsMarca.RHCMjustificacion#</textarea>
										<cfelse>
											<textarea name="RHCMjustificacion" cols="30" rows="6" id="RHCMjustificacion"></textarea>
										</cfif>
									</td>
								  </tr>
								  <tr>
						...
						<cfelseif modo EQ "CAMBIO">
							<table width="100%" border="0" cellspacing="0" cellpadding="2" style="border: 1px solid black;">
							  ...
							  <tr>
								<td align="right" valign="top" nowrap class="fileLabel"><cf_translate key="LB_Justificacion">Justificaci&oacute;n:</cf_translate></td>
								<td colspan="3" nowrap>
									<textarea name="RHCMjustificacion" cols="30" rows="6" id="RHCMjustificacion" style="border:none;" readonly>#rsMarca.RHCMjustificacion#</textarea>
								</td>
							  </tr>
							  <tr>
								<td align="right" nowrap class="fileLabel"><cf_translate key="LB_Accion_a_Seguir">Accion a Seguir:</cf_translate></td>
    ...
    <cfif rsPermisoGenMarca.recordCount GT 0>
		<script language="javascript" type="text/javascript">
			function validarInconsistencia(f) {
				<cfif modo EQ "CAMBIO" and rsMarca.RHCMinconsistencia EQ 1>
					if (!f.obj.RHCMinconsistencia.checked) {
						...
						<cfoutput>
						alert('#MSG_NoEliminarInconsistencia#');
						</cfoutput>
						return false;
					}
				...
				<cfelseif modo EQ "CAMBIO" and rsMarca.RHCMinconsistencia EQ 0>
					...
					if (f.obj.RHCMinconsistencia.checked) {
						<cfoutput>
						alert('#MSG_TieneIncidenciasAsociadas#');
						</cfoutput>
						return false;
					}
					</cfif>
				...
				if (parseFloat(qf(document.form1.RHCMhorasadicautor.value)) < minValorPos) {
					alert(" Las horas adicionales autorizadas no puede ser menor a " + minValorPos);
					return false;
				}
				if (parseFloat(qf(document.form1.RHCMhorasrebajar.value)) < minValorNeg) {
					alert(" Las horas a rebajar no puede ser menor a " + minValorNeg);
					return false;
				}*/
				<!---</cfif>---->
				return true;
			...
			function deshabilitarValidacion(){
				objForm.RHCMfcapturada.required = false;
			}
			<!----********************************* MODIFICACIONES 09-02-2006 *************************************---->
			function funcCambio(){
				<cfif isdefined("rsVerificaIncidencias") and rsVerificaIncidencias.RecordCount NEQ 0 and rsVerificaIncidencias.cantInsidencias GT 0>
					<cfoutput>
					if (confirm("#MSG_YaGeneraronIncidencias#")){
						return true;
					}
					else{
						return false;
...
function verInconsistencias(id) {
			...
			var dir = '/cfmx/rh/marcas/operacion/DetalleInconsistencias.cfm?RHCMid='+ id;
