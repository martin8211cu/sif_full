...
<cfinvoke Key="LB_Falta" Default="Falta" returnvariable="LB_Falta" component="sif.Componentes.Translate" method="Translate"/>
<cfinvoke Key="LB_Retardo" Default="Retardos" returnvariable="LB_Retardo" component="sif.Componentes.Translate" method="Translate"/>
<cfinvoke Key="LB_Ausencia" Default="Ausencias" returnvariable="LB_Ausencia" component="sif.Componentes.Translate" method="Translate"/>
<cfinvoke component="sif.Componentes.Translate"
				method="Translate"
				Key="LB_RepCtrlAssitencia"
...
<style>
	...
	<cfif len(trim(form.fhasta))>
        <cfset vd_fechaFinal = form.fhasta>
	</cfif>
    
<!---	<cfset vd_fechaFinal = DateAdd("d", 1, "#LSParseDateTime(form.fhasta)#")>
--->    
	...
	<cf_dbtemp name="TempCMHL" returnvariable="TempCMHL" datasource="#session.DSN#">
		...
		<cf_dbtempcol name="tmpJustificacion" 	type="varchar(100)"	mandatory="no">
		<cf_dbtempcol name="tmpRHJhoradiaria" 		type="money"		mandatory="no">		
	</cf_dbtemp>														

	<!---Inserta las fechas ---->
	<cfset vd_fecha = CreateDateTime(year(LSParseDateTime(form.fdesde)), month(LSParseDateTime(form.fdesde)), day(LSParseDateTime(form.fdesde)), 00, 00,0)>
	<cfset vd_fechaFinal = CreateDateTime(year(LSParseDateTime(form.fhasta)), month(LSParseDateTime(form.fhasta)), day(LSParseDateTime(form.fhasta)), 23, 59,0)>
	...
	<cfloop condition = "#vd_fecha# LTE #vd_fechaFinal#">
    	<cfset vd_fechaSig = DateAdd("d", 1, vd_fecha)><!---Siguiente día del rango--->
		<cfquery datasource="#session.DSN#">
			insert into #TempDias# (tmpDfecha)
			values (<cfqueryparam cfsqltype="cf_sql_timestamp" value="#vd_fecha#">)
		</cfquery>
        
    <!---Inserta los empleados y marcas  ----> 
        <cfquery datasource="#session.DSN#">
			insert into #TempCMHL# (tmpfecha,tmpDEid,tmpEntrada,tmpJustificacion,tmpRHJid)
            select 	<cfqueryparam cfsqltype="cf_sql_timestamp" value="#vd_fecha#">,
					lt.DEid,cm.fechahoramarca,cm.justificacion,lt.RHJid
            from LineaTiempo lt
            left outer join RHControlMarcas cm
	 				on lt.DEid=cm.DEid and cm.tipomarca='E' 
                    and cm.fechahoramarca between <cfqueryparam cfsqltype="cf_sql_timestamp" value="#vd_fecha#">
                    and <cfqueryparam cfsqltype="cf_sql_timestamp" value="#vd_fechaSig#">
            ...
            <cfloop query="rsJornada">
                <cfif isdefined('form.ExcluirJ_#RHJid#')>
                    and lt.RHJid != <cfqueryparam cfsqltype="cf_sql_numeric" value="#RHJid#">
                </cfif>
            </cfloop>
		</cfquery>
    ...
    <cfquery name="HistMarca" datasource="#session.DSN#">
        select a.DEid,a.fechahoramarca,coalesce(a.Justificacion,'') as Justificacion from RHHControlMarcas a
        where   a.numlote is not null and a.tipomarca='E'
	...
	<cfloop query="HistMarca">
    	<cfset vd_DEid = #HistMarca.DEid#>
        <cfset vd_fecha = #HistMarca.fechahoramarca#>
        <cfset vd_justi = #HistMarca.Justificacion#>
        
    	<cfquery datasource="#session.DSN#">
    	Update #TempCMHL# set
        	tmpEntrada = <cfqueryparam cfsqltype="cf_sql_timestamp" value="#vd_fecha#">,
          	tmpJustificacion = '#vd_justi#'
        where #TempCMHL#.tmpDEid = #vd_DEid# 
              and <cf_dbfunction name="date_format" args="tmpfecha,yyyymmdd"> = <cfqueryparam cfsqltype="varchar" value="#LSDateFormat(LSParseDateTime(vd_fecha),'yyyymmdd')#">
    	</cfquery>     
    </cfloop>
...
----->
	...
	<cfquery name="rsHorario" datasource="#session.DSN#" >
		select 	tmpDia,
				tmpfecha,
				tmpPuesto,
				tmpLTsalario,tmpJustificacion,tmpRHJhoradiaria,tmpRHJid,
        ...
        on a.tmpDEid=de.DEid and de.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
        
		order by de.DEapellido1,a.tmpDEid, a.tmpfecha asc
	</cfquery>

	<cfquery name="rsDias" datasource="#session.DSN#">
		select tmpDfecha,tmpDDia from #TempDias#
<!---Cambio para que no salgan Sabados y Domingos--->
    	<cfif not isdefined('form.weekend')>    
        	where tmpDDia<>'Sabado' and tmpDDia<>'Domingo' 
        </cfif>
		order by tmpDfecha asc
	</cfquery>
	
	<cfif rsHorario.RecordCount gt 0 and rsHorario.RecordCount lt 10000 >
			<cfset LvarFileName = "ControlAsistencia#Session.Usucodigo#-#DateFormat(Now(),'yyyymmdd')#-#TimeFormat(Now(),'hhmmss')#.xls">
	
			<cfset ir  = 'Controlasistencia.cfm'>
		...
		<cfelse>
			...
			<cfif isdefined("rsHorario") and rsHorario.RecordCount NEQ 0>
				<tr>
					...
					<td width="3%" class="LetraTitulo" align="right"><cf_translate key="LB_Retardos">Retardos</cf_translate></td>
					<td width="3%" class="LetraTitulo" align="right"><cf_translate key="LB_Ausencia">Ausencia</cf_translate></td>                    
				</tr>
...
<!---						<tr class="TituloListas">
						<td colspan="10" style=" border-bottom:1px solid black;"><strong>#LB_Centro_Funcional#&nbsp;:&nbsp;#rsCFuncional.CFuncional#</strong></td></tr>
...
--->				
						...
						<cfloop query="rsDias">
                            ...
                            <cfquery name="rsEntr" datasource="#session.DSN#">
                                select RHJhoraini,RHJMarcar from RHJornadas
                                where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
                                and RHJid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsHorario.tmpRHJid#">
                            </cfquery>
							<cfif len(trim(rsDatos.HoraInicio))>
								<cfset vd_HoraInicio = LSTimeFormat(rsDatos.HoraInicio,'HH:mm')>
								<td width="5%" class="LetraHora" align="center" valign="top" style="padding-right:1px solid black; border-bottom:1px solid black; border-right:1px solid black;">
                                    <cfset hojaInicioJor= mid(#DateAdd("n",30,rsEntr.RHJhoraini)#,17,5)>
                                	<cfif #vd_HoraInicio# gt #hojaInicioJor#>
                                    	<strong> #vd_HoraInicio# </strong>
                                        <cfset vn_TotalRet 	= vn_TotalRet +1>
                                    <cfelse>
                                    	<cfif not isdefined("form.retardos")>
                                    		#vd_HoraInicio#
                                        <cfelse>
                                        	&nbsp;
                                        </cfif>
...
--->                                
							<cfelse>						
								<td class="LetraHora" style="padding-right:1px solid black; border-bottom:1px solid black; border-right:1px solid black;" align="center">
									...
									<cfelse>
                                        <cfif #DatePart("w",rsDias.tmpDfecha)# neq 1 and #DatePart("w",rsDias.tmpDfecha)# neq 7>
<!---											<cfif rsEntr.RHJMarcar eq 1>                                         
												<cf_translate  key="LB_Falta">Falta</cf_translate>
												<cfset vn_TotalFalta 	= vn_TotalFalta +1>
                                            <cfelse>
--->                                            <!---Tengo que buscar el acciones de Personal--->
                            					<cfquery name="rsFalta" datasource="#session.DSN#">
                                                	select count(*) as Faltas from LineaTiempo
                                                    where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#"> and 
                                                    DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#vn_empleado#"> and
                                                    RHTid = 3 and
                                                    
                                                    <cfqueryparam cfsqltype="cf_sql_date" value="#rsDias.tmpDfecha#"> between LTdesde and LThasta
                                                </cfquery>
                                                <cfif #rsFalta.Faltas# gt 0>
                                                    <cf_translate  key="LB_Falta">Falta</cf_translate>
                                                    <cfset vn_TotalFalta 	= vn_TotalFalta +1>
                                                <cfelse>
                                                	<cfif not isdefined("form.retardos")>
                                                    	<cf_translate  key="LB_Falta">OK</cf_translate>
                                                    <cfelse>
                                                        &nbsp;
                                                    </cfif>
                                                </cfif>
...
<!---					<tr>
					...
					<tr>
						<td align="right" colspan="9" class="LetraTitulo"><strong><cf_translate key="LB_TotalGeneral">Total General</cf_translate>:&nbsp;</strong></td>
...
--->				
			<cfelse>
				<tr>
					<td colspan="10" align="center"><strong>------ <cf_translate key="LB_NoSeEncontraronRegistros">No se encontraron registros</cf_translate> ------</strong></td>
				</tr>
			</cfif>
		</cfif>
		<tr><td>&nbsp;</td></tr>
