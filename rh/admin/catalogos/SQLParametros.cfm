...
<cfif isdefined("form.GuardaAdicional")>
	<cfinclude template="parametros_adicionales-sql.cfm">
<cfelseif isDefined("Form.AgregaConfiguracion") or isdefined("form.ModificarConfiguracion") or isdefined("form.EliminaConfiguracion")>
	<cfinclude template="parametros_config-tab-sql.cfm">
<cfelseif isDefined("Form.btnAceptar")>
	...
	<cftransaction>
		...
		<cfif isdefined("form.CHK_IntegracionModuloTesoreria")>
			<cfset datos(545, 'Integracion con modulo de tesoreria', 1)>
		<cfelse>
			<cfset datos(545, 'Integracion con modulo de tesoreria', 0)>
		</cfif>
        ...
        <!---3.5 Tabla Mensual para Liquidaciones y Finiquitos ERBG --->
        <cfif isdefined("form.ImpuestoRenta") >
			<cfset datos(35, 'Tabla Mensual para Liquidaciones y Finiquitos','#form.ImpuestoRenta#' ) >
		</cfif>
       ...
       <!--- SML.--->
		...
		<cfif isdefined("Form.CIID_F")>
			<cfset datos(2033, 'Deduccion Subsidio salario', #form.CIID_F#) >
		</cfif>
		...
		<cfif IsDefined("Form.UMA")>
			<cfset datos(2051,'Valor de la Unidad de Medida y Actualización (UMA)',#form.UMA#)>
		</cfif>

		<cfif IsDefined("form.DAguinaldo")>
			<cfset datos(2052,'Dias aguinaldo para calculo de Factor de Integracion',#form.DAguinaldo#)>
		</cfif>
		...
		<!---5.5 Régimen fiscal para facturación CFDI ERBG --->
        <cfif isdefined("form.RegFiscal") >
			<cfset datos(45, 'Regimen fiscal para facturación CFDI','#form.RegFiscal#' ) >
		</cfif>
		...
		<!---15.1 Cuenta Contable de Asimilados --->
		<cfif len(trim(form.cuentaAsimilados)) gt 0><cfset cuentaAsimilados = form.cuentaAsimilados><cfelse><cfset cuentaAsimilados = '' ></cfif>
		<cfset datos(141, 'Cuenta Contable de Asimilados', cuentaAsimilados) >
		...
		<!--- CURP del Emisor cuando es Persona Fisica --->
		<cfset datos(162, 'CURP persona fisica', #form.CurpPFisica#)>
		...
		<!--- OPARRALES 2018-07-02 Host Correos Autogestion --->
		<cfset datos(14600701,'Host Correos Autogestion',trim(form.txtHostCorreos))>

		<!--- OPARRALES 2018-08-17 Check para decidir si se aplican las acciones desde la interfa del reloj checador --->
		<cfset datos(14600702,'Host Correos Autogestion',(IsDefined('form.PvalorAplicarAccionesIRC')?1:0))>

		<!--- OPARRALES 2018-08-31 Codigo cliente para despensas --->
		<cfset datos(14600703,'Codigo de cliente para despensas',form.PvalorCodClienteDespensa)>

		<!--- OPARRALES 2019-01-08 Check para calcular cargas basado en Semanas por Mes --->
		<cfset datos(14600704,'Calcular Cargas Obrero - Patron basado en Semanas por Mes',(IsDefined('form.chkSemanasXMes') ? 1 : 0))>

		<!--- OPARRALES 2019-01-31 Cuenta Contable para pagos de Despensa Exenta --->
		<cfset datos(14600705,'Cuenta Contable para Pagos de Despensa Exenta',(IsDefined('form.Ccuenta') ? form.Ccuenta : 0))>
		...
		<cfif isdefined("Form.RHCalculoINFONAVITP")>
			<cfset datos(14600707, 'Calculo INFONAVIT por periodo',1) >
		<cfelse>
			<cfset datos(14600707, 'Calculo INFONAVIT por periodo',0) >
		</cfif>
		...
		<!--- Concepto de Pago para Subsidio de Incapacidades (Cuando la accion de personal no escribe en la linea del tiempo) --->
		<cfif isdefined("form.CIidPSI") and len(trim(form.CIidPSI))>
			<cfset datos(2525, 'Concepto de Pago para Subsidio de Incapacidades', #form.CIidPSI#) >
		<cfelse>
			<cfset datos(2525, 'Concepto de Pago para Subsidio de Incapacidades', '') >
		</cfif>
        ...
        <!---SML. Modificacion para que considere mas conceptos de Prima Vacacional para Ajuste Anual parametro 2560--->
        <cfif (isdefined('Form.CIidPrimVac') and LEN(TRIM(Form.CIidPrimVac))) or (isdefined('form.chkPrimVacacional') and LEN(TRIM(form.chkPrimVacacional)))>
			<cfset Lvar_lista = ''>
			<cfif isdefined('form.CIidVac') and LEN(TRIM(form.CIidPrimVac))>
				<cfset Lvar_lista = ListAppend(Lvar_lista,form.CIidPrimVac)>
			</cfif>
			<cfif isdefined('form.chkPrimVacacional') and LEN(TRIM(form.chkPrimVacacional))>
				<cfset Lvar_lista = ListAppend(Lvar_lista,form.chkPrimVacacional)>
			</cfif>
			<cfset datos(2560, 'Concepto Pago Prima Vacacional Ajuste Anual', Lvar_lista) >
		<cfelse>
			<cfset datos(2560, 'Concepto Pago Prima Vacacional Ajuste Anual', '') >
		</cfif>
       <!--- SML.--->

       <!---SML. Modificacion para que considere mas conceptos de Aguinaldo para Ajuste Anual parametro 2561--->
        <cfif (isdefined('Form.CIidAguin') and LEN(TRIM(Form.CIidAguin))) or (isdefined('form.chkAguinaldo') and LEN(TRIM(form.chkAguinaldo)))>
			<cfset Lvar_lista = ''>
			<cfif isdefined('form.CIidAguin') and LEN(TRIM(form.CIidAguin))>
				<cfset Lvar_lista = ListAppend(Lvar_lista,form.CIidAguin)>
			</cfif>
			<cfif isdefined('form.chkAguinaldo') and LEN(TRIM(form.chkAguinaldo))>
				<cfset Lvar_lista = ListAppend(Lvar_lista,form.chkAguinaldo)>
			</cfif>
			<cfset datos(2561, 'Concepto Pago Aguinaldo Ajuste Anual', Lvar_lista) >
		<cfelse>
			<cfset datos(2561, 'Concepto Pago Aguinaldo Ajuste Anual', '') >
		</cfif>
       <!--- SML.--->

       <!---SML. Modificacion para que considere mas conceptos de Separacion para Ajuste Anual parametro 2562--->
        <cfif (isdefined('Form.CIidSep') and LEN(TRIM(Form.CIidSep))) or (isdefined('form.chkSeparacion') and LEN(TRIM(form.chkSeparacion)))>
			<cfset Lvar_lista = ''>
			<cfif isdefined('form.CIidSep') and LEN(TRIM(form.CIidSep))>
				<cfset Lvar_lista = ListAppend(Lvar_lista,form.CIidSep)>
			</cfif>
			<cfif isdefined('form.chkSeparacion') and LEN(TRIM(form.chkSeparacion))>
				<cfset Lvar_lista = ListAppend(Lvar_lista,form.chkSeparacion)>
			</cfif>
			<cfset datos(2562, 'Concepto Pago Separacion Ajuste Anual', Lvar_lista) >
		<cfelse>
			<cfset datos(2562, 'Concepto Pago Separacion Ajuste Anual', '') >
		</cfif>
       <!--- SML.--->

       <!---SML. Modificacion para que considere mas conceptos de Separacion para PTU parametro 2563--->
        <cfif (isdefined('Form.CIidPTU') and LEN(TRIM(Form.CIidPTU))) or (isdefined('form.chkPTU') and LEN(TRIM(form.chkPTU)))>
			<cfset Lvar_lista = ''>
			<cfif isdefined('form.CIidPTU') and LEN(TRIM(form.CIidPTU))>
				<cfset Lvar_lista = ListAppend(Lvar_lista,form.CIidPTU)>
			</cfif>
			<cfif isdefined('form.chkPTU') and LEN(TRIM(form.chkPTU))>
				<cfset Lvar_lista = ListAppend(Lvar_lista,form.chkPTU)>
			</cfif>
			<cfset datos(2563, 'Concepto Pago PTU Ajuste Anual', Lvar_lista) >
		<cfelse>
			<cfset datos(2563, 'Concepto Pago PTU Ajuste Anual', '') >
		</cfif>
       <!--- SML.--->

       <!---SML. Modificacion para que considere mas conceptos de Separacion para PTU parametro 2563--->
        <cfif (isdefined('Form.CIidFOA') and LEN(TRIM(Form.CIidFOA))) or (isdefined('form.chkFOA') and LEN(TRIM(form.chkFOA)))>
			<cfset Lvar_lista = ''>
			<cfif isdefined('form.CIidFOA') and LEN(TRIM(form.CIidFOA))>
				<cfset Lvar_lista = ListAppend(Lvar_lista,form.CIidFOA)>
			</cfif>
			<cfif isdefined('form.chkFOA') and LEN(TRIM(form.chkFOA))>
				<cfset Lvar_lista = ListAppend(Lvar_lista,form.chkFOA)>
			</cfif>
			<cfset datos(2564, 'Concepto Pago FOA Ajuste Anual', Lvar_lista) >
		<cfelse>
			<cfset datos(2564, 'Concepto Pago FOA Ajuste Anual', '') >
		</cfif>
       <!--- SML.--->

		<!--- ERBG Tipo de Riesgo Parametro Recibo CFDI  --->
        <cfif isdefined("form.tipoRiesgo") and LEN(TRIM(form.tipoRiesgo))>
			<cfset datos(301, 'Tipo de Riesgo Parametro Recibo CFDI', Trim(form.tipoRiesgo))>
		</cfif>
	</cftransaction>
</cfif>
