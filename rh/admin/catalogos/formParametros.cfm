...
<cfinvoke key="TAB_Adicional" default="Adicionales" returnvariable="TAB_Adicional" component="sif.Componentes.Translate" method="Translate"/>
<cfinvoke key="TAB_Config" default="Configuracion" returnvariable="TAB_Config" component="sif.Componentes.Translate" method="Translate"/>
...
<script language="JavaScript1.2" type="text/javascript">
	...
	function validar(){
		...
		var regFisc = document.getElementById("RegFiscal");
		var idSelect = regFisc.options[regFisc.selectedIndex].value;
		var miHidden = 'tipoP_'.concat(idSelect);
		var personaF = document.getElementById(miHidden).value;
		var curpPF 	 = document.getElementById('CurpPFisica').value;
		if(personaF == 1)
		{
			if(curpPF == '')
			{
				document.getElementById("CurpPFisica").focus();
				alert('El Regimen Fiscal es de Persona f&iacute;sica: <li> Es necesario capturar CURP.');
				return false;
			}
			else if(curpPF.length != 18)
			{
				document.getElementById("CurpPFisica").focus();
				alert('El Regimen Fiscal es de Persona f&iacute;sica: <li> CURP no valido.');
				return false;
			}
		}
...
<cfset PvalorTablaMensualLiqFin 	= ObtenerDato(35)>	<!--- Indicador de tabla mensual para Liq y Finiquito 2014-01-16 ERBG --->
<cfset PvalorTPSemanal        		= ObtenerDato(40)>	<!--- Tipo Pago Semanal --->
<cfset PvalorRegFiscalCFDI 			= ObtenerDato(45)>	<!--- Indicador de Regimen Fiscal para Facturación CFDI 2014-01-16 ERBG --->
...
<cfset PvalorCuentaAsimilados      	= ObtenerDato(141)>	<!--- Cuenta Contable de Asimilados --->
...
<cfset PvalorCurpPFisica			= ObtenerDato(162)> <!--- Curp cuando el Emisor es persona fisica Oparrales--->
...
<cfset PvalorTpoRiesgo   			= ObtenerDato(301)> <!--- Tipo de Riesgo para Numero Patronal --->
...
<cfset PvalorIntegracionTesoreria	= ObtenerDato(545)> <!--- Integración con módulo de tesoreria --->
...
<cfset PvalorUMA					= ObtenerDato(2051)> <!--- MEX - Valor de la Unidad de Medida y Actualización (UMA) 2017 --->
<cfset PvalorDAguinaldo				= ObtenerDato(2052)> <!--- MEX - Dias Aguinaldo para calculo de Factor de Integracion. 2017 --->
...
<!--- OPARRALES 2018-07-02 Host Correos Autegestion--->
<cfset PvalorHostCorreos				= ObtenerDato(14600701)>
<!--- OPARRALES 2018-08-17 Check para decidir si se aplican las acciones desde la interfa del reloj checador --->
<cfset PvalorAplicarAccionesIRC			= ObtenerDato(14600702)>
<!--- OPARRALES 2018-08-30 Codigo cliente Despensas --->
<cfset PvalorCodClienteDespensa			= ObtenerDato(14600703)>
<!--- OPARRALES 2019-01-08 Check para calcular cargas basado en Semanas por Mes --->
<cfset PvalorCargasSemanasXMes			= ObtenerDato(14600704)>
<!--- OPARRALES 2019-01-31 Cuenta Contable para Pago de Despensa Exenta --->
<cfset PvalorCCuentaDespensaE			= ObtenerDato(14600705)>
<!--- OPARRALES 2019-05-23 Unidad Mixta Infonavit (UMI) parametros dinamicos OCUPADO 14600706 --->

<!--- Calculo INFONAVIT por periodo --->
<cfset PvalorCalculoINFONAVIT			= ObtenerDato(14600707)>
<!---►►Imagen de Ayuda◄◄--->
...
<!--- Cuenta Contable de Asimilados --->
<cfif PvalorCuentaAsimilados.RecordCount GT 0 ><cfset existeCuentaAsimilados = 1><cfelse><cfset existeCuentaAsimilados = 0 ></cfif>
...
<cfquery name="rsEsFisica" datasource="#session.dsn#">
	...
	from  FARegFiscal f
	where
		f.id_RegFiscal = (select cast(p.Pvalor as numeric)
							from RHParametros p
							where p.Pcodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#PvalorRegFiscalCFDI.Pvalor#" null="true">)
	and Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
		...
		<table width="100%" cellpadding="2" cellspacing="0" style="vertical-align:top; ">
			...
			<cf_tabs width="100%" onclick="tab_set_current_param">
				...
				<cf_tab text="<cfoutput>#TAB_Adicional#</cfoutput>" id="7" selected="#form.tab is '7'#">
					<cfinclude template="parametros_adicionales-tab.cfm">
				</cf_tab>
				<cf_tab text="<cfoutput>#TAB_Config#</cfoutput>" id="8" selected="#form.tab is '8'#">
					<cfinclude template="parametros_config-tab.cfm">
				</cf_tab>
			</cf_tabs>
			</form>
			</td></tr>
