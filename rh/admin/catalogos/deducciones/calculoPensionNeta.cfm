<!---SML.Creacion de Archivo para Considerar la Pension Alimenticia Neta.
Se tomo como referencia el de archivo de SolQuim --->

<cfquery name="rsPeriodo" datasource="#session.dsn#">
	select CPperiodo,CPmes
		from CalendarioPagos
	where CPid = <cfif isdefined('rsEmpresa') and len(trim(rsEmpresa.Ecodigo)) GT 0> 
		...
		and Ecodigo = <cfif isdefined('rsEmpresa') and len(trim(rsEmpresa.Ecodigo)) GT 0> 
...
<cfloop query="rsDeducEmpleados">
    ...
    <cfquery datasource="#session.DSN#" name="rsOtro">
		select se.DEid, se.RCNid, se.DEid, se.SEsalariobruto, SErenta, se.SEincidencias, se.SEespecie, SEcargasEmpleado
	...
	<cfset vActual = (rsOtro.SEsalariobruto + rsOtro.SEespecie + SEincidencias  - rsOtro.SErenta - rsOtro.SEcargasEmpleado) * (rsDeducEmpleados.Dvalor/100)>
