...
--->		
        ...
        <cfquery name="rs_diasfaltanominasproceso" datasource="#session.DSN#">
            select 	coalesce(sum(<cf_dbfunction name="datediff" args="lt.LTdesde,lt.LThasta,DD,true"> + 1 ),0) as dias
                ...
                where lt.DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.DEid#">
                    and lt.LThasta >=  (select min(a.LTdesde) <!---SML. Modificacion para que considere la fecha de inicio de la nomina para que traiga solo las faltas del periodo del layout de importar incidencias de calculo--->
										from LineaTiempo a
        ...
        <cfquery name="rs_diasincapnominasproceso" datasource="#session.DSN#">
		 ...
		 select coalesce(sum(<cf_dbfunction name="datediff" args="lt.LTdesde,lt.LThasta,DD,true"> + 1 ),0)
         		 <!---SML. Aplicar comentario Inicio para que considere la fecha de inicio de la nomina para que traiga solo las incapacidades del periodo del layout de importar incidencias de calculo--->
                <!---case when lt.LTdesde > <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha1_Accion#"> then 
                 ...
                 <!---SML. Aplicar comentario para que considere la fecha de inicio de la nomina para que traiga solo las incapacidades del periodo del layout de importar incidencias de calculo---> as dias
                from LineaTiempo lt
                inner join RHTipoAccion ta
                    on ta.RHTid=lt.RHTid
                    ...
                    and ta.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
                where lt.DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.DEid#">
                    and lt.LThasta >= (select min(a.LTdesde) <!---SML. Modificacion para que considere la fecha de inicio de la nomina para que traiga solo las incapacidades del periodo del layout de importar incidencias de calculo--->
										from LineaTiempo a
										inner join CalendarioPagos b 
										on a.Ecodigo = b.Ecodigo
										and a.Tcodigo = b.Tcodigo
...
<!---
 ...
 --->         
        ...
        select 
         ...
         case when lt.LTdesde >= <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha1_Accion#"> then 
                		case when lt.LThasta > <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha2_Accion#"> then 
                                coalesce(sum(datediff(dd, lt.LTdesde, <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha2_Accion#">) + 1) ,0)
                               
                                
                             when lt.LThasta <= <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha2_Accion#"> then 
                                coalesce(sum(datediff(dd, lt.LTdesde, lt.LThasta) + 1 ),0)
                               
                        end
                        
                    else 
                    	case when lt.LThasta > <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha2_Accion#"> then 
                                coalesce(sum(datediff(dd, <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha1_Accion#"> , <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha2_Accion#">) + 1 ),0)
                             when lt.LThasta = <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha2_Accion#"> then 
                                coalesce(sum(datediff(dd, <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha1_Accion#"> , lt.LTdesde)),0) 
                                
                             when lt.LThasta < <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha2_Accion#"> then 
                                coalesce(sum(datediff(dd,<cfqueryparam cfsqltype="cf_sql_date" value="#Fecha1_Accion#">,lt.LThasta)+1),0)   
