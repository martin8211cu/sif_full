<cfsavecontent variable="presets_ret">
	...
	<cfset vDias_habiles = get_dias_habiles(Fecha1_Accion, Fecha2_Accion, RHJid)>
	Dias_habiles  =  <cfoutput>#vDias_habiles#</cfoutput>
	...
	<!---ljimenez se busca la fecha de antiguedad del empleado para determinar a partir de cuando fue nombrado esto
	con el proposito de controlar si el empleado tiene mas de un nombramiento en el mismo año entonces usamos el
	valor de esta fecha para sacar la informacion de faltas y de incapacidades para el año con fecha de inicio
	apartir del ultimo nombramiento--->
	...
	<cfset finicio = CreateDate(year(Fecha1_Accion), 01, 01)>
	<cfset ffinal  = CreateDate(year(Fecha1_Accion), 12, 31)>


	<cfif #finicio# GT #rsFIngreso.EVfantig#>
		<cfset finicio = #CreateDate(year(Fecha1_Accion), 01, 01)#>
	<cfelse>
		<cfset finicio = #rsFIngreso.EVfantig#>
	</cfif>

	<cfif #Fecha1_Accion# LT #ffinal# >
		<cfset ffinal  = #dateadd('d',-1,Fecha1_Accion)#>
	<cfelse>
		<cfset ffinal  = #CreateDate(year(Fecha1_Accion), 12, 31)#>
	</cfif>

	<cfset STRfinicio = "'#dateFormat(finicio,"DD/MM/YYYY")#'">
	<cfset STRffinal  = "'#dateFormat(ffinal,"DD/MM/YYYY")#'">

	<cfquery name="rsDiasFalta1" datasource="#session.dsn#">
		select  case when b.PEdesde > <cfqueryparam cfsqltype="cf_sql_date" value="#finicio#"> then
					case when b.PEhasta > <cfqueryparam cfsqltype="cf_sql_date" value="#ffinal#"> then
							coalesce(sum(<cf_dbfunction name="datediff" args="b.PEdesde, #preserveSingleQuotes(STRffinal)#,DD,true"> + 1 ),0)
							when b.PEhasta <= <cfqueryparam cfsqltype="cf_sql_date" value="#ffinal#"> then
							coalesce(sum(<cf_dbfunction name="datediff" args="b.PEdesde,b.PEhasta,DD,true"> + 1 ),0)
					end
				else
					case when b.PEhasta > <cfqueryparam cfsqltype="cf_sql_date" value="#ffinal#"> then
							coalesce(sum(<cf_dbfunction name="datediff" args=" #preserveSingleQuotes(STRfinicio)#,#preserveSingleQuotes(STRffinal)#,DD,true"> + 1 ),0)
							when b.PEhasta <= <cfqueryparam cfsqltype="cf_sql_date" value="#ffinal#"> then
							coalesce(sum(<cf_dbfunction name="datediff" args=" #preserveSingleQuotes(STRfinicio)#,b.PEhasta,DD,true"> + 1 ),0)
					end
			end as dias
			...
			where b.DEid = #DEid#
				...
				and c.Ecodigo = #Ecodigo#
				and b.PEhasta >= <cfqueryparam cfsqltype="cf_sql_date" value="#finicio#">
				and b.PEdesde <= <cfqueryparam cfsqltype="cf_sql_date" value="#ffinal#">
			group by b.DEid, b.PEdesde, b.PEhasta
	</cfquery>

	<cfif isdefined('rsDiasFalta1') and rsDiasFalta1.RecordCount NEQ 0> <!---SML. Modificacion para la validacion de los dias de falta Inicio--->
		<cfquery name="rsDiasFalta" dbtype="query">
			select sum(dias) as dias
			from rsDiasFalta1
		</cfquery>
		<cfif isdefined('rsDiasFalta') and rsDiasFalta.RecordCount NEQ 0>
			<cfset Dias_Falta = <!---Dias_Falta +---> rsDiasFalta.dias >
		<cfelseif isdefined('rsDiasFalta') and rsDiasFalta.RecordCount EQ 0>
			<cfset Dias_Falta = 0>
		</cfif>
	<cfelseif isdefined('rsDiasFalta1') and rsDiasFalta1.RecordCount EQ 0>
		<cfset Dias_Falta = 0>
	</cfif>             <!---SML. Modificacion para la validacion de los dias de falta Fin--->
	...
	<!----
	<!---ljimenez Dias_falta
	devuelve los dias de Falta para el año 01/01 al 31/12  o desde que entro y due cesado--->
	<cfquery name="rsDiasFalta" datasource="#session.dsn#">
		select coalesce(sum(b.PEcantdias),0) as DiasFalta
			...
			where b.DEid = #DEid#
				...
				<cfelse>
					and b.PEdesde >= <cfqueryparam cfsqltype="cf_sql_date" value="#rsFIngreso.EVfantig#">
				</cfif>
				<cfif #Fecha1_Accion# LT #ffinal# >
					and b.PEhasta <=  <cfqueryparam cfsqltype="cf_sql_date" value="#Fecha1_Accion#">
				<cfelse>
					and b.PEhasta <=  <cfqueryparam cfsqltype="cf_sql_date" value="#CreateDate(year(Fecha1_Accion), 12, 31)#">
				</cfif>
	...
	<!---<cfthrow message="#rsDiasIncap.recordcount#">--->

	<cfset Dias_IncapacidadMX = 0 >
	<cfif isdefined('rsDiasIncap') and rsDiasIncap.recordcount gt 0>
		<cfset Dias_IncapacidadMX=rsDiasIncap.DiasIncap>
	<!---<cfelseif isdefined('rsDiasIncap') and rsDiasIncap.recordcount eq 0>
		<cfset Dias_IncapacidadMX=0>--->
	</cfif>
	Dias_IncapacidadMX =  <cfoutput>#Dias_IncapacidadMX#</cfoutput>

	<!--- SML. Inicio Variable para la devolucion de faltas para aguinaldo--->
	<cfquery name="rsDiasDevolucion" datasource="#session.dsn#">
		select coalesce(sum(RHDFcantdias),0) as devolucion
		from RHDevolucionFaltas
		where DEid = #DEid#
		<cfif #finicio# GT #rsFIngreso.EVfantig#>
			and RHDFfechadesde >= <cfqueryparam cfsqltype="cf_sql_date" value="#CreateDate(year(Fecha1_Accion), 01, 01)#">
		<cfelse>
			and RHDFfechadesde >= <cfqueryparam cfsqltype="cf_sql_date" value="#rsFIngreso.EVfantig#">
		</cfif>
	</cfquery>

	<cfset diasdevolucion = 0 >
	<cfif isdefined('rsDiasDevolucion') and rsDiasDevolucion.recordcount gt 0>
		<cfset diasdevolucion=rsDiasDevolucion.devolucion>
	</cfif>
	diasdevolucion =  <cfoutput>#diasdevolucion#</cfoutput>
	...
	<!--- ljimenez Salario Hoy es el salario que tiene el empleado al momento que se aplica la accion o realiza el calculo (fecha  de aplicacio no fecha rige)
	es requerido para mexico ya que pagan por ejemplo las vacaciones con el salario de aplicacion --->

	<cfset salariohoy = 0>

	<cfquery name="rsSalarioHoy" datasource="#session.DSN#">
		select coalesce((select DLTmonto
			from DLineaTiempo
		where LTid=a.LTid
			and CSid in (select CSid
							from ComponentesSalariales
							where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
							and CIid is null and CSsalariobase = 1)),0) as SalarioHoy
			from LineaTiempo a
			where DEid = <cfqueryparam cfsqltype="cf_sql_integer" value="#DEid#">
				and Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
	...
	<cfif rsSalarioHoy.RecordCount NEQ 0>
		<cfset salariohoy = salariohoy + rsSalarioHoy.SalarioHoy>
	</cfif>
	salariohoy = <cfoutput>#salariohoy#</cfoutput>
...
<!---			Tcodigo = <cfoutput>#Tcodigo#</cfoutput> --->
		...
		<cfif Len(Trim(Fecha1))>
			...
			<cfquery name="tbl_PagosEmpleadoInsert" datasource="#session.DSN#">
				...
				select distinct a.RCNid, #DEid#, a.RCdesde, a.RChasta, 0, <cfqueryparam cfsqltype="cf_sql_numeric" value="#lvarRHAlinea#">,cp.CPtipo
						...
						<cfloop from="#Lvar_desde#" to="#Lvar_hasta#"  index="i">
							<cfquery name="rs" datasource="#session.DSN#">
								...
								where hp.RCNid = #rsNom.RCNid #
									and  <cfqueryparam cfsqltype="cf_sql_date" value="#i#"> between hp.PEdesde and hp.PEhasta
	...
	SalarioPromedio = <cfoutput>#SalarioPromedio#</cfoutput>
		...
		<!--- SML variable para obtener el monto acumulado para el trabajador cuando tiene su finiquito--->
	<!---<cf_dump var = "#form#">--->
	<cfset acum_fondoahorro = 0.00>
	<cfquery name="rsFondoAhorro" datasource="#session.DSN#">
		select coalesce(SUM(FAmonto),0) as monto
		from RHHFondoAhorro
		where FAEstatus = 0
			and DEid = #DEid#
			and Ecodigo = #Ecodigo#
	</cfquery>
	<cfset acum_fondoahorro = #rsFondoAhorro.monto#/ 2>
	acum_fondoahorro = <cfoutput>#acum_fondoahorro#</cfoutput>
	

	<!--- OPARRALES 2019-04-01
		Variables para complementar los calculos de incidencias.
	 --->
	<cfset varProporcionFaltasHoras 	= 0>
	<cfset varDiasFaltaXPeriodo			= 0>
	<cfset varDiasIncapacidadXPeriodo	= 0>
	<cfset varFini = now()>
	<cfset varFfin = now()>
	<cfset varTcodigo = "">
	<cfset varComodin = "04">
	...
	<cfif IsDefined('Arguments.DEid') and Arguments.DEid gt 0>
		<cfif IsDefined('Arguments.CPdesde') and Arguments.CPdesde neq ''>
			<!--- Calculo desde Interfaz Reloj Checador Fiscal --->
			<cfset varFini = Arguments.CPdesde>
			<cfset varFfin = Arguments.CPhasta>
			<cfset varTcodigo = Arguments.Tcodigo>
		<cfelseif isDefined('form.TCPdesde') and form.TCPdesde neq ''>
			<!--- Calculo desde Interfaz Reloj Checador Temporada --->
			<cfset varFini = form.TCPdesde>
			<cfset varFfin = form.TCPhasta>
			<cfset varTcodigo = form.TTcodigo>
			
		<cfelse>
			<!--- Calculo desde proceso de Incidencias --->
			<cfquery name="rsUltimaAccion" datasource="#session.dsn#">
				select 
					...
					Tcodigo
				from 
					DLaboralesEmpleado dl
				inner join RHTipoAccion ta
					on dl.RHTid = ta.RHTid
				where dl.DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#DEid#">
				and ta.RHTcomportam <> <cfqueryparam cfsqltype="cf_sql_numeric" value="2">
				and dl.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
				order by DLfvigencia desc
			</cfquery>

			<cfset varTcodigo = rsUltimaAccion.Tcodigo>

			<cfif IsDefined('form.IFecha') and form.IFecha neq ''>
				<cfquery name="rsCP" datasource="#session.DSN#">
					select 
						CPdesde,
						CPhasta
					from CalendarioPagos
					where <cfqueryparam cfsqltype="cf_sql_timestamp" value="#LSDateFormat(form.IFecha,'YYYY-MM-dd')#"> 
					between CPdesde and CPhasta
					and Tcodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#varTcodigo#">
					and Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
				</cfquery>
				<cfset varFini = rsCP.CPdesde>
				<cfset varFfin = rsCP.CPhasta>
			<cfelse>
				<cfif IsDefined('form.CPdesde') and form.CPdesde neq ''>
					<cfset varFini = form.CPdesde>
					<cfset varFfin = form.CPhasta>
				<cfelse>
					<cfset varFini = LSDateFormat(Arguments.FECHA1_ACCION,'YYYY-MM-dd')>
					<cfset varFfin = LSDateFormat(Arguments.FECHA2_ACCION,'YYYY-MM-dd')>
				</cfif>

			</cfif>
		</cfif>
		
		<!--- Si calcula cargas es nomina normal si no es de temporada --->
		<cfquery name="rsTNomina" datasource="#session.dsn#">
			select 
				CalculaCargas 
			from TiposNomina 
			where
				Tcodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Trim(varTcodigo)#">
			and Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
		</cfquery>

		<cfif rsTNomina.RecordCount gt 0 and rsTNomina.CalculaCargas eq 0>
			<cfset varComodin = "FEF">
		</cfif>

		<cfquery name="rsFaltaHors" datasource="#session.DSN#">
			select 
				coalesce(Ivalor,0) Ivalor
			from Incidencias i
			inner join CIncidentes ci
			on i.CIid = ci.CIid
			where ci.CIcodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#varComodin#">
			and DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#DEid#">
			and i.Ifecha
			between <cfqueryparam cfsqltype="cf_sql_timestamp" value="#varFini#"> and <cfqueryparam cfsqltype="cf_sql_timestamp" value="#varFfin#">
			and ci.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
		</cfquery>
		<cfset varProporcionFaltasHoras = (rsFaltaHors.RecordCount gt 0 ? rsFaltaHors.Ivalor : 0)>
		
		<!--- OPARRALES 2019-03-20 Obtener dias de falta del empleado --->
		<cfquery name="rsDiasFalta" datasource="#session.dsn#">
			SELECT
				SUM(CASE
					<!--- Validacion cuando la falta empieza antes y termina despues del periodo de pago seleccionado --->
					WHEN lt.LTdesde < '#varFini#' AND lt.LThasta > '#varFfin#'
						THEN DATEDIFF(DD,'#varFini#','#varFfin#')+1
					<!--- Validacion cuando las fechas de la falta dentro del periodo de pago seleccionado --->
					WHEN lt.LTdesde >= '#varFini#' AND lt.LThasta <= '#varFfin#'
						THEN DATEDIFF(DD, LTdesde, lt.LThasta)+1
					<!--- Validacion cuando la falta empieza antes o igual de la fecha inicio del periodo de pago 
							pero termina antes o igual de la fecha fin del periodo de pago seleccionado --->
					WHEN lt.LTdesde <= '#varFini#' AND lt.LThasta <= '#varFfin#'
						THEN DATEDIFF(DD, '#varFini#', lt.LThasta)+1
					<!--- Validacion cuando la falta empieza igual o despues de la fecha inicio del periodo de pago 
							pero termina despues o igual de la fecha fin del periodo de pago seleccionado --->
					WHEN lt.LTdesde >= '#varFini#' AND lt.LThasta >= '#varFfin#'
						THEN DATEDIFF(DD, lt.LTdesde, '#varFfin#')+1
				END) AS PEcantdias
			FROM LineaTiempo lt
			INNER JOIN RHTipoAccion ta 
			...
			AND (
					'#varFini#' BETWEEN lt.LTdesde AND lt.LThasta
					OR '#varFfin#' BETWEEN lt.LTdesde AND lt.LThasta
					or lt.LTdesde between '#varFini#' and '#varFfin#'
					or lt.LThasta between '#varFini#' and '#varFfin#'
				)
			AND lt.DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.DEid#">
		</cfquery>

		<cfset varDiasFaltaXPeriodo = ((rsDiasFalta.RecordCount gt 0 and rsDiasFalta.PEcantdias gt 0) ? rsDiasFalta.PEcantdias : 0)>
		...
		<cfquery name="rsDiasFalta" datasource="#session.dsn#">
			SELECT
				SUM(CASE
					<!--- Validacion cuando la falta empieza antes y termina despues del periodo de pago seleccionado --->
					WHEN lt.DLfvigencia < '#varFini#' AND lt.DLffin > '#varFfin#'
						THEN DATEDIFF(DD,'#varFini#','#varFfin#')+1
					<!--- Validacion cuando las fechas de la falta dentro del periodo de pago seleccionado --->
					WHEN lt.DLfvigencia >= '#varFini#' AND lt.DLffin <= '#varFfin#'
						THEN DATEDIFF(DD, DLfvigencia, lt.DLffin)+1
					<!--- Validacion cuando la falta empieza antes o igual de la fecha inicio del periodo de pago 
							pero termina antes o igual de la fecha fin del periodo de pago seleccionado --->
					WHEN lt.DLfvigencia <= '#varFini#' AND lt.DLffin <= '#varFfin#'
						THEN DATEDIFF(DD, '#varFini#', lt.DLffin)+1
					<!--- Validacion cuando la falta empieza igual o despues de la fecha inicio del periodo de pago 
							pero termina despues o igual de la fecha fin del periodo de pago seleccionado --->
					WHEN lt.DLfvigencia >= '#varFini#' AND lt.DLffin >= '#varFfin#'
						THEN DATEDIFF(DD, lt.DLfvigencia, '#varFfin#')+1
				END) AS PEcantdias
			FROM RHAcciones lt
			INNER JOIN RHTipoAccion ta 
				ON ta.RHTid = lt.RHTid
			...
			AND ta.RHTcomportam = <cfqueryparam cfsqltype="cf_sql_numeric" value="13">
			AND (
					'#varFini#' BETWEEN lt.DLfvigencia AND lt.DLffin
					OR '#varFfin#' BETWEEN lt.DLfvigencia AND lt.DLffin
					or lt.DLfvigencia between '#varFini#' and '#varFfin#'
					or lt.DLffin between '#varFini#' and '#varFfin#'
				)
			AND lt.DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.DEid#">
		</cfquery>
		...
		<cfquery name="rsIncap" datasource="#session.dsn#">
			SELECT
				SUM(CASE
		...
		<cfset varDiasIncapacidadXPeriodo = ((rsIncap.RecordCount gt 0 and rsIncap.PEcantdias gt 0) ? rsIncap.PEcantdias: 0)>
	</cfif>

	<cfset varDias_PIngreso = 0>
	<cfquery name="rsDiasDif" datasource="#session.dsn#">
		select top 1
			case 
				when DLfvigencia between <cfqueryparam cfsqltype="cf_sql_date" value="#varFini#"> 
						and <cfqueryparam cfsqltype="cf_sql_date" value="#varFfin#"> 
						then DATEDIFF(day,<cfqueryparam cfsqltype="cf_sql_date" value="#varFini#">,DLfvigencia)
				else 0
			end AS diasDif
		from 
		...
		order by coalesce(DLffin,'6100-01-01') desc
	</cfquery>
	<cfif rsDiasDif.RecordCount gt 0>
		<cfset varDias_PIngreso = rsDiasDif.diasDif>
	</cfif>
	<cfset varDias_SLabor = 0>
	<cfquery name="rsDiasDifSL" datasource="#session.dsn#">
		select top 1
			case 
				when DLfvigencia between <cfqueryparam cfsqltype="cf_sql_date" value="#varFini#"> 
						and <cfqueryparam cfsqltype="cf_sql_date" value="#varFfin#"> 
						then DATEDIFF(day,DLfvigencia,<cfqueryparam cfsqltype="cf_sql_date" value="#varFfin#">)
				else 0
			end AS diasDif
		from 
			DLaboralesEmpleado dle
	...
	<cfif rsDiasDifSL.RecordCount gt 0>
		<cfset varDias_SLabor = rsDiasDifSL.diasDif>
	</cfif>

	ProporcionFaltasHoras 	= <cfoutput>#varProporcionFaltasHoras#</cfoutput>
	DiasFaltaXPeriodo 		= <cfoutput>#varDiasFaltaXPeriodo#</cfoutput>
	DiasIncapacidadXPeriodo = <cfoutput>#varDiasIncapacidadXPeriodo#</cfoutput>
	Dias_PIngreso			= <cfoutput>#varDias_PIngreso#</cfoutput>
	Dias_PDiasSL			= <cfoutput>#varDias_SLabor#</cfoutput>

	<cfquery name="rsHoraExtra" datasource="#session.dsn#">
		select Imonto
		from incidencias a
		inner join CIncidentes b on a.CIid=b.CIid
		where a.DEid=#Arguments.DEid# and CIcodigo='4'
			and a.Ifecha
			between <cfqueryparam cfsqltype="cf_sql_timestamp" value="#varFini#"> and <cfqueryparam cfsqltype="cf_sql_timestamp" value="#varFfin#">
	</cfquery>
	HoraExtra	= <cfoutput>#(rsHoraExtra.recordCOunt gt 0)?rsHoraExtra.Imonto:0#</cfoutput>
	
	<cfquery name="rsDiaDescanso" datasource="#session.dsn#">
		select Imonto
		from incidencias a
		inner join CIncidentes b on a.CIid=b.CIid
		where a.DEid=#Arguments.DEid# and CIcodigo='11'
			and a.Ifecha
			between <cfqueryparam cfsqltype="cf_sql_timestamp" value="#varFini#"> and <cfqueryparam cfsqltype="cf_sql_timestamp" value="#varFfin#">
	</cfquery>
	DiaDescanso	= <cfoutput>#(rsDiaDescanso.recordCOunt gt 0)?rsDiaDescanso.Imonto:0#</cfoutput>
