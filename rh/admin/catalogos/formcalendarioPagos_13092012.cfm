				...
				<tr> <!--- lista --->
					<td> <!--- TD: Lista--->
						...
						<cfinvoke component="rh.Componentes.pListas" method="pListaRH" returnvariable="pListaCalenPagos">
							...
							<cfinvokeargument name="columnas" 	value="CPcodigo,CPid,Tcodigo,CPdesde,CPhasta,CPfpago, case when CPtipo = 0 then 'N' when CPtipo = 1 then 'E' when CPtipo = 2 then 'A' when CPtipo = 3 then 'R' when CPtipo = 4 then 'PTU' end as tipo #preservesinglequotes(campos_adicionales)#"/>
					...
					<td> <!--- TD: mant. principal --->
						<table width="100%" cellpadding="0" cellspacing="0"> <!--- TABLE: mant. principal --->
							...
							<tr> 
								...
								<td>
									<select name="CPtipo" onChange="javascript: activa_desactiva(); <cfif modo NEQ 'ALTA'>mostrarCargas();</cfif> " <cfif modo EQ 'CAMBIO' and LEN(TRIM(rsCalenPago.CPfcalculo))>disabled</cfif>>
										<cfif modo NEQ 'ALTA'>
											<option value="0" <cfif rsCalenPago.CPtipo EQ 0>selected</cfif>><cf_translate   key="CMB_Normal">Normal</cf_translate></option>
											<option value="1" <cfif rsCalenPago.CPtipo EQ 1>selected</cfif>><cf_translate   key="CMB_Especial">Especial</cf_translate></option>
											<option value="2" <cfif rsCalenPago.CPtipo EQ 2>selected</cfif>><cf_translate   key="CMB_Anticipo">Anticipo</cf_translate></option>
											<cfif usaEstructuraSalarial>
											<option value="3" <cfif rsCalenPago.CPtipo EQ 3>selected</cfif>><cf_translate   key="CMB_Anticipo">Retroactivo</cf_translate></option>
											</cfif>
                                            <cfif CalculaPTU>
												<option value="4" <cfif rsCalenPago.CPtipo EQ 4>selected</cfif>>PTU</option>
											</cfif>
										<cfelse>
											<option value="0" selected><cf_translate   key="CMB_Normal">Normal</cf_translate></option>
											<option value="1"><cf_translate key="CMB_Especial">Especial</cf_translate></option>
											<option value="2"><cf_translate key="CMB_Anticipo">Anticipo</cf_translate></option>
											<cfif usaEstructuraSalarial>
											<option value="3"><cf_translate key="CMB_Retroactivo">Retroactivo</cf_translate></option>
											</cfif>
                                            <cfif CalculaPTU>
							...
							<tr>
								<td colspan="3">
									...
									<table width="100%" cellpadding="0" cellspacing="0" border="0">
										<tr>
											<td width="1%" >
												<input type="checkbox" name="CPnorenta" id="CPnorenta" value="checkbox" onclick="changeNoRenta()"
													   <cfif modo NEQ 'ALTA' and rsCalenPago.CPnorenta EQ 1>checked</cfif> >
											</td>
...
<script language="JavaScript1.2" type="text/javascript">
	...
	{ 
		...
		// Tipo: Especial = 1, Normal = 0, Anticipo = 2, 4 = PTU
		if (f.CPtipo.value == '1')  
		{		
		...
		else 
		{
			...
			// oculta los mantenimientos de Tipos de Deducción y Conceptos de Pago
			<cfif modo NEQ 'ALTA'>
				tablaDetalles_cp.style.display = 'none';
				//tablaDetalles_td.style.display = 'none';
			</cfif>	
