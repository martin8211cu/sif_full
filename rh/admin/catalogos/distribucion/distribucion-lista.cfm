						...
						<cfif isdefined("Form.sel") and form.sel NEQ 1>
	   					...
	   					<table width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<tr style="display: ;" id="verFiltroListaEmpl"> 
								<td> 
							  		<form name="formFiltroListaEmpl" method="post" action="distribucion-lista.cfm">
											...
											<td>
												<cfquery name="rsLista" datasource="#session.DSN#">
													select DEid,
														   ...
														   1 as sel,
														   'ALTA' as modo,
                                                           case when (select count(DECSid) from DistEmpCompSal  where DatosEmpleado.DEid = DistEmpCompSal.DEid) > 0
                                                           then
                                                           	'<img src=''/cfmx/rh/imagenes/w-check.gif'' border=''0''>'
                                                           else
                                                           	'&nbsp;' 
                                                           end as salarios,
                                                           case when (select count(DEIid) from DistEmpIncidencias  where DatosEmpleado.DEid = DistEmpIncidencias.DEid) > 0
                                                           then
                                                           	'<img src=''/cfmx/rh/imagenes/w-check.gif'' border=''0''>'
                                                           else
                                                           	'&nbsp;' 
                                                           end as incidencias,
                                                           case when (select count(DECid) from DistEmpCargas  where DatosEmpleado.DEid = DistEmpCargas.DEid) >0
                                                           then
                                                           	'<img src=''/cfmx/rh/imagenes/w-check.gif'' border=''0''>'
                                                           else
                                                           	'&nbsp;' 
												...
												<cfinvoke 
													...
													<cfinvokeargument name="desplegar" value="DEidentificacion, nombreEmpl,salarios,incidencias,cargas"/>
													<cfinvokeargument name="etiquetas" value="#vIdentificacion#,#vEmpleado#,#LB_Salarios#,#LB_Incidencias#,#LB_Cargas#"/>
													...
													<cfinvokeargument name="irA" value="distribucion.cfm"/>
