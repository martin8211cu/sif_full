...
<cfif isdefined("Form.CIid") and  len(trim(Form.CIid))>
    ...
    <cfquery datasource="#Session.DSN#" name="rsConceptosInc">
		Select a.DEIid,a.CFid,b.CFcodigo,b.CFdescripcion,a.DEIporcentaje
		from DistEmpIncidencias a
	...
	<cfquery datasource="#Session.DSN#" name="rstotal">
		Select coalesce(sum(DEIporcentaje),0) as DEIporcentaje ,count(DEIid) as cantidad
		from DistEmpIncidencias
...
<cfset navegacionSal = "">
<cfset navegacionSal = navegacionSal & Iif(Len(Trim(navegacionSal)) NEQ 0, DE("&"), DE("")) & "o=2">

<cfif isdefined("Form.DEid")>
	<cfset navegacionSal = navegacionSal & Iif(Len(Trim(navegacionSal)) NEQ 0, DE("&"), DE("")) & "DEid=" & Form.DEid>
</cfif>
<cfif isdefined("Form.CIid")>
	<cfset navegacionSal = navegacionSal & Iif(Len(Trim(navegacionSal)) NEQ 0, DE("&"), DE("")) & "CIid=" & Form.CIid>
</cfif>
...
<cfinvoke component="sif.Componentes.Translate"
	method="Translate"
	key="LB_ConceptoIncidente"
	Default="Concepto Incidente"
	returnvariable="LB_ConceptoIncidente"/>	
	...
	<table  width="100%" border="0" cellspacing="0" cellpadding="0">	
		<tr>
					...
					<td>
					  ...
					  <cfinvoke 
						 ...
						 returnvariable="pListaSal">
							<cfinvokeargument name="tabla" value="CIncidentes a "/>
							<cfinvokeargument name="columnas" value="a.CIid,a.CIcodigo,a.CIdescripcion ,
																	case when (select count(b.CIid) from  DistEmpIncidencias  b 
																			where a.CIid = b.CIid 
																	...
																	case when (select count(b.CIid) from  DistEmpIncidencias  b 
																			...
																			and b.DEid = #form.DEid#)> 0 then 
																				(select sum(b.DEIporcentaje) from  DistEmpIncidencias  b 
																				where a.CIid = b.CIid 
																				and a.Ecodigo = b.Ecodigo
																				and b.DEid = #form.DEid#)
																	else null end as porcentaje,2 as o,#form.DEid# as DEid,2 as sel"/>
							<cfinvokeargument name="desplegar" value="CIcodigo,CIdescripcion,tiene,porcentaje"/>
							 <cfinvokeargument name="keys" value="DEid,CIid"/>		
				
                            <cfinvokeargument name="etiquetas" value="#LB_codigo#,#LB_Incidencia#,#LB_Distribuido#,%"/> 
							<cfinvokeargument name="formatos" value=""/>
							<cfinvokeargument name="formName" value="listaIncidencias"/>	
							<cfinvokeargument name="filtro" value="a.Ecodigo =#session.Ecodigo# order by CIcodigo"/>
			...
			<td width="70%" valign="top">
				<cfoutput>
				<form name="form1" method="post" action="incidencias-sql.cfm"  onSubmit="return valida();" >
					<table  id="tbldynamic"  width="100%" border="0" cellspacing="0" cellpadding="0">
					  <cfif not isdefined("Form.CIid")>
						  <tr>
							<td colspan="2">
								<font color="##FF0000"> Tiene que seleccionar un concepto incidente de la lista</font>
							</td>
					  ...
					  <cfelse>
						  <tr>
							<td width="35%" nowrap="nowrap">
								<font  style="font-size:10px">#LB_ConceptoIncidente#</font>
							</td>
							<td colspan="2">
								<font  style="font-size:10px"><strong>#rsConcepto.CIcodigo#-#rsConcepto.CIdescripcion#</strong></font>
							</td>
						  </tr>
					  </cfif>
					...
					<table  width="100%" border="0" cellspacing="0" cellpadding="0">	
						<cfif isdefined("rstotal")>
							...
							<cfelse>
								<cfif rstotal.DEIporcentaje eq 100>
									<cfinvoke component="sif.Componentes.Translate"
									method="Translate"
								...
								<cfelseif rstotal.DEIporcentaje gt 100>
									<cfinvoke component="sif.Componentes.Translate"
									method="Translate"
									...
									<tr>
										<td  colspan="2">
											<b>#MSG_distribucion_supera_el_100_porcierto#</b> &nbsp;&nbsp;<font color="##FF0000"> (#rstotal.DEIporcentaje# %)	</font>
										</td>
									</tr>
								<cfelseif rstotal.DEIporcentaje lt 100>
									<cfinvoke component="sif.Componentes.Translate"
									method="Translate"
									...
									<tr>
										<td  colspan="2">
											<b>#MSG_distribucion_menor_al_100_porcierto#</b> &nbsp;&nbsp;  <font color="##FF0000"> (#rstotal.DEIporcentaje# %)	</font>
										</td>
									</tr>
								</cfif>
								<cfif rstotal.DEIporcentaje neq 100>
									<cfinvoke component="sif.Componentes.Translate"
									method="Translate"
									Key="MSG_Advertencia"
					  ...
					  <tr>
						<td colspan="2" align="center">
							<fieldset><legend><cf_translate  key="LB_Nota">Nota</cf_translate></legend>
								<table>
									<tr>
										<td>
											<li><cf_translate  key="LB_Ayuda1">Si no se indica una incidencia la distribuci&oacute;n se realizar&aacute; masivamente.</cf_translate></li>	
										</td>	
									</tr>
								</table>		
							</fieldset>
					...
					<input type="hidden" name="CIid"   			id="CIid"    value="<cfif isdefined("Form.CIid") and  len(trim(Form.CIid))>#form.CIid#</cfif>">
...
<cfinvoke component="sif.Componentes.Translate"
	method="Translate"
	Key="MSG_Es_requerido_seleccionar_un_concepto_incidente_de_la_lista_antes_de_distribuirla"
	Default="Es requerido seleccionar un concepto incidente de la lista antes de distribuirla"
	returnvariable="MSG_Es_requerido_seleccionar_un_concepto_incidente_de_la_lista_antes_de_distribuirla"/>
...
<script language="JavaScript" type="text/javascript">
	...
	<cfif isdefined("rsConceptosInc") and rsConceptosInc.recordcount gt 0 >
		<cfloop query="rsConceptosInc">
			<cfoutput>
				<cfset descripcion =  LSNumberFormat(rsConceptosInc.DEIporcentaje,'____.__')  & '  % ' & trim(rsConceptosInc.CFcodigo) & '-' &  trim(rsConceptosInc.CFdescripcion)  >
			</cfoutput>
			fnNuevoTRSP('<cfoutput>#rsConceptosInc.CFid#</cfoutput>','<cfoutput>#rsConceptosInc.DEIporcentaje#</cfoutput>','<cfoutput>#descripcion#</cfoutput>');
		</cfloop>
	</cfif>	
	...
	function fnNuevoTR(){
	  ...
	  var p2 		= qf(document.form1.DEIporcentaje.value);//cod
 	  var p3 		=  document.form1.DEIporcentaje.value + '  % ' +document.form1.CFcodigo.value  +'-'+ document.form1.CFdescripcion.value  ;//descripcion
	  // Valida no agregar vacíos
	  if (p1=="" || p2=="") {
	  alert('<cfoutput>#MSG_Debe_seleccionar_un_centro_funcional_e_indicar_el_porcentaje_de_distribucion#</cfoutput>')
	  return;
	...
	function valida(){
		if (validar){
			...
			<!--- if ( trim(document.form1.CIid.value) == '' ){
				error = true;
				mensaje += " - <cfoutput>#MSG_Es_requerido_seleccionar_un_concepto_incidente_de_la_lista_antes_de_distribuirla#</cfoutput>.\n";
			} --->
