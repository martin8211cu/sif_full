...
<!--- Valida que el empleado no esté repetido --->
<cfquery name="rsCheck0" datasource="#session.dsn#">
	insert into #ERRORES_TEMP# (Mensaje,ErrorNum)  <!------>
	select {fn concat('Empleado repetido: ',tarjeta)},0
	from #table_name# a
	group by snnumero, tarjeta, referencia, tdcodigo
	having count(1) > 1
</cfquery>
...
<cfquery name="rsCheck3" datasource="#session.dsn#">
	insert into #ERRORES_TEMP# (Mensaje,ErrorNum)
	select distinct { fn concat('Empleado no Existe (', { fn concat (tarjeta,')')})},3
	from #table_name# a
	where not exists(
		select 1
			...
			<cftransaction>
				...
				<cfquery datasource="#session.dsn#">
					...
					select <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsEIDlote.EIDlote#">, 
						DEidentificacion, referencia, metodo, valor, fechaini, fechafin, monto, controlsaldo
					from #table_name# a, DatosEmpleado b
					where snnumero = <cfqueryparam cfsqltype="cf_sql_char" value="#SNnumero#">
					and tdcodigo = <cfqueryparam cfsqltype="cf_sql_char" value="#TDcodigo#">
					and b.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
					and a.tarjeta = b.DEtarjeta
				</cfquery>
			</cftransaction>	
	</cfloop>
