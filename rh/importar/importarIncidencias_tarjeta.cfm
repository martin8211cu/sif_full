...
<cfquery name="rsCheck0" datasource="#session.dsn#">
	...
	from #table_name# a
	group by tarjeta, CIcodigo, Ifecha
	having count(tarjeta) != 1
</cfquery>
...
<cfif bcheck1>
	...
	<cfquery name="rsCheckEmpl" datasource="#session.dsn#">
		...
		from #table_name# 
		where tarjeta not in (select DEtarjeta
											from DatosEmpleado 
	...
	<cfif bcheckEmpl >
			...
			<cfif rsCheckJornada.checkJornada eq 0  >
					...
					<cfif rsCheckCFuncional.checkFuncional eq 0 >		
							...
							<cfif rsCheckConcepto.checkConcepto eq 0  >		
										...
										<cfif session.Incidencias.impcalculo><!---Si es importacion de incidencias tipo calculo--->											
											<cfquery datasource="#session.DSN#">
												...
												select 	( select min(DEid) from DatosEmpleado where ltrim(rtrim(DEtarjeta))=ltrim(rtrim(a.tarjeta)) and Ecodigo=#session.Ecodigo# ) as DEid, 
							...
							<cfelse><!--- bcheckConcepto --->
									<cfquery name="ERR" datasource="#session.dsn#">
										select 'Concepto Incidente no existe' as Motivo, tarjeta, CIcodigo, Ifecha, Ivalor 
										from #table_name# a
					...
					<cfelse><!--- bcheckCfuncional --->
							<cfquery name="ERR" datasource="#session.dsn#">
								select 'Centro Funcional no existe' as Motivo, tarjeta, CIcodigo, Ifecha, Ivalor 
								from #table_name# a
								where CFcodigo is not null
								and CFcodigo not in (select rtrim(ltrim(CFcodigo))
													from CFuncional
			...
			<cfelse><!--- bcheckJornada --->
					<cfquery name="ERR" datasource="#session.dsn#">
						select 'Jornada no existe' as Motivo, tarjeta, CIcodigo, Ifecha, Ivalor 
						from #table_name# a
						where Jornada is not null
						and Jornada not in (select rtrim(ltrim(RHJcodigo))
											from RHJornadas 
	...
	<cfelse><!--- bcheckEmpl --->
		<cfquery name="ERR" datasource="#session.dsn#">
			select 'Empleado no existe' as Motivo, tarjeta, CIcodigo, Ifecha, Ivalor 
			from #table_name# a
			where a.tarjeta not in ( 	select DEtarjeta 
...
<cfelse><!--- bchek1 --->
	<cfquery name="ERR" datasource="#session.dsn#">
		select 'Registro ya existe' Motivo, a.tarjeta, b.DEidentificacion, DEnombre+' '+DEapellido1+' '+DEapellido2,a.CIcodigo, a.Ifecha, a.Ivalor
		from #table_name# a, DatosEmpleado b, CIncidentes c, Incidencias d
		where a.tarjeta = b.DEtarjeta
		and rtrim(ltrim(a.CIcodigo))=rtrim(ltrim(c.CIcodigo))
		and b.DEid=d.DEid
...
<cfelse><!--- bchek0 --->
	<cfquery name="ERR" datasource="#session.dsn#">
		select 'Existen registros duplicados dentro del archivo cargado' Motivo, a.tarjeta, a.CIcodigo, a.Ifecha
		from #table_name# a
		group by tarjeta, CIcodigo, Ifecha
		having count(tarjeta) != 1
	</cfquery>
