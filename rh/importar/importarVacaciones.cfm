...
<!--- Valida la cantidad de dias de Vacaciones --->
<cfquery name="rsDatosVac" datasource="#session.dsn#">
	select id, CodigoEmpleado, TI, FechaDesde, FechaHasta, DiasVacaciones, j.RHJid
	from #table_name# tmp
...
<cfloop query="rsDatosVac">
	<cfset lvarVacMax = RH_Calculadora.get_dias_habiles(rsDatosVac.FechaDesde, rsDatosVac.FechaHasta, rsDatosVac.RHJid)>
	<cfif rsDatosVac.DiasVacaciones lt 1 or rsDatosVac.DiasVacaciones gt lvarVacMax>
		<cfset fnInsertarError(7, "<strong>Error:</strong> La cantidad de vacaciones debe de estar entre 1 y #lvarVacMax# de días, actualmente presenta #rsDatosVac.DiasVacaciones# día(s). <br/><strong>Datos del registro</strong> : Empleado(#rsDatosVac.CodigoEmpleado#), Identificación(#rsDatosVac.TI#), Fecha desde(#LSDateFormat(rsDatosVac.FechaDesde, 'DD/MM/YYYY')#), Fecha hasta(#LSDateFormat(rsDatosVac.FechaHasta, 'DD/MM/YYYY')#).<br /><strong>Error en el registro Num°:</strong> #rsDatosVac.id#")>
	</cfif>
</cfloop>
...
<cf_dbfunction name="to_char"	args="DiasVacaciones"  isNumber="true" returnvariable="lvarDiasVacaciones">
<cfquery datasource="#session.dsn#">
	insert into #ERRORES_TEMP# (Mensaje,ErrorNum)
	select '<strong>Error:</strong> La cantidad de vacaciones debe de ser mayor a 0. Vacaciones actuales: ' #_CAT# #lvarDiasVacaciones# #_CAT# '.<br/><strong>Datos del registro</strong> : Empleado(' #_CAT# CodigoEmpleado #_CAT# '), Identificación(' #_CAT# TI #_CAT# '), Fecha desde(' #_CAT# #lvarFechaDesde# #_CAT# '), Fecha hasta(' #_CAT# #lvarFechaHasta# #_CAT# ').<br /><strong>Error en el registro Num°:</strong> ' #_CAT# #lvarID#, 10
	from #table_name#
	where DiasVacaciones <= 0
</cfquery>
...
<cfif err.recordcount eq 0>
	...
	<cfquery name="rsVacacionesLineas" datasource="#session.DSN#">
		select de.DEid, tmp.TI, tmp.FechaDesde, tmp.FechaHasta, tmp.Observaciones, tmp.DiasVacaciones, ta.RHTespecial, ta.RHTid, coalesce(ta.RHTporcPlazaCHK, 0) as RHTporcPlazaCHK, ta.RHTcsalariofijo, coalesce(ta.RHTporcsal,100) as  RHTporcsal, coalesce(ta.RHTporc, 100) as RHTporc
		from #table_name# tmp
			inner join DatosEmpleado de
				on de.Ecodigo = #session.Ecodigo#
		  		  and de.DEidentificacion = tmp.CodigoEmpleado
	...
	<cftransaction>
		<cfoutput query="rsVacacionesLineas">
			<cfset fnProcesoGuardar(rsVacacionesLineas.DEid, rsVacacionesLineas.RHTid, rsVacacionesLineas.FechaDesde, rsVacacionesLineas.FechaHasta, rsVacacionesLineas.DiasVacaciones, rsVacacionesLineas.Observaciones, rsVacacionesLineas.RHTespecial, rsVacacionesLineas.RHTporcPlazaCHK, rsVacacionesLineas.RHTcsalariofijo, rsVacacionesLineas.RHTporcsal, rsVacacionesLineas.RHTporc, usaEstructuraSalarial)>
		</cfoutput>
	</cftransaction>
</cfif>