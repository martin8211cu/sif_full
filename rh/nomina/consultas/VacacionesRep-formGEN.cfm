<!---SML. Modificacion para que se visualice los mismos saldos que en el reporte de Consultas de Saldos de Vacaciones--->
<!---23/10/2013--->
...
<cf_dbtemp name="tbl_SaldosVacaciones" returnvariable="tbl_SaldosVacaciones">
	...
	<cfquery name="rsins" datasource="#Session.DSN#">
		...
		select f.CFdescripcion, f.CFid, f.CFcodigo, b.RVid, 
				{fn concat({fn concat({fn concat({fn concat(a.DEapellido1 , ' ' )}, a.DEapellido2 )}, ' ' )}, a.DEnombre )} as nombrecompleto,
				 a.DEid, a.DEtarjeta, 
				coalesce(c.EVfantig, <cf_dbfunction name="today">) as FAntiguedad,
				coalesce(EVfecha, EVfantig) as cortevacaciones,
				coalesce(sum(d.DVEdisfrutados), 0.00) as Disponibles
		...
		a.DEtarjeta, coalesce(c.EVfantig, <cf_dbfunction name="today">),coalesce(EVfecha, EVfantig)
	</cfquery>
							...
							where rv.RVid = #tbl_SaldosVacaciones#.Regimen
			  				and rv.DRVcant = 
										( 
											...
											and rv2.DRVcant <= #tbl_SaldosVacaciones#.AnyosLaborados
											)
		  				)		
	</cfquery>
    ...
    <cfquery name="rsDias" datasource="#session.DSN#">
		...
		<cfif Application.dsinfo[session.DSN].type neq 'oracle'>
			set saldoProyectado = ((DiasRegimen/365)  * ( datediff(dd, FechaCorteRegimen, FechaHoy)))
		<cfelse>
	...
	<cfsavecontent variable="ENCABEZADO_IMP">
		<td colspan="10">
			...
			<cfoutput>
				<tr><td>
					<cfinvoke key="LB_ConsultaDeSaldosDeVacaciones" default="Consulta General Saldos de Vacaciones" returnvariable="LB_ConsultaDeSaldosDeVacaciones" component="sif.Componentes.Translate"  method="Translate"/>
		...
		<tr>
		<td class="titulolistas">Tarjeta</td>
				...
				<cfoutput>
					<cfloop query="rsSaldos">
						<cfif (isdefined('Url.imprimir') and contadorlineas gte 50 and rsSaldos.CurrentRow NEQ 1) or  primercorte>
						  #ENCABEZADO_IMP#
						 <!--- <tr>
							<td class="listaCorte" colspan="10" nowrap>Centro Funcional: #rsSaldos.CFcodigo# - #rsSaldos.CFdescripcion#</td>
						  </tr>--->
						...
						<!---  <tr>
						  ...
						  <tr>
							...
							<td class="detalle">#rsSaldos.CFcodigo# - #rsSaldos.CFdescripcion#</td>
