...
<cfset LB_Empresa = t.translate('LB_Empresa','Empresa','/rh/indexReportes.xml')>
...
<cfset LB_Nomina = t.translate('LB_Nomina','Nómina','/rh/generales.xml')/>
<cfset CHK_NominaAplicadas = t.translate('CHK_NominaAplicadas','Nóminas Aplicadas','/rh/generales.xml')/>
<cfset CHK_UtilizarFiltroDeFechas = t.translate('CHK_UtilizarFiltroDeFechas','Utilizar filtro de fechas','/rh/generales.xml')/>
<cfset LB_ListaDeCargasSocialesAExcluir = t.translate('LB_ListaDeCargasSocialesAExcluir','Lista de Cargas Sociales a excluir',rut)/>
<cfset LB_ListaDeDeduccionesAExcluir = t.translate('LB_ListaDeDeduccionesAExcluir','Lista de Deducciones a excluir',rut)/>
<cfset LB_Codigo= t.translate('LB_Codigo','Código','/rh/generales.xml')/>
<cfset LB_Descripcion = t.translate('LB_Descripcion','Descripción','/rh/generales.xml')/>
<cfset MSG_NominasRequeridas = t.translate('MSG_NominasRequeridas','Debe seleccionar la menos una nómina','/rh/generales.xml')/>
<cfset MSG_Agrupacion = t.translate('MSG_Agrupacion','Agrupación según el catálogo de Agrupación de Rubros de Nómina General','/rh/generales.xml')/>


<div class="text-right">
	<cf_PreferenciasUsuario Tipo="Reporte" llave="nomina.consultas.RepDetallePlanillaRevision.cfm" form="form1" onLoad='cargarListaVisualListaAExcluirLista'>
</div>
...
<form class="bs-example form-horizontal" action="RepDetallePlanillaRevision-sql.cfm" method="post" name="form1" id="form1" style="margin:0">
	<cfoutput>
	 <!--- Agrupación general RHAgrupadorRubro y RHDAgrupadorRubro, catalogo se encuentra en parametros RH,  --->
    <div class="form-group">
        <span class="col-sm-12 nominas">
            <input type="checkbox" name="Detallado" id="Detallado"> 
            <label><strong>#LB_Detallado#</strong></label>
            <cf_notas titulo="#MSG_Agrupacion#" link="#helpimg#" pageIndex="1" animar="true">
        </span>
    </div>
    ...
    <!---<div class="form-group"> 
		<div class="checkbox col-md-offset-2">
            <label>
              <input type="checkbox" id="chkHistorico" name="chkHistorico"> <cfoutput>#CHK_NominaAplicadas#</cfoutput>
            </label>
        </div>
	</div>

	<div class="form-group">
		<label for="Tcodigo2" class="col-lg-2 control-label"><cfoutput>#LB_Nomina#</cfoutput>:</label>
		<div class="col-lg-10" style="display:none" id="divHistorico">
        	<cf_rhcalendariopagos Historicos="1" index="1" descsize="30">
        </div>
		<div class="col-lg-10" id="divProceso">
        	<cf_rhcalendariopagos index="2" descsize="30">
        </div>
	</div>
--->
	
	<div class="form-group">
		<label for="DCcodigo" class="col-lg-2"><cfoutput>#LB_ListaDeCargasSocialesAExcluir#</cfoutput>:</label>
		<div class="col-lg-10">
			<cf_translatedata name="get" col="DCdescripcion" tabla="DCargas" returnvariable="LvarDCdescripcion">
	        <cf_conlis
				Campos="DClinea,Edescripcion,DCcodigo,DCdescripcion"
				Desplegables="N,S,S,S"
				Modificables="N,N,N,N"
				Size="0,10,15,25"
				tabindex="1" 
				Title="#LB_ListaDeCargasSocialesAExcluir#"
				Tabla="DCargas a inner join Empresas e on a.Ecodigo=e.Ecodigo"
				Columnas="distinct e.Edescripcion,a.DClinea,a.DCcodigo,#LvarDCdescripcion# as DCdescripcion"
				Desplegar="Edescripcion,DCcodigo,DCdescripcion"
				Etiquetas="#LB_Empresa#,#LB_Codigo#,#LB_Descripcion#"
				filtro="a.Ecodigo in ($jtreeListaItem,numeric$)  order by #LvarDCdescripcion#,DCcodigo"
				filtrar_por="Edescripcion,DCcodigo,DCdescripcion"
				Formatos="S,S,S"
				Align="left,left,left"
				Asignar="DClinea,Edescripcion,DCcodigo,DCdescripcion"
				TranslateDataCols="DCdescripcion"
				agregarEnLista="true" 
	...
	<div class="form-group">
		<label for="TDcodigo" class="col-lg-2"><cfoutput>#LB_ListaDeDeduccionesAExcluir#</cfoutput>:</label>
		<div class="col-lg-10">  
			<cf_translatedata name="get" col="TDdescripcion" tabla="TDeduccion" returnvariable="LvarTDdescripcion">
	        <cf_conlis
				Campos="TDid,TDEdescripcion,TDcodigo,TDdescripcion"
				Desplegables="N,S,S,S"
				Modificables="N,N,N,N"
				Size="0,10,15,25"
				tabindex="2" 
                index="4" 
				Title="#LB_ListaDeDeduccionesAExcluir#"
				Tabla="TDeduccion a inner join Empresas e on a.Ecodigo=e.Ecodigo"
				Columnas="distinct e.Edescripcion as TDEdescripcion,a.TDid,a.TDcodigo,#LvarTDdescripcion# as TDdescripcion"
				Desplegar="TDEdescripcion,TDcodigo,TDdescripcion"
				Etiquetas="#LB_Empresa#,#LB_Codigo#,#LB_Descripcion#"
				filtro="a.Ecodigo in ($jtreeListaItem,numeric$) order by #LvarTDdescripcion#,TDcodigo"
				filtrar_por="TDEdescripcion,TDcodigo,TDdescripcion"
				Formatos="S,S,S"
				Align="left,left,left"
				Asignar="TDid,TDEdescripcion,TDcodigo,TDdescripcion"
...
<script type="text/javascript">
	function cargarListaVisualListaAExcluirLista(objResponse){
   		var _ListaDClinea = "0";
   		var _ListaTDid = "0";
   		objResponse.DATA = decodeURIComponent(objResponse.DATA);
   		$("input[name=ListaDClinea]").parent().parent().remove();
   		$("input[name=ListaTDid]").parent().parent().remove();
		var lista = objResponse.DATA.split('&');

		for(i=0; i<lista.length; i++){
			ele = lista[i].split('=');
			if(ele[0] == 'ListaDClinea'){
				_ListaDClinea += ','+ele[1];
			}else if(ele[0] == 'ListaTDid'){
				_ListaTDid += ','+ele[1];
			}
		} 

		//Pintar los detalles 1
		try{
			$.ajax ({
				type: "get",
				url: "/cfmx/rh/nomina/consultas/RepDetallePlanillaRevision-sql.cfm?ajaxresponse=1&DClinea="+_ListaDClinea,
				dataType: "html",
				success: function( objResponse ){
					$("#Conlis_Table_1").append(objResponse);
					},
				error:  function( objRequest, strError ){
					var MensajeError = 'Error al intentar cargar la configuracion.: ' + strError;
					alert(MensajeError);
			...
			var MensajeError = 'Error Inesperado: ' + ss;
			alert(MensajeError);
			console.log(MensajeError);
		}

		//Pinta los detalles 2
		try{
			$.ajax ({
				type: "get",
				url: "/cfmx/rh/nomina/consultas/RepDetallePlanillaRevision-sql.cfm?ajaxresponse=1&TDid="+_ListaTDid,
				dataType: "html",
				success: function( objResponse ){
					$("#Conlis_Table_2").append(objResponse);
					},
				error:  function( objRequest, strError ){
					var MensajeError = 'Error al intentar cargar la configuracion.: ' + strError;
					alert(MensajeError);
...
<script language="JavaScript1.2">
	<!---objForm.Tcodigo2.description = '<cfoutput>#LB_Nomina#</cfoutput>';
	objForm.Tcodigo1.description = '<cfoutput>#LB_Nomina#</cfoutput>';
	objForm.DCcodigo.description = '<cfoutput>#LB_ListaDeCargasSocialesAExcluir#</cfoutput>';
	...
	--->
	<!---bjForm.ListaNomina.description = '<cfoutput>#LB_Nomina#</cfoutput>';
	objForm.ListaNomina.required = true;
	--->
	function funcConsultar(){
		if(!document.form1.ListaNomina){
			alert('<cfoutput>#MSG_NominasRequeridas#</cfoutput>');
			return false;
		}
