...
<cfinvoke key="LB_ReporteDeAuxiliarDeProvisionDeVacaciones" default="Reporte de Auxiliar de Provisi&oacute;n de Prima Vacacional" returnvariable="LB_ReporteDeAuxiliarDeProvisionDeVacaciones" component="sif.Componentes.Translate" method="Translate"/>
...
<cfquery datasource="#session.dsn#" name="CalendarioPagos">	
    ...
    where Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
    	and <cf_whereInList Column="CPid" ValueList="#form.CPidlist1#" cfsqltype="cf_sql_numeric">
</cfquery>
...
<!---<cfif  listFindNoCase('1,0',form.tipoEmpleado,',') GT 0	>--->
	...
	<cfquery datasource="#session.DSN#" name="rsGetDatos">
		...
		from HRCalculoNomina a	
            ...
            and b.RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.CPidlist1#">
   ...
   <cfquery name="rsDatosEmpleado" datasource="#session.DSN#">
	...
	<cfloop query="rsDatosEmpleado">
        ...
        <cfquery name="rsDiasPrima" datasource="#session.DSN#">
        	select coalesce(rv.DRVdiasprima, 0) as DRVdias
        ...
        <cfset factorDiario = (#rsDiasPrima.DRVdias#/365) >
        
        <cfquery datasource="#session.DSN#">
        	insert into #PrimaVacacional# (PElinea,DEid,Dias,SueldoMensual)
        	select PElinea,DEid, SUM(PEcantdias) as diasLab,PEsalarioref
			from HPagosEmpleado
			where DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsDatosEmpleado.DEid#">
				and RCNid = <cfqueryparam cfsqltype="cf_sql_integer" value="#CalendarioPagos.CPid#"> 
			group by PElinea,DEid,PEsalarioref
        </cfquery>
        ...
        <cfloop query="rsSueldoDiario">
        	...
        	<cfset ImportePrimaVacional = #ProvisionDiaria# * #rsSueldoDiario.Dias#>
...
<cf_htmlReportsHeaders 
	title="#LB_ReporteDeAuxiliarDeProvisionDeVacaciones#" 
	filename="#LvarFileName#"
	irA="repAuxiliarProVacaciones-filtro.cfm">
		...
		<td>
			<table width="100%" cellpadding="0" cellspacing="0" align="center">
				<tr><td>					
					...
					<cf_EncReporte
						Titulo="#LB_ReporteDeAuxiliarDeProvisionDeVacaciones#"
						Color="##E3EDEF"
						filtro1="#filtro1#"
						filtro2="#filtro2#"
					>
				...
				<table width="100%" cellpadding="0" cellspacing="0" border="0" align="center">
					...
					<tr>
						<td valign="top" style="border-bottom:1px solid black;" nowrap="nowrap">
                        	<strong><cf_translate key="LB_Identificacion">Identificicacion</cf_translate></strong></td>
						<td valign="top" style="border-bottom:1px solid black;" nowrap="nowrap" >
						...
						<td align="right" style="border-bottom:1px solid black;" nowrap="nowrap">
							<strong><cf_translate key="LB_SaldoProvision">Importe <br />Prima Vacacional</cf_translate></strong>
						</td>
