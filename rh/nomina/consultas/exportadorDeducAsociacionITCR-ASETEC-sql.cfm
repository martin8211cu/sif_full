...
<!---Codigos Cargas Capitalizacion Empleado exportador ASETEC--->
<cfinvoke component="rh.Componentes.RHParametros" method="get" datasource="#session.dsn#" 
                ecodigo="#session.Ecodigo#" pvalor="2558" default="-1" returnvariable="lvarCapEmpASETEC"/>

<!---Codigos Deducciones usadas exportador ASETEC--->
<cfinvoke component="rh.Componentes.RHParametros" method="get" datasource="#session.dsn#" 
                ecodigo="#session.Ecodigo#" pvalor="2559" default="-1" returnvariable="lvarDedEmpASETEC"/>

<!---Codigos Cargas Capitalizacion Patrono exportador ASETEC--->
<cfinvoke component="rh.Componentes.RHParametros" method="get" datasource="#session.dsn#" 
                ecodigo="#session.Ecodigo#" pvalor="2560" default="-1" returnvariable="lvarCapPatASETEC"/>
...
<cfelse>
	<cfquery datasource="#session.DSN#">					
		<!----Tipos de deduccion--->
		insert into #ASOCIA# (Codigo,Identificacion,Monto)
        select ltrim(rtrim(td.TDcodigo)), ltrim(rtrim(e.DEidentificacion)), sum(coalesce(b.DCvalor,0))
		from HSalarioEmpleado a
			...
			inner join TDeduccion td
				on c.TDid=td.TDid
				and td.TDcodigo in (<cfqueryparam cfsqltype="cf_sql_varchar" value="#lvarDedEmpASETEC#" list="true">)
		...
		union all
		<!---Cargas Empleado --->
		select ltrim(rtrim(dc.DCcodigo)),ltrim(rtrim(e.DEidentificacion)), sum(coalesce(b.CCvaloremp,0)) as  Monto
		from HSalarioEmpleado a
			...
			inner join DCargas dc
				...
				and dc.DCcodigo in (<cfqueryparam cfsqltype="cf_sql_varchar" value="#lvarCapEmpASETEC#" list="true">)
			inner join CalendarioPagos d
				on a.RCNid = d.CPid
				<cfif isdefined("url.periodo") and url.periodo NEQ -1 and isdefined("url.mes") and url.mes NEQ -1>
					and d.CPperiodo = <cfqueryparam cfsqltype="cf_sql_integer" value="#url.periodo#">
			...
			group by e.DEidentificacion, dc.DCcodigo


		union all
		<!---Cargas Patrono--->
		select ltrim(rtrim(dc.DCcodigo)),ltrim(rtrim(e.DEidentificacion)),sum(coalesce(b.CCvalorpat,0)) as Monto
		from HSalarioEmpleado a
			inner join HCargasCalculo b
				on a.RCNid = b.RCNid
				and a.DEid = b.DEid
			inner join DCargas dc
				on b.DClinea=dc.DClinea
				and dc.DCcodigo in (<cfqueryparam cfsqltype="cf_sql_varchar" value="#lvarCapPatASETEC#" list="true">)
			inner join CalendarioPagos d
				on a.RCNid = d.CPid
				<cfif isdefined("url.periodo") and url.periodo NEQ -1 and isdefined("url.mes") and url.mes NEQ -1>
					and d.CPperiodo = <cfqueryparam cfsqltype="cf_sql_integer" value="#url.periodo#">
    ...
    <cfquery name="ERR" datasource="#session.DSN#">
            Select  rtrim(#preservesinglequotes(LvarReferencia)#) #CAT# #preservesinglequotes(LvarReferenciaS)# #CAT#
                    #preservesinglequotes(LvarIdentificacionS)# #CAT# rtrim(#preservesinglequotes(LvarIdentificacion)#) #CAT#
                    #preservesinglequotes(LvarMontoStrLS)# #CAT# rtrim(#preservesinglequotes(LvarMontoStr)#)                 
            from #ASOCIA#
            order by Identificacion,Codigo
    </cfquery>    
