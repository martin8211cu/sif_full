...
<cfsilent>
	...
	<cfsavecontent variable="pNavegacion">
   	...
   	<cfinvoke component="sif.Componentes.Translate" method="Translate" Key="LB_TituloReporte" Default="Reporte Detallado de Factores del IMSS" returnvariable="LB_TituloReporte"/>
...
<cfloop query="rsCantidadColumnas">
    ...
    <cfset vsTrab = 0>						<!---Afecta IMSS Patrón Default--->
    <cfquery datasource="#session.dsn#" name="rsAfectaIMSS">
    	select RHRPTNAfecta from RHColumnasReporte
		where RHCRPTid = #rsCantidadColumnas.cantidad#
	</cfquery>
    <cfif rsAfectaIMSS.RHRPTNAfecta eq 2>   <!---Afecta IMSS Trabajador--->
    	<cfset vsTrab = 1>
    </cfif>
...
<!---                                   <cfif isdefined('rsCargasPatronales') and rsCargasPatronales.DCvaloremp GT 0 and rsCargasPatronales.DCvalorpat LTE 0>--->
									<cfif isdefined('rsCargasPatronales') and vsTrab eq 1 and rsCargasPatronales.DCvaloremp GT 0>
                                   		sum(a.CCvaloremp)
<!---								 <cfelseif isdefined('rsCargasPatronales') and rsCargasPatronales.DCvalorpat GT 0 and  rsCargasPatronales.DCvaloremp LTE 0>--->
                                 	<cfelseif isdefined('rsCargasPatronales') and vsTrab eq 0 and rsCargasPatronales.DCvalorpat GT 0>
									 	sum(a.CCvalorpat)
                                    <cfelse>
										0
									</cfif>
...
<cf_htmlReportsHeaders irA="repDetalleIMSS.cfm"
