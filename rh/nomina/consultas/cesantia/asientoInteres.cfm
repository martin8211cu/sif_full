	...
	<!--- Valiad Parametro 870 (cuenta de acumulacion de cesantia )--->
	<cfquery name="rs_880" datasource="#session.DSN#">
		select Pvalor
		from RHParametros 
		where Ecodigo=#session.Ecodigo# 
		  and Pcodigo=880	
	</cfquery>
	<cfif len(trim(rs_880.Pvalor)) eq 0 >
		<cfinvoke component="sif.Componentes.Translate"
		method="Translate"
		Key="LB_No_ha_definido_la_cuenta_de_Interes_de_Cesantia_Verifique_sus_datos"
		Default="Error. No ha definido la cuenta de Inter&eacute;s de Cesant&iacute;a. Verifique sus datos"
		returnvariable="LB_No_ha_definido_la_cuenta_de_Interes_de_Cesantia_Verifique_sus_datos"/> 
		<cf_throw message="#LB_No_ha_definido_la_cuenta_de_Interes_de_Cesantia_Verifique_sus_datos#" errorCode="1070">
	</cfif>
	...
	<cf_templateheader title="#LB_RecursosHumanos#" template="#session.sitio.template#">
		<table width="100%" cellpadding="1" cellspacing="0">
			<tr>
				<td valign="top">	
					<cf_web_portlet_start border="true" titulo="Asiento de Inter&eacute;s de Cesant&iacute;a" skin="#Session.Preferences.Skin#"> 
						<table width="100%">
							<tr>
								<td>
									<cfif isdefined("url.DEid") and len(trim(url.DEid))>
										<iframe width="100%" height="500" frameborder="0" src="asientoInteres-form.cfm?periodo=<cfoutput>#url.periodo#</cfoutput>&mes=<cfoutput>#url.mes#</cfoutput>&DEid=<cfoutput>#url.DEid#</cfoutput>" ></iframe>
									<cfelse>										
										<iframe width="100%" height="500" frameborder="0" src="asientoInteres-form.cfm?periodo=<cfoutput>#url.periodo#</cfoutput>&mes=<cfoutput>#url.mes#</cfoutput>" ></iframe>
									</cfif>
								</td>
							</tr>
						</table>
