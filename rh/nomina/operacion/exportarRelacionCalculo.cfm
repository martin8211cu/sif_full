	...
	<cf_templateheader title="#LB_RecursosHumanos#" template="#session.sitio.template#">
		...
		<cfinvoke component="sif.Componentes.Translate"
			method="Translate"
			key="LB_Asiento_de_Relacion_de_Calculo"
			Default="Exportar Asiento de Relaci&oacute;n de C&aacute;lculo"
			returnvariable="vExportar"/>
		...
		<cf_web_portlet_start border="true" skin="#session.preferences.skin#" titulo="#vExportar#">
			<table width="100%" cellpadding="2" cellspacing="0">
				<tr>
					<td>
						<cfoutput>
						<form method="post" name="filtro" action="exportarRelacionCalculo.cfm" style="margin:0;">
					...
					<td>
						...
						<cfquery name="rsLista" datasource="#Session.DSN#">
							select a.RCNid, 
								   ...
								   a.RChasta, 
								   a.RCdesde, 
								   {fn concat( {fn concat(rtrim(a.Tcodigo), ' - ')},  b.Tdescripcion)} as Tcodigo
						...
						<form name="form1" method="post" style="margin:0" action="exportarRelacionCalculo-sql.cfm"  >
							<cfoutput>
							<input type="hidden" name="fTcodigo" value="<cfif isdefined("form.fTcodigo") and len(trim(form.fTcodigo))>#form.fTcodigo#</cfif>" />
							<input type="hidden" name="fCPcodigo" value="<cfif isdefined("form.fCPcodigo") and len(trim(form.fCPcodigo))>#form.fCPcodigo#</cfif>" />
							<input type="hidden" name="fecha1" value="<cfif isdefined("form.fecha1") and len(trim(form.fecha1))>#form.fecha1#</cfif>" />
							<input type="hidden" name="fecha2" value="<cfif isdefined("form.fecha2") and len(trim(form.fecha2))>#form.fecha2#</cfif>" />
							<input type="hidden" name="pageNum_lista" value="<cfif isdefined("form.pageNum_lista") and len(trim(form.pageNum_lista))>#form.pageNum_lista#</cfif>" />
							</cfoutput>
			...
			<cfif isdefined("url.no_data")>
				<script>
					alert('La nomina seleccionada no tiene registros asociados.');
				</script>
			</cfif>
