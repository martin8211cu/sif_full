...
<cfsilent>
<cfinvoke Key="MSG_NoSeEncuentraLaAccion" Default="No se encuentra la acción" returnvariable="MSG_NoSeEncuentraLaAccion" component="sif.Componentes.Translate" method="Translate"/>
<cfinvoke Key="MSG_NoSeEncuentraNombradoRecargo" Default="No se encuentra nombrado en esta plaza para las fechas de la acción" returnvariable="MSG_NoSeEncuentraNombradoRecargo" component="sif.Componentes.Translate" method="Translate"/>
</cfsilent>
...
<cfif modo NEQ "ALTA" AND isdefined("Session.Ecodigo") AND isdefined("Form.RHAlinea") AND Len(Trim(Form.RHAlinea)) GT 0>
	...
	<cfquery name="rsTipoAccionComp" datasource="#Session.DSN#">
		...
		a.EcodigoRef, b.RHTcomportam, b.RHTcempresa, b.RHTnoveriplaza,RHCatParcial
    ...
    <!--- verificar si la plaza existe entre ese grango de fechas --->
    <cfquery name="PlazaActual" datasource="#session.DSN#">
    	select RHPid
        from LineaTiempoR
        where LTRid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsAccion.RHAccionRecargo#">
    </cfquery>
    	<cfquery name="rsVerifNombR" datasource="#session.DSN#">
        	select 1
            from LineaTiempoR a
            ...
            and a.RHPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#PlazaActual.RHPid#">    
        </cfquery>
    	<cfif rsVerifNombR.RecordCount EQ 0>
            <cfthrow message="#MSG_NoSeEncuentraNombradoRecargo#">
            <cfabort>
        </cfif>
	...
	<cfquery name="rsEstadoActual" datasource="#Session.DSN#">
		...
		where <cfqueryparam cfsqltype="cf_sql_timestamp" value="#rsAccion.DLfvigencia#"> between a.LTdesde and a.LThasta
		and a.DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsAccion.DEid#" null="#Len(rsAccion.DEid) is 0#">
        and a.RHPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsAccion.RHPid#">
	...
	<cfif rsEstadoActual.recordCount GT 0>
        ...
        <cfquery name="rsVerTAccion" datasource="#session.dsn#">
         ...
         <cfset LvarMontoCatProp=0>
        <cfif rsVerTAccion.RHCatParcial eq 1 and rsAccion.RHCPlineaP GT 0>
            ...
            <cfquery name="rsMonto" datasource="#session.dsn#">
                ...
                where a.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
                  and RHVTestado = 'A'
                  and a.RHCPlinea = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsAccion.RHCPlinea#">
            </cfquery>
            <cfdump var="#rsMonto#">
            ...
            <cfquery name="rsMontoP" datasource="#session.dsn#">
                ...
                where a.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
                  and RHVTestado = 'A'
                  and a.RHCPlinea = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsAccion.RHCPlineaP#">                                                     			
            </cfquery>
            <cf_dump var="#rsMontoP#">
            <cfif isdefined ('rsMontoP') and rsMontoP.recordcount gt 0>
                <cfset LvarMontoCatProp=rsMonto.SB+abs(((rsMontoP.SB-rsMonto.SB)/2))>
                <cfset LvarMontoCatProp= LSNumberFormat(LvarMontoCatProp,",0.00")>
            </cfif>
		</cfif>
		...
		<cfelse>
			<cfquery name="rsComponentesActual" datasource="#Session.DSN#">
				select c.CSid, 
					   ...
					   coalesce(d.RHMCvalor, 1.00) as valor
				from RHDAcciones a
					 left outer join DLineaTiempoR b
						on b.CSid = a.CSid
						and b.LTRid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsEstadoActual.LTRid#">
					 inner join ComponentesSalariales c
						on c.CSid = a.CSid
					 left outer join RHMetodosCalculo d
						on d.Ecodigo = c.Ecodigo
				...
				from DLineaTiempoR b
					 ...
					 left outer join RHMetodosCalculo d
						...
						and <cfqueryparam cfsqltype="cf_sql_date" value="#rsAccion.rige#"> between d.RHMCfecharige and d.RHMCfechahasta
						and d.RHMCestadometodo = 1
				where b.LTRid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsEstadoActual.LTRid#">
				order by 4, 2, 3
			</cfquery>
