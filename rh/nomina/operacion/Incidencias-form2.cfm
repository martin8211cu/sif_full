...
<cfif isdefined("url.DEid1") and len(trim(url.DEid1)) gt 0>
	...
	<cfquery name="rsEmpleadoFiltro" datasource="#Session.DSN#">
		select a.DEid as DEid1, 
			   {fn concat({fn concat({fn concat({ fn concat( a.DEnombre, ' ') }, a.DEapellido1)}, ' ')}, a.DEapellido2) } as 	NombreEmp1,
...
<form  name="form1" method="get" action="Incidencias-sql.cfm" onsubmit="javascript: return validaForm(this);">
	<table width="95%" align="center" border="0" cellspacing="0" cellpadding="1">		
		...
		<!--- Línea No. 2 --->
     	<tr> 
			<td  colspan="2"> 
				<cfif modo NEQ "ALTA">
					<cf_rhempleado query="#rsIncidencia#" tabindex="1" size = "50">
				<cfelseif isdefined("rsEmpleadoDef")>
					<cf_rhempleado query="#rsEmpleadoDef#" tabindex="1" size = "50">
				<cfelse>
			...
			<td>
		  		<!---
				<cfif modo NEQ "ALTA">
					<cf_rhCIncidentes onBlur="changeValLabel()" query="#rsIncidencia#" tabindex="1" ExcluirTipo="" form="form1" > <!--- jc ExcluirTipo="3" --->
		  		<cfelse>
					<cf_rhCIncidentes onBlur="changeValLabel()" tabindex="1" ExcluirTipo="" form="form1" > <!--- jc ExcluirTipo="3" --->
		  		</cfif>			
				---->
				<cf_conlis title="#LB_ListaDeIncidencias#"
		...
		<tr>
			...
			<!--- Valor --->
			<td id="TDValorLabel" class="fileLabel" nowrap>&nbsp;</td>
		</tr>		
		<!--- Línea No. 4 --->
		<tr>
			<!--- <cfif not centrocosto.Pvalor eq 1 > --->
			...
			<td>
				<cfif modo NEQ "ALTA">
					<cf_sifcalendario form="form1" name="Ifecha" value="#LSDateFormat(rsIncidencia.fecha,'dd/mm/yyyy')#">
				<cfelse>
					<cf_sifcalendario form="form1" name="Ifecha" value=""><!----#LSDateFormat(Now(),'dd/mm/yyyy')#--->
				</cfif>			
			</td>
			<!--- Valor --->
    	...
    	<td>
			...
			<cfinvoke component="rh.Componentes.pListas"
			 ...
			 returnvariable="pListaRet">
				...
				<cfinvokeargument name="desplegar" value="Ifecha, CIdescripcion, NombreEmp, Ivalor, Icpespecial"/>
				<cfinvokeargument name="etiquetas" value="#vFecha#, #vConcepto#, #vEmpleado#, #vCantidadMonto#, #vIcpespecial#"/>
				<cfinvokeargument name="formatos" value="D,V,V,V,V"/>
				<cfinvokeargument name="align" value="center, left, left, right, center"/>
...
>>>>>>> .r14541
			...
			<cfinvoke 
			 ...
			 method="pListaRH"
			 returnvariable="pListaRet">
				<cfinvokeargument name="tabla" value="Incidencias a, CIncidentes b, DatosEmpleado c"/>
				<cfinvokeargument name="columnas" value="'#Form.Usuario#' as Usuario, a.Iid, b.CIdescripcion, a.Ifecha, 
															 case b.CItipo when 0 then convert(varchar, a.Ivalor, 1) + ' hora(s)' when 1 then convert(varchar, a.Ivalor, 1) + ' día(s)' else convert(varchar, a.Ivalor, 1) end as Ivalor,
															 c.DEnombre + ' ' + c.DEapellido1 + ' ' + c.DEapellido2 as NombreEmp"/>
				<cfinvokeargument name="desplegar" value="Fecha, CIdescripcion, NombreEmp, Ivalor"/>
				<cfinvokeargument name="etiquetas" value="Fecha, Concepto de Incidencia, Empleado, Cantidad/Monto"/>
				<cfinvokeargument name="formatos" value=""/>
				<cfinvokeargument name="filtro" value="#filtro# order by a.Ifecha desc"/>
				<cfinvokeargument name="align" value="center, left, left, right"/>
				<cfinvokeargument name="ajustar" value="N"/>
