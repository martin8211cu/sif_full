...
<cfif isdefined("Application.dsinfo") and Application.dsinfo[session.dsn].type is 'oracle'>
	<cfquery datasource="#session.DSN#">
		...
		select 	e.NTIcodigo, 
				...
				case when len(coalesce(e.DESeguroSocial,e.DEdato3,e.DEidentificacion))>0 then
						rpad(right(coalesce(e.DESeguroSocial,e.DEdato3,e.DEidentificacion),25),25,' ')
					 else
					 	rpad(right('0000000000',25),25,' ')
					 end,				
				'0000000',
				'M',       
				0,
		...
		group by  e.NTIcodigo, h.DEid, e.DEidentificacion, e.DEnombre, e.DEapellido1,e.DEapellido2, case when len(coalesce(e.DESeguroSocial,e.DEdato3,e.DEidentificacion))>0 then
						rpad(right(coalesce(e.DESeguroSocial,e.DEdato3,e.DEidentificacion),25),25,' ')
					 else
					 	rpad(right('0000000000',25),25,' ')
					 end
...
<cfelse><!----============================= SYBASE =============================---->
	...
	<cfquery datasource="#session.DSN#">
		...
		select 	e.NTIcodigo, 
				...
				<cf_dbfunction name="string_part"   args="e.DEapellido2,1,15">+replicate(' ',15-len(e.DEapellido2)),
				case when len(coalesce(e.DESeguroSocial,e.DEdato3,e.DEidentificacion))>0 then
						coalesce(e.DESeguroSocial,e.DEdato3,e.DEidentificacion)
					 else
					 	'0000000000'
					 end,
				'0000000',
...
<cfif isdefined("Application.dsinfo") and Application.dsinfo[session.dsn].type is 'oracle'>
	...
	<cfquery datasource="#session.DSN#">
		...
		select 	4, 			
				...
				||jornada
				||condicion	
				||lpad(<cf_dbfunction name="string_part"   args="ocupacion,1,5">,5,'0')
				as salida				
		from #reporte#
		where salario > 0
			and (	
