	...
	<cfelseif isdefined("form.btnAgregarDCF")>
		...
		<cfquery name="chkExists1" datasource="#session.DSN#">
			...
			select
				...
				RHDporcentajecf
                ,LTid,RHCPlinea, TLinea
			from
			(
				select 	<cfqueryparam cfsqltype="cf_sql_numeric" value="#form.RHAid#"> RHAid,
					de.NTIcodigo,
					de.DEidentificacion,
					<cfif isdefined("form.RHAtipo") and form.RHAtipo eq 'M' >#replace(form.RHDvalorcf, ',', '', 'all')#<cfelse>0</cfif> RHDvalorcf,
					de.DEid,
					#session.Usucodigo# Usucodigo,
					<cfif isdefined("form.RHAtipo") and form.RHAtipo eq 'P' >#replace(form.RHDporcentajecf, ',', '', 'all')#<cfelse>0</cfif> RHDporcentajecf
