...
<cfif isdefined('comportamiento') and comportamiento EQ 12>	<!---Si es una accion de tipo recargo lo desvia al funte que maneja las Acciones de Tipo de Recargo por Plaza--->
	<cfinclude template="AccionesRecargos-sql.cfm">	
<cfelse>													<!---Cualquier otro tipo de Accion--->
...
<cfif not isdefined("Form.Nuevo") or not isdefined("Form.btnregresar") >
	...
	<cfelseif isdefined("Form.Cambio") or isdefined("Form.btnAplicar")>
		<cfquery name="rsValidar" datasource="#session.dsn#">
			select RHTcomportam from RHAcciones a
				inner join RHTipoAccion t
				on a.RHTid=t.RHTid
				and a.Ecodigo=t.Ecodigo
			and a.Ecodigo=#session.Ecodigo#
			and a.RHAlinea=#form.RHAlinea#
		</cfquery>
		<cfif rsValidar.RHTcomportam eq 1 or rsValidar.RHTcomportam eq 12>
			<!---****Averiguar si no se sobrepasa el porcentaje asignado a la plaza****--->
			<!---Porcentaje asignado a la plaza--->
			<cfquery name="rsPorc" datasource="#session.dsn#">
				select RHPporcentaje from RHPlazas where RHPid=#form.RHPid#
			</cfquery>
			<cfset LvarPorc=#form.LTporcPlaza#><!---porcentaje asignado en la accion de personal--->
			<!---Otros porcentajes asignados--->
			<cfif len(trim(rsPorc.RHPporcentaje)) gt 0 and rsPorc.RHPporcentaje gt 0>
				<cfquery name="rsLT" datasource="#session.dsn#">
						select sum(LTporcplaza) as total from LineaTiempo
						where RHPid=#form.RHPid#
						and <cfqueryparam cfsqltype="cf_sql_date" value="#form.DLfvigencia#"> between LTdesde and LThasta
				</cfquery>
			
			<cfif len(trim(rsLT.total)) gt 0 and rsLT.total gt 0>
				<cfset LvarTotal=rsLT.total>
			<cfelse>
				<cfset LvarTotal=0>
			</cfif>
			<cfset Porc=LvarPorc+LvarTotal>
			<cfif Porc gt rsPorc.RHPporcentaje>
				<cfthrow message="Ha sobrepasado en porcentaje máximo asignado a la plaza">
				<cfabort>
			</cfif>
			</cfif>
		</cfif>
		...
		<!--- ********************************************************************************** --->
			...
			<cftransaction>
				...
				<cfif isdefined('Form.RHTTid1') and Len(Trim(Form.RHTTid1)) NEQ 0 and isdefined('Form.RHCid1') and Len(Trim(Form.RHCid1)) NEQ 0 and isdefined('Form.RHMPPid1') and Len(Trim(Form.RHMPPid1)) NEQ 0>
					<cfquery name="rsCatPaso" datasource="#Session.DSN#">
						select RHCPlinea
						from RHCategoriasPuesto
						where RHTTid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHTTid1#">
						and RHCid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHCid1#">
						and RHMPPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHMPPid1#">
						<cfif rsConsTipoAccion.RHTcomportam EQ 9 and rsConsTipoAccion.RHTcempresa EQ 1 and isdefined("Form.EcodigoRef") and Len(Trim(Form.EcodigoRef))>
				...
				<!--- Categoria de Puesto Propuesta
					...
					<cfif rsProp.RHCatParcial eq 1>
						<cfif isdefined('Form.RHTTid2') and Len(Trim(Form.RHTTid2)) NEQ 0 and isdefined('Form.RHCid2') and Len(Trim(Form.RHCid2)) NEQ 0 and isdefined('Form.RHMPPid2') and Len(Trim(Form.RHMPPid2)) NEQ 0>
							<cfquery name="rsCatPaso" datasource="#Session.DSN#">
								select RHCPlinea
								from RHCategoriasPuesto
								where RHTTid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHTTid2#">
								and RHCid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHCid2#">
								and RHMPPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHMPPid2#">
								<cfif rsConsTipoAccion.RHTcomportam EQ 9 and rsConsTipoAccion.RHTcempresa EQ 1 and isdefined("Form.EcodigoRef") and Len(Trim(Form.EcodigoRef))>
								and Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Form.EcodigoRef#">
								<cfelse>
								and Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">
				...
				<cfif isdefined("Form.cantComp") and Form.cantComp GT 0>
					<cfloop from="1" to="#Form.cantComp#" index="i">
						...
						<cfquery name="rsAccion" datasource="#Session.DSN#">
							select b.DLfvigencia, b.DLffin, coalesce(b.RHCPlinea, 0) as RHCPlinea, a.CSid,a.RHDAmetodoC
						...
						<cfinvoke 
						 ...
						 returnvariable="calculaComponenteRet">
							...
							<cfinvokeargument name="Metodo" value="#rsAccion.RHDAmetodoC#"/>
							<cfinvokeargument name="TablaComponentes" value="RHDAcciones"/>
							<cfinvokeargument name="CampoLlaveTC" value="RHAlinea"/>
							<cfinvokeargument name="ValorLlaveTC" value="#Form.RHAlinea#"/>
							<cfinvokeargument name="CampoMontoTC" value="RHDAmontores"/>
				...
				<cfquery name="rsConceptos" datasource="#Session.DSN#">
					select a.DLfvigencia, 
						   ...
						   c.CIid, 
						   c.CIcantidad, c.CIrango, c.CItipo, c.CIcalculo, c.CIdia, c.CImes,CIsprango, coalesce(CIspcantidad,0) as CIspcantidad
				...
				<cfif isdefined('rsValidar') and (rsValidar.RHTcomportam eq 1 or rsValidar.RHTcomportam eq 6 or rsValidar.RHTcomportam eq 8 or rsValidar.RHTcomportam eq 11 or rsValidar.RHTcomportam eq 12)>
				<!---<cfif isdefined("Form.btnAplicar")>--->
