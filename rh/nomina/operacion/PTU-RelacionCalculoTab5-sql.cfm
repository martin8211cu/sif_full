...
<cfif isdefined("Form.btnAplicar")>
	...
	<cftransaction action="begin">
        ...
        <cfquery name="rsEncPTU" datasource="#session.dsn#">
            select 
                FechaDesde,
                FechaHasta 
            FROM 
                RHPTUE
            where 
                RHPTUEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.RHPTUEid#">
		...
		<cfquery datasource="#Session.DSN#">
			...
			values (
                ...
                <cfqueryparam cfsqltype="cf_sql_date" value="#LSParseDateTime(rsEncPTU.FechaDesde)#">,
                <cfqueryparam cfsqltype="cf_sql_date" value="#LSParseDateTime(rsEncPTU.FechaHasta)#">,
                0,
		...
		<cfquery datasource="#session.DSN#">
            ...
            outer apply 
                (select top 1
                        lt.LTdesde,lt.LThasta,lt.Tcodigo,lt.Ecodigo,lt.DEid 
                    from LineaTiempo lt
                    where lt.DEid = a.DEid
                    order by lt.LThasta desc
                ) b
            inner join RCalculoNomina c
                on c.RHPTUEid = a.RHPTUEid
            inner join CalendarioPagos d
            ...
            where 
                a.RHPTUEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.RHPTUEid#">
...
<!--- --->
        <cfquery datasource="#Session.DSN#" name="validaCIncidentes">
            select CIid, CImontolimite, CIidexceso from CIncidentes where  CImontolimite is not null 
            and CImontolimite > 0
            and CIidexceso is not null
            and CIid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.CIid#">
        </cfquery>
        <cfset haySobrante = false>
        <cfif validaCIncidentes.RecordCount gt 0>
            <cfset haySobrante = true>        
        </cfif>
		<cfquery datasource="#Session.DSN#">
			insert into IncidenciasCalculo(
                ...
                select
                    ...
                    <cfif haySobrante>
                        CASE
                        WHEN a.rhptuemtotalptu > ci.CImontolimite
                        THEN
                            Round(ci.CImontolimite, 2)
                        ELSE
                            Round(a.RHPTUEMTotalPTU, 2)
                        END as monto1,
                    <cfelse>
                        ROUND(a.RHPTUEMTotalPTU, 2),                        
                    </cfif>
                ...
                on ci.CIid = #form.CIid#
                and a.Ecodigo = ci.Ecodigo
                where a.RHPTUEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.RHPTUEid#">
                and a.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsEmpresa.Ecodigo#">
                <!---and a.FechaHasta >= b.LTdesde
                and a.FechaHasta <= b.LThasta--->
            ...
            <cfquery datasource="#Session.DSN#">
                insert into IncidenciasCalculo(
                    ...
                    select
                        #form.RCNid#,
                        a.DEid,
                        #validaCIncidentes.CIidexceso#,
                        c.RChasta,
                        <cfif haySobrante>
                            CASE
                            WHEN a.rhptuemtotalptu > ci.CImontolimite
                            THEN
                                Round(a.rhptuemtotalptu - ci.CImontolimite, 2)
                            END as monto1,
                        <cfelse>
                            ROUND(a.RHPTUEMTotalPTU, 2),                        
                        </cfif>
                    ...
                    and c.RCestado = 0
                    and d.CPtipo = 4  <!--- Calendarios tipo PTU --->
                    AND a.rhptuemtotalptu > ci.CImontolimite
                    and not exists ( select 1
                        from IncidenciasCalculo i
                        where i.DEid=a.DEid
                        and i.CIid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#validaCIncidentes.CIidexceso#">
                        and i.ICfecha = c.RChasta)
            </cfquery>            
        </cfif>
