...
<cfelse>										<!---AUTOGESTION--->			
	...
	<cfif reqAprobacion and reqAprobacionJefe>
		<cfif rol EQ 1> 							<!---JEFE--->
...
<!---Si no viene definida la fecha de inicio se pone por defecto la fecha actual--->
<cfif not isdefined("url.filtro_fechaI") or not len(trim(url.filtro_fechaI))>
	<cfset url.filtro_fechaI = LSDateformat(now(),'dd/mm/yyyy')>
</cfif>
...
<cfquery name="rsLista" datasource="#Session.DSN#">
	select 
		...
		,<cfqueryparam cfsqltype="cf_sql_date" value="#LSParseDateTime(url.filtro_fechaI)#"> as filtro_fechaI
	  	</cfif>
		<cfif isdefined("url.filtro_fechaF") and len(trim(url.filtro_fechaF))>
		,<cfqueryparam cfsqltype="cf_sql_date" value="#LSParseDateTime(url.filtro_fechaF)#"> as filtro_fechaF
	  	</cfif>
	from (
	select 	a.Iid, 
			...
			<cfif aprobarIncidencias and aprobarIncidenciasJefe>			
				case when Iestado = 2  then  
					 ...
					 when Iestado = 0 and Iestadoaprobacion in (0,1,2) then 
					...
					when Iestado = 0 and Iestadoaprobacion in (0,1,2) then 
						'Aprobado' 
					else ''  end as estadoRep,																														<!--- aun no requiere mostrar el estado por parte del administrador, debido a rechazo del jefe o que aun esta en proceso de aprobacion  por jefe --->			
	...
	where a.Ivalor != 0
	  ...
	  <!--- --->
	  and coalesce(a.Iestadoaprobacion,2) in (#I_estadoAprobacion#)
	  and a.Iingresadopor in (#I_ingresadopor#)
	  and a.Iestado in (#I_estado#)
	  
	  <!---Si esta en Autogestion (y es jefe) filtra por sus subAlternos--->
	  <cfif Menu EQ 'AUTO' and len(trim(SubalternosList))>
			and a.DEid in (<cfqueryparam cfsqltype="cf_sql_numeric" value="#SubalternosList#" list="yes">)
	  </cfif>
	  
	  <!---incidencias que no hayan sido rechazadas por super admin--->
	  <!---and (select  case when a.Iestado = 1 and a.NAP is null then 0 else 1 end) = 1--->
	  ...
	  select 	a.Iid, 
			...
			<cfif aprobarIncidencias and aprobarIncidenciasJefe>			
				case 
					 ...
					 <cf_dbfunction name="concat" args="'<img src=/cfmx/rh/imagenes/Borrar01_12x12.gif   onclick=funcShowEstado(18,' | #vIid# |') style=cursor:pointer />'" delimiters="|"><!---rechazado Adm Inc--->
					 when Iestado = 1 then 
					 <cf_dbfunction name="concat" args="'<img src=/cfmx/rh/imagenes/Borrar01_12x12.gif   onclick=funcShowEstado(10,' | #vIid# |') style=cursor:pointer />'" delimiters="|"><!---rechazado Adm  Inc--->
					 when Iestado = 0 and Iestadoaprobacion = 2 then 
					 <cf_dbfunction name="concat" args="'<img src=/cfmx/rh/imagenes/Borrar01_12x12.gif   onclick=funcShowEstado(12,' | #vIid# |') style=cursor:pointer />'" delimiters="|"><!---rechazado Adm  Inc--->
					 when Iestado = 2 then 
					<cf_dbfunction name="concat" args="'<img src=/cfmx/rh/imagenes/Borrar01_12x12.gif   onclick=funcShowEstado(6,' | #vIid# |') style=cursor:pointer />'" delimiters="|"><!---rechazado Adm--->
					 else ''  end as estado,
				...
				case 
					 ...
					 when Iestado = 2 then 
					 'Rechazada'
					 else ''  end as estadoRep,																														<!--- aun no requiere mostrar el estado por parte del administrador, debido a rechazo del jefe o que aun esta en proceso de aprobacion  por jefe --->																																	<!--- aun no requiere mostrar el estado por parte del administrador, debido a rechazo del jefe o que aun esta en proceso de aprobacion  por jefe --->			
			<cfelse>
				case when Iestado = 2 then 
					<cf_dbfunction name="concat" args="'<img src=/cfmx/rh/imagenes/Borrar01_12x12.gif   onclick=funcShowEstado(6,' | #vIid# |') style=cursor:pointer />'" delimiters="|"><!---rechazado Adm--->
					 when Iestado = 1 and NAP is not null then 
	...
	from  HIncidencias a 
		inner join CIncidentes b
			on a.CIid = b.CIid
			and b.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.ecodigo#">
		inner join DatosEmpleado c			<!--- no valida incidencia de empleados activos debido a que puede darse el caso de incidencias para empleados inactivos--->
			on a.DEid = c.DEid
		left outer join CFuncional cf
			on a.CFid = cf.CFid
	where a.Ivalor != 0
...
<!---Para el pintado del reporte--->
<cfset session.reporteTemp = rsLista>
...
<script language="javascript" type="text/javascript">
	...
	function ShowMensaje(Iid,cod){
        ...
        'http://<cfoutput>#session.sitio.host#</cfoutput>/cfmx/rh/nomina/operacion/EstadoIncidenciasProcesoMensaje.cfm?Iid='+Iid + '&cod='+cod,
...
<form name="filtro" method="get" action="EstadoIncidenciasProceso.cfm">
	<cfoutput>
	...
	<table width="100%" cellpadding="2" cellspacing="0" class="areaFiltro" border="0">
		<tr>
			...
			<td>
				<table cellpadding="2" cellspacing="2" border="0"><tr><td>#vFechaI#:</td>
				<td><cf_sifcalendario form="filtro" name="filtro_fechaI" value="#valor_fechaI#">
				</td></tr></table>
			</td>
		...
		<tr>
			...
			<td>
				...
				<table cellpadding="2" cellspacing="2" border="0"><tr><td>#vFechaF#:</td>
...
<form name="form1" method="get" action="">
	...
	<table width="100%" cellpadding="1" cellspacing="0">
		<tr><td align="right"> 
		<!---Para el pintado del reporte--->
		<img src="../../imagenes/impresora2.gif" onclick="javascript: document.location.href='EstadoIncidenciasProceso-Reporte.cfm'"/>
		</td></tr>
		<tr>
			<td>
				<cfset maxrows = 25 >
				<cfif aprobarIncidenciasJefe and  aprobarIncidencias >
					<cfinvoke component="rh.Componentes.pListas"
					 method="pListaQuery"
					 returnvariable="pListaRet">
						<cfinvokeargument name="query" value="#rsLista#"/>
						<cfif usaPresupuesto >
							<cfinvokeargument name="desplegar" value="NombreEmp,CIdescripcion,Ifecha, CItipo,Ivalor,espacio,Imonto,estadoJefe,estado,estadoPres,pagada"/>
							<cfinvokeargument name="etiquetas" value="#vEmpleado#, #vConcepto#,#vFecha#, #vTipo#, #vValor#,&nbsp;,#vValor#,#vestadoJefe#,#vestadoAdmin#,#vPres#,#LB_Pagada#"/>
							<cfinvokeargument name="formatos" value="V,V,D,V,M,V,M,V,V,V,S"/>
							<cfinvokeargument name="align" value="left,left,center,left,right,left,right,center,center,center,center"/>
						<cfelse>
							<cfinvokeargument name="desplegar" value="NombreEmp,CIdescripcion,Ifecha, CItipo,Ivalor,espacio,Imonto,estadoJefe, estado,pagada"/>
							<cfinvokeargument name="etiquetas" value="#vEmpleado#, #vConcepto#,#vFecha#, #vTipo#, #vValor#,&nbsp;,#vMonto#,#vestadoJefe#,#vestadoAdmin#,#LB_Pagada#"/>
							<cfinvokeargument name="formatos" value="V,V,D,V,M,V,M,V,S,S"/>
							<cfinvokeargument name="align" value="left,left,center,left, right,left, right,center,center,center"/>
						</cfif>
				...
				<cfelse>
					<cfinvoke component="rh.Componentes.pListas"
					 ...
					 returnvariable="pListaRet">
						<cfinvokeargument name="query" value="#rsLista#"/>
						<cfif usaPresupuesto >
							<cfinvokeargument name="desplegar" value="NombreEmp,CIdescripcion,Ifecha, CItipo,Ivalor,espacio,Imonto, estado,estadoPres,mes,pagada"/>
							<cfinvokeargument name="etiquetas" value="#vEmpleado#, #vConcepto#,#vFecha#, #vTipo#,  #vValor#,&nbsp;,#vMonto#,#vestadoAdmin#,#vPres#,#LB_Mes#,#LB_Pagada#"/>
							<cfinvokeargument name="formatos" value="V,V,D,V,M,V,M,V,V,S,S"/>
							<cfinvokeargument name="align" value="left,left,center,left, right,left,right,center,center,center,center"/>
						<cfelse>
							<cfinvokeargument name="desplegar" value="NombreEmp,CIdescripcion,Ifecha, CItipo,Ivalor,espacio,Imonto,estado,pagada"/>
							<cfinvokeargument name="etiquetas" value="#vEmpleado#, #vConcepto#,#vFecha#, #vTipo#, #vValor#, &nbsp;,#vMonto#,#vestadoAdmin#,#LB_Pagada#"/>
							<cfinvokeargument name="formatos" value="V,V,D,V,M,V,M,V,S"/>
							<cfinvokeargument name="align" value="left,left,center,left, right,left, right,center,center"/>
						</cfif>
						<cfinvokeargument name="ajustar" value="N"/>
						<cfinvokeargument name="showlink" value="False"/>
						<cfinvokeargument name="showEmptyListMsg" value="true"/>
