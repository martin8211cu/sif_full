...
<cfif isdefined("form.Tcodigo") and not isdefined("Form.RCTcodigo")>
	<cfset form.RCTcodigo = #form.Tcodigo#>
</cfif>
<cfset filtro = "">
<cfset navegacion = "">
  ...
  <tr>
    <td width="90%">
        ...
        <!---<cfquery datasource = "#session.DSN#" name="rsEmpresa">
			select Ecodigo from CalendarioPagos
        ...
        <cfquery name="rsVerifica" datasource="#session.DSN#">
			SELECT 
				...
				a.CPfenvio,
				c.CPid AS CPidPTU,
				c.CPdesde AS CPdesdePTU,
				c.CPhasta AS CPhastaPTU,
				c.CPtipo AS CPtipoPTU,
				c.CPfenvio AS CPfenvioPTU,
				c.CPhasta , a.CPdesde
			FROM CalendarioPagos a,
				CalendarioPagos c,
				(select RHPTUEid,FechaDesde,FechaHasta from RHPTUE)d
			WHERE a.CPid NOT IN
				(SELECT RCNid
			...
			AND c.CPhasta <= d.FechaHasta
			and a.CPdesde >= d.FechaDesde
			AND a.Ecodigo IN (#ValueList(rsEmpresa.Ecodigo)#)
			AND a.CPid <> <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.RCNid#">
			AND c.CPtipo = 4
			and d.RHPTUEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.RHPTUEid#">
			ORDER BY 1
        </cfquery>
