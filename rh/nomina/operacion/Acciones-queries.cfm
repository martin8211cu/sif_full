<cfif modo NEQ "ALTA" AND isdefined("Session.Ecodigo") AND isdefined("Form.RHAlinea") AND Len(Trim(Form.RHAlinea)) GT 0>
	...
	<cfif rsTipoAccionComp.RHTcomportam EQ 9 and rsTipoAccionComp.RHTcempresa EQ 1>
		<cfquery name="rsAccion" datasource="#Session.DSN#">
			select 	a.RHAlinea,
				   ...
				   coalesce(a.RHfolio,'') as RHfolio,
				   coalesce(a.RHporcimss,0) as RHporcimss,
                   RHAccionRecargo
	...
	<cfif rsAccion.RHAccionRecargo GT 0>
        <cfquery name="rsEstadoActual" datasource="#Session.DSN#">
            ...
            and a.LTRid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsAccion.RHAccionRecargo#">
        </cfquery>
   	<cfelse>
        <cfquery name="rsEstadoActual" datasource="#Session.DSN#">
	...
	<cfif rsEstadoActual.recordCount GT 0>
        ...
        <cfif rsVerTAccion.RHCatParcial eq 1 and rsAccion.RHCPlineaP GT 0>
            ...
            <cfquery name="rsMonto" datasource="#session.dsn#">
            	select a.RHTTid,b.RHVTid,b.RHMCmonto as SB
		...
		<cfquery name="rsComponentesAccion" datasource="#Session.DSN#">
			select a.RHDAlinea,
				   ...
				   b.CSusatabla,
				   b.CSsalariobase,
				   coalesce(b.CSexcluyeCB,0) CSexcluyeCB,
				   a.RHDAtabla,
				   a.RHDAunidad,
				   a.RHDAmontobase,
				   a.RHDAmontores,
			...
			<cfif rsTipoAccionComp.RHTcomportam EQ 9 and rsTipoAccionComp.RHTcempresa EQ 1>
                    <cfquery name="rsComponentesActual" datasource="#Session.DSN#">
                        select c.CSid,
                               ...
                               c.CSusatabla,
                               c.CSsalariobase,
							coalesce(c.CSexcluyeCB,0) CSexcluyeCB,
                               b.DLTtabla,
                               coalesce(b.DLTunidades, 0.00) as DLTunidades,
                               case
                                    when b.DLTmetodoC = '' then round(coalesce(b.DLTmonto, 0.00),2)
                                    when b.DLTmetodoC is not null and b.DLTmetodoC = 'P' then round(coalesce(b.DLTmonto, 0.00) / coalesce(b.DLTunidades, 1.00), 2) * 100
                                    else round(coalesce(b.DLTmonto, 0.00), 2)
                               end as DLTmontobase,
                    ...
                    <cfquery name="rsComponentesActual" datasource="#Session.DSN#">
                        select c.CSid,
							...
							coalesce(c.CSexcluyeCB,0) CSexcluyeCB,
                               ...
                               case
                                    when b.DLTmetodoC = '' then round(coalesce(b.DLTmonto, 0.00),2)
                                    when b.DLTmetodoC is not null and b.DLTmetodoC = 'P' then round(coalesce(b.DLTmonto, 0.00) / coalesce(b.DLTunidades, 1.00), 3) * 100
                                    else round(coalesce(b.DLTmonto, 0.00), 2)
                               end as DLTmontobase,
                               coalesce(b.DLTmonto, 0.00) as DLTmonto,
                               coalesce(c.CIid, -1) as CIid,
                               coalesce(d.RHMCcomportamiento, 1) as RHMCcomportamiento,
                               coalesce(d.RHMCvalor, 1.00) as valor,DLTmetodoC
                        from RHDAcciones a
