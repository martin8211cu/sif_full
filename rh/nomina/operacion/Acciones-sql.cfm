...
<script language="javascript" type="text/javascript">
	function ValidarNomina {
		<cfoutput>
			alert('No se puede aplicar acciones de personal en nominas cerradas')
		</cfoutput>
	}
</script>
...
<cfif (isdefined('comportamiento') and comportamiento EQ 12) or (isdefined('form.indicaRecargo') and form.indicaRecargo GT 0)>	<!---Si es una accion de tipo recargo lo desvia al funte que maneja las Acciones de Tipo de Recargo por Plaza--->
	<cfinclude template="AccionesRecargos-sql.cfm">
<cfelse>													<!---Cualquier otro tipo de Accion--->
	<cfif isdefined("Form.btnAgregar")>
    	<!---SML. Incio Validacion para que no se agregue acciones de personal despues de una nomina cerrada--->
    	...
    	<cfif isdefined('rsTipoAccion') and rsTipoAccion.RHTcomportam EQ 3 or rsTipoAccion.RHTcomportam EQ 5 or rsTipoAccion.RHTcomportam EQ 13 or rsTipoAccion.RHTcomportam EQ 6>
        <cfquery name="rsValidaNomina" datasource="#session.DSN#">
        	select 1 validaFecha
			from CalendarioPagos
			where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">
				and <cfqueryparam cfsqltype="cf_sql_timestamp" value="#LSParseDateTime(form.DLfvigencia)#"> between CPdesde and CPhasta
				and CPfcalculo is not null
				and Tcodigo = (select top 1 (Tcodigo)
			   				   from LineaTiempo
        ...
        <cfif isdefined('rsValidaNomina') and rsValidaNomina.validaFecha EQ 1>
        	<cf_throw message = "No se puede aplicar acciones de personal en nominas cerradas">
        </cfif>
        </cfif>
        <!---SML. Fin Validacion para que no se agregue acciones de personal despues de una nomina cerrada--->
	...
	<!--- 2018-08-02 Complementamos la consulta del Componente Salarial Relacionadoa RHMontosCategoria --->
	<cfset varCSid = 0>
	<cfif IsDefined('form.RHCid3') and Trim(form.RHCid3) neq ''>
		<cfquery name="rsCS" datasource="#session.dsn#">
			select CSid from RHMontosCategoria
			where RHCid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.RHCid3#">
		</cfquery>
	...
	<cfelseif isdefined("Form.Cambio") or isdefined("Form.btnAplicar")>
		...
		<!--- ********************************************************************************** --->
			...
			<cftransaction>
				...
				<cfquery datasource="#session.dsn#" name="rsExisteCS">
					select CSid from RHDAcciones
					where RHAlinea = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHAlinea#">
				</cfquery>

				<cfif varCSid neq 0 and rsExisteCS.CSid neq varCSid>
                        ...
                        <cfelse>
							...
							<!---Datos de la Acción--->
								...
								<cfinvoke component="rh.Componentes.RH_SinEstructuraSalarial" method="calculaComponente" returnvariable="calculaComponenteRet">
									...
									<cfinvokeargument name="ValorLlaveTC" value="#Form.RHAlinea#"/>
									<cfinvokeargument name="CampoMontoTC" value="RHDAmontores"/>
									<!---<cfinvokeargument name="RHTTid" value="#Lvar_RHTTid#">--->  <!---Sin tabla salarial--->
									<!---<cfinvokeargument name="RHCid" value="#Lvar_RHMPPid#">--->  <!---Sin categoria por ahora--->
									<!---<cfinvokeargument name="RHMPPid" value="#Lvar_RHCid#">--->  <!---Sin Puesto de Maestro Puesto--->
									<cfinvokeargument name="PorcSalario" value="#Form.LTporcsal#"/>
									<cfinvokeargument name="RHCPlineaP" value="#rsAccionES.RHCPlineaP#"/>
				...
				<!---ljimenez - lz se actualiza la fecha fin para las acciones con un comportamiento
				diferente de nombramiento ya que cuando se hace el llamodo a la calculadora
				y el empleado tiene fecha hasta definida menor al 6100 el calculo de vacaciones
				periodo se pierde por ejemplo 2010-12-02--->
