<cfif isDefined("Form.btnNuevo")>
	...
	<cfset Action = "RelacionCalculo.cfm">
<cfelse>
...
<cfif isDefined("Form.chk") and isDefined("Form.btnEliminar")>
	...
	<cfloop from="1" index="i" to="#ArrayLen(vchk)#">
		...
		<cfquery datasource="#Session.DSN#">
			delete Incidencias where RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#vchk[i]#">
		</cfquery>
		<cfquery datasource="#Session.DSN#">
			delete BMovimientoIncidencias where RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#vchk[i]#">
		</cfquery>
		<!--- --->
		<cfquery name="delRHfondoahorro" datasource="#session.dsn#">
        	delete RHfondoahorro where RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#vchk[i]#">
        </cfquery>
        ...
        <cfquery name="ABC_Resultado" datasource="#Session.DSN#">	
			delete from RHSubsidio where RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#vchk[i]#">
		</cfquery>
		<cfquery name="ABC_Resultado" datasource="#Session.DSN#">	
