	<cfif not isdefined("Form.Nuevo")>
		 ...
		 <cfif isdefined("Form.Alta")>
			<cfquery name="RHLiqIngresos" datasource="#session.DSN#">
				insert into RHLiqIngresos (DLlinea, DEid, CIid, Ecodigo, RHLPdescripcion, importe, BMUsucodigo, fechaalta)
				select 
					DLlinea,
					DEid,
		...
		<cfelseif isdefined("Form.Guardar")>
			<cftransaction >
				...
				<cfquery datasource="#session.DSN#">
					insert into RHLiqIngresos (DLlinea, DEid, CIid, Ecodigo, RHLPdescripcion, importe, BMUsucodigo, fechaalta, cesantia, motivoliq)
					select 
						DLlinea,
						DEid,
						<cfqueryparam cfsqltype="cf_sql_numeric" value="#form.CIid#">,
		...
		<cfelseif isdefined("form.btnRecalcular")>
			...
			<cfloop query="rs_recalcular">
					...
					<!--- 2011-0606 Se le Resta un día a la FVigencia, pues al Recalcular los Calculos Especiales, usando la Fecha de Rige del Cese, el Empleado no tiene Linea de Tiempo--->
					<cfset presets_text = RH_Calculadora.get_presets(CreateDate(ListGetAt(FVigencia,3,'/'), ListGetAt(FVigencia,2,'/'), ListGetAt(FVigencia,1,'/')-1),
					...
					<cfset vdiferencia = round(values.get('resultado').toString()) - round(vmonto_origen)>
