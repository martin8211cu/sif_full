...
<script language="JavaScript1.2" type="text/javascript">
	...
	<table align="right" width="98%" cellpadding="0" cellspacing="0" border="0">
		<tr>
		  <td align="center" class="tituloAlterno"><cf_translate  key="LB_SeleccioneUnEmpleado">Seleccione un Empleado</cf_translate>
</td>
	  </tr>
		<tr><td>&nbsp;</td></tr>
		<tr>
					...
					select 1 as paso, a.DLlinea, a.DEid, RHLPestado, fechaalta, 
							...
							coalesce(
								(
									select 
										case	
											when count(*) > 0 and stsTimbre <> 1 then 'Problemas al timbrar'
											when count(*) > 0 and stsTimbre = 1 then 'Timbrado'
											when count(*) = 0 then 'Sin Timbrar'
											else 'Sin Timbrar'
										end
									from RH_CFDI_RecibosNomina 
									where RCNid = -1
									and DLlinea = a.DLlinea
									and DEid = a.DEid
									group by stsTimbre
								),'Sin Timbrar'
							) as Estatus
				...
				<cfif isdefined("form.Fhasta")>
				 ...
				 <cfinvoke 
					...
					returnvariable="pListaRet">
						...
						<cfinvokeargument name="desplegar" value="DEidentificacion, nombre, DLfvigencia, Estatus "/>
						<cfinvokeargument name="etiquetas" value="#vIdentificacion#, #vNombre#, #vFechaLiq#,Estatus"/>
