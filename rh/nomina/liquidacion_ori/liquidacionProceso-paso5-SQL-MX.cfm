<cfif isdefined("Form.Aprobar")>
	...
	<cftransaction>
		...
		<cfset varCcuenta = "">
		<cfloop from="1" to="#ArrayLen(arrCcuenta)-1#" index="count">
			<cfset varCcuenta &= Trim(arrCcuenta[count]) & "-">
		</cfloop>
		...
		<cfquery name="rsLTEmp" datasource="#session.dsn#">	
			select Max(LTid),Ocodigo from LineaTiempo where DEid = 1
			group by Ocodigo 
		</cfquery>
		...
		<cfloop query="rsPerc">
			...
			<cfquery datasource="#session.dsn#">
				...
				values
				(
					'RHPN', 1, '#varDoc#', 'Liquidacion-Finiquito #varDoc#', Round(#RESULTADO#,4),	'D',
		...
		<cfif rsISR.RecordCount gt 0>
			...
			<cfquery datasource="#session.dsn#">
				...
				values
				(
					'RHPN', 1, '#varDoc#', 'Liquidacion-Finiquito #varDoc#', #LSNumberFormat(rsISR.RESULTADO,'9.0000')#,	'C',
		...
		<cfloop query="rsDeducs">
			...
			<cfquery datasource="#session.dsn#">
				...
				values
				(
					'RHPN', 1, '#varDoc#', 'Liquidacion-Finiquito #varDoc#', Round(#RESULTADO#,4),	'C',
		...
		<cfif difSueldos gt 0 >
			...
			<cfquery datasource="#session.dsn#">
				...
				values
				(
					'RHPN', 1, '#varDoc#', 'Liquidacion-Finiquito #varDoc#', #difSueldos#,	'C',
		...
		<cfinvoke component="sif.Componentes.CG_GeneraAsiento" method="GeneraAsiento" returnvariable="LvarIDcontable">
			...
			<cfinvokeargument name="Ereferencia" 		value="Liquidacion-Finiquito #varDoc#"/>
			<cfinvokeargument name="Edescripcion"		value="Liquidacion-Finiquito"/>
		</cfinvoke>
		<!--- FIN GENERAR POLIZAS --->
