...
<cfquery name="rsFactor" datasource="#Session.DSN#">
	select top 1 coalesce(FactorDiasIMSS,FactorDiasSalario) as FactorDiasSalario
...
<cfif isdefined("Form.RHPLPid") and Len(Trim(Form.RHPLPid))>

	<cfquery name="rsEncabezado" datasource="#Session.DSN#">
		select b.DEid,
			   {fn concat({fn concat({fn concat({fn concat(b.DEapellido1, ' ')}, b.DEapellido2)}, ' ')}, b.DEnombre)} as NombreCompleto,
			   b.DEidentificacion,
			   a.RHPLPfingreso as EVfantig,
			   <cf_dbfunction name="dateadd" args="-1,a.RHPLPfsalida"> as DLfvigencia, 
			   d.RHPdescpuesto,
			   f.Ddescripcion,
			  <!--- g.RHTdesc,--->
			   a.Ecodigo,
			   ...
			   i.Msimbolo,
			   i.Mnombre,
			   coalesce(a.RHPLPrenta, 0) as renta,
			   coalesce(a.RHPLPfecha,getdate()) as RHLPfecha,
			   <cf_dbfunction name="to_number" args="h.FactorDiasSalario" dec="4"> as FactorDiasSalario,
			   <cf_dbfunction name="datediff" args="a.RHPLPfingreso, a.RHPLPfsalida"> as diasAnt
		...
		where a.RHPLPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHPLPid#">
	</cfquery>
	
	<cfquery name="rsDetallePrestaciones" datasource="#Session.DSN#">
		select a.DEid, a.RHPLPid, a.CIid, a.RHLPdescripcion as Descripcion, a.importe, 
			   b.RHCAPLres as Resultado, b.RHCAPLcant as Cantidad, b.RHCAiPLmporte as Monto
		from RHLiqIngresosPrev a
			inner join RHConceptosAccionPreLiq b
				on a.RHPLPid = b.RHPLPid and a.CIid = b.CIid
	...
	<cfquery name="rsDetalleOtrasPrestacionesSP" datasource="#Session.DSN#">
		select a.RHLPdescripcion as Descripcion, 
			case when b.RHCAPLcant is null then coalesce(a.importe,0) else coalesce(b.RHCAiPLmporte,0) end as importe,
			coalesce(b.RHCAPLres,0) as Resultado, 
			coalesce(b.RHCAPLcant,0) as Cantidad
		from RHLiqIngresosPrev a
		left outer join RHConceptosAccionPreLiq  b
			on b.CIid = a.CIid
			and b.RHPLPid = a.RHPLPid
		...
		<tr>
			<td>
				<table width="90%" border="0" cellspacing="0" cellpadding="0" align="center">
					...
					<tr><td colspan="2"><table width="80%" border="0" cellpadding="0" cellspacing="0">
						<tr>
						...
						<tr>
                            ...
                            <cfif rsLF.RHLFLisptF gt 0>
                            	<cfset TotalD = TotalD + #rsLF.RHLFLisptF#/100>
                            </cfif>
