...
<cfif not isdefined("Form.Nuevo") and not isdefined("form.baja")>
	<cfif isdefined("Form.RHLPid") AND Len(Trim(Form.RHLPid)) GT 0>
		<cfset modo = "CAMBIO">
	<cfelse>
		<cfset modo = "ALTA">
	</cfif>
...
<cfif modo NEQ "ALTA">
	<cfquery name="rsIncidencia" datasource="#Session.DSN#">
		select a.RHLPid, 
			   ...
			   a.BMUsucodigo,
			   b.*
		from RHLiqIngresos a, CIncidentes b
		where a.RHLPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHLPid#">
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" align="center">
	...
	<cfif ucase(rs_parametro_810.Pvalor) eq 'YES'>
		...
		<cfinvoke component="sif.Componentes.Translate"
			method="Translate"
			key="LB_Despido_Sin_Responsabilidad"
			Default="Despido Sin Responsabilidad"
			returnvariable="vDespSinResp"/>				
		<cfinvoke component="sif.Componentes.Translate"
			method="Translate"
						...
						<tr>
							...
							<td width="1%">
								<select name="motivo" >
									...
									<option value="S" <cfif rs_concepto_cesantia.motivoliq eq 'S'>selected</cfif> >#vDespSinResp#</option>									
								</select>
							</td>
						</tr>
    ...
    <td>
		<form action="#CurrentPage#" method="post" name="form1" onSubmit="javascript: return validaForm(this);">
			...
			<table width="100%" border="0" cellpadding="3" class="areaFiltro" style="background-color:##f5f5f5;" cellspacing="0" align="center">
				<tr><td colspan="4"  bgcolor="##E8E8E8"><strong>Incidencias Adicionales</strong></td></tr>
			  <tr>
  ...
  <tr>
    <td>
		 <cfquery name="rsRHLiqIngresos" datasource="#session.DSN#">
			select	1 as paso, a.DLlinea, a.DEid, b.RHLPid, b.RHLPdescripcion as nombre, b.importe, c.CIcodigo, c.CIdescripcion
