...
<cfinvoke component="sif.Componentes.Translate"
	method="Translate"
	key="LB_vPagosLF"
	Default="Pagos Liquidación y Finiquito"
	returnvariable="vPagosLF"/>
...
<cfscript>
	...
	if (isdefined('url.paso')) { //se hizo click sobre una opción del menú de pasos
		...
		if (Gpaso gt 0) {
			...
			if (not isdefined('url.RHPLPid') or (isdefined('url.RHPLPid') and len(trim(url.RHPLPid)) eq 0)) {
				Gpaso = 0;
			}
			else {
				RHPLPid = url.RHPLPid;
				Gmaxpasoallowed = varGmaxpasoallowed;
			}
		}
	}
	...
	if (isdefined('form.paso')) { //se hizo submit en uno de los forms de las pantallas 0, 1, 2, 3 o 4.
		...
		if (Gpaso gt 0){
			...
			if (Gpaso eq 1 and (isdefined('form.btnNuevo') or isdefined('form.Nuevo'))) {
				...
				if ( isdefined("form.RHPLPid") and len(trim(form.RHPLPid)) ){
					RHPLPid = form.RHPLPid;				
				}
			}
			//en el paso 1(cuando no está definifo el botón nuevo),2 y 3 se requiere el concurso
			else{
				if (not isdefined('form.RHPLPid') or (isdefined('form.RHPLPid') and len(trim(form.RHPLPid)) eq 0)) {
					Gpaso = 0;
				}
				else {
					RHPLPid = form.RHPLPid;
