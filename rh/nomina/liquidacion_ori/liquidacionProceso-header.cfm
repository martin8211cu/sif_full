...
<!---
<cfif rsRHLiqIngresos.recordcount gt 0 >
	<cfquery name="rs_vigencia" datasource="#session.DSN#">
		select DLfvigencia
		from DLaboralesEmpleado
		where DLlinea = <cfqueryparam cfsqltype="cf_sql_numeric" value="#DLlinea#">
		  and DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsRHLiqIngresos.DEid#">
	</cfquery>
	<cfset fecha_cese = rs_vigencia.DLfvigencia >
	<cfquery name="rs_ultima_nomina" datasource="#session.DSN#">
		select max(RChasta) as RChasta
		from HSalarioEmpleado se, HRCalculoNomina cn
		where cn.RCNid=se.RCNid
		and DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsRHLiqIngresos.DEid#">
		and cn.RChasta < <cfqueryparam cfsqltype="cf_sql_date" value="#fecha_cese#">
	</cfquery>
	<cfif len(trim(rs_ultima_nomina.RChasta))>
		<cfset fecha_nomina = rs_ultima_nomina.RChasta >
	</cfif>
	
	<cfset dias_nomina = 0 >
	<cfif isdefined("fecha_nomina")>
		<cfset dias_nomina = abs(datediff('d', fecha_nomina, fecha_cese )) >
	</cfif>

	Fechas de ces: <cfdump var="#fecha_cese#"><br />
	Fecha Ultiam Nomina: <cfdump var="#fecha_nomina#"> <br />
	DIAS NOMINA: <cfdump var="#dias_nomina#">
</cfif>
--->  
