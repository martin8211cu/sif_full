					  ...
					  <td>
						  <cfquery name="rsSumRHLiqIngresos" datasource="#session.DSN#">
					       ...
					       <cfquery name="rsRHLiqIngresos" datasource="#session.DSN#">
							  select <!---1 as paso, a.DLlinea, a.DEid,
                              		b.RHLPdescripcion as nombre, --->
                              		b.RHLPid, b.importe, c.CIcodigo, substring(c.CIdescripcion,1,25) as CIdescripcion
							...
							<cfquery name="rsSumRHLiqCargas" datasource="#session.DSN#">
							 ...
							 <cfquery name="rsRHLiqCargas" datasource="#session.DSN#">
								select <!---3 as paso, a.DLlinea, a.DEid,
									b.DEid, b.DClinea, b.SNcodigo, b.RHLCdescripcion, b.fechaalta,
									b.RHLCautomatico, b.BMUsucodigo, d.ECid, d.SNcodigo, d.DCmetodo, d.DCvaloremp,
									d.DCvalorpat, d.DCprovision, d.DCnorenta, d.DCtipo, d.SNreferencia, d.DCcuentac,
									d.DCtiporango, d.DCrangomin, d.DCrangomax, d.BMUsucodigo,--->
									b.RHLCid, d.DCcodigo, d.DCdescripcion, b.importe
								from RHLiquidacionPersonal a
						...
						<tr>
							<td width="183">
								<cfquery name="rsRHLiqDeduccion" datasource="#session.DSN#">
									...
									b.DLlinea, b.Did, b.RHLDreferencia, b.SNcodigo,
									b.fechaalta, b.RHLDautomatico, b.BMUsucodigo, td.TDdescripcion,--->
                                    b.RHLDid,
                                    case when b.Did is null then sn.SNnumero else td.TDcodigo end as Codigo,
                                    case when b.Did is null then sn.SNnombre else RHLDdescripcion end as nombre,  b.importe
									from RHLiquidacionPersonal a
								...
								<cfquery name="rsSumRHLiqDeduccion" dbtype="query">
									select sum(importe) as totDeduc
									from rsRHLiqDeduccion
								</cfquery>

	<!--- Consulta para obtener el finiquito del registro--->
								<cfquery name="rsRHLiqFiniquito" datasource="#session.DSN#">
									select 
                                    b.RHLDid,
                                    case when b.Did is null then sn.SNnumero else td.TDcodigo end as CodigoF,
                                    case when b.Did is null then sn.SNnombre else RHLDdescripcion end as nombreF,
									0.00 as importeDeducciones
									from RHLiquidacionPersonal a
									inner join RHLiqDeduccion b
											on a.DEid = b.DEid and a.DLlinea = b.DLlinea 
									left outer join DeduccionesEmpleado de 
											on b.DEid = de.DEid and b.Did = de.Did 
									left outer join TDeduccion td 
											on de.Ecodigo = td.Ecodigo and de.TDid = td.TDid 
									inner join SNegocios sn 
											on b.SNcodigo = sn.SNcodigo and a.Ecodigo = sn.Ecodigo 
									where a.DLlinea = <cfqueryparam cfsqltype="cf_sql_numeric" value="#DLlinea#">
	
									union 
									select -1 as RHLDid, 
											'Deducciones' as codigoF, 
											'I.S.R' as nombrFe,
										   	RHLFLtotalL-RHLFLresultado-0.01 as importeDeducciones
									from RHLiqFL 
									where DLlinea = <cfqueryparam cfsqltype="cf_sql_numeric" value="#DLlinea#">
									and RHLFLisptF = 0
								</cfquery>
									...
									<!--- Consulta para obtener el total finiquito del registro--->
								<cfquery name="rsSumRHLiqFiniquito" dbtype="query">
									select sum(importeDeducciones) as totDeduc
									from rsRHLiqFiniquito
								</cfquery>

	<!--- Validación deducciones total 								#LSNumberformat(ISPT,'9.00')#--->

								<cfif LSCurrencyFormat(rsSumRHLiqFiniquito.totDeduc,'none') gt 0>

									<cfset  totLFinic="#rsSumRHLiqFiniquito.totDeduc#">
								<cfelse>																		
									<cfset  totLFinic="#LSCurrencyFormat(rsSumRHLiqFiniquito.totDeduc,'none')#">
								</cfif>


								<cfif LSCurrencyFormat(rsSumRHLiqDeduccion.totDeduc,'none') gt 0>
									<cfset  totLDeduc="#rsSumRHLiqDeduccion.totDeduc#">
								<cfelse>		
									<cfset  totLDeduc="#LSCurrencyFormat(rsSumRHLiqDeduccion.totDeduc,'none')#">
								</cfif>

								<cfset TDecuc = totLDeduc+totLFinic>


	<!--- Registros de la tabla rsRHLiqDeduccion--->
								 <cfinvoke
									component="rh.Componentes.pListas"
									method="pListaQuery"
									returnvariable="pListaRet">
										<cfif rsRHLiqDeduccion.RecordCount GT 0>
											<cfinvokeargument name="query" value="#rsRHLiqDeduccion#"/>
											<cfinvokeargument name="desplegar" value="Codigo, nombre, importe "/>
										</cfif>
										<cfif rsRHLiqFiniquito.RecordCount gt 0>
											<cfinvokeargument name="query" value="#rsRHLiqFiniquito#"/>
											<cfinvokeargument name="desplegar" value="CodigoF, nombreF, importeDeducciones "/>
										</cfif>
										<cfinvokeargument name="etiquetas" value="#vCodigo#, #vDescripcion#, #vImporte#"/>
										<cfinvokeargument name="formatos" value="S, S, M"/>
										<cfinvokeargument name="align" value="left, left, right"/>
			...
			<tr>
				...
				<td align="right">#vSubtotal#:   <strong>#LSCurrencyFormat(TDecuc,'none')#</strong></td>
			</tr>
			<tr align="right" nowrap>
				<td>
					<cfset totLiq = rsRHLiqIngresosAutom.totIngresos + rsSumRHLiqIngresos.totIngresos - rsSumRHLiqCargas.totCargas - (Trim(rsSumRHLiqDeduccion.totDeduc) eq '' ? 0 : rsSumRHLiqDeduccion.totDeduc)>
...
--->				<cfset totLiq = rsLF.RHLFLresultado + rsSumRHLiqIngresos.totIngresos - rsSumRHLiqCargas.totCargas - (Trim(rsSumRHLiqDeduccion.totDeduc) eq '' ? 0 : rsSumRHLiqDeduccion.totDeduc)>

					<cf_translate key="LB_Total_Liquidacion">Total Liquidaci&oacute;n</cf_translate>: <strong>#LSCurrencyFormat(totLiq,'none')#</strong>
				</td>
				<td colspan="2" align="center" nowrap>
					<input type="button" name="Anterior" value="#vAnterior#" id="Anterior" onclick="javascript: location.href='liquidacionProceso.cfm?paso=4&DLlinea=#DLlinea#';" class="btnAnterior">
					<input type="submit" name="Aprobar" value="#vAprobar#" onclick="javascript: return funcAprobar();">
					<!--- <cf_botones values=",#vAprobar#" names="Aprobar"> --->
				</td>
			</tr>
