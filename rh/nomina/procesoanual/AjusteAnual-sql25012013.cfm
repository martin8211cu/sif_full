    ...
    <cfif isdefined("rsSelectAjusteAnualSalario") and rsSelectAjusteAnualSalario.RecordCount GT 0>
    	<cfloop query="rsSelectAjusteAnualSalario">
    		...
    		<cfquery name="rsInsertConceptos" datasource="#session.DSN#">
                ...
                select <cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.RHAAid#">,a.DEid, SUM(coalesce(a.RHLIgrabado,0)), a.CIid
				...
				<cfif conceptos NEQ ''>
					...
					and fechaalta between dateadd(day,-1,'#FechaDesde#') and dateadd(day,1,'#FechaHasta#')
            ...
            <cfquery name="rsInsertConceptos" datasource="#session.DSN#">
    			insert into #DRHAjusteAnual# (RHAAid,DEid,DRHAAAcumulado,CIid)
                select <cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.RHAAid#">,a.DEid, SUM(coalesce(a.RHLIexento,0)), a.CIid
				from RHLiqIngresos a
					inner join DDConceptosEmpleado b
						on a.DLlinea = b.DLlinea and a.CIid = b.CIid
					inner join CIncidentes c
            ...
            <cfquery name="rsInsertConceptos" datasource="#session.DSN#">
             ...
             <cfquery name="rsInsertConceptos" datasource="#session.DSN#">
				...
				from IncidenciasCalculo hi  
	 				inner join CIncidentes ci on hi.CIid = ci.CIid
                    inner join HRCalculoNomina rc on rc.RCNid = hi.RCNid and ci.Ecodigo = rc.Ecodigo
...
<cfif isdefined("Form.BTN_GenerarE")>
           ...
           <!--- <cfquery name="rsSelectEmpleadoAcumulado" datasource="#session.DSN#">
            ...
            <cfquery name="rsInsertTotalAcumulado" datasource="#session.DSN#">
				...
				from RHAjusteAnualAcumulado rhaas 
					inner join DRHAjusteAnual drhaa on drhaa.DEid = rhaas.DEid
                ...
                where DEid = #rsSelectEmpleadoAcumulado.DEid#
            </cfquery>
            </cfloop>
            </cfif>
           ...
           <!--- <cf_dump var = "#SelectAB#">--->
            ...
            <cfquery name="rsSelectSalarioEmpleado" datasource="#session.DSN#">
                ...
                where DEid not in (#ValueList(SelectAB.DEid)#)
                	and RHAAid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.RHAAid#"> 
    		</cfquery>
	...
	<cfif isdefined ("rsEmpleadosAjusteAnual") and rsEmpleadosAjusteAnual.RecordCount GT 0>
        ...
        <cfloop query="rsEmpleadosAjusteAnual">
        	<cfquery name="rsInsertPVacacional" datasource="#session.DSN#">
			 ...
			 where drha.RHAAid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.RHAAid#">
	  			and rhaa.Ecodigo = #session.Ecodigo#
	  			and ci.CIcodigo in ('06E','06G','40E','40G','10E','10G')
            ...
            <cfquery name="rsInsertAguinaldo" datasource="#session.DSN#">
			 ...
			 where drha.RHAAid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.RHAAid#">
	  			and rhaa.Ecodigo = #session.Ecodigo#
	  			and ci.CIcodigo in ('01E','01G','11E','11G')
	  			and DEid = #rsEmpleadosAjusteAnual.DEId#
				group by drha.RHAAid,DEid
            </cfquery>
            ...
            <cfquery name="rsUpdateEmpleados" datasource="#session.DSN#">
				...
				set RHAAAcumuladoGravado = ((select coalesce(sum(DRHAAAcumulado),0.00)
											...
											where drha.CIid = ci.CIid 
	  										...
	  										and ci.CIcodigo not in ('06E','06G','40E','40G','10E','10G','01E','01G','11E','11G','12','13','14')
	  										and ci.CInorenta = 0
	  										and drha.DEid = #rsEmpleadosAjusteAnual.DEId# 
	  										and ci.Ecodigo = #session.Ecodigo#
                                            and drha.RHAAid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.RHAAid#">)  + 
                                            coalesce((select AcumuladoGravable from #PrimaVacacional# where DEid = #rsEmpleadosAjusteAnual.DEId#),0.00) + coalesce((select AcumuladoGravable from #Aguinaldo# where DEid = #rsEmpleadosAjusteAnual.DEId#),0.00) + coalesce((select AcumuladoGravable from #Separacion# where DEid = #rsEmpleadosAjusteAnual.DEId#),0.00))
				where DEId = #rsEmpleadosAjusteAnual.DEId#
	  			and RHAAid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.RHAAid#">
            </cfquery>
            ...
            <cfquery name="rsUpdateEmpleados" datasource="#session.DSN#">
				...
				set RHAAAcumuladoExento = ((select coalesce(sum(DRHAAAcumulado),0.00)
							               ...
							               and ci.Ecodigo = #session.Ecodigo#
                                           and drha.RHAAid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.RHAAid#">) +
                                           coalesce((select AcumuladoExento from #PrimaVacacional# where DEid = #rsEmpleadosAjusteAnual.DEId#),0.00) + coalesce((select AcumuladoExento from #Aguinaldo# where DEid = #rsEmpleadosAjusteAnual.DEId#),0.00)+ coalesce((select AcumuladoExento from #Separacion# where DEid = #rsEmpleadosAjusteAnual.DEId#),0.00))
				where DEId = #rsEmpleadosAjusteAnual.DEId#
	  			and RHAAid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.RHAAid#">
            </cfquery>
           ...
           <!--- <cfquery name="rsUpdateEmpleados" datasource="#session.DSN#">
            ...
            <cfquery name="rsUpdateEmpleados" datasource="#session.DSN#">
				...
				set RHAAAcumuladoSubsidio = ((select coalesce(SUM(RHSvalor),00)
 											from HRHSubsidio hrhs,RHAjusteAnual rhaa
 											where hrhs.Ecodigo = rhaa.Ecodigo
												and hrhs.DEid = #rsEmpleadosAjusteAnual.DEId#
												and RHSFechaDesde >= '#rsSelectFechas.RHAAfecharige#'
												and RHSFechaHasta <= '#rsSelectFechas.RHAAfechavence#'
												and hrhs.Ecodigo = #session.Ecodigo#) +
                                            (select coalesce(SUM(RHSvalor),00)
 											from RHSubsidio hrhs,RHAjusteAnual rhaa
 											where hrhs.Ecodigo = rhaa.Ecodigo
												and hrhs.DEid = #rsEmpleadosAjusteAnual.DEId#
												and RHSFechaDesde >= '#rsSelectFechas.RHAAfecharige#'
												and RHSFechaHasta <= '#rsSelectFechas.RHAAfechavence#'
                ...
                <cfquery name="rsUpdateEmpleados" datasource="#session.DSN#">
                update RHAjusteAnualAcumulado 
				set RHAAAcumuladoRenta = ((select coalesce(sum(SErenta),0.00)
...
<!---        Empieza Calculo de Ajuste Anual--->
    ...
    <cfquery datasource="#session.DSN#">
    	Insert into #TRentaAnual# (DIRid,LimInf,LimSup,MontoFijo,Porcentaje)
            ...
            where a.IRcodigo = 'MXAN'
            	and a.EIRestado = 1
                and a.EIRdesde >=  <cfqueryparam cfsqltype="cf_sql_date" value="#rsSelectFechas.RHAAfecharige#">
        order by b.DIRinf
    </cfquery>
    ...
    <cfquery datasource="#session.DSN#">
     update #CalculoAjusteAnual# 
     	set ISRT = coalesce(((ISR - Subsidio) - ISPT),0)
    </cfquery>    
...
<cfif isdefined ("BTN_DescargarI")>
    ...
    <cfquery name="rsSelect" datasource="#session.DSN#">
    	select de.DEidentificacion,RHAAAcumuladoISPT
			from RHAjusteAnualAcumulado rhaaa 
    ...
    <cfloop query="rsSelect">
            ...
            <cfset columna = columna & '' & '#chr(44)#'>
            <cfset columna = columna & #rsSelect.RHAAAcumuladoISPT# & '#chr(44)#'>
...
<cfelseif isdefined ("BTN_DescargarD")>
    ...
    <cfset metodo = 1>
    <cfset cnominas = #form.txtCantidad#>
