...
<cfoutput>
	<select tabindex="1" name="Tcodigo#index#" id="Tcodigo#index#" <cfif len(trim(Attributes.onChange))>onChange="javascript: #Attributes.onChange#();"</cfif>>
		<cfif Attributes.todas>
			<option value="" <cfif not isdefined("Attributes.query.Tcodigo")>selected="selected"</cfif>>--Seleccione--</option>	
		</cfif>
