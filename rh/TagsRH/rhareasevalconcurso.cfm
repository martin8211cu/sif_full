...
<cfparam name="Attributes.id" default="RHEAid" type="string"> <!--- Nombre del id --->
<cfparam name="Attributes.name" default="RHEAcodigo" type="string"> <!--- Nombre del Código --->
<cfparam name="Attributes.desc" default="RHEAdescripcion" type="string"> <!--- Nombre de la Descripción --->
<cfparam name="Attributes.peso" default="RHEApeso" type="string"> <!--- Peso --->
<cfparam name="Attributes.frame" default="frareaevalconcurso" type="string"> <!--- Nombre del frame --->
...
<script language="JavaScript" type="text/javascript">
	...
	//Llama el conlis
	function doConlisareaevalconcurso<cfoutput>#Attributes.name#</cfoutput>() {
		var params ="";
		params = "<cfoutput>?form=#Attributes.form#&id=#Attributes.id#&name=#Attributes.name#&desc=#Attributes.desc#&peso=#Attributes.peso#&conexion=#Attributes.conexion#&empresa=#Attributes.empresa#</cfoutput>";

		popUpWindow<cfoutput>#Attributes.name#</cfoutput>("/cfmx/rh/Utiles/Conlisareaevalconcurso.cfm"+params,250,200,650,400);
	}
	//Obtiene la descripción con base al código
	function Traeareaevalconcurso<cfoutput>#Attributes.name#</cfoutput>(dato) {
		var params ="";
		params = "<cfoutput>&id=#Attributes.id#&name=#Attributes.name#&desc=#Attributes.desc#&peso=#Attributes.peso#&conexion=#Attributes.conexion#&ecodigo=#Attributes.empresa#</cfoutput>";
		if (dato!="") {
			var fr = document.getElementById("<cfoutput>#Attributes.frame#</cfoutput>");
			fr.src = "/cfmx/rh/Utiles/rhareaevalconcursoquery.cfm?dato="+dato+"&form="+"<cfoutput>#Attributes.form#</cfoutput>"+params;
		}
		else{
			limpiarareaevalconcurso();
		}
		return;
	}
	function limpiarareaevalconcurso(){
	<cfoutput>
		...
		document.#Attributes.form#.#Attributes.peso#.value="";
	</cfoutput>
	}
</script>
<table width="" border="0" cellspacing="0" cellpadding="0">
	<cfif isdefined('Attributes.query') and ListLen(Attributes.query.columnList) GT 1>
		...
		<cfset peso = "Trim('#Evaluate('Attributes.query.#Evaluate('Attributes.peso')#')#')">		
	</cfif>
	<cfoutput>
	<tr>
		<td>
			...
			<input type="text"
				...
				onBlur="javascript: Traeareaevalconcurso#Attributes.name#(document.#Attributes.form#.#Evaluate('Attributes.name')#.value); 
		...
		<td nowrap>
			<input type="text"
				...
				maxlength="80">
			<a href="##" tabindex="-1"><img src="/cfmx/rh/imagenes/Description.gif" alt="Lista de &Aacute;reas a Evaluar" name="imagen" width="18" height="14" border="0" align="absmiddle" onClick='javascript: doConlisareaevalconcurso#Attributes.name#();'></a>
		</td>
	</tr>
	</cfoutput>
</table>
