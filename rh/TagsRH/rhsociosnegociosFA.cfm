...
<!--- consultas --->
<cf_translatedata name="get" tabla="SNegocios" col="SNnombre" returnvariable="LvarSNnombre">
<cfquery name="rsSocios" datasource="#Attributes.Conexion#">
	select SNcodigo, SNnumero, SNtiposocio, #LvarSNnombre# as SNnombre, SNFecha, SNtipo, SNvencompras, SNvenventas, SNinactivo, ts_rversion 
	from SNegocios 
...
function doConlis<cfoutput>#Attributes.SNcodigo#</cfoutput>() {
	...
	popUpWindow("/cfmx/rh/Utiles/ConlisSociosNegociosFA.cfm"+params,250,200,650,350);
}
...
function TraeSocio<cfoutput>#Attributes.SNcodigo#</cfoutput>(dato) {
	...
	if (dato=="") {dato="-1"}
		...
		frame.src="/cfmx/rh/Utiles/rhsociosnegociosFAquery.cfm?SNnumero="+dato+"&form="+'<cfoutput>#Attributes.form#</cfoutput>'+params;
