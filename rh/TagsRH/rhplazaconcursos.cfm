...
<cfif isDefined("session.Ecodigo") and isDefined("form.RHCconcurso") and len(trim(#form.RHCconcurso#)) NEQ 0>
	...
	<cfquery name="rsRHConcursos" datasource="#Session.DSN#" >
		Select RHCconcurso, RHCcodigo, #LvaRHCdescripcion# as RHCdescripcion, 
			...
			a.RHPcodigo, #LvarRHPdescpuesto# as RHPdescpuesto, RHCcantplazas, a.RHCfecha,
...
<script language="JavaScript" type="text/javascript">
	...
	//Llama el conlis
	function doConlisPlazaconcursos<cfoutput>#Attributes.name#</cfoutput>() {
		var params ="";
		params = "<cfoutput>?form=#Attributes.form#&id=#Attributes.id#&name=#Attributes.name#&desc=#Attributes.desc#&conexion=#Attributes.conexion#&empresa=#Attributes.empresa#&RHPpuesto=#rsRHConcursos.RHPcodigo#&CFid=#rsRHConcursos.CFid#</cfoutput>";
		params+= "&vfyplz=<cfoutput>#Attributes.NoCheckPlazaOcupada#</cfoutput>";
		popUpWindow<cfoutput>#Attributes.name#</cfoutput>("/cfmx/rh/Utiles/ConlisPlazaconcursos.cfm"+params,250,200,650,400);
	}
	...
	<tr>
		...
		<td nowrap>
			<input type="text"
				...
				maxlength="80">
			<a href="##" tabindex="-1"><img src="/cfmx/rh/imagenes/Description.gif" alt="Lista de Plazas" name="imagen" width="18" height="14" border="0" align="absmiddle" onClick='javascript: doConlisPlazaconcursos#Attributes.name#();'></a>
		</td>
	</tr>
	</cfoutput>
</table>
