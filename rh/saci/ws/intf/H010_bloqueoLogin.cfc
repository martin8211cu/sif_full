<cfcomponent hint="Ver SACI-03-H010.doc" extends="base">
	<cffunction name="bloqueoLogin" access="public" returntype="void">
		...
		<cfargument name="motivo" type="string" required="yes" default="">
		<cfargument name="S02CON" type="numeric" required="yes" default="0">
		
		<cfset bloqueoLGnumero(origen, Arguments.login, 0, saldo, spam, motivo, S02CON)>
	</cffunction>
	
	<cffunction name="bloqueoLGnumero" access="public" returntype="void">
		<cfargument name="origen" type="string" required="yes">
		<cfargument name="login" type="string" required="yes">
		...
		<cfset control_inicio( Arguments, 'H010', 'LGnumero=' & Arguments.LGnumero )>
		<cftry>
			...
			<cfif Arguments.origen is 'siic'>
				...
				<cfquery datasource="#session.dsn#" name="ISBmotivoBloqueo">
					...
					where MBmotivo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.motivo#">
				</cfquery>
				...
				<cfset control_mensaje( 'ISB-0010', 'Bloqueando LGnumero #Arguments.LGnumero#' )>
				...
				<cfif rsBLQ.RecordCount gt 0>
					<cfthrow message="El login ya se encuentra bloqueado por el motivo '#Arguments.motivo#'" errorcode="ISB-0009">
				<cfelse>							
				...
				<cfinvoke component="saci.comp.ISBbloqueoLogin" method="Alta"
					...
					BLQdesbloquear="false"
					MSsaldo="#Arguments.saldo#"
					MStexto="Bloqueo de login"
					BLorigen = "SIIC"/>
				</cfif>
			</cfif>
			...
			<cfif Arguments.origen eq 'siic' or (Arguments.origen is 'saci' and Arguments.OrigenbloqueoLogin eq 'SACI')>
				...
				<cfif ListFind(accesos, 'A')>
					...
					<cfinvoke component="CiscoService" method="changeUserParent"
						...
						newParent="__SOIN_DEFUALT_DISABLED_GROUPS__" />
				</cfif>
								
				<!--- iplanet: acceso C => Correo --->
				<cfif ListFind(accesos, 'C')>
					<cfset control_servicio( 'correo' )>
					<cfinvoke component="IPlanetService" method="lock"
						usuario="#ISBlogin.LGlogin#"/>
				</cfif>
