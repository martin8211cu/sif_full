<cfquery name="rsSitActual" datasource="#session.DSN#">
	select 	b.CCPid, CCPcodigo, CCPdescripcion,CCPvalor,
			UECPdescripcion,TCCPdesc,coalesce(b.LTvalor,0) as valor
			,c.CCPequivalenciapunto as puntos
			,(coalesce(c.CCPmaxpuntos,0)-coalesce(b.LTvalor,0)) as faltante
			,(select coalesce(max(LTpuntosTotales),0)
				from LineaTiempoCP x
				where b.DEid = x.DEid
					and <cfqueryparam cfsqltype="cf_sql_timestamp" value="#now()#"> between x.LTdesde and x.LThasta			
			) as totalPuntos
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="3" class="sectionTitle" style="padding-left: 5px; padding-right: 5px;">
		<cfoutput>
			...
			<tr>
				<td colspan="5" class="corteLista"><b><cf_translate key="LB_TotalDePuntos">Total de puntos</cf_translate>:</b>&nbsp;#rsSitActual.totalPuntos#</td>
			</tr>
			<tr class="listTitle">
				<td>#LB_Codigo#</td>
