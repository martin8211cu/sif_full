...
<cfcomponent>
			...
			<cfif LvarRCNid NEQ "">
				<cftry>
					...
					<cfquery name="rsTcodigo" datasource="#session.DSN#">
			        	select h.Tcodigo,t.Contabilizar
						  from HERNomina h
						  inner join TiposNomina t on h.Ecodigo=t.Ecodigo and h.Tcodigo =t.Tcodigo
						 where h.ERNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.ERNid#">
					</cfquery>
			
					<!--- Normal --->
					<cfif yesNoFormat(rsTcodigo.Contabilizar)>
					...
					<cftransaction>
                        ...
                        <cfquery datasource="#session.dsn#"> <!---Elimina lo que hay en RHSubsidio SML--->
	...
	<cffunction name="funcDiferenciaAsiento" access="public" output="true" returntype="numeric">
		...
		<cfquery name="rsDebitos" datasource="#session.DSN#">
			select round(coalesce(sum(montores),0),2)  as debitos
			from RCuentasTipo rt
			left join CIncidentes ci
				on ci.CIid = rt.referencia
			where rt.RCNid = #Arguments.RCNid#
				and rt.tipo = 'D'
				and rt.Ecodigo = #session.Ecodigo#
		...
		<cfquery name="rsCreditos" datasource="#session.DSN#">
			select round(coalesce(sum(montores),0),2)  as creditos
			from RCuentasTipo
			where RCNid = #Arguments.RCNid#
				and tipo = 'C'
				and Ecodigo = #session.Ecodigo#
	...
	<cffunction name="sbAsientoContable" access="public" output="true" returntype="any">
		...
		<!---<cf_dump var="#Arguments.historico#">--->
...
<!--- 						<cfinvokeargument name="Lprm_Cdetalle"	value="#rsCNH_cursor.Cformato#">  2011-06-01 EL MODO EN QUE SE MANDA LA CUENTA ES INCORRECTA, DEBE ENVIARSE SIN LA CUENTA MAYOR--->
		...
		<cfelseif LSNumberFormat(LvarDebitos,'9.00') NEQ LSNumberFormat(LvarCreditos,'9.00')>
			<cfif Arguments.Tipo NEQ "C">		<!--- Gastos o Unificado --->
		...
		<cfelse>
			<cfquery name="rsfechaPago" datasource="#session.DSN#">
				select CONVERT(VARCHAR(2),MONTH(CPfpago)) as fechaPago,CONVERT(VARCHAR(4),YEAR(CPfpago)) as CPperiodo from CalendarioPagos where CPid=#LvarReferencia#
			</cfquery>
			<!--- Genera el Asiento Contable--->
			<cfinvoke component="sif.Componentes.CG_GeneraAsiento" method="GeneraAsiento" returnvariable="LvarIDcontable">
				<cfinvokeargument name="Ecodigo" 		value="#session.Ecodigo#"/>
				<cfinvokeargument name="Oorigen" 		value="RHPN"/>
				<cfinvokeargument name="Cconcepto" 		value="#LvarCconcepto#"/>
				<cfinvokeargument name="Eperiodo" 		value="#rsfechaPago.CPperiodo#"/>
				<cfinvokeargument name="Emes" 			value="#rsfechaPago.fechaPago#"/> 
				<cfinvokeargument name="Efecha" 		value="#LvarFecha#"/>
				<cfinvokeargument name="Edocbase" 		value="#LvarCPcodigo#"/>
				<cfinvokeargument name="Ereferencia" 	value="#LvarReferencia#"/>
				<cfif Arguments.Tipo NEQ "C">		<!--- Gastos o Unificado --->
