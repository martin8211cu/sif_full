<cfcomponent>
    ...
    <cffunction name="fnGenerarEmpleados" access="private" output="true" returntype="numeric"  hint="Privada - Generación de Empleados Para cálculo del PTU">
        ...
        <cfargument name="validaTcodigo"  type="boolean" required="no" hint="Codigo de la Nomina" default="false">
           ...
           <!--- <cfthrow message= "#Arguments.validaTcodigo#">--->
            ...
            <cfquery datasource="#Arguments.conexion#" <!---name="Prueba"--->>
                insert into RHPTUEMpleados (
                        DEid,
                        RHPTUEid,
                        Ecodigo,
            ...
            <!--- Insertar Empleados que pertenecen a diferentes Empresas SML--->
            <cfquery datasource="#Arguments.conexion#" name = "Prueba">
            	insert into RHPTUEMpleados (
                        DEid,
                        RHPTUEid,
                        ...
                        RHPTUEMfecha,
                        BMUsucodigo)
            		select distinct a.DEid, c.RHPTUEid,a.Ecodigo, min(b.EVfantig) as FechaDesde, max(LThasta) as FechaHasta,
						d.Dcodigo,
                        d.Ocodigo,
                        d.RHPcodigo,
                        <cfqueryparam cfsqltype="cf_sql_date" value="#Now()#">,
            		...
            		from LineaTiempo a
                		...
                		inner join RHPTUD d on d.Ecodigo = a.Ecodigo
                		inner join RHPTUE c on c.RHPTUEid= d.RHPTUEid
            		where d.RHPTUEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHPTUEid#">
            			<!---and a.DEid =  305--->
                        and LTid = (select top 1 lt.LTid from LineaTiempo lt where lt.DEid = e.DEid order by LTid desc)
            			and d.CFid is null
						and d.Dcodigo is null
						and d.Ocodigo is null
						and d.RHPcodigo is null
						and d.DEid is null
   	            		group by a.DEid, b.EVfantig,c.RHPTUEid,a.Ecodigo,d.Dcodigo,d.Ocodigo, d.RHPcodigo

					<!---select c.DEid, b.RHPTUEid,a.Ecodigo, min(c.LTdesde) as FechaDesde, max(c.LThasta) as FechaHasta,
                        a.Dcodigo,
                        a.Ocodigo,
                        a.RHPcodigo,
                        <cfqueryparam cfsqltype="cf_sql_date" value="#Now()#">,
                        <cfqueryparam cfsqltype="cf_sql_numeric" value="#Session.Usucodigo#"> as BMUsucodigo
				from RHPTUD a inner join RHPTUE b on a.RHPTUEid= b.RHPTUEid
					inner join LineaTiempo c
                        on c.Ecodigo = a.Ecodigo
                        <!---and b.FechaHasta between c.LTdesde and c.LThasta--->
                        and not exists (select 1
                    					from RHPTUEMpleados eam
                    					where eam.RHPTUEid = a.RHPTUEid
                    					and eam.DEid = c.DEid)
                    inner join EVacacionesEmpleado d
                		on d.DEid = c.DEid <!---and d.Ecodigo = a.Ecodigo--->
				where a.RHPTUEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHPTUEid#">
					 and not exists ( select 1 from RHPTUEMpleados eam
                    				 where eam.RHPTUEid = a.RHPTUEid
                          			 and eam.DEid = c.DEid)
					and a.CFid is null
                    and a.Dcodigo is null
                    and a.Ocodigo is null
					and a.RHPcodigo is null
                    and a.DEid is null
                group by c.DEid, d.EVfantig,b.RHPTUEid,a.Ecodigo,c.LTdesde,c.LThasta,a.Dcodigo, a.Ocodigo, a.RHPcodigo--->
            </cfquery>

            <!---<cf_dump var ="#Prueba#">--->
            <!---Eliminar trabajadores que se dieron de baja antes de la FechaDesde SML--->
          	<cfquery datasource="#arguments.conexion#" name="prueba">
                delete from RHPTUEMpleados
                        where FechaHasta < (select FechaDesde from RHPTUE where RHPTUEid=<cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHPTUEid#">)
                        and  RHPTUEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHPTUEid#">
            </cfquery>
            <cfquery datasource="#arguments.conexion#" name="prueba">
                delete from RHPTUEMpleados
                        where FechaDesde > (select FechaHasta from RHPTUE where RHPTUEid=<cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHPTUEid#">)
                        and  RHPTUEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHPTUEid#">
            </cfquery>
		...
		<!---SML. Obtener su sueldo anual Inicio   --->
        ...
        <cfset LvarFechaDesdePTU = rsRHPTUE.FechaDesde>
		<cfset LvarFechaHastaPTU = rsRHPTUE.FechaHasta>
...
--->
        <cfloop query="rsEmpleados">
        	...
        	<cfset LvarRHPTUEMDiasAPagar 			= fnDiasPagar(LvarDeid,LvarEcodigo,arguments.RHPTUEid,LvarRHPTUEMid,LvarFechaDesdePTU,LvarFechaDesde,LvarFechaHasta,LvarFechaHastaPTU,LvarRHPTUEDescFaltas,LvarRHPTUEDescIncapa)>

            <!---SML Modificar Para que solo considere a los trabajadores que tengan los dias laborados, de acuerdo a lo solicitado por el usuario--->
             <cfquery datasource="#arguments.conexion#" name="CantDias">
                select RHPTUECantDias from RHPTUE
                        where  RHPTUEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHPTUEid#">
        	</cfquery>

        	<cfif isdefined("CantDias") and CantDias.RecordCount GT 0 >
        		<cfif LvarRHPTUEMDiasAPagar LT CantDias.RHPTUECantDias>
        			<cfquery datasource="#arguments.conexion#" name="prueba">
                		delete from RHPTUEMpleados
                        	where RHPTUEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHPTUEid#">
                          	and DEid = #LvarDeid#
            	...
            	<cfelse>
		     		<cfset LvarRHPTUEMSueldoAnual 	= fnSueldoAnual(LvarDeid,LvarFechaDesdePTU,LvarFechaHastaPTU,LvarEcodigo)>   <!--- se comenta ya que ahora se saca el sueldo anual = sueldoMensual / 30 * DíasaPagar, solicitado por el cliente a Luis --->

     				<cfquery datasource="#arguments.conexion#">
            			update RHPTUEMpleados
                		set RHPTUEMDiasAPagar 				= #LvarRHPTUEMDiasAPagar#,
                		RHPTUEMSueldoAnual 				    = #LvarRHPTUEMSueldoAnual#<!---((RHPTUEMSueldoMensual / 30) * #LvarRHPTUEMDiasAPagar#)--->
        ...
        <!---SML. Obtener su sueldo anual Final  --->
		<cfset error = 0>
        <cfreturn error>
	</cffunction>
    ...
    <cffunction name="CalculoPTU" access="public" output="true" returntype="numeric"  hint="Cálculo del PTU">
		...
		<cfargument name="Ecodigo" 			type="numeric" required="no" default="#Session.Ecodigo#" hint="Empresa">
		<cfargument name="conexion"			type="string" required="no" default="#Session.DSN#" hint="cache">
		<cfargument name="MontoMaximoS" 	type="numeric" required="no">
        <cfargument name="MontoMaximoNS"	type="numeric" required="no">


		<cfquery name="rsUMA" datasource="#arguments.conexion#">
        	select Pvalor
            from RHParametros
            where Ecodigo = #session.Ecodigo#
            and Pcodigo = 2051
		...
		<cfif rsUMA.RecordCount eq 0>
			<cfthrow message="No se ha definido el parámetro: Unidad de Medida y Actualización, en el catálogo de parámetros de RH, proceso cancelado!">
		</cfif>
        ...
        <cfloop query="rsEmpleados">
           ...
           <!--- <!--- Calculo de todos los valores --->
            ...
            <cfquery datasource="#arguments.conexion#">
            	update RHPTUEMpleados
                set RHPTUEMDiasAPagar 				= #LvarRHPTUEMDiasAPagar#,
                RHPTUEMSueldoAnual 				    = ((RHPTUEMSueldoMensual / 30.4) * #LvarRHPTUEMDiasAPagar#)
                where RHPTUEMid = #LvarRHPTUEMid#
                and DEid = #LvarDeid#
            </cfquery>
        </cfloop>--->
        ...
        <cfquery name="rsEmpleados" datasource="#Session.DSN#">
            select RHPTUEMid,RHPTUEMSueldoAnual, DEid, Ecodigo
        ...
        <!--- 
            <cfloop query="rsEmpleados">
                ...
                <cfset MontoTope = #arguments.MontoMaximoNS#>
                <cfquery datasource="#arguments.conexion#">
                    update RHPTUEMpleados
                        set
                        RHPTUEMSueldoAnual 	= <cfif SueldoAnual GT MontoTope>
                                                #MontoTope#
                                            <cfelseif SueldoAnual LTE MontoTope>
                                                #SueldoAnual#
                                            </cfif>
                        where RHPTUEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHPTUEid#">
       ...
       <cfquery name="rsSumas" datasource="#arguments.conexion#">
        ...
        <cfloop query="rsEmpleados">
			<cfset LvarDeid 	  = rsEmpleados.DEid>
            <cfset LvarSueldoAnual = rsEmpleados.RHPTUEMSueldoAnual>
            <cfset LvarDiasAPagar = rsEmpleados.RHPTUEMDiasAPagar>
        ...
        <cfquery datasource="#arguments.conexion#">
            ...
            set
                ...
                RHPTUEMxDiasLaborados			= (#LvarDiasAPagar# * ((#LvarRHPTUEMonto# / 2) / #LvarTotalDiasLaborados#)),
                RHPTUEMCuotaDiaria				= <cfif #LvarSueldoAnual# GT #arguments.MontoMaximoNS# and Arguments.MontoMaximoNS gt 0>
                									(Arguments.MontoMaximoNS * ((#LvarRHPTUEMonto# / 2) / <cfif LvarTotalSueldoPagado eq 0>1<cfelse>#LvarTotalSueldoPagado#</cfif>)),
                                                  <cfelseif #LvarSueldoAnual# GT #arguments.MontoMaximoNS# and arguments.MontoMaximoNS eq 0 >
                									(RHPTUEMSueldoAnual * ((#LvarRHPTUEMonto# / 2) / <cfif LvarTotalSueldoPagado eq 0>1<cfelse>#LvarTotalSueldoPagado#</cfif>)),
                                                  <cfelseif #LvarSueldoAnual# LTE #arguments.MontoMaximoNS#>
                									(#LvarSueldoAnual# * ((#LvarRHPTUEMonto# / 2) / <cfif LvarTotalSueldoPagado eq 0>1<cfelse>#LvarTotalSueldoPagado#</cfif>)),
                                                   </cfif>
                RHPTUEMTotalPTU					= round((#LvarDiasAPagar# * ((#LvarRHPTUEMonto# / 2) / #LvarTotalDiasLaborados#))
                									 + (RHPTUEMSueldoAnual * ((#LvarRHPTUEMonto# / 2) / <cfif LvarTotalSueldoPagado eq 0>1<cfelse>#LvarTotalSueldoPagado#</cfif>)),2),

                RHPTUEMSalarioMinimo			= #rsUMA.Pvalor#,
                RHPTUEMExenta					= #rsUMA.Pvalor# * 15,
                RHPTUEMGravable					= ((#LvarDiasAPagar# * ((#LvarRHPTUEMonto# / 2) / #LvarTotalDiasLaborados#)) + (#LvarSueldoAnual# * ((#LvarRHPTUEMonto# / 2) / <cfif LvarTotalSueldoPagado eq 0>1<cfelse>#LvarTotalSueldoPagado#</cfif>))) - (#rsUMA.Pvalor# * 15)
            where RHPTUEid = #arguments.RHPTUEid#
            and DEid = #LvarDeid#
        </cfquery>
        </cfloop>
        ...
        <cfquery datasource="#arguments.conexion#">
        	update RHPTUEMpleados
            	set 
                    RHPTUEMExenta = case when RHPTUEMTotalPTU >= RHPTUEMExenta then RHPTUEMExenta else RHPTUEMTotalPTU end,
                    RHPTUEMGravable = case when RHPTUEMTotalPTU > RHPTUEMExenta then RHPTUEMTotalPTU - RHPTUEMExenta else 0.00 <!---RHPTUEMGravable---> end
            from RHPTUEMpleados
            where RHPTUEid = #arguments.RHPTUEid#
        </cfquery>
        ...
        <cfquery datasource="#arguments.conexion#" name= "tbImpuesto">
        	select EIRid from RHPTUE
            where RHPTUEid = #arguments.RHPTUEid#
        </cfquery>

        <cfif isdefined('tbImpuesto') and tbImpuesto.RecordCount GT 0>
        	<cfset IRcodigo = #tbImpuesto.EIRid#>
        <cfelse>
        ...
        <cfquery name="rsTablaImp" datasource="#arguments.conexion#">
            select top 1 EIRid, IRcodigo, EIRdesde, EIRhasta, EIRestado, ts_rversion
            from EImpuestoRenta
            where IRcodigo = '#IRcodigo#'
            order by EIRhasta desc
        </cfquery>
            ...
            update RHPTUEMpleados
                    ...
                    (
                        ...
                        from  EImpuestoRenta a
                            inner join DImpuestoRenta b
                            on a.EIRid = b.EIRid
                        where a.EIRid = #rsTablaImp.EIRid#
                        and a.EIRestado > 0
        ...
        <cfquery datasource="#arguments.conexion#">
            update RHPTUEMpleados
            set   	RHPTUEMISPTRetencionPTU = round(coalesce(RHPTUEMProporcionPTU,0) * coalesce(RHPTUEMGravable,0) ,2),
            		RHPTUEMNetaRecibir =	 round(RHPTUEMTotalPTU - (coalesce(RHPTUEMProporcionPTU,0) * coalesce(RHPTUEMGravable,0))  ,2)
            where RHPTUEid = #arguments.RHPTUEid#
        </cfquery>
    ...
    <cffunction name="fnDiasPagar" access="public" output="true" returntype="numeric"  hint="Calculos de Días a Pagar">
           ...
           <!--- <cfset LvarDias = DateDiff("d", LvarFechaDesde, LvarFechaHasta)+1> ---><!---  FechaHasta - FechaDesde --->
			<!--- Puede ser que se quieran los días nómina pero tienen que confirmar los 360 días, en vez de los 365, se deja la prevista--->
			 <cfquery name="rsDiasNomina" datasource="#arguments.conexion#">
				select coalesce(sum(a.PEcantdias),0) as PEcantdias
				from HPagosEmpleado a
					inner join HRCalculoNomina b
					on b.RCNid = a.RCNid
				where b.RCdesde >= '#varFechaDesde#'
					and b.RChasta <= '#varFechaHasta#'
					and a.DEid = #arguments.DEid#
					and a.PEtiporeg in (0,1) <!--- Normales --->
			</cfquery>
            <cfset LvarDias = 0>
            ...
            <cfif arguments.FvarDescFaltas>
                <cfquery name="rsDiasFalta" datasource="#arguments.conexion#">
                    ...
                    where b.DEid = #arguments.DEid#
                        ...
                        and a.RCdesde >= '#varFechaDesde#'
                        and a.RChasta <= '#varFechaHasta#'
                        and b.PEdesde >= '#varFechaDesde#'
                        and b.PEhasta <= '#varFechaHasta#'
                </cfquery>
        ...
        <!---SML. Modificacion para tener el sueldo mensual sin vales Inicio--->
        <cfquery name="rsSueldoMensual" datasource="#arguments.conexion#">
        	select Ecodigo,FechaDesde,FechaHasta
            from RHPTUE
            where RHPTUEid = #arguments.RHPTUEid#
        </cfquery>

        <cfquery name="rsSueldoMensualLT" datasource="#arguments.conexion#">
        	select top 1 coalesce(d.DLTmonto,0) as LTsalario
                ...
                on d.CSid = c.CSid and c.Ecodigo = a.Ecodigo
			where CSsalariobase = 1 and a.Ecodigo = #arguments.Ecodigo#
				and l.LTdesde between '#rsSueldoMensual.FechaDesde#' and '#rsSueldoMensual.FechaHasta#'
				<!---and l.LThasta between '#rsSueldoMensual.FechaDesde#' and '#rsSueldoMensual.FechaHasta#'--->
				and a.DEid = #arguments.DEid#
   			order by l.LTdesde desc
        ...
        <!---SML. Modificacion para tener el sueldo mensual sin vales Final--->
        <!---<cfthrow message="#rsSueldoMensualLT.RecordCount#">--->

        <!--- si rsSueldoMensualLT da 0 entonces se debe calcular el último salario del empleado (solicitado por Guillermo en México) --->
        <cfif rsSueldoMensualLT.RecordCount eq 0>
        	<cfquery name="rsSueldoMensualLT" datasource="#arguments.conexion#">
            	select coalesce(d.DLTmonto,0) as LTsalario
                from DatosEmpleado a
                inner join LineaTiempo l
                on a.DEid = l.DEid
                inner join DLineaTiempo d
        ...
        <cfif isdefined('rsSueldoMensualLT') and rsSueldoMensualLT.RecordCount GT 0>
        <cfset LvarSueldoMensual = rsSueldoMensualLT.LTsalario>
        <cfelse>
        <cfset LvarSueldoMensual = 0>
        </cfif>
        <cfquery datasource="#arguments.conexion#">
        	update RHPTUEMpleados
            	set RHPTUEMSueldoMensual = #LvarSueldoMensual#,
