<cfcomponent>
    ...
    <cffunction name="CuentasTipo" access="public" output="true" >
		...
		<cfquery name="rsCT_Datos" datasource="#arguments.Conexion#">
			select a.RCdesde, a.RChasta, b.CPperiodo, b.CPmes, c.Ttipopago, b.CPtipo, c.TRegimen
		...
		<cfquery name="rsCT_Datos1" datasource="#arguments.Conexion#">
			...
			from RHParametros
			where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.Ecodigo#">
			  <cfif rsCT_Datos.TRegimen eq '09'>
					and Pcodigo = 141
				  <cfelse>
			  and Pcodigo = 140
			  </cfif>
		...
		<cfset vCuentabanco = ObtenerDato(150).Pvalor>
		...
		<cfquery datasource="#arguments.conexion#">
		  ...
		  <!--- SE ACTUALIZAN LOS DATOS DE PLAZA Y CENTRO FUNCIONAL PARA LA GENERACION DEL ASIENTO 
			Para tipo 4 de PTU se modifico el update
--->
		  
        <cfif vCPtipo EQ 4 >
        	<cfquery datasource="#arguments.conexion#">
        		update #Empleados#
                	set RHPid = coalesce((select top 1 dle.RHPid
                      from DLaboralesEmpleado dle
                      inner join RHPlazas rp
                        on rp.RHPid=dle.RHPid
                      where  dle.DEid=#Empleados#.DEid
                      order by dle.DLlinea desc),-1)
                where DEid=#Empleados#.DEid
            </cfquery>
			<cfquery datasource="#arguments.conexion#">
				update #Empleados#
	                	set CFid =coalesce((select top 1 rp.CFid
	                      from DLaboralesEmpleado dle
	                      inner join RHPlazas rp
	                        on rp.RHPid=dle.RHPid
	                      where  dle.DEid=#Empleados#.DEid
              ...
              <cfquery datasource="#arguments.conexion#">
               update #Empleados#
                set RHPid = coalesce(( select lt.RHPid
                      from LineaTiempo lt
					<!--- OPARRALES 2018-11-01 SE AGREGA VALIDACION PARA NOMINAS ESPECIALES, PARA ESTE CASO AGUINALDO --->
						INNER JOIN RHTipoAccion ta
						on ta.RHTid = lt.RHTid
						<!--- and ta.RHTcomportam = 1 --->
					<!--- OPARRALES 2018-11-01 Fin validacion --->
		...
		<cfdump  var="vContabilizaGastosMes: #vContabilizaGastosMes# vTipoPago: #vTipoPago# vRCdesde: #vRCdesde# vRChasta: #vRChasta#">
		<cfif vContabilizaGastosMes eq 1 and listfind('0,1', vTipoPago) and (datepart('m', vRCdesde) neq datepart('m', vRChasta)) >
		<cfdump  var="STOPED" ABORT>
			...
			<cfdump  var="#vCuentaPasivoFmt#" abort>
	...
	<cffunction name="CuentasTipo_Incidencias_D" access="private" output="false">
		...
		<cfquery datasource="#arguments.conexion#">
			...
			where a.RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#">
              ...
              and a.ICespecie = 0 <!---SML. Modificacion para que solo se consideren los vales de despensa positivos--->
	...
	<cffunction name="CuentasTipo_Incidencias_CuentaFija" access="private" output="false">
		...
		<cf_dbfunction name="findOneOf" args="b.CFcuentac, ?!*" returnvariable="findoneof">
		<!---Jarr validacion para calculo de ptu=4--->
		<cfquery datasource="#arguments.conexion#" name="valPTU">
			select CPtipo from CalendarioPagos
			where CPid=<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#">
		</cfquery>

		<cfquery datasource="#arguments.conexion#" name="prueba2">
		insert into RCuentasTipo(RCNid, Ecodigo, tiporeg, cuenta, valor, Cformato, Ccuenta, CFcuenta, tipo, CFid, Ocodigo, Dcodigo, montores, BMfechaalta, BMUsucodigo, DEid, RHPPid,referencia, vpresupuesto, Periodo, Mes)
		select 	<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#">,
					<cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.Ecodigo#">,
					20,
			...
			from IncidenciasCalculo a
                ...
                inner join CIncidentes c
                	on c.CIid = a.CIid
					AND c.CItimbrar = 0 <!--- OPARRALES 2018-08-21 (Pagos en EFECTIVO)--->
				<cfif IsDefined('valPTU') and valPTU.recordcount GT 0 and valPTU.CPtipo EQ 4>
                outer apply
                (select top 1 rp.RHPid,dle.Ecodigo
                      from DLaboralesEmpleado dle
                      inner join RHPlazas rp
                        on rp.RHPid=dle.RHPid
                      where  dle.DEid=a.DEid
                      order by dle.DLlinea desc) lt
                <cfelse>
				...
				<!--- OPARRALES 2018-08-21 Validacion para nominas especiales (Aguinaldo) --->
				<cfif IsDefined('vCPtipo') and vCPtipo eq 1>
					left join RHTipoAccion ta
						on lt.RHTid = ta.RHTid
						<!---and ta.RHTcomportam = 1  Acciones de nombramiento --->
				</cfif>
		...
		<cfquery name="rsquery" datasource="#arguments.conexion#">
			select	<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#">,
					<cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.Ecodigo#">,
					20,
					b.CFcuentac,
	...
	<cffunction name="CuentasTipo_Pagos_No_Realizados" access="private" output="false">
		...
		<cf_dbfunction name="findOneOf" args="b.CFcuentac, ?!*" returnvariable="findoneof">
			...
			<!--- --->
		<cfquery datasource="#arguments.conexion#" name="prueba3">
			insert into RCuentasTipo(RCNid,Ecodigo, tiporeg, cuenta, valor, Cformato, Ccuenta, tipo, CFid, Ocodigo, Dcodigo, montores, BMfechaalta, BMUsucodigo, DEid, referencia, vpresupuesto, Periodo, Mes)
			 select 	<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#">,
					...
					c.Cformato,
					c.Cformato,
					null, null, 'D', b.CFid, b.Ocodigo, b.Dcodigo, 
					ROUND(sum(a.ICmontores), 2),
					<cf_dbfunction name="now">,
					<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.Usucodigo#">,
					-1, -1, 0,
...
<!--- 20110519 LZ Insertar Cargas en RCuentasTipo para el Segundo Mes --->
	...
	<cffunction name="CuentasTipo_tiporeg_50_55" access="private" output="false">
		...
		<cfquery datasource="#arguments.conexion#">
			...
			select 	<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#"> ,
					...
					60,
					case
						when sum(a.DCvalor - a.DCinteres) < 0 then cf.CFformato
						else cc.Cformato
					end,
					null, null, null, 'C', emp.CFid, emp.Ocodigo, emp.Dcodigo, sum(a.DCvalor - a.DCinteres),
					<cf_dbfunction name="now">,
					<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.Usucodigo#">,
					-1, a.Did, 0,
					<cfqueryparam cfsqltype="cf_sql_integer" value="#vPeriodo#">,
			...
			from #Empleados# emp
				...
				inner join TDeduccion td
					on td.TDid = c.TDid
					and td.Ecodigo = c.Ecodigo
				left join CFinanciera cf
					on cf.Ccuenta = td.Ccuenta
					and cf.Ecodigo = td.Ecodigo
                inner join SNegocios s
			...
			where a.DCvalor != 0
			  and cc.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.Ecodigo#">
			group by cc.Cformato,cf.CFformato, emp.CFid, emp.Ocodigo, emp.Dcodigo, a.Did
		</cfquery>
	</cffunction>
	...
	<cffunction name="CuentasTipo_tiporeg_70" access="private" output="false">
		...
		<cfquery datasource="#arguments.conexion#">
			...
			select 	<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#"> ,
					...
					null, null, null, 'C', emp.CFid, emp.Ocodigo, emp.Dcodigo, ROUND(sum(a.SErenta), 2),
					<cf_dbfunction name="now">,
					<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.Usucodigo#">,
					-1, -1, 0,
					<cfqueryparam cfsqltype="cf_sql_integer" value="#vPeriodo#">,
	...
	<cffunction name="CuentasTipo_tiporeg_80" access="private" output="false">
		...
		<cfquery name="rsCcuentaDespensa" datasource="#session.dsn#">
			select Pvalor
			from RHParametros
			where Pcodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="14600705">
			and Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
		</cfquery>

		<cfquery name="rsIncDesp" datasource="#session.dsn#">
			select
				*
			from
		...
		<cfif rsIncDesp.RecordCount gt 0 >
			...
			<cfif rsCcuentaDespensa.RecordCount eq 0 or Trim(rsCcuentaDespensa.Pvalor) eq ''>
				<cfthrow detail="No se ha configurado la Cuenta para Despensa Exenta" message="RHParametros - Nomina: ">
			</cfif>
			...
			<cfquery name="rsCcuenta" datasource="#Arguments.conexion#">
				...
				from
					CContables
				where Ccuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsCcuentaDespensa.Pvalor#">
				and Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
			</cfquery>
			...
			<cfquery datasource="#arguments.conexion#">
				...
				select 	<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#"> ,
						<cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.Ecodigo#">,
						85,
						<cfqueryparam cfsqltype="cf_sql_varchar" value="#rsCcuenta.Cformato#">,
						null, null, null, 'C', emp.CFid, emp.Ocodigo, emp.Dcodigo, 
						ROUND(sum(ic.ICmontores), 2),
						<cf_dbfunction name="now">,
						<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.Usucodigo#">,
						emp.DEid, ci.CIid, 0,
						<cfqueryparam cfsqltype="cf_sql_integer" value="#vPeriodo#">,
				...
				group by emp.CFid, emp.Ocodigo, emp.Dcodigo, emp.DEid,ci.CIid
			</cfquery>
		</cfif>
		...
		<cfquery datasource="#arguments.conexion#">
			...
			select 	
					...
					null, null, null, 'C', emp.CFid, emp.Ocodigo, emp.Dcodigo, 
					sum(a.SEliquido - coalesce(ic.ICmontores,0)) montores,
					<cf_dbfunction name="now">,
					<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.Usucodigo#">,
					emp.DEid, -1, 0,
					<cfqueryparam cfsqltype="cf_sql_integer" value="#vPeriodo#">,
      ...
      inner join DatosEmpleado de
		...
		<cfquery datasource="#arguments.conexion#" name="ultimo">
			insert into RCuentasTipo(RCNid,Ecodigo, tiporeg, cuenta, valor, Cformato, Ccuenta, tipo, CFid, Ocodigo, Dcodigo, montores, BMfechaalta, BMUsucodigo, DEid, referencia, vpresupuesto, Periodo, Mes)
			select 	
					<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#"> ,
					<cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.Ecodigo#">,
	...
	<cffunction name="CuentasTipo_DistribucionCargas" access="private" output="false">
		...
		<cfquery name="rsInsert" datasource="#arguments.conexion#">
			...
			select
				...
				c.CIid,
				a.DClinea,
				<!---coalesce(a.CEvalorpat,coalesce(b.DCvalorpat,0)) as Evalorpat, ERBG Cambio de Cargas Patronales por Oficina--->
                coalesce(co.CEvalorpat,coalesce(a.CEvalorpat,coalesce(b.DCvalorpat,0))) as Evalorpat,
				DCmetodo,
				c.ICmontores,
				0.00,
			...
			inner join CIncidentes d
				...
				and d.CIredondeo   = 0
				AND d.CItimbrar = 0 <!--- OPARRALES 2018-08-21 (Pagos en EFECTIVO) --->
			inner join  LineaTiempo lt
				on c.DEid = lt.DEid
...
<!---ERBG Cambio de Cargas Patronales por Oficina Inicia--->
            left join CargasOficina co
                on lt.Ocodigo = co.Ocodigo
                and lt.Ecodigo=co.Ecodigo
...
<!---ERBG Cambio de Cargas Patronales por Oficina Fin--->
			where coalesce(co.CEvalorpat,coalesce(a.CEvalorpat,coalesce(b.DCvalorpat,0))) > 0
		</cfquery>
