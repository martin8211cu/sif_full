...
<cfcomponent>
    ...
    <cffunction name="AddPreLiquidacionConceptos" access="public">
        ...
        <cfquery name="rsConceptos" datasource="#Arguments.conexion#">
            select <cfqueryparam cfsqltype="cf_sql_timestamp" value="#LSParseDateTime(Arguments.Fhasta)#">  as Finicio
            ...
            from ConceptosTipoAccion b, CIncidentesD c, CIncidentes d 
            where b.CIid = c.CIid
            	and c.CIid = d.CIid
    	        and b.RHTid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHTid#">
                and d.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.Ecodigo#">
        </cfquery>

        <!--- OPARRALES 2019-03-25 Modificacion para insertar linea temporal de Accion y realice los calculos correctos. --->
        <cfquery datasource="#Arguments.conexion#">
            INSERT INTO RHAcciones(DEid,RHTid,Ecodigo,Tcodigo,DLfvigencia,DLffin,RVid,RHJid,RHCPlinea,
                    Dcodigo,Ocodigo,RHPid,RHPcodigo,RHAporc,RHAporcsal,DLobs)
            select 
                top 1
                <cfqueryparam cfsqltype="cf_sql_numeric" 	value="#Arguments.DEid#">,
                <cfqueryparam cfsqltype="cf_sql_numeric" 	value="#Arguments.RHTid#">,
                ...
                Tcodigo,
                <cfqueryparam cfsqltype="cf_sql_timestamp" 	value="#rsConceptos.Finicio#">,
                <cfqueryparam cfsqltype="cf_sql_timestamp" 	value="#rsConceptos.Ffinal#">,
                RVid,
                RHJid,
                RHCPlinea,
                Dcodigo,
                ...
                LTporcplaza,
                LTporcsal,
                'ACCION TEMPORAL PARA CALCULOS DE PRE-LIQUIDACION'
            from LineaTiempo 
            where DEid = <cfqueryparam cfsqltype="cf_sql_numeric" 	value="#Arguments.DEid#">
            order by LTid desc
        ...
        <cfquery datasource="#Arguments.conexion#">
            ...
            inner join DLineaTiempo dlt
                on lt.LTid = dlt.LTid
            where 
                lt.LTid = (select max(LTid) from LineaTiempo where DEid = <cfqueryparam cfsqltype="cf_sql_numeric" 	value="#Arguments.DEid#">)
        </cfquery>
        ...
        <!--- OPARRALES 2019-03-25 Modificacion para ELIMIINAR linea temporal de Accion al realizar los calculos correctos. --->
        <cfquery datasource="#Arguments.conexion#">
            delete from RHDAcciones
            where RHAlinea = #MInfoid#
        </cfquery>
