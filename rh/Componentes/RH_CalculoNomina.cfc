...
<!---ljimenez - Llamado a la componente de calculo de renta.--->
	<cfif CalendarioPagos.CPnorenta is 0>
		<cfquery name="rsPagoSemana" datasource="#session.DSN#">
			select count(*) semanasPago from CalendarioPagos where CPid in (select CPid from CalendarioPagos 
														where MONTH(CPfpago) = (select MONTH(CPfpago) 
															from CalendarioPagos 
															where CPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RCNid#">
															and Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.Ecodigo#">) 
														and YEAR(CPfpago) = (select YEAR(CPfpago) 
														from CalendarioPagos
														where CPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RCNid#"> 
														and Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.Ecodigo#">))
			and Tcodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.Tcodigo#">
		...
		<cfinvoke component="#comp#" method="CalculoNominaRenta">
			...
			<cfinvokeargument name="PeriodosM" 			 value="#rsPagoSemana.semanasPago#">
        </cfinvoke>
	</cfif>
