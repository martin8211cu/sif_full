...
Escrito por: Giancarlo Ben�tez V.
Version: 1.0
    ...
    <cffunction name="CadenaOriginalFiniquitosLiqCFDI" returntype="string">
        ...
        <cfif len(trim('rsLF.RHLFLisptF')) and rsLF.RHLFLisptF GT 0>
         ...
         <cfif rsLF.RHLFLISPTRealL GT 0>
         	<cfset DeduccionISPTRL = "ISPT Real Liquidaci�n">
         ...
         <cfif len(trim('rsLF.RHLFLinfonavit')) and rsLF.RHLFLinfonavit GT 0>
            ...
            <cfset lVarDeducciones=#lVarDeducciones#&"|009|000000INFONAVIT|Cr�dito Infonavit|0.00|"&#importeDeduccionInfonavit#>
         </cfif>
...
<!--- INICIA - Generacion de XML para Recibo de Liquidacion --->
    <cffunction name="XML32ReciboLiqFinCFDI" acces="private" returntype="string" >
	    ...
	    <cfset TotalD = ISPT + rsLF.RHLFLinfonavit+rsSumRHLiqDeduccion.totDeduc+0.01>
        ...
        <cfset lVarFechaExpedicion  = DateFormat(now(),'yyyy-mm-ddTHH:mm:00')>
        <cfset lVarNoEmp            = datosEmpLiqFin.DEidentificacion>
         <!--- Se coloca el nombre del empleado por nombre --->
		<cfset lVarNombreEmp        = datosEmpLiqFin.NombrecompletoR>
        <cfset lVarRFCEmp           = datosEmpLiqFin.RFC>
        <cfset lVarCURP             = datosEmpLiqFin.CURP>
        <cfset lVarNoSegSocial      = datosEmpLiqFin.SeguroSocial>
        <cfset lVarMotivoBaja       = datosEmpLiqFin.MotivoBaja>
		...
		<cfset fechaEmision = trim(rsNow.fechaEmision)>
	...
	<!--- Validaci�n de TipoRegimen, si el tipo de percepci�n es = 025	el tipoRegimen debe ser 13	 --->
        <cfloop query = "rsPercepciones">
            <cfif rsPercepciones.RHCSATid eq '025'>
                <cfset _TipoRegimen="13">
            <cfelse>
                <cfset _TipoRegimen="02">
            </cfif>
        </cfloop>
    <!---FIN de Validaci�n	 --->
        ...
        <cfoutput>
			<CFXML VARIABLE="xml32">
				<Comprobante
					...
					Fecha="#fechaEmision#"
					Sello="#vSello#"
					Folio="#Trim(fnSecuenciasEscape(Arguments.DLlinea))#"
					...
					<Receptor
                        ...
                        UsoCFDI="CN01"
                        DomicilioFiscalReceptor="#trim(datosEmpLiqFin.DEcodPostal)#"
				...
				<Complemento>
					<nomina12:Nomina
						...
						<nomina12:Receptor
							...
							PeriodicidadPago="99"
                            <cfif len(trim(datosEmpLiqFin.DEcuenta)) NEQ 18>
                                Banco="#trim(datosEmpLiqFin.ClaveBanco)#"
                            </cfif>
                            CuentaBancaria="#trim(datosEmpLiqFin.DEcuenta)#"
							SalarioBaseCotApor="#LSNumberformat(datosEmpLiqFin.SDI,'_.00')#"
							SalarioDiarioIntegrado="#LSNumberformat(salarioDiario,'_.00')#"
            			...
            			<cfloop query = "rsPercepciones">
			            	<Percepcion
			            		TipoPercepcion="#trim(rsPercepciones.RHCSATid)#"
	            	    ...
	            	    <cfif TotalD gt 0>
						    <Deducciones
							    TotalOtrasDeducciones="#Abs(LSNumberFormat(TotalD-ISPT,'9.00'))#"
							    <cfif ISPT gt 0>
...
<!--- INICIO - Funciones privadas para Recibo de Liquidacion --->
    ...
    <cffunction name="fnGetdatosEmpLiqFin"  returntype="query" access="public">
        ...
        <cfquery name="rsDatosEmpLiqFin" datasource="#Session.DSN#">
            select b.DEid,
                   ...
                   {fn concat({fn concat({fn concat({fn concat(b.DEnombre, ' ')}, b.DEapellido1)}, ' ')}, b.DEapellido2)} as NombreCompletoR,
                   b.DEidentificacion,
                   b.RFC,
                   b.CURP,
                   b.DEsdi as SDI,
                   ...
                   b.DEcodPostal,
                   bnc.Iaba as ClaveBanco,
                   b.DEcuenta,
                   bnc.Bdescripcion as NomBanco,
        ...
        <cfif Trim(rsDatosEmpLiqFin.DEcodPostal) eq ''>
			<cfthrow detail="No se ha configurado el C�digo Postal para el empleado #rsDatosEmpLiqFin.NombreCompletoR#" message="DATOS EMPLEADO: ">
		</cfif>
        
        <cfif Trim(rsDatosEmpLiqFin.NomBanco) eq ''>
			<cfthrow detail="No se ha configurado el Banco para el empleado #rsDatosEmpLiqFin.NombreCompletoR#" message="DATOS EMPLEADO: ">
		</cfif>
        
        <cfif Trim(rsDatosEmpLiqFin.DEcuenta) eq ''>
			<cfthrow detail="No se ha configurado la Cuenta Bancaria para el empleado #rsDatosEmpLiqFin.NombreCompletoR#" message="DATOS EMPLEADO: ">
		</cfif>
    </cffunction>
    	 ...
    	 <cfargument name="DLlinea" 		type="numeric" required="yes">
            ...
            <cfquery name="rsPercepciones" datasource="#Session.DSN#">
                select
	                ...
	                Round(a.RHLIexento,2) as RHLIexento,
	                ...
	                Round(a.RHLIgrabado,2) as RHLIgrabado,
					<!---ERBG Parte ISPT Exenta FIN--->
    ...
    <cffunction name="fnGetTotalDeducciones" returntype="string" access="public">
        ...
        <cfelse>
           	...
           	<cfset TotalD = "#LSNumberformat(ISPT,'9.00')#">
