<!---
	...
	Componente desarrollado para la regeracion del complemento de pago.
                                ...
                                <cfloop query="ReciboPago">
									<cfquery name="rsDocRel" datasource="#session.dsn#">
                                        SELECT
                                            ...
                                            AND RTRIM(LTRIM(d.Ddocumento)) = '#TRIM(ReciboPago.Ddocumento)#'
                                            AND t.CCTtipo = 'D'
						            </cfquery>
                                    <cfif isdefined('rsDocRel') and rsDocRel.RecordCount GT 0>
                                        ...
                                        <pago10:DoctoRelacionado IdDocumento = "#ReciboPago.TimbreFiscal#"
                                            ...
                                            <cfif isdefined('rsDocRel') and rsDocRel.RecordCount GT 0>
                                                ImpSaldoAnt="#NumberFormat(lVarPtotal + lVarImportePagado,'0.00')#"
                                                <cfif lVarPtotal GT 0>
                                                    ImpSaldoInsoluto="#NumberFormat((lVarPtotal),'0.00')#"
                                                <cfelse>
                                                    ImpSaldoInsoluto="#NumberFormat((lVarPtotal),'0.00')#"
                                                </cfif>
                                            </cfif>
