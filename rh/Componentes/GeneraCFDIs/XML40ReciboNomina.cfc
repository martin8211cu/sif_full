...
Escrito por: Giancarlo Benitez V.
Version: 1.0
...
<!---<cf_dump  var="#rsReporte#">--->
        <cfloop query="rsReporte">
            ...
            <cfif len(trim(#rsReporte.Nombre#))>
                <cfset vsNombre =trim(#rsReporte.Nombre#) &' '&  #vsNombre#>
            </cfif>
            ...
            <cfif Trim(rsReporte.ClaveBanco) eq ''>
				<cfthrow detail="No se ha configurado la clave del Banco para el empleado #vsNombre#" message="DATOS EMPLEADO: ">
			</cfif>

            <cfset rsSalarioEmpleado = GetSalarioEmpleado(rsReporte.DEid,rsReporte.IdNomina)>
            <cfset rsVacacionesEmpleado = GetVacacionesEmpleado(rsReporte.DEid,rsReporte.IdNomina)>
            ...
            <cfset PercepcionesGrabFaltas  = 0>
            <cfloop query="rsPercepciones">
                <cfif rsPercepciones.ImporteGravado lt 0>
                    <cfset PercepcionesGrabFaltas += rsPercepciones.ImporteGravado>
                </cfif>
            ...
            <cfquery name="rsOtrosPagosDeducciones" datasource="#session.dsn#">
                select Clave,Concepto,DEid,ABS(ImporteExento) as VImporteExento,ImporteGravado,
                case when TipoDeduccion = '071' then 1 else OtroPago end as OtroPago,RCNid,
                case when TipoDeduccion = '071' then '999' else TipoDeduccion end as TipoDeduccion
                from #Deducciones# 
                where ((OtroPago = 0 and TipoDeduccion in ('001')) 
                    or (OtroPago = 1 and TipoDeduccion in ('001','005','007','008'))
                    or (OtroPago = 0 and TipoDeduccion in ('071') and ImporteExento < 0))
            </cfquery>
            ...
            <cfquery name="rsDeducciones" datasource="#session.dsn#">
                select Clave,Concepto,DEid,ImporteExento,ImporteGravado,OtroPago,RCNid,TipoDeduccion 
                from #Deducciones# 
                where ((OtroPago = 0 and TipoDeduccion not in ('001','071')) 
                    or (OtroPago = 1 and TipoDeduccion not in ('001','005','007','008'))
                    or (OtroPago = 0 and TipoDeduccion in ('071') and ImporteExento > 0))
            </cfquery>

            <!--- SML27102022 Se actualiza el TipoDeduccion '071' a '999' para que se pueda timbrar y aparezca en otros pagos---> 
            <cfquery datasource="#session.dsn#">
                update #Deducciones# 
                set TipoDeduccion = '999',
                    OtroPago = 1
                where OtroPago = 0 and TipoDeduccion in ('071') and ImporteExento < 0
            </cfquery>

            <!---<cfdump var="#rsOtrosPagosDeducciones#">
            <cfdump var="#rsDeducciones#">
            <cf_dumptable var="#Deducciones#">--->
       ...
       <!---     <cfset vTasa = 0>
        ...
        --->
            ...
            <cfset rsSalarioSATcod = GetSalarioSATCod( (rsFactorNomina.TRegimen eq '09') ? '046' : '001' )>
            ...
            <!--- 
                <cfset diasAguinaldo = fnGetDato(2052).Pvalor>
                <cfset varSD = varSD + (varSD*((diasAguinaldo neq "")? diasAguinaldo : 1))/365> 
            --->
            <!--- Obtenemos el folio --->
			<cfquery name="rsFolio" datasource="#session.dsn#">
				select
                    <cfif Arguments.Retimbrar>
                        concat(PElinea,9999) as Folio
                    <cfelse>
                        PElinea as Folio
                    </cfif>
				from
            ...
            <Comprobante
				...
				Sello="#vSello#"
				Folio="#Trim(fnSecuenciasEscape(rsReporte.CodigoNomina))#-#rsFolio.Folio#"
                ...
                <Emisor	
                Rfc="#trim(rsLugarExpedicion.Eidentificacion)#" 
                Nombre="#trim(fnSecuenciasEscape(rsLugarExpedicion.Enombre))#"
				RegimenFiscal="#trim(rsLugarExpedicion.codigo_RegFiscal)#"/>
					<!---<cf_dump  var="#trim(vsNombre)#"> RegimenFiscal="#trim(rsLugarExpedicion.codigo_RegFiscal)#"/>  <DomicilioFiscal pais="#trim(fnSecuenciasEscape(rsLugarExpedicion.Pais))#"/>--->
                
				<Receptor
					Rfc="#trim(fnSecuenciasEscape(rsReporte.RFC))#"
					Nombre="#trim(vsNombre)#"
                    UsoCFDI="CN01"
                    DomicilioFiscalReceptor="#trim(rsReporte.DEcodPostal)#"
				...
				<!--- <Impuestos
				 ...
				 <cfquery name="rsGetTipoCalendarioPagos" datasource="#session.dsn#">
                            SELECT CPTipo
                            FROM CalendarioPagos
                            WHERE Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
                            AND CPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RCNid#">
                </cfquery>
                <cfif trim(rsGetTipoCalendarioPagos.CPTipo) EQ 0> <!--- 0: Nómina Normal --->
                    <cfset diasPagados="#(rsReporte.CPTipoCalRenta eq 2 ? LSNumberFormat(varDiasAguinaldo,'9.000') : LSNumberFormat(lVarDiasCP,'9.000'))#">
                <cfelse>
                    <cfset diasPagados = '1'>
                </cfif>
				<Complemento>
					<nomina12:Nomina
                        ...
                        <!---"#(rsReporte.CPTipoCalRenta eq 2 ? LSNumberFormat(varDiasAguinaldo,'9.000') : LSNumberFormat(lVarDiasCP,'9.000'))#"--->
						<cfif totalPercepc gt 0>
							TotalPercepciones="#LSNumberFormat(totalPercepc,'_.00')#"
						</cfif>
						<cfif sumTotDeduc gt 0>
                    ...
                    >
                        <nomina12:Emisor
                            <cfif listFind('601,603,607,609,620,622,623,624,628',rsLugarExpedicion.codigo_RegFiscal) eq 0>
                                <cfset curpPFisica 	= fnGetDato(162)>
                                Curp="#curpPFisica.Pvalor#"																			<!--- CURP Persona Fisica --->
                            </cfif>
                            RegistroPatronal="#trim(fnSecuenciasEscape(rsReporte.RegistroPatronal))#">
                        </nomina12:Emisor>
                        <cfif rsFactorNomina.TRegimen neq '09'>
                        </cfif>

                       

                        <cfif trim(rsGetTipoCalendarioPagos.CPTipo) EQ 0> <!--- 0: Nómina Normal --->
                            <cfset lVarPeriodicidad = trim(rsReporte.PeriodicidadPago)>
                        <cfelse>
                            <cfset lVarPeriodicidad = '99'>
                        </cfif>
                        <nomina12:Receptor
                            ...
                            PeriodicidadPago="#lVarPeriodicidad#"
                            <cfif len(trim(rsReporte.DEcuenta)) NEQ 18>
                            Banco="#trim(rsReporte.ClaveBanco)#"
                            </cfif>
                            CuentaBancaria="#trim(rsReporte.DEcuenta)#"
                            <cfif rsFactorNomina.TRegimen neq '09'>
                                ...
                                SalarioBaseCotApor="#LSNumberformat(rsReporte.SalarioDiarioIntegrado,'_.00')#"
                                SalarioDiarioIntegrado="#LSNumberformat(varSD,'_.00')#"
                            </cfif>
        ...
        <!---Banco="#trim(rsReporte.Banco)#"--->
                        ...
                        >
                            <!--- EGS: Se agrega validación porque la operación (rsSalarioEmpleado.SEsalariobruto + PercepcionesGrabFaltas - rsVacacionesEmpleado.PEmontores)
                                da como resultado 0, al enviarse a timbrar el 0 no está permitido.
                                Error: 	[{"message":"La suma de los importes de los atributos ImporteGravado e ImporteExento no es mayor que cero.","code":"NOM75","additionalInfo":null,"field":null}]
                                 ---->
                            <cfif rsSalarioEmpleado.SEsalariobruto gt 0 AND (rsSalarioEmpleado.SEsalariobruto + PercepcionesGrabFaltas - rsVacacionesEmpleado.PEmontores) GT 0>
                                <nomina12:Percepcion
                                    ...
                                    ImporteGravado="#LSNumberformat(rsSalarioEmpleado.SEsalariobruto + PercepcionesGrabFaltas - rsVacacionesEmpleado.PEmontores ,'_.00')#"
                                    ImporteExento="0.00"
                                />
                            </cfif>
                            <cfif rsVacacionesEmpleado.PEmontores gt 0>
                                <nomina12:Percepcion
                                    TipoPercepcion="001"
                                    Clave="00000000001"
                                    Concepto="Vacaciones"
                                    ImporteGravado="#LSNumberformat(rsVacacionesEmpleado.PEmontores,'_.00')#"
                                    ImporteExento="0.00"
                                />
                            </cfif>

                            <cfloop query = "rsPercepciones">
                                <cfif rsPercepciones.ImporteGravado gt 0 or rsPercepciones.ImporteExento gt 0>
                                    <Percepcion
                        ...
                        <!---VALIDACION DE DEDUCCIONES --->
                        <cfset rsValDeduccion = GetValDeduccionISR(Arguments.DEid,Arguments.RCNid,session.Ecodigo)>
                        ...
                        <!--- <cfif rsReporte.CPTipoCalRenta neq 2> --->
                            ...
                            <Deducciones
                                ...
                                >
                                <cfif rsSalarioEmpleado.ImporteExento neq 0.00>
                                    <cfset rsValDeduccion = ValDeduccionISR(session.Ecodigo)>	
                                    <Deduccion
                                        TipoDeduccion="#rsValDeduccion.RHCSATcodigo#"
                            ...
                            <nomina12:OtrosPagos>
                                <cfif (isdefined("rsOtrosPagos") and rsOtrosPagos.RecordCount gt 0)>
                                    ...
                                    <cfloop query="rsOtrosPagos">
                                        <cfif rsFactorNomina.TRegimen neq '09' and  listFind('002,999', trim(rsOtrosPagos.TIPOPERCEPCION)) gt 0>
                                            <nomina12:OtroPago
                                            ...
                                            >
                                                <!--- Validacion en base a la clave de catalogos del SAT OtrosPagos --->
                                                <cfif listFind('002', trim(rsOtrosPagos.TIPOPERCEPCION)) gt 0>
                                            ...
                                            <cfif listFind('999', trim(rsOtrosPagos.TIPOPERCEPCION)) gt 0>
	                                            <nomina12:OtroPago
                                ...
                                <cfelse>
                                    <cfif rsFactorNomina.TRegimen neq '09'>
                                        <cfset listaOtroPagos = 0>
                                        <cfloop query ="rsOtrosPagosDeducciones">
                                            <cfif listFind('007', trim(rsOtrosPagosDeducciones.TipoDeduccion)) gt 0>
                                                <cfset listaOtroPagos = listaOtroPagos + 1>
                                            </cfif>
                                        </cfloop>

                                        <cfif listaOtroPagos eq 0>
	                                        <nomina12:OtroPago
	                                            Clave="000000000000009"
	                                            Concepto="Subsidio al Empleo"
	                                            Importe="#LSNumberFormat(0,'_.00')#"
				...
				<cfif rsSaldosAFavor.RecordCount gt 0>
					...
					<cfloop query = "rsOtrosPagosDeducciones">
	                                    <nomina12:OtroPago 
	                                        TipoOtroPago="#trim(rsOtrosPagosDeducciones.TipoDeduccion)#" 
	                                        Clave="#trim(rsOtrosPagosDeducciones.Clave)#" 
	                                        Concepto="#trim(rsOtrosPagosDeducciones.Concepto)#" 
	                                        Importe="#LSNumberformat(rsOtrosPagosDeducciones.VImporteExento,'_.00')#" >
									    </nomina12:OtroPago>
					</cfloop>
                                </cfif>
                        ...
                        <cfelse>
                            <cfif rsFactorNomina.TRegimen neq '09'>
                                <cfset var_SubsidioCausado = getSubsidioCausado(RCNid=Arguments.RCNid,DEid=arguments.DEid)>
                                <nomina12:OtrosPagos>
                                    <nomina12:OtroPago
                                        Clave="000000000000009"
                                        Concepto="Subsidio al Empleo"
                                        Importe="#LSNumberFormat(0,'_.00')#"
                                        TipoOtroPago="002">
                                        <nomina12:SubsidioAlEmpleo SubsidioCausado="#LSNumberFormat(var_SubsidioCausado,'_.00')#"/>
                                    </nomina12:OtroPago>	
                                </nomina12:OtrosPagos>
                            </cfif>							
                        </cfif>
        ...
        <cfset xml32 = replace(#xml32#,"Antiguedad","Antigüedad")>
        ...
        <!--- <cfset <cf_dump  var="#xml32#"> xml32 = cleanXML(xml32)> --->
        <cfreturn xml32>
     </cffunction>
...
<!--- INICIA - Funciones privadas para Recibo de Nomina --->
    ...
    <cffunction name="GetVacacionesEmpleado" access="public" returntype="query">
        <cfargument name="DEid" 		type="numeric" required="yes">
        <cfargument name="RCNid" 		type="numeric" required="yes">

		<cfquery name="rsVacacionesEmpleado" datasource="#Session.DSN#">
            select isnull(sum(hpe.PEmontores),0) as PEmontores
            from HPagosEmpleado hpe
            inner join RHTipoAccion ta on hpe.RHTid = ta.RHTid and ta.RHTcomportam = 3
            where hpe.RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RCNid#">
              and hpe.DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.DEid#">
        </cfquery>
        <cfreturn rsVacacionesEmpleado>
    </cffunction>

    <cffunction name="GetSalarioSATCod" access="public" returntype="query">
        <cfargument  name="satCodigo" required="false" default="001">

        <cfquery name="rsDatosSalarioSATcod" datasource="#Session.DSN#">
            select right(('000000000000000'+cs.CScodigo),15) as CScodigo, cs.CSdescripcion, cSAT.RHCSATcodigo 
            ...
            inner join dbo.RHCFDIConceptoSAT cSAT
                ...
                and cs.Ecodigo = #session.Ecodigo#
            where cSAT.RHCSATcodigo='#arguments.satCodigo#'
        </cfquery>
    ...
    <cffunction name="GetInfoNomina" access="public" returntype="query">
        ...
        <cfquery datasource="#session.dsn#" name="rsValidaBanco">
            select * from DatosEmpleado a
            inner join Bancos bnc
                on bnc.Bid=a.Bid
            inner join CEBancos bnc2
                on bnc2.Id_Banco=bnc.CEBSid
         ...
         <cfif rsValidaBanco.RecordCount eq 0>
			<cfthrow detail="El banco del empleado no tiene configurado el Código SAT" message="DATOS BANCO: ">
		</cfif>
        ...
        <cfif Trim(rsunEmp.DEcuenta) eq ''>
			<cfthrow detail="El número de cuenta no se ha configurado para el empleado #rsunEmp.NombreEmp#" message="DATOS EMPLEADO: ">
		</cfif>
        <cfif Trim(rsunEmp.DEcodPostal) eq ''>
			<cfthrow detail="El Código Postal no se ha configurado para el empleado #rsunEmp.NombreEmp#" message="DATOS EMPLEADO: ">
		</cfif>
        ...
        <cf_dbtemp name="salida" returnvariable="salida">
            ...
            <cf_dbtempcol name="DEcodPostal"	type="varchar(50)"     mandatory="no">
            <cf_dbtempcol name="DEcuenta"		type="char(25)"     mandatory="no">
            <cf_dbtempcol name="ClaveBanco"	    type="char(25)"     mandatory="no">
        ...
        <cfquery datasource="#session.dsn#" name="rsEmpleados">
		 ...
		 <cfquery datasource="#session.dsn#" name="rsEmpleados">
            ...
            TipoJornada,PeriodicidadPago,SalarioBaseCotApor,
			RiesgoPuesto,SalarioDiarioIntegrado,Mcodigo,
			TipoNomina,DESindicalizado,TipoRegimenDesc,CodigoNomina,RegistroPatronalO,fechaEmision,CPTipoCalRenta,OficinaD,Tcodigo,DEcodPostal,DEcuenta,ClaveBanco)
           ...
           <!---     (select top(1) lt.LTsalarioSDI
            ...
            --->
				...
				,b.Tcodigo,a.DEcodPostal,a.DEcuenta,bnc2.Clave
            from DatosEmpleado a
				inner join CSATTiposContrato tc
				on tc.TCid = a.DEtipocontratacion
                left join LineaTiempo b
	...
	<!---VALIDACIÓN DE DEDUCCIONES CUANDO NO HAY CONFIGURACIÓN EN CONCEPTO DE SAT en TIPO DE DEDUCCIÓN --->
    <cffunction name="GetValDeduccionISR" access="public" returntype="query">
        <cfargument name="DEid" 		type="numeric" required="yes">
        <cfargument name="RCNid" 		type="numeric" required="yes">
        <cfargument name="Ecodigo"      type="numeric" required="yes">

        <cfquery name="rsvalTDeduccion" datasource="#session.dsn#" >
            select  DCvalor as Valor,RHCSATcodigo as Codigo,CI.RHCSATid,CI.TDdescripcion
            from HSalarioEmpleado sa
            inner join HDeduccionesCalculo hic
            on sa.DEid=hic.DEid and sa.RCNid=hic.RCNid
            inner join DeduccionesEmpleado de
            ...
            where sa.RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RCNid#">
              and sa.DEid =  <cfqueryparam cfsqltype="cf_Sql_numeric" value="#Arguments.DEid#">
              and de.Ecodigo=<cfqueryparam cfsqltype="cf_Sql_numeric" value="#Arguments.Ecodigo#">

        </cfquery>
        <cfif rsvalTDeduccion.RecordCount GT 0>
            <cfif rsvalTDeduccion.RHCSATid eq 0>
                    <cfthrow message="La deducción #rsvalTDeduccion.TDdescripcion# no tiene configurado el concepto de SAT" type="Application">
            </cfif>
        </cfif>
        <cfreturn rsvalTDeduccion>
    </cffunction>

        <cffunction name="ValDeduccionISR" access="public" returntype="query">
            <cfargument name="Ecodigo"      type="numeric" required="yes">
	
		    <cfquery name="rsTDeduccionISR" datasource="#session.dsn#">
                select CI.RHCSATid,TDdescripcion,isnull(RHCSATcodigo,'0') RHCSATcodigo from TDeduccion CI 
			    left join RHCFDIConceptoSAT cs 
                     on CI.RHCSATid = cs.RHCSATid 
                     and CI.Ecodigo=cs.Ecodigo
                where CI.Ecodigo=<cfqueryparam cfsqltype="cf_Sql_numeric" value="#Arguments.Ecodigo#">
                      and TDdescripcion='ISR'
		    </cfquery>

            <cfif rsTDeduccionISR.RecordCount GT 0>
                <cfif rsTDeduccionISR.RHCSATid eq 0 OR rsTDeduccionISR.RHCSATcodigo eq 0>
                    <cfthrow message="La deducción #rsTDeduccionISR.TDdescripcion# no tiene configurado el concepto de SAT" type="Application">
                </cfif>
            <cfelse>
                <cfthrow message="No existe la deducción #rsTDeduccionISR.TDdescripcion# en el catálogo de Tipo de Deducciones" type="Application">
            </cfif>
		    <cfreturn rsTDeduccionISR>
	    </cffunction>
	<!---Fin de validaciÃ³n --->
