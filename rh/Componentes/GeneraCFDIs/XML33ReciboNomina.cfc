...
<!--- INICIA - Generacion de CadenaOriginal para Recibo de Nomina --->
    <cffunction name="CadenaOriginalReciboNominaCFDI"  access="public" returntype="string">
        ...
        --->
            ...
            "|"&"#rsSalarioSATcod.RHCSATcodigo#"&																	<!---Tipo Percepciï¿½n--->
            "|"&"#trim(rsSalarioSATcod.CScodigo)#"&																	<!---Clave Percepciï¿½n--->
            "|"&"#trim(rsSalarioSATcod.CSdescripcion)#"&															<!---Descripciï¿½n Percepciï¿½n--->
            "|"&"#LSNumberformat(rsSalarioEmpleado.SEsalariobruto,'_.00')#"&										<!---Importe Gravado Percepciï¿½n--->
            "|"&"0.00">																								<!---Importe Exento Percepciï¿½n--->
            <cfloop query = "rsPercepciones">
                <cfset vCadenaOriginal = #vCadenaOriginal#&
                "|"&"#trim(rsPercepciones.TipoPercepcion)#"& 														<!---Tipo Percepciï¿½n--->
                "|"&"#trim(rsPercepciones.Clave)#"&																	<!---Clave Percepciï¿½n--->
                "|"&"#trim(rsPercepciones.Concepto)#"&																<!---Concepto Percepciï¿½n--->
                "|"&"#LSNumberformat(rsPercepciones.ImporteGravado,'_.00')#"&										<!---Importe Gravado Percepciï¿½n--->
                "|"&"#LSNumberformat(rsPercepciones.ImporteExento,'_.00')#">										<!---Importe Exento Percepciï¿½n--->
            </cfloop>
			...
			<cfif rsSalarioEmpleado.ImporteExento neq 0.00>
				...
				"|"&"002"&																								<!---Tipo Deducciï¿½n--->
	            "|"&"#right(('000000000000000'&trim(LB_Renta)),15)#"&													<!---Clave Deducciï¿½n--->
            ...
            <cfloop query = "rsCargasEmpl">
                <cfset vCadenaOriginal = #vCadenaOriginal#&
                "|"&"#trim(rsCargasEmpl.TipoDeduccion)#"& 															<!---Tipo Deducciï¿½n--->
                "|"&"#trim(rsCargasEmpl.Clave)#"&																	<!---Clave Deducciï¿½n--->
                "|"&"#trim(rsCargasEmpl.Concepto)#"&																<!---Concepto Deducciï¿½n--->
                "|"&"#LSNumberformat(rsCargasEmpl.ImporteGravado + rsCargasEmpl.ImporteExento,'_.00')#">			<!--- Importe--->
            </cfloop>
            <cfloop query = "rsDeducciones">
                <cfset vCadenaOriginal = #vCadenaOriginal#&
                "|"&"#trim(rsDeducciones.TipoDeduccion)#"& 															<!---Tipo Deducciï¿½n--->
                "|"&"#trim(rsDeducciones.Clave)#"&																	<!---Clave Deducciï¿½n--->
                "|"&"#trim(rsDeducciones.Concepto)#"&																<!---Concepto Deducciï¿½n--->
    ...
    <cffunction name="XML32ReciboNominaCFDI" access="public" returntype="string">
        ...
        <cfloop query="rsReporte">
            ...
            <cfquery name="rsOtrosPagosDeducciones" datasource="#session.dsn#">
                select ABS(ImporteExento) as VImporteExento,* from #Deducciones# where OtroPago = 1 and TipoDeduccion in ('001','005','007','008')
            </cfquery>
		...
		where (OtroPago = 0 and TipoDeduccion not in ('001')) 
                    and (OtroPago != 1 <!--- and TipoDeduccion not in ('005')--->)
            </cfquery>
...
<!--- INICIA - Funciones privadas para Recibo de Nomina --->
	...
	<!---VALIDACIï¿½N DE DEDUCCIONES CUANDO NO HAY CONFIGURACIï¿½N EN CONCEPTO DE SAT en TIPO DE DEDUCCIï¿½N --->
