...
<cfcomponent>
	<cfif Not IsDefined('Request.HEAD_PLISTA1_JS')>
		...
		<script type="text/javascript" language="JavaScript" src="/cfmx/rh/js/pLista1.js"></script>
	</cfif>
	<cf_templatecss />
...
<cffunction name="pListaRH" access="public" returntype="string" output="true">
	...
	<cfargument name="TranslateDataCols" 		type="string" default="" required="no"><!----- este argumento se agrega para deteminar cuales columnas utilizan traduccion de datos--->
	<cfargument name="funcion_cambioPagina_ajax" type="string" default="" required="no"> <!--- Funcion que se utilizara en el callback del query con ajax luego de cambiar de pagina   --->
    ...
    <cfelse>
        ...
        <cfset LvarMaxRowsQuery 		= 9999>
    </cfif>		
	<cfif mostrar_filtro and filtrar_automatico>
		<!--- FILTRO AUTOMATICO --->
		<cfquery name="rsLista" result="rsResult" datasource="#Conexion#">
			<cf_dbrowcount1 rows="#LvarMaxRowsQuery#" datasource="#Conexion#">
			...
			<cfloop from="1" to="#ArrayLen(arDesplegar)#" index="i">
				<cfif     IsDefined('form.filtro_'&Trim(arDesplegar[i]))>
					<cfset temp_value="#HTMLEditFormat(form['filtro_'&Trim(arDesplegar[i])])#">
				<cfelseif IsDefined('url.filtro_' &Trim(arDesplegar[i]))>
					<cfset temp_value="#HTMLEditFormat(url ['filtro_'&Trim(arDesplegar[i])])#">
				<cfelse>
					<cfset temp_value="">
				</cfif>
				<cfif     IsDefined('form.hfiltro_'&Trim(arDesplegar[i]))>
					<cfset htemp_value="#HTMLEditFormat(form['hfiltro_'&Trim(arDesplegar[i])])#">
				<cfelseif IsDefined('url.hfiltro_' &Trim(arDesplegar[i]))>
					<cfset htemp_value="#HTMLEditFormat(url ['filtro_'&Trim(arDesplegar[i])])#">
				<cfelse>
					<cfset htemp_value="">
				</cfif>
				<cfset x = Replace(arFiltrarPor[i],"''","'","all")>
				<cfif (temp_value neq htemp_value) and ((len(trim(temp_value))) 
						or len(trim(htemp_value)))>
					<cfset Arguments.filtro_nuevo = true>
				</cfif>
				<cfif len(trim(temp_value))>
					...
					<cfif Arraylen(arFormatos)>
						...
						<cfswitch expression="#LvarFormato#">
							<cfcase value="C">
								<cfswitch expression="#LvarFormatoCombo#">
									...
									<cfcase value="M,P,R,N,F,I">
                                    	<cfif temp_value neq "" and  temp_value neq -1 and  temp_value neq -1.0 and temp_value neq "-1">
							...
							<cfcase value="L">
								and #PreserveSingleQuotes(x)# = <cfqueryparam cfsqltype="cf_sql_bit" value="#temp_value#">
							</cfcase>
							<cfcase value="I">
								and #PreserveSingleQuotes(x)# >= <cfqueryparam cfsqltype="cf_sql_money" value="#Replace(trim(temp_value), ',', '','all')#">
							</cfcase>
							...
							<cfcase value="S">
								<cfif isdefined("arguments.TranslateDataCols") and len(trim(arguments.TranslateDataCols)) gt 0 and listfindnocase(trim(arguments.TranslateDataCols),trim(x))>
									<cf_translatedata name="get" tabla="#PreserveSingleQuotes(tabla)#" col="#PreserveSingleQuotes(x)#" returnvariable="x">
								</cfif>
								and upper(#PreserveSingleQuotes(x)#) 
                                like  <cfqueryparam cfsqltype="cf_sql_char" value="%#Ucase(Trim(temp_value))#%">
							</cfcase>
							<cfdefaultcase>
                                and upper(rtrim(<cf_dbfunction name="to_char" args="#PreserveSingleQuotes(x)#" datasource="#Conexion#" isNumber="no">)) 
                                like  <cfqueryparam cfsqltype="cf_sql_char" value="%#Ucase(Trim(temp_value))#%">
							</cfdefaultcase>
						</cfswitch>
					<cfelse>
			...
			<cf_dbrowcount2 rows="#LvarMaxRowsQuery#" datasource="#Conexion#">
		</cfquery>
	<cfelse>
		<!--- QUERY --->
		...
		<cfquery name="rsLista" result="rsResult" datasource="#Conexion#">
			...
			<cfif NOT Find('ORDER BY',UCASE(filtro))>
				<cf_dbrowcount2 rows="#LvarMaxRowsQuery#" datasource="#Conexion#">
			</cfif>
		</cfquery>
		<cfif debug EQ "S">			
	...
	<cfinvoke component="pListas" method="pListaQuery" returnvariable="pListaRet">
			...
			<cfinvokeargument name="funcion_cambioPagina_ajax" 			value="#Arguments.funcion_cambioPagina_ajax#">
...
<cffunction name="pListaQuery" access="public" returntype="string" output="true">
	...
	<cfargument name="FilasParaBotonesSuperiores" type="numeric" default="25" required="no">
	<cfargument name="funcion_cambioPagina_ajax" type="string" default="" required="no">
	<!--- **************************************************************************************** --->
	...
	<cfif Arguments.usaAJAX>
		<cfif NOT isdefined("request.Plista_AJAX")>
			<script type='text/javascript'>_ajaxConfig = {'_cfscriptLocation':'/cfmx/rh/Componentes/pListasAJX.cfc', '_jsscriptFolder':'/cfmx/rh/Componentes/ajax_js'};</script>
			<script type='text/javascript' src='/cfmx/rh/Componentes/ajax_js/ajax.js'></script>
			<script language="javascript">
				...
				function fnPlistas_conAjax(Automatic)
				{
					...
					{
						document.getElementById("ifrPlistas_conAjax").src = "/cfmx/rh/Componentes/pListasAJX.cfc?METHOD=updateTS&IDs=" + escape(Plistas_conAjax);
		...
		<script type="text/javascript">
			...
			function doQuery_Plista#Arguments.PageIndex#(PlistaPage)
			{
				...
				DWREngine._execute(_ajaxConfig._cfscriptLocation, null, 'doQuery', '#LvarPlistaID#|'+Plistas_conAjax , PlistaPage, #Session.Usucodigo#, doQueryResults_Plista#Arguments.PageIndex#);
			...
			function doQueryResults_Plista#Arguments.PageIndex# (html) {
				...
				try{
					fnExistElement('div_Plista#Arguments.PageIndex#').innerHTML = html;
				} catch (e) {
...
<!---
	<cfset LvarParametrosJSON = createobject("component","rh.Componentes.json").encode(arguments.parametros)>
--->	
	...
	<cflock scope="application" throwontimeout="yes" timeout="5000">
		...
		<cfquery name="rsSQL" datasource="sifcontrol">
					INSERT INTO Plista (PlistaPRM, PlistaTS, Usucodigo) 
					VALUES(
						 <cfqueryparam cfsqltype="cf_sql_clob" value="#LvarParametrosJSON#">
						,<cf_dbfunction name="now" datasource="sifcontrol">
						,#session.Usucodigo#
			) 
			...
			<cfcatch type="any">
				<cfquery name="rsSQL" datasource="sifcontrol">
					INSERT INTO Plista (PlistaPRM, PlistaTS, Usucodigo) 
					VALUES(
						 <cf_jdbcquery_param cfsqltype="cf_sql_clob" value="#LvarParametrosJSON#">
						,<cf_dbfunction name="now" datasource="sifcontrol">
				,#session.Usucodigo#
			)
			<cf_dbidentity1 name="rsSQL" datasource="sifcontrol" returnvariable="LvarID" verificar_transaccion="no">
		...
		<cfquery name="rsSQL" datasource="asp">
			select count(1) as cantidad
			 ...
			 WHERE SScodigo	= 'sys'
			   AND SMcodigo	= 'public'
			   AND SPcodigo	= 'tags'
			   AND SCuri	= '/rh/Componentes/pListasAJX.cfc'
		</cfquery>
		<cfif rsSQL.cantidad EQ 0>
			<cfquery name="rsSQL" datasource="asp">
				insert into SComponentes 
					(SScodigo, SMcodigo, SPcodigo, SCuri, SCtipo, SCauto, BMfecha, BMUsucodigo)
				values ('sys', 'public', 'tags', '/rh/Componentes/pListasAJX.cfc', 'P', 0, {fn Now()}, 1)
			</cfquery>
			<cfquery name="rsSQL" datasource="asp">
				insert into SComponentes 
					(SScodigo, SMcodigo, SPcodigo, SCuri, SCtipo, SCauto, BMfecha, BMUsucodigo)
				values ('sys', 'public', 'tags', '/rh/Componentes/ajax.cfc', 'P', 0, {fn Now()}, 1)
			</cfquery>
		</cfif>
	</cfcatch>
	</cftry>
