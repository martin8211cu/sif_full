...
<cfcomponent>
	...
	<cffunction name="Parte2" access="private" output="false">
		...
		<cfargument name="debug" 		type="boolean" 	required="no" default="false">
            ...
            <!--- SML. Inicio Modificacion para que en DLaboralesEmpleado aparezca el cambio de Salario--->
       		<cfquery name="AccionMasiva" datasource="#Arguments.conexion#">
				select RHAid
				from RHAcciones
        	   	where RHAlinea = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHAlinea#">
			</cfquery>

        	<cfif isdefined('AccionMasiva') and AccionMasiva.RecordCount GT 0 and len(trim(AccionMasiva.RHAid)) GT 0>
        		<cfquery name="TipoAccionMasiva" datasource="#Arguments.conexion#">
					select b.RHTid from RHAccionesMasiva a
						inner join RHTAccionMasiva b
						on a.RHTAid = b.RHTAid and a.Ecodigo = b.Ecodigo
					where a.RHAid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#AccionMasiva.RHAid#">
						and a.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
				</cfquery>
        	</cfif>
        <!--- SML. Final Modificacion para que en DLaboralesEmpleado aparezca el cambio de Salario--->
			...
			<cfquery name="insDLaboralesEmpleado" datasource="#Arguments.conexion#">
				...
				select
					...
					a.DEid,
                    <cfif isdefined('TipoAccionMasiva') and TipoAccionMasiva.RecordCount GT 0>
                    <cfqueryparam cfsqltype="cf_sql_numeric" value="#TipoAccionMasiva.RHTid#"> as RHTid,
                    <cfelse>
                    a.RHTid,
                    </cfif>
					<cfif dataTipoAccion.RHTcomportam EQ 9 and dataTipoAccion.RHTcempresa EQ 1>
		...
		<cfargument name="debug" 		type="boolean" 	required="no" default="false">
			...
			<cfif dataTipoAccion.RHTcomportam EQ 1 OR dataTipoAccion.RHTcomportam EQ 3 >
				...
				<!--- OPARRALES 2018-05-23
					- Cuando la accion sea del Tipo Comportamiento Vacaciones
					- Se agrega validacion asignar variable en session que permita enviar correos a usuarios del Rol RHNotify
					- En las validaciones del WorkFlow en el Componente "ActivityMgmt.cfc" en la funcion "notifyActivityStateChange"
				 --->
				<cfif dataTipoAccion.RHTcomportam EQ 3>
					<cfset session.CorreoVacaciones = true>
				</cfif>
	...
	<cffunction name="AplicaAccion" access="public" output="true" returntype="boolean">
		...
		<cfif RHTespecial eq  0 >
			...
			<cfif #vUsaSBC# EQ 1 >
				...
				<cfif dataTipoAccion.RHTcomportam EQ 1 OR dataTipoAccion.RHTcomportam EQ 6> <!---Nombramiento--->
					<cfinvoke component="rh.Componentes.RH_CalculoSDI" method="fnSDINombramiento" returnvariable="vSDI">
                        ...
                        <cfinvokeargument name="RHTcomportam" value="#dataTipoAccion.RHTcomportam#"/>  <!---SML. Modificacion para que cuando se de alta a un trabajador en el bimestre anterior y ya se Calcularon SDI, considere el bimestre actual--->
					</cfinvoke>
				...
				<!--- SML modificacion para no calcular sdi para tipo de accion de cambio--->
				<!---<cfelseif dataTipoAccion.RHTcomportam EQ 6 and #vDLsalario# NEQ #vLTsalario# and dataTipoAccion.RHTccomp EQ 1>---> <!---Aumento--->
