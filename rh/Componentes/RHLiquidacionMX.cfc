<cfcomponent>
	...
	<cffunction name="fnCalculaEG" access="public" >
		<cfargument name="RHLPid" 	type="numeric" required="true">
        ...
        <!--- ERBG Cambio para que tome el salario mínimo de la Zona Económica correspondiente al empleado INICIA--->
        <cfquery name="rsEmplLiq" datasource="#Arguments.Conexion#">
        	select DEid from RHLiqIngresos
			where RHLPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHLPid#">
			  and Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.Ecodigo#">
        </cfquery>
        <!--- ERBG Cambio para que tome el salario mínimo de la Zona Económica correspondiente al empleado FIN--->

		<cfset Exento = 0>
		<cfset rsLiqIngresos = getLiqIngresos(Arguments.RHLPid,Arguments.Ecodigo,Arguments.Conexion)>
		...
		<cfif rsConcepto.CIlimitaconcepto eq 1 >
			<cfif rsConcepto.CItipometodo eq 4 >
                <!--- ERBG Cambio para que tome el salario mínimo de la Zona Económica correspondiente al empleado INICIA--->
				<cfif not isdefined('rsEmplLiq.DEid')>
					<!--- MEX - Salario minimo general zona A (SMGA) (mexico)--->
                    <cfset rsSMGA  = fnGetSalarioMinimo(true,Arguments.Ecodigo,Arguments.Conexion)>
                <cfelse>
                    <!--- MEX - Salario minimo general zona del empleado (mexico) --->
                    <cfinvoke component="rh.Componentes.RH_CalculoSDI" method="SMG" returnvariable="rsSMGA">
                       <cfinvokeargument name="DEid" value="#rsEmplLiq.DEid#">
                    </cfinvoke>
                </cfif>
                <!--- ERBG Cambio para que tome el salario mínimo de la Zona Económica correspondiente al empleado FIN--->
		...
		<!--- Adecuación para actualizar el monto en tabla de DDConceptosEmpleado --->
		<cfquery name="rsId" datasource="#Arguments.Conexion#">
			select coalesce(CIid, 0) as CIid, coalesce(DLlinea, 0) as DLlinea, coalesce(importe, 0) as importe from RHLiqIngresos
			where RHLPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RHLPid#">
			  and Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.Ecodigo#">
		</cfquery>
		<cfif rsId.RecordCount gt 0>
			<cfquery datasource="#Arguments.Conexion#">
				update DDConceptosEmpleado set
					DDCres = <cfqueryparam cfsqltype="cf_sql_float" value="#rsId.importe#"> 
				where CIid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsId.CIid#">
				and DLlinea = <cfqueryparam cfsqltype="cf_sql_integer" value="#rsId.DLlinea#">
			</cfquery>
	    </cfif>
	</cffunction>
	...
	<cffunction name="getLiqIngresos" returntype="query">
		...
		<cfquery name="rsLiqIng" datasource="#Arguments.Conexion#">
			select RHLPid, DLlinea, DEid, CIid, Ecodigo, RHLPdescripcion, importe, fechaalta, RHLPautomatico, Mcodigo, tipocambio,
	...
	<cffunction name="fnCalculaISPT" access="public" returntype="numeric">
		...
		<cfset IRcodigoN1 = #Arguments.IRcodigo#> <!--- fnGetDato(30,Arguments.Ecodigo,Arguments.Conexion).Pvalor>	 Impuesto Renta --->
	...
	<cffunction name="fnGetMontoEG" access="public" returntype="query">
		...
		<cfif fnEsPrimeraVez(Arguments.DLlinea,Arguments.Conexion)>
...
<!---		<cfquery name="rsSalarioMensual" datasource="#Arguments.Conexion#">
			...
			where DEid = #Arguments.DEid#
				and LThasta < <cfqueryparam cfsqltype="cf_sql_date" value="#Arguments.Fecha#">
		</cfquery>--->
	...
	<cffunction name="fnEjecutarCalc" access="public" returntype="struct">
		...
		<cfset current_formulas = rsConcepto.RHFVDcalculo> <!---operaciones a efectuar--->
	...
	<cffunction name="fnEjecutarCalculoLoco" access="public" returntype="struct">
		...
		<cfset resultado = 0>
        <cfset resultadoTotal = 0>
		<cfloop query="rsNivelesCL">
			<cfswitch expression="#rsNivelesCL.RHDVDtipo#">
				<cfcase value="1">
					<cfset rCalculo = fnEjecutarCalc(Arguments.DEid,Arguments.DLlinea,rsNivelesCL.RHDVDid,Arguments.Fecha,Arguments.Ecodigo,Arguments.Conexion)>
					<cfset resultado = rCalculo.Resultado>
                   <!--- <cfthrow message="#resultado#">--->
            ...
            <cfset resultadoTotal = #resultadoTotal# + #resultado#>
		</cfloop>
       <!--- <cfthrow message="#resultadoTotal#">--->
		<!--- Ultimo nivel de la operacion es el resultado del calculo--->
		<cfset structRNiveles.Resultado = #resultadoTotal# >
		<!---<cfdump
		var="#structRNiveles#"/>--->
	...
	<cffunction name="fnExisteLF" access="public" returntype="boolean">
		...
		<cfquery name="rsExiste" datasource="#Arguments.Conexion#">
			select RHLFLid
			from RHLiqFL
			where DLlinea = #Arguments.DLlinea# and DEid = #Arguments.DEid#
		</cfquery>
	...
	<cffunction name="fnCambioLF" access="public">
		...
		<cfargument name="Conexion"  type="string">
        <cfargument name="RHLFLISPTRealL" 	 	type="numeric" default="0">
		...
		<cfif not isdefined('Arguments.RHLFLid') and ( not isdefined('Arguments.DLlinea') or not isdefined('Arguments.DEid'))>
			<cfthrow message="Debe definir un campo, RHLFLid ó DLlinea y DEid">
		</cfif>
		...
		<cfquery datasource="#Arguments.Conexion#">
			update RHLiqFL set
                ...
                RHLFLISPTRealL		= <cfqueryparam cfsqltype="cf_sql_money" value="#Arguments.RHLFLISPTRealL#">,
			...
			<cfelse>
				DLlinea = #Arguments.DLlinea# and DEid = #Arguments.DEid#
			</cfif>
		</cfquery>
	</cffunction>
	...
	<cffunction name="fnAltaLF" access="public" returntype="numeric">
		...
		<cfquery name="rsAltaFL" datasource="#Arguments.Conexion#">
			insert into RHLiqFL (DLlinea, DEid, RHLFLtGrabadoF, RHLFLtExentoF, RHLFLisptF, RHLFLISPTRealL, RHLFLinfonavit, RHLFLtotalL, RHLFLsalarioMensual,
                ...
                <cfqueryparam cfsqltype="cf_sql_money" value="#Arguments.RHLFLISPTRealL#">,
	...
	<cffunction name="fnGetLF" access="public" returntype="query">
		...
		<cfquery name="rsAltaFL" datasource="#Arguments.Conexion#">
			select RHLFLid, DLlinea, DEid, RHLFLtGrabadoF, RHLFLtExentoF, RHLFLisptF, RHLFLinfonavit, RHLFLtotalL, RHLFLsalarioMensual,
				RHLFLisptSalario, RHLFLisptL, RHLFLresultado, Ecodigo, BMUsucodigo, RHLFLISPTRealL
			from RHLiqFL
			where
			<cfif isdefined('Arguments.RHLFLid')>
				RHLFLid = #Arguments.RHLFLid#
	...
	<cffunction name="fnValidaModificaciones" access="public" returntype="boolean">
		...
		<cfset rsMontoEGF = fnGetMontoEG(Arguments.DLlinea,"F", Arguments.Ecodigo, Arguments.Conexion)>
		...
		<cfset ISPTF = fnCalculaISPT(rsMontoEGF.Grabado, Arguments.Fecha, Arguments.Ecodigo, Arguments.Conexion, '#IRcodigoN1#')>
		...
		<cfset rsMontoEGL = fnGetMontoEG(Arguments.DLlinea, "L", Arguments.Ecodigo, Arguments.Conexion)>
		<cfset SalarioMensual = fnGetSalarioMensual(Arguments.DEid, Arguments.Fecha, Arguments.Conexion)>
		<cfset ISPTSL = fnCalculaISPT(SalarioMensual, Arguments.Fecha, Arguments.Ecodigo, Arguments.Conexion,'#IRcodigoN1#')>
		...
		<cfset CalculoLoco = fnEjecutarCalculoLoco(Arguments.DEid,Arguments.DLlinea,Arguments.Fecha,SalarioMinimo,Arguments.Ecodigo,Arguments.Conexion)>
...
<!---		<cfset Neto = TotalRealF - ISPTF + MontoGrabadoL - ISPTL>--->
   ...
   <!---   Obtiene la retencion. 90 dias de salario minimo por cada año que laboro--->

  	<cffunction name="fnRetencionISPT" access="public" returntype="numeric">
      <cfargument name="DEid" type="numeric" required="yes">
		<!---
    	<cfinvoke component="rh.Componentes.RH_CalculoSDI" method="SMG" returnvariable="SalarioMinimo">
           <cfinvokeargument name="DEid" value="#Arguments.DEid#">
        </cfinvoke>
 --->

		<cfquery name="rsUMA" datasource="#session.dsn#">
			select coalesce(Pvalor,'') Pvalor
			from RHParametros
			where
				Pcodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="2051">
			and Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
		</cfquery>

		<cfif rsUMA.RecordCount eq 0 or (rsUMA.RecordCount gt 0 and rsUMA.Pvalor eq '')>
			<cfthrow detail="Parametros RH " message="No se ha configurado el parametro: *Valor de la Unidad de Medida y Actualización*">
		</cfif>

        <cfset Salario90dias = rsUMA.Pvalor*90> <!---Obtiene el salario por noventa dias--->
        ...
        <cfquery name="rsDetalleRHLiquidacionPersonal" datasource="#session.DSN#">
         ...
         <cfquery name="rsDetalleRHLiquidacionPersonal" datasource="#session.DSN#">
           ...
           <cfset anio = DateDiff('yyyy',rsDetalleRHLiquidacionPersonal.EVfantig,rsDetalleRHLiquidacionPersonal.DLfechaaplic)>
           <cfset mes = DateDiff('m',DateAdd('yyyy',anio,rsDetalleRHLiquidacionPersonal.EVfantig),rsDetalleRHLiquidacionPersonal.DLfechaaplic)>
           <cfset dias = DateDiff('d',DateAdd('m',mes,DateAdd('yyyy',anio,rsDetalleRHLiquidacionPersonal.EVfantig)),rsDetalleRHLiquidacionPersonal.DLfechaaplic)>
           <cfif dias eq 30 or dias eq 31>
        		<cfset mes = mes + 1>
                	<cfelseif mes gt 5>
                	<cfset anio = anio + 1>
        	<cfelse>
