 <!---Para mexico se puede calcular la renta de tres modos
	...
	<cffunction name="CalculoNominaRenta" access="public" output="true" >
        ...
        <cfif CalendarioPago.CPTipoCalRenta EQ 1> 
            ...
            <cf_dbtemp name="EmpleadosRenta" returnvariable="EmpleadosRenta" datasource="#session.DSN#">
                ...
                <cf_dbtempcol name="salHistTotal" 			type="money" 			mandatory="no">	<!--- Salarios Histor --->				<!---L--->
                <cf_dbtempcol name="salHist" 				type="money" 			mandatory="no">		
                <cf_dbtempcol name="salIncHist" 			type="money" 			mandatory="no">				
                ...
                <cf_dbtempcol name="historIncid" 			type="money" 			mandatory="no">	<!--- Histor Incid No Proyec --->		<!---Q--->
                ...
                <cf_dbtempcol name="totalDiasHist" 			type="money" 			mandatory="no">	<!--- TOTAL Dias Nom Hist Labor --->	<!---S1--->
                ...
                <cf_dbtempcol name="diasNomAcum" 			type="numeric" 			mandatory="no">	<!--- Dias Nom Acum--->					<!---AB--->
                ...
                <cf_dbtempcol name="MontoFijo" 		type="money" 	mandatory="no">
                    ...
                    <!--- LZ Verifica que a partir de la Vigencia de la Tabla de Renta, 
                            ...
                            <cfquery datasource="#session.DSN#" name="VerificaCalendarios">
                                select count(CPid) as CantCalendarios, case b.Ttipopago  
                                                                                      when  0 then 52
                                                                                      when  1 then 26
                                                                                      when  2 then 24
                                                                                      when  3 then 12
                        ...
                        <cfif VerificaCalendarios.CantCalendarios LT VerificaCalendarios.CantEsperados and Arguments.ValidarCalendarios>
                            <cfset mens ="Hay MENOS calendarios de los esperados. Revise la cantidad de nominas para el periodo de la tabla de renta que se esta calculando." >
                          
                            <cfthrow detail="Error utilizando Renta Anual. Deben definirse la totalidad de Calendarios de Pago del Año, 
                                             para una correcta distribución de la retención por nómina a calcular. #mens#">
                        </cfif>
                  ...
                  <cfif Arguments.ValidarCalendarios>
                    <!---Cantidad de nominas o calendarios de pago programados para el mes del calendario de pago--->	
                    <cfquery datasource="#session.DSN#" name="rsNominasMensuales">
                        select count(b.CPid) as total
                            ...
                            where a.CPid = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.RCNid#">
                            and b.CPtipo = 0	<!---ordinarias--->
                    </cfquery>
                 <cfelse>
                    <cfquery datasource="#session.DSN#" name="rsPrimerNominaDelMes">
                        select a.CPfpago
                            from CalendarioPagos a
                         where a.CPmes 		= <cfqueryparam cfsqltype="cf_sql_numeric" value="#CalendarioPago.CPmes#">
                           and a.CPperiodo 	= <cfqueryparam cfsqltype="cf_sql_numeric" value="#CalendarioPago.CPperiodo#">
                           and a.Ecodigo	= <cfqueryparam cfsqltype="cf_sql_numeric" value="#CalendarioPago.Ecodigo#">
                           and a.Tcodigo 	= <cfqueryparam cfsqltype="cf_sql_varchar" value="#CalendarioPago.Tcodigo#">
                           and a.CPtipo		= <cfqueryparam cfsqltype="cf_sql_numeric" value="#CalendarioPago.CPtipo#">
                    ...
                    <cfif NOT rsPrimerNominaDelMes.RecordCount>
                    	<cfthrow message="No se pudo Recuperar la primer Calendario de #CalendarioPago.CPperiodo# -  #CalendarioPago.CPmes#">
                    </cfif>
                    <cfif CalendarioPago.Ttipopago EQ 0><!---Semanal--->
                        ...
                        <cfloop condition="Continuar EQ true">
                            ...
                            <cfif DatePart("yyyy", CPfpagoTemp) EQ CalendarioPago.CPperiodo and DatePart("m", CPfpagoTemp) EQ CalendarioPago.CPmes>
                            	<cfset CantCalendarioMensuales = CantCalendarioMensuales + 1>
                            <cfelse>
                   				<cfset Continuar = false>
                            </cfif>
                   		</cfloop>
                   	<cfelseif CalendarioPago.Ttipopago EQ 1><!---Bisemanal--->
                    	<cfset CantCalendarioMensuales 	= 1>
                        <cfset Continuar				= true>
                        <cfset CPfpagoTemp 				= rsPrimerNominaDelMes.CPfpago>
                        <cfloop condition="Continuar EQ true">
                    ...
                    <cfelseif  CalendarioPago.Ttipopago EQ 2><!---Quincenal--->
                    	<cfset CantCalendarioMensuales = 2>
                    <cfelseif  CalendarioPago.Ttipopago EQ 3><!---Mensual--->
                    	<cfset CantCalendarioMensuales = 1>
                    <cfelse>
                    	<cfthrow message="Frecuencia de Pago del Tipo de nomina no implementado">
                    </cfif>
					<cfset rsNominasMensuales.total = CantCalendarioMensuales>
                 </cfif>
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# set nomMes =  #rsNominasMensuales.total#
                    </cfquery>
                    
                    
                    
                    <!---Cantidad de nominas pagadas del mes + la nomina que se va a pagar actualmente del mes--->
                    <cfquery datasource="#session.DSN#" name="rsNominasPagadas">
                        select count(c.RCNid) as pagadas
                        from CalendarioPagos a
                            inner join CalendarioPagos b
                                on a.CPmes = b.CPmes
                                    and a.CPperiodo = b.CPperiodo
                    ...
                    <cfif #CalendarioPago.CPtipo# EQ 0>
                        <cfset NominasPagadas = rsNominasPagadas.pagadas + 1> <!--- + la nomina que se va a pagar actualmente del mes --->
                    <cfelse>
                        <cfset NominasPagadas = rsNominasPagadas.pagadas> <!--- solo el historico de nominas --->
                    </cfif>
                    
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# set numNomina =  #NominasPagadas#
                    </cfquery>
                    
                    <!---Cantidad de nominas pedientes a pagar--->
                    <cfset NominasPendientes = rsNominasMensuales.total - rsNominasPagadas.pagadas>
                    
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# set
                        nomPend =  #NominasPendientes#
                    </cfquery>

<!-------------ljimenez modifica calculo de dias para nominas que son quincenales ni mensuales ----------------------->                    

                    <!---dias de nomina pagados--->
                    <cfquery datasource="#session.DSN#" name="rsDiasPagados">
                        select <cf_dbfunction name="datediff" args="b.CPdesde, b.CPhasta">+1 as DiasPagados
                        from CalendarioPagos a
                            inner join CalendarioPagos b
                                on a.CPmes = b.CPmes
                                    and a.CPperiodo = b.CPperiodo
                    ...
                    <cfif isdefined('rsDiasPagados') and rsDiasPagados.RecordCount NEQ 0>
                        <cfquery dbtype="query" name="rsCanDiasPagadoss">
                            select sum(DiasPagados) as CanDiasPagados from rsDiasPagados
                        </cfquery>
                        <cfset vDiasPagados = #rsCanDiasPagadoss.CanDiasPagados#>
                    <cfelse>
                    	<cfset vDiasPagados = 0 >
						<!---#diasNom# --->
                    </cfif>
                    
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta#  set diasNominaPagados = #vDiasPagados#
                    </cfquery>
<!--------------------------------------------------------------------------------------------------------->
                    
                    
					<!---???????Se Actualiza Factor de Dias para salario diario, dependiendo del Tipo de Nomina?????--->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# set
                         	diasMes = (SELECT <cf_dbfunction name="to_float" args="FactorDiasSalario" dec="4">
                                       		FROM TiposNomina
                                        WHERE Ecodigo = <cfqueryparam value="#Session.Ecodigo#"   cfsqltype="cf_sql_integer">
                                          and Tcodigo = <cfqueryparam value="#arguments.Tcodigo#" cfsqltype="cf_sql_char">)
                    </cfquery>
                    
                    
                    <!---Cantidad Dias  Reales Laborados Ordinarios en Nomina Columna F --->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# set
                    ...
                    <!---Cantidad Dias  Reales Laborados Ordinarios en Nomina Historicas Columna S1 --->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# set
                        totalDiasHist =  (select coalesce(sum(PEcantdias),0) 
                                    from HRCalculoNomina a
                                         ...
                                         inner join CalendarioPagos d
                                            on a.RCNid=d.CPid
                                            and d.CPtipo = 0
                                         inner join CalendarioPagos e
                                            on d.CPmes = e.CPmes
                                            and d.CPperiodo = e.CPperiodo
                                            and d.Tcodigo = e.Tcodigo
                                            and d.Ecodigo = e.Ecodigo
                                        where 
                                            e.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.Ecodigo#">
                                            and e.CPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#">
                                            and c.PEmontores > 0
                                            and c.PEtiporeg = 0
                                            and c.DEid = #EmpleadosRenta#.DEid )
                    </cfquery>
					...
					<cfquery datasource="#session.DSN#" name="rsDiasFaltaHist">
                       update #EmpleadosRenta# set
                        totalDiasHist =  totalDiasHist - 
                        				coalesce((
                                            select sum((coalesce(ta.RHTfactorfalta,1) * pe.PEcantdias) -  pe.PEcantdias)
                                            from HPagosEmpleado pe
                                                ...
                                                inner join CalendarioPagos e
                                                    on x.CPmes = e.CPmes
                                                        and x.CPperiodo = e.CPperiodo
                                                        and x.Tcodigo = e.Tcodigo
                                                        and x.Ecodigo = e.Ecodigo
                                                
                                                 where pe.DEid = #EmpleadosRenta#.DEid
                                                 	and  e.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.Ecodigo#">
                                                    and  e.CPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#">),0)
                                                 
					</cfquery>
                    
            <!--- Dias Nomina--->					<!---(columnas F & AA)--->
			<!--- LZ Verifica la cantidad de dias segun la frecuencia de pago
                        Mensual 	--> 30 dias. 
                        Bisemanal	--> 14 dias 
                        Quincenal	--> 15 dias
                        Semanal		-->  7 dias --->								
                    ...
                    <cfquery datasource="#session.DSN#" name="rsCantDiasCalendario">
                        ...
                        from CalendarioPagos a, TiposNomina b
                            ...
                            and a.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.Ecodigo#">
                            and a.CPhasta >= <cfqueryparam cfsqltype="cf_sql_date" value="#LSDateformat(FechaTabla,'dd/mm/yyyy')#">
                            group by a.Tcodigo, case b.Ttipopago when 0 then 7 
                                                                 when 1 then 14 
                                                                 when 2 then 15 
                                                                 when 3 then 30 
                                                end
                    ...
                    <cfif #CalendarioPago.CPtipo# EQ 0> <!--- Dias nomina --->
                         <cfset diasNom = #rsCantDiasCalendario.CantDiasCalendario#>
                    <cfelse>
                         <cfset diasNom = 0>
                    </cfif>
                   
                    
                    <cfif #CalendarioPago.CPtipo# EQ 0> <!---renta en mes--->
                        <cfquery datasource="#session.DSN#">
                            update #EmpleadosRenta#  set diasNomina = #diasNom#
                        </cfquery>
                    <cfelse>
                        <cfquery datasource="#session.DSN#">
...
<!---------------------------------------------------------------------------------------->
                    ...
                    <!---<cfdump var="#rsCantDiasCalendario#">--->
                    ...
                    <cfif #rsCantDiasCalendario.Ttipopago# LT 2 and  #rsCantDiasCalendario.CantDiasCalendario# LT 15>
                         <cfquery datasource="#session.DSN#">
                            update #EmpleadosRenta# set 
                                ...
                                <!---, diasMes = #rsCanDiasMes.CanDiasMes#--->
                                , diasNomina = #diasNom#
						 </cfquery>
                    <cfelse>
                    	 <cfquery datasource="#session.DSN#">
                            ...
                            <!--- #diasNom# * #rsNominasMensuales.total#--->
                        </cfquery>
                    </cfif>
                    
                    <cfif #CalendarioPago.CPtipo# NEQ 0> <!---renta en mes--->
                        <cfquery datasource="#session.DSN#">
                            update #EmpleadosRenta#  set diasNomina = 0
                        </cfquery>
                    </cfif>
...
<!---------------------------------------------------------------------------------------->
                    ...
                    <!--- "Dias Mes Calendario": Dias Nomina Ordinario * Cantidad de Calendarios Ordinarios Mes
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# set diasMesCalendar =   #diasNom# * #rsNominasMensuales.total#
                    </cfquery>
                    --->
                    ...
                    <!---<cfquery datasource="#session.DSN#">
                        ...
                        set diasNomAcum	= (diasMesCalendar / nomMes) * numNomina
                    </cfquery>--->
                    
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 
                        set diasNomAcum	=  diasNominaPagados + diasNomina
                    </cfquery>
                     
                    <!---"Salario Men Referencia Max(PEsalario)" --limpiar query --->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# set
                        salMensual =  (select coalesce(sum(PEsalario),0)  
                                    from RCalculoNomina a, PagosEmpleado c,CalendarioPagos d
                                        where a.RCNid=d.CPid
                                        and d.CPhasta >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#FechaTabla#">
                                        and d.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.Ecodigo#">
                    ...
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# set
                        salRecPeriodo =  (usar / diasMes * diasCalendar)
                    </cfquery>
                    
              
                    <!--- Salario Devengado En Nominas Históricas para el Mes--->		
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 	
                        set salHist = (
                                select coalesce(sum(SEsalariobruto),0)	
                        ...
                        <cfif isdefined("arguments.DEid") and len(trim(arguments.DEid))><!---CarolRS--->
                            where DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.DEid#">
                        </cfif>	
                    </cfquery>
                    
                    <!--- Incidencias en Nominas históricas para el MES--->	
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 	
                        set salIncHist = (
                                ...
                                inner join HIncidenciasCalculo c
                                    on a.RCNid = c.RCNid	
                                    and c.DEid = #EmpleadosRenta#.DEid  
                    ...
                    <!--- Salario  Histórico para el Mes---><!--- Salario Historico del Salario Bruto para el Mes + Incidencias en Nominas históricas para el MES--->	
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 	
                        set salHistTotal = salHist <!--- + salIncHist---->
                    </cfquery>
                    ...
                    <!---Total de Salarios Proyectados (N)--->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 
                        set totalSalProyect =   salRecPeriodo + salHistTotal + incRecProyect  <!---(L) + (K) + (M)--->
                    </cfquery>
                    
                    <!---"Dias Nom Pend Pagar" : "Total de Dias Nom-Mes - (Cant Dias Nomina Hist del Mes + En Proceso)"  (O)--->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# set
                        diasNomPend =  diasMesCalendar - diasNomAcum
    
                    </cfquery>
                    
                    <!---"Promedio Salarios": Promedio de Salarios Proyectados N+((J/E)*O)   (P)--->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# set
                        promSal = totalSalProyect+(( usar/ diasMes) * diasNomPend)
                    </cfquery>
                    ...
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 
                        set totalSal =  promSal  <!---(P)--->
                    </cfquery>
                    
                    <!--- "Histor Incid No Proyec": Incidencias en Nominas históricas para el MES q no se proyectan (Q)--->	
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 	
                        set historIncid = (
                                Select coalesce(sum(ICmontores),0)
                                ...
                                inner join CIncidentes d
                                    on c.CIid = d.CIid
                                    and d.CInorenta = 0
                                    and d.CInopryrenta =0 <!--- NO se proyecta--->
                    ...
                    <!---"Total de dias reales laborados por la persona en el mes tomando en cuenta los laborado (HISTORICO) + los dias en proceso (NOMIINA ACtual) + 
                    Lo pendiente por laborar   columnas S1 + F + O --->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 
                        set TotalDiasPromProyect = totalDiasHist + diasCalendar  + diasNomPend
                    </cfquery>
                    ljimenez
                    
                    
                    <!---"Salario PromDiario Proyec" Promedio Diario de Salarios =+S/G (T)--->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 
                        set salPromDiaPr = promSal / TotalDiasPromProyect
                        where promSal > 0
                    </cfquery>
                    
                    <!---Según la Nomina ejecuta los Montos Pend de Pagar =+O*T  (U)--->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 
                        set salPend = diasNomPend * salPromDiaPr
                    </cfquery>
                    
                    <!---Total de Salarios, salario referencia para la renta =+K+U+L+Q+R+M  (V)--->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 
                        set totalSalRent = 	salRecPeriodo + salPend	+ salHistTotal + historIncid +	incidNoProyect	+ incRecProyect
                    </cfquery>
                    ...
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 
                        set rentaDiaria = rentaReal / diasMesCalendar
                    </cfquery>
                    
                    <!--- Renta Acum--->					<!---(AC)--->
                    <!--- Sacamos la renta retenida de las nominas historicas para el mes periodo en que esta la nomina actual--->
                    <cfquery datasource="#session.DSN#">																
                        update #EmpleadosRenta# set
                        rentaAcum	= 
                                coalesce(( 
                    ...
                    <cfquery datasource="#session.DSN#">																
                        update #EmpleadosRenta# set
                        rentaAcum	=  rentaAcum + rentaAcumDed
                    </cfquery>
                    
                    <!--- Renta Real al Corte--->			<!---(AE)--->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 
                        set retaRealCorte	= round(rentaDiaria * diasNomAcum,2)
                    </cfquery>		
                    
                    <!--- Retencion--->						<!---(AD)--->
                    <cfquery datasource="#session.DSN#">
                        update #EmpleadosRenta# 
                        set retencion	= retaRealCorte - rentaAcum
                    </cfquery>
                    ...
                    <cfquery datasource="#arguments.conexion#">
                        ...
                        set SEacumulado = (select totalSalProyect
                                        from #EmpleadosRenta# 
                    ...
                    <cfquery datasource="#arguments.conexion#">
                        update SalarioEmpleado
                        set SEproyectado =(select promSal
                                            from #EmpleadosRenta# 
                                            where #EmpleadosRenta#.DEid = SalarioEmpleado.DEid)
                        where RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.RCNid#">
                        and (SalarioEmpleado.SEsalariobruto+SalarioEmpleado.SEincidencias) > 0
...
--->                  
    ...
    <cffunction name="fnGetCalendarioPago" access="public" returntype="query">
    	...
    	<cfquery name="rsCP" datasource="#Arguments.conexion#">	
       		select CPid, CPTipoCalRenta, CPtipo, b.CPperiodo, b.CPmes, b.Tcodigo, b.Ecodigo, c.Ttipopago
            	from RCalculoNomina a 
                	inner join CalendarioPagos b
                    	on  b.CPid = a.RCNid
    ...
    <cffunction name="fnCalculaISPT" access="public" returntype="numeric">
   	...
   	<cffunction name="fnCalculoRentaMetodo2" access="public">
        ...
        <cfquery datasource="#session.DSN#" name="rsEmpleadosRenta" >
			...
			select distinct b.DEid, b.SEincidencias, 0,  0,  b.SEincidencias, l.RVid , 
				case 
					when <cf_dbfunction name="datediff" args="ev.EVfantig, #Fecha2# ,YYYY "> = 0
						then 1
					else
...
<!---        <cfquery datasource="#session.DSN#" name="upAguiMes">
        ...
        <cfquery datasource="#session.DSN#" name="upAguiMes">
			update  #EmpleadosRenta#
				set AguiMensual = (((((SalarioM / 365) * (DiasTrab - Faltas -  Incapa))- AguiExc)/365) * 30.4 )
		</cfquery>
		...
		<cfloop query="rsDatos">
			<cfset ISPT_AguiMenGra = fnCalculaISPT(rsDatos.AguiMenGra)>
			<cfset ISPT_SalarioM = fnCalculaISPT(rsDatos.SalarioM)>
...
<!---			<br>
		...
		--->
         ...
         <cfquery datasource="#session.DSN#">
         	update SalarioEmpleado
            	set SErenta    = (select distinct coalesce(ISPT_Aguinado,0) from #EmpleadosRenta# where DEid = SalarioEmpleado.DEid)
             where RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RCNid#">	
         </cfquery>
	</cffunction>	
   	...
   	<cffunction name="fnCalculoRentaMetodo4" access="public">
        ...
        <cfif NOT isdefined('Arguments.conexion') OR NOT LEN(TRIM(Arguments.conexion))>
            ...
            <cf_dbtemp name="EmpleadosRenta" returnvariable="EmpleadosRenta" datasource="#session.DSN#">
                ...
                <cf_dbtempcol name="incRecProyect" 			type="numeric" 			mandatory="no">	<!--- Incidenc Recib Proyec --->		<!---M--->
                <cf_dbtempcol name="incidNoProyect" 		type="numeric" 			mandatory="no">	<!--- Inciden No Proyect --->			<!---R--->
