...
<cfquery datasource="#Arguments.datasource#" name="UpdtaeGrava">
	...
	<cfif CalendarioPagos.CPnocargas is 0 or CalendarioPagos.CPnocargasley is 0 >
			...
			<cfloop query="rsPrioridades">
                ...
                <cfquery datasource="#Arguments.datasource#">
					...
					coalesce((select 
							...
							when DCmetodo = 1 
								...
								then 
                                	case when (round(((CargasCalculo.CCSalarioBaseCotizacion - (#vSMGA# * 3)) * CargasCalculo.CCdbc) * ((coalesce(b.CEvaloremp, c.DCvaloremp))/100), 2) < 0)
                                    	then 0
                                        else round(((CargasCalculo.CCSalarioBaseCotizacion - (#vSMGA# * 3)) * CargasCalculo.CCdbc) * ((coalesce(b.CEvaloremp, c.DCvaloremp))/100), 2)
                                     end   
                            when DCmetodo = 2
									and coalesce(ECSalarioBaseC,0) = 1 
									and coalesce(DCdisminuyeSBC,0) = 0		
				...
				<cfquery datasource="#Arguments.datasource#">
					<!--- ljimenez PATRONO es aca donde se hace el calculo de las cargas utilizando alguno de los 2 metodos 0 = monto, 1 = %.
							si la opcion diminuye en x VSMG  la restamos al SBC para hacer el calculo de la carga
							y verificamos cuando hacemos el calculo de las cargas utilizando el SBC como base del calculo 
							o lo hacemos con el Salario calculado(normal) --->
				 	update CargasCalculo 
					...
					coalesce((select 
						case 
							...
							when DCmetodo = 1 
									...
									then 
                                    	case 
                                        	when (round(((CargasCalculo.CCSalarioBaseCotizacion - (#vSMGA# * 3)) * CargasCalculo.CCdbc) * ((coalesce(b.CEvalorpat, c.DCvalorpat))/100), 2) < 0)	
                                        	then 0
                                        else round(((CargasCalculo.CCSalarioBaseCotizacion - (#vSMGA# * 3)) * CargasCalculo.CCdbc) * ((coalesce(b.CEvalorpat, c.DCvalorpat))/100), 2) 
                                    end
                             when DCmetodo = 2
                                        and coalesce(ECSalarioBaseC,0) = 1 
                                        and coalesce(DCdisminuyeSBC,0) = 0		
