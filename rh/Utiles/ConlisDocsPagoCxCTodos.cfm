...
<!--- Recibe conexion, form, name, desc, id, DatoCCTcodigo--->
...
<script language="JavaScript" src="/cfmx/rh/js/calendar.js"></script>
...
<cfif isdefined("form.SNcodigo") and len(trim(form.SNcodigo))>
	<cfquery datasource="#session.dsn#">
		update PlanPagos
			...
			PPpagomora = case when datediff (dd, PPfecha_vence, getdate()) <= 0
				<!---  cuota no ha vencido  --->
				then 0
			else
...
<cfquery name="rsMonedas" datasource="#Session.DSN#">
	...
	and Dsaldo > 0 
	<!--- cambiar usar la llave de  Pagos --->
	<!--- <cfif isdefined("Form.GSNidm") and Form.GSNidm neq "">
	and exists 
		(select 1 from SNegocios s 
		  where s.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
		    and s.GSNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.GSNidm#">
			and s.SNcodigo = a.SNcodigo
		)
	<cfelseif isdefined("form.SNcodigo") and Form.SNcodigo neq ""> --->
	<cfif isdefined("rsNombreSocio") and rsNombreSocio.recordcount gt 0 and rsNombreSocio.SNid NEQ ''>
		and (c.SNidPadre = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsNombreSocio.SNid#"> OR  c.SNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsNombreSocio.SNid#">)
	<cfelse>
		<cfif isdefined("form.SNcodigo") and len(trim(form.SNcodigo))>
...
<cfif isdefined("Form.GSNidm") and Len(Trim(Form.GSNidm))>
	<cfquery name="rsGrupoSNegocios" datasource="#session.DSN#">
		...
		from GrupoSNegocios
		where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
		and GSNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.GSNidm#">
	</cfquery>
</cfif>
...
<cfif rsParametroCcuentaTransitoConlis.Pvalor eq ''>
	...
	<cfquery name="rsCuentaConlis" datasource="#Session.DSN#">
		select Ccuenta, convert(varchar(40),Cdescripcion) as Cdescripcion 
		from CContables 
		where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">
		and Ccuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsParametroCcuentaTransitoConlis.Pvalor#"> 
	</cfquery>
...
<cfloop query="conlis"> 
 ...
 <script language="JavaScript" type="text/javascript"> 
	function Asignar(cod,desc,name,tipo) { 	
		...
		//Esto selecciona en el campo del combo del tag sifDocsPagoCxC el tipo de documento que se seleccionó en el conlis 
...
<!--- <cfdump var="#form#">
<cf_dump var="#url#"> --->
<cfform action="ConlisDocsPagoCxCTodos.cfm" name="conlis" method="post">
  <table width="100%" border="0" cellspacing="0" cellpadding="2">
	<tr>
		...
		<cfif isdefined("rsNombreSocio") and rsNombreSocio.recordcount gt 0 and rsNombreSocio.SNid NEQ '' and rsNombreSocio.SNidPadre EQ ''>
			Documentos&nbsp;de&nbsp;<cfoutput>#rsNombreSocio.SNnombre#</cfoutput>&nbsp;y&nbsp;Relacionados
    ...
    <tr> 
	  ...
	  <cfif isdefined("rsNombreSocio") and rsNombreSocio.recordcount gt 0 and rsNombreSocio.SNid NEQ '' and rsNombreSocio.SNidPadre EQ ''>
	  	<td align="center" class="encabezado">Socio&nbsp;de&nbsp;Negocios</td>
    ...
    <tr> 
      ...
      <td align="center" nowrap> <cfoutput> 
            <input name="Dfecha" type="text" size="10" maxlength="10" onBlur="javascript: onblurdatetime(this);"
			value="<cfif isdefined('Form.Dfecha')>#Form.Dfecha#</cfif>">
        </cfoutput> <a href="#" > <img src="/cfmx/rh/imagenes/DATE_D.gif" alt="Calendario" name="Calendar" width="16" height="14" border="0" onClick="javascript:showCalendar('document.conlis.Dfecha');"></a> 
      </td>
	  <cfif isdefined("rsNombreSocio") and rsNombreSocio.recordcount gt 0 and rsNombreSocio.SNid NEQ '' and rsNombreSocio.SNidPadre EQ ''>
		  <td align="center">
	  ...
	  <cfinvoke 
		...
		returnvariable="pListaRet">
			...
			<cfelseif isdefined("form.SNcodigo") and form.SNcodigo gt 0> 
				...
				<cfinvokeargument name="etiquetas" value="Transacci&oacute;n,Documento,Cuota,Moneda,Saldo,Cuota,Mora,Fecha,Fecha&nbsp;Pago"/>
			...
			<cfinvokeargument name="irA" value= "ConlisDocsPagoCxCTodos"/>
	</cfinvoke>
      </td>
    </tr>
  </table>
