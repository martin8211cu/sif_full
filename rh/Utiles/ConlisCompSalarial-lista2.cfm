<cfquery name="rsComponentes" datasource="#Session.DSN#">
	select a.CSid, a.CSdescripcion, a.CSusatabla, a.CSsalariobase,
		   ...
		   <cfelse>
		   case a.CSusatabla when 0 then 'Monto Fijo' 
	...
	and (
		...
		or (
			...
			exists (
				select 1
				from RHMovPlaza x, RHMetodosCalculo b
				where a.Ecodigo = x.Ecodigo
				and x.RHMPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.id#">
				and a.Ecodigo = b.Ecodigo
				and a.CSid = b.CSid
				and b.RHMCestadometodo = 1
				and x.RHMPfdesde between b.RHMCfecharige and b.RHMCfechahasta
			)
		)
	) 
	...
	and not exists (
		select 1
		from RHCMovPlaza f
		where f.RHMPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.id#">
		and f.CSid = a.CSid
	) 
