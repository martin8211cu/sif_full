<!--- 
	Modificado por: Gustavo Fonseca H.
		Fecha: 28-11-2005.
		Motivo: Se eliminan cfoutput anidados dentro de otro cfoutput. Esto para evitar que se genere una 
		página muy pesada.
--->
...
<form action="" method="post" name="conlis">
  <table width="53%" border="0" cellspacing="0">
    ...
    <cfoutput query="conlis" startRow="#StartRow_conlis#" maxRows="#MaxRows_conlis#"> 
      <tr> 
        <td <cfif #conlis.CurrentRow# MOD 2>class="listaPar"<cfelse>class="listaNon"</cfif>><input type="hidden" name="SNidentificacion#conlis.CurrentRow#" value="#conlis.SNidentificacion#"> 
          <a href="javascript:Asignar('#conlis.SNcodigo#', '#JSStringFormat(conlis.SNnombre)#', '#JSStringFormat(conlis.SNidentificacion)#');">#conlis.SNidentificacion#</a></td>
        <td colspan="2" <cfif #conlis.CurrentRow# MOD 2>class="listaPar"<cfelse>class="listaNon"</cfif>><a href="javascript:Asignar('#conlis.SNcodigo#', '#JSStringFormat(conlis.SNnombre)#', '#JSStringFormat(conlis.SNidentificacion)#');">#conlis.SNnombre#</a></td>
      </tr>
    </cfoutput> 
    <tr> 
      <td colspan="3">&nbsp; </td>
