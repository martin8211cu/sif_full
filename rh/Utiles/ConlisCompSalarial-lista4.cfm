<cfquery name="rsComponentes" datasource="#Session.DSN#">
	...
	and (
		...
		or (
			...
			exists (
				select 1
				from RHSituacionActual x, RHMetodosCalculo b
				where a.Ecodigo = x.Ecodigo
				and x.RHSAid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.id#">
				and a.Ecodigo = b.Ecodigo
				and a.CSid = b.CSid
				and b.RHMCestadometodo = 1
				and x.fdesdeplaza between b.RHMCfecharige and b.RHMCfechahasta
			)
		)
	) 
	...
	and not exists (
		select 1
		from RHCSituacionActual f
		where f.RHSAid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.id#">
		and f.CSid = a.CSid
	) 
