...
<form style="margin:0; " name="form1" action="TEncuestadoras.cfm"  method="post" onSubmit="javascript: return validaInfoEnc();">
	<cfoutput>
		<input type="hidden" name="Paso" value="2">
		<input type="hidden" name="tab" value="4">
		<input type="hidden" name="regenerar" value="0">
		...
		<table width="100%"  border="0">
		  <tr>
			<td>
				<table width="20%" align="center" class="areaFiltro">
					...
					<tr>
						...
						<td width="93%" nowrap>
							<cfquery name="rsMonedaEnc" datasource="#session.DSN#">
								select distinct Moneda, Mnombre, Mcodigo
							...
							<select name="Mcodigo_F">
								<option value="-1" selected>-- Todas --</option>								
								<cfif isdefined('rsMonedaEnc') and rsMonedaEnc.recordCount GT 0>									
									<cfloop query="rsMonedaEnc">
										<option value="#rsMonedaEnc.Moneda#" <cfif isdefined('form.Mcodigo_F') and form.Mcodigo_F EQ rsMonedaEnc.Moneda> selected</cfif>>#rsMonedaEnc.Mnombre#</option>
									</cfloop>
								</cfif>												
							</select>							
						</td>
			...
			<td align="right" valign="bottom"  style="font-size:12px; font-variant:small-caps; font-weight: bold;">
				<cfif isdefined('rsFormES') and rsFormES.recordCount GT 0>
					<a href="javascript: RegenerarPuestosEnc();">
						regenerar
						<img src="/cfmx/rh/imagenes/refresh_o.gif" border="0" align="absmiddle" title="Regenerar Puestos para esta encuesta">
					</a>
					<input type="hidden" name="regen_ETid" value="#rsFormES.ETid#">
				<cfelse>
					&nbsp;
				</cfif>
			</td>
...
<script language="javascript" type="text/javascript">
	...
	function RegenerarPuestosEnc(){
		if(confirm('Desea insertar los nuevos puestos que no existen ?')){
