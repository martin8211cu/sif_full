...
<cfquery datasource="#session.dsn#" name="lista" maxrows="200">
	select c.RHCid as item, <cf_dbfunction name="concat" args="rtrim(c.RHCcodigo),' - ', c.RHCdescripcion"> as name, coalesce(c.RHCidpadre,0) as ancestro, 
		(select 
...
<cfquery datasource="#session.dsn#" name="ARBOL">
	select c.RHCid as pk, c.RHCcodigo as codigo, c.RHCdescripcion as descripcion, c.nivel as nivel,  
	...
	where c.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
	  and (c.RHCidpadre is null
	  	<cfif Len(path)>
			or c.RHCidpadre in (<cfqueryparam cfsqltype="cf_sql_integer" value="#path#" list="yes">)
		</cfif>
	  )
	order by c.path
</cfquery>
