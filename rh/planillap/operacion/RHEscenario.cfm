...
<cfif isdefined ('url.RHETEid') and len(trim(url.RHETEid)) gt 0>
...
<form action="RHEscenario-sql.cfm" method="post" name="form1" id="form1">
	<input type="hidden" name="RHETEid" value="<cfif isdefined ('form.RHETEid')>#form.RHETEid#</cfif>" />
	<table width="100%" border="0" cellspacing="0">		
		...
		<tr>
			...
			<td width="32%">
				<input name="RHEdescripcion" size="60" id="RHEdescripcion" type="text" 
					value="<cfif modo NEQ 'ALTA'>#trim(data.RHEdescripcion)#</cfif>" maxlength="80" onfocus="this.select()"
					<cfif  isdefined('Lvar_ModificaE')>readonly</cfif>>
  	  	  </td>
		</tr>
		<tr>
			<td width="9%">&nbsp;</td>
		...
		<tr>
			...
			<td colspan="3">
				<input name="RHEobs" type="text" value="<cfif modo NEQ 'ALTA'>#data.RHEobs#</cfif>" size="95" <cfif  isdefined('Lvar_ModificaE')>readonly</cfif>>
			</td>
		</tr>	
		<cfif not isdefined('Lvar_ModificaE')>
		<tr>
		...
		<tr>
			<td colspan="5" class="formButtons" align="center">
				<input type="button" name="btn_regresar" value="Regresar" class="btnRegresar" onClick="javascript: location.href='ModificarEscenario-lista.cfm';">
			</td>
		</tr>
		</cfif>	
		<tr><td>&nbsp;</td></tr>
