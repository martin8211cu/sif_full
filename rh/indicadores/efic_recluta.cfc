...
<cffunction name="calcular">
	...
	<cfquery datasource="#Arguments.datasource#" >
		...
		select p.CFid, lt.Ocodigo, lt.Dcodigo, ife.fecha, 
			avg (( <!--- antiguedad.  Se usa min() porque si hay varios nombramientos, vale el ultimo --->
				select coalesce (min( datediff (yy, nombramiento.DLfvigencia, ife2.fecha) ), 0)
				from DLaboralesEmpleado nombramiento
					join RHTipoAccion nombramiento_t
					  on  nombramiento.RHTid   = nombramiento_t.RHTid
					  and nombramiento.Ecodigo = nombramiento_t.Ecodigo
					, IndicadorFecha ife2
						 <!--- no se usa el mismo 'ife' porque sybase no aguanta el avg (( min )) con tablas de otro query --->
				where nombramiento.Ecodigo = lt.Ecodigo
				  and nombramiento.DEid    = lt.DEid
				  and nombramiento.DLfvigencia <= ife.fecha
				  and ife2.fecha = ife.fecha
			))
		 as total 
	...
	<cfquery datasource="#Arguments.datasource#" >
		...
		select <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.indicador#">, #Arguments.Ecodigo#, #Arguments.CEcodigo#, 
			...
			cf.CFdescripcion,
			coalesce(j.valor,0) as valor, i.valor as total,
