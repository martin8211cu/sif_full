		...
		<cfelse>
			...
			<!--- 
				...
				<cfif rsExisteOferente.recordCount GT 0>
					<cflocation url="/cfmx/rh/Reclutamiento/curriculumExterno/errorPages/BDerror.cfm?errType=0&errMsg='El Oferente Externo ya existe. Verifique.'" 
						addtoken="no">
					<cfabort> 
				</cfif>
			--->
		...
		<cfif isdefined("Form.RHEEid") and len(trim(Form.RHEEid)) gt 0>
			...
			<cfif isdefined("form.Actualmente")>
				...
				<cfif rsValidaUpdate.recordcount gt 0 >
					<cflocation url="/cfmx/rh/Reclutamiento/curriculumExterno/errorPages/BDerror.cfm?errType=0&errMsg='No puede estar laborando actualmente en la misma empresa m&aacute;s de una vez'" 
						addtoken="no">
					<cfabort> 
				</cfif>			
			</cfif> 	
...
<form action="/cfmx/rh/Reclutamiento/curriculumExterno/curriculum-form.cfm" method="post" name="sql">
	<cfoutput>
		<cfif isdefined("session.RHOid") and len(trim(session.RHOid)) gt 0>
