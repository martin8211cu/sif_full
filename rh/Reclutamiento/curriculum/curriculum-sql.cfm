<!---  <cf_dump var="#Form#">
 <cfdump var="#Form.RHOid#">
 <cfdump var="#Form.RHEElinea#">
 <cf_dump var="#Form.RHEEid#">
  <cf_dump var="#Form.RHEEfunclogros#"> --->
  ...
  <cfif isdefined("form.AccionAEjecutar") and len(trim(form.AccionAEjecutar)) gt 0>
	<cfif form.AccionAEjecutar eq 'ADD-MOD'>
		...
		<!--- **************     DATOS PERSONALES           ********************* --->
		<!--- ******************************************************************* --->
		<cfif isdefined("Form.RHOid") and len(trim(Form.RHOid)) gt 0>
				...
				<cftransaction>
					...
					<cfquery name="ABC_datosOferente" datasource="#Session.DSN#">
						update DatosOferentes set
							...
							RHOfechanac 	  = <cf_jdbcquery_param cfsqltype="cf_sql_date" value="#LSPARSEDATETIME(form.RHOfechanac)#">,
		...
		<cfelse>
			...
			<cftransaction>
				...
				<cfset Form.RHOid = ABC_datosOferente.identity>	
				<cfif isdefined("Form.rutafoto") and form.rutafoto NEQ "">
		...
		<cfif isdefined("Form.RHEEid") and len(trim(Form.RHEEid)) gt 0>
			<cfinvoke component="sif.Componentes.Translate"
				...
				Default="No puede estar laborando actualmente en la misma empresa ms de una vez"
			...
			<cfif isdefined("form.Actualmente")>
				...
				<cfif rsValidaUpdate.recordcount gt 0 >
					<cf_throw message="#MSG_NoPuedeEstarLaborandoActualmenteEnLaMismaEmpresaMasDeUnaVez#" errorcode="3000">
				</cfif>	
			</cfif> 	
		...
		<cfelse>
			<cfif isdefined("Form.RHEEnombreemp") 	and len(trim(Form.RHEEnombreemp)) gt 0 and
			  ...
			  isdefined("Form.RHOPid")    		and len(trim(Form.RHOPid)) gt 0 and 
		...
		<cfif isdefined("Form.RHOElinea") and len(trim(Form.RHOElinea)) gt 0>
			<cfquery name="update" datasource="#Session.DSN#">
				update RHObservacionesEmpleado
				set	
					RHOEfecha  =  <cfqueryparam cfsqltype="cf_sql_date" value="#form.fechaObs#">,
					RHOEobservacion = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.obs#">
				where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#" >
					and RHOElinea = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.RHOElinea#" >
			</cfquery> 
		<cfelse>
			<cfif isdefined("Form.fechaObs") 	and len(trim(Form.fechaObs)) gt 0 and
			  isdefined("Form.obs")    	and len(trim(Form.obs)) gt 0>	
					
					<cfinvoke component="sif.Componentes.Translate"
						method="Translate"
						Key="MSG_NoPuedeIngresarUnaObservacionEnUnaFechaExistente"
						Default="No se puede ingresar una observación en una fecha existente"
						returnvariable="MSG_NoPuedeIngresarUnaObservacionEnUnaFechaExistente"/>
							<cfquery name="rsValida" datasource="#session.DSN#">
								select *
								from RHObservacionesEmpleado
								where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">
									and RHOEfecha =<cfqueryparam cfsqltype="cf_sql_date" value="#form.fechaObs#">
							</cfquery>
							<cfif rsValida.recordcount gt 0 >
								<cf_throw message="#MSG_NoPuedeIngresarUnaObservacionEnUnaFechaExistente#" errorcode="3000">
							</cfif>	
							<cfquery name="insert" datasource="#Session.DSN#">
								insert into RHObservacionesEmpleado (DEid,RHOid,RHOEfecha,RHOEobservacion,BMUsucodigo,BMfecha,Ecodigo)
								values(	null,
										<cfqueryparam cfsqltype="cf_sql_numeric" value="#form.RHOid#">,
										<cfqueryparam cfsqltype="cf_sql_date" value="#form.fechaObs#">,
		 ...
		 <cfif isdefined("Form.RHEElinea") and len(trim(Form.RHEElinea)) gt 0>
			<cfquery name="update" datasource="#Session.DSN#">
				...
				set RHEtitulo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.RHOTDescripcion#">,
					...
					RHOEid = <cfif isdefined("form.RHOEid") and len(trim(form.RHOEid)) gt 0>#form.RHOEid#<cfelse>null</cfif>,
	...
	<cfelseif form.AccionAEjecutar eq 'DEL-EXP'>
		...
		<cfset form.RHEEid = "">
	<cfelseif form.AccionAEjecutar eq 'DEL-OBS'>
		<cfquery name="delete" datasource="#Session.DSN#">
			delete RHObservacionesEmpleado
			where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">
			  and RHOElinea = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.RHOElinea#">
 			  and RHOid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.RHOid#">
		</cfquery> 
		<cfset form.RHEEid = "">
	<cfelseif form.AccionAEjecutar eq 'ADD-CON'>
		<!--- Primero genera el nuevo password --->
		<cfquery name="rs_cedula" datasource="#Session.DSN#">
			select  ltrim(rtrim(RHOidentificacion)) as RHOidentificacion,RHOemail  from DatosOferentes
			where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
			and RHOid   = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHOid#">
		</cfquery>
		...
		<cfquery name="update_password" datasource="#Session.DSN#">
		...
		<cfquery name="RS_user_datos_personales" datasource="#Session.DSN#">
			select RHOnombre ,RHOapellido1 ,RHOapellido2 from DatosOferentes
			where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
			and RHOid   = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHOid#">
		</cfquery>
		<cfset LvarHTTP = 'http://'>
		<cfif findnocase('HTTPS',ucase(cgi.SERVER_PROTOCOL))>
			<cfset LvarHTTP = 'https://'>
		</cfif>
		<cfset  hostname = LvarHTTP & cgi.HTTP_HOST>

		<!--- fcastro 20170111, si existe un dominio para el reclutamiento y selección se utiliza como valor por defecto para el currriculumExterno---->
		<cfquery datasource="asp" name="rsDominio">
			select ssl_todo,dominio from MSDominio where SMcodigo='RECLUTA' and SPcodigo='RHOFEREXTE'
		</cfquery>
		<cfif len(trim(rsDominio.dominio))>
			<cfset LvarHTTP = 'http://'>
			<cfif rsDominio.ssl_todo eq 1>
				<cfset LvarHTTP = 'https://'>
			</cfif>
			<cfset hostname=LvarHTTP&rsDominio.dominio>
		</cfif>
		<cfset hostname&='/cfmx/'>
		...
		<cfset FromEmail= "Curriculum@soin.co.cr">
		...
		<cfquery datasource="#Session.DSN#">
			...
			values (
				...
				<cfqueryparam cfsqltype="cf_sql_varchar" value="#rs_cedula.RHOemail#">,
				<cfqueryparam cfsqltype="cf_sql_varchar" value="Nueva contraseña">,
				<cfqueryparam cfsqltype="cf_sql_varchar" value="#_mail_body#">, 1)
		</cfquery>

		<!--- fcastro, según la necesidad del usuario, una vez enviado el password se procede a autorizar el usuario---->
		<cfquery datasource="#Session.DSN#">
			update DatosOferentes set
				RHAutentificar 		  = 1
			where RHOid   = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHOid#">
		</cfquery>
	<cfelseif form.AccionAEjecutar eq 'ADD-HAB'>
		<cfquery name="ABC_datosOferente" datasource="#Session.DSN#">
			update DatosOferentes set
				RHAprobado 		  = 1
			where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
	...
	<cfelseif form.AccionAEjecutar eq 'ADD-DES'>	
		<cfquery name="ABC_datosOferente" datasource="#Session.DSN#">
			update DatosOferentes set
				RHAprobado 		  = 0
			where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
	...
	<cfelseif form.AccionAEjecutar eq 'ADD-LIST'>
		<cfquery name="ABC_datosOferente" datasource="#Session.DSN#">
			update DatosOferentes set
				RHOlistanegra  = 1
			where RHOid   = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHOid#">
		</cfquery>
	<cfelseif form.AccionAEjecutar eq 'DEL-LIST'>	
		<cfquery name="ABC_datosOferente" datasource="#Session.DSN#">
			update DatosOferentes set
				RHOlistanegra  = 0
			where RHOid   = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RHOid#">
...
<form action="curriculum.cfm" method="post" name="sql">
	<cfoutput>
		<cfif isdefined("Form.RHOid") and len(trim(Form.RHOid)) gt 0>
				<input name="RHOid" type="hidden" value="#Form.RHOid#">
		</cfif>
		<cfif isdefined("Form.RHEElinea") and len(trim(Form.RHEElinea)) gt 0>
