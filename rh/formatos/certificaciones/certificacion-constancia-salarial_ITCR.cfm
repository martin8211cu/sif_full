...
--->
		<cf_dbtemp name="constancias_ITCR_v8" returnvariable="datos" datasource="#session.dsn#">
			...
			<cf_dbtempcol name="nombramiento"		    type="varchar(255)"		mandatory="no">
...
<cfif len(trim(fechasalida)) and datecompare(fechasalida, now()) lt 0 >
	...
	--->
			...
			<cfset alafecha = '' >
			<cfset labora = 'labor' >
			<cfset porlotanto = 'Por lo tanto a partir de esta fecha no tiene ninguna relacin laboral con esta empresa' >
...
<cfelse>
	...
	--->
			...
			<cfif isdefined('NominaAnterior') and NominaAnterior.recordcount gt 0>
...
--->
	...
	<!--- CALCULAR LA CANTIDAD DE AOS Y MESES LABORADO --->
...
--->
	...
	<cfif isnumeric(v_salariopromedio)>
...
<cfquery name="rsRentaP" datasource="#session.DSN#">
	...
	<!--- ASOCIACION Y CARGAS A DESPLEGAR EN CERTIFICACION --->
		...
		<cfset cargas_promedio 		= v_salariopromedio * 0.09 >
	...
	<!--- PORCENTAJE DE OCUPACION DE LA PLAZA --->
		<cfset v_PorcOcupP = 0>
		<cfquery name="rsPorcP" datasource="#session.DSN#">
			select LTporcplaza
			from LineaTiempo
			where DEid = (select DEid from DatosEmpleado where DEidentificacion ='#rsData.DEidentificacion#' and Ecodigo=#session.Ecodigo#)
			and <cfqueryparam cfsqltype="cf_sql_date" value="#LSParseDateTime(rsData.Fecha)#"> between LTdesde and LThasta
		</cfquery>
		<cfif isdefined ('rsPorcP') and rsPorcP.recordcount gt 0 and len(trim(rsPorcP.LTporcplaza)) gt 0>
			<cfset LvarLTporcplazaP= rsPorcP.LTporcplaza>
		<cfelse>
			<cfset LvarLTporcplazaP= 0>
		</cfif>
		<cfquery name="rsPorcPR" datasource="#session.DSN#">
			select coalesce(LTporcplaza,0) as LTporcplaza
			from LineaTiempoR
			where DEid = (select DEid from DatosEmpleado where DEidentificacion ='#rsData.DEidentificacion#' and Ecodigo=#session.Ecodigo#)
			and <cfqueryparam cfsqltype="cf_sql_date" value="#LSParseDateTime(rsData.Fecha)#"> between LTdesde and LThasta
		</cfquery>
		<cfif isdefined ('rsPorcPR') and rsPorcPR.recordcount gt 0 and len(trim(rsProcPR.LTporcplaza)) gt 0>
			<cfset LvarLTporcplaza= rsPorcPR.LTporcplaza>
		<cfelse>
			<cfset LvarLTporcplaza= 0>
		</cfif>
		
		<cfset v_PorcOcupP = LvarLTporcplazaP+ LvarLTporcplaza>

	<!---SALARIO ESCOLAR--->
		<cfquery name="rsDClinea" datasource="#session.dsn#">
			select DClinea from DCargas where DCcodigo='084' and Ecodigo=#session.Ecodigo#
		</cfquery>
		<cfif rsDClinea.recordcount gt 0 and len(trim(rsDClinea.DClinea)) gt 0>
			<cfquery name="rsDE" datasource="#session.dsn#">
				select CEvaloremp from CargasEmpleado
				where DEid = (select DEid from DatosEmpleado where DEidentificacion ='#rsData.DEidentificacion#' and Ecodigo=#session.Ecodigo#)
				and DClinea=#rsDClinea.DClinea#	
			</cfquery>
			<cfif isdefined('rsDE') and len(trim(rsDE.CEvaloremp)) gt 0>
				<cfset LvarSalarioEscolar=rsDE.CEvaloremp>
			<cfelse>
				<cfset LvarSalarioEscolar=0>
			</cfif>
		</cfif>
		
	<!---TIPO DE NOMBRAMIENTO--->
	<cfquery name="rsNomb" datasource="#session.dsn#">
		select max(LTid) as LTid from LineaTiempo where DEid=(select DEid from DatosEmpleado where DEidentificacion ='#rsData.DEidentificacion#' and Ecodigo=#session.Ecodigo#)
	</cfquery>

	<cfif isdefined('rsNomb') and rsNomb.recordcount gt 0>
		<cfquery name="rsLT" datasource="#session.dsn#">
			select LThasta from LineaTiempo where LTid=#rsNomb.LTid#
		</cfquery>
	</cfif>

	<cfif LSDateFormat(rsLT.LThasta,'DD/MM/YYYY') eq '01/01/6100'>
		<cfset LvarNombramiento='Indefinido'>
	<cfelse>
		<cfset LvarNombramiento='Temporal'>
	</cfif>
		
	<!---Categoria--->
	<cfquery name="rsCat" datasource="#session.dsn#">
		select RHCPlinea from LineaTiempo where DEid=(select DEid from DatosEmpleado where DEidentificacion ='#rsData.DEidentificacion#' and Ecodigo=#session.Ecodigo#)
		and #now()# between LTdesde and LThasta
	</cfquery>
	
	<cfif isdefined('rsCat') and rsCat.recordcount gt 0 and len(trim(rsCat.RHCPlinea)) gt 0>
		<cfquery name="Cat" datasource="#session.dsn#">
		select p.RHCid,p.RHCcodigo,p.RHCdescripcion 
		from RHCategoriasPuesto c
			inner join RHCategoria p
			on c.RHCid=p.RHCid
		where RHCPlinea=#rsCat.RHCPlinea#
		</cfquery>
		<cfif Cat.recordcount gt 0>
			<cfset Lvarcat=#Cat.RHCdescripcion#>
		<cfelse>
			<cfset Lvarcat=0>
		</cfif>
	<cfelse>
	...
	<!--- ACTUALIZACION DE DATOS EN TABLA TEMPORAl--->
		<cfquery datasource="#session.DSN#">
			...
			set	fechaingreso 		= '#fechaingreso#', 
				...
				liquido_todasdeduc	= #salario_base - cargas_base  - deducciones#,
				embargo 			= '#embargos#',
...
<cfquery name="rsQuery" datasource="#session.DSN#">
	select 	DEid,
			...
			'#LvarLTporcplazaP#' #LvarCNCT# '%' as porcentaje,
			'#v_PorcOcupP#' #LvarCNCT# '%' as porcentaje_total,
			'#v_salario_base#' as salario_base,
			'#LvarSalarioEscolar#' as salario_escolar,
			...
			'#LvarNombramiento#' as nombramiento,
			embargo,
			tipoembargo,
			labora, 
			centrofunc,
			usuariocentrofunc,
			'#session.Usulogin#' as usuario,
			'#Lvarcat#' as categoria,
