...
<cftransaction>
	<cfloop collection="#objEmps#" item="key">
		...
		<cfquery name="rsEmp" datasource="#session.dsn#">
			...
			left join EmpleadosTipo et on de.DEid=et.DEid
		...
		<cfif DiasFalta gt 0>
			...
			<cfif rsAccion.RecordCount eq 0>
				<cfthrow detail="El tipo de acci&oacute;n con el codigo #varCodAccion# no ha sido definido.">
			</cfif>
			...
			<cfquery name="rsEstadoActual" datasource="#Session.DSN#">
	            ...
	            where 
					(a.LTdesde between <cfqueryparam cfsqltype="cf_sql_timestamp" value="#rsCalendario.CPdesde#"> AND <cfqueryparam cfsqltype="cf_sql_timestamp" value="#rsCalendario.CPhasta#">
						or a.LThasta between <cfqueryparam cfsqltype="cf_sql_timestamp" value="#rsCalendario.CPdesde#"> and <cfqueryparam cfsqltype="cf_sql_timestamp" value="#rsCalendario.CPhasta#">)
				OR	((<cfqueryparam cfsqltype="cf_sql_timestamp" value="#rsCalendario.CPhasta#"> between a.LTdesde and a.LThasta))
	            and a.DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsEmp.DEid#" null="#Len(rsEmp.DEid) is 0#">
	            and a.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
				) EstadoActual where DEid= <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsEmp.DEid#" null="#Len(rsEmp.DEid) is 0#"> and Ecodigo=<cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
	        </cfquery>
		...
		<cfif unObj.Monto1PP gt 0>
			...
			<cfquery name="rsDatosConcepto" datasource="#Session.DSN#"><!---Obtener datos de la incidencia, requeridos por la calculadora--->
				select 	coalesce(b.CItipo,'m') as CItipo,
						...
						<cfqueryparam cfsqltype="cf_sql_numeric" value="#unObj.Monto1PP#"> as Ivalor,
		...
		<cfif unObj.Monto1PA gt 0>
			...
			<cfquery name="rsDatosConcepto" datasource="#Session.DSN#"><!---Obtener datos de la incidencia, requeridos por la calculadora--->
				select 	coalesce(b.CItipo,'m') as CItipo,
						...
						<cfqueryparam cfsqltype="cf_sql_date" value="#LSDateFormat(rsCalendario.CPhasta,'YYYY-MM-dd')#"> as Ifecha,
						<!--- OPARRALES 2018-12-27 Si horasFalta > 0 se convierte a dias y se le resta a los dias laborados (7) --->
						<cfqueryparam cfsqltype="cf_sql_numeric" value="#unObj.Monto1PA#"> as Ivalor,
						<cfqueryparam cfsqltype="cf_sql_numeric" value="#rsEmp.DEid#"> as DEid,
						0 as RHJid,
						'#Trim(form.Tcodigo)#' as Tcodigo,
						coalesce(b.CIcantidad,0) as CIcantidad,
		...
		<cfif Trim(form.Tcodigo) neq '02'> <!--- Faltas menores a 0 Dias, 0 Horas y 30 minutos --->
