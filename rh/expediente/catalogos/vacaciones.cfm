...
<cfinvoke component="sif.Componentes.Translate"
	method="Translate"
	key="BTN_Nuevo_Concepto"
	Default="Nuevo Concepto"
	returnvariable="vNuevoConcepto"/>
...
<cfif isdefined('form.DEid') and form.DEid NEQ ''>
	<cfquery datasource="#Session.DSN#" name="rsEncab">
		...
		where
			...
			and  --->
			eve.DEid=<cfqueryparam cfsqltype="cf_sql_numeric" value="#form.DEid#">
	</cfquery>
</cfif>
...
<cfif isdefined("Form.vaca_ver") and Len(Trim(Form.vaca_ver)) NEQ 0>
	<cfset navegacionVaca = navegacionVaca & Iif(Len(Trim(navegacionVaca)) NEQ 0, DE("&"), DE("")) & "vaca_ver=" & Form.vaca_ver>
	<cfset f_vaca = f_vaca & "vaca_ver='" & Form.vaca_ver & "',">
</cfif>
					...
					<td>
						<form name="formFiltroListaVaca" method="post" action="<cfoutput>#GetFileFromPath(GetTemplatePath())#</cfoutput>">
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0" class="areaFiltro">
								<tr>
									<td class="fileLabel"><cfoutput>#vDescripcion#:</cfoutput></td>
									<cfif trim(rs_p960.Pvalor) eq 1 ><td class="fileLabel"><cfoutput>#vMostrar#:</cfoutput></td></cfif>
									<td rowspan="2" valign="middle"><input class="btnFiltrar" name="btnFiltrarVaca" type="submit" id="btnFiltrarVaca" value="<cfoutput>#vFiltrar#</cfoutput>"></td>
								</tr>

									<tr>
										<td><input name="txtDescripcion" type="text" id="txtDescripcion" size="35" maxlength="60" value="<cfif isdefined('form.txtDescripcion')><cfoutput>#form.txtDescripcion#</cfoutput></cfif>"></td>
										<cfif trim(rs_p960.Pvalor) eq 1 >
											<td class="fileLabel">
												<select name="vaca_ver">
													<option value="">Todos</option>
													<option value="1" <cfif isdefined("form.vaca_ver") and form.vaca_ver eq 1>selected</cfif> ><cf_translate key="LB_Dias_de_Vacaciones">D&iacute;as de Vacaciones</cf_translate></option>
													<option value="2" <cfif isdefined("form.vaca_ver") and form.vaca_ver eq 2>selected</cfif> ><cf_translate key="LB_Dias_de_Enfermedad">D&iacute;as de Enfermedad</cf_translate></option>
												</select>
											</td>
										</cfif>
									</tr>
						  ...
						  <tr>
							...
							<cfquery name="rsLista" datasource="#session.DSN#">
								...
								where
								  <!--- dve.Ecodigo= <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
							      and --->
								   dve.DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.DEid#">
								   <cfif isdefined("filtroVaca") and Len(trim(filtroVaca))>
									  #PreserveSingleQuotes(filtroVaca)#
								  </cfif>

									<cfif isdefined("form.vaca_ver") and len(trim(form.vaca_ver)) >
										<cfif trim(form.vaca_ver) eq 2 >
											and dve.DVEenfermedad != 0
										<cfelse>
											and dve.DVEenfermedad = 0
									  	</cfif>
								  	</cfif>
							...
							<cfinvoke
								 ...
								 returnvariable="pListaFam">
									...
									<cfinvokeargument name="desplegar" value="DVEdescripcion,DVEfecha,DVEperiodo"/>
									<cfinvokeargument name="etiquetas" value="#vDescripcion#,#vFecha#,#vPeriodo#"/>
									<cfinvokeargument name="formatos" value="V,D,I"/>
...
<!--- 							</cfquery>
								<cfinvoke
								 ...
								 method="pListaRH"
								 returnvariable="pListaFam">
									<cfinvokeargument name="tabla" value="DVacacionesEmpleado dve,
												DatosEmpleado de"/>
									<cfinvokeargument name="columnas" value="#f_vaca#
											DVElinea,
											DEid,
											substring(DVEdescripcion,1,25)
											as DVEdescripcion,
									...
									<cfinvokeargument name="formName" value="listaVacaciones"/>
									<cfinvokeargument name="filtro" value="dve.Ecodigo=#session.Ecodigo#
											and dve.DEid=#form.DEid#
											and dve.DEid=de.DEid
											and dve.Ecodigo=de.Ecodigo
											 #filtroVaca#
											 order by DVEfecha"/>
 ...
 --->							</td>
				  ...
				  <cfoutput>
					...
					<form name="formVacacionesEmpl" action="/cfmx/rh/expediente/catalogos/SQLvacaciones.cfm" enctype="multipart/form-data" method="post" onSubmit="return validar();">
						<cfif isdefined("Form.RHAlinea") and Len(Trim(Form.RHAlinea))>
							<input type="hidden" name="RHAlinea" value="<cfoutput>#form.RHAlinea#</cfoutput>">
						</cfif>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
						  ...
						  <tr>
							<td>
								<cfif MODO EQ "ALTA">
									<cf_sifcalendario form="formVacacionesEmpl" name="DVEfecha" value="#LSDateFormat(Now(),'DD/MM/YYYY')#" onBlur="funSugierePeriodo()">
								<cfelse>
									<cfoutput>
										<cf_sifcalendario form="formVacacionesEmpl" name="DVEfecha" value="#LSDateFormat(rsForm.DVEfecha,'DD/MM/YYYY')#" onBlur="funSugierePeriodo()">
									</cfoutput>
								</cfif>
							</td>
						...
						<tr>
							<td colspan="2">
								...
								<select name="DVEperiodo">
									<cfloop query="rsPeriodosDisponibles">
										<option id="#rsPeriodosDisponibles.DVEperiodo#" value="#rsPeriodosDisponibles.DVEperiodo#"
											<cfif modo neq 'ALTA' and rsForm.DVEperiodo EQ rsPeriodosDisponibles.DVEperiodo>selected</cfif> >#rsPeriodosDisponibles.DVEperiodo#
										</option>
									</cfloop>
									<cfif not listFind(valueList(rsPeriodosDisponibles.DVEperiodo),year(now())	)>
										<option id="#year(now())#" value="#year(now())#" <cfif modo neq 'ALTA' and rsForm.DVEperiodo EQ year(now())>selected</cfif>>
											#year(now())#
										</option>
									</cfif>
								</select>
					  ...
					  <tr>
						<td colspan="2" align="center">
							<cfif modo NEQ 'ALTA'>
								<input type="submit" name="btnNuevo"   value="#vNuevoConcepto#" onClick="javascript: setBtn(this); deshabilitarValidacion();" >
							<cfelse>
...
<script language="JavaScript" type="text/javascript">
	...
	function funSugierePeriodo(){
		var f_antig = '#LSDateFormat(rsEncab.EVfecha,"dd/mm/yyyy")#';
		var va_fdigitada = document.formVacacionesEmpl.DVEfecha.value.split('/');

		var va_fantig = f_antig.split('/');

		var vf_digitada = new Date(va_fdigitada[2],va_fdigitada[1],va_fdigitada[0]);
		var vf_antig = new Date(va_fantig[2],va_fantig[1],va_fantig[0]);

		if(vf_digitada > vf_antig){
			if( document.getElementById(va_fdigitada[2]) ) {
				document.getElementById(va_fdigitada[2]).selected = "true";
			}
		}
		if(vf_digitada <= vf_antig){
			if( document.getElementById(va_fdigitada[2]-1) ) {
				document.getElementById(va_fdigitada[2]-1).selected = "true";
			}
		}
	}
