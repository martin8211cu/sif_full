...
<cfquery name="rsMoneda" datasource="#Session.DSN#">
	...
	where b.RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.RCNid#">
</cfquery>
...
<cfif isdefined('form.DEid')>
	<cfquery name="rsEncabEmpleado" datasource="#Session.DSN#">
		select (DEapellido1 || ' ' || DEapellido2 || ', ' || DEnombre) as nombreEmpl, DEemail, DEidentificacion, NTIdescripcion
...
<cfinvoke component="rh.Componentes.RH_ArmaMail" method="ArmaMail" returnvariable="info">
					...
					<cfinvokeargument name="chkIncidencias" value="#form.chkIncidencias#"/>
					<cfinvokeargument name="chkCargas" value="#form.chkCargas#"/>
					<cfinvokeargument name="chkDeducciones" value="#form.chkDeducciones#"/>
					</cfinvoke>
<cfinvoke component="rh.Componentes.RH_ArmaMail" method="tituloMail" returnvariable="titulo">
					<cfinvokeargument name="RCNid" value="#form.RCNid#"/>					
					</cfinvoke>
...
<cfset dequien = getRHPvalor(190)>	

	<cfif len(trim(dequien)) gt 0>
	
		    <cfset paraquien = "">
		
		    <cfset LvarPvalor1 = getRHPvalor(170)>	
			
		<cfif LvarPvalor1 eq 1>
	
	  	   <cfset paraquien = getEmailFromAdmin(getRHPvalor(180))>
		<cfelse>
		
			<cfif isdefined ('rsEncabEmpleado') and len(trim(rsEncabEmpleado.DEemail)) gt 0>
			
				<cfset paraquien = rsEncabEmpleado.DEemail>
			   
			<cfelse>
			
				<cfset emailJefe = getEmailFromJefe(Form.DEid)>
				<cfif len(trim(emailJefe)) gt 0>
				
					<cfset paraquien = emailJefe>
		...
		<cfif  len(trim(paraquien)) gt 0 >
		 ...
		 <cfset Enviar= enviarCorreo(dequien,paraquien,titulo,mensaje)>
	</cfif>
	</cfif>
...
<form action="HResultadoCalculo.cfm" method="get" name="form1">
	...
	<input type="hidden" name="Regresar" value="<cfif isdefined("form.regresar") and len(trim(form.regresar))><cfoutput>#form.regresar#</cfoutput><cfelse>/cfmx/rh/nomina/consultas/ConsultaRCalculo.cfm</cfif>">
	...
	<cfif isdefined('Enviar') and Enviar eq "YES">
	<input type="hidden" name="enviadoMail" id="enviado" value="true">
	<cfelse>
	 <input type="hidden" name="enviadoMail" id="enviado" value="false">
	</cfif>
</form>
</cfoutput>
