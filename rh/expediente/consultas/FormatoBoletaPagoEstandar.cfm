...
<!--- Cambios OPARRALES
	  Con la impresion de boleta de pago con XSLT
	  solo se crea o se consulta el XML guardado.
--->

 <!--- Validacion y creacion de carpetar para almacenar Facturas --->
 <cf_foldersFacturacion>
<cfif ConceptosPago.RecordCount gt 0>
	<cfoutput query="ConceptosPago" group="DEid">
		...
		<!---
			  Si ya esta timbrado se consulta por ruta, se parsea y se imprime en el Formato seleccionado
			  Si no existe se consulta texto y se crea XML
		--->
		<cfif rsReciboCFDI.RecordCount gt 0>
			<cfif fileexists('#vsPath_A#\Enviar\#Session.FileCEmpresa#\#Session.Ecodigo#\xmlST\#rsReciboCFDI.NombreXML#')>

				<cffile action="read" variable="xml32" file="#vsPath_A#\Enviar\#Session.FileCEmpresa#\#Session.Ecodigo#\xmlST\#rsReciboCFDI.NombreXML#" charset="utf-8">
				...
				<cfquery name="rsReporte" datasource="#session.dsn#">
					select
						...
						and a.CPfcalculo is not null and b.RCNid = #ConceptosPago.RCNid#) as CodigoNomina,
						(Select RHRegimendescripcion from RHCFDI_Regimen where RHRegimenid = de.RHRegimenid) as regimenDesc
					FROM
						DatosEmpleado de
					where de.DEid = #ConceptosPago.DEid#
				</cfquery>
				...
				<cfquery name="rsCodigoNomina" datasource="#session.dsn#">
					...
					and b.RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#ConceptosPago.RCNid#">
					and a.CPfcalculo is not null
				</cfquery>

				<cfset ImgQR = "#vsPath_A#\Enviar\#Session.FileCEmpresa#\#Session.Ecodigo#\imgQR\#rsReporte.DEid#_#ConceptosPago.RCNid#.jpg">

				<cfset xmlParams = Structnew()>
				<cfset xmlParams["FolioFiscal"] = rsOP.timbre>
				<cfset xmlParams['NumeroCertificado'] = rsOP.certificadoSAT>
				<cfset xmlParams['FechaCertificado'] = rsOP.fechaTimbrado eq '' ? 'Sin Fecha' : lsdateFormat(replace(rsOP.fechaTimbrado,'T',' '),'dd-MM-YYYY HH-mm-ss')>
				...
				<cfset xmlParams['FechaEmision'] = LSDateFormat(replace(rsOP.fechaTimbrado,'T',' '),'YYYY-MM-dd HH:mm:ss')>
				<cfset xmlParams['ImgQR'] = ImgQR>

				<cfset miHTML = XmlTransform(miXML, miXSLT,xmlParams)>
				<cfoutput>#miHTML#</cfoutput>
				<cfset nombre = "#trim(rsReporte.CLABE)#_"&"#trim(rsReporte.NOMBRE)#"&"-#trim(rsReporte.APE1)#"&"-#trim(rsReporte.APE2)#"&"_#trim(rsReporte.CODIGONOMINA)#.pdf">

				<cfoutput>
				<cfdocument  format="PDF" filename="#vsPath_A#\Enviar\#Session.FileCEmpresa#\#Session.Ecodigo#\PDF\#nombre#" overwrite="true">
					<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
						#miHTML#
				</cfdocument>
				</cfoutput>
				<cfcontent type="application/PDF" file="#vsPath_A#\Enviar\#Session.FileCEmpresa#\#Session.Ecodigo#\PDF\#nombre#" deletefile="true">
...
<!--- OPARRALES
	  Esta funcion se encarga de consultar si existe el archivo XML de la boleta de pago timbrada
	  para leerlo y crear nuevamente el PDF

 --->
<cffunction name="getBoleta" output="true" access="public">
		...
		<cfoutput query="ConceptosPago" group="DEid">
			...
			<cfsavecontent variable="Encabezado">
                ...
                <cfif rsReciboCFDI.existe EQ 0>
                   <cfset valido= Mid(form.cpcodigo,1,4)>
					<cfif valido GT 2013>
                            <cfinvoke component="rh.Componentes.GeneraCFDIs" method="obtenerCFDI">
                                <cfinvokeargument name="DEid" value="#ConceptosPago.DEid#">
                                <cfinvokeargument name="RCNid" value="#ConceptosPago.RCNid#">
                            </cfinvoke>
			...
			<!--- OPARRALES INICIO BOLETA DE PAGO CON XSLT --->

       <!---ERBG Cambio del path fijo INICIA--->
			<cfquery name="rsXML" datasource="#session.dsn#">
				select NombreXML from RH_CFDI_RecibosNomina
				where RCNid = #ConceptosPago.RCNid# and DEid = #ConceptosPago.DEid#
			</cfquery>
			...
			<cffile action="read" file="#vsPath_A#\Enviar\#Session.FileCEmpresa#\#Session.Ecodigo#\xmlST\#rsXML.NombreXML#" variable="xml32" charset="windows-1252">
			<cfset miNombre = "#vsPath_A#\Enviar\#Session.FileCEmpresa#\#Session.Ecodigo#\PDF\#trim(rsReporte.CLABE)#_"&"#trim(rsReporte.NOMBRE)#"&"-#trim(rsReporte.APE1)#"&"-#trim(rsReporte.APE2)#"&"_#trim(rsReporte.CODIGONOMINA)#.pdf">

			<cfif fileexists(miNombre)>
				<cffile file="#miNombre#" action="read" variable="xml32" charset="windows-1252">
			<cfelse>
		        <cfset miXml = replace(#xml32#,"="" ","=""","ALL")>
		        <cfset miXml = replace(#miXml#,"descuento="," descuento=","ALL")>
		        <cfset miXml = replace(#miXml#,"serie="," serie=","ALL")>
				...
				<CFFILE ACTION="read" FILE="#expandPath('/rh/XSLT/CFDI_PDF.xslt')#" variable="miXSLT">
					...
					<!---
					  --- Inicio cambios OPARRALES
					  --- Parametros extras que no vienen en el XML original
					--->

					<cfquery name="rsOP" datasource="#session.dsn#" >
	                    select
	                    	coalesce(timbre,'no hay timbre') as timbre,
							coalesce(certificadoSAT,'no hay certificado') as certificadoSAT,
							coalesce(fechaTimbrado,'') as fechaTimbrado
	                    from RH_CFDI_RecibosNomina
	                    where DEid=<cfqueryparam cfsqltype="cf_sql_numeric" value="#ConceptosPago.DEid#">
	                    and RCNid =<cfqueryparam cfsqltype="cf_sql_numeric" value="#ConceptosPago.RCNid#">
					...
					<cfdocument format="PDF" filename="C:\Enviar\#Session.FileCEmpresa#\#Session.Ecodigo#\PDF\#rsCodigoNomina.CodigoNomina#\#nombre#" overwrite="Yes">
						<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
							#miHTML#
					</cfdocument>
			</cfif>
		...
		<!--- OPARRALES FIN BOLETA DE PAGO CON XSLT --->
           ...
           <!--- <table width="98%" cellpadding="0" cellspacing="0" border="0">
			...
			<cfif continuar>
				...
				<cfelse>
					<tr><td colspan="7" width="950" style="font-size:11px;">#trim(rsEtiquetaPie.Mensaje)#</td></tr>
				</cfif>
                ...
                <cfif rsReciboCFDI.stsTimbre EQ 1>
                <cfset imgQR = "C:\Enviar\imgQR\#rsReciboCFDI.DEid#_#rsReciboCFDI.RCNid#.jpg">
                        ...
                        <table>
                            <tr>
                                <td width="21%" valign="top" class="logoTop" rowspan="2">
                                    <!--- mostrar una imagen desde la base de datos --->
                ...
                <tr>
                    <td colspan="7" width="950">
                        ...
                        <cfset longCadena = len(#rsReciboCFDI.CadenaSAT#)>
                        <cfset iCadena = 1>
                        <cfloop condition="longCadena gt 0">
                        	<tr><td nowrap="nowrap" style="font-size:8px;">#Mid(rsReciboCFDI.CadenaSAT,iCadena,150)#</td></tr>
                        ...
                        <cfset longCadena = len(#rsReciboCFDI.SelloDigital#)>
                        <cfset iCadena = 1>
                        <cfloop condition="longCadena gt 0">
                        	<tr><td nowrap="nowrap" style="font-size:8px;">#Mid(rsReciboCFDI.SelloDigital,iCadena,150)#</td></tr>
                            <cfset longCadena = #longCadena# - 150>
                        	<cfset iCadena = #iCadena# + 150>
                        </cfloop>
                        <!---<tr ><td>#rsReciboCFDI.SelloDigital#</td></tr>--->
                        <tr ><td  nowrap="nowrap" style="font-size:11px;"><strong>Sello digital del SAT</strong></td></tr>
                        <cfset longCadena = len(#rsReciboCFDI.SelloSAT#)>
                        <cfset iCadena = 1>
                        <cfloop condition="longCadena gt 0">
                        	<tr><td nowrap="nowrap" style="font-size:8px;">#Mid(rsReciboCFDI.SelloSAT,iCadena,150)#</td></tr>
                            <cfset longCadena = #longCadena# - 150>
                        	<cfset iCadena = #iCadena# + 150>
                        </cfloop>
                        <!---<tr ><td>#rsReciboCFDI.SelloSAT#</td></tr>--->
                        <tr ><td  nowrap="nowrap" style="font-size:11px;"><strong>No de Serie del Certificado del SAT:</strong> #rsReciboCFDI.certificadoSAT#</td></tr>
                        <tr ><td  nowrap="nowrap" style="font-size:11px;"><strong>Folio Fiscal:</strong> #rsReciboCFDI.Timbre#</td></tr>
                        <tr ><td  nowrap="nowrap" style="font-size:11px;"><strong>Fecha Timbrado:</strong> #rsReciboCFDI.FechaTimbrado#</td></tr>
                     </table>
                   </td>
                </tr>
                <cfelse>
                	<cfset longCadena = len(#rsReciboCFDI.xmlTimbrado#)>
                    <cfset iCadena = 1>
                    <cfloop condition="longCadena gt 0">
                        <tr ><td nowrap="nowrap" style="font-size:11px;"><strong>-#Mid(rsReciboCFDI.xmlTimbrado,iCadena,120)#-</strong></td></tr>
                        <cfset longCadena = #longCadena# - 120>
                        <cfset iCadena = #iCadena# + 120>
                    </cfloop>
                </cfif>

		        <tr><td colspan="7" width="950" align="left" style="font-size:11px;">Este documento es una representación impresa de un CFDI</td></tr>
				<cfset lineaspagina = 0>
                <cfset iniciopagina = true>
                <tr><td style="page-break-before:always;"></td></tr>

<!---				<cfset lineaslleva = lineaslleva+15>
				<cfset lineaspagina = lineaspagina+15>
				<cfset continuar = true>
--->
    ...
    <cfelseif ConceptosPago.RecordCount EQ 0 and ConceptosPagoEspecieTodos.RecordCount NEQ 0>
    	...
    	<cfquery dbtype="query" name="ConceptosPagoEspecie">
            ...
            <cfif ConceptosPagoEspecieTodos.RecordCount NEQ 0>
                <cfquery name="rsReciboCFDIEspecie" datasource="#session.dsn#" >
                    select count(*) existe from RH_CFDI_RecibosNomina
                    where DEid=<cfqueryparam cfsqltype="cf_sql_numeric" value="#ConceptosPagoEspecieTodos.DEid#">
                    and RCNid = #ConceptosPagoEspecie.RCNid#
                    and stsTimbre=1
                </cfquery>
            </cfif>
            ...
            <cfif rsReciboCFDIEspecie.existe EQ 0>
			   <cfset valido= Mid(form.cpcodigo,1,4)>
                <cfif valido GT 2013>
                        <cfinvoke component="rh.Componentes.GeneraCFDIs" method="obtenerCFDI">
                            <cfinvokeargument name="DEid" value="#ConceptosPagoEspecie.DEid#">
                            <cfinvokeargument name="RCNid" value="#ConceptosPagoEspecie.RCNid#">
                        </cfinvoke>
                 </cfif>
            </cfif>
           ...
           <table width="98%" cellpadding="0" cellspacing="0" border="0">
			...
			<cfif continuar>
                ...
                <cfquery name="rsReciboCFDI" datasource="#session.dsn#" >
                    select *  from RH_CFDI_RecibosNomina
                    where DEid=<cfqueryparam cfsqltype="cf_sql_numeric" value="#ConceptosPagoEspecie.DEid#">
                    and RCNid = #ConceptosPagoEspecie.RCNid#
                    and stsTimbre=1
            	</cfquery>
                <cfif rsReciboCFDI.stsTimbre EQ 1>
                <cfset imgQR =#vsPath_A#&"\Enviar\#Session.FileCEmpresa#\#Session.Ecodigo#\imgQR\#rsReciboCFDI.DEid#_#rsReciboCFDI.RCNid#.jpg">
