...
<cfparam name="formato" default="pdf">
...
<!--- Ejemplo Nomina2019 --->
<cfset rutaDirTmp = '#ruta#/Nomina#Year(rsCP.CPfpago)#'>
...
<cfset rutaDirTmp = '#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#'>
<cfif !directoryExists(rutaDirTmp)>
    <cfset directoryCreate(rutaDirTmp)>
</cfif>

<!--- Ejemplo Nomina2019/Nomina-DEMO/2019-01-001/PDF --->
<cfset rutaDirTmp = '#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/PDF'>
<cfif !directoryExists(rutaDirTmp)>
    <cfset directoryCreate(rutaDirTmp)>
</cfif>

<!--- Ejemplo Nomina2019/Nomina-DEMO/2019-01-001/imgQR --->
<cfset rutaDirTmp = '#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/imgQR'>
<cfif !directoryExists(rutaDirTmp)>
    <cfset directoryCreate(rutaDirTmp)>
</cfif>

<!--- Ejemplo Nomina2019/Nomina-DEMO/2019-01-001/xmlST --->
<cfset rutaDirTmp = '#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/xmlST'>
<cfif !directoryExists(rutaDirTmp)>
    <cfset directoryCreate(rutaDirTmp)>
</cfif>

<!--- Ejemplo Nomina2019/Nomina-DEMO/2019-01-001/xmlT --->
<cfset rutaDirTmp = '#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/xmlT'>
<cfif !directoryExists(rutaDirTmp)>
    <cfset directoryCreate(rutaDirTmp)>
</cfif>
...
<cffunction name="PDFxEmpleado" access="public" returntype="String">
    ...
    <cfquery name="rsReporte" datasource="#session.dsn#">
        select
            ...
            coalesce(de.DEdireccion,', ',de.DEcodPostal) as DEdireccion,
            de.DEcodPostal as RecidenciaReceptor,
            de.DEcuenta as noCuenta,
    ...
    <cfquery name="rsBanco" datasource="#session.dsn#">
        SELECT 	b.Iaba as clave,a.Bid,
				b.Bdescripcion as NombreBanco
        FROM DatosEmpleado a INNER JOIN Bancos b
        ON  a.Bid= b.Bid
        WHERE a.DEid=#Arguments.DEid#
    </cfquery>
    ...
    <cfelse>
        <cffile action="read" variable="xml32" file="#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/xmlST/#Arguments.DEid#_#Arguments.RCNid#.xml" charset="utf-8">
        <cfset rsEmp.Emp = replace("#rsEmp.Emp#","/","-","All")>
        <cfset rsNomina.RCDescripcion = replace("#rsNomina.RCDescripcion#","/","-","All")>
        <cffile action="read" variable="xml32Timbrado" file="#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/xmlT/#rsEmp.Emp#_#rsNomina.RCDescripcion#T.xml" charset="utf-8">
        ...
        <cfset miXml = replace(#miXml#,"</nomina12:OtrosPagos>","</OtrosPagos>")>
        <cfset miXml = replace(#miXml#,"<nomina12:OtroPago ","<OtroPago ","ALL")>
        <cfset miXml = replace(#miXml#,"</nomina12:OtroPago>","</OtroPago>","ALL")>
        ...
        <CFFILE ACTION="read" FILE="#expandPath('/rh/XSLT/CFDI_PDF40.xslt')#" variable="miXSLT">


        <!--- VALIDACION 30 Y 40-336------------------------------------------------------------------------------------- OPARRALES
        ...
        <!---cfquery name="rsPuesto" datasource="#session.dsn#">
         ...
         <cfquery name="rsPuesto" datasource="#session.dsn#">
            select top(1) p.RHPdescpuesto
            from LineaTiempo lt
            inner join RHPuestos p on lt.RHPcodigo = p.RHPcodigo
            where lt.Deid=#Arguments.DEid# order by LTdesde DESC
        </cfquery>

         <cfquery name="rsPDFCFDI" datasource="#session.dsn#">
            select Pvalor from RHParametros where Pcodigo = 2800
            and Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
        </cfquery>

        <cfset ImgPath = "#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/imgQR/#rsReporte.DEid#_#Arguments.RCNid#.jpg">
        ...
        <cftry>
            ...
            <cfset lVarQRFE= sXMLT['cfdi:Comprobante']['cfdi:Complemento']['XmlChildren'][2].XmlAttributes.SelloCFD>
        ...
        <cfset CadenaOriginal =rsOP.CadenaOriginal >
        ...
        <cfset xmlParams['ImgQR']        = "file:///#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/imgQR/#Trim(rsReporte.DEid)#_#Trim(Arguments.RCNid)#.jpg">
        <cfset xmlParams['Puesto']       = rsPuesto.RHPdescpuesto>
        <cfset xmlParams['Direccion']    = wrap(rsReporte.DEdireccion,45,true)>
        <cfset xmlParams['CodigoPostal']  = rsReporte.RecidenciaReceptor>
        <cfset xmlParams['Cuenta']       = rsReporte.noCuenta>
        <cfset xmlParams['Banco']        = rsBanco.NombreBanco>
        <cfset xmlParams['selloSAT']     = wrap(lVarQRFE,90,true)>
        <cfset xmlParams['selloCFDI']    = wrap(selloCFD,90,true)>
        <cfset xmlParams['cadenaSAT']    = wrap(CadenaOriginal,90,true)>
        <cfset xmlParams['valCero']      = "0.00">
        <cfset xmlParams['Exportacion']      = "NO APLICA">
        <cfset xmlParams['CveServicio']      = "84111505">
        <cfif rsPDFCFDI.Pvalor eq 'S'>
            <cfset xmlParams['PDFCFDI']      = "1">
        <cfelse>
            <cfset xmlParams['PDFCFDI']      = "0">
        </cfif>
        <cfif dirFiscEmpresa.DirecFisc neq '1'>
            <cfset xmlParams['DirFiscal']        = "0">
        <cfelse>
            <cfset xmlParams['DirFiscal']        = "1">
        </cfif>
        <cfif dirFiscEmpresa.DirecFisc eq '1'>
            <cfset xmlParams['calle']     = trim(datosEmpresa.Calle)>
            <cfset xmlParams['nInt']    = trim(datosEmpresa.NumInt)>
            <cfset xmlParams['nExt']    = trim(datosEmpresa.NumExt)>
            <cfset xmlParams['colonia']    = datosEmpresa.Colonia>
            <cfset xmlParams['localidad']    = datosEmpresa.Localidad>
            <cfset xmlParams['delegacion']    = datosEmpresa.Delegacion>
            <cfset xmlParams['pais']    = datosEmpresa.Pais>
            <cfif isDefined('datosEmpresa.Referencia') and len('datosEmpresa.Referencia')>
                <cfset xmlParams['referencia']    = datosEmpresa.Referencia>
            <cfelse>
                <cfset xmlParams['referencia']    = "ND">
            </cfif>
            <cfset xmlParams['cPostal']    = datosEmpresa.codPostal>
            <cfset xmlParams['estado']    = datosEmpresa.Estado>
        <cfelse>
            <cfset xmlParams['direccion1']    = datosEmpresa.direccion1>
            <cfset xmlParams['direccion2']    = datosEmpresa.direccion2>
            <cfset xmlParams['estado']    = datosEmpresa.Estado>
            <cfset xmlParams['cPostal']    = datosEmpresa.codPostal>
        </cfif>
        ...
        <cfset xmlParams['cssRecibo'] = "file:///#Replace(expandPath('/rh/css/BoletasPagoconXSLT.css'), "\", "/", "all")#">

        <cfset miHTML = XmlTransform(miXML, miXSLT,xmlParams)>

        <cfoutput>
            <cfif !directoryExists('#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/PDF/')>
                <cfset directoryCreate('#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/PDF/')>
            </cfif>

            <!--- Al poner filename guarda el archivo en la ruta especificada con el nombre dado. --->  
            <cfdocument  format="PDF" filename="#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/PDF/#miNombre#" mimetype="image/png" overwrite="true" localurl="false">
                <!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 5.0 Transitional//EN">
                    #miHTML#
            </cfdocument>
        </cfoutput>
...
<cffunction name="correoXEmpleado" access="private" returntype="struct">
    ...
    <cftransaction>
        ...
        <!--- Ejemplo Nomina2019/Nomina-DEMO/2019-01-001/PDF --->
        <cfset rutaDirTmpPDF = '#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/PDF'>
        ...
        <cfset rutaDirTmpXML = '#ruta#/Nomina#Year(rsCP.CPfpago)#/#Trim(rsCP.CPcodigo)#/xmlT'>
...
<cffunction  name="getDirFiscE">
    <cfquery name="rsDirFiscE" datasource="#session.dsn#">
			SELECT  DirecFisc
			FROM    Empresas
