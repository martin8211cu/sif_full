...
<!---		<cfquery datasource="#session.dsn#" name="rsDatosG">
			...
			c.PEsalario/ <cf_dbfunction name="to_number" args="b.FactorDiasSalario">, d.DESeguroSocial,
			(select sum(PEcantdias) from HPagosEmpleado e where e.RCNid = a.CPid and e.DEid = #ConceptosPago.DEid#  and c.PEtiporeg = 0) Dias
		...
		<cfquery datasource="#session.dsn#" name="RsDias">
			select coalesce(sum(PEcantdias),0) as PEcantdias
			  ...
			  where a.DEid  	   = #ConceptosPago.DEid#
				and a.RCNid 	   = #form.CPid#
				and a.PEtiporeg    = 0
<!---				and d.RHTcomportam not in (13,5) --->
		</cfquery>
		
	
        <cfif RsFaltas.RecordCount>
            <cfset DiasFaltas = #lsnumberformat(RsFaltas.PEcantdias,'99.999')# & ' Dias'>
        <cfelse>
            <cfset DiasFaltas = '0 Dias'>
        </cfif>

<!---        <cfquery datasource="#session.dsn#" name="RsSalDir">
            select distinct coalesce((PEsalarioref / 30),0) as Monto
            from HPagosEmpleado a
            where a.DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#ConceptosPago.DEid#">
            ...
            <cfif isdefined('RsFaltas') and RsFaltas.recordcount GT 0>
             	<cfset LvarDias 		= RsDias.PEcantdias  - RsFaltas.PEcantdias  >
            <cfelse>
           ...
           <table width="98%" cellpadding="0" cellspacing="0" border="0">	
			...
			<cfset lineaslleva = 10>	<!----Lineas del encabezado---->
				...
				<tr style="font-size:10px"> 
					...
					<td valign="top" nowrap="nowrap">						
						<cfif len(trim(ConceptosPagoEspecie.montoconceptoB)) NEQ 0 and trim(ConceptosPagoEspecie.montoconceptoB) LT 0 >
							<cfset ConceptosPagoEspecie.descconceptoB = '#LB_DEVOLUCION# #ConceptosPagoEspecie.descconceptoB#'>
						</cfif>						
						<cfif len(trim(descconceptoB))>
							<cfif len(descconceptoB) GT 29>
								#Mid(descconceptoB, 1, 29)#
			...
			<cfset lineaslleva = lineaslleva+3>	
			<cfset lineaspagina = lineaspagina+3>			
			<cfset continuar = true>
			...
			<cfif continuar>
				...
				<cfset lineaslleva = lineaslleva+7>	
				<cfset lineaspagina = lineaspagina+7>			
				<cfset continuar = true>
