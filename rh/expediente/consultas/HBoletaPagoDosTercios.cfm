...
<cf_dbtemp name="TMPConceptos" returnvariable="TMPConceptos" datasource="#session.DSN#">
	...
	<cfif arguments.columna EQ 1>
		<cfquery datasource="#session.DSN#">
			insert into #TMPConceptos# (DEid,RCNid,descconcepto,cantconcepto,montoconcepto,linea,columna,nomina,
										desdenomina,hastanomina,empleado,cuenta,departamento,puntoventa,EtiquetaCuenta)
...
<cfif rsSalBrutoRelacion.RecordCount NEQ 0>
	<cfset x= funcInsertaLinea(form.DEid,Form.RCNid,'#LB_SalarioBruto#','#rsSalBrutoRelacion.cantidad#',rsSalBrutoRelacion.Monto,VN_CUENTALINEAS,1)>
</cfif>
...
<cfif rsRetroactivos.RecordCount NEQ 0 and rsRetroactivos.Monto NEQ 0>
	<cfif rsParametro.Pvalor EQ 30><!----Cuando el calculo es para la boleta de 1/2 de Coopelesca--->
		<cfset x= funcInsertaLinea(form.DEid,Form.RCNid,'#LB_Retroactivos#','',rsRetroactivos.Monto,VN_CUENTALINEAS,1)>
	<cfelse>	
		<cfset x= funcInsertaLinea(form.DEid,Form.RCNid,'#LB_Retroactivos#','#rsRetroactivos.cantidad#',rsRetroactivos.Monto,VN_CUENTALINEAS,1)>
	</cfif>
</cfif>
...
<cfif rsSalarioEmpleado.RecordCount NEQ 0 and rsSalarioEmpleado.SErenta NEQ 0>
	<cfset x= funcInsertaLinea(form.DEid,Form.RCNid,'#LB_Renta#','',rsSalarioEmpleado.SErenta,VN_CUENTALINEAS_2,2)>
</cfif>
...
<cfif rsIncidenciasCalculo.recordCount GT 0>
	...
	<cfloop query="rsIncidenciasCalculo">		
		<cfset x= funcInsertaLinea(form.DEid,Form.RCNid,rsIncidenciasCalculo.CIdescripcion,'#rsIncidenciasCalculo.Valor#',rsIncidenciasCalculo.ICmontores,VN_CUENTALINEAS,1)>
	</cfloop>
...
<cfloop query="rsCargas">
    <cfif rsCargas.ECresumido is 1>		
	       <cfif idBandera is not #rsCargas.ECid#>
			...
			<!---====== Insertar en la temporal ======----->
			<cfset x= funcInsertaLinea(form.DEid,Form.RCNid,rsCargasResumido.descripcion,'',rsCargasResumido.empleado,VN_CUENTALINEAS_2,2)>
			<cfset	idBandera=#rsCargas.ECid# >	
	      </cfif>
	<cfelse>
	<cfset x= funcInsertaLinea(form.DEid,Form.RCNid,rsCargas.DCdescripcion,'',rsCargas.CCvaloremp,VN_CUENTALINEAS_2,2)>
	</cfif>			
</cfloop>
...
<cfloop query="rsDeducciones">
	<!---====== Insertar en la temporal ======----->
	<cfset x= funcInsertaLinea(form.DEid,Form.RCNid,rsDeducciones.Ddescripcion,'',rsDeducciones.DCvalor,VN_CUENTALINEAS_2,2)>
</cfloop>
