...
<cfquery name="rsReporte1" datasource="#session.DSN#">
		select	1 as tipo,
				...
				e.RHHtipo as prioridad,
				case
					when e.RHHtipo = 0 then 'Competencia Primordial' 
					when e.RHHtipo = 1 then 'Competencia Básica' 
		...
		select	2 as tipo,
				...
				e.RHCtipo as prioridad,
				case
					when e.RHCtipo = 0 then 'Competencia Primordial' 
					when e.RHCtipo = 1 then 'Competencia Básica' 
...
<cfquery name="rsReporte2" dbtype="query">
	...
	order by CFid,identificacion,brecha desc, tipo, prioridad,competencia
</cfquery>
<cfreport format="#url.formato#" template= "PlanCompetencias.cfr" query="rsReporte2">
