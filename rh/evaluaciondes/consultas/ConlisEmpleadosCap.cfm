...
<cfif isdefined("Url.filtro") and not isdefined("Form.filtro")>
	<cfparam name="Form.filtro" default="#Url.filtro#">
</cfif>
...
<cfif isdefined("Form.FDEnombre") and Len(Trim(Form.FDEnombre)) NEQ 0>
	<cf_dbfunction name = 'OP_concat' returnvariable='OP_CAT'>
 	<cfset filtro = filtro & " and upper(a.DEapellido1 #OP_CAT# ' ' #OP_CAT# a.DEapellido2 #OP_CAT# ', ' #OP_CAT# a.DEnombre) like '%" & UCase(Form.FDEnombre) & "%'">
...
<cfif isdefined("Form.filtro")>
	<cfset navegacion = navegacion & Iif(Len(Trim(navegacion)) NEQ 0, DE("&"), DE("")) & "filtro=" & Form.filtro>
</cfif>
...
<input type="hidden" name="filtro" value="#Form.filtro#">
...
<table width="98%" border="0" cellpadding="2" cellspacing="0" class="areaFiltro">
  <tr> 
    <td align="right"><cf_translate key="LB_Identificacion" XmlFile="/rh/generales.xml">Identificaci&oacute;n</cf_translate></td>
    <td> 
    ...
    <td align="right"><cf_translate key="LB_Nombre" XmlFile="/rh/generales.xml">Nombre</cf_translate></td>
    <td> 
...
<cfquery name="data" datasource="#session.DSN#">
	...
	<cfif isdefined("form.filtro") and len(trim(form.filtro))>
		and a.DEid in ( select DEid from RHListaEvalDes where Ecodigo = #session.Ecodigo# and RHEEid = #form.filtro# )
	</cfif>
