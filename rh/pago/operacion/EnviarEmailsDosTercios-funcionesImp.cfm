...
<cffunction access="private" name="prepararCorreo" output="true" returntype="string">
	...
	<cf_dbtemp name="TMPConceptos78RRX" returnvariable="TMPConceptosRX" datasource="#session.DSN#">
	...
	<cfquery name="rsEncabEmpleado" datasource="#Session.DSN#">
		select 	{fn concat (ltrim(rtrim(DEtarjeta)), {fn concat('   ',{fn concat({fn concat({fn concat({fn concat(de.DEapellido1 , ' ' )}, de.DEapellido2 )}, ' ' )}, de.DEnombre )})})}as nombreEmpl	, DEemail, DEidentificacion, NTIdescripcion
				,DEinfo1
	...
	<cfquery name="rsCargas" datasource="#Session.DSN#">
		<!---select <cf_dbfunction name="to_char" args="a.DClinea"  >  as DClinea, CCvaloremp, CCvalorpat, DCdescripcion, ECauto--->
		...
		where RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RCNid#">
		  ...
		  and CCvaloremp <> 0
		group by DCdescripcion,a.DEid, c.ECresumido, c.ECid
		order by c.ECid  
	</cfquery>
	
	<!---====== Insertar en la temporal, verifica si la carga se tiene que mostrar en forma resumida si es asi la agrupa de acuerdo al id del encabezado de la carga ======----->
    ...
    <cfloop query="rsCargas">				
	   <cfif rsCargas.ECresumido is 1>		
		...
		<cfset x= funcInsertaLinea(arguments.DEid,arguments.RCNid,rsCargasResumido.descripcion,'',rsCargasResumido.empleado,VN_CUENTALINEAS_2,2,0,0)>				
	...
	<cfloop query="rsDeducciones">
		<cfif rsDeducciones.Dcontrolsaldo EQ 1> 
			...
			<cfquery name="rsSaldo" datasource="#Session.DSN#">		  
				...
				where a.RCNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.RCNid#">
					and a.DEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.DEid#">		
					and b.Did = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Did#"> 		
					and a.Did = b.Did
			...
			<cfset saldoDeducc = rsSaldo.Dsaldo - rsSaldo.DCvalor>				
