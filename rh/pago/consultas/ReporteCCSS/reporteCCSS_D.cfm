<cffunction name="fnDetalleCCSS1" access="private" output="no">
	...
	<!---ljimenez lee el parametro para ver si toma en cuenta los pagos retroactivos para el reporte--->
	...
	<cfelse>
		<cfquery  datasource="#Session.DSN#" name="LZ">
			insert into  #Temp# (DEid, Numpatronal, Ecodigo, FECHDE, FECHAS, Salario, CPmes, CPperiodo)
			select 
				pe.DEid, 
				'#Arguments.NumeroPatronal#' as NumPat, 
				#session.Ecodigo# as Ecodigo, 
	  ...
	  <!---		and cp.CPperiodo 	= pe.CPperiodo
			  	...
			  	and pe.Ocodigo in (#Arguments.ListaOficinas#)
			group by pe.DEid,pe.CPmes, pe.CPperiodo
			order by pe.CPmes, pe.CPperiodo
		</cfquery>
		...
		<cfquery datasource="#Session.DSN#">
			...
			set Incidencias = coalesce((
									...
									<!---	and ic.CPperiodo 	= cp.CPperiodo
											...
											and ci.CInocargasley 	= 0
											and ic.DEid     		= #Temp#.DEid
											and ic.CPperiodo 	= #Temp#.CPperiodo
											and ic.CPmes 		= #Temp#.CPmes
		...
		<cfquery datasource="#Session.DSN#">
			...
			set Salarioa = coalesce((
					...
					where cp.Ecodigo        = #session.Ecodigo#
						...
						and cp.CPnocargasley 	= 0
						and #Temp#.DEid 	= pe.DEid
						and #Temp#.CPperiodo     	= pe.CPperiodo 
						and #Temp#.CPmes          	= pe.CPmes
		...
		<cfquery datasource="#Session.DSN#">
			...
			set Incidenciasa = isnull((select sum(ICmontores)
									...
									where cp.Ecodigo        	= #session.Ecodigo#
										...
										and cp.CPnocargasley 	= 0
										and #Temp#.DEid 		= ic.DEid 
										and #Temp#.CPperiodo   	= cp.CPperiodo  
										and #Temp#.CPmes       	= cp.CPmes
								),0)
		</cfquery>
	</cfif>
...
<cffunction name="fnDetalleCCSS2" access="private" output="no">
	...
	<cfquery datasource="#Session.DSN#">
		...
		where  p.Ecodigo   = #session.Ecodigo#
			and a.CPperiodo	= #form.CPperiodo#   
			and a.CPmes    	= #form.CPmes# 	
	</cfquery>
	...
	<!--- 	la.DLfvigencia  LZ 27-3-1976 Se Elimina esta opción 
		...
		where a.CPperiodo	= #form.CPperiodo#   
			and a.CPmes    	= #form.CPmes# 	
	</cfquery>
	...
	<cfquery datasource="#Session.DSN#">
		INSERT  into #registro35#( DEid, FECINI, MONTO, TIPCAM, FECFIN, TINCAPACI, NUMJOR, RHJid, CLAJOR, RHOcodigo, CONSE)
			...
			where not exists (select 1 
								from #registro35# r35
								where r35.DEid = a.DEid
								and r35.TIPCAM = 'IC'
								and r35.FECINI= lt.LTdesde)		  
					and a.CPperiodo	= #form.CPperiodo#   
					and a.CPmes    	= #form.CPmes# 	
	<!--- el reporte estaba insertado 2 líneas de IC (inclusión)  ---> 
	</cfquery>
		...
		INSERT  into #registro35#( DEid, FECINI, MONTO, TIPCAM, FECFIN, TINCAPACI, NUMJOR, RHJid, RHOcodigo, CONSE ) 
			...
			from LineaTiempo lt
				inner join RHSaldoPagosExceso sp
					...
					and   lt.DEid = sp.DEid
					and   ((sp.RHSPEfdesde >= #Arguments.fecini# 
...
<cffunction name="fnDetalleCCSS8a" access="private" output="no">
		 ...
		 where lt.Ecodigo = #session.Ecodigo#
			...
			and exists (select 1 
						from DLaboralesEmpleado dl, 
							RHTipoAccion rt						
						where dl.RHTid = rt.RHTid
						and rt.RHTcomportam = 2
						and dl.DEid = lta.DEid
						and dl.DLfvigencia < #Arguments.fecini#)
		<!--- se agregó el exists ya que estaba insertando una exclusión sin existir un cese de por medio --->																	
		 <!--- 
	...
	<!--- ***********************************************************************************************************************************---> 
	<!--- 10.1 Inserto los cambios de salario para los ceses que se hicieron el Primero del Mes y aun se cargaron con salario 0 en #Temp#  --->
	<!--- ***********************************************************************************************************************************---> 
	<cfquery datasource="#Session.DSN#">
		 INSERT  into #registro35#( DEid, FECINI, MONTO, TIPCAM, FECFIN, TINCAPACI, RHOcodigo, CONSE)
		 select  
	...
	<!--- *****************************************************---> 
	<!--- 10.1.1 Ceses que recibieron salario                    --->
	<!--- *****************************************************---> 
	<cfquery datasource="#Session.DSN#">
		 INSERT  into #registro35#( DEid, FECINI, MONTO, TIPCAM, FECFIN, TINCAPACI, RHOcodigo, CONSE)
		 select  
	...
	<!--- ********************************************************************************************************************************---> 
	<!--- 10.2 Inserto los cambios de salario para los ceses que se hicieron el Primero del Mes y por eso no tinen salario en el periodo  --->
	<!--- ********************************************************************************************************************************---> 
	<cfquery datasource="#Session.DSN#">
		 INSERT  into #registro35#( DEid, FECINI, MONTO, TIPCAM, FECFIN, TINCAPACI, RHOcodigo, CONSE)
		 select  
	...
	<!--- ****************************************************************************************************---> 
	<!--- 10.3 Inserto los ceses que se hicieron el Primero del Mes y por eso no tinen salario en el periodo  --->
	<!--- ****************************************************************************************************---> 
	<cfquery datasource="#Session.DSN#">
		 INSERT  into #registro35#( DEid, FECINI, MONTO, TIPCAM, FECFIN, TINCAPACI, RHOcodigo, CONSE)
		 select  
	...
	<cfquery datasource="#Session.DSN#">
		...
		and exists (select 1 
					...
					and exists (Select 1 
								from DLaboralesEmpleado dl,
									RHTipoAccion ta
								Where a.DEid=dl.DEid
								and dl.RHTid=ta.RHTid
								and ta.RHTcomportam=2
								and <cf_dbfunction name="dateadd" args="-1, a.LTdesde"> = dl.DLfvigencia					
								)
					)
	</cfquery>
	<!---cambio realizado para evitar el borrado de una inclusión mes anterior mes anterior, teniendo de por medio una acción de comportamiento tipo cambio--->
	<cfquery datasource="#Session.DSN#">
		delete #registro35#
		where TIPCAM = 'EX'
		and exists (select 1 
...
<cffunction name="fnDetalleCCSS9b" access="private" output="no">
	...
	<cfif len(rtrim(ltrim(#vCMagisterio#)))  GT 0 >
		<cfquery datasource="#Session.DSN#">
			update #registro35#
			set CLASEG = '#vIndRep#'
			where #registro35#.DEid = (select DEid
											from CargasEmpleado
											where DEid = #registro35#.DEid
											and CEhasta is null
	...
	<!---ljimenez fin actualiza el tipo de seguro---> 
	...
	<cfloop query="RS_Datos">
		 <cfset HILERA = "35"
			...
			& trim(mid(RS_Datos.RHOcodigo,1,4))&  RepeatString(' ', (4 -len(trim(mid(RS_Datos.RHOcodigo,1,4)))))
