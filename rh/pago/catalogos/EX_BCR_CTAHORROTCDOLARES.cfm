...
<!--- ARCHIVO QUE GENERA TXT CON LOS DATOS DE LA PLANILLA PARA EL BCR CUANDO SE UTILIZA EL FORMATO DE LA CUENTA DE AHORRO
	...
	FUNCIONA EN DOLARES CON TIPO DE CAMBIO, POR LO QUE SE USA EL MISMO ALMACENADO EN NOMINA (CASO BARODA)
<!---=====================================================================================================================================---->
...
<!---====== 2. NUMERO NEGOCIO: NÃºmero de cÃ©dula jurÃ­dica de la empresa ====---->
...
<cfelse>
	<cfquery name="ERR" datasource="#session.DSN#">
		select 'No se ha definido la cÃ©dula jurÃ­dica (IdentificaciÃ³n) de la empresa, indicarlo en InformaciÃ³n de Bancos [Codigo Cliente]' as Error
		from dual
	</cfquery>
...
<cfelse>
	<cfquery name="ERR" datasource="#session.DSN#">
		select 'El usuario no puede realizar la exportaciÃ³n' as Error
		from dual
	</cfquery>
...
<cfif vb_cedregistro and vb_cedjuridica>
	...
	<!----====================== Datos de creditos ======================---->
	<!----=====================================================================---->
	<cfset NominaHistorica='P'>
	<cfquery name="Historico" datasource="#session.DSN#">
	...
	<cfif Historico.recordcount EQ 0>
			<cfset NominaHistorica='H'>
			<cfquery name="Historico" datasource="#session.DSN#">
					Select distinct coalesce(b.RCtc,1) as TipoCambio
	...
	<cfif NominaHistorica EQ 'P'>
				<cfquery name="rsDatos" datasource="#session.DSN#"><!---Seleccionar los datos----->
					select 	ltrim(rtrim(c.DEcuenta)) as CtaAhorro,
							...
							coalesce(<cf_dbfunction name="to_number" args="(b.DRNliquido/#Historico.TipoCambio#) * 100">,0) as Monto,
							'2' as Moneda,					
	...
	<cfelse>
				<cfquery name="rsDatos" datasource="#session.DSN#"><!---Seleccionar los datos----->
					select 	ltrim(rtrim(c.DEcuenta)) as CtaAhorro,
							case when c.CBTcodigo = 0 then 1 else 2 end as TipoCuenta, <!---1=Corriente,2=Ahorros--->
							coalesce(<cf_dbfunction name="to_number" args="(b.HDRNliquido/#Historico.TipoCambio#) * 100">,0) as Monto,
							'2' as Moneda,					
							coalesce(a.HERNdescripcion,'Pago de planilla') as razon,	
							(select max(y.CPfpago) from CalendarioPagos y where y.CPid = a.RCNid) as fechapago
							<!----Datos para tabla de errores--->
							,c.DEidentificacion
							,{fn concat(c.DEapellido1,
...
<cfif NominaHistorica EQ 'P'>
		...
		<cfquery name="rsMonto"  datasource="#session.DSN#">		
			select 	sum(coalesce(<cf_dbfunction name="to_number" args="(b.DRNliquido/#Historico.TipoCambio#) * 100">,0)) as Monto,					
					'2' as Moneda,
					a.RCNid	
			from ERNomina a
				inner join DRNomina b
...
<cfelse>
	<cfquery name="rsCuentaCliente" datasource="#session.DSN#">
		select 	ltrim(rtrim(CBcc)) as CuentaCliente
				,coalesce(HERNdescripcion,'Pago de planilla') as razon	
		from HERNomina
		where Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
			and ERNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#url.ERNid#">  
	</cfquery>
	<!--- Suma de los montos a acreditar a c/empleado (Registros de detalle) (Total a debitar)---->
	<cfquery name="rsMonto"  datasource="#session.DSN#">		
		select 	sum(coalesce(<cf_dbfunction name="to_number" args="(b.HDRNliquido/#Historico.TipoCambio#) * 100">,0)) as Monto,					
				'2' as Moneda,
				a.RCNid	
		from HERNomina a
			inner join HDRNomina b
	...
	<cfif rsErrores.RecordCount NEQ 0>
		...
		<cfquery datasource="#session.DSN#">
			insert into #Errores# (error,ordenerror)
			values('El formato de la cuenta debe ser XXX-YYYYYYYZ. Donde X corresponde a la oficina, Y corresponde a la cuenta y Z corresponde a el dÃ­gito de la cuenta',1)
		</cfquery>	
		<cfquery name="ERR" datasource="#session.DSN#">
			select * from #Errores#
			order by ordenerror
	...
	<cfelse>		
		...
		<cfelse>
				<cfquery name="rs1" datasource="#session.DSN#">
					...
					select  <cf_dbfunction name="string_part" args="b.DEcuenta,1,3"> as oficina,
							...
							<cf_dbfunction name="string_part" args="b.DEcuenta,1,12"> as oficinacuenta,
							rtrim(b.DEcuenta) as DECUENTA,
							case HDRNliquido when 0 then 1 else HDRNliquido end,
							(select max(y.CPfpago) 
									from HERNomina x, CalendarioPagos y 
									where x.RCNid = y.CPid and x.ERNid = a.ERNid) as fechapago
					from HDRNomina a
						inner join DatosEmpleado b
							on a.DEid = b.DEid				
		...
		<cfif temporal.RecordCount NEQ 0>
			...
			<!----=====================================================================---->
			<cfloop query="rsDatos"><!----Insertar en tabla para exportaciÃ³n---->
		...
		<cfelse>
			<cfquery name="ERR" datasource="#session.DSN#">
				select 'Existen cuentas de ahorro con formato incorrecto. El formato de la cuenta es xxx-yyyyyyyz.  
						Donde x corresponde a la oficina, y corresponde a la cuenta y z corresponde a el dÃ­gito de la cuenta'
				from dual
			</cfquery>
