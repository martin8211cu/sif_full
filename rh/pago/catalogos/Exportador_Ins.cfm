...
<!----<cfset vs_salida = mid(rsDatosEmpresa.Poliza,1,7)&'M'&url.CPperiodo+RepeatString('0',2-len(trim(url.CPmes)))&trim(url.CPmes)&' '&rsDatosEmpresa.Eidentificacion&rsDatosEmpresa.Etelefono1&rsDatosEmpresa.Efax>---->
<cfset vs_salida = mid(rsDatosEmpresa.Poliza,1,7)&'M'&url.CPperiodo&RepeatString('0',2-len(trim(url.CPmes)))&trim(url.CPmes)&' '&rsDatosEmpresa.Eidentificacion&rsDatosEmpresa.Etelefono1&rsDatosEmpresa.Efax>
...
<cfif isdefined("Application.dsinfo") and Application.dsinfo[session.dsn].type is 'oracle'>
	<cfquery datasource="#session.DSN#">
		...
		group by  	e.NTIcodigo, h.DEid, 
					...
					e.DEnombre, e.DEapellido1,e.DEapellido2, e.DEdato3		
	</cfquery>
...
<cfelse><!----============================= SYBASE =============================---->
	<cfquery name="rs" datasource="#session.DSN#">
		...
		select 	e.NTIcodigo, 
				...
				,
				<!---right(e.DEidentificacion,15),  ---->
				<cf_dbfunction name="string_part"   args="e.DEnombre,1,15">+ coalesce(replicate(' ',15-len(e.DEnombre)),''),	--- cambio realizado		
				<cf_dbfunction name="string_part"   args="e.DEapellido1,1,15">+coalesce(replicate(' ',15-len(e.DEapellido1)),''), --- cambio realizado
				<cf_dbfunction name="string_part"   args="e.DEapellido2,1,15">+coalesce(replicate(' ',15-len(e.DEapellido2)),''), --- cambio realizado
				right(e.DEdato3,25),  
				'0000000',
				'M',       
				0,
...
<cfif isdefined("Application.dsinfo") and Application.dsinfo[session.dsn].type is 'oracle'>
	...
	<cfquery datasource="#session.DSN#">
		...
		select 	4, 			
				...
				||jornada
				||condicion	
				||lpad(<cf_dbfunction name="length" args="ltrim(rtrim(#ocupacion#))">,5,'0')
				as salida				
		from #reporte#
		where salario > 0
			and (	
...
<cfelse><!----============================= SYBASE =============================---->
	...
	<cfquery datasource="#session.DSN#">
		...
		select 	4, 				
				...
				jornada+
				condicion+
				replicate('0', 5-datalength(ltrim(rtrim(convert(char,ocupacion))))) + ltrim(rtrim(convert(char(5), ocupacion))) as salida				
		from #reporte#
		where salario > 0
			and (	(dias > 0 )
					 or 
					(dias = 0 and condicion in ('03','04'))
				)
		order by nombre
	</cfquery>
