	...
	<cftransaction>
		<cftry>
			<cfquery datasource="#arguments.conexionD#" name="rsExisten">
				Select Ecodigo,Cmayor from CPVigencia Where Ecodigo=#EcodigoNuevo#
				...
				<cfloop query="rsExisten" >
					<cfloop index = "lcampo" list = "#campos#" delimiters = ",">
						<cfquery datasource="#arguments.conexionD#" name="x">
							update CPVigencia  set  #lcampo#   = 	(select  #lcampo#   from #TablaOrigen# v
			...
			<cfquery datasource="#arguments.conexionD#" name="x">
				insert into CPVigencia(Ecodigo,#campos#)
...
<cfelse>
	...
	<cfset txtfile = #LvarDir#&'004-CPVigencia.txt'>
	<cffile action="write" nameconflict="overwrite" file="#txtfile#" output="#hilera#" charset="utf-8">

	<cfset session.LvarFiles = session.LvarFiles & '004-CPVigencia.txt,'>
