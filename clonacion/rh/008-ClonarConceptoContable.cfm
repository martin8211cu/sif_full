	...
	<cftransaction>
		<cftry>
			<cfquery datasource="#arguments.conexionD#" name="rsExisten">
				Select Ecodigo,Oorigen from ConceptoContable Where Ecodigo=#EcodigoNuevo#
					and Oorigen in (select Oorigen from #TablaOrigen# where Ecodigo = #EcodigoViejo#)
			</cfquery>
				...
				<cfloop query="rsExisten" >
					<cfloop index = "lcampo" list = "#campos#" delimiters = ",">
						<cfquery datasource="#arguments.conexionD#" name="x">
							update ConceptoContable  set  #lcampo#   = 	(select  #lcampo#   from #TablaOrigen# v
																	 Where v.Ecodigo = #EcodigoViejo# 
																		and v.Oorigen = '#rsExisten.Oorigen#' )
								 Where Ecodigo = #EcodigoNuevo# 
										and Oorigen = '#rsExisten.Oorigen#'
						</cfquery>
					</cfloop>
				</cfloop>
			</cfif>
			...
			<cfquery datasource="#arguments.conexionD#" name="x">
				insert into ConceptoContable(Ecodigo,#campos#)
					Select #EcodigoNuevo#, #campos#
						From #TablaOrigen# 
							Where Ecodigo= #EcodigoViejo#
								and Oorigen not in (select Oorigen from #TablaDestino# a where a.Ecodigo = #EcodigoNuevo#)
			</cfquery>
...
<cfelse>
	...
	<cfset txtfile = #LvarDir#&'008-ConceptoContable.txt'>
	<cffile action="write" nameconflict="overwrite" file="#txtfile#" output="#hilera#" charset="utf-8">

	<cfset session.LvarFiles = session.LvarFiles & '008-ConceptoContable,'>	
