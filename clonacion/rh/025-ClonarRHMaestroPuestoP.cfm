	...
	<cftransaction>
		<cftry>
			<cfquery datasource="#arguments.conexionD#" name="rsExisten">
				Select Ecodigo,RHMPPcodigo,RHCid from RHMaestroPuestoP Where Ecodigo=#EcodigoNuevo#
					and RHMPPcodigo in (select RHMPPcodigo from #TablaOrigen# where Ecodigo = #EcodigoViejo#)
			</cfquery>
				...
				<cfloop query="rsExisten" >
					<cfloop index = "lcampo" list = "#campos#" delimiters = ",">
						<cfquery datasource="#arguments.conexionD#" name="x">
							update RHMaestroPuestoP  set  #lcampo#   = 	(select  #lcampo#   from #TablaOrigen# v
																	 Where v.Ecodigo = #EcodigoViejo# 
																		and v.RHMPPcodigo = '#rsExisten.RHMPPcodigo#' )
								 Where Ecodigo = #EcodigoNuevo# 
										and RHMPPcodigo = '#rsExisten.RHMPPcodigo#'
						</cfquery>
					</cfloop>
				</cfloop>
			...
			<cfquery datasource="#arguments.conexionD#" name="x">
				insert into RHMaestroPuestoP(Ecodigo,#campos#)
					Select #EcodigoNuevo#,#campos#
						From #TablaOrigen# 
							Where Ecodigo= #EcodigoViejo#
								and RHMPPcodigo not in (select RHMPPcodigo from #TablaDestino# a where a.Ecodigo = #EcodigoNuevo#)
			
<!---			INSERT INTO RHMaestroPuestoP(
				 RHCid, Ecodigo, RHMPPcodigo, 
				RHMPPdescripcion, BMfecha, BMUsucodigo, Complemento)
			Select  RHCid, @EcodigoNuevo, RHMPPcodigo, 
				RHMPPdescripcion, BMfecha, BMUsucodigo, Complemento
...
<cfelse>
	...
	<cfset insert_campos = ''>
	<cfset hilera = '#chr(13)#/* Maestro de Puestos Presupuestarios */#chr(13)#'>
	<cfloop query="x">
	...
	<cfset txtfile = #LvarDir#&'025-RHMaestroPuestoP.txt'>
	<cffile action="write" nameconflict="overwrite" file="#txtfile#" output="#hilera#" charset="utf-8">

	<cfset session.LvarFiles = session.LvarFiles & '025-RHMaestroPuestoP.txt,'>
