	...
	<cftransaction>
		<cftry>
			<cfquery datasource="#arguments.conexionD#" name="rsExisten">
				Select Ecodigo,Cconcepto from ConceptoContableE Where Ecodigo=#EcodigoNuevo#
					and Cconcepto in (select Cconcepto from #TablaOrigen# where Ecodigo = #EcodigoViejo#)
			</cfquery>
				...
				<cfloop query="rsExisten" >
					<cfloop index = "lcampo" list = "#campos#" delimiters = ",">
						<cfquery datasource="#arguments.conexionD#" name="x">
							update ConceptoContableE  set  #lcampo#   = 	(select  #lcampo#   from #TablaOrigen# v
																	 Where v.Ecodigo = #EcodigoViejo# 
																		and v.Cconcepto = #rsExisten.Cconcepto# )
								 Where Ecodigo = #EcodigoNuevo# 
										and Cconcepto = #rsExisten.Cconcepto#
						</cfquery>
					</cfloop>
				</cfloop>
			</cfif>
			...
			<cfquery datasource="#arguments.conexionD#" name="x">
				insert into ConceptoContableE(Ecodigo,#campos#)
					Select #EcodigoNuevo#, #campos#
						From #TablaOrigen# 
							Where Ecodigo= #EcodigoViejo#
								and Cconcepto not in (select Cconcepto from #TablaDestino# a where a.Ecodigo = #EcodigoNuevo#)
			</cfquery>
			<cfcatch type="any">
...
<cfelse>
	...
	<cfset txtfile = #LvarDir#&'007-ConceptoContableE.txt'>
	<cffile action="write" nameconflict="overwrite" file="#txtfile#" output="#hilera#" charset="utf-8">

	<cfset session.LvarFiles = session.LvarFiles & '007-ConceptoContableE,'>	
</cfif>
