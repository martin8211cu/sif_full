	...
	<cftransaction>
		<cftry>
			<cfquery datasource="#arguments.conexionD#" name="rsExisten">
				Select Ecodigo,RHEcol from RHEtiquetasEmpresa Where Ecodigo=#EcodigoNuevo#
					and RHEcol in (select RHEcol from #TablaOrigen# where Ecodigo = #EcodigoViejo#)
			</cfquery>
				...
				<cfloop query="rsExisten" >
					<cfloop index = "lcampo" list = "#campos#" delimiters = ",">
						<cfquery datasource="#arguments.conexionD#" name="x">
							update RHEtiquetasEmpresa  set  #lcampo#   = 	(select  #lcampo#   from #TablaOrigen# v
																	 Where v.Ecodigo = #EcodigoViejo# 
																		and v.RHEcol = '#rsExisten.RHEcol#' )
								 Where Ecodigo = #EcodigoNuevo# 
										and RHEcol = '#rsExisten.RHEcol#'
						</cfquery>
					</cfloop>
				</cfloop>
			</cfif>
			...
			<cfquery datasource="#arguments.conexionD#" name="x">
				insert into RHEtiquetasEmpresa(Ecodigo,#campos#)
					Select #EcodigoNuevo#, #campos#
						From #TablaOrigen# 
							Where Ecodigo= #EcodigoViejo#
								and RHEcol not in (select RHEcol from RHEtiquetasEmpresa a where a.Ecodigo = #EcodigoNuevo#)
			</cfquery>
...
<cfelse>
	...
	<cfset insert_campos = ''>
	<cfset hilera = '#chr(13)#/* Etiquetas de empresa*/#chr(13)#'>
	<cfloop query="x">
	...
	<cfset txtfile = #LvarDir#&'017-RHEtiquetasEmpresa.txt'>
	<cffile action="write" nameconflict="overwrite" file="#txtfile#" output="#hilera#" charset="utf-8">

	<cfset session.LvarFiles = session.LvarFiles & '017-RHEtiquetasEmpresa.txt,'>
</cfif>
