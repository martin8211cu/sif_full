...
<cfset tmpCalendarioPagos = 'tmpCalendarioPagos' >

<cfif not isdefined("form.chkSQL")>
	<cftransaction>
		<cftry>
			<cfquery datasource="#arguments.conexionD#" name="rsExisten">
				Select Ecodigo,CPcodigo from CalendarioPagos Where Ecodigo=#EcodigoNuevo#
					and CPcodigo in (select Tcodigo from #TablaOrigen# where Ecodigo = #EcodigoViejo#)
			</cfquery>
				...
				<cfloop query="rsExisten" >
					<cfloop index = "lcampo" list = "#campos#" delimiters = ",">
						<cfquery datasource="#arguments.conexionD#" name="x">
							update CalendarioPagos  set  #lcampo#   = 	(select  #lcampo#   from #TablaOrigen# v
																	 Where v.Ecodigo = #EcodigoViejo# 
																		and v.CPcodigo = '#rsExisten.CPcodigo#' )
								 Where Ecodigo = #EcodigoNuevo# 
										and CPcodigo = '#rsExisten.CPcodigo#'
						</cfquery>
					</cfloop>
				</cfloop>
			</cfif>
			...
			<cfquery datasource="#arguments.conexionD#" name="x">
				insert into CalendarioPagos(Ecodigo,#campos#)
				Select #EcodigoNuevo#, #campos#
					From #TablaOrigen# 
						Where Ecodigo= #EcodigoViejo#
							and CPcodigo not in (select CPcodigo from #TablaDestino# a where a.Ecodigo = #EcodigoNuevo#)
			</cfquery>
...
<!---incluir los datos de origen para hacer la actualizacion regla de tres--->
	...
	<cfset insert_campos = ''>
	<cfset hilera = '/* Calendario de Pagos #chr(13)# Clonacion Calendario de Pagos, se crea una tabla temporal tmpCalendarioPagos que es usada para hacer actualizaciones mas adelante */ #chr(13)#' >
	
	<cfset temporal = 'create table #tmpCalendarioPagos# (#cinsert#)'>
	...
	<cfloop query="rsOrigen">
		...
		<cfset sql_insert = "insert into #tmpCalendarioPagos#(#icampos#) values (#insert_campos#">
...
<!-------------------------------------------------------------------------------------------->
	...
	<cfset insert_campos = ''>
	<cfset hilera = hilera & '#chr(13)#/* Calendario de Pagos*/ #chr(13)#'>
	<cfloop query="x">
	...
	<cfset txtfile = #LvarDir#&'036-CalendarioPagos.txt'>
	<cffile action="write" nameconflict="overwrite" file="#txtfile#" output="#hilera#" charset="utf-8">

	<cfset session.LvarFiles = session.LvarFiles & '036-CalendarioPagos.txt,'>
</cfif>
