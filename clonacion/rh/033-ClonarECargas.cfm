...
<cfset tmpECargas = 'tmpECargas' >

<cfif not isdefined("form.chkSQL")>
	<cftransaction>
		<cftry>
			<cfquery datasource="#arguments.conexionD#" name="rsExisten">
				Select Ecodigo,ECcodigo from ECargas Where Ecodigo=#EcodigoNuevo#
					and ECcodigo in (select ECcodigo from #TablaOrigen# where Ecodigo = #EcodigoViejo#)
			</cfquery>
				...
				<cfloop query="rsExisten" >
					<cfloop index = "lcampo" list = "#campos#" delimiters = ",">
						<cfquery datasource="#arguments.conexionD#" name="x">
							update ECargas  set  #lcampo#   = 	(select  #lcampo#   from #TablaOrigen# v
																	 Where v.Ecodigo = #EcodigoViejo# 
																		and v.ECcodigo = '#rsExisten.ECcodigo#' )
								 Where Ecodigo = #EcodigoNuevo# 
										and ECcodigo = '#rsExisten.ECcodigo#'
						</cfquery>
					</cfloop>
				</cfloop>
			</cfif>
			...
			<cfquery datasource="#arguments.conexionD#" name="x">
				insert into ECargas(Ecodigo, #campos#)
					Select #EcodigoNuevo#, #campos#
						From #TablaOrigen# 
							Where Ecodigo= #EcodigoViejo#
							and ECcodigo not in (select ECcodigo from #TablaDestino# a where a.Ecodigo = #EcodigoNuevo#)
			</cfquery>
...
<!---incluir los datos de origen para hacer la actualizacion regla de tres--->
	...
	<cfset insert_campos = ''>
	<cfset hilera = '/* Encabezado de Cargas  #chr(13)# Clonacion Cargas, se crea una tabla temporal tmpCargas que es usada para hacer actualizaciones mas adelante */ #chr(13)#' >
	
	<cfset temporal = 'create table #tmpECargas# (#cinsert#)'>
	...
	<cfloop query="rsOrigen">
		...
		<cfset sql_insert = "insert into #tmpECargas#(#icampos#) values (#insert_campos#">
...
<!-------------------------------------------------------------------------------------------->
	...
	<cfset insert_campos = ''>
	<cfset hilera = hilera & '#chr(13)#/*Encabezado de Cargas*/#chr(13)#'>
	...
	<cfset txtfile = #LvarDir#&'033-ECargas.txt'>
	<cffile action="write" nameconflict="overwrite" file="#txtfile#" output="#hilera#" charset="utf-8">

	<cfset session.LvarFiles = session.LvarFiles & '033-ECargas.txt,'>
</cfif>
