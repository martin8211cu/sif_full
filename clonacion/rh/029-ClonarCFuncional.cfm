...
<cfset tmpCFuncional = 'tmpCFuncional' >

<cfif not isdefined("form.chkSQL")>
	<cftransaction>
		<cftry>
			<cfquery datasource="#arguments.conexionD#" name="rsExisten">
				Select Ecodigo,CFcodigo from CFuncional Where Ecodigo=#EcodigoNuevo#
					and CFcodigo in (select CFcodigo from #TablaOrigen# where Ecodigo = #EcodigoViejo#)
			</cfquery>
				...
				<cfloop query="rsExisten" >
					<cfloop index = "lcampo" list = "#campos#" delimiters = ",">
						<cfquery datasource="#arguments.conexionD#" name="x">
							update CFuncional  set  #lcampo#   = 	(select  #lcampo#   from #TablaOrigen# v
																	 Where v.Ecodigo = #EcodigoViejo# 
																		and v.CFcodigo = '#rsExisten.CFcodigo#' )
								 Where Ecodigo = #EcodigoNuevo# 
										and CFcodigo = '#rsExisten.CFcodigo#'
						</cfquery>
					</cfloop>
				</cfloop>
			</cfif>
			...
			<cfquery datasource="#arguments.conexionD#" name="x">
				insert into CFuncional(Ecodigo,#campos#)
					Select #EcodigoNuevo#, #campos#
						From #TablaOrigen# 
							Where Ecodigo= #EcodigoViejo#
								and CFcodigo not in (select CFcodigo from #TablaDestino# a where a.Ecodigo = #EcodigoNuevo#)
			</cfquery>
			
			<cfquery datasource="#arguments.conexionD#" name="x">
				update CFuncional set CFidresp = (Select c.CFid 
					from #TablaOrigen#  b, #TablaDestino#  c
						Where CFuncional.CFidresp = b.CFid
...
<cfelse>
	...
	<cfset insert_campos = ''>
	<cfset hilera = '#chr(13)#/*Crea tabla temporal para actualizar luego informacion */#chr(13)#'>

	<cfset temporal = 'create table #tmpCFuncional# (#cinsert#)'>
	...
	<cfloop query="rsOrigen">
		...
		<cfset sql_insert = "insert into #tmpCFuncional#(#icampos#) values (#insert_campos#">
...
<!-------------------------------------------------------------------------------------------->
	...
	<cfset insert_campos = ''>
	<cfset hilera = hilera & '#chr(13)#/*Insert datos de clonacion */#chr(13)#'>
	<cfloop query="x">
	...
	<cfset hilera = hilera & '#chr(13)#/*Actuliza la infomacion con datos de la temporal */#chr(13)#'>
	<cfset hilera = hilera & 'update CFuncional set CFidresp = (Select c.CFid 
	from #tmpCFuncional#  b, CFuncional  c
	Where CFuncional.CFidresp = b.CFid
		and b.Ecodigo  = #EcodigoViejo#
		and b.CFcodigo = c.CFcodigo
		and c.Ecodigo  = #EcodigoNuevo#)
	...
	<cfset txtfile = #LvarDir#&'029-CFuncional.txt'>
	<cffile action="write" nameconflict="overwrite" file="#txtfile#" output="#hilera#" charset="utf-8">

	<cfset session.LvarFiles = session.LvarFiles & '029-CFuncional.txt,'>	
