...
<cfset tmpRegimenVacaciones = 'tmpRegimenVacaciones' >
	
<cfif not isdefined("form.chkSQL")>
	<cftransaction>
		<cftry>
			<cfquery datasource="#arguments.conexionD#" name="rsExisten">
				Select Ecodigo, RVcodigo from RegimenVacaciones Where Ecodigo=#EcodigoNuevo#
					and RVcodigo in (select RVcodigo from #TablaOrigen# where Ecodigo = #EcodigoViejo#)
			</cfquery>
				...
				<cfloop query="rsExisten" >
					<cfloop index = "lcampo" list = "#campos#" delimiters = ",">
						<cfquery datasource="#arguments.conexionD#">
							update RegimenVacaciones  set  #lcampo#   = (select  #lcampo#   from #TablaOrigen# v
																	 Where v.Ecodigo = #EcodigoViejo# 
																		and v.RVcodigo = '#rsExisten.RVcodigo#' )
								 Where Ecodigo = #EcodigoNuevo# 
										and RVcodigo = '#rsExisten.RVcodigo#'
						</cfquery>
					</cfloop>
				</cfloop>
			</cfif>
			...
			<cfquery datasource="#arguments.conexionD#" name="x">
				insert into RegimenVacaciones(Ecodigo,#campos#)
					Select #EcodigoNuevo#, #campos#
					From #TablaOrigen#
					Where Ecodigo= #EcodigoViejo#
						and RVcodigo not in (select RVcodigo from RegimenVacaciones a where a.Ecodigo = #EcodigoNuevo#)
			</cfquery>
...
<!---incluir los datos de origen para hacer la actualizacion regla de tres--->
	...
	<cfset insert_campos = ''>
	<cfset hilera = '/* Clonacion de datos Regimen de vacaciones, se crea una tabla temporal tmpRegimenVacaciones que es usada para hacer actualizaciones mas adelante */'& '#chr(13)#' >
	
	<cfset temporal = 'create table #tmpRegimenVacaciones# (#cinsert#)'>
	...
	<cfloop query="rsOrigen">
		...
		<cfset sql_insert = "insert into #tmpRegimenVacaciones#(#icampos#) values (#insert_campos#">
...
<!-------------------------------------------------------------------------------------------->
	...
	<cfset txtfile = #LvarDir#&'011-RegimenVacaciones.txt'>
	<cffile action="write" nameconflict="overwrite" file="#txtfile#" output="#hilera#" charset="utf-8">

	<cfset session.LvarFiles = session.LvarFiles & '011-RegimenVacaciones,'>
