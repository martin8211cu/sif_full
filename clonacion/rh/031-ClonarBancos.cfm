...
<cfif not isdefined("form.chkSQL")>
	<cftransaction>
		<cftry>
			...
			<cfquery datasource="#arguments.conexionD#" name="x">
				insert into Bancos(Ecodigo,#campos#)
...
<cfelse>
	...
	<cfset insert_campos = ''>
	<cfset hilera = ''>
	<cfset temporal = 'create table #tmpBancos# (#cinsert#)'>
	...
	<cfloop query="rsOrigen">
		...
		<cfset sql_insert = "insert into #tmpBancos#(#icampos#) values (#insert_campos#">
...
<!-------------------------------------------------------------------------------------------->
	...
	<cfset txtfile = #LvarDir#&'031-Bancos.txt'>
	<cffile action="write" nameconflict="overwrite" file="#txtfile#" output="#hilera#" charset="utf-8">

	<cfset session.LvarFiles = session.LvarFiles & '031-Bancos.txt,'>
</cfif>
