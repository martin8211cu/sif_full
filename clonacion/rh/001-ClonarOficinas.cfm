...
<cfset tmpOficinas = 'tmpOficinas' >

<cfif not isdefined("form.chkSQL")>
	<cftransaction>
		<cftry>
			<cfquery datasource="#arguments.conexionD#" name="rsExisten">
				Select Ecodigo,Ocodigo from Oficinas Where Ecodigo=#EcodigoNuevo#
					and Ocodigo in (select Ocodigo from #TablaOrigen# where Ecodigo = #EcodigoViejo#)
			</cfquery>
			...
			<cfquery datasource="#arguments.conexionD#" name="x">
				insert into Oficinas(Ecodigo,#campos#)
					Select #EcodigoNuevo#, #campos#
						From #TablaOrigen# 
							Where Ecodigo= #EcodigoViejo#
								and (select count(1) from #TablaDestino# a where a.Ecodigo = #EcodigoNuevo# and Ocodigo = a.Ocodigo) = 0
<!---								and Ocodigo not in (select Ocodigo from #TablaDestino# a where a.Ecodigo = #EcodigoNuevo#)--->
			</cfquery>
			<!---<cf_dump var="#x#">--->
			<cfcatch type="any">
...
<cfelse>
	...
	<cfset insert_campos = ''>
	<cfset hilera = '/* Oficinas #chr(13)# Clonacion de datos de oficinas, se crea una tabla temporal tmpOficinas que es usada para hacer actualizaciones mas adelante */ #chr(13)#' >
	<cfset temporal = 'create table #tmpOficinas# (#cinsert#)'>
	...
	<cfloop query="rsOrigen">
		...
		<cfset sql_insert = "insert into #tmpOficinas#(#icampos#) values (#insert_campos#">
...
<!-------------------------------------------------------------------------------------------->
	...
	<cfset txtfile = #LvarDir#&'001-Oficinas.txt'>
	<cffile action="write" nameconflict="overwrite" file="#txtfile#" output="#hilera#" charset="utf-8">

	<cfset session.LvarFiles = session.LvarFiles & '001-Oficinas.txt,'>
