<cf_dbcreate name="DBMcontrolParches" datasource="asp">
	<cf_dbcreatecol name="IDparche"		type="numeric"		mandatory="yes" identity="yes">
...
<cf_templatecss>
	...
	<cfquery name="rsModelos" datasource="asp">
		select 	m.IDsch, d.IDdsn, m.IDmod, d.activo, s.sch, m.modelo, d.dsn, coalesce(v.IDver,0) as IDverUltDsn
			 ...
			 on d.IDmod = m.IDmod
		order by m.IDsch, m.IDmod
	</cfquery>

	<script language="javascript">
		var dis = false;
		var LvarWnd = null;
		function sbOP (op, ID, ID_2)
		{
			if (dis) 
			{
				return false;
			...
			{
				dis = true;
				location.href = "DBMupgrades_sql.cfm?OP=" + op + "&IDdsn=" + ID + "&IDmod=" + ID_2;
			}
			else if ( (op == 6) || (op == 7) )
			{
				dis = true;
	...
	<cfif Application.dsinfo["asp"].type EQ "db2">
		...
		Reorganizar Todas las tablas DB2<BR>
	</cfif>
	<cfparam name="session.DBM_DEPENDENCIAS" default="false">
	<cfif session.DBM_DEPENDENCIAS>
		<input type="checkbox" id="DBM_DEPENDENCIAS" checked="checked" onclick="location.href='DBMupgrades_sql.cfm?DBM_DEPENDENCIAS=0';"/> 
		<script language="javascript">setTimeout("document.getElementById('DBM_DEPENDENCIAS').checked = true;",1000);</script>
	<cfelse>
		<input type="checkbox" id="DBM_DEPENDENCIAS" onclick="location.href='DBMupgrades_sql.cfm?DBM_DEPENDENCIAS=1';"/> 
		<script language="javascript">setTimeout("document.getElementById('DBM_DEPENDENCIAS').checked = false;",1000);</script>
	</cfif>
	Forzar Dependencias
	...
	<cfoutput query="rsModelos" group="IDsch">
		<cfset LvarSchTD = sch>
				...
				<cfif dsn NEQ "">
					...
					<cfelse>
						...
						<cfquery name="rsGens" datasource="#dsn#">
							select 	coalesce(max(IDverFin),0) as IDverFin,
									coalesce(min(IDverFin),0) as IDverFin2
							  from DBMgens
							 where IDmod = #IDmod#
						</cfquery>

						<cfif rsGens.IDverFin LT 0>
							<cfset LvarModoVerif = true>
							<cfset LvarIDverGens = "0">
							<cfset LvarNuevas = true>
							<cfset QuerySetCell(rsModelos, "IDverUltDsn", "#numberFormat(-rsGens.IDverFin2,"0")#", rsModelos.currentRow)>
						<cfelse>
							<cfset LvarModoVerif = false>
							...
							<cfif IDverUltDsn NEQ LvarIDverGens>
								...
								<cfset QuerySetCell(rsModelos, "IDverUltDsn", "#numberFormat(LvarIDverGens,"0")#", rsModelos.currentRow)>
							</cfif>
						</cfif>
					...
					<tr>
						...
						<cfelseif LvarNuevas>
							...
							<td nowrap="nowrap" style="#LvarStyle#">
								<cfif LvarModoVerif>
									<img src="/cfmx/asp/parches/images/iedit.gif" 			style="cursor:pointer" onclick="return sbOP(3,#IDdsn#,#IDmod#);" 		title="Verificar el DBM">
									<img src="/cfmx/asp/parches/images/stop.gif" 		style="cursor:pointer" onclick="return sbOP(9,#IDdsn#,#IDmod#);" 	title="Desactivar Modo Verificación">
								<cfelse>
									<img src="/cfmx/asp/parches/images/findsmall.gif" 		style="cursor:pointer" onclick="return sbOP(2,#IDdsn#);" title="Consultar Parches">
									<img src="/cfmx/asp/parches/images/genScriptDL.gif" 	style="cursor:pointer" onclick="return sbOP(3,#IDdsn#,#IDmod#);" title="Generar Script para revisión">
									<img src="/cfmx/asp/parches/images/genScript.gif" 		style="cursor:pointer" onclick="return sbOP(4,#IDdsn#,#IDmod#);" title="Generar y Ejecutar Script en '#lcase(dsn)#'">
								</cfif>
								&nbsp;&nbsp;
							</td>
							<td nowrap colspan="2" style="#LvarStyle#">
								<cfif LvarModoVerif>
									<strong style="color:##CC0000">Modo Verificación del DBM</strong>
								<cfelse>
									<strong style="color:##CC0000">Debe Actualizar el DBM</strong>
								</cfif>
							</td>
							<td nowrap align="center" style="#LvarStyle#">
								<cfif LvarModoVerif>
									<cfquery name="rsSQL" datasource="asp">
										select count(1) as tabs
										  from DBMtab
										 where IDmodORI 	= #IDmod#
									</cfquery>
								<cfelse>
									<cfquery name="rsSQL" datasource="asp">
										select count(distinct IDtab) as tabs
										  from DBMtabV t
											inner join DBMversiones v
												 on v.IDver = t.IDver
												and v.IDmod = #IDmod#
										  where t.IDver > (select IDverUlt from DBMdsn where IDdsn = #IDdsn#)
									</cfquery>
								</cfif>
								#rsSQL.tabs#
							</td>
						...
						<cfelseif IDverUltMod EQ 0>
							<cfset LvarStyle= "background-color: ##FFFF00">
							<td nowrap="nowrap" style="#LvarStyle#">
								&nbsp;&nbsp;
							</td>
							<td nowrap colspan="4" style="#LvarStyle#"><strong style="color:##CC0000">No existen versiones DBM</strong></td>
						<cfelse>
							<td nowrap>
								<img src="/cfmx/asp/parches/images/findsmall.gif" 	style="cursor:pointer" onclick="return sbOP(2,#IDdsn#);" title="Consultar Parches">
								&nbsp;&nbsp;
							...
							<td>
								<strong>OK</strong>
								<img src="/cfmx/asp/parches/images/start.gif" 	style="cursor:pointer" onclick="return sbOP(8,#IDdsn#,#IDmod#);" title="Activar modo Verificacion del DBM para todas las tablas en '#lcase(dsn)#'">
							</td>
							<cfquery name="rsSQL" datasource="asp">
								select count(1) as tabs
								  from DBMtab
