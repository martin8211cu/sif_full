<cfcomponent output="false" mixin="none" environment="design,development,maintenance">
	...
	<cffunction name="migrateTo" access="public" returntype="string" hint="migrates database to a specified version">
		...
		<cfargument name="dataSourceName" type="string" required="false" default="">
		<cfargument name="stragglers" type="boolean" required="false" default="false" hint="Includes all not migrated versions, even when earlier than currentVersion">
		<cfset request.dataSourceName = arguments.dataSourceName>
		<cfset var loc = {}>
		<cfset loc.feedback = "">
		<cfset loc.versionsCurrentlyMigrated = $getVersionsPreviouslyMigrated(arguments.dataSourceName)>
		...
		<cfelse>
			...
			<cfset loc.migrations = getAvailableMigrations(arguments.dataSourceName)>
            ...
            <!--- <cftransaction action="begin"> --->
  						<cftry>
							...
							<cfset loc.migration.cfc = $createObjectFromRoot(path="db.migrate",fileName=loc.migration.cfcfile, method="init", argumentCollection=arguments)>
			...
			<cfelse>
				...
				<cfloop index="loc.i" from="1" to="#ArrayLen(loc.migrations)#">
					<cfset loc.migration = loc.migrations[loc.i]>
					<cfif (arguments.stragglers or loc.migration.version lte arguments.version) and loc.migration.status neq "migrated">
            <!--- <cftransaction action="begin"> --->
  						<!--- <cftry> --->
  							<cfset loc.feedback = loc.feedback & "#chr(13)#-------- " & loc.migration.cfcfile & " #RepeatString("-",Max(5,50-Len(loc.migration.cfcfile)))##chr(13)#">
  							<cfset Request.migrationOutput = "">
	...
	<cffunction name="readMigration" displayname="readMigration" access="public" returntype="string">
		<cfargument name="migrationName" type="string" required="true" />
		<cfset var loc = {}/>
		<cfset loc.templateFile = expandPath("db/migrate") & "/" & arguments.migrationName & ".cfc"/>
		<cfif not FileExists(loc.templateFile)>
			<cfreturn "Migration #arguments.migrationName# could not be found">
		</cfif>
		<cftry>
			<cffile action="read" file="#loc.templateFile#" variable="loc.migrationContent">
			<cfcatch type="any">
				<cfreturn "There was an error when reading the migration: #cfcatch.message#">
			</cfcatch>
		</cftry>
		<cfreturn loc.migrationContent />
	</cffunction>

	<cffunction name="writeMigration" displayname="writeMigration" access="public">
		<cfargument name="migrationName" type="string" required="true" />
		<cfargument name="migrationContent" type="string" required="true" />
		<cfset var loc = {}/>
		<cfset loc.templateFile = expandPath("db/migrate") & "/" & arguments.migrationName & ".cfc"/>
		<cftry>
			<cffile action="write" file="#loc.templateFile#" output="#arguments.migrationContent#">
			<cfcatch type="any">
				<cfreturn "Ocurrio un error salvando la migracion: #cfcatch.message#">
			</cfcatch>
		</cftry>
		<cfreturn "La migracion #arguments.migrationName# se guardo correctamente" />
	</cffunction>
	...
	<cffunction name="getCurrentMigrationVersion" access="public" returntype="string" hint="returns current database version">
		...
		<cfset var loc = {}>
		<cfset loc.listPreviouslyMigratedVersions = $getVersionsPreviouslyMigrated(arguments.dataSourceName)>
		<cfreturn ListLast(loc.listPreviouslyMigratedVersions)>
	</cffunction>

	<cffunction name="getDatasourceType" access="public" returntype="string" hint="Regresa el tipo de datasource. para asp, aspmonitor, sifcontrol y sifpublica regresa el mismo nombre, para las dem�s regresa minisif">
		<cfargument name="dataSourceName" type="string" required="true" default="">
		<cfif listFind('asp,aspmonitor,sifcontrol,sifinterfaces,sifpublica', arguments.datasourceName) GT 0>
			<cfreturn arguments.datasourceName>
		<cfelse>
			<cfreturn "minisif">
		</cfif>
	</cffunction>
	...
	<cffunction name="getAvailableMigrations" access="public" returntype="array" hint="searches db/migrate folder for migrations">
		...
		<cfset var loc = {}>
		<cfset loc.listVersionsPreviouslyMigrated = $getVersionsPreviouslyMigrated(arguments.dataSourceName)>
		<cfset loc.migrations = ArrayNew(1)>
		<cfset loc.migrationRE = "^([\d]{3,14})_([^\.]*)\-#getDatasourceType(arguments.dataSourceName)#.cfc$">
		...
		<cfloop query="qMigrationFiles">
			<cfif REFind(loc.migrationRE,Name)>
				...
				<cfset loc.migration.cfcfile = REReplace(Name,loc.migrationRE,"\1_\2-#getDatasourceType(arguments.dataSourceName)#")>
				<cfset loc.migration.loadError = "">
				<cfset loc.migration.details = "">
	...
	<cffunction name="$setVersionAsMigrated" access="private">
		...
		<cfquery datasource="#request.dataSourceName#"> <!--- username="#application.wheels.dataSourceUserName#" password="#application.wheels.dataSourcePassword#" --->
	...
	<cffunction name="$createObjectFromRoot" returntype="any" access="private" output="false">
		...
		<cfscript>
			...
			loc.component = arguments.path & "." & arguments.fileName;
	...
	<cffunction name="$copyTemplateMigrationAndRename" displayname="$copyTemplateMigrationAndRename" access="private" returntype="string">
		...
		<cfset var loc = {}/>
		<cfset loc.migrationsPath = expandPath("db/migrate")/>
		<cfset loc.templateFile = expandPath("templates") & "/" & arguments.templateName & ".cfc"/>
		<cfset loc.extendsPath = "asp.admin.dbmigrate.Migration"/>
		<cfif not FileExists(loc.templateFile)>
		...
		<cftry>
			...
			<cfset loc.migrationFile = $getNextMigrationNumber(arguments.migrationPrefix) & "_#loc.migrationFile#-#arguments.dataSourceName#.cfc">
	...
	<cffunction name="$getVersionsPreviouslyMigrated" access="private" returntype="string">
		<cfargument name="dataSourceName" type="string" default="#request.dataSourceName#">
		<cfset var loc = {}>
		<cfif arguments.dataSourceName neq "">
			<cftry>
				<cfquery name="loc.qMigratedVersions" datasource="#arguments.dataSourceName#">
				SELECT version FROM schemainfo ORDER BY version ASC
				</cfquery>
				<cfcatch type="database">
					<cfquery datasource="#arguments.dataSourceName#">
