<cfcomponent>
	...
	<cffunction name="foreignKeySQL" returntype="string" access="public" hint="generates sql for foreign key constraint">
		...
		<cfreturn "CONSTRAINT #quoteTableName(arguments.name)# FOREIGN KEY (#quoteColumnName(arguments.column)#) REFERENCES #(arguments.referenceTable)#(#quoteColumnName(arguments.referenceColumn)#)">
	</cffunction>
	...
	<cffunction name="addIndex" returntype="string" access="public" hint="generates sql to add database index on a table column">
		...
		<cfargument name="indexName" type="string" required="false" default="#(arguments.table)#_#ListFirst(arguments.columnNames)#" hint="override the default index name">
		<cfscript>
		var sql = "CREATE ";
