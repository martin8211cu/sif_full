...
<cfcomponent extends="Base">
	...
	<cfset CT_CreatedTable = t.Translate('CT_CreatedTable','Tabla Creada','/TableDefinition.xml')>
	<cfset Colum_Add = t.Translate('Colum_Add','Columna Agregada','/TableDefinition.xml')>
	<cfset Dropped_table = t.Translate('Dropped_table','Tabla Eliminada','/TableDefinition.xml')>
	...
	<cffunction name="binary" returntype="void" access="public" hint="adds binary columns to table definition">
		...
		<cfargument name="default" type="string" required="no" hint="default value" default=''>
		<cfargument name="null" type="boolean" required="no" hint="whether nulls are allowed">
		<cfscript>
		var loc = {};
		arguments.columnType = "binary";
		loc.iEnd = ListLen(arguments.columnNames);
		if(Trim(arguments.default) == ''){
			structDelete(arguments, "default", true);
		}		
		for (loc.i=1; loc.i <= loc.iEnd; loc.i++) {
	...
	<cffunction name="numeric" returntype="void" access="public" hint="adds numeric columns to table definition">
		<cfargument name="columnNames" type="string" required="yes" hint="one or more column names, comma delimited">
		<cfargument name="default" type="string" required="no" hint="default value" default=''>
		<cfargument name="null" type="boolean" required="no" hint="whether nulls are allowed">
	...
	<cffunction name="money" returntype="void" access="public" hint="adds money columns to table definition">
		<cfargument name="columnNames" type="string" required="yes" hint="one or more column names, comma delimited">
		<cfargument name="default" type="string" required="no" hint="default value" default=''>
		<cfargument name="null" type="boolean" required="no" hint="whether nulls are allowed">
		<cfscript>
	...
	<cffunction name="char" returntype="void" access="public" hint="adds char columns to table definition">
		<cfargument name="columnNames" type="string" required="yes" hint="one or more column names, comma delimited">
		<cfargument name="limit" type="numeric" required="no" hint="char size">
		<cfargument name="default" type="string" required="no" hint="default value" default=''>
		<cfargument name="null" type="boolean" required="no" hint="whether nulls are allowed">
		<cfscript>
		var loc = {};
	...
	<cffunction name="references" returntype="void" access="public" hint="adds integer reference columns to table definition and creates foreign key constraints">
		...
		<cfargument name="reference" type="string" required="no" hint="one reference" default="id">
		...
		<cfargument name="columnType" type="string" required="no" default="integer" hint="define columnType">
		<cfscript>
		var loc = {};

		loc.iEnd = ListLen(arguments.referenceNames);
		loc.iEnd = ListLen(arguments.reference);
		for (loc.i=1; loc.i <= loc.iEnd; loc.i++) {
			loc.referenceName = ListGetAt(arguments.referenceNames,loc.i);
			loc.reference = ListGetAt(arguments.reference,loc.i);
			column(columnName = loc.referenceName & loc.reference,columnType=arguments.columnType);
			...
			if(arguments.foreignKey && !arguments.polymorphic) {
				loc.referenceTable = loc.referenceName;
				loc.referenceColumn = loc.reference;
				loc.foreignKey = CreateObject("component","ForeignKeyDefinition").init(adapter=this.adapter,table=this.name,referenceTable=loc.referenceTable,column="#loc.referenceName##loc.reference#",referenceColumn=loc.referenceColumn);
	...
	<cffunction name="change" returntype="void" access="public" hint="alters existing table in the database">
		...
		for (loc.i=1; loc.i <= loc.iEnd; loc.i++) {
			if(arguments.addColumns || !ListFindNoCase(loc.existingColumns,this.columns[loc.i].name)) {
				...
				announce("#Colum_Add# #this.columns[loc.i].name# to table #this.name#");
			} else {
