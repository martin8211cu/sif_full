...
<cfif modo neq "ALTA">
	...
	<cfquery name="rsEmpSalarios" datasource="sifpublica">
		Select 1
...
<script language="JavaScript">
	...
	function deshabilitarValidacion(){
		if (document.form1.botonSel.value == 'Baja' || document.form1.botonSel.value == 'Nuevo' ){
			objForm.EEid.required = false;
			objForm.ETdescripcion.required = false;
		}
	}
...
<form method="post" name="form1" action="SQLTEOrganizaciones.cfm" onSubmit="return valida();">
			...
			<td nowrap align="right">Descripción:</td>
		  	<td colspan="3">
		  		<input type="text" name="ETdescripcion" 
					   value="<cfif modo NEQ 'ALTA'><cfoutput>#rsForm.ETdescripcion#</cfoutput></cfif>" 
			...
			<td colspan="3">
				<cfoutput>
					<select name="EEid" id="EEid"
						<cfif isdefined("rsEmpSalarios") and rsEmpSalarios.RecordCount GT 0>
							disabled
						</cfif>>
  ...
  <input type="hidden" name="ETid" 
		   value="<cfif modo neq "ALTA"><cfoutput>#rsform.ETid#</cfoutput></cfif>">
</form>

<script language="JavaScript">

	function valida(){
		document.form1.ETid.disabled = false;
		return true;
	}
	
	function __CodeExists(){
		<cfoutput query="rsCodigos">
			var valor = "#Trim(rsCodigos.ETid)#".toUpperCase( );
			if ( valor == trim(this.value.toUpperCase( ))
			<cfif modo neq "ALTA">
				&& "#Trim(rsForm.ETid)#".toUpperCase( ) != trim(this.value.toUpperCase( ))
			</cfif>
			) {
	...
	objForm.ETdescripcion.required = true;
	objForm.ETdescripcion.description="Descripción del Tipo de Organización";
