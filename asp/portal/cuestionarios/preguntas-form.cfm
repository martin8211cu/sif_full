<!---AJAX para la actualizacion de la lista--->
	
<script language="javascript" type="text/javascript">
<!-- 
//Browser Support Code
function ajaxFunction1_ComboParte(){
	var ajaxRequest1;  // The variable that makes Ajax possible!
	vID_parte = document.form3.PPparte.value;
	vID_id=document.form1.PCid.value
//	vmodoD1 = document.formAntD.modoDep.value;
	...
	ajaxRequest1.open("GET", '/cfmx/asp/portal/cuestionarios/cuestionarioI.cfm?PPparte='+vID_parte+'&PCid='+vID_id, false);
...
<cfif isdefined("form.PPid")>
	...
	<cfquery name="ppdata" datasource="sifcontrol">
		select PPid, PPparte, PPnumero, PPpregunta, PPtipo, PPvalor, PPrespuesta, PPorden, PPmantener, coalesce(PPorientacion,0) as PPorientacion 
		from PortalPregunta
...
<cfif isdefined ('form.pPPparte')>
	<cfquery name="partes2" datasource="sifcontrol">
		select PPparte, PCPdescripcion
		from PortalCuestionarioParte
		where PCid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.PCid#">
		and PPparte=#form.pPPparte#
	</cfquery>
</cfif>
...
<table border="0" cellpadding="0" cellspacing="2" width="100%" align="center" >
	...
	<tr>
	  <td align="right" width="1%" nowrap ><cf_translate key="LB_Parte">Parte</cf_translate>:&nbsp;</td>
		<td>
			...
			<select name="PPparte" onFocus="restaurar_color(this);" onchange="ajaxFunction1_ComboParte();">
				<cfloop query="partes">
					<option value="#partes.PPparte#" <cfif modo_pregunta eq 'CAMBIO' and ppdata.PPparte eq partes.PPparte >selected</cfif><cfif isdefined ('form.Pppparte') and modo_pregunta eq 'ALTA' and partes2.PPparte eq partes.PPparte >selected</cfif> >#partes.PPparte# - #partes.PCPdescripcion#</option>
				</cfloop>
			</select>		</td>
	</tr>
	...
	<tr>
		...
		<td>
			...
			<cfif modo_pregunta neq 'ALTA'>
				<cf_rheditorhtml name="PPpregunta" value="#ppdata.PPpregunta#">
			<cfelse>
				<cf_rheditorhtml name="PPpregunta">
			</cfif>
		</td>	
	</tr>
	<tr>
    ...
    <tr id="TR_1" style="display:none">
		<td align="right" ><cf_translate key="LB_Orientacion">Orientaci&oacute;n</cf_translate>:&nbsp;</td>
		<td>
            <select name="PPorientacion" >
				<option value="0" <cfif modo_pregunta neq 'ALTA' and ppdata.PPorientacion eq 0>selected</cfif> ><cf_translate key="CMB_Vertical">Vertical</cf_translate></option>
				<option value="1" <cfif modo_pregunta neq 'ALTA' and ppdata.PPorientacion eq 1>selected</cfif> ><cf_translate key="CMB_Horizontal">Horizontal</cf_translate></option>
			</select>		
         </td>
	</tr>
	<tr>
	...
	<!--- ASI ESTABA ANTES CON HTML DINAMICO NO BORRAR PARA CORREGIR
				...
				<cfif modo_pregunta neq 'ALTA'>
					<cfloop query="prdata">
						...
						<tr>
							...
							<td><img src="/cfmx/sif/imagenes/iindex.gif" border="0" onClick="javascript:formatear(#prdata.PRid#, #prdata.currentrow+50#);" style="cursor:hand;"></td>
						</tr>
					</cfloop>
				</cfif>
				<tr>
		...
		<tr>
			...
			<td>
				<table width="100%" id="myTable" cellpadding="3" cellspacing="0" align="center" style="border:1px solid gray;">
					<tr><td><strong><cf_translate key="LB_OpcionDeRespuesta">Opciones de Respuesta</cf_translate>:</strong>&nbsp;Descripci&oacute;n para mostrar.</td></tr>
					<tr><td><strong><cf_translate key="LB_Valor">Valor</cf_translate>:</strong>&nbsp;Valor de la respuesta se pondera con el valor asignado a la pregunta. Valor en -1 indica que no aplica para la ponderaci&oacute;n.</td></tr>
					<tr><td><strong><cf_translate key="LB_Ref">Ref.</cf_translate>:</strong>&nbsp;Pregunta de referencia.</td></tr>	
					<tr><td><strong><cf_translate key="LB_Orden">Orden</cf_translate>:</strong>&nbsp;Orden de aparici&oacute;n del &iacute;tem.</td>	</tr>	
					</tr>
				</table>
			</td>
		</tr>
			...
			<table width="100%" id="myTable" cellpadding="3" cellspacing="0" align="center" style="border:1px solid gray;">
					...
					<cfloop query="prdata">
						...
						<tr>
							<td>
								<cfset expresion = "\<[A-Za-z=\'" >
								<cfset expresion = expresion & '\"\0-9.\(\)\:\;\-## ]*>' >
								<input type="text" name="_PRtexto_#prdata.currentrow+8#" size="45" maxlength="400" value="#trim(REReplace(prdata.PRtexto,expresion,'','ALL'))#" onClick="javascript:this.select();" onChange="javascript: setvalor(this);">
								<input type="hidden" name="PRtexto_#prdata.currentrow+8#" value="#HTMLEditFormat(trim(prdata.PRtexto))#" onClick="this.select();">
								<input type="hidden" name="PRid_#prdata.currentrow+8#" value="#prdata.PRid#">							</td>
							<!--- <td><input type="text" name="PRvalor_#prdata.currentrow+8#" size="7" maxlength="7" value="#prdata.PRvalor#"  style="text-align: right;"  onFocus="javascript:this.value=qf(this); restaurar_color(this); this.select();" onBlur="javascript: if (trim(this.value) != ''){fm(this,2);}"   onKeyUp="javascript:if(snumber(this,event,2)){ if(Key(event)=='13') {this.blur();}}"></td> --->
							<td><cf_inputNumber name="PRvalor_#prdata.currentrow+8#"  value="#prdata.PRvalor#" enteros="4" decimales="2" negativos="true" comas="yes"></td>
							<td>
								<select name="PRira_#prdata.currentrow+8#">
									<option value=""></option>
									<cfloop query="dataira">
										<option value="#dataira.PPid#" <cfif ira eq dataira.PPid >selected</cfif>>#dataira.PPnumero#</option>
									</cfloop>
								</select>							</td>
							<td><input type="text" name="PRorden_#prdata.currentrow+8#" size="3" maxlength="3" value="#prdata.PRorden#"  style="text-align: right;"  onFocus="javascript:this.value=qf(this); restaurar_color(this); this.select();" onBlur="javascript:fm(this,0);"  onKeyUp="javascript:if(snumber(this,event,0)){ if(Key(event)=='13') {this.blur();}}"></td>
							<td colspan="2" nowrap="nowrap">
								<img src="/cfmx/rh/imagenes/iindex.gif" border="0" onClick="javascript:formatear(#prdata.PRid#, #prdata.currentrow+8#);" style="cursor:hand;">
								<img src="/cfmx/rh/imagenes/Borrar01_S.gif" border="0" onClick="javascript: borraDet(#prdata.PRid#);" style="cursor:hand;">
							</td>
						</tr>
					</cfloop>
				</cfif>
				...
				<cfloop from="1" to="8" index="i">
					<tr>
						...
						<td>
							<!--- <input type="text" name="PRvalor_#i#" size="7" maxlength="7" style="text-align: right;"  onFocus="javascript:this.value=qf(this); restaurar_color(this); this.select();" value="" onBlur="javascript: if (trim(this.value) != ''){fm(this,2);}"  onKeyUp="javascript:if(snumber(this,event,2)){ if(Key(event)=='13') {this.blur();}}"> --->
	...
	<tr>
		...
		<cfif modo_pregunta neq 'ALTA'>
				...
				<input type="button" name="NuevaPregunta" value="<cfoutput>#BTN_Nuevo#</cfoutput>" onClick="location.href='cuestionario.cfm?PCid=#form.PCid#&tab=3<cfif isdefined("form.PPparte")>&PPparte=#form.PPparte#</cfif>';" >
			</cfif>
			
			<!--- NO BORRAR <input type="hidden" name="filas_total" value="<cfif isdefined("prdata")>#prdata.recordcount+50#<cfelse>1</cfif>"> --->
			<input type="hidden" name="filas_total" value="<cfif isdefined("prdata")>#prdata.recordcount+8#<cfelse>8</cfif>">
...
<script language="javascript1.2" type="text/javascript">
	...
	function insRow(){
		...
		//var input_formato  = '<img src="/cfmx/sif/imagenes/iindex.gif" border="0" onClick="javascript:formatear();" style="cursor:hand;" >';
		var input_formato  = '';
	...
	function  ocultarcampos(){
		...
		if(document.form3.PPtipo.value == 'M' || document.form3.PPtipo.value == 'U'){
			tr1.style.display = "";
	...
	function borraDet(id){		
		if (Confirm1()){
		document.form3.BorrarDet.value = id;
		document.form3.BorrarDet.click();
		}			
	}
