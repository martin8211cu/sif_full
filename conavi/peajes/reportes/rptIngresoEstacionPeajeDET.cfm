...
<cfif isdefined("url.Generar")>
	<cfsavecontent variable="rsReporte">
		...
		<cfoutput>
			...
			group by pet.PETfecha,pt.PTid, pt.PTcodigo,p.Pid,p.Pdescripcion,pv.PVid,pv.PVdescripcion
			order by  p.Pid, pet.PETfecha, pt.PTid, pt.PTcodigo,p.Pdescripcion,pv.PVid,pv.PVdescripcion
...
<cftry>
	...
	<cfset totalpeajeMontoGrl = 0 >
    <cfset totalpeajeCantidadGrl = 0 >
	...
	<cfoutput query="data" group="peaje"  maxrows="5000">
		...
		<tr><td class="tituloListas" style="font-size:16px" colspan="20"><strong>PEAJE: </strong> #peaje#</td></tr>
		 <cfoutput group="fecha"> 
		 <tr><td class="tituloListas" colspan="20" style="font-size:16px" ><strong>FECHA:</strong> #DateFormat(fecha,'DD/MM/YYYY')#</td></tr>
		...
		select   
			pdtv.PDTVcantidad * pr.PPrecio * Coalesce(htc.TCcompra,1) Dinero,
			 pdtv.PDTVcantidad as cantidad,
			 pet.PETfecha as fecha,
			 pt.PTid as PTid,
             pt.PTcodigo as turno,
             p.Pid as Pid,
			...
			and pet.PETfecha = <cfqueryparam cfsqltype="cf_sql_date" value="#fecha#">
			and pt.PTcodigo = '#turno#'
			and p.Pid = #Pid#
		
		order by pet.PETfecha,pt.PTid,pt.PTcodigo,p.Pid,pdtv.PDTVcarril,pv.PVid,pv.PVdescripcion
		</cfquery>
		...
		<cfquery name="totalesCar" dbtype="query">
			select sum(cantidad) cantidad,
				   ...
				   PVid
				   from CarrosPasaron
				   group by fecha,PTid,turno,Pid,PVid
		</cfquery>
		...
		<cfset totalcarrilMonto=0>
		 ...
		 <cfloop query="totalesCar">
			...
			<cfset totalFinalMonto = totalFinalMonto + totalesCar.dinero >
				...
				<cfset totalpeajeMontoGrl = totalpeajeMontoGrl + totalFinalMonto >
				<cfset totalpeajeCantidadGrl = totalpeajeCantidadGrl + totalFinal >
			<cfif isdefined ("url.cantidad") and isdefined ("url.monto")>
			<td nowrap="nowrap" style="padding:3px;font-size:9px" align="right">
				#totalFinal# 
			<td nowrap="nowrap" style="padding:3px;font-size:9px" align="right">	
		...
		</cfoutput><!---cierra la fecha--->
		<tr>
		  <td><strong>Total peaje</strong></td>	
		  <td nowrap="nowrap" style="padding:3px;font-size:12px;" align="right"><strong>Cantidad: #totalpeajeCantidadGrl# &nbsp;&nbsp; Monto: ¢#numberFormat(totalpeajeMontoGrl,',9.00')#</strong></td>
		</tr>
		<tr><td>&nbsp;</td></tr>
    	<cfset totalpeajeMontoGrl = 0 >
        <cfset totalpeajeCantidadGrl = 0 >
