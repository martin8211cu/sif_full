	...
	<cfsavecontent variable="rsReporte">
		...
		<cfoutput>
			select
			 ...
			 <cf_dbfunction name="date_part" args="YYYY,pet.PETfecha"> as agnio,
			...
			group by  p.Pid,p.Pdescripcion,  <cf_dbfunction name="date_part" args="YYYY,pet.PETfecha">,<cf_dbfunction name="date_part" args="MM,pet.PETfecha">, pv.PVid,pv.PVdescripcion
			order by  p.Pid,p.Pdescripcion,  pv.PVid,pv.PVdescripcion, <cf_dbfunction name="date_part" args="YYYY,pet.PETfecha">,<cf_dbfunction name="date_part" args="MM,pet.PETfecha">
...
<table width="100%" nowrap="nowrap"  border="0" cellpadding="2" cellspacing="0">
...
<cftry>
	...
	<cfoutput query="data" group="peaje"  maxrows="5000">
		...
		<tr>
			<td class="tituloListas" style="font-size:16px" colspan="20"><strong>PEAJE: </strong> #peaje#</td>
		</tr>
		...
		<cfelse>
		 ...
		 <cfif isdefined("url.monto") and isdefined ("url.cantidad")> 
		  ...
		  <tr>
			  <td style="padding:3px;font-size:15px" width="2%" bgcolor="CCCCCC" nowrap="nowrap">Fecha</td>
		...
		select 
			...
			pv.PVid as PVid, 
			pv.PVdescripcion as vehiculo,
			sum (coalesce(pdtv.PDTVcantidad,0)) as cantidad,
			sum (coalesce(pdtv.PDTVcantidad,0) * coalesce(pr.PPrecio,0) * Coalesce(htc.TCcompra,1)) as dinero
			...
			group by p.Pid,p.Pdescripcion,<cf_dbfunction name="date_part" args="YYYY,pet.PETfecha">,<cf_dbfunction name="date_part" args="MM,pet.PETfecha">, pv.PVid,pv.PVdescripcion
			order by p.Pid,<cf_dbfunction name="date_part" args="YYYY,pet.PETfecha">,<cf_dbfunction name="date_part" args="MM,pet.PETfecha">,pv.PVid,pv.PVdescripcion
		</cfquery>
		...
		<cfloop query="CarrosPasaron">
			...
			<cfif rowfin2 EQ 0>	
				<cfset rowfin = CarrosPasaron.mes>	
				<cfset rowfin2 = 1>
			<cfelseif rowfin EQ CarrosPasaron.mes>
			<cfelse>
				<cfset rowfin = CarrosPasaron.mes>
				...
				<td nowrap="nowrap" style="padding:3px;font-size:9px" align="right">
					#totalmes# 
				<td nowrap="nowrap" style="padding:3px;font-size:9px" align="right">	
					¢#numberFormat(totalmesMonto,',9.00')#
				<cfelseif isdefined ("url.cantidad")>
				<td nowrap="nowrap" style="padding:3px;font-size:9px" align="right">
					#totalmes#
				<cfelse>
			...
			<cfif row  NEQ CarrosPasaron.mes>
				<cfset row = CarrosPasaron.mes>
				<cfif CarrosPasaron.currentrow NEQ 1>
					</tr>
				</cfif>	
				<tr>
				<td nowrap="nowrap" style="padding:3px;font-size:9px">#ListGetAt(meses, CarrosPasaron.mes)#<cfif url.periodoInicial neq url.periodoFinal>-#CarrosPasaron.agnio#</cfif></td>
			</cfif>
			...
			<cfset totalmes = totalmes+ CarrosPasaron.cantidad>
			<cfset totalmesMonto = totalmesMonto+ CarrosPasaron.Dinero>
		</cfloop>
		...
		<cfset totalmesMonto=0>
		 ...
		 <tr>
			 <td style="padding:3px;font-size:15px" bgcolor="CCCCCC" nowrap="nowrap">Total</td>
			 <cfset totalFinal = 0>
