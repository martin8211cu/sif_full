<!---MCZ Se busca cuales modulos tiene asociados la empresa esto en caso de que la empresa solo maneje rh entonces se activara el skin de SAPIENS--->
<cfquery name="rsModEmp" datasource="asp">
	select count(1) as cantidad from ModulosCuentaE where CEcodigo=#session.CEcodigo# and SScodigo !='RH'
</cfquery>
<cfif rsModEmp.cantidad eq 0>
	<cfset session.sapiens=true>
<cfelse>
	<cfset session.sapiens=false>
</cfif>
<!------>
...
<cfquery name="rsContents" datasource="asp" cachedwithin="#CreateTimeSpan(0,0,10,0)#" >
	select 
	  ...
	  m.SMdescripcion as SMdescripcionOriginal,
		coalesce( (	select VSdesc
					...
					where Iid = <cfif len(trim(rsIdioma.Iid))>#rsIdioma.Iid#<cfelse>0</cfif>
					  and VSgrupo = 102
					  and VSvalor =  rtrim(s.SScodigo)#_Cat# '.' #_Cat# rtrim(m.SMcodigo) ), m.SMdescripcion) as SMdescripcion,
	  m.SMhomeuri,
    ...
    <cfif rsContents.RecordCount gt 0 >
      <table width="968" border="0" cellpadding="2" cellspacing="0" align="center" >
        ...
        <cfoutput query="rsContents" group="SScodigo">
          ...
          <td width="#ancho_sistema#" valign="top"><cf_web_portlet_start border="true" width="#ancho_sistema-32#" skin="Gray" tituloalign="left" titulo='#translated_Sistema#'>
                <table width="100%" cellpadding="3" cellspacing="0" border="0" >
                  <tr>
                    <td align="center" colspan="4"><cfif rsContents.SSlogo GT 1>
                        <cfinvoke 
						...
						<cfif rsContents.SScodigo eq 'RH'>
							<img align="middle" src="../public/logo_sistema.cfm?s=#URLEncodedFormat(rsContents.SScodigo)#&amp;ts=#tsurl#" border="0" alt="" width="115" height="68" >
						<cfelse>
                  ...
                  <cfoutput group="SMcodigo">
                    ...
                    <td valign="middle" width="#ancho_modulo#"><a class="fbox" href="#uri#"><cfif len(trim(rsContents.SMdescripcion))>#rsContents.SMdescripcion#<cfelse>#rsContents.SMdescripcionOriginal#</cfif></a></td>
                      <cfif not j mod 2>
                        </tr>
                      </cfif>
                      <cfset j = j+1 >
