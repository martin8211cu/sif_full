<!--- OJO se incluyo este .js que estaba el login.cfm --->
<cfif (not isdefined("session.usucodigo")) or session.usucodigo is 0>
<script language="JavaScript1.2" type="text/javascript" src="/cfmx/home/public/login.js">//</script>
</cfif>
<cfparam name="url.uri" default="/cfmx/plantillas/iglesias06/index.cfm">
<style type="text/css">
<!--
.style1login {font-weight: bold}
.style2login {color: #FF0000}
-->
</style>
<cfif (not isdefined("session.usucodigo")) or session.usucodigo is 0>
	<form name="login" onsubmit="javascript:return validarLogin(this)" action="<cfoutput>#url.uri#</cfoutput>" method="post" style="margin:0">
		<table width="400" border="0" cellpadding="0" cellspacing="0" bordercolor="#111111" id="AutoNumber4" style="border-collapse: collapse;margin:0">
			<tr>
			  <td valign="middle"> 
			  <div align="left" class="style9"><strong><font size="1" face="Verdana">Usuario:</font></strong></div></td>
			  <td valign="top">
			 
				<p style="margin-top: 0; margin-bottom: 0">
			  <input type="text" name="j_username" size="12" style="font-size: 8pt" onFocus="this.select()"></p>			  </td>
			  <td valign="middle"><div align="left" class="style9"><strong><font size="1" face="Verdana">Contrase&ntilde;a:</font></strong></div></td>
			  <td valign="top"><input type="password" name="j_password" size="14" style="font-size: 8pt" onFocus="this.select()"></td>
			  <td valign="top"><input type="submit" value="Conectarse" name="Submit2" style="font-size: 8pt"></td>
			  <tr><td></td>
			<cfif IsDefined("url.errormsg")>
			  <td  valign="top" nowrap>
				<strong class="style2login">No se pudo iniciar la sesi&oacute;n.</strong>
			  </td>
			  <td width="150"><a href="/cfmx/home/public/recordar.cfm"><font color="#0000FF">¿Necesita
		  ayuda para ingresar?</font></a>  
			  </td></cfif></tr>
			</tr>
	  </table><input type="hidden" name="recordar">
  </form>	
<cfelse>
	<form name="login" action="/cfmx/home/public/logout.cfm" method="post" style="margin:0">
	<table width="150" height="27" border="0" cellpadding="0" cellspacing="0">
	  <tr>
		<td align="center"><input type="submit" name="Submit" value="Desconectarse"></td>
	  </tr>
	</table>	
	<input type="hidden" name="uri" value="/cfmx/plantillas/iglesias06/index.cfm">
	</form>
</cfif>
<cfif (not isdefined("session.usucodigo")) or session.usucodigo is 0>
<script type="text/javascript">
<!--
llenarLogin(document.login);
-->
</script>
</cfif>