<cfcomponent>
    ...
    <cffunction  name="createReciboPago"> <!--- FUNCION PARA CREAR EL PDF DE RECIBOS DE PAGO POR CUENTA (MERGE DE CADA PAGINA)--->
        ...
        <cftry>
            ...
            <cfelse>
                <cfthrow Message="No se generaron paginas para el recibo de pago." detail="Posible causa: Todos los recibos estan Pagados.">
            </cfif>
    ...
    <cffunction  name="RP_Data"> <!--- FUNCION PARA CREAR LAS PAGINAS DE CADA RECIBO DE PAGO POR CUENTA --->
        ...
        <cfquery name="q_ReciboPagos" datasource="#arguments.dsn#">
            ...
            left join (
                ...
                on B.Cliente = cn.Cliente and B.CURP = cn.CURP and B.CRCCuentasid = cn.CuentaId <!--- NAva, Duplicidad de recibo --->
            left join (
        ...
        <cfif q_ReciboPagos.recordCount gt 0>
            ...
            <cfloop query="#q_ReciboPagos#">
                ...
                <cfset e_cliente.ePagueAntes           = "#DateFormat(calculateDueDate(corte=codCorte, fecha=q_ReciboPagos.FechaFin),'yyyy.mm.dd')#">
    ...
    <cffunction  name="toLSCurrencyFormat">
     ...
     <cffunction name="calculateDueDate" hint="Calcula la fecha de vencimiento de un recibo de pago.">
        <cfargument  name="corte"      type="string"   required="true">
        <cfargument  name="fecha"      type="date"     required="true">
        <!---
            <cfset year = Year(arguments.fecha)>
            <cfset month = Month(arguments.fecha)>
            <cfset day = Day(arguments.fecha)>
        <cfset corteNum = right(arguments.corte,1)>
        
        <cfif corteNum eq '2'>
            <cfset dueDate = CreateDate(year, month, DaysInMonth(CreateDate(year, month + 1, 1)))>
        <cfelse>
            <cfset dueDate = CreateDate(year, month, 15)>
        </cfif>

        <cfreturn dueDate>
        --->        
        <cfreturn DateAdd("d", 6, arguments.fecha)> <!--- Se agrega dia mas, por fecha de corte ---> 
    </cffunction>
