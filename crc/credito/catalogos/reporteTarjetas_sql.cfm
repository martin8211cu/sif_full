...
<cfset LB_Titulo2		= t.Translate('LB_Titulo2','Reporte de uso de Tarjetas')>
...
<cfset LB_Tarjeta		= t.Translate('LB_Tarjeta', 'Tarjeta de la Cuenta')>
<cfset LB_Monto			= t.Translate('LB_Monto', 'Monto')>
<cfset LvarPagina = "reporteVales.cfm">


<cfset prevPag="reporteTarjetas.cfm">
<cfset targetAction="reporteTarjetas_sql.cfm">
<cfinclude template="imprimirReporte.cfm" >
	
<cfquery name="rsReporteTarjetas" datasource="#Session.DSN#">

select 
	  t.Tienda as tienda
	, t.Fecha as fecha
	, c.Numero as cuenta
	, sn.SNnombre as cliente
	, tr.Numero as tarjeta
	, t.Monto as monto
	, co.Codigo as corte
	, sn.SNid as SNID
	, t.Parciales
from CRCTransaccion t 
	...
	inner join CRCTarjeta tr
		on tr.id = t.CRCTarjetaid
	inner join CRCCortes co
		on t.Fecha between co.FechaInicio and dateadd(day,1,co.FechaFin)
		and c.Tipo  = co.Tipo
where
	c.ecodigo = #session.ecodigo#
	and (co.Tipo = 'TC' or co.Tipo = 'TM' )
	<cfif !isDefined('url.p')> and 1=0 </cfif>
	and Rtrim(Ltrim(isNull(t.CRCtarjetaid,''))) <> ''
				...
				<table  width="100%" cellpadding="2" cellspacing="0" border="0">
					...
					<tr>
						<table width="100%" border="0">
							...
							<cfif rsReporteTarjetas.RecordCount gt 0>
								<cfloop query="rsReporteTarjetas">
									<tr>
										<td>#rsReporteTarjetas.tienda#</td>
										<td>#DateFormat(rsReporteTarjetas.fecha,"dd/mm/yyyy")#</td>
										<td>#rsReporteTarjetas.cuenta#</td>
										<td>#rsReporteTarjetas.cliente#</td>
										<td>#rsReporteTarjetas.tarjeta#</td>
										<td>#rsReporteTarjetas.Parciales#</td>
										<td>#LSCurrencyFormat(rsReporteTarjetas.monto)#</td>
									</tr>
								</cfloop>
							<cfelse>
