...
<cfif parentEntrancePoint eq 'CuentasAdmin.cfm' or parentEntrancePoint eq 'Cuentas.cfm' or parentEntrancePoint eq 'LineaCredito.cfm'>
	<cfquery name="q_usuario" datasource="#session.DSN#">
		...
		<cfif parentEntrancePoint neq 'CuentasAdmin.cfm' and parentEntrancePoint neq 'LineaCredito.cfm'>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
	...
	<tr>
		...
		<cfif isdefined('q_usuario') and #q_usuario.llave# neq ''>
			<td valign="top">
				<cfinvoke component="commons.Componentes.pListas" method="pListaRH"
					...
					columnas="c.id, c.Numero,c.SNegociosSNid,c.Tipo, c.MontoAprobado,
								case c.Tipo
									when 'D' then 'Vales'
									when 'TC' then 'Tarjeta de Credito'
									when 'TM' then 'Tarjeta Mayorista'
									else ''
								end as TipoDescripcion,c.CRCCategoriaDistid,c.CRCEstatusCuentasid,s.SNnombre, case when c.Tipo = 'D' then cd.Titulo else '' end Titulo,ec.Descripcion as Estado"
					desplegar="Numero,SNnombre,TipoDescripcion,Estado,Titulo,MontoAprobado"
					etiquetas="Numero de Cuenta,Socio de Negocio,Tipo,Estado,Categoria, Limite de Credito"
		...
		<cfelse>
			<td valign="top">
				<p align="center">
					<font color="red">
						El usuario no ha sido asignado a un gestor / abogado.
					</font>
				</p>
			</td>
		</cfif>
