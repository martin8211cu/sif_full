...
<cfoutput>
		...
		<cfset corteNum = right(CodCorte,1)>
		
		<!--- Estructura de datos para números de mes a nombres --->
		<cfset mesesNombres = {
			1 = "Enero",
			2 = "Febrero",
			3 = "Marzo",
		...
		<cffunction name="getNombreMes" access="private" returntype="string">
			<cfargument name="numeroMes" type="numeric" required="true">
			<cfif numeroMes gte 1 AND numeroMes lte 12>
				<cfreturn mesesNombres[numeroMes]>
			<cfelse>
				<cfreturn "Mes inválido">
			</cfif>
		</cffunction>
							...
							<td width="55%" align="center">
								<table #noBorder# width="100%">
									<tr>
										<td>
											<cfif corteNum eq "1">
												<cfquery name="q_bonos" datasource="#session.DSN#">													
													...
													WHERE Mes_Venta = '#DateFormat(DateAdd('m', -1, q_Corte.fechafin), "yyyy-MM")#'
														AND Cuenta = '#q_infoCuenta.Numero#'
													GROUP BY Cuenta, Mes_Venta
												</cfquery>
												<cfif q_bonos.recordCount gt 0>
													<div style="border: 3px solid ##2196F3; background-color: ##E3F2FD; padding: 20px; margin: 8px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
														<div style="text-align: center; font-size: 30px; font-weight: bold; color: ##1976D2; margin-bottom: 8px;">
															¡FELICIDADES!
														</div>
														<div style="text-align: center; font-size: 20px; color: ##333; line-height: 1.6;">
															Te has ganado <span style="color: ##0D47A1; font-weight: bold; font-size: 26px;">$#LSCurrencyFormat(q_bonos.Bono, "none")#</span> en <span style="font-weight: bold;">GIFTCARD</span> 
															<br/>por tus ventas de #getNombreMes(Month(ParseDateTime(q_bonos.Mes_Venta & "-01")))#.
														</div>
													</div>
												<cfelse>
													&nbsp;
				...
				<td width="75%" align="center">
					...
					<table #noBorder# style="#resumenE#" width="100%">
						...
						<cfloop from="1" to="30" index="i">
							...
							<cfif dateDiffVar lt val.valor + 2 and dateDiffVar gte 0 >
					...
					<table #noBorder# style="#resumenE#" width="100%">
						...
						<cfloop query="#q_MovCuenta#">
							...
							<tr>
								...
								<td style="#resumenF_tdL#" nowrap>#left(Ucase(q_MovCuenta.Cliente),30)#</td> <!---  - CAmbio posible --->
						...
						<table #noBorder# style="#resumenE#" width="100%">
							...
							<cfloop query="q_ComprasDiferidas">
								<tr>
									...
									<td style="#resumenF_tdL#">#Ucase(q_ComprasDiferidas.Cliente)#</td>
