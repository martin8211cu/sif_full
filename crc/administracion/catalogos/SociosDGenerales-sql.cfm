...
<cffunction access="public" name="validaSNnumero" output="true">
	<cfargument name="empresa" type="string" required="yes" default="" >
	<cfargument name="SNnumero" type="string" required="no" default="" >

	<cfif len(trim(arguments.empresa)) and len(trim(arguments.SNnumero))>
		<cfquery name="rsSNnumero" datasource="#session.DSN#">
			select count(1) as cantidad
			from SNegocios
			where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.empresa#">
			and SNnumero = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.SNnumero#">
		</cfquery>
		
		<cfif rsSNnumero.cantidad gt 0>
			<cfset request.Error.Backs = 1 >
			 <cfif session.Ecodigo neq arguments.empresa>
				<cf_errorCode	code = "50007"
								msg  = "El número de Socio @errorDat_1@ digitado, esta asignado a otro Socio de Negocios para la empresa corporativa."
								errorDat_1="#SNnumero#"
				>
			<cfelse>
				<cf_errorCode	code = "50008"
								msg  = "El número de Socio @errorDat_1@ digitado, esta asignado a otro Socio de Negocios."
								errorDat_1="#SNnumero#"
				>
			</cfif>
		</cfif>
...
<cfif not isdefined("Form.Nuevo")>
	...
	<cfif isdefined("Form.ALTA")>
		...
		<cftransaction>
			...
			<cfquery name="rsNextSNcodigoext" datasource="#session.DSN#">
				select coalesce(max(cast(SNcodigoext as numeric)),0)+1 as SNcodigoext
				from SNegocios
			...
			<cfif modalidad.replicar and form.ALTA_CORPORATIVO and session.EcodigoCorp NEQ session.Ecodigo>
				...
				<cfset validaSNnumero(session.EcodigoCorp, Form.SNnumero) >
				...
				<cfquery name="insertCorp" datasource="#session.DSN#">
					insert into SNegocios (Ecodigo, SNcodigo, SNidentificacion, SNidentificacion2,  SNnombre, SNcodigoext, Mcodigo,
										   ...
										   SNtelefono, SNFax, SNemail, SNFecha, SNFechaNacimiento, SNtipo,
										   SNinactivo, usaINE, SNactivoportal, SNnumero,
										   Ppais, SNcertificado, LOCidioma,  
										   SNesCorporativo, SNinclusionAutoriz, EcodigoInclusion,esIntercompany,Intercompany,
										   SNtiposocio, SNMid, intfazLD, sincIntfaz,IdFisc,Nacional,SNenviarEmail,BMUsucodigo
										   )
						values (<cfqueryparam cfsqltype="cf_sql_integer" value="#session.EcodigoCorp#">,
								...
								<cfqueryparam cfsqltype="cf_sql_timestamp" 	 value="#LSParseDateTime(form.SNFechaNacimiento)#" null="#len(trim(form.SNFechaNacimiento)) eq 0#">,
								...
								,<cfif isDefined('form.SNenviarEmail')>1<cfelse>0</cfif>
								,<cfqueryparam cfsqltype="cf_sql_numeric" value="#session.usucodigo#">
						)
						<cf_dbidentity1 datasource="#session.DSN#">
				</cfquery>
				...
				<cfquery datasource="#session.dsn#">
					...
					values (
						...
						<cfif isDefined('form.d_fac')>1<cfelse>0</cfif>,
						<cfif isDefined('form.d_env')>1<cfelse>0</cfif>,
						<cfif isDefined('form.d_act')>1<cfelse>0</cfif>, 0, null, <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Usucodigo#">,
						<cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.SNtelefono#">,
						<cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.SNFax#">,
						<cfqueryparam cfsqltype="cf_sql_varchar" value="#Form.SNemail#">,
			...
			<cfquery name="insert" datasource="#session.DSN#">
				...
				insert into SNegocios (Ecodigo, SNcodigo, SNidentificacion, SNidentificacion2,  SNnombre, SNcodigoext, Mcodigo,
									   ...
									   SNinactivo, usaINE, SNactivoportal, SNnumero,
									   Ppais, SNcertificado, LOCidioma,
									   SNidCorporativo, SNinclusionAutoriz, EcodigoInclusion,esIntercompany,Intercompany,
									   SNtiposocio,SNMid,intfazLD,sincIntfaz,IdFisc,Nacional,disT,TarjH,Mayor,SNenviarEmail,BMUsucodigo
									   )
					values ( #Session.Ecodigo# ,
							...
							<cfqueryparam cfsqltype="cf_sql_varchar" value="#la_direccion.direccion1#">,
							<cfqueryparam cfsqltype="cf_sql_numeric" value="#la_direccion.id_direccion#">,
							...
							<cfqueryparam cfsqltype="cf_sql_numeric" value="#insertCorp.identity#" null="#Len(insertCorp.identity) EQ 0#">,
							0,  #Session.Ecodigo#
							<cfif isdefined("Form.esIntercompany") and len(trim(Form.esIntercompany))>
								, 1
							<cfelse>
							...
							<cfif isdefined('form.disT')>
								,1
							<cfelse>
								,0
							</cfif>
							<cfif isdefined('form.TarjH')>
								,1
							<cfelse>
								,0
							</cfif>
							<cfif isdefined('form.Mayor')>
								,1
							<cfelse>
								,0
							</cfif>
			...
			<cfquery datasource="#session.dsn#">
				...
				values (
					...
					,<cfif isDefined('form.d_fac')>1<cfelse>0</cfif>
					,<cfif isDefined('form.d_env')>1<cfelse>0</cfif>
					,<cfif isDefined('form.d_act')>1<cfelse>0</cfif>
				)
			</cfquery>
			<cfset modo="CAMBIO">
		</cftransaction>
	...
	<!--- INICIA - Invocación del WS de sincronización de clientes --->
	...
	<cfelseif isdefined("Form.Baja")>
		...
		<!---  Verifica que no hayan documentos del socio que se quiere dar de baja.--->
			...
			<cfif LvarValida eq 1>
				<cfset resultT = "El Socio de Negocios que intenta eliminar posee al menos un documento en el sistema. PROCESO CANCELADO">
			...
			<cfelseif LvarValida eq 0>
				<cfquery datasource="#Session.DSN#">
					update SNegocios set eliminado = CURRENT_TIMESTAMP
					where Ecodigo =  #Session.Ecodigo#
						and SNcodigo = <cfqueryparam value="#Form.SNcodigo#" cfsqltype="cf_sql_integer">
				</cfquery>
				<cfquery datasource="#Session.DSN#">
					update c set c.CRCEstatusCuentasid = 7
					from CRCCuentas c
						inner join SNegocios sn
							on sn.SNid = c.SNegociosSNid	
					where c.Ecodigo =  #Session.Ecodigo#
						and sn.SNcodigo = <cfqueryparam value="#Form.SNcodigo#" cfsqltype="cf_sql_integer">
				</cfquery>
			...
			<cfquery name="update" datasource="#Session.DSN#">
				...
				set  				 
					...
					<cfif not modalidad.readonly>,
						...
						SNdireccion 	 = 	<cfqueryparam cfsqltype="cf_sql_varchar" 	value="#la_direccion.direccion1#">,
						...
						SNFechaNacimiento = <cfqueryparam cfsqltype="cf_sql_timestamp" 	 value="#LSParseDateTime(form.SNFechaNacimiento)#" null="#len(trim(form.SNFechaNacimiento)) eq 0#">,
						...
						SNenviarEmail 	 =  <cfif isDefined('form.SNenviarEmail')>1<cfelse>0</cfif>
						<cfif isdefined("Form.esIntercompany") and len(trim(Form.esIntercompany))>
							, esIntercompany = 1
			...
			<cfquery datasource="#session.DSN#">
				update SNDirecciones
					set
						...
						SNDfacturacion	 = <cfif isDefined('form.d_fac')>1<cfelse>0</cfif>,
						SNDenvio	 	 = <cfif isDefined('form.d_env')>1<cfelse>0</cfif>,
						SNDactivo	 	 = <cfif isDefined('form.d_act')>1<cfelse>0</cfif>,
						<cfif isdefined("Form.SNDirFiscSATmx")>
							<cfif Form.SNDirFiscSATmx eq 'on'>
...
<cfoutput>
	<form action="Socios.cfm" method="post" name="sql">
			<input type="hidden" name="SNcodigo" id="SNcodigo" value="#form.SNcodigo#">
