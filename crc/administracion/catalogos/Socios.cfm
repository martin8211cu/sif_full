...
<cfinvoke component="sif.Componentes.Translate" method="Translate" key = "LB_Adenda" default = "Asignaci&oacute;n de Adendas al Socio de Negocios" returnvariable="LB_Adenda" xmlfile = "Socios.xml">
...
<cfinvoke component="sif.Componentes.Translate" method="Translate" key = "LB_Distribuidor" default = "Distribuidor" returnvariable="LB_Distribuidor" xmlfile = "Socios.xml">
<cfinvoke component="sif.Componentes.Translate" method="Translate" key = "LB_TarHabiente" default = "Tarjeta Habiente" returnvariable="LB_TarHabiente" xmlfile = "Socios.xml">
<cfinvoke component="sif.Componentes.Translate" method="Translate" key = "LB_Mayorista" default = "Mayorista" returnvariable="LB_Mayorista" xmlfile = "Socios.xml">
...
<cf_templateheader title="#LB_SociosNegocio#">
		...
		<cfif isdefined("form.SNcodigo") and len(trim(form.SNcodigo))>
			<cfquery name="rsSocios" datasource="#Session.DSN#" >
				select yo.SNenviarEmail,yo.SNplazoentrega,yo.SNplazocredito,rtrim(yo.LOCidioma) as LOCidioma,yo.Ecodigo, yo.SNcodigo, yo.SNidentificacion,yo.SNidentificacion2, yo.SNtiposocio, yo.SNnombre, yo.SNdireccion,
				 yo.CSNid, yo.ESNid, yo.DEidEjecutivo, yo.DEidVendedor, yo.DEidCobrador, yo.SNnombrePago,
				 ...
				 coalesce(yo.SNactivoportal, 0) as SNactivoportal, yo.SNnumero, yo.ts_rversion, yo.Ppais, yo.SNcodigoext, yo.cuentac,
				 ...
				 yo.esIntercompany,yo.Intercompany,yo.TESRPTCid,yo.TESRPTCidCxC, yo.SNMid, yo.IdFisc,yo.Nacional,
                 		 yo.saldoCliente,yo.remisionProveedor, yo.sincIntfaz,yo.disT,yo.TarjH,yo.Mayor, yo.SNFechaNacimiento
				from SNegocios yo
					...
					<tr><td align="center" valign="top" height="600">
						...
						<cf_tabs width="99%">
							...
							<cfif Len(form.SNcodigo)>
								...
								<cfif rsSocios.disT EQ 1>
									<cf_tab text="#LB_Distribuidor#" selected="#form.tab eq 7#">
										<cf_web_portlet_start border="true" titulo="#LB_Distribuidor#">
											<cfinclude template="CRCTCParametrosD.cfm">
										<cf_web_portlet_end>
									</cf_tab>
								</cfif>
																																		
								<cfif rsSocios.TarjH EQ 1>	
									<cf_tab text="#LB_TarHabiente#" selected="#form.tab eq 8#">
										<cf_web_portlet_start border="true" titulo="#LB_TarHabiente#">
											<cfinclude template="CRCTCParametrosTC.cfm">
										<cf_web_portlet_end>
									</cf_tab>
								</cfif>
																
								<cfif rsSocios.mayoR EQ 1>
									<cf_tab text="#LB_Mayorista#" selected="#form.tab eq 9#">
										<cf_web_portlet_start border="true" titulo="#LB_Mayorista#">
											<cfinclude template="CRCTCParametrosTCM.cfm">
										<cf_web_portlet_end>
									</cf_tab>
								</cfif>
								<cfset moduloCred = 1>
			
								<cf_tab text="#LB_ClasificacionGeneral#" selected="#form.tab eq 10#">
									<cf_web_portlet_start border="true" titulo="#LB_Clasificacion#" >
										 <cfinclude template="/sif/ad/catalogos/SociosClasif.cfm">
									<cf_web_portlet_end>
								</cf_tab>
								
								<!--- <cf_tab text="Adenda" selected="#form.tab eq 8#">
									<!---Falta agregar a la base de datos el text de "Adendas del Socio de Negocios" que probablemente sería "#LB_Adendas#" --->
...
<script language="JavaScript" type="text/javascript">
	...
	<cfoutput>
		...
		try{
			...
			try{
				SociosNegociosMask.attach(document.form[0][0], SociosNegociosMask.mask, "string");
			}catch(e2){ }
		}
		...
		try{
			...
			try{
				oCedulaMask.attach(document.form[0][4], oCedulaMask.mask, "string");
			}catch(e2){ }
		}
		...
		<cfif isdefined("form.SNcodigo") and len(trim(form.SNcodigo))>
			function funcDirecciones(){
				<!---
				alert("SociosDireccion.cfm NO EXISTE");
				return false;
				C:\ColdFusion2016\tiendas_full\wwwroot\sif\ad\catalogos\SociosDireccion.cfm
				C:\ColdFusion2016\tiendas_full\wwwroot\crc\administracion\catalogos\Socios.cfm 
				document.form.action='SociosDireccion.cfm?SNcodigo=#form.SNcodigo#&SNcat=1';--->
				window.location.href ='SociosDireccion.cfm?SNcodigo=#form.SNcodigo#&SNcat=1';
				return false;
			}
		</cfif>
	</cfoutput>
