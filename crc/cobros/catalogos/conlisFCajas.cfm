...
<cfquery name="conlis" datasource="#Session.DSN#">
  ...
  <cfif isdefined('url.crc')>
    and FCid not in (select FCid from TipoTransaccionCaja where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">)
  </cfif>
</cfquery>
...
<form action="" method="post" name="conlis">
  <table width="100%" border="0" cellspacing="0">
    ...
    <tr>
      <td><input type="text" name="FCcodigo" size="20" maxlength="15"></td>
      <td colspan="3"><input type="text" name="FCdesc" size="50" maxlength="60"></td>
    </tr>
    <cfoutput query="conlis" startRow="#StartRow_conlis#" maxRows="#MaxRows_conlis#">
      <tr>
	  	<td <cfif #conlis.CurrentRow# MOD 2> class="listaPar" <cfelse> class="listaNon" </cfif>>
          <a href="javascript: Asignar('#JSStringFormat(conlis.FCid)#','#JSStringFormat(conlis.FCdesc)#');">#conlis.FCcodigo#</a></td>
        <td colspan="3" <cfif #conlis.CurrentRow# MOD 2>class="listaPar"<cfelse>class="listaNon"</cfif>>
