...
<cfcomponent output="no">
	<cffunction name="CG_AplicaAsiento" access="public" output="no">
		...
		<cfquery datasource="#session.dsn#" name="EContables">
			select 	Eperiodo, Emes, Cconcepto, Edocumento, NAP,
					<cfif not arguments.MesCierre>
						ECtipo, 
					<cfelse>
						1 as ECtipo,
					</cfif>
					Edocbase, Ereferencia, Efecha, ECreversible, ECauxiliar
			 from EContables
		...
		<cfif DContables.cantidad GT 500000>
			...
			<cfquery datasource="#session.dsn#">
				insert into #LvarTablaAsiento# (
					...
					Dtipocambio, Doriginal, Dlocal, CFid)
				select 
					...
					Dmovimiento, Mcodigo, 
					'Resumen', 'Resumen', 
					avg(Dtipocambio), sum(Doriginal), sum(Dlocal), CFid
				from DContables
				where IDcontable = #Arguments.IDcontable#
				group by Ecodigo, Cconcepto, Ocodigo, Eperiodo, Emes, Edocumento, Ccuenta, CFcuenta, Dmovimiento, Mcodigo
			</cfquery>
