...
<cfif rsverexistencia.Cantidad EQ 0>
	<cfquery name="rsVariables" datasource="sifinterfaces">
		select
			...
			,  Falta
			,  ECIreversible
            ,  coalesce(Ecodigo,0) as Ecodigo
			,  month(Efecha) as mesDoc
		from IE18
		where ID = #GvarID#
	</cfquery>
	<cfset varIDIE18 = #GvarID#>
	<cfif rsVariables.recordcount EQ 0>
		<cfthrow message="Se envio un Asiento sin registro de control">
	</cfif>

	<cfset LvarEcodigo        = rsVariables.Ecodigo>
	...
	<cfquery name="rsControl" datasource="sifinterfaces">
		Select coalesce(Ecodigo,0) Ecodigo
		from ID18
		where ID = #GvarID#
	</cfquery>

	<cfif rsControl.recordcount EQ 0>
		<cfthrow message="Se envio a procesar un Asiento sin datos (en blanco)!">
	</cfif>

    <!---
		Verificar que el Ecodigo indicado pertenezca a una Empresa valida
		Si el codigo de la empresa no existe, el proceso se cancela!
	--->
    <cfif LvarEcodigo eq 0>
	...
	<cfif rsEmpresa.recordcount EQ 0 >
        <cfthrow message="Error en Interfaz 18. No se encontro la Empresa con el codigo #LvarEcodigo#, por la tanto no se puede generar los Documentos Contables. Proceso Cancelado!.">
    </cfif>
	
	<cfquery name="rsMes" datasource="#session.dsn#">
		select Pvalor as mesAux from Parametros
		where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#LvarEcodigo#">
		and Pcodigo = 60
	</cfquery>mes 
	
	<cfif rsVariables.mesDoc NEQ rsMes.mesAux>
		<cfthrow message=" Fecha de Documento no Corresponde al Periodo de Auxiliares. Proceso Cancelado!.">
	</cfif>
	
    <!--- Validar que el Concepto Contable exista para la empresa seleccionada --->
    ...
    <cfif rsCConcepto.recordcount EQ 0 >
    	<cfquery name="rsNEmpresa" datasource="#session.dsn#">
            select Edescripcion
            from Empresas
            WHERE Ecodigo  = <cfqueryparam cfsqltype="cf_sql_integer" value="#LvarEcodigo#">
        </cfquery>
        <cfthrow message="Error en Interfaz 18. No se encontro el Concepto Contable '#LvarCconcepto#' para la empresa #rsNEmpresa.Edescripcion#, por la tanto no se puede generar los Documentos Contables. Proceso Cancelado!.">
    </cfif>
	<!---
	...
	<cfset LvarTotalRegistros = rsTotales.Cantidad>
		...
		<cfquery name="rsInput" datasource="#session.DSN#">
			...
			select	#ECImp_id#, 		a.DCIconsecutivo,		a.Ecodigo, 			a.DCIEfecha
					...
					, a.Referencia3,    <cf_jdbcquery_param cfsqltype="cf_sql_decimal" value="null">,					a.CFcodigo
			  from #LvarID18# a
			 where ID = #GvarID#
		</cfquery>
		<!--- ********************************************************** --->
		<!---      Insercion de timbre fiscal en repositorio temporal.   --->
		<!--- ********************************************************** --->
		<cfquery name="rsTimbres" datasource="sifinterfaces">
			WITH timbre as (SELECT Distinct TimbreFiscal FROM ESIFLD_Facturas_Venta e
			WHERE ID18 = #varIDIE18#
			AND Tipo_Venta !='C'
			AND TimbreFiscal != '')
			SELECT TimbreFiscal, xml from timbre t
			INNER JOIN SIFLD_Timbres_Fiscales x ON t.TimbreFiscal = x.Fiscal_UUID
		</cfquery>
		...
		<cftry>
			<cfloop query="rsTimbres">
				...
				<cfset varTimbreFiscal = rsTimbres.TimbreFiscal>
				<cfquery name="insRepTmp" datasource="#session.DSN#">
					INSERT INTO CERepoTMP (Ecodigo,TimbreFiscal,ID_Documento,ID_Linea,Documento,Origen,TipoComprobante,Mcodigo,TipoCambio,xmlTimbrado,BMUsucodigo)
					VALUES	(
							<cfqueryparam cfsqltype="cf_sql_integer" value="#LvarEcodigo#">,
							<cfqueryparam cfsqltype="cf_sql_varchar" value="#varTimbreFiscal#">,
							<cfqueryparam cfsqltype="cf_sql_integer" value="#ECImp_id#">,
							1,
							<cfqueryparam cfsqltype="cf_sql_char" 	 value="#LvarEdocbase#">,
							'#varOrigen#',
							1,
							<cfqueryparam cfsqltype="cf_sql_numeric" value="#LvarMcodigo#">,
							<cfqueryparam cfsqltype="cf_sql_float" 	 value="#varTipoCambio#">,
							<cfqueryparam cfsqltype="cf_sql_varchar" value="#rsTimbres.xml#">,
		...
		<cfquery name="rsEncInterfaz18" datasource="#session.DSN#">
			select count(1) as Cantidad
			from EContablesInterfaz18
			where ID = #GvarID#
		</cfquery>

		<cfif rsEncInterfaz18.Cantidad GT 0>
       ...
       <cfquery name="rsPvalor" datasource="#session.DSN#">
	      select Pvalor from Parametros where Ecodigo = #LvarEcodigo# and Pcodigo = 2400
	   </cfquery>
	...
	<cfquery name="rsPolizaOrigen" datasource="sifinterfaces">
		select * from IE18 where ID = #GvarID#
	</cfquery>
	<cfif isDefined("rsPolizaOrigen.IDcontableOri") and #rsPolizaOrigen.IDcontableOri# neq "">
		<cfquery datasource="#session.DSN#">
			update PolizasTransferidas
			set IDE18 = <cfqueryparam cfsqltype="cf_sql_numeric" value="#GvarID#">
			where IDcontableOri = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsPolizaOrigen.IDcontableOri#">
		</cfquery>
	</cfif>
