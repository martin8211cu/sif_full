<!---
	...
	Fecha de U. Modificación: 16/3/2009
	Motivo de la Modificación: Se Modifico la Interpretación y Validación de la Información, además se agregaron algunos campos a las Tablas IE1o y ID10
							   Se agrego el Concepto de pagos a Terceros
...
<cftransaction isolation="read_uncommitted">
	...
	<cfquery name="readInterfaz10" datasource="sifinterfaces">
		select IE10.ID, EcodigoSDC, NumeroSocio, Modulo, CodigoTransacion, Documento, Estado, CodigoMoneda, FechaDocumento, 
		month(FechaDocumento) mesDoc, FechaVencimiento,
		...
		<cfelse>
			...
			ID10.codIEPS,
			isnull(ID10.MontoIEPS,0) as MontoIEPS,
			isnull(ID10.afectaIVA,0) as afectaIVA,    <!--- 0 = afecta IVA 1 = no afecta IVA default = afectaiva --->
			IE10.DEordenCompra,
			IE10.DEnumReclamo,
			IE10.DEobservacion,
			ID10.DDdescripcion,
			IE10.Dtipocambio,
			IE10.ConceptoCobroPago,
			TimbreFiscal,
			isnull(EDieps,0) as EDieps
		from IE10, ID10
