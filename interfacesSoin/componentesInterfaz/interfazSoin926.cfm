...
<cfset LvarFecha         = createODBCdate(rsVariables.FechaSolicitud)>
...
<cfif rsControl.Cantidad EQ 0>
	<cfthrow message="Se envio a procesar un registro sin datos!">
</cfif>
...
<cfquery name="rsDatos" datasource="#session.DSN#">
	...
	where Ecodigo   = #session.Ecodigo#
	  and Oficodigo = '#rsActualiza.Oficodigo#'
</cfquery>
<cfif rsDatos.recordcount GT 0>
	<cfset LvarOcodigo = rsDatos.Ocodigo>
<cfelse>
	<cfthrow message="La Oficina #rsActualiza.Oficodigo# no existe">
</cfif>
...
<cfquery name="rsOrdenes" datasource="sifinterfaces">
	select * from ID926 d
	inner join IE926 e on e.ID = d.ID
	where ----Genera_Orden = 1 and
	 e.ID = #GvarID#
	order by e.Empleado, d.Gasto asc 
</cfquery>
...
<!---	<cfquery name="rsFormatos" datasource="sifinterfaces">
   ...
   <!-----PROCESO DE CANCELACIÓN---->
			...
			<cfloop query="rsOrdenes"> 
			   <cfif not rsOrdenes.Gasto>
				   <cfquery name="rsNominaAnt" datasource="#session.DSN#">
						...
						from #sifinterfacesdb#..IE925 E
						inner join #sifinterfacesdb#..ID925 D on E.ID = D.ID
						where 
						Num_Nomina = <cfqueryparam cfsqltype="cf_sql_varchar" value="#LvarEreferencia#">
						and Empleado = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsOrdenes.Empleado#">
						and Total_Linea = <cfqueryparam cfsqltype="cf_sql_money" value="#rsOrdenes.Total_Linea#">
		   			</cfquery>
	...
	--->
<!-----TERMINA PROCESO DE CANCELACIÓN---->
