/*
@M:\SIF_EP\Modelo\scripts\WebServiceInterfaz\Ejemplo_interfaz8.sql
call dbms_java.grant_permission('USUARIO_ORACLE', 'java.net.SocketPermission', '10.7.7.195', 'accept, connect, listen, resolve'); 
call dbms_java.revoke_permission('USUARIO_ORACLE', 'java.net.SocketPermission', '10.7.7.195', 'accept, connect, listen, resolve'); 
*/
...
RETURN NUMBER AS
	...
	VALUES (ID,'CGDC',to_char(LvarNUMDOC),'prueba interfaz', getdate(), 2003, 2, null, 0);

	/* Genera la Tabla Entrada Salida (DEBE SER UN CICLO) */
	INSERT INTO ID8 
	(    ID,
	     NumeroLinea,
	     TipoMovimiento,
	...
	VALUES (ID,1,'C','0600-11-002-001-025-000','1','CRC',-1000,1,-1000,187,1);
	INSERT INTO ID8 
	(    ID,
	     NumeroLinea,
	     TipoMovimiento,
	...
	VALUES (ID,2,'C','0011-02','1','CRC',10000,1,10000,180,1);
	/* FIN DE CICLO */
...
BEGIN
	...
	/* LvarMSG := interfazDosPinosToSoin('http://oracle.dev.soin.net/cfmx/interfacesSoin/interfaz-service.cfm','soin','2','marcel','sup3rman','8',TO_CHAR(LvarID));
	*/
	LvarMSG := interfazDosPinosToSoin('http://oracle.des.soin.net/cfmx/interfacesSoin/interfaz-service.cfm','soin','2','marcel','sup3rman','8',TO_CHAR(LvarID));
	...
	IF LvarMSG <> 'OK' THEN
		...
		COMMIT;
	ELSE
		/* Solamente en el caso de Interfaz de Control de Presupuesto se verifica */
		/* el resultado para saber si se acepto (NAP) o no la transaccion (NRP) */
		SELECT OE8.NAP,OE8.NRP INTO LvarNAP, LvarNRP
 		  FROM OE8
		 WHERE OE8.ID = LvarID;
