...
public class interfazToSoin
{
         ...
         throws Exception
  {
    ...
    try 
    {
      String LvarUrl = UrlSoin + "?AU=" + fnCodifica(CESoin, ESoin, UidSoin, PwdSoin, NumeroInterfaz, IdProceso) + "&NI=" + NumeroInterfaz + "&ID=" + IdProceso;
      ...
      String LvarMSG = conn.getHeaderField("SOIN-MSG");
      if (LvarMSG != null)
        RETORNO = URLDecoder.decode(LvarMSG);
      else
        RETORNO = "ERROR ANTES DE LA INVOCACION DE LA INTERFAZ: no hubo resultado";
    }  
    catch (Exception e) 
    {
      RETORNO = "ERROR ANTES DE LA INVOCACION DE LA INTERFAZ: " + e.toString();
    }

    if (RETORNO.length() > 255)
    {
      RETORNO = RETORNO.substring(0,255);
    }
...
create or replace function interfazDosPinosToSoin 
 (urlSoin IN VARCHAR2, CESoin IN VARCHAR2, ESoin IN VARCHAR2, uidSoin IN VARCHAR2, pwdSoin IN VARCHAR2, interfaz VARCHAR2, id VARCHAR2)
RETURN VARCHAR2 AS
LANGUAGE java NAME
'interfazToSoin.sendToSoin
...
/
/*
call dbms_java.grant_permission('USUARIO_ORACLE', 'java.net.SocketPermission', '10.5.10.17', 'accept, connect, listen, resolve'); 
...
*/
select interfazToSoin('http://10.5.10.17/cfmx/interfacesSoin/interfaz-service.cfm','','2','INTERFAZ','passw0rd','8','12345') as MSG from dual;
select interfazDosPinosToSoin('http://10.5.10.17/cfmx/interfacesSoin/interfaz-service.cfm','','2','INTERFAZ','passw0rd','8','12345') as MSG from dual;
