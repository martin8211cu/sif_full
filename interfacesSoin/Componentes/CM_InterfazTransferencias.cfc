<cfcomponent>
	...
	<cffunction name="Alta_ETransferencias" access="public" returntype="numeric">	
		...
		<cfquery name="insertTrasp" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			insert into ETraspasos( Ecodigo, ETperiodo, ETmes, ETusuario, ETdescripcion, ETfecha, BMUsucodigo, Edocbase)
				 values ( <cfqueryparam value="#Request.CM_InterfazTransferencias.GvarEcodigo#"    cfsqltype="cf_sql_integer">,
						...
						,	<cfqueryparam value="#LvarNumeroDocumento#" cfsqltype="cf_sql_varchar">								  
						)
	...
	<cffunction name="Alta_DTransferencias" access="public" returntype="numeric">	
		...
		<cfargument name="TipoMovimientoOrigen" type="string" required="false" default="">								
		...
		<!---Valida la transaccion de Origen--->
        <cfif len(Arguments.TipoMovimientoOrigen) GT 0>
        	<cfset vBTidori = valida_BTid(Arguments.TipoMovimientoOrigen,'C','Alta_DTransferencias')>
        <cfelse>
        	<cfset vBTidori = 0>
		</cfif>
        ...
        <!---Valida la transaccion de Origen--->
        <cfif len(Arguments.TipoMovimientoDestino) GT 0>
        	<cfset vBTiddest = valida_BTid(Arguments.TipoMovimientoDestino,'D','Alta_DTransferencias')>
        <cfelse>
        	<cfset vBTiddest = 0>
		</cfif>
		...
		<cfquery  name="mLocal" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			select Mcodigo from Empresas where Ecodigo = #Request.CM_InterfazTransferencias.GvarEcodigo#
		</cfquery>
		<cfif mLocal.recordcount eq 0>
			<cfthrow message="CM_InterfazTransferencias:La empresa no tiene definida una moneda local. Proceso Cancelado!">
		<cfelseif mLocal.Mcodigo eq Arguments.CodigoMonedaOrigen>
			<cfset Arguments.TipoCambio = 1>
		</cfif>
		<cfif Arguments.TipoCambio lte 0>
			<cfset Arguments.TipoCambio = "">
		</cfif>
		<cfif isdefined('Arguments.CodigoMonedaOrigen') and len(trim(Arguments.CodigoMonedaOrigen))>
			<cfif  isdefined('Arguments.TipoCambio') and len(trim(Arguments.TipoCambio)) and Arguments.TipoCambio gt 0>
				<cfset vTCambio = Arguments.TipoCambio>
		...
		<cfquery name="updTraspasos" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			update ETraspasos
			set ETdescripcion = <cfqueryparam cfsqltype="cf_sql_char" value="#vETdescripcion#"> #_Cat# rtrim(ETdescripcion)
		...
		<cfquery name="insertTraspD" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			insert into DTraspasos 
				( 
					...
					DTdocumento, DTreferencia, DTdocumentodest, DTreferenciadest,
                    DTtipocambio, DTtipocambiof, BMUsucodigo) 
			values ( 
    ...
    <cffunction access="private" name="valida_BTid" output="false" returntype="numeric">
		<cfargument name="BTcodigo" required="yes" type="string">
        <cfargument name="BTtipo" required="yes" type="string">
		<cfargument name="InvokerName" required="no" type="string" default="">
        
        <cfquery name="rsResultado" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			select BTid 
            from BTransacciones 
            where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Request.CM_InterfazTransferencias.GvarEcodigo#">
            and BTcodigo like <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.BTcodigo#">
            and BTtipo like <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.BTtipo#">
		</cfquery>
        <cfif rsResultado.recordcount EQ 1>
        	<cfreturn trim(rsResultado.BTid)>	
        <cfelse>
			<cfreturn 0>
        </cfif>
	</cffunction>
	...
	<cffunction name="Aplica_TransferenciaEstimacion" access="public" returntype="boolean">
		...
		<cfargument name="NumeroDocumento" 			type="string" 	required="no"  default="0">
		<cfargument name="Debug" 					type="boolean" 	required="no"  default="false">
		<cfargument name="TipoCambio" 				type="numeric" 	required="no"  default="-1">
		<cfargument name="ImpRetencion" 			type="numeric" 	required="yes" default="-1">
		<cfargument name="CodRentencion" 			type="string" 	required="no"  default="">

			<cfset BTEtipoEst_BancoDestino = getBTEtipoEst(getBid(Arguments.CodigoBancoDestino),Arguments.TipoMovimientoDestino)>
		<cfif Arguments.ImpRetencion GT 0>
			<cfset Ccuentaretp_Destino = getCcuentaretp(Arguments.CodRentencion)>
		</cfif>
		...
		<cfelseif BTEtipoEst_BancoDestino EQ 'I'>
			<cfset lvarEtiqueta = "Intereses No Devengados">
		<cfelseif BTEtipoEst_BancoDestino EQ 'C' >
			<cfset lvarEtiqueta = "Intereses No Reintegrados">
		<cfelse>
        ...
        <cfif (isdefined('Arguments.IndMovConta') and Arguments.IndMovConta eq "2")>
			<cfset lvarEstimacion = 2>
		</cfif>
		<cfif isdefined('Arguments.NumeroDocumento') and len(trim(Arguments.NumeroDocumento))>
		...
		<!---
			...
			CONTROL DE EVENTOS
		--->	
        <!--- Obtiene el Ecodigo de la empresa --->
        <cfquery name="rsEmpresaCodigo" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
        	select Ecodigo
            from Empresas 
			where EcodigoSDC = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Request.CM_InterfazTransferencias.GvarEcodigoSDC#">
        </cfquery>
        <cfset varEcodigoEV = rsEmpresaCodigo.Ecodigo>
       
               <cfquery name = "MovFlujoEfectivo" datasource="#Session.Dsn#">
                   select isnull(max(Pvalor),2) Pvalor from Parametros
                   where Ecodigo = #Session.Ecodigo#
                   and Pcodigo = 200035
               </cfquery>
               <cfquery name = "Conceptos" datasource="#Session.Dsn#">
                     select FlujoEfec from Conceptos
                     where Ecodigo = #Session.Ecodigo#
                     and Ccodigo = '#Arguments.ConceptoComision#'
               </cfquery>
                     
				   <cfif (MovFlujoEfectivo.RecordCount GT 0 and MovFlujoEfectivo.Pvalor EQ 1) and Conceptos.FlujoEfec EQ 1 > 
                        
					 <!--- Si el parametro esta activo y el concepto de servicio debe generar Flujo de Efectivo MBFE Aplica para CIM--->
                     <!---Cambio RVD--->
                
							<cfif len(Arguments.MontoOrigen) and len(trim(Arguments.CodigoBancoOrigen)) and len(trim(Arguments.CuentaBancariaOrigen))>
                            <!--- Control de Eventos para la cuenta Origen --->
                                <!--- Se valida el control de eventos para la transaccion de Tesoreria --->
                                <cfinvoke component="sif.Componentes.CG_ControlEvento" 
                                    method="ValidaEvento" 
                                    Origen="MBFE"
                                    Transaccion="#Arguments.TipoMovimientoOrigen#"
                                    Complemento="#Arguments.CuentaBancariaOrigen#"
                                    Conexion="#Request.CM_InterfazTransferencias.GvarConexion#"
                                    Ecodigo="#varEcodigoEV#"
                                    returnvariable="varValidaEvento"
                                /> 	
                                <cfset varNumeroEventoO = "">
                                ...
                                <cfif varValidaEvento GT 0>
                                    <cfinvoke component="sif.Componentes.CG_ControlEvento" 
                                            ...
                                            Transaccion="#Arguments.TipoMovimientoOrigen#"
                                            Complemento="#Arguments.CuentaBancariaOrigen#"
                                            Documento	= "#Arguments.NumeroDocumento#"
                                            Conexion="#Request.CM_InterfazTransferencias.GvarConexion#"
                                            Ecodigo="#varEcodigoEV#"
                                        ...
                                        <cfset varNumeroEventoO = arNumeroEvento[3]>
                                </cfif>					
                            </cfif>
                            <!--- Control de Eventos para la cuenta Destino --->
                                <!--- Se valida el control de eventos para la transaccion de Tesoreria --->
                                <cfinvoke component="sif.Componentes.CG_ControlEvento" 
                                    method="ValidaEvento" 
                                    Origen="MBFE"
                                    Transaccion="#Arguments.TipoMovimientoDestino#"
                                    Complemento="#Arguments.CuentaBancariaDestino#"
                                    Conexion="#Request.CM_InterfazTransferencias.GvarConexion#"
                                    Ecodigo="#varEcodigoEV#"
                                    returnvariable="varValidaEvento"
                                /> 	
                                <cfset varNumeroEventoD = "">
                                <!--- Si esta activo el control de Eventos se procede a generar el Numero de Evento--->
                                <cfif varValidaEvento GT 0>
                                    <cfinvoke component="sif.Componentes.CG_ControlEvento" 
                                            method		= "CG_GeneraEvento"  
                                        ...
                                        <cfset varNumeroEventoD = arNumeroEvento[3]>
                                </cfif>
                        
            
            		<cfelseif (MovFlujoEfectivo.RecordCount GT 0 and MovFlujoEfectivo.Pvalor EQ 1) and Conceptos.FlujoEfec EQ 0 >         
						<!--- Si el parametro esta activo y el concepto de servicio debe generar Flujo de Efectivo MBNF Aplica para CIM--->
                             <!---Cambio RVD--->
                        ...
                        <cfelseif (MovFlujoEfectivo.RecordCount GT 0 and (MovFlujoEfectivo.Pvalor EQ 2 or MovFlujoEfectivo.Pvalor EQ 0)) 
									and Conceptos.FlujoEfec EQ 0 > 
                        	<!--- Si el parametro no esta activo y el concepto de servicio no genera Flujo de Efectivo se comporta como antes ESBA --->
                     <!---Cambio RVD--->
                    ...
                    </cfif><!---TERMINA COMO ESTABA--->
            
            
<cfif (MovFlujoEfectivo.RecordCount GT 0 and MovFlujoEfectivo.Pvalor EQ 1) and Conceptos.FlujoEfec EQ 1 > <!---Cambio RVD--->
		...
		<cfif len(Arguments.MontoOrigen) and len(trim(Arguments.CodigoBancoOrigen)) and len(trim(Arguments.CuentaBancariaOrigen))>
			<cfquery datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
				insert into #INTARC#
					...
					INTMOE, NumeroEvento)
				select
					'MBFE', 0, <cfqueryparam cfsqltype="cf_sql_char" value="#lvarDocumento#"> ,<cfqueryparam cfsqltype="cf_sql_char" value="#lvarEtiqueta#"> ,
					...
					, c.Mcodigo, c.Ocodigo,
                    <cfif lvarEstimacion eq 1> <!--- 1 movimiento reversible, aplica en el caso de que el TipoMovimientoDestino sea EL --->
						<cfqueryparam cfsqltype="cf_sql_money" value="#arguments.MontoOrigen#">
                    <cfelseif lvarEstimacion eq 2 and arguments.ImpRetencion EQ 0> <!--- 3 Pago de intereses reales de la línea de crédito SIN RETENCION --->
                    	<cfqueryparam cfsqltype="cf_sql_money" value="#arguments.MontoOrigen#">
                    <cfelseif lvarEstimacion eq 2 and arguments.ImpRetencion GT 0><!--- 2 intereses por línea de crédito, tipo movimiento origen y destino es IL --->
                    	#arguments.MontoDestino# - #arguments.ImpRetencion#
                    </cfif>,
                    '#varNumeroEventoO#'
		...
		<cfif len(Arguments.MontoDestino)>
        	<cfif lvarEstimacion eq 2>
	            <cfset LvarCuentaConceptoCxP = ''>
            	<cfset LvarCuentaConceptoCxP = obtieneCuentaCostoConcepto(Arguments.ConceptoComision, Arguments.CuentaBancariaDestino, "CuentaConcepto", "Aplica_TransferenciaEstimacion")>
            </cfif>
			...
			<cfquery datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
				...
				select
					...
					<cfif isdefined('Ccuentaretp_Destino')> - #arguments.ImpRetencion# </cfif>, 
					t1.BTtipo, 
					<cfif len(trim(arguments.Observacion))>
						<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.Observacion#">
					<cfelse>
					...
					<cfif lvarEstimacion eq 2>
	                    coalesce(#LvarCuentaConceptoCxP#,
                    <cfelse>
						<cfif BTEtipoEst_BancoDestino EQ 'I'><!---Estimación de Inversión--->
                            coalesce(c.Ccuentaint,
                        <cfelseif BTEtipoEst_BancoDestino EQ 'C'><!---Estimación de Credito--->
                            coalesce(c.CcuentaintPag,
                        </cfif>
                    </cfif>
                    ...
                    <cfelse>
						<cfif listfind('C,I',BTEtipoEst_BancoDestino)>
                            )
                        </cfif>
                    </cfif>
					
					, c.Mcodigo, c.Ocodigo, 
					<cfqueryparam cfsqltype="cf_sql_money" value="#arguments.MontoDestino#"><cfif isdefined('Ccuentaretp_Destino') and lvarEstimacion neq 2> - #arguments.ImpRetencion# </cfif>,
                    '#varNumeroEventoD#'
				from Empresas e, BTransacciones t1, Bancos b, CuentasBancos c
				where e.EcodigoSDC = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Request.CM_InterfazTransferencias.GvarEcodigoSDC#">
					and t1.Ecodigo = e.Ecodigo
			...
			<!---Linea de la Retención--->
			<cfif isdefined('Ccuentaretp_Destino')>
            
            	<cfset LvarTransaccionBanco = Arguments.TipoMovimientoDestino>
            	<cfif lvarEstimacion eq 2 and arguments.ImpRetencion GT 0>
                	<cfset LvarCuentaRet = ''>
					<cfset LvarCuentaRet = getCcuentaretp(Arguments.CodRentencion)>
					<cfset LvarTransaccionBanco = Arguments.TipoMovimientoOrigen>
            	</cfif>
				<cfquery datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
				insert into #INTARC# 
					(INTORI, INTREL, INTDOC, INTREF, 
					...
					INTMOE, NumeroEvento)
				select
					<cfqueryparam cfsqltype="cf_sql_char"  value="MBFE">,
					0, 
					<cfqueryparam cfsqltype="cf_sql_char"  value="#lvarDocumento#">,
					<cfqueryparam cfsqltype="cf_sql_char"  value="Retención">,
					<cfqueryparam cfsqltype="cf_sql_money" value="#arguments.ImpRetencion#">, 
					t1.BTtipo, 
					<cfif len(trim(arguments.Observacion))>
						<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.Observacion#">
					<cfelse>
                    ...
                    c.Mcodigo, c.Ocodigo, 
					<cfqueryparam cfsqltype="cf_sql_money" value="#arguments.ImpRetencion#"> <!--- (convertido a pesos cuando la moneda es distinta de la local) --->,
                    '#varNumeroEventoD#'
				from Empresas e, BTransacciones t1, Bancos b, CuentasBancos c
				where e.EcodigoSDC = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Request.CM_InterfazTransferencias.GvarEcodigoSDC#">
					and t1.Ecodigo = e.Ecodigo
					and t1.BTcodigo = <cfqueryparam cfsqltype="cf_sql_char" value="#LvarTransaccionBanco#">
					and b.Ecodigo = e.Ecodigo
					and b.Iaba = <cfqueryparam cfsqltype="cf_sql_char" value="#Arguments.CodigoBancoDestino#">
					and c.Bid = b.Bid
					and c.CBcodigo = <cfqueryparam cfsqltype="cf_sql_char" value="#Arguments.CuentaBancariaDestino#">
...
<cfelseif (MovFlujoEfectivo.RecordCount GT 0 and MovFlujoEfectivo.Pvalor EQ 1) and Conceptos.FlujoEfec EQ 0 >
		...
		<cfif len(Arguments.MontoOrigen) and len(trim(Arguments.CodigoBancoOrigen)) and len(trim(Arguments.CuentaBancariaOrigen))>
			<cfquery datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
				insert into #INTARC#
					...
					INTMOE, NumeroEvento)
				select
					'MBNF', 0, <cfqueryparam cfsqltype="cf_sql_char" value="#lvarDocumento#"> ,<cfqueryparam cfsqltype="cf_sql_char" value="#lvarEtiqueta#"> ,
					<cfqueryparam cfsqltype="cf_sql_money" value="#arguments.MontoOrigen#">, 
					t1.BTtipo, 
					<cfif len(trim(arguments.Observacion))>
						<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.Observacion#">
		...
		<cfif len(Arguments.MontoDestino)>
			...
			<cfif isdefined('Ccuentaretp_Destino')>
				...
				insert into #INTARC# 
					...
					INTMOE, NumeroEvento)
				select
					<cfqueryparam cfsqltype="cf_sql_char"  value="MBNF">,
					0, 
					<cfqueryparam cfsqltype="cf_sql_char"  value="#lvarDocumento#">,
					<cfqueryparam cfsqltype="cf_sql_char"  value="Retención">,
					<cfqueryparam cfsqltype="cf_sql_money" value="#arguments.ImpRetencion#">, 
	...
	<cfelseif (MovFlujoEfectivo.RecordCount GT 0 and (MovFlujoEfectivo.Pvalor EQ 2 or MovFlujoEfectivo.Pvalor EQ 0)) and Conceptos.FlujoEfec EQ 0 > 
		...
		<cfif len(Arguments.MontoDestino)>
			...
			<cfif isdefined('Ccuentaretp_Destino')>
				...
				insert into #INTARC# 
					...
					INTMOE, NumeroEvento)
				select
					<cfqueryparam cfsqltype="cf_sql_char"  value="ESBA">,
					0, 
					<cfqueryparam cfsqltype="cf_sql_char"  value="#lvarDocumento#">,
					<cfqueryparam cfsqltype="cf_sql_char"  value="Retención">,
					<cfqueryparam cfsqltype="cf_sql_money" value="#arguments.ImpRetencion#">, 
        ...
        <!----ANGELES--->
        <!---Obtiene el código de la moneda local--->
       	<cfquery name="rsMonedalocal" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			select Mcodigo
			from Empresas
			where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Request.CM_InterfazTransferencias.GvarEcodigo#">
		</cfquery>
        ...
        <cfif (trim(Arguments.TipoMovimientoDestino) EQ 'IV' or trim(Arguments.TipoMovimientoDestino) EQ 'IR') and Arguments.CodigoMonedaDestino NEQ #LvarMonedalocal#  and Request.CM_InterfazTransferencias.GvarEcodigo EQ 22>
			<!---Actualiza los montos de la cuenta del concepto a moneda local--->	
    	    <cfquery datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
				update #INTARC#
				set INTMOE  = round(INTMOE * INTCAM, 2),
                	Mcodigo	= (select Mcodigo from Empresas where Ecodigo = #session.Ecodigo#), 
                    INTCAM	= 1
	           		where Ccuenta = #LvarCuentaConcepto#
			</cfquery>            
        <cfelse>        
            <cfquery datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
				update #INTARC#
		...
		</cfif> <!----TERMINA CAMBIO ANGELES--->
 			
        <!---Actualiza los montos locales--->	
	    <cfquery datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			update #INTARC#
			set INTMON = round(INTMOE * INTCAM, 2)
	    </cfquery>
		<!---Transforma las lineas de la retención a moneda local --->
		<cfquery datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			update #INTARC#
			set INTMOE  = round(INTMOE * INTCAM, 2)
			  , INTCAM	= 1
			  , Mcodigo	= (select Mcodigo from Empresas where Ecodigo = #session.Ecodigo#)
			where INTREF = 'Retención'
		</cfquery>
		<!---- Carga de la  CuentaPuente --->
			<cfquery name="rsCuentaPuente" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			...
			<cfelse>
				<cfthrow message="La empresa no tienen definida la Cuenta Balance Multimoneda">
			</cfif>
       <!----ANGELES--->        
        <cfif (Arguments.TipoMovimientoDestino EQ 'IV' or Arguments.TipoMovimientoDestino EQ 'IR') and Arguments.CodigoMonedaDestino NEQ #LvarMonedalocal#  and Request.CM_InterfazTransferencias.GvarEcodigo EQ 22>
		<!---Hace el Balance Multimoneda--->
		<cfquery name="rsBalanceMon" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
				insert into #INTARC# ( INTORI, INTREL, INTDOC, INTREF, INTTIP, INTDES, INTFEC, Periodo, Mes, Ccuenta, Mcodigo, Ocodigo, INTCAM, INTMOE, INTMON, NumeroEvento)
				select 
					'ESBA',
					1,
					<CF_jdbcquery_param cfsqltype="cf_sql_char" value="#lvarDocumento#">,
					'Balance-MultiMoneda',
					'C',
					'Balance por Moneda',
					...
					<CF_jdbcquery_param cfsqltype="cf_sql_integer" value="#Year(Arguments.FechaAplicacion)#">,
					<CF_jdbcquery_param cfsqltype="cf_sql_integer" value="#Month(Arguments.FechaAplicacion)#">, 
					#CuentaPuente#,
					Mcodigo,
					Ocodigo,                                 
					<cfif  isdefined('Arguments.TipoCambio') and len(trim(Arguments.TipoCambio)) and Arguments.TipoCambio gt 0>
					...
					<cfelse>
						coalesce(
								...
								where tc.Ecodigo = #session.Ecodigo# 
									and tc.Mcodigo = c.Mcodigo 
									and tc.Hfecha = (select max(Hfecha) from Htipocambio tc1
									where tc1.Ecodigo = #session.Ecodigo#
										and tc1.Mcodigo = c.Mcodigo
										and tc1.Hfecha <= <cfqueryparam cfsqltype="cf_sql_date" value="#Arguments.FechaValor#">)
								), 1.0)
					</cfif>,                     
        ...
        <cfquery name="rsOrigen" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			select *
			from #INTARC#
		</cfquery>
		...
		<cfinvoke component="sif.Componentes.CG_GeneraAsiento" method="GeneraAsiento" returnvariable="IDcontable">
			<cfinvokeargument name="Ecodigo" value="#Request.CM_InterfazTransferencias.GvarEcodigo#"/>
			<cfinvokeargument name="Conexion" value="#Request.CM_InterfazTransferencias.GvarConexion#"/>
			<cfinvokeargument name="Oorigen" value="#rsOrigen.INTORI#"/>
			...
			<cfinvokeargument name="Edocbase" value="#Arguments.NumeroDocumento#"/>
			<cfinvokeargument name="Ereferencia" value="#Arguments.MontoDestino#"/>
			<cfinvokeargument name="debug" value="0"/>
            <cfinvokeargument name="NumeroEvento"	value="#varNumeroEventoD#"/>
		</cfinvoke>
		...
		<cfif lvarEstimacion eq 1 and "#Request.CM_InterfazTransferencias.GvarEcodigo#" NEQ 1>
            <cf_dbfunction name="OP_concat"	returnvariable="_Cat">
			<!--- Ajustar los registros en INTARC --->
			<cfquery datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
				update #INTARC#
				set 
					INTDES  = 'Reversion Estimación: ' #_Cat# INTDES,
			...
			<cfif (MovFlujoEfectivo.RecordCount GT 0 and MovFlujoEfectivo.Pvalor EQ 1) and Conceptos.FlujoEfec EQ 1 > 
				<cfinvoke component="sif.Componentes.CG_GeneraAsiento" method="GeneraAsiento" returnvariable="IDcontable">
					<cfinvokeargument name="Ecodigo" value="#Request.CM_InterfazTransferencias.GvarEcodigo#"/>
					<cfinvokeargument name="Conexion" value="#Request.CM_InterfazTransferencias.GvarConexion#"/>
					<cfinvokeargument name="Oorigen" value="MBFE"/>
					...
					<cfinvokeargument name="Edescripcion" value="Reversion Estimación: #lvarDescripcionAsiento#"/>
					<cfinvokeargument name="Edocbase" value="#Arguments.NumeroDocumento#"/>
					<cfinvokeargument name="Ereferencia" value="-#Arguments.MontoDestino#"/>
			...
			<cfelseif (MovFlujoEfectivo.RecordCount GT 0 and MovFlujoEfectivo.Pvalor EQ 1) and Conceptos.FlujoEfec EQ 0 >         
				<cfinvoke component="sif.Componentes.CG_GeneraAsiento" method="GeneraAsiento" returnvariable="IDcontable">
					<cfinvokeargument name="Ecodigo" value="#Request.CM_InterfazTransferencias.GvarEcodigo#"/>
					<cfinvokeargument name="Conexion" value="#Request.CM_InterfazTransferencias.GvarConexion#"/>
					<cfinvokeargument name="Oorigen" value="MBNF"/>
					<cfinvokeargument name="Eperiodo" value="#lvarPeriodo#"/>
					<cfinvokeargument name="Emes" value="#lvarMes#"/>
					<cfinvokeargument name="Efecha" value="#LvarFechaValorsigMes#"/>
					<cfinvokeargument name="Edescripcion" value="Reversion Estimación: #lvarDescripcionAsiento#"/>
...
<!---==============Se valida el IABA del Banco se correcto====================================--->
	<cffunction access="private" name="getBid" output="false" returntype="numeric">
		<cfargument name="Iaba" 	   	   required="yes" type="string">
		
		<cfquery name="rsBanco" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			select Bid
			from Bancos
			where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Request.CM_InterfazTransferencias.GvarEcodigo#">
			  and Iaba	  = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.Iaba#">
		</cfquery>
		
		<cfif rsBanco.recordcount EQ 0>
			<cfthrow message="Aplica_Transferencia. El Banco '#Arguments.Iaba#' no existe en la Base de Datos. Proceso Cancelado!">		
		</cfif>
		
		<cfreturn rsBanco.Bid>
	</cffunction>
<!---==============Se valida que la transacción bancaria se correcto-Posibles valores a retornar(N=Ninguno/I=Estimacion de Inversion/C=Estimacion Credito)=====--->
	<cffunction access="private" name="getBTEtipoEst" output="false" returntype="string">
		<cfargument name="Bid" 	   	   required="yes" type="numeric">
		<cfargument name="BTcodigo"   required="no"  type="string">

		<cfif len(trim(Arguments.BTcodigo)) eq 0>
			<cfthrow message="Aplica_Transferencia. El Tipo de Movimiento Destino está vacio. Proceso Cancelado!">
		</cfif>
		
		<cfquery name="rsTransaccionBanco" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			select BTtipoEst 
					from BTransacciones 
				where Ecodigo  = <cfqueryparam cfsqltype="cf_sql_integer" value="#Request.CM_InterfazTransferencias.GvarEcodigo#">
		...
		<cfif rsTransaccionBanco.recordcount EQ 0>
			<cfthrow message="Aplica_Transferencia. La transacción '#Arguments.BTcodigo#' no existe en la Base de Datos, para el banco espeficicado. Proceso Cancelado!">		
		</cfif>
		
		<cfreturn rsTransaccionBanco.BTtipoEst>
	</cffunction>
<!---==============Se valida que el codigo de la retención=====--->
	<cffunction access="private" name="getCcuentaretp" output="false" returntype="string">
		<cfargument name="Rcodigo"     required="no"  type="string" default="">
		
		<cfif NOT LEN(TRIM(Arguments.Rcodigo))>
			<cfthrow message="CM_InterfazTransferencias: Se envió Monto en el importe de la Retención, pero no se envió código de Retención. Proceso Cancelado!">
		</cfif>
		
		<cfquery name="rsRetencion" datasource="#Request.CM_InterfazTransferencias.GvarConexion#">
			select Ccuentaretp 
					from Retenciones 
				where Ecodigo  = <cfqueryparam cfsqltype="cf_sql_integer" value="#Request.CM_InterfazTransferencias.GvarEcodigo#">
				  and rtrim(Rcodigo)  = <cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(Arguments.Rcodigo)#">
		</cfquery>
		
		<cfif rsRetencion.recordcount EQ 0>
			<cfthrow message="Aplica_Transferencia. La retencion '#Arguments.Rcodigo#' no existe en la Base de Datos. Proceso Cancelado!">		
		<cfelseif NOT LEN(TRIM(rsRetencion.Ccuentaretp))>
			<cfthrow message="Aplica_Transferencia. La retencion '#Arguments.Rcodigo#' no tiene configurada la cuenta de Pago. Proceso Cancelado!">		
		</cfif>
		
		<cfreturn rsRetencion.Ccuentaretp>
	</cffunction>
</cfcomponent>
