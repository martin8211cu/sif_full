<!--- 
	Interfaz 40: Interfaz de Cancelacion de Saldos Menores
	Dirección de la Inforamción: Sistema Externo - SIF
	Elaborado por: Alejandro Bolaños / Maria de los Angeles Blanco López 
	Creacion: 04/03/2010
--->
	...
	<cffunction name="process" access="public" returntype="string" output="no">
		...
		<cfoutput query="query" group="ID">
            ...
            <cfif (query.ConceptoTesoreria EQ "705-I" or query.ConceptoTesoreria EQ "705-G") and query.Transaccion EQ "X3">
				...
				<cfquery name="rsCConcepto" datasource="#GvarConexion#">
					...
					where Ccodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(query.ConceptoTesoreria)#">
					and Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#GvarEcodigo#">
				</cfquery>
				
				<cfif rsCConcepto.Cformato EQ "">					
					<cfthrow message="El concepto #trim(query.ConceptoTesoreria)# no tiene su cuenta parametrizada">
				</cfif>
				
				<cfif isdefined("rsCuentaCom") and rsCuentaCom.recordcount EQ 1 and len(rsCuentaCom.CBcc) NEQ 0>
					<cfset varCuentaCom = Replace("#rsCConcepto.Cformato#", "???", "#rsCuentaCom.CBcc#", "ALL")>
				 <cfelse>
		...
		<cfoutput query="query" group="ID">
            ...
            <cf_dbtemp name="TmpID10" returnvariable="TmpID10" datasource="sifinterfaces">
                ...
                <cf_dbtempcol name="CuentaFinancieraDet" type="char(100)">
	...
	<cffunction access="private" name="getValidCcuenta" output="no" returntype="numeric">
		...
		<cfif len(trim(Arguments.CFormato)) gt 0>
			...
			<cfif LvarError NEQ "OLD" AND LvarError NEQ "NEW">
				<cfthrow message="Error en Interfaz 40. Cuenta #Arguments.CFormato#: #LvarERROR#. Proceso Cancelado!">
			</cfif>
			...
			<cfelse>
				<cfthrow message="Error en Interfaz 40. Cuenta #Arguments.CFormato#: Cuenta Inválida! para la empresa #Enombre#. Proceso Cancelado!">
			</cfif>
		</cfif>
