...
<!---        Inicia cambio para procesar pago de anticipos CxP  --->
				<cfif query.Anticipo NEQ 0>
					...
					<cfif isdefined('rsSaldoDocumento') and rsSaldoDocumento.recordcount GT 0>
                        ...
                        <cfif #LvarSaldoActual# lt abs(#query.MontoPagoDocumentoL#)>
                            <cfthrow message="Error. El saldo del anticipo #trim(query.DocumentoL)# #LvarSaldoActual# es menor al saldo del documento #abs(query.MontoPagoDocumentoL)#" detail="El saldo del documento debe ser menor o igual al saldo del Anticipo">
                            <cfabort>
                        </cfif>
                    </cfif>
                </cfif>
<!---        Termina cambio para procesar pago de anticipos CxP  --->
			...
			<cfif isdefined("LTipoCP") and LTipoCP EQ 'C'>
				<cfquery name="insertE" datasource="#Request.CP_InterfazPagos.GvarConexion#">
					...
					, 'Documento Generado por la Interfaz de Pago de SOIN, Doc. Num: ' #_Cat# <cfqueryparam cfsqltype="cf_sql_char"    value="#trim(query.NumeroDocumento)#">  
