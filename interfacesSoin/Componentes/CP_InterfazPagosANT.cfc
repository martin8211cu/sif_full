<cfcomponent>
	...
	<cffunction name="Procesa_Pago" access="public" returntype="boolean">
		...
		<cfoutput query="query" group="ID">
			...
			<cfset LvarMontoAnticipoDiferencias = 0.00> 
			<cfoutput>
				<!--- Inserta detalles --->
				<cfif query.Anticipo EQ 0>
					...
					<cfif query.MontoPagoDocumentoL GT LmontoPago>
						...
						<cfset LvarMontoAnticipoDiferencias = LvarMontoAnticipoDiferencias + (query.MontoPagoL - LmontoPagoRec)>
					...
					<cfif isdefined("LTipoCP") and LTipoCP EQ 'C'>
						...
						<cfquery name="insertD"  datasource="#Request.CP_InterfazPagos.GvarConexion#">
								...
								, <cfqueryparam cfsqltype="cf_sql_float"   value="#LmontoPago/LmontoPagoRec#"><!--- Tipo de Cambio entre el documento de pago y el documento --->
								, <cfqueryparam cfsqltype="cf_sql_money"   value="#NumberFormat(LmontoPagoRec,"9.00")#">
								, 0
								, 1
						...
						<cfquery datasource="#Request.CP_InterfazPagos.GvarConexion#">
							update Pagos 
							 ...
							 where Ecodigo			= <cfqueryparam cfsqltype="cf_sql_integer" value="#Request.CP_InterfazPagos.GvarEcodigo#">
							   and CCTcodigo		= 'RE'
							   and Pcodigo			= <cfqueryparam cfsqltype="cf_sql_char"    value="#trim(query.NumeroDocumento)#">
						</cfquery> 
					<cfelseif isdefined("LTipoCP") and LTipoCP EQ 'P'>
						<cfquery datasource="#Request.CP_InterfazPagos.GvarConexion#">
							update EPagosCxP 
			...
			<cfif LvarMontoAnticipoDiferencias NEQ 0.00>
				<cfif LvarYaHayAnticipos>
					<cfif isdefined("LTipoCP") and LTipoCP EQ 'C'>
						<cfquery datasource="#Request.CP_InterfazPagos.GvarConexion#">
							update Pagos
							set NC_total 		= NC_total + #NumberFormat(LvarMontoAnticipoDiferencias,"9.00")#>
							where Ecodigo		= <cfqueryparam cfsqltype="cf_sql_integer" value="#Request.CP_InterfazPagos.GvarEcodigo#">
							and CCTcodigo		= 'RE'
							and Pcodigo			= <cfqueryparam cfsqltype="cf_sql_char"    value="#trim(query.NumeroDocumento)#">
						</cfquery>
							...
							update Pagos 
							   set NC_CCTcodigo 	= 'AN'
								...
								<cfif isdefined("rsSNpago.id_direccion") and len(trim(rsSNpago.id_direccion))>
							     ...
							     , NC_total 		= <cfqueryparam cfsqltype="cf_sql_money" value="#NumberFormat(LvarMontoAnticipoDiferencias,"9.00")#">
							 where Ecodigo			= <cfqueryparam cfsqltype="cf_sql_integer" value="#Request.CP_InterfazPagos.GvarEcodigo#">
							   and CCTcodigo		= 'RE'
							   and Pcodigo			= <cfqueryparam cfsqltype="cf_sql_char"    value="#trim(query.NumeroDocumento)#">
						</cfquery> 
	...
	<cffunction access="private" name="getDoc" output="false" returntype="query">
		...
		<cfif isdefined("Arguments.TipoMovCP") and Arguments.TipoMovCP EQ 'C'>
			<cfquery name="rsDocumentos" datasource="#Request.CP_InterfazPagos.GvarConexion#">
				select Ddocumento as Doc, Ccuenta as Cuenta, CCTcodigo as CodigoTransaccion
				  from Documentos
		...
		<cfelseif isdefined("Arguments.TipoMovCP") and Arguments.TipoMovCP EQ 'P'>			
			<cfquery name="rsDocumentos" datasource="#Request.CP_InterfazPagos.GvarConexion#">
				select IDdocumento as Doc, Ccuenta as Cuenta, CPTcodigo as CodigoTransaccion
				  from EDocumentosCP
				 where Ecodigo 		= <cfqueryparam cfsqltype="cf_sql_integer" 	value="#Request.CP_InterfazPagos.GvarEcodigo#">
