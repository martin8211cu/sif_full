...
<cfcomponent>
	...
	<cffunction name="process" access="public" returntype="string" output="no">
	    ...
	    <cfelse>
			...
			<!---<cfset Valid_Ccuenta 			= getValidCcuenta(Valid_Modulo,Valid_SNcodigo.SNcodigo,Valid_Ccodigo,query.CuentaFinanciera,Valid_SNcodigo.id_direccion_fact)> ABG--->
			...
			<cftransaction>
				<cfif Valid_Modulo EQ "CC">
					...
					<cfset Valid_Ccuenta 		= getValidCcuenta(Valid_Modulo,Valid_SNcodigo.SNcodigo,Valid_Ccodigo,query.CuentaFinanciera,Valid_SNcodigo.id_direccion_fact, Valid_CCTcodigo.CCTcodigo)> <!---ABG--->
				...
				<cfelse>
					...
					<cfset Valid_Ccuenta 		= getValidCcuenta(Valid_Modulo,Valid_SNcodigo.SNcodigo,Valid_Ccodigo,query.CuentaFinanciera,Valid_SNcodigo.id_direccion_fact, Valid_CPTcodigo.CPTcodigo)> <!---ABG--->
					<cfif Not Valid_Estimacion>
	...
	<cffunction access="private" name="getValidCcuenta" output="no" returntype="numeric">
		...
		<cfelse>
			...
			<cfif Arguments.Modulo EQ "CC">
                ...
                <cfelseif isdefined("rsCuentaEx") and rsCuentaEx.recordcount EQ 1 and rsCuentaEx.Ccuenta NEQ 0> <!---ABG--->
                	<cfset LvarCcuenta = rsCuentaEx.Ccuenta> <!---ABG--->
	...
	<cffunction access="private" name="getValidIcodigo" output="no" returntype="string">
		...
		<cfif len(trim(Arguments.Icodigo))>
			...
			<cfelseif Arguments.ImporteImpuesto NEQ -1 AND abs(query.Impuesto - Arguments.ImporteImpuesto) GT 1>
				<cfthrow message="Error en Interfaz 10. PorcentajeImpuesto es inválido. El Monto de Impuesto no corresponde con el porcentaje de Impuesto '#query.Icodigo#' de la Empresa #GvarEnombre#. Proceso Cancelado!.">
			</cfif>
		<cfelse>
