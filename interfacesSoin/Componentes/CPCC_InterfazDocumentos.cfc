<!---
	...
	Se Agrega el campo de TimbreFiscal  para Contabilidad Electrónica  IRR Oct 2014
--->
<cfcomponent>
	<!--- Variables Globales --->
    ...
    <cfif isdefined("session.repoDsn") and session.repoDsn NEQ "">
    	<cfset GvarRepoCFDis = Session.repoDsn>
    </cfif>
	...
	<cffunction name="process" access="public" returntype="string" output="no">
		...
		<cfoutput query="query" group="ID">
			...
			<cfset Valid_MesDocvsMesAux     = getValid_MesDocvsMesAux(query.mesDoc) >
	    ...
	    <cfelse>
			...
			<cfset Valid_Ccodigo 			= getValidCcodigo(query.CodigoConceptoServicio)>
					<!--- <cf_dumptofile var=#Valid_SNcodigo#> --->
			...
			<cfquery name="rsMonLocal" datasource="#GvarConexion#">
				SELECT  ltrim(rtrim(m.Miso4217)) Miso4217
				FROM Empresas e
				inner join Monedas m on e.Ecodigo = m.Ecodigo and e.Mcodigo = m.Mcodigo
				where e.Ecodigo = #GvarEcodigo#
			</cfquery>
            <cfif isdefined('query.TimbreFiscal') and query.TimbreFiscal NEQ "">
            <!--- Valida si  el  Timbre Fiscal UUID  existe en Repositorio --->
            	<cfset getValidTimbre = getValidTimbre(query.TimbreFiscal)>
            <cfelseif isdefined('query.CodigoMoneda') and query.CodigoMoneda NEQ "#rsMonLocal.Miso4217#">
            <!--- Si no tiene Timbre Fiscal y la Moneda no es Peso, se generara un comprobante de tipo Extrangero --->
            <!--- Se usa el nombre del Documento coo timbre Fiscal --->
            <!--- Valida si  el  Timbre Fiscal UUID  existe en Repositorio --->
				<cfset getValidTimbre = getValidTimbre(query.Documento)>
            	<cfset tipoComprobante = 2>
            <cfelse>
                <cfset getValidTimbre = -1 >
            </cfif>
			<!---Inicia Transacción --->
			<cftransaction>
					...
					<cfif Not Valid_Estimacion>
						...
						<cfquery name="rsInsert" datasource="#GvarConexion#">
							...
							values (
								...
								<cfqueryparam cfsqltype="cf_sql_varchar" value="#query.TimbreFiscal#">,
								<cfqueryparam cfsqltype="cf_sql_money" value="0.00">
								)
					 ...
					 <!--- Inserta en  el  Repositorio Temporal de CFDIs si el Timbre Fiscal No  existe en  los Respositorios--->
						<cfif getValidTimbre EQ 0>
                            <cfquery name="insRepTmp" datasource="#GvarConexion#">
                                insert into CERepoTMP (Ecodigo,TimbreFiscal,ID_Documento,Documento,Origen,TipoComprobante,Mcodigo,TipoCambio,BMUsucodigo)
                                values(
                                <cfqueryparam cfsqltype="cf_sql_integer" value="#GvarEcodigo#">,
                                <cfif tipoComprobante EQ 1>
                                <cfqueryparam cfsqltype="cf_sql_varchar" value="#query.TimbreFiscal#">,
                                <cfelse>
                                	<cfqueryparam cfsqltype="cf_sql_varchar" value="#query.Documento#">,
                                </cfif>
                                <cfqueryparam cfsqltype="cf_sql_integer" value="#rsInsert.identity#">,
                                <cfqueryparam cfsqltype="cf_sql_char" value="#Valid_EDdocumento#">,
                                'CCFC',
                                #tipoComprobante#,
								<cfqueryparam cfsqltype="cf_sql_numeric" value="#Valid_Mcodigo#">,
					...
					<cfif Not Valid_Estimacion>
						...
						<cfif getValidTimbre EQ 0>
                            <cfquery name="insRepTmp" datasource="#GvarConexion#">
                                ...
                                <cfqueryparam cfsqltype="cf_sql_integer" value="#rsInsert.identity#">,
                                <cfqueryparam cfsqltype="cf_sql_varchar" value="#Valid_EDdocumento#">,
                                'CPFC',
                                #tipoComprobante#,
								<cfqueryparam cfsqltype="cf_sql_numeric" value="#Valid_Mcodigo#">,
								<cfqueryparam cfsqltype="cf_sql_float" value="#Valid_TipoCambioCXP#">,
				...
				<cfoutput>
					...
					<!--- futura validación IEPS --->
					<cfif query.MontoIEPS NEQ 0> <!--- para evitar nulos --->
						<cfset Valid_ImporteIEPS = query.MontoIEPS>						
					<cfelse>
						<cfset Valid_ImporteIEPS = 0>
					</cfif>
					...
					<cfelseif (Valid_CantidadTotal * Valid_PrecioUnitario) GT 0.00>
						<cfset Valid_ImporteTotal = (Valid_CantidadTotal * Valid_PrecioUnitario) - Valid_ImporteDescuento - Valid_ImporteIEPS>
					</cfif>
					...
					<cfif LvarCalcularImpuesto>
						<cfif query.CodigoImpuesto EQ "">
							<cfset Valid_Icodigo = getValidIcodigo(query.CodigoImpuesto,0,Valid_ImporteTotal,Valid_ImporteIEPS)>
						<cfelse>
							<cfset Valid_Icodigo = getValidIcodigo(query.CodigoImpuesto,-1,Valid_ImporteTotal,Valid_ImporteIEPS)>
						</cfif>
						<cfif len(trim(Valid_Icodigo))>
							...
							<cfif query.afectaIVA EQ 0>
								<cfset Valid_ImporteImpuesto =  ((Valid_ImporteTotal + Valid_ImporteIEPS)*queryImpuesto.Iporcentaje)/100 >
							<cfelse>
					...
					<cfelse>
						...
						<cfset Valid_Icodigo = getValidIcodigo(query.CodigoImpuesto,Valid_ImporteImpuesto,Valid_ImporteTotal,Valid_ImporteIEPS)>
					</cfif>
						...
						<cfif Not Valid_Estimacion>
							...
							<cfquery datasource="#GvarConexion#">
								...
								values (
									...
									<cfqueryparam cfsqltype="cf_sql_numeric" value="#Valid_DCcuenta.CFcuenta#">,
									<cfqueryparam cfsqltype="cf_sql_numeric" value="#GvarUsucodigo#">,
									<cfqueryparam cfsqltype="cf_sql_varchar" value="#query.codIEPS#">,
									<cfqueryparam cfsqltype="cf_sql_money" value="#Valid_ImporteIEPS#">,
									<cfqueryparam cfsqltype="cf_sql_numeric" value="#query.afectaIVA#">
								)
							</cfquery>
							<cfquery datasource="#GvarConexion#">
								...
								set EDdescuento = 0.00,
									...
									EDieps 		= isnull(EDieps,0) + <cfqueryparam cfsqltype="cf_sql_money" value="#Valid_ImporteIEPS#">,
									EDtotal 	= EDtotal + <cfqueryparam cfsqltype="cf_sql_money" value="#Valid_ImporteTotal+Valid_ImporteImpuesto + Valid_ImporteIEPS#">
						...
						<cfelse>
							...
							<cfquery datasource="#GvarConexion#">
								...
								SELECT	<cfqueryparam cfsqltype="cf_sql_char" value="CPFC">,
										...
										<cfqueryparam cfsqltype="cf_sql_char" value="#Valid_EDdocumento#">,
										<cfqueryparam cfsqltype="cf_sql_char" value="#Valid_CCTcodigo.CCTcodigo#">,
										round(<cfqueryparam cfsqltype="cf_sql_float" value="#(Valid_ImporteTotal+Valid_ImporteImpuesto+Valid_ImporteIEPS)*Valid_TipoCambioCXP#">,2),
										INTTIP,
										INTDES,
										<cfqueryparam cfsqltype="cf_sql_date" value="#Valid_FechaDocumento#">,
										...
										<cfqueryparam cfsqltype="cf_sql_numeric" value="#Valid_Mcodigo#">,
										<cfqueryparam cfsqltype="cf_sql_integer" value="#Valid_Ocodigo#">,
										round(<cfqueryparam cfsqltype="cf_sql_money" value="#Valid_ImporteTotal+Valid_ImporteImpuesto+Valid_ImporteIEPS#">,2)
								FROM #INTARCDIFF#
							</cfquery>
						</cfif><!--- <cfif Not Valid_Estimacion> --->
					<cfelse>
						...
						<cfif Not Valid_Estimacion>
							...
							<cfquery datasource="#GvarConexion#">
								...
								set EDdescuento = 0.00,
									...
									EDimpuesto = EDimpuesto + <cfqueryparam cfsqltype="cf_sql_money" value="#Valid_ImporteImpuesto#">,
									EDTiEPS	= EDTiEPS + <cfqueryparam cfsqltype="cf_sql_money" value="#Valid_ImporteIEPS#">,
									EDtotal = EDtotal + <cfqueryparam cfsqltype="cf_sql_money" value="#Valid_ImporteTotal+Valid_ImporteImpuesto+Valid_ImporteIEPS#">
	...
	<cffunction access="private" name="getValidTESRPTCid" output="no" returntype="numeric">
		...
		<cfif len(trim(Arguments.TESRPTCcodigo)) GT 0>
			<cfquery name="rsSQL" datasource="#GvarConexion#" >
			 ...
			 <cfif rsSQL.TESRPTCid NEQ "">
			  ...
			  <cfelse>
			  		<cfthrow message="Error en Interfaz 10. No existen el Concepto de Pagos a Terceros '#Arguments.TESRPTCcodigo#' para el modulo '#Arguments.Modulo#'.">
			  </cfif>
		</cfif>
		...
		<cfif IETU.cantidad EQ 0>
			<cfthrow message="Error en Interfaz 10. El Socio #RPTCSocio.SNidentificacion#-#RPTCSocio.SNnombre#  no tiene definido el Conceptos de Pagos a Terceros.">
		</cfif> --->
		...
		<cfelse>
			<cfthrow message="Error en Interfaz 10. No existen Conceptos de Pagos a Terceros en Tesorería.">
		</cfif>
	</cffunction>
		...
		<cfif len(trim(Arguments.CodigoDireccionEnvio))>
			...
			<cfquery name="query1" datasource="#GvarConexion#">
				SELECT s.SNid,
					   s.SNcodigo,
					   s.id_direccion
				FROM SNDirecciones d
				INNER JOIN SNegocios s ON d.SNid = d.SNid
				AND d.SNcodigo = s.SNcodigo
				AND d.Ecodigo = s.Ecodigo
				AND s.SNinactivo = 0
				WHERE s.SNcodigoext = <cfqueryparam cfsqltype="cf_sql_char" value="#Arguments.CodigoDireccionEnvio#">
				  AND d.Ecodigo     = <cfqueryparam cfsqltype="cf_sql_numeric" value="#GvarEcodigo#">
				  AND d.SNDenvio    = 1
			</cfquery>
			<cfif query1.recordcount eq 0>
				<cfquery name="query1" datasource="#GvarConexion#">
					...
					AND d.Ecodigo = s.Ecodigo
					AND s.SNinactivo = 0
					WHERE d.SNDcodigo = <cfqueryparam cfsqltype="cf_sql_char" value="#Arguments.CodigoDireccionEnvio#">
					  AND d.Ecodigo     = <cfqueryparam cfsqltype="cf_sql_numeric" value="#GvarEcodigo#">
					  AND d.SNDenvio    = 1
				</cfquery>
			</cfif>
	...
	<!---
		Metodo:
			getValid_MesDocvsMesAux
		Resultado:
			Devuelve Verdadero si el Mes Contable de Auxiliares es igual al Mes del la Fecha del  documento 
	--->
	<cffunction access="private" name="getValid_MesDocvsMesAux" output="no">
		<cfargument name="mesDoc" required="yes" type="string">
		<cfset Valid_Mes = getValidMesAuxiliares()>
		<cfif mesDoc EQ Valid_Mes >
			<cfset ejecutaInterfaz = 1>
		<cfelse>
			<cfset ejecutaInterfaz = 0>
			<cfthrow message="Error en Interfaz 10. Fecha de Documento no Corresponde al Periodo de Auxiliares. Proceso Cancelado!.">
		</cfif>		
		<cfreturn ejecutaInterfaz>
	</cffunction>
	<!---
		Metodo:
...
<!---             	<cfif isdefined("request.PMI")> --->
			...
			<cfelse>
				...
				<cfelse>
					<cfthrow message="Error en Interfaz 10. El Socio de Negocios no tiene definida  #Arguments.SNcodigo# #rsCuentaEx.Ccuenta# correctamente la cuenta de Cuentas por Pagar. Proceso Cancelado!.">
				</cfif>
			</cfif>
	...
	<cffunction access="private" name="getValid_DCcuenta" output="no" returntype="query">
		...
		<cfif len(trim(rsCuenta.Ccuenta))>
			<cfset LvarCcuenta = rsCuenta.Ccuenta>
		<cfelse>
			<cfthrow message="Error en Interfaz 10. Cuenta #Arguments.CFormato#: Cuenta Contable Inválida! para la empresa #Enombre#. Proceso Cancelado!">
		</cfif>

		<cfif len(trim(rsCuenta.CFcuenta))>
			<cfset LvarCFcuenta = rsCuenta.CFcuenta>
		<cfelse>
			<cfthrow message="Error en Interfaz 10. Cuenta #Arguments.CFormato#: Cuenta Financiera Inválida! para la empresa #Enombre#. Proceso Cancelado!">
		</cfif>
		<cfreturn rsCuenta>
	</cffunction>
	<!---
	...
	<cffunction access="private" name="getValidIcodigo" output="no" returntype="string">
		...
		<cfargument name="ImporteIEPS" 		required="no"  type="numeric" default="0">
		<cfargument name="NoAfectaIVA" 		required="no"  type="numeric" default="0">

		<cfif len(trim(Arguments.Icodigo))>
			<cfquery name="query" datasource="#GvarConexion#">
				select Icodigo,
				<cfif Arguments.NoAfectaIVA EQ 0>
					round(Iporcentaje*(#Arguments.ImporteTotal#+#Arguments.ImporteIEPS#)/100,2) as Impuesto
				<cfelse>
					round(Iporcentaje*#Arguments.ImporteTotal#/100,2) as Impuesto
				</cfif>
				from Impuestos
				where Icodigo = <cfqueryparam cfsqltype="cf_sql_char" value="#Arguments.Icodigo#">
				  and Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#GvarEcodigo#">
			</cfquery>
						<!--- <cf_dumptofile var=#arguments#> --->
			<cfif query.recordcount eq 0>
	...
	<!---
		Metodo:
			getValidTimbre
		Resultado:
			Devuelve si Existe el  Timbre en  el Repositorio Temporal,  o  en  el Repositorio de CFDIs.
	--->
	<cffunction access="private" name="getValidTimbre" output="no" returntype="numeric">
    	<cfargument name="Timbre" 	   required="yes" type="string">
        <cfset existe = 0>
        <cfquery name="validaCE" datasource="#GvarConexion#">
            select ERepositorio from Empresa where Ereferencia=#GvarEcodigo#
        </cfquery>
        <cfif isdefined('validaCE') and validaCE.ERepositorio EQ 1>
            <cfquery name="existeCFDI" datasource="#GvarConexion#">
                select count(TimbreFiscal) as existe, TimbreFiscal
                from CERepoTMP
                where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#GvarEcodigo#">
                    and TimbreFiscal = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.Timbre#">
                group by TimbreFiscal
            </cfquery>
            <cfquery name="existeCFDIrep" datasource="#GvarConexion#">
               select count(timbre) existe, timbre
               from CERepositorio
               where Ecodigo= <cfqueryparam cfsqltype="cf_sql_integer" value="#GvarEcodigo#">
               and timbre = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.Timbre#">
               group by timbre
            </cfquery>
            <cfif (existeCFDIrep.existe GT 0 and existeCFDIrep.timbre NEQ "") OR (existeCFDI.existe GT 0 and existeCFDI.TimbreFiscal NEQ "") >
                <cfset existe = 1>
            </cfif>
		<cfelse>
        	<cfset existe = 1>
