<!--- 
	Interfaz 22: Interfaz de Intercambio de Información de Documentos Prefactura
...
<cfcomponent>
	...
	<!--- Process: Procesamiento de la información de la Interfaz 22 debe ser llamada de la siguiente manera:
		<cfinvoke component="interfacesSoin.Componentes.PREFACT_InterfazDocumentos" method="process" returnvariable="MSG" query="#readInterfaz22#"/>
		Argumentos:
			MSG = Mensaje de Resultados de la operación.
			Query = Consulta de la Interfaz 22 (Encabezado y Detalles).
	 --->
	<cffunction name="process" access="public" returntype="string" output="yes">
		<!--- Argumentos --->
		<cfargument name="query" required="yes" type="query">
		<!--- Procesamiento de Encabezado de la Interfaz 22  --->
...
<!---------------------------- Detalle ---------------------------------------------------------------------------------------------------------------------->                
				<!--- Procesamiento de Detalles de la Interfaz 22 --->
                      ...
                      update FAPrefacturaE
                      		set Descuento = <cfqueryparam cfsqltype="cf_sql_money" value="#Descu#">,
                                ...
                                Total = <cfqueryparam cfsqltype="cf_sql_money" value="#Total#"> + <cfqueryparam cfsqltype="cf_sql_money" value="#Impue#"> - 
                                		<cfqueryparam cfsqltype="cf_sql_money" value="#Descu#">
...
<!------------------------------------------------------------------------------------------------------------------------------------------------------------>                
	...
	<cffunction access="private" name="getValidSNcodigo" output="no" returntype="query">
		...
		<cfif Lvar_SNid lte 0 or len(trim(Lvar_SNcodigo)) lte 0>
			<cfthrow message="Error en Interfaz 22. NumeroSocio es inválido, La Dirección de Envío, Dirección de Facturación y El Numero de Socio no corresponden con ningún Socio de la Empresa #GvarEnombre#. Proceso Cancelado!.">
		</cfif>
	...
	<cffunction access="private" name="getValidCCEDdocumento" output="no" returntype="string">
		...
		<cfif query.recordcount GT 0>
			<cfthrow message="Error en Interfaz 22. Documento es inválido, El Documento ya existe en la Empresa #GvarEnombre# en el Módulo de Prefacturacion. Proceso
            					 Cancelado!.">
		</cfif>
	...
	<cffunction access="private" name="getValidCCTcodigo" output="no" returntype="query">
		...
		<cfif query.recordcount EQ 0>
			<cfthrow message="Error en Interfaz 22. CodigoTransaccion es inválido, El Código de Transacción no corresponde con ninguna Transacción de la Empresa #GvarEnombre#
	...
	<cffunction access="private" name="getValidMcodigo" output="no" returntype="numeric">
		...
		<cfif query.recordcount EQ 0 >
			<cfthrow message="Error en Interfaz 22. CodigoMoneda es inválido, El Código de la Moneda no corresponde con ninguna moneda en la Empresa #GvarEnombre#. Proceso
            		 		Cancelado!.">
		</cfif>
	...
	<cffunction access="private" name="getValidOCodigo" output="no" returntype="numeric">
		...
		<cfif query.recordcount EQ 0 or len(trim(query.Ocodigo)) eq 0>
			<cfthrow message="Error en Interfaz 22. CodigoOficina es inválido, El Código de Oficina #Arguments.Oficodigo# no corresponde con ninguna Oficina de la Empresa #GvarEnombre#. Proceso Cancelado!.">
		</cfif>
	...
	<cffunction access="private" name="getValidFechaDocumento" output="no" returntype="date">
		...
		<cfif Arguments.Fecha lt GvarMinFecha or Arguments.Fecha gt DateAdd('yyyy',99,GvarMinFecha)>
			<cfthrow message="Error en Interfaz 22. FechaDocumeno es inválido, La Fecha del Documento no es válida en la Empresa #GvarEnombre#. Proceso Cancelado!.">
		</cfif>
	...
	<cffunction access="private" name="getValidFechaVencimiento" output="no" returntype="date">
		...
		<cfelseif Arguments.FechaVencimiento NEQ "">
			...
			<cfcatch type="any">
				<cfthrow message="Error en Interfaz 22. FechaVencimiento es inválida '#Arguments.FechaVencimiento#'. Proceso Cancelado!.">
			</cfcatch>
			</cftry>
		</cfif>
		<cfif LvarFechaVencimiento lt Arguments.FechaDocumento>
			<cfthrow message="Error en Interfaz 22. FechaVencimiento es inválido, La Fecha del Vencimiento debe ser mayor o igual que la Fecha de Documento. Proceso Cancelado!.">
		</cfif>
	...
	<cffunction name="getValidTipoItem" access="private" returntype="string" output="no">
		...
		<cfif Arguments.TipoItem neq "A" and Arguments.TipoItem neq "S" and Arguments.TipoItem neq "O">
			<cfthrow message="Error en Interfaz 22. TipoItem es Inválido, El Tipo de Ítem debe ser A=Articulo, S=Servicio,  O= Transito con orden comercial. Proceso Cancelado!.">
		</cfif>
	...
	<cffunction access="private" name="getValidAid" output="no" returntype="query">
		...
		<cfif query.recordcount EQ 0>
			<cfthrow message="Error en Interfaz 22. CodigoItem es inválido, El Código de Item no corresponde con ningún Artículo válido en la Empresa #GvarEnombre#. Proceso Cancelado!">
		</cfif>
		<cfreturn query>
	</cffunction>
	...
	<cffunction access="private" name="getValidAlmAid" output="no" returntype="numeric">
		...
		<cfif query.recordcount eq 0>
			<cfthrow message="Error en Interfaz 22. CodigoAlmacen (#Arguments.CodigoAlmacen#) es inválido, El Código de Almacén no corresponde con ningún Almacén de la Empresa #GvarEnombre#. Proceso Cancelado!.">
		</cfif>
		<cfreturn query.Aid>
	</cffunction>    
	...
	<cffunction access="private" name="getValidCid" output="no" returntype="query">
		...
		<cfif query.recordcount EQ 0>
			...
			<cfif query.recordcount EQ 0>
				<cfthrow message="Error en Interfaz 22. CodigoItem #Arguments.Ccodigo# es inválido, El Código de Item no corresponde con ningún Concepto de Servicio válido en la Empresa #GvarEnombre#. Proceso Cancelado!.">
			</cfif>
		</cfif>
		<cfreturn query>
	</cffunction>
	...
	<cffunction access="private" name="getValidCFid" output="no" returntype="numeric">
		...
		<cfelse>
			...
			<cfif query.recordcount EQ 0 or len(trim(query.CFid)) eq 0>
				<cfthrow message="Error en Interfaz 22. No Existe ningún Centro Funcional definido en la Empresa #GvarEnombre#. Proceso Cancelado!.">
				<cfabort>
			</cfif>
			<cfreturn query.CFid>
		</cfif>
	...
	<cffunction access="private" name="getValidIcodigo" output="no" returntype="string">
		...
		<cfif len(trim(Arguments.Icodigo))>
			...
			<cfif query.recordcount eq 0>
				<cfthrow message="Error en Interfaz 22. CodigoImpuesto (#Arguments.Icodigo#) es inválido. El Código de Impuesto no corresponde con ningún Impuesto de la Empresa #GvarEnombre#. Proceso Cancelado!.">
			<cfelseif Arguments.ImporteImpuesto NEQ -1 AND abs(query.Impuesto - Arguments.ImporteImpuesto) GT 1>
				<cfthrow message="Error en Interfaz 22. PorcentajeImpuesto es inválido. El Monto de Impuesto (#NumberFormat(Arguments.ImporteImpuesto,'0.00')#) no corresponde (#NumberFormat(query.Impuesto,'0.00')#) según el porcentaje de Impuesto '#query.Icodigo#' aplicado al monto total de (#NumberFormat(Arguments.ImporteTotal,'0.00')#) de la Empresa #GvarEnombre#. Proceso Cancelado!.">
			</cfif>
		<cfelse>
			...
			<cfif query.recordcount EQ 0>
				<cfif ImporteImpuesto EQ 0>
					<cfthrow message="Error en Interfaz 22. ImporteImpuesto no válido. No existe Codigo de Impuesto Exento en la Empresa #GvarEnombre#. Proceso Cancelado!.">
                <cfelse>
					<cfthrow message="Error en Interfaz 22. ImporteImpuesto no válido. No existe Codigo de Impuesto con #numberFormat(PorcentajeImpuesto,",9.99")#% en la Empresa #GvarEnombre# (Impuesto=#numberFormat(ImporteImpuesto,",9.99")# / TotalLinea=#numberFormat(ImporteTotal,",9.99")#). Proceso Cancelado!.">
                </cfif>
			</cfif>
		</cfif>
...
<!---
	...
	<cffunction access="private" name="getValid_DCcuenta" output="no" returntype="query">
		...
		<cfif LvarError NEQ "OLD" AND LvarError NEQ "NEW">
			<cfthrow message="Error en Interfaz 22. Cuenta #Arguments.CFormato#: #LvarERROR#. Proceso Cancelado!">
		</cfif>
		...
		<cfelse>
			<cfthrow message="Error en Interfaz 22. Cuenta #Arguments.CFormato#: Cuenta Inválida! para la empresa #Enombre#. Proceso Cancelado!">
		</cfif>
		<cfreturn rsCuenta>
	</cffunction> 
	...
	<cffunction access="private" name="obtieneCuentaConceptoServicio_Gasto_Ingreso" output="no" returntype="query">
		...
		<cfif len(trim(query.CuentaFinancieraDet)) eq 0 >
