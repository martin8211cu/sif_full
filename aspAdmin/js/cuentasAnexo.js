...
function cuentas_snumber(obj,e,d){
...
//alert('KEY ==> ' + t);
	...
	// valida que pueda digitar numeros Y los caracteres: _,x,X,?,- 
	
	if(t==9 || t==8 || t==13 || t==20 || t==27 || t==45 || t==46 || t==189 || t==88 || t==191 || t == 109)  return true;
...
function cuentas_fm(campo) {
	var ndec = 2    // siempre es cero
	var s = "";
...
function formato(obj, comodin){
	obj.value = obj.value.replace(/[Xx?_]/g, comodin);
}
...
function cuentas_snumber(obj,e,d)
{
	...
	if(!ok){    
		//str=cuentas_fm(str,d)
		var str2 = str.toUpperCase();
		alert(str2.charCodeAt(2));
		alert("key ==> " + t);		
		obj.value=str
	}
	return true
...
//Formatea como float un valor de un campo
//Recibe como parametro el campo y la cantidad de decimales a mostrar
function cuentas_fm(campo,ndec) {

alert(campo.charCodeAt(2));
