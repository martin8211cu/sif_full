...
<cfif Session.secfw.soy_pso>
	...
	<cfset campos = "' Roles Personales' as nombre_cuenta_empresarial, null as nombre_empresa, r.rol, r.nombre as nombre_rol, up.id,
		u.Pnombre, u.Papellido1, u.Papellido2, u.Papellido1 + ' ' + u.Papellido2 as Papellidos, u.Pemail1,
		porque = 'soy pso'">
	...
	<cfif not isdefined("form.reimp")>
		<cfset filtro = filtro & " and up.num_impreso = 0">
	</cfif>
  
	<cfset extra = extra & " union">
	<!--- Query para obtener los roles de TODOS los usuarios,
		porque pso es agente de todos --->
	<cfset extra = extra & " select ' Roles Personales' as nombre_cuenta_empresarial, null as nombre_empresa, r.rol, r.nombre as nombre_rol, up.id, 
		u.Pnombre, u.Papellido1, u.Papellido2, u.Papellido1 + ' ' + u.Papellido2 as Papellidos, u.Pemail1, 
	...
	<cfif not isdefined("form.reimp")>
		<cfset extra = extra & " and up.num_impreso = 0">
	</cfif>

	<cfset extra = extra & " union">
	<cfset extra = extra & " select cce.nombre as nombre_cuenta_empresarial, e.nombre_comercial as nombre_empresa, r.rol, r.nombre as nombre_rol, up.id, 
		ue.Pnombre, ue.Papellido1, ue.Papellido2, ue.Papellido1 + ' ' + ue.Papellido2 as Papellidos, isnull(u.Pemail1,ue.Pemail1)as Pemail1, 
...
<cfif not Session.secfw.soy_agente and not Session.secfw.soy_pso>
	<!--- Query para obtener los roles de los usuarios 
		de las empresas que yo administre --->
	<cfset campos = "cce.nombre as nombre_cuenta_empresarial, e.nombre_comercial as nombre_empresa, r.rol, r.nombre as nombre_rol, up.id, 
		ue.Pnombre, ue.Papellido1, ue.Papellido2, ue.Papellido1 + ' ' + ue.Papellido2 as Papellidos, isnull(u.Pemail1,ue.Pemail1)as Pemail1, 
		porque = 'soy admin de ' + cce.nombre">
...
function checkall(f) {
	...
	if (f.chk.length) {
		...
		for (i=0;i<f.chk.length;i++){
			if (i % 10 == 0) {
				window.status = i + " de " + f.chk.length;
			}
			//if (f.chk[i].checked != checked) {
				f.chk[i].checked = checked;
			//}
		}
		window.status = "";
		cntchecked = f.chkall.checked ? f.chk.length : 0;
	} else {
		f.chk.checked = f.chkall.checked;
		cntchecked = f.chkall.checked ? 1 : 0;
	}
}

function checkinv(f,c) {
	cntchecked += c.checked ? +1 : -1;
	f.chkall.checked = (cntchecked == f.chk.length);
	window.status = cntchecked + " de " + f.chk.length;
	return;
	if (!f.chk) return;
	if (f.chk.length) {
		var checked = true;
		for (i=0;i<f.chk.length;i++){
			checked &= f.chk[i].checked;
		}
		f.chkall.checked = checked;
	} else {
		f.chkall.checked = f.chk.checked;
	}
}
...
function algunoMarcado(f) {
	...
	alert("Marque alguno o todos los registros para realizar la acción!");
	return false;
}
...
<script language="JavaScript1.2" type="text/javascript">
<!--//
// Especifica el path donde se encuentra la carpeta "/qforms/"
qFormAPI.setLibraryPath("/cfmx/sif/js/qForms/");
// Carga todas las librerías por defecto
qFormAPI.include("*");
//-->
</script>
<form name="f" method="post" 
	<cfif (usarCarta)>
		...
		target="_blank"
	<cfelse>
		action="expedito_apply.cfm"
		target="_self"
	</cfif>>
	<table width="100%" border="0" cellpadding="2" cellspacing="0">
		<tr align="center">
			<td align="left"><b>
			<input type="checkbox" name="chkall" value="0" onClick="checkall(document.f)"> Seleccionar Todos.
			</b></td>
		</tr>
	</table>
	...
	<cfinvoke 
	 ...
	 returnvariable="pListaRet">
		...
		<cfinvokeargument name="desplegar" value="Pnombre, Papellidos, Pemail1, nombre_empresa"/>
		<cfinvokeargument name="etiquetas" value="Nombre, Apellidos, Correo, Empresa"/>
		...
		<cfinvokeargument name="formName" value="f"/>
	</cfinvoke>
	...
	<table width="100%" border="0" cellpadding="2" cellspacing="0">
		<tr align="center">
			<td align="center"><b>
			  <cfif (usarCarta)>
			  	<input name="carta" type="submit" id="carta" value="Preparar Cartas de afiliaci&oacute;n" onClick="javascript: return algunoMarcado(document.f);">
			  <cfelse>
			  	<input name="expedito" type="submit" id="expedito" value="Afiliar usuario y enviar contrase&ntilde;as" onClick="javascript: return algunoMarcado(document.f);">
			  </cfif>
			</b></td>
		</tr>
	</table>
...
objForm = new qForm("f");
if ( document.f.chk ){
	objForm.chk.addEvent("onClick","checkinv(document.f,this)");
}	
//-->
</script>