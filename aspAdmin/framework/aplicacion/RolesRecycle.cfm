...
<table width="100%" cellpadding="0" cellspacing="0">
	<tr class="itemtit"><td colspan="2"><font size="2"><b>Lista de Roles Inactivos</b></font></td></tr>
	...
	<tr>				
		<td valign="top">
			<cfoutput>
			<form name="filtro" style="margin:0;" action="RolesRecycle.cfm" method="post">
				<table width="100%" cellpadding="0" cellspacing="0" border="0" class="areaFiltro">
					<tr>
						<td align="right"><b>Rol:&nbsp;</b></td>
						<td><input name="frol" type="text" size="12" maxlength="12" value="<cfif isdefined("form.frol") and len(trim(form.frol)) gt 0>#form.frol#</cfif>" ></td>
			...
			<cfif isdefined("form.Filtrar") and isdefined("form.frol") and len(trim(form.frol)) gt 0>
				<cfset where = where & " and upper(rol) like upper('%#form.frol#%')" >
			</cfif> 
			...
			<cfset where = where & " order by upper(rol) " >
			...
			<cfinvoke 
			 ...
			 returnvariable="pListaRet">
				...
				<cfinvokeargument name="columnas" value="rtrim(sistema) as sistema, rtrim(rol) as rol, nombre"/>
	...
	function funcActivar(){
		if ( checkeados() ){
			if ( confirm('Va a activar a los usuarios seleccionados. Desde continuar?' ) ){
				...
				document.lista.action = 'SQLRoles.cfm';
