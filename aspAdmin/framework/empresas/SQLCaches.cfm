...
<cfquery name="rsCacheUnico" datasource="sdc">
...
<cfif not isdefined("error") >
	...
	<cftry>
		<cfif not isdefined("error") and ListFind('SIF,RH,ME,EDU,SED', trim(Ucase(form.sistema)), ',') neq 0 >
			...
			<cfif not isdefined("error")>
				...
				<cfquery name="rsDatosEmpresa" datasource="sdc">
					select e.nombre_comercial, u.Usucuenta, u.Usucodigo, u.Ulocalizacion, e.cliente_empresarial
					from Empresa e, Usuario u
					where e.Ecodigo   = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.Ecodigo#">
				...
				<cfif ListFind('SIF,RH,ME', trim(Ucase(form.sistema)), ',') neq 0 >
					...
					<cfquery name="rsExisteEmpresa" datasource="#form.nombre_cache#">
						select Ecodigo
						from Empresas 
						where EcodigoSDC = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.Ecodigo#">
					</cfquery>
					...
					<cfif not isdefined("error")>
						...
						<cfelse>
							<cfquery name="insert_empresa" datasource="#form.nombre_cache#">
								...
								select @Mcodigo=min(Mcodigo)  from Monedas
	...
	<cfcatch type="any">
		<cfset error = 2 > <!--- el cache dado por el usuario no corresponde a base de datos de SIF, RH, ME --->
	</cfcatch>
	</cftry>
...
<cfoutput>
<form action="Caches.cfm" method="post" name="sql">
	<cfif isdefined("error")><input type="hidden" name="sql_error" value="#error#"></cfif>
	<input type="hidden" name="sdcEcodigo" value="#form.Ecodigo#">
