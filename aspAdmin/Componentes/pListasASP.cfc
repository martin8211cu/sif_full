<cfcomponent>
<script type="text/javascript" language="JavaScript" src="/cfmx/sif/js/pLista1.js"></script>
<link href="/cfmx/aspAdmin/css/sif2.css" rel="stylesheet" type="text/css">
<cffunction name="pLista" access="public" returntype="query" output="true">
	...
	<cfargument name="funcionBorrar" type="string" required="false" default="">
	...
	<input name="Ecodigo" type="hidden" value="<cfoutput><cfif IsDefined("Session.Ecodigo")>#Session.Ecodigo#</cfif></cfoutput>">
	...
	<cfoutput query="rsLista" startrow="#StartRow_lista#" maxrows="#MaxRows_lista#">
		...
		<cfif funcionBorrar NEQ "">
			<cfset fb = funcionBorrar>
			<cfif fparams NEQ "">
				<cfset fb = fb & "(">
				<cfloop index="fidx" list="#fparams#">
					<cfset fb = fb & "'" & #Evaluate('rsLista.#Trim(fidx)#')# & "', ">
				</cfloop>
				<cfset fb = Left(fb, Len(fb)-2) & ");">
			</cfif>
		</cfif>
		...
		<cfloop index="cName" list="#desplegar#">
					...
					<cfif col EQ 1 and showLink>
						<td align="left" width="18" height="18" nowrap onclick="javascript: <cfif funcion NEQ "">#f#<cfelseif showLink>Procesar('#rsLista.CurrentRow#','#ArrayLen(columnas)#','#formName#'<cfif not incluyeForm>, '#irA#'</cfif>);</cfif>">
							<cfif comparenocase(trim(datosBD),trim(datosPost)) EQ 0 and (not isdefined("Nuevo"))>
								<img src="/cfmx/sif/imagenes/addressGo.gif" width="18" height="18"> 
							</cfif>
						</td>
					</cfif>
					<cfif col EQ 1 and funcionBorrar NEQ "">
						<td align="left" width="18" height="18" nowrap>
							<img type="image" src="/cfmx/aspAdmin/imagenes/Borrar01_S.gif"/ width="18" height="18" style="cursor=hand;" onclick="javascript:#fb#;return false;">
						</td>
					</cfif>
...
<cfreturn rsLista>
	...
	<cffunction name="valLista" access="public" returntype="array" output="true">
		<cfargument name="valores" type="string" required="yes">
		<cfset dato=Form.chk>
		<cfset Lista = ListtoArray(dato)>
		<cfreturn #Lista#>
	</cffunction>
