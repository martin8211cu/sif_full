...
<cf_templateheader title="SIF - Quick Pass">
	<cf_web_portlet_start border="true" titulo="Aceptación de Ventas">
	   ...
	   <tr> 
		   <td valign="top"> 
			   <cfinvoke component="sif.Componentes.pListas" method="pListaQuery">
				   ...
				   <cfinvokeargument name="desplegar"        value="QPcteDocumento,QPcteNombre,QPvtaTagFecha,QPTPAN,QPctaSaldosTipo"/>
				   <cfinvokeargument name="etiquetas"        value="Identificación,Cliente,Fecha de la Venta, N° TAG, Tipo"/>
				   <cfinvokeargument name="formatos"         value="S,S,D,S,U"/>
				   <cfinvokeargument name="align"            value="left,left,left,left,left"/>
				   <cfinvokeargument name="ajustar"          value="S"/>
				   <cfinvokeargument name="irA"              value="QPassAceptaVenta_SQL.cfm"/>
				   ...
				   <cfinvokeargument name="botones"          value="Aceptar,Anular"/>
			   </cfinvoke>
		   </td>
	   </tr>
	</table>
...
<cffunction name="fnObtieneDatosLista" access="private" output="false">
   ...
   <cfquery name="rsLista" datasource="#session.DSN#">
		...
		<!--- <cfif isdefined ('form.filtro_QPctaBancoNum') and len(trim(form.filtro_QPctaBancoNum)) gt 0>
			 and b.QPctaBancoNum like '%#form.filtro_QPctaBancoNum#%'
		</cfif> --->
	   ...
	   order by QPvtaTagFecha desc
   </cfquery>
</cffunction>

    <script language="javascript1" type="text/javascript">
		function funcAceptar(){
			if (confirm("Esta seguro de que desea aceptar la venta?")) {
				return true;
			}
			return false;
		}
