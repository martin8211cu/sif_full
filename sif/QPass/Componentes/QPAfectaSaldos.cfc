<cfcomponent output="no">
	...
	<cffunction name="ProcesaMovimiento" access="public" returntype="boolean" output="no">
		...
		<cfset LvarFechaMaxima  = dateadd("s",360,now())>
		...
		<!---<cflock scope="application" type="exclusive" timeout="10">--->
			<cfif isdefined("Application.QPAplicandoProcesaMovimiento") and Application.QPAplicandoProcesaMovimiento EQ 1>
				...
				<cfset LvarFechaControl = dateadd("s",430,LvarFechaControl)>
	...
	<cffunction name="fnProcesaMovimientos" access="private" output="no" returntype="numeric">
		...
		<cfquery name="_rsMovimientosTag" datasource="#Arguments.Conexion#" maxrows="5000">
			select QPMCid
			from QPMovCuenta 
		...
		<cfset _LvarTodosRegistros = true>

		<cfloop query="_rsMovimientosTag">
			<cfset LvarQPMCid = _rsMovimientosTag.QPMCid>
			<cfif LvarFechaMaxima LTE now()>
				<cfif _LvarTodosRegistros>
                    <cfreturn LvarQPMCid>
                <cfelse>
                    <cfreturn Arguments.ArgUltimoId>
                </cfif>
			</cfif>
			<cfset _LvarCorrecto = fnAfectaMovimiento(LvarQPMCid, Arguments.Conexion)>
            <cfif _LvarTodosRegistros and not _LvarCorrecto>
            	<cfset _LvarTodosRegistros = false>
            </cfif>
		</cfloop>
		<cfif _LvarTodosRegistros>
			<cfreturn LvarQPMCid>
		...
		<cftransaction action="begin">
			...
			<cfquery name="_rsActualizaRegistroLectura" datasource="#Arguments.Conexion#">
				select 
					...
					QPMCFInclusion, 
					convert(char, BMFecha, 111) as BMFechaSinHora, 
					convert(char, BMFecha, 108) as Hora,
					BMFecha
				from QPMovCuenta 
				where QPMCid = #arguments.QPMCid#
				  and QPMCFAfectacion is null
			...
			<cfif len(trim(_rsActualizaRegistroLectura.QPMCMontoLoc)) EQ 0 or _rsActualizaRegistroLectura.QPMCMontoLoc EQ 0>
				...
				<cfif _rsObtieneConversion.recordcount EQ 0>
					<!--- No hay registros de tipo de cambio.  Se asume que solo ocurre con la moneda Local --->
					<cfset LvarQPMCMontoLoc = NumberFormat(_rsActualizaRegistroLectura.QPMCMonto , "9.00")>
				<cfelseif _rsObtieneConversion.recordcount EQ 1>
					<!--- Hay registros de tipo de cambio.  Se procesa el dato con el tipo de cambio encontrado --->
					<cfset LvarQPMCMontoLoc = NumberFormat(_rsActualizaRegistroLectura.QPMCMonto * _rsObtieneConversion.TCventa, "9.00")>

					<!---  Desarrollo
					<cfif _rsActualizaRegistroLectura.BMFechaSinHora eq '2010/04/22' and (_rsActualizaRegistroLectura.QPMCMonto LT 0 or _rsActualizaRegistroLectura.QPMCMonto gT 0)> 
					   <cfset LvarQPMCMontoLoc = NumberFormat(_rsActualizaRegistroLectura.QPMCMonto * 524.78, "9.00")>
					</cfif> --->

					<!--- Negativos 
					<cfif _rsActualizaRegistroLectura.BMFechaSinHora eq '2010/04/28' and _rsActualizaRegistroLectura.QPMCMonto LT 0>
						<cfset LvarQPMCMontoLoc = NumberFormat(_rsActualizaRegistroLectura.QPMCMonto * 518.5, "9.00")>
					</cfif>
					<cfif _rsActualizaRegistroLectura.BMFechaSinHora eq '2010/04/29' and _rsActualizaRegistroLectura.QPMCMonto LT 0>
						<cfset LvarQPMCMontoLoc = NumberFormat(_rsActualizaRegistroLectura.QPMCMonto * 517, "9.00")>
					</cfif> --->

					<!--- Positivos --->
					<!--- -1: si la fecha 1 es anterior a la fecha 2
					       1: si la fecha 1 es posterior a la fecha 2
					 --->

					<!---Para Desarrollo
					<cfif datecompare(_rsActualizaRegistroLectura.BMFecha, '2010/04/22 9:16:00', 'n') eq -1  and _rsActualizaRegistroLectura.QPMCMonto GT 0>
						<cfset LvarQPMCMontoLoc = NumberFormat(_rsActualizaRegistroLectura.QPMCMonto * 518.5, "9.00")>
					</cfif>
					<cfif datecompare(_rsActualizaRegistroLectura.BMFecha, '2010/04/22 9:17:00', 'n') eq 1 and _rsActualizaRegistroLectura.QPMCMonto GT 0>
						<cfset LvarQPMCMontoLoc = NumberFormat(_rsActualizaRegistroLectura.QPMCMonto * 517, "9.00")>
					</cfif> --->

					<!--- Produccion 
					<cfif _rsActualizaRegistroLectura.BMFechaSinHora eq '2010/04/29' and datecompare(_rsActualizaRegistroLectura.BMFecha, '2010/04/29 21:00:00', 'n') eq -1  and _rsActualizaRegistroLectura.QPMCMonto GT 0>
						<cfset LvarQPMCMontoLoc = NumberFormat(_rsActualizaRegistroLectura.QPMCMonto * 518.5, "9.00")>
					</cfif> 
					<cfif _rsActualizaRegistroLectura.BMFechaSinHora eq '2010/04/29' and datecompare(_rsActualizaRegistroLectura.BMFecha, '2010/04/29 21:00:00', 'n') eq 1 and _rsActualizaRegistroLectura.QPMCMonto GT 0>
						<cfset LvarQPMCMontoLoc = NumberFormat(_rsActualizaRegistroLectura.QPMCMonto * 517, "9.00")>
						...
						Se encontro mas de un registro vigente, 
						por lo tanto, se dejan los valores para reproceso posterior de los registros
					---> 
                    		<cftransaction action="rollback"/>
                   		   <cfreturn false>
                		</cfif>
