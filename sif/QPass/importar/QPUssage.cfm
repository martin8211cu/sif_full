...
<!--- Verifica la cantidad de registros a procesar --->
...
<!--- Verifica que solo exista un Detalle --->
...
<!--- Verifica que solo exista un encabezado --->
<cfquery name="rsVerifica3" datasource="#session.dsn#">
	select count(1) as Cantidad
	from #table_name#
	where Registro like '0%'
</cfquery>

<cfif rsVerifica3.cantidad NEQ 1>
	<cfquery name="Err" datasource="#session.dsn#">
		select 
		  'El archivo no contiene un único encabezado' as Error
		from dual
	</cfquery>
</cfif>

<cfif rsVerifica2.recordcount NEQ 1>
	<cfquery name="Err" datasource="#session.dsn#">
		select 
		  'El archivo no contiene un único fin de archivo' as Error
		from dual
	</cfquery>
</cfif>
...
<cfif LvarCantidadTeorica NEQ lvarCantidadRecibida>
	<cfquery name="Err" datasource="#session.dsn#">
		select 
		  #LvarCantidadTeorica# as Teoricos, 
		  #LvarCantidadRecibida# as Recibidos, 
		  'Se esperan #numberformat(LvarCantidadTeorica,"9")# registros seg&uacute;n el registro de totales, 
	           pero los Registros de Detalle son #LvarCantidadRecibida#' as Error
		from dual
	</cfquery>

<cfelse>
   <!--- <cfquery name="rsdb1" datasource="#session.dsn#">
	select count(1) as cantidad
	from QPMovCuenta
    </cfquery>
    <cfdump var='#rsdb1#'> --->

    <cfquery name="rsverificamovimientos" datasource="#session.dsn#">
	select count(1) as cantidad
	from QPMovCuenta
	where upper(QPMCdescripcion) like '%SENTIDO%'
    </cfquery>
    ...
    <!---<cfquery name="rsdb" datasource="#session.dsn#">
	select count(1) as cantidad
	from QPMovCuenta
    </cfquery>
	<cfdump var='#rsdb#'>--->
