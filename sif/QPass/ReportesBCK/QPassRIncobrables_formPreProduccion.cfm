...
<cffunction name="generaReporte" output="no">
	...
	<cfquery datasource="#session.DSN#" name="rsIncobrable">
		...
		<cfif isdefined("form.fechaDesde") and len(trim(form.fechaDesde)) and isdefined("form.fechaHasta") and len(trim(form.fechaHasta))>
			a.QPMCAFAplicacion between <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(form.fechaDesde)#"> and <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(form.fechaHasta)#">
...
<!---<cfif isdefined('form.generar')>--->
	<!--- Query:
		...
		<cfset write_to_buffer('	</tr>')>
		<cfset write_to_buffer('	<tr>')>
		<cfset write_to_buffer('		<td colspan="10" align="center"><strong>Reporte de Incobrables Desde:#form.fechaDesde# hasta:#form.fechaHasta#</strong></td>')>
		<cfset write_to_buffer('	</tr>')>
		<cfset write_to_buffer('	<tr>')>
