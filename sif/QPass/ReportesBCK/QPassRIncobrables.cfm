<cf_templateheader title="Reporte de Incobrables">
	<cf_web_portlet_start titulo="Generaci&oacute;n de Incobrables">
	<br>
	<cfset session.Reporte.QPIncobrable = 0>
    <!--- <cfoutput>#session.Reporte.QPIncobrable#</cfoutput> --->
	<cfoutput>
	<form name="form1" action="QPassRIncobrables_Wait.cfm" method="post" onsubmit="return validar(this);">
...
<script language="javascript1" type="text/javascript">
	...
	function validar(formulario)
	{
		...
		if (document.form1.fechaDesde.value == ""  & document.form1.fechaHasta.value == "") 
			{
				alert ("Debe definirse un rango de Fechas");
				return false;
		   }
		else
			{ 
