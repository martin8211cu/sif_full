...
<cfif isdefined("url.Guardar")>
        ...
        <cfquery name="rsReporte" datasource="#session.DSN#">
			select 
                        ...
                        sn.SNnombre,
                        Dsaldo as Saldo,
						CASE WHEN ((tce.Mcodigo is null)or(tce.Mcodigo = #rsParamConversion.Pvalor#))  then Dsaldo else (Dsaldo * tce.TCEtipocambioventa) end as SaldoMonedaLocal,		
                        CASE WHEN Dantiguedad = 0 THEN Dsaldo ELSE 0 END AS Corriente,
                        CASE WHEN Dantiguedad < 0 THEN Dsaldo ELSE 0 END AS SinVencer,
                        CASE WHEN Dantiguedad BETWEEN 0 AND 30 THEN Dsaldo ELSE 0 END AS P1,
                        CASE WHEN Dantiguedad BETWEEN 31 AND 60 THEN Dsaldo ELSE 0 END AS P2,
                        CASE WHEN Dantiguedad BETWEEN 61 AND 90 THEN Dsaldo ELSE 0 END AS P3,
                        CASE WHEN Dantiguedad BETWEEN 91 AND 120 THEN Dsaldo ELSE 0 END AS P4,
                        CASE WHEN Dantiguedad BETWEEN 121 AND 150 THEN Dsaldo ELSE 0 END AS P5,
                        CASE WHEN Dantiguedad > 150 THEN Dsaldo ELSE 0 END AS P5p,
                        si.Dantiguedad as Morosidad,
                        si.Ddocumento
				...
				FROM SNClasificacionD cd 
					...
					INNER JOIN SNSaldosInicialesCPD si
        				on si.SNid = cs.SNid
	...
	<cf_QueryToFile query="#rsReporte#" filename="AntiguedadSaldosXMes.xls" titulo="Antigüedad de Saldos por Mes Cerrado Detallado">
	<cfabort>
</cfif>
...
<cf_web_portlet_start border="true" skin="#Session.Preferences.Skin#" tituloalign="center" titulo='Antigüedad&nbsp;de&nbsp;Saldos&nbsp;por&nbsp;Clasificaci&oacute;n y Mes Detallado'>
<script language="JavaScript" src="../../js/fechas.js"></script>

<!--- Filtro Periodo y Mes --->
...
<cfoutput>
<form name="form1" action="Antiguedad_Saldos_ClasiFxMesDetallado.cfm" method="get">
...
<cf_templatefooter>	
	...
	<cfset LvarReporte = "AntiguedadSaldosxClas_X_MesDetallado_Cliente.cfr">
    ...
    <cfif rsactivatejsreports.valParam EQ 1 AND coldfusionV EQ 2018 and  formatos eq "excel">
	 ...
	 <!---  <cfif formatos EQ "pdf">
	  ...
	  <cf_js_reports_service_tag queryReport = "#rsReporte#" 
		isLink = False 
		typeReport = #typeRep#
		fileName = "cp.consultas.reportes.AntiguedadSaldosxClas_X_MesDetallado_Cliente"
	...
	<cfelse>	
		<cfreport format="#formatos#" template="#LvarReporte#" query="rsReporte">	
			...
			<cfif isdefined("url.TClasif") and url.TClasif EQ 0>
					<cfreportparam name="Titulo" value="Antigüedad de Saldos,  Detallado por Clasificación / Valor/ Socio de Negocios (Mes Cerrado): por Socio">
			<cfelse>
					<cfreportparam name="Titulo" value="Antigüedad de Saldos,  Detallado por Clasificación / Valor/ Socio de Negocios (Mes Cerrado): por Dirección">
			</cfif>
		</cfreport>	
	</cfif>
</cfif>
