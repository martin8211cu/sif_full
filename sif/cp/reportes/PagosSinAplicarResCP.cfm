...
select 	
	distinct <!--- se usa porque solo interesa un documento por N líneas ---> 
			 	a.EPdocumento,  
				a.EPfecha, 
				a.CPTcodigo,
				a.EPtotal,
		...
		where a.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
			...
			<cfif isdefined("url.Moneda") and len(trim(url.Moneda)) and isdefined("url.Moneda2") and len(trim(url.Moneda2))>
				<cfif url.Moneda gt url.Moneda2><!--- si el primero es mayor que el segundo. --->
    ...
    <cfif rsactivatejsreports.valParam EQ 1 AND coldfusionV EQ 2018 and formatos eq "excel">
	  ...
	  <cf_js_reports_service_tag queryReport = "#rsReporte#" 
		isLink = False 
		typeReport = #typeRep#
		fileName = "cp.consultas.reportes.PagosSinAplicarResCP"/>
	<cfelse>
		<cfreport format="#formatos#" template= "PagosSinAplicarResCP.cfr" query="rsReporte">	
