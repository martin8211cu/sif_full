...
<cfif isdefined("url.Generar")>
    ...
    <cfquery name="rsReporte" datasource="#session.DSN#" maxrows="5001">
		select 	'#desc#' as Descripcion,
        		f.Mnombre,
				a.EDdocumento,
				a.EDfecha,
				a.CPTcodigo as CPTcodigo,
				a.EDvencimiento,
				case when tr.CPTtipo = 'D' then -a.EDtotal else a.EDtotal end as EDtotal,
				b.SNnombre,
				b.SNnumero,
				b.SNidentificacion,
				c.Cformato,
				c.Cdescripcion,
				<cf_dbfunction name="concat" args="cc.Cformato  + cc.Cdescripcion" delimiters="+"> as CdescripcionDetalle,
				a.IDdocumento,
				e.Linea,
				e.DDdescripcion,
				e.DDdescalterna,
				...
				e.DDdesclinea,
				e.DDporcdesclin,
				(e.DDtotallinea * case when tr.CPTtipo = 'D' then -1.00 else 1.00 end) as DDtotallinea,
				...
				a.EDreferencia as DEobservacion,
				e.Icodigo,
				d.Odescripcion,
				case when tr.CPTtipo = 'D' then -EDimpuesto else EDimpuesto end as Impuesto,
				case when tr.CPTtipo = 'D' then -(EDtotal - EDimpuesto) else (EDtotal - EDimpuesto) end as SubTotal,
                oc.EOnumero OrdenCompra
		 from EDocumentosCxP a
			left outer join DDocumentosCxP e
				on e.IDdocumento = a.IDdocumento
            LEFT OUTER JOIN DOrdenCM oc
       			on oc.DOlinea = e.DOlinea
			...
			inner join CPTransacciones tr
				on tr.CPTcodigo = a.CPTcodigo
				and tr.Ecodigo = a.Ecodigo
		...
		where a.Ecodigo =  #session.Ecodigo#
			...
			<cfif isdefined("url.Ocodigo") and len(trim(url.Ocodigo))>
			 ...
			 <!---►►Se envio la fecha desde y la fecha hasta◄◄--->
			 <cfif isdefined("url.fechaDes") and len(trim(url.fechaDes)) and isdefined("url.fechaHas") and len(trim(url.fechaHas))>
				<!---►►Fecha desde es Igual a Fecha Hasta◄◄--->
				<cfif datecompare(url.fechaDes, url.fechaHas) eq 0>
					and #fecha# between <cf_jdbcquery_param cfsqltype="cf_sql_timestamp" value="#lsparsedatetime(url.fechaDes)#">
						and <cf_jdbcquery_param cfsqltype="cf_sql_timestamp" value="#dateAdd("s",86399,lsparsedatetime(url.fechaDes))#">
				<!---►►Fecha desde es Menor a Fecha Hasta◄◄--->
				<cfelseif datecompare(url.fechaDes, url.fechaHas) eq -1>
					and #fecha# between <cfqueryparam cfsqltype="cf_sql_timestamp" value="#lsparsedatetime(url.fechaDes)#">
						and <cfqueryparam cfsqltype="cf_sql_timestamp" value="#dateAdd("s",86399,lsparsedatetime(url.fechaHas))#">
				<!---►►Fecha desde es mayor a Fecha Hasta◄◄--->
				<cfelseif datecompare(url.fechaDes, url.fechaHas) eq 1>
					and #fecha# between <cfqueryparam cfsqltype="cf_sql_timestamp" value="#lsparsedatetime(url.fechaHas)#">
						and <cfqueryparam cfsqltype="cf_sql_timestamp" value="#dateAdd("s",86399,lsparsedatetime(url.fechaDes))#">
				</cfif>
             <!---►►Se envio solo la fecha desde◄◄--->
			<cfelseif isdefined("url.fechaDes") and len(trim(url.fechaDes))>
				and #fecha# >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#lsparsedatetime(url.fechaDes)#">
			<!---►►Se envio solo la fecha Hasta◄◄--->
			<cfelseif isdefined("url.fechaHas") and len(trim(url.fechaHas))>
				and #fecha# <= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#dateAdd("s",86399,lsparsedatetime(url.fechaHas))#">
			</cfif>
		...
		order by f.Mnombre, b.SNnombre, a.IDdocumento, e.Linea
	</cfquery>
...
<cfset LB_NombreReporte = t.Translate('LB_NombreReporte','Documentos sin Aplicar (Detallado)')>
...
<cfset LB_TotSocio = t.Translate('LB_TotSocio','Total Socio')>
<cfset LB_Obser = t.Translate('LB_Obser','Observaciones')>
<cfset LB_TotLinea = t.Translate('LB_TotLinea','Total Línea')>
<cfset LB_TotDocImp = t.Translate('LB_TotalDoc','Total Documento (con impuestos):')>
<cfset LB_Identificacion = t.Translate('LB_Identificacion','Identificaci&oacute;n','/sif/generales.xml')>
<cfset LB_OrdenC = t.Translate('LB_OrdenC','Orden C.')>
<cfset LB_Cantidad = t.Translate('LB_Cantidad','Cantidad')>
    ...
    <cfif rsactivatejsreports.valParam EQ 1 AND coldfusionV EQ 2018 and  formatos eq "Excel">
	  ...
	  <cf_js_reports_service_tag queryReport = "#rsReporte#" 
		isLink = False 
		typeReport = #typeRep#
		fileName = "cp.consultas.reportes.DocumentosSinAplicarDetCP"
		headers = "title:#LB_NombreReporte#"/>
	<cfelse>
		<cfreport format="#formatos#" template="DocumentosSinAplicarDetCP.cfr" query="rsReporte">
			...
			<cfreportparam name="LB_TotDocImp" value="#LB_TotDocImp#">
			...
			<cfreportparam name="LB_OrdenC"   value="#LB_OrdenC#">
