...
<cfquery name="rsData" datasource="#Session.DSN#">
	<!--- Facturas y NC --->
	select sn.SNnumero, sn.SNnombre, a.CPTcodigo, t1.CPTdescripcion,
	case
	when a.Dfecha < <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(form.fecha1)#"> then '-SALDO INICIAL CXP-'
	else a.Ddocumento end as Ddocumento
	, a.Dtotal as Dtotal, a.Mcodigo as Mcodigo, a.Dfecha,
	...
	union all
	<!---Saldo de NC  NO APLICA PARA LA CONSULTA POR DOC.--->
	select sn.SNnumero, sn.SNnombre, a.CPTcodigo, t1.CPTdescripcion,
	case
	when a.Dfecha < <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(form.fecha1)#"> then '-SALDO INICIAL CXP-'
	else a.Ddocumento end as Ddocumento,
	...
	coalesce(a.BMmontoref,a.Dtotal) as Dtotal, coalesce(a.Mcodigoref,a.Mcodigo) as Mcodigo, a.Dfecha,
	...
	union all
	<!--- Neteos --->
	select sn.SNnumero, sn.SNnombre, a.CPTRcodigo, t2.CPTdescripcion,
	case
	when a.Dfecha < <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(form.fecha1)#"> then '-SALDO INICIAL CXP-'
	else a.DRdocumento end as Ddocumento,
	coalesce(a.BMmontoref,a.Dtotal) as Dtotal, coalesce(a.Mcodigoref,a.Mcodigo) as Mcodigo, a.Dfecha,
...
<cfoutput>
	<table width="98%" align="center" border="0" cellspacing="0" cellpadding="0" class="">
		...
		<tr>
			<td class="style1">Integración de Saldos por Documento en Cuentas por Pagar</td>
		</tr>
		<tr>
			<td class="style2">
...
<cfoutput query="rsData" group="Mcodigo">
  ...
  <tr>
	<td style="border-bottom: 1px solid black; border-top:1px solid black; " class="listaCorte" align="left" colspan="6" nowrap >
		<cfif rsMoneda.recordcount and len(rsMoneda.Mnombre)>#rsMoneda.Mnombre#<cfelse>Desconocida</cfif>	</td>
  </tr>
	...
	<cfoutput group="Ddocumento">
		...
		<cfoutput>
			<cfif len(rsData.Dtotal) and IsNumeric(rsData.Dtotal) and rsData.Dtotal gt 0.00>
				<cfif DateCompare(rsData.Dfecha,LSParseDateTime(form.fecha1)) lt 0>
					<cfif rsData.CPTtipo eq 'C'>
		...
		<tr class=<cfif LvarListaNon>"listaNon"<cfelse>"listaPar"</cfif>>
			 <td class=<cfif CurrentRow MOD 2>"listaNon"<cfelse>"listaPar"</cfif>>
				   <cfloop from="1" to="10" index="i">&nbsp;</cfloop>#Ddocumento#
			 </td>
