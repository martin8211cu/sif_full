<!--- 
	...
	Modificado el 7-6-2005
		Motivo: Incluir el encabezado de la orden de compra cuando se ven las líneas de la orden.
	Modificado por Gustavo Fonseca H.
		Fecha: 29-7-2005.
		Motivo: se elimina el CMCid que identificaba si el usuario era comprador. Esto por que se utiliza en CxP y no en compras.
 --->

<!--- Validar tipo de documento (FACTURA. REMISION)--->
<cfset form.tipoDocumento = "FACTURA">
<cfif isdefined("url.tipoDocumento")>
    <cfset form.tipoDocumento = url.tipoDocumento>
</cfif>
<cfset sqlOrdenCompra = "OrdenCompra-SQL.cfm">
<cfif form.tipoDocumento EQ "REMISION">
  <cfset sqlOrdenCompra = "remisiones/OrdenCompraRemision-SQL.cfm">
</cfif>
			...
			<table width="99%" align="center" border="0" cellspacing="0" cellpadding="0">
	    		<tr>
     				 <td>
						...
						<cfoutput>
							<form style="margin: 0" action="OrdenCompra.cfm" name="fsolicitud" id="fsolicitud" method="post">
							...
							<input type = "hidden" name = "tipoDocumento" value="<cfoutput>#form.tipoDocumento#</cfoutput>">
							</form>
						</cfoutput>
						<!---  /Filtro  --->
						...
						<cfquery name="rsLista" datasource="#session.DSN#">
							select a.EOidorden, a.Ecodigo, a.EOnumero, a.SNcodigo, #form.IDdocumento# as IDdocumento,
								...
								and ( 
        							 select count(1) 
									  ...
									  where g.EOidorden = a.EOidorden 
									    and g.DOcantidad - g.DOcantsurtida > 0 ) > 0
							  and ( 
									  ((select count(1) from DOrdenCM g 
										where g.EOidorden = a.EOidorden)
										>
							  			(select count(1)  
										from DOrdenCM g 
										...
										>
							  			(
										select count(1) from DOrdenCM g 
										inner join DDocumentosCPR aa 
										on aa.Linea = g.DRemisionlinea
										where g.EOidorden = a.EOidorden ))
									)
						...
						<cfinvoke component="sif.Componentes.pListas" method="pListaQuery" returnvariable="pListaRet">
							...
							<cfinvokeargument name="irA" 				value="OrdenCompra.cfm?tipoDocumento=#form.tipoDocumento#"/>
							<cfinvokeargument name="showEmptyListMsg" 	value="true"/>
							<cfinvokeargument name="keys" 				value="EOidorden, IDdocumento"/>
...
<script language="javascript1.2" type="text/javascript">
	function funcAplicar(){
		...
		if ( continuar ){
			document.lista.action = '<cfoutput>#sqlOrdenCompra#</cfoutput>';
...
<cfquery name="rsListaItems" datasource="#session.dsn#">
	select 	a.DOlinea, 
			...
			a.DOdescripcion, 
			a.DOcantidad - a.DOcantsurtida as DOSaldocantidad, 
			a.DOcantidad,
            ...
            case 
                  ...
                  inner join DOrdenCM bb
                  on bb.DOlinea = aa.DOlinea
                  where aa.DOlinea = a.DOlinea) = 0 
									and (select count(1) from DDocumentosCPR aa
									inner join DOrdenCM bb
									on aa.Linea = bb.DRemisionlinea
									where bb.DOlinea = a.DOlinea) = 0
									then
                	0
                else
                	#form.IDdocumento# 
	...
	where a.Ecodigo =  #Session.Ecodigo# 
		<cfif isdefined("form.EOidorden") and len(trim(form.EOidorden))>
 		...
 		<!--- 22/10/2014 ERBG Cambio para evitar la cancelación de orden relacionada a un documento de CXP Inicia---> 
       		and a.DOcantidad - a.DOcantcancel > 0	
		<!--- 22/10/2014 FIN--->
		    ...
		    <!---and ( select count(1)
							from DDocumentosCxP aa
								inner join DOrdenCM bb
									on bb.DOlinea = aa.DOlinea
									where aa.DOlinea = a.DOlinea
							) = 0--->
		<!---<cfif isDefined("form.tipoDocumento") and form.tipoDocumento EQ "REMISION">
		 and a.CMtipo = 'S' 
		</cfif>---->
	...
	<tr>
		<td>
			...
			<cfinvoke component="sif.Componentes.pListas" method="pListaQuery">
				...
				<cfinvokeargument name="desplegar" value="DOconsecutivo,CMTipodesc,Codigo,DOdescripcion,DOcantidad, DOSaldocantidad,DOmontoSurtido,Udescripcion,DOpreciou,DOmontodesc,total">
				<cfinvokeargument name="etiquetas" value="L&iacute;nea,Tipo,C&oacute;digo,Descripci&oacute;n,Cantidad Ordenada, Cantidad sin Surtir,Monto Surtido,Unidad Medida,Precio,Descuento,Total">
				<cfinvokeargument name="formatos" value="V,V,V,V,V,M,M,V,M,M,M,S">
				<cfinvokeargument name="align" value="left,left,left,left,right,right,center,right,right,right,right,right">
				<cfinvokeargument name="ajustar" value="S">
				<cfif isdefined ("rsListaItems") and rsListaItems.recordcount GT 0>
					<cfinvokeargument name="botones" value="Agregar"/>
				</cfif>
                <cfinvokeargument name="showEmptyListMsg" value="true">
				<cfinvokeargument name="irA" value="#sqlOrdenCompra#">
				<cfinvokeargument name="formName" value="form3">
				<cfinvokeargument name="keys" value="IDdocumento,DOlinea">
