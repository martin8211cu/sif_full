...
<cfquery name="rsTransaccion" datasource="#Session.DSN#">
	...
	union all
	select distinct HEAplicacionCP.CPTcodigo as value, 
	CPTransacciones.CPTdescripcion  as description 
	from HEAplicacionCP, CPTransacciones 
	where HEAplicacionCP.Ecodigo=  #Session.Ecodigo# 
	  and CPTransacciones.CPTcodigo = HEAplicacionCP.CPTcodigo 
	  and CPTransacciones.Ecodigo = HEAplicacionCP.Ecodigo 
      and HEAplicacionCP.HEACPreversa = 0
</cfquery>
<cfquery name="rsUsuarios" datasource="#Session.DSN#">
    ...
    and a.HEACPreversa = 0
</cfquery>
<cfquery name="rsMonedas" datasource="#Session.DSN#">
	select -1 as value, '-- #LB_Todas# --' as description from dual
	union all
	select distinct HEAplicacionCP.Mcodigo as value, Monedas.Mnombre as description
	from HEAplicacionCP, Monedas 
	where HEAplicacionCP.Ecodigo=  #Session.Ecodigo# 
	  and Monedas.Mcodigo = HEAplicacionCP.Mcodigo 
      and HEAplicacionCP.HEACPreversa = 0
</cfquery>
</cfoutput>
...
<cfif isDefined("Form.BtnAplicar") and isdefined("Form.chk")>
  ...
  <cfloop index="CountVar" from="1" to="#cuantos#">
	  ...
	  <cfinvoke component="sif.Componentes.CP_RevPosteoDocsFavorPagosAnticipos" method="CP_PosteoDocsFavorCxP"
	  		HEACPid     = "#valores[1]#"
			ID 			= "#valores[2]#"
...
<cf_dbfunction name="to_char" args="a.HEACPid" returnvariable="HEACPid">
<cf_dbfunction name="concat" returnvariable="LvarImagen" args="<img border=''0'' src=''/cfmx/sif/imagenes/findsmall.gif'' onClick=''javascript:imprimeDoc2('+#ID_char#+','+#HEACPid#+')''>" delimiters="+">
...
<cfset LB_Titulo = t.Translate('LB_Titulo','Lista de Documentos a Favor Aplicados')>

<cf_templateheader title="SIF - Cuentas por Pagar">	
	<cf_web_portlet_start border="true" skin="#Session.Preferences.Skin#" tituloalign="center" titulo='#LB_Titulo#'>
		<cfinclude template="/sif/portlets/pNavegacionCP.cfm">
        <cfoutput>
			<form style="margin:0" action="" method="post" name="form1">           
						...
						<td width="85%" valign="top">
							<cfinvoke component="sif.Componentes.pListas" method="pLista" returnvariable="pListaRet">
							 <cfinvokeargument name="columnas"  
								 value="a.HEACPid, a.ID as IDpago, t.CPTdescripcion, rtrim(a.CPTcodigo) as CPTcodigo, rtrim(a.Ddocumento) as Ddocumento,EAfecha,m.Mcodigo, m.Mnombre,a.EAusuario,EAtotal,(select count(*) from HDAplicacionCP x where a.ID = x.ID) as Balance, EAusuario,
										case (select count(*) from HDAplicacionCP x where a.ID = x.ID) when 0 then 
											#LvarActivo#
											else '' end as activo 
											#campos_extra# , 
											'#LvarImagen#' as imagen"/>
								<cfinvokeargument name="tabla"         value="HEAplicacionCP a, Monedas m, CPTransacciones t"/>
								<cfinvokeargument name="filtro"  	   value="a.Ecodigo = #Session.Ecodigo#
                                                                                          ...
                                                                                          and ((coalesce(t.CPTpago,0)=1 and coalesce(t.CPTanticipo,0)=0) or (coalesce(t.CPTpago,0)= 0 and coalesce(t.CPTanticipo,0)=1)) and a.HEACPreversa=0"/>
								...
								<cfinvokeargument name="keys"             		value="HEACPid,IDpago,Ddocumento,CPTcodigo"/>
								...
								<cfinvokeargument name="FilasParaBotonesSuperiores"			value="100"/>
							</cfinvoke>
						</td>
					</tr>
				</table>
...
<script language="JavaScript1.2">
		...
		if (f.chk != null) {
			if (f.chk.value) {
				if (!f.chk.checked) { 
					...
					f.action='listaRevDocsAfavorPagosAnticipos.cfm';
					return true;
				}
			} else {
	...
	var popupdoc = 0 ;
	function imprimeDoc2(id,HEACPid){
		...
		var top = 80;
		var left = 200;
		popupdoc = open('/cfmx/sif/cp/consultas/RevDocsFavorComprobantePagosAnticipos.cfm?id='+id+'&HEACPid='+HEACPid, 'popupdoc', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=yes,width='+width+',height='+height+',left='+left+',top='+top+',screenX='+left+',screenY='+top);
	}
</script>