<cfset LvarOBJ_PrecioU = createObject("Component","sif.Componentes.CM_PrecioU").init()>
	<cfif isdefined("Form.modoDet") and form.modoDet eq ''>
		<cfset modoDet = "ALTA">
	</cfif>
	...
	<cfif isdefined('Url.tipoF') and not isdefined('Form.tipoF')>
		<cfset form.tipoF = url.tipoF>
	</cfif>
	...
	<cfif isdefined('form.tipoF') and len(trim(form.tipoF))>
		<cfset params = params & '&tipo=#form.tipoF#' >
	</cfif>
	...
	<cfif not isDefined("Form.NuevoE")>
		<cfif isDefined("Form.datos") and Len(Trim(Form.datos)) NEQ 0 >
			...
			<cfset form.IDdocumento = Trim(arreglo[1])>
			<cfif sizeArreglo EQ 2>
				<cfset Linea = Trim(arreglo[2])>
				<cfset modoDet = "CAMBIO">
			</cfif>		
		<cfelse>
			<cfset IDdocumento = Trim(IDdocumento)>
	...
	<cfif modo NEQ "ALTA">
		...
		<cfquery name="rsDocumento" datasource="#Session.DSN#">
			...
			where d.Ecodigo = #Session.Ecodigo#
			  and d.IDdocumento = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.IDdocumento#">
		</cfquery>
		...
		<cfquery name="rsReferencia" datasource="#Session.DSN#">
			select a.CPTcodigo, a.Ddocumento
		...
		<cfif rsLinea.recordcount gt 0 and rsUsaPlanCuentas.Pvalor eq LvarParametroPlanCom>
	...
	var popUpWin=0; 
		...
		function doConlisItem() {
			...
			<cfif isdefined("form.tipo") and len(trim(form.tipo))> 
					location.href="<cfoutput>#root#</cfoutput>"+params;	
			</cfif>
		}
...
function funcRefrescar(){
		document.form1.action = 'RegistroFacturasxUsuarioCP.cfm?modo=Cambio&tipo=' + document.form1.tipo.value;
		document.form1.submit();
	}

<cfif isdefined("form.tipo") and len(trim(form.tipo)) and form.tipo eq 'D'> <!--- Notas de Crédito --->
	function funcSNnumero(){ 
			document.form1.action = 'RegistroFacturasxUsuarioCP.cfm?tipo=' + document.form1.tipo.value + '&tipoF=' + document.form1.tipoF.value  + '&IDdocumento=' + document.form1.IDdocumento.value;
			document.form1.submit();
		}

<cfelseif isdefined("form.tipo") and len(trim(form.tipo)) and form.tipo neq 'D'> <!--- Facturas --->
	function funcSNnumero(){ 
			document.form1.action = '<cfoutput>#root#</cfoutput>?tipo=' + document.form1.tipo.value + '&tipoF=' + document.form1.tipoF.value  + '&IDdocumento=' + document.form1.IDdocumento.value;
			document.form1.submit();
		}
</cfif>
</script>
...
<cfflush interval="128">
		...
		<cfif Len(Trim(form.IDdocumento)) NEQ 0 >
...
<form name="form1" id="form1" action="SQLRegistroFacturasCP.cfm" method="post">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td width="10%" nowrap="nowrap"><div align="right">Transacción&nbsp;(
                  <cfif form.tipo EQ "C" or form.tipo EQ "F">
                    CR
                      <cfelse>
                    DB
                  </cfif>
          ...
          <tr>
            ...
            </cfoutput> <td><cfif modo NEQ "ALTA"> <input type="button" tabindex="1" name="Garantias" value="Garantias" onclick="funcGarantias(<cfoutput>#form.IDdocumento#</cfoutput>)" /></cfif></td></td>
             </tr>
          <tr>
          	<!---►►Fecha de Arribo◄◄--->
          ...
          <!---►►Direccion Facturacion◄◄--->
          	<td align="right"   nowrap="nowrap" valign="top">Direcci&oacute;n facturación:&nbsp;</td>
          ...
          <tr>
           <!---►►Referencia◄◄--->
      		<td valign="top" align="right"> <cfif form.tipo NEQ "C"> Referencia:&nbsp;</cfif></td>
            <td valign="top" align="left">
                ...
                <input type="text" name="Referencia1" tabindex="-1" readonly size="20" maxlength="20" 
					value="<cfif modo NEQ "ALTA" and isdefined("rsReferencia") and rsReferencia.recordcount EQ 1><cfoutput>#rsReferencia.Ddocumento#</cfoutput></cfif>" />
                <script language="JavaScript1.2">
					if ("D" == "<cfoutput>#form.tipo#</cfoutput>")
					{
          ...
          <tr>
            <td><input type="hidden" name="tipo" value="<cfoutput>#form.tipo#</cfoutput>" />
				<cfif isdefined ('form.tipoF') and len(Trim(form.tipoF)) gt 0>
					<input type="hidden" name="tipoF" value="<cfoutput>#form.tipoF#</cfoutput>" />
				</cfif>
				<cfif isdefined ('form.LvarTipo') and len(Trim(form.LvarTipo)) gt 0>
					<input type="hidden" name="LvarTipo" value="<cfoutput>#form.LvarTipo#</cfoutput>" />
				</cfif>
				
				<cfif modo NEQ "ALTA">
					<input type="hidden" name="SNid" value="<cfoutput>#rsNombreSocio.SNid#</cfoutput>" />
				</cfif>
					<input type="hidden" name="root" value="<cfoutput>#form.root#</cfoutput>" />
		...
		<table style="width:100%" border="0" cellpadding="1" cellspacing="0">
		  ...
		  <tr>
			...
			<td style="width:100%" colspan="8" >
				<select id="DDtipo" name="DDtipo" onChange="javascript:limpiarDetalle();cambiarDetalle();Verifica();" 
...
--->					<cfif form.tipo NEQ "D">
			...
			<td  align="left" colspan="9" >
				...
				<table  align="left" border="0">
					<tr>
						...
						<td id="AlmacenImput">
							<cfif modoDet neq 'ALTA'>
								<cf_sifalmacen id="#rsLinea.Alm_Aid#"  size="14" aid="Almacen" tabindex="1" readOnly= "#LvarPlanDeCompras#">
							<cfelse>
								<cf_sifalmacen size="15" aid="Almacen" tabindex="1">
					 	  </cfif>
					  </td>					  
						<!--- ARTICULO --->
						...
						<td id="articulo">
							<cfif modoDet neq 'ALTA'>
								...
								<cf_sifarticulos query=#rsArticulo# almacen="Almacen" size="15" tabindex="1" readonly= "#LvarPlanDeCompras#">
							<cfelse>
								<cf_sifarticulos almacen="Almacen" size="15" tabindex="1">
							</cfif>						
					    </td>
						<!--- ARTICULO T--->
						<td id="articuloT">
						...
						<td id="centrofuncional">
							<cfif modoDet neq 'ALTA'>
								...
								<cfelse>		
									<cf_rhcfuncionalCxP Ccuenta="CcuentaD" form="form1" size="23" id="CFid" name="CFcodigo" desc="CFdescripcion" query="#rsCF#" tabindex="1">
								</cfif>
							<cfelse>
          ...
          <tr>
            ...
            <td colspan="3">
              <input name="DDdescripcion" tabindex="1" onFocus="javascript:this.select();" type="text" 
					value="<cfif modoDet NEQ "ALTA"><cfoutput>#HTMLeditFormat(rsLinea.DDdescripcion)#</cfoutput></cfif>" size="60" maxlength="255"  <cfif rsUsaPlanCuentas.Pvalor eq LvarParametroPlanCom> readonly= "#LvarPlanDeCompras#" </cfif>>
            </td>
            <td align="right" nowrap="nowrap">Desc. Alterna:&nbsp;</td>
            <td colspan="4">
              <input name="DDdescalterna" tabindex="1" onFocus="javascript:this.select();" type="text" 
		...
		<tr>
<!---		<cfif modoDet NEQ 'ALTA'>
		  <input type="hidden" name="Icodigo" tabindex="-1" value="<cfoutput>#rsLinea.Icodigo#</cfoutput>" />
		 </cfif>
--->			<td  align="right" valign="top">Impuesto:&nbsp;</td>
			<td colspan="4" valign="top">
				<select name="Icodigo" tabindex="1" <cfif modoDet NEQ 'ALTA' and rsUsaPlanCuentas.Pvalor eq LvarParametroPlanCom >disabled="disabled"</cfif>>
          ...
          <tr>
            <td colspan="11"><div align="center">
                ...
                <input type="submit" class="btnAplicar" tabindex="1" name="btnAplicar"  value="Aplicar" 	 onClick="javascript: fnNoValidar(); return Postear();">
					 ...
					 <input type="submit" class="btnNormal"  tabindex="1" name="btnAnular2"    value="Anular" 		 onClick="javascript: fnNoValidar();return true;">
                ...
                <input name="BorrarE" class="btnEliminar" tabindex="1" type="submit" value="Borrar Documento" 
					...
					<cfif isdefined("tipoF") and #tipoF#  eq 'C'>
					 <input type="submit" class="btnAplicar"  tabindex="1" name="btnEnviar_a_Aplicar"    value="Enviar a Aplicar" 	onClick="javascript: fnNoValidar();return true;">
					</cfif>
					<cfif isdefined("tipoF") and #tipoF#  eq 'D' or  #tipoF#  eq 'F'>
                <input type="submit" class="btnAplicar" tabindex="1" name="btnAplicar"  value="Aplicar" 	 onClick="javascript: fnNoValidar(); return Postear();">
					</cfif>
                <input type="submit" class="btnNormal"  tabindex="1" name="Calcular"    value="Ver" 		 onClick="javascript: fnNoValidar();return true;">
...
<!---	<input type="hidden" name="Icodigo" value="<cfif isdefined('form.Icodigo')>#form.Icodigo#</cfif>" />
--->	</cfoutput>
	<!--- ======================================================================= --->
	<!--- ======================================================================= --->
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<form action="<cfoutput>#root#</cfoutput>" method="post" name="form2">
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
									<tr bgcolor="E2E2E2" class="subTitulo">
										...
										<td width="40%" valign="bottom" align="center"><strong>&nbsp;Centro Funcional</strong></td>
									...
									<cfoutput query="rsLineas"> 
										<tr onClick="javascript:Editar('#rsLineas.IDdocumento#|#rsLineas.Linea#');" 
											...
											<td align="center">#rsLineas.CFdescripcion#</td>
			...
			<script language="JavaScript1.2">
				function Editar(data) {
					if (data!="") {
						//document.form2.action='RegistroDocCP.cfm?tipo=<cfoutput>#form.tipo#</cfoutput>';
						<cfif isdefined("form.tipo") and len(trim(form.tipo)) >
							document.form2.action='<cfoutput>#root#</cfoutput>';
						</cfif>
                    ...
                    function borrar(documento, linea){ 
                        ...
                        document.form2.action = "SQLRegistroFacturasCP.cfm";	
...
<cfoutput query="direcciones"> 
	...
	{
	   IDdocumento = document.form1.IDdocumento.value;			
       window.open('OrdenesFacturadas.cfm?documento='+IDdocumento,'popup','width=1100,height=350,left=100,top=50,scrollbars=yes');
	}
</script>
<script language="javascript" type="text/javascript">
