...
<cfquery name="conlis" datasource="#Session.DSN#">
	SELECT c.IACdescripcion, d.Cformato, d.Cdescripcion, a.Adescripcion, a.Acodigo,
			<cf_dbfunction name="to_char" args="a.Aid"> as Aid,
			<cf_dbfunction name="to_char" args="c.IACinventario"> as IACinventario,
			<cf_dbfunction name="to_char" args="b.IACcodigo"> as IACcodigo,
			<cf_dbfunction name="to_char" args="d.Ccuenta"> as Ccuenta
...
function Asignar(valor1, valor2, valor3, valor4, valor5) {
	...
	window.opener.document.<cfoutput>#url.form#</cfoutput>.CmayorD_CformatoD.value=valor4.substring(0,4);
	if (valor4.length > 4) {
...
<form action="" method="post" name="conlis">
  <table width="53%" border="0" cellspacing="0">
    <tr>
      <td width="43%" nowrap class="tituloListas"><div align="left">Código</div></td>
      <td width="23%" nowrap  class="tituloListas"><div align="left">Descripción</div></td>
      <td width="34%" nowrap class="tituloListas"><div align="right">
    ...
    <tr>
      ...
      <td colspan="2" nowrap>
        <input name="Adescripcion" type="text" size="40" maxlength="60">
      </td>
    </tr>
    <cfoutput query="conlis" startRow="#StartRow_conlis#" maxRows="#MaxRows_conlis#">
      <tr>
        <td nowrap <cfif conlis.CurrentRow MOD 2><cfoutput>class="listaPar"</cfoutput><cfelse><cfoutput>class="listaNon"</cfoutput></cfif>>
			<input type="hidden" name="Acodigo#conlis.CurrentRow#" value="#HTMLEditFormat(conlis.Acodigo)#">
			<input type="hidden" name="IACinventario#conlis.CurrentRow#" value="#HTMLEditFormat(conlis.IACinventario)#">
			<input type="hidden" name="Cformato#conlis.CurrentRow#" value="#HTMLEditFormat(conlis.Cformato)#">
			<input type="hidden" name="Cdescripcion#conlis.CurrentRow#" value="#HTMLEditFormat(conlis.Cdescripcion)#">
	    <a href="javascript:Asignar(#conlis.Aid#, '#JSStringFormat(HTMLEditFormat(conlis.Adescripcion))#', '#conlis.IACinventario#', '#JSStringFormat(conlis.Cformato)#', '#HTMLEditFormat(JSStringFormat(conlis.Cdescripcion))#');">#HTMLEditFormat(conlis.Acodigo)#</a>		</td>
        <td colspan="2" nowrap <cfif #conlis.CurrentRow# MOD 2><cfoutput>class="listaPar"</cfoutput><cfelse><cfoutput>class="listaNon"</cfoutput></cfif>>
          <a href="javascript:Asignar(#conlis.Aid#, '#JSStringFormat(HTMLEditFormat(conlis.Adescripcion))#', '#conlis.IACinventario#', '#JSStringFormat(conlis.Cformato)#', '#HTMLEditFormat(JSStringFormat(conlis.Cdescripcion))#');">#HTMLEditFormat(conlis.Adescripcion)#</a> </td>
      </tr>
    </cfoutput>
    <tr>
      <td colspan="3">&nbsp; </td>
