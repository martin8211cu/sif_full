...
<cfif isDefined('url.cambioMasivoFechaEvento')>
    <cfset fechaMasiva = url.fechaMasiva>
	<cfif isdefined("form.chk")>
	    <cftransaction>
	    <cfloop list="#form.chk#" index="ncid">
            <cfquery name="rsDeleteEDocCP" datasource="#session.DSN#">
                update EDocumentosCxP
                set EDFecha = <cfqueryparam cfsqltype="cf_sql_date" value="#LSDateFormat(fechaMasiva,'YYYY/MM/DD')#">
                where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">
                and IDdocumento = <cfqueryparam cfsqltype="cf_sql_numeric" value="#ncid#">
            </cfquery>
        </cfloop>
		</cftransaction>
	</cfif>
...
<cffunction name="validacionIEPSEscalonado">
    ...
    <cfquery name="detalleImpuestos" datasource="#Session.DSN#">
        Select b.ieps, b.IEscalonado from DDocumentosCxP a
            inner join Impuestos b
                on a.CODIEPS = b.ICODIGO and a.Ecodigo=b.Ecodigo
...
<cfelseif isdefined("Form.BorrarE")>
    ...
    <cfif isdefined("rsTieneInclusion") and rsTieneInclusion.registros gt 0 >
        <cfthrow message="El documento tiene Documentos de Inclusion Relacionados no se puede Borrar">
    </cfif>
	...
	<cftransaction>
		...
		<cfquery name="rsDeleteLineaRem" datasource="#session.DSN#">
			UPDATE DDocumentosCPR SET DFacturalinea = null
				FROM DDocumentosCxP dcxp
				INNER JOIN DDocumentosCPR dcpr ON dcxp.linea = dcpr.dfacturalinea
				WHERE dcxp.IDdocumento = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.IDdocumento#">
		</cfquery>
        <cfquery name="rsDeleteEDocCP" datasource="#session.DSN#">
		...
		<!---Actualizar a null Asociacion Manual y Relación de un XML--->
			<cfquery name="rsExisteDBitacoraDescargaSAT" datasource="#session.DSN#">
				...
				where Ecodigo  =  #Session.Ecodigo#
				and UUID_Factura=<cf_jdbcquery_param cfsqltype="cf_sql_varchar"  value="#Form.Timbre#">
			</cfquery>
...
<cfelseif isdefined("Form.AgregarD")>
	...
	<cftransaction>
			<cfquery name="rsInsertDDocCP" datasource="#session.DSN#">
                ...
                <cfqueryparam cfsqltype="cf_sql_money" value="#replace(Form.DDMieps,",","","all")#">,
			<!---JMRV. Inicio del Cambio. 30/04/2014 --->
				<cfif isdefined("form.DDtipo") and (Form.DDtipo eq 'A' or Form.DDtipo eq 'S') and isdefined("form.CheckDistribucionHidden") and form.CheckDistribucionHidden is 1 and isdefined("form.PlantillaDistribucion") and form.PlantillaDistribucion is -1>
					<cfthrow message="No ha seleccionado una distribucion.">
				<cfelseif isdefined("form.DDtipo") and (Form.DDtipo eq 'A' or Form.DDtipo eq 'S') and isdefined("form.CheckDistribucionHidden") and form.CheckDistribucionHidden is 1>
...
<cfelseif isdefined("Form.BorrarD")>
		...
		<cfif getOrden.recordCount GT 0 >
			...
			<cfquery name="rsDeleteLineaRem" datasource="#session.DSN#">
			    update DDocumentosCPR set DFacturalinea = null
				where DFacturalinea = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.Linea#">
			</cfquery>
		</cfif>
...
<cfelseif isdefined("Form.Cambiar")>
    ...
    <cfset modo="CAMBIO">
    <cfset modoDet="ALTA">
<cfelseif isdefined("Form.chkChgTimbre") and Form.chkChgTimbre EQ 1>
    <!--- <cf_Dump vaR="#form#"> --->
    <cfquery name="rsUpdateEDocCxP" datasource="#session.DSN#">
            update EDocumentosCxP set
            <cfif isDefined("Form.Timbre") and Form.Timbre NEQ "undefined" and Form.Timbre NEQ "">
                TimbreFiscal  = <cf_jdbcquery_param cfsqltype="cf_sql_varchar"  value="#Form.Timbre#">
            <cfelse>
                TimbreFiscal = null
            </cfif>
...
<!---JMRV. Fin del Cambio. 30/04/2014 --->
        ...
        <cfquery name="rsUpdateDDocCxP" datasource="#session.DSN#">
            ...
            <cfif  isDefined("Form.DDdescripcion") and  len(trim(form.DDdescripcion)) neq 0>
                ...
                DDMontoIeps = <cfqueryparam cfsqltype="cf_sql_money" value="#replace(Form.DDMieps,",","","all")#">,
	...
	<cfif isdefined("form.chk") and len(trim(form.chk)) GT 0>
		<cfloop list="#form.chk#" index="chk">
			...
			<cfif rsSQL.cantidad LT 1>
				<cfset Msg_NoExLin = t.Translate('Msg_NoExLin','No Existen Líneas para el Documento (DDdescripcion) Seleccionado. Proceso Cancelado!')>
				<cf_errorCode	code = "51153" msg = "#Msg_NoExLin#">
			</cfif>
		</cfloop>
        ...
        <cftransaction>
			<cfloop list="#form.chk#" index="item">
                ...
                <cfif NOT rsEstado.RecordCount>
					<cfset Msg_NoDefEdoCta = t.Translate('Msg_NoDefEdoCta','No se ha definido el estado de la factura (Enviar a aprobar)')>
                    <cfthrow message="#Msg_NoDefEdoCta#">
                </cfif>
				...
				<cfquery name="rsRetencionMonto" datasource="#session.dsn#">
					...
					from EDocumentosCxP e
						left join Retenciones r
						 on r.Ecodigo = e.Ecodigo
						and r.Rcodigo = e.Rcodigo
					where e.IDdocumento = <cfqueryparam cfsqltype="cf_sql_numeric" value="#item#">
				</cfquery>
                ...
                <cfquery name="datosTimbre" datasource="#session.dsn#">
                    ...
                    where Ecodigo  =  <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">
                    and Origen = 'CPFC'
                      and ID_Documento = <cfqueryparam cfsqltype="cf_sql_numeric" value="#item#">
                </cfquery>
				...
				and datosTimbre.TipoComprobante EQ 1 and datosTimbre.totalCFDI GT 0>
                    ...
                    <cfset totalFactura=rsDato.EDtotal>
                    ...
                    <cfif totalTimbre NEQ totalFactura>
                        ...
                        <!---En CCOM se dejo una tolerancia de 1.00 desarrollar esta tolerancia como un parametro ajustable--->
                ...
                <!---<cfquery name="rsDFacturas" datasource="#session.dsn#">
                    select s.CMSid, s.CMSnombre, ef.EDdocumento, ds.DSlinea, ds.ESnumero, do.EOnumero, coalesce(des.DEemail, dps.Pemail1, dps.Pemail2) as EmailS,
                        case ds.DStipo
                          when 'A' then a.Adescripcion
                          when 'S' then c.Cdescripcion
                          when 'F' then ds.DSdescripcion
                        end as Descripcion, do.DOcantidad,
                        cm.CMCnombre, coalesce(dec.DEemail, dpc.Pemail1, dpc.Pemail2) as EmailC
                    from EDocumentosCxP ef
                        inner join DDocumentosCxP df
                            on df.IDdocumento = ef.IDdocumento
                        inner join DOrdenCM do
                            on do.DOlinea = df.DOlinea
                        inner join DSolicitudCompraCM ds
                            on ds.DSlinea = do.DSlinea and ds.ESidsolicitud = do.ESidsolicitud
                        ...
                        inner join Usuario us
                       	...
                       	inner join CMCompradores cm
                        	on cm.CMCid = es.CMCid
                        ...
                        where ef.IDdocumento = <cfqueryparam cfsqltype="cf_sql_numeric" value="#item#">
                        order by s.CMSnombre
                    </cfquery>
                    <cfquery name="rsSocios" dbtype="query">
                    	select distinct CMSid, CMSnombre, EmailS, EmailC, CMCnombre, EDdocumento from rsDFacturas
                    </cfquery>
                    <cfloop query="rsSocios">
                    	<cfquery name="rsLineas" dbtype="query">
                    		select EDdocumento, EOnumero, ESnumero, DOcantidad, Descripcion from rsDFacturas where CMSid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsSocios.CMSid#">
                    	</cfquery>
                        <cfset email_body = fnEmail(rsSocios.CMSnombre, rsSocios.EDdocumento, rsLineas)>
                        <cfif len(trim(rsSocios.EmailS)) gt 0>
							<cfquery datasource="#session.dsn#">
								...
								values (
									...
									<cfqueryparam cfsqltype="cf_sql_varchar" value='#rsSocios.EmailS#'>,
									<cfqueryparam cfsqltype="cf_sql_varchar" value="Recibo en Bodega">,
									<cfqueryparam cfsqltype="cf_sql_varchar" value="#email_body#">, 1)
							</cfquery>
						</cfif>
                        <cfset email_body = fnEmail(rsSocios.CMCnombre, rsSocios.EDdocumento, rsLineas)>
                        <cfif len(trim(rsSocios.EmailC)) gt 0>
							<cfquery datasource="#session.dsn#">
								insert into SMTPQueue (SMTPremitente, SMTPdestinatario, SMTPasunto, SMTPtexto, SMTPhtml)
								values (
									<cfqueryparam cfsqltype="cf_sql_varchar" value='gestion@soin.co.cr'>,
									<cfqueryparam cfsqltype="cf_sql_varchar" value='#rsSocios.EmailC#'>,
									<cfqueryparam cfsqltype="cf_sql_varchar" value="Recibo en Bodega">,
									<cfqueryparam cfsqltype="cf_sql_varchar" value="#email_body#">, 1)
							</cfquery>
						</cfif>
...
<!---►►►►►►►►►►ENVIAR A EMAIl, COMPRADOR Y SOLICITANTE◄◄◄◄◄◄◄◄◄◄ --->
<cfif isDefined("Form.btnEnviarEmail")>
	<cfif (NOT ISDEFINED('form.chk') OR NOT LEN(TRIM(form.chk))) AND (ISDEFINED('form.IDdocumento') AND LEN(TRIM(form.IDdocumento)))>
    	<CFSET form.chk = form.IDdocumento>
    </cfif>
	<cfif isdefined("form.chk") and len(trim(form.chk)) GT 0>
		<cfloop list="#form.chk#" index="chk">
			...
			<cfif rsSQL.cantidad LT 1>
				<cfset Msg_NoExisLin = t.Translate('Msg_NoExisLin','No Existen Líneas para el Documento (DDdescripcion) Seleccionado. Proceso Cancelado!')>
				<cf_errorCode	code = "51153" msg = "#Msg_NoExisLin#">
			</cfif>
		</cfloop>
        <cftransaction>
			<cfloop list="#form.chk#" index="item">
                <cfquery name="rsDFacturas" datasource="#session.dsn#">
                    ...
                    <cfloop query="rsSocios">
                    	<cfquery name="rsLineas" dbtype="query">
                    		select CFdescripcion, EDdocumento, EOnumero, ESnumero, DOcantidad, Descripcion from rsDFacturas where CMSid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsSocios.CMSid#">
                    	</cfquery>
                        <cfset email_body = fnEmail(rsSocios.CMSnombre, rsSocios.EDdocumento, rsLineas)>
                        <cfif len(trim(rsSocios.EmailS)) gt 0>
							<cfquery datasource="#session.dsn#">
	...
	<cfif isdefined("form.chk") and len(trim(form.chk)) GT 0>
		<cftransaction>
        	<cfloop list="#form.chk#" index="item">
                ...
                <cfif NOT rsEstado.RecordCount>
					<cfset Msg_NosehadefEdoCta = t.Translate('Msg_NosehadefEdoCta','No se ha definido el estados de la factura (Devuelta)')>
                    <cfthrow message="#Msg_NosehadefEdoCta#">
                </cfif>
                <cfquery datasource="#session.DSN#">
                    update EDocumentosCxP
                    set EVestado 	  = 3
...
<cfif isDefined("Form.btnAplicar")>
    ...
    <!--- JARR revisa el parametro de validacion de UUID 17100--->
    <cfquery name="rspVUUID" datasource="#session.DSN#">
		...
		<cftransaction>
        	...
        	<cfloop list="#form.chk#" index="item">
                ...
                <!---Se inserta el UUID si no se inserto  --->
                 <cfif isDefined("rsDato") and len(trim(rsDato.TimbreFiscal))>
                    <cfquery name="rsTimbre" datasource="#session.dsn#">
                        select * from CERepoTMP
                        where Ecodigo    = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">
                        and Origen = 'CPFC'
                        and ID_Documento = <cfqueryparam cfsqltype="cf_sql_numeric" value="#item#">
                        and TimbreFiscal ='#rsDato.TimbreFiscal#'
                    </cfquery>
                    <cfif isDefined("rsTimbre") and rsTimbre.recordcount EQ 0>
                        ...
                        values(#Session.Ecodigo#,'#rsDato.TimbreFiscal#',#item#,'#rsDato.EDdocumento#','CPFC',
                            0.0,#Session.Usucodigo#,(select Miso4217 from Monedas where Ecodigo = #Session.Ecodigo# and Mcodigo=#rsDato.Mcodigo#),#rsDocumento.Mcodigo#,#rsDato.EDtipocambio#)
                        </cfquery>
                    <cfelseif rsTimbre.Documento EQ ''>
                        <cfquery datasource="#session.dsn#">
                            update CERepoTMP
                                set Documento = '#rsDato.EDdocumento#'
                            where Ecodigo    = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">
                            and Origen = 'CPFC'
                            and ID_Documento = <cfqueryparam cfsqltype="cf_sql_numeric" value="#item#">
                        </cfquery>
                ...
                <cfif NOT rsEstado.RecordCount>
					<cfset Msg_NosehadefEdoCtaAplic = t.Translate('Msg_NosehadefEdoCtaAplic','No se ha definido el estados de la factura (Aplicada)')>
                    <cfthrow message="#Msg_NosehadefEdoCtaAplic#">
                </cfif>
                <cfquery name="rsInsertaEventoA" datasource="#session.dsn#">
                    insert into EventosFact (EVfactura,EVestado,SNcodigo,CPTcodigo,EVObservacion,BMUsucodigo,Ecodigo,BMfecha)
                        values(
                            <cf_jdbcquery_param cfsqltype="cf_sql_char" 	 value="#rsDato.EDdocumento#">,
                            <cf_jdbcquery_param cfsqltype="cf_sql_integer"   value="#rsEstado.FTidEstado#">,
                            <cf_jdbcquery_param cfsqltype="cf_sql_integer"   value="#rsDato.SNcodigo#">,
                            <cf_jdbcquery_param cfsqltype="cf_sql_char"      value="#rsDato.CPTcodigo#">,
                            <cf_jdbcquery_param cfsqltype="cf_sql_varchar"   value="null">,
                            <cf_jdbcquery_param cfsqltype="cf_sql_integer" 	 value="#Session.Usucodigo#">,
                ...
                <cf_dbidentity2 datasource="#Session.DSN#" name="rsInsertaEventoA" verificar_transaccion="false" returnvariable="EVid">
                    ...
                    <cfset totalFactura=rsDato.EDtotal>
                    <!--- VAlida UUId --->
                    <cfif isdefined("rspVUUID") and rspVUUID.Pvalor GT 0>
                        <cfquery name="rsValUID" datasource="#session.DSN#">
                            select count(*) as cant from CERepoTMP 
                                where TimbreFiscal = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsDato.TimbreFiscal#">
                                <!---and xml is not null
                                and xmlTimbrado is not null --->
                                and ID_Documento = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsDato.IDdocumento#">
                                and Ecodigo    = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">
                                and Origen = 'CPFC'
                                and Documento = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsDato.EDdocumento#">
                        </cfquery>
                        <cfif isdefined('rsValUID') and rsValUID.cant eq 0 >
                             <cfset Request.Error.URL='#URLira#?datos=#rsDato.IDdocumento#&tipo=#form.tipo#'>
                             <cfset Request.Error.mycolor='blue'>
                             <cf_errorCode  code = "50888" msg = "No se ha registrado el UUID para completar la aplicación de la Factura.">
                            <!--- <cfthrow message="No se ha registrado el UUID para completar la aplicación de la Factura."> --->
                            <cfabort>
                        </cfif>    
                    </cfif>
                    <cfif isdefined('datosTimbre') and datosTimbre.RecordCount GT 0 and datosTimbre.TipoComprobante EQ 1 and datosTimbre.totalCFDI GT 0>
						<cfset totalTimbre=datosTimbre.totalCFDI>
                        <cfset totalFactura=rsDato.EDtotal>
                        <cfset RetencionFactura=rsRetencionMonto.Monto>
						<cfif RetencionFactura GT 0>
	...
	<cfif isDefined("Form.chk")>
		...
		<cfloop index="CountVar" from="1" to="#cuantos#">
			...
			<cfif parametroRec.Pvalor neq 0>
				...
				<cfif len(trim(recurrente.IDdocumentorec))>
					...
					<cfif len(trim(rsFechaUltima.HEDfechaultaplic)) and datecompare(fechaaplic, rsFechaUltima.HEDfechaultaplic) lte 0>
						<cfset request.Error.backs = 1 >
						<cfset Msg_ElDocNoPuedeSerAplic = t.Translate('Msg_ElDocNoPuedeSerAplic','El documento no puede ser aplicado, pues ya existe un documento aplicado con el mismo documento recurrente para el mes')>
						<cfset Msg_yPeriodo = t.Translate('Msg_yPeriodo','y período')>
						<cf_errorCode	code = "50344"
							msg  = "#Msg_ElDocNoPuedeSerAplic# @errorDat_1@ #Msg_yPeriodo# @errorDat_2@."
			...
			<!---<cfset LobjInterfaz.fnProcesoNuevoSoin(110,"CXTcodigo=#rsDatos.CXTcodigo#&Ddocumento=#rsDatos.Ddocumento#&SNcodigo=#rsDatos.SNcodigo#&MODULO=CP","R")>---><!---SML. 25/06/2014 Se comento, ya que en Colegio de Bachilleres se tiene conflictos al aplicar las factura.--->
...
<cffunction name="cambiarEncabezado" returntype="void">
	...
	<cfif not existe>
		...
		<cfquery name="rsUpdateEDocCxP" datasource="#session.DSN#">
            ...
            <cfif isDefined("Form.Timbre") and Trim(Form.Timbre) NEQ "">
                ,TimbreFiscal  = <cf_jdbcquery_param cfsqltype="cf_sql_varchar"  value="#Form.Timbre#">
            <cfelse>
                ,TimbreFiscal=null
            </cfif>
		...
		<!---Asociacion Manual y Relación de un XML--->
			<cfquery name="rsExisteDBitacoraDescargaSAT" datasource="#session.DSN#">
				select Relacionado ,UUID_Factura from DBitacoraDescargaSAT
				where Ecodigo  =  #Session.Ecodigo#
				and UUID_Factura=<cf_jdbcquery_param cfsqltype="cf_sql_varchar"  value="#Form.Timbre#">
			...
			<cfif rsExisteDBitacoraDescargaSAT.recordCount GT 0>
					<cfquery datasource="#session.DSN#">
							update DBitacoraDescargaSAT
							set Relacionado = <cfqueryparam cfsqltype="cf_sql_bit"  value="1">
							where Ecodigo  	=  #Session.Ecodigo#
							and UUID_Factura=<cf_jdbcquery_param cfsqltype="cf_sql_varchar"  value="#Form.Timbre#">
					</cfquery>
			</cfif>
