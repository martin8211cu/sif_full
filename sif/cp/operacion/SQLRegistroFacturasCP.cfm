...
<cfif isdefined("url.OP") AND url.OP EQ "GENCF">
	...
	<cfset LvarCFformato = mascara.fnComplementoItem(session.Ecodigo, url.CFid, "S", "", url.Cid, "", "",url.CFComplemento)>
	...
	<script language="javascript" type="text/javascript">
		...
		<cfelse>
			window.parent.fnSetCuenta ("#rsTraeCuenta.Ccuenta#","#rsTraeCuenta.CFcuenta#","#trim(rsTraeCuenta.CFformato)#","#trim(rsTraeCuenta.CFdescripcion)#");
		</cfif>
	</script>
	</cfoutput>
	<cfabort>
 	...
 	<cfif isdefined("Form.AgregarE") >
		...
		<cftransaction>
 			<cfif not existe>
                ...
                <cfif NOT rsEstado.RecordCount>
                	<cfthrow message="No se han definido los estados de la factura">
                </cfif>
				...
				<cfquery name="rsInsertEDocCP" datasource="#session.DSN#">
					insert into EDocumentosCxP (Ecodigo, CPTcodigo, EDdocumento, SNcodigo, Mcodigo, EDtipocambio,
												...
												Rcodigo, EDusuario, EDselect, EDdocref, EDfechaarribo, id_direccion, TESRPTCid, BMUsucodigo,TESRPTCietu,EVestado,EDvencimiento)
		...
		<cfelseif isdefined("Form.AgregarD")>	
 			...
 			<cfif cambioEncab> 	
				...
				redirect="RegistroFacturasxUsuarioCP.cfm"
...
--->				</cfif>
			...
			<cfquery name="rsInsertDDocCP" datasource="#session.DSN#">
				...
				values (
					...
					<cfelse>
                    	<cf_jdbcquery_param cfsqltype="cf_sql_date" value="null">,
                    </cfif>
					<cfif  isDefined("Form.DDtipo") and (Form.DDtipo eq 'T' or Form.DDtipo eq 'O')>
			...
			<cftransaction>
				...
				<cfquery name="rsUpdateDDocCxP" datasource="#session.DSN#">
					...
					<cfif  isDefined("Form.DDtipo") and (Form.DDtipo eq 'T' or Form.DDtipo eq 'O')>
						...
						<cfif isDefined("Form.Icodigo") and Len(Trim(Form.Icodigo)) GT 0 > 
							, Icodigo =	<cfqueryparam cfsqltype="cf_sql_char" value="#rtrim(ltrim(Form.Icodigo))#">
						<cfelse>	
...
<cffunction name="cambiarEncabezado" returntype="void">
	...
	<cfquery name="rsUpdateEDocCxP" datasource="#session.DSN#">
		update EDocumentosCxP set
			...
			EDtipocambio = <cfqueryparam cfsqltype="cf_sql_float"   value="#Form.EDtipocambio#">,
			EDdescuento  = <cfqueryparam cfsqltype="cf_sql_money"   value="#Form.EDdescuento#">,					
			EDimpuesto   = <cfqueryparam cfsqltype="cf_sql_money"   value="#Form.EDimpuesto#" scale="2">,
			EDtotal      = <cfqueryparam cfsqltype="cf_sql_money"   value="#Form.EDtotal#"    scale="2">,									
			Rcodigo 	 = 
				<cfif isDefined("Form.Rcodigo") and Trim(Form.Rcodigo) NEQ "-1">
					<cfqueryparam cfsqltype="cf_sql_char" value="#Form.Rcodigo#">,
				<cfelse>
