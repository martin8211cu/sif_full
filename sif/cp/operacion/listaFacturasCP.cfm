...
<cfif isdefined('AplicaFact')>
	<cfset AplicaFact = true>
<cfelse>
	<cfset AplicaFact = false>
</cfif>
...
<cfif ( isDefined("url.tipoF") and len(Trim(url.tipoF)) gt 0 ) and not isdefined("form.tipoF")>
...
<cfif isdefined("tipoF") and #tipoF#  eq 'A'>
	<cfset filtro = "a.Ecodigo = #Session.Ecodigo# and a.EVestado not in (1,3) "> 
	
<cfelseif isdefined("tipoF") and #tipoF#  eq 'C'>
	<cfset filtro = "a.Ecodigo = #Session.Ecodigo# and a.BMUsucodigo=#session.Usucodigo# and (a.EVestado in (1,3) or a.EVestado is null)"> 
...
<cfif isdefined("tipoF") and #tipoF#  eq 'A'>
	<cfset botones = "Aplicar,Anular"> 
	
<cfelseif isdefined("tipoF") and #tipoF#  eq 'C'>
	<cfset botones = "Enviar_a_Aplicar,Nuevo,Importar_Facturas,Importar_Transito,Reporte"> 
	
<cfelseif isdefined("tipoF") and #tipoF#  eq 'D'>
	<cfset botones = "Aplicar,Nuevo,Importar_Facturas,Importar_Transito,Reporte">
...
<cfsetting enablecfoutputonly="no">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr> 
			<td> 
				<form style="margin: 0" name="filtros" action="#root#" method="post">
	  ...
	  <tr> 
			...
			<td>
				<cfif isdefined("form.tipoF") and len(trim(form.tipoF)) and form.tipoF EQ 'D'>
				<!--- D = Notas de Credito en CxP --->
				...
				<cfelseif isdefined("form.tipoF") and len(trim(form.tipoF)) and form.tipoF EQ 'C' or form.tipoF EQ 'A' or form.tipoF EQ 'F'> <!--- C = Facturas en CxP  --->
					...
					<cfinvoke component="sif.Componentes.pListas" method="pListaRH" 
						...
						columnas="a.IDdocumento as IDdocumento, 
								...
								m.Miso4217 as Mnombre,
								a.EDtotal as EDtotal ,a.EVestado as EVestado
								#campos_extra#" 
						desplegar="CPTdescripcion, EDdocumento, EDusuario, EDfecha, Mnombre, EDtotal"
						etiquetas="Transacción, Documento, Usuario, Fecha, Moneda, Total"
						formatos="S,S,S,S,S,M"
						...
						checkboxes="S"
						nuevo="#root#"
						ira="#root#?AplicaFact=#AplicaFact#&tipoF=#tipoF#"
						botones="#botones#"
						keys="IDdocumento"
						MaxRows="#Registros#"
						incluyeForm ="true"
...
function funcAplicar(){
		if (!Marcar2()){
			alert("Sepresentaron los siguientes errores:\n - Debe marcar uno o más registros, de la lista para Aplicar.");
			return false;
		}else{
			//document.lista.action='<cfoutput>#root#</cfoutput>';
			//document.lista.root.value = "#root#";
			//alert(document.lista.rootAux)
			document.lista.action= "SQLRegistroFacturasCP.cfm?root=<cfoutput>#root#</cfoutput>";
	...
	function funcAnular(){
		if (!Marcar2()){
			alert("Sepresentaron los siguientes errores:\n - Debe marcar uno o más registros, de la lista para Anular.");
			return false;
		}else{
			document.lista.action= "SQLRegistroFacturasCP.cfm?root=<cfoutput>#root#</cfoutput>";
			document.lista.submit();
	...
	function funcEnviar_a_Aplicar(){
		if (!Marcar2()){
			alert("Sepresentaron los siguientes errores:\n - Debe marcar uno o más registros, de la lista para Enviar a Aplicar.");
			return false;
		}else{
			//document.lista.action='<cfoutput>#root#</cfoutput>';
			document.lista.action= "SQLRegistroFacturasCP.cfm?root=<cfoutput>#root#</cfoutput>";
...
<!---	document.lista.TIPO.value = "<cfoutput>#trim(form.tipo)#</cfoutput>";
	...
	<cfif isdefined("form.moneda") and len(trim(form.moneda)) >
		document.lista.MONEDA.value = '#form.moneda#';
			...
			document.lista.action= "SQLRegistroFacturasCP.cfm";
			document.lista.submit();
}--->

function funcImportar_Facturas(){
	location.href = "ImportadorMateriaPrima.cfm";
	return false;
}

function funcImportar_Transito(){
	location.href = "ImportadorCrudo.cfm";
	return false;
}
