...
<cfset LB_ListadeProv = t.Translate('LB_ListadeProv','Lista de Proveedores')>
...
function limpiar() {
	window.opener.document.<cfoutput>#url.form#</cfoutput>.EDdocref.value = "";
	window.opener.document.<cfoutput>#url.form#</cfoutput>.Referencia.value = "";	
	window.opener.document.<cfoutput>#url.form#</cfoutput>.Referencia1.value = "";
}
</script>
...
<form action="" method="post" name="conlis">
  <table width="53%" border="0" cellspacing="0">
  	<cfoutput>
    <tr> 
      <td width="44%"  class="tituloListas"><div align="left">#LB_PROVEEDOR#</div></td>
      <td width="28%"class="tituloListas"><div align="left">#LB_Nombre#</div></td>
      <td width="1%" class="tituloListas"><div align="right">
          <input type="submit" name="Filtrar" value="#LB_btnFiltrar#">
          </div></td>
    </tr>
    </cfoutput>
    <tr> 
