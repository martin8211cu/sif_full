...
<cfif isdefined("url.AcodigoIni") and not isdefined("form.AcodigoIni")>
	<cfset form.AcodigoIni = url.AcodigoIni>
</cfif>
<cfif isdefined("url.AcodigoFin") and not isdefined("form.AcodigoFin")>
	<cfset form.AcodigoFin = url.AcodigoFin>
</cfif>
...
<cfif isdefined("url.ERdocumento") and not isdefined("form.ERdocumento")>
...
from HERequisicion a
	...
	order by upper(art.Acodigo), cf.CFcodigo, a.ERFecha, a.ERdescripcion
</cfquery>
		...
		<td colspan="7">
			<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px; padding-right: 5px;" align="center" class="areaFiltro">
				...
				<tr > 
					<td colspan="7" align="center">
							<cfif isdefined("form.FechaDes") and len(trim(form.FechaDes)) and isdefined("form.FechaHas") and len(trim(form.FechaHas))>
								<cfif form.FechaDes neq form.FechaHas >
									<font size="2"><strong>Fecha Desde:  #form.FechaDes# - Fecha Hasta:  #form.FechaHas#</strong></font>
								<cfelse>
									<font size="2"><strong>Fecha: #form.FechaDes#</strong></font>
								</cfif>
							<cfelseif isdefined("form.FechaDes") and len(trim(form.FechaDes))>
								<font size="2"><strong>Fecha Desde: #form.FechaDes#</strong></font>
							<cfelseif isdefined("form.FechaHas") and len(trim(form.FechaHas))>
								<font size="2"><strong>Fecha Hasta: #form.FechaHas#</strong></font>
							<cfelse>
								<font size="1"><strong>Sin rango de fechas</strong></font>
							</cfif>
					</td>
				</tr>
				</cfoutput>
	...
	<cfif data.RecordCount gt 0>
		...
		<cfoutput query="data" group="Codigo">
				...
				<cfoutput group="CFcodigo">
					...
					<tr>
						<td colspan="4" align="right"><strong>Total Art&iacute;culo #Codigo# - #Descripcion#, para el Centro Funcional #CFcodigo# - #CFdescripcion#</strong></td>
						<td align="right"><strong>#LSNumberFormat(TotalArt,",9.0000")#</strong></td>
					</tr>
			</cfoutput>
		</cfoutput>
		<tr><td>&nbsp;</td></tr>
		<tr><td colspan="6" align="center"><strong>------ Fin del Reporte ------</strong></td></tr>
	<cfelse>
		<tr><td>&nbsp;</td></tr>
		<tr><td colspan="6" align="center"><strong>--- No se encontraron Registros ---</strong></td></tr>
	</cfif>
</table>
<br>
