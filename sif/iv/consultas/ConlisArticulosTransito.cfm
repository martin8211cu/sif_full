...
<cfquery name="conlis" datasource="#session.DSN#">
	...
	and Acodigo in ( select distinct Acodigo 
				 from Transito 
				 where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#"> 
			   )
...
<form action="" method="post" name="conlis">
  <table width="100%" border="0" cellspacing="0">
    <tr> 
      ...
      <td width="1%" class="tituloListas"><div align="right"><input type="submit" name="Filtrar" value="Filtrar">
          </div></td>
    </tr>
    ...
    <cfoutput query="conlis" startRow="#StartRow_conlis#" maxRows="#MaxRows_conlis#"> 
      <tr> 
        <td <cfif #conlis.CurrentRow# MOD 2><cfoutput>class="listaPar"</cfoutput><cfelse><cfoutput>class="listaNon"</cfoutput></cfif>><input type="hidden" name="Acodigo#conlis.CurrentRow#" value="#conlis.Acodigo#"> 
          <a href="javascript:Asignar('#conlis.Aid#', '#JSStringFormat(conlis.Adescripcion)#' );">#conlis.Acodigo#</a></td>
        <td colspan="2" nowrap <cfif #conlis.CurrentRow# MOD 2><cfoutput>class="listaPar"</cfoutput><cfelse><cfoutput>class="listaNon"</cfoutput></cfif>><a href="javascript:Asignar('#conlis.Aid#', '#JSStringFormat(conlis.Adescripcion)#');">#conlis.Adescripcion#</a></td>
      </tr>
    </cfoutput> 
    <tr> 
      <td colspan="3">&nbsp; </td>
