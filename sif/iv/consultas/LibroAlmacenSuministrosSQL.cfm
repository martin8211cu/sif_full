...
<cf_dbfunction name="spart" args="c.Cformato,1,14" returnvariable ="Cuenta1"> 
...
<cfif #Url.Periodo# lt year(NOW()) || #Url.Mes# lt month(NOW())>
	<cfset Fecha = #DateFormat(DateAdd("m",1,CreateDate(#url.Periodo#,#url.mes#,01))-1,"dd/mm/yyyy")#>
<!---<cfelse>
	<cfif #Url.Mes# lt month(NOW())>
    	<cfset Fecha = #DateFormat(CreateDate(#url.periodo#,#url.mes#+1,01)-1,"dd/mm/yyyy")#>--->
    <cfelse>
        <cfset Fecha = #DateFormat(CreateDate(year(NOW()),month(NOW()),day(NOW())),"dd/mm/yyyy")#>
   <!--- </cfif>--->
</cfif>
...
<!---<cfif Url.Mostrar eq 1>--->
	...
	<!---</cfif>	--->
				Select 
						...
						descCuenta = (select Cdescripcion from CContables where Cformato = substring (c.Cformato, 1, 14))
...
<!---						<cfif Url.Mostrar eq 2>
			...
			<!---<cfif Url.Mostrar eq 1>			--->
			 ...
			 group by Oficina, Cuenta, descCuenta, Empresa		
...
<!---    <cfif Url.Mostrar eq 1><!--- Mostrar resumido --->--->
        <cfset lvarTemplate = "LibroAlmacenSuministros.cfr">
    
<!---    </cfif>--->
    <!--- INVOCA EL REPORTE --->
    <cfreport format="#formatos#" template= "#lvarTemplate#" query="rsReporte">
        <cfreportparam name="Fecha" 	value="#Fecha#">
