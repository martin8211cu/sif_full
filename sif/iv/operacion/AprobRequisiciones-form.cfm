...
<cfif (modo neq 'ALTA')>
	...
	<cfquery datasource="#session.DSN#" name="rsForm">
		select a.ERid, a.ERdescripcion, a.Aid, rtrim(a.ERdocumento) as ERdocumento, a.Ocodigo, rtrim(upper(a.TRcodigo)) as TRcodigo, a.ERFecha, a.ERtotal, a.ERusuario, a.Dcodigo, <!--- coalesce(PRJAid, -1) as PRJAid,  --->a.ts_rversion 
				, a.EcodigoRequi, a.Ecodigo, a.ERidref, (dR.Papellido1 #_Cat# ' ' #_Cat# dR.Papellido2 #_Cat# ' ' #_Cat# dR.Pnombre) as NombreCompletoR,uD.Usulogin, a.UsucodigoD,(dD.Papellido1 #_Cat# ' ' #_Cat# dD.Papellido2 #_Cat# ' ' #_Cat# dD.Pnombre) as NombreCompleto, a.Externo, a.Observaciones	
	...
	<cfif rsForm.recordcount eq 0>
		...
		<cfelse>
			<cflocation url="AprobRequisiciones-lista.cfm">
		</cfif>
	</cfif>
	...
	<cfif (dmodo neq 'ALTA')>
		<cfquery datasource="#session.DSN#" name="rsFormDetalle">
			select a.DRlinea, a.ERid, a.Aid as aAid, a.DRcantidad, a.DRcosto, b.Acodigo, b.Adescripcion, c.CFid, c.CFcodigo, c.CFdescripcion, a.ts_rversion, a.FPAEid, a.CFComplemento, Udescripcion
			from DRequisicion a 
				inner join Articulos b 
                	on b.Aid = a.Aid
				left outer join CFuncional c 
                	on c.CFid = a.CFid
				inner join Unidades u on b.Ucodigo = u.Ucodigo
			where a.ERid    = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.ERid#">
...
<cfelse>
	<cfset LvarDir = "AprobRequisiciones-sql.cfm">
</cfif>
<style type="text/css">
.RequiExterna {
	color: #F00;
...
<cfoutput> 
<input type="hidden" name="LvarAprobación" value="NO">
<cfif (modo neq 'ALTA')>
...
<table width="100%"  border="0" cellspacing="1" cellpadding="1">
	...
	<tr>
		...
		<td> 
			<textarea cols="50" rows="3" name="ERObservaciones" id="ERObservaciones"><cfif (modo neq 'ALTA')>#rsForm.Observaciones#</cfif> </textarea> 
		</td>
	</tr>
    ...
    <tr>
   		<td colspan="6" align="center">
                ...
                <cfelse>
                    ...
                    <cfelse>
                    	<cf_botones modo="#modo#" mododet="#dmodo#" generoenc="F" nameenc="Requisición" tabindex="3" include="#Aplicar#,#Imprime#,Rechazar" exclude="#excluir#Alta,Baja,Nuevo">
               		</cfif>
                </cfif>
    		</td>
           </tr>
		...
		<table width="100%"  border="0" cellspacing="1" cellpadding="1">
			...
			<tr>
   				<td width="50%" valign="top" align="center">
					<table width="100%"  border="0" cellspacing="1" cellpadding="1">
						...
						<tr>
							<td nowrap="nowrap" align="right"><strong>Unidad de Medida</strong></td>
                            <td nowrap="nowrap"> <input type="text" name="UnidadMedida" value="<cfif dmodo NEQ 'ALTA'> #rsFormDetalle.Udescripcion#</cfif>" class="cajasinborde" readonly></td>
						</tr>
    	   ...
    	   <tr>
		   	<td width="50%" valign="top" align="center">
                ...
                <cfelse>
				    <cfset LvarDirec = "AprobRequisiciones.cfm">
                    <cfset LvarLink = "true">					
                </cfif>
...
<script language="JavaScript" type="text/javascript">
	...
	function funcRechazar(){
		if (!confirm ('¿Esta seguro que desea rechazar la requisición?'))
			return false;					
			document.requisicion.action = 'AprobRequisiciones-sql.cfm';
