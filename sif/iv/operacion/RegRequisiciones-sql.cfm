...
<cfelseif isdefined("form.btnAplicar") or isdefined("form.Aplicar")>
	...
	<cflocation url="RegRequisiciones-lista.cfm?LvarUsuarioAprobador=#LvarUsuarioAprobador#&LvarUsuarioDespachador=#LvarUsuarioDespachador#">
...
<cfelseif isdefined("form.Alta")>
	...
	<cfelse>
		<cftransaction>
		<cfquery name="insertr" datasource="#session.dsn#">
			insert into ERequisicion ( Ecodigo, ERdescripcion, Aid, ERdocumento, Ocodigo, TRcodigo, ERFecha, ERtotal, ERusuario, Dcodigo<!--- , PRJAid  --->, EcodigoRequi, ERidref, Estado)
			values ( 
				 #Session.Ecodigo# ,
				...
				,<cfif isdefined("form.EnviarAprobar")>
					1
				<cfelse>
					0
				</cfif>
...
<cfelseif isdefined("LvarCaptura") and LvarCaptura EQ 'SI' <!---and modo EQ 'CAMBIO'--->>  <!----Identifica si esta en la captura de la requisicion---->
	<cfif isdefined("form.chk")>
		<cfset lista = form.chk>
	<cfelseif isdefined("form.ERid")>
		<!---<cfset Cambio()>--->
	...
	<cfif isdefined("Lista")>
	<cfset arreglo = listtoarray(lista)>
	<cfloop from="1" to="#arraylen(arreglo)#" index="idx">
		<cfquery datasource="#session.dsn#">			
			update ERequisicion
			set Estado     = 1 <!---Enviado a aprobar--->
			where ERid     = 
			<cfif isdefined("form.chk")>
				<cfqueryparam value="#arreglo[idx]#"    cfsqltype="cf_sql_numeric">
			<cfelseif isdefined("form.ERid")>
				<cfqueryparam value="#Form.ERid#"    cfsqltype="cf_sql_numeric">	
			</cfif>
