...
<cfif (modo neq 'ALTA')>
	...
	<cfif rsForm.recordcount eq 0>
		...
		<cfelse>
			<cflocation url="RegRequisiciones-lista.cfm">
		</cfif>
	</cfif>
	...
	<cfif (dmodo neq 'ALTA')>
		<cfquery datasource="#session.DSN#" name="rsFormDetalle">
			select a.DRlinea, a.ERid, a.Aid as aAid, a.DRcantidad, a.DRcosto, b.Acodigo, b.Adescripcion, c.CFid, c.CFcodigo, c.CFdescripcion, a.ts_rversion, a.FPAEid, a.CFComplemento, u.Udescripcion
			from DRequisicion a 
	...
	<cfif rsFormLineas.recordcount>
		...
		<cfelseif LvarUsuarioDespachador eq 'true'>
			...
			<cfset Imprime = "Imprimir">
		<cfelse>
			<cfset Aplicar = "Enviar a Aprobar">
			<cfset Imprime = "Imprimir">
		</cfif>
	</cfif>
<cfelse>
...
<cfelse>
	<cfset LvarDir = "RegRequisiciones-sql.cfm">
</cfif>
<style type="text/css">
.RequiExterna {
	color: #F00;
...
<table width="100%"  border="0" cellspacing="1" cellpadding="1">
	...
	<tr>
		...
		<td> 
			<input type="text" name="ERdocumento" size="20" maxlength="16" value="<cfif (modo neq 'ALTA')>#rsForm.ERdocumento#</cfif>" onfocus="javascript:this.select();" tabindex="1">
		</td>
	...
	<tr>
		...
		<td> 
			<textarea cols="50" rows="3" name="ERObservaciones" id="ERObservaciones" readonly="readonly"> <cfif (modo neq 'ALTA')>#rsForm.Observaciones#</cfif></textarea>	
		</td>
	</tr>
    	   ...
    	   <tr>
		   	<td width="50%" valign="top" align="center">
				...
				<cfif LvarUsuarioAprobador eq 'true'>
                    ...
                    <cfset LvarLink = "true">
                <cfelseif LvarUsuarioDespachador eq 'true' >
                    <cfset LvarDirec = "R2. FSI-18 Manual Técnico equisicionesDesp-form.cfm">
                    <cfset LvarLink = "false">
                <cfelse>
				    <cfset LvarDirec = "RegRequisiciones.cfm">
                    <cfset LvarLink = "true">					
                </cfif>
...
<script language="JavaScript" type="text/javascript">
	...
	function funcAnular(){
		if (!confirm ('¿Esta seguro que desea eliminar la Requisición?'))
			return false;					
			document.requisicion.action = 'RegRequisiciones-sql.cfm';
