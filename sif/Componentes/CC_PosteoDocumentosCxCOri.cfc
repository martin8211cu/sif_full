	...
	<cffunction name="PosteoDoc" access="private" returntype="string" output="no">
		...
		<cfargument name='Conexion' 	type='string' 	required='false' default	= "#Session.DSN#">
			...
			<cfif rsCuentaTransitoria.cantidad GT 0>
				...
				<cfset LvarCFcuentaTransitoriaGeneral = rsCuentaTransitoria.CFuenta> 
			</cfif>
			...
			<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
				...
				select 
						...
						EDtipocambio,
						EDtotal,
						<cfif LvarContado>0.00<cfelse>EDtotal</cfif>, 
						EDfecha,
						EDvencimiento,
						Ccuenta,
						EDtipocambio,
		...
		<cfset 	MovimientosContables (Arguments.EDid,Arguments.Ecodigo,Arguments.usuario,Arguments.debug,Arguments.Conexion)>
		<cfset 	Contabiliza (Arguments.EDid,Arguments.Ecodigo,Arguments.usuario,Arguments.debug,Arguments.Conexion)>
		...
		<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
			insert into BMovimientos 
				(
					Ecodigo, CCTcodigo, Ddocumento, CCTRcodigo, DRdocumento, BMfecha, Ccuenta, Ocodigo, SNcodigo, Mcodigo, Dtipocambio, Dtotal, Dfecha, Dvencimiento, BMperiodo, BMmes, Dtcultrev, BMusuario, Rcodigo, BMmontoretori, BMtref, BMdocref, Dtotalloc, Dtotalref, Icodigo, Dreferencia, CFid,
					IDcontable
				)
			select 
					a.Ecodigo, '#rsDoc.CCTcodigo#', a.Ddocumento, '#rsDoc.CCTcodigo#', a.Ddocumento, 
					<cfqueryparam cfsqltype="cf_sql_timestamp" value="#now()#">, 
					Ccuenta, Ocodigo, SNcodigo, Mcodigo, Dtipocambio, Dtotal, Dfecha, Dvencimiento, #LvarAnoAux#, #LvarMesAux#, Dtipocambio, Dusuario, Rcodigo, Dmontoretori, Dtref, Ddocref, round(Dtotal * Dtipocambio,2), Dtotal, Icodigo, Dreferencia, CFid,
					#LvarIDcontable#
			  from Documentos a
			 where Ecodigo 		= #Arguments.Ecodigo#
			   and CCTcodigo 	= '#rsDoc.CCTcodigo#'
	...
	<cffunction name="MovimientosContables" access="private" returntype="string" output="no">
		...
		<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
			...
			select
					...
					cal.ingresoLinea,
					a.EDtipocambio,
					round(cal.ingresoLinea*a.EDtipocambio,2)
			from EDocumentosCxC a
					inner join DDocumentosCxC b
		...
		<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
			...
			select 
					...
					sum(impuesto),
					#rsDoc.EDtipoCambio#, 
					sum(round(impuesto*#rsDoc.EDtipoCambio#,2))
			from #CC_impLinea# 
			group by  
					descripcion, ccuenta
		</cfquery>
		...
		<cfif LvarCostoVentaPendiente EQ "0">
			...
			<cfset CxC_AplicaCostoVenta (Arguments.Ecodigo, rsDoc.CCTcodigo, rsDoc.EDdocumento, LvarAnoAux, LvarMesAux, Arguments.Conexion)>
		...
		<cfelse>
			...
			<cfquery datasource="#Arguments.Conexion#">
				insert into #CC_calculoLin# (
					...
					descuentoDoc, 
					impuestoInterfaz, 
					impuesto, impuestoBase, ingresoLinea, totalLinea
				)
				select 
			...
			<cfquery datasource="#Arguments.Conexion#">
				insert into #CC_impLinea# (
					EDid, DDid, ecodigo,   icodigo,  dicodigo,		
					descripcion,    
					ccuenta,CFcuenta,
					montoBase,
					porcentaje,  impuesto, icompuesto)
				select 
					EDid, DDlinea, d.Ecodigo, i.Icodigo, i.Icodigo, 	
			...
			<cfquery datasource="#Arguments.Conexion#">
				insert into #CC_impLinea# (
					...
					porcentaje,    		impuesto, icompuesto)
				select 
					EDid, DDlinea, 	d.Ecodigo, 	di.Icodigo, di.DIcodigo, 	
