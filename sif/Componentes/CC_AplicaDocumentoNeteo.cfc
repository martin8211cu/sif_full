<!--- Componente: CC_AplicaDocumentoNeteo --->
<cfcomponent>
<cf_dbfunction name="OP_concat" returnvariable="_Cat">
	<!--- Método: CC_AplicaDocumentoNeteo: Postea Una Transacción de Neteo de Documentos de CxC y CxP --->
	<cffunction name="CC_AplicaDocumentoNeteo" access="public" returntype="boolean">
		<cfargument name="idDocumentoNeteo" type="numeric" 	required="true">
		...
		<cfelse>
			<cf_errorCode	code = "50969" msg = "CC_AplicaDocumentoNeteo: Error, No se ha definido la Moneda de la Empresa! Proceso Cancelado!">
		</cfif>
		...
		<cfelse>
			<cf_errorCode	code = "50970" msg = "CC_AplicaDocumentoNeteo: Error, No se ha definido el parámetro de Periodo para los sistemas Auxiliares! Proceso Cancelado!">
		</cfif>
		...
		<cfelse>
			<cf_errorCode	code = "50971" msg = "CC_AplicaDocumentoNeteo: Error, No se ha definido el parámetro de Mes para los sistemas Auxiliares! Proceso Cancelado!">
		</cfif>
		<cfquery name="rsDocumentoNeteo" datasource="#Arguments.Conexion#">
			select CCTcodigo, DocumentoNeteo, Dfechadoc, Mcodigo, Ocodigo, TipoNeteoDocs
			from DocumentoNeteo
			where idDocumentoNeteo = #Arguments.idDocumentoNeteo#
		</cfquery>
		<cfif rsDocumentoNeteo.recordcount>
			<cfif len(trim(rsDocumentoNeteo.DocumentoNeteo)) and len(trim(rsDocumentoNeteo.DocumentoNeteo))>
				<cfset lvDocumentoNeteo = rsDocumentoNeteo.DocumentoNeteo>
			<cfelse>
				<cf_errorCode	code = "50972" msg = "CC_AplicaDocumentoNeteo: Error, No se ha definido el documento de neteo para el documento de neteo! Proceso Cancelado!">
			</cfif>
			<cfif len(trim(rsDocumentoNeteo.CCTcodigo)) and len(trim(rsDocumentoNeteo.CCTcodigo))>
				<cfset lvCCTcodigo = rsDocumentoNeteo.CCTcodigo>
			<cfelse>
				<cf_errorCode	code = "50973" msg = "CC_AplicaDocumentoNeteo: Error, No se ha definido el tipo de transacción para el documento de neteo! Proceso Cancelado!">
			</cfif>
			<cfif len(trim(rsDocumentoNeteo.Dfechadoc)) and len(trim(rsDocumentoNeteo.Dfechadoc))>
				<cfset lvFecha = rsDocumentoNeteo.Dfechadoc>
			<cfelse>
				<cf_errorCode	code = "50974" msg = "CC_AplicaDocumentoNeteo: Error, No se ha definido la fecha del documento de neteo! Proceso Cancelado!">
			</cfif>
			<cfset lvDescripcion = lvDescripcion & ' ' & rsDocumentoNeteo.DocumentoNeteo>
			<cfset LvarMcodigo = rsDocumentoNeteo.Mcodigo>
			<cfset LvarOcodigo = rsDocumentoNeteo.Ocodigo>

			<cfset LvarTipoNeteoDocs = rsDocumentoNeteo.TipoNeteoDocs>
			<cfif LvarTipoNeteoDocs EQ 0>
				<cfset LvarTipoNeteoDocs = DeterminarTipoNeteoDocs(Arguments.idDocumentoNeteo)>
				<cfif LvarTipoNeteoDocs EQ -1>
					<cfthrow message="Hay una mezcla de Tipos de Documento incorrecta para netear">
				</cfif>
			<cfelse>
				<cfset LvarTipoNeteoMSG = VerificarTipoNeteoDocs(Arguments.idDocumentoNeteo)>
				<cfif LvarTipoNeteoMSG NEQ "">
					<cfthrow message="#LvarTipoNeteoMSG#">
				</cfif>
			</cfif>
		<cfelse>
			<cf_errorCode	code = "50975" msg = "CC_AplicaDocumentoNeteo: Error, No se ha definido el documento de neteo! Proceso Cancelado!">
		</cfif>
		...
		<cfif Arguments.debug>
			<cfoutput>
				...
				lvMes = #lvMes#<br>
				lvDocumentoNeteo = #lvDocumentoNeteo#<br>
				lvError = #lvError#<br>
				lvFecha = #lvFecha#<br>
				   ...
				   set DmontoRet =
				   		case
							when (
									select d.Rcodigo
									  from Documentos d
									 where d.Ecodigo	= DocumentoNeteoDCxC.Ecodigo
									   and d.CCTcodigo	= DocumentoNeteoDCxC.CCTcodigo
									   and d.Ddocumento	= DocumentoNeteoDCxC.Ddocumento
								  ) is null	then 0
							when DmontoRet = 0 then
								coalesce(
									(
										select DocumentoNeteoDCxC.Dmonto / (1 - r.Rporcentaje/100) * r.Rporcentaje/100
										  from Documentos d
				   ...
				   set DmontoRet =
				   		case
							...
							when DmontoRet = 0 then
								coalesce(
									(
										select DocumentoNeteoDCxP.Dmonto / (1 - r.Rporcentaje/100) * r.Rporcentaje/100
										  from EDocumentosCP d
											inner join Retenciones r
												 on r.Ecodigo = d.Ecodigo
												and r.Rcodigo = d.Rcodigo
		...
		<cfquery name="rsBalance1" datasource="#Arguments.Conexion#">
			...
			from DocumentoNeteoDCxC a <cf_dbforceindex name="Idx1_DocumentoNeteoDCxC">
				inner join CCTransacciones b <cf_dbforceindex name="PK_CCTRANSACCIONES">
					on b.CCTcodigo = a.CCTcodigo
					and b.Ecodigo = a.Ecodigo
			where a.idDocumentoNeteo =#Arguments.idDocumentoNeteo#
		</cfquery>
		...
		<cfquery name="rsBalance2" datasource="#Arguments.Conexion#">
			...
			from DocumentoNeteo aa <cf_dbforceindex name="PK_DOCUMENTONETEO">
				inner join DocumentoNeteoDCxP a <cf_dbforceindex name="Idx1_DocumentoNeteoDCxP">
					on a.idDocumentoNeteo = aa.idDocumentoNeteo
				inner join CPTransacciones b <cf_dbforceindex name="PK_CPTRANSACCIONES">
					on b.CPTcodigo = a.CPTcodigo
					and b.Ecodigo = aa.Ecodigo
			where aa.idDocumentoNeteo = #Arguments.idDocumentoNeteo#
		</cfquery>
		...
		<cfif lvBalance Neq 0.00>
			<cf_errorCode	code = "50976" msg = "CC_AplicaDocumentoNeteo: Error, Los documentos no están balanceados! Proceso Cancelado!">
		</cfif>
		<!--- b. Verificar que los documentos tengan un saldo >= al monto que se aplica --->
		<!--- Existencia de los Documentos de CxC --->
		<cfquery name="rsDocumentoNeteoDCxC" datasource="#Arguments.Conexion#">
		...
		<cfif rsDocumentoNeteoDCxC.CantidadDocNeteo gt 0>
			<cf_errorCode	code = "50977" msg = "CC_AplicaDocumentoNeteo: Existen Documentos Inconsistentes en la Transaccion de CxC. Proceso Cancelado!">
		</cfif>
		<!--- Saldos de Documentos de CxC --->
		<cfquery name="rsDocumentoNeteoDCxC" datasource="#Arguments.Conexion#">
			select 1
			from DocumentoNeteoDCxC a <cf_dbforceindex name="Idx1_DocumentoNeteoDCxP">
		...
		<cfif rsDocumentoNeteoDCxC.recordcount>
			<cfthrow  message="CC_AplicaDocumentoNeteo: Existen Documentos con Saldo menor al monto a Eliminar en CxC. Proceso Cancelado!">
		</cfif>

		<!--- Existencia de los Documentos de CxP --->
		<cfquery name="rsDocumentoNeteoDCxP" datasource="#Arguments.Conexion#">
			select count(1) as cantidad
			from DocumentoNeteoDCxP a
			where idDocumentoNeteo = #Arguments.idDocumentoNeteo#
			and not exists (
				select 1
		...
		<cfif rsDocumentoNeteoDCxP.cantidad gt 0>
			<cf_errorCode	code = "50978" msg = "CC_AplicaDocumentoNeteo: Existen Documentos Inconsistentes en la Transaccion de CxP. Proceso Cancelado!">
		</cfif>
		<!--- Saldos de Documentos de CxP --->
		<cfquery name="rsDocumentoNeteoDCxP" datasource="#Arguments.Conexion#">
			select 1
			from DocumentoNeteoDCxP a <cf_dbforceindex name="Idx1_DocumentoNeteoDCxP">
		...
		<cfif rsDocumentoNeteoDCxP.recordcount>
			<cf_errorCode	code = "50979" msg = "CC_AplicaDocumentoNeteo: Existen Documentos con Saldo menor al monto a Eliminar en CxP. Proceso Cancelado!">
		</cfif>
		...
		<cftransaction>
			<!--- Solo se contabiliza cuando interactua mas de una cuenta --->
			 <cfif Lvar_Contabiliza>
				 <cfset contabilizarNeteo(Arguments.Ecodigo, arguments.conexion, lvFecha, lvPeriodo, lvMes, idDocumentoNeteo, lvMonLoc, Debug, lvCCTcodigo, lvDescripcion, lvDocumentoNeteo)>
			 </cfif>
			...
			<cfquery datasource="#Arguments.Conexion#">
				...
				select 	a.Ecodigo, c.CCTcodigo, c.DocumentoNeteo, a.CCTcodigo, a.Ddocumento,
						...
						<cfif isdefined("IDcontable")>, #IDcontable#</cfif>
				from DocumentoNeteoDCxC b <cf_dbforceindex name="Idx1_DocumentoNeteoDCxC">
					inner join Documentos a <cf_dbforceindex name="PK_DOCUMENTOS">
						on a.Ecodigo = b.Ecodigo
						and a.CCTcodigo = b.CCTcodigo
						and a.Ddocumento = b.Ddocumento
					inner join DocumentoNeteo c <cf_dbforceindex name="PK_DOCUMENTONETEO">
						on c.idDocumentoNeteo = b.idDocumentoNeteo
				where b.idDocumentoNeteo = #Arguments.idDocumentoNeteo#
			</cfquery>
			...
			<cfquery datasource="#Arguments.Conexion#">
				insert into BMovimientosCxP (
						...
						<cfif isdefined("IDcontable")>, IDcontable</cfif>)
				select 	a.Ecodigo, c.CCTcodigo, c.DocumentoNeteo, a.CPTcodigo, a.Ddocumento, <cf_dbfunction name="now">, a.Ccuenta, a.Ocodigo, a.SNcodigo,
						<cf_jdbcquery_param cfsqltype="cf_sql_timestamp" value="#lvFecha#">,
						<cf_jdbcquery_param cfsqltype="cf_sql_timestamp" value="#lvFecha#">,
						...
						<cfif isdefined("IDcontable")>, #IDcontable#</cfif>
				from DocumentoNeteoDCxP b <cf_dbforceindex name="Idx1_DocumentoNeteoDCxP">
				...
				<cfquery datasource="#Arguments.Conexion#" name="rsDContable">
					...
					inner join (
						...
						inner join CERepositorio r
							...
							and r.origen = 'CCFC'
							and b.idDocumentoNeteo = #Arguments.idDocumentoNeteo#
						union all
			...
			<cfquery datasource="#Arguments.Conexion#">
				...
				set Dsaldo = Dsaldo -
						coalesce((
							...
							from DocumentoNeteoDCxC c
							  where c.idDocumentoNeteo = #Arguments.idDocumentoNeteo#
	...
	<cffunction name="contabilizarNeteo" access="private" returntype="any" output="false">
		...
		<cfargument name="lvDocumentoNeteo"	type="string" 	required="true">

		<!--- Control Evento Inicia --->
        <!--- Busca la transacción y el documento  --->
        <cfquery name="rsNeteo" datasource="#Arguments.Conexion#">
        	select a.CCTcodigo,a.SNcodigo,a.DocumentoNeteo
            from DocumentoNeteo a
			where a.Ecodigo = #Arguments.Ecodigo#
			  and a.idDocumentoNeteo 	= <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.idDocumentoNeteo#">
        </cfquery>
        ...
        <cfif varValidaEvento GT 0>
            <cfinvoke
                ...
                Origen			= "CCND"
                Transaccion		= "#rsNeteo.CCTcodigo#"
                Documento 		= "#rsNeteo.DocumentoNeteo#"
            ...
            <cfquery name="rsDocumentoNeteoDCxC" datasource="#Arguments.Conexion#">
                select a.idDocumentoNeteo, a.idDetalle, a.Ecodigo, b.CCTcodigo,
                 ...
                 and idDocumentoNeteo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.idDocumentoNeteo#">
            </cfquery>
            <cfloop query="rsDocumentoNeteoDCxC">
                <cfinvoke
                component		= "sif.Componentes.CG_ControlEvento"
                method			= "CG_RelacionaEvento"
                IDNEvento       = "#varIDEvento#"
                Origen			= "CCFC"
                Transaccion		= "#rsDocumentoNeteoDCxC.CCTcodigo#"
                Documento 		= "#rsDocumentoNeteoDCxC.Ddocumento#"
                SocioCodigo		= "#rsDocumentoNeteoDCxC.SNcodigo#"
                Conexion		= "#Arguments.Conexion#"
                Ecodigo			= "#Arguments.Ecodigo#"
                ...
                <cfquery datasource="#Arguments.Conexion#">
                    ...
                    set NumeroEvento = <cfqueryparam cfsqltype="cf_sql_varchar" value="#varNumeroEventoDP#">
                     where idDocumentoNeteo	= <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsDocumentoNeteoDCxC.idDocumentoNeteo#">
                       and idDetalle  = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsDocumentoNeteoDCxC.idDetalle#">
                </cfquery>
            </cfloop>
			<!---Detalle de documentos CxP--->
            <cfquery name="rsDocumentoNeteoDCxP" datasource="#Arguments.Conexion#">
                select a.idDocumentoNeteo, a.idDetalle, b.Ecodigo, b.CPTcodigo,
                b.Ddocumento, b.SNcodigo
                from DocumentoNeteoDCxP a
            ...
            <cfloop query="rsDocumentoNeteoDCxP">
                <cfinvoke
                component		= "sif.Componentes.CG_ControlEvento"
                method			= "CG_RelacionaEvento"
                IDNEvento       = "#varIDEvento#"
                Origen			= "CPFC"
                Transaccion		= "#rsDocumentoNeteoDCxP.CPTcodigo#"
                Documento 		= "#rsDocumentoNeteoDCxP.Ddocumento#"
                SocioCodigo		= "#rsDocumentoNeteoDCxP.SNcodigo#"
                Conexion		= "#Arguments.Conexion#"
                Ecodigo			= "#Arguments.Ecodigo#"
                returnvariable	= "arRelacionEvento"
                />
                ...
                <cfquery datasource="#Arguments.Conexion#">
                    update DocumentoNeteoDCxP
                    set NumeroEvento = <cfqueryparam cfsqltype="cf_sql_varchar" value="#varNumeroEventoDP#">
                    where idDocumentoNeteo	= <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsDocumentoNeteoDCxP.idDocumentoNeteo#">
                       and idDetalle  = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsDocumentoNeteoDCxP.idDetalle#">
                </cfquery>
            </cfloop>
        </cfif>
		...
		<!--- Valida que exista la cuenta de  XS para el socio --->
		<cfquery name="rsCuentaXS_SN" datasource="#Arguments.Conexion#">
		 select count(1) as Cuenta
		from DocumentoNeteo a 
						inner join CCTransacciones b 
							on b.CCTcodigo = a.CCTcodigo
							and b.Ecodigo = a.Ecodigo
						inner join SNCCTcuentas c 
							on c.Ecodigo = a.Ecodigo
							and c.SNcodigo = a.SNcodigo
							and c.CCTcodigo=b.CCTcodigo
						inner join CFinanciera d 
							on d.CFcuenta = c.CFcuenta 
					where a.idDocumentoNeteo = #Arguments.idDocumentoNeteo#
					
		</cfquery>
		<cfif rsCuentaXS_SN.Cuenta eq 0>
			 <cfthrow message="ERROR CONTROL EVENTO: No se han definido las Cuenta de Eliminacion de Saldos del Socio de Negocios">
            <!--- <cf_errorCode	code = "516500" msg = "No se han definido las Cuenta de Eliminacion de Saldos del Socio de Negocios."> --->
        </cfif>
        ...
        <cfquery name="rsValidaModuloNeteoCxC" datasource="#Arguments.Conexion#">
        	select count(1) as Cuenta
            from DocumentoNeteo aa <cf_dbforceindex name="PK_DOCUMENTONETEO">
                inner join DocumentoNeteoDCxC a <cf_dbforceindex name="Idx1_DocumentoNeteoDCxC">
                on a.idDocumentoNeteo = aa.idDocumentoNeteo
            where aa.idDocumentoNeteo = #Arguments.idDocumentoNeteo#
        </cfquery>

        <!--- Valida si la eliminacion de saldo tiene un RE --->
        <cfquery name="rsValDetRE" datasource="#Arguments.Conexion#">
        	select count(1) as valRE
			 from DocumentoNeteo dn
			 inner join DocumentoNeteoDCxC dcc
			 on dcc.idDocumentoNeteo = dn.idDocumentoNeteo
			 and dcc.CCTcodigo='RE'
			 where dn.idDocumentoNeteo = #Arguments.idDocumentoNeteo#
			 and dn.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.Ecodigo#">
        </cfquery>

        <cfif rsValidaModuloNeteoCXC.recordcount GT 0 and rsValidaModuloNeteoCxC.Cuenta GT 0 and rsValDetRE.valRE EQ 0 >
	...
	<!--- Control Evento Fin --->
				...
				select 	'CCND',
						...
						<cf_dbfunction name="sPart"		args="b.CCTcodigo #_Cat# ' ' #_Cat# b.Ddocumento;1;20" delimiters=";">,
       					<cf_dbfunction name="sPart"		args="aa.CCTcodigo #_Cat# ' ' #_Cat# aa.DocumentoNeteo;1;25" delimiters=";">,
...
<!--- Afecta cuentas por pagar ---->
	...
	<!--- Control Evento Fin --->
					...
					values ('CCND',
							1,
							<cf_dbfunction name="sPart"		args="'#arguments.lvDocumentoNeteo#';1;20" delimiters=";">,
	...
	<!--- Control Evento Fin --->
				...
				<cfquery name="rsImpuestosCxC" datasource="#Arguments.Conexion#">
					select
							...
							aa.DocumentoNeteo as DocumentoNeteo,
							cc.CCTtipo,
							i.Idescripcion,
							i.CcuentaCxCAcred,
							b.Ocodigo
					...
					<cfquery datasource="#Arguments.Conexion#">
						...
						select 	'CCND' as INTORI,
								...
								<cf_dbfunction name="sPart"		args="'#CCTcodigo#' #_Cat# ' ' #_Cat# '#Documento#';1;20" delimiters=";"> as INTDOC,
       							<cf_dbfunction name="sPart"		args="'#CCTcodigoNeteo#' #_Cat# ' ' #_Cat# '#DocumentoNeteo#';1;25" delimiters=";"> as INTrEF,
				...
				<!--- Inserta Monto Pagado en Tabla de Detalle CxP--->
					<cfquery datasource="#Arguments.Conexion#">
						...
						select #idDocumento#, '#Icodigo#', #Ecodigo#, <cfqueryparam cfsqltype="cf_sql_date" value="#LvFecha#">,
								#MontoPagado#, '#CCTcodigoNeteo#', '#DocumentoNeteo#', 0,
...
<!--- ** IEPS ** --->
			...
			<!--- Inserta Monto Pagado en Tabla de Detalle CxP--->
					<cfquery datasource="#Arguments.Conexion#">
						...
						select #idDocumento#, #Ecodigo#, <cfqueryparam cfsqltype="cf_sql_date" value="#LvFecha#">,
								'#CCTcodigoNeteo#', '#DocumentoNeteo#', 0,
	...
	<!--- Control Evento Fin --->
					...
					<cfif LvarGenerarEjercido>
						...
						<cfset sbPresupuestoAntsEfectivo_CxC (arguments.Conexion,Arguments.idDocumentoNeteo,arguments.lvFecha,arguments.lvPeriodo,arguments.lvMes,false,'EJ')>
						<cfset sbPresupuestoAntsEfectivo_CxC (arguments.Conexion,Arguments.idDocumentoNeteo,arguments.lvFecha,arguments.lvPeriodo,arguments.lvMes,false,'P')>
					<cfelse>
	...
	<!--- Control Evento Fin --->
    							,aa.CFid
						from DocumentoNeteo  aa
							inner join DocumentoNeteoDCxP a
								on a.idDocumentoNeteo = aa.idDocumentoNeteo
							inner join EDocumentosCP b
								on b.IDdocumento = a.idDocumento
					...
					<cfif LvarGenerarEjercido>
						<!--- Movimientos de EJERCIDO Y PAGADO: Unicamente si está prendido parámetro de Contabilidad Presupuestaria --->
						<cfset sbPresupuestoAntsEfectivo_CxP (arguments.Conexion,Arguments.idDocumentoNeteo,arguments.lvFecha,arguments.lvPeriodo,arguments.lvMes,false,'EJ')>
						<cfset sbPresupuestoAntsEfectivo_CxP (arguments.Conexion,Arguments.idDocumentoNeteo,arguments.lvFecha,arguments.lvPeriodo,arguments.lvMes,false,'P')>
					<cfelse>
						<!--- Movimientos de PAGADO: Presupuesto normal --->
						<cfset sbPresupuestoAntsEfectivo_CxP (arguments.Conexion,Arguments.idDocumentoNeteo,arguments.lvFecha,arguments.lvPeriodo,arguments.lvMes,false,'P')>
					</cfif>
	...
	<!--- Control Evento Fin --->
					...
					select
							'CCND',
							1,
							<cf_dbfunction name="sPart"		args="a.CCTcodigo #_CAT# ' ' #_CAT# a.DocumentoNeteo;1;20" delimiters=";">,
       						<cf_dbfunction name="sPart"		args="a.CCTcodigo #_CAT# ' ' #_CAT# a.DocumentoNeteo;1;25" delimiters=";">,
							<!--- b.CCTtipo ,--->
							 (SELECT top 1 ctd.CCTtipo
							   FROM DocumentoNeteoDCxC dcc
							   INNER JOIN CCTransacciones ctd
								on ctd.CCTcodigo = dcc.CCTcodigo
								and ctd.Ecodigo = dcc.Ecodigo
							   WHERE idDocumentoNeteo=a.idDocumentoNeteo) AS CCTtipo,

							<cf_dbfunction name="concat" args="'Neteo: Eliminación de Saldos de Documento ' + a.DocumentoNeteo" delimiters="+">,
							'#YEAR(arguments.lvFecha)##RepeatString('0',2-Len(MONTH(arguments.lvFecha)))##MONTH(arguments.lvFecha)##RepeatString('0',2-Len(DAY(arguments.lvFecha)))##DAY(arguments.lvFecha)#',
							...
							* #TC#, 2)
    						,'#varNumeroEvento#'
                            ,a.CFid
					from DocumentoNeteo a <cf_dbforceindex name="PK_DOCUMENTONETEO">
						inner join CCTransacciones b <cf_dbforceindex name="PK_CCTRANSACCIONES">
							on b.CCTcodigo = a.CCTcodigo
							and b.Ecodigo = a.Ecodigo
						inner join SNCCTcuentas c <cf_dbforceindex name="PK_SNCCTCUENTAS">
							on c.Ecodigo = a.Ecodigo
							and c.SNcodigo = a.SNcodigo
							and c.CCTcodigo=b.CCTcodigo
						inner join CFinanciera d <cf_dbforceindex name="PK_CFINANCIERA">
							on d.CFcuenta = c.CFcuenta 
					where a.idDocumentoNeteo = #Arguments.idDocumentoNeteo#
				...
				<cfinvoke
					...
					edocbase="#arguments.lvDocumentoNeteo#"
					ereferencia="#arguments.lvCCTcodigo#"
					debug="#Arguments.Debug#"
					Ocodigo="#rsDocumentoNeteo.Ocodigo#"
	...
	<cffunction name="DeterminarTipoNeteoDocs" returntype="numeric" access="public">
	...
	<cffunction name="sbPresupuestoAntsEfectivo_CxC" access="private">
		...
		<cfargument name="idDocumentoNeteo">
		...
		<cfquery datasource="#Arguments.Conexion#">
			...
			select  'CCND',
				...
				from DocumentoNeteo aa
					inner join DocumentoNeteoDCxC d
						 on d.idDocumentoNeteo = aa.idDocumentoNeteo
 ...
 <cf_dump var="#rs_revisa_Intarc#">   --->
        ...
        <cfquery datasource="#Arguments.Conexion#">
			...
			select  'CCND',
					...
					<cfif Arguments.Anulacion>-</cfif>nap.PCGDcantidad as Cantidad_1
				from DocumentoNeteo  aa
					inner join DocumentoNeteoDCxP d
						on d.idDocumentoNeteo = aa.idDocumentoNeteo
