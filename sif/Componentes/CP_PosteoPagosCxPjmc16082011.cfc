...
<!---=======Valida que este balancedo (Monto Total Pago = Monto de las lineas + monto de Anticipos)=======--->
		...
		<cftransaction>
			...
			<!--- Carga de Variables>	---->
				...
				<!---   en la tabla ImpDocumentosCxPMov                      --->
				<!--- ****************************************************** --->
				...
				<cfloop query="DPagosCxP">
					<cfquery name="rsUPImpDocumentosCxP" datasource="#Arguments.Conexion#">
						...
						set MontoPagado = coalesce(MontoPagado,0) +  round(( (#DPagosCxP.DPmontodoc# + #DPagosCxP.DPmontoretdoc#) * MontoCalculado)/TotalFac,2)
						where #DPagosCxP.Ecodigo#    = Ecodigo
						and #DPagosCxP.IDdocumento# = IDdocumento		
					</cfquery>
				</cfloop>
					...
					<!--- 2d) Débito: Anticipos --->
						<cfquery name="rsInsertAsientoAnticipos" datasource="#Arguments.Conexion#">
							insert into #INTARC# ( INTORI, INTREL, INTDOC, INTREF, INTTIP, INTDES, INTFEC, Periodo, Mes, Ccuenta,  Ocodigo,Mcodigo,INTCAM,INTMON,INTMOE)
							select 
								'CPRE',
								1,
                    ...
                    <!--- 4) Balance por Moneda --->
						...
						<cfif LvarBalancear>
							...
							<cfquery name="rsBalanceMon" datasource="#Arguments.Conexion#">
								insert into #INTARC# ( INTORI, INTREL, INTDOC, INTREF, INTTIP, INTDES, INTFEC, Periodo, Mes, Ccuenta, Mcodigo, Ocodigo, INTCAM, INTMOE, INTMON)
								select 
									'CPRE',
									1,
									'#rsSQL.EPdocumento#',
	...
	<cffunction name="FN_DiferencialCambiario" access="public"  output="false">
		...
		<cfquery name="rsINTARC" datasource="#session.DSN#">
			insert into #Arguments.INTARC# ( INTORI, INTREL, INTDOC, INTREF, INTMON, INTTIP, INTDES, INTFEC, INTCAM, Periodo, Mes, Ccuenta, Mcodigo, Ocodigo, INTMOE)
			select
				'CPRE',
				1,
				c.Ddocumento,
	...
	<cffunction name="FN_trasladoCuenta_IVA" access="public"  output="false">
		...
		<cfquery name="rsINTARC" datasource="#session.DSN#">
			insert 	into #Arguments.INTARC# ( INTORI, INTREL, INTDOC, INTREF, INTMON, INTTIP, 
					INTDES, INTFEC, INTCAM, Periodo, Mes, Ccuenta, Mcodigo, Ocodigo, INTMOE)
			select
				'CPRE',
				1,
				c.Ddocumento,
				c.CPTcodigo ,
				round(
					round( ( (det.DPmontodoc + det.DPmontoretdoc) * b.MontoCalculado) / b.TotalFac, 2 )
					*
		...
		<cfquery name="rsINTARC" datasource="#session.DSN#">
			...
			select
				...
				, 2),	
				d.CPTtipo,
				<cf_dbfunction name="concat"	args="i.Idescripcion, ' ', c.Ddocumento, '(Acreditado Cuenta balance multimoneda)'">,
				'#LvarFechaLinea#',
				1.00,
				#Arguments.Periodo#,
				#Arguments.Mes#, 
	...
	<cffunction name="sbPresupuestoAntsEfectivo_CxP" access="private">
		...
		<cfquery datasource="#session.DSN#">
			...
			select  '#Arguments.Origen#',					
					...
					<cfif Arguments.TipoMov NEQ 'P'>-</cfif>IDLinea as NumeroLineaID,
					...
					<cfif Arguments.Anulacion>-</cfif>round((d.DPmontodoc + d.DPmontoretdoc) / cp.Dtotal * nap.CPNAPDmontoOri
