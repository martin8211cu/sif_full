<!--- Componente: CC_AplicaDocumentoCompensacion --->
<cfcomponent>
<cf_dbfunction name="OP_concat" returnvariable="_Cat">
	<!--- Método: CC_AplicaDocumentoCompensacion: Postea Una Transacción de Compensacion de Documentos de CxC y CxP --->
	<cffunction name="CC_AplicaDocumentoCompensacion" access="public" returntype="boolean">
		...
		<!--- Las siguientes variables son locales a esta función, por definición de lenguaje deben estar juntas en esta sección y no debe haber otro tipo de
			código que no sea la propia definición de las mismas hasta que termine la definición de todas.
		--->
		<cfset  myResult=false>
		<cfset  lvCCTcodigo="">
		<cfset  lvPeriodo=0>
		<cfset  lvMes=0>
		<cfset  lvDocCompensacion=0>
		<cfset  lvError=0>
		<cfset  lvFecha=Now()>
		<cfset  lvDescripcion="Documento de Compensacion">
		<cfset  lvBalance=0.00><!--- utilizada en el punto a. --->
		<cfset  Lvar_Contabiliza = true>
		<cfset  lvMonLoc = 0>
		...
		<cfelse>
			<cf_errorCode	code = "50969" msg = "CC_AplicaDocumentoCompensacion: Error, No se ha definido la Moneda de la Empresa! Proceso Cancelado!">
		</cfif>
		...
		<cfelse>
			<cf_errorCode	code = "50970" msg = "CC_AplicaDocumentoCompensacion: Error, No se ha definido el parámetro de Periodo para los sistemas Auxiliares! Proceso Cancelado!">
		</cfif>
		...
		<cfelse>
			<cf_errorCode	code = "50971" msg = "CC_AplicaDocumentoCompensacion: Error, No se ha definido el parámetro de Mes para los sistemas Auxiliares! Proceso Cancelado!">
		</cfif>
		<cfquery name="rsDocCompensacion" datasource="#Arguments.Conexion#">
			select CCTcodigo, DocCompensacion, Dfechadoc, Mcodigo, Ocodigo, TipoCompensacionDocs
		...
		<cfif rsDocCompensacion.recordcount>
			...
			<cfelse>
				<cf_errorCode	code = "50972" msg = "CC_AplicaDocumentoCompensacion: Error, No se ha definido el documento de compensaci&oacute;n para el documento de compensaci&oacute;n! Proceso Cancelado!">
			</cfif>
			...
			<cfelse>
				<cf_errorCode	code = "50973" msg = "CC_AplicaDocumentoCompensacion: Error, No se ha definido el tipo de transacción para el documento de compensaci&oacute;n! Proceso Cancelado!">
			</cfif>
			...
			<cfelse>
				<cf_errorCode	code = "50974" msg = "CC_AplicaDocumentoCompensacion: Error, No se ha definido la fecha del documento de compensaci&oacute;n! Proceso Cancelado!">
			</cfif>
			...
			<cfset LvarTipoCompensacionDocs = rsDocCompensacion.TipoCompensacionDocs>
			<cfif LvarTipoCompensacionDocs EQ 0>
				<cfset LvarTipoCompensacionDocs = DeterminarTipoCompensacionDocs(Arguments.idDocCompensacion)>
				<cfif LvarTipoCompensacionDocs EQ -1>
					<cfthrow message="Hay una mezcla de Tipos de Documento incorrecta para compensar">
				</cfif>
			<cfelse>
				<cfset LvarTipoCompensacionMSG = VerificarTipoCompensacionDocs(Arguments.idDocCompensacion)>
				<cfif LvarTipoCompensacionMSG NEQ "">
					<cfthrow message="#LvarTipoCompensacionMSG#">
				</cfif>
			</cfif>
		<cfelse>
			<cf_errorCode	code = "50975" msg = "CC_AplicaDocumentoCompensacion: Error, No se ha definido el documento de compensaci&oacute;n! Proceso Cancelado!">
		</cfif>
		...
		<!---
			Calculo de Retención:
				...
				Si no tiene monto, lo calcula con base en monto a compensar:
		...
		<cfquery datasource="#Arguments.Conexion#">
			<cfif LvarTipoCompensacionDocs NEQ 2>
		...
		<cfquery datasource="#Arguments.Conexion#">
			<cfif LvarTipoCompensacionDocs NEQ 3>
		...
		<!---
		a. verificar que los documentos están balanceados
			La suma de los documentos de CxC a compensar
			Debe corresponder a la suma de los documentos de CxP a compensar
		...
		<cfif lvBalance Neq 0.00>
			<cf_errorCode	code = "50976" msg = "CC_AplicaDocumentoCompensacion: Error, Los documentos no están balanceados! Proceso Cancelado!">
		</cfif>
		...
		<cfquery name="rsDocCompensacionDCxC" datasource="#Arguments.Conexion#">
			select count(1) as CantidadDocCompensacion
			from DocCompensacionDCxC a <cf_dbforceindex name="Idx1_DocCompensacionDCxC">
			where idDocCompensacion = #Arguments.idDocCompensacion#
			and not exists (
		...
		<cfif rsDocCompensacionDCxC.CantidadDocCompensacion gt 0>
			<cf_errorCode	code = "50977" msg = "CC_AplicaDocumentoCompensacion: Existen Documentos Inconsistentes en la Transaccion de CxC. Proceso Cancelado!">
		</cfif>
		...
		<cfif rsDocCompensacionDCxC.recordcount>
			<cfthrow  message="CC_AplicaDocumentoCompensacion: Existen Documentos con Saldo menor al monto a Eliminar en CxC. Proceso Cancelado!">
		</cfif>
		...
		<cfif rsDocCompensacionDCxP.cantidad gt 0>
			<cf_errorCode	code = "50978" msg = "CC_AplicaDocumentoCompensacion: Existen Documentos Inconsistentes en la Transaccion de CxP. Proceso Cancelado!">
		</cfif>
		...
		<cfif rsDocCompensacionDCxP.recordcount>
			<cf_errorCode	code = "50979" msg = "CC_AplicaDocumentoCompensacion: Existen Documentos con Saldo menor al monto a Eliminar en CxP. Proceso Cancelado!">
		</cfif>
		<!--- Obtiene el Tipo de Cambio de la moneda del compensacion --->
		<cfset TC = 1>
		...
		<cftransaction>
			<!--- Solo se contabiliza cuando interactua mas de una cuenta --->
			 <cfif Lvar_Contabiliza>
				 <cfset contabilizarCompensacion(Arguments.Ecodigo, Arguments.Conexion, lvFecha, lvPeriodo, lvMes,Arguments.idDocCompensacion, lvMonLoc, Debug, lvCCTcodigo, lvDescripcion, lvDocCompensacion)>
			 </cfif>
	...
	<!--- Fin ContabilidadElectronica --->
			...
			<!--- Actualiza el Documento de Compensacion --->
			<cfquery datasource="#Arguments.Conexion#">
	...
	<cffunction name="contabilizarCompensacion" access="private" returntype="any" output="false">
        ...
        <cfquery name="rsCompensacion" datasource="#Arguments.Conexion#">
        ...
        <cfinvoke
            ...
            method			= "ValidaEvento"
            Origen			= "CCND"
            Transaccion		= "#rsCompensacion.CCTcodigo#"
        ...
        <cfif varValidaEvento GT 0>
            <cfinvoke
                ...
                Origen			= "CCND"
                Transaccion		= "#rsCompensacion.CCTcodigo#"
                Documento 		= "#rsCompensacion.DocCompensacion#"
                SocioCodigo		= "#rsCompensacion.SNcodigo#"
                Conexion		= "#Arguments.Conexion#"
                Ecodigo			= "#Arguments.Ecodigo#"
        ...
        <cfquery name="rsValidaModuloCompensacionCxC" datasource="#Arguments.Conexion#">
        ...
        <cfif rsValidaModuloCompensacionCXC.recordcount GT 0 and rsValidaModuloCompensacionCxC.Cuenta GT 0>
		...
		<cfif TC NEQ 1.00>
			<!--- Afectar Diferencial Cambiario de los documentos ---MOVIMIENTOS CONTABLES PARA GENERAR LA POLIZA--->
				<cf_dbfunction name="OP_concat" returnvariable="_Cat">
				<cfquery datasource="#Arguments.Conexion#">
						insert into #INTARC# (
			...
			<cfquery datasource="#Arguments.Conexion#">
				...
				from DocCompensacion aa <cf_dbforceindex name="PK_DocCompensacion">
					...
					inner join Impuestos i
						...
						and i.Icodigo = fi.Icodigo
				where aa.idDocCompensacion = #Arguments.idDocCompensacion#
				  and 	round(round((a.Dmonto+a.DmontoRet) * fi.MontoCalculado / fi.TotalFac,2)	
				  		* (#TC# - b.Dtcultrev), 2) <> 0
			</cfquery>
		...
		<!--- b. Diferencial Cambiario por IEPS Crédito Fiscal de Facturas de CxP --->
			...
			<!---<cf_dump select="select * from #INTARC#">--->
				...
				<!--- ** --->
					<cfquery name="rsImpuestosCxC" datasource="#Arguments.Conexion#">
						select
							...
							aa.CCTcodigo as CCTcodigoCompensacion,
					...
					<cfquery datasource="#Arguments.Conexion#">
						...
						select
							...
							'#CCTcodigoCompensacion#', '#DocCompensacion#',
					...
					<cfquery datasource="#Arguments.Conexion#">
						...
						select 	'CCND' as INTORI,
								...
								<cf_dbfunction name="sPart"		args="'#CCTcodigo#' #_Cat# ' ' #_Cat# '#Documento#';1;20" delimiters=";"> as INTDOC,
       							<cf_dbfunction name="sPart"		args="'#CCTcodigoCompensacion#' #_Cat# ' ' #_Cat# '#DocCompensacion#';1;25" delimiters=";"> as INTrEF,
        ...
        <!--- ----------------------------------------------------------- --->
			<!--- Balance multimoneda CxC 
				...
				<cfquery datasource="#Arguments.Conexion#">
					...
					<!--- Control Evento Fin --->
							,aa.CFid
					from DocCompensacion aa <cf_dbforceindex name="PK_DocCompensacion">
					inner join DocCompensacionDCxP a <cf_dbforceindex name="Idx1_DocCompensacionDCxC">
						on a.idDocCompensacion = aa.idDocCompensacion
					inner join EDocumentosCP b <cf_dbforceindex name="PK_DOCUMENTOS">
						on b.Ecodigo = aa.Ecodigo
						and b.CPTcodigo = a.CPTcodigo
						and b.Ddocumento = a.Ddocumento
					inner join CPTransacciones  cc <cf_dbforceindex name="PK_CCTRANSACCIONES">
						on cc.CPTcodigo = b.CPTcodigo
						and cc.Ecodigo = b.Ecodigo
					inner join ImpIEPSDocumentosCxP fi
						on  fi.Ecodigo     = b.Ecodigo
						and fi.CPTcodigo   = b.CPTcodigo
						and fi.IDdocumento = a.idDocumento
					inner join Impuestos i
						on  i.Ecodigo = fi.Ecodigo
						and i.Icodigo = fi.codIEPS
					where aa.idDocCompensacion = #Arguments.idDocCompensacion#
					...
					<!--- Inserta Monto Pagado en Tabla de Detalle CxP--->
						<cfquery datasource="#Arguments.Conexion#">
							...
							select #idDocumento#, '#Icodigo#', #Ecodigo#, <cfqueryparam cfsqltype="cf_sql_date" value="#LvFecha#">,
								#MontoPagado#, '#CCTcodigoCompensacion#', '#DocCompensacion#', 0,
				...
				<cfloop query="rsImpuestosIepsCxP">
					...
					<cfquery datasource="#Arguments.Conexion#">
						...
						select #idDocumento#, #Ecodigo#, <cfqueryparam cfsqltype="cf_sql_date" value="#LvFecha#">,
								'#CCTcodigoCompensacion#', '#DocCompensacion#', 0,
			...
			<!--- Balance multimoneda CxP--->
				<cfset InsertBalMultimonedaCxP(Arguments.Ecodigo, Arguments.Conexion, lvFecha, lvPeriodo, lvMes,Arguments.idDocCompensacion, lvMonLoc)>
				<!--- Balance multimoneda CxP IVA
				<cfquery  datasource="#Arguments.Conexion#">
					insert into #INTARC# (
							INTORI, INTREL, INTDOC, INTREF,
							INTTIP, INTDES, INTFEC,
			...
			<!---------------- VALORES DEL IEPS ----------------->
				...
				<cfquery datasource="#Arguments.Conexion#">
					...
					and b.Mcodigo != #arguments.lvMonLoc#
					and  round(
							round((a.Dmonto+a.DmontoRet) * coalesce(fi.MontoCalculado,0) / fi.TotalFac,2)
						* (#TC# - b.EDtcultrev), 2) <> 0
				</cfquery>
		...
		<!------------fin del ieps ---------------------->
			...
			<!--- Retenciones: unicamente cuando es Aplicación de Anticipos --->
				...
				<cfif LvarTipoCompensacionDocs EQ 2>
					<cfquery datasource="#Arguments.Conexion#">
						insert into #INTARC# (
								INTORI, INTREL, INTDOC, INTREF,
								INTTIP, INTDES, INTFEC,
				...
				<cfif LvarTipoCompensacionDocs EQ 3>
					<cfquery datasource="#Arguments.Conexion#">
						insert into #INTARC# (
								INTORI, INTREL, INTDOC, INTREF,
								INTTIP, INTDES, INTFEC,
				...
				<!--- Encabezado de Compensacion, cuando no es balanceado ??? --->
				<cfquery datasource="#Arguments.Conexion#">
					insert into #INTARC# (
							INTORI, INTREL, INTDOC, INTREF,
							INTTIP, INTDES, INTFEC,
					...
					select
							...
							<cf_dbfunction name="concat" args="'Compensacion: Eliminación de Saldos de Documento ' + a.DocCompensacion" delimiters="+">,
							'#YEAR(arguments.lvFecha)##RepeatString('0',2-Len(MONTH(arguments.lvFecha)))##MONTH(arguments.lvFecha)##RepeatString('0',2-Len(DAY(arguments.lvFecha)))##DAY(arguments.lvFecha)#',
	...
	<cffunction name="DeterminarTipoCompensacionDocs" returntype="numeric" access="public">
		...
		<cfelseif rsDocs.cxcANTs+rsDocs.cxpANTs EQ 0>
			<!--- Sin Anticipos es Compensacion Documentos --->
			<cfset LvarTipo = 1>
		...
		<cfelseif rsDocs.cxcDOCs+rsDocs.cxcFAVs EQ 0 AND rsDocs.cxpDOCs+rsDocs.cxpFAVs EQ 0>
			<!--- S�lo Anticipos es Compensacion Anticipos --->
			<cfset LvarTipo = 4>
		<cfelse>
			<!--- ERROR --->
			<cfreturn -1>
		...
		<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
			update DocCompensacion
			   set TipoCompensacionDocs = #LvarTipo#
	...
	<cffunction name="VerificarTipoCompensacionDocs" returntype="string" access="public">
		<cfargument name="idDoc" type="numeric">
		<cfargument name="Ecodigo" type="string" default="#session.Ecodigo#">
		<cfargument name="conexion" type="string" default="#session.dsn#">
		...
		<cfif rsDocs.CxCs+rsDocs.CxPs EQ 0>
			...
			<cfreturn "">
		<cfelseif rsDocs.TipoCompensacionDocs EQ 1 AND NOT (rsDocs.cxcANTs+rsDocs.cxpANTs EQ 0)>
			<!--- Sin Anticipos es Compensacion Documentos --->
			<cfreturn "Compensación de Documentos de CxC y CxP: No se permiten Anticipos de Efectivo">
		<cfelseif rsDocs.TipoCompensacionDocs EQ 2 AND NOT (rsDocs.cxcANTs GTE 0 AND rsDocs.CxPs EQ 0)>
		...
		<cfelseif rsDocs.TipoCompensacionDocs EQ 2 AND NOT (rsDocs.cxcANTs GTE 0 AND rsDocs.cxcFAVs EQ 0)>
			<!--- Anticipos CxC sin CxP es Aplicacion Anticipos CxC --->
			<cfreturn "Aplicación de Anticipos de CxC: No se permiten Documentos a favor de CxC que no sean de Anticipos">
		<cfelseif rsDocs.TipoCompensacionDocs EQ 3 AND NOT (rsDocs.cxpANTs GTE 0 AND rsDocs.CxCs EQ 0)>
		...
		<cfelseif rsDocs.TipoCompensacionDocs EQ 3 AND NOT (rsDocs.cxpANTs GTE 0 AND rsDocs.cxpFAVs EQ 0)>
			<!--- Anticipos CxP sin CxC es Aplicacion Anticipos CxP --->
			<cfreturn "Aplicación de Anticipos de CxP: No se permiten Documentos a favor que no sean de Anticipos">
		<cfelseif rsDocs.TipoCompensacionDocs EQ 4 AND NOT (rsDocs.cxcDOCs+rsDocs.cxcFAVs EQ 0 AND rsDocs.cxpDOCs+rsDocs.cxpFAVs EQ 0)>
			<!--- S�lo Anticipos es Compensacion Anticipos --->
			<cfreturn "Compensacion de Anticipos de CxC y CxP: No se permiten Documentos que no son Anticipos de Efectivo">
		</cfif>
		<cfreturn "">
	</cffunction>
	...
	<!--- Balance multimoneda CxP IEPS--->
	<cffunction  name="InsertBalMultimonedaCxP" access="private">
		<cfargument name="Ecodigo">
		<cfargument name="Conexion">
		<cfargument name="lvFecha">
		<cfargument name="lvPeriodo">
		<cfargument name="lvMes">
		<cfargument name="idDocCompensacion">
		<cfargument name="lvMonLoc">
