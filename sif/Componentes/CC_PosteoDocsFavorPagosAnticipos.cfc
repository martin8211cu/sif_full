...
<!--- Control Evento Fin --->
       ...
       <cfif isdefined('arguments.DIFERENCIAL') and len(trim(#arguments.DIFERENCIAL#)) gt 0>
		...
		<cfquery datasource="#arguments.conexion#">
			...
			select
				...
				'C',
				round(
				  ( round(((det.DFmontodoc * c.MontoCalculado) / c.TotalFac),2) )
				* (
					(DFtotal / (DFmontodoc) * a.EFtipocambio)
					-
					b.Dtcultrev
				) , 2),
				case when d.CCTtipo = 'D' then 'C' else 'D' end,
		...
		<!--- PASA LOS DATOS DE LA TABLA #DIFERENCIAL A LA #INTARC ---> <!---SML No--->
		<cfquery datasource="#arguments.conexion#">
		...
		<cfquery datasource="#arguments.conexion#"> <!---SML No--->
			insert into #INTARC# ( INTORI, INTREL, INTDOC, INTREF, INTMON, INTTIP, INTDES, INTFEC, INTCAM, Periodo, Mes, Ccuenta, Mcodigo, Ocodigo, INTMOE, NumeroEvento)
			select
				'CCPA',
				1,
		...
		<!--- Facturas --->
		<cfquery datasource="#arguments.conexion#"> <!--- SML Si Cuenta 127 IVA Debito--->
			insert into #INTARC# ( INTORI, INTREL, INTDOC, INTREF, INTMON, INTTIP, INTDES, INTFEC, INTCAM, Periodo, Mes, Ccuenta, Mcodigo, Ocodigo, INTMOE, NumeroEvento)
			select
				'CCPA',
				1,
		...
		<!--- 2) Asiento Contable --->
		<!--- 2a) Débito: Documento a Favor ---><!--- SML Si Cuenta 248 CxC: Documento a Favor --->
		<cfquery datasource="#arguments.conexion#">
			insert into #INTARC# ( INTORI, INTREL, INTDOC, INTREF, INTMON, INTTIP, INTDES, INTFEC, INTCAM, Periodo, Mes, Ccuenta, Mcodigo, Ocodigo, INTMOE, NumeroEvento)
			select
				'CCPA',
		...
		<cfquery datasource="#arguments.conexion#"> <!--- SML Si Cuenta 248 CxC: Pago Documento--->
			insert into #INTARC# ( INTORI, INTREL, INTDOC, INTREF, INTMON, INTTIP, INTDES, INTFEC, INTCAM, Periodo, Mes, Ccuenta, Mcodigo, Ocodigo, INTMOE, NumeroEvento)
			select
				'CCPA',
				1,
        ...
        <cfset fnContabilizaGasto(CCTcodigo,Ddocumento)>
		...
		<cfif arguments.debug>
			...
			<cfset sbAfectacionIETU("CCPA", arguments.Ecodigo, CCTcodigo, Ddocumento,
						#EFecha#, #Periodo#, #Mes#,
						Arguments.conexion)>
			...
			<!--- 6) Insertar en el Histórico de CxC --->
			<cfquery datasource="#arguments.conexion#">
				insert into BMovimientos (
					...
					Dtotalloc, Dtotalref, Mcodigoref, BMmontoref, BMfactor)
				select
					#IDcontable#,
					#Ecodigo#,
					'#CCTcodigo#',
					...
					b.CCTRcodigo,
					b.DRdocumento,
					<cfqueryparam cfsqltype="cf_sql_date" value="#lsParseDateTime(Fecha)#">,
					a.SNcodigo,
					c.Mcodigo,
					c.EFtipocambio,
					b.DFtotal,
			...
			<cfif isdefined("getContE.ERepositorio") and getContE.ERepositorio EQ "1">
				...
				<cfquery name="rsDContable" datasource="#Session.DSN#">
					...
					select dc.IDcontable, rep.IdDocumento, rep.numDocumento,
						'CCPA', #rsLIneaNC.Dlinea#, rep.timbre, rep.xmlTimbrado, rep.archivoXML, rep.archivo, rep.nombreArchivo,
				...
				<cfquery name="rsDContable" datasource="#Session.DSN#">
					insert into CERepositorio(
						...
						CEtipoLinea,CESNB,CEtranOri,CEdocumentoOri,Miso4217)
					select dc.IDcontable, rep.IdDocumento, rep.numDocumento,
						'CCPA', dc.Dlinea, rep.timbre, rep.xmlTimbrado, rep.archivoXML, rep.archivo, rep.nombreArchivo,
	                    	rep.extension, #session.Ecodigo#,#session.Usucodigo#,
	                    	rep.TipoComprobante,rep.Serie,
	                    	rep.Mcodigo,
	                    	rep.TipoCambio,
    ...
    <!---SML 13052015 Agregar el gasto de la factura--->
    ...
    <cfargument name="Ddocumento" 	type="string" 	required="true">
		...
		<cfif LvarGenerarEjercido>
			...
			<cfset sbPresupuestoAntsEfectivo_CxC ('CCPA',Periodo,Mes,false,'EJ',Arguments.CCTcodigo,Arguments.Ddocumento)>
			<cfset sbPresupuestoAntsEfectivo_CxC ('CCPA',Periodo,Mes,false,'P',Arguments.CCTcodigo,Arguments.Ddocumento)>
		<!---<cfelse>
			<!--- Movimientos de PAGADO: Presupuesto normal --->
			<cfset sbPresupuestoAntsEfectivo_CxC (arguments.Oorigen,Arguments.IDpago,arguments.Efecha,arguments.Eperiodo,arguments.Emes,false,'P')>--->
		</cfif>
	</cffunction>
		...
		<cfquery datasource="#session.DSN#" name="rsPrueba">
			...
			<!--- 'EJ/P:PAGADO' --->
			 SELECT '#Arguments.Origen#',
					b.Ddocumento,
					b.CCTcodigo,
                    <cf_dbfunction name="concat" args="'CxP: '; b.CCTcodigo;'-';rtrim(b.Ddocumento)" delimiters=";">,
					b.Dfecha,
					#Arguments.Periodo# as Periodo,
					#Arguments.Mes# as Mes,
					coalesce((select max(INTLIN) from #request.intarc#),0)+1 as NumeroLinea,
					<cfif Arguments.TipoMov NEQ 'P'>-</cfif>g.CPNAPnum*100000+g.CPNAPDlinea as NumeroLineaID,
					g.CFcuenta,																				<!--- CFuenta --->
					g.Ocodigo,																				<!--- Oficina --->
					g.Mcodigo,																				<!--- Mcodigo --->
					<cfif Arguments.Anulacion>-</cfif>g.CPNAPDmontoOri as MontoOrigen,
					g.CPNAPDtipoCambio as TC,
					<cfif Arguments.Anulacion>-</cfif>g.CPNAPDmonto as MontoLocal,
					'#Arguments.TipoMov#' as Tipo,
					g.CPNAPnum, g.CPNAPDlinea,
					g.PCGDid,
					<cfif Arguments.Anulacion>-</cfif>g.PCGDcantidad as Cantidad_1
             FROM DFavor a
             	inner join EFavor c
                	on a.Ecodigo = c.Ecodigo
                    	and a.CCTcodigo = c.CCTcodigo
                ...
                INNER JOIN DDocumentos d 
                    on d.Ddocumento = b.Ddocumento
                        and d.Ecodigo = b.Ecodigo                         
                
                INNER JOIN CFinanciera e
                    on e.Ecodigo = d.Ecodigo and e.Ccuenta = d.Ccuenta           
                            
                inner join CPNAP f 
                    on f.Ecodigo = b.Ecodigo and f.CPNAPdocumentoOri = b. Ddocumento            
                
                inner join CPNAPdetalle g
                    on g.Ecodigo = f.Ecodigo and g.CPNAPnum = f.CPNAPnum 
                        and g.CPNAPDtipoMov = 'E' and g.CPcuenta = e.CPcuenta                        
