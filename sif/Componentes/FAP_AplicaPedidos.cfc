			...
			<cfif rsLineasProceso.cantidad gt 0 and rsValorP.Pvalor eq 1 and rsPlanCompras.Pvalor eq 1>
			   <cfloop query="dataCuentaF">
			       <cfquery name="rsUpdate" datasource="#session.dsn#">
				    update DSolicitudCompraCM set DScantsurt = (select DOcantidad  from DRemisionesFA do
                      ...
                      where  exists ( select 1 from  DRemisionesFA do
					  				  where do.DSlinea = DSolicitudCompraCM.DSlinea
										and do.EOidorden = #Arguments.EOidorden#
										and do.DSlinea = #dataCuentaF.DSlinea#
										and do.DOcantidad  > 0
				   ...
				   <cfif ObtCantCancelar.CantCancelar gt 0 >
					...
					<cfquery name="rsUpdate" datasource="#session.dsn#">
				    	update DSolicitudCompraCM set DScantsurt =  DScantsurt - (select DOcantidad  from DRemisionesFA do
					                                                      where do.DSlinea = DSolicitudCompraCM.DSlinea
																		   and do.EOidorden = #Arguments.EOidorden#
																		   and do.DSlinea = #dataCuentaF.DSlinea#
																		   and do.DOcantidad  > 0
		...
		<cfquery name="rsPeriodoAuxiliar" datasource="#session.DSN#">
			...
			<cfif NOT esMultiperiodo()>
                ...
                <cfquery name="rsEnviaEmail" datasource="#session.DSN#">
                    ...
                    from ERemisionesFA eo
                    	inner join DRemisionesFA do
            ...
            <cfquery name="rsOCD" datasource="#session.DSN#">
            	select COUNT(1) reg from ERemisionesFA eoc
				inner join DRemisionesFA doc on doc.EOidorden=eoc.EOidorden
            ...
            <cfquery name="rsOCD1" datasource="#session.dsn#">
            	select COUNT(1) as reg from ERemisionesFA eoc inner join DRemisionesFA doc on doc.EOidorden=eoc.EOidorden
...
<!--- JMRV Fin --->
			...
			<cfif LvarNAP GTE 0>
				...
				<cfquery name="rsCantidadSinSurtir" datasource="#session.DSN#">
					...
					from DRemisionesFA
						...
						on  DRemisionesFA.ESidsolicitud = DSolicitudCompraCM.ESidsolicitud
						and DRemisionesFA.Ecodigo       = DSolicitudCompraCM.Ecodigo
					where DRemisionesFA.EOidorden = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.EOidorden#">
		...
		<cfargument name="CRid"				default="-1"					type="numeric">
			...
			<cflock name="LCK_EOrdenCM#Arguments.Ecodigo#" timeout="20" throwontimeout="yes" type="exclusive">
				...
				<cfquery name="insert" datasource="#session.DSN#">
					insert into ERemisionesFA ( Ecodigo, EOnumero, SNcodigo, CMTOcodigo, CMCid, Mcodigo, Rcodigo, EOfecha, Observaciones, EOtc, EOrefcot, Impuesto, EOdesc, EOtotal, Usucodigo, EOplazo, EOhabiles, EOfalta, EOporcanticipo, CMFPid, CMIid, EOdiasEntrega, EOtipotransporte, EOlugarentrega, CRid)
					values (
						<cfqueryparam cfsqltype="cf_sql_integer" 	value="#Arguments.Ecodigo#">,
	...
	<cffunction name="insert_DOrdenCM" access="public" output="no" returntype="numeric">
        ...
        <cfif len(TRIM(Arguments.CTDContid))>
			<cfquery name="rsOrdenTC" datasource="#session.dsn#">
				select Mcodigo,EOtc from ERemisionesFA where EOidorden = #EOidorden#
			</cfquery>
		...
		<cfquery name="insert" datasource="#session.DSN#">
			insert into DRemisionesFA ( Ecodigo, EOidorden, EOnumero, DOconsecutivo, ESidsolicitud, DSlinea,
	...
	<cffunction name="update_DOrdenCM" access="public" output="no" returntype="void">
		...
		<cfif getOrden.recordCount GT 0 >
			...
			<cfif getOrden.CTDContid NEQ ''>
				<cfquery name="rsOrdenTC" datasource="#session.dsn#">
					select Mcodigo,EOtc from ERemisionesFA where EOidorden = #Arguments.EOidorden#
				</cfquery>
				<cfset tcidorden = #rsOrdenTC.EOtc#>
                <cfquery name="updDetContrato" datasource="#session.dsn#">
	...
	<cffunction name="calculaTotalesEOrdenCM" access="public" output="no">
       ...
       <cfelse>
				   				...
				   				( 1 +
										coalesce((
											select i.Iporcentaje/100
											  from Impuestos i
											 where i.Ecodigo = DRemisionesFA.Ecodigo
											   and i.Icodigo = DRemisionesFA.Icodigo
											   and i.Icompuesto = 0
										...
										coalesce((
											select di.DIporcentaje/100
											 ...
											 where d.Ecodigo	= DRemisionesFA.Ecodigo
											   and d.EOidorden	= DRemisionesFA.EOidorden
											   and i.Icompuesto	= 1
		...
		<cfquery datasource="#session.DSN#">
			update DRemisionesFA
			   set DOimpuestoCosto =
				coalesce((
			select
			  case when (DRemisionesFA.CMtipo = 'S' or DRemisionesFA.CMtipo = 'A') and (e.afectaIVA = 1 or f.afectaIVA = 1) THEN
			     round( round(DRemisionesFA.DOtotal-DRemisionesFA.DOmontodesc,2) * i.Iporcentaje/100,2)
			  else
			     round((round(DRemisionesFA.DOtotal-DRemisionesFA.DOmontodesc,2) + round(ROUND(DRemisionesFA.DOtotal-DRemisionesFA.DOmontodesc,2) * COALESCE(d.ValorCalculo/100,0),2)) * i.Iporcentaje/100,2)
			  end as baseIVA
					  from Impuestos i
					  left join Impuestos d
						on  DRemisionesFA.Ecodigo=d.Ecodigo
					    and DRemisionesFA.codIEPS=d.Icodigo
					  left join Conceptos e
						on e.Cid = DRemisionesFA.Cid
					  left join Articulos f
						on f.Aid= DRemisionesFA.Aid
					 where i.Ecodigo = DRemisionesFA.Ecodigo
					   and i.Icodigo = DRemisionesFA.Icodigo
					   and i.Icompuesto = 0
					   and i.Icreditofiscal = 0
		...
		<cfquery datasource="#session.DSN#">
			update DRemisionesFA
			   set DOMontIeps =
				coalesce((
			select
			     round((round(ROUND(DRemisionesFA.DOtotal-DRemisionesFA.DOmontodesc,2) * COALESCE(d.ValorCalculo/100,0),2)),2)
					 from Impuestos d
					 where d.Ecodigo = DRemisionesFA.Ecodigo
					   and d.Icodigo = DRemisionesFA.codIEPS
					   and d.Icompuesto = 0
		...
		<cfquery name="rsRedondeoOC" datasource="#session.DSN#">
			select 	round(sum(DRemisionesFA.DOimpuestoCosto),2)	as DOimpuestoCosto,
					sum(round(DRemisionesFA.DOimpuestoCosto,2))	as DOimpuestoCosto2,

					round(sum(DRemisionesFA.DOimpuestoCF),2)		as DOimpuestoCF,
					sum(round(DRemisionesFA.DOimpuestoCF,2))		as DOimpuestoCF2,

					max(coalesce(DRemisionesFA.DOimpuestoCosto,0)+coalesce(DRemisionesFA.DOimpuestoCF,0))	as MaxImpuesto
			 from DRemisionesFA
			where DRemisionesFA.EOidorden = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.EOidorden#">
		</cfquery>
		<cfquery name="rsSQL" datasource="#session.DSN#">
			select coalesce(min(DOlinea),0) as DOlinea
			  from DRemisionesFA
			 where DRemisionesFA.EOidorden = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.EOidorden#">
			   and coalesce(DRemisionesFA.DOimpuestoCosto,0)+coalesce(DRemisionesFA.DOimpuestoCF,0) = #rsRedondeoOC.MaxImpuesto#
		</cfquery>
		...
		<cfquery name="rsTotalesOC" datasource="#session.DSN#">
			select 	coalesce(sum( round(DRemisionesFA.DOtotal-DRemisionesFA.DOmontodesc,2) ),0)	as TotalLineas,
					coalesce(sum(DRemisionesFA.DOimpuestoCosto + DRemisionesFA.DOimpuestoCF),0)	as TotalImpuestos,
					coalesce(sum(round(DRemisionesFA.DOMontIeps + DRemisionesFA.DOMontIepsCF,2)),0) as TotalIeps,
	...
	<cffunction name="esMultiperiodo" access="private" output="no" returntype="boolean">
		...
		<cfquery name="insert" datasource="#session.DSN#">
			insert into ERemisionesFA ( 	Ecodigo, EOnumero, EOsecuencia,
