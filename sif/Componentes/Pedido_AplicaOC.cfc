     ...
     <cffunction name="fnAplicaOCPrivate" access="private" output="no" returntype="numeric">
			...
			<cfif rsLineasProceso.cantidad gt 0 and rsValorP.Pvalor eq 1 and rsPlanCompras.Pvalor eq 1>
			   <cfloop query="dataCuentaF">
			       <cfquery name="rsUpdate" datasource="#session.dsn#">
				    update DSolicitudCompraCM set DScantsurt = (select DOcantidad  from DPedido do
                      ...
                      where  exists ( select 1 from  DPedido do
					  				  where do.DSlinea = DSolicitudCompraCM.DSlinea
										and do.EOidorden = #Arguments.EOidorden#
										and do.DSlinea = #dataCuentaF.DSlinea#
										and do.DOcantidad  > 0
				   ...
				   <cfif ObtCantCancelar.CantCancelar gt 0 >
					...
					<cfquery name="rsUpdate" datasource="#session.dsn#">
				    	update DSolicitudCompraCM set DScantsurt =  DScantsurt - (select DOcantidad  from DPedido do
					                                                      where do.DSlinea = DSolicitudCompraCM.DSlinea
																		   and do.EOidorden = #Arguments.EOidorden#
																		   and do.DSlinea = #dataCuentaF.DSlinea#
																		   and do.DOcantidad  > 0
		...
		<cfquery name="rsPeriodoAuxiliar" datasource="#session.DSN#">
            ...
            <cfquery name="rsOCD" datasource="#session.DSN#">
            	select COUNT(1) reg from Epedido eoc
				inner join DPedido doc on doc.EOidorden=eoc.EOidorden
            ...
            <cfquery name="rsOCD1" datasource="#session.dsn#">
            	select COUNT(1) as reg from Epedido eoc inner join DPedido doc on doc.EOidorden=eoc.EOidorden
...
<!--- JMRV Fin --->
			...
			<cfif LvarNAP GTE 0>
				...
				<cfquery name="rsCantidadSinSurtir" datasource="#session.DSN#">
					...
					from DPedido
						...
						on  DPedido.ESidsolicitud = DSolicitudCompraCM.ESidsolicitud
						and DPedido.Ecodigo       = DSolicitudCompraCM.Ecodigo
					where DPedido.EOidorden = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.EOidorden#">
		...
		<cfargument name="CRid"				default="-1"					type="numeric">
			...
			<cflock name="LCK_EOrdenCM#Arguments.Ecodigo#" timeout="20" throwontimeout="yes" type="exclusive">
				...
				<cfquery name="insert" datasource="#session.DSN#">
					insert into EPedido ( Ecodigo, EOnumero, SNcodigo, CMTOcodigo, CMCid, Mcodigo, Rcodigo, EOfecha, Observaciones, EOtc, EOrefcot, Impuesto, EOdesc, EOtotal, Usucodigo, EOplazo, EOhabiles, EOfalta, EOporcanticipo, CMFPid, CMIid, EOdiasEntrega, EOtipotransporte, EOlugarentrega, CRid)
					values (
						<cfqueryparam cfsqltype="cf_sql_integer" 	value="#Arguments.Ecodigo#">,
	...
	<cffunction name="insert_DOrdenCM" access="public" output="no" returntype="numeric">
        ...
        <cfif len(TRIM(Arguments.CTDContid))>
			<cfquery name="rsOrdenTC" datasource="#session.dsn#">
				select Mcodigo,EOtc from Epedido where EOidorden = #EOidorden#
			</cfquery>
		...
		<cfquery name="insert" datasource="#session.DSN#">
			insert into DPedido ( Ecodigo, EOidorden, EOnumero, DOconsecutivo, ESidsolicitud, DSlinea,
	...
	<cffunction name="update_DOrdenCM" access="public" output="no" returntype="void">
		...
		<cfif getOrden.recordCount GT 0 >
			...
			<cfif getOrden.CTDContid NEQ ''>
				<cfquery name="rsOrdenTC" datasource="#session.dsn#">
					select Mcodigo,EOtc from Epedido where EOidorden = #Arguments.EOidorden#
				</cfquery>
				<cfset tcidorden = #rsOrdenTC.EOtc#>
                <cfquery name="updDetContrato" datasource="#session.dsn#">
	...
	<cffunction name="calculaTotalesEOrdenCM" access="public" output="no">
       ...
       <cfelse>
				   				...
				   				( 1 +
										coalesce((
											select i.Iporcentaje/100
											  from Impuestos i
											 where i.Ecodigo = DPedido.Ecodigo
											   and i.Icodigo = DPedido.Icodigo
											   and i.Icompuesto = 0
										...
										coalesce((
											select di.DIporcentaje/100
											 ...
											 where d.Ecodigo	= DPedido.Ecodigo
											   and d.EOidorden	= DPedido.EOidorden
											   and i.Icompuesto	= 1
		...
		<cfquery datasource="#session.DSN#">
			update DPedido
			   set DOimpuestoCosto =
				coalesce((
			select
			  case when (DPedido.CMtipo = 'S' or DPedido.CMtipo = 'A') and (e.afectaIVA = 1 or f.afectaIVA = 1) THEN
			     round( round(DPedido.DOtotal-DPedido.DOmontodesc,2) * i.Iporcentaje/100,2)
			  else
			     round((round(DPedido.DOtotal-DPedido.DOmontodesc,2) + round(ROUND(DPedido.DOtotal-DPedido.DOmontodesc,2) * COALESCE(d.ValorCalculo/100,0),2)) * i.Iporcentaje/100,2)
			  end as baseIVA
					  from Impuestos i
					  left join Impuestos d
						on  DPedido.Ecodigo=d.Ecodigo
					    and DPedido.codIEPS=d.Icodigo
					  left join Conceptos e
						on e.Cid = DPedido.Cid
					  left join Articulos f
						on f.Aid= DPedido.Aid
					 where i.Ecodigo = DPedido.Ecodigo
					   and i.Icodigo = DPedido.Icodigo
					   and i.Icompuesto = 0
					   and i.Icreditofiscal = 0
		...
		<cfquery datasource="#session.DSN#">
			update DPedido
			   set DOMontIeps =
				coalesce((
			select
			     round((round(ROUND(DPedido.DOtotal-DPedido.DOmontodesc,2) * COALESCE(d.ValorCalculo/100,0),2)),2)
					 from Impuestos d
					 where d.Ecodigo = DPedido.Ecodigo
					   and d.Icodigo = DPedido.codIEPS
					   and d.Icompuesto = 0
		...
		<cfquery name="rsRedondeoOC" datasource="#session.DSN#">
			select 	round(sum(DPedido.DOimpuestoCosto),2)	as DOimpuestoCosto,
					sum(round(DPedido.DOimpuestoCosto,2))	as DOimpuestoCosto2,

					round(sum(DPedido.DOimpuestoCF),2)		as DOimpuestoCF,
					sum(round(DPedido.DOimpuestoCF,2))		as DOimpuestoCF2,

					max(coalesce(DPedido.DOimpuestoCosto,0)+coalesce(DPedido.DOimpuestoCF,0))	as MaxImpuesto
			 from DPedido
			where DPedido.EOidorden = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.EOidorden#">
		</cfquery>
		<cfquery name="rsSQL" datasource="#session.DSN#">
			select coalesce(min(DOlinea),0) as DOlinea
			  from DPedido
			 where DPedido.EOidorden = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.EOidorden#">
			   and coalesce(DPedido.DOimpuestoCosto,0)+coalesce(DPedido.DOimpuestoCF,0) = #rsRedondeoOC.MaxImpuesto#
		</cfquery>
		...
		<cfquery name="rsTotalesOC" datasource="#session.DSN#">
			select 	coalesce(sum( round(DPedido.DOtotal-DPedido.DOmontodesc,2) ),0)	as TotalLineas,
					coalesce(sum(DPedido.DOimpuestoCosto + DPedido.DOimpuestoCF),0)	as TotalImpuestos,
					coalesce(sum(round(DPedido.DOMontIeps + DPedido.DOMontIepsCF,2)),0) as TotalIeps,
	...
	<cffunction name="esMultiperiodo" access="private" output="no" returntype="boolean">
		...
		<cfquery name="insert" datasource="#session.DSN#">
			insert into EPedido ( 	Ecodigo, EOnumero, EOsecuencia,
