<cfcomponent>
	<cffunction name="fnMontoEnLetras" output="false" returntype="string" access="public">
		...
		<cfargument name="Moneda" 	type="string" required="no" default="">
			<!--- 
					Ingles = 
						0 Español
						1 USA
		...
		<cfif Arguments.Moneda NEQ "">
			<cfset Arguments.Moneda = " " & trim(Arguments.Moneda)>
		</cfif>
		<cfif Arguments.Ingles EQ 0>
			<cfset LvarEnLetras = "#trim(LvarEnLetras)# pesos #LvarDecimales#/100#replace(Arguments.Moneda,'Pesos ','','all')#">
		<cfelse>
			<cfset LvarEnLetras = "#trim(LvarEnLetras)##Arguments.Moneda# and #LvarDecimales#/100">
		</cfif>
	...
	<cffunction name="fnProcesaGrupo" output="false" returntype="string" access="private">
		...
		<cfscript>
			...
			{
				LvarGrupoEnLetras = LvarGrupoEnLetras & "";
			}
