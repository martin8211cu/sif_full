...
<!--- 	Balance General	--->
	...
	<cffunction name="CreaCuentas" access="public" output="false" returntype="string">
        <cf_dbtemp name="cuenta1" returnvariable="cuentas">
            ...
            <cf_dbtempcol name="PCDvalor"	    type="varchar(100)"     mandatory="no">
            ...
            <cf_dbtempcol name="debito"	  		type="money"    		mandatory="yes">
	...
	<cffunction name='estadoResultados' access='public' output='true' returntype="query">
        ...
        <cfargument name='Resumido' type='boolean' required='false' default="0">
		<cfargument name='CorteMayor' type='boolean' required='false' default="0">
		...
		<cfquery datasource="#session.DSN#" >
			...
			where 1=1
            <cfif arguments.CGARid GT 0>
            	and ar.CGARid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.CGARid#">
            </cfif>				
			  and cm.Cmayor in ( 
        ...
        <!--- Actualiza los Valores del Plan de Cuentas --->
        <cfquery datasource="#session.DSN#">
        	update #cuentas#
            set PCDvalor = coalesce((select PCDvalor 
            							from PCDCatalogo 
                                        where PCDcatid = #cuentas#.PCDcatid),'')
        </cfquery>
		...
		<cfif arguments.CGARepid eq 2 >
			...
			<cfquery name="rsUtil1" datasource="#Arguments.conexion#">
				insert INTO #cuentas# (CGARid, PCDcatid, nivel, cmayor, descrip, ccuenta, saldoini, debito, credito, saldofin, movmes, csubtipo, corte, ctipo, ntipo, cbalancen, periodo, mes)
		...
		<cfquery name="datos" datasource="#session.DSN#">
			select
                ...
                c1.nivel,
                <cfif not arguments.Resumido OR arguments.CorteMayor> 
                	c1.cmayor as mayor, 
                </cfif>
                <cfif not arguments.Resumido> 
                	c1.cmayor as mayor, 
                    c1.formato, 
                ...
                sum(c1.debito) as debito,
                sum(c1.credito) as credito,
				sum(c1.saldofin) as saldofin,
				sum(c1.movmes) as movmes,
                min(PCDvalor) as PCDvalor
			from #cuentas# c1
			where nivel <= #arguments.nivel#
            <cfif not arguments.Resumido>
            	group by c1.CGARid, c1.corte, c1.cmayor, c1.formato,c1.nivel, c1.csubtipo, c1.ntipo, c1.ctipo, c1.cbalancen, c1.PCDcatid
                order by c1.CGARid, c1.corte, c1.cmayor, c1.formato, c1.PCDcatid
            <cfelseif arguments.Resumido and arguments.CorteMayor>
            	group by c1.CGARid, c1.cmayor, c1.nivel, c1.PCDcatid
                order by c1.CGARid, c1.cmayor, c1.PCDcatid
            <cfelse>
            	group by c1.CGARid, <!---c1.corte, c1.cmayor, ---> c1.nivel, c1.PCDcatid
                order by c1.CGARid, <!---c1.corte, c1.cmayor, ---> c1.PCDcatid
            </cfif>
		</cfquery>
		<cfreturn datos >
	</cffunction>
