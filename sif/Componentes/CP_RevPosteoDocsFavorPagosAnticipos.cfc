...
<cfcomponent>
	<cffunction name="CP_PosteoDocsFavorCxP" access="public" output="false" returntype="string">
		<cfargument name='HEACPid' 		type='numeric' 	required='true' 	hint="ID del historico">
		<cfargument name='ID' 			type='numeric' 	required='true' 	hint="ID del Documento">
		<cfargument name='Ecodigo'		type='numeric' 	required='true' 	default="#Session.Ecodigo#" hint="Codigo empresa">
		...
		<cfquery name="rsSQL" datasource="#LvarGblConexion#">
			...
			from HEAplicacionCP a
			where a.HEACPid = #LvarHEACPid#
				and a.ID = #LvarGblID#
		</cfquery>
		...
		<cfset LvarDescripcion = "Cancelación Aplicación de Doc. Favor CxP: " & LvarCPTcodigo & "-" & LvarDdocumento>

		<cfquery datasource="#LvarGblConexion#" name="rsSQL">
			select sum(DAmonto) as EAmonto
			  from HDAplicacionCP b
			 where b.HEACPid = #LvarHEACPid#
				and b.ID = #LvarGblID#
		</cfquery>
		...
		<cftransaction>
			...
			<cfif varValidaEvento GT 0>
                ...
                <cfloop query="rsDocumentoAplica">
                    ...
                    <cfquery datasource="#LvarGblConexion#">
                        ...
                        where HEACPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#LvarHEACPid#">
						and ID = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsDocumentoAplica.ID#">
            ...
            <!---<cfset fnContabilizaGasto(varNumeroEvento)> Revisar cuando se ponga en empresa Gubernamental--->
...
<!--- Control Evento Fin --->
			...
			--->
			<cfquery datasource="#LvarGblConexion#" name="rsBMovimientosCxP">
				select
				...
				from HEAplicacionCP c
					inner join HDAplicacionCP b
					...
					on b.ID = c.ID and b.HEACPid = c.HEACPid
				where c.HEACPid = #LvarHEACPid# 
			  		and c.ID = #LvarGblID#
			</cfquery>

			<cfloop query="rsBMovimientosCxP">
                <cfquery datasource="#LvarGblConexion#">
                    delete from BMovimientosCxP
                    where Ecodigo = #rsBMovimientosCxP.Ecodigo#
                        and CPTcodigo = '#rsBMovimientosCxP.CPTcodigo#'
                        and Ddocumento = '#rsBMovimientosCxP.Ddocumento#'
                        and CPTRcodigo = '#rsBMovimientosCxP.DAtransref#'
                        and DRdocumento = '#rsBMovimientosCxP.DAdocref#'
                </cfquery>
            </cfloop>
			...
			--->
			<!---<cf_dbsetIdentity table="EDocumentosCP" setting="ON" datasource="#LvarGblConexion#">
			<cf_dbsetIdentity table="DDocumentosCP" setting="ON" datasource="#LvarGblConexion#">--->
			...
			<cfloop query="rsSQL">
				...
				<cfquery datasource="#LvarGblConexion#" name="rsEDocumentosCP">
					select count(*) existe
					from EDocumentosCP a
					WHERE a.Ecodigo = #LvarDEcodigo#
						AND a.CPTcodigo = '#LvarDDAtransref#'
						AND a.Ddocumento = '#LvarDDAdocref#'
						AND a.IDdocumento = #LvarDDAidref#
						AND a.SNcodigo = #LvarDSNcodigo#
						AND a.IDdocumento = #LvarDDAidref#
				</cfquery>
				
				<!--- Actualizacion de saldos a nivel Encabezado --->

				<cfif isDefined("rsEDocumentosCP") and rsEDocumentosCP.Existe GT 0>
					<cfquery datasource="#LvarGblConexion#">
						UPDATE EDocumentosCP
						SET EDsaldo = EDsaldo + coalesce(
														( SELECT sum(DAmontodoc)
															FROM HDAplicacionCP c
															WHERE c.HEACPid = #LvarHEACPid# 
															AND c.ID = #LvarGblID#
				...
				<cfelse>
					<cftry>
						<cf_dbsetIdentity table="EDocumentosCP" setting="ON" datasource="#session.dsn#">
							<cfquery datasource="#LvarGblConexion#">
								insert into EDocumentosCP (IDdocumento,Ecodigo,CPTcodigo,Ddocumento,SNcodigo,Mcodigo,Ocodigo,Ccuenta,Rcodigo,Icodigo,CFid,id_direccion,
											Dtipocambio,Dtotal,EDsaldo,Dfecha,Dfechavenc,EDtcultrev,EDtref,EDdocref,EDmontoretori,EDretporigen,
											Dfechaarribo,EDusuario,BMUsucodigo,TESDPaprobadoPendiente,TESRPTCid,NAP,NRP,EDtipocambioFecha,
											EDtipocambioVal,TESanticipoAnulado,TESRPTCietu,EVestado,folio,EDAdquirir, EDexterno, TimbreFiscal,EDTiEPS,
											EDretencionVariable,FolioReferencia,afecIVA)
								select
									t.IDdocumento,t.Ecodigo,t.CPTcodigo,t.Ddocumento,t.SNcodigo,t.Mcodigo,t.Ocodigo,t.Ccuenta,t.Rcodigo,t.Icodigo,t.CFid,t.id_direccion,
									t.Dtipocambio,t.Dtotal,t.Dtotal,t.Dfecha,t.Dfechavenc,t.EDtcultrev,t.EDtref,t.EDdocref,t.EDmontoretori,t.EDretporigen,
									t.Dfechaarribo,t.EDusuario,t.BMUsucodigo,0.00,t.TESRPTCid,t.NAP,t.NRP,EDtipocambioFecha,
									t.EDtipocambioVal,t.TESanticipoAnulado,t.TESRPTCietu,t.EVestado,t.folio,t.EDAdquirir, t.EDexterno, t.TimbreFiscal,t.EDTiEPS,
									t.EDretencionVariable,t.FolioReferencia,0
								from HEDocumentosCP t
								WHERE t.Ecodigo = #LvarDEcodigo#
									AND t.CPTcodigo = '#LvarDDAtransref#'
									AND t.Ddocumento = '#LvarDDAdocref#'
									AND t.IDdocumento = #LvarDDAidref#
									AND t.SNcodigo = #LvarDSNcodigo#
									AND t.IDdocumento = #LvarDDAidref# 
							</cfquery>
					...
					<cftry>
						<cf_dbsetIdentity table="DDocumentosCP" setting="ON" datasource="#session.dsn#">
							<cfquery datasource="#LvarGblConexion#">
								insert into DDocumentosCP (IDdocumento,DDlinea,Ecodigo,Dcodigo,CPTcodigo, Ddocumento,SNcodigo,DDtransref,DDcoditem, Aid, Ccuenta, DDtipo, DDcantidad,
											DDpreciou,DDtotallin,DDescripcion,DDdescalterna,DDdesclinea,DDtransito,DDembarque,DDfembarque,DDobservaciones,BMUsucodigo,
											Icodigo,DOlinea,CFid,NumeroBOL,FechaBOL,TripNo,ContractNo,Ucodigo, OCTid, OCCid, OCid,EDid,DDid,CFcuenta,FPAEid,
											CFComplemento,UsucodigoModifica,CPDCid,CTDContid,codIEPS,DDMontoIeps,DDdocref)
								select
									IDdocumento,DDlinea,Ecodigo,Dcodigo,CPTcodigo,Ddocumento,SNcodigo,DDtransref,DDcoditem, Aid, Ccuenta, DDtipo, DDcantidad,
									DDpreciou,DDtotallin,DDescripcion,DDdescalterna,DDdesclinea,DDtransito,DDembarque,DDfembarque,DDobservaciones,BMUsucodigo,
									Icodigo,DOlinea,CFid,NumeroBOL,FechaBOL,TripNo,ContractNo,Ucodigo, OCTid, OCCid, OCid,EDid,DDid,CFcuenta,FPAEid,
									CFComplemento,UsucodigoModifica,CPDCid,CTDContid,codIEPS,DDMontoIeps,DDdocref
								from HDDocumentosCP 
								WHERE Ecodigo = #LvarDEcodigo#
									AND CPTcodigo = '#LvarDDAtransref#'
									AND Ddocumento = '#LvarDDAdocref#'
									AND IDdocumento = #LvarDDAidref#
									AND SNcodigo = #LvarDSNcodigo#
									AND IDdocumento = #LvarDDAidref# 
							</cfquery>
						<cf_dbsetIdentity table="DDocumentosCP" setting="OFF" datasource="#session.dsn#">
					<cfcatch type="any">
						<cf_dbsetIdentity table="DDocumentosCP" setting="OFF" datasource="#session.dsn#">
						<cfrethrow>
					</cfcatch>
			...
			<cfquery datasource="#LvarGblConexion#" name="rsEDocumentosCP">
				select count(*) existe
				from EDocumentosCP a
				where a.Ecodigo    = #LvarEEcodigo#
				  and a.CPTcodigo  = '#LvarECPTcodigo#'
				  and a.Ddocumento = '#LvarEDdocumento#'
				  and a.SNcodigo   = #LvarESNcodigo#
			</cfquery>
			...
			<cfelse>
				<cftry>
					<cf_dbsetIdentity table="EDocumentosCP" setting="ON" datasource="#session.dsn#">
						<cfquery datasource="#LvarGblConexion#" name="rsPrueba">
							insert into EDocumentosCP (IDdocumento,Ecodigo,CPTcodigo,Ddocumento,SNcodigo,Mcodigo,Ocodigo,Ccuenta,Rcodigo,Icodigo,CFid,id_direccion,
										Dtipocambio,Dtotal,EDsaldo,Dfecha,Dfechavenc,EDtcultrev,EDtref,EDdocref,EDmontoretori,EDretporigen,
										Dfechaarribo,EDusuario,BMUsucodigo,TESDPaprobadoPendiente,TESRPTCid,NAP,NRP,EDtipocambioFecha,
										EDtipocambioVal,TESanticipoAnulado,TESRPTCietu,EVestado,folio,EDAdquirir, EDexterno, TimbreFiscal,EDTiEPS,
							...
							select
								t.IDdocumento,t.Ecodigo,t.CPTcodigo,t.Ddocumento,t.SNcodigo,t.Mcodigo,t.Ocodigo,t.Ccuenta,t.Rcodigo,t.Icodigo,t.CFid,t.id_direccion,
								t.Dtipocambio,t.Dtotal, t.Dtotal - (EDsaldo - #LvarEAtotal#) as Saldo,t.Dfecha,t.Dfechavenc,t.EDtcultrev,t.EDtref,t.EDdocref,t.EDmontoretori,t.EDretporigen,
								t.Dfechaarribo,t.EDusuario,t.BMUsucodigo,0.00,t.TESRPTCid,t.NAP,t.NRP,EDtipocambioFecha,
								t.EDtipocambioVal,t.TESanticipoAnulado,t.TESRPTCietu,t.EVestado,t.folio,t.EDAdquirir, t.EDexterno, t.TimbreFiscal,t.EDTiEPS,
								t.EDretencionVariable,t.FolioReferencia,0
							from HEDocumentosCP t
							where t.Ecodigo    = #LvarEEcodigo#
								and t.CPTcodigo  = '#LvarECPTcodigo#'
								and t.Ddocumento = '#LvarEDdocumento#'
								and t.SNcodigo   = #LvarESNcodigo#
						</cfquery>
			...
			<!---<cf_dbsetIdentity table="EDocumentosCP" setting="OFF" datasource="#LvarGblConexion#">
			<cf_dbsetIdentity table="DDocumentosCP" setting="OFF" datasource="#LvarGblConexion#">--->
			<cfquery datasource="#LvarGblConexion#">
				update HEAplicacionCP
				set HEACPreversa = 1
				where HEACPid = #LvarHEACPid# 
			  		and ID = #LvarGblID#
			</cfquery>
		...
		<cfquery datasource="#LvarGblConexion#">
			...
			from HEAplicacionCP c
				inner join HDAplicacionCP a
				 ...
				 on a.ID = c.ID and a.HEACPid = c.HEACPid
			where c.HEACPid = #LvarHEACPid# 
			  and c.ID = #LvarGblID#
		</cfquery>
		<!---
	...
	<cffunction name="fnInsertaMovimientoImpuestos" access="private" output="no">
		...
		<cfquery name="rsMovimientoImpuestos" datasource="#LvarGblConexion#">
			...
			AND b.Icodigo = i.Icodigo ON a.ID = det.ID and a.HEACPid = det.HEACPid
			where a.HEACPid = #LvarHEACPid#
				and a.ID = #LvarGblID#
		</cfquery>
		...
		<!---1.  Nota Crédito --->
		<cfquery datasource="#LvarGblConexion#" name="rsImpEncabezado">
			select
				x.IDdocumento,
				b.Icodigo,
				b.Ecodigo,
		...
		<cfloop query="rsImpEncabezado">
            <cfquery datasource="#LvarGblConexion#">
                delete from ImpDocumentosCxPMov
                where Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsImpEncabezado.Ecodigo#"> 
                    and IDdocumento = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsImpEncabezado.IDdocumento#"> 
                    and Fecha = <cfqueryparam cfsqltype="cf_sql_date" value="#rsImpEncabezado.EAfecha#">
                    and Icodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsImpEncabezado.Icodigo#">
                    and CPTpago = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsImpEncabezado.CPTpago#"> 
                    and CPTcodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsImpEncabezado.CPTcodigo#"> 
            </cfquery>
        </cfloop>
		
		<!--- 2. Facturas --->
		<cfquery datasource="#LvarGblConexion#" name="rsImpDetalle">
			select
				b.IDdocumento,
				b.Icodigo,
				b.Ecodigo,
			...
			from HEAplicacionCP a
				inner join HDAplicacionCP det
						...
						on b.Ecodigo = i.Ecodigo
						and b.Icodigo = i.Icodigo
				on a.ID = det.ID and a.HEACPid = det.HEACPid
			where a.HEACPid = #LvarHEACPid#
				and a.ID = #LvarGblID#
		</cfquery>

		<cfloop query="rsImpDetalle">
            <cfquery datasource="#LvarGblConexion#">
                delete from ImpDocumentosCxPMov
                where Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsImpDetalle.Ecodigo#"> 
                    and IDdocumento = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsImpDetalle.IDdocumento#"> 
                    and Fecha = <cfqueryparam cfsqltype="cf_sql_date" value="#rsImpDetalle.EAfecha#">
                    and Icodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsImpDetalle.Icodigo#">
                    and CPTpago = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsImpDetalle.CPTpago#"> 
                    and CPTcodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsImpDetalle.CPTcodigo#"> 
            </cfquery>
        </cfloop>
	...
	<cffunction name="fnContabilizaTrasladoImpuestos" access="private" output="no">
        ...
        <cfquery name="rsint" datasource="#LvarGblConexion#">
			...
			SELECT INTORI,
				   ...
				   CPTcodigo,
				   SUM(iva1) iva1,
				   case when CPTtipo = 'D' then 'C' else 'D' end,
				   Idescripcion,
				   fecha,
				   EAtipocambio,
				   periodo,
			...
			FROM HEAplicacionCP a
			INNER JOIN HDAplicacionCP da ON a.Ecodigo = da.Ecodigo
			AND a.ID = da.ID and a.HEACPid = da.HEACPid
			INNER JOIN HEDocumentosCP x ON da.Ecodigo = x.Ecodigo
		...
		<cfquery datasource="#LvarGblConexion#">
			...
			FROM HDAplicacionCP AS Table_A
				INNER join  (SELECT
			...
			WHERE Table_A.ID = #LvarGblID#
				and Table_A.HEACPid = #LvarHEACPid#
		</cfquery>
...
<!--- 			insert into #intarc# ( INTORI, INTREL, INTDOC, INTREF, INTMON, INTTIP, INTDES, INTFEC, INTCAM, Periodo, Mes, Ccuenta, Mcodigo, Ocodigo, INTMOE, NumeroEvento) --->
			select
				...
				case when d.CPTtipo = 'C' then 'D' else 'C' end,
...
<!--- <cfdump var="#rsIva3#"> --->
        ...
        <cfquery name="rsIva4" datasource="#LvarGblConexion#">
			...
			FROM HEAplicacionCP a
			INNER JOIN HDAplicacionCP da ON a.ID = da.ID
			AND a.Ecodigo = da.Ecodigo and a.HEACPid = da.HEACPid
			INNER JOIN HEDocumentosCP ed ON ed.IDdocumento = da.DAidref
...
<!--- <cf_dump var="#rsIva4#"> --->
		...
		<cfquery name="rsD" datasource="#LvarGblConexion#">
			...
			SELECT 'CPPA',
			       ...
			       '#Arguments.NumeroEvento#'
			FROM HEAplicacionCP a
				 INNER JOIN HDAplicacionCP da ON a.ID = da.ID and a.HEACPid = da.HEACPid
				 AND a.Ecodigo = da.Ecodigo
				 inner join HEDocumentosCP e on e.IDdocumento = da.DAidref
	...
	<cffunction name="sbAfectacionIETU" access="public"  output="false">
		...
		<cfquery name="rsIETUpago" datasource="#Arguments.Conexion#">
			...
			select 	e.Ecodigo,
					...
					0		<!--- Es una Aplicacion de Anticipo --->
			   from HEAplicacionCP e
		...
		<cfquery datasource="#Arguments.Conexion#">
			...
			from HEAplicacionCP e
				inner join HDAplicacionCP d
				 ...
				 on d.ID = e.ID and d.HEACPid = e.HEACPid
			where e.HEACPid = #LvarHEACPid# 
			and e.ID = #LvarGblID#
		</cfquery>
    ...
    <cffunction name="sbPresupuestoAntsEfectivo_CxP" access="private">
		...
		<cfquery datasource="#session.DSN#" name="rsPrueba">
			...
			select '#Arguments.Origen#',
                ...
                from HEAplicacionCP c
                    inner join HDAplicacionCP a
                    inner join HEDocumentosCP b on b.Ecodigo = a.Ecodigo and b.CPTcodigo = a.DAtransref
                        and b.Ddocumento = a.DAdocref and b.SNcodigo = a.SNcodigo and b.IDdocumento = a.DAidref on a.ID = c.ID and a.HEACPid = c.HEACPid
                    inner join HDDocumentosCP d on d.IDdocumento = b.IDdocumento AND d.Ecodigo = b.Ecodigo AND d.DDtipo <> 'I'
