...
<cfcomponent>
	...
	<cffunction name="CG_VerificaImportacionAsiento_local" access="private" output="no">
		...
		<cf_dbtemp name="Iincorrectas" returnvariable="Lvar_Iincorrectas">
			...
			<cfquery datasource="#LvarConexion#">
				...
				from #LvarTablaValidacion# i
					...
					inner join #Lvar_MAYvalidas# v
						...
						and v.Cmayor = substring(i.CFformato, 1, 4)
				where ECIid     = #LvarECIid#
				  and Resultado < 1
				  and not exists(
						select 1 
						...
						where r2.Cmayor    = v.Cmayor
						  ...
						  and i.CFformato like r2.PCRregla
						  and o.Oficodigo like coalesce(r2.OficodigoM, '%')
						  and not exists (
								select 1 
								from PCReglas r3
								where r3.PCRref = r2.PCRid
								  ...
								  and i.CFformato like r3.PCRregla
								  and o.Oficodigo like coalesce(r3.OficodigoM, '%')
							)
						)
			</cfquery>
				...
				where ECIid     = #LvarECIid#
				  ...
				  and exists(
						...
						where r4.Cmayor    = v.Cmayor
						  ...
						  and i.CFformato like r4.PCRregla
						  and o.Oficodigo like coalesce(r4.OficodigoM, '%')
						  and not exists (
								select 1 
								from PCReglas r5
								where r5.PCRref = r4.PCRid
								  ...
								  and i.CFformato like r5.PCRregla
								  and o.Oficodigo like coalesce(r5.OficodigoM, '%')
							)
						)
			</cfquery>
			...
			<cfif rsVerificaCuentasValidas.CantidadIncorrectas NEQ 0>
				<cfset LvarVerificacionOK = "NO. Hay Cuentas Erroneas en la Importacion">
			</cfif>
	...
	<cffunction name="CG_AplicaImportacionAsiento" access="public" output="no">
		...
		<cfif (LvarPeriodoAsiento) GT (LvarPeriodoActual + 1)>
			<cfthrow message="El Asiento corresponde al periodo #rsEncabezado.Emes#-#rsEncabezado.Eperiodo#. El periodo actual es  #rsMes.Pvalor#-#rsPeriodo.Pvalor#.  Proceso Cancelado!">
		</cfif>
		...
		<cfif oficinas_invalidas.cantidad GT 0>
			<cfthrow message="Existen oficinas no definidas en el documento contable a importar.">
		</cfif>
		...
		<cfif Intercompany>
			...
			<cfif rsbalanceo.recordCount GT 0>
				<cfthrow message="El monto en Moneda Origen no está balanceado para alguna de las Monedas o la Moneda NO esta definida en la Empresa destino!">
			</cfif>
			...
			<cfif rsbalanceo.recordCount GT 0>
				<cfthrow message="El monto en Moneda Local no está balanceado para alguna de las monedas o la Moneda NO esta definida en la Empresa destino!">
			</cfif>
		...
		<cfelse>
			...
			<cfif balanceo_origen.recordCount GT 0>
				<cfthrow message="El monto en moneda origen por oficina no está balanceado para alguna de las monedas">
			</cfif>
			...
			<cfif balanceo_local.recordCount GT 0>
				<cfthrow message="El monto en moneda local por oficina no está balanceado para alguna de las monedas">
			</cfif>
		</cfif>
		...
		<cfif rsVerificaCuentasValidas.CantidadIncorrectas NEQ 0>
			...
			<cfif LvarVerificacionOK NEQ "OK">
				<cfthrow message="No se han Validado las cuentas o Existen Cuentas NO Válidas.  Por Favor Verifique las cuentas">
			</cfif>
		</cfif>
	...
	<cffunction name="importarRegistros" access="private" output="no" returntype="numeric" hint="Regresa el número del Asiento Contable generado ( IDcontable )">
		...
		<cfinvoke 
		 ...
		 returnvariable="Nuevo_AsientoRet">
		 	<cfinvokeargument name="Cconcepto" value="#rsEncabezado.Cconcepto#"/>
		...
		<cfquery name="rsEContables" datasource="#Arguments.Conexion#">
			insert into EContables (
					...
					ECusuario, ECusucodigo, ECfechacreacion, ECipcrea, ECestado, BMUsucodigo, ECtipo, ECreversible)
			values (
				<cfqueryparam cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">,
				<cfqueryparam cfsqltype="cf_sql_integer" value="#rsEncabezado.Cconcepto#">, 
		...
		<cfelse>
			...
			<cfquery name="rsDContables" datasource="#Arguments.Conexion#">
				...
				select
					...
					Ecodigo, 
					Cconcepto, 
					<cfqueryparam cfsqltype="cf_sql_integer" value="#rsEncabezado.Eperiodo#">, 
					<cfqueryparam cfsqltype="cf_sql_integer" value="#rsEncabezado.Emes#">,
					#Nuevo_AsientoRet#, 
					Ocodigo, 
					Ddescripcion, 
					Ddocumento, 
				...
				<cfquery datasource="#Arguments.Conexion#">
						...
						and exists(
							...
							from GACMayor c
								...
								and b.CFformato like {fn concat(c.Cmascara,'%')}
						)
				</cfquery>
		</cfif>
