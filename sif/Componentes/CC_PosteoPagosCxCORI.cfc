...
<cffunction name="sbAplicaProceso" access="public">
	...
	<cfquery datasource="#Arguments.Conexion#" name="rsValidaPagos">
		...
		where p.Ecodigo 	= #Arguments.Ecodigo#
		  ...
		  and #preservesingleQuotes(LvarPfecha)# < #preservesingleQuotes(LvarDfecha)#       
	</cfquery>
	...
	<cfif NumberFormat((#rsPagos.Ptotal# - #rsAPagosCxCTotal.TotalAnticipos# - #rsAPagosCxCTotal.TotalCubierto#),',9.00') NEQ 0.00>
		...
		<cf_errorCode	code = "51010"
						...
						errorDat_2="#NumberFormat(rsAPagosCxCTotal, ',9.00')#"
           ...
           <cfelse> 
			...
			<cfquery datasource="#Arguments.Conexion#">
				...
				select 
					...
					a.CCTcodigo, 
						<!--- 
							MONTO * Dtcultrev - MONTO * TC_DOC
							MONTO * (Dtcultrev - TC_DOC)
							MONTO * (Dtcultrev - (Ptipocambio / DPtipocambio))
							MONTO * (Dtcultrev - TC_DOC * MONTO/MONTO)
							MONTO * (Dtcultrev - round(TC_DOC * MONTO,2)/round(MONTO,2))
							abs( round( MONTO * (Dtcultrev - round(TC_DOC * MONTO,2)/round(MONTO,2)),2))
								TC_DOC = (Ptipocambio / DPtipocambio)
								MONTO  = (DPmontodoc + DPmontoretdoc)
							abs( round( (DPmontodoc + DPmontoretdoc) * (Dtcultrev - round((Ptipocambio / DPtipocambio) * (DPmontodoc + DPmontoretdoc),2)/round((DPmontodoc + DPmontoretdoc),2)),2))
						--->
  ...
  <cfset sbAfectacionIETU("CCRE", Arguments.Ecodigo, Arguments.CCTcodigo, Arguments.Pcodigo, 
				...
				<cfif rsVerificaNegativos.Recordcount GT 1>
					...
					<cfloop query="rsVerificaNegativos">
						<cfset LvarMensajeNegativos = LvarMensajesNegativos & " #rsVerificaNegativos.CCTcodigo# #rsVerificaNegativos.Ddocumento# #rsVerificaNegativos.SNnombre#">
					</cfloop>
				...
				<!--- 8) Grabar los Documento de los Anticipo Generado (si Tienen anticipos) --->
                    ...
                    <cfquery name="rsPagosInfo" datasource="#session.dsn#">
                     select CBid, Mcodigo,Pfecha, Preferencia, Ptotal, Ptipocambio from 
                      Pagos a							
						where a.Ecodigo   = #Arguments.Ecodigo#
						  and a.CCTcodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.CCTcodigo#">
						  and a.Pcodigo   = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.Pcodigo#">
