<cfcomponent>
 	<cffunction name="CreaTablaIntPresupuesto" output="no" returntype="string" access="public">
		...
		<cf_dbtemp name="#LvarNombre#" returnvariable="CP_Interfase_Presupuesto_NAME" datasource="#Arguments.Conexion#">
			...
			<cf_dbtempcol name="NumeroDocumento"	type="char(30)"     mandatory="yes">
			...
			<cf_dbtempcol name="CPCanoP"    			type="int"          mandatory="no">
			<cf_dbtempcol name="CPCmesP"    			type="int"          mandatory="no">
		</cf_dbtemp>
...
<!--- Control Evento Fin --->
		<cfargument name="PagoAnoAnteriores"type="string"  	required='false' default="">
		<cfargument name="validaComp" 		type="boolean" 	required='false' default="false"> <!--- para validar si es una modificacion al compromiso programa-robro-subrubro --->
		...
		<cfif rsSQL.recordCount LTE 1>
			...
			<cfset LvarIC.NAP = ControlPresupuestario (
								...
								Conexion, Ecodigo,Arguments.Momentos,
...
<!--- Control Evento Inicia --->
								,Arguments.PagoAnoAnteriores
								,Arguments.validaComp
								)>
		...
		<cfloop query="LvarIC.rsIntPresupuesto">
			...
			<cftry>
				...
				<cfset LvarIC.NAP = ControlPresupuestario (
									ModuloOrigen, NumeroDocumento, NumeroReferencia,
									FechaDocumento, AnoDocumento, MesDocumento,
									Conexion, LvarIC.rsIntPresupuesto.EcodigoDst, Arguments.Momentos,
									NAPreversado, SoloConsultar, VerErrores,
...
<!--- Control Evento Inicia --->
        <cfargument name="NumeroEvento" 	type="string"  	required='false' default="">
<!--- Control Evento Fin --->
		<cfargument name="PagoAnoAnteriores"type="string"  	required='false' default="0">
		<cfargument name="validaComp" 		type="boolean" 	required='false' default="false"> <!--- para validar si es una modificacion al compromiso programa-robro-subrubro --->
		...
		<cfif rsPRFO.recordCount GT 0>
			<cfif  ModuloOrigen EQ 'PRFO' AND rsPRFO.recordCount EQ 1>
				<cfset sbGeneracionMasiva(trim(rsPRFO.TipoMovimiento),Arguments.Conexion, Arguments.Ecodigo)>
				<cflog type="information" file="FormulacionAplica" text="Inicia Generación de NAP">
				...
				<cfinvoke 	component	= "PRES_ContaPresupuestaria"
                            ...
                            Momentos    = "#Arguments.Momentos#"
				>
				<cflog type="information" file="FormulacionAplica" text="fnGeneraNAP">
				<cfreturn LvarNAP>
			<cfelse>
        ...
        <cfset varNumeroDocumento = #Arguments.NumeroDocumento#>
		...
		<cfif rsNAP.recordCount GT 0>
        	<cfif Arguments.ModuloOrigen eq "CCAP" and Arguments.NumeroReferencia eq "NC">
                <cfquery name="rsNAPCont" datasource="#Arguments.Conexion#">
                    select COUNT(*) as contador
                     from CPNAP
                     where Ecodigo 				= #Arguments.Ecodigo#
                       and EcodigoOri		 	= #Arguments.EcodigoOrigen#
                       and CPNAPmoduloOri 		= <cf_jdbcquery_param cfsqltype="cf_sql_varchar" LEN="4" value="#Arguments.ModuloOrigen#">
                       and CPNAPdocumentoOri	like <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.NumeroDocumento#%">
                       and CPNAPreferenciaOri	= <cf_jdbcquery_param cfsqltype="cf_sql_varchar" LEN="25" value="#Arguments.NumeroReferencia#">
                </cfquery>
            	<cfset varNumeroDocumento = #Arguments.NumeroDocumento# &"-"& #rsNAPCont.contador#>
            <cfelse>
				<cf_errorCode	code = "51295"
							msg  = "ERROR EN CONTROL PRESUPUESTARIO: El documento ya fue aprobado y aplicado con el NAP=@errorDat_1@"
							errorDat_1="#Arguments.NumeroDocumento#""#rsNAP.CPNAPnum#"
				>
        	</cfif>
		</cfif>
		...
		<cfif isdefined("request.CP_DescompromisosAutomatico")>
			...
			<cfset sbDescompromisosAutomatico(Arguments.Ecodigo, Arguments.Conexion,LvarCPPid)>
		</cfif>
...
<!---        <cfif rsComprAut.recordCount GT 0 and rsComprAut.CPCompromiso eq 'True'>
            <cfquery name="rsConsMensNRCD" datasource="#Arguments.Conexion#">
                select count(*) as cantidad from MensNRCD
            </cfquery>
			<cfif rsConsMensNRCD.cantidad GT 0>
                <cf_errorCode	code = "51689"
                                msg  = "ERROR EN CONTROL PRESUPUESTARIO: Se encontraron errores al aplicar la póliza: Origen=@errorDat_1@, Documento=@errorDat_2@, Referencia=@errorDat_3@"
		...
		<cfquery name="rsSQLUno" datasource="#Arguments.Conexion#">
			select count(1) as cantidad
			from #Request.intPresupuesto#
		</cfquery>

		<cfif rsSQLUno.cantidad EQ 0>
...
<!---ERBG-1 ACA INICIA EL PROCESO--->
		...
		<cfquery name="rsPRES" datasource="#Arguments.Conexion#">
			...
			order by p.CPCano, p.CPCmes, NAPreferencia desc, p.SignoMovimiento*Monto desc
		</cfquery>
		...
		<cfloop query="rsPRES">
			...
			<cfelse>
				...
				<cfelseif (rsPRES.CPCano*100+rsPRES.CPCmes LT LvarAnoMesInferior AND
		(rsPRES.TipoMovimiento NEQ "RP" and (rsPRES.TipoMovimiento NEQ "CC" and rsPRES.ModuloOrigen NEQ "CTLC")) AND trim(rsPRES.TipoMovimiento) NEQ "E" ) and not ConciliaML>
...
<!---ERBG-1 ACA ENTRA POR SER MAYOR AÑOMES--->
				<cfelseif rsPRES.CPCano*100+rsPRES.CPCmes GT LvarAnoMesSuperior AND rsPRES.TipoMovimiento NEQ "RP" AND rsPRES.TipoMovimiento NEQ "VC" AND trim(rsPRES.TipoMovimiento) NEQ "T" AND rsPRES.TipoMovimiento NEQ "TE">
					...
					<!---ERBG Parametrización Compromiso Automático INICIA--->
                	<cfif (rsComprAut.recordCount EQ 0) or (rsComprAut.recordCount GT 0 and rsComprAut.CPCompromiso eq 'False')>
				...
				<cfelseif abs( numberFormat( (rsPRES.MontoOrigen*rsPRES.TipoCambio - rsPRES.Monto),"0.00") ) GT 0.01>
					<cfif NOT (rsPRES.Mcodigo NEQ LvarMcodigoLocal AND rsPRES.MontoOrigen EQ 0 AND (rsPRES.TipoCambio EQ 0 OR rsPRES.TipoCambio EQ 1))>
						<cf_errorCode	code = "51301"
										msg  = "ERROR EN CONTROL PRESUPUESTARIO: La Linea '@errorDat_1@' del Documento intenta afectar el presupuesto con un error cambiario: '@errorDat_2@ x @errorDat_3@ = @errorDat_4@' que no corresponde al monto en moneda local enviado de '@errorDat_5@'"
										errorDat_1="#rsPRES.NumeroLinea#"
				...
				<cfset LvarREF = fnProcesaNAPReferencia(rsPRES.NumeroLinea, rsPRES.NAPreferencia, rsPRES.LINreferencia, rsPRES.CPcuenta, rsOficina.Ocodigo, rsPRES.TipoMovimiento, rsPRES.Monto, Arguments.Conexion, Arguments.Ecodigo,Arguments.validaComp)>
				...
				<cfelse>
					...
					<cfif NOT LvarControl.ConError>
						...
						<cfset sbAfectacionPresupuestaria (
									...
									LvarREF.Monto,
									LvarREF.TipoMovimiento,
									Arguments.Conexion, Arguments.Ecodigo,Arguments.PagoAnoAnteriores)>
					</cfif>
...
<!--- <cf_dump var="stop"> --->
	...
	<cffunction name="fnProcesaNAPReferencia" output="no" returntype="struct" access="private">
		...
		<cfargument name="validaComp" 		type="boolean" required='false'	default="false"> <!--- para validar si es una modificacion alcompromiso programa-rubro-subrubro --->
		...
		<cfif LvarRet.NAPreferencia EQ "" AND LvarRet.LINreferencia EQ "">
			...
			<cfelse>
				<cfif (LvarRet.TipoMovimiento EQ "RP" OR LvarRet.TipoMovimiento EQ "RC" OR LvarRet.TipoMovimiento EQ "CC") AND Arguments.Monto LT 0 AND Arguments.NumeroLinea LT 0>
					<cf_errorCode	code = "51303"
									msg  = "ERROR EN CONTROL PRESUPUESTARIO: La Linea '@errorDat_1@' del Documento está tratando de utilizar fondos reservados sin indicar ninguna Linea de NAP de Referencia"
									errorDat_1="#Arguments.NumeroLinea#"
					>
		...
		<cfif rsSQL.CPcuenta NEQ Arguments.CPcuenta>
			<cf_errorCode	code = "51314"
							msg  = "ERROR EN CONTROL PRESUPUESTARIO: La Linea '@errorDat_1@' del Documento esta referenciando una Linea de NAP = '@errorDat_2@-@errorDat_3@' de diferente Cuenta '@errorDat_4@' de Presupuesto '@errorDat_5@'"
							errorDat_1="#Arguments.NumeroLinea#"
							errorDat_2="#LvarRet.NAPreferencia#"
							errorDat_3="#LvarRet.LINreferencia#"
                            errorDat_4="#rsSQL.CPcuenta#"
                            errorDat_5="#Arguments.CPcuenta#"
			>
		</cfif>
		...
		<cfif not listFind("P,EJ",LvarRet.TipoMovimiento)>
			...
			<!--- <cfif Arguments.CPcuenta eq 10491 or Arguments.CPcuenta eq 692>
				<cfsavecontent variable="savedump">
			    <cfdump var ="Saldo = #abs(rsSQL.CPNAPDsaldo)# Monto = #abs(LvarRet.Monto)# = #abs(rsSQL.CPNAPDsaldo) - abs(LvarRet.Monto)#"><br>
			</cfsavecontent>
				<cffile action="append" file="c:\temp\cflog\dump.html" output="#savedump#" >
			</cfif> --->
				<!--- Un Monto <= 0 referenciando una linea significa que va a utilizar fondos del saldo de la referencia --->
				<cfif (rsSQL.CPNAPDsaldo EQ 0 and validaXCPVal EQ 'N') or (rsSQL.CPNAPDsaldo EQ 0 and validaXCPVal EQ 'S' and Arguments.validaComp EQ 'false')>
					...
					>
				<cfelseif (abs(LvarRet.Monto) - 0.02 GTE rsSQL.CPNAPDsaldo and validaXCPVal EQ "N") or (abs(LvarRet.Monto) - 0.02 GTE rsSQL.CPNAPDsaldo and validaXCPVal EQ 'S' and Arguments.validaComp EQ 'false')>
				...
				<cfelseif (rsSQL.CPNAPDsaldo - abs(LvarRet.Monto) LT 0 and validaXCPVal EQ "N") or (rsSQL.CPNAPDsaldo - abs(LvarRet.Monto) LT 0 and validaXCPVal EQ 'S' and Arguments.validaComp EQ 'false')>
					<cfset LvarRet.Monto = -rsSQL.CPNAPDsaldo>
				<cfelseif rsSQL.CPNAPDsaldo GT 0 and validaXCPVal EQ "S" and Arguments.validaComp EQ 'true'>
					<cfif LvarRet.Monto GT 0>
						<cfset LvarRet.Monto = (-1)*LvarRet.Monto>
					</cfif>
				</cfif>
			<cfelseif LvarRet.Monto GT 0>
				...
				<cfif (rsSQL.CPNAPDutilizado EQ 0 and validaXCPVal EQ "N") or (rsSQL.CPNAPDutilizado EQ 0 and validaXCPVal EQ 'S' and Arguments.validaComp EQ 'false')>
					<cf_errorCode	code = "51318"
									...
									errorDat_6="#fnNumberFormat(LvarRet.Monto)#"
					>
				<cfelseif (LvarRet.Monto - 0.02 GTE rsSQL.CPNAPDutilizado and validaXCPVal EQ "N") or ( LvarRet.Monto - 0.02 GTE rsSQL.CPNAPDutilizado and validaXCPVal EQ 'S' and Arguments.validaComp EQ 'false')>
				...
				<cfelseif (LvarRet.Monto GT rsSQL.CPNAPDutilizado and validaXCPVal EQ "N") or ( LvarRet.Monto GT rsSQL.CPNAPDutilizado and validaXCPVal EQ 'S' and Arguments.validaComp EQ 'false')>
	...
	<cffunction name="sbAfectacionPresupuestaria" output="no" access="private">
		...
		<cfargument name="Conexion" 			type="string"  	required="yes">
		<cfargument name="Ecodigo" 				type="numeric" 	required="yes">
   		<cfargument name="PagoAnoAnteriores" 	type="string" 	required="no">



		<!--- <cfif Arguments.CPcuenta eq 10491 or Arguments.CPcuenta eq 692>
			<cfsavecontent variable="savedump">
					    <cfdump var ="Monto = #Arguments.Monto#"><br>
					</cfsavecontent>
			<cffile action="append" file="c:\temp\cflog\dump.html" output="#savedump#" >
		</cfif> --->
			...
			update CPresupuestoControl
			   set
				<cfswitch expression="#trim(Arguments.TipoMovimiento)#">
                    ...
                    <cfif isdefined ('Arguments.PagoAnoAnteriores') and Arguments.PagoAnoAnteriores NEQ "" and Arguments.PagoAnoAnteriores EQ 1
							and "#trim(Arguments.TipoMovimiento)#" EQ 'P'>
                    	, CPCPagado_Anterior = round(isnull(CPCPagado_Anterior,0.00) + #Arguments.Monto#, 2)
	...
	<cffunction name="fnGeneraNAP" output="no" returntype="numeric" access="public">
		...
		<cfif rsNAP.recordCount GT 0>
            ...
            <cfelse>
				<cf_errorCode	code = "51295"
							msg  = "ERROR EN CONTROL PRESUPUESTARIO: El documento ya fue aprobado y aplicado con el DOC=#url.CVid# y  NAP=@errorDat_1@"
		...
		<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
			...
			values 	(#Arguments.Ecodigo#, #LvarNAP#,
					 ...
					 #Arguments.EcodigoOrigen#,
					 <cf_jdbcquery_param cfsqltype="cf_sql_varchar" LEN="4" value="#Arguments.ModuloOrigen#">,
                     <!---<cf_jdbcquery_param cfsqltype="cf_sql_varchar" LEN="20" value="#trim(Arguments.NumeroDocumento)#">,--->
					 <cf_jdbcquery_param cfsqltype="cf_sql_varchar" LEN="20" value="#trim(varNumeroDocumento)#">,
					 <cf_jdbcquery_param cfsqltype="cf_sql_varchar" LEN="25" value="#Arguments.NumeroReferencia#">,
		...
		<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
			...
			select 	#Arguments.Ecodigo#, #LvarNAP#,
					...
					NAPreferencia, LINreferencia,
	 				...
	 				PCGDid, PCGDcantidad, PCGDcantidadAntes, PCGDdisponibleAntes,CPCanoP,CPCmesP
			  from #Request.intPresupuesto#
			 where CPcuenta IS NOT NULL
			   <!--- and TipoCuenta <> 'X' --->
			   and TipoMovimiento NOT in ('P','EJ')
...
<!---   		<cfif Arguments.CFcuenta EQ "" or (Arguments.CFcuenta NEQ "" and Arguments.CPcuenta NEQ "" and Arguments.CFcuenta NEQ Arguments.CPcuenta)>--->
       <!--- ERBG Cambio Fin--->
	...
	<cffunction name="sbGeneracionMasiva" access="private" output="no">
		...
		<cfset varMes = rsSQL.Pvalor>
		<cfset varAnoMes = varAno*100+varMes>
...
<cflog type="information" file="FormulacionAplica" text="Inicio sbGeneracionMasiva: Tipo Movimiento=#Arguments.TipoMovimiento#, Cantidad Detalle=#rsPRES.cantidad#">
		...
		<cfquery name="rsComprAut" datasource="#session.dsn#"> <!---ANGELES BLANCO GENERA DESCOMPROMISOS SI ESTA HABILITADO PARAMETRO RAUL--->
            select CPCompromiso
			from CPparametros
			where Ecodigo = #session.Ecodigo#
			and CPPid =  (
								select top 1 CPPid from #Request.intPresupuesto#
									where CPPid is not null
						)

        </cfquery>

		<cfif (rsComprAut.recordCount GT 0 and rsComprAut.CPCompromiso eq 'False') or rsComprAut.recordCount EQ 0>
		...
		<cfquery datasource="#Arguments.Conexion#">
 		...
 		<cfif GvarCompromisosAutomatico_Anterior and TipoMovimiento EQ 'CC' >
 			    ,	CPCcomprometido 		= round(CPCcomprometido, 2) +
						...
						,0)
<!---Realiza Descompromiso cuando venga de una modificacion al presupuesto en meses anteriores--->
<!---Cambio Realizado  Por: ***RVD, APH*** Fecha: 12/06/2013--->
		<cfelseif GvarCompromisosAutomatico_Anterior and TipoMovimiento EQ 'M' >
               ,CPCcomprometido 		= round(CPCcomprometido, 2) +
                            coalesce(
                                (
                                    select round(Monto, 2)
                                     ...
                                     where CPCano	= CPresupuestoControl.CPCano
                                       ...
                                       and CPcuenta	= CPresupuestoControl.CPcuenta
                                       and CPcuenta in (
                                    select distinct a.CPcuenta from CPresupuesto a
                                    inner join CPresupuestoComprometidas b
                                    on a.Cmayor = b.Cmayor
                                    inner join CPresupuestoControl c
                                    on c.CPcuenta = a.CPcuenta
                                    and a.Ecodigo = b.Ecodigo
                                    where a.CPmovimiento = 'S')
                                       and Ocodigo	= CPresupuestoControl.Ocodigo
                           and TipoMovimiento = 'M'
                                )
                            ,0)
		</cfif>
			 where	Ecodigo	= #session.Ecodigo#
...
<cflog type="information" file="FormulacionAplica" text="update CPresupuestoControl">
...
<cflog type="information" file="FormulacionAplica" text="update intPresupuesto set MontoAcum">
		<cfif Arguments.TipoMovimiento EQ "A">
			<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
				update  #Request.intPresupuesto#
...
<cflog type="information" file="FormulacionAplica" text="update intPresupuesto set Disponibles">
		<cfelse>
			...
			<!--- se calcula el disponible acumulado para las cuentas de control abierto y las restrictivas hasta el mes de auxiliares --->
			<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
				...
				<cfelse>
				  ...
				  from #Request.intPresupuesto#
				  where ((#Request.intPresupuesto#.CalculoFin <= <cfqueryparam cfsqltype="integer" value="#varAnoMes#">
							and TipoControl <> 0 and CalculoControl = 2)
					or (TipoControl <> 0 and CalculoControl > 2)
					or TipoControl = 0)
			</cfquery>


			<!--- se calcula el disponible mensual para las cuentas de control restrictivo a meses futuros y las cuentas con calculocontrol mensual --->
			<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
				insert into #DISPONIBLES# (	CPPid, CPcuenta, Ocodigo, CPCanoMes, Disponible, Pendientes,
											Autorizacion, CompromisoAutom)
				select	distinct
					...
					(
						select
						 ...
						 where d.Ecodigo	= #session.Ecodigo#
						   ...
						   and d.CPcuenta	= #Request.intPresupuesto#.CPcuenta
						   and d.Ocodigo	= #Request.intPresupuesto#.Ocodigo
						   and d.CPCano	= #Request.intPresupuesto#.CPCano
						   and d.CPCmes	= #Request.intPresupuesto#.CPCmes
					)
					,
					(
						select
				...
				<cfelse>
				  ...
				  where ((#Request.intPresupuesto#.CalculoFin > <cfqueryparam cfsqltype="integer" value="#varAnoMes#">
						and TipoControl <> 0 and CalculoControl = 2)
					or (TipoControl <> 0 and CalculoControl = 1))
			</cfquery>
...
<cflog type="information" file="FormulacionAplica" text="update intPresupuesto set Disponible Generado">
			<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
				update  #Request.intPresupuesto#
				   set	DisponibleAnterior = DisponibleGenerado - (SignoMovimiento * Monto)
			</cfquery>

<cflog type="information" file="FormulacionAplica" text="update intPresupuesto set Disponible Anterior">


<!---Se valida el Parametro "Permitir Formulacion para cuentas de control abiertas negativas para meses anteriores"--->
<!---Cambio Realizado  Por: ***RVD, APH*** Fecha: 12/06/2013--->
...
<cflog type="information" file="FormulacionAplica" text="Verificacion Excesos">
		</cfif>
	</cffunction>
...
<cflog type="information" file="FormulacionAplica" text="Genera Descompromisos de NAPs de Comprimisos Automáticos Anteriores">
...
<cflog type="information" file="FormulacionAplica" text="Genera Compromisos Automáticos de las cuentas parametrizadas que estén en el NAP">
	...
	<!--- Compromete Compromisos Automáticos de la Formulación Actual --->
	<cffunction name="sbCompromisosManuales" access="private" output="no">
		<cfargument name="TipoMovimiento" required="yes">
		<cfargument name="Conexion" required="yes">

		<!--- Compromete Compromisos Manuales  --->

		<!--- Determina si existe por lo menos una Cuenta Parametrizada con Compromiso Automático en el Período --->
		<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
			select count(1) as cantidad
			  from CPresupuestoComprAut m
			 where m.Ecodigo	= #session.Ecodigo#
			   and m.CPPid		= #LvarCPPid#
		</cfquery>
		<cfif rsSQL.cantidad EQ 0>
		...
		<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
			select count(1) as cantidad
			  from #request.intPresupuesto# i
				inner join CPresupuesto c
					on c.CPcuenta = i.CPcuenta
				inner join CPresupuestoComprAut m
					 on m.Ecodigo	= #session.Ecodigo#
					and m.CPPid		= #LvarCPPid#
					and m.Cmayor	= c.Cmayor
					and c.CPformato = m.CPCCmascara
		</cfquery>
		<cfif rsSQL.cantidad EQ 0>
			<cfreturn>
		</cfif>
...
--->
		...
		<cfquery datasource="#Arguments.Conexion#">
			...
			select 	distinct
					...
					s.CPperiodo, s.CPmes, s.CPperiodo*100 + s.CPmes,
					i.TipoControl, i.CalculoControl, i.PCGDid
			  from #request.intPresupuesto# i
				...
				inner join CPmeses mm
					on mm.CPPid		= #LvarCPPid#
                inner join CPresupuestoComprAut cp
                	on cp.Ecodigo =  c.Ecodigo
                   and cp.CPcuenta = c.CPcuenta
                   and CP.CPPid		= #LvarCPPid#
				inner join CPresupuestoComprAutD s
				   and s.CPPid		= #LvarCPPid#
				   and s.CPperiodo	= mm.CPCano
				   and s.CPmes		= mm.CPCmes
				   and s.CPCCid		= cp.CPCCid
			 ...
			 where TipoMovimiento <> 'CC'
			   ...
			   and (
					select count(1)
					  from CPresupuestoComprometidasNAPs nn
					 where nn.Ecodigo	= cp.Ecodigo
					   and nn.CPPid		= cp.CPPid
					   and nn.CPcuenta	= cp.CPcuenta
					   and nn.Ocodigo	= cp.Ocodigo
					   and nn.CPCano	= s.CPCano
					   and nn.CPCmes	= s.CPCmes
					) > 0
	...
	<cffunction name="sbDescompromisosAutomatico" access="private" output="no">
		<cfargument name="Ecodigo" required="yes">
		<cfargument name="Conexion" required="yes">
		<cfargument name="CPPid" required="no" default="#session.CPPid#">

		<cfset rsValidaXCP = ObtenerDato(1131) /> <!--- Parametro validacion por cuenta presupuestal --->
		...
		<cfif (rsComprAut.recordCount GT 0 and rsComprAut.CPCompromiso eq 'True') >
        	<cfif isdefined("request.PRES_ContaPresupuestaria_IDcontable")>
            	<cfset varIDconta ="#request.PRES_ContaPresupuestaria_IDcontable#">
            <cfelse>
            	<cfset varIDconta = 0>
            </cfif>

            <cfquery name="rsEcontables" datasource="#Arguments.Conexion#">
                select Edocumento,Oorigen,Cconcepto,Eperiodo,Emes,Edocbase from EContables
                where IDcontable = #varIDconta#
            </cfquery>

            <cfif rsEcontables.recordCount EQ 0>
                <cfset varEdocumento = 0>
                <cfset varOorigen 	= "">
                <cfset varCconcepto = 0>
                <cfset varEdocbase 	= "">
                <cfset varEperiodo 	= 0>
                <cfset varEmes 		= 0>
            <cfelse>
                <cfset varEdocumento 	= #rsEcontables.Edocumento#>
                <cfset varOorigen 		= "#rsEcontables.Oorigen#">
                <cfset varCconcepto 	= #rsEcontables.Cconcepto#>
                <cfset varEdocbase 		= "#rsEcontables.Edocbase#">
                <cfset varEperiodo 		= "#rsEcontables.Eperiodo#">
                <cfset varEmes 			= "#rsEcontables.Emes#">
            </cfif>
		...
		<cfset request.Errores = arrayNew(1)>
        <cfset Idx = 0>
		...
		<!--- Validacion de cuentas con compromiso obligatorio --->
		<cfif (rsComprAut.recordCount GT 0 and rsComprAut.CPCompromiso eq 'True' ) >
			<cfquery name="rsCtas" datasource="#Arguments.Conexion#">
				SELECT  f.CPcuenta, f.CFcuenta,
						f.Ocodigo, f.CPCano, f.CPCmes,
						f.CPCPtipoControl,
						f.CPCPcalculoControl,
						f.Monto
				from CPresupuesto a
				inner join(
					SELECT distinct d.CPcuenta
				 		from DContables a
				...
				inner JOIN  CPresupuestoObligatorias b
					...
					AND a.Ecodigo = b.Ecodigo
				inner join(
					select i.CPcuenta, i.CFcuenta,
						i.Ocodigo, i.CPCano, i.CPCmes,
			...
			<cfif rsCtas.recordCount GT 0>
				<cfquery datasource="#session.dsn#">
			             update MensNRCE set Estado = 0
			             where Ecodigo = #session.Ecodigo#
			             and NRC_Periodo = #varEperiodo#
			             and NRC_Mes = #varEmes#
			             and NRC_ModuloOrigen = '#varOorigen#'
			             and NRC_RefOrigen = '#varEdocbase#'
			             and NRC_DocumentoOrigen = '#varEdocumento#'
			    </cfquery>
			</cfif>
			<cfloop query="rsCtas">
				...
				<cfif validaXCPVal EQ 'N'>
					<!--- Si la validacion es por cuenta presupuestal --->
					<cfset Idx = Idx+1 />
					<cfset request.Errores[Idx] 			= structNew() />
					<cfset request.Errores[Idx].IDconta		= #varIDconta# />
					<cfset request.Errores[Idx].Ecodigo		= #session.Ecodigo# />
					<cfset request.Errores[Idx].Eperiodo 	= #varEperiodo# />
					<cfset request.Errores[Idx].Emes		= #varEmes# />
					<cfset request.Errores[Idx].Oorigen		= #varEdocumento# />
					<cfset request.Errores[Idx].Edocbase	= #varEdocbase# />
					<cfset request.Errores[Idx].Cconcepto	= #varCconcepto# />
					<cfset request.Errores[Idx].NRC_ModuloOrigen = #varOorigen# />
					<cfset request.Errores[Idx].CPNAPnum	= 0 />
					<cfset request.Errores[Idx].CPcuenta	= #rsCtas.CPcuenta# />
					<cfset request.Errores[Idx].LvarMontoNAP = 0 />
					<cfset request.Errores[Idx].LvarMonto	= #LvarMonto# />
					<cfset request.Errores[Idx].Dperiodo 	= #rsCtas.CPCano# />
					<cfset request.Errores[Idx].Dmes		= #rsCtas.CPCmes# />
				</cfif>
			</cfloop>
		...
		<cfloop query="rsCtas">
			...
			<cfquery name="rsNAPs" datasource="#Arguments.Conexion#">
				select
				 ...
				 where r.Ecodigo	= #Arguments.Ecodigo#
				   ...
				   and r.CPCmes	= #LvarCPCmes#
				   and r.CPCano = #LvarCPCano#
			...
			<cfloop query="rsNAPs">
				...
				<cfelse>
					...
					<cfif rsComprAut.recordCount GT 0 and rsComprAut.CPCompromiso eq 'True' and validaXCPVal EQ 'N'>
						<cfif LvarMontoNAP LT LvarMonto or LvarMontoNAP EQ 0>
		                	<cfset Idx = Idx+1>
    		            	<cfset request.Errores[Idx] 			= structNew()>
    		                ...
    		                <cfset request.Errores[Idx].CPNAPnum	= #rsNAPs.CPNAPnum#>
        		            <cfset request.Errores[Idx].CPcuenta	= #rsNAPs.CPcuenta#>
            		        <cfset request.Errores[Idx].LvarMontoNAP = #LvarMontoNAP#>
                		    <cfset request.Errores[Idx].LvarMonto	= #LvarMonto#>

                   			<cfset request.Errores[Idx].Dperiodo 	= #rsNAPs.CPCano#>
		                    <cfset request.Errores[Idx].Dmes		= #rsNAPs.CPCmes#>
	...
	<cffunction name="fnReversarNAPcompleto" access="public" returntype="numeric" output="no">
		...
		<cfreturn ControlPresupuestario
			(
				...
				FechaDocumento=Arguments.FechaDocumento,
				AnoDocumento=Arguments.AnoDocumento,
				MesDocumento=Arguments.MesDocumento,
...
<!--- Control Evento Inicia --->
        <cfargument name='NumeroEvento' type='string' required='false' default="">
<!--- Control Evento Fin --->
		<cfargument name='PagoAnoAnteriores' type='string' required='false'>
		<cfargument name='Momentos' type='string' required='false' default="0">
		...
		<cfif listfind("MBMV,MBTR,CPRE,CCRE,ESBA,MBFE",Arguments.ModuloOrigen)>
		...
		<cfreturn ControlPresupuestarioIntercompany
										(Arguments.ModuloOrigen, Arguments.NumeroDocumento, Arguments.NumeroReferencia,
										 Arguments.FechaDocumento, Arguments.AnoDocumento, Arguments.MesDocumento,
										 Arguments.Conexion, Arguments.Ecodigo, Arguments.Momentos,
										 -1,false,false,
										 Arguments.Intercompany,ConciliaML
<!--- Control Evento Inicia --->
										 ,Arguments.NumeroEvento<!--- ,Arguments.PagoAnoAnteriores --->
                                         )>
<!--- Control Evento Inicia --->
	...
	<cffunction name="ControlPresupuestarioEContables" output="no" returntype="void" access="public">
		...
		<cfargument name='PeriodoCerrado'	type='boolean' 	default='false'>
...
<!--- Control Evento Fin --->
		...
		<cfif listfind("TEPN,CGPN",EContables.Oorigen)>
		...
		<cfif EContables.Oorigen EQ "CGPN" OR EContables.Oorigen EQ "CCFE">
			<cfset GenerarEjecucionConPago(Arguments.Conexion,Arguments.Ecodigo)>
		</cfif>
...
<!--- Control Evento Inicia --->
										 ,Arguments.NumeroEvento, Arguments.validaComp
										 )>
