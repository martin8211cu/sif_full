...
--->
	...
	<cffunction name="CreaIntarc" access="public" output="false" returntype="string">
		...
		<cf_dbtemp name="CG_INTARC_V10" returnvariable="CG_GeneraAsiento_INTARC_NAME" datasource="#Arguments.Conexion#">
			...
			<cf_dbtempcol name="INTREF"    type="varchar(35)"  mandatory="yes">
			<cf_dbtempcol name="INTMON"    type="money"        mandatory="yes">
			<cf_dbtempcol name="INTTIP"    type="char(1)"      mandatory="yes">
			<cf_dbtempcol name="INTDES"    type="varchar(250)"  mandatory="yes">
			...
			<!--- Columnas nuevas para facturacion (existen en ambiente de CR) --->
			<cf_dbtempcol name="INTllave1"      type="char(20)"      mandatory="no">
            <cf_dbtempcol name="INTllave2"      type="char(20)"      mandatory="no">
			<cf_dbtempcol name="INTdescripcion" type="varchar(200)"  mandatory="no">
			<cf_dbtempcol name="INTdoc_original" type="varchar(100)"  mandatory="no">
			<cf_dbtempcol name="INTtipoPersona" type="varchar(10)"   mandatory="no">
			<cf_dbtempcol name="INTidPersona"   type="numeric"       mandatory="no">
<!--- Control Evento Inicia --->
            <cf_dbtempcol name="NumeroEvento" type="varchar(25)" mandatory="no">
<!--- Control Evento Fin --->

			<cf_dbtempcol name="CFcuenta_ppto"  type="numeric"      mandatory="no">
			<cf_dbtempcol name="Ccuenta_ppto"   type="numeric"      mandatory="no">
<!--- Contabilidad Electronica Inicia --->
			<cf_dbtempcol name="CEtipoLinea" type="integer" mandatory="no">
			<cf_dbtempcol name="CEdocumentoOri" type="varchar(45)" mandatory="no">
			<cf_dbtempcol name="CEtranOri" type="varchar(25)" mandatory="no">
			<cf_dbtempcol name="CESNB" type="numeric" mandatory="no">
<!--- Contabilidad Electronica Fin --->
	...
	<cffunction name='GeneraAsiento' access='public' output='true' returntype="numeric" hint="Regresa el IDcontable del asiento generado.  Si no se genera el asiento, regresa 0">
		...
		<cfargument name='Ecodigo' type='numeric' required='false' default="-1">
		<cfargument name='Conexion' type='string' required='false' default="">
		<cfargument name='Ocodigo' type='numeric' required='false' default="-1">		<!--- Oficina del Documento --->
...
<!--- Control Evento Inicia --->
        ...
        <cfset NumeroEvento=#Arguments.NumeroEvento#>
<!--- Control Evento Fin --->
		...
		<cfif resultado LT -50>
			...
			<cfif Len(Trim(MonedasDesbalanceadas))>
				...
				<cfelse>
					...
					<cf_abort errorInterfaz="La póliza no esta Balanceada en las siguientes Monedas: #MonedasDesbalanceadas#. Proceso Cancelado!">
				</cfif>
			</cfif>

			<cfif Arguments.PintaAsiento AND NOT LvarVieneDeInterfaz>
				<!--- Pinta el Asiento Contable --->
				<cfinvoke 	component		= "sif.Componentes.CG_GeneraAsiento"
							method			= "PintaAsiento"
				>
...
<!--- Control Evento Fin --->
		...
		<cfif INTARC_count.INTARC_count is 0>
			<cfif Arguments.debug><cfdump var="Cantidad líneas de Asiento: #INTARC_count.INTARC_count#" label="INTARC_count"></cfif>
		...
		<cfquery name="rsSQL" datasource="#Arguments.Conexion#">
			select min(i.Mcodigo) as Mcodigo
			  from #Request.intarc# i
				inner join CFinanciera cf
				   on cf.CFcuenta = i.CFcuenta
				inner join Monedas m
				   on m.Mcodigo = i.Mcodigo
			where cf.Ecodigo <> m.Ecodigo
		</cfquery>
		<cfif rsSQL.Mcodigo NEQ "">
					   		...
					   		(
								  ...
								  from CFinanciera cf
									inner join Monedas m1
										 on m1.Mcodigo = #Request.intarc#.Mcodigo
								  	inner join Monedas m2
										 on m2.Ecodigo = cf.Ecodigo
										and m2.Miso4217 = m1.Miso4217
								 where cf.CFcuenta = #Request.intarc#.CFcuenta
								   and cf.Ecodigo <> m1.Ecodigo
							)
					 where
					   		(
					   			select count(1)
			...
			<cfelse>
				<cfthrow message="Moneda Mcodigo=[#rsSQL.Mcodigo#] no pertenecen a la Empresa">
			</cfif>
		</cfif>
		...
		<cfif Arguments.interfazconta>
			...
			<cfinvoke component="Contabilidad" method="Nuevo_Asiento" returnvariable="Edocumento">
				...
				<cfinvokeargument name="Eperiodo" value="#Arguments.Eperiodo#">
				<cfinvokeargument name="Emes" value="#Arguments.Emes#">
				<cfif isdefined("Arguments.Ocodigo") AND Arguments.Ocodigo NEQ -1>
		...
		<!--- <cfquery datasource="#Arguments.Conexion#">
			update #Request.intarc#
			   set INTCAM = 1.0 * INTMON / INTMOE
			 where INTMOE != 0
			   AND (INTCAM <= 1 OR INTCAM IS NULL)
			   and round(INTMOE * INTCAM, 2) != round(INTMON, 2)
		</cfquery> --->
		...
		<cfif NOT Arguments.Intercompany>
			...
			<cfif distinctOcodigo.cnt gt 1>
				...
				<cfquery name="rs_InsertaCredito" datasource="#session.dsn#">
					select
						min(a.INTORI) as INTORI,
						min(a.INTREL) as INTREL,
						abs(sum(a.INTMON * (case a.INTTIP when 'C' then -1 else 1 end))) as INTMON,
						case when sum(a.INTMON * (case a.INTTIP when 'C' then -1 else 1 end)) > 0 then 'C' else 'D' end as INTTIP,
						'Balance de Saldos por Oficina' as INTDES,
						min(a.INTFEC) as INTFEC,
						abs(min(a.INTMON/a.INTMOE)) as INTCAM,
						min(a.Periodo) as Periodo,
						min(a.Mes) as Mes,
						a.Mcodigo,
						a.Ocodigo,
						abs(sum(a.INTMOE * (case a.INTTIP when 'C' then -1 else 1 end))) as INTMOE
					from #Request.intarc# a
               where a.INTMOE <> 0
				...
				<cfloop query="rs_InsertaCredito" >
					<cfquery datasource="#Arguments.Conexion#">
						...
						values(
							'#rs_InsertaCredito.INTORI#',
							#rs_InsertaCredito.INTREL#,
							'AJ',
							'AJ',
							#rs_InsertaCredito.INTMON#,
							'#rs_InsertaCredito.INTTIP#',
							'#rs_InsertaCredito.INTDES#',
							'#rs_InsertaCredito.INTFEC#',
							#rs_InsertaCredito.INTCAM#,
							#rs_InsertaCredito.Periodo#,
							#rs_InsertaCredito.Mes#,
							#CuentaSaldosOficina.Ccuenta#,
							#rs_InsertaCredito.Mcodigo#,
							#rs_InsertaCredito.Ocodigo#,
							#rs_InsertaCredito.INTMOE#
					)
					</cfquery>
				</cfloop>
				...
				<cfloop query="cursorMonedas">
					...
					<cfelseif NumberFormat(cursorMonedas.diferencia_local, ',9.00') neq 0.00>
						...
						<cfquery datasource="#Arguments.Conexion#">
							...
							select a.INTORI, a.INTREL, 'AJ', 'AJ',
								<cfqueryparam cfsqltype="cf_sql_money" value="#Abs(cursorMonedas.diferencia_local)#">,
								<cfif cursorMonedas.diferencia_local gt 0>'D'<cfelse>'C'</cfif>,
								'Balance de Saldos por Redondeo de Monedas', a.INTFEC, 0.00, a.Periodo, a.Mes,
								<cfqueryparam cfsqltype="cf_sql_numeric" value="#CuentaRedondeos.Ccuenta#">,
								a.Mcodigo, a.Ocodigo, 0.00
							from #Request.intarc# a
		...
		<!--- <cfif abs(abs(NumberFormat(balanceado.balance, ',9.00')) - abs(NumberFormat(cursorMonedas.diferencia_local, ',9.00'))) gt 0.05> --->
        ...
        <!---SML. 14042015. Inicio Creación de Variable para saber si se borra la intPresupuesto cuando esta viene de una Aplicacion de Pagos y Anticipos--->
        <cfif isdefined('Arguments.Oorigen') and #Arguments.Oorigen# EQ 'CPPA'>
        	<cfset varBorrarIntPres = false>
        <cfelse>
        	<cfset varBorrarIntPres = true>
        </cfif>
		<!---SML. 14042015. Fin Creación de Variable para saber si se borra la intPresupuesto cuando esta viene de una Aplicacion de Pagos y Anticipos--->
		<cfif Arguments.NAP EQ "">
			<cfinvoke component	= "PRES_Presupuesto" method= "ControlPresupuestarioINTARC" returnvariable	= "LvarIC">
						...
						<cfinvokeargument name="DocaFavor" 		value="#Arguments.DocaFavor#"/>
						<cfinvokeargument name="ConciliaML" 		value="#Arguments.ConciliaML#"/>
<!--- Control Evento Inicia --->
        				<cfinvokeargument name='NumeroEvento' 		value="#Arguments.NumeroEvento#"/>
<!--- Control Evento Fin --->
						<cfinvokeargument name='PagoAnoAnteriores' 		value="#Arguments.PagoAnoAnteriores#"/>
                        <cfinvokeargument name='BorrarIntPres' 		value="#varBorrarIntPres#"/> <!---SML. 14042015. Creación de Variable para saber si se borra la intPresupuesto cuando esta viene de una Aplicacion de Pagos y Anticipos--->
			</cfinvoke>
		...
		<cfif Arguments.interfazconta AND LvarNAP GTE 0>
			<cfif Lvaridentity EQ "">
				<cfquery datasource="#Arguments.Conexion#" name="insert_Econtables">
					...
					values (
						...
						<cf_jdbcquery_param cfsqltype="cf_sql_char" value="#Arguments.Edescripcion#" len="100">,

						'#Left(Arguments.Edocbase, 20)#',
						<cf_jdbcquery_param cfsqltype="cf_sql_char" value="#Arguments.Ereferencia#" len="25">,
						'S',
						'#Arguments.usuario#',
						'#Arguments.Oorigen#',
						<cfif isdefined("Arguments.Ocodigo") AND Arguments.Ocodigo NEQ -1>
			...
			<cfquery name = "rsObtieneLinea" datasource="#Arguments.Conexion#">
				select coalesce(max(Dlinea),0) as Linea
				from DContables d
			...
			<cfif ListFind('sybase', Application.dsinfo[Arguments.conexion].type)>
			...
			<cfelse>
				<cfquery datasource="#Arguments.Conexion#">
					update #Request.intarc#
					   set Dlinea = #lvarLinea# + (select count(1) from #Request.intarc# x where x.INTLIN <= #Request.intarc#.INTLIN)
				</cfquery>
			</cfif>
...
<!--- Contabilidad Electronica Fin --->
	...
	<cffunction name="CG_ValidacionesAsiento" access="public">
		...
		>
		<cfif LvarMSG NEQ "OK">
			<cfthrow message="Error Verificando Cuenta:#LvarMSG#">
		</cfif>
	...
	<cffunction name="PintaAsiento" output="true" access="public">
		...
		<cfif Not IsDefined('Arguments.Cconcepto') or Arguments.Cconcepto EQ -10>
			...
			<cfif ConceptoContable.Cconcepto eq "">
				...
				<cf_errorCode	code = "51066"
								msg  = "#MSG_NosehaDefelConcContOri# '@errorDat_1@=@errorDat_2@'."
								errorDat_1="#Arguments.Oorigen#"
								errorDat_2="#rsSQL.Odescripcion#"
				>
			</cfif>
		...
		<cfif rsConceptoContableE.recordCount eq 0>
			<cf_errorCode	code = "51077"
							msg  = "#MSG_ElConcCont# '@errorDat_1@' #MSG_noexisteenCia#"
		...
		<cfquery name="rsAsiento" datasource="#session.dsn#">
			select 	INTLIN,
					...
					INTTIP,
					Miso4217, Oficodigo,
					round(INTMOE,2) as INTMOE,
					INTCAM,
					round(INTMON,2) as INTMON,
                    CFid
			  from #request.INTARC# i
				left join Monedas m
					 on m.Mcodigo = i.Mcodigo
		...
		<cfset CMB_Debito 	= t.Translate('CMB_Debito','Débito','/sif/generales.xml')>
		<cfset CMB_Credito 	= t.Translate('CMB_Credito','Credito','/sif/generales.xml')>
		...
		<cf_templatecss>
			...
			<font color="##0000FF" size="4">
				#LB_EMPRESA#: #rsEmpresa.Edescripcion#
			</font>
			<BR>
        <cfoutput>
		<table border="0" cellspacing="0" cellpadding="2" style="border:1px solid ##000000;">
			<tr>
				<td>
					#LB_Concepto#:&nbsp;#rsConceptoContableE.Cdescripcion#&nbsp;
				</td>
				<td>
					#LB_Periodo#:&nbsp;#Arguments.Eperiodo# Mes #Arguments.Emes#&nbsp;
				</td>
				<td>
					#LB_Fecha#: #dateFormat(Arguments.Efecha,"DD/MM/YYYYY")#&nbsp;
				</td>
			</tr>
			<tr>
				<td>
					#LB_Origen#: #Arguments.Oorigen#&nbsp;
				</td>
				<td>
					#LB_Referencia#: #Arguments.Ereferencia#&nbsp;
				</td>
				<td>
					#LB_Documento#: #Arguments.Edocbase#&nbsp;
				</td>
			</tr>
		</table>
		<table border="0" cellspacing="0" cellpadding="2" style="border:1px solid ##000000;">
