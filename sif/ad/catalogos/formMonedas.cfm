...
<cfif isDefined("Session.Ecodigo") and isDefined("Form.Mcodigo") and Len(Trim(Form.Mcodigo)) NEQ 0 >
	<cfquery name="rsMonedas" datasource="#Session.DSN#" >
        ...
        Msimbolo, Miso4217, ClaveSAT,ts_rversion
		from Monedas
  ...
  <table align="center">
    ...
    <tr valign="baseline">
      <td nowrap align="right">
      	<cf_translate key=LB_Simbolo>Símbolo</cf_translate>:
      </td>
      <td><input name="Msimbolo" type="text" tabindex="1" value="<cfif modo NEQ "ALTA"><cfoutput>#fnGetMsimbolo(rsMonedas.Msimbolo)#</cfoutput><cfelse><cfoutput>#rsMonedasPortal.Msimbolo#</cfoutput></cfif>" size="3" maxlength="3"></td>
    </tr>
    <tr valign="baseline">
      <td nowrap align="right"><cf_translate key=LB_Sigla>Sigla</cf_translate>:</td>
	...
	<tr>
		<td valign="baseline" align="right"><cf_translate key=LB_ClaveSAT>Clave SAT</cf_translate>:</td>
		<td>
			<cfset valuesArray = ArrayNew(1)>
			<cfif modo eq 'CAMBIO' and IsDefined('form.MCodigo') and IsDefined('rsMonedas')>
				<cfquery name="rsCatalogo" datasource="#session.dsn#">
					select CSATcodigo,CSATdescripcion
					from CSATMoneda
					where CSATcodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsMonedas.ClaveSAT#">
				</cfquery>
	...
	<tr>
		<td align="center" colspan="2">
			<table width="75%" align="center" class="ayuda">
				<tr><td><p><cf_translate key=LB_Mensaje>Las monedas definidas por este cat&aacute;logo las podr&aacute; utilizar su empresa al generar documentos, registros de pago etc.</cf_translate></p></td></tr>
			</table>
		</td>
	</tr>
