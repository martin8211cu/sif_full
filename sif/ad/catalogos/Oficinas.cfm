...
<cfif isdefined("form.FILTRO_ODESCRIPCION") and len(trim(form.FILTRO_ODESCRIPCION)) NEQ 0 >
	<cfset ffiltro = ffiltro & " and Odescripcion = " & form.FILTRO_ODESCRIPCION >
</cfif>
<cfif isdefined("form.FILTRO_OFICODIGO") and len(trim(form.FILTRO_OFICODIGO)) NEQ 0 >
	<cfset ffiltro = ffiltro & " and Oficodigo = '" & form.FILTRO_OFICODIGO & "'">
</cfif>
<cfif isdefined("form.FILTRO_TELEFONO") and len(trim(form.FILTRO_TELEFONO)) NEQ 0 >
	<cfset ffiltro = ffiltro & " and telefono = '" & form.FILTRO_TELEFONO & "'">
</cfif>
<cfquery name="rsQueryOfi" datasource="#Session.DSN#">
	SELECT 0 as CODNEW,* 
	FROM Oficinas  
	WHERE ISNUMERIC(Oficodigo)<>1
	and Ecodigo=#session.Ecodigo# 
	<cfif isdefined("form.FILTRO_ODESCRIPCION") and len(trim(form.FILTRO_ODESCRIPCION)) NEQ 0 >
	 and UPPER(Odescripcion) like '#UCase(form.FILTRO_ODESCRIPCION)#%'  
	</cfif>
	<cfif isdefined("form.FILTRO_OFICODIGO") and len(trim(form.FILTRO_OFICODIGO)) NEQ 0 >
	 and Oficodigo = '#form.FILTRO_OFICODIGO#'
	</cfif>
	<cfif isdefined("form.FILTRO_TELEFONO") and len(trim(form.FILTRO_TELEFONO)) NEQ 0 >
	 and telefono = '#form.FILTRO_TELEFONO#'
	</cfif>
	union
	select *
	from(
			...
			<tr> 
				<td valign="top">
					<cfinvoke component="sif.Componentes.pListas" method="pListaQuery" returnvariable="pLista">
						<cfinvokeargument name="query" value="#rsQueryOfi#"/>
