		...
		<cfif isdefined("form.SNcodigo") and len(trim(form.SNcodigo))>
			<cfquery name="rsSocios" datasource="#Session.DSN#" >
				select yo.SNplazoentrega,yo.SNplazocredito,rtrim(yo.LOCidioma) as LOCidioma,yo.Ecodigo, yo.SNcodigo, yo.SNidentificacion,yo.SNidentificacion2, yo.SNtiposocio, yo.SNnombre, yo.SNdireccion,
				 ...
				 yo.Mcodigo, isNull(yo.SNmontoLimiteCC,0) SNmontoLimiteCC, yo.SNdiasVencimientoCC, yo.SNdiasMoraCC, yo.SNdocAsociadoCC,
				 ...
				 yo.SNcontratos,yo.IdFisc,yo.Nacional,
                 		 COALESCE(yo.saldoCliente,0) AS saldoCliente,yo.remisionProveedor, yo.sincIntfaz,
				isnull(btr.Monto,0) MontoTransito, yo.SNesCorporativo, yo.Contrato, yo.IdRegimenFiscal, yo.SNnombreFiscal
				from SNegocios yo
					...
					) btr on yo.Ecodigo = btr.Ecodigo and yo.SNcodigoext = btr.SNcodigoExt
			...
			<cfset lVarMontoTransito = 0>
			<cfset lVarDisponibleActual = 0>
			<cfif rsSocios.recordCount gt 0>
				<cfoutput>
					<cfset lVarSaldoActual = #rsSocios.saldoCliente#>
					<cfset lVarMontoTransito = #rsSocios.MontoTransito#>
					<cfset lVarDisponibleActual = #rsSocios.SNmontoLimiteCC# - #rsSocios.saldoCliente# - #rsSocios.MontoTransito#>
				</cfoutput>
			</cfif>
		</cfif>
...
<script language="JavaScript" type="text/javascript">
	var f = document.formSDG;
	<cfoutput>
	<cfset MascaraSN = #Replace(MascaraNumeroSN,'?','x','ALL')#>
	var SociosNegociosMask = new Mask("#replace(MascaraSN,'X','##','ALL')#", "string");
	SociosNegociosMask.attach(document.formSDG.SNnumero, SociosNegociosMask.mask, "string");

		<cfif LvarSNtipo EQ ''>
			<cfset LvarSNtipo = 'F'>
		</cfif>
		var oCedulaMask = new Mask("#replace(LvarSNtipo,'X','##','ALL')#", "string");
		oCedulaMask.attach(document.formSDG.SNidentificacion, oCedulaMask.mask, "string");
		function cambiarMascara(v)
		{
			document.formSDG.SNidentificacion.value = "";
			if (v == 'F')
			{
		...
		try{
			document.formSDG.SNnumero.focus();
		}catch(e){
		}
		<cfif isdefined("form.SNcodigo") and len(trim(form.SNcodigo))>
			function funcDirecciones(){
				document.formSDG.action='SociosDireccion.cfm?SNcodigo=#form.SNcodigo#&SNcat=1';
				return true;
