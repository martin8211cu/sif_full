...
<cfinvoke component="sif.Componentes.Translate" method="Translate" key = "LB_RegimenFiscal" default = "Regimen Fiscal" returnvariable="LB_RegimenFiscal" xmlfile = "SociosDGenerales.xml">
...
<cfinvoke component="sif.Componentes.Translate" method="Translate" key = "LB_usaINE" default = "Corporativo" returnvariable="LB_SNesCorporativo" xmlfile = "SociosDGenerales.xml">
...
<cfinvoke component="sif.Componentes.Translate" method="Translate" key = "LB_IdentificacionFiscal" default = "Nombre Fiscal" returnvariable="LB_IdentificacionFiscal" xmlfile = "SociosDGenerales.xml">
...
<cfquery name="rsRFiscal" datasource="#session.DSN#">
	select IdRegFiscal, CSATcodigo, CSATdescripcion 
	from CSATRegFiscal
</cfquery>

<cfquery name="rsUsoCFDI" datasource="#session.DSN#">
	select IdUsoCFDI,CSATcodigo, CSATdescripcion 
	from CSATUsoCFDI
</cfquery>
...
function validarDGenerales(form){
	...
	if(form.botonSel.value != 'Baja' && form.botonSel.value != 'Nuevo'){
		...
		if ( document.formSDG.SNidentificacion.value != '') {
			var isValid = SNidentificacion_validarInput(document.formSDG.SNidentificacion);
			if (!isValid) {
				alert("RFC Invalido");
				
				formValid = false;
			} else {
				var lVarRfc = document.formSDG.SNidentificacion.value.trim();
				var lvarLimpio = lVarRfc.replace(/-/g, '');
				if(lvarLimpio !== 'XAXX010101000' && lvarLimpio !== 'XEXX010101000'){
					$.ajax({
						method: "post",
						url: "/cfmx/sif/Componentes/AjaxValidacionesRFC.cfc",
						async:false,
						data: {
							method: "fnValidaRFC",
							RFC: lVarRfc,
							Ecodigo: <cfoutput>#session.ecodigo#</cfoutput>,
							conexion: <cfoutput>'#session.dsn#'</cfoutput>,
							returnFormat: "JSON"
						},
						dataType: "json",
						beforeSend: function() {
							document.getElementById("loader").style.display = "initial";
							document.getElementById("formSDG").style.display = "block";
						},
						...
						complete: function() {
							...
							document.getElementById("formSDG").style.display = "initial";
						},
					});
				}
				formValid = isValid;
		...
		if(document.formSDG.esIntercompany.checked){
			if(document.formSDG.Intercompany.value == '-1'){
				alert('La empresa a la que pertenece, es requerida.');
				document.formSDG.Intercompany.focus();
				formValid = false;
			}
		}
		<cfif rsPCodigoOBJImp.Pvalor eq "4.0">
		if(document.formSDG.SNnombre.value =='' || document.formSDG.codPostal.value == '' || document.formSDG.IdRegFiscal.value == ''){

		socios_validateForm(form,'SNnombre','','R','codPostal','','R','IdRegFiscal','','R');
		formValid = false;

		}
		<cfelse>
		if(document.formSDG.SNnombre.value ==''){

		socios_validateForm(form,'SNnombre','','R');
		formValid = false;
		...
		formValid = document.MM_returnValue */
	} else{
		if (form.botonSel.value == 'Baja' && socio_generico() ) {
			alert('El Socio de Negocios Generico no puede ser eliminado.'); //Cambios AFGM
			formValid = false;
		}
	}
...
<form action="SociosDGenerales-sql.cfm" method="post" id="formSDG" name="formSDG" onSubmit="return validarDGenerales(this);">
	<div id="loader"></div>
<cfoutput>
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="0" >
			...
			<table>
				...
				<tr>
					...
					<td>
						...
						<cf_inputRFC name="SNidentificacion" 
									value="#(modo NEQ "ALTA") ? trim(rsSocios.SNidentificacion) : ''#" 
									size="40" >
						<!--- <input  tabindex="1" type="text" name="SNidentificacion" id="SNidentificacion" size="40"
							 value="<cfif modo NEQ "ALTA">#trim(rsSocios.SNidentificacion)#</cfif>"
							 onfocus="javascript:this.select();"
							 onchange="validaRFCRepetido(this);"
							 alt="Identificación" > --->
						<input  tabindex="-1" type="text" name="SNmask" size="40" readonly value="#LvarSNtipo#" style="border:none;">
				...
				<tr>
					...
					<td align="right" nowrap="nowrap"><strong><cf_translate  key="LB_RegimenFiscal">Regimen Fiscal</cf_translate>:</strong>&nbsp;</td>
			      	<td valign="top">
					  	<cfset valuesArray = ArrayNew(1)>
						  <cfif modo NEQ "ALTA" and trim(rsSocios.IdRegimenFiscal) neq '' >
							<cfquery name="rsRFiscal" datasource="#session.DSN#">
								select IdRegFiscal, CSATcodigo, CSATdescripcion 
								from CSATRegFiscal 
								where  IdRegFiscal = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsSocios.IdRegimenFiscal#">
							</cfquery>
							<cfset ArrayAppend(valuesArray, '#trim(rsRFiscal.IdRegFiscal)#')>
							<cfset ArrayAppend(valuesArray, '#trim(rsRFiscal.CSATcodigo)#')>
							<cfset ArrayAppend(valuesArray, '#trim(rsRFiscal.CSATdescripcion)#')>
						</cfif>
						<cf_conlis title="Regimen Fiscal"
							alt = "El campo Regimen Fiscal del Socio" 
					     	campos = "IdRegFiscal,CSATcodigo,CSATdescripcion"
							desplegables = "N,S,S"
							modificables = "S,S"
							size = "4,5,32"
							tabla="CSATRegFiscal"
							columnas="IdRegFiscal, CSATcodigo, CSATdescripcion "
							desplegar="CSATcodigo,CSATdescripcion"
							etiquetas="Codigo,Estado"
							formatos="S,S"
							align="left,left"
							asignar="IdRegFiscal,CSATcodigo, CSATdescripcion"
							asignarformatos="S,S"
	    ...
	    <td/>
			...
			<td>
			    <input  tabindex="1" type="checkbox" 
					  onchange = "socioCorporativoRelacionado(this);"
					  name="SNesCorporativo" id="SNesCorporativo"
				      <cfif modo NEQ "ALTA" and rsSocios.SNesCorporativo EQ 1> checked</cfif>>
			    <label for="SNesCorporativo"><strong>#LB_SNesCorporativo#</strong></label>
			</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td valign="middle">
			<cfif modo neq 'ALTA' and rsSocios.GSNid gt 0>
				...
				<cf_sifGrupoSN  tabindex="1" form ="formSDG" query =#rsGrupoSNCon#>
			<cfelse>
				<cf_sifGrupoSN  tabindex="1" form ="formSDG">
			</cfif>
		</td>
	</tr>
	...
	<tr>
					<td valign="top" nowrap/>
			      	<td valign="top"><strong>#LB_IdentificacionFiscal#:</strong>&nbsp;</td>
	</tr>
	
	<tr>
					<td valign="top" nowrap/>
			      	<td valign="top">
						<input  tabindex="1" type="text" style="width:400px"name="SNnombreFiscal" size="75" maxlength="255"
						 onblur="javascript:ValidarID(this);"
						 value="<cfif modo NEQ "ALTA">#trim(rsSocios.SNnombreFiscal)#</cfif>"
						 >
					</td>
				</tr>
	<tr>
	...
	<tr>
		<td align="right" valign="top" nowrap>&nbsp;</td>
		<td valign="baseline">
			<input  tabindex="1" readonly name="SNcodigoext" type="text" size="30"  maxlength="25" value="<cfif modo NEQ "ALTA">#Trim(rsSocios.SNcodigoext)#</cfif>" ><!---- onFocus="javascript:this.select();" ----->
		</td>
	...
	<tr class = "socio_corporativo_relacionado">
		<td align="right" valign="top" nowrap>&nbsp;</td>
	...
	<tr>
		...
		<td valign="top" class = "socio_corporativo_relacionado">
			<cfif modo neq 'ALTA'>
			...
			<cfif not tengo_hijos>
				<!---
				 ...
				 --->
				<cf_sifsocios_corp tabindex="1" form="formSDG" SNcodigo="SNcodigoPadre" SNnombre="SNnombrePadre"
  	...
  	<tr>
		...
		<td valign="top">
			<cfset NoContrato = ( modo NEQ "ALTA") ? rsSocios.Contrato : ''>
			<label for="Contrato"><strong>Contrato</strong>
				<input  tabindex="1" name="Contrato" type="text" size="30"  maxlength="50" value="#Trim(NoContrato)#" >
			</label>
		</td>
	...
	<tr>
	  <td colspan="2" rowspan="10" valign="top">
			<cfif IsDefined('rsSocios.id_direccion') And Len(rsSocios.id_direccion)>
				...
				<cfif modalidad.readonly>
					<div style="width:80% ">
		          		<cf_sifdireccion tabindex="1" action="display" key="#rsSocios.id_direccion#" form="formSDG">
					</div>
				<cfelse>
		          <cf_sifdireccion tabindex="1" action="input" key="#rsSocios.id_direccion#" form="formSDG">
				</cfif>
			<cfelse>
		          <cf_sifdireccion  tabindex="1" action="input" form="formSDG">
			</cfif></td>
	...
	<tr id="tipoExt" <cfif (isDefined("rsSocios.SNtipo") AND "E" EQ rsSocios.SNtipo)>style="visibility:visible;"<cfelse> style="visibility:hidden;"</cfif>>
		...
		<td align="right" nowrap>
			<table>
	 			...
	 			<td><input name="IDFis" id="IDFis" size="30" maxlength="30" value="nFiscal"></td>
			 	<td width="41%">&nbsp;</td>
				<tr>
...
<script language="javascript" type="text/javascript">
	$("#SNidentificacion").change(function(){
		// validaRFCRepetido(this);
	}); 
	$("#SNidentificacion").focus(function(){
		this.select();
	}); 
...
--->
function evalua(){
document.getElementById("SNidentificacion").value = '';
...
function getMask(seleccionar,SNMid)
{
	...
	//Se comentan estás lineasde código dado que asignan otro tipo de formato a "SNidentificacion", cuando se realiza su modificación en "Identificación"
	var oCedulaMask = new Mask("#replace(LvarSNtipo,'X','##','ALL')#", "SocioNegocioRFC");
		oCedulaMask.attach(document.formSDG.SNidentificacion, oCedulaMask.mask, "SocioNegocioRFC");
			...
			for(row = 0; row < nRows; ++row)
			{
				...
				if (SNMascaras.getField(row, "SNtipo") == inpSNtipo.value)
				{
					...
					if (selected)
					{
						...
						document.formSDG.SNmask.value = SNMascaras.getField(row,"SNMascara");
					}
				}
			}
...
function socioCorporativoRelacionado(cb)
{
    var scr = document.getElementsByClassName("socio_corporativo_relacionado");
		if(cb.checked){
...
<cfif modo NEQ 'ALTA' AND rsSocios.SNesCorporativo>
    var scr = document.getElementsByClassName("socio_corporativo_relacionado");
    for (var i = 0; i < scr.length; i++) {
					scr[i].style.display = "none";
			}
...
function validaRFCRepetido(loRFC){
	
	var lVarRfc = loRFC.value.trim();
	var lvarLimpio = lVarRfc.replace(/-/g, '');
	if(lvarLimpio !== 'XAXX010101000' && lvarLimpio !== 'XEXX010101000'){
		$.ajax({
	        method: "post",
	        url: "AjaxValidacionesRFC.cfc",
	        async:false,
	        data: {
	            method: "fnValidaRFC",
	            RFCvalue: lVarRfc,
	        ...
	        dataType: "json",
	        success: function(obj) {
	        	if(obj.MSG == 'ExisteRFC'){
					document.getElementById("valRFC").innerHTML = 'El RFC ' + lVarRfc + ', ya se encuentra en uso!';
					document.getElementById("SNidentificacion").value = ''
	        	} else {
					document.getElementById("valRFC").innerHTML = '';
	        	}
	        }
	    });
	}
