...
<cfif modo neq "ALTA">
	<cfquery name="rsSNClasificacion" datasource="#Session.DSN#">
		select SNCEid, Ecodigo, SNCEcodigo, SNCEdescripcion, 
			SNCEcorporativo, PCCEobligatorio, PCCEactivo, PCEcatid,
			 SNCEalertar, SNCtiposocio, ts_rversion, SNCredyC
...
<cfquery name="rsHayCredCobranza" datasource="#Session.DSN#">
	select SNCredyC
	from SNClasificacionE 
	where CEcodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Session.CEcodigo#">
		<cfif isdefined('session.Ecodigo') and 
...
<form method="post" name="form1" id="form1" action="SQLSNClasificaciones.cfm">
<cfoutput>
	<cfif rsHayCredCobranza.RecordCount gt 0>
		<input type="hidden" name="hayCredCob" value="1">
	<cfelse>
		<input type="hidden" name="hayCredCob" value="0">
	</cfif>
	<table width="100%" cellpadding="1" cellspacing="0" border="0">
		...
		<tr > 
			<td nowrap></td>
			<td align="left">
				<input name="SNCredyC" id="SNCredyC" type="checkbox" tabindex="1"
					<cfif modo NEQ "ALTA" and rsSNClasificacion.SNCredyC EQ 1>checked</cfif>>
				<label for="SNCredyC" style="font-style:normal; font-variant:normal; font-weight:normal">Crédito y Cobranza</label>
			</td>
		</tr>
	</table>
	<cfif modo neq "ALTA">
...
<script language="javascript" type="text/javascript">
	function funcAlta(){
		if(document.form1.hayCredCob.value == '1' && document.form1.SNCredyC.checked){
			if (confirm("Ya existe un registro asociado a Crédito y Cobranza, ¿Desea actualizarlo a este?")) {
				return true;
			} else {
				return false;
			}
