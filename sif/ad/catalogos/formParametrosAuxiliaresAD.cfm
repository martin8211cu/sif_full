<cfset hayRolAplicadorInventario = 0 >
<cfset hayRolAutorizadorSN = 0 >
<cfset hayAutorizadorSNCheck = 0 >
<cfset hayRolAutorizadorTcd = 0 >
<cfset hayAutorizadorTcdCheck = 0 >
<cfset hayCuentaBalAsi = 0 >
<cfset hayCuentaDifCambAsi  = 0>
<cfset hayCuentaDifCamb2  = 0>
<cfset hayCtaExcepNIC21V2 =0>
<cfset hayCuentaFluctuacion  = 0>
<cfset hayCuentaEstimacion  = 0>
<cfset hayVencimiento1 = 0 >
<cfset hayVencimiento2 = 0 >
<cfset hayVencimiento3 = 0 >
<cfset hayVencimiento4 = 0 >
<cfset hayTipoRequisicion = 0 >
<cfset haySolicitante = 0 >
<cfset hayAutorizacion 	= 0 >
<cfset hayConsReq 		= 0 >
<cfset haycalculoImp = 0 >
<cfset haySupervisor = 0 >
<cfset hayTNCredito = 0 >
<cfset hayTPCancelacion = 0 >
<cfset hayCierreCaja = 0 >
<cfset hayAumentaConsecutivo = 0 >
<cfset hayPlanCuentas = 0 >
<cfset hayValidaProduccion = 0 >
<cfset haySocioImportacion = 0 >
<cfset haySocioImportacionC = 0 >
<cfset hayCodigoArt = 0 >
<cfset hayTransaccionCP = 0 >
<cfset hayTransaccionCC = 0 >
<cfset hayImpuestos = 0 >
<cfset hayNomina = 0 >
<cfset hayTransaccionCCInterfaz = 0>
<cfset hayTransaccionCPInterfaz = 0>
<cfset recordarFormularioAsientos = 0>
<cfset  hayClienteContado = 0>

<cfset hayFormatoCedulaJuridica = 0 >
<cfset hayFormatoCedulaFisica = 0 >
<cfset hayFormatoCedulaActFijo = 0 >
<cfset hayCierreConta = 0 >
<cfset hayFechaCierreConta = 0 >
<cfset hayVerClasifCC = 0 >
<cfset hayVerClasifCP = 0 >

<cfset hayNoContaCosto = 0 >
<cfset hayVerGruCtas = 0 >

<cfset hayRepCotProveedoresLocal = 0 >
<cfset hayRepCotProveedoresImportacion = 0 >
<cfset hayRepCotProcesoLocal = 0 >
<cfset hayRepCotProcesoImportacion = 0 >

<cfset hayRolSolic = 0 >
<cfset hayRolComprador = 0 >
<cfset hayRecurrentes = 0 >

<cfset hayLeyenda = 0 >
<cfset hayLeyendaCxC = 0 >

<cfset hayVerAstDet = 0>
<cfset hayRetAuto = 0>
<cfset hayAutorizacionCustodia = 0>

<cfset hayUsuarioWS = 0>
<cfset  hayVendedorCxC = 0>
<cfset  hayControlInventarios = 0>
<cfset  hayVersion = 0>
<cfset  hayDeciFac = 0>
<cfset  hayrepCajaAsignado = 0>
<cfset  hayrepVencimientoSocio = 0>
<cfset  hayMontoMaximoVuelto = 0>
<cfset  hayCFRemesas = 0>
<cfset  hayintegraFacturaDigital = 0>
<cfset  hayProveedorFacturaDigital = 0>
<cfset  hayInterfazSGA = 0>
<cfset  hayContabilizarAplicarTransa = 0>
<cfset  hayValidacionCorreoYtelefono = 0>
<cfset  hayTFiscal = 0>
<cfset  hayRemision = 0>
<cfset  hayCVerano = 0>

<cfset hayCuentaDifCamMultiIngreso = 0>
<cfset hayCuentaDifCamMultiEgreso = 0>
<cfset hayCuentaDifCamMultiCosto = 0>
<!----NO ASOCIAR BENEFICIARIO CON BANCO---->
<cfset hayTESbeneTCE = 0 >

<cfset hayAutorizadorTcdCheck = 0>

<!----Control de Evento Inicia---->
<cfset hayControlEvento = 0>
<!----Control de Evento Fin---->
<!----Obtinen el Filtro de empresa, Segun si es corporativo o NO---->
 <cfquery name="rsConsultaCorp" datasource="asp">
	select *
	from CuentaEmpresarial
	where Ecorporativa is not null
	  and CEcodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Session.CEcodigo#">
</cfquery>
<cfif isdefined('session.Ecodigo') and isdefined('session.Ecodigocorp') and session.Ecodigo NEQ session.Ecodigocorp and rsConsultaCorp.RecordCount GT 0>
	  <cfset filtroE = " Ecodigo=#session.Ecodigo# ">
<cfelse>
	  <cfset filtroE = " Ecodigo is null ">
</cfif>
 <!---- Obtiene los datos de la tabla de Parámetros según el pcodigo ---->
<cffunction name="ObtenerDato" returntype="query">
	<cfargument name="pcodigo" type="numeric" required="true">
	<cfquery name="rs" datasource="#Session.DSN#">
		select Pvalor
		from Parametros
		where Ecodigo = #Session.Ecodigo#
		  and Pcodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.pcodigo#">
	</cfquery>
	<cfreturn rs>
</cffunction>

<!---- PARAMETROS NUEVOS RELACIONADO CON LOS CAMBIOS PEDIDOS PARA EL MODULO CxC ---->
<cfset maskCedJ = ObtenerDato(620)>
<cfset maskCedF = ObtenerDato(630)>
<cfset maskCedE = ObtenerDato(5600)>

<cfset NoModificar = 0 >

<cfset LvarSNCEid = ObtenerDato(695)>
<cfset LvarActEmp = ObtenerDato(1801).Pvalor>
<cfset LvarActEmpF = ObtenerDato(1803).Pvalor>
<cfset LvarConGastoI = ObtenerDato(1802).Pvalor>

<!----Categoría Informática---->
<cfset LvarCategoria = ObtenerDato(4200).Pvalor>

<!---- PARAMETROS NUEVOS RELACIONADO CON LOS CAMBIOS PEDIDOS PARA EL MODULO CxC ---->
<cfif maskCedJ.Recordcount GT 0 ><cfset hayFormatoCedulaJuridica = 1 ></cfif>
<cfif maskCedF.Recordcount GT 0 ><cfset hayFormatoCedulaFisica = 1 ></cfif>
<cfif maskCedE.Recordcount GT 0 ><cfset hayFormatoCedulaExtranjero = 1 ></cfif>

<cfset maskFCorte = ObtenerDato(670)>
<cfset chkFCorte = ObtenerDato(700)>
<cfif maskFCorte.Recordcount GT 0 ><cfset hayFechaCierreConta = 1 ></cfif>
<cfif chkFCorte.Recordcount GT 0 ><cfset hayCierreConta = 1 ></cfif>

<cfset maskVerClasifCC = ObtenerDato(680)>
<cfif maskVerClasifCC.Recordcount GT 0 >
	<cfset hayVerClasifCC = 1 >
	<cfset ValorVerClasifCC = maskVerClasifCC.Pvalor>
<cfelse>
	<cfset ValorVerClasifCC = 0>
</cfif>

<!---- PARAMETROS NUEVOS RELACIONADO CON LOS CAMBIOS PEDIDOS PARA EL MODULO CxP ---->
<cfset maskVerClasifCP = ObtenerDato(690)>
<cfif maskVerClasifCP.Recordcount GT 0 ><cfset hayVerClasifCP = 1 ></cfif>
<cfif maskVerClasifCP.Recordcount GT 0 >
	<cfset hayVerClasifCP = 1 >
	<cfset ValorVerClasifCP = maskVerClasifCP.Pvalor>
<cfelse>
	<cfset ValorVerClasifCP = 0>
</cfif>

<!---- PARAMETRO NUEVO RELACIONADO CON LA PRESENTACION DEL ASIENTO EN FORMA DETALLADA ---->
<cfset maskVerAstDet = ObtenerDato(930)>
<cfif maskVerAstDet.Recordcount GT 0 ><cfset hayVerAstDet = 1 ></cfif>
<cfif maskVerAstDet.Recordcount GT 0 >
	<cfset hayVerAstDet = 1 >
	<cfset ValorVerAstDet = maskVerAstDet.Pvalor>
<cfelse>
	<cfset ValorVerAstDet = 0>
</cfif>
<!----PARAMETRO 1600: CONSIDERAR TRASLADOS, RETIROS Y REVALUACIÓNES DEL ÚLTIMO PERIODO PARA LA REVALUACIÓN---->
<cfset ProRev = ObtenerDato(1600)>
<cfif ProRev.Recordcount GT 0><cfset ProRev =ProRev.Pvalor><cfelse><cfset ProRev =0></cfif>
<!---- PARAMETRO NUEVO RELACIONADO CON LA APLICACION DE RETIROS DE FORMA AUTOMATICA ---->
<cfset maskRetAuto = ObtenerDato(960)>
<cfif maskRetAuto.Recordcount GT 0 ><cfset hayRetAuto = 1 ></cfif>
<cfif maskRetAuto.Recordcount GT 0 >
	<cfset hayRetAuto = 1 >
	<cfset ValorRetAuto = maskRetAuto.Pvalor>
<cfelse>
	<cfset ValorRetAuto = 0>
</cfif>

<!---- PARAMETRO NUEVO RELACIONADO CON LA AUTORIZACION EN AUTOGESTION POR ENCARGADO DE CENTRO DE CUSTODIA ---->
<cfset maskAutorizacionCustodia = ObtenerDato(990)>
<cfif maskAutorizacionCustodia.Recordcount GT 0 ><cfset hayAutorizacionCustodia = 1 ></cfif>
<cfif maskAutorizacionCustodia.Recordcount GT 0 >
	<cfset hayAutorizacionCustodia = 1 >
	<cfset ValorAutorizacionCustodia = maskAutorizacionCustodia.Pvalor>
<cfelse>
	<cfset ValorAutorizacionCustodia = 0>
</cfif>


<cfset rschkUtilAuto   = ObtenerDato(15500)>
<cfset LvarchkUtilAuto = 0 >
<cfif rschkUtilAuto.Recordcount GT 0 and rschkUtilAuto.Pvalor EQ 1>
	<cfset LvarchkUtilAuto = 1 >
</cfif>

<cfquery name="rsExistenDatosSNCorp" datasource="#session.DSN#">
	select count(1) as cantidad
	from SNegocios
	where SNidentificacion is not null
	and Ecodigo = #session.Ecodigo#
	and SNcodigo <> 9999
</cfquery>

<cfif isdefined('rsExistenDatosSNCorp') and rsExistenDatosSNCorp.cantidad GT 0>
	<cfset NoModificar = 1 >
</cfif>
 

<cfset definidos = ObtenerDato(5)>
<cfset existenParametrosDefinidos = false >
<cfif definidos.RecordCount GT 0 >
	<cfif definidos.Pvalor NEQ "N" >
		<cfset existenParametrosDefinidos = true >
	</cfif>
</cfif>
<cfset vencimiento = "">

<cfquery name="rsTipoRequisicion" datasource="#Session.DSN#">
	select Ecodigo, TRcodigo, TRdescripcion, ts_rversion from TRequisicion
	where Ecodigo = #Session.Ecodigo#
</cfquery>
<cfset trequisicion = "">

<cfset solicitante = "">

<cfquery name="rsTransaccionCP" datasource="#Session.DSN#">
	select CPTcodigo, CPTdescripcion
	from CPTransacciones
	where Ecodigo=#Session.Ecodigo#
	  and CPTpago = 0
</cfquery>

<cfquery name="rsTransaccionCC" datasource="#Session.DSN#">
	select CCTcodigo, CCTdescripcion
	from CCTransacciones
	where Ecodigo=#Session.Ecodigo#
	  and CCTpago = 0
</cfquery>

<cfquery name="rsImpuestos" datasource="#Session.DSN#">
	select Icodigo, Idescripcion
	from Impuestos
	where Ecodigo=#Session.Ecodigo#
</cfquery>


<cfset maskNoContaCosto = ObtenerDato(740)>
<cfif maskNoContaCosto.Recordcount GT 0 >
	<cfset hayNoContaCosto = 1 >
	<cfset ValorNoContaCosto = maskNoContaCosto.Pvalor>
<cfelse>
	<cfset ValorNoContaCosto = 0>
</cfif>

<cfset maskVerGruCtas = ObtenerDato(741)>
<cfif maskVerGruCtas.Recordcount GT 0 >
	<cfset hayVerGruCtas = 1 >
	<cfset VerGruCtas = maskVerGruCtas.Pvalor>
<cfelse>
	<cfset VerGruCtas = 0>
</cfif>


<!---- Parámetro Nuevo Relacionado con la Leyenda de Balance General y Estado de Resultados ---->
<cfset leyenda = ObtenerDato(920)>
<cfif leyenda.Recordcount GT 0 ><cfset hayLeyenda = 1 ></cfif>

<cfset leyendaCxC = ObtenerDato(921)>
<cfif leyendaCxC.Recordcount GT 0 ><cfset hayLeyendaCxC = 1 ></cfif>

<!---- Parámetro Tipo de Transacción en Depositos de Peaje ---->
<cfset ValorTipoTransPeaje = ObtenerDato(1800).Pvalor>
<cfset ValorCuentadePeaje = ObtenerDato(1804).Pvalor>

<!------  Valores para Banco y cuentas de Peaje  ------->
<cfset Pid="">
<cfset Bid="">
<cfset BTid="">
<cfset PAdocumento="">
<cfset PAdescripcion="">
<cfset PAmonto=0.0>
<cfset PAfecha=Dateformat(now(),'dd/mm/yyyy')>
<cfset Mcodigo="">
<cfset Mnombre="">
<cfset CBid="">
<cfset CBcodigo="">
<cfset CBdescripcion="">
<cfset tipoCambio=1>

<cfif ValorCuentadePeaje neq ''> <!-----Si ya existe una cuenta -------->

	<cfset CBid= #ValorCuentadePeaje#>
	<cfquery name="rsCuentaBanco" datasource="#session.dsn#">
		Select cb.CBid,cb.CBcodigo,cb.CBdescripcion, b.Bid
	   from  CuentasBancos cb
			inner join Bancos b
				on cb.Bid = b.Bid
				and cb.Ecodigo = b.Ecodigo
			inner join Monedas m
				on m.Mcodigo = cb.Mcodigo
			  where cb.CBid = #CBid#
              	and cb.CBesTCE = <cfqueryparam value="0" cfsqltype="cf_sql_bit">
	</cfquery>
		<cfif rsCuentaBanco.recordcount gt 0>
			<cfset Bid = rsCuentaBanco.Bid>
			<cfset CBcodigo=rsCuentaBanco.CBcodigo>
			<cfset CBdescripcion=rsCuentaBanco.CBdescripcion>
		</cfif>
</cfif>
<!------------->


<!----Codigo 2600 = Cuenta de ingreso por Financiamiento---->
<cfquery name="rsCFcuentaFinanciamiento" datasource="#Session.DSN#">
	select Pvalor as CFcuentac,
		   Pvalor as Cformato,
		   '' as Cdescripcion,
		   '' as Ccuenta
	from Parametros
	where Ecodigo = #Session.Ecodigo#
	and Pcodigo = 2600
</cfquery>
<!----Codigo 2800 = Cuenta de Egresos por Amortización de préstamos---->
<cfquery name="rsCFcuentacAmortizacion" datasource="#Session.DSN#">
	select Pvalor as CFcuentac,
		   Pvalor as Cformato,
		   '' as Cdescripcion,
		   '' as Ccuenta
	from Parametros
	where Ecodigo = #Session.Ecodigo#
	and Pcodigo = 2800
</cfquery>

<!----Codigo 2920: Incluir el Socio de Negocios en las Lineas de la Poliza Contable para CXC y CXP---->
<cfif ObtenerDato(2920).recordCount GT 0 and ObtenerDato(2920).Pvalor EQ 1><cfset SNPoliza =1><cfelse><cfset SNPoliza =0></cfif>

<cfsavecontent variable="helpmsj">
Formato de máscara se utilizara<br />
&nbsp;'X': para Letras.<br />
&nbsp;'?': para Numeros. <br />
&nbsp;'*': para Letras y Números. <br />
</cfsavecontent>
<cfsavecontent variable="helpimg">
	<img src="../../imagenes/Help01_T.gif" width="25" height="23" border="0"/>
</cfsavecontent>

<script language="JavaScript1.2" >

	function validaCedula(){
		var dato = document.form1.NoModificar.value;

		if (dato == '1'){
			alert('La cédula no puede ser modificada, ya hay datos relacionados.');
			return false;
		}
	}

	//Valida solo X mayúsculas y -. Llamar en evento onKeyPress Ejem. onKeyPress="return acceptX(event)"
	function acceptX(evt){
		// NOTE: Backspace = 8, Enter = 13, '0' = 48, '9' = 57
		var key = nav4 ? evt.which : evt.keyCode;
		return (key <= 13 || key == 88 || key == 45);
	}

	//Valida solo *
	function acceptAs(evt){
		// NOTE: Backspace = 8, Enter = 13, '0' = 48, '9' = 57
		var key = nav4 ? evt.which : evt.keyCode;
		return (key <= 13 || key == 42);
	}

	// valida los campos
	function valida() {
		<cfif not existenParametrosDefinidos >
			alert('¡No están definidos los parámetros generales!');
			return false;
		</cfif>

		if(document.form1.chkRCDND.checked){
			if((document.form1.RCDND.value == '') || (document.form1.RCDND.value <= 0) || (document.form1.RCDND.value > 16)){
				alert("Error,el campo 'Cantidad de Digitos en Número de Documento' es requerido y no puede ser cero ni mayor que 16 !");
				return false;
			}
		}

		if(document.form1.chkAgrupacionOrdenes.checked && document.form1.selFrecAgrupOC.value == 'M')
		{
			if(document.form1.txtDiaMesAgrupOC.value == '' || document.form1.txtDiaMesAgrupOC.value <= 0 || document.form1.txtDiaMesAgrupOC.value > 31)
			{
				alert("Error, el campo 'El día' es requerido, debe ser mayor a 0 y menor a 32!");
				return false;
			}
		}

		if(document.form1.mascaraCodArt.value.length > 0 && document.form1.mascaraCodArt.value.length < 10)
		{
			alert("Error, la mascara del codigo del articulo debe ser mayor o igUsar Boleta de Trasladoual a 10!");
			return false;
		}
		<!----SML.---->
		var TipoPlaca = document.getElementById("cboTipoPlaca").value;
        document.getElementById('hdTipoPlaca').value = TipoPlaca;
		return true;





	}

	var popUpWin=0;

	function popUpWindow(URLStr, left, top, width, height)
	{
	  if(popUpWin)
	  {
		if(!popUpWin.closed) popUpWin.close();
	  }
	  popUpWin = open(URLStr, 'popUpWin', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbar=yes,resizable=yes,copyhistory=yes,width='+width+',height='+height+',left='+left+', top='+top+',screenX='+left+',screenY='+top+'');
	}

	function doSolicitantes() {
		popUpWindow("ConlisSolicitantesCM.cfm?form=form1&id=CMSid&nombre=CMSnombre",250,200,650,350);
	}

	function doSupervisor() {
		popUpWindow("ConlisUsuariosFA.cfm",250,200,650,350);
	}

	function doSocioImportacionC() {
		popUpWindow("ConlisSocioImportacion.cfm",250,200,650,350);
	}
	function doSocioImportacion() {
		popUpWindow("ConlisSocioImportacion.cfm",250,200,650,350);
	}

	function cantDig(ck){
		var _divCantDig = document.getElementById("divCantDig");

		if(ck.checked)
			_divCantDig.style.display = '';
		else
			_divCantDig.style.display = 'none';
	}

	function reestablecerDiasSemana()
	{
		document.form1.chkAgruparD.checked = false;
		document.form1.chkAgruparL.checked = false;
		document.form1.chkAgruparK.checked = false;
		document.form1.chkAgruparM.checked = false;
		document.form1.chkAgruparJ.checked = false;
		document.form1.chkAgruparV.checked = false;
		document.form1.chkAgruparS.checked = false;
	}

	function cambiaAgrupacionOrdenes()
	{
		if(document.form1.chkAgrupacionOrdenes.checked)
		{
			document.getElementById("trFrecAgrupOC").style.display = '';
		}
		else
		{
			document.getElementById("trFrecAgrupOC").style.display = 'none';
			document.getElementById("trDiasSemanaAgrupOC").style.display = 'none';
			document.getElementById("trDiaMesAgrupOC").style.display = 'none';

			document.form1.selFrecAgrupOC.value = 'D';
			reestablecerDiasSemana();
			document.form1.txtDiaMesAgrupOC.value = '1';
		}
	}

	function cambiaFrecAgrupOC(frecuencia)
	{
		if(frecuencia == 'D')
		{
			document.getElementById("trDiasSemanaAgrupOC").style.display = 'none';
			document.getElementById("trDiaMesAgrupOC").style.display = 'none';
			reestablecerDiasSemana();
			document.form1.txtDiaMesAgrupOC.value = '1';
		}
		else if(frecuencia == 'S')
		{
			document.getElementById("trDiasSemanaAgrupOC").style.display = '';
			document.getElementById("trDiaMesAgrupOC").style.display = 'none';
			document.form1.txtDiaMesAgrupOC.value = '1';
		}
		else if(frecuencia == 'M')
		{
			document.getElementById("trDiasSemanaAgrupOC").style.display = 'none';
			document.getElementById("trDiaMesAgrupOC").style.display = '';
			reestablecerDiasSemana();
		}
	}
</script>


<!---- <cfdump var="#session.menues.SMcodigo#">---->
<cfif isDefined("url.SMcodigo")>
	<cfset LvarSMcodigo="#url.SMcodigo#">
<cfelse>
	<cfset LvarSMcodigo="">
</cfif>


<!---- Formulario para filtro ---->
<div class="form-inline" >
	<div class="col-md-8" style="margin:15px">
		<div class="form-group">
			<select name="listmod" id="listmod" onchange="rslistmod()">
				<option value="">
					-Seleccione modulo
				</option>
				<option value="CG ">
					Contabilidad General
				</option>
				<option value="TR">
					Trámites
				</option>
				<option value="PRES">
					Control de Presupuesto
				</option>
				<option value="CEVENTO">
					Control de Evento
				</option>
				<option value="GN">
					Vencimientos
				</option>
				<option value="CM">
					Compras
				</option>
				<option value="FAC">
					Facturación
				</option>
				<option value="IV">
					Producción y transformación de inventario
				</option>
				<option value="INTERPMI">
					Interfaces
				</option>
				<option value="FA">
					Manejo del Descuento a nivel de Documento ..
				</option>
				<option value="CC">
					Cuentas por Cobrar
				</option>
				<option value="CP">
					Cuentas por Pagar
				</option>
				<option value="MB">
					Bancos
				</option>
				<option value="TES">
					Tesorería
				</option>
				<option value="AF">
					Activos Fijos
				</option>
				<option value="QPASS">
					Quick Pass
				</option>
				<option value="PE">
					Peaje
				</option>
				<option value="GA">
					Garantía
				</option>
				<option value="PC">
					Plan de Compras Gobierno
				</option>
				<option value="TCE">
					Tarjetas de Crédito Empresarial
				</option>
				<option value="SN">
					Socios de negocio
				</option>
			</select>
			<label for="filtrp">
				Realiza tu busqueda:
			</label>
			<input class="form-control" name="filtrp" id="filtrop" type="text" value="" readonly>
		</div>


		<button type="button" class="btn btn-default" name="bfil" id="bfil">
			Filtrar
		</button>
	</div>
</div>


<!--- Parametros --->
<form action="SQLParametrosAuxiliaresAD.cfm" method="post" name="form1" onsubmit=" return FrameFunction();">

	<section>
	<!--- CONTABILIDAD GENERAL --->
	<div class="CG">
		<table border="0" width="100%">
			<tr>
				<td colspan="3" class="listaPar" align="center" >Contabilidad General</td>
			</tr>
			<tr>
            	<td>&nbsp;</td>
				<td align="right">Cuenta Financiera de Balance para ajustar diferencias de Tipos de Cambio en Asientos:</td>
				<td>
					<cfset rsCuentaBalAs = ObtenerDato(25)>
					<cfif rsCuentaBalAs.RecordCount GT 0 >
						<cfset hayCuentaBalAsi = 1 >
					</cfif>

					<cfquery datasource="#session.dsn#" name="param25">
						select Pvalor
						from Parametros
						where Ecodigo = #Session.Ecodigo#
						and Pcodigo = 25
					</cfquery>

					<cfquery name="rsCuentaBalAs" datasource="#Session.DSN#">
						select CFdescripcion, CFcuenta, CFformato, Ccuenta
						from CFinanciera
						where CFcuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#param25.Pvalor#" null="#Len(Trim(param25.Pvalor)) EQ 0#" >
					</cfquery>

					<cfif isdefined('rsCuentaBalAs') and rsCuentaBalAs.RecordCount GT 0 and rsCuentaBalAs.CFcuenta NEQ ''>
						<cf_cuentas
						conexion="#Session.DSN#"
						conlis="S"
						query="#rsCuentaBalAs#"
						auxiliares="N"
						movimiento="S"
						form="form1"
						frame="frame1"
						descwidth="50"
						Ccuenta="CCuentaBalanceAsi"
						Cformato="FCBalanceAsi"
						Cmayor="MCBalanceAsi"
						>
						<cfelse>
						<cf_cuentas
						conexion="#Session.DSN#"
						conlis="S"
						auxiliares="N"
						movimiento="S"
						form="form1"
						frame="frame1"
						descwidth="50"
						Ccuenta="CCuentaBalanceAsi"
						Cformato="FCBalanceAsi"
						Cmayor="MCBalanceAsi"
						>
					</cfif>
				</td>
			</tr>

			<!---- *************************************************** ---->
			<tr>
				<td colspan="2" align="right" nowrap="nowrap">Cuenta de diferencial cambiario para conversión de estados:&nbsp;</td>
				<td>
					<cfset rsCuentaDifCambAs = ObtenerDato(810)>
					<cfif rsCuentaDifCambAs.RecordCount GT 0 >
						<cfset hayCuentaDifCambAsi = 1 >
					</cfif>

					<cfquery datasource="#session.dsn#" name="param810">
						select Pvalor
						from Parametros
						where Ecodigo = #Session.Ecodigo#
						and Pcodigo = 810
					</cfquery>
					<cfquery name="rsCuentaDifCambAs" datasource="#Session.DSN#">
						select CFdescripcion, CFcuenta, CFformato, Ccuenta
						from CFinanciera
						where CFcuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#param810.Pvalor#" null="#Len(Trim(param810.Pvalor)) EQ 0#" >
					</cfquery>

					<cfif isdefined('rsCuentaDifCambAs') and rsCuentaDifCambAs.RecordCount GT 0 and rsCuentaDifCambAs.CFcuenta NEQ ''>
						<cf_cuentas
						conexion="#Session.DSN#"
						conlis="S"
						query="#rsCuentaDifCambAs#"
						auxiliares="N"
						movimiento="S"
						form="form1"
						frame="frame1"
						descwidth="50"
						Ccuenta="CCuentaDifCambAsi"
						Cformato="FCDifCambAsi"
						Cmayor="MCDifCambAsi">
					<cfelse>
						<cf_cuentas
						conexion="#Session.DSN#"
						conlis="S"
						auxiliares="N"
						movimiento="S"
						form="form1"
						frame="frame1"
						descwidth="50"
						Ccuenta="CCuentaDifCambAsi"
						Cformato="FCDifCambAsi"
						Cmayor="MCDifCambAsi"
						>
					</cfif>
				</td>
			</tr>
                            <!----- 2a Conversion  de Estados Financieros ---->
                            <tr>

								<td colspan="2" align="right" nowrap="nowrap">Cuenta de diferencial cambiario para segunda conversión de estados:&nbsp;</td>
								<td>
									<cfset rsCuentaDifCamb2 = ObtenerDato(4500)>
									<cfif rsCuentaDifCamb2.RecordCount GT 0 >
										<cfset hayCuentaDifCamb2 = 1 >
									</cfif>

									<cfquery datasource="#session.dsn#" name="param4500">
										select Pvalor
										from Parametros
										where Ecodigo = #Session.Ecodigo#
										and Pcodigo = 4500
									</cfquery>
									<cfquery name="rsCuentaDifCamb2" datasource="#Session.DSN#">
										select CFdescripcion, CFcuenta, CFformato, Ccuenta
										from CFinanciera
										where CFcuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#param4500.Pvalor#" null="#Len(Trim(param4500.Pvalor)) EQ 0#" >
									</cfquery>

									<cfif isdefined('rsCuentaDifCamb2') and rsCuentaDifCamb2.RecordCount GT 0 and rsCuentaDifCamb2.CFcuenta NEQ ''>
										<cf_cuentas
										conexion="#Session.DSN#"
										conlis="S"
										query="#rsCuentaDifCamb2#"
										auxiliares="N"
										movimiento="S"
										form="form1"
										frame="frame1"
										descwidth="50"
										Ccuenta="CCuentaDifCamb2"
										Cformato="FCDifCamb2"
										Cmayor="MCDifCamb2">
									<cfelse>
										<cf_cuentas
										conexion="#Session.DSN#"
										conlis="S"
										auxiliares="N"
										movimiento="S"
										form="form1"
										frame="frame1"
										descwidth="50"
										Ccuenta="CCuentaDifCamb2"
										Cformato="FCDifCamb2"
										Cmayor="MCDifCamb2"
										>
									</cfif>
								</td>
							</tr>
	        <!----►►►Parametros 3600: Cuenta de Diferencial Cambiario Primera conversión de estados B15◄◄◄---->
	        <tr>
				<td colspan="2" align="right" nowrap="nowrap">Cuenta de diferencial cambiario primera conversión de estados B15:&nbsp;
				</td>
				<td>
					<cfset rsCFcuentaB15_P = ObtenerDato(3600)>

					<cfquery name="rsCtaB15_P" datasource="#Session.DSN#">
						select CFdescripcion, CFcuenta, CFformato, Ccuenta
						from CFinanciera
	                    <cfif rsCFcuentaB15_P.Recordcount and LEN(TRIM(rsCFcuentaB15_P.Pvalor))>
							where CFcuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsCFcuentaB15_P.Pvalor#">
	                    <cfelse>
	                     	where 1=2
	                    </cfif>
					</cfquery>

					<cfif rsCtaB15_P.RecordCount>
						<cf_cuentas
						conexion="#Session.DSN#"
						conlis="S"
						query="#rsCtaB15_P#"
						auxiliares="N"
						movimiento="S"
						form="form1"
						frame="frame1"
						descwidth="50"
						Ccuenta="B15PCcuenta"
						Cformato="B15PCformato"
						Cmayor="B15PCmayor">
					<cfelse>
						<cf_cuentas
						conexion="#Session.DSN#"
						conlis="S"
						auxiliares="N"
						movimiento="S"
						form="form1"
						frame="frame1"
						descwidth="50"
						Ccuenta="B15PCcuenta"
						Cformato="B15PCformato"
						Cmayor="B15PCmayor">
					</cfif>
				</td>
			</tr>
            <!----►►►Parametros 3700: Cuenta de Diferencial Cambiario Segunda conversión de estados B15◄◄◄---->
            <tr>
				<td colspan="2" align="right" nowrap="nowrap">Cuenta de diferencial cambiario segunda conversión de estados B15:&nbsp;
				</td>
				<td>
					<cfset rsCFcuentaB15_S = ObtenerDato(3700)>

					<cfquery name="rsCtaB15_S" datasource="#Session.DSN#">
						select CFdescripcion, CFcuenta, CFformato, Ccuenta
						from CFinanciera
                        <cfif rsCFcuentaB15_S.Recordcount and LEN(TRIM(rsCFcuentaB15_S.Pvalor))>
							where CFcuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsCFcuentaB15_S.Pvalor#">
                        <cfelse>
                         	where 1=2
                        </cfif>
					</cfquery>

					<cfif rsCtaB15_S.RecordCount>
						<cf_cuentas
						conexion="#Session.DSN#"
						conlis="S"
						query="#rsCtaB15_S#"
						auxiliares="N"
						movimiento="S"
						form="form1"
						frame="frame1"
						descwidth="50"
						Ccuenta="B15SCcuenta"
						Cformato="B15SCformato"
						Cmayor="B15SCmayor">
					<cfelse>
						<cf_cuentas
						conexion="#Session.DSN#"
						conlis="S"
						auxiliares="N"
						movimiento="S"
						form="form1"
						frame="frame1"
						descwidth="50"
						Ccuenta="B15SCcuenta"
						Cformato="B15SCformato"
						Cmayor="B15SCmayor">
					</cfif>
				</td>
			</tr>
			<!----Inicia Cuenta de Fluctuación Cambiaria RVD 24/09/2014---->

            <tr>
				<td colspan="2" align="right" nowrap="nowrap">Cuenta de fluctuación cambiaria:&nbsp;</td>
				<td>
					<cfset rsCuentaFluctuacion = ObtenerDato(4000)>

                    <cfquery name="rsCtaFluctuacion" datasource="#Session.DSN#">
						select CFdescripcion, CFcuenta, CFformato, Ccuenta,Cmayor
						from CFinanciera
                        <cfif rsCuentaFluctuacion.Recordcount and LEN(TRIM(rsCuentaFluctuacion.Pvalor))>
							where Cmayor = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsCuentaFluctuacion.Pvalor#">
                            and CFpadre is null
                            and Ecodigo = #Session.Ecodigo#
                        <cfelse>
                         	where 1=2
                        </cfif>
					</cfquery>

					<cfif rsCtaFluctuacion.RecordCount>
						<cf_sifCuentasMayor idquery="#rsCtaFluctuacion.Cmayor#" size="50" tabindex = "1">
					<cfelse>
						<cf_sifCuentasMayor size="50" tabindex = "1">
					</cfif>
				</td>
			</tr>


           <!----Termina Cuenta de Fluctuación Cambiaria RVD 24/09/2014---->

						   <!---- SML Inicia 14032016 Cuenta especial de comportamiento “tipo multimoneda” ---->

                             <tr>

								<td colspan="2" align="right" nowrap="nowrap">Cuenta de comportamiento "tipo moneda":&nbsp;</td>
								<td>
									<cfset rsCuentaTipoMoneda= ObtenerDato(4900)>

                                    <cfquery name="rsCtaTipoMoneda" datasource="#Session.DSN#">
										select CFdescripcion, CFcuenta, CFformato, Ccuenta,Cmayor
										from CFinanciera
                                        <cfif rsCuentaTipoMoneda.Recordcount and LEN(TRIM(rsCuentaTipoMoneda.Pvalor))>
											where Cmayor = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsCuentaTipoMoneda.Pvalor#">
                                            and CFpadre is null
                                            and Ecodigo = #Session.Ecodigo#
                                        <cfelse>
                                         	where 1=2
                                        </cfif>
									</cfquery>

									<cfif rsCtaTipoMoneda.RecordCount>
										<cf_sifCuentasMayor idquery="#rsCtaTipoMoneda.Cmayor#" size="50" tabindex = "1" Cmayor="CmayorTipoMoneda" Cdescripcion="CmayorTipoMonedaDesc">
									<cfelse>
										<cf_sifCuentasMayor size="50" tabindex = "1" Cmayor="CmayorTipoMoneda" Cdescripcion="CmayorTipoMonedaDesc">
									</cfif>
								</td>
							</tr>


                           <!----SML Fin---->


			<!---- *************************************************** ---->
			<tr>
			  <td>&nbsp;</td>
			  <td align="right" nowrap>Validar Fecha de Asientos contra Periodo y Mes antes de Generar o Aplicar:&nbsp;</td>
			  <td>
			  	<input type="checkbox" name="chkCierreConta" value="S" <cfif chkFCorte.Pvalor EQ 'S'> checked</cfif>>
			  </td>
			</tr>
			<tr>
			  <td>&nbsp;</td>
			  <td align="right" nowrap>Fecha de Cierre de Contabilidad:&nbsp;</td>
			  <td><cf_sifcalendario form="form1" value="#LSDateFormat(maskFCorte.Pvalor,'dd/mm/yyyy')#" name="fechacierreConta"></td>
			</tr>
			<tr>
			  <td>&nbsp;</td>
			  <td align="right" nowrap>Leyenda para Balance General y Estado de Resultados:&nbsp;</td>
			  <td><input name="verLeyenda" type="text" size="50" maxlength="100" value="<cfoutput>#HTMLEditFormat(leyenda.Pvalor)#</cfoutput>"></td>
			</tr>
			<tr>
				<td colspan="2" align="right" nowrap="nowrap">Cuenta contable de balance para reversi&oacute;n de estimaci&oacute;n:&nbsp;</td>
				<td>
					<cfset param980 = ObtenerDato(980)>
					<cfif param980.RecordCount GT 0 >
						<cfset hayCuentaEstimacion = 1 >
					</cfif>

					<cfquery name="rsCuentaEstimacion" datasource="#Session.DSN#">
						select CFdescripcion, CFcuenta, CFformato, Ccuenta
						from CFinanciera
						where CFcuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#param980.Pvalor#" null="#Len(Trim(param980.Pvalor)) EQ 0#" >
					</cfquery>

					<cfif isdefined('rsCuentaEstimacion') and rsCuentaEstimacion.RecordCount GT 0 and rsCuentaEstimacion.CFcuenta NEQ ''>
						<cf_cuentas conexion="#Session.DSN#" conlis="S" auxiliares="N" movimiento="S" form="form1" frame="frame1" descwidth="50" Ccuenta="CCuentaEstimacion" Cformato="FCEstimacion" Cmayor="MCEstimacion" query="#rsCuentaEstimacion#">
					<cfelse>
						<cf_cuentas conexion="#Session.DSN#" conlis="S" auxiliares="N" movimiento="S" form="form1" frame="frame1" descwidth="50" Ccuenta="CCuentaEstimacion" Cformato="FCEstimacion" Cmayor="MCEstimacion">
					</cfif>
				</td>
			</tr>
			<!---- Agregar descripción de cuenta en consulta Pólizas Contables ---->
			<cfset rsAgregarDesc = ObtenerDato(981)>
			<cfset hayAgregarDesc = 0 >
			<cfif rsAgregarDesc.RecordCount GT 0 >
				<cfset hayAgregarDesc = 1 >
				<cfset AgregarDesc = rsAgregarDesc.Pvalor>
			<cfelse>
				<cfset AgregarDesc = '0' >
			</cfif>
			<tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Agregar descripci&oacute;n de cuenta en consulta P&oacute;lizas Contables:&nbsp;</td>
				<td><input type="checkbox" name="chkAgregarDesc" <cfif AgregarDesc eq '1'>checked</cfif> ></td>
			</tr>

            <!---- Importar Asientos Contables con Centro Funcional ---->
			<cfset rsImportaConCF = ObtenerDato(982)>
			<cfset hayImportaConCF = 0 >
			<cfif rsImportaConCF.RecordCount GT 0>
				<cfset hayImportaConCF = 1 >
				<cfset LvarImportaConCF = rsImportaConCF.Pvalor>
			<cfelse>
				<cfset LvarImportaConCF = '0' >
			</cfif>
			<tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Importar Asientos Contables con Centro Funcional:&nbsp;</td>
				<td><input type="checkbox" name="chkImportaConCF" <cfif LvarImportaConCF eq '1'>checked</cfif> ></td>
			</tr>

            <!---- En el cierre de auxiliares no generar en bancos asientos de diferencial cambiario ---->
			<cfset rsParam = ObtenerDato(2210)>
            <cfset hayMBAsientoDifCam = 0 >
			<cfif rsParam.RecordCount GT 0>
				<cfset hayMBAsientoDifCam = 1 >
				<cfset LvarMBAsientoDifCam = rsParam.Pvalor>
			<cfelse>
				<cfset LvarMBAsientoDifCam = '0' >
			</cfif>
			<tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Cierre de auxiliares no genera en Bancos asientos de diferencial cambiario:&nbsp;</td>
				<td><input type="checkbox" name="chkMBAsientoDifCam" <cfif LvarMBAsientoDifCam eq '1'>checked</cfif> ></td>
			</tr>

            <!---- Descripcion Alterna de Valores de Catálogos ---->
			<cfset rsParam = ObtenerDato(99)>
            <cfset hayPlanCtasDescAlterna = 0 >
			<cfif rsParam.RecordCount GT 0>
				<cfset hayPlanCtasDescAlterna = 1 >
				<cfset LvarPlanCtasDescAlterna = rsParam.Pvalor>
			<cfelse>
				<cfset LvarPlanCtasDescAlterna = '0' >
			</cfif>
			<tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Utilizar Descripción Alterna de Valores de Catálogos Corporativos al Generar Cuentas Financieras:&nbsp;</td>
				<td>
					<input type="checkbox" 	name="chkPlanCtasDescAlterna" value="1" <cfif LvarPlanCtasDescAlterna eq '1'>checked</cfif>>
			        <input type="hidden" 	name="hayPlanCtasDescAlterna" value="#hayPlanCtasDescAlterna#">
				</td>
			</tr>
            <!---- Descripcion Alterna de Valores de Catálogos ---->
			<cfset rsParam = ObtenerDato(15410)>
           <cfif rsParam.RecordCount GT 0>
				<cfset recordarFormularioAsientos = rsParam.Pvalor>
            <cfelse>
            	<cfset recordarFormularioAsientos = 0 >
			</cfif>
            <tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Mostrar check para recordar temporalmente el formulario al agregar un detalle en documentos contables:&nbsp;</td>
				<td>
					<input type="checkbox" 	name="chkrecordarFormAC" value="1" <cfif recordarFormularioAsientos eq '1'>checked</cfif>>
			        <input type="hidden" 	name="recordarFormAC" value="0">
				</td>
			</tr>
            <!---- Descripcion de Cuentas Financieras en el Idioma de la Empresa ---->
			<cfset rsParam = ObtenerDato(200005)>
           	<cfif rsParam.RecordCount GT 0>
				<cfset GenDescCtasFinIEmpresa = rsParam.Pvalor>
            <cfelse>
            	<cfset GenDescCtasFinIEmpresa = 0 >
			</cfif>
            <tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Generar la Descripción de Cuentas Financieras en el Idioma de la Empresa:&nbsp;</td>
				<td>
					<input type="checkbox" name="chkGenDescCtasFinI" value="1" <cfif GenDescCtasFinIEmpresa eq '1'>checked</cfif>>
				</td>
			</tr>

            <!---- Presentar la descripción de las Cuentas según el idioma del Usuario ---->
			<cfset rsParam = ObtenerDato(200010)>
           	<cfif rsParam.RecordCount GT 0>
				<cfset DescIUsuario = rsParam.Pvalor>
            <cfelse>
            	<cfset DescIUsuario = 0 >
			</cfif>
            <tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Presentar la descripción de las Cuentas según el idioma del Usuario:&nbsp;</td>
				<td>
					<input type="checkbox" 	name="chkDescIUsuario" value="1" <cfif DescIUsuario eq '1'>checked</cfif>>
				</td>
			</tr>

             <!---- Utilizar Conversión NIC 21 Historico---->
			<cfset rsParam = ObtenerDato(200011)>
           	<cfif rsParam.RecordCount GT 0>
				<cfset B15Historico = rsParam.Pvalor>
            <cfelse>
            	<cfset B15Historico = 0 >
			</cfif>
            <tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Utilizar conversión NIC 21:&nbsp;</td>
				<td>
					<input type="checkbox" 	name="chkB15Historico" value="1" <cfif B15Historico eq '1'>checked</cfif>>
				</td>
			</tr>


           <!----Considerar cuentas de orden para conversión---->
			<cfset rsParam = ObtenerDato(200012)>
           	<cfif rsParam.RecordCount GT 0>
				<cfset CtasOrden = rsParam.Pvalor>
            <cfelse>
            	<cfset CtasOrden = 0 >
			</cfif>
            <tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Considerar cuentas de orden para Conversión:&nbsp;</td>
				<td>
					<input type="checkbox" 	name="chkCtasOrden" value="1" <cfif CtasOrden eq '1'>checked</cfif>>
				</td>
			</tr>
			<!----Considerar si en la carga masiva de polizas, puede contener documentos de intercompañias---->
			<cfset rsParam = ObtenerDato(200013)>
           	<cfif rsParam.RecordCount GT 0>
				<cfset polInter = rsParam.Pvalor>
            <cfelse>
            	<cfset polInter = 0 >
			</cfif>
            <tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Aceptar intercompa&ntilde;ias en carga de polizas:&nbsp;</td>
				<td>
					<input type="checkbox" 	name="chkPolizaIntercompany" value="1" <cfif polInter eq '1'>checked</cfif>>
				</td>
			</tr>
			 <!----Utilizar  segunda conversion moneda funcional---->
							<cfset rsParam = ObtenerDato(200013)>
                           	<cfif rsParam.RecordCount GT 0>
								<cfset Ctas2aConv = rsParam.Pvalor>
                            <cfelse>
                            	<cfset Ctas2aConv = 0 >
							</cfif>
                            <tr>
								<td>&nbsp;</td>
								<td align="right" nowrap>Generar segunda conversión de estados financieros:&nbsp;</td>
								<td>
									<input type="checkbox" 	name="chkCtas2aConv" value="1" <cfif Ctas2aConv eq '1'>checked</cfif>>
								</td>
							</tr>
			 <!----Utilizar  segunda conversion estados financieros versión 2.0---->
							<!---- <cfset rsParam = ObtenerDato(200014)>
                           	<cfif rsParam.RecordCount GT 0>
								<cfset Ctas2aConV2 = rsParam.Pvalor>
                            <cfelse>
                            	<cfset Ctas2aConV2 = 0 >
							</cfif>
                            <tr>
								<td>&nbsp;</td>
								<td align="right" nowrap>Utilizar conversión de Estados Financiero versión 2.0:&nbsp;</td>
								<td>
									<input type="checkbox" 	name="chkCtas2aConV2" value="1" <cfif Ctas2aConV2 eq '1'>checked</cfif>>
								</td>
							</tr> ---->
                            <!----- Cuenta de Excepcion de Conversión estados financieros versión 2.0---->
                            <tr>

								<td colspan="2" align="right" nowrap="nowrap">Cuenta de Excepcion de Conversión estados financieros versión 2.0:&nbsp;</td>
								<td>
									<cfset rsCtaExcepNIC21V2 = ObtenerDato(4600)>
									<cfif rsCtaExcepNIC21V2.RecordCount GT 0 >
										<cfset hayCtaExcepNIC21V2 = 1 >
									</cfif>

									<cfquery datasource="#session.dsn#" name="param4600">
										select Pvalor
										from Parametros
										where Ecodigo = #Session.Ecodigo#
										and Pcodigo = 4600
									</cfquery>
									<cfquery name="rsCtaExcepNIC21V2" datasource="#Session.DSN#">
										select CFdescripcion as Cdescripcion, CFcuenta, CFformato as Cformato, Ccuenta
										from CFinanciera
										where CFformato = <cfqueryparam cfsqltype="cf_sql_varchar" value="#param4600.Pvalor#" null="#Len(Trim(param4600.Pvalor)) EQ 0#" >
									</cfquery>
									<cfif  isdefined('rsCtaExcepNIC21V2') and rsCtaExcepNIC21V2.RecordCount EQ 0 >
                                    		<cfquery  name="rsCtaExcepNIC21V2" datasource="#session.dsn#">
                                    			select distinct Cmayor , '#param4600.Pvalor#' as Cformato,CFdescripcion as Cdescripcion,Ccuenta
                                                from CFinanciera
                                                where Ecodigo = #session.Ecodigo#
                                                and Cmayor=substring('#param4600.Pvalor#',1,4)
                                                and CFformato = substring('#param4600.Pvalor#',1,4)
                                            </cfquery>
                                    </cfif>
									<cfif isdefined('rsCtaExcepNIC21V2') and rsCtaExcepNIC21V2.RecordCount GT 0>
										<cf_cuentasanexo
										conexion="#Session.DSN#"
										conlis="S"
										query="#rsCtaExcepNIC21V2#"
										auxiliares="N"
										movimiento="S"
										form="form1"
										frame="frame1"
										descwidth="50"
										Ccuenta="CCuentaExcepNIC21V2"
										Cformato="Cformato"
										Cmayor="MCExcepNIC21V2"
                                        comodin="?"
                                        >
									<cfelse>
										<cf_cuentasanexo
										conexion="#Session.DSN#"
										conlis="S"
										auxiliares="N"
										movimiento="S"
										form="form1"
										frame="frame1"
										descwidth="50"
										Ccuenta="CCuentaExcepNIC21V2"
										Cformato="Cformato"
										Cmayor="MCExcepNIC21V2"
                                        comodin="?"
										>
									</cfif>
								</td>
							</tr>
							<!----Activación de póliza complemetaria---->
								<cfset rsParam = ObtenerDato(11100801)>
	                           	<cfif rsParam.RecordCount GT 0>
									<cfset PolCom = rsParam.Pvalor>
	                            <cfelse>
	                            	<cfset PolCom = 0 >
								</cfif>
                            <tr>
								<td>&nbsp;</td>
								<td align="right" nowrap>Genera póliza complemetaria:&nbsp;</td>
								<td>
									<input type="hidden" name="chkPolCIn" value="<cfoutput>#PolCom#</cfoutput>">
									<input type="checkbox" 	name="chkPolCompl" value="1" <cfif PolCom eq 'S'>checked</cfif>>
								</td>
							</tr>
        </table>
	</div><!--- Fin CG --->


<!---- TRAMITES ---->
	<div class="TR">
		<table width="100%">
			<!---- TRAMITES ---->
			<tr>
				<td colspan="3" align="center" class="listaPar"><hr size="0"><b><font size="2">Trámites</font></b></td>
			</tr>
			<cfset rsWS_CFusuario = ObtenerDato(3500)>
			<cfif rsWS_CFusuario.RecordCount GT 0 >
				<cfset hayWS_CFusuario = 1 >
				<cfset WS_CFusuario = rsWS_CFusuario.Pvalor>
			<cfelse>
				<cfset hayWS_CFusuario = 0 >
				<cfset WS_CFusuario = '' >
			</cfif>
			<tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Determinacion de Centro Funcional por Usuario:&nbsp;</td>
				<td>
					<input type="hidden" name="hayWS_CFusuario" value="<cfoutput>#hayWS_CFusuario#</cfoutput>">
					<select name="WS_CFusuario">
						<option value="">Nombramientos de Recursos Humanos</option>
						<option value="USU" <cfif WS_CFusuario EQ "USU">selected</cfif>>Usuarios por Centro Funcional</option>
						<option value="EMP" <cfif WS_CFusuario EQ "EMP">selected</cfif>>Importacion de Empleados por Centro Funcional</option>
					</select>
				</td>
			</tr>
            <tr>
            	<td>&nbsp;</td>
            	<td align="right">
                	Utilizar Gestión de Autorizaciones:
                </td>
                <td align="left"><input type="checkbox" name="chkUtilAuto" value="1" <cfif LvarchkUtilAuto eq '1'>checked</cfif>>
            	</td>
            </tr>
		</table>
	</div>	<!--- Fin del CNTP --->


<!---- CONTROL DE PRESUPUESTO ---->
	<div class="PRES">
		<table width="100%">
			<!---- PRESUPUESTO ---->
			<tr>
				<td colspan="4" align="center" class="listaPar"><hr size="0"><b><font size="2">Control de Presupuesto</font></b>
				</td>
			</tr>
			<cfset rsTrasladosAbierto = ObtenerDato(549)>
			<cfif rsTrasladosAbierto.RecordCount GT 0 >
				<cfset hayTrasladosAbierto = 1 >
				<cfset TrasladosAbierto = rsTrasladosAbierto.Pvalor>
			<cfelse>
				<cfset hayTrasladosAbierto = 0 >
				<cfset TrasladosAbierto = 'N' >
			</cfif>
			<tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Permitir Cuentas con Control Abierto en Traslados de Presupuesto:&nbsp;</td>
				<td>
					<input type="hidden" name="hayTrasladosAbierto" value="<cfoutput>#hayTrasladosAbierto#</cfoutput>">
					<input type="checkbox" value="S" name="chkTrasladosAbierto" <cfif TrasladosAbierto eq 'S'>checked</cfif> >
				</td>
				<td>&nbsp;</td>
			</tr>

			<cfset rsCPresupuestoDeVersion = ObtenerDato(541)>
			<cfif rsCPresupuestoDeVersion.RecordCount GT 0 >
				<cfset hayCPresupuestoDeVersion = 1 >
				<cfset CPresupuestoDeVersion = rsCPresupuestoDeVersion.Pvalor>
			<cfelse>
				<cfset hayCPresupuestoDeVersion = 0 >
				<cfset CPresupuestoDeVersion = 'N' >
			</cfif>
			<tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Crear Cuentas de Presupuesto en la Versión de Formulación:&nbsp;</td>
				<td>
					<input type="hidden" name="hayCPresupuestoDeVersion" value="<cfoutput>#hayCPresupuestoDeVersion#</cfoutput>">
					<input type="checkbox" value="S" name="chkCPresupuestoDeVersion" <cfif CPresupuestoDeVersion eq 'S'>checked</cfif> >
				</td>
				<td>&nbsp;</td>
			</tr>

        	<!----1120. Permitir Cuentas con Tipo de Control Restringido y aprobación de NRPs---->
			<cfset rsCTCRestringigo = ObtenerDato(1120)>
			<cfif rsCTCRestringigo.RecordCount GT 0 >
				<cfset hayCTCRestringigo = 1 >
				<cfset CTCRestringigo = rsCTCRestringigo.Pvalor>
			<cfelse>
				<cfset hayCTCRestringigo = 0 >
				<cfset CTCRestringigo = 'N' >
			</cfif>

			<tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Permitir Cuentas con Tipo de Control Restringido y aprobación de NRPs:&nbsp;</td>
				<td>
					<input type="hidden" name="hayCTCRestringigo" value="<cfoutput>#hayCTCRestringigo#</cfoutput>">
					<input type="checkbox" value=" " name="chkCTCRestringigo" <cfif CTCRestringigo eq 'S'>checked</cfif> >
				</td>
				<td>&nbsp;</td>
			</tr>

        	<!----1121. Permitir Formulaciones negativas---->
			<cfset rsCPFnegativo = ObtenerDato(1121)>
			<cfif rsCPFnegativo.RecordCount GT 0 >
				<cfset hayCPFnegativo = 1 >
				<cfset CPFnegativo = rsCPFnegativo.Pvalor>
			<cfelse>
				<cfset hayCPFnegativo = 0 >
				<cfset CPFnegativo = 'N' >
			</cfif>

			<tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Permitir Aprobacion de Versiones con Formulaciones Negativas:&nbsp;</td>
				<td>
					<input type="hidden" name="hayCPFnegativo" value="<cfoutput>#hayCPFnegativo#</cfoutput>">
					<input type="checkbox" value=" " name="chkCPFnegativo" <cfif CPFnegativo eq ' '>checked</cfif> >
				</td>
				<td>&nbsp;</td>
			</tr>

        	<!----1130. Control Especial de Presupuesto para Asientos Retroactivos en Períodos Contables ya Cerrados---->
			<cfset rsCPcontrolEspecial = ObtenerDato(1130)>
			<cfif rsCPcontrolEspecial.RecordCount GT 0>
				<cfset hayCPcontrolEspecial = 1>
				<cfset CPcontrolEspecial = rsCPcontrolEspecial.Pvalor>
			<cfelse>
				<cfset hayCPcontrolEspecial = 0 >
				<cfset CPcontrolEspecial = '0' >
			</cfif>

			<tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Control Especial de Presupuesto para Asientos Retroactivos en el Período Contable
					inmediato Anterior:&nbsp;
				</td>
				<td>
					<input type="hidden" name="hayCPcontrolEspecial" value="<cfoutput>#hayCPcontrolEspecial#</cfoutput>">
					<select name="cboCPcontrolEspecial">
						<option value="0" <cfif CPcontrolEspecial eq '0'>selected</cfif>>Control Normal de Presupuesto</option>
						<option value="1" <cfif CPcontrolEspecial eq '1'>selected</cfif>>No Controlar Presupuesto (genera NAP = 0)</option>
						<option value="2" <cfif CPcontrolEspecial eq '2'>selected</cfif>>Activar Origen con Control Abierto (generación de Excesos Autorizados)</option>
						<option value="3" <cfif CPcontrolEspecial eq '3'>selected</cfif>>Activar Control Abierto a todas las Cuentas (permite disponibles negativos)</option>
					</select>
				</td>
				<td>&nbsp;</td>
			</tr>

            <!---- Genera contabilidad presupuestaria solo para México ---->
			<cfset rsGCONpresupuestaria = ObtenerDato(1140)>
			<cfif rsGCONpresupuestaria.RecordCount GT 0 >
				<cfset hayGCONpresupuestaria = 1 >
				<cfset GCONpresupuestaria = rsGCONpresupuestaria.Pvalor>
			<cfelse>
				<cfset hayGCONpresupuestaria = 0 >
				<cfset GCONpresupuestaria = 'N' >
			</cfif>
			<tr>
				<td>&nbsp;</td>
				<td align="right" nowrap>Generar Contabilidad Presupuestaria (Contabilidad Gubernamental MX):&nbsp;</td>
				<td>
					<input type="hidden" name="hayGCONpresupuestaria" value="<cfoutput>#hayGCONpresupuestaria#</cfoutput>">
					<input type="checkbox" value=" " name="chkGCONpresupuestaria" <cfif GCONpresupuestaria eq 'S'>checked</cfif> >
				</td>
				<td>&nbsp;</td>
			</tr>
			<tr>
				<!------Parametro 1370 Complemento del Socio por Default---->
				<cfset CompleDefaultSN= ObtenerDato(1370)>
				<td>&nbsp;</td>
				<td align="right">Complemento Default para el Socio de Negocio:&nbsp;
				</td>
				<td>
					<input type="text" name="ComplemSN" size="12" maxlength="100"
						value="<cfoutput>#CompleDefaultSN.Pvalor#</cfoutput>">
				</td>
				<td>&nbsp;</td>
			</tr>

			<tr>
				<!----Parametro 1380 CLASIFICADOR DEL GASTO PARA CUENTAS DE ORDEN---->
				<cfset ClasGasto= ObtenerDato(1380)>
				<cfif ClasGasto.Pvalor NEQ ''>
					<cfset PCEcatid = ClasGasto.Pvalor>
				<cfelse>
					<cfset PCEcatid = 0>
				</cfif>
				<td>&nbsp;</td>
				<td align="right">Clasificador de Gasto para Cuentas de Orden:&nbsp;</td>
				<td> <cf_conlis
						Campos="PCEcatid, PCEcodigo, PCEdescripcion"
						Desplegables="N,S,S"
						Modificables="N,S,N"
						Size="0,10,40"
						tabindex="2"
						Title="Catálogo al Plan de Cuentas"
					    Tabla= "PCECatalogo"
						Columnas="PCEcatid, PCEcodigo, PCEdescripcion"
						Filtro=""
						Desplegar="PCEcodigo, PCEdescripcion"
					    Etiquetas="Codigo, Descripción"
						filtrar_por="PCEcodigo, PCEdescripcion"
						Formatos="S,S"
						Align="left,left"
						form="form1"
						Asignar="PCEcatid, PCEcodigo, PCEdescripcion"
						Asignarformatos="S,S,S"
						traerinicial="true"
						traerfiltro="PCEcatid = #PCEcatid#"
						index="2"/>

				</td>
				<td  align="center"><cf_botones values="Filtrar" names="Filtrar" tabindex="2"></td>
			</tr>

			<tr>
				<!------Parametro 13600201 Catalogo del proyecto---->
				<cfset CatProyecto= ObtenerDato(13600201)>
				<cfif CatProyecto.RecordCount and CatProyecto.Pvalor NEQ ''>
					<cfset PCEcatidP = CatProyecto.Pvalor>
				<cfelse>
					<cfset PCEcatidP = 0>
				</cfif>
				<td>&nbsp;</td>
				<td align="right">Clasificador de Proyecto:&nbsp;</td>
				<td> <cf_conlis
						Campos="PCEcatidP, PCEcodigoP, PCEdescripcionP"
						Desplegables="N,S,S"
						Modificables="N,S,N"
						Size="0,10,40"
						tabindex="2"
						Title="Catálogo al Plan de Cuentas"
						Tabla= "PCECatalogo"
						Columnas="PCEcatid as PCEcatidP, PCEcodigo as PCEcodigoP, PCEdescripcion as PCEdescripcionP"
						Filtro=""
						Desplegar="PCEcodigoP, PCEdescripcionP"
						Etiquetas="Codigo, Descripción"
						filtrar_por="PCEcodigo, PCEdescripcion"
						Formatos="S,S"
						Align="left,left"
						form="form1"
						objForm ="objFormProy"
						Asignar="PCEcatidP, PCEcodigoP, PCEdescripcionP"
						Asignarformatos="S,S,S"
						traerinicial="true"
						traerfiltro="PCEcatid = #PCEcatidP#"
						index="2"/>
					</td>
					<td  align="center"></td>
			</tr>

			<!---- Inicia:Permitir Formulación para cuentas de control abiertas negativas para meses anteriores---->
            <!----Cambio Realizado  Por: ***RVD, APH*** Fecha: 12/06/2013---->
				<cfset AbiertoNegativo = ObtenerDato(542)>
				<cfif AbiertoNegativo.RecordCount GT 0 >
					<cfset aceptaNeg = 1 >
					<cfset aceptaNegVal = AbiertoNegativo.Pvalor>
				<cfelse>
					<cfset aceptaNeg = 0 >
					<cfset aceptaNegVal = 'N' >
				</cfif>
				<tr>
					<td>&nbsp;</td>
					<td align="right" nowrap>Permitir Formulación Para Cuentas de Control Abierto Negativas para Meses Anteriores:&nbsp;
					</td>
					<td>
						<input type="hidden" name="aceptaNeg" value="<cfoutput>#aceptaNeg#</cfoutput>">
						<input type="checkbox" value=" " name="chkCtasAbierNegat" <cfif aceptaNegVal eq 'S'>checked</cfif> >
					</td>
					<td>&nbsp;</td>
				</tr>
			<!---- Termina: Permitir Formulación para cuentas de control abiertas negativas para meses anteriores---->

				<!---- Inicia:Validar Compromiso por cuenta presupuestal---->
				<cfset rsValidaXCP = ObtenerDato(1131)>
				<cfif rsValidaXCP.RecordCount GT 0 >
					<cfset validaXCP = 1 >
					<cfset validaXCPVal = rsValidaXCP.Pvalor>
				<cfelse>
					<cfset validaXCP = 0 >
					<cfset validaXCPVal = 'N' >
				</cfif>
				<tr>
					<td>&nbsp;</td>
					<td align="right" nowrap>Validar Compromiso por Agrupación de Cuentas Presupuestales:&nbsp;</td>
					<td>
						<input type="hidden" name="validaXCP" value="<cfoutput>#validaXCP#</cfoutput>">
						<input type="checkbox" value=" " name="chkValidaXCPVal" <cfif validaXCPVal eq 'S'>checked</cfif> >
					</td>
					<td>&nbsp;</td>
				</tr>
				<!---- Megs: Se agregan nuevo parámetros para Presupuesto ---->
				<!---- Fecha: 10/08/2022 ---->
				<!---- INICIA: 13600301: Cuenta de Diferencial Cambiario para Multimoneda Ingreso---->
				<tr>
					<td>&nbsp;</td>
					<td align="right" nowrap>Cuenta de Diferencial Cambiario para Multimoneda Ingreso:&nbsp;</td>

					<!---- Valida existencia del parámetro ---->
					<cfset param13600301 = ObtenerDato(13600301)>
					<cfif param13600301.RecordCount GT 0 >
						<cfset hayCuentaDifCamMultiIngreso = 1 >
					</cfif>
					<!---- CONLIS CUENTAS ---->
					<cfquery name="rsCuentas_Param13600301" datasource="#Session.DSN#">
						select CFdescripcion, CFcuenta, CFformato, Ccuenta
						from CFinanciera
						where CFcuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#param13600301.Pvalor#" null="#Len(Trim(param13600301.Pvalor)) EQ 0#" >
					</cfquery>
					<td>
						<cfif isdefined('rsCuentas_Param13600301') and rsCuentas_Param13600301.RecordCount GT 0 and rsCuentas_Param13600301.CFcuenta NEQ ''>
							<cf_cuentas conexion="#Session.DSN#" conlis="S" auxiliares="N" movimiento="S" form="form1" frame="frame1" descwidth="50" Ccuenta="CCuentaMultimonIngreso" Cformato="FCMultimonIngreso" Cmayor="MCMultimonIngreso" query="#rsCuentas_Param13600301#">
						<cfelse>
							<cf_cuentas conexion="#Session.DSN#" conlis="S" auxiliares="N" movimiento="S" form="form1" frame="frame1" descwidth="50" Ccuenta="CCuentaMultimonIngreso" Cformato="FCMultimonIngreso" Cmayor="MCMultimonIngreso">
						</cfif>
					</td>
				</tr>		
				<!---- TERMINA: 13600301: Cuenta de Diferencial Cambiario para Multimoneda Ingreso---->
				<tr>

					<cfquery name="hayIdent" datasource="#Session.dsn#">
						Select Pvalor from Parametros
						where Pcodigo = 544
						and Ecodigo = #Session.Ecodigo#
					</cfquery>
					<cfset rsIdentificador = hayIdent.Pvalor>
					<cfif rsIdentificador eq '1'>
						<cfset hayIdentificador = 1 >
					<cfelse>
						<cfset hayIdentificador = 0 >
					</cfif>
					<td>&nbsp;</td>
					<td align="right" nowrap>Habilitar Edición del Identificador del Proyecto:&nbsp;</td>
					<td>
						<input type="hidden" name="hayIdentificador" value="<cfoutput>#hayIdentificador#</cfoutput>">
						<input type="checkbox" value=" " name="chkIdentificador" <cfif hayIdentificador eq '1'>checked</cfif> >
					</td>
					<td>&nbsp;</td>
				</tr>		


				<!---- INICIA: 13600101: Cuenta de Diferencial Cambiario para Multimoneda Egreso---->
				<tr>
					<td>&nbsp;</td>
					<td align="right" nowrap>Cuenta de Diferencial Cambiario para Multimoneda Egreso:&nbsp;</td>

					<!---- Valida existencia del parámetro ---->
					<cfset param13600101 = ObtenerDato(13600101)>
					<cfif param13600101.RecordCount GT 0 >
						<cfset hayCuentaDifCamMultiEgreso = 1 >
					</cfif>

					<!---- CONLIS CUENTAS ---->
					<cfquery name="rsCuentas_Param13600101" datasource="#Session.DSN#">
						select CFdescripcion, CFcuenta, CFformato, Ccuenta
						from CFinanciera
						where CFcuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#param13600101.Pvalor#" null="#Len(Trim(param13600101.Pvalor)) EQ 0#" >
					</cfquery>
					<td>
						<cfif isdefined('rsCuentas_Param13600101') and rsCuentas_Param13600101.RecordCount GT 0 and rsCuentas_Param13600101.CFcuenta NEQ ''>
							<cf_cuentas conexion="#Session.DSN#" conlis="S" auxiliares="N" movimiento="S" form="form1" frame="frame1" descwidth="50" Ccuenta="CCuentaMultimonEgreso" Cformato="FCMultimonEgreso" Cmayor="MCMultimonEgreso" query="#rsCuentas_Param13600101#">
						<cfelse>
							<cf_cuentas conexion="#Session.DSN#" conlis="S" auxiliares="N" movimiento="S" form="form1" frame="frame1" descwidth="50" Ccuenta="CCuentaMultimonEgreso" Cformato="FCMultimonEgreso" Cmayor="MCMultimonEgreso">
						</cfif>
					</td>
				</tr>				
				<!---- TERMINA: 13600101: Cuenta de Diferencial Cambiario para Multimoneda Egreso---->

				<!---- INICIA: 13600202: Cuenta de Diferencial Cambiario para Multimoneda Costo---->
				<tr>
					<td>&nbsp;</td>
					<td align="right" nowrap>Cuenta de Diferencial Cambiario para Multimoneda Costo:&nbsp;</td>

					<!---- Valida existencia del parámetro ---->
					<cfset param13600202 = ObtenerDato(13600202)>
					<cfif param13600202.RecordCount GT 0 >
						<cfset hayCuentaDifCamMultiCosto = 1 >
					</cfif>


					<!---- CONLIS CUENTAS ---->
					<cfquery name="rsCuentas_Param13600202" datasource="#Session.DSN#">
						select CFdescripcion, CFcuenta, CFformato, Ccuenta
						from CFinanciera
						where CFcuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#param13600202.Pvalor#" null="#Len(Trim(param13600202.Pvalor)) EQ 0#" >
					</cfquery>
					<td>
						<cfif isdefined('rsCuentas_Param13600202') and rsCuentas_Param13600202.RecordCount GT 0 and rsCuentas_Param13600202.CFcuenta NEQ ''>
							<cf_cuentas conexion="#Session.DSN#" conlis="S" auxiliares="N" movimiento="S" form="form1" frame="frame1" descwidth="50" Ccuenta="CCuentaMultimonCosto" Cformato="FCMultimonCosto" Cmayor="MCMultimonCosto" query="#rsCuentas_Param13600202#">
						<cfelse>
							<cf_cuentas conexion="#Session.DSN#" conlis="S" auxiliares="N" movimiento="S" form="form1" frame="frame1" descwidth="50" Ccuenta="CCuentaMultimonCosto" Cformato="FCMultimonCosto" Cmayor="MCMultimonCosto">
						</cfif>
					</td>
				</tr>				
				<!---- TERMINA: 13600202: Cuenta de Diferencial Cambiario para Multimoneda Costo---->
		</table>
	</div><!--- Fin pres --->


	<!---- CONTROL EVENTO ---->
	<div class="CEVENTO">
		<table width="100%">
			<!----Control de Evento Inicia---->
			<!---- CONTROL EVENTO ---->
			<tr>
				<td colspan="2" align="center" class="listaPar"><hr size="0"><b><font size="2">Control de Evento</font></b></td>
			</tr>
			<cfset rsControlEvento = ObtenerDato(1350)>
	        <cfif rsControlEvento.RecordCount GT 0 >
	            <cfset hayControlEvento = 1 >
	            <cfset ControlEvento = rsControlEvento.Pvalor>
	        <cfelse>
	            <cfset hayControlEvento = 0 >
	            <cfset ControlEvento = 'N' >
	        </cfif>

	        <tr>
				<td align="center" nowrap>Habilitar Control de Evento:&nbsp;
					<input type="checkbox" value="S" name="chkControlEvento" <cfif ControlEvento eq 'S'>checked</cfif> >
					<input type="hidden" name="hayControlEvento" value="<cfoutput>#hayControlEvento#</cfoutput>">
				</td>
				<td>&nbsp;</td>
			</tr>
			<!----Control de Evento Fin---->
		</table>
	</div><!--- CEVENTO --->


	<!--- Vencimientos --->
	<div class="GN">
		<table width="100%">
			<tr>
				<td colspan="4" align="center" class="listaPar"><hr size="0"><b><font size="2">Vencimientos</font></b></td>
			</tr>
			<tr>
				<cfset venc = ObtenerDato(310)>
				<cfif venc.RecordCount GT 0 >
					<cfset hayVencimiento1 = 1 >
					<cfset vencimiento = venc.Pvalor>
				</cfif>
				<td align="right" nowrap >1&ordm; Vencimiento:&nbsp;</td>
				<td>
					<input name="vencimiento1" alt="1º vencimiento" style="text-align:right"
						onkeyup="if(snumber(this,event,0)){ if(Key(event)=='13') {this.blur();}}"
						onFocus="javascript:this.select();" onChange="javascript:fm(this,0);" type="text"
						value="<cfoutput>#vencimiento#</cfoutput>" size="4" maxlength="4">
				</td>

				<cfset venc = ObtenerDato(320)>
				<cfif venc.RecordCount GT 0 >
					<cfset hayVencimiento2 = 1 >
					<cfset vencimiento = venc.Pvalor>
				</cfif>
				<td align="left" nowrap >&nbsp;2&ordm; Vencimiento:&nbsp;
					<input name="vencimiento2" alt="2º vencimiento" style="text-align:right"
						onkeyup="if(snumber(this,event,0)){ if(Key(event)=='13') {this.blur();}}"
						onFocus="javascript:this.select();" onChange="javascript:fm(this,0);" type="text"
						value="<cfoutput>#vencimiento#</cfoutput>" size="4" maxlength="4">
				</td>
				<td>&nbsp;</td>
			</tr>
			<tr>
				<cfset venc = ObtenerDato(330)>
				<cfif venc.RecordCount GT 0 >
					<cfset hayVencimiento3 = 1 >
					<cfset vencimiento = venc.Pvalor>
				</cfif>
				<td align="right" nowrap >3&ordm; Vencimiento:&nbsp;</td>
				<td>
					<input name="vencimiento3" alt="3º vencimiento" style="text-align:right"
						onkeyup="if(snumber(this,event,0)){ if(Key(event)=='13') {this.blur();}}"
						onFocus="javascript:this.select();" onChange="javascript:fm(this,0);"  type="text"
						value="<cfoutput>#vencimiento#</cfoutput>" size="4" maxlength="4">
				</td>

				<cfset venc = ObtenerDato(340)>
				<cfif venc.RecordCount GT 0 >
					<cfset hayVencimiento4 = 1 >
					<cfset vencimiento = venc.Pvalor>
				</cfif>
				<td align="left" nowrap >&nbsp;4&ordm; Vencimiento:&nbsp;
					<input name="vencimiento4" alt="4º vencimiento" style="text-align:right"
					onkeyup="if(snumber(this,event,0)){ if(Key(event)=='13') {this.blur();}}"
					onFocus="javascript:this.select();" onChange="javascript:fm(this,0);"  type="text"
					value="<cfoutput>#vencimiento#</cfoutput>" size="4" maxlength="4">
				</td>
				<td>&nbsp;</td>
			</tr>
		</table>
	</div>


	<!---- COMPRAS ---->
	<div class="CM">
		<table width="100%">
			<!---- COMPRAS ---->
			<tr><td colspan="3" align="center" class="listaPar"><hr size="0"><b><font size="2">Compras</font></b></td>
			</tr>

			<cfset rsNomina = ObtenerDato(520)>
			<cfif rsNomina.RecordCount GT 0 >
				<cfset hayNomina = 1 >
				<cfset interfazNomina = rsNomina.Pvalor>
			<cfelse>
				<cfset interfazNomina = 'N' >
			</cfif>
			<tr>
				<td align="right" nowrap>Integraci&oacute;n con Recursos Humanos:&nbsp;</td>
				<td><input type="checkbox" name="chkNomina" <cfif interfazNomina eq 'S'>checked</cfif> ></td>
			</tr>

			<cfset aprobar = ObtenerDato(410)>
			<cfif aprobar.RecordCount GT 0 >
				<cfset hayAutorizacion = 1 >
				<cfset aprobacion = aprobar.Pvalor>
			<cfelse>
				<cfset aprobacion = '1' >
			</cfif>
			<tr>
				<td align="right" nowrap>Aprobar Solicitudes de Compra:&nbsp;</td>
				<td><input type="checkbox" name="chkAutorizar" <cfif aprobacion eq '1'>checked</cfif> ></td>
			</tr>

			<cfset publicar = ObtenerDato(570)>
			<cfset hayPublicacion = 0 >
			<cfif publicar.RecordCount GT 0 >
				<cfset hayPublicacion = 1 >
				<cfset publicacion = publicar.Pvalor>
			<cfelse>
				<cfset publicacion = '1' >
			</cfif>
			<tr>
				<td align="right" nowrap>Publicar Procesos de Compra:&nbsp;</td>
				<td><input type="checkbox" name="chkPublicar" <cfif publicacion eq '1'>checked</cfif> ></td>
			</tr>

			<cfset rsJerarquia = ObtenerDato(580)>
			<cfset hayJerarquia = 0 >
			<cfif rsJerarquia.RecordCount GT 0 >
				<cfset hayJerarquia = 1 >
				<cfset jerarquia = rsJerarquia.Pvalor>
			<cfelse>
				<cfset jerarquia = '0' >
			</cfif>
			<tr>
				<td align="right" nowrap>Utiliza jerarqu&iacute;a para aprobar:&nbsp;</td>
				<td><input type="checkbox" name="chkJerarquia" <cfif jerarquia eq '1'>checked</cfif> ></td>
			</tr>

			<!---- Parametro para validar para compras directas bienes en contratos ---->
			<cfset rsValidarCDC = ObtenerDato(710)>
			<cfset hayValidarCDC = 0 >
			<cfif rsValidarCDC.RecordCount GT 0 >
				<cfset hayValidarCDC = 1 >
				<cfset validarCDC = rsValidarCDC.Pvalor>
			<cfelse>
				<cfset validarCDC = '0' >
			</cfif>
			<tr>
				<td align="right" nowrap>Validar para compras directas bienes en contratos:&nbsp;</td>
				<td><input type="checkbox" name="chkValidarCDC" <cfif validarCDC eq '1'>checked</cfif> ></td>
				</tr>
				<!--- Parametro para Utilizar Trámites para contratos de compra --->
				<cfset rsTramitesContratos = ObtenerDato(4320)>
				<cfset hayTramitesContratos = 0 >
				<cfif rsTramitesContratos.RecordCount GT 0 >
					<cfset hayTramitesContratos = 1 >
					<cfset TramitesContratos = rsTramitesContratos.Pvalor>
				<cfelse>
					<cfset TramitesContratos = '0' >
				</cfif>
				<tr>
					<td align="right" nowrap>Utilizar Trámites para contratos de compra:&nbsp;</td>
					<td><input type="checkbox" name="chkTramitesContratos" <cfif TramitesContratos eq '1'>checked</cfif> ></td>
			</tr>

			<cfset nivelA = ObtenerDato(530)>
			<input type="hidden" name="hayNivelA" value="<cfif nivelA.RecordCount gt 0 and len(trim(nivelA.Pvalor))>1<cfelse>0</cfif>">
			<tr>
				<td nowrap align="right">Nivel m&aacute;ximo de Clasificaciones de Art&iacute;culos:&nbsp;</td>
				<td><input name="MaxNivelA" style="text-align:right" onkeyup="if(snumber(this,event,0)){ if(Key(event)=='13') {this.blur();}}" onFocus="javascript:this.select();" onChange="javascript:fm(this,0);" type="text" value="<cfif nivelA.RecordCount gt 0 and len(trim(nivelA.Pvalor))><cfoutput>#nivelA.Pvalor#</cfoutput><cfelse>1</cfif>" size="4" maxlength="4"></td>
			</tr>

			<cfset nivelS = ObtenerDato(540)>
			<input type="hidden" name="hayNivelS" value="<cfif nivelS.RecordCount gt 0 and len(trim(nivelS.Pvalor))>1<cfelse>0</cfif>">
			<tr>
				<td nowrap align="right">Nivel m&aacute;ximo de Clasificaciones de Conceptos:&nbsp;</td>
				<td><input name="MaxNivelS" style="text-align:right" onkeyup="if(snumber(this,event,0)){ if(Key(event)=='13') {this.blur();}}" onFocus="javascript:this.select();" onChange="javascript:fm(this,0);" type="text" value="<cfif nivelS.RecordCount gt 0 and len(trim(nivelS.Pvalor))><cfoutput>#nivelS.Pvalor#</cfoutput><cfelse>1</cfif>" size="4" maxlength="4"></td>
			</tr>

			<!---- ---->
			<cfset CantPPC = ObtenerDato(535)>
			<input type="hidden" name="hayCantPPC" value="<cfif CantPPC.RecordCount gt 0 and len(trim(CantPPC.Pvalor))>1<cfelse>0</cfif>">
			<tr>
				<td nowrap align="right">Cantidad de Proveedores a Invitar en el Proceso de Compra:&nbsp;</td>
				<td><input name="MaxCantPPC" style="text-align:right" onkeyup="if(snumber(this,event,0)){ if(Key(event)=='13') {this.blur();}}" onFocus="javascript:this.select();" onChange="javascript:fm(this,0);" type="text" value="<cfif CantPPC.RecordCount gt 0 and len(trim(CantPPC.Pvalor))><cfoutput>#CantPPC.Pvalor#</cfoutput><cfelse>1</cfif>" size="4" maxlength="4"></td>
			</tr>

			<cfset ConsReq = ObtenerDato(361)>
			<cfif ConsReq.RecordCount GT 0 >
				<cfset hayConsReq = 1 >
				<cfset valorConsReq = ConsReq.Pvalor>
			<cfelse>
				<cfset valorConsReq = '0' >
			</cfif>
			<tr>
				<td align="right" nowrap>Consecutivos separados para las requisiciones:&nbsp;</td>
				<td><input type="checkbox" name="chkConsReq" <cfif valorConsReq eq '1'>checked</cfif> ></td>
			</tr>
            <!----►►5310-Empresa en el codigo de la Requisicion◄◄---->
            <tr>
                <td width="39%" align="right" nowrap>Agregar Identificación Representante de la empresa a las requisiciones cuando se usa Proveeduría Corporativa:&nbsp;</td>
                <td>
                    <cfset chkEmpresaRequis = ObtenerDato(5310).Pvalor>
                    <input type="checkbox" value="1" name="chkEmpresaRequis"<cfif trim(chkEmpresaRequis) eq '1'>checked</cfif>>
                    <cf_notas titulo="Agregar Empresa a Requisiciones" link="#helpimg#" pageIndex="4" msg = "<UL type = circle><LI>Cuando se usa Proveeduría Corporativa, se genera el documento de la requisición: Solicitud Compra+Almacén+Empresa<br><br><LI>Si se usa Consecutivos separados para las requisiciones, este parametro se omite</UL>" animar="true">
                </td>
			</tr>

			<tr>
				<cfset req = ObtenerDato(360)>
				<cfif req.RecordCount GT 0 >
					<cfset hayTipoRequisicion = 1 >
					<cfset trequisicion = req.Pvalor>
				</cfif>
				<td width="39%" align="right" nowrap>Requisici&oacute;n Default:&nbsp;</td>

				<td width="35%">
					<select name="TipoRequisicion">
						<cfif rsTipoRequisicion.RecordCount EQ 0>
						<option value="-1"></option>
						</cfif>
						<cfoutput query="rsTipoRequisicion">
						<option value="#rsTipoRequisicion.TRcodigo#" <cfif trim(rsTipoRequisicion.TRcodigo) eq trim(trequisicion)> selected </cfif> >#rsTipoRequisicion.TRdescripcion#</option>
						</cfoutput>
					</select>
				</td>
				<td CNTPspan="4" valign="middle">
				<!----
				<table width="100%" class="ayuda">
				<tr><td>Poner Ayuda</td></tr>
				</table>
				---->
				</td>
			</tr>

			<tr>
				<cfset solic = ObtenerDato(370)>
				<cfif solic.RecordCount GT 0 >
					<cfset haySolicitante = 1 >
					<cfset solicitante = solic.Pvalor>
					<cfif Len(Trim(solicitante)) GT 0>
						<cfquery name="rsSolicitante" datasource="#Session.DSN#">
							select CMSnombre from CMSolicitantes
							where Ecodigo = #Session.Ecodigo#
							and CMSid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#solicitante#">
						</cfquery>
					</cfif>
				</cfif>
				<td align="right" nowrap>Solicitante Default:&nbsp;</td>
				<td nowrap>
					<input name="CMSnombre" type="text" value="<cfif Len(Trim(solicitante)) GT 0><cfoutput>#rsSolicitante.CMSnombre#</cfoutput></cfif>" id="CMSnombre" size="40" maxlength="80" readonly tabindex="-1">
					<a href="#" tabindex="-1"><img src="../../imagenes/Description.gif" alt="Lista de Solicitantes" name="imagen" width="18" height="14" border="0" align="absmiddle" onClick="javascript:doSolicitantes();"></a>
					<input name="CMSid" type="hidden" id="CMSid" value="<cfoutput>#solicitante#</cfoutput>">
				</td>
			</tr>

			<cfset cantDig = ''>
			<cfset cantDig = ObtenerDato(720)>
			<cfif cantDig.RecordCount GT 0 >
				<cfset cantDig = cantDig.Pvalor>
			<cfelse>
				<cfset cantDig = '' >
			</cfif>

			<tr nowrap>
				<td align="right" nowrap>Cantidad de d&iacute;gitos en n&uacute;mero de Documentos de Recepci&oacute;n:&nbsp;</td>
				<td>
					<table width="100%"  border="0">
					  <tr>
						<td width="8%">
							<input type="checkbox" <cfif isdefined('cantDig') and cantDig NEQ ''> checked</cfif> name="chkRCDND" onClick="javascript: cantDig(this);">
						</td>
						<td width="92%">
							<div id="divCantDig" style="display:none">
								Cantidad de D&iacute;gitos: &nbsp;&nbsp;

								<input name="RCDND" style="text-align:right"
									onkeyup="if(snumber(this,event,0)){ if(Key(event)=='13') {this.blur();}}"
									onFocus="javascript:this.select();" onChange="javascript:fm(this,0);"
									type="text" value="<cfoutput>#cantDig#</cfoutput>"
									size="4" maxlength="4">
							</div>
						</td>
					  </tr>
					</table>
				</td>
			</tr>

			<cfset MultCont = ''>
			<cfset MultCont = ObtenerDato(730)>
			<cfif MultCont.RecordCount GT 0 >
				<cfset MultCont = MultCont.Pvalor>
			<cfelse>
				<cfset MultCont = '' >
			</cfif>
			<tr nowrap>
				<td align="right" nowrap>M&uacute;tiples contratos simult&aacute;neamente para art&iacute;culos o servicios</td>
				<td>
					<table width="100%"  border="0">
					  <tr>
						<td width="8%">
							<input value="1" type="checkbox" <cfif isdefined('MultCont') and MultCont NEQ ''> checked</cfif> name="chkmultiples">
						</td>
					 </tr>
					</table>
				</td>
			</tr>

			<!---- Parámetro que habilita la Aprobación de Excesos de Tolerancia para Documentos de Recepción ---->
			<cfset rsAprobarTolerancia = ObtenerDato(760)>
			<cfset hayAprobarTolerancia = 0>
			<cfif rsAprobarTolerancia.RecordCount GT 0>
				<cfset hayAprobarTolerancia = 1>
				<cfset aprobarTolerancia = rsAprobarTolerancia.Pvalor>
			<cfelse>
				<cfset aprobarTolerancia = '0'>
			</cfif>
			<tr>
				<td align="right" nowrap>Aprobación de Excesos de Tolerancia para Documentos de Recepción:&nbsp;</td>
				<td><input type="checkbox" name="chkAprobarTolerancia" <cfif aprobarTolerancia eq '1'>checked</cfif>></td>
			</tr>
            <!---- Parámetro que habilita la validacion para que no se permitan excesos en los Documentos de recepcion ---->
			<cfset rsExcesos = ObtenerDato(15651)>
			<cfset hayValidacionExcesos = 0>
			<cfif rsExcesos.RecordCount GT 0>
				<cfset hayValidacionExcesos = 1>
				<cfset HabilitaValidacion = rsExcesos.Pvalor>
			<cfelse>
				<cfset HabilitaValidacion = '0'>
			</cfif>
			<tr>
				<td align="right" nowrap>No permite excesos para los Documentos de Recepci&oacute;n:&nbsp;</td>
				<td><input type="checkbox" name="chkNoExcesos" <cfif HabilitaValidacion eq '1'>checked</cfif>></td>
			</tr>

			<!---- Parámetro que habilita la Funcionalidad de Agrupación de Ordenes de Compra ---->
			<cfset rsAgrupacionOrdenes = ObtenerDato(770)>
			<cfset hayAgrupacionOrdenes = 0>
			<cfif rsAgrupacionOrdenes.RecordCount GT 0>
				<cfset hayAgrupacionOrdenes = 1>
				<cfset AgrupacionOrdenes = rsAgrupacionOrdenes.Pvalor>
			<cfelse>
				<cfset AgrupacionOrdenes = '0'>
			</cfif>
			<tr>
				<td align="right" nowrap>Habilitar Funcionalidad de Agrupación de Ordenes de Compra:&nbsp;</td>
				<td><input type="checkbox" name="chkAgrupacionOrdenes" onClick="javascript: cambiaAgrupacionOrdenes();" <cfif AgrupacionOrdenes eq '1'>checked</cfif>></td>
			</tr>

			<!---- Parámetros de Frecuencia de Ejecución del Proceso de Agrupación de Ordenes de Compra ---->

			<!---- Parámetro principal de frecuencia de ejecución del proceso
				  de agrupación (diariamente, semanalmente, mensualmente) ---->
			<cfset rsFrecAgrupOC = ObtenerDato(780)>
			<cfset hayFrecAgrupOC = 0>
			<cfif rsFrecAgrupOC.RecordCount GT 0>
				<cfset hayFrecAgrupOC = 1>
				<cfset FrecAgrupOC = rsFrecAgrupOC.Pvalor>
			<cfelse>
				<cfset FrecAgrupOC = 'D'>
			</cfif>

			<tr id="trFrecAgrupOC" style="display:<cfif AgrupacionOrdenes eq '0'>none</cfif>">
				<td align="right" nowrap>Ejecutar Proceso de Agrupación:&nbsp;</td>
				<td>
					<select name="selFrecAgrupOC" onChange="javascript: cambiaFrecAgrupOC(this.value);">
						<option value="D" <cfif FrecAgrupOC eq 'D'>selected</cfif>>Diariamente</option>
						<option value="S" <cfif FrecAgrupOC eq 'S'>selected</cfif>>Semanalmente</option>
						<option value="M" <cfif FrecAgrupOC eq 'M'>selected</cfif>>Mensualmente</option>
					</select>
				</td>
			</tr>

			<!---- Parámetro de días de la semana que se va a realizar el proceso de agrupación
				  (usado cuando el parámetro de frecuencia sea semanalmente) ---->
			<cfset rsDiasSemanaAgrupOC = ObtenerDato(790)>
			<cfset hayDiasSemanaAgrupOC = 0>
			<cfif rsDiasSemanaAgrupOC.RecordCount GT 0>
				<cfset hayDiasSemanaAgrupOC = 1>
				<cfset DiasSemanaAgrupOC = rsDiasSemanaAgrupOC.Pvalor>
			<cfelse>
				<cfset DiasSemanaAgrupOC = '0000000'>
			</cfif>

			<tr id="trDiasSemanaAgrupOC" style="display:<cfif AgrupacionOrdenes eq '0' or FrecAgrupOC neq 'S'>none</cfif>">
				<td align="right" nowrap>Los días:&nbsp;</td>
				<td>
					<table>
						<tr>
							<td>Domingo</td>
							<td><input type="checkbox" name="chkAgruparD" <cfif Mid(DiasSemanaAgrupOC, 1, 1) eq '1'>checked</cfif>></td>
							<td>Lunes</td>
							<td><input type="checkbox" name="chkAgruparL" <cfif Mid(DiasSemanaAgrupOC, 2, 1) eq '1'>checked</cfif>></td>
							<td>Martes</td>
							<td><input type="checkbox" name="chkAgruparK" <cfif Mid(DiasSemanaAgrupOC, 3, 1) eq '1'>checked</cfif>></td>
							<td>Miércoles</td>
							<td><input type="checkbox" name="chkAgruparM" <cfif Mid(DiasSemanaAgrupOC, 4, 1) eq '1'>checked</cfif>></td>
						</tr>
						<tr>
							<td>Jueves</td>
							<td><input type="checkbox" name="chkAgruparJ" <cfif Mid(DiasSemanaAgrupOC, 5, 1) eq '1'>checked</cfif>></td>
							<td>Viernes</td>
							<td><input type="checkbox" name="chkAgruparV" <cfif Mid(DiasSemanaAgrupOC, 6, 1) eq '1'>checked</cfif>></td>
							<td>Sábado</td>
							<td><input type="checkbox" name="chkAgruparS" <cfif Mid(DiasSemanaAgrupOC, 7, 1) eq '1'>checked</cfif>></td>
							<td colspan="2">&nbsp;</td>
						</tr>
					</table>
				</td>
			</tr>

			<!---- Parámetro de día del mes que se va a realizar el proceso de agrupación
				  (usado cuando el parámetro de frecuencia sea mensualmente) ---->
			<cfset rsDiaMesAgrupOC = ObtenerDato(800)>
			<cfset hayDiaMesAgrupOC = 0>
			<cfif rsDiaMesAgrupOC.RecordCount GT 0>
				<cfset hayDiaMesAgrupOC = 1>
				<cfset DiaMesAgrupOC = rsDiaMesAgrupOC.Pvalor>
			<cfelse>
				<cfset DiaMesAgrupOC = '1'>
			</cfif>

			<tr id="trDiaMesAgrupOC" style="display:<cfif AgrupacionOrdenes eq '0' or FrecAgrupOC neq 'M'>none</cfif>">
				<td align="right" nowrap>El día:&nbsp;</td>
				<td>
					<cf_monto decimales="0" size="2" name="txtDiaMesAgrupOC" form="form1" value="#DiaMesAgrupOC#">
				</td>
			</tr>


			<cfquery name="rsCot" datasource="#session.DSN#">
				select FMT01COD, FMT01DES
				from FMT001
				where Ecodigo= <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#" >
				and FMT01TIP = 15	<!---- Tipo de reportes de Cotizacion de Compras ---->
			</cfquery>


			<cfset repcot = ObtenerDato(820)>
			<cfif repcot.RecordCount GT 0 >
				<cfset hayRepCotProveedoresLocal = 1 >
			</cfif>
			<tr>
				<td align="right">Formato Cotizaci&oacute;n de Proveedores Local</td>
				<td>
					<select name="repCotTipo1">
						<option value="" >-seleccionar-</option>
						<cfoutput query="rsCot">
							<option value="#rsCot.FMT01COD#" <cfif trim(repcot.Pvalor) eq trim(rsCot.FMT01COD) >selected</cfif> >#rsCot.FMT01DES#</option>
						</cfoutput>
					</select>
				</td>
			</tr>

			<cfset repcot = ObtenerDato(830)>
			<cfif repcot.RecordCount GT 0 >
				<cfset hayRepCotProveedoresImportacion = 1 >
			</cfif>
			<tr>
				<td align="right">Formato Cotizaci&oacute;n de Proveedores Importaci&oacute;n</td>
				<td>
					<select name="repCotTipo2">
						<option value="" >-seleccionar-</option>
						<cfoutput query="rsCot">
							<option value="#rsCot.FMT01COD#" <cfif trim(repcot.Pvalor) eq trim(rsCot.FMT01COD) >selected</cfif> >#rsCot.FMT01DES#</option>
						</cfoutput>
					</select>
				</td>
			</tr>

			<cfset repcot = ObtenerDato(840)>
			<cfif repcot.RecordCount GT 0 >
				<cfset hayRepCotProcesoLocal = 1 >
			</cfif>
			<tr>
				<td align="right">Formato Cotizaci&oacute;n de Proceso Local</td>
				<td>
					<select name="repCotTipo3">
						<option value="" >-seleccionar-</option>
						<cfoutput query="rsCot">
							<option value="#rsCot.FMT01COD#" <cfif trim(repcot.Pvalor) eq trim(rsCot.FMT01COD) >selected</cfif> >#rsCot.FMT01DES#</option>
						</cfoutput>
					</select>
				</td>
			</tr>

			<cfset repcot = ObtenerDato(850)>
			<cfif repcot.RecordCount GT 0 >
				<cfset hayRepCotProcesoImportacion = 1 >
			</cfif>
			<tr>
				<td align="right">Formato Cotizaci&oacute;n de Proceso Importaci&oacute;n</td>
				<td>
					<select name="repCotTipo4">
						<option value="" >-seleccionar-</option>
						<cfoutput query="rsCot">
							<option value="#rsCot.FMT01COD#" <cfif trim(repcot.Pvalor) eq trim(rsCot.FMT01COD) >selected</cfif> >#rsCot.FMT01DES#</option>
						</cfoutput>
					</select>
				</td>
			</tr>

			<cfset rolsolic = ObtenerDato(860)>
			<cfif rolsolic.RecordCount GT 0 >
				<cfset hayRolSolic = 1 >
			</cfif>
			<cfquery name="roles" datasource="asp" >
				select SRcodigo, SRdescripcion
				from SRoles
				where SScodigo='SIF'
				and SRcodigo in ( 	select distinct SRcodigo
								from SProcesosRol
								where SScodigo='SIF'
								and SMcodigo='CM'  )
				order by SRdescripcion
			</cfquery>
			<tr>
				<td align="right">Rol default para Solicitantes</td>
				<td>
					<select name="rolsolic">
						<option value="" >-seleccionar-</option>
						<cfoutput query="roles">
							<option value="#roles.SRcodigo#" <cfif trim(rolsolic.Pvalor) eq trim(roles.SRcodigo) >selected</cfif> >#roles.SRdescripcion#</option>
						</cfoutput>
					</select>
				</td>
			</tr>

			<cfset rolcomprador = ObtenerDato(870)>
			<cfif rolcomprador.RecordCount GT 0 >
				<cfset hayRolComprador = 1 >
			</cfif>
			<tr>
				<td align="right">Rol default para Compradores</td>
				<td>
					<select name="rolcomprador">
						<option value="" >-seleccionar-</option>
						<cfoutput query="roles">
							<option value="#roles.SRcodigo#" <cfif trim(rolcomprador.Pvalor) eq trim(roles.SRcodigo) >selected</cfif> >#roles.SRdescripcion#</option>
						</cfoutput>
					</select>
				</td>
			</tr>

			<cfset LvarCorreoAprobador = ObtenerDato(735)>
			<tr>
				<td align="right">Envio de correo al aprobador (traslado de Presupuesto):&nbsp;</td>
				<td>
					<input name="chkCorreoAprobador" type="checkbox" value="1" <cfif isdefined("LvarCorreoAprobador") and LvarCorreoAprobador.Pvalor eq 1>checked="checked"</cfif>>
				</td>
			</tr>

			<tr>
				<cfset req = ObtenerDato(545)>
				<cfif req.RecordCount GT 0 >
					<cfset hayPrecioUdec = 1 >
					<cfset LvarPrecioUdec = req.Pvalor>
				<cfelse>
					<cfset hayPrecioUdec = 0 >
					<cfset LvarPrecioUdec = 4>
				</cfif>
				<td width="39%" align="right" nowrap>Número de decimales para Precio Unitario:&nbsp;</td>

				<td width="35%">
					<select name="PrecioUdec" onchange="if(this.form.PrecioUdecRPT.selectedIndex > this.form.PrecioUdec.selectedIndex) this.form.PrecioUdecRPT.selectedIndex = this.form.PrecioUdec.selectedIndex;">
						<option value="2" <cfif LvarPrecioUdec EQ 2>selected</cfif>>15 enteros 2 decimales</option>
						<option value="3" <cfif LvarPrecioUdec EQ 3>selected</cfif>>14 enteros 3 decimales</option>
						<option value="4" <cfif LvarPrecioUdec EQ 4>selected</cfif>>13 enteros 4 decimales</option>
						<option value="5" <cfif LvarPrecioUdec EQ 5>selected</cfif>>12 enteros 5 decimales</option>
						<option value="6" <cfif LvarPrecioUdec EQ 6>selected</cfif>>11 enteros 6 decimales</option>
						<option value="7" <cfif LvarPrecioUdec EQ 7>selected</cfif>>10 enteros 7 decimales</option>
						<option value="8" <cfif LvarPrecioUdec EQ 8>selected</cfif>>09 enteros 8 decimales</option>
					</select>
				</td>
				<td CNTPspan="4" valign="middle">
				<!----
				<table width="100%" class="ayuda">
				<tr><td>Poner Ayuda</td></tr>
				</table>
				---->
				</td>
			</tr>

			<tr>
				<cfset req = ObtenerDato(546)>
				<cfif req.RecordCount GT 0 >
					<cfset hayPrecioUdecRPT = 1 >
					<cfset LvarPrecioUdecRPT = req.Pvalor>
				<cfelse>
					<cfset hayPrecioUdecRPT = 0 >
					<cfset LvarPrecioUdecRPT = 2>
				</cfif>
				<td width="39%" align="right" nowrap>Decimales del Precio Unitario en Reportes:&nbsp;</td>

				<td width="35%">
					<select name="PrecioUdecRPT" onchange="if(this.form.PrecioUdecRPT.selectedIndex > this.form.PrecioUdec.selectedIndex) this.form.PrecioUdecRPT.selectedIndex = this.form.PrecioUdec.selectedIndex;">
						<option value="2" <cfif LvarPrecioUdecRPT EQ 2>selected</cfif>>15 enteros 2 decimales</option>
						<option value="3" <cfif LvarPrecioUdecRPT EQ 3>selected</cfif>>14 enteros 3 decimales</option>
						<option value="4" <cfif LvarPrecioUdecRPT EQ 4>selected</cfif>>13 enteros 4 decimales</option>
						<option value="5" <cfif LvarPrecioUdecRPT EQ 5>selected</cfif>>12 enteros 5 decimales</option>
						<option value="6" <cfif LvarPrecioUdecRPT EQ 6>selected</cfif>>11 enteros 6 decimales</option>
						<option value="7" <cfif LvarPrecioUdecRPT EQ 7>selected</cfif>>10 enteros 7 decimales</option>
						<option value="8" <cfif LvarPrecioUdecRPT EQ 8>selected</cfif>>09 enteros 8 decimales</option>
					</select>
				</td>
				<td CNTPspan="4" valign="middle">
				<!----
				<table width="100%" class="ayuda">
				<tr><td>Poner Ayuda</td></tr>
				</table>
				---->
				</td>
			</tr>

			<tr>
				<cfset req = ObtenerDato(548)>
				<cfif req.RecordCount GT 0 >
					<cfset hayCPcompras = 1 >
					<cfset LvarCPcompras = req.Pvalor>
				<cfelse>
					<cfset hayCPcompras = 0 >
					<cfset LvarCPcompras = 0>
				</cfif>
				<td width="39%" align="right" nowrap>Control de Presupuesto en Compras de Artículos de Inventario:&nbsp;</td>

				<td width="35%">
					<select name="CPcompras">
						<option value="0" <cfif LvarCPcompras EQ 0>selected</cfif>>Controlar el Consumo</option>
						<option value="1" <cfif LvarCPcompras EQ 1>selected</cfif>>Controlar la Compra</option>
					</select>
				</td>
				<td CNTPspan="4" valign="middle">
				<!----
				<table width="100%" class="ayuda">
				<tr><td>Poner Ayuda</td></tr>
				</table>
				---->
				</td>
			</tr>
			<!----►►►Parametro 4100:Controlar existencias de artículos en solicitudes de requisición◄◄◄---->
            <cfset chkContExisSol = ObtenerDato(4100).Pvalor>
            <tr>
            	<td align="right">Controlar existencias de artículos en solicitudes de requisición:</td>
                <td><input name="chkContExisSol" type="checkbox" value="1" <cfif chkContExisSol EQ 1>checked="checked"</cfif>></td>
            </tr>
			<!----►►►Parametro 4300:Desreservar Presupuesto si la Orden de Compra no cubre el total de la Solicitud (Plan Compras)◄◄◄---->
			<cfset chkDesReservSC = ObtenerDato(4300).Pvalor>
			<cfset rsformularPor         = ObtenerDato(2300)>
			<cfif rsformularPor.Pvalor eq 1>
			<tr>
            	<td align="right">Desreservar Presupuesto si la Orden de Compra no cubre el total de la Solicitud (Plan Compras):</td>
                <td><input name="chkDesReservSC" type="checkbox" value="1" <cfif chkDesReservSC EQ 1>checked="checked"</cfif>></td>
            </tr>
			</cfif>

           <!----Modifica Orden de Compra---->
           <tr>
			<cfset ModOC = ObtenerDato(4310)><!----TraslDirectoCR---->
				<td align="right">Modifica Orden de Compra x Solicitud de Compra Directa o Proceso de Compra:&nbsp;</td>
				<td><input name="ModOC" type="checkbox" <cfif ModOC.Pvalor eq 1>checked</cfif>></td>
			</tr>

            <!---- No Validar Moneda en Contratos ---->
           <tr>
			<cfset ValidarMoneda = ObtenerDato(5305)>
				<td align="right">No Validar Moneda En Contratos:&nbsp;</td>
				<td><input name="ValidarMoneda" type="checkbox" <cfif ValidarMoneda.Pvalor eq 1>checked</cfif>></td>
			</tr>
			<!----►►Parametro 5306: Indicar Cuentas Contables Manualmente◄◄---->
            <cfset rsCuentaManual = ObtenerDato(5306)>
            <tr>
                <td align="right" nowrap>Permitir cambiar Cuentas Manualmente:&nbsp;</td>
                <td><input type="checkbox" name="chkCuentaManual" <cfif rsCuentaManual.RecordCount gt 0 and rsCuentaManual.Pvalor eq 'S' >checked</cfif>></td>
            </tr>

            <!----►►Parametro 5307: Descripción Alterna Requerida◄◄---->
            <cfset rsDescripcionAlternaR = ObtenerDato(5307)>
            <tr>
                <td align="right" nowrap>Descripción Alterna Requerida en Solicitudes de Compra:&nbsp;</td>
                <td><input type="checkbox" name="chkDescripcionAR" <cfif rsDescripcionAlternaR.RecordCount gt 0 and rsDescripcionAlternaR.Pvalor eq 'S' >checked</cfif>></td>
            </tr>

			<!----►►Parametro 15652: Habilitar validacion de campos obligatorios  del catalogo de Actividad de Tracking , esto para
									 el formulario de la poliza, y cierre de la poliza. En caso de que se deje habilitada la validacion
									 se deben agregar las actividades faltantes si el mensaje aparece en rojo  señalando cuales son,
									 de lo contrario no se va a poder cerrar la poliza◄◄---->
            <cfset rsSeguimientoTrackingObligatorios = ObtenerDato(15652)>
            <tr>
                <td align="right" nowrap>Validar campos obligatorios del seguimiento de Tracking en la p&oacute;liza de Desalmacenaje:</td>
                <td><input type="checkbox" name="chkValidaCamposTrack" <cfif rsSeguimientoTrackingObligatorios.RecordCount gt 0 and rsSeguimientoTrackingObligatorios.Pvalor eq 'S' >checked</cfif>></td>
            </tr>
		</table>
	</div><!--- Fin CM --->


	<!---- FACTURACION ---->
	<div class="FAC">
		<table width="100%">
			<!---- FACTURACION ---->
				<tr>
					<td colspan="3" align="center" class="listaPar"><hr size="0"><b><font size="2">Facturaci&oacute;n</font></b>
					</td>
				</tr>
				<cfset rsSupervisor = ObtenerDato(430)>
				<cfif rsSupervisor.RecordCount GT 0 >
					<cfset haySupervisor = 1 >
					<cfset supervisor = rsSupervisor.Pvalor>
				<cfelse>
					<cfset supervisor = '' >
				</cfif>
				<tr>
					<td align="right">Supervisor de Cierre de Cajas:&nbsp;</td>
					<td>
						<cfif Len(Trim(supervisor)) GT 0>
						<cfquery name="rsNombre" datasource="asp">
							select
								{fn concat({fn concat({fn concat({fn concat(
								Pnombre , ' '  )}, Papellido1 )}, ' ' )}, Papellido2 )}as Nombre
							from Usuario a, DatosPersonales b
							where a.Usucodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#supervisor#">
							and a.datos_personales=b.datos_personales
						</cfquery>
						<cfset nombre = rsNombre.Nombre >
						<cfelse>
							<cfset nombre = '' >
						</cfif>
						<input name="FANombreSupervisor" type="text" value="<cfoutput>#nombre#</cfoutput>" id="FANombreSupervisor" size="40" maxlength="80" readonly tabindex="-1">
						<a href="#" tabindex="-1"><img src="../../imagenes/Description.gif" alt="Lista de Usuarios de SIF" name="imagen" width="18" height="14" border="0" align="absmiddle" onClick="javascript:doSupervisor();"></a>
						<input name="FASupervisor" type="hidden" id="FASupervisor" value="<cfoutput>#supervisor#</cfoutput>">
					</td>
					<td valign="middle">
					<!----
					<table width="100%" class="ayuda">
					<tr><td>Falta Ayuda</td></tr>
					</table>
					---->
					</td>
				</tr>

				<cfset rsCierreCaja = ObtenerDato(500)>
				<cfif rsCierreCaja.RecordCount GT 0 >
					<cfset hayCierreCaja = 1 >
					<cfset cierreCaja = rsCierreCaja.Pvalor>
				<cfelse>
					<cfset cierreCaja = '' >
				</cfif>
				<tr>
					<td align="right">Usa Cierre de Caja:&nbsp;</td>
					<td><input type="checkbox" name="chkCierreCaja" <cfif trim(cierreCaja) eq 'S'>checked</cfif> ></td>
				</tr>

				<cfset rsAumentaConsecutivo = ObtenerDato(510)>
				<cfif rsAumentaConsecutivo.RecordCount GT 0 >
					<cfset hayAumentaConsecutivo = 1 >
					<cfset aumentaConsecutivo = rsAumentaConsecutivo.Pvalor>
				<cfelse>
					<cfset aumentaConsecutivo = '' >
				</cfif>
				<tr>
					<td align="right" nowrap>Modifica n&uacute;mero de factura al reimprimir:&nbsp;</td>
					<td><input type="checkbox" name="chkAumentaConsecutivo" <cfif trim(aumentaConsecutivo) eq 'S'>checked</cfif> ></td>
				</tr>
                <tr>
					<td align="right" nowrap>Cuenta de Ingresos por intereses corrientes:&nbsp;</td>
						<cfset cuentaCorrientes = ObtenerDato(550)>
						<cfif cuentaCorrientes.RecordCount gt 0 and len(trim(cuentaCorrientes.Pvalor))>
							<cfquery name="rsCuentaCor" datasource="#session.DSN#">
								select Ccuenta, Cmayor, Cformato, Cdescripcion
								from CContables
								where Ccuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#cuentaCorrientes.Pvalor#">
							</cfquery>
							<cfset hayCorrientes = 1 >
						<cfelse>
							<cfset hayCorrientes = 0 >
						</cfif>

						<td>
							<cfif isdefined("rsCuentaCor") and rsCuentaCor.RecordCount gt 0>
								<cf_cuentas frame="frCuentaCorrientes" Ccuenta="CCorrientes" Cformato="FCCorrientes" Cmayor="MCCorrientes" Cdescripcion="DCCorrientes" form="form1" query="#rsCuentaCor#" >
								<cfelse>
								<cf_cuentas frame="frCuentaCorrientes" Ccuenta="CCorrientes" Cformato="FCCorrientes" Cmayor="MCCorrientes" Cdescripcion="DCCorrientes" form="form1" >
							</cfif>
						</td>
						<td>&nbsp;</td>
					</tr>

					<tr>
						<td align="right" nowrap>Cuenta de Ingresos por intereses moratorios:&nbsp;</td>
							<cfset cuentaMoratorios = ObtenerDato(560)>
							<cfif cuentaMoratorios.RecordCount gt 0 and len(trim(cuentaMoratorios.Pvalor))>
								<cfquery name="rsCuentaMor" datasource="#session.DSN#">
									select Ccuenta, Cmayor, Cformato, Cdescripcion
									from CContables
									where Ccuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#cuentaMoratorios.Pvalor#">
								</cfquery>
								<cfset hayMoratorios = 1 >
							<cfelse>
								<cfset hayMoratorios = 0 >
							</cfif>

							<td>
								<cfif isdefined("rsCuentaMor") and rsCuentaMor.RecordCount gt 0>
									<cf_cuentas frame="frCuentaMoratorios" Ccuenta="CMoratorios" Cformato="FCMoratorios" Cmayor="MCMoratorios" Cdescripcion="DCMoratorios" form="form1" query="#rsCuentaMor#" >
									<cfelse>
									<cf_cuentas frame="frCuentaMoratorios" Ccuenta="CMoratorios" Cformato="FCMoratorios" Cmayor="MCMoratorios" Cdescripcion="DCMoratorios" form="form1" >
								</cfif>
							</td>
							<td>&nbsp;</td>
						</tr>
                          <!----- Cuenta para facturas en transito ---->
				<tr>
					<td align="right" nowrap>Cuenta transitoria com&uacute;n para Facturaci&oacute;n:&nbsp;</td>
						<cfset cuentaTransitoria = ObtenerDato(565)>
						<cfif cuentaTransitoria.RecordCount gt 0 and len(trim(cuentaTransitoria.Pvalor))>
							<cfquery name="rsCuentaTran" datasource="#session.DSN#">
								select Ccuenta, Cmayor, Cformato, Cdescripcion
								from CContables
								where Ccuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#cuentaTransitoria.Pvalor#">
							</cfquery>
							<cfset hayTransitoria = 1 >
						<cfelse>
							<cfset hayTransitoria = 0 >
						</cfif>
						<td>
							<cfif isdefined("rsCuentaTran") and rsCuentaTran.RecordCount gt 0>
								<cf_cuentas frame="frCuentaTransitoria" Ccuenta="CTransitoria" Cformato="FTransitoria" Cmayor="MTransitoria" Cdescripcion="DTransitoria" form="form1" query="#rsCuentaTran#" >
								<cfelse>
								<cf_cuentas frame="frCuentaTransitoria" Ccuenta="CTransitoria" Cformato="FTransitoria" Cmayor="MTransitoria" Cdescripcion="DTransitoria" form="form1" >
							</cfif>
						</td>
						<td>&nbsp;</td>
				</tr>
                <tr>
                    <td align="right" nowrap>Transacci&oacute;n de Notas de Cr&eacute;dito de Clientes:&nbsp;</td>
                    	<cfset TransaccionNCredito = ObtenerDato(575)>
						<cfif TransaccionNCredito.RecordCount GT 0 >
							<cfset hayTNCredito = 1 >
                            <cfset TNCredito = TransaccionNCredito.Pvalor>
                        <cfelse>
                            <cfset TNCredito = '' >
                        </cfif>
						<cf_dbfunction name="string_part"  args="a.CCTdescripcion, 1, 10" returnvariable="dato">
                        <cfquery name="rsTransacciones" datasource="#Session.DSN#">
                            select 	a.CCTcodigo,
                            	case when coalesce(a.CCTvencim,0) < 0 then
									<cf_dbfunction name="concat" args="#dato#+' (contado)'" delimiters="+">
                            	else
                                	<cf_dbfunction name="string_part"  args="a.CCTdescripcion, 1, 20">
                            	end as CCTdescripcion,
                            	case when coalesce(a.CCTvencim,0) >= 0 then 1 else 2 end as CCTorden,
                            	a.CCTtipo
                           	from CCTransacciones a
                            where a.Ecodigo = #Session.Ecodigo#
                            and a.CCTtipo = <cfqueryparam cfsqltype="cf_sql_char" value="C"><!---- 'D' ---->
                            and coalesce(a.CCTpago,0) != 1
                            and NOT a.CCTdescripcion like '%Tesorer_a%'
                            order by case when coalesce(a.CCTvencim,0) >= 0 then 1 else 2 end, a.CCTcodigo
                        </cfquery>
                    <td>
                        <select name="NC_CCTcodigo">
                            <option value=""></option>
                            <cfoutput query="rsTransacciones">
                            	<option value="#CCTcodigo#" <cfif TNCredito EQ rsTransacciones.CCTcodigo>selected</cfif>>#CCTdescripcion#</option>
                            </cfoutput>
                        </select>
                    </td>
                </tr>
				<!----- Cuenta por Pagar a Notas de Credito ---->
                <tr>
                    <td align="right" nowrap>Cuenta por Pagar a Notas de Cr&eacute;dito:&nbsp;</td>
                    <cfset cuentaNCredito = ObtenerDato(555)>
                    <cfif cuentaNCredito.RecordCount gt 0 and len(trim(cuentaNCredito.Pvalor))>
                        <cfquery name="rsCuentaNCredito" datasource="#session.DSN#">
                            select Ccuenta, Cmayor, Cformato, Cdescripcion
                            from CContables
                            where Ccuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#cuentaNCredito.Pvalor#">
                        </cfquery>
                        <cfset hayCNCredito = 1 >
                    <cfelse>
                    	<cfset hayCNCredito = 0 >
                    </cfif>
                    <td>
						<cfif isdefined("rsCuentaNCredito") and rsCuentaNCredito.RecordCount gt 0>
                            <cf_cuentas frame="frCuentaNCredito" Ccuenta="CNCredito" Cformato="FNCredito" Cmayor="MNCredito" Cdescripcion="DNCredito" form="form1" query="#rsCuentaNCredito#" >
                        <cfelse>
                            <cf_cuentas frame="frCuentaNCredito" Ccuenta="CNCredito" Cformato="FNCredito" Cmayor="MNCredito" Cdescripcion="DNCredito" form="form1" >
                        </cfif>
                    </td>
                    <td>&nbsp;</td>
                </tr>
                <!----Cliente de Contado---->
				<tr>
					<td align="right" >Cliente de Contado:&nbsp;</td>
					<cfset  clienteContado = ObtenerDato(435) >
					<cfif clienteContado.RecordCount gt 0 and len(trim(clienteContado.Pvalor))>
						<cfset  hayClienteContado = 1>
					</cfif>
					<td>
						<cfif isdefined("clienteContado") and clienteContado.RecordCount gt 0>
							<cf_sifsociosnegocios2 SNtiposocio="C" idquery="#clienteContado.Pvalor#">
						<cfelse>
							<cf_sifsociosnegocios2 SNtiposocio="C">
						</cfif>
					</td>
					<td>&nbsp;</td>
				</tr>
				<!---- 15834. Parametro para indicar de donde se va tomar el catalogo de vendedores. De cxc o de facturacion ---->
				<cfset  rsValidaVendedor = ObtenerDato(15834) >
				<cfif rsValidaVendedor.RecordCount GT 0 and rsValidaVendedor.Pvalor eq 1 >
					<cfset  hayVendedorCxC = 1>
					<cfset  VendedorCxC = rsValidaVendedor.Pvalor >
				<cfelse>
					<cfset  VendedorCxC = ''>
				</cfif>
				<tr>
					<td width="39%" align="right" >Cat&aacute;logo vendedores de CxC:&nbsp;</td>
					<td><input type="checkbox" name="chkVendedorCxC" <cfif trim(VendedorCxC) eq '1'>checked</cfif> ></td>
					<td rowspan="5" valign="middle">
					</td>
				</tr>
				<!---- 15835. Valida si se va a registrar los costos de inventarios y la deduccion ---->
				<cfset  rsControlInventarios = ObtenerDato(15835) >
				<cfif rsControlInventarios.RecordCount GT 0 and rsControlInventarios.Pvalor eq 1 >
					<cfset  hayControlInventarios = 1>
					<cfset  ControlInventarios = rsControlInventarios.Pvalor >
				<cfelse>
					<cfset  ControlInventarios = ''>
				</cfif>
				<tr>
					<td width="39%" align="right" >Controla inventarios en facturacion</td>
					<td><input type="checkbox" name="chkControlInventarios" <cfif trim(ControlInventarios) eq '1'>checked</cfif> ></td>
					<td rowspan="5" valign="middle">
					</td>
				</tr>				
				<!----- 1401. Manejo de cuenta de descuentos  en detalles de facturacion ---->
				<cfset  rsCuentadedescuentosFact = ObtenerDato(1401) >
				<cfif rsCuentadedescuentosFact.RecordCount GT 0>
					<cfset  hayCuentadedescuentosFact = 1 >
					<cfset  CuentadedescuentosFact = rsCuentadedescuentosFact.Pvalor >
				<cfelse>
					<cfset  hayCuentadedescuentosFact = ''>
				</cfif>
				<tr>
					<td width="39%" align="right" >Cuenta para descuentos en detalles de facturacion</td>
					<td>
						<select name="cuentaDescuentoDetallesFac">
							<option value="0" <cfif rsCuentadedescuentosFact.Pvalor eq 0>selected</cfif> >Cuenta de descuentos de la caja</option>
							<option value="1" <cfif rsCuentadedescuentosFact.Pvalor eq 1>selected</cfif>> Cuenta del centro funcional</option>
						</select>
					</td>
					<td rowspan="5" valign="middle">
					</td>

				</tr>

				<!---- PARAMETRO PARA DEFINIR EL CONSECUTIVO DEL DOCUMENTO DE COBROS ---->
				<tr>
					<!---- Obtener el parametro en la base de datos---->
					<cfset  LvarConsecutivoRE = ObtenerDato(16200) >
					<cfset  hayConsecutivoRe = 0>
					<cfif LvarConsecutivoRE.RecordCount GT 0 and Len(Trim(LvarConsecutivoRE.Pvalor))>
						<cfset  hayConsecutivoRe = 1 >
						<cfset  ConsecutivoRE = LvarConsecutivoRE.Pvalor >
					<cfelse>
						<cfset  ConsecutivoRE = '' >
					</cfif>
					<td align="right" >Consecutivo Recibos de pagos:&nbsp;</td>
					<td><input name="faConsecutivoRE" style="text-align:left" type="text" value="<cfoutput>#ConsecutivoRE#</cfoutput>" size="25" maxlength="50"></td>
				</tr>
				<!---- Caja por defecto para NC sin Refactura o Con Refactura de tipo Credito ---->
				<cfquery name="rsCajas" datasource="#session.dsn#">
					select FCid,FCcodigo,FCdesc
					from FCajas fc
					where fc.Ecodigo = #session.Ecodigo#
				</cfquery>
				<cfset  repCaja = ObtenerDato(16305) >
				<cfif repCaja.RecordCount GT 0 >
					<cfset  hayrepCajaAsignado = 1>
				</cfif>
				<tr>
					<td align="right">Caja Predeterminada para Notas de Cr&eacutedito SIN Re-Factura, o con Refacturas de tipo Cr&eacutedito.</td>
					<td>
						<select name="repCajasDefault">
							<option value="" >-seleccionar-</option>
							<cfoutput query="rsCajas">
								<option value="#rsCajas.FCid#" <cfif trim(repCaja.Pvalor) eq trim(rsCajas.FCid) >selected</cfif> >#rsCajas.FCdesc#</option>
							</cfoutput>
						</select>
					</td>
				</tr>
		<!---- Utilizar vencimiento por socio de negocio ---->
				<!---- a la hora de crear una ReFactura, Si este bit esta encendido, entonces busca el socio de negocio cliente obtiene los dias de vencimiento y se los suma a la fecha de la factura, si no es el caso, entonces obtiene los dias de vencimiento de la transacciones(CTransacciones) que se selecciono en la ReFactura. ---->
				<cfset  repVencimientoSocio = ObtenerDato(16306) >
				<cfset  _repVencimientoSocio = 0>
				<cfif repVencimientoSocio.recordCount GT 0>
					<cfset  hayrepVencimientoSocio = 1 >
					<cfset  _repVencimientoSocio = repVencimientoSocio.Pvalor >
				</cfif>
				<tr>
					<td align="right">Utilizar Vencimiento por socios de Negocio:</td>
					<td>
						<input type="checkbox" name="chkVencimientoSocio" <cfif trim(_repVencimientoSocio) eq '1'>checked</cfif> >
					</td>
				</tr>

				<!---- Lista de Codigo Empresa Origen para evitar que puedan refacturar ---->
				<!---- Se valida que si el codigo de empresa origen esta en esta lista (FAERecuperacion), no le permitar ReFacturar ---->
				<cfset  LvarConsecutivoCodEmpOrigen = ObtenerDato(16307) >
				<cfset  hayConsecutivoCodEmpOrigen = 0>
				<cfif LvarConsecutivoCodEmpOrigen.RecordCount GT 0 and Len(Trim(LvarConsecutivoCodEmpOrigen.Pvalor))>
					<cfset  hayConsecutivoCodEmpOrigen = 1 >
					<cfset  ConsecutivoCodEmpOrigen = LvarConsecutivoCodEmpOrigen.Pvalor >
				<cfelse>
					<cfset  ConsecutivoCodEmpOrigen = '' >
				</cfif>
				<tr>
					<td align="right" >Códigos de Sistemas Origen que no permiten Re-Facturacion(separados por ','):&nbsp;</td>
					<td><input name="faConsecutivoCodEmpOrigen" style="text-align:left" type="text" value="<cfoutput>#ConsecutivoCodEmpOrigen#</cfoutput>"/>
				</tr>

				<!---- Monto Maximo para devolucion en cajas  Pcodigo: 16039---->
				<tr>
					<!---- Obtener el parametro en la base de datos---->
					<cfset  LvarMontoMaximoVuelto = ObtenerDato(16039) >
					<cfset  hayMontoMaximoVuelto = 0>
					<cfif LvarMontoMaximoVuelto.RecordCount GT 0 and Len(Trim(LvarMontoMaximoVuelto.Pvalor))>
						<cfset  hayMontoMaximoVuelto = 1 >
						<cfset  MontoMaximoVuelto = LvarMontoMaximoVuelto.Pvalor >
					<cfelse>
						<cfset  MontoMaximoVuelto = 0.00 >
					</cfif>
					<td align="right" >Monto Maximo para dar vuelto:&nbsp;</td>
					<td><input name="faMontoMaximoVuelto" style="text-align:left" type="text" value="<cfoutput>#MontoMaximoVuelto#</cfoutput>" size="25" maxlength="50"></td>
				</tr>

				<tr>
					<!---- Centro Funcional para remesas Pcodigo: 16309---->
					<cfset  LvarHayCFid = ObtenerDato(16309) >
					<cfset  hayCFRemesas = 0>
					<cfif LvarHayCFid.RecordCount GT 0 and Len(Trim(LvarHayCFid.Pvalor))>
						<cfset  hayCFRemesas = 1 >
						<cfset  LvarCFid = LvarHayCFid.Pvalor >
					<cfelse>
						<cfset  LvarCFid = "" >
					</cfif>
					<td align="right" >Centro Funcional para Remesas:&nbsp;</td>
					<td><input name="CFParaRemesas" style="text-align:left" type="text" value="<cfoutput>#LvarCFid#</cfoutput>" size="25" maxlength="50"></td>
				</tr>

				<!---- Integración con facturación digital. ---->
				<!---- Si este bit esta encendido, entonces agrega los registros en la bitácora de envíos electrónicos. ---->
				<cfset  integraFacturaDigital = ObtenerDato(16317) >
				<cfset  _integraFacturaDigital = 0>
				<cfif integraFacturaDigital.recordCount GT 0>
					<cfset  hayintegraFacturaDigital = 1 >
					<cfset  _integraFacturaDigital = integraFacturaDigital.Pvalor >
				</cfif>
				<tr>
					<td align="right">Integración con facturación digital:</td>
					<td>
						<input type="checkbox" name="chkintegraFacturaDigital" <cfif trim(_integraFacturaDigital) eq '1'>checked</cfif> >
					</td>
				</tr>

				<!---- Proveedor de facturación Digital ---->
				<cfset  ProveedorFacturaDigital = ObtenerDato(16371) >
				<cfset  _ProveedorFacturaDigital = 0>
				<cfif ProveedorFacturaDigital.recordCount GT 0>
					<cfset  hayProveedorFacturaDigital = 1 >
					<cfset  _ProveedorFacturaDigital = ProveedorFacturaDigital.Pvalor >
				</cfif>
				<tr>
					<td align="right">Proveedor facturación digital:</td>
					<td>
						<select name="ProveedorFE">
							<option value="" >-seleccionar-</option>
							<option value="1" <cfif trim(ProveedorFacturaDigital.Pvalor) eq "1" >selected</cfif> >GTI</option>
							<option value="2" <cfif trim(ProveedorFacturaDigital.Pvalor) eq "2" >selected</cfif> >NACION</option>
						</select>
					</td>
				</tr>

				<!---- Permite activar la interfaz de conexión para SGA ---->
			<tr>
				<cfset  rsInterfazSGA = ObtenerDato(16318) >
				<cfset  interfazSGA = 0>
				<cfif rsInterfazSGA.RecordCount GT 0>
					<cfset  hayInterfazSGA = 1 >
					<cfset  interfazSGA = rsInterfazSGA.Pvalor >
				</cfif>

				<td align="right">Activar la interfaz de conexión para SGA:&nbsp;</td>
				<td>
					<input type="checkbox" name="chkInterfazSGA" <cfif interfazSGA eq 1>checked</cfif> >
				</td>
			</tr>

			<!---- Permite contabilizar transacciones al aplicar ---->
			<tr>
				<cfset  rsContabilizarAplicarTransa = ObtenerDato(16372) >
				<cfset  ContabilizarAplicarTransa = 0>
				<cfif rsContabilizarAplicarTransa.RecordCount GT 0>
					<cfset  hayContabilizarAplicarTransa = 1 >
					<cfset  ContabilizarAplicarTransa = rsContabilizarAplicarTransa.Pvalor >
				</cfif>

				<td align="right">Contabilizar transacciones al aplicar:&nbsp;</td>
				<td>
					<input type="checkbox" name="chkContabilizarAplicarTransa" <cfif ContabilizarAplicarTransa eq 1>checked</cfif> >
				</td>
			</tr>

			<tr>
				<cfset  rsValidarCorreoYtelefono = ObtenerDato(16373) >
				<cfset  ValidaCorreoYtelefono = 0>
				<cfif rsValidarCorreoYtelefono.RecordCount GT 0>
					<cfset  hayValidacionCorreoYtelefono = 1 >
					<cfset  ValidaCorreoYtelefono = rsValidarCorreoYtelefono.Pvalor >
				</cfif>

				<td align="right">Valida correo y telefono al registrar factura:&nbsp;</td>
				<td>
					<input type="checkbox" name="chkValidarCorreoTelefono" <cfif ValidaCorreoYtelefono eq 1>checked</cfif> >
				</td>
			</tr>
			<tr>
				<cfset  rsValidarPlantillaFE = ObtenerDato(17000) >
				<cfset  ValidarPlantillaFE = ''>
				<cfset  hayValidarPlantillaFE = 0>
				<cfif rsValidarPlantillaFE.RecordCount GT 0>
					<cfset  hayValidarPlantillaFE = 1>
					<cfset  ValidarPlantillaFE = rsValidarPlantillaFE.Pvalor>
				</cfif>

			<!---- 	<cfset archivoname=#vsPath_R#&""> ---->
				<!---- wwwroot\sif\fa\Plantillas ---->
				<cfdirectory action="list" directory="#ExpandPath('/sif/fa/Plantillas/')#" name="listRoot">

				<!---- <cfoutput>
					<cfloop query="listRoot">
					#replace(name,".cfc","","all")#<br />
					</cfloop>
				</cfoutput>	
				 ---->
				<td align="right">Plantilla factura:&nbsp;</td>
				<td>
					<select name="plantillaFE">
						<cfoutput>
						<option value="" <cfif rsCuentadedescuentosFact.Pvalor eq "">selected</cfif> >
						Seleccione la Plantilla
						</option>
						<cfloop query="listRoot">
							<cfif find("FEDatosBase",name) EQ 0 and find(".cfc",name) GT 0 >
							<!---- <cfdump var="#listRoot.name#---#find(".cfc",listRoot.name)#"> ---->
								<option value="#replace(name,".cfc","","all")#" <cfif ValidarPlantillaFE eq #replace(name,".cfc","","all")#>selected</cfif> >
								#replace(name,".cfc","","all")#
								</option>
							</cfif>
						</cfloop>
						</cfoutput>
					</select>
				</td>
			
			</tr>
			<!---Parametro para Validar Timbre Fiscal en CxC y CxP  --->
			<tr>
				<cfset  rsTFiscal = ObtenerDato(17100) >
				<cfset  TFiscal = 0>
				<cfif rsTFiscal.RecordCount GT 0>
					<cfset  hayTFiscal = 1 >
					<cfset  TFiscal = rsTFiscal.Pvalor >
				</cfif>

				<td align="right">Validacion Timbre Fiscal UUID:&nbsp;</td>
				<td>
					<input type="checkbox" name="chkTFiscal" <cfif TFiscal eq 1>checked</cfif> >
				</td>
			</tr>
			<!----- Version CFDI  ---->
				<cfset  rsControlVersion = ObtenerDato(17200)>
				<cfif rsControlVersion.RecordCount GT 0 and rsControlVersion.Pvalor eq 1 >
					<cfset  hayVersion = 1>
				</cfif>
				<tr>
					<td align="right">Versión CFDI:&nbsp;</td>
					<td>
						<select id="cbxhayVersion" name="cbxhayVersion">
							<cfoutput>
								<option value="0" <cfif rsControlVersion.Pvalor eq 0>selected</cfif> >Seleccione la Versión</option>
								<option value="3.3" <cfif rsControlVersion.Pvalor eq 3.3>selected</cfif> >3.3</option>
								<option value="4.0" <cfif rsControlVersion.Pvalor eq 4.0>selected</cfif> >4.0</option>
							</cfoutput>
						</select>
					</td>
				</tr>
				<cfset  rsControlDeciFac = ObtenerDato(17300)>
				<cfif rsControlDeciFac.RecordCount neq "" and rsControlDeciFac.Pvalor eq 1 >
					<cfset  hayDeciFac = 1>
				</cfif>
				<tr>
					<td align="right">Decimales Valor Unitario:&nbsp;</td>
					<td>
						<select id="cbxDecimalesFac" name="cbxDecimalesFac">
							<cfoutput>
								<option value="" <cfif rsControlDeciFac.Pvalor eq "">selected</cfif> >Selecciona Decimales</option>
								<option value="0" <cfif rsControlDeciFac.Pvalor eq 0>selected</cfif> >0</option>
								<option value="1" <cfif rsControlDeciFac.Pvalor eq 1>selected</cfif> >1</option>
								<option value="2" <cfif rsControlDeciFac.Pvalor eq 2>selected</cfif> >2</option>
								<option value="3" <cfif rsControlDeciFac.Pvalor eq 3>selected</cfif> >3</option>
								<option value="4" <cfif rsControlDeciFac.Pvalor eq 4>selected</cfif> >4</option>
								<option value="5" <cfif rsControlDeciFac.Pvalor eq 5>selected</cfif> >5</option>
								<option value="6" <cfif rsControlDeciFac.Pvalor eq 6>selected</cfif> >6</option>
							</cfoutput>
						</select>
					</td>
				</tr>
				<tr>
				<cfset  rsValorRuta = ObtenerDato(17400)>
				<cfif rsValorRuta.RecordCount neq "" and rsValorRuta.Pvalor eq 1 >
					<cfset  hayRuta = rsValorRuta.Pvalor>
				</cfif>
					<td align="right">Ruta de Guardado de Documentos:&nbsp;</td>
					<td>
						<input <cfif rsValorRuta.Pvalor neq ""> value="<cfoutput>#rsValorRuta.Pvalor#</cfoutput>"</cfif> type="text" name="ruta" id="ruta" placeholder="Ejemplo: C:\Enviar"/>
					</td>
				</tr>
				<tr>
				<cfset  rsZonaHoraria = ObtenerDato(17500)>
				<cfif rsZonaHoraria.RecordCount neq "" and rsZonaHoraria.Pvalor eq 1 >
					<cfset  hayZona = rsZonaHoraria.Pvalor>
				</cfif>
				<cfset  rsVerano = ObtenerDato(17600) >
				<cfset  CVerano = 0>
				<cfif rsVerano.RecordCount GT 0>
					<cfset  hayCVerano = 1 >
					<cfset  CVerano = rsVerano.Pvalor >
				</cfif>
				<td align="right">Zona Horaria para Timbrado de Factura:&nbsp;</td>
					<td>
						<select id="cbxZonahoraria" name="cbxZonahoraria">
							<cfoutput>
								<option value="" <cfif rsZonaHoraria.Pvalor eq " ">selected</cfif> >Selecciona la Zona Horaria</option>
								<option value="0,0" <cfif rsZonaHoraria.Pvalor eq "0" and CVerano eq 0>selected</cfif> >Sin Zona Horaria</option>
								<option value="-08,0" <cfif rsZonaHoraria.Pvalor eq "-08" and CVerano eq 0>selected</cfif> >UTC-8 Tijuana/Hermosillo (Estándar)</option>
								<option value="-08,1" <cfif rsZonaHoraria.Pvalor eq "-08" and CVerano eq 1>selected</cfif> >UTC-7 Tijuana/Hermosillo (Horario de Verano)</option>
								<option value="-07,0" <cfif rsZonaHoraria.Pvalor eq "-07" and CVerano eq 0>selected</cfif> >UTC-7 Mazatlán (Estándar)</option>
								<option value="-07,1" <cfif rsZonaHoraria.Pvalor eq "-07" and CVerano eq 1>selected</cfif> >UTC-6 Mazatlán (Horario de Verano)</option>
								<option value="-06,0" <cfif rsZonaHoraria.Pvalor eq "-06" and CVerano eq 0>selected</cfif> >UTC-6 México/Guadalajara (Estándar)</option>
								<option value="-06,1" <cfif rsZonaHoraria.Pvalor eq "-06" and CVerano eq 1>selected</cfif> >UTC-5 México/Guadalajara (Horario de Verano)</option>
								<option value="-05,0" <cfif rsZonaHoraria.Pvalor eq "-05" and CVerano eq 0>selected</cfif> >UTC-5 Cancún</option>
							</cfoutput>
						</select> 
					</td>
				</tr>
								
	<!---Producccion  y transformacion de inventario  --->
	<div class="IV">
		<table width="100%">
			<!---- Producción y Transformación de Inventario ---->
			<tr>
				<td colspan="3" align="center" class="listaPar"><hr size="0"><b><font size="2">Producci&oacute;n y Transformaci&oacute;n de Inventario</font></b></td>
			</tr>
			<!---- 490. Valida Producción antes que Ventas o Mantener Costo de Venta Pendientes ---->
			<cfset rsValidaProduccion = ObtenerDato(490)>
			<cfif rsValidaProduccion.RecordCount GT 0 >
				<cfset hayValidaProduccion = 1 >
				<cfset ValidaProduccion = rsValidaProduccion.Pvalor>
			<cfelse>
				<cfset ValidaProduccion = '' >
			</cfif>
			<tr>
				<td width="39%" align="right" nowrap>Mantener Costo de Ventas Pendiente:&nbsp;</td>
				<td><input type="checkbox" name="chkValidaProduccion" <cfif trim(ValidaProduccion) eq '1'>checked</cfif> ></td>
				<td rowspan="5" valign="middle">
					<!----
					<table width="100%" class="ayuda">
					<tr><td>Poner Ayuda</td></tr>
					</table>
					---->
				</td>
			</tr>

			<!---- 441. Moneda de Valuación de Inventarios ---->
			<cfquery name="rsMonedas" datasource="#Session.DSN#">
				select Mcodigo, Mnombre from Monedas
				where Ecodigo = #Session.Ecodigo#
			</cfquery>

			<cfquery name="rsMonedaEmpresa" datasource="#Session.DSN#">
				select Mcodigo from Empresas
				where Ecodigo = #Session.Ecodigo#
			</cfquery>

			<cfset rsMcodigoValuacionIV = ObtenerDato(441)>
			<cfif rsMcodigoValuacionIV.RecordCount GT 0 AND rsMcodigoValuacionIV.Pvalor NEQ "">
				<cfset hayMcodigoValuacionIV = true>
				<cfset McodigoValuacionIV = rsMcodigoValuacionIV.Pvalor>
			<cfelse>
				<cfset hayMcodigoValuacionIV = false>
				<cfset McodigoValuacionIV = rsMonedaEmpresa.Mcodigo>
			</cfif>
			<tr>
				<td width="39%" align="right" nowrap>Moneda Valuación Inventarios:&nbsp;</td>
				<td>
					<select name="McodigoValuacionIV" <cfif hayMcodigoValuacionIV> disabled </cfif>>
						<cfoutput query="rsMonedas">
							<option value="#rsMonedas.Mcodigo#" <cfif rsMonedas.Mcodigo EQ McodigoValuacionIV> selected </cfif>>#rsMonedas.Mnombre#</option>
						</cfoutput>
					</select>
				</td>
				<td rowspan="5" valign="middle">
				</td>
			</tr>
			<!---- 455. Socio para Importaciones Compras---->
						<cfset rsSocioImportacionC = ObtenerDato(455)>
							
						<cfif rsSocioImportacionC.RecordCount GT 0 >
							<cfset haySocioImportacionC = 1 >
							<cfset SocioImportacionC = rsSocioImportacionC.Pvalor>
						<cfelse>						
							<cfset SocioImportacionC = '' >
						</cfif>
						
						<tr>
							<td width="39%" align="right" nowrap>Socio de Importaci&oacute;n Compras:&nbsp;</td>
							<td>
							<cfif Len(Trim(SocioImportacionC)) GT 0>
								<cfquery name="rsNombreSocio" datasource="#session.DSN#">
									select SNnombre, SNcodigo from SNegocios
									where SNcodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#SocioImportacionC#">
                                    and Ecodigo = #session.Ecodigo#
								</cfquery>
								<cfset nombreC = rsNombreSocio.SNnombre >
							<cfelse>
								<cfset nombreC = '' >
							</cfif>
							<cfif isdefined('rsNombreSocio') and LEN(trim(rsNombreSocio.SNcodigo))>
								<cf_sifsociosnegocios2 tabindex="3" size="55" idquery="#SocioImportacionC#">								   
							<cfelse>
								<cfif haySocioImportacionC EQ 1>
									 <cf_sifsociosnegocios2 tabindex="3" size="55" idquery="#SocioImportacionC#">  
								<cfelse>
									<cf_sifsociosnegocios2 tabindex="3" SNtiposocio="P" size="55" frame="frame2">
								</cfif>								
							</cfif>								
							<input name="SocioImportacionC" type="hidden" id="SocioImportacionC" value="<cfoutput>#SocioImportacionC#</cfoutput>">
							</td>
						</tr>

			<!---- 450. Socio para Importaciones ---->
			<cfset rsSocioImportacion = ObtenerDato(450)>
			<cfif rsSocioImportacion.RecordCount GT 0 >
				<cfset haySocioImportacion = 1 >
				<cfset SocioImportacion = rsSocioImportacion.Pvalor>
			<cfelse>
				<cfset SocioImportacion = '' >
			</cfif>
			<tr>
				<td width="39%" align="right" nowrap>Socio de Importaci&oacute;n:&nbsp;</td>
				<td>
					<cfif Len(Trim(SocioImportacion)) GT 0>
						<cfquery name="rsNombreSocio" datasource="#session.DSN#">
						select SNnombre from SNegocios
						where SNcodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#SocioImportacion#">
                        and Ecodigo = #session.Ecodigo#
						</cfquery>
						<cfset nombre = rsNombreSocio.SNnombre >
					<cfelse>
						<cfset nombre = '' >
					</cfif>
					<input name="NombreSocioImportacion" type="text" value="<cfoutput>#nombre#</cfoutput>" id="NombreSocioImportacion" size="40" maxlength="80" readonly tabindex="-1">
					<a href="#" tabindex="-1"><img src="../../imagenes/Description.gif" alt="Lista de Socios de Negocio" name="imagen" width="18" height="14" border="0" align="absmiddle" onClick="javascript:doSocioImportacion();"></a>
					<input name="SocioImportacion" type="hidden" id="SocioImportacion" value="<cfoutput>#SocioImportacion#</cfoutput>">
				</td>
			</tr>

			<!---- 460. Tipo de TRansaccion CxP para Importaciones---->
			<tr>
				<cfset transaccion_cp = ''>
				<cfset trcp = ObtenerDato(460)>
				<cfif trcp.RecordCount GT 0 >
					<cfset hayTransaccionCP = 1 >
					<cfset transaccion_cp = trcp.Pvalor>
				</cfif>
				<td align="right">Transacci&oacute;n CxP para Importaciones:&nbsp;</td>
				<td>
					<select name="TransaccionCP">
						<cfoutput query="rsTransaccionCP">
						<option value="#rsTransaccionCP.CPTcodigo#" <cfif rsTransaccionCP.CPTcodigo eq transaccion_cp > selected </cfif> >#rsTransaccionCP.CPTdescripcion#</option>
						</cfoutput>
					</select>
				</td>
			</tr>

			<!---- 480. Tipo de TRansaccion CxC para Importaciones---->
			<tr>
				<cfset transaccion_cc = ''>
				<cfset trcc = ObtenerDato(480)>
				<cfif trcc.RecordCount GT 0 >
					<cfset hayTransaccionCC = 1 >
					<cfset transaccion_cc = trcc.Pvalor>
				</cfif>
				<td width="39%" align="right" nowrap>Transacci&oacute;n CxC para Importaciones:&nbsp;</td>
				<td>
					<select name="TransaccionCC">
						<cfoutput query="rsTransaccionCC">
						<option value="#rsTransaccionCC.CCTcodigo#" <cfif rsTransaccionCC.CCTcodigo eq transaccion_cc > selected </cfif> >#rsTransaccionCC.CCTdescripcion#</option>
						</cfoutput>
					</select>
				</td>
			</tr>

			<!---- 470. Tipo de TRansaccion CxC para Importaciones---->
			<tr>
				<cfset impuesto = ''>
				<cfset impu = ObtenerDato(470)>
				<cfif impu.RecordCount GT 0 >
					<cfset hayImpuesto = 1 >
					<cfset impuesto = impu.Pvalor>
				</cfif>
				<td width="39%" align="right" nowrap>Impuesto para Importaciones:&nbsp;</td>
				<td>
					<select name="Icodigo">
						<cfoutput query="rsImpuestos">
						<option value="#rsImpuestos.Icodigo#" <cfif trim(rsImpuestos.Icodigo) eq trim(impuesto) >selected</cfif> >#rsImpuestos.Idescripcion#</option>
						</cfoutput>
					</select>
				</td>
			</tr>
			<tr>
				<td width="39%" align="right" nowrap>No contabiliza el Costo de Ventas ni Ajustes:&nbsp;</td>
				<td><input type="checkbox" name="chkValorNoContaCosto"<cfif trim(ValorNoContaCosto) eq '1'>checked</cfif>></td>
			</tr>

            <!---- 5300. Máscara de Código de Articulos ---->
            <tr>
            	<cfset mascara = "">
				<cfset maskCodArt = ObtenerDato(5300)>
				<cfif maskCodArt.RecordCount GT 0 >
					<cfset hayCodigoArt = 1 >
					<cfset mascara = maskCodArt.Pvalor>
				</cfif>
				<td width="39%" align="right" nowrap>M&aacute;scara de C&oacute;digo de Articulos:&nbsp;</td>
				<td>
                	<input name="mascaraCodArt" type="text" size="20" maxlength="15"
					onKeyPress="return acceptAs(event)"
                    value="<cfoutput>#trim(mascara)#</cfoutput>">
                    <cf_notas titulo="M&aacute;scara de C&oacute;digo de Articulos" link="#helpimg#" pageIndex="0" msg = "Formato de máscara se utilizara<br />&nbsp;'*': para Letras y Números. <br />" animar="true">
                </td>
			</tr>
			<tr>
				<td width="39%" align="right" nowrap>Verificar grupo de cuentas al aplicar movimientos interalmacen:&nbsp;</td>
				<td>
                	<input type="checkbox" name="chkVerGruCtas"<cfif trim(VerGruCtas) eq '1'>checked</cfif>>
					<cf_notas titulo="Verifica Cuenta" link="#helpimg#" pageIndex="3" msg = "Verifica si esta definido en el catalogo de articulos, en las existencias,
                    la cuenta contable del almacen final, al aplicar movimientos interalmacen" animar="true">
                </td>
			</tr>
		</table>
	</div>


	<!--- Interfaces --->
	<div class="INTERPMI">
		<table width="100%">
			<tr>
				<td colspan="3" align="center" class="listaPar"><hr size="0"><b><font size="2">Interfaces</font></b></td>
			</tr>

			<!---- 590. Código de Transacción para CxP ---->
			<tr>
				<cfset transaccion_cp = ''>
				<cfset trcp = ObtenerDato(590)>
				<cfif trcp.RecordCount GT 0 >
					<cfset hayTransaccionCPInterfaz = 1 >
					<cfset transaccion_cp = trcp.Pvalor>
				</cfif>
				<td align="right">Transacci&oacute;n CxP por Defecto:&nbsp;</td>
				<td>
					<select name="CPTransInterfaz">
						<cfoutput query="rsTransaccionCP">
						<option value="#rsTransaccionCP.CPTcodigo#" <cfif rsTransaccionCP.CPTcodigo eq transaccion_cp > selected </cfif> >#rsTransaccionCP.CPTdescripcion#</option>
						</cfoutput>
					</select>
				</td>
			</tr>

			<!---- 600. Código de Transacción para CxC ---->
			<tr>
				<cfset transaccion_cc = ''>
				<cfset trcc = ObtenerDato(600)>
				<cfif trcc.RecordCount GT 0 >
					<cfset hayTransaccionCCInterfaz = 1 >
					<cfset transaccion_cc = trcc.Pvalor>
				</cfif>
				<td width="39%" align="right" nowrap>Transacci&oacute;n CxC por Defecto:&nbsp;</td>
				<td>
					<select name="CCTransInterfaz">
						<cfoutput query="rsTransaccionCC">
							<option value="#rsTransaccionCC.CCTcodigo#" <cfif rsTransaccionCC.CCTcodigo eq transaccion_cc > selected </cfif> >#rsTransaccionCC.CCTdescripcion#</option>
						</cfoutput>
					</select>
				</td>
			</tr>
			<tr>
			     <cfset LvarAsientoAut = ObtenerDato(2400)>
				<td width="39%" align="right" nowrap>Aplicar Asientos Importados Automáticamente:&nbsp;</td>
				<td>
				<input name="AsientosAutom" type="checkbox" <cfif LvarAsientoAut.Pvalor eq '1'>checked</cfif> />
				</td>
			</tr>

			<!---- FORMA DE CALCULO DE IMPUESTOS EN COMPRAS, CXP y CXC ---->
                        <cfset calculo = ObtenerDato(420)>
                        <cfif calculo.RecordCount GT 0 >
                            <cfset haycalculoImp = 1 >
                        </cfif>
                        <tr><td colspan="3" align="center" class="listaPar"><hr size="0"><b><font size="2">Manejo del Descuento a nivel de Documento para calcular Impuestos en CXC y CXP</font></b></td></tr>
                        <tr><td colspan="3" align="center"><font style="font-size:10px">(El Descuento a nivel de Línea siempre disminuye la base del Impuesto y el Ingreso)</font></td></tr>
                        <tr>
                            <td width="39%" align="right" nowrap>Forma de Cálculo del Impuesto:&nbsp;</td>
                            <td>
                                <select name="CalculoImp">
                                    <option value="1" <cfif trim(calculo.Pvalor) eq 1>selected</cfif> >Calcular Impuesto sin rebajar el Descuento Documento</option>
                                    <option value="0" <cfif trim(calculo.Pvalor) eq 0>selected</cfif> >Prorratear Descuento Documento y calcular Impuesto</option>
                                </select>
                            </td>
                        </tr>
		</table>
	</div><!--- Fin INTERPMI --->

						

	<!--- Cuentas por Cobrar  --->
	<div class="CC">
		<table width="100%">
			<!---- PARAMETROS NUEVOS RELACIONADO CON LOS CAMBIOS PEDIDOS PARA EL MODULO CxC ---->
			<tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Cuentas por Cobrar</font></b>
				</td>
			</tr>
			<!----FORMATO DE MASCARA PARA CEDULA DE SOCIO JURIDICO (MODIFICACIONES REQUERIDAS PAR CXC)---->
			<tr>
				<td align="right" nowrap>M&aacute;scara de C&eacute;dula de Socio Jur&iacute;dico:&nbsp;</td>
				<td>
					<input name="mascaraCedJ" type="text" size="50" maxlength="100"
						onChange="javascript: validaCedula();"
						onKeyPress="return acceptX(event)"
						value="<cfoutput>#maskCedJ.Pvalor#</cfoutput>">
				</td>
				<td width="20%">&nbsp;</td>
			</tr>
			<!---- FORMATO DE MASCARA PARA CEDULA DE SOCIO FISICO (MODIFICACIONES REQUERIDAS PAR CXC)---->
			<tr>
				<td align="right" nowrap>M&aacute;scara de C&eacute;dula de Socio F&iacute;sico:&nbsp;</td>
				<td>
					<input name="mascaraCedF" type="text" size="50" maxlength="100"
						onChange="javascript: validaCedula();"
						onKeyPress="return acceptX(event)"
						value="<cfoutput>#maskCedF.Pvalor#</cfoutput>">
				</td>
				<td width="20%">&nbsp;</td>
			</tr>
			<!---- FORMATO DE MASCARA PARA CEDULA DE SOCIO EXTRANJEROS (MODIFICACIONES REQUERIDAS PAR CXC)---->
			<tr>
				<td align="right" nowrap>M&aacute;scara de C&eacute;dula de Socio Extranjero:&nbsp;</td>
				<td>
					<input name="mascaraCedE" type="text" size="50" maxlength="100"
						onChange="javascript: validaCedula();"
						onKeyPress="return acceptX(event)"
						value="<cfoutput>#maskCedE.Pvalor#</cfoutput>">
				</td>
				<td width="20%">&nbsp;</td>
			</tr>
			<tr>
			<tr>
				<td align="right" nowrap>Mostrar Clasificaci&oacute;n:&nbsp;</td>
				<td>
					<input name="VerClasificaCC" type="checkbox" <cfif ValorVerClasifCC eq 1>checked</cfif>  >
				</td>
				<td width="20%">&nbsp;</td>
			</tr>
			<!---- Clasificación a utilizar en el reporte de Consolidado Contable ---->
			<tr>
				<td align="right" nowrap>Clasificación Reporte Consolidado Contable</td>
				<td>

					<cfif isdefined('LvarSNCEid') and LvarSNCEid.recordcount gt 0>
						<cfquery name="rsClasif" datasource="#session.DSN#">
							select SNCEid, SNCEcodigo, SNCEdescripcion
							from SNClasificacionE
							where #filtroE#
							  and SNCEid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#LvarSNCEid.Pvalor#">
					</cfquery>
						<cf_sifSNClasificacion form="form1" tabindex="1" query="#rsClasif#">
					<cfelse>
						<cf_sifSNClasificacion form="form1" tabindex="1">
					</cfif>
				</td>
				<td width="20%">&nbsp;</td>
			</tr>	

						<cfset hayDescDoc = 0>
						<cfset descDoc = ObtenerDato(421)>
						<cfif descDoc.RecordCount GT 0 >
							<cfset hayDescDoc = 1 >
						</cfif>

						<tr>
							<td align="right" nowrap>Forma de Contabilizar Descuento Documento:&nbsp;</td>
							<td>
								<select name="descDoc">
									<option value="D" <cfif trim(descDoc.Pvalor) eq "D" >selected</cfif> >Utilizar Cuenta de Descuentos de CxC</option>
									<option value="I" <cfif trim(descDoc.Pvalor) eq "I" >selected</cfif> >Prorratear Descuento Documento y Disminuir Cuenta Ingreso</option>
								</select>
							</td>
						</tr>
			<!----Concatenar Socio de Negocio a las lineas de las Facturas de CXC y CXP---->
			<tr>
				<td align="right" nowrap>Incluir Socio de Negocios en las líneas de la póliza Contable:&nbsp;</td>
				<td><input name="SNPoliza" value="1" type="checkbox" <cfif SNPoliza eq 1>checked</cfif>></td>
			</tr>

            <tr>
              <td align="right" nowrap>Leyenda para Estados de Cuenta CxC:&nbsp;</td>
              <td><input name="verLeyendaCxC" type="text" size="70" maxlength="100" value="<cfoutput>#HTMLEditFormat(leyendaCxC.Pvalor)#</cfoutput>"></td>
            </tr>

			<cfset TransPagoCanc = ObtenerDato(2930)>
			<cfif TransPagoCanc.RecordCount GT 0 >
				<cfset hayTPCancelacion = 1 >
                <cfset TPCancelacion = TransPagoCanc.Pvalor>
            <cfelse>
                <cfset TPCancelacion = '' >
            </cfif>
			<cf_dbfunction name="string_part"  args="a.CCTdescripcion, 1, 10" returnvariable="dato">
            <cfquery name="rsTransCanc" datasource="#Session.DSN#">
                select 	a.CCTcodigo,
                	case when coalesce(a.CCTvencim,0) < 0 then
						<cf_dbfunction name="concat" args="#dato#+' (contado)'" delimiters="+">
                	else
                    	<cf_dbfunction name="string_part"  args="a.CCTdescripcion, 1, 20">
                	end as CCTdescripcion,
                	case when coalesce(a.CCTvencim,0) >= 0 then 1 else 2 end as CCTorden,
                	a.CCTtipo
               	from CCTransacciones a
                where a.Ecodigo = #Session.Ecodigo#
                and a.CCTpago = 1
            </cfquery>

			<tr>
				<td align="right" nowrap>Transacci&oacute;n de Pago para Cancelaci&oacute;n de Documentos:&nbsp;</td>
				<td>
					<select name="TPC_CCTcodigo">
                        <option value=""></option>
                        <cfoutput query="rsTransCanc">
                            <option value="#CCTcodigo#" <cfif TPCancelacion EQ rsTransCanc.CCTcodigo>selected</cfif>>#CCTdescripcion#</option>
                        </cfoutput>
                    </select>
				</td>
			</tr>

			<!--- manejo de descuento a nivel de documento para calcular impuestos en CxC y CxP --->
			<div class="CC">
			<tr>
				<td colspan="2" width="100%">
					<table width="100%">
						<!---- FORMA DE CALCULO DE IMPUESTOS EN COMPRAS, CXP y CXC ---->
						<cfset calculo = ObtenerDato(420)>
						<cfif calculo.RecordCount GT 0 >
							<cfset haycalculoImp = 1 >
						</cfif>
						<tr>
							<td colspan="3" align="center" class="listaPar"><hr size="0"><b><font size="2">Manejo del Descuento a nivel de Documento para calcular Impuestos en CXC y CXP</font></b></td>
						</tr>
						<tr>
							<td colspan="3" align="center"><font style="font-size:10px">(El Descuento a nivel de Línea siempre disminuye la base del Impuesto y el Ingreso)</font></td>
						</tr>
						<tr>
							<td align="right" nowrap>Forma de Cálculo del Impuesto:&nbsp;</td>
							<td>
								<select name="CalculoImp">
									<option value="1" <cfif trim(calculo.Pvalor) eq 1>selected</cfif> >Calcular Impuesto sin rebajar el Descuento Documento</option>
									<option value="0" <cfif trim(calculo.Pvalor) eq 0>selected</cfif> >Prorratear Descuento Documento y calcular Impuesto</option>
								</select>
							</td>
						</tr>
						<cfset hayDescDoc = 0>
						<cfset descDoc = ObtenerDato(421)>
						<cfif descDoc.RecordCount GT 0 >
							<cfset hayDescDoc = 1 >
						</cfif>

						<tr>
							<td align="right" nowrap>Forma de Contabilizar Descuento Documento:&nbsp;</td>
							<td>
								<select name="descDoc">
									<option value="D" <cfif trim(descDoc.Pvalor) eq "D" >selected</cfif> >Utilizar Cuenta de Descuentos de CxC</option>
									<option value="I" <cfif trim(descDoc.Pvalor) eq "I" >selected</cfif> >Prorratear Descuento Documento y Disminuir Cuenta Ingreso</option>
								</select>
							</td>
						</tr>
						
						<!---- SML02052022 Muestra la Cuenta Contable de Descuentos---->
						<cfset cuentaDescuentos = ObtenerDato(70)>
						<cfif cuentaDescuentos.RecordCount gt 0 and len(trim(cuentaDescuentos.Pvalor))>
							<cfquery name="rscuentaDescuentos" datasource="#session.DSN#">
								select Ccuenta, Cmayor, Cformato, Cdescripcion
								from CContables
								where Ccuenta = <cfqueryparam cfsqltype="cf_sql_numeric" value="#cuentaDescuentos.Pvalor#">
							</cfquery>
							<cfset haycuentaDescuentos = 1 >
						<cfelse>
							<cfset haycuentaDescuentos = 0 >
						</cfif>

						<tr>
							<td align="right" nowrap>Cuenta Contable para Descuento Documento:&nbsp;</td>
							<td>
								<cfif isdefined("rscuentaDescuentos") and rscuentaDescuentos.RecordCount gt 0>
									<cf_cuentas frame="frcuentaDescuentos" Ccuenta="CDescuentos" Cformato="FCDescuentos" Cmayor="MCDescuentos" Cdescripcion="DCDescuentos" form="form1" query="#rscuentaDescuentos#" >
									<cfelse>
									<cf_cuentas frame="frcuentaDescuentos" Ccuenta="CDescuentos" Cformato="FCDescuentos" Cmayor="MCDescuentos" Cdescripcion="DCDescuentos" form="form1" >
								</cfif>
							</td>
						</tr>
					</table>
				</td>
			</tr>
			</div>
		</table>
	</div><!--- Fin CC --->


	<!--- Cuentas por pagar --->
	<div class="CP">
		<table width="100%">
			<!---- PARAMETROS NUEVOS RELACIONADO CON LOS CAMBIOS PEDIDOS PARA EL MODULO CxP ---->
			<tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Cuentas por Pagar</font></b>
				</td>
			</tr>
			<tr>
				<td align="right" nowrap>Mostrar Clasificaci&oacute;n:&nbsp;</td>
				<td>
					<input name="VerClasificaCP" type="checkbox" <cfif ValorVerClasifCP eq 1>checked</cfif>>
				</td>
				<td width="20%">&nbsp;</td>
			</tr>

			<cfset rsRecurrentes = ObtenerDato(880)>
			<cfset hayRecurrentes = 0 >
			<cfif rsRecurrentes.recordcount gt 0 >
				<cfset hayRecurrentes = 1 >
			</cfif>
			<tr>
				<td align="right" nowrap>No permitir el mismo documento recurrente para varias facturas en un mismo per&iacute;odo y mes:&nbsp;</td>
				<td>
					<input name="chkRecurrentes" type="checkbox" <cfif not (rsRecurrentes.Pvalor eq 0) >checked</cfif>>
				</td>
				<td width="20%">&nbsp;</td>
			</tr>

			<tr>
				<td align="right">Tipo de Movimiento Bancario Pago de Renta</td>
                <td><cf_dbfunction name="OP_CONCAT" returnvariable="_Cat">
					<cfset Lvar_BTidPR = ObtenerDato(2000).Pvalor>
					<cfquery name="rsBTransacciones" datasource="#session.dsn#">
						select BTid, BTcodigo #_Cat#' - '#_Cat# BTdescripcion Descripcion
						from BTransacciones
						where Ecodigo = #Session.Ecodigo#
                        order by BTid
					</cfquery>
					<select name="BTidPR">
						<cfoutput query="rsBTransacciones">
							<option value="#rsBTransacciones.BTid#"
								<cfif Lvar_BTidPR eq rsBTransacciones.BTid>
									selected
								</cfif>
							>
								#rsBTransacciones.Descripcion#
							</option>
						</cfoutput>
					</select>
				</td>
			</tr>

	<!---- ---->
			 <tr>
				<td><div align="right">Oficina Estimación de Avance:&nbsp;</div></td>
				<cfset Lvar_Oficina = ObtenerDato(2900).Pvalor>
				<cfquery name="rsOficinas" datasource="#Session.DSN#">
					select Ocodigo, Odescripcion from Oficinas
					where Ecodigo = #Session.Ecodigo#
					order by Ocodigo
				</cfquery>

				<td><select name="Ocodigo" tabindex="1">
					 <cfoutput query="rsOficinas">
							<option value="#rsOficinas.Ocodigo#"
								<cfif Lvar_Oficina eq rsOficinas.Ocodigo>
									selected
								</cfif>
							>
								#rsOficinas.Odescripcion#
							</option>
						</cfoutput>
					</select>
				</td>
			</tr>

			 <tr>
				<td><div align="right">Tipo de Transacción para la Estimación de Avance:&nbsp;</div></td>
				<cfset Lvar_Transaccion = ObtenerDato(2910).Pvalor>
				<cfquery name="rsTransac" datasource="#Session.DSN#">
					select CPTcodigo, CPTdescripcion , CPTtipo
					from CPTransacciones
					where Ecodigo= #session.Ecodigo#
					and CPTpago = 0
					and NOT CPTdescripcion like '%Tesorer_a%'
					and CPTtipo = 'C'
				</cfquery>

				<td><select name="Transaccion" tabindex="1">
					 <cfoutput query="rsTransac">
							<option value="#rsTransac.CPTcodigo#"
								<cfif Lvar_Transaccion eq rsTransac.CPTcodigo>
									selected
								</cfif>
							>
								#rsTransac.CPTcodigo#
							</option>
						</cfoutput>
					</select>
				</td>
			</tr>
            <!----Mostrar Cuenta Contable en Detalles de Registro Factura ---->
            <cfset LvarCCRegistroFacturas = ObtenerDato(891).Pvalor>
			<tr>
				<td align="right" nowrap>Mostrar Cuenta Contable en Detalles de Registro Factura:&nbsp;</td>
				<td>
					<input name="chkCCRegistroFacturas" type="checkbox" <cfif LvarCCRegistroFacturas eq 1>checked</cfif>>
                    <cf_notas titulo="Muestra Cuenta Contable" link="#helpimg#" pageIndex="1" msg = "Muestra la cuenta contable en Registro de Facturas" animar="true">
				</td>
				<td width="20%">&nbsp;</td>
			</tr>
			<!----►►Realizar adquisición de activos fijos, posterior a la aplición de facturas◄◄---->
            	<cfset rsPostAdqAF = ObtenerDato(15600)>
            <cfif NOT rsPostAdqAF.RecordCount>
            	<cfset LvarPostAdqAF = 0>
            <cfelse>
            	<cfset LvarPostAdqAF = rsPostAdqAF.Pvalor>
            </cfif>
            <tr>
            	<td align="right">Realizar adquisición de activos fijos, posterior a la aplición de facturas:</td>
				<td>
					<input name="chkPostAdqAF" value="1" type="checkbox" <cfif LvarPostAdqAF eq 1>checked</cfif>>
                    <cf_notas titulo="Postergar Adquisición AF" link="#helpimg#" pageIndex="5" msg = "Si se marca esta opción, las facturas no realizaran adquisiciones de Activos, ni validara la existencia de Documentos de responsabilidad en el momento de la aplicación de la misma. La Adquisición y búsqueda de Documentos se realizara desde Activos Fijos." animar="true">
				</td>
                <td width="20%">&nbsp;</td>
            </tr>
			<!----►►Realizar cambios de la cuenta e item a los detalles de las Facturas CxP provenientes de Orden de Compra◄◄---->
            <cfset rsDETCXP = ObtenerDato(15650)>
            <cfif NOT rsDETCXP.RecordCount>
            	<cfset LvarDETCXP = 0>
            <cfelse>
            	<cfset LvarDETCXP = rsDETCXP.Pvalor>
            </cfif>
            <tr>
            	<td align="right">Realizar cambios en los item y cuentas de los detalles de las Facturas de CxP:</td>
				<td>
					<input name="chkDETCXP" value="1" type="checkbox" <cfif LvarDETCXP eq 1>checked</cfif>>
                    <cf_notas titulo="Cambiar Detalle CxP" link="#helpimg#" pageIndex="6" msg = "Si se marca esta opcion, se podra realizar el cambio de los item y las cuentas de los detalles de las Facturas de Cuentas por Pagar" animar="true">
				</td>
                <td width="20%">&nbsp;</td>
            </tr>
			<!---- Valida fecha arribo ---->
			<!----►►Realizar cambios de la cuenta e item a los detalles de las Facturas CxP provenientes de Orden de Compra◄◄---->
            <cfset rsOmitirValFechaArribo = ObtenerDato(00190401)>
            <cfif NOT rsOmitirValFechaArribo.RecordCount>
            	<cfset LvarOmitirValFechaArribo = 0>
            <cfelse>
            	<cfset LvarOmitirValFechaArribo = rsOmitirValFechaArribo.Pvalor>
            </cfif>
			<!---- Eduardo González (27-02-2018): Omitir validacion de fecha de arribo ---->
			<tr>
            	<td align="right">Omitir validaci&oacute;n en Fecha de arribo:</td>
				<td>
					<input name="chkOmitirValFechaArribo" value="1" type="checkbox" <cfif LvarOmitirValFechaArribo eq 1>checked</cfif>>
                    <cf_notas titulo="Omitir validaci&oacute;n en Fecha de arribo" link="#helpimg#" pageIndex="7" msg = "Si se marca esta opci&oacute;n, ya no se validar&aacute; que la fecha de arribo sea mayor a la fecha de la factura. Adem&aacute;s se calcular&aacute; la fecha de vencimiento con base a la de arribo." animar="true">
				</td>
                <td width="20%">&nbsp;</td>
			</tr>
			<!---- CxC-  Credenciales para consulta de Api de SIC---->
			<cfset  rsUrlBase= ObtenerDato(191001)>
			<cfset  rsUserWS= ObtenerDato(191002)>
			<cfset  rsPasswordWS= ObtenerDato(191003)>
			<cfset  rsApiKeyWS= ObtenerDato(191004)>
            
			<tr>
				<td align="right">URL Base Servicio de Portal de Proveedores SIC:</td>
				<td><input name="urlBase" value="<cfoutput>#rsUrlBase.Pvalor#</cfoutput>" type="text" size="50" maxlength="100"></td>
			</tr>
			<tr>
				<td align="right">Usuario Servicio de Portal de Proveedores SIC:</td>
				<td><input name="userWS" value="<cfoutput>#rsUserWS.Pvalor#</cfoutput>" type="text" size="20" maxlength="50"></td>
			<tr>
			</tr>
				<td align="right">Contraseña Servicio de Portal de Proveedores SIC:</td>
				<td><input name="passwordWS" value="<cfoutput>#rsPasswordWS.Pvalor#</cfoutput>" type="text" size="20" maxlength="50"></td>
			<tr>
			<tr>
				<td align="right">Apikey Servicio de Portal de Proveedores SIC:</td>
				<td><input name="apiKeyWS" value="<cfoutput>#rsApiKeyWS.Pvalor#</cfoutput>" type="text" size="50" maxlength="100"></td>
			</tr>
			<!----  Aplicación de Documentos  ---->
			<cfset  rsapd_enc= ObtenerDato(191005)>
			<cfset  rsapd_det= ObtenerDato(191006)>
			<tr>
				<td align="right"> Aplicación de Documentos Transacciones Encabezado:</td>
				<td colspan="2"><input name="apd_enc" value="<cfoutput>#rsapd_enc.Pvalor#</cfoutput>" type="text" size="50" maxlength="100">
					(Por default se tiene NC; agregar transacciones separadas por comas HP,FG...)
				</td>
			</tr>
			<tr>
				<td align="right"> Aplicación de Documentos Transacciones Detalle:</td>
				<td colspan="2"><input name="apd_det" value="<cfoutput>#rsapd_det.Pvalor#</cfoutput>" type="text" size="50" maxlength="100">
				(Agregar transacciones separadas por comas HP,FG...)
				</td>
			</tr>
			<cfset rolCancelaCxP = ObtenerDato(5004)>
			<cfquery name="rolesCxP" datasource="asp" >
				select SRcodigo, SRdescripcion
				from SRoles
				where SScodigo='SIF'
			</cfquery>
			<tr>
				<td align="right">Rol para la Cancelaci&oacute;n de Documentos CxP:</td>
				<td>
					<select name="rolCancelaCxP">
						<option value="" >-seleccionar-</option>
						<cfoutput query="rolesCxP">
							<option value="#rolesCxP.SRcodigo#"<cfif trim(rolCancelaCxP.Pvalor) eq trim(rolesCxP.SRcodigo) >selected</cfif>>#rolesCxP.SRdescripcion#</option>
						</cfoutput>
					</select>
				</td>
			</tr>
			<cfset montoComPago = ObtenerDato(5005)>
			<tr>
				<td align="right">Tolerancia para agregar un Complemento de Pago:</td>
				<td>
					<input type="number" id="montoComPago" name="montoComPago" value = "<cfoutput>#montoComPago.Pvalor#</cfoutput>" step="0.01">
				</td>
			</tr>
			<tr>
				<cfset rsLN= ObtenerDato(1381)>
				<td align="right">URL WebService Lista Negra:</td>
				<td><input name="urlLN" value="<cfoutput>#rsLN.Pvalor#</cfoutput>" type="text" size="50" maxlength="100"></td>				
			</tr>
			<tr>
				<cfset rsCK= ObtenerDato(1382)>
				<td align="right">RFC Cliente Konesh:</td>
				<td><input name="RFCCK" value="<cfoutput>#rsCK.Pvalor#</cfoutput>" type="text" size="15" maxlength="15"></td>				
			</tr>
			
            <cfset ListaNegraAct = ObtenerDato(1383)>
            <cfif NOT ListaNegraAct.RecordCount>
            	<cfset LvarListaNegra = 0>
            <cfelse>
            	<cfset LvarListaNegra = ListaNegraAct.Pvalor>
            </cfif>			
			<tr>
            	<td align="right">Activar Validaci&oacute;n de Lista Negra:</td>
				<td>
					<input name="chkListaNegra" value="1" type="checkbox" <cfif LvarListaNegra eq 1>checked</cfif>>
                    <cf_notas titulo="Activar Validaci&oacute;n de Lista Negra" link="#helpimg#" pageIndex="8" msg = "Si se marca esta opci&oacute;n, se verificara si est&aacute; en la lista negra." animar="true">
				</td>
                <td width="20%">&nbsp;</td>
			</tr>

		

			<cfset RemisionAct = ObtenerDato(1711)>			
            <cfif NOT RemisionAct.RecordCount>
            	<cfset LvarRemision = 0>
            <cfelse>
            	<cfset LvarRemision = RemisionAct.Pvalor>
            </cfif>	
				
			<tr>
            	<td align="right">Activar recepci&oacuten de remisiones de interfaz LDCOM:</td>
				<td>
					<input name="chkRemision" value="1" type="checkbox" <cfif LvarRemision eq 1>checked</cfif>>
                    <cf_notas titulo="Activar recepci&oacuten de remisiones de interfaz LDCOM" link="#helpimg#" pageIndex="9" msg = "Si se marca esta opci&oacute;n, se verificar&aacute si se recibir&aacuten Remisiones de LDCOM." animar="true">
				</td>
                <td width="20%">&nbsp;</td>
			</tr>

		</table>
	</div><!--- Fin del CP --->



	<!--- Bancos --->
	<div class="MB">
		<table width="100%">
			<!----BANCOS ---->
			<tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Bancos</font></b>
				</td>
			</tr>

			<cfquery name="btransacciones" datasource="#session.DSN#">
				select BTid, BTcodigo, BTdescripcion, BTtipo
				from BTransacciones
				where Ecodigo=#Session.Ecodigo#
			</cfquery>
			<tr>
				<cfset rsMBOrigen = ObtenerDato(160)>
				<cfset origenmb = ''>
				<cfset hayMBOrigen = 0 >
				<cfif rsMBOrigen.RecordCount GT 0 >
					<cfset hayMBOrigen = 1 >
					<cfset origenmb = rsMBOrigen.Pvalor>
				</cfif>
				<td width="39%" align="right" nowrap>Tipo de movimiento origen para transferencias bancarias:&nbsp;</td>

				<td width="35%">
					<select name="origenmb">
						<cfif rsMBOrigen.RecordCount EQ 0>
						<option value="-1"></option>
						</cfif>
						<cfoutput query="btransacciones">
							<cfif btransacciones.BTtipo eq 'C'>
							<option value="#btransacciones.BTid#" <cfif trim(btransacciones.BTid) eq trim(origenmb)> selected </cfif> >#trim(btransacciones.BTcodigo)# - #btransacciones.BTdescripcion#</option>
							</cfif>
						</cfoutput>
					</select>
				</td>
				<td CNTPspan="4" valign="middle">
				<!----
				<table width="100%" class="ayuda">
				<tr><td>Poner Ayuda</td></tr>
				</table>
				---->
				</td>
			</tr>

			<tr>
				<cfset rsMBDestino = ObtenerDato(170)>
				<cfset destinomb = ''>
				<cfset hayMBDestino = 0 >
				<cfif rsMBDestino.RecordCount GT 0 >
					<cfset hayMBDestino = 1 >
					<cfset destinomb = rsMBDestino.Pvalor>
				</cfif>
				<td width="39%" align="right" nowrap>Tipo de movimiento destino para transferencias bancarias:&nbsp;</td>

				<td width="35%">
					<select name="destinomb">
						<cfif rsMBDestino.RecordCount EQ 0>
						<option value="-1"></option>
						</cfif>
						<cfoutput query="btransacciones">
							<cfif btransacciones.BTtipo eq 'D'>
							<option value="#btransacciones.BTid#" <cfif trim(btransacciones.BTid) eq trim(destinomb)> selected </cfif> >#trim(btransacciones.BTcodigo)# - #btransacciones.BTdescripcion#</option>
							</cfif>
						</cfoutput>
					</select>
				</td>
				<td CNTPspan="4" valign="middle">
				<!----
				<table width="100%" class="ayuda">
				<tr><td>Poner Ayuda</td></tr>
				</table>
				---->
				</td>
			</tr>

			<!----CUENTA POR TRANSACCION DE SOCIO DE NEGOCIOS---->
			<cfset UtilCtaTrans= ObtenerDato(200030)>
            <tr>
				<td align="right">Utilizar cuenta en Transacción del Socio de Negocios para Movimientos Bancarios:&nbsp;</td>
				<td>
					<input name="UtilCtaTrans" type="checkbox" <cfif UtilCtaTrans.Pvalor eq 1>checked</cfif>>
				</td>
			</tr>

            	<!----Movimientos de Flujo de Efectivo---->
                 <!----Agregado por RVD 15/05/2014---->
			<cfset MovFlujoEfectivo= ObtenerDato(200035)>
            <tr>
				<td align="right">Movimientos Bancarios con Flujo de Efectivo:&nbsp;</td>
				<td>
					<input name="MovFlujoEfectivo" type="checkbox" <cfif MovFlujoEfectivo.Pvalor eq 1>checked</cfif>>
				</td>
			</tr>
		</table>
	</div><!--- Fin MB --->



	<!---Tesoreria  --->
	<div class="TES">
		<table width="100%" border="0">
			<!----TESORERIA---->
			<tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Tesorer&iacute;a</font></b>
				</td>
			</tr>
			<tr>
				<td align="right" width="50%">Emitir pago de cheques en la impresión o en la entrega:&nbsp;&nbsp;
				</td>

				<td>
	                <cfset ValorTESemitPag = ObtenerDato(5001)>
	                <cfif  len(trim(ValorTESemitPag.Pvalor)) eq 0>
	                 <cfset ValorEmitPag = 0>
	                <cfelse>
	                  <cfset ValorEmitPag = ValorTESemitPag.Pvalor>
	                </cfif>

					<input type="hidden" name="EmitPago" value="<cfif ValorTESemitPag.recordcount eq 0>0 <cfelse> 1 </cfif>"/>

					<select name="TESemitPag">
						<option value="0"
					          <cfif   ValorTESemitPag.recordcount gt 0 and ValorTESemitPag.Pvalor eq ''> selected </cfif> >
	               			En impresión
	               		</option>
		                <option value="1"
						          <cfif  ValorTESemitPag.Pvalor eq 1> selected</cfif>   >
		               		En Entrega
		               	</option>
	                </select>
				</td>
			</tr>

			<!---- TIPO DE CUENTA PARA DISPERSIÓN DE PAGOS ---->
			<tr>
				<td align="right" width="50%">Tipo de cuenta para Dispersi&oacute;n de pagos (Bancomer):&nbsp;&nbsp;
				</td>

				<td>
	                <cfset lVarGetTipoCta_5002  = ObtenerDato(5002)>
	                <!---- Por default se tomará el numero 40 (Cuenta clabe) ---->
	                <cfset lValorTipoCta = "40">

	                <cfif lVarGetTipoCta_5002.recordcount GT 0 AND TRIM(lVarGetTipoCta_5002.Pvalor) NEQ "">
		                <cfset lValorTipoCta = #TRIM(lVarGetTipoCta_5002.Pvalor)#>
					</cfif>

					<cfif lVarGetTipoCta_5002.recordcount EQ 0>
		                <input type="hidden" name="lVarExistCta" value="1"/>
					</cfif>


					<select name="cboValorTipoCta">
						<option value="03"
					          <cfif lValorTipoCta eq '03'> selected </cfif>>
	               			  Tarjeta de D&eacute;bito
	               		</option>
		                <option value="40"
		                	  <cfif lValorTipoCta eq '40'> selected </cfif>>
			               	  Cuenta Clabe
		               	</option>
	                </select>
				</td>
			</tr>
			<!---- Validar UUID de Facturas vigentes ---->
			<tr>
				<td align="right" width="50%">Validar Facturas antes del pago:&nbsp;&nbsp;
				</td>
				<td>
	                <cfset lvarValidaUUID_5003  = ObtenerDato(5003)>
	                <!---- Por default se tomará el numero 40 (Cuenta clabe) ---->
	                <cfset lValorValidaUUID = "0">
	                <cfif lvarValidaUUID_5003.recordcount GT 0 AND TRIM(lvarValidaUUID_5003.Pvalor) NEQ "">
		                <cfset lValorValidaUUID = #TRIM(lvarValidaUUID_5003.Pvalor)#>
					</cfif>
					<cfif lvarValidaUUID_5003.recordcount EQ 0>
		                <input type="hidden" name="lVarExistValidaUUID" value="1"/>
					</cfif>
					<select name="cboValidaUUID">
						<option value="0"
					          <cfif lValorValidaUUID eq "0"> selected </cfif>>
	               			  No
	               		</option>
		                <option value="1"
		                	  <cfif lValorValidaUUID eq "1"> selected </cfif>>
			               	  Si
		               	</option>
	                </select>
				</td>
			</tr>
			<!---- Inicio Cambio OPARRALES 2017-10-25---->
			<tr>
				<td align="right">Filtrar Tipo de Gasto por Empleado:&nbsp;</td>

				<cfset valorTGEmp = ObtenerDato(14200701).Pvalor eq '' ?  0 : ObtenerDato(14200701).Pvalor>
				<td>&nbsp;
					<input type="checkbox" name="chkTGE" <cfif valorTGEmp eq 1>checked</cfif>>
				</td>
			</tr>
			<!---- Fin Cambio OPARRALES ---->
			<!---- INICIO: Eduardo González Sarabia 2019-02-26 ---->
			<tr>
				<cfset rsGetAutorizadorTcd = ObtenerDato(00380401)>
				<cfif rsGetAutorizadorTcd.RecordCount GT 0 >
					<cfset hayAutorizadorTcdCheck = 1 >
				</cfif>
				<td align="right" nowrap>Autorizar Modificaciones a Cuentas Destino para Pago:&nbsp;</td>
				<td>
					<input type="checkbox" name="chkAutorizadorTcd" value="1" <cfif rsGetAutorizadorTcd.Pvalor EQ '1'> checked</cfif>>
				</td>
			</tr>
			<tr>
				<cfset rsGetRolAutorizadorTcd = ObtenerDato(00380402)>
				<cfset lvarFiltroTcd = "SScodigo='SIF'">
				<cfif rsGetRolAutorizadorTcd.RecordCount GT 0 >
					<cfset hayRolAutorizadorTcd = 1 >
					<cfset SRcodigoTcd = rsGetRolAutorizadorTcd.Pvalor>
					<cfset lvarFiltroTcd = lvarFiltroTcd & " AND SRcodigo = '#rsGetRolAutorizadorTcd.Pvalor#'">
				</cfif>
				<td align="right" nowrap>Rol para Autorizar Modificaciones a Cuentas Destino para Pago:&nbsp;</td>
				<td>
					<cf_conlis
					conexion="asp"
					Campos="SRcodigoTcd, SRdescripcionTcd"
					Asignar="SRcodigoTcd, SRdescripcionTcd"
					Size="10,40"
					Desplegables="S,S"
					Modificables="S,S"
					Title="Roles del sistema SIF"
					Tabla= "SRoles"
					Columnas="SRcodigo AS SRcodigoTcd, SRdescripcion AS SRdescripcionTcd"
					filtrar_por="SRcodigo, SRdescripcion"
					Desplegar="SRcodigoTcd, SRdescripcionTcd"
					Etiquetas="Codigo, Descripción"
					Formatos="S,S"
					Align="left,left"
					form="form1"
					debug="false"
					Asignarformatos="S,S"
					filtro="SScodigo='SIF'"
					traerinicial="true"
					traerfiltro="#lvarFiltroTcd#"
					EmptyListMsg=" --- Sin registros --- "/>
				</td>
			</tr>
			<!---- FIN: Eduardo González Sarabia 2019-02-26 ---->
		</table>
	</div><!--- Fin TES  --->


	<!----ACTIVOS FIJOS---->
	<div class="AF">
		<table width="100%">
			<!----ACTIVOS FIJOS---->
			<tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Activos Fijos</font></b>
				</td>
			</tr>
			<tr>
				<td align="right">Diferencia de Meses de Saldos para Reevaluaci&oacute;n:&nbsp;</td>
				<td>
					<!---- Este parámetro es solo visible, no modificable, porque no se puede cambiar en ningún momento, cuando se
					realiza la implementación del sistema se deja listo. ---->
					<cfoutput>#ObtenerDato(900).Pvalor#</cfoutput>
				</td>
			</tr>

			<tr>
				<td align="right">Control de Transacciones de Control de Responsables:&nbsp;</td>
				<td>
					<!---- Este parámetro es solo visible, no modificable, porque no se puede cambiar en ningún momento, cuando se
					realiza la implementación del sistema se deja listo. ---->
					<cfif ObtenerDato(970).Pvalor eq 1>Activo<cfelse>Inactivo</cfif>
				</td>
			</tr>

			<tr>
				<td align="right">Tipo de Documento para Vales de Adqusici&oacute;n:&nbsp;</td>
				<td>
					<cfset Lvar_CRTDid = ObtenerDato(910).Pvalor>
					<cfquery name="rsCRTipoDocumento" datasource="#session.dsn#">
						select CRTDid, CRTDcodigo, CRTDdescripcion
						from CRTipoDocumento
						where Ecodigo = #Session.Ecodigo#
					</cfquery>
					<select name="P910">
						<cfoutput query="rsCRTipoDocumento">
							<option value="#rsCRTipoDocumento.CRTDid#"
								<cfif Lvar_CRTDid eq rsCRTipoDocumento.CRTDid>
									selected
								</cfif>
							>
								#rsCRTipoDocumento.CRTDcodigo# - #rsCRTipoDocumento.CRTDdescripcion#
							</option>
						</cfoutput>
					</select>
				</td>
			</tr>

			<tr>
				<td align="right">Asiento Contable de deprecici&oacute;n detallado:&nbsp;</td>
				<td>
					<input name="VerAstDet" type="checkbox" <cfif ValorVerAstDet eq 1>checked</cfif>>
				</td>
			</tr>


			<tr>
				<cfset rsMeses = ObtenerDato(940)>
				<cfset destinomeses = ''>
				<cfset haymeses = 0 >
				<cfif rsMeses.RecordCount GT 0 >
					<cfset haymeses = 1 >
					<cfset destinomeses = rsMeses.Pvalor>
				</cfif>

				<td align="right" nowrap>Meses para calcular Fecha de Inicio de Depreciación:&nbsp;</td>

				<td>
					<select name="cbomesesdep">
						<option value=""></option>
						<cfloop from="1" to="12" step="1" index="ind">
							<option value="<cfoutput>#ind#</cfoutput>" <cfif trim(ind) eq trim(destinomeses)> selected </cfif> ><cfoutput>#ind#</cfoutput></option>
						</cfloop>
					</select>
				</td>
			</tr>
			<tr>
				<cfset rsMeses1 = ObtenerDato(950)>
				<cfset destinomeses1 = ''>
				<cfset haymeses1 = 0 >
				<cfif rsMeses1.RecordCount GT 0 >
					<cfset haymeses1 = 1 >
					<cfset destinomeses1 = rsMeses1.Pvalor>
				</cfif>
				<td align="right" nowrap>Meses para calcular Fecha de Inicio de Revaluación:&nbsp;</td>

				<td>
					<select name="cbomesesrev">
						<option value=""></option>
						<cfloop from="1" to="12" step="1" index="ind">
							<option value="<cfoutput>#ind#</cfoutput>" <cfif trim(ind) eq trim(destinomeses1)>selected="selected"</cfif>><cfoutput>#ind#</cfoutput></option>
						</cfloop>
					</select>
				</td>
			</tr>
			<tr>
				<td align="right">Considerar traslados, retiros y Revaluaciónes del último periodo en la Revaluación:&nbsp;</td>
				<td>
					<input name="ProRev" type="checkbox" <cfif ProRev eq 1>checked</cfif>>
				</td>
			</tr>
			<tr>
				<td align="right">Aplicar retiros automáticamente:&nbsp;</td>
				<td>
					<input name="RetAuto" type="checkbox" <cfif ValorRetAuto eq 1>checked</cfif>>
				</td>
			</tr>

			<tr>
				<td align="right">Traspasos de documentos en autogestion autorizados por el jefe del Centro de Custodia:&nbsp;</td>
				<td>
					<input name="AutorizacionCustodia" type="checkbox" <cfif ValorAutorizacionCustodia eq 1>checked</cfif>>
				</td>
			</tr>
			<cfset ValesMejora= ObtenerDato(1300)>
			<tr>
				<td align="right">NO Permitir Vales por Mejora de Activos Fijos:&nbsp;</td>
				<td>
					<input name="ValesMejora" type="checkbox" <cfif ValesMejora.Pvalor eq 1>checked</cfif>>
				</td>
			</tr>
			<cfset configNoMej= ObtenerDato(3100)>
			<tr>
				<td align="right">Activar Configuración "No permite mejoras o adiciones":&nbsp;</td>
				<td>
					<input name="configNoMej" type="checkbox" value="1" <cfif configNoMej.Pvalor eq 1>checked</cfif>>
				</td>
			</tr>

			<cfset OrigenA = ObtenerDato(1110)>
			<tr>
				<td nowrap align="right">Etiqueta "Origen" de los Vales:&nbsp;</td>
				<td><input name="Origen" style="text-align:left" type="text" value="<cfif OrigenA.RecordCount gt 0 and len(trim(OrigenA.Pvalor))><cfoutput>#OrigenA.Pvalor#</cfoutput><cfelse></cfif>" size="25" maxlength="100"></td>
			</tr>


			<tr>
				<td align="right">Categoría Informática:</td>
          		<td>
					<cfif LvarCategoria neq '' >
						<cfset hayCategoria = 1 >

							<cfquery name="Concep" datasource="#Session.DSN#">
							  select ACcodigo, ACcodigodesc, ACdescripcion ,ACmascara
								from ACategoria
							  where Ecodigo = #session.Ecodigo#
								and ACcodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#LvarCategoria#" null="#Len(Trim(LvarCategoria)) EQ 0#" >
							</cfquery>
					</cfif>

					<cfif len(trim(LvarCategoria)) GT 0>
						<cfset ValuesArray=ArrayNew(1)>
						<cfset ArrayAppend(ValuesArray,Concep.ACcodigo)>
						<cfset ArrayAppend(ValuesArray,Concep.ACcodigodesc)>
						<cfset ArrayAppend(ValuesArray,Concep.ACdescripcion)>
						<cfset ArrayAppend(ValuesArray,Concep.ACmascara)>
					<cfelse>
						<cfset ValuesArray=ArrayNew(1)>
					</cfif>
							<cf_conlis
							Campos="ACcodigo_ALL, ACcodigodesc_ALL, ACdescripcion_ALL,ACmascara_ALL"
							ValuesArray="#ValuesArray#"
							Desplegables="N,S,S,N"
							Modificables="N,S,N,N"
							Size="0,10,30,0"
							Title="Lista de Categor&iacute;as"
							Tabla="ACategoria"
							Columnas="ACcodigo as ACcodigo_ALL,
									  ACcodigodesc as ACcodigodesc_ALL,
									  ACdescripcion as ACdescripcion_ALL,
									  ACmascara as ACmascara_ALL"
							Filtro="Ecodigo = #Session.Ecodigo# order by ACcodigodesc, ACdescripcion"
							Desplegar="ACcodigodesc_ALL, ACdescripcion_ALL"
							Etiquetas="C&oacute;digo,Descripci&oacute;n"
							filtrar_por="ACcodigodesc, ACdescripcion"
							Formatos="S,S"
							Align="left,left"
							Asignar="ACcodigo_ALL, ACcodigodesc_ALL, ACdescripcion_ALL, ACmascara_ALL"
							Asignarformatos="I,S,S,S"
							tabindex="1"/>
				</td>
			</tr>
			<tr>
			<cfset BoletaTras= ObtenerDato(3800)>
				<td align="right">Usar Boleta de Traslado:&nbsp;</td>
				<td>
					<input name="BoletaTras" type="checkbox" <cfif BoletaTras.Pvalor eq 1>checked</cfif>>
				</td>
			</tr>
            <tr>
			<cfset FechaAdquisicion = ObtenerDato(3805)>
				<td align="right">Fecha de Adquisición igual a la fecha del encabezado de la factura de CxP:&nbsp;</td>
				<td>
					<input name="FechaAdquisicion" type="checkbox" <cfif FechaAdquisicion.Pvalor eq 1>checked</cfif>>
				</td>
			</tr>

			<tr>
			<cfset TraslDirecto= ObtenerDato(4400)>
				<td align="right">Realiza traslado directo si el empleado que traslada es el encargado de Centro de Custodia (Autogesti&oacute;n):&nbsp;</td>
				<td>
					<input name="TraslDirecto" type="checkbox" <cfif TraslDirecto.Pvalor eq 1>checked</cfif>>
				</td>
			</tr>

			<tr>
			<cfset TraslDirectoCR= ObtenerDato(4402)>
				<td align="right">Realizar traslado  directo de un activo que pertenece al encargado del centro de custodia (Control de Responsables):&nbsp;</td>
				<td>
					<input name="TraslDirectoCR" type="checkbox" <cfif TraslDirectoCR.Pvalor eq 1>checked</cfif>>
				</td>
			</tr>

            <!----CUENTA DE GASTO POR DEPRECIACION---->
			<cfset UtilCtaGastoDep= ObtenerDato(1360)>
            <tr>
				<td align="right">Utilizar la cuenta de Gasto por Depreciación configurada en la Clasificación de Activo:&nbsp;</td>
				<td>
					<input name="UtilCtaGastoDep" type="checkbox" <cfif UtilCtaGastoDep.Pvalor eq 1>checked</cfif>>
				</td>
			</tr>

            <!---- FORMATO DE MASCARA PARA CEDULA DE CATALOGO PARA ACTIVOS FIJOS---->
            <cfset mascaraCedActFijo = ObtenerDato(4401)>
            <cfif mascaraCedActFijo.Recordcount GT 0 ><cfset hayFormatoCedulaActFijo = 1 ></cfif>

			<tr>
				<td align="right" nowrap>M&aacute;scara de C&eacute;dula de Catalogo de Activos Fijos:&nbsp;</td>
				<td>
					<input name="mascaraCedActFijo" type="text" size="50" maxlength="100" value="<cfoutput>#mascaraCedActFijo.Pvalor#</cfoutput>">
					<cf_notas titulo="Mascara Catalogo de responsable de activo fijo" link="#helpimg#" pageIndex="2" msg = "#helpmsj#" animar="true">
				</td>
			</tr>

            <!----CUENTA DE IMPUESTO PARA RETIROS---->
            <cfset ImpuestoRetiro = ObtenerDato(200020)>
            <tr>
				<td align="right" nowrap>Cuenta de Impuesto para Retiro de Activos:&nbsp;</td>
				<td>
                	<select name="cboImpuestoTipo">
                        <option value="1" <cfif ImpuestoRetiro.Pvalor EQ 1 OR ImpuestoRetiro.Pvalor EQ "">selected</cfif>>Facturas Proveedor</option>
                        <option value="2" <cfif ImpuestoRetiro.Pvalor EQ 2>selected</cfif>>Facturas Proveedor Acreditadas</option>
                        <option value="3" <cfif ImpuestoRetiro.Pvalor EQ 3>selected</cfif>>Facturas Cliente</option>
                        <option value="4" <cfif ImpuestoRetiro.Pvalor EQ 4>selected</cfif>>Facturas Cliente Acreditadas</option>
                    </select>
				</td>
			</tr>
            <!----Parametro para Generar Mascara Automatica. SML 25/02/2014---->
            <cfset PlacaAut = ObtenerDato(200050)>
            <tr>
				<td align="right" nowrap>Generar M&aacute;scara Autom&aacute;tica:&nbsp;</td>
				<td>
                	<input 	type="checkbox" tabindex="1" name="chkAFconsecutivo" <cfif PlacaAut.Pvalor EQ 1>checked</cfif> />
				</td>
			</tr>
            <!----Parametro de opciones para elegir la mascara sea por clasificacion o categoria SML 25/02/2014---->
            <cfset TipoPlaca = ObtenerDato(200060)>

            <cfquery name="rsAFConsecutivo" datasource="#session.DSN#">
            	select COUNT(1) as cantidad
				from AFConsecutivo
				where Ecodigo = #session.Ecodigo#
            </cfquery>

            <tr id="trTipoPlaca">
				<td align="right" nowrap>Generar Consecutivo de la Placa por:&nbsp;</td>
				<td>
                	<select id="cboTipoPlaca" name="cboTipoPlaca" <cfif isdefined("rsAFConsecutivo") and rsAFConsecutivo.cantidad GT 0> disabled="disabled"</cfif> >
                        <option value="1" <cfif TipoPlaca.Pvalor EQ 1> selected </cfif>>Categor&iacute;a de Activos</option>
                        <option value="2" <cfif TipoPlaca.Pvalor EQ 2> selected </cfif>>Clasificaci&oacute;n de Activos</option>
                    </select>
                    <input type="hidden" id="hdTipoPlaca" name="hdTipoPlaca"/>
				</td>
			</tr>

			<!----Parametro Generar polisa---->
            <cfset PolizaIAF = ObtenerDato(200070)>
            <tr>
				<td align="right" nowrap>Generar p&oacute;liza en el importador de Activos Fijos:&nbsp;</td>
				<td>
                	<input 	type="checkbox" tabindex="1" name="chkPoliIAF" <cfif PolizaIAF.Pvalor EQ 1>checked</cfif> />
				</td>
			</tr>
		</table>
	</div><!--- Fin AF --->


	<!---- Quick Pass ---->
	<div class="QPASS">
		<table width="100%">
		<!---- Quick Pass ---->
			<tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Quick Pass</font></b>
				</td>
			</tr>
			<tr>
				<td align="right">Moneda de la Cuenta de Saldos&nbsp;</td>
	            <td align="left" width="50%">
					<cfset Lvar_Mcodigo = ObtenerDato(447).Pvalor>
					<cfquery name="rsQPcuentaSaldos" datasource="#session.dsn#">
						select Mcodigo, Mnombre
						from Monedas
						where Ecodigo = #Session.Ecodigo#
	                    order by Mnombre
					</cfquery>
					<select name="P447">
						<cfoutput query="rsQPcuentaSaldos">
							<option value="#rsQPcuentaSaldos.Mcodigo#"
								<cfif Lvar_Mcodigo eq rsQPcuentaSaldos.Mcodigo>
									selected
								</cfif>
							>
								#rsQPcuentaSaldos.Mnombre#
							</option>
						</cfoutput>
					</select>
				</td>
			</tr>
		</table>
	</div>
	<!--- Fin QPASS --->



	<!--- Peaje --->
	<div class="PE">
		<table width="100%">
			<!--- Peaje --->
			<tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Peaje</font></b>
				</td>
			</tr>
			<tr>
				<td align="right">Tipo de Transacci&oacute;n:&nbsp;</td>
                <td align="left" width="50%"><cf_dbfunction name="OP_CONCAT" returnvariable="_Cat">
					<cfset Lvar_BTid = ObtenerDato(1800).Pvalor>
					<cfquery name="rsBTransacciones" datasource="#session.dsn#">
						select BTid, BTcodigo #_Cat#' - '#_Cat# BTdescripcion Descripcion
						from BTransacciones
						where Ecodigo = #Session.Ecodigo#
                        order by BTid
					</cfquery>
					<select name="BTid">
						<cfoutput query="rsBTransacciones">
							<option value="#rsBTransacciones.BTid#"
								<cfif Lvar_BTid eq rsBTransacciones.BTid>
									selected
								</cfif>
							>
								#rsBTransacciones.Descripcion#
							</option>
						</cfoutput>
					</select>
				</td>
			</tr>
			<tr>
				<td align="right">Banco:</td>
				<td align="left" width="50%">
					<cfquery datasource="#Session.DSN#" name="rsBancos">
						select Bid as BancoId, Bdescripcion as bancoDescripcion
						from Bancos
						where Ecodigo	=	<cfqueryparam value="#Session.Ecodigo#" cfsqltype="cf_sql_integer" >
						order by Bdescripcion
					</cfquery>
					<cfset selected="selected">
					<select name="Bid" tabindex="1" onchange="javascript:limpiarCuenta();">
					<option value="">-- Seleccione un Banco --</option>
					<cfloop query="rsBancos">
						<option value="<cfoutput>#rsBancos.BancoId#</cfoutput>" <cfif #Bid# eq #rsBancos.BancoId#> selected="selected"</cfif>><cfoutput>#BancoDescripcion#</cfoutput></option>
					</cfloop>
					</select>
				</td>
			</tr>
			<tr>
				<td align="right">Cuenta Bancaria: <input type="hidden" name="fecha" value="<cfoutput>#PAfecha#</cfoutput>"/></td>
				<td align="left" width="50%">
					<cf_conlis title="Lista de Cuentas Bancarias"
					campos = "CBid, CBcodigo, CBdescripcion, Mcodigo"
					values="#CBid#,#CBcodigo#,#CBdescripcion#,#Mcodigo#"
					desplegables = "N,S,S,N"
					modificables = "N,S,N,N"
					size = "0,0,40,0"
					tabla="CuentasBancos cb
					inner join Monedas m
					on cb.Mcodigo = m.Mcodigo
					inner join Empresas e
					on e.Ecodigo = cb.Ecodigo
					left outer join Htipocambio tc
					on 	tc.Ecodigo = cb.Ecodigo
					and tc.Mcodigo = cb.Mcodigo
					and tc.Hfecha  <= $fecha,date$
					and tc.Hfechah >  $fecha,date$ "
					columnas="cb.CBid, cb.CBcodigo, cb.CBdescripcion, cb.Mcodigo,
					m.Mnombre,
					round(
					coalesce(
					(	case
					when cb.Mcodigo = e.Mcodigo then 1.00
					else tc.TCcompra
					end
					)
					, 1.00)
					,2) as EMtipocambio"
					filtro="cb.Ecodigo = #Session.Ecodigo# and cb.CBesTCE = 0 and cb.Bid = $Bid,numeric$ order by Mnombre, Hfecha"
					desplegar="CBcodigo, CBdescripcion"
					etiquetas="C&oacute;digo, Descripci&oacute;n"
					formatos="S,S"
					align="right,right"
					asignar="CBid, CBcodigo, CBdescripcion, Mcodigo, Mnombre, EMtipocambio"
					asignarformatos="S,S,S,S,S,M"
					showEmptyListMsg="true"
					debug="false"
					tabindex="1">
				</td>
			</tr>

			<tr>
				<td align="right">Concepto de Ingreso:</td>
          		<td align="left" width="50%">
					<cfif LvarConGastoI neq '' >
						<cfset hayValorConceptoIngreso = 1 >

							<cfquery name="Concep" datasource="#Session.DSN#">
								select a.FPCid,a.FPCcodigo,a.FPCdescripcion,b.FPCCExigeFecha
									from FPConcepto a inner join FPCatConcepto b on b.FPCCid =  a.FPCCid
								where a.FPCid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#LvarConGastoI#" null="#Len(Trim(LvarConGastoI)) EQ 0#" >
							</cfquery>
					</cfif>

					<cfif len(trim(LvarConGastoI)) GT 0>
						<cfset ValuesArray=ArrayNew(1)>
						<cfset ArrayAppend(ValuesArray,Concep.FPCid)>
						<cfset ArrayAppend(ValuesArray,Concep.FPCcodigo)>
						<cfset ArrayAppend(ValuesArray,Concep.FPCdescripcion)>
						<cfset ArrayAppend(ValuesArray,Concep.FPCCExigeFecha)>
					<cfelse>
						<cfset ValuesArray=ArrayNew(1)>
					</cfif>
							<cf_conlis
							Campos="FPCid_ALL, FPCcodigo_ALL, FPCdescripcion_ALL,FPCCExigeFecha_ALL"
							ValuesArray="#ValuesArray#"
							Desplegables="N,S,S,N"
							Modificables="N,S,N,N"
							Size="0,10,30,0"
							Title="Lista de Conceptos"
							Tabla="FPConcepto a inner join FPCatConcepto b on b.FPCCid =  a.FPCCid"
							Columnas="FPCid as FPCid_ALL,
									  FPCcodigo as FPCcodigo_ALL,
									  FPCdescripcion as FPCdescripcion_ALL,
									  FPCCExigeFecha as FPCCExigeFecha_ALL"
							Filtro="a.Ecodigo = #Session.Ecodigo# and b.FPCCtipo = 'I'
							order by FPCcodigo, FPCdescripcion"
							Desplegar="FPCcodigo_ALL, FPCdescripcion_ALL"
							Etiquetas="Código,Descripción"
							filtrar_por="FPCcodigo, FPCdescripcion"
							Formatos="S,S"
							Align="left,left"
							Asignar="FPCid_ALL, FPCcodigo_ALL, FPCdescripcion_ALL,FPCCExigeFecha_ALL"
							Asignarformatos="I,S,S,I"
							form="form1"
							funcion="fnExigeFechas"
							/>
				</td>
			</tr>
			<tr>
				<td align="right">Actividad Empresarial:</td>
				<td align="left" width="50%">
					<cfif LvarActEmp neq '' >
						<cfset hayValorActividadEmpresarial = 1 >
					</cfif>

					<cfif LvarActEmpF neq '' >
						<cfset hayValorActividadEmpresarialFormato = 1 >
					</cfif>

					 <cfif isdefined('hayValorActividadEmpresarial') and hayValorActividadEmpresarial eq 1
					 and isdefined('hayValorActividadEmpresarialFormato') and hayValorActividadEmpresarialFormato eq 1>
							<cf_ActividadEmpresa etiqueta="" formname="form1" name="CFComplemento_ALL" idActividad="#LvarActEmp#" valores="#LvarActEmpF#" style="border:solid 1px ##CCCCCC; background:inherit;">
			 		<cfelse>
							<cf_ActividadEmpresa name="CFComplemento_ALL" formname="form1">
					 </cfif>
				</td>
			</tr>
		</table>
	</div><!--- Peaje --->

	<!--- Garantia --->
	<div class="GA">
		<table width="100%">
			<!--- Garantia --->
			<tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Garantía</font></b>
				</td>
			</tr>
			<tr>
				<td align="right">Transacción Bancaria para Depósitos de Garantía:&nbsp;</td>
                <td align="left" width="50%"><cf_dbfunction name="OP_CONCAT" returnvariable="_Cat">
					<cfset LvarBTid = ObtenerDato(1810).Pvalor>
					<cfquery name="rsBTranGarantia" datasource="#session.dsn#">
						select BTid, BTcodigo #_Cat#' - '#_Cat# BTdescripcion Descripcion
						from BTransacciones
						where Ecodigo = #Session.Ecodigo#
                        order by BTid
					</cfquery>
					<select name="BTidGarantia">
						<cfoutput query="rsBTranGarantia">
							<option value="#rsBTranGarantia.BTid#"
								<cfif LvarBTid eq rsBTranGarantia.BTid>
									selected
								</cfif>
							>
								#rsBTranGarantia.Descripcion#
							</option>
						</cfoutput>
					</select>
				</td>
			</tr>
		</table>
	</div><!---Fin Garantia--->


	<!----Plan de Compras Gobierno---->
	<div class="PC">
		<table width="100%">
		 	<!----Plan de Compras Gobierno---->
            <tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Plan de Compras Gobierno</font></b>
				</td>
			</tr>
			<tr>
				<td align="right">Cuenta de ingreso por Financiamiento:</td>
				<td align="left" width="50%"><cf_cajascuenta index="1" tabindex="-1" objeto="CFcuentaFinanciamiento" query="#rsCFcuentaFinanciamiento#"></td> <!----Pcodigo 2600---->
			</tr>
			<tr>
				<td align="right">Cuenta de Egresos por Amortización de préstamos:</td>
				<td align="left" width="50%"><cf_cajascuenta index="2" tabindex="-1" objeto="CFcuentacAmortizacion" query="#rsCFcuentacAmortizacion#"></td>   <!----Pcodigo 2800---->
			</tr>
		</table>
	</div>
	<!----Fin plan de compras gobierno---->


	<!---- Tarjetas de Credito Empresarial	---->

	<div class="TCE">
		<table width="100%" border="0">
			<!----Tarjetas de Credito Empresarial---->
			<tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Tarjetas de Cr&eacute;dito Empresarial</font></b>
				</td>
			</tr>
			<!----NO ASOCIAR BENEFICIARIO CON BANCO---->
			<!----PARAMETROS RELACIONADOS CON TARJETAS DE CREDITO EMPRESARIAL---->
			<cfset chkTESbeneTCE = ObtenerDato(5900)>
			<cfif chkTESbeneTCE.Recordcount GT 0 >
				<cfset hayTESbeneTCE = 1 >
			</cfif>
			<tr>
				<td align="right" nowrap>
					No asociar al TESbeneficiario la informaci&oacute;n del Banco o Emisor de TCE:&nbsp;
				</td>
				<td  align="left" width="50%">
					<input type="checkbox" name="chkTESbenTCE" value="S"
					<cfif chkTESbeneTCE.Pvalor EQ 'S'>
						checked
					</cfif>
					>
				</td>
			</tr>
			<!----Moneda Local---->
			<cfquery name="rsMonedaLoc" datasource="#session.dsn#">
                    select e.Mcodigo, m.Mnombre, m.Miso4217
                    from Empresas e
                    inner join Monedas m
                    on m.Mcodigo = e.Mcodigo
                    where e.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
                </cfquery>
			<cfquery name="rsMonedaExt" datasource="#session.dsn#">
                	select Mcodigo, Mnombre, Miso4217
                    from Monedas
                    where Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
                    and Mcodigo not in (#rsMonedaLoc.Mcodigo#)
                </cfquery>
			<cfset TCEMonedaExt = ObtenerDato(200040)>
			<tr>
				<td align="right" nowrap>
					Moneda Extranjera para las TCE:&nbsp;
				</td>
				<td align="left" width="50%">
					<select name="cboMonedaExt">
						<option value="X" selected>
							-Ninguna-
						</option>
						<cfloop query="rsMonedaExt">
							<cfoutput>
								<option value="#rsMonedaExt.Miso4217#"
								<cfif TCEMonedaExt.Pvalor EQ rsMonedaExt.Miso4217>
									selected
								</cfif>
								>#rsMonedaExt.Miso4217# - #rsMonedaExt.Mnombre#</option>
							</cfoutput>
						</cfloop>
					</select>
				</td>
			</tr>
			<tr>
				<td colspan="2">
					&nbsp;
				</td>
			</tr>

		</table>
	</div><!---Fin TCE --->

	<!--- Socios de negocio	--->
	<!---Inicia SN --->
	<div class="SN">
		<table width="100%">
			<tr>
				<td colspan="2" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Socios de Negocios</font></b>
				</td>
			</tr>
			<tr>
				<cfset rsGetAutorizadorSN = ObtenerDato(00030401)>
				<cfif rsGetAutorizadorSN.RecordCount GT 0 >
					<cfset hayAutorizadorSNCheck = 1 >
				</cfif>

			  <td align="right" nowrap>Autorizar Modificaciones a los Socios de Negocio:&nbsp;</td>
			  <td align="left" width="50%">
				   <input type="checkbox" name="chkAutorizadorSN" value="1" <cfif rsGetAutorizadorSN.Pvalor EQ '1'> checked</cfif>>
			  </td>
			</tr>
			<tr>
				  <cfset rsGetRolAutorizadorSN = ObtenerDato(00030402)>
				  <cfset lvarFiltroSN = "SScodigo='SIF' ">
				<cfif rsGetRolAutorizadorSN.RecordCount GT 0 >
					<cfset hayRolAutorizadorSN = 1 >
					<cfset lvarFiltroSN = lvarFiltroSN & " AND SRcodigo = '#rsGetRolAutorizadorSN.Pvalor#'">
				</cfif>

			  <td align="right" nowrap>Rol para Autorizar Modificaciones a los Socios de Negocio:&nbsp;</td>
			  <td align="left" width="50%">
				  <cf_conlis
				conexion="asp"
				Campos="SRcodigoSN, SRdescripcionSN"
				Asignar="SRcodigoSN, SRdescripcionSN"
				Size="10,40"
				Desplegables="S,S"
				Modificables="S,S"
				Title="Roles del sistema SIF"
				Tabla= "SRoles"
				Columnas="SRcodigo AS SRcodigoSN, SRdescripcion AS SRdescripcionSN"
				filtrar_por="SRcodigo, SRdescripcion"
				Desplegar="SRcodigoSN, SRdescripcionSN"
				Etiquetas="Codigo, Descripción"
				Formatos="S,S"
				Align="left,left"
				form="form1"
				debug="false"
				Asignarformatos="S,S"
				filtro="SScodigo='SIF'"
				traerinicial="true"
				traerfiltro="#lvarFiltroSN#"
				EmptyListMsg=" --- Sin registros --- "/>
			  </td>
			</tr>
			
			<tr>
				<td colspan="2">
					&nbsp;
				</td>
			</tr>
		</table>
	</div><!---Fin SN --->

	<!--- Modulos de prueba --->
	<div class="GE" style="display:none">
		<table width="100%">
			<tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Gastos de empleados</font></b>
				</td>
			</tr>
			<tr>
				<td width="39%" align="right" nowrap>Monto máximo para viáticos al interior:&nbsp;</td>
				<td width="35%">
					<input type="text" value="1000000.00">
				</td>
			</tr>
            <tr>
				<td align="right">Permite digitar Fecha en Recepcion de Efectivo:&nbsp;</td>
				<td>
					<input name="UtilCtaTrans" type="checkbox">
				</td>
			</tr>
            <tr>
				<td align="right">Permite digitar el gasto en moneda de TCE:&nbsp;</td>
				<td>
					<input name="MovFlujoEfectivo" type="checkbox" checked>
				</td>
			</tr>
			 <tr>
				<td align="right">Cuenta Presupuestal para Anticipo:&nbsp;</td>
				<td>
					<input name="MovFlujoEfectivo" type="checkbox" checked>
				</td>
			</tr>
			 <tr>
				<td align="right">Utilizar cuenta por pagar a empleados:&nbsp;</td>
				<td>
					<input name="MovFlujoEfectivo" type="checkbox" checked>
				</td>
			</tr>
			 <tr>
				<td align="right">Solicitar Anticipos con Anticipos del mismo tipo sin Liquidar:&nbsp;</td>
				<td>
					<input name="MovFlujoEfectivo" type="checkbox" checked>
				</td>
			</tr>
		</table>
	</div>


	<div class="OC" style="display:none">
		<table width="100%">
			<tr>
				<td colspan="3" align="center" class="listaPar">
					<hr size="0"><b><font size="2">Órdenes Comerciales en Tránsito</font></b>
				</td>
			</tr>
			<tr>
				<td width="39%" align="right" nowrap>Socio de Negocios default (propia Empresa) para Movs Origenes diferentes a CxP:&nbsp;</td>
				<td width="35%">
					<input type="text" value="241">
				</td>
			</tr>
            <tr>
				<td align="right">Porcentaje permitido de sobrantes para Cierre de Transportes (0-99%):&nbsp;</td>
				<td>
					<input name="UtilCtaTrans" type="checkbox">
				</td>
			</tr>
			 <tr>
				<td align="right">Porcentaje permitido para Existencias Negativas de Tránsito (0-9%):&nbsp;</td>
				<td>
					<input name="UtilCtaTrans" type="checkbox">
				</td>
			</tr>
		</table>
	</div>
	<!--- Fin modulos de prueba --->

<section><!--- FIN DE SECCION DE PARAMETROS --->
<span>
	<div align="center">
		<input type="submit" name="Aceptar" class="btnGuardar" value="Aceptar" onClick="javascript: return valida();">
	</div>
</span>


<!---- Variables para saber si hay que hacer un insert o un update en el .sql de cada uno de estos registros ----->
	<cfif definidos.Recordcount GT 0 >
		<cfset hayParametrosDefinidos = 1 >
	</cfif>
	<cfoutput>
		<input type="hidden" name="hayCuentaBalAsi" value="#hayCuentaBalAsi#">
		<input type="hidden" name="hayCuentaDifCambAsi" value="#hayCuentaDifCambAsi#">
        <input type="hidden" name="hayCuentaDifCamb2" value="#hayCuentaDifCamb2#">
		<input type="hidden" name="hayCuentaFluctuacion" value="#hayCuentaFluctuacion#">
		<input type="hidden" name="hayCuentaEstimacion" value="#hayCuentaEstimacion#">
		<input type="hidden" name="hayVencimiento1" value="#hayVencimiento1#">
		<input type="hidden" name="hayVencimiento2" value="#hayVencimiento2#">
		<input type="hidden" name="hayVencimiento3" value="#hayVencimiento3#">
		<input type="hidden" name="hayVencimiento4" value="#hayVencimiento4#">
		<input type="hidden" name="hayTipoRequisicion" value="#hayTipoRequisicion#">
		<input type="hidden" name="haySolicitante" value="#haySolicitante#">
		<input type="hidden" name="hayAutorizacion" value="#hayAutorizacion#">
		<input type="hidden" name="hayConsReq" value="#hayConsReq#">
		<input type="hidden" name="hayPublicacion" value="#hayPublicacion#">
		<input type="hidden" name="hayJerarquia" value="#hayJerarquia#">
		<input type="hidden" name="hayValidarCDC" value="#hayValidarCDC#">
		<input type="hidden" name="hayAprobarTolerancia" value="#hayAprobarTolerancia#">
		<input type="hidden" name="hayValidacionExcesos" value="#hayValidacionExcesos#">
		<!---- Parámetros de agrupación de órdenes de compra ---->
		<input type="hidden" name="hayAgrupacionOrdenes" value="#hayAgrupacionOrdenes#">
		<input type="hidden" name="hayFrecAgrupOC" value="#hayFrecAgrupOC#">
		<input type="hidden" name="hayDiasSemanaAgrupOC" value="#hayDiasSemanaAgrupOC#">
		<input type="hidden" name="hayDiaMesAgrupOC" value="#hayDiaMesAgrupOC#">
		<input type="hidden" name="haycalculoImp" value="#haycalculoImp#">
		<input type="hidden" name="hayDescDoc" value="#hayDescDoc#">
		<input type="hidden" name="haycuentaDescuentos" value="#haycuentaDescuentos#">
		<input type="hidden" name="haySupervisor" value="#haySupervisor#">
		<input type="hidden" name="hayTNCredito" value="#hayTNCredito#">
		<input type="hidden" name="hayTPCancelacion" value="#hayTPCancelacion#">
		<input type="hidden" name="hayPrecioUdec" value="#hayPrecioUdec#">
		<input type="hidden" name="hayPlanCuentas" value="#hayPlanCuentas#">
		<input type="hidden" name="hayCierreCaja" value="#hayCierreCaja#">
		<input type="hidden" name="hayAumentaConsecutivo" value="#hayAumentaConsecutivo#">
		<input type="hidden" name="hayValidaProduccion" value="#hayValidaProduccion#">
		<input type="hidden" name="haySocioImportacion" value="#haySocioImportacion#">
		<input type="hidden" name="haySocioImportacionC" value="#haySocioImportacionC#">
		<input type="hidden" name="hayCodigoArt" value="#hayCodigoArt#">
		<input type="hidden" name="hayTransaccionCP" value="#hayTransaccionCP#">
		<input type="hidden" name="hayTransaccionCC" value="#hayTransaccionCC#">
		<input type="hidden" name="hayImpuestos" value="#hayImpuestos#">
		<input type="hidden" name="hayNomina" value="#hayNomina#">
		<input type="hidden" name="hayCorrientes" value="#hayCorrientes#">
		<input type="hidden" name="hayMoratorios" value="#hayMoratorios#">
		<input type="hidden" name="hayTransitoria" value="#hayTransitoria#">
		<input type="hidden" name="hayCNCredito" value="#hayCNCredito#">
		<input type="hidden" name="hayTransaccionCPInterfaz" value="#hayTransaccionCPInterfaz#">
		<input type="hidden" name="hayTransaccionCCInterfaz" value="#hayTransaccionCCInterfaz#">
		<!---- PARAMETROS NUEVOS RELACIONADO CON LOS CAMBIOS PEDIDOS PARA EL MODULO CxC ---->
		<input type="hidden" name="hayFormatoCedulaJuridica" value="#hayFormatoCedulaJuridica#">
		<input type="hidden" name="hayFormatoCedulaFisica" value="#hayFormatoCedulaFisica#">
		<input type="hidden" name="NoModificar" value="#NoModificar#">

		<!---- Parametros Control Presupuesto ---->
		<input type="hidden" name="hayCuentaDifCamMultiIngreso" value="#hayCuentaDifCamMultiIngreso#">
		<input type="hidden" name="hayCuentaDifCamMultiEgreso" value="#hayCuentaDifCamMultiEgreso#">
		<input type="hidden" name="hayCuentaDifCamMultiCosto" value="#hayCuentaDifCamMultiCosto#">

		<!---- PARAMETROS DE FECHA DE CIERRE DE CONTABILIDAD ---->
		<input type="hidden" name="hayCierreConta" value="#hayCierreConta#">
		<input type="hidden" name="hayFechaCierreConta" value="#hayFechaCierreConta#">
		<!---- PARAMETRO DE MOSTRAR CLASIFICACION CUENTAS POR COBRAR---->
		<input type="hidden" name="hayVerClasifCC" value="#ValorVerClasifCC#">
		<!---- PARAMETRO DE MOSTRAR CLASIFICACION CUENTAS POR PAGAR---->
		<input type="hidden" name="hayVerClasifCP" value="#ValorVerClasifCP#">
		<input type="hidden" name="hayMBOrigen" value="#hayMBOrigen#">
		<input type="hidden" name="hayMBDestino" value="#hayMBDestino#">
		<!---- PARAMETRO ACTIVOS FIJOS---->
		<input type="hidden" name="hayFormatoCedulaActFijo" value="#hayFormatoCedulaActFijo#">
		<!---- PARAMETRO DE NO CONTABILIZAR COSTO DE VENTAS NI AJUSTES ---->
		<input type="hidden" name="hayNoContaCosto" value="#ValorNoContaCosto#">
		<!---- PARAMETRO DE VERIFICAR GRUPOS DE CUENTA ---->
		<input type="hidden" name="hayVerGruCtas" value="#ValorNoContaCosto#">
		<!---- Reportes de Compras ---->
		<input type="hidden" name="hayRepCotProveedoresLocal" value="#hayRepCotProveedoresLocal#">
		<input type="hidden" name="hayRepCotProveedoresImportacion" value="#hayRepCotProveedoresImportacion#">
		<input type="hidden" name="hayRepCotProcesoLocal" value="#hayRepCotProcesoLocal#">
		<input type="hidden" name="hayRepCotProcesoImportacion" value="#hayRepCotProcesoImportacion#">
		<!---- Roles de Solicitante y Comprador ---->
		<input type="hidden" name="hayRolSolic" value="#hayRolSolic#">
		<input type="hidden" name="hayRolComprador" value="#hayRolComprador#">
		<input type="hidden" name="hayRecurrentes" value="#hayRecurrentes#">
		<!---- Parámetros nuevo para Leyenda de Balance General y Estado de Resultados ---->
		<input type="hidden" name="hayLeyenda" value="#hayLeyenda#">
		<input type="hidden" name="hayLeyendaCxC" value="#hayLeyendaCxC#">
		<input type="hidden" name="hayAgregarDesc" value="#hayAgregarDesc#">
		<input type="hidden" name="hayImportaConCF" value="#hayImportaConCF#">
		<input type="hidden" name="hayMBAsientoDifCam" value="#hayMBAsientoDifCam#">
		<!---- PARAMETRO DE MOSTRAR ASIENTO EN FORMA DETALLADA ---->
		<input type="hidden" name="hayVerAstDet" value="#ValorVerAstDet#">
		<!---- PARAMETRO PARA APLICAR RETIROS AUTOMATICAMENTE ---->
		<input type="hidden" name="hayRetAuto" value="#ValorRetAuto#">
		<!---- PARAMETRO PARA TRANSACCION PEAJE ---->
		<input type="hidden" name="hayTipoTransPeaje" value="#ValorTipoTransPeaje#">
		<input type="hidden" name="hayCuentaMovPeaje" value="#ValorCuentadePeaje#">
		<!----NO ASOCIAR BENEFICIARIO CON BANCO---->
		<!---- PARAMETROS DE TARJETAS DE CREDITO EMPRESARIAL ---->
		<input type="hidden" name="hayTESbeneTCE" value="#hayTESbeneTCE#">
		<!---- FACTURACION ---->
		<input type="hidden" name="hayVendedorCxC"        value="#hayVendedorCxC#">
		<input type="hidden" name="hayControlInventarios" value="#hayControlInventarios#">
		<input type="hidden" name="hayVersion" value="#hayVersion#">
		<input type="hidden" name="hayDeciFac" value="#hayDeciFac#">
		<input type="hidden" name="hayCuentadedescuentosFact" value="#hayCuentadedescuentosFact#">
		<input type="hidden" name="consecutivoRE"    value="#hayConsecutivoRe#">
		<!---- Codigo 16305, caja default para notas de credito sin refactura o con refactura pero de tipo credito ---->
		<input type="hidden" name="hayrepCajaAsignado" value="#hayrepCajaAsignado#">
		<!---- Codigo 16306, Utilizar fecha de vencimiento por socio de negocio ---->
		<input type="hidden" name="hayrepVencimientoSocio" value="#hayrepVencimientoSocio#">
		<!---- Codigo 16307, Lista de Codigos de empresa origen que no permite ReFacturar ---->
		<input type="hidden" name="hayConsecutivoCodEmpOrigen" value="#hayConsecutivoCodEmpOrigen#">
		<input type="hidden" name="MontoMaximoVuelto"    value="#hayMontoMaximoVuelto#">
		<input type="hidden" name="hayCFRemesas"    value="#hayCFRemesas#">
		<!---- Codigo 16317,  Integración con facturación digital ---->
		<input type="hidden" name="hayintegraFacturaDigital" value="#hayintegraFacturaDigital#">
		<!---- Codigo 16371,  Integración con facturación digital ---->
		<input type="hidden" name="hayProveedorFacturaDigital" value="#hayProveedorFacturaDigital#">
		<!---- Codigo 16318,  Activar la interfaz de conexión para SGA ---->
		<input type="hidden" name="hayInterfazSGA" value="#hayInterfazSGA#">
		<!---- Codigo 16318,  Activar la interfaz de conexión para SGA ---->
		<input type="hidden" name="hayContabilizarAplicarTransa" value="#hayContabilizarAplicarTransa#">
		<!---- Codigo 16373,  Validar correo y telefono en pantalla de registro de facturas ---->
		<input type="hidden" name="hayValidacionCorreoYtelefono" value="#hayValidacionCorreoYtelefono#">
		<!---- Codigo 17100,  Validar UUID Facturacion ---->
		<input type="hidden" name="hayTFiscal" value="#hayTFiscal#">
		<!----►►Parametro 5306: Indicar Cuentas Contables Manualmente◄◄---->
		<input type="hidden" name="hayCuentaManual" value="<cfif rsCuentaManual.RecordCount gt 0 and rsCuentaManual.Pvalor eq 'S'>1<cfelseif rsCuentaManual.RecordCount gt 0 and rsCuentaManual.Pvalor eq 'N'>0<cfelse>-1</cfif>">
		<!---- Codigo 1711,  Validar Remisión ---->
		<input type="hidden" name="hayRemision" value="#hayRemision#">
		<!---- Codigo 17600,  Horario de Verano ---->
		<input type="hidden" name="hayCVerano" value="#hayCVerano#">
		<input type="hidden" name="hayAutorizadorTcdCheck" value="#hayAutorizadorTcdCheck#">
		<!--- SOCIOS DE NEGOCIO --->
		<!--- Parámetro para check autorizador --->
		<input type="hidden" name="hayAutorizadorSNCheck" value="#hayAutorizadorSNCheck#">
		<!--- Parámetro para autorizador --->
		<input type="hidden" name="hayRolAutorizadorSN" value="#hayRolAutorizadorSN#">
		<!--- TESORERIA --->
		<!--- Parámetro para check autorizador  Cuentas Destino para Pago--->
		<input type="hidden" name="hayAutorizadorTcdCheck" value="#hayAutorizadorTcdCheck#">
		<!--- Parámetro para autorizador Cuentas Destino para Pago--->
		<input type="hidden" name="hayRolAutorizadorTcd" value="#hayRolAutorizadorTcd#">
        <!---►►Parametro 5306: Indicar Cuentas Contables Manualmente◄◄--->
        <input type="hidden" name="hayCuentaManual" value="<cfif rsCuentaManual.RecordCount gt 0 and rsCuentaManual.Pvalor eq 'S'>1<cfelseif rsCuentaManual.RecordCount gt 0 and rsCuentaManual.Pvalor eq 'N'>0<cfelse>-1</cfif>">
	</cfoutput>
</form>
<cfoutput>
	<script language="javascript" type="text/javascript">
	cantDig(document.form1.chkRCDND);
</script>
	<script language="javascript1.2" type="text/javascript">
	//Dispara la funcion del iframe que retorna los datos de la cuenta
	function FrameFunction()
	{
		// RetornaCuenta2() es máscara completa, rellena con comodín
		window.cuentasIframe1.RetornaCuenta2();
		window.cuentasIframe2.RetornaCuenta2();
		///window.cuentasIframe3.RetornaCuenta2();
		return true;
	}

	<!----  Funcciones java script---->
	function  PrecioUdec(){
		if(this.form.PrecioUdecRPT.selectedIndex > this.form.PrecioUdec.selectedIndex)
			this.form.PrecioUdecRPT.selectedIndex = this.form.PrecioUdec.selectedIndex;
	}

	function PrecioUdecRPT(){
		if(this.form.PrecioUdecRPT.selectedIndex > this.form.PrecioUdec.selectedIndex)
			this.form.PrecioUdecRPT.selectedIndex = this.form.PrecioUdec.selectedIndex;
	}
</script>
</cfoutput>
<cfoutput>
	<cf_qforms>
	<script language="javascript" type="text/javascript">
	function limpiarCuenta(){
	objForm.CBid.obj.value="";
    objForm.CBcodigo.obj.value="";
	objForm.CBdescripcion.obj.value="";
	}

	function resetEmpleado(){
	objForm.PCEcatid.obj.value = 'XX';
	objForm.PCEcodigo.obj.value = 'XXX';
	objForm.PCEdescripcion.obj.value = 'XXXXXXXXX';
	}
	<!----Activar el parametro de 200060 dependiendo del chkAFconsecutivo---->
	function fnActivar(form,tr1)
	{
		if (form.chkAFconsecutivo.checked == true)
    	{
	  		if (!document.getElementById) return false;

  	  		fila = document.getElementById(tr1);

  			if (fila.style.display != "none") {
    			fila.style.display = "none"; //ocultar fila
  			} else {
   				fila.style.display = ""; //mostrar fila
  			}
    	}
		else
		{
			fila = document.getElementById(tr1);

			if (fila.style.display != "none") {
    			fila.style.display = "none"; //ocultar fila
  				}
		}
	}

	<!---- function filtro(){
		alert("hola");
		return false;
	} ---->
</script>
</cfoutput>
<!--- Script del filtro --->
<script>
	$("#bfil").click(function() {
		var filtro = $("#filtrop").val();
		$("section > div").not('.'+filtro).hide();
		$("section > div").filter('.'+filtro).show();
	});

	function rslistmod(){
		var posicion=document.getElementById('listmod').value;
		console.log(posicion);
		document.getElementById('filtrop').value = posicion;
	}

	var LvarSMcodigo = '<cfoutput>#LvarSMcodigo#</cfoutput>';
	/*console.log(LvarSMcodigo);*/
	$("section > div").not('.'+LvarSMcodigo).hide();
	$("section > div").filter('.'+LvarSMcodigo).show();
</script>

