...
<style type="text/css">
	...
	.iframe {
		...
		<cf_web_portlet_start border="true" skin="#Session.Preferences.Skin#" tituloalign="center" titulo="Registro de Complementos Financieros">
...
--->
					   ...
					   <td>
						<cfif isdefined("form.OPtabla_F") and len(trim(form.OPtabla_F))>
							<cfquery name="data" datasource="sifcontrol">
								...
								<cfelseif compareNocase('sifclasificacionconcepto', data.rutatag) eq 0>
									<cf_sifclasificacionconcepto form="form2" id="ODchar_F">
								<cfelse>
                                	<cfif not isdefined("form.Nuevo")>
										<input type="hidden" name="ODchar_F" value="<cfif isdefined("Form.ODchar")>#Form.ODchar#</cfif>">
										<cfif isdefined("Form.ODchar") and isdefined("Form.Procesar")>
                                            <cfset varvalue = Form.ODchar>
                                            <cfset Form.ODchar_F = Form.ODchar>
                                        <cfelse>
                                            <cfset varvalue = "">
                                        </cfif>
                                        <cf_conlis
                                        ...
                                        tabindex="8"
                                        Values="#varvalue#"
                                        Title="Lista de valores para el catalogo"
                                        Tabla="OrigenDatos"
                                        Columnas="ODchar"
                                        Filtro=" Ecodigo = #Session.Ecodigo# and  OPtabla = '#form.OPtabla_F#' group by ODchar"
                                        Desplegar="ODchar"
                                        Etiquetas="C&oacute;digo"
                                        filtrar_por=""
                                        Formatos="S"
                                        Align="left"
                                        form="form2"
			...
			<script type="text/javascript" language="javascript1.2">
				...
				function validar(f){
					...
					<cfif isdefined("form.OPtabla_F") and len(trim(form.OPtabla_F)) and len(trim(form.OPtabla_F)) LTE 0>
