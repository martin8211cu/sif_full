...
<cffunction name="pLista" access="public" returntype="string" output="true">
	...
	<cfargument name="RowCount" type="numeric" required="false" default="350">


	<!--- QUERY --->
	<cfset sql = " set rowcount " & RowCount & " select " & columnas & " from " & tabla >
	<cfif filtro NEQ "">
		<cfset sql = sql & " where " & filtro>
	</cfif>
	<cfset sql = sql & " set rowcount 0" >
