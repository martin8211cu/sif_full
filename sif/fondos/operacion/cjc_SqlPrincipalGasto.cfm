...
<cfif isdefined("form.ResetA")>
	<cftransaction> 
		<cftry>
			<cfquery datasource="#session.Fondos.dsn#"  name="sql" >
				Select isnull(max(CJX19NGA),1) as MGrupo
				from CJX019
				where CJ01ID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#session.Fondos.Caja#">
			</cfquery>
			<cfset NewGroup = sql.MGrupo>
			
			<cfquery datasource="#session.Fondos.dsn#"  name="sql" >
				Update CJX019
				set CJX19NGA = <cfqueryparam cfsqltype="cf_sql_integer" value="#NewGroup#">
				from CJX020 A
				where A.CJX19REL = CJX019.CJX19REL
				  and CJX019.CJX19EST = 'P'
				  and CJX019.CJX19NGA is null
...
<cfelse>
		...
		<cftry>
			<cfquery datasource="#session.Fondos.dsn#"  name="sql" > 
				...
				<cfif isdefined("form.Alta")>
					...
					<cfif isdefined("form.EMPCOD") and len(trim(form.EMPCOD)) GT 0>
					...
					<cfelse>
						<script language="JavaScript">
							alert("Es necesario el c&oacute;digo de empleado.")
							history.back()
						</script>
					</cfif>	
			...
			<cfif NOT isdefined("form.Nuevodet")>
				<cfif isdefined("form.Altadet") OR isdefined("form.Cambiodet")>
				   ...
				   <!---************************************************ --->
					...
					<cfloop query="sql2"> 
						<cfswitch expression = #sql2.CJM08TIP#>
							<cfcase value="50;62;56;60" delimiters=";"> <!--- NUMERICO --->
								...
								<cfset CJX22INF = CJX22INF & sql2.CJM08NOM &"="& Trim('#Evaluate('form.#Evaluate('sql2.CJM08NOM')#')#') & "Â¶">
							</cfcase>
					...
					<!---**   ESTE CARACTER 'Â¶' AL FINAL DE LA HILERA     ** --->
					<!---************************************************** --->
			...
			<cfquery datasource="#session.Fondos.dsn#"  name="sql" > 
				...
				<cfif isdefined("form.Altadet")>
					declare @CJX19REL int ,@CJX20NUM  int ,@CJX21LIN int, @error int
					...
					<cfif  isdefined("form.CGM1CD") and find('@',form.CGM1CD,1) GT 0>
						<script language="JavaScript">
							alert("Error! La cuenta de gasto no pudo ser registrada. Por favor intente de Nuevo. Proceso Cancelado.")
							history.back()
						</script>
					<cfelse>
						INSERT CJX021(
						...
						exec @error =  cj_CalculaImpuestos_masivo
						@CJX19REL  = @CJX19REL,
						@CJX20NUM  = @CJX20NUM,
						@MCJX21LIN = @CJX21LIN
				...
				<cfelseif isdefined("form.Cambiodet")>
					...
					<cfelse>					
						...
						@CJX19REL = <cfqueryparam cfsqltype="cf_sql_integer" value="#trim(CJX19REL)#" >,
						@CJX20NUM = <cfqueryparam cfsqltype="cf_sql_integer" value="#trim(CJX20NUM)#" >,
						@MCJX21LIN = <cfqueryparam cfsqltype="cf_sql_integer" value="#trim(form.CJX21LIN)#"> 
	...
	<cfif isdefined("form.Posteo")>
		<!---******************************************************** --->
		<!---** Consulta de las lineas para procesar actualizaciÃ³n ** --->
		<!---******************************************************** --->
		...
		<cfif lineas.recordcount gt 0>  <!---** inicia (si hay lineas en la CJX023 las procesa )  ** --->
			...
			<cfloop query = "lineas"> 
				...
				<cfset arreglo = listtoarray(lineas.CJX22INF,"Â¶")>
				...
				<cfif lineas.recordcount gt 0  and rs1.Resultado neq 'N'>
					<cfloop from="1" to="#arraylen(procs)#" index="valor">
						...
						<cfquery name="rs2" datasource="#session.Fondos.dsn#">	
							...
							if @error != 0 begin
							raiserror 40000 'Error en actualizaciÃ³n de auxiliares'	
							return
							end
						</cfquery>
					</cfloop>  
