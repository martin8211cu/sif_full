...
<cfparam name="Form.CJX20NUM" default="-1">
	...
	<!--- Acumulador --->
	<cfset ACUMULADOR = "0.00">
	<cfquery name="rsAcumulado" datasource="#session.Fondos.dsn#">
	Select convert(varchar(20),isnull(sum(CJX20MNT),0),1) as Total
	from CJX020 A, CJX019 B
	where A.CJX19REL = B.CJX19REL
	  and B.CJX19EST = 'P'
	  and B.CJX19NGA is null
	  and CJ01ID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#session.Fondos.Caja#">		
	</cfquery>
	<cfset ACUMULADOR = "#rsAcumulado.Total#">
	...
	<cfquery name="rsCTA" datasource="#session.Fondos.dsn#">
		...
		where b.PERCOD = <cfqueryparam  cfsqltype="cf_sql_integer" value="#LvarCJ1PER#">
			...
			<cfif isdefined("session.Fondos.Fondo") and len(trim(session.Fondos.Fondo)) gt 0 >
				and a.PR1CTP like '#session.Fondos.Fondo#%' 
			<cfelse>
				and a.PR1CTP like '' 
			</cfif>
	...
	<cfquery name="rsPROY" datasource="#session.Fondos.dsn#">
		select A.PRT7IDE LLAVE, A.PRT7DESC DES,A.PR1COD
		from PRT007 A, PRM001 B
		where A.PERCOD = <cfqueryparam  cfsqltype="cf_sql_integer" value="#LvarCJ1PER#">
		   and A.PRT22SUB = 1
		   and A.PR1COD = B.PR1COD
			<cfif isdefined("session.Fondos.Fondo") and len(trim(session.Fondos.Fondo)) gt 0 >
				and B.PR1CTP like '#session.Fondos.Fondo#%' 
			<cfelse>
				and B.PR1CTP like '' 
			</cfif>
		order by PR1COD
			...
			<!---********************************************************************************* --->
				...
				<!---*********************** --->
				  <table width="100%" border="0" >
					...
					<tr>					 
					  <td><p><strong>Acumulado:</strong></p> </td>
 					  <td>
					  	<INPUT  tabindex="-1" 
							ONFOCUS="this.blur();" 
							NAME="GASTOS" 
							VALUE="<cfoutput>#ACUMULADOR#</cfoutput>" 
							size="17"  
							style="border: medium none; text-align:left; size:auto;"
							tabindex="2"
						>						  
					  ...
					  <td colspan="2">
							...
							<tr>
								<td valign="middle">
									...
									{
										//novalidar();
										document.form1.botonSel.value = "ResetA"; 
										if ( confirm('¿Desea Inicializar el Acumulador de Gastos?') )
										{
											document.form1.submit();											
										}										
									}
									</script>
									<a href="javascript: Reinicializar();">
									<img src="../../imagenes/refresh_o.gif" alt="Inicializar Acumulador" name="ResetA" border="0" align="absmiddle">
					...
					<tr>
					  ...
					  <td width="20%">
					  <select name="CGE5COD" tabindex="2"
  					  	  onChange="AgregarComboGR(document.form1.CJM12ID,document.form1.CGE5COD.value)">
                          <cfloop query="rsSEG">
                          	<cfif isdefined("TraeSqlDet") and mododet neq "ALTA">
						       <option value="#trim(CGE5COD)#" <cfif rsSEG.CGE5COD EQ TraeSqlDet.CGE5COD>selected</cfif>>#CGE5DES#</option>
							<cfelse>	
