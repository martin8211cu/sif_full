<!--- 
Conlis de empleados y tarjetas para reporte de recepción de vouchers, 
este conlis no filtra por fondo, si no que busca todas las tarjetas y 
todos los empleados asociados al fondo al que este asociado el empleado.
 --->
...
<cfif rsUsuarioFondo.cantidad gt 0 > 
	<cfset UsuarioConFondo = true>
<cfelse>
	<cfset UsuarioConFondo = false>
</cfif>
...
<cfquery datasource="#session.Fondos.dsn#"  name="Tarjetas" >	
	...
	WHERE CJM014.TS1COD = CATR01.TS1COD 
		...
		<cfif UsuarioConFondo > 
			AND CJM00COD in (select CJM00COD
							 from CJM010
							 where  CJM10LOG = <cfqueryparam cfsqltype="cf_sql_varchar" value="#session.usuario#">
							   and CJM10EST = 'A'							 
							 )
		</cfif>							 
	...
	<cfinvoke 
	 ...
	 returnvariable="pListaRet">
		...
		<cfinvokeargument name="filtro" value=" exists (Select 1 
														from CATR01 B, CJM014 C, CJM010 D
														where B.TS1COD = C.TS1COD
														  and B.TR01NUT = C.TR01NUT
														  and C.CJM00COD = D.CJM00COD
														  and D.CJM10LOG = '#session.usuario#'
														  and D.CJM10EST = 'A'													  
  														  and B.EMPCOD = PLM001.EMPCOD
														) #filtro#"/>
