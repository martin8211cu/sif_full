...
<!--- 
	...
	Filtros[i]:
		...
		*	6=Tipo de Dato, NIVEL_DATO=Dato al que sustituye, VALOR=Default en combo
...
<form name="form1" method="post" action="rptSaldos_imprimir.cfm" onSubmit="return sbSubmit();">
	...
	<table width="100%" border="0">
		...
		<tr>
			...
			<td colspan="2">
				<cf_CPSegUsu_cboCFid value="#form.CFid#" Consultar="true" Estado="A">
			</td>
		</tr>
	...
	<cfloop index="f" from="1" to="#arrayLen(LarrFiltros)#">
		...
		<cfif LvarFiltro[5] NEQ "F">
			...
			<tr>
				...
				<cfparam name="form.Dato#LvarFiltro[2]#" default="#listGetAt("[A],[M],[*PF],[T],[TE],[VC],[*PP],[ME],[*PA],[RA],[CA],[RC],[CC],[E],[E2],[ET],[*PCA],[RP],[*PC],[NP],[*DN]",LvarFiltro[2])#">
				<cfset LvarValor  = evaluate("form.Dato#LvarFiltro[2]#")>
				<cfset LvarValorD = LvarFiltro[3]>
				<cfif LvarValorD EQ "" OR LvarValorD EQ "[]">
					<cfset LvarValorD = LvarValor>
				</cfif>
				
				<td colspan="2">
					<select name="Dato<cfoutput>#LvarFiltro[2]#</cfoutput>">
						<option value="[A]" 	<cfif LvarValorD EQ "[A]">	selected</cfif>>[A]  = Aprobación Presupuesto Ordinario</option>
						<option value="[M]" 	<cfif LvarValorD EQ "[M]">	selected</cfif>>[M]  = Modificación Presupuesto Extraordinario</option>
						<option value="[*PF]" 	<cfif LvarValorD EQ "[*PF]">	selected</cfif>>[*PF]:PRESUPUESTO FORMULADO =[A]+[M]</option>
						<option value="[T]" 	<cfif LvarValorD EQ "[T]">	selected</cfif>>[T]  = Traslados de Presupuesto Internos</option>
						<option value="[TE]" 	<cfif LvarValorD EQ "[TE]">	selected</cfif>>[TE] = Traslados con Autorización Externa</option>
						<option value="[VC]" 	<cfif LvarValorD EQ "[VC]">	selected</cfif>>[VC] = Variación Cambiaria</option>
						<option value="[*PP]" 	<cfif LvarValorD EQ "[*PP]">	selected</cfif>>[*PP]:PRESUPUESTO PLANEADO =[*PF]+[T]+[TE]+[VC]=[A]+[M]+[T]+[TE]+[VC]</option>
						<option value="[ME]"	<cfif LvarValorD EQ "[ME]">	selected</cfif>>[ME] = Modificación por Excesos Autorizados</option>
						<option value="[*PA]" 	<cfif LvarValorD EQ "[*PA]">	selected</cfif>>[*PA]:PRESUPUESTO AUTORIZADO =[*PP]+[ME]=[A]+[M]+[T]+[TE]+[VC]+[ME]</option>
						<option value="[RA]" 	<cfif LvarValorD EQ "[RC]">	selected</cfif>>[RA] = Presupuesto Reservado Período Anterior</option>
						<option value="[CA]" 	<cfif LvarValorD EQ "[CC]">	selected</cfif>>[CA] = Presupuesto Comprometido Período Anterior</option>
						<option value="[RC]" 	<cfif LvarValorD EQ "[RC]">	selected</cfif>>[RC] = Presupuesto Reservado</option>
						<option value="[CC]" 	<cfif LvarValorD EQ "[CC]">	selected</cfif>>[CC] = Presupuesto Comprometido</option>
						<option value="[E]" 	<cfif LvarValorD EQ "[E]">	selected</cfif>>[E]  = Ejecutado Contable</option>
						<option value="[E2]" 	<cfif LvarValorD EQ "[E2]">	selected</cfif>>[E2] = Ejecutado No Contable</option>
						<option value="[ET]" 	<cfif LvarValorD EQ "[ET]">	selected</cfif>>[ET]: EJECUTADO TOTAL ([E]+[E2])</option>
						<option value="[*PCA]" 	<cfif LvarValorD EQ "[*PCA]">selected</cfif>>[*PCA]:CONSUMO DE AUXILIARES Y CONTA =[RA]+[CA]+[RC]+[CC]+[E]+[E2]</option>
						<option value="[RP]" 	<cfif LvarValorD EQ "[RP]">	selected</cfif>>[RP] = Provisiones Presupuestarias</option>
						<option value="[*PC]" 	<cfif LvarValorD EQ "[*PC]">selected</cfif>>[*PC]:PRESUPUESTO CONSUMIDO =[*PCA]+[RP]=[RA]+[CA]+[RC]+[CC]+[E]+[E2]+[RP]</option>
						<option value="[NP]" 	<cfif LvarValorD EQ "[NP]">	selected</cfif>>[NP] = Rechazos Aprobados Pendientes de Aplicar</option>
						<option value="[*DN]" 	<cfif LvarValorD EQ "[*DN]">selected</cfif>>[*DN]:DISPONIBLE NETO = [*PA] - [*PC] - [NP]</option>
						<option disabled>Movimientos de Flujo de Efectivo</option>
						<option value="[E3]" 	<cfif LvarValorD EQ "[E3]">	selected</cfif>>[E3] = Devengado o Ejecutado No Pagado</option>
					...
					<cfif rsSQL.Pvalor EQ "S">
						<option value="[EJ]" 	<cfif LvarValorD EQ "[EJ]">	selected</cfif>>[EJ]  = Ejercido Total</option>
						<option value="[EJ3]" 	<cfif LvarValorD EQ "[EJ3]">selected</cfif>>[EJ3] = Ejercido No Pagado</option>
						<option value="[P]" 	<cfif LvarValorD EQ "[P]">	selected</cfif>>[P]   = Presupuesto Pagado</option>
					<cfelse>
						<option value="[P]" 	<cfif LvarValorD EQ "[P]">	selected</cfif>>[P]  = Presupuesto Pagado</option>
					</cfif>
					</select>
			...
			<tr>
				...
				<td colspan="2">
					...
					<select name="CPCtipoCta">
						...
						<cfif listFind(LvarFiltro[3],"P")>
							<option value="P" <cfif form.CPCtipoCta EQ "P">selected</cfif>>Pasivos</option>
						</cfif>
					</select>
				</td>
