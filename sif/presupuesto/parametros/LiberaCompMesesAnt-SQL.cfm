...
<cfif isdefined("form.BTNLIBERAR")>
	...
	<cfquery name="rsLineas" datasource="#Session.DSN#">
			select n.CPNAPmoduloOri,n.CPCano, n.CPCmes, n.CPNAPDlinea, n.CPNAPnum, n.CPPid, n.CPcuenta, n.NAPMonto,
				cpp.CPCPtipoControl, cpp.CPCPcalculoControl,
				cpc.CPcuenta as CPCCPCuenta, cpc.Ocodigo, cpc.CPCpresupuestado, cpc.CPCmodificado, cpc.CPCmodificacion_Excesos,
				cpc.CPCvariacion, cpc.CPCtrasladado, cpc.CPCtrasladadoE, cpc.CPCreservado_Anterior, cpc.CPCcomprometido_Anterior,
				cpc.CPCreservado_Presupuesto, cpc.CPCreservado, cpc.CPCcomprometido, cpc.CPCejecutado, cpc.CPCejecutadoNC,
				cpc.CPCnrpsPendientes,
				cpt.CPformato
			from (
				select f.CPNAPnum, CPNAPDlinea, e.Ecodigo, f.CPPid, f.CPcuenta, f.CPCano, f.CPCmes, f.CPNAPnumRef,
					f.CPNAPDtipoMov, e.CPNAPmoduloOri, f.CPNAPDmonto, f.CPNAPDutilizado, f.CPNAPDmonto-f.CPNAPDutilizado as NAPMonto
				from dbo.CPNAPdetalle f inner join dbo.CPNAP e on f.CPNAPnum = e.CPNAPnum
				where f.CPNAPDmonto-f.CPNAPDutilizado <> 0
					<!--- and f.CPNAPDutilizado > 0 --->)n
			inner join CPresupuesto cpt
				on cpt.CPcuenta = n.CPcuenta
				and cpt.Ecodigo = n.Ecodigo
			inner join CPresupuestoComprometidasNAPs cpn
				on cpn.CPNAPnum = n.CPNAPnum
				and cpn.CPNAPDlinea = n.CPNAPDlinea
				and cpn.CPCano = n.CPCano
				and cpn.CPCmes = n.CPCmes
				and cpn.CPcuenta = n.CPcuenta
				and cpn.CPPid = <cfqueryparam  cfsqltype="cf_sql_numeric" value="#CPPid#">
				and cpn.CPCmes = <cfqueryparam  cfsqltype="cf_sql_numeric" value="#cmbMeses#">
			...
			inner join CPresupuestoControl cpc
				...
				and cpc.CPCano = n.CPCano
				and cpc.CPCmes = n.CPCmes
			where n.Ecodigo = <cfqueryparam  cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
</cfquery>
			...
			<cfquery name="rsInsertintPresupuesto" datasource="#session.dsn#">
				insert into #request.intPresupuesto#
			        ...
			        values (<cfqueryparam  cfsqltype="cf_sql_varchar" value="#rsLineas.CPNAPmoduloOri#">,
			            ...
			            'CC',
			            #qry_monedaEmpresa.Mcodigo#, -1*#rsLineas.NAPMonto#,
			            1, -1*#rsLineas.NAPMonto#, #rsLineas.CPNAPnum#,#rsLineas.CPNAPDlinea#
			        )
			   </cfquery>
			   <!--- </cfif> --->
		</cfloop>
	...
	<cfif rs_regporCompr.recordCount NEQ 0>
		<cfset LvarNAP = LobjControl.ControlPresupuestario(ModuloOrigen="PRFO", NumeroDocumento=LvarDocumentoAprobo, NumeroReferencia="MODIFICACION", FechaDocumento=LvarFecha,
															AnoDocumento=LvarAno, MesDocumento=LvarMes,ConciliaML="true") />
	<cfelse>
		<cfset LvarNAP = -1 />
	</cfif>
	...
	<cfelse>
		...
		<cfquery name="insCPresupuestoComprometidasNAPs" datasource="#session.dsn#">
	        insert into CPresupuestoComprometidasNAPs
	                    ...
	                    SELECT <cfqueryparam  cfsqltype="cf_sql_integer" value="#Session.Ecodigo#">, <cfqueryparam  cfsqltype="cf_sql_numeric" value="#CPPid#">,
...
<cfoutput>
<form action="LiberaCompMesesAnt.cfm" method="post" name="sql" onSubmit="return validar();">
    <input name="cmbMeses" type="hidden" value="<cfif isdefined("cmbMeses")>#cmbMeses#</cfif>">
	<input name="CPPid" type="hidden" value="<cfif isdefined("CPPid")>#CPPid#</cfif>">
</form>
</cfoutput>
<HTML>
<head>
