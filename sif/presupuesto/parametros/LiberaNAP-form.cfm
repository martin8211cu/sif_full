<cfinvoke component="sif.Componentes.Translate" method="Translate" key="MSG_DeseaReversarNAP" default ="Desea REVERSAR la(s) linea(s) seleccioanda(s)" returnvariable="MSG_DeseaReversarNAP" xmlfile = "ReversarNAPs.xml">
			...
			<cf_web_portlet_start titulo="Detalle del NAP de Aprobación del Rechazo Presupuestario">
				...
				<cfinvoke
				 ...
				 returnvariable="pListaRet">
					...
					<cfinvokeargument name="desplegar" value="CPNAPDlinea, MesDescripcion, CuentaPresupuesto, Odescripcion, TipoControl, CalculoControl, TipoMovimiento, DisponibleAnterior, Signo, Monto, DisponibleGenerado, Referencia"/>
					<cfinvokeargument name="etiquetas" value="Linea, Mes, Cuenta<BR>Presupuesto, Oficina, Tipo<BR>Control, Cálculo<BR>Control, Tipo<BR>Movimiento, Disponible<BR>Anterior,Signo<BR>Monto,Monto<BR>Autorizado, Disponible<BR>Generado, NAP<BR>Ref."/>
					<cfinvokeargument name="formatos" value="V,V,V,V,V,V,V,M,V,M,R CPNAPDconExceso EQ 1,V"/>
					<cfinvokeargument name="align" value="left, center, left, left, center, center, center, right, center, right, right, center"/>
		...
		<td align="center" style="padding-left: 10px; padding-right: 10px;">
			<cf_web_portlet_start titulo="Detalle del NAP">
				...
				<cfquery name="rsListaNAPS" datasource="#Session.DSN#">
					...
					from CPNAP n, CPNAPdetalle a, CPresupuesto b, Oficinas o
					where a.Ecodigo = #session.ecodigo#
						and n.CPNAPnum = a.CPNAPnum
						and a.CPNAPnum = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.CPNAPnum#">
						and b.CPcuenta = a.CPcuenta
						and b.Ecodigo  = a.Ecodigo
						and o.Ecodigo = a.Ecodigo
				...
				<cfinvoke
				 ...
				 returnvariable="pListaRet">
					...
					<cfinvokeargument name="navegacion" value="#navegacion#"/>
					<cfinvokeargument name="debug" value="N"/>
					<cfinvokeargument name="keys" value="CPPid,CPNAPnum,CPNAPDlinea,CPCano,CPCmes,CPNAPmoduloOri"/>
					<cfinvokeargument name="mostrar_filtro" value="false"/>
					<cfif rsListaNAPS.recordcount GT 0>
						<cfinvokeargument name="botones" value="Reversar">
					</cfif>
				</cfinvoke>
				<!--- <cfoutput>
...
<cfoutput>
	<script language="javascript1.2" type="text/javascript">
		...
		function algunoMarcado(){
			var aplica = false;
			if (document.listaNAPS.chk) {
				if (document.listaNAPS.chk.value) {
					aplica = document.listaNAPS.chk.checked;
				}else{
					for (var i=0; i<document.listaNAPS.chk.length; i++) {
						if (document.listaNAPS.chk[i].checked) {
							var Documento = document.listaNAPS.chk[i].value;
							aplica = true;
							break;
						}
					}
			...
			if (aplica) {
				return (confirm("¿<cfoutput>#MSG_DeseaReversarNAP#</cfoutput>?"));
			} else {
				<!--- alert('#MSG_NAPAReversar#'); --->
				return false;
			}
		}
		...
		function funcReversar(){
			if (algunoMarcado()){
				...
				if (document.listaNAPS.chk.value) {
					var Documento = document.listaNAPS.chk.value;
				}else{
					for (var i=0; i<document.listaNAPS.chk.length; i++) {
						if (document.listaNAPS.chk[i].checked) {
							var Documento = document.listaNAPS.chk[i].value;
				...
				document.listaNAPS.action = "LiberaNAP-SQL.cfm";
				return true;
			}
			return false;
		}
