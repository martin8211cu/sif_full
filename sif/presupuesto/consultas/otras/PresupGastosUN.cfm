...
from CPresupuesto cp
	left outer join CPresupuesto cp1
 	...
 	inner join CPresupuestoControl cpc
	  ...
	  and cpc.CPCano = #trim(url.CPCano)#
	  and cpc.CPCmes = #trim(url.CPCmes)#
...
<cfreport format="flashpaper" query="rsReporte" template="PresupGastosUN.cfr">
	...
	<cfif isDefined("url.ECfechacotI") and len(trim(url.ECfechacotI)) NEQ 0>
		<cfreportparam name="ECfechacotI"   value="#LSParseDateTime(url.ECfechacotI)#">
	<cfelse>
		<cfreportparam name="ECfechacotI"   value="#LSParseDateTime('01/01/1900')#">
	</cfif>
	
	<cfif isDefined("url.ECfechacotF") and len(trim(url.ECfechacotF)) NEQ 0>
		<cfreportparam name="ECfechacotF"   value="#LSParseDateTime(form.ECfechacotF)#">
	<cfelse>
			<cfreportparam name="ECfechacotF"    value="#LSParseDateTime('01/01/2100')#">
	</cfif>
