...
<cfquery  name="rsImportador" datasource="#session.dsn#">
  ...
  order by CPformato, Oficodigo, Miso4217
</cfquery>
...
<cfset LvarCtaAnterior = "">
	...
	<cfloop query="rsImportador">
		...
		<cfscript>
			LvarCtaActual = trim(rsImportador.CPformato);
			LvarOfiActual = trim(rsImportador.Oficodigo);
			LvarMoneda = trim(rsImportador.Miso4217);
			...
			if (LvarCtaAnterior NEQ LvarCtaActual)
			{
				...
				if (rsImportador.CVPtipoControl EQ "")
					LvarCVPtipoControl = -1;
				else
					LvarCVPtipoControl = rsImportador.CVPtipoControl;
				
				if (rsImportador.CVPcalculoControl EQ "")
					LvarCVPcalculoControl = -1;
				else
					LvarCVPcalculoControl = rsImportador.CVPcalculoControl;
				
				LvarError = fnGeneraCuentaVersion (rsImportador.CPformato, LvarFechaIni, session.CVid, LvarCVPtipoControl, LvarCVPcalculoControl, rsImportador.CPdescripcion);
				...
				if (LvarError EQ "NEW" OR LvarError EQ "OLD")
				{
										...
										select 	cp.CPcuenta
										  from CPresupuesto cp
											inner join CPVigencia vg
											  ...
											  and #rsImportador.CPCano1*100 + rsImportador.CPCmes1# between vg.CPVdesdeAnoMes and vg.CPVhastaAnoMes
										 where cp.CPformato 	='#rsImportador.CPformato#'
					...
					rsSQL = fnQuery("
										select CVPcuenta, CVPtipoControl, CVPcalculoControl, CPdescripcion
										 ...
										 where Ecodigo=#session.Ecodigo# 
										   ...
										   AND Cmayor='#mid(rsImportador.CPformato,1,4)#'
										   AND CPformato='#rsImportador.CPformato#'
									");
					...
					if ( (LvarCVPtipoControl NEQ -1 AND LvarCVPtipoControl NEQ rsSQL.CVPtipoControl) OR (LvarCVPcalculoControl NEQ -1 AND LvarCVPcalculoControl NEQ rsSQL.CVPcalculoControl) )
					{
						sbExecute ("
									...
									");
						sbError ("INFO", "La Cuenta de Presupuesto '#rsImportador.CPformato#' ya existía y se actualizó su tipo y método de Control");
					}
				...
				{
					LvarCVPcuenta = "";
					sbError ("FATAL", "Cuenta de Presupuesto '#rsImportador.CPformato#': " & LvarError);
					continue;
				}
			}
			...
			if (LvarCVPcuenta NEQ "" AND LvarOcodigo NEQ "")
			{
				...
				{
					sbError ("FATAL", "Se cambio el año y mes inicial de la Importacion con la Fecha Inicial del Período: Cuenta '#rsImportador.CPformato#', Oficina '#rsImportador.Oficodigo#', Moneda '#rsImportador.Miso4217#', Mes Inicial '#rsImportador.CPCano1#-#rsImportador.CPCmes1#'");
					continue;
				}
				...
				for (i=1; i LTE 12; i=i+1)
				{
					...
					{
						sbError ("FATAL", "El monto debe ser numerico '#LvarMonto#': Cuenta '#rsImportador.CPformato#', Oficina '#rsImportador.Oficodigo#', Moneda '#rsImportador.Miso4217#', Mes '#LvarAno#-#LvarMes#'");
						break;
					}
					if (NOT (LvarMonto EQ "" OR LvarMonto EQ 0 AND LvarAnoMes GT LvarAnoMesFin)) //  AND val(LvarMonto) GT 0 
					{
						...
						{
							sbError ("FATAL", "Se definieron más montos que el numero de meses del Periodo: Cuenta '#rsImportador.CPformato#', Oficina '#rsImportador.Oficodigo#', Moneda '#rsImportador.Miso4217#', Mes '#LvarAno#-#LvarMes#'");
							break;
						}
						...
						if (LvarPrimerMonedaXoficina[i] or true)
						{
							...
							{
								sbError ("INFO", "Se sustituyó la Formulación para: Cuenta '#rsImportador.CPformato#', Oficina '#rsImportador.Oficodigo#', Mes '#LvarAno#-#LvarMes#'");
								sbExecute ("
