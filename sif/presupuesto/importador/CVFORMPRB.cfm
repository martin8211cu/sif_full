...
<cfquery  name="rsImportador" datasource="#session.dsn#">
  ...
  order by Programa,Rubro,Subrubro, Oficodigo, Miso4217
</cfquery>
...
<!--- <cfelseif trim(rsImportador.CPformato) EQ "">
...
<cf_dbtemp name="Datos" returnvariable="GRUPO" datasource="#session.dsn#">
	<cf_dbtempcol name="Grupo"		type="varchar(100)"  	 mandatory="yes">
...
<cfset LvarCtaAnterior = "">
	...
	<cfloop query="rsImportador">
		<cfset LvarOficina = trim(rsImportador.Oficodigo)>
		<cfset LvarMoneda = trim(rsImportador.Miso4217)>
		<cfset LvarTipoControl = trim(rsImportador.CVPtipoControl)>
		<cfset LvarCalculoControl = trim(rsImportador.CVPcalculoControl)>
		<cfset LvarMes = trim(rsImportador.CPCmes1)>
		<cfset LvarPeriodo = trim(rsImportador.CPCano1)>
		...
		<cfset myTable = LobjControl.CreaTablaValPresupuesto(#session.dsn#,true,#rsImportador.Programa#,#rsImportador.Rubro#,#rsImportador.Subrubro#)>
...
<!--- 		<cfquery datasource="#Session.Dsn#">
	...
	select a.Cmayor +'-'+ b.Grupo as CPformato, b.Oficina,Moneda,TipoControl,CalculoControl,Mes,Periodo
	from (
			select Cmayor from  CtasMayor
			where Ecodigo = #Session.Ecodigo#
			and CtaFormulacion = 1
		 ) a,
		 (
		  	select * from #GRUPO# 
		 )b
	...
	<cfloop query="rsCtasMayor">
		<cfscript>
			LvarCuenta = trim(rsCtasMayor.CPformato);
			LvarOficina = trim(rsCtasMayor.Oficina);
			LvarMoneda = trim(rsCtasMayor.Moneda);
			LvarMes = trim(rsCtasMayor.Mes);
			LvarPeriodo = trim(rsCtasMayor.Periodo);
				
			/* if (LvarCtaAnterior NEQ LvarCtaActual)
			{ */
				LvarCtaAnterior = LvarCuenta;
				LvarOfiAnterior = "";
				LvarCPcuenta 	= "";

				if (rsCtasMayor.TipoControl EQ "")
					LvarCVPtipoControl = -1;
				else
					LvarCVPtipoControl = rsCtasMayor.TipoControl;
				
				if (rsCtasMayor.CalculoControl EQ "")
					LvarCVPcalculoControl = -1;
				else
					LvarCVPcalculoControl = rsCtasMayor.CalculoControl;
				
				LvarError = fnGeneraCuentaVersion (rsCtasMayor.CPformato, LvarFechaIni, session.CVid, LvarCVPtipoControl, LvarCVPcalculoControl,'Cuenta creada en Formulación Presuestal '/* , rsImportador.CPdescripcion */);
				...
				if (LvarError EQ "NEW" OR LvarError EQ "OLD")
				{
										...
										select 	cp.CPcuenta
										  from CPresupuesto cp
											inner join CPVigencia vg
											  ...
											  and #rsCtasMayor.Periodo*100 + rsCtasMayor.Mes# between vg.CPVdesdeAnoMes and vg.CPVhastaAnoMes
										 where cp.CPformato 	='#rsCtasMayor.CPformato#'
					...
					rsSQL = fnQuery("
										select CVPcuenta, CVPtipoControl, CVPcalculoControl, CPdescripcion
										 ...
										 where Ecodigo=#session.Ecodigo# 
										   ...
										   AND Cmayor='#mid(rsCtasMayor.CPformato,1,4)#'
										   AND CPformato='#rsCtasMayor.CPformato#'
									");
					...
					if ( (LvarCVPtipoControl NEQ -1 AND LvarCVPtipoControl NEQ rsSQL.CVPtipoControl) OR (LvarCVPcalculoControl NEQ -1 AND LvarCVPcalculoControl NEQ rsSQL.CVPcalculoControl) )
					{
						sbExecute ("
									...
									");
						sbError ("INFO", "La Cuenta de Presupuesto '#rsCtasMayor.CPformato#' ya existía y se actualizó su tipo y método de Control");
					}
				...
				{
					LvarCVPcuenta = "";
					sbError ("FATAL", "Cuenta de Presupuesto '#rsCtasMayor.CPformato#': " & LvarError);
					continue;
				}
			/* } */
 ...
 */
		...
		/* 	} */
			...
			if (LvarCVPcuenta NEQ "")
			{
				...
				rsQRY = fnQuery("
									select Ocodigo from Oficinas where Ecodigo=#session.Ecodigo# and Oficodigo='#LvarOficina#'
								");
				...
				{
					LvarOcodigo = "";
					sbError ("FATAL", "Codigo de Oficina '#LvarOficina#' no existe");
					continue;
				}
			}
			...
			if (LvarCVPcuenta NEQ "" AND LvarOcodigo NEQ "")
			{
				...
				if (rsCtasMayor.Periodo NEQ LvarAnoIni OR rsCtasMayor.Mes NEQ LvarMesIni)
				{
					sbError ("FATAL", "Se cambio el año y mes inicial de la Importacion con la Fecha Inicial del Período: Cuenta '#rsCtasMayor.CPformato#', Oficina '#rsCtasMayor.Oficina#', Moneda '#rsCtasMayor.Moneda#', Mes Inicial '#rsCtasMayor.Periodo#-#rsCtasMayor.Mes#'");
					continue;
				}
				...
				for (i=1; i LTE 12; i=i+1)
				{
					...
					{
						sbError ("FATAL", "El monto debe ser numerico '#LvarMonto#': Cuenta '#rsCtasMayor.CPformato#', Oficina '#rsCtasMayor.Oficina#', Moneda '#rsCtasMayor.Moneda#', Mes '#LvarAno#-#LvarMes#'");
						break;
					}
					if (NOT (LvarMonto EQ "" OR LvarMonto EQ 0 AND LvarAnoMes GT LvarAnoMesFin)) //  AND val(LvarMonto) GT 0 
					{
						...
						{
							sbError ("FATAL", "Se definieron más montos que el numero de meses del Periodo: Cuenta '#rsCtasMayor.CPformato#', Oficina '#rsCtasMayor.Oficina#', Moneda '#rsImportador.Moneda#', Mes '#LvarAno#-#LvarMes#'");
							break;
						}
						...
						if (LvarPrimerMonedaXoficina[i] or true)
						{
							...
							{
								sbError ("INFO", "Se sustituyó la Formulación para: Cuenta '#rsCtasMayor.CPformato#', Oficina '#rsCtasMayor.Oficina#', Mes '#LvarAno#-#LvarMes#'");
								sbExecute ("
