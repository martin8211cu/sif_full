...
<td align="center">    
	<cfoutput>
        <form action="exportar_SMaskUser-form-SQL.cfm" method="post" name="formFoundUsucodigo" onsubmit="javascript:return validacion();">
            <input type="hidden" name="Usucodigo" value="<cfif isdefined("form.Usucodigo") and  len(trim(form.Usucodigo)) NEQ 0 >#form.Usucodigo#</cfif>">
...
<cfloop index="x" from="1" to="#arrayLen(listaCod)#">
	...
	setTimeout("mostrarPerf('<cfoutput>#temp#</cfoutput>')",1500);
</cfloop> 
</script>