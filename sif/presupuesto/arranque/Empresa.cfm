<!---
	...
	PROCESO:
		. Se establecen las cuentas de mayor cuya máscara tiene niveles de presupuesto
...
<cfif isdefined("url.MSG")>
	<cf_templatecss>
	<cfparam name="application.CPactEcodigoMSG" default="ACTIVACION DEL CONTROL DE PRESUPUESTO DE UNA EMPRESA">
	<cfoutput>
		<cfif application.CPactEcodigoMSG EQ "*">
			<strong>
				PROCESO FINALIZADO CON EXITO:
				<BR><BR><BR>
				PERIODO DE PRESUPUESTO ACTIVADO
			</strong>
		<cfelse>
			<strong>POR FAVOR ESPERE</strong>
			<BR><BR><BR>
			#application.CPactEcodigoMSG#
			<script language="javascript">
				setTimeout('location.href = "Empresa.cfm?MSG"',2*1000);
			</script>
		</cfif>
	</cfoutput>
	<cfabort>
...
<cf_templateheader title="Activar Control Presupuestario a una Cuenta de Mayor">
	...
	<cfif rsCPP.recordCount EQ 0>
		<cfset LvarError = "No existe un Período de Presupuesto para el Mes Contable Actual. Debe crear un periodo de presupuesto en estado Inactivo">
	<cfelseif rsCPP.CPPestado EQ 1>
		<cfset LvarError = "El Período de Presupuesto #rsCPP.PeriodoPresupuesto# para el Mes Contable Actual ya está Activo">
	<cfelseif rsCPP.CPPestado NEQ 0 AND rsCPP.CPPestado NEQ 5>
		<cfset LvarError = "El Período de Presupuesto #rsCPP.PeriodoPresupuesto# para el Mes Contable Actual no está Inactivo">
	</cfif>
	...
	<cfelse>
		<cfset LvarPeriodoProcesar = "">
	</cfif>

	<cf_dbtemp name="CPC_M_V1" returnvariable="CtasMayor" datasource="#session.DSN#">
		<cf_dbtempcol name="Ecodigo" 		type="int"      	mandatory="yes">
		<cf_dbtempcol name="Cmayor" 		type="varchar(4)"   mandatory="yes">
		<cf_dbtempcol name="Cdescripcion" 	type="varchar(40)"  mandatory="yes">
		...
		<cf_dbtempcol name="conPlan" 		type="int"      	mandatory="yes">
		<cf_dbtempcol name="PCEMformato" 	type="varchar(100)" mandatory="yes">
		<cf_dbtempcol name="PCEMformatoC" 	type="varchar(100)" mandatory="yes">
		<cf_dbtempcol name="PCEMformatoP" 	type="varchar(100)" mandatory="yes">
	</cf_dbtemp>
	<cfquery name="rsCMs" datasource="#session.DSN#">
		insert into #CtasMayor#
			(Ecodigo, Cmayor, Cdescripcion, Ctipo, CPVid, PCEMid, conPlan, PCEMformato, PCEMformatoC, PCEMformatoP)
		select m.Ecodigo, m.Cmayor, m.Cdescripcion, m.Ctipo, 
				vg.CPVid, vg.PCEMid, 
				coalesce(mp.PCEMplanCtas,0) as conPlan,
				mp.PCEMformato, mp.PCEMformatoC, mp.PCEMformatoP
		  from CtasMayor m
			inner join CPVigencia vg
				inner join PCEMascaras mp 
					 on mp.PCEMid		= vg.PCEMid
					and mp.PCEMformatoP	IS NOT NULL 
	...
	<cfquery name="rsCMs" datasource="#session.DSN#">
		select Ecodigo, Cmayor, Cdescripcion, CPVid, PCEMid, conPlan, PCEMformato, PCEMformatoC, PCEMformatoP
		  from #CtasMayor#
	</cfquery>

	<cfoutput>
	<form name="frmArranque" method="post" action="Empresa.cfm">
		<table>
			<tr><td>&nbsp;</td></tr>
			<tr>
				<td width="80%" align="center" colspan="2" style="font-size:16px; font-weight:bold; color:##0000FF">
					Este proceso activa el Control de Presupuesto para todas las cuentas con Presupuesto de la Empresa
				</td>
			</tr>
			<tr><td>&nbsp;</td></tr>
			<tr>
			...
			<tr>
				...
				<td>
					<cfif rsCMs.recordCount EQ 0>
						<strong>(No hay cuentas de Presupuesto a Activar)</strong>
						<br>
						Después de Activar el Período de Presupuesto, tiene que activar las Cuentas de Mayor una por una
					<cfelse>
						<select name="Cmayor" id="Cmayor" onchange="this.selectedIndex=-1;"
							size="<cfif rsCMs.recordCount LT 8>#rsCMs.recordCount#<cfelse>8</cfif>"
						>
						<cfloop query="rsCMs">
							<option value="#rsCMs.Cmayor#">
								#rsCMs.Cmayor# - #rsCMs.Cdescripcion# 
								<cfif rsCMs.PCEMid EQ "">
									(Sin Máscara)
								<cfelseif rsCMs.conPlan EQ "1">
									(Con Plan)
								<cfelse>
									(Sin Plan)
								</cfif>
		...
		<cfif LvarError NEQ "">
			<table width="100%">
				...
				<tr><td>&nbsp;</td></tr>
				<tr><td>&nbsp;</td></tr>
				<tr><td align="center" style="font-size:14px; font-weight:bold; color:##FF0000">
					#LvarError#
				</td></tr>
			</table></form>
			<cfabort>
		...
		<cfif isdefined("url.Activar") and url.Activar EQ "1">
			<cfset application.CPactEcodigo = false>
		</cfif>
		<cfparam name="application.CPactEcodigo" default="false">
		<cfif application.CPactEcodigo>
			<table align="center">	
				<tr><td>&nbsp;</td></tr>
				<tr>
			...
			<BR><BR><BR>
			<iframe src="Empresa.cfm?MSG" id="ifrMSG" height="100" width="800" frameborder="0">
			<cfset application.CPactEcodigoMSG = "*">
			<cfabort>
		</cfif>

		<cfset application.CPactEcodigoMSG = "ACTIVACION DEL CONTROL DE PRESUPUESTO DE UNA EMPRESA">
		<cfif isdefined("form.btnActivar")>
			<!--- cboTipo eq 2: Se presiono btnActivar y se visualiza la pantalla --->
			<cfoutput>
			<BR><BR><BR>
			<strong>Activando el Período de Presupuesto #rsCPP.PeriodoPresupuesto# para la Empresa #session.Enombre#...</strong>
			<BR><BR><BR>
			</cfoutput>
			<cfset LvarParams = "CVMcalculoControl=#form.CVMcalculoControl#&CVMtipoControl=#form.CVMtipoControl#&CtaOfiFormular=#form.CtaOfiFormular#">
			<iframe src="Empresa.cfm?MSG" id="ifrMSG" height="100" width="800" frameborder="0">
			<iframe src="Empresa.cfm?activar&#LvarParams#" 
		...
		<cfelseif isdefined("url.Activar")>
			...
			<cfset LvarResultado = fnActivarEcodigo()>
			<script language="javascript">
				//parent.document.getElementById("ifrMSG").load();
				//parent.document.getElementById("ifrMSG").src="Empresa.cfm?MSG";
				//alert(parent.document.getElementById("ifrMSG").src);
				setTimeout('sbFinalizar()',2*1000);
				function sbFinalizar()
				{
					alert ("#LvarResultado#");
		...
		<cfelse>
			...
			<table align="center" align="100%">
				<tr>
					<td>
						<input type="submit" name="btnActivar" value="Activar" onclick="if (this.form.CtaOfiFormular.value == '') {alert('Debe escoger el tipo de Formulación'); return false;} return confirm('¿Desea Activar el Período de Presupuesto #rsCPP.PeriodoPresupuesto# para la Empresa?');">
					</td>
				</tr>
			</table>
			<table align="center" align="100%">
				...
				<tr>
					<td>
						&nbsp;&nbsp;&nbsp;&nbsp;
						. Se generan todas las cuentas de presupuesto en CPrespuesto a partir de las cuentas financieras existentes que no tengan cuenta de Presupuesto
					</td>
				</tr>
				<tr>
					<td>
...
<cffunction name="fnActivarEcodigo" returntype="string" output="false">
	...
	<cftry>
		<cfset application.CPactEcodigo = true>
		<cftransaction>
			<cfset application.CPactEcodigoMSG = "Creacion Cuentas de Presupuesto">
			<cfset LvarResultado = fnCrearCtasPresupuesto()>
			<cfset application.CPactEcodigoMSG = "Generacion de Saldos de Presupuesto">
			<cfset LvarResultado = LvarResultado & fnGenerarSaldosPresupuesto()>
			<cfset application.CPactEcodigoMSG = "Generacion de Movimientos de Ejecucion">
...
<cffunction name="fnObtenerSQL_CPformato" output="false" returntype="string">
	<cfargument name="PCEMid" type="numeric" required="yes">
	...
	<cfquery name="rsNiveles" datasource="#session.DSN#">
		select 	n.PCNid, n.PCNdescripcion, 
		 ...
		 where n.PCEMid = #Arguments.PCEMid#
	</cfquery>
	<cfset LvarSubstring = "">
	<cfset LvarPos = "5">
	<cfset LvarNivF = 0>
	...
	<cfloop query="rsNiveles">
		<cfif rsNiveles.PCNpresupuesto EQ 1>
			...
			<cfset LvarNivP = LvarNivP + 1>
		</cfif>
		<cfset LvarNivF = LvarNivF + 1>
		<cfset LvarPos = LvarPos + rsNiveles.PCNlongitud + 1>
	</cfloop>

	<cfif LvarNivF EQ LvarNivP>
		<cfset LvarSubstring = "CFformato">
	</cfif>
	<cfreturn LvarSubstring>
</cffunction>


<cffunction name="fnCrearCtasPresupuesto" output="false" returntype="string">
	<cfset var LvarResultado = "Se Activaron #rsCMs.recordCount# cuentas de Mayor:">

	<!--- Activa el Periodo de Presupuesto --->
	<cfquery datasource="#session.dsn#">
		update CPresupuestoPeriodo
		   set CPPestado = 1
		 where Ecodigo	= #session.Ecodigo#
		   and CPPid	= #rsCPP.CPPid#
	</cfquery>
	
	<cfloop query="rsCMs">
		<cfset application.CPactEcodigoMSG = "Creacion Cuentas de Presupuesto #rsCMs.Cmayor#">
		<!--- Genera las Cuentas de Presupuesto --->
		<cfset LvarSQL_CPformato  = fnObtenerSQL_CPformato (rsCMs.PCEMid)>
		<cfquery name="rsCFinanciera" datasource="#session.dsn#">
			select 	distinct c.CFcuenta, c.CPcuenta, c.CFformato, #LvarSQL_CPformato# as CPformato,
					mp.PCEMformato, mp.PCEMformatoP
			  from	CFinanciera c
				  inner join #CtasMayor# mp
					 on mp.Ecodigo = c.Ecodigo
					and mp.Cmayor  = c.Cmayor
			  where c.Ecodigo	= #session.Ecodigo#
				and c.Cmayor	= '#rsCMs.Cmayor#'
				and c.CPVid		= #rsCMs.CPVid#
	
				and c.CFmovimiento	= 'S'
				and c.Cmayor		<> c.CFformato
		</cfquery>
		<cfset LvarMascaraN = len(trim(rsCMs.PCEMformato))>
		<cfset LvarNew = 0>
		<cfset LvarOld = 0>
		<cfloop query="rsCFinanciera">
			<cfset application.CPactEcodigoMSG = "Creacion Cuentas de Presupuesto #rsCMs.Cmayor# (#rsCFinanciera.currentRow# de #rsCFinanciera.recordCount#)">
			<cfif len(trim(rsCFinanciera.CFformato)) NEQ LvarMascaraN>
				<cf_errorCode	code = "50461"
								...
								errorDat_2="#rsCMs.PCEMformato#"
				>
			</cfif>
			<cfif rsCFinanciera.CPcuenta EQ "">
				<cfset LvarNew = LvarNew + 1>
					...
					(
						select CPcuenta
						 ...
						 where Ecodigo = CFinanciera.Ecodigo
						   and CPformato = #replace(LvarSQL_CPformato,"CFformato","CFinanciera.CFformato","ALL")#
					)
			  where Ecodigo		 = #session.Ecodigo#
				and Cmayor		 = '#rsCMs.Cmayor#'
				and CPVid		 = #rsCMs.CPVid#
				and CFmovimiento = 'S'
				and Cmayor		 <> CFformato
				and CPcuenta	 IS NULL
		</cfquery>
	
		<cfset LvarResultado = LvarResultado & "\n  . Mayor #rsCMs.Cmayor#: Se activaron #rsCFinanciera.recordCount# Cuentas Financieras: #LvarNew# nuevas de Presupuesto y #LvarOld# ya existentes">
	</cfloop>
...
<cffunction name="fnGenerarSaldosPresupuesto" output="false" returntype="string">
	...
	--->
	<cfset LvarCVdescripcion = "ACTIVA PRESUPUESTO A EMPRESA #session.Enombre#">
	...
	<cfset application.CPactEcodigoMSG = "Generacion de Saldos de Presupuesto: Version de Presupuesto">
	<cfquery name="rsSQL" datasource="#session.dsn#">
	...
	<cfif url.CtaOfiFormular EQ "M">
		<cfquery datasource="#session.dsn#">
			...
			select 	DISTINCT c.CPcuenta, d.Ocodigo
			  from	CFinanciera c
			  	inner join #CtasMayor# vm
					 on vm.Ecodigo	= c.Ecodigo
					and vm.Cmayor	= c.Cmayor
					and vm.CPVid	= c.CPVid
	...
	<cfset application.CPactEcodigoMSG = "Generacion de Saldos de Presupuesto: Cuentas de Version de Presupuesto">
	<cfquery name="rsSQL" datasource="#session.dsn#">
		...
		select	DISTINCT 
		  ...
		  from CPresupuesto cp
			inner join #CtasMayor# vm
				 on vm.Ecodigo	= cp.Ecodigo
				and vm.Cmayor	= cp.Cmayor
				and vm.CPVid	= cp.CPVid
	...
	<cfset application.CPactEcodigoMSG = "Generacion de Saldos de Presupuesto: Totales de Version de Presupuesto">
	<cfquery name="rsSQL" datasource="#session.dsn#">
	...
	<cfset application.CPactEcodigoMSG = "Generacion de Saldos de Presupuesto: Monedas de Version de Presupuesto">
	<cfquery name="rsSQL" datasource="#session.dsn#">
	...
	--->
	<cfset application.CPactEcodigoMSG = "Generacion de Saldos de Presupuesto: Actualiza Formulacion de Version de Presupuesto con SaldosContablesP">
	<cfquery name="rsSQL" datasource="#session.dsn#">
	...
	<cfset application.CPactEcodigoMSG = "Generacion de Saldos de Presupuesto: Ajuste Formulacion de Version de Presupuesto">
	<!---
	...
	<cfset application.CPactEcodigoMSG = "Generacion de Saldos de Presupuesto: Genera CPresupuestoControl">
	<cfquery name="rsSQL" datasource="#session.dsn#">
	...
	<cfset application.CPactEcodigoMSG = "Generacion de Saldos de Presupuesto: Genera CPControlMoneda">
	<cfquery name="rsSQL" datasource="#session.dsn#">
	...
	<cfset application.CPactEcodigoMSG = "Generacion de Saldos de Presupuesto: Genera NAP de APROBACION">
	<cfquery name="rsSQL" datasource="#session.dsn#">
	...
	<cfquery name="rsSQL" datasource="#session.dsn#">
		...
		select 	#session.Ecodigo#, #LvarNAP#, 
		 ...
		 where fm.Ecodigo 	 = #session.Ecodigo#
		   and fm.CVid 	     = #LvarCVid#
		   and fm.CVFMmontoAplicar <> 0.00
	</cfquery>

	<cfquery name="rsSQL" datasource="#session.dsn#">
		select 	count(1) as Cantidad,
				count(distinct fm.CVPcuenta)	as CantidadC,
				count(distinct fm.Ocodigo)		as CantidadO,
				count(distinct fm.CPCmes)		as CantidadM
		  from CVFormulacionMonedas fm
		 where fm.Ecodigo 	 = #session.Ecodigo#
		   and fm.CVid 	     = #LvarCVid#
	</cfquery>
...
<cffunction name="fnEjecucion" output="false" returntype="string">
	...
	<cfset LvarReferenciaOri = "ACTIVA PRESUP. EMPRESA">
	...
	<cfloop condition="LvarMes LTE LvarMesAct">
		...
		<cfset application.CPactEcodigoMSG = "Generacion de Movimientos de Ejecucion: #LvarDocumentoOri#">
