...
function llenacaja() {
	...
	frame.src = "/cfmx/sif/V5/fondos/operacion/cajanegra/Pintacajas.cfm"+params;
}
<cfif mododet eq "CAMBIO">
	function llenacajaCambio() {
		...
		var frame = document.getElementById("FRAMECJNEGRA");
		frame.src = "/cfmx/sif/V5/fondos/operacion/cajanegra/PintacajasCambio.cfm"+params;
	}

	function DesCuenta(CGM1ID) {
			params = "?CGM1ID="+CGM1ID;
			var frame = document.getElementById("FRAMECJNEGRA");
			frame.src = "/cfmx/sif/V5/fondos/operacion/cajanegra/TraeDes.cfm"+params;
	}
</cfif>	
...
function doKey(EVENTO,CGM1IM,NIVEL){ 
	...
	if (k==TECLA_CONLIS) {
		...
		popUpWindow("/cfmx/sif/V5/fondos/operacion/cajanegra/ConlisCJNegra.cfm"+params,250,200,650,400);
	}
}
...
function validaceldas(nivel) {
	...
	if (llenas){
		...
		var frame = document.getElementById("FRAMECJNEGRA");
		frame.src = "/cfmx/sif/V5/fondos/operacion/cajanegra/CreaCuenta.cfm"+params;
	}
}
...
function TraeDescripcion(DATO,CGM1IM,NIVEL) {
	if (DATO != "") {
		...
		StringNivel = StrNivel(NIVEL);
		var frame = document.getElementById("FRAMECJNEGRA");
		frame.src = "/cfmx/sif/V5/fondos/operacion/cajanegra/CJNegraquery.cfm?DATO="+DATO+"&CGM1IM="+CGM1IM+"&NIVEL="+NIVEL+"&StringNivel="+StringNivel;
	}
	else{
		eval("document.form1.CG13ID_"+NIVEL+".value=''")
