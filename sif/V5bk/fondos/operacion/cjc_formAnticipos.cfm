...
<cfquery name="rsDigitado" datasource="#session.Fondos.dsn#">
	select convert(varchar(14),isnull(sum(CJX20MNT),0),1) CJX20MNT
	from CJX019, CJX020
...
<cfquery name="rsAnticipos" datasource="#session.Fondos.dsn#">
	select convert(varchar(14),isnull(sum(CJX23MON),0),1) CJX23MON
	from CJX023
	where CJX19REL = <cfqueryparam cfsqltype="cf_sql_integer" value="#form.CJX19REL#">
</cfquery>
...
<table width="100%" border="0" >
		...
		<!---********************************************************************************* --->
			<form action="cjc_SqlAnticipos.cfm" method="post" name="form1" 
			...
			<cfoutput>
					<table width="100%" border="0">
					  ...
					  <tr>
						...
						<td width="56%"><select name="CJX19REL" onChange="validaREL()">
...
<!--- 							<OPTION  VALUE="V" <cfif modo NEQ 'ALTA' and TraeSql.CJX23TIP eq "V" >selected</cfif>>VALE</OPTION>
 --->							</SELECT>				
					  ...
					  <cfif Condicion neq "1">
							...
							<tr>
								<td>
									<input type="hidden" name="CJX23FEC" value=""  >						  	  </td>
							</tr> 					  
							<!---********************************************************* --->			
							 <tr>
								<td>
						...
						<tr>
					     ...
					     <!---********************************************************* --->
						  <TR>
						      <td ></td>
							  <TD colspan="4">
                                <input type="radio" style="visibility:" name="CJX23TTR" id="CJX23TTR1" value="C"<cfif modo NEQ 'ALTA' and TraeSql.CJX23TTR eq "C" >checked <cfelse>checked </cfif>>								
                                Compra	Directa							
                                <INPUT TYPE="radio" style="visibility:" NAME="CJX23TTR" ID="CJX23TTR2" VALUE="D" <cfif modo NEQ 'ALTA' and TraeSql.CJX23TTR eq "D" >checked </cfif>>
								Devoluci&oacute;n &nbsp;&nbsp;&nbsp;
								<INPUT TYPE="radio" style="visibility:" NAME="CJX23TTR" ID="CJX23TTR3"VALUE="R" <cfif modo NEQ 'ALTA' and TraeSql.CJX23TTR eq "R" >checked </cfif>>
								Retiro
							</TD>
						</TR>			
					     <!---********************************************************* --->
...
<script language="JavaScript1.2" type="text/javascript">
	...
	function TRAE_AUTO(obj){
		...
		frame.src = "/cfmx/sif/V5/fondos/operacion/CargaAuto.cfm"+params;		
	...
	function Llenado_Auto (){
		...
		frame.src = "/cfmx/sif/V5/fondos/operacion/cjc_CargaAuto.cfm"+params;
	}
