...
<!--- Regresar datos aleatorios --->
<cfquery datasource="sdc" name="nombre" maxrows="1">
	select issue_dt, expiry_dt, first_names, last_name, extra_char_1 from pasaportes..CONSULTA
	where personal_id_nbr = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Trim(form.cedula)#">
	order by first_names
</cfquery>
<cfset __estados = "NUEVO,VIGENTE,RENOVAR,VALIDAR">
<cfset __nombres = "JUAN,ARMANDO,CARLA,LIGIA,DANIEL,MAURICIO,ERNESTO,VICTOR,HANNIA,CARLOS,SAUL,MANUEL,ALFONSO,JESSICA">
<cfset __apellidos = "PEREZ,LOPEZ,GARCIA,RODRIGUEZ,PACHECO,JIMENEZ,PALMA,CHAVARRIA,PORRAS,BOLAÑOS,BALTODANO,MENA,FUENTES,MORENO">
	<cfset WSestado=ListGetAt(__estados,Int(Rand()*4)+1)>
	<cfset WSnombre=ListGetAt(__nombres,  Int(Rand()*ListLen(__nombres  ))+1) & " " &
					ListGetAt(__apellidos,Int(Rand()*ListLen(__apellidos))+1) & " " &
					ListGetAt(__apellidos,Int(Rand()*ListLen(__apellidos))+1)>
	<cfset WSexpedido=CreateDate(1998 + Int(Rand()*5), Rand()*12+1,Rand()*28+1) >
	<cfset WSexpira=DateAdd("y", 5, WSexpedido)>
	<cfset WSnombre=nombre.first_names & " " & nombre.last_name & " " & nombre.extra_char_1> 

	<cfset Pestado='N'>
	<cfif nombre.RecordCount EQ 0>
		<cfset Pestado='N'><!--- nuevo --->
	<cfelseif nombre.expiry_dt GT Now()>
		<cfset Pestado='V'><!--- vigente --->
	<cfelseif DateDiff("yyyy", nombre.issue_dt, Now()) LE 10>
		<cfset Pestado='R'><!--- revalidar --->
	<cfelse>
		<cfset Pestado='E'><!--- expirado --->
	</cfif>
