<!--- 
	...
	Modificado por M. Esquivel 20 / Bov / 2005
	-	Cambio en la función replace(replace(replace(replace(Movimiento ...
		porque al parecer tiene que levantar 4 instancias de la clase /usr/lib/libC.a que mide casi 7 mb y esto le duele a WAS  
		Se modifica el select para que el campo de ordenamiento venga separado del campo de documento afectado,
		de esta forma, no se requiere utilizar una función de string sino que se imprime directamente el resultado obtenido de la BD.
		
	-	Evitar que se seleccionen filtros que generen gran cantidad de registros
...
<cfquery name="rsData" datasource="#Session.DSN#">
	...
	<!---  Documentos Debito o Credito generados directamente CCTcodigo = CCTRcodigo and Ddocumento = DRdocumento --->
	select 
		sn.SNnombre, 
		a.CCTcodigo, 
		t1.CCTdescripcion, 
		...
		a.Dfecha, 
		'1' as Orden, 
		'Creación' as Movimiento, 
		t1.CCTtipo, 
		a.CCTRcodigo, 
		a.DRdocumento
	from SNegocios sn, BMovimientos a, CCTransacciones t1
	where 
		  sn.Ecodigo =  <cfqueryparam cfsqltype="cf_sql_integer" value="#session.ecodigo#">
	  and sn.SNnumero
	  <cfif isdefined("Form.SNnumero1") and Len(Trim(Form.SNnumero1)) and isdefined("Form.SNnumero2") and Len(Trim(Form.SNnumero2)) and  form.SNnumero2 GTE SNnumero1 >
		 between <cfqueryparam cfsqltype="cf_sql_char" value="#Form.SNnumero1#"> and <cfqueryparam cfsqltype="cf_sql_char" value="#Form.SNnumero2#">
	  </cfif>

	  <cfif isdefined("Form.SNnumero1") and Len(Trim(Form.SNnumero1)) and isdefined("Form.SNnumero2") and Len(Trim(Form.SNnumero2)) and form.SNnumero2 LT SNnumero1 >
 		 between <cfqueryparam cfsqltype="cf_sql_char" value="#Form.SNnumero2#"> and <cfqueryparam cfsqltype="cf_sql_char" value="#Form.SNnumero1#">
	  </cfif>

	  <cfif isdefined("Form.SNnumero1") and Len(Trim(Form.SNnumero1)) and isdefined("Form.SNnumero2") and len(trim(form.SNnumero2)) eq 0> 
		 >= <cfqueryparam cfsqltype="cf_sql_char" value="#Form.SNnumero1#">
	  </cfif>

	  <cfif isdefined("Form.SNnumero2") and Len(Trim(Form.SNnumero2)) and  isdefined("Form.SNnumero1") and len(trim(Form.SNnumero1)) eq 0>
		 <= <cfqueryparam cfsqltype="cf_sql_char" value="#Form.SNnumero2#">
	  </cfif>
	  ...
	  and a.DRdocumento = a.Ddocumento
	  ...
	  <cfif isdefined("form.fecha1") and len(trim(form.fecha1)) and isdefined("form.fecha2") and len(trim(form.fecha2))>
		...
		<cfelseif datecompare(lsparsedatetime(form.fecha1), lsparsedatetime(form.fecha2)) eq 1>
			...
			and <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(form.fecha1)#"> <cf_dump var="hola">
	...
	<!---  Documentos de Credito generados desde un Recibo de Pago --->
	select 
		sn.SNnombre, 
		a.CCTcodigo, 
		t1.CCTdescripcion, 
		...
		a.CCTRcodigo, 
		a.DRdocumento
	from SNegocios sn, BMovimientos a, CCTransacciones t1, CCTransacciones t2 
	where 
			sn.Ecodigo =  <cfqueryparam cfsqltype="cf_sql_integer" value="#session.ecodigo#">
		and sn.SNnumero
		<cfif isdefined("Form.SNnumero1") and Len(Trim(Form.SNnumero1)) and isdefined("Form.SNnumero2") and Len(Trim(Form.SNnumero2)) and  form.SNnumero2 GTE SNnumero1 >
	...
	<!--- Aplicación de Documento de Credito a un Documento de Debito --->
	select 
		sn.SNnombre, 
		a.CCTRcodigo, 
		t2.CCTdescripcion, 
		a.DRdocumento, 
		coalesce(a.BMmontoref,a.Dtotal) as Dtotal, 
		coalesce(a.Mcodigoref,a.Mcodigo) as Mcodigo, 
		a.BMfecha, 
		'3' as Orden, 
		'Aplicación ' #_Cat# a.CCTcodigo #_Cat# ' ' #_Cat# a.Ddocumento as Movimiento, 
		case t2.CCTtipo when 'D' then 'C' else 'D' end, 
		a.CCTcodigo, 
		a.Ddocumento
	from SNegocios sn, BMovimientos a, CCTransacciones t1, CCTransacciones t2
	where 
	...
	<!--- Aplicación a Documentos de Credito --->
	select 
		sn.SNnombre, 
		a.CCTcodigo, 
		t1.CCTdescripcion, 
		...
		a.BMfecha, 
		'4' as Orden,  
		'Aplicación a NC' #_Cat# a.CCTRcodigo #_Cat# ' ' #_Cat# a.DRdocumento, 
		case t1.CCTtipo when 'D' then 'C' else 'D' end, 
		a.CCTRcodigo, 
		a.DRdocumento
	from  SNegocios sn, BMovimientos a, CCTransacciones t1, CCTransacciones t2
	where 
		...
		<!--- Fechas Desde / Hasta --->
		 <cfif isdefined("form.fecha1") and len(trim(form.fecha1)) and isdefined("form.fecha2") and len(trim(form.fecha2))>
			...
			<cfelseif datecompare(lsparsedatetime(form.fecha1), lsparsedatetime(form.fecha2)) eq 1>
				and a.BMfecha between <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(form.fecha2)#">
					and <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(form.fecha1)#">
			<cfelseif datecompare(form.lsparsedatetime(fecha1), lsparsedatetime(form.fecha2)) eq 0>
				and a.BMfecha between <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(form.fecha1)#">
					and <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(form.fecha2)#">
			</cfif>
		<cfelseif isdefined("form.fecha1") and len(trim(form.fecha1))>
	...
	<!--- Ordenamiento:  Moneda, Socio, Documento, Orden, Fecha --->
	order by 6, 1, 4, 8, 7
</cfquery>
...
<br>
<cfoutput>
	<cfif rsData.recordcount GT 10000>
		<cf_errorCode	code = "50166"
						msg  = "Se han procesado más de 10000 registros. El total es de: @errorDat_1@ registros. Por favor limite los filtros de la consulta"
						errorDat_1="#rsData.recordcount#"
		>
	</cfif>
	...
	<table width="98%" align="center" border="0" cellspacing="0" cellpadding="0" class="">
		...
		<tr>
			<td class="style1">Integración de Saldos por Movimiento de Documento en Cuentas por Cobrar</td>
		</tr>
		<tr>
			<td class="style2">
...
<cflog text="Recordcount: #rsData.RecordCount#">
  ...
  <cfoutput group="SNnombre">
	...
	<cfoutput group="Ddocumento">
		...
		<tr class=<cfif LvarListaNon>"listaNon"<cfelse>"listaPar"</cfif>>
			...
			--->
			<td colspan="6" class=<cfif LvarListaNon>"listaNon"<cfelse>"listaPar"</cfif>>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>#Ddocumento#</strong></td>
		</tr>
		...
		<cfoutput>
			...
			<tr class=<cfif LvarListaNon>"listaNon"<cfelse>"listaPar"</cfif>>
				<td class=<cfif LvarListaNon>"listaNon"<cfelse>"listaPar"</cfif>>
					<!--- Se sustituye la siguiente línea para evitar que se instancie la clase libC.a 4 veces
					<cfloop from="1" to="20" index="i">&nbsp;</cfloop>
					#Replace(Replace(Replace(Replace(Movimiento,'1. ',''),'2. ',''),'3. ',''),'4. ','')#
					--->
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					#Movimiento#
