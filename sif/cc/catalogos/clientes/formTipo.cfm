...
<cfif isDefined("session.Ecodigo") and isDefined("Form.CDTid") and len(trim(#Form.CDTid#)) NEQ 0>
	<cfquery name="rsClienteDetallistaTipo" datasource="#Session.DSN#" >
		Select CEcodigo, CDTid, CDTdescripcion,  ts_rversion
...
function validar(form){
	...
	if ( form.botonSel.value != 'Nuevo' && form.botonSel.value != 'Baja' ){
		MM_validateForm('CDTdescripcion','','R'); 
		if (document.MM_returnValue){ 
			document.form.CDTdescripcion.disabled=false; 
...
<body>
<form action="SQLTipo.cfm" method="post" name="form" onSubmit="return validar(this);">
	...
	<table width="67%" height="75%" align="center" cellpadding="2" cellspacing="0">
		...
		<tr> 
			...
			<td>
				<input name="CDTdescripcion" type="text"  value="<cfif modo neq "ALTA"><cfoutput>#rsClienteDetallistaTipo.CDTdescripcion#</cfoutput></cfif>" size="35" maxlength="30" onfocus="this.select();"  alt="La Descripción del tipo de Categor&iacute;a">
			</td>
		</tr>
  ...
  <input type="hidden" name="CDTid" value="<cfif modo NEQ "ALTA"><cfoutput>#rsClienteDetallistaTipo.CDTid#</cfoutput></cfif>">
