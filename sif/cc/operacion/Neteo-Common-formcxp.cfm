<!--- 
	...
	Modificado por Gustavo Fonseca Hernández.
		Fecha 8-9-2005.
		Motivo: Se crea la función "funcBajacxp" para que no valide el formulario en modo Baja.
...
<cfif (modocxp NEQ "ALTA")>
	<cfquery name="rsDocumentoNeteoDCxP" datasource="#session.dsn#">
		select 	a.idDetalle, a.idDocumentoNeteo, a.idDocumento, a.CPTcodigo, a.Ddocumento, a.BMUsucodigo, a.Referencia, 
...
<cfoutput>
	<form action="Neteo-Common-sqlcxp.cfm" name="formcxp" method="post" onSubmit="javascript: return fnfinalcxp();">
		<table width="100%" align="center"  border="0" cellspacing="0" cellpadding="0">
				...
				<td>
					<cfif (modocxp NEQ "ALTA")>
						<cf_sifsociosnegocios2 form="formcxp" size="45" frame="frSNcodigocxp" SNcodigo="SNcodigocxp" SNnumero="SNnumerocxp" 
							idquery="#rsDocumentoNeteoDCxP.SNcodigo#"
							modificable="false" 
						>
					<cfelse>
						<cf_sifsociosnegocios2 form="formcxp" size="45" frame="frSNcodigocxp" SNcodigo="SNcodigocxp" SNnumero="SNnumerocxp"
				...
				<td>
					<select name="CPTcodigo" onChange="javascript: limpiarDocumentoCxP();" <cfif (modocxp NEQ "ALTA")>disabled</cfif>>
						...
						<cfloop query="rsTransaccionesCxP">
							...
							<option value="#rsTransaccionesCxP.CPTcodigo#" <cfif (modocxp NEQ "ALTA" AND rsTransaccionesCxP.CPTcodigo EQ rsDocumentoNeteoDCxP.CPTcodigo)>selected</cfif>>#rsTransaccionesCxP.CPTdescripcion#</option>
						</cfloop>
					</select>	
				</td>
			</tr>
						...
						<tr>
							<td>
								<input name="HDdocumento" type="hidden"
									value="<cfif (modocxp NEQ "ALTA")>#rsDocumentoNeteoDCxP.Ddocumento#</cfif>">
						...
						<tr>
							<td>
								<input name="idDocumento" type="hidden"
									value="<cfif (modocxp NEQ "ALTA")>#rsDocumentoNeteoDCxP.idDocumento#</cfif>">
								<input name="EDsaldo" type="text" readonly="true" size="20" maxlength="18" style="text-align: right" class="cajasinborde"
									value="<cfif (modocxp NEQ "ALTA")>#LSCurrencyFormat(rsDocumentoNeteoDCxP.EDsaldo,'none')#<cfelse>0.00</cfif>">
			...
			<tr>
				...
				<td>
					<cfif (modocxp neq "ALTA")>
					<cf_monto form="formcxp" name="DmontoDocCxP" value="#rsDocumentoNeteoDCxP.DmontoDoc#" tabindex="1">
					<cfelse>
					<cf_monto form="formcxp" name="DmontoDocCxP" tabindex="1">
					</cfif>
					<strong id="lblRetencionCxP">- #LB_Reten#:</strong>
					<cfif (modocxp neq "ALTA")>
					<cf_monto form="formcxp" name="DmontoRetCxP" value="#rsDocumentoNeteoDCxP.DmontoRet#" tabindex="1">
					<cfelse>
			...
			<tr>
				...
				<td>
					<cfif (modocxp neq "ALTA")>
					<cf_monto form="formcxp" name="Dmonto" value="#rsDocumentoNeteoDCxP.Dmonto#" tabindex="1" readonly="#LvarMontoReadOnly#">
					<cfelse>
		...
		<cfif isdefined("rsDocumentoNeteoDCxP.idDetalle")>
			<input type="hidden" name="idDetalle" value="#rsDocumentoNeteoDCxP.idDetalle#">
			<cfinvoke component="sif.Componentes.DButils" 
			method="toTimeStamp" 
			returnvariable="ts" 
			arTimeStamp="#rsDocumentoNeteoDCxP.ts_rversion#"/>
...
<cfset MSG_DebeSelTT = t.Translate('MSG_DebeSelTT','Debe seleccionar el Tipo de Transacción','Neteo-Common-formcxc.cfm')>
	...
	<script language="javascript" type="text/javascript">
		<!--//
			...
			<cfif not isdefined('rsDocumentoNeteoDCxP')>
		...
		<!--//
			...
			function getDdocumentocxp(Ddocumento){
				<cfif not isdefined('rsDocumentoNeteoDCxP')>
					if (document.formcxp.HDdocumento.value!=Ddocumento&&listoParaObtenerDocCxP()){
						...
						document.getElementById("frDdocumentocxp").src = "Neteo-Ddocumentocxpquery.cfm?Ddocumento="+Ddocumento+"&Mcodigo=#rsDocumentoNeteo.Mcodigo#&CPTcodigo="+document.formcxp.CPTcodigo.value+"&SNcodigo="+document.formcxp.SNcodigocxp.value;
					}
				</cfif>
			}
			function conlisDdocumentoscxp(){
				<cfif not isdefined('rsDocumentoNeteoDCxP')>
					if (listoParaObtenerDocCxP()){
						...
						var nuevo = window.open("Neteo-Ddocumentocxpconlis.cfm?Mcodigo=#rsDocumentoNeteo.Mcodigo#&CPTcodigo="+document.formcxp.CPTcodigo.value+"&SNcodigo="+document.formcxp.SNcodigocxp.value,'ListaDocumentosCxP','menu=no, scrollbars=yes, top='+top+', left='+left+', width='+width+', height='+height);
						nuevo.focus();
					}
				</cfif>
			}
			...
			<cfif LvarTipoNeteoDocs EQ 2 or LvarTipoNeteoDocs EQ 3>
				...
				<cfif isdefined("form.RcodigoCxP")>
					...
					{
						document.formcxp.DmontoRetCxP.value	= fm("#rsDocumentoNeteoDCxP.DmontoRet#",2);
						document.formcxp.DmontoDocCxP.value	= fm("#rsDocumentoNeteoDCxP.Dmonto + rsDocumentoNeteoDCxP.DmontoRet#",2);
						document.formcxp.Dmonto.value		= fm("#rsDocumentoNeteoDCxP.Dmonto#",2);
					}
				</cfif>
					else
					{
				...
				<cfif isdefined("form.RcodigoCxP")>
					<cfoutput>
					sbRetencionCxP ("#form.RcodigoCxP#", "#form.Rporcentaje#", "#rsDocumentoNeteoDCxP.Dmonto#", true);
					</cfoutput>	
				</cfif>
			</cfif>
		//-->
