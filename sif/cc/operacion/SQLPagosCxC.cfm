<!--- 
	...
	Modificado por: Alejandro Bolaños
		Fecha: 11-07-2013
		Motivo: Se modifica para tomar de forma correcta el campo Ccuenta cuando se utilizan cuentas bancarias.
--->
<cfif form.UsarCatalogoCts>
	<!--- Si usa Catalogo de Cuentas y no se tiene definido el valor form.ID, es por que se esta modificando a una Cuenta Bancaria
	El valor de ID se busca dentro del catalogo de CuentasCxC respecto a la cuenta bancaria --->
    <cfif isdefined("form.CBid") and form.CBid NEQ "">
    ...
    <cfelseif isdefined("form.ID")>
     ...
     <cfelse>
     	<cfthrow message="Error Fatal, no se ha logrado obtener la cuenta Transitoria ni Bancaria para aplicar el Cobro">
    </cfif>
<cfelse>
    <cfquery name="CuentaE" datasource="#Session.DSN#">
...
<cfif isdefined('form.moneda') and len(trim(form.moneda)) and form.moneda neq -1 >
	...
	<cfif isdefined("Form.AgregarE")>
		...
		<cfif not existPago>
			<cfset pFecha = createDateTime(
				year(LSParseDateTime(Form.Pfecha)),
				month(LSParseDateTime(Form.Pfecha)),
				day(LSParseDateTime(Form.Pfecha)),
				hour(now()),
				minute(now()),
				second(now())
			)>
			...
			<cfquery datasource="#Session.DSN#">
				insert into Pagos(Ecodigo, CCTcodigo, Pcodigo, Mcodigo, Ptipocambio, Seleccionado, 
							 ...
							 Pusuario <cfif isdefined('Form.ID')>, ID</cfif>, CBid, PMetPago)
                values (
					#Session.Ecodigo#,
					...
					<cfqueryparam cfsqltype="cf_sql_timestamp"value="#pFecha#">,
				...
				,<cfqueryparam cfsqltype="cf_sql_varchar" value="#replace(trim(form.PMetPago),',','')#" null="#form.PMetPago EQ ''#">
				)
			</cfquery>
			<cfset modo="CAMBIO">
			<cfset modoDet="ALTA">
	...
	<cfelseif isdefined("Form.CambiarE")>
		...
		<cfquery datasource="#Session.DSN#">
			...
			set Ptotal = #Form.Ptotal#
			  , Pfecha = <cfqueryparam cfsqltype="cf_sql_timestamp" value="#pFecha#">
			 ...
			 , PMetPago = <cfqueryparam cfsqltype="cf_sql_varchar" value="#replace(trim(form.PMetPago),',','')#" null="#form.PMetPago EQ ''#">
			...
			<cfif isdefined("AgregarD") and not amount_overflow and not existDoc>
				<cfquery datasource="#Session.DSN#">
					insert into DPagos(Ecodigo, CCTcodigo, Pcodigo, Doc_CCTcodigo, Ddocumento, Mcodigo, 
						Ccuenta, DPmonto, DPtipocambio, DPmontodoc, DPtotal, DPmontoretdoc, PPnumero, DPTipoCambioME)
					values ( 
						#Session.Ecodigo#,
						...
						<cfqueryparam cfsqltype="cf_sql_money" value="#replace(trim(form.TipoCambioMonExt),',','')#" null="#form.TipoCambioMonExt EQ ''#">
					)
				</cfquery>
			...
			<cfelseif isdefined("CambiarD")>
				<cfquery name="ABC_Pago" datasource="#Session.DSN#">
					...
					set DPmonto = <cfqueryparam cfsqltype="cf_sql_money" value="#Form.DPtotal#">
					  ...
					  , DPTipoCambioME = <cfqueryparam cfsqltype="cf_sql_money" value="#replace(trim(form.TipoCambioMonExt),',','')#" null="#form.TipoCambioMonExt EQ ''#">
					where Ecodigo = #Session.Ecodigo#
					  and DPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.DPid#">
                </cfquery>
			</cfif>	
	...
	<cfelseif isdefined("Form.Aplicar") OR isdefined("Form.Ver")> 
		...
		<cfquery name="rsValida" datasource="#session.DSN#" maxrows="1">
			select 
				(case when (p.Pfecha) < (select d.Dfecha               
								from Documentos d 
