...
<cfset LB_ClRef = t.Translate('LB_ClRef','Cliente Referencia','DocumentosAplicadosRes.xml')>
...
<cfif isdefined("url.Generar")>
	...
	<cfquery name="rsReporte" datasource="#session.DSN#" maxrows="5001">
		select 	
				...
				a.Dtotal * case when t.CCTtipo = 'C' then -1.00 else 1.00 end as EDtotal,
				b.SNnombre, 
				b.SNidentificacion,
				b.SNnumero,
				((
				...
				d.Odescripcion,
				a.EDid,
				<cfqueryparam cfsqltype="cf_sql_varchar" value="#LvarEdescripcion#"> as Edescripcion
				<cfif url.Formato eq 3>
					, cd.CDCidentificacion
					, cd.CDCnombre
				</cfif>
		 from HDocumentos a
	...
	<cfif url.Formato eq 3>
		<cfquery name="rs" dbtype="query">
			...
			from rsReporte
			where CDCidentificacion is not null
				<!---or len(CDCnombre) > 0  --->
		</cfquery>
	...
	<cfelse>
		<cfreport format="#formatos#" template= "DocumentosSinAplicarRes.cfr" query="rsReporte">	
            ...
            <cfreportparam name="LB_ClRef"		value="#LB_ClRef#">
