...
<cfif isdefined("url.Generar")>
	<cfquery name="rsReporte" datasource="#session.DSN#" maxrows="5001">
		select 	
				distinct a.Pcodigo,  
				rtrim(b.SNnombre) as SNnombre, 
				b.SNidentificacion,
				
				a.CCTcodigo,
				a.Pfecha, 
				(select coalesce(sum(Anti.NC_total),0.00) 
					from APagosCxC Anti 
				  where Anti.Ecodigo = a.Ecodigo 
				    and Anti.CCTcodigo = a.CCTcodigo 
					and Anti.Pcodigo = a.Pcodigo) as NC_total,
				d.Odescripcion,
				a.Ptotal,
				g.Edescripcion,
				f.Mnombre
		...
		where a.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
			...
			and c.Mcodigo = 2 <!--- Para Clientes (CC) --->
			--->
...
<cfset Tit_PagosSAplResum = t.Translate('Tit_PagosSAplResum','Pagos sin Aplicar (Resumido)')>
...
<cfset LB_TransaccInicial = t.Translate('LB_TransaccInicial','Transacción Inicial')>
    ...
    <cfif rsactivatejsreports.valParam EQ 1 AND coldfusionV EQ 2018>
	  ...
	  <cf_js_reports_service_tag queryReport = "#rsReporte#" 
		isLink = False 
		typeReport = #typeRep#
		fileName = "cc.reportes.PagosSinAplicarRes"/>
	<cfelse>
	  <cfreport format="#formatos#" template= "PagosSinAplicarRes.cfr" query="rsReporte">	
		...
		<cfreportparam name="Tit_PagosSAplResum" value="#Tit_PagosSAplResum#">
		...
		<cfreportparam name="LB_Anticipo" value="#LB_Anticipo#">
