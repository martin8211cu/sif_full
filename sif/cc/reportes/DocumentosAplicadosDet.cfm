...
<cfset LB_Cantidad = t.Translate('LB_Cantidad','Cantidad','DocumentosAplicadosDet.xml')>
<cfset LB_Subtotal 	= t.Translate('LB_Subtotal','Subtotal','/sif/generales.xml')>
<cfset LB_Impuesto = t.Translate('LB_Impuesto','Impuesto','DocumentosAplicadosDet.xml')>
<cfset LB_MontoDoc = t.Translate('LB_MontoDoc','Monto del Documento','/sif/generales.xml')>
<cfset LB_Linea = t.Translate('LB_Linea','Línea','DocumentosAplicadosDet.xml')>
<cfset LB_Descripcion 	= t.Translate('MSG_Descripcion','Descripción','/sif/generales.xml')>
<cfset LB_PrecioU = t.Translate('LB_PrecioU','Precio Unitario','DocumentosAplicadosDet.xml')>
<cfset LB_TotDoctCI = t.Translate('LB_TotDoctCI','Total del Documento (con impuestos)','DocumentosAplicadosDet.xml')>
<cfset LB_TotSocio = t.Translate('LB_TotSocio','Total Socio','DocumentosAplicadosDet.xml')>
<cfset LB_Obser = t.Translate('LB_Obser','Observaciones','DocumentosAplicadosDet.xml')>
<cfset LB_TotLinea = t.Translate('LB_TotLinea','Total Línea','DocumentosAplicadosDet.xml')>
...
<cfif isdefined("url.Generar")>
	...
	<cfquery name="rsReporte" datasource="#session.DSN#" maxrows="5001">
		select 	
				f.Mnombre,
				a.Ddocumento as EDdocumento,  
				a.Dfecha as EDfecha, 
				a.CCTcodigo, 
				a.Dvencimiento as EDvencimiento, 
				case when t.CCTtipo = 'C' then -a.Dtotal else a.Dtotal end as EDtotal,
				b.SNnombre, 
				b.SNidentificacion,
				b.SNnumero,
				((
					select min(ltrim(rtrim(c.Cformato)) #_Cat# ' ' #_Cat# c.Cdescripcion)
					from CContables c
					where c.Ccuenta = a.Ccuenta
				))	as Cdescripcion, 
				((
					select min(ltrim(rtrim(cc.Cformato)) #_Cat# ' ' #_Cat# cc.Cdescripcion)
					from CContables cc
					where cc.Ccuenta = e.Ccuenta 
				)) as CdescripcionDetalle,
				d.Odescripcion,
				a.HDid as EDid,
				e.HDDlinea as DDlinea,
				coalesce(e.DDescripcion, 'N/A') as DDdescripcion,
				e.DDdescalterna,
				case when t.CCTtipo = 'C' then -1.00 else 1.00 end * coalesce(e.DDcantidad, 1) as DDcantidad, 
				coalesce(e.DDpreciou, a.Dtotal) as DDpreciou,
				case when t.CCTtipo = 'C' then -1.00 else 1.00 end * e.DDdesclinea as DDdesclinea,
				case when t.CCTtipo = 'C' then -1.00 else 1.00 end * coalesce(e.DDcantidad * e.DDpreciou, a.Dtotal) as DDtotallinea,
				e.Ccuenta,
				...
				((
					...
					from Usuario u
					where u.Usulogin = a.Dusuario
					and u.CEcodigo = #session.CEcodigo#
				)) as Usulogin,
				a.DEobservacion,
				case when t.CCTtipo = 'C' then -1.00 else 1.00 end * DDimpuesto as Impuesto, 
				case when t.CCTtipo = 'C' then -1.00 else 1.00 end * coalesce(e.DDtotal - e.DDimpuesto, a.Dtotal) as SubTotal
		 from HDocumentos a
			...
			left outer join HDDocumentos e
				on e.HDid = a.HDid
		...
		where a.Ecodigo =  #session.Ecodigo# 
			...
			<!--- Fechas Desde / Hasta --->
			 <cfif isdefined("url.fechaDes") and len(trim(url.fechaDes)) and isdefined("url.fechaHas") and len(trim(url.fechaHas))>
				<cfif datecompare(url.fechaDes, url.fechaHas) eq -1> 
					and a.Dfecha between <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(url.fechaDes)#"> 
				...
				<cfelseif datecompare(url.fechaDes, url.fechaHas) eq 1>
					and a.Dfecha between <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(url.fechaHas)#">
			...
			<cfelseif isdefined("url.fechaDes") and len(trim(url.fechaDes))>
				and a.Dfecha >= <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(url.fechaDes)#">
			<cfelseif isdefined("url.fechaHas") and len(trim(url.fechaHas))>
				and a.Dfecha <= <cfqueryparam cfsqltype="cf_sql_date" value="#lsparsedatetime(url.fechaHas)#">
			</cfif>
			
			<!--- Usuario --->
			<cfif isdefined("url.Usuario") and len(trim(url.Usuario)) and url.Usuario NEQ '-1'>
				and rtrim(ltrim(upper(a.Dusuario))) = <cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(Ucase(url.Usuario))#">
			</cfif>

		order by f.Mnombre, b.SNnombre, a.HDid, e.DDlinea
	</cfquery>
	...
	<cfelse>  
		  <cfreport format="#formatos#" template= "DocumentosSinAplicarDet.cfr" query="rsReporte">
			...
			<cfelseif isdefined("url.Ocodigo")	and len(trim(url.Ocodigo)) eq 0>
				<cfreportparam name="Oficina" value="Todas">
			</cfif>
			...
			<cfelseif isdefined("url.moneda")	and len(trim(url.moneda)) and url.moneda EQ '-1'>
				<cfreportparam name="MonedaIni" value="Todas">
			</cfif>		
			...
			<cfelseif isdefined("url.Transaccion")	and len(trim(url.Transaccion)) and url.Transaccion EQ '-1'>
				<cfreportparam name="Transaccion" value="Todas">
			</cfif>
