...
<!----------********************************************************************************---->           
        ...
        <cfquery name="rsCuentasLoc" datasource="#session.dsn#">
          select distinct CuentaTCE
          from #table_name# 
          where SaldoActMonLoc > 0.00 and tipoMov in ('C', 'D')
        </cfquery>  
			...
			<cfif rsCuentasLoc.recordcount gt 0 >
             <cfloop query="rsCuentasLoc">
                   ...
                   <cfquery name="rsFechaEC" datasource="#session.dsn#">
					...
					<cfif (LvarMes eq 02 or LvarMes eq 2) and (LvarDiaCorte eq 30 or LvarDiaCorte eq 31)>
                    	<!---Ver si el año es Bisiesto--->
						<cfif (LvarYear mod 4 eq 0) and ((LvarYear mod 100 neq 0) or (LvarYear mod 400 eq 0)) >
                        	<cfset LvarDiaCorte = 29>
                        <cfelse>
                        	<cfset LvarDiaCorte = 28>    
                        </cfif>
                    </cfif>
                   ...
                   <cfquery name="rsMovSaldo" datasource="#session.dsn#">
                     select top 1  
                        SaldoAntMonLoc
                     from #table_name# a 
					    where 
				   ...
				   <cfif rsMovSaldo.recordcount gt 0>
                      <cfquery name="rsUpdSF" datasource="#session.dsn#">
                          update ECuentaBancaria
                          set ECsaldofin = #rsMovSaldo.SaldoAntMonLoc#
                   ...
                   <cfquery name="rsTMBD" datasource="#session.dsn#">
                    select BTEcodigo from TransaccionesBanco 
                    where Bid = #rsDatosLoc.Bid#
                    and BTEtce = 1
                    and BTEtipo = 'D'    
                   </cfquery>
                   <cfif rsTMBD.recordcount eq 0>
                       <cfthrow message="No existe una transacción de tipo 'D&eacute;bito' en tarjeta de crédito para el banco:#rsBanco.Bdescripcion#. Proceso cancelado!">
                   </cfif>
                    ...
                    <cfquery name="ERR" datasource="#session.DSN#">
                        insert into DCuentaBancaria (ECid, 
                                                     ...
                                                     Bid) 
                        select <cf_jdbcquery_param cfsqltype="cf_sql_numeric" value="#LvarEncabezado#">,
                                <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsTMBD.BTEcodigo#">,
                                <cf_dbfunction name="spart" args="a.Detalle,1,20">,
                                FechaEstadoCta, 
                                ImporteLoc,
                                ImporteLoc, 
...
<!--------------**************************************************************------------------------->
        <cfquery name="rsCuentasOri" datasource="#session.dsn#">
          select distinct CuentaTCE
          from #table_name# 
          where SaldoActMonOri > 0.00 and tipoMov in ('C', 'D')
        </cfquery>  
        <!---- Obtengo los dolares ---->
                     ...
                     <cfquery name="rsMovOri" datasource="#session.dsn#">
                      ...
                      <cfif  rsMovOri.recordcount eq 1>
                         <cfloop query="rsMovOri">
                           ...
                           <cfquery name="rsUpdSal" datasource="#session.dsn#">
                               select top 1  
                               SaldoAntMonOri
                               from #table_name# a 
                               where 
                               	CuentaTCE = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsCuentasOri.CuentaTCE#"> 
                               	and SaldoActMonOri > 0.00 
                               	and tipoMov in ('C','D')
                           </cfquery> 
                           
                           <cfif rsUpdSal.recordcount gt 0> 
                               <cfquery name="ERRCreditos" datasource="#session.DSN#">
                                update ECuentaBancaria  
                                 set 
                                  ECsaldofin = #rsUpdSal.SaldoAntMonOri#
                                where ECid =  #LvarEncabezado#              
                               </cfquery>   
                           </cfif> 
