<cfcomponent>
<script type="text/javascript" language="JavaScript" src="/cfmx/sif/crm/js/pLista1.js"></script>
<cf_templatecss>
<link href="/cfmx/sif/crm/css/lista2.css" rel="stylesheet" type="text/css">
<cffunction name="pListaCRM" access="public" returntype="string" output="true">
	...
	<cfargument name="skin" type="string" required="false" default="default">

	<!--- setea los valores par los skins --->
	<cfif skin eq 'ocean'>
		<cfset tituloListas = "tituloListasOcean" >
		<cfset tituloCorte  = "tituloCorteOcean" >
		<cfset listaNon     = "listaNonOcean" >
		<cfset listaPar     = "listaParOcean" >
		<cfset listaCorte   = "listaCorteOcean" >
		<cfset colorPar     = "##DBE6FB" >
		<cfset colorNon     = "##FAFAFA" >
	<cfelse>
		<cfset tituloListas = "tituloListas" >
		<cfset tituloCorte  = "tituloCorte" >
		<cfset listaNon     = "listaNon" >
		<cfset listaPar     = "listaPar" >
		<cfset listaCorte   = "listaCorte" >
		<cfset colorPar     = "##FAFAFA" >
		<cfset colorNon     = "##FFFFFF" >
	</cfif>
	...
	<cfoutput query="rsLista" startrow="#StartRow_lista#" maxrows="#MaxRows_lista#">
		...
		<tr class=<cfif CurrentRow MOD 2>"#listaNon#"<cfelse>"#listaPar#"</cfif> onmouseover="style.backgroundColor='##E4E8F3';" onmouseout="style.backgroundColor='<cfif CurrentRow MOD 2>#colorNon#<cfelse>#colorPar#</cfif>';">
		...
		<cfloop index="cName" list="#desplegar#">
					...
					<cfif col EQ 1>
						<td align="left" width="18" height="18" nowrap onclick="javascript: <cfif funcion NEQ "">#f#<cfelseif showLink>Procesar('#rsLista.CurrentRow#','#ArrayLen(columnas)#','#formName#'<cfif not incluyeForm>, '#irA#'</cfif>);</cfif>">
							<cfif (trim(datosBD) EQ trim(datosPost)) and (not isdefined("Nuevo"))>
		...
		<tr><td colspan="#Iif(checkboxes EQ 'S', colsp+2, colsp+1)#"><cfloop index="i" from="1" to="#ArrayLen(columnas)#"><input name="#ucase(Trim(ucase(columnas[i])))#_#rsLista.CurrentRow#" type="hidden" value="#JSStringFormat(Evaluate('rsLista.#Trim(columnas[i])#'))#"></cfloop></td></tr>
	</cfoutput>
	<cfif showEmptyListMsg and rsLista.RecordCount lte 0>
		<tr><td align="center" colspan="#Iif(checkboxes EQ 'S', colsp+2, colsp+1)#" class="<cfoutput>#listaCorte#</cfoutput>">
