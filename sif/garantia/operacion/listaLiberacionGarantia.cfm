...
<cfquery name="rsGarantias" datasource="#session.dsn#">
	select  a.COEGid, a.COEGReciboGarantia, case when a.COEGTipoGarantia = 1 then 'Participación' else 'Cumplimiento' end as tipoGarantia, b.Mnombre, a.COEGMontoTotal
		,a.COEGReciboGarantia, c.SNnombre, a.COEGVersion, 1 as tipoMovimiento,a.COEGPersonaEntrega,a.COEGIdentificacion, case when a.COEGContratoAsociado = 'N' then  '<img src=''/cfmx/sif/imagenes/unchecked.gif'' border=''0''>'  else  '<img src=''/cfmx/sif/imagenes/checked.gif'' border=''0''>' end as COEGContratoAsociado
	from COHEGarantia a
		inner join Monedas b
			on b.Mcodigo = a.Mcodigo
		inner join SNegocios c
			on c.SNid = a.SNid
	where a.COEGEstado = 1 <!--- Estado Vigente --->
		and a.COEGVersionActiva = 1 <!--- Estado Activa --->
		<!--- and a.COEGUsuCodigo = #session.usucodigo# --->
		and a.Ecodigo = #session.Ecodigo#
		<cfif isdefined('form.filtro_COEGReciboGarantia') and len(trim(form.filtro_COEGReciboGarantia))>
		and a.COEGReciboGarantia = <cfqueryparam cfsqltype="cf_sql_numeric" value="#replace(form.filtro_COEGReciboGarantia,',','','ALL')#">
		</cfif>
		...
		<cfif isdefined('form.filtro_COEGVersion') and len(trim(form.filtro_COEGVersion))>
		and a.COEGVersion = <cfqueryparam cfsqltype="cf_sql_numeric" value="#replace(form.filtro_COEGVersion,',','','ALL')#">
		</cfif>
		<cfif isdefined('form.filtro_COEGMontoTotal') and len(trim(form.filtro_COEGMontoTotal))>
		and a.COEGMontoTotal = <cfqueryparam cfsqltype="cf_sql_numeric" value="#replace(form.filtro_COEGMontoTotal,',','','ALL')#">
		</cfif>
		...
		and a.COEGContratoAsociado = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.filtro_COEGContratoAsociado#">
		</cfif>	
	...
	<tr>
		<td valign="top">		        
			<cfinvoke component="sif.Componentes.pListas" method="pListaQuery" 
				...
				formatos="S, S, S, S, S, I, M, S, S"
				mostrar_filtro="true"
				align="right, left, left,left, left, right, right, right, center"
				ira="liberacionGarantia.cfm"
				checkboxes="N"
				rsMnombre="#rsMonedas#"
