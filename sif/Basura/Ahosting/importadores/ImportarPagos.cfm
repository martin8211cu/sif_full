...
<cfelse>
    ...
    <cfelseif not bcheck6>
		<cfquery name="ERR" datasource="#session.DSN#">
            ...
            MontoPago, (select sum(MontoDoc) 
			        		from #table_name# c
            			    where a.DocPago = c.DocPago
			                group by c.DocPago) as PagoDocumentos
			from #table_name# a
			where MontoPago < (select sum(MontoDoc) 
			        		from #table_name# b
            			    where a.DocPago = b.DocPago
    ...
    <cfelseif not bcheck8>
		<cfquery name="ERR" datasource="#session.DSN#">
			select distinct 'Saldo de Documento menor al Monto a Pagar: ' as MSG, a.Documento, a.SNcodigoext,
            	(a.MontoDoc + isnull(a.MontoRet,0)) * a.Tcambio as MontoPagar 
			from #table_name# a
			where exists
     	   (
				select 1
