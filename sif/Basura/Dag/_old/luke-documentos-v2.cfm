...
Manejado por Niveles 
	Análisis por Cliente ***  Variable Analisis = 1 ***
	...
	Análisis por Moneda *** Variable Analisis = 2 ***
...
<cfquery name="Lista" datasource="#session.dsn#">
	select #Campos# /*Informacion del documento y la moneda*/
		round(sum(coalesce(b.Dtotal,0)),2) as Dtotal, round(sum(coalesce((b.Dtotal * b.Dtcultrev),0)),2) as Ltotal, 
		round(sum(coalesce(b.Dsaldo,0)),2) as Dsaldo, round(sum(coalesce((b.Dsaldo * b.Dtcultrev),0)),2) as Lsaldo /*Informacion del Documento*/
	from SNegocios a
	...
	<cfif isdefined("Form.doc")>
		and b.Ddocumento = <cfqueryparam cfsqltype="cf_sql_char" value="#Form.doc#">
	</cfif>
	Group By #Group_By# 
...
<table width="100%" border="0" cellspacing="2" cellpadding="0">
	<tr>
	<td>
	<table width="99.5%" align="center"  border="0" cellspacing="0" cellpadding="0" class="table_border_gray">
	<cfoutput>
		<tr>
		  <td class="smenu1" colspan="4" align="center">
			Vencimientos de Saldos de #LSMonthAsString(Month(Now()))# (Analisis por <a href="##" onClick="javascript: funcAnalisis(2);"><cfif Form.Analisis eq 2>
			  <span style="color: ##FF0000;	font-weight: bold; ">Moneda</span>
			  <cfelse>Moneda</cfif></a> / <a href="##" onClick="javascript: funcAnalisis(1);"><cfif Form.Analisis eq 1>
			    <span  style="color: ##FF0000;	font-weight: bold; ">Cliente</span>
			    <cfelse>Cliente</cfif></a>)
		  </td>
		</tr>
		<tr>
			<td class="smenu2" align="left" width="50%"><a href="##"onclick="javascript: funcOrderBy(1);"><strong>#Descripcion#</strong></a> (<cfif Form.Nivel EQ 1><a href="##" onClick="javascript: funcNivel(2);">Ver Detalles</a><cfelse><a href="##" onClick="javascript: funcNivel(1);">Cerrar Detalles</a></cfif>)</td>
			<td class="smenu2" align="right" width="25%"><a href="##"onclick="javascript: funcOrderBy(2);"><strong>Total</strong></a></td>
			<td class="smenu2" align="right" width="25%"><a href="##"onclick="javascript: funcOrderBy(3);"><strong>Saldo</strong></a></td>
		</tr>
		<!--- <cfif Form.Nivel eq 3>
			<form name="formfiltrodoc" method="post" action="#CurrentPage#">
			<!--- <input type="hidden" name="PageNum_lista#PageIndex#" value="#PageNum_Lista#"> --->
			<cfset Arr = ListToArray(QueryString_lista,'&')>
			<cfloop from="1" to="#ArrayLen(Arr)#" index="i">
			<cfset Item = ListToArray(Arr[i],'=')>
			<input type="hidden" name="#Item[1]#" value="#Item[2]#">
			</cfloop>
			...
			<tr class="Areafiltro">
				<td><input type="text" name="doc" <cfif isdefined("Form.doc")>value="#Form.doc#"</cfif>></td>
				<td>&nbsp;</td>
		...
		<cfif Form.Analisis eq 1>
			<cfif Form.Nivel eq 2>
				<cfif isdefined("Lista.SNid") and _Corte_Nivel2 NEQ Lista.SNid>
					...
					<tr>
						<td class="smenu3" align="left" colspan="3"><strong>#SNnombre#</strong> (<cfif Form.Nivel EQ 2><a href="##" onClick="javascript: funcNivel(3);">Ver Documentos</a><cfelse><a href="##" onClick="javascript: funcNivel(2);">Cerrar Documentos</a></cfif>)</td>
					</tr>
				</cfif>
			<cfelseif Form.Nivel eq 3>
				...
				<cfif isdefined("Lista.Mcodigo") and _Corte_Nivel3 NEQ Lista.Mcodigo>
					...
					<tr>
						<td class="smenu3" align="left" colspan="3"><strong>#Mnombre#</strong></td>
					</tr>
				</cfif>
			</cfif>
		<cfelse>
			<cfif Form.Nivel eq 2>
				<cfif isdefined("Lista.Mcodigo") and _Corte_Nivel2 NEQ Lista.Mcodigo>
					...
					<tr>
						<td class="smenu3" align="left" colspan="3"><strong>#Mnombre#</strong> (<cfif Form.Nivel EQ 2><a href="##" onClick="javascript: funcNivel(3);">Ver Documentos</a><cfelse><a href="##" onClick="javascript: funcNivel(2);">Cerrar Documentos</a></cfif>)</td>
					</tr>
				</cfif>
			<cfelseif Form.Nivel eq 3>
				<cfif isdefined("Lista.Mcodigo") and _Corte_Nivel2 NEQ Lista.Mcodigo>
				...
				<cfif isdefined("Lista.SNid") and _Corte_Nivel3 NEQ Lista.SNid>
					...
					<tr>
						<td class="smenu3" align="left" colspan="3"><strong>#SNnombre#</strong></td>
					</tr>
				</cfif>
			</cfif>
		</cfif>
