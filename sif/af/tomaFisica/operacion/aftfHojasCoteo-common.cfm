...
<!---VARIABLE LLAVE DEL DETALLE --->
<cfif isdefined("url.AFTFid_detallehoja") and len(trim(url.AFTFid_detallehoja))>
	<cfset form.AFTFid_detallehoja = url.AFTFid_detallehoja>
</cfif>
...
<cfif not request.calledFromSQL>
	...
	<!--- Definición del Modo y del Mododet --->
	<cfset modo="ALTA">
	<cfif (
				isdefined("Url.Imprimir") or isdefined("Form.BtnDownLoad")
			)>
		<cfset modo="REPORTE">
	<cfelseif (
				isdefined("Form.btnImportar")
			)>
		<cfset modo="IMPORTAR">
	<cfelseif (	
				isdefined("Form.AFTFid_hoja") 
				and len(trim("Form.AFTFid_hoja")) gt 0 
				and Form.AFTFid_hoja gt 0
			)>
		<cfset modo="CAMBIO">
	</cfif>
	...
	<cfif (
				...
				and isdefined("form.AFTFid_detallehoja") 
				and len(trim(form.AFTFid_detallehoja)) gt 0 
				and form.AFTFid_detallehoja gt 0
			)>
		<cfset mododet="CAMBIO">
	<cfelseif (
				isdefined("form.btnAgregar_Activos") 
				or isdefined("form.NuevoDet")
			)>
		<cfset mododet="ALTA">
	</cfif>
	
	<cfif (modo EQ "IMPORTAR") or (modo EQ "REPORTE") or (mododet NEQ "LISTA")>
		<cfinclude template="aftfHojasCoteo-commonhoja.cfm">
	</cfif>
