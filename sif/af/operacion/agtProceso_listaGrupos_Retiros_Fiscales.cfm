...
<!---filtro--->
<cfset filtro = "a.Ecodigo=#session.Ecodigo# and a.IDtrans IN (#IDtrans#) and a.AGTPecodigo > 0 and a.AGTPestadp <= 2 ">
...
<cfset botonAccion[14][1] = "Generar">
<cfset botonAccion[14][2] = "Retiro_Fiscal">
<cfset botonAccion[14][3] = "Lista de Grupos de Transacciones de Retiro Fiscal">
<cfset botonAccion[14][4] = "">
<cfset botonAccion[14][5] = "programa">
<cfset botonAccion[14][6] = 0>
<cfset botonAccion[14][7] = "detalles">
<!---Estados--->
...
<cf_templateheader title="Activos Fijos">
	  <cf_web_portlet_start border="true" skin="#Session.Preferences.Skin#" tituloalign="center" titulo="#botonAccion[IDtrans][3]#">
            ...
            <cfif (IDtrans EQ 5 and (isdefined("session.LvarJA") and session.LvarJA)) or (IDtrans EQ 2 and (isdefined("session.LvarJA") and session.LvarJA)) or (IDtrans EQ 11 and (isdefined("session.LvarJA") and session.LvarJA)) or (IDtrans EQ 14 and (isdefined("session.LvarJA") and session.LvarJA))>
            ...
            <cfelseif (IDtrans EQ 10 and (isdefined("session.LvarJA") and not session.LvarJA))>
            ...
            <cfset LvarPar = ''>
            <cfif isdefined("session.LvarJA") and session.LvarJA and ((IDtrans eq 11) or (IDtrans eq 5) or (IDtrans eq 2))>
				<cfset LvarPar = ''>
            <cfelseif isdefined("session.LvarJA") and not session.LvarJA and ((IDtrans eq 11) or (IDtrans eq 5) or (IDtrans eq 2))>
                <cfset LvarPar = ''>
            </cfif>
