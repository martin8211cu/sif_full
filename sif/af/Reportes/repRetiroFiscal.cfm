	...
	<cfif not isdefined("form.AGTPid") or  (isdefined('url.AGTPid') and len(trim(url.AGTPid)) eq  0)>
		...
		<cf_templateheader title="#nav__SPdescripcion#">
			...
			<cf_web_portlet_start titulo="#nav__SPdescripcion#">
					...
					<cfif isdefined("Form.btnFiltrar") and Len(Trim(Form.btnFiltrar)) NEQ 0>
						<cfinclude template="repRetiroFiscal-lista.cfm">
					</cfif>
				</table> 
			<cf_web_portlet_end>
		<cf_templatefooter>
		...
		<table width="100%" border="0">
		  ...
		  <tr>
			...
			<td>
				...
				<cfinclude template="repRetiroFiscal-form.cfm">
			</td>
			<td>&nbsp;</td>
		  </tr>
		  <tr>
...
<cfelse>
	...
	<cfloop query="rsADTProceso">
		...
		<cfinvoke component="sif.Componentes.AF_ContabilizarRetiroFiscal" method="AplicarMascara" returnvariable="lVarCuentaDep">
	...
	<cfquery name="data" datasource="#session.dsn#">
		select 
            ...
            ta.TAmontodepadqFiscal as DepreciacionFiscalAdquision,
            ta.TAmontodepmejFiscal as DepreciacionMejorasFiscales,
			ta.TAvaladq + ta.TAvalmej + ta.TAvalrev -
	...
	<cf_exportQueryToFile query="#data#" filename="RetiroFiscal_#session.Usucodigo##LSDateFormat(Now(),'ddmmyyyy')##LSTimeFormat(Now(),'hh:mm:ss')#.xls" jdbc="false">
</cfif>