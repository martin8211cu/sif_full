	...
	<cfif not isdefined("form.AGTPid") or  (isdefined('url.AGTPid') and len(trim(url.AGTPid)) eq  0)>
		...
		<cf_templateheader title="#nav__SPdescripcion#">
			...
			<cf_web_portlet_start titulo="#nav__SPdescripcion#">
					...
					<cfif isdefined("Form.btnFiltrar") and Len(Trim(Form.btnFiltrar)) NEQ 0>
						<cfinclude template="repDepreciacionPTU-lista.cfm">
					</cfif>
				</table> 
			<cf_web_portlet_end>
		<cf_templatefooter>
		...
		<table width="100%" border="0">
		  ...
		  <tr>
			...
			<td>
				...
				<cfinclude template="repDepreciacionPTU-form.cfm">
			</td>
			<td>&nbsp;</td>
		  </tr>
		  <tr>
...
<cfelse>
	...
	<cfquery name="data" datasource="#session.dsn#">
		select 
			...
			act.Aserie as Serie,
			ta.TAvaladq as Monto_Inverison, 
            ta.TAmontolocmej as Porcentaje_PTU, 
            ta.TAmontoorimej as SaldoAnterior_PTU,
            ta.TAmontolocadq as Disminucion_PTU,
            ta.TAmontooriadq as Saldo_PTU,
	...
	<cf_exportQueryToFile query="#data#" filename="Depreciacion_PTU_#session.Usucodigo##LSDateFormat(Now(),'ddmmyyyy')##LSTimeFormat(Now(),'hh:mm:ss')#.xls" jdbc="false">
</cfif>