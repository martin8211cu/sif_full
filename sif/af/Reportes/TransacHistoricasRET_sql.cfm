...
<cfsavecontent variable="myQuery">
	<cfoutput>
		...
		select 
			...
			coalesce(a.TAmontolocadq + a.TAmontolocmej + a.TAmontolocrev,0.00) as Valor, 
			coalesce(a.TAdepacumadq + a.TAdepacummej + a.TAdepacumrev,0.00) as DepreciacionAcum,
		...
		order by c.ACdescripcion, CFdescripcion
	</cfoutput>
</cfsavecontent>
...
<cftry>
	...
	<cf_exportQueryToFile query="#data#" filename="Retiro_Det_#session.Usucodigo#_#dateformat(now(),'ddmmyyyy')#_#hour(now())#_#Minute(now())#_#second(now())#.xls" jdbc="true">
	<cfelseif isdefined("url.Formato") and url.Formato eq 3>
		<cf_exportQueryToFile query="#data#" separador="#chr(9)#" filename="Retiro_Det_#session.Usucodigo#_#dateformat(now(),'ddmmyyyy')#_#hour(now())#_#Minute(now())#_#second(now())#.txt" jdbc="true">
	</cfif>
