...
*******Activos Fijos**********************
*******Reporte de Revaluaciones de Activos por Periodo Mes*
	...
	<cfinclude template="/home/menu/pNavegacion.cfm">
		...
		<cf_web_portlet_start titulo="<cfoutput>#nav__SPdescripcion#</cfoutput>">
			...
			<cfif (isdefined("url.Periodo") and len(trim(url.Periodo)) gt 0 and url.Periodo gt 0)
				...
				<cf_rhimprime datos="/sif/af/Reportes/rptRevaluacionesMes#Strvar_Resumido#-sql.cfm" paramsuri="#navegacion#" regresar="/cfmx/sif/af/Reportes/rptRevaluacionesMes.cfm">
				<cfinclude template="rptRevaluacionesMes#Strvar_Resumido#-sql.cfm">
				
			<cfelseif (isdefined("url.Periodo") and len(trim(url.Periodo)) gt 0 and url.Periodo gt 0)
				and (isdefined("url.Mes") and len(trim(url.Mes)) gt 0 and url.Mes gt 0)
				and (isdefined("url.Archivo"))>
				<cfinclude template="rptRevaluacionesMes#Strvar_Resumido#-query.cfm">
				<cftry>
				...
				<cf_jdbcquery_close>
			<cfelse>
				<cfinclude template="rptRevaluacionesMes-form.cfm">
			</cfif>
		<cf_web_portlet_end>
	<cf_templatefooter>
