...
<cfif not isdefined("url.Archivo")>
	...
	<cfset n = 3000>
	<cfif rsxyzcant.recordcount*2 gt n>
...
<cfsavecontent variable="consulta_reporte">
	<cfoutput>
		select 
				...
				({fn concat({fn concat(cat.ACcodigodesc,' - ')},cat.ACdescripcion)}) as CategoriaSinRepetidos,
				({fn concat({fn concat(cat.ACcodigodesc,' - ')},cat.ACdescripcion)}) as Categoria,
				({fn concat({fn concat(clas.ACcodigodesc,' - ')},clas.ACdescripcion)}) as ClaseSinRepetidos,
				...
				sum(ta.TAvaladq + ta.TAvalmej + ta.TAvalrev) as MontoTotal,
				<cf_dbfunction name="to_float" args="coalesce(ta.AFIindice,0.00)" dec="8"> as Indice,
				<!---ta.AFIindice as Indice,--->
				sum(ta.TAmontolocadq + ta.TAmontolocmej + ta.TAmontolocrev) as MontoRevaluacion,'' as espacio
			from TransaccionesActivos ta 
				inner join Activos act
				on act.Ecodigo = ta.Ecodigo
				and act.Aid = ta.Aid
		...
		select 
				({fn concat({fn concat(cf.CFcodigo,' - ')},cf.CFdescripcion)}) as CFuncional,	
				('') as CategoriaSinRepetidos,
				({fn concat({fn concat(cat.ACcodigodesc,' - ')},cat.ACdescripcion)}) as Categoria,
				('') as ClaseSinRepetidos,
				({fn concat({fn concat(clas.ACcodigodesc,' - ')},clas.ACdescripcion)}) as Clase,
				ta.TAperiodo as Periodo, 
				ta.TAmes as Mes,
				'Depreciacion' as TipoMonto,
				sum(ta.TAdepacumadq) as Adquisicion, 
				sum(ta.TAdepacummej) as Mejoras, 
				sum(ta.TAdepacumrev) as Revaluacion, 
				sum(ta.TAdepacumadq + ta.TAdepacummej + ta.TAdepacumrev) as MontoTotal,
				<cf_dbfunction name="to_float" args="coalesce(ta.AFIindice,0.00)" dec="8"> as Indice,
				<!---ta.AFIindice as Indice,--->
				sum(ta.TAmontodepadq + ta.TAmontodepmej + ta.TAmontodeprev) as MontoRevaluacion,'' as espacio
			from TransaccionesActivos ta 
				inner join Activos act
				on act.Ecodigo = ta.Ecodigo
				and act.Aid = ta.Aid
