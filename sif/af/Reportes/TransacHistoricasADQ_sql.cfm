<!--- 
	Creado por Gustavo Fonseca H/DorianA.
		Motivo: Nueva consulta para exportaciÃ³n a Excel del mÃ³dulo de Activos Fijos.
		Fecha:16-5-2006.
 --->
...
<cfsavecontent variable="myQuery">
	<cfoutput>
		...
		select 
			...
			{fn concat ( {fn concat ( d.CFcodigo, ' - ')}, d.CFdescripcion)} as CentroF, 
			b.Aplaca as Placa, 
			b.Adescripcion as Descripcion,
			b.Afechainidep as FechaIniDep,
			a.TAmontolocadq as Valor,
			{fn concat ( {fn concat ( cl.ACcodigodesc, ' - ')}, cl.ACdescripcion)} as Clase,
			ofi.Oficodigo as Oficina,
			a.TAvutil as Vida_Util
			...
			,a.TAdocumento as Documento
			,hct.Edocumento as Contable
			,hct.Cconcepto as Concepto
			,(select Usulogin from Usuario where Usucodigo = a.Usucodigo) as usuario
...
<cftry>
	...
	<cfif isdefined("url.Formato") and url.Formato eq 2>
		<cf_exportQueryToFile query="#data#" filename="Adquisicion_Det_#session.Usucodigo#_#dateformat(now(),'ddmmyyyy')#_#hour(now())#_#Minute(now())#_#second(now())#.xls" jdbc="true">
	<cfelseif isdefined("url.Formato") and url.Formato eq 3>
		<cf_exportQueryToFile query="#data#" separador="#chr(9)#" filename="Adquisicion_Det_#session.Usucodigo#_#dateformat(now(),'ddmmyyyy')#_#hour(now())#_#Minute(now())#_#second(now())#.txt" jdbc="true">
	</cfif>
	...
	<cfset totalValorCat = 0>
		<tr>
			...
			<tr style="padding:10px;">
				...
				<td style="padding:3px;" bgcolor="CCCCCC" nowrap="nowrap"><strong>Vida util</strong></td>
				<cfif isdefined("url.chkveradq")>
				...
				<td style="padding:3px;" bgcolor="CCCCCC" nowrap="nowrap"><strong>AS.Contable</strong></td>
				...
				<td style="padding:3px;" bgcolor="CCCCCC" nowrap="nowrap"><strong>FechaIni_Depreciaci&oacute;n</strong></td>
				<td style="padding:3px;" bgcolor="CCCCCC" nowrap="nowrap"><strong>Usuario</strong></td>
			</tr>
			<cfoutput>
				<tr>
					<td>#Placa#</td>
					...
					<td>#Documento#</td>
					<td>#Concepto#</td>
					<td>#LSDateFormat(FechaIniDep,"DD/MM/YYYY")#</td>
					<td>#usuario#</td>
