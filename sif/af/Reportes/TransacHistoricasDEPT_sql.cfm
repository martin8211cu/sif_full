...
<cfsavecontent variable="myQuery">
	...
	select 
		...
		coalesce(e.AFSvaladq + e.AFSvalmej + e.AFSvalrev,0.00) as Valor, 
		coalesce(e.AFSdepacumadq + e.AFSdepacummej + e.AFSdepacumrev,0.00) as DepreciacionAcum,
		cl.ACdescripcion as Clase,
	...
	where a.IDtrans = 4  <!--- Depreciacion --->
...
<cftry>
	...
	<cfif isdefined("url.Formato") and url.Formato eq 2>
		<cf_exportQueryToFile query="#data#" filename="Depreciacion_Tot_Det_#session.Usucodigo#_#dateformat(now(),'ddmmyyyy')#_#hour(now())#_#Minute(now())#_#second(now())#.xls" jdbc="true">
	<cfelseif isdefined("url.Formato") and url.Formato eq 3>
		<cf_exportQueryToFile query="#data#" separador="#chr(9)#" filename="Depreciacion_Tot_Det_#session.Usucodigo#_#dateformat(now(),'ddmmyyyy')#_#hour(now())#_#Minute(now())#_#second(now())#.txt" jdbc="true">
	</cfif>
...
<table width="100%" cellpadding="2" cellspacing="0">
	...
	<cfset totalDepCF = 0>
		...
		<cfset totalValorCF = 0>
			<tr>
				<td><strong>Centro&nbsp;Funcional:&nbsp;</strong>#CentroF#</td>
			</tr>
