...
<cfloop from="1" to="100" index="dummy">
	...
	<cfif listFind(path,current)>
		<cfthrow message="Arbol inconsistente: La Clasificación '#anterior#' tiene como padre un Descendiente '#current#'">
	</cfif>
...
<cfquery datasource="#session.dsn#" name="ARBOL">
	select c.AFCcodigo, c.AFCcodigoclas, c.AFCdescripcion, coalesce(c.AFCnivel,0) as nivel,  
	       ( select count(1) 
		     from AFClasificaciones c2
...
<script language="JavaScript" type="text/javascript">
	...
	function eclk(arbol_pos){<!--- event: Click --->
		location.href="tipoactivo.cfm?ARBOL_POS="+arbol_pos+"<cfoutput>#JSStringFormat(QueryString_ARBOL)#</cfoutput>";
	}
</script>
...
<table cellpadding="0" cellspacing="1" border="0" width="100%">
	...
	<cfloop query="ARBOL">
		<tr valign="middle"	
			<cfif ARBOL.AFCcodigoclas is url.ARBOL_POS> class='ar1'
			<cfelse>class='ar2' onMouseOver="eovr(this)" onMouseOut="eout(this)" onClick="eclk('#ARBOL.AFCcodigo#')"
			</cfif> >
			<td nowrap>
