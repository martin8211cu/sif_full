...
<cfif isdefined("Form.Estado") and trim(Form.Estado) EQ 1 >
	<cfset fnGeneraQueryTransito()>
</cfif>
<cfif isdefined("Form.Estado") and trim(Form.Estado) EQ 2>
	<cfset fnGeneraQueryRetiros()>
</cfif>
...
<cfelseif isdefined("Form.Estado") and trim(Form.Estado) EQ 2>
...
<cfif isdefined("Form.formato") and trim(Form.formato) EQ "pantalla">
    ...
    <cf_htmlreportsheaders
        title="#Lvar_Title#" 
        filename="#Lvar_FileName#.xls" 
        ira="ActivosEnTransito.cfm">
	<cfoutput>
		<table width="100%" align="center"  border="0" cellspacing="0" cellpadding="2">
			<tr><td align="center" colspan="8"><cfinclude template="RetUsuario.cfm"></td></tr>
			<tr><td align="center" colspan="8"><font size="3"><strong>#session.Enombre#</strong></font></td></tr>
			<tr><td align="center" colspan="8"><font size="2"><strong>#Lvar_Title#</strong></font></td></tr>
			<cfif isdefined("form.CRCCid") and len(trim(form.CRCCid)) gt 0 and form.CRCCid gt 0>
				...
				<tr><td align="center" colspan="8"><strong>Centro de Custodia: #rsCRCentroCustodia.CRCCdescripcion#</strong></td></tr>
			</cfif>
			<tr><td align="center" colspan="8"><strong>Fecha Inicial: #Form.FechaInicio# - Fecha Final: #Form.FechaFinal#</strong></td></tr>
		...
		<cfoutput query="rsActivoTransito" group="CRCCid">
			<cfif not Lvar_bx>
				...
				<tr>
					...
					<td nowrap bgcolor="##CCCCCC"><strong>&nbsp;Documento Origen:</strong></td>
					<td nowrap bgcolor="##CCCCCC"><strong>&nbsp;<cfif rsDato.RecordCount gt 0 and len(trim(rsDato.Pvalor))>#rsDato.Pvalor#<cfelse>Origen</cfif>:</strong></td>		
				</tr>
			</cfif>
			<tr>
				<td nowrap colspan="8"><strong>Centro de Custodia:</strong>&nbsp;#CRCCcodigo# - #CRCCdescripcion#</td>
			</tr>
			<cfoutput group="CFid">
				<tr>
					<td nowrap colspan="8" style="padding-left: 20px;"><strong>Centro Funcional:</strong>&nbsp;#CFcodigo# - #CFdescripcion#</td>
				</tr>
				<cfoutput>
				...
				<tr >
					...
					<td nowrap>#Documento_Org#</td>
					<td nowrap>#Etiqueta_Origen#</td>
				</tr>
				</cfoutput>
	...
	<cfif isdefined("queryTransacionRetiro") and len(trim(queryTransacionRetiro)) gt 0>
		...
		<cfoutput query="rsTransacionRetiro" group="CRCCid">
			<cfif not Lvar_by>
				<cfset Lvar_by = true>
				<tr><td nowrap colspan="6" bgcolor="##999999"><strong>Lista de Transacciones de Retiro</strong></td></tr>
				<tr>
					<td nowrap bgcolor="##CCCCCC" style="padding-left: 40px; "><strong>Placa</strong></td>
					<td nowrap bgcolor="##CCCCCC"><strong>Descripci&oacute;n Detallada</strong></td>
					<td nowrap bgcolor="##CCCCCC"><strong>Tipo Documento</strong></td>
...
<cffunction name="fnGeneraSalida" output="no" access="private">
    ...
    <cfif isdefined("queryActivoTransito") and len(trim(queryActivoTransito)) gt 0>
        ...
        <cfoutput query="rsActivoTransito">
            <cfset Lvar_nrecordcount = Lvar_nrecordcount + 1>
            <cfif not Lvar_bx>
				<cfset temp_tab_contenido = "CentroCustodia" & LvarSeparador & "CentroFuncional" & LvarSeparador
					& "Placa" & LvarSeparador
					& "Descripcion" & LvarSeparador
                    & "TipoDocumento" & LvarSeparador
                    & "TipoCompra" & LvarSeparador
					& "Fecha" & LvarSeparador
					& "Usuario" & LvarSeparador
					& "DocumentoOrigen" & LvarSeparador
					& "Origen"	& chr(13)>
            ...
            & trim(Usulogin) & LvarSeparador
			& trim(Documento_Org) & LvarSeparador
			& trim(Etiqueta_Origen)	& chr(13)>
			<cfset fnGraba(temp_tab_contenido,false)>
        </cfoutput>
    </cfif>
    ...
    <cfif isdefined("queryTransacionRetiro") and len(trim(queryTransacionRetiro)) gt 0>
        ...
        <cfoutput query="rsTransacionRetiro">
            <cfset Lvar_nrecordcount = Lvar_nrecordcount + 1>
            <cfif not Lvar_by>
	            <cfset temp_tab_contenido = "CentroCustodia" & LvarSeparador
                    & "CentroFuncional" & LvarSeparador
                    & "Placa" & LvarSeparador
                    & "Descripcion" & LvarSeparador
                    & "TipoDocumento" & LvarSeparador
                    & "TipoCompra" & LvarSeparador
...
<cffunction name="fnGeneraQueryTransito" access="private" output="no">
...
<cffunction name="fnGeneraQueryRetiros" access="private" output="no">
