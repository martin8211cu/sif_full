									...
									<tr>
										...
										<td>
											...
											<cf_conlis
												...
												Columnas="A.DEid as DEid ,A.DEidentificacion as DEidentificacion, #PreserveSingleQuotes(DEnombrecompleto)# as DEnombrecompleto"
												Filtro="A.Ecodigo = #Session.Ecodigo#
														and (
															select count(1) from AFResponsables B
															where B.CRCCid = $CRCCid,numeric$
															  and B.DEid = A.DEid
...
<cffunction name="fnProcesarQueries" access="private" output="no">
	...
	<cfquery name="rsVerificaRRHH" datasource="#Session.dsn#">
		select count(1) as Cantidad
		from TiposNomina
		where Ecodigo = #session.Ecodigo#  
	</cfquery>
	<cfif rsVerificaRRHH.Cantidad GT 0>
		<cfoutput>
		<cfset LvarTablaQueryEmpleado = 
			"DatosEmpleado d 
				inner join LineaTiempo a on a.DEid = d.DEid and #hoy# between a.LTdesde and a.LThasta 
				inner join RHPlazas b on b.RHPid = a.RHPid
				inner join CFuncional cf on cf.CFid = b.CFid
			">
		</cfoutput>
	<cfelse>
		<cfoutput>
		<cfset LvarTablaQueryEmpleado = 
			"DatosEmpleado d 
				inner join EmpleadoCFuncional decf on decf.DEid = d.DEid and #hoy# between decf.ECFdesde and decf.ECFhasta
				inner join CFuncional cf on cf.CFid = decf.CFid
			">
		</cfoutput>
	</cfif>
</cffunction>