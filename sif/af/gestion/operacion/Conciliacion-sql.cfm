...
<cfelseif isdefined("form.btnConciliar")>
	...
	<cfquery name="rsValida2B" datasource="#session.dsn#">
		...
		and   a.Cconcepto  = #Form.Cconcepto#
		and   a.Edocumento = #Form.Edocumento#
		and <cf_dbfunction name="like" args="b.CFformato,c.Cmascara">
	</cfquery>
...
<cffunction access="private" name="fnAplicarConciliacion" returntype="any">
	...
	<cfloop list="#form.chk#" index="llave">
		...
		<cfif rsErr.recordcount GT 0>
				...
				<tr>
					<td colspan="2" align="center"><input type="button" name="btncerrar" value="Regresar" onClick="javascript:cerrarVentana()"> </td>
				</tr>	
				<tr>
					<td colspan="2"><hr></td>
				</tr>	
		...
		<cfquery datasource="#session.dsn#">
			...
			and not exists( 
					...
					from Activos d
						...
						and c.Aid = d.Aid
						and <cf_jdbcquery_param cfsqltype="cf_sql_timestamp" value="#now()#"> between c.AFRfini and c.AFRffin
					where d.Ecodigo = a.Ecodigo
					  and d.Aplaca  = a.GATplaca)
		</cfquery>
		
		<cf_dbfunction name="to_char" args="a.GATmes" returnvariable="LenMes">
		<cf_dbfunction name="length" args="#LenMes#" returnvariable="LenGATmes">
      <cf_dbfunction name="srepeat" args="'0';2-#LenGATmes#"  returnvariable="GATmes" delimiters=";" >
		...
		<cfquery datasource="#session.dsn#">
			...
			Select 	
				a.Ecodigo, 
				'GA', 
				<cf_dbfunction name="concat" args="#PreserveSingleQuotes(GATperiodoC)# ° #PreserveSingleQuotes(GATmes)# ° #PreserveSingleQuotes(GATmesC)# ° #PreserveSingleQuotes(CconceptoC)# ° '-' ° #PreserveSingleQuotes(EdocumentoC)#" delimiters="°">,
				a.ID, 
				a.Ocodigo, 
...
<cffunction name="fnGeneraRelacionesActivos" access="private" returntype="any">
		...
		<cfquery datasource="#session.dsn#">
			...
			select	Ecodigo, 	EAcpidtrans, 	EAcpdoc, 		EAcplinea, 
					...
					BMUsucodigo, <cf_jdbcquery_param cfsqltype="cf_sql_timestamp" value="#now()#">,
					IDcontable 		 dd					
			from 	#GAADQUISICIONES#		
			order by EAcplinea
		</cfquery>
		...
		<cfquery datasource="#session.dsn#">
			...
			select 	Ecodigo, 	EAcpidtrans, 	EAcpdoc, 	EAcplinea,
					...
					<cf_jdbcquery_param cfsqltype="cf_sql_money" value="1.00">, 
					EAtotalloc,
					#session.usucodigo#,	
					#session.usucodigo#
			from #GAADQUISICIONES#	
		...
		<cfquery datasource="#session.dsn#">
			...
			select 	Ecodigo, 		
					...
					CFid, 		
					DEid, 			
					<cf_jdbcquery_param cfsqltype="cf_sql_numeric" value="null"> as Alm_Aid, 
					<cf_jdbcquery_param cfsqltype="cf_sql_numeric" value="null"> as Aid, 			
					<cf_jdbcquery_param cfsqltype="cf_sql_numeric" value="#LvarMonedaEmpresa#">,
					AFCcodigo, 		
					<cf_jdbcquery_param cfsqltype="cf_sql_float" value="1.00"> as DSAtc, 
					...
					GATfechainirev, 	
					EAtotalloc, 	
					<cf_jdbcquery_param cfsqltype="cf_sql_integer" value="1"> as Status,
					(
						select max(b.CRDRid) 
						from CRDocumentoResponsabilidad b 
...
<cffunction name="fnContabilizaAjusteConciliacion" access="private" returntype="any">
	...
	<cfquery name="rstemp" datasource="#session.dsn#">
		insert into #INTARC# ( INTORI, INTREL, INTDOC, INTREF, INTMON, INTTIP, INTDES, INTFEC, INTCAM, Periodo, Mes, Ccuenta, Cgasto, Cformato, Mcodigo, Ocodigo, INTMOE,CFid)
		select 
				'AFGA',
				1,
