...
Mask.prototype.attach = function (o, mascara, tipo){
	o.onkeydown = new Function("return " + this.ref + ".isAllowKeyPress(this)");
	o.onkeyup = new Function("return " + this.ref + ".getKeyPress(this)");
	...
	{
		o.onblur = new Function("this.value = " + this.ref + ".format(this.value);MaskTest(this,this.value,'"+mascara+"',this.value);");	
	}
}
...
e = window.event;
	...
	{
		var LvarRange = document.selection.createRange();
		GvarRLon = LvarRange.text.length;
	}
...
//	var k = String.fromCharCode(xe.keyCode);
	...
	if( (xe.keyCode > 47) || (_u == true) || (xe.keyCode == 8 || xe.keyCode == 46) ){
		...
		if (document.selection)
		{
			...
			LvarPos = LvarRange.moveStart("textedit",-1);
		...
		if (document.selection && (LvarPos != LvarLon) )
		{
			...
			if ( (/[Yy]/).test(this.mask.charAt(LvarPos-1)) )
				;
			else if (GvarRLon>1)
...
Mask.prototype.setNumber = function(_v, _d){
	...
	if( !/^[\$]?((\$?[\+-]?([0#]{1,3},)?[0#]*(\.[0#]*)?)|([\+-]?\([\+-]?([0#]{1,3},)?[0#]*(\.[0#]*)?\)))$/.test(m) )
		return this.throwError(1, "Una máscara inválida fue indicada en el constructor de la \nMáscara.", _v);
	...
	if( v.length == 0 ) v = NaN;
	var vn = Number(v);
	if( isNaN(vn) ) return this.throwError(2, "El valor digitado no es un número.", _v);
...
Mask.prototype.setDateKeyPress = function (_v, _d){
	...
	{
		if ( new RegExp("[0-9/]").test(v.charAt(i)) )
			nv = nv + v.charAt(i)
	}
	v = nv;
