...
function SuperNote(myName, config)
{
 ...
 for (var p in defaults)
  this[p] = (typeof config[p] == 'undefined') ? defaults[p] : config[p];
   ...
   if(this.animar)
  	this.animations[this.animations.length] = this.animFade;
 ...
 this.setWidth(this.myName,this.width,this.pageindex);
...
/*==================FUNCIÓN PARA MODIFICAR EL TAMAÑO DE LA CAJA DEL MENSAJE============================*/
SuperNote.prototype.setWidth = function(myName,width, pageindex)
{
  document.getElementById(myName + '-note-'+ 'demo'+pageindex).style.width = width;
};
...
SuperNote.prototype.animFade = function(ref, counter) { with (this)
{
