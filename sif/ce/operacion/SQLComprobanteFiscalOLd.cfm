...
<cfelse>
	...
	<cfif (Form.Origen NEQ "TES" AND Form.Origen NEQ "TSGS") or (Form.Origen EQ "TES" and  valTES EQ "N")>
		...
		<cfif  trim(lVarRFCemisor) NEQ  trim(Replace(getSocio.SNidentificacion, "-", "" ,"all")) and lVarRFCreceptor NEQ "" and  Form.Origen EQ "CCFC">
			<script language="JavaScript1.2" type="text/javascript">
...
try{
	if (window.opener.funcRefrescar#form.nombre#) {
		window.opener.funcRefrescar#form.nombre#()
	}
