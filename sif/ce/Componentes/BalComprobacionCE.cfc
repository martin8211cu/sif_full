    ...
    <cffunction name="ValidaBalCompSATMesPost" access="public">
     ...
     <cffunction name="GenerarBalCompSAT" access="public" returntype="numeric">
		...
		'I' para moneda Informe--->
								<!--- REQUERIMIENTO PMI PARA BALANZA DE CIERRE DE AÑO EN MONEDA INFORME --->
		<cfif hayCambio EQ "S" or Arguments.TipoMoneda eq 'I'>
	        <cfquery name="insBalCompSAT" datasource="#session.DSN#">
	            ...
	            values  (<cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">,
	                     ...
	                     <cfif Arguments.TipoMoneda eq 'I'>
		                    1,
		                 <cfelse>
	        ...
	        <cfquery name="insBalDetCompSAT" datasource="#session.DSN#">
				...
				select distinct <cfqueryparam cfsqltype="cf_sql_numeric" value="#insBalCompSAT.identity#">,
   				...
   				from (
					...
					inner join (
						select
							min(b.Ecodigo) as Ecodigo,
							min(b.Ccuenta) as Ccuenta,
							min(#Arguments.Periodo#) as Periodo,
							min(#Arguments.Mes#) as Mes,
							<cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.Mcodigo#"> Mcodigo,
							sum(a.saldoini) as saldoini,
					...
					where b.CEBperiodo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.Periodo#">
						and b.CEBmes = <cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.Mes#">
						and b.CEBTipo = 0
						and exists (select 1 from CEMapeoSAT a where bd.Ccuenta = a.Ccuenta and bd.Ecodigo = a.Ecodigo
						<cfif Arguments.GEid NEQ -1>
							and a.GEid = #Arguments.GEid#
						</cfif>
						and a.CAgrupador = '#Arguments.idAgrupador#')
				...
				right join (
		        	...
		        	<cfif isdefined('Arguments.Nivel') and #Arguments.Nivel# EQ -1>
		            	...
		            	and not exists (select 1 from CEInactivas a
		            					where a.Ccuenta = b.Ccuenta and a.Ecodigo = b.Ecodigo
		            					<cfif Arguments.GEid NEQ -1>
											and a.GEid = #Arguments.GEid#
										</cfif>)
					group by
				...
				where bal2.Periodo = <cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.Periodo#">
					...
					and bal2.Ecodigo = #Session.Ecodigo#
					and bal1.Ccuenta is null
					and exists (select 1 from CEMapeoSAT a where bal2.Ccuenta = a.Ccuenta and bal2.Ecodigo = a.Ecodigo
					<cfif Arguments.GEid NEQ -1>
						and a.GEid = #Arguments.GEid#
					</cfif>
					and a.CAgrupador = '#Arguments.idAgrupador#')
      ...
      <cffunction name="BalCompSAT" access="public" returntype="query">
        ...
        <cfquery name="rsReporte" datasource="#session.DSN#">
            SELECT distinct a.Ccuenta,b.Cformato as formato,b.Cdescripcion as descrip,SLinicial as saldoini,DLdebitos as debitos,
                    ...
                    CASE WHEN coalesce(c.CEBTipo,0) = 0 THEN 'N'
						...
						ELSE 'N'
					END as CEBTipo,c.Sello,c.CodCertificado,c.Certificado,b.Cmayor, b.Cformato
     ...
     <cffunction name="ValPreBalCompSATA" access="public">
		...
		<cfquery name="rsBalanzaGrupo" datasource="#session.DSN#">
        	SELECT a.GEid from CEBalanzaSAT a
			Where CEBalanzaId = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.idBalComp#">
		</cfquery>
		<!--- si es balanza de grupo no se muestra el error --->
		<cfif rsBalanzaGrupo.GEid EQ "-1">
	        <cfquery name="rsValBalCompSATA" datasource="#session.DSN#">
	            ...
	            WHERE a.CEBmes = ( CASE
		    			WHEN BalComSAT.CEBmes = 1 THEN 12
     					ELSE BalComSAT.CEBmes - 1
	                               END)
		    			and a.CEBperiodo <= BalComSAT.CEBperiodo
	                and a.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
                    and a.CETipoMoneda = (select CETipoMoneda from CEBalanzaSAT where CEBalanzaId = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.idBalComp#">)
	    	</cfquery>
     ...
     <cffunction name="ValPreBalCompSATP" access="public">
        ...
        <cfquery name="rsValBalCompSATP" datasource="#session.DSN#">
            ...
            WHERE a.CEBmes = ( CASE WHEN BalComSAT.CEBmes = 12 THEN 1 ELSE BalComSAT.CEBmes + 1 END)
	                    and a.CEBperiodo >= BalComSAT.CEBperiodo
				and a.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">
                and a.CETipoMoneda = (select CETipoMoneda from CEBalanzaSAT where CEBalanzaId = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.idBalComp#">)
    	</cfquery>
