<cfcomponent>
     <cffunction name="RepositorioCFDIs" access="public" output="no">
		...
		<cfargument name='uuidArray'    type='boolean' 	required='false' default="false">
		<cfargument name='ECIid'    	type='numeric' 	required='false' default="-1">
        ...
        <cfquery name="datosCFDI" datasource="#session.dsn#">
	        	select distinct rt.TimbreFiscal from CERepoTMP rt #NOLOCK#
				...
				union all
				select distinct timbre as TimbreFiscal from #repositorio#CERepositorio rt #NOLOCK#
				inner join TESdetallePago ds
            ...
            <cfif isdefined('Arguments.uuidArray') AND isdefined('Arguments.ECIid')  AND  Arguments.uuidArray EQ true >
            	<cfquery name="rsUUIDDet" datasource="#session.dsn#">
				  select uuid,DCIconsecutivo
				...
				<cfloop query="rsUUIDDet">
				    <cfquery  name="insertCERepo" datasource="#session.dsn#" result="resInsertCERepo">
						insert into CERepositorio (IdContable, IdDocumento, origen, linea, timbre,
						BMUsucodigo, Ecodigo)
						values (#Arguments.idContable#, #Arguments.idDocumento#, '#Arguments.Origen#', 
							#rsUUIDDet.DCIconsecutivo# , 
							'#rsUUIDDet.uuid#', #session.Usucodigo#, #session.Ecodigo#)
					</cfquery>
				</cfloop>
			</cfif>
	...
	<cffunction name="getInfoXML" access="public" output="false">
		...
		<cfif getDoc.recordCount GT 0>
			<cfif getDoc.xmlTimbrado NEQ "">
				...
				<cfset valPago = find("pago10:Pagos",getDoc.xmlTimbrado) >
				...
				<cfloop from="1" to="#arraylen(ConceptoNodes)#" index="i">
					...
					<cfif isdefined('ConceptoXML.Concepto.XmlAttributes.descuento')>
						<cfset stConcepto.Descuento = "#LSParseNumber(ConceptoXML.Concepto.XmlAttributes.descuento)#">
					<cfelse>
						<cfset stConcepto.Descuento = 0>
					</cfif>   
					<cfset stConcepto.Importe = #LSParseNumber(ConceptoXML.Concepto.XmlAttributes.importe)# - #LSParseNumber(stConcepto.Descuento)#>
	            ...
	            <cfif valPago GT 0>
	            	<cfset strXML.TotalNodoPagos = archXML.Comprobante.Complemento.Pagos.Pago.XmlAttributes.Monto>
	            </cfif>
			...
			<cfif trim(getDoc.xmlTimbrado) NEQ "">
				<cftry>
                    ...
                    <cfif isdefined('stringXML') and stringXML NEQ "">
                        ...
                        <cfset filename = "#tempfile#archivoXML#DateFormat(now(),'yyyy-mm-dd-')##TimeFormat(now(),'HH-mm-ss-lll')#.xml">
                        <cffile action="write" file="#filename#" output="#getDoc.xml#" addnewline="No" >
                        <cffile action="read" file="#filename#" variable="varXML">
                    </cfif>
				...
				<cfif Arguments.origen NEQ "TES" or Arguments.origen NEQ "TSGS" or (Arguments.origen EQ "TES" and  valTES EQ "N")>
					<cfif isdefined("getSocio.SNidentificacion")>
						...
						<cfif  trim(lVarRFCemisor) NEQ  trim(Replace(getSocio.SNidentificacion, "-", "" ,"all")) and lVarRFCreceptor NEQ "" and  Arguments.origen EQ "CPFC">
							<cfif Arguments.cod NEQ "">
	...
	<cffunction name="eliminarRepositorioCFDIs" access="public" output="no">
    	<cfargument name='idDocumento' 	type='numeric' 	required='false' default="-1">	 <!--- ID Documento ---->
        <cfargument name='idContable'	type='numeric' 	required='true'>	 <!--- ID Contable ---->
        <cfargument name='Origen'		type='string' 	required='true'>	 <!--- Origen ---->
		<cfargument name='idLineaP' 	type='numeric' 	required='false' >	 <!--- ID Linea Poliza ---->
		<cfquery datasource="#session.dsn#">
			...
			where IdContable = #Arguments.idContable#
				...
				and origen = '#Arguments.Origen#'
				and linea = #Arguments.idLineaP#
				and Ecodigo = #session.Ecodigo#
		</cfquery>
	</cffunction>
</cfcomponent>