<cfcomponent extends="taffy.core.resource" taffy_uri="/authenticate">
			...
			<cfelse>
				<cfquery name="rsToken" datasource="aspmonitor">
					select CONVERT(VARCHAR(32),HashBytes('MD5', convert(varchar(32),max(sessionid))),2) token, 
							login as usuario, #rsUsu.codigo# usucodigo, CEcodigo as empresa
					from MonProcesos mp
					where cerrada = 0
						and login = '#arguments.username#'
					group by login, CEcodigo
				</cfquery>

				<cfquery name="rsEmpresas" datasource="asp" cachedwithin="#CreateTimeSpan(0,0,20,0)#">
					select
					...
					where e.Ecodigo in (
							...
							where up.Usucodigo = #rsUsu.codigo#
					)
					and e.CEcodigo = #rsToken.empresa#
					order by Enombre
				</cfquery>
	...
	<cffunction name="get" access="public" output="false">
		...
		<cftry>
			...
			<cfset result["result"] = BooleanFormat(rsToken.valid)>
			...
			<cfelse>
				...
				<cfset result["data"] =  queryToStruct(rsUsu)>
			</cfif>
		<cfcatch type="any">
			<!--- TODO --->
			<cfset status = 500>
