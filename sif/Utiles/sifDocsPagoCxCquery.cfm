...
<cfif isdefined("url.documentoc") and Len(Trim(url.documentoc))>
	<cfquery name="rs" datasource="#url.conexion#">
		select a.Mcodigo, b.Ccuenta, Dtipocambio, c.Cdescripcion, isnull (b.Rcodigo, '%') as Rcodigo,
			   ...
			   (select min(pp.PPnumero)
				...
				coalesce( (select sum(pp.PPprincipal + pp.PPinteres)
					from PlanPagos pp
					where pp.Ecodigo    = b.Ecodigo
					  and pp.CCTcodigo  = b.CCTcodigo
					  and pp.Ddocumento = b.Ddocumento
				...
				coalesce( (select sum(pp.PPpagomora)
					from PlanPagos pp
					where pp.Ecodigo    = b.Ecodigo
					  and pp.CCTcodigo  = b.CCTcodigo
					  and pp.Ddocumento = b.Ddocumento
		...
		----->
		 <cfif isdefined("Form.DocumentoC") and (Len(Trim(Form.DocumentoC)) NEQ 0)> 
		  	and b.Ddocumento = <cfqueryparam cfsqltype="cf_sql_char" value="#form.DocumentoC#"><!----'#Ucase(Form.DocumentoC)#'----->
		 </cfif>
	...
	<script language="JavaScript"> 
		...
		parent.document.#url.form#.#url.desc#.value="#rs.Descripcion#";
		parent.document.#url.form#.#url.name#.value="#rs.Ddocumento#";
		parent.document.#url.form#.#url.CCTcodigoConlis#.value = "#rs.CCTcodigo#";
		
		<cfif isdefined("rs") and rs.recordcount GT 0>
		parent.document.#url.form#.Cod#url.id#.value = "#Codigo#";
		</cfif>

		if (parent.func#url.name#) { parent.func#url.name#(); }		
		// si el valor devuelto por el "rs" es diferente al que se digitó en el conlis... entonces limpia el conlis con la funcion limpiarDocsPagoCxC
		 if (descAnt != parent.document.#url.form#.#url.name#.value.toUpperCase() && parent.limpiarDocsPagoCxC) {
			parent.limpiarDocsPagoCxC(); parent.LimpiarCampos();
		} 
