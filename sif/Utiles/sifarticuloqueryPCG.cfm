...
<cfif isdefined("url.dato") and url.dato NEQ "">
	<cfif find("Cformato", url.IACcampo)>
			  ...
			  from Articulos a 
				...
				inner join Clasificaciones cl 
					inner join FPCatConcepto cc 
						on cc.Ecodigo = cl.Ecodigo and cc.FPCCTablaC = cl.Ccodigo 
					on cl.Ecodigo = a.Ecodigo and cl.Ccodigo = a.Ccodigo and cc.FPCCid in (<cfqueryparam cfsqltype="cf_sql_numeric" value="#filtroFPCCid#" list="yes">)
			 	inner join FPPreciosArticulo pa
					on pa.CPPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#url.CPPid#"> and pa.Aid = a.Aid and pa.FPPAPrecio > 0
		...
		<cfquery name="rs" datasource="#url.conexion#">
			select a.Aid, a.Adescripcion, a.Acodigo, a.Ucodigo, b.Eexistencia, d.Cmayor, d.Ccuenta , d.Cformato, d.Cdescripcion, a.Icodigo  
			from Articulos a
	...
	<cfelse>
		<cfquery name="rs" datasource="#url.conexion#">
			select a.Aid, a.Acodigo, a.Adescripcion, a.Ucodigo, '' as Ccuenta, '' as Cmayor, '' as Cformato, '' as Cdescripcion, a.Icodigo
			from Articulos a
				inner join Clasificaciones cl 
					inner join FPCatConcepto cc 
						on cc.Ecodigo = cl.Ecodigo and cc.FPCCTablaC = cl.Ccodigo 
