...
<cfif isdefined("url.Reporte")>
	...
	<cfheader name="Content-Disposition" value="attachment; filename=PlanCuentas_General.xls">
...
<title><cf_translate  key="LB_Titulo">Listado del Plan de Cuentas Financieras</cf_translate></title>
...
<cfif rsMayor.PCEMid EQ "" OR rsMayor.PCEMplanCtas NEQ 1>
	<cf_errorCode	code = "50801"
					msg  = "No tiene plan de cuentas definidas."				
	>
</cfif>
...
<cfoutput>
<body scroll="yes">
<!---- INICIO DEL REPORTE DE LAS CUENTAS DE MAYOR --->	
<cfloop query="rsMayor">	
	...
	<table border="0" width="100%">
		<tr>
			...
			<td>
				...
				<cf_translate key=LB_Estructura>Estructura de la Máscara para la Cuenta</cf_translate> #rsMayor.Cmayor#
				</strong>
			<cfelse>
			<td colspan="10">
				<strong style="font-size:24px">
				<cf_translate key=LB_Detalle>Detalle del Plan de Cuenta</cf_translate> #rsMayor.Cmayor#
				</strong>
			</td></cfif>
	...
	<table border="0">
		<tr>
			<td style="border:solid 1px ##CCCCCC;" colspan="#rsNiveles.recordCount+2#" align="center"><strong><cf_translate key=LB_FCV>Formatos, Catálogos y Valores<</cf_translate>/strong></td>
			<td style="border:solid 1px ##CCCCCC;" rowspan="2" colspan="2"><strong><cf_translate key=LB_Descripciono>Descripción</cf_translate></strong></td>
		<cfif isdefined("url.Reporte")>
			<td style="border:solid 1px ##CCCCCC;" colspan="#rsOficinas.recordCount+1#" align="center"><strong><cf_translate key=LB_Oficina>Oficinas</cf_translate></strong></td>
		</cfif>
		</tr>

		<tr>
			<td style="border:solid 1px ##CCCCCC;"><strong><cf_translate key = LB_Mayor>Mayor</cf_translate></strong></td>
		<cfloop query="rsNiveles">
			<td align="center" style="border:solid 1px ##CCCCCC;" <cfif rsNiveles.currentRow EQ rsNiveles.recordCount>colspan="2"</cfif> nowrap><strong><cf_translate key=LB_Nivel>Nivel</cf_translate> #rsNiveles.currentRow#</strong></td>
		</cfloop>
		<cfif isdefined("url.Reporte")>
			<td style="border:solid 1px ##CCCCCC;"><strong><cf_translate key=LB_Todas>TODAS</cf_translate></strong></td>
	...
	<cfset LvarMascaraN 		= arrayLen(LvarMascara)>
		...
		<tr>
			...
			<td><cf_translate key=LB_Mayor>Cuenta Mayor</cf_translate></td>
			<td>&nbsp;</td>
		</tr>
	<cfloop query="rsNiveles">
		...
		<tr><td>&nbsp;</td></tr>
		<tr>
			<td x:str><strong>#rsMayor.Cmayor#</strong></td>
	...
	<cfif not isdefined("url.Reporte")>			
		...
		<cfloop query="rsNiveles">
			<td align="center">
				<input type="checkbox" id="chk#rsMayor.Cmayor##rsNiveles.currentRow#" name="chk#rsMayor.Cmayor##rsNiveles.currentRow#" checked="checked">
			</td>
		</cfloop>
			<td>&nbsp;</td>
			<td nowrap>(<cf_translate key = LB_NDV>Indique los niveles a detallar sus Valores</cf_translate>)</td>
			<td>&nbsp;</td>
		</tr>
...
<cfif isdefined("url.Reporte")>
	...
	<cfloop index="i" from="1" to="#LvarMascaraN-1#">
		...
		<tr><td>&nbsp;</td></tr>
			...
			<cfset sbImprimeCatalogo(i, LvarPCEcatids[i], LvarHijos[i],0,'#rsMayor.Cmayor#')>
		</cfif>
	</cfloop>		
</cfif>
	...
	<table>
		...
		<tr>
			<td colspan="#LvarMascaraN+2#" align="center">
				<input type="button" name="btnReporte" value="<cf_translate key=LB_Detalle>Detalle del Plan de Cuenta</cf_translate>" onClick="sbEjecutar();">
			</td>
		</tr>
	</table>
	</body>
	...
	<script language="javascript">
		...
		{		
			...
			LvarValOficina = cboOficina.options[cboOficina.selectedIndex].value
				LvarParam = "ConlisCuentasFinancierasTot.cfm?Ecodigo=#URLEncodedFormat(url.Ecodigo)#&CPVfecha=#URLEncodedFormat(url.CPVfecha)
				#&Reporte&Ocodigo="+LvarValOficina;
	<cfloop query="rsMayor">
	<cfquery name="rsNiveles" datasource="#Session.DSN#">
		select PCNid, PCEcatid, PCNlongitud, coalesce(PCNdep,0) as PCNdep, PCNdescripcion
		  from PCNivelMascara
		...
		<cfloop query="rsNiveles">
			if (document.getElementById("chk#rsMayor.Cmayor##rsNiveles.currentRow#").checked)
			{
				LvarParam += "&chk#rsMayor.Cmayor##rsNiveles.currentRow#";
			}
		</cfloop>			
	</cfloop>
...
<cffunction name="sbImprimeCatalogo" output="true">
	...
	<cfif not isdefined("chk#Arguments.Cmayor##Arguments.Nivel#")>
		<cfreturn>
	</cfif>
			  ...
			  from PCDCatalogo d
			   ...
			   <cfif rsCat.PCEvaloresxmayor EQ 1>
					...
					inner join PCDCatalogoPorMayor cxm
						...
						and cxm.Cmayor   = '#Arguments.Cmayor#'
	...
	<cfelse>
		...
		<cfif rsSQL.Pvalor EQ "1">
			<cfset LvarPCDdescripcion = "coalesce(v.PCDdescripcionA,v.PCDdescripcion) as PCDdescripcion">
		<cfelse>
		...
		<cfquery name="rsDet" datasource="#Session.DSN#">
			select PCDvalor, PCEcatidref, #LvarPCDdescripcion#
			  from PCDCatalogo v
			 where PCEcatid		= #Arguments.PCEcatid#
			   and Ecodigo		is null
			   and PCDactivo	= 1
	...
	<!--- <cfset LvarOfisIdx = 0> --->
	<cfset LvarNivel = Arguments.nivel>
	...
	<cfloop query="rsDet">
		...
		<cfif Arguments.Hijos NEQ 0 and Arguments.Hijos NEQ "">
			...
			<cfelse>
				...
				<cfset sbImprimeCatalogo(LvarNivel, rsDet.PCEcatidref, listDeleteAt(Arguments.Hijos, 1), LvarOfisIdx,#arguments.Cmayor#)>				
			</cfif>		
		<cfelse>
			<cfset ListaNiveles = fnMantieneCSC(Arguments.Nivel,LvarNivel,LvarOfisIdx)>
