...
<cfif isdefined("Url.SNcontratos") and not isdefined("Form.SNcontratos")>
	<cfparam name="Form.SNcontratos" default="#Url.SNcontratos#">
</cfif>
<cfif isdefined("Url.soloClientesAmbos") and not isdefined("Form.soloClientesAmbos")>
	<cfparam name="Form.soloClientesAmbos" default="#Url.soloClientesAmbos#">
</cfif>
<cfif isdefined("Url.soloProveedores") and not isdefined("Form.soloProveedores")>
	<cfparam name="Form.soloProveedores" default="#Url.soloProveedores#">
</cfif>
...
function Asignar(id, codigo,desc<cfif isdefined('form.SNid')>, SNid</cfif>) {
	if (window.opener != null) {
		...
		<cfif isdefined('form.SNid')>window.opener.document.#form.formulario#.#form.SNid#.value = SNid;</cfif>
...
function fnCargaCtaOrigenSN(SNcodigo) {
	    ...
	    ajaxRequest.open("GET", '/cfmx/sif/tesoreria/Solicitudes/ajaxTESMedioPago.cfm?modo=solPagoManualCTA&SNcodigo='+SNcodigo, false);
	    ajaxRequest.send(null);
	    window.opener.document.getElementById("datosCtaOrigen").innerHTML = ajaxRequest.responseText;
	}

	function fnCargaMEOrigenSN(SNcodigo) {
	    var ajaxRequest; // The variable that makes Ajax possible!
	    var vID_tipo_gasto = '';
	    var vmodoD = '';
	    try {
	    ...
	    ajaxRequest.open("GET", '/cfmx/sif/tesoreria/Solicitudes/ajaxTESMedioPago.cfm?modo=solPagoManualMP&SNcodigo='+SNcodigo, false);
	    ajaxRequest.send(null);
...
<cfif isdefined("form.SNcontratos") and len(trim(form.SNcontratos))>
		<cfset filtro = filtro & " and SNcontratos = #form.SNcontratos#">
</cfif>
...
<cfif isdefined("Form.SNcontratos") and Len(Trim(Form.SNcontratos)) NEQ 0>
 	<cfset filtro = filtro & " and SNcontratos =" & Form.SNcontratos>
	<cfset navegacion = navegacion & "&SNcontratos=" & Form.SNcontratos>
</cfif>
		...
		<table width="100%" border="0" cellpadding="2" cellspacing="0" class="areaFiltro">
			<tr>
				...
				<td align="center">
                    ...
                    <cfif isdefined("form.SNcontratos") and len(trim(form.SNcontratos))>
						<input type="hidden" name="SNcontratos" value="#form.SNcontratos#">
					</cfif>

                    <cfif isdefined("form.soloClientesAmbos") and len(trim(form.soloClientesAmbos))>
						<input type="hidden" name="soloClientesAmbos" value="#form.soloClientesAmbos#">
					</cfif>

                    <cfif isdefined("form.soloProveedores") and len(trim(form.soloProveedores))>
						<input type="hidden" name="soloProveedores" value="#form.soloProveedores#">
					</cfif>
...
<cfquery name="rsLista" datasource="#session.DSN#">
	select SNcodigo, SNid,
	 ...
	 where Ecodigo=<cfqueryparam cfsqltype="cf_sql_integer" value="#Form.Ecodigo#">
	   ...
	   <cfif isDefined("form.soloClientesAmbos")>
		   and SNtiposocio IN ('A','C')
	   </cfif>
	   <cfif isDefined("form.soloProveedores")>
