...
<cfquery datasource="#session.dsn#" name="rs">
	select e.AnexoDef, e.AnexoXLS
	from Anexoim e
	where AnexoId = <cfqueryparam cfsqltype="cf_sql_numeric" value="#url.AnexoId#">
</cfquery>

<cfif len(rs.AnexoXLS) LTE 1>
	<!--- Upload del XLM viejo, porque no existe el XLS --->
	<cfset LvarType = "XML_VIEJO">
	<cfset LvarData = rs.AnexoDef>
<cfelseif isdefined("url.xls") OR rs.AnexoDef EQ "EOF">
	<!--- Upload del XLS y existe el XLS --->
	<cfset LvarType = "XLS_NUEVO">
	<cfset LvarData = rs.AnexoXLS>
<cfelse>
...
<cfif Len(rs.AnexoDef) LTE 1 AND LvarType NEQ "XLS_NUEVO">
	<cfoutput>No existe la Imagen del Anexo</cfoutput>
<cfelse>
	<cfset LvarFileName = REReplace(rsDes.AnexoDes, '[^A-Za-z0-9]', '_', 'all')>
	<cfset LvarFileExt  = "xls">
	<cfset LvarIsZIP = NOT (LvarType EQ "XML_NUEVO" OR LvarType EQ "XML_VIEJO") AND LvarData[1] EQ 80 and LvarData[2] EQ 75>
	...
	<cfif LvarIsZIP>
		...
		<cfif find(".",LvarName)>
			<cfset LvarFileExt  = mid(LvarName,find(".",LvarName)+1,100)>
		</cfif>
		<cffile action="delete" file="#tempfile#">
	</cfif>
	<cfheader name="Content-Disposition" value="Attachment; filename=#LvarFileName#.#LvarFileExt#">
