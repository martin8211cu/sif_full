	...
	where ERN.Ecodigo = T.Ecodigo 
		...
		select ERN.ERNid, 
			...
			(select coalesce(sum (DRNliquido), 0) 
			 FROM DRNomina c 
			 WHERE c.ERNid = ERN.ERNid) as Importe
		from  ERNomina ERN inner join TiposNomina T
		on ( ERN.Ecodigo = T.Ecodigo and 
			   ...
			   ERN.ERNid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.ERNid#"> ) inner join  Monedas M
		...
		<td nowrap colspan="4" align="center">
			...
			<cfoutput>
				...
				<cfif ArrayLen(errarry) eq 0>
					<input type="submit" name="Verificar" value="#Request.Translate('VerificarRNomina','Marcar como Verificada','/sif/rh/Utiles/Generales.xml')#" alt="Pasa la Relación de Pago al Proceso de Autorización." onClick="javascript: return confirm('¿Desea verificar la relación de pago?');">
				</cfif>
				<cfif rsERNomina.ERNcapturado eq 1>
					<input type="submit" name="Modificar" value="#Request.Translate('ModificarRNomina','Marcar para Modificar','/sif/rh/Utiles/Generales.xml')#" alt="Pasa la Relación de Pago al Proceso de Modificación." onClick="javascript: return confirm('¿Desea devolver la relación de pago al proceso de modificación?');">
				<cfelseif (Session.RHParams.RHPARAM7 EQ Session.RHParams.RH_sin_Banco_Virtual) or (Session.RHParams.RHPARAM7 EQ Session.RHParams.RH_con_Banco_Virtual)>
					<input type="submit" name="RHModificar" value="#Request.Translate('RHModificarRNomina','Devolver a Sistema de Nómina','/sif/rh/Utiles/Generales.xml')#" alt="Pasa la Relación de Pago al Sistema de Nómina." onClick="javascript: return confirm('¿Desea devolver la relación de pago al sistema de nómina?');">
				</cfif>
			</cfoutput>
			</form>
		</td>
	...
	<tr> 
		<td nowrap colspan="4">			
			<table width="100%" border="0" align="center" cellpadding="2" cellspacing="2" class="ayuda">
				...
				<tr valign="middle"> 
					<td width="7%" nowrap class="fileLabel"><img src="../../imagenes/Alto.gif" width="48" height="48"></td>
                      ...
                      <ol>
                        <cfloop from="1" to="#ArrayLen(errarry)#" index="erridx">
                          <cfloop from="1" to="#ArrayLen(bibarry)#" index="bibidx">
                            <!---<cfoutput>#errarry[erridx]# / #bibarry[bibidx]#</cfoutput>--->
                            <cfif ucase(trim(errarry[erridx])) eq ucase(trim(bibarry[bibidx]))>
                              <li> <cfoutput> #Request.Translate(bibarry[bibidx],desarry[bibidx])# </cfoutput> </li>
                            </cfif>
                          </cfloop>
                        </cfloop>
                      </ol>
				...
				<cfelse>
				<tr> 
					<td nowrap><img src="../../../imagenes/Check01_T.gif" width="24" height="24"></td>
