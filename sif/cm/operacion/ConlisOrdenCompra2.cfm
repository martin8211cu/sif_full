...
function Asignar(EOidorden,Descripcion,Observaciones,EOnumero) {
	if (window.opener != null) {
		window.opener.document.form1.EOidorden.value = EOidorden;
		window.opener.document.form1.Observaciones.value = Descripcion;
...
<cfif isdefined("Form.EOnumero") and Len(Trim(Form.EOnumero)) NEQ 0>
 	<cfset filtrotipo = filtrotipo & " and a.EOnumero =" & form.EOnumero>
...
<cfquery name="rsLista" datasource="#session.DSN#">
	select distinct a.EOidorden, a.CMTOcodigo#_Cat#'-'#_Cat#c.CMTOdescripcion as CMTOcodigo, a.CMTOcodigo as tipoOrden, c.CMTOdescripcion, a.EOnumero, a.EOfecha, a.SNcodigo, a.Observaciones, b.SNnumero, b.SNnombre
	from EOrdenCM a
	...
	<cfif isdefined("Form.fecha") and Len(Trim(Form.fecha)) NEQ 0>
		...
		and (d.DOcantidad -(
								...
								from DDocumentosI x, EDocumentosI y
								where x.DOlinea = d.DOlinea
									and x.Ecodigo = d.Ecodigo
									and x.EDIid =  y.EDIid
									and y.EDIestado = 10		
							) 
			) > 0
	  and a.EOidorden in (  select distinct EOidorden 
						from DOrdenCM
...
<table width="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td>
			<form style="margin:0;" name="filtroOrden" method="post" action="ConlisOrdenCompra2.cfm">
	...
	<tr>
		<td>
			<cfinvoke 
				...
				<cfinvokeargument name="irA" value="ConlisOrdenCompra2.cfm"/> 
				<cfinvokeargument name="formname" value="listaOC"/> 
				<cfinvokeargument name="maxrows" value="15"/> 
				
				<cfinvokeargument name="funcion" value="Asignar"/>
				<cfinvokeargument name="fparams" value="EOidorden,Observaciones,Observaciones,EOnumero"/>
