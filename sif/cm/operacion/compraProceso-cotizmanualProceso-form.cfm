...
<cfquery name="rsLineasProceso" datasource="#session.DSN#">
	select c.AFMid,d.AFMid, a.DSlinea, c.NumeroParte, 
		  a.CMPid, a.ESidsolicitud, a.DSlinea, b.Aid, 
		  b.Cid,b.Ecodigo,
		  b.DScant, b.DScant-b.DScantsurt as CantDisponible, rtrim(b.Ucodigo) as Ucodigo, rtrim (b.Icodigo) as Icodigo,
		  case when c.AFMid = d.AFMid then d.AFMid else null end as marca,               
		  case when b.Aid is not null then d.Acodigo else null end as Acodigo,
		  case when b.Cid is not null then e.Ccodigo else null end as Ccodigo, 
		  b.DSdescripcion, b.DSdescalterna, b.ESnumero, b.DSconsecutivo,
		  b.DSobservacion, /*c.NumeroParte*/
		  <cfif isdefined("form.SNcodigo1") and len(trim(form.SNcodigo1))>
                    coalesce(c.NumeroParte, d.Acodalterno) as AuxNumeroParte			
		  <cfelse>
		     d.Acodalterno as AuxNumeroParte
		  </cfif>
	from CMLineasProceso a     
	   ...
	   where a.CMPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Session.Compras.ProcesoCompra.CMPid#"> 
	   Order by b.ESnumero,b.DSconsecutivo
</cfquery>
...
<cfquery name="rsLineas" datasource="#session.DSN#">
	select a.CMPid, a.ESidsolicitud, a.DSlinea, b.Aid, b.Cid,b.Ecodigo,
		   ...
		   case when b.Aid is not null then e.Acodigo else null end as Acodigo,
		   case when b.Cid is not null then d.Ccodigo else null end as Ccodigo,
		  <cfif isdefined("form.SNcodigo1") and len(trim(form.SNcodigo1))>
                    coalesce(c.NumeroParte, e.Acodalterno) as AuxNumeroParte			
		  <cfelse>
		     e.Acodalterno as AuxNumeroParte
		  </cfif>		
	from CMLineasProceso a	
...
<cfif modo EQ "CAMBIO">
	...
	<cfquery name="rsLineasProceso" datasource="#session.DSN#">
		select 	a.ECid, a.CMPid, a.DSlinea, a.Ecodigo, 
			    ...
			    case when b.Aid is not null then c.Acodigo else null end as Acodigo,
		  	    case when b.Cid is not null then d.Ccodigo else null end as Ccodigo,
...
<script language="JavaScript" type="text/javascript">
	...
	function FuncTotLinea(vnLinea){
		...
		if(vnSubtotal != 0){
			...
			document.form1['MtoSImpuesto_'+vnLinea].value = 0.00;	
		}
...
<form action="compraProceso-cotizmanual-sql.cfm" method="post" name = "form1" onSubmit="javascript: return _endform();" >
	...
	<!---- El input opcion me indica si se selecciono la opcion a) o la b) de la cotización manual
	loa valores son: 0: para la opcion a y 1:para la opcion b----->
	<input type="hidden" name="opcion" value="<cfif isdefined("form.opcion")><cfoutput>#form.opcion#</cfoutput><cfelse>0</cfif>">
	<cfoutput>
	<table width="100%" cellpadding="0" cellspacing="0" align="center">
		<tr>
			<td class="subTitulo" align="center"><strong><font size="2">Encabezado de Cotizaci&oacute;n</font></strong></td>
		</tr>
		<tr> 
			<td>
				<cfif modo neq 'ALTA'>
						...
						<td align="right" nowrap><strong>Proveedor:</strong>&nbsp;</td>
					 	<td colspan="3" nowrap>								
							<cfquery name="rsSocio" datasource="#session.dsn#">
							 ...
							 <cfoutput>
								<input type="hidden" name="SNcodigo1" value="<cfif not isdefined("vnSocio") and isdefined("form.btnNueva_cotizacion_del_proceso")>jejejeej<cfelseif isdefined("rsSocio") and (rsSocio.RecordCount NEQ 0)>#rsSocio.SNcodigo#</cfif>">
								<input type="text" size="10" name="SNnumero1" value="<cfif not isdefined("vnSocio") and isdefined("form.btnNueva_cotizacion_del_proceso")><cfelseif isdefined("rsSocio") and rsSocio.RecordCount NEQ 0>#rsSocio.SNnumero#</cfif>" onBlur="javascript:traerSocioNegocio(this.value,1);">								
								<input type="text" size="40" name="SNnombre1" value="<cfif not isdefined("vnSocio") and isdefined("form.btnNueva_cotizacion_del_proceso")><cfelseif isdefined("rsSocio") and rsSocio.RecordCount NEQ 0>#rsSocio.SNnombre#</cfif>" readonly="" disabled>									
							 </cfoutput>
		...
		<tr>
			<td class="subTitulo" align="center"><strong><font size="2">Detalle de Cotizaci&oacute;n</font></strong></td></tr>
		<tr> 

			<td align="center">
				<div style="overflow:auto; height:200; width:778; margin:0;">
			      <table width="100%" border="0" cellspacing="0" cellpadding="2">
                    <tr>
                      <td class="tituloListas" align="center" style="border-left: 1px solid black; border-bottom: 1px solid black; border-right: 1px solid black; ">N° Solicitud</td>
                      <td class="tituloListas" align="center" style="border-left: 1px solid black; border-bottom: 1px solid black; border-right: 1px solid black; ">Línea</td>
				   ...
				   <cfoutput query="rsLineasProceso" group="DSlinea">					  
                      ...
                      <tr>
                        <td colspan="12" style="border-bottom: 1px solid black; background-color:##999999; color:##FFFFFF; font-weight: bold;">
						  ...
						  <cfset varDSdescalterna = trim(Replace(rsLineasProceso.DSdescalterna,chr(34),'&quot;','ALL' ))>
						  <cfset varDSobservacion = trim(Replace(rsLineasProceso.DSobservacion,chr(34),'&quot;','ALL' ))>
                          <input name="DSdescalterna#rsLineasProceso.DSlinea#" type="hidden" value="#varDSdescalterna#">
                          <input name="DSobservacion#rsLineasProceso.DSlinea#" type="hidden" value="#varDSobservacion#">
						  ...
						  &nbsp;Item: 
									...
									<cfif isdefined("rsLineasProceso.Acodigo") and rsLineasProceso.Acodigo NEQ ''>
						   			...
						   			<cfif isdefined("rsLineasProceso.Ccodigo") and rsLineasProceso.Ccodigo NEQ ''>
											&nbsp;#rsLineasProceso.Ccodigo# &nbsp; -
									</cfif>
									---->  
						  ...
						  <cfif isdefined("vsArticulos") and len(trim(vsArticulos))>
							  <cfif ListContains(vsArticulos,rsLineasProceso.Aid,',') NEQ 0>
									<strong>N° Parte:</strong>&nbsp;<input name="NumeroParte#rsLineasProceso.DSlinea#" type="text" value="<cfif modo EQ 'ALTA'>#rsLineasProceso.AuxNumeroParte#<cfelse>#rsDetalleLinea.numparte#</cfif>" size="15" maxlength="20">								
                      ...
                      <tr>
                     	<!---►►Numero de Solicitud◄◄--->
                      	<td align="center" style="border-left: 1px solid black; border-bottom: 1px solid black; border-right: 1px solid black; ">#rsLineasProceso.ESnumero#</td>
                      	<!---►►Linea de la Solicitud◄◄--->
                      	<td align="center" style="border-left: 1px solid black; border-bottom: 1px solid black; border-right: 1px solid black; ">#rsLineasProceso.DSconsecutivo#</td>
                        <!---►►cantidad◄◄--->
                        ...
                        <td align="center" style="border-bottom: 1px solid black; border-right: 1px solid black; ">
							...
							<cfelse>
						  	...
						  	<input readonly="" class="cajasinbordeb" name="MtoImpuesto_#rsLineasProceso.DSlinea#" type="text" size="12" maxlength="20" 
		...
		<tr>
			<td align="center">
				...
				--->
				<input type="submit" name="btnGuardar" value="Guardar" onClick="javascript: return habilitarValidacion();" tabindex="3" class="btnGuardar">
				<cfif modo EQ "CAMBIO">
					<input type="submit" name="btnEliminar" value="Eliminar" onClick="javascript: habilitarValidacion();" tabindex="3" class="btnEliminar">
                    <input type="submit" name="btnAplicar" value="Aplicar" onClick="return fnAplicar();" tabindex="3" class="btnAplicar">
				</cfif>
				<input type="button" name="btnRegresar" value="Regresar" onClick="javascript: deshabilitarValidacion(); funcRegresar();" tabindex="3" class="btnAnterior">
			</td>
		</tr>
		<tr><td>&nbsp;</td></tr>
	</table>
...
<!--//
	...
	function habilitarValidacion(){
		...
		error = "";
		<cfoutput query="rsLineasProceso">
			if (document.form1.DCpreciou_#rsLineasProceso.DSlinea#.value <= 0)
				error = error + '\n La Cotización de la SC #rsLineasProceso.ESnumero# Linea #rsLineasProceso.DSconsecutivo# no puede tener Precio menor o igual a cero';
				
			if (document.form1.DCcantidad_#rsLineasProceso.DSlinea#.value <= 0)
				error = error + '\n La Cotización de la SC#rsLineasProceso.ESnumero# Linea #rsLineasProceso.DSconsecutivo# no puede tener la cantidad menor o igual a cero';
		</cfoutput>
		
		if (error != ""){
			alert('Se presentaron los siguientes Errores:'+ error);
			return false;
	...
	function fnAplicar(){
		if(!confirm("Desea aplicar la cotización?"))
			return false;
		deshabilitarValidacion();
