...
<!---Agrego los cambios en el fuente para usar en Gestion de Autorizaciones--->
    ...
    <cftransaction>
        <cfquery name="updProceso" datasource="#Session.DSN#">
            ...
            set CMPestado = 79, <!--- Buscar un estado, por el momento este es temporal --->
            	 CMPfechaEnvAprob = <cf_dbfunction name="now">
            where CMPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#lvarCMPid#">
            and Ecodigo = #Session.Ecodigo#
        </cfquery>
...
<cfelseif isdefined('form.btnAprobar') or isdefined('url.btnAprobar') >
	<cfif isdefined('form.chk') and len(trim(form.chk)) and len(trim(form.CMPid)) eq 0>
    ...
    <cfquery name="rsEmpresaProv" datasource="#session.DSN#">
        ...
        where dpc.DPCecodigo = #lvarFiltroEcodigo#
    </cfquery>
    ...
    <cftransaction>
        <cfquery name="updProceso" datasource="#Session.DSN#">
            ...
            set CMPestado = 81, <!--- Buscar un estado, por el momento este es temporal --->
            	 CMPfechaAprob = <cf_dbfunction name="now">
            where CMPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.CMPid#">
            and Ecodigo = #lvarFiltroEcodigo#
        </cfquery>
 ...
 <cfelseif isdefined('form.btnRechazar') or isdefined('url.btnRechazar')>
	<cfif isdefined('form.chk') and len(trim(form.chk)) and len(trim(form.CMPid)) eq 0>
    	<cfset form.CMPid = form.chk>
		<cfset lvarFiltroEcodigo = session.Ecodigo>
...
<cfif isdefined('url.btnAprobar') or isdefined('url.btnRechazar')>
	<form action="/cfmx/proyecto7/cotizaciones.cfm" method="post" name="sql"></form>
</cfif>

<cfif isdefined('form.AprobarS')>
<form action="evaluarCotizaciones.cfm" method="post" name="sql"></form>
<cfelseif isdefined('form.btnAprobar') or isdefined('form.btnRechazar')>
<form action="evaluarCotizacionesSolicitante.cfm" method="post" name="sql"></form>
<cfelse>
<form action="evaluarCotizaciones.cfm?<cfif LEN(TRIM(params))>EOidorden=#params#</cfif>" method="post" name="sql"></form>
</cfif>
</cfoutput>
