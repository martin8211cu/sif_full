...
<cfelse>
  <cfset campos= "DOconsecutivo,ref,CMTipodesc,Codigo,DOdescripcion,DOcantidad,Udescripcion,DOpreciou,DOtotal,totieps,DOimpuestoCF,MTotal">
  <cfset etiquetas= "L&iacute;nea,Ref.,Tipo,C&oacute;digo,Descripci&oacute;n,Cantidad,Unidad Medida,Precio,Subtotal,IEPS,Impuesto,Total">
  <cfset formatos= "V,V,V,V,V,M,V,V,M,M,M,M">
  <cfset posiciones= "left,center,left,left,left,right,right,right,right,right,right,right">
...
<cfquery name="rsListaItems" datasource="#session.dsn#">
	select 	a.DOlinea as Linea,
			...
			Udescripcion,
			a.DOconsecutivo,
			case a.CMtipo when 'A' then 'Art&iacute;culo' when 'S' then 'Servicio' when 'F' then 'Activo' when 'P' then 'Obras' end as CMTipodesc,
			a.DOdescripcion, a.DOimpuestoCF,(a.DOMontIepsCF+ a.DOMontIeps) as totieps,
			a.DOcantidad as  DOcantidad,
			#LvarOBJ_PrecioU.enSQL_AS("a.DOpreciou")#,
			coalesce(round(a.DOtotal - a.DOmontodesc,2),0) as DOtotal,
			coalesce(round(a.DOtotal - a.DOmontodesc + a.DOimpuestoCF + (a.DOMontIepsCF+ a.DOMontIeps),2),0) as MTotal,
    ...
    <td>
	  <table align="right" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td align="right">
					<input  type="text" style="text-align:right" readonly id="_subtotal" name="subtotal" value="<cfif modo EQ "CAMBIO" >#LSCurrencyFormat(rsTotales.subtotal,'none')#<cfelse>0.00</cfif>">
				</td>
			</tr>
			...
			<tr>
				...
				<td align="right">
					<input  type="text" style="text-align:right" readonly id="_descuento" name="_descuento" value="<cfif modo EQ "CAMBIO">#LSCurrencyFormat(rsTotales.descuento,'none')#<cfelse>0.00</cfif>">
				</td>
			</tr>
			...
			<tr>
				...
				<td align="right">
					<input type="text" style="text-align:right" readonly id="_iEPS" name="_iEPS" value="<cfif modo EQ "CAMBIO">#LSCurrencyFormat(rsTotales.ieps,'none')#<cfelse>0.00</cfif>">
				</td>
			</tr>
			...
			<tr>
				...
				<td align="right">
					<input  type="text" style="text-align:right" readonly id="_impuesto" name="_impuesto" value="<cfif modo EQ "CAMBIO">#LSCurrencyFormat(rsTotales.impuesto,'none')#<cfelse>0.00</cfif>">
				</td>
			</tr>
			...
			<tr>
				...
				<td align="right">
					<input type="text" style="text-align:right; " readonly id="_total" name="_total" value="<cfif modo EQ "CAMBIO">#LSCurrencyFormat(rsTotales.total,'none')#<cfelse>0.00</cfif>">
				</td>
			</tr>
	  </table>
