<cfsetting requesttimeout="16600">
			...
			<tr>
				...
				<td align="right">          
					<cfquery name="rsCicloD" datasource="#session.DSN#">
						select 
								coalesce(sum(
									coalesce(<cf_dbfunction name="datediff" args="(select max(EOfalta)
																								from EOrdenCM eo
																									inner join DOrdenCM do
																										on eo.Ecodigo = do.Ecodigo
																										and eo.EOidorden = do.EOidorden
																										and eo.EOestado = 10
																								where do.ESidsolicitud = d.ESidsolicitud),e.CMPfmaxofertas">,0)
								), 0.00) as vnDif,
								coalesce(count(1), 0) as vnCant
						from ESolicitudCompraCM a 
