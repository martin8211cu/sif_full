...
<table width="98%" cellpadding="2" cellspacing="0" border="0">
	...
	<cfloop list="#vsTipos#" index="vntip">
		...
		<cfloop condition="finicial lte ffinal">
			...
			<tr>
				...
				<td align="right">  
				<cfset VarFini = (#form.anoIni# * 12 + #form.mesIni# - 1)>
				<cfset VarFfin = (#form.anoFin# * 12 + #form.mesFin# - 1)>
				
				    <cfquery name="rsMaxEOfAlta" datasource="#session.DSN#">
					select max(EOfalta) as EOfalta
						from EOrdenCM eo
						...
						where do.ESidsolicitud = d.ESidsolicitud	
							...
							and (<cf_dbfunction name="date_part" args="YY,a.ESfecha"> * #factorAgrupar# + floor((<cf_dbfunction name="date_part" args="MM,a.ESfecha"> - 1) / #divisorAgrupar#)) = <cfqueryparam cfsqltype="cf_sql_integer" value="#(vna * factorAgrupar + Int((vni - 1) / divisorAgrupar))#">
							and (<cf_dbfunction name="date_part" args="YY,a.ESfecha"> * 12 + <cf_dbfunction name="date_part" args="MM,a.ESfecha"> - 1) >= <cfqueryparam cfsqltype="cf_sql_integer" value="#VarFini#">
							and (<cf_dbfunction name="date_part" args="YY,a.ESfecha"> * 12 + <cf_dbfunction name="date_part" args="MM,a.ESfecha"> - 1) <= <cfqueryparam cfsqltype="cf_sql_integer" value="#VarFfin#">										 
					</cfquery> 
			
					<cfif len(trim(rsMaxEOfAlta.EOfAlta)) gt 0>
						   <cfset LvarEOfalta = rsMaxEOfAlta.EOfAlta>					   
					<cfelse>
	    					<cfset LvarEOfalta = '1900-01-01'> 	
					</cfif> 
	...
	<!---				<cf_dbfunction name="to_date00"	args="#rsMaxEOfAlta.EOfalta#"  returnvariable="LvarMaxEOfalta" >--->
					...
					<cfquery name="rsCicloD" datasource="#session.DSN#">
						select 
								coalesce(sum(
									coalesce(<cf_dbfunction name="datediff" args="#LSParseDateTime(LvarEOfalta)#, e.CMPfmaxofertas">,0)
								), 0.00) as vnDif,
								coalesce(count(1), 0) as vnCant
						from ESolicitudCompraCM a 
							inner join DSolicitudCompraCM b 
