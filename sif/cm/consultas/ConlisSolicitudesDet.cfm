...
<cfset navegacion = navegacion & Iif(Len(Trim(navegacion)) NEQ 0, DE("&"), DE("")) & "idx=" & index>
...
<cfif isdefined("Form.CMSid") and Len(Trim(Form.CMSid)) NEQ 0>
 	<cfset filtroObserv = filtroObserv & " and a.CMSid=" & form.CMSid >
	<cfset navegacion = navegacion & Iif(Len(Trim(navegacion)) NEQ 0, DE("&"), DE("")) & "CMSid=" & Form.CMSid>
</cfif>
...
<cfquery name="rsLista" datasource="#session.DSN#">
	...
	where a.ESestado in (-10,0,20,25,40,50,60)
		...
		<!---<cfif isdefined("session.compras.solicitante") and len(trim(session.compras.solicitante))>
			and a.CMSid=<cfqueryparam cfsqltype="cf_sql_numeric" value="#session.compras.solicitante#">
		</cfif>--->
...
<!--- 	
	...
	where a.ESestado in (20,25,40,50)
	and a.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
	#preservesinglequotes(filtroObserv)#
	order by a.CMTScodigo --->
</cfquery>
...
<tr>
<td>
<form style="margin:0;" name="filtroOrden" method="post" action="ConlisSolicitudesDet.cfm">
...
<td>
	<cfinvoke 
		...
		<cfinvokeargument name="irA" value="ConlisSolicitudesDet.cfm"/> 
