...
<cfif isdefined("url.OC") and len(#url.OC#) and not isdefined("form.OC")>
...
<cfquery name="qryLista" datasource="#session.dsn#">
	...
	<cfquery name="rsDSLinea" datasource="#session.dsn#">
	   select distinct(b.CMPid) from DOrdenCM a
	                    ...
	                    where a.Ecodigo = #url.Ecodigo# and a.EOidorden =  <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.EOidorden#">
	</cfquery>
	<cfif rsDSLinea.recordcount gt 0>
		<cfquery name="rsCodigoProceso" datasource="#session.dsn#" maxrows="1">
	     Select CMPcodigoProceso from CMProcesoCompra where Ecodigo = 	#url.Ecodigo# and 
...
<table width="99%" align="center"  border="0" cellspacing="0" cellpadding="2">
	<tr>
		<td colspan="2" class="tituloListas"><strong>Datos de la Orden de Compra</strong></td>
		<td class="tituloListas" nowrap="nowrap">&nbsp;</td>
		<td class="tituloListas" nowrap="nowrap" style="padding-left:20px;">&nbsp;</td>
	</tr>	
	<tr>
		<td colspan="4"><strong>Tipo de Orden:</strong>&nbsp; #CMTOdescripcion#</td>
	</tr>
  <tr>
  <td colspan="4">&nbsp;</td>
  </tr>
 ...
 <tr>
  	...
  	<tr>
		...
		<!--- Estados posibles de la Orden--->
			<cfif isdefined('qryLista.EOestado') and len(trim(qryLista.EOestado)) EQ 0>- No especificado -</cfif>
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ 0>Pendiente</cfif>
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ 5>Pendiente Vía Proceso</cfif>
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ 7>Pendiente O.C. Directa</cfif>
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ -7>En proceso de firmas</cfif>
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ -8>Rechazada por el aprobador</cfif>
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ 8>Pendiente Vía Contrato</cfif>
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ -10>Pendiente de Aprobación Presupuestal</cfif>
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ 10>Aplicadas</cfif>
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ 15>Anuladas</cfif>			
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ 55>Canceladas Parcialmente Surtidas</cfif>
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ 60>Canceladas</cfif>
			<cfif isdefined('qryLista.EOestado') and qryLista.EOestado EQ 70>Anuladas</cfif>
		</td>
...
<table width="99%" align="center"  border="0" cellspacing="0" cellpadding="2">
	<tr>
		...
		<td width="2%" nowrap class="tituloListas" style="padding-right: 5px; border-bottom: 1px solid black; border-left: 1px solid black; ">Cant. Canc.</td>
	...
	<cfoutput query="qryLista"><!----group="EOidorden"> --->
		...
		<tr class=<cfif (currentRow MOD 2)>"listaNon"<cfelse>"listaPar"</cfif>>
			...
			<td width="2%" style="padding-right: 5px; border-bottom: 1px solid black; border-left: 1px solid black;">&nbsp;#DOcantcancel#</td>
		...
		<tr class=<cfif (currentRow MOD 2)>"listaNon"<cfelse>"listaPar"</cfif>>
			...
			<td colspan="11" width="5%" style="padding-left: 5px; border-bottom: 1px solid black;">#DOalterna#&nbsp;</td>
		</tr>
		<tr class=<cfif (currentRow MOD 2)>"listaNon"<cfelse>"listaPar"</cfif>>
			<td style="padding-right: 5px; border-bottom: 1px solid black;">&nbsp;</td>
			<td style="padding-right: 5px; border-bottom: 1px solid black;"><strong>Observaciones:&nbsp;</strong></td>
			<td colspan="11" width="5%" style="padding-left: 5px; border-bottom: 1px solid black;">#DOobservaciones#&nbsp;</td>
		</tr>
