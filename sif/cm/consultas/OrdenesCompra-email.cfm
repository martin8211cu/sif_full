...
<cf_templateheader title="Envío de Órdenes de Compra Aplicadas">
		<cfquery name="hdr" datasource="#session.dsn#">
			select a.EOnumero, coalesce (a.EOtotal, 0) as EOtotal, b.SNnombre, b.SNemail, m.Msimbolo, m.Miso4217, a.SNcodigo
			from EOrdenCM a
        ...
        <cfquery name="rsContactoSN" datasource="#session.DSN#">
            ...
            where Ecodigo=<cfqueryparam cfsqltype="cf_sql_integer" value="#url.Ecodigo#">
              <cfif len(trim(hdr.SNcodigo)) gt 0>
              	and SNcodigo=<cfqueryparam cfsqltype="cf_sql_integer" value="#hdr.SNcodigo#">
              <cfelse>
              	and SNcodigo = -1
              </cfif>
              and SNCarea = 1
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" align="center">
						...
						<tr>
							<td>&nbsp;</td>
	  						<td>Cc:</td>
	  						<td>&nbsp;</td>
	  						<td><input size="60" type="text" name="Ccemail" id="Ccemail" onFocus="this.select()" value="#HTMLEditFormat(Trim(hdr.SNemail))#;#HTMLEditFormat(Replace(ValueList(rsContactoSN.SNCemail),',',';','ALL'))#">&nbsp;Use punto y coma ';' como separador</td>
	  					</tr>
						<tr>
							<td>&nbsp;</td>
							<td>Asunto:</td>
