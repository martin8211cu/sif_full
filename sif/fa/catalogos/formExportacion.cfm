...
<cfif modo NEQ "ALTA">
	<cfquery name="rsExportacion" datasource="#Session.DSN#">
		...
		where IdExportacion = <cfqueryparam cfsqltype="cf_sql_integer" value="#Form.IdExportacion#">
	</cfquery>
</cfif>
...
<cfquery name="rsCodigos" datasource="#session.DSN#">
	...
	where 1=1
	<cfif modo neq "ALTA">
		and CSATcodigo != <cfqueryparam cfsqltype="cf_sql_char" value="#rsExportacion.CSATcodigo#">
	</cfif>
</cfquery>

<!---SML18082022 Valida si ya existe un Tipo de Exportacion por default --->
<cfquery name="rsExportacionDefault" datasource="#Session.DSN#">
	select IdExportacion
	from CSATExportacion
	where CSATdefault = 1
</cfquery>
...
<form action="<cfoutput>#LvarSQLPagina#</cfoutput>" method="post" name="form1">
  ...
  <tr>
    ...
    <td nowrap>
		...
		value="<cfif isdefined("rsExportacion.CSATcodigo") and len(trim(rsExportacion.CSATcodigo))>#trim(rsExportacion.CSATcodigo)#</cfif>">
    </td>
  </tr>
  <tr>
    <td align="right" nowrap>Descripción:&nbsp;</td>
    <td nowrap><input name="descSAT" type="text"
	value="<cfif isdefined("rsExportacion.CSATdescripcion") and len(trim(rsExportacion.CSATdescripcion))>#trim(rsExportacion.CSATdescripcion)#</cfif>">
  </tr>
<tr>
    <td align="right" nowrap>Fecha de Vigencia:&nbsp;</td>
	<td>
	<cfif isdefined("rsExportacion.CSATfechaVigencia") and len(trim(rsExportacion.CSATfechaVigencia))>
	<cf_sifcalendario name="fechaVigencia" value="#rsExportacion.CSATfechaVigencia#">
...
<tr>
	...
	<td><input type="checkbox" id="estatus" name="estatus" <cfif isdefined("rsExportacion.CSATestatus") and trim(rsExportacion.CSATestatus) EQ 1> checked <cfelseif modo EQ "ALTA"> checked </cfif>></td>

</tr>
<tr>
    <td align="right" nowrap>Default:&nbsp;</td>
	<td><input type="checkbox" id="default" name="default" <cfif isdefined("rsExportacion.CSATdefault") and trim(rsExportacion.CSATdefault) EQ 1> checked <cfelseif modo EQ "ALTA" and isdefined("rsExportacionDefault") and rsExportacionDefault.RecordCount EQ 0> checked </cfif>></td>
  	...
  	<td nowrap colspan="2">
		<cfif modo NEQ "ALTA">
			...
			<cfinvoke
				...
				method="toTimeStamp"
				returnvariable="ts">
				<cfinvokeargument name="arTimeStamp" value="#rsExportacion.ts_rversion#"/>
			</cfinvoke>
		</cfif>
		<cfif modo NEQ "ALTA">
			<cfoutput>
				<input type="hidden" name="IdExportacion" value="#rsExportacion.IdExportacion#">
...
<script language="JavaScript1.2" type="text/javascript">
	...
	function __CodeExists(){
		...
		<cfif modo neq "ALTA">
		&& "#Trim(rsExportacion.CSATcodigo)#".toUpperCase( ) != trim(this.value.toUpperCase( ))
		</cfif>
		) {
		this.error = "El Código de Exportación que intenta insertar ya existe";
		}
		</cfoutput>
	}
	...
	objForm.codSAT.description="Código Exportación";
	objForm.descSAT.description="Descripcion de Exportación";
