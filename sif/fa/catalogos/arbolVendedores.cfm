...
--->	
	<cfquery datasource="#session.dsn#" name="siguiente">
		select FVjefe as padre 
		from FVendedores
		where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
		  and FVid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#current#">
	</cfquery>
...
--->
<cfquery datasource="#session.dsn#" name="ARBOL">
	select convert(varchar, c.FVid) as FVid, c.FVnombre, c.FVnivel as nivel,  
	       ( select count(1) 
		     from FVendedores c2
			 where c2.FVjefe = c.FVid
			   and c2.Ecodigo = c.Ecodigo ) as  hijos
	from FVendedores c
	where c.Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
	  and ( c.FVjefe is null
			<cfif Len(path)>
				or c.FVjefe in (<cfqueryparam cfsqltype="cf_sql_integer" value="#path#" list="yes">)
			</cfif> )
	order by c.FVpath
</cfquery>
...
<script language="JavaScript" type="text/javascript">
	...
	function eclk(arbol_pos){<!--- event: Click --->
		location.href="Vendedores.cfm?ARBOL_POS="+arbol_pos+"<cfoutput>#JSStringFormat(QueryString_ARBOL)#</cfoutput>";
	}
</script>
...
<table cellpadding="0" cellspacing="1" border="0" width="100%">
	...
	<cfloop query="ARBOL">
		<tr valign="middle"	
			<cfif ARBOL.FVid is url.ARBOL_POS> class='ar1'
			<cfelse>class='ar2' onMouseOver="eovr(this)" onMouseOut="eout(this)" onClick="eclk('#ARBOL.FVid#')"
			</cfif> >
			<td nowrap>
				#RepeatString('&nbsp;', ARBOL.nivel*2+2)#
				<cfif ARBOL.hijos and ListFind(path,ARBOL.FVid)>
					<img src="../../js/xtree/images/openfoldericon.png" width="16" height="16" border="0" align="absmiddle">
				...
				#HTMLEditFormat(Trim(ARBOL.FVid))# - #HTMLEditFormat(Trim(ARBOL.FVnombre))#
			</td>
		</tr>
	</cfloop>
</table>
