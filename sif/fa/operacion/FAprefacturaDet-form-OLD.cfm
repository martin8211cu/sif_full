...
<cfif modoDet NEQ 'ALTA'>
    ...
    <cfquery name="rsFormDet" datasource="#session.DSN#">
		Select
            ...
            Ccuenta,
            fd.CFid,
            CFcodigo as CFcodigoresp, CFdescripcion as CFdescripcionresp,
			Icodigo,
			Descripcion,
            Descripcion_Alt,
			PrecioUnitario,
...
<form name="formDetCoti" id="formDetCoti" method="post" action="FAprefactura-sql.cfm" onSubmit="enableIieps(); javascript: return valida();">
    ...
    <tr>
     ...
     <tr>
        ...
        <td><input name="PrecioUnitario" type="text" id="PrecioUnitario" <cfif modoDet NEQ 'Alta'> value="#LSNumberFormat(rsFormDet.PrecioUnitario,',9.00')#"<cfelse> value="0.00"</cfif> style="text-align: right" size="20" maxlength="18" tabindex="26" onFocus="this.value=qf(this); this.select();" onBlur="fm(this,2);"  onKeyUp="if(snumber(this,event,2)){ if(Key(event)=='13') {this.blur();}calcTotal();}; calculaIEPS();"></td>
      </tr>

      <tr>
        <td align="right"><strong>#LB_Impuesto#:&nbsp;</strong></td>
        <td nowrap>
			<select name="Icodigo" id="Icodigo" tabindex="23" style="width: 146px">
			  ...
			  <option value = "-1">--Sin Retenci&oacute;n--</option>
				<cfloop query="rsRetenciones1">
					<!----<cfif #rsRetenciones1.Rcodigo# NEQ 1>--->
						<option value="#rsRetenciones1.Rcodigo#" <cfif modoDet NEQ 'ALTA' and rsRetenciones1.Rcodigo EQ rsFormDet.Rcodigo>selected</cfif>>#rsRetenciones1.Rcodigo# - #rsRetenciones1.Rdescripcion#</option>
					<!---</cfif>--->
				</cfloop>
			</select>
			<!--- <input type="hidden" name="Icodigo" value="<cfif modoDet NEQ 'Alta'>#rsFormDet.Icodigo#</cfif>"> --->
...
<script language="javascript" type="text/javascript">
	...
	function ValidaCantidades() {
		if (document.formDetCoti.Cantidad.value <= 0 || document.formDetCoti.PrecioUnitario.value <= 0 || document.formDetCoti.TotalLinea.value <= 0)
		{
			alert ('<cfoutput>#MSG_NoPuedeHaberCantidadPrecioUnitarioTotalDeLineaEnCero#</cfoutput>');
			return false;
		}
		else
		{
