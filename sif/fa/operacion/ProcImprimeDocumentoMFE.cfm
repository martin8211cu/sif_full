...
<cfif isdefined("form.OImpresionID") and len(trim(form.OImpresionID))>
	...
	<cfif isdefined("form.modo") and form.modo EQ "GUARDA">
		...
		<cfif form.RegFisc EQ "">
        	<cfabort showerror="El Registro Fiscal no puede contener un valor nulo">
		</cfif>
  		<cfif isdefined("form.TipoCambio") and form.TipoCambio EQ "">
        	<cfabort showerror="Especifique un valor para Tipo de Cambio">
	  	</cfif>

   		<cfif isdefined ("form.cuentaB") and form.cuentaB NEQ "">
	   		<cfset NcuentaB=#form.cuentaB#>
	    <cfelse>
	    	<cfset NcuentaB="No identificado">
	   </cfif>
        ...
        <cfquery datasource="#session.DSN#">
			...
			c_metPago = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.metPago#">,
            Cta_tipoPago = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.usocfdi#">
			<!--- Complemento INEE --->
			<cfif isdefined("form.usaINE") and form.usaINE eq "1">
				,OITipoProcesoINE = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.OITipoProcesoINE#">
				,OIComiteAmbito = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.OIComiteAmbito#">
				<cfif form.OIComiteAmbito eq 'Ejecutivo Estatal'
				or form.OIComiteAmbito eq 'Directivo Estatal'
					or form.OITipoProcesoINE eq 'Ejecutivo Campaña'
					or (form.OITipoProcesoINE eq 'Precampaña' or form.OITipoProcesoINE eq 'Campaña')>
					,OIEntidad = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.OIEntidad#">
				<cfelse>
					,OIEntidad = null
				</cfif>
				<cfif len(trim(form.OIIdContabilidad)) gt 0>
					,OIIdContabilidadINE = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.OIIdContabilidad#">
				</cfif>
			</cfif>
			<cfif isdefined("form.hdTipoRelac") and len(trim(form.hdTipoRelac)) GT 0>
				,OItipoRel = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.hdTipoRelac#">
			<cfelse>
				,OItipoRel = null
			</cfif>
			<cfif (isdefined("form.sust") and len(trim(form.sust)) GT 0) or (isdefined("form.anticipo") and len(trim(form.anticipo)) GT 0)>
				,OIsustitucion = 1
			<cfelse>
				,OIsustitucion = 0
			</cfif>
		...
		<cfif (isDefined('form.sust') or isDefined('form.anticipo')) and isDefined('form.OImpresionID') and (isdefined("form.hdTipoRelac") and len(trim(form.hdTipoRelac)) GT 0)>
			<cfquery name="rsRelacionados" datasource="#session.dsn#">
				select *
				from FA_CFDI_Relacionado
				where OImpresionID = <cfqueryparam cfsqltype="cf_sql_integer" value="#form.OImpresionID#">
				and TipoRelacion != <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.hdTipoRelac#">
				and Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
			</cfquery>
			<cfif rsRelacionados.recordCount gt 0>
				<cfquery datasource="#session.dsn#">
					update FA_CFDI_Relacionado
					set TipoRelacion = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.hdTipoRelac#">
					where OImpresionID = <cfqueryparam cfsqltype="cf_sql_integer" value="#form.OImpresionID#">
					and TipoRelacion != <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.hdTipoRelac#">
					and Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
				</cfquery>
		...
		<cfif isDefined("form.valueschecked")>
		
		<cfif form.CCTCODIGO eq 'NC' and form.valueschecked neq ''>
		
			<!--- <cfquery name="rsExisteR" datasource="#session.dsn#">
				delete from FA_CFDI_Relacionado
				where Ecodigo = #session.Ecodigo#
				and OImpresionID = #form.OImpresionID#
			</cfquery>
			<cfset fol = ListToArray(form.valueschecked,'|')>
			...
			<cfloop query="docsSelec">
				<cfquery datasource="#session.dsn#">
					...
					<cfqueryparam cfsqltype="cf_sql_numeric" value="#form.OImpresionID#">,
					'',
					<cfqueryparam cfsqltype="cf_sql_numeric" value="0">,
					'',
					<cfqueryparam cfsqltype="cf_sql_varchar" value="#form.CMBTIPOREL#">,
    ...
    <cfelseif isdefined("form.modo") and form.modo EQ "CANCELAOI">
		...
		<!--- Elimina relacion de notas de credito. --->
		<cfquery datasource="#session.dsn#">
			delete from FA_CFDI_Relacionado
			where Ecodigo = #session.Ecodigo#
			and OImpresionID = #form.OImpresionID#
    ...
    <cfelseif isdefined("form.modo") and form.modo EQ "REGISTRA">
		...
		<cfelse>
	                ...
	                <cfquery datasource="#session.DSN#">
    	            	insert DDocumentosCxC
        	            	(EDid, Aid, Cid, Alm_Aid,
                        	 ...
                        	 BMUsucodigo,Dcodigo,codIEPS,DDMontoIeps,afectaIVA)
	                    values
    	                	(<cfqueryparam cfsqltype="cf_sql_integer" value="#insertDoc.identity#">,
            	             ...
            	             0,
                              <cfif rsOIDinsert.OIMontoIEPSLinea GT 0 and rsOIDinsert.afectaIVA Eq 1>
          ...
          <!--- <cfoutput> <cf_dump var = "#llavecxc#"> </cfoutput> --->
  ...
  <!--- Impresion para la factura electronica   --->
	<cfif isdefined("form.modo") and form.modo EQ "impFACele">
      <cfswitch expression="#session.CEcodigo#">
      	<cfcase value="2"> <cfinclude template="ImpresionMFECCO.cfm"> 		</cfcase>
  ...
  <!--- Codifo de prueba para la factura electronica   --->
    ...
    <cfif not isdefined("form.foliofacele")>
      ...
      <cfif campofactura EQ 1>
        <cfif rsfacele.foliofacele eq '0'>
          <!---<cfthrow message="#form.OImpresionID#">--->
            ...
            webservice="http://serverws/mfe/WS_MFE.asmx?wsdl" --->
              ...
              <!--- webservice="http://localhost/ccom/ws_mfe.asmx?wsdl"
                method="emitirfactura"
            returnvariable="rawXMLBookList">
            <cfinvokeargument name="numdoc" value = #form.OImpresionID#/>
            </cfinvoke> --->

		<cfquery name="validaFE" datasource="#session.dsn#">
            select * from FA_CFDI_Emitido
            where Ecodigo=#session.Ecodigo#
            and OImpresionID=#form.OImpresionID#
            and Serie='#rsfacele.CCTcodigo#'
        </cfquery>
		

          <cfset addendaDetalleStruct = StructNew() />
				<cfloop collection = "#form#" item = "fieldFormAdd">
						<cfif fieldFormAdd is not "fieldNames">
						    <cfset findFieldAdd = REFindNoCase("^addendaDetallePrompt", fieldFormAdd) />
							  <cfif findFieldAdd GT 0>
								    <cfset addendaDetalleStruct[REReplaceNoCase(fieldFormAdd, "addendaDetallePrompt_", "")] =
									      form[fieldFormAdd] />
								</cfif>
						</cfif>
				</cfloop>
			
          <cfif validaFE.RecordCount GT 0>
            <cfif validaFE.stsTimbre EQ 1>
              <cfthrow message="Esta factura ya fue Timbrada...">
              <cfabort>
            <cfelse>
              <cfinvoke component="rh.Componentes.GeneraCFDIs" method="obtenerCFDI">
                <cfinvokeargument name="OImpresionID" value = "#form.OImpresionID#">
                <cfinvokeargument name="adenda" value = "#form.ADDcodigo#">
				<cfinvokeargument name = "addDetStruct" value = "#addendaDetalleStruct#">
              </cfinvoke>
            </cfif>
          <cfelse>
            <cfinvoke component="rh.Componentes.GeneraCFDIs" method="obtenerCFDI">
          ...
          <cfquery name="validaFE" datasource="#session.dsn#">
            select top 1 * from FA_CFDI_Emitido
            where Ecodigo=#session.Ecodigo#
            and OImpresionID=#form.OImpresionID#
            order by Folio desc
          </cfquery>
          <cfif validaFE.ststimbre EQ 1>
           ...
           <!---  <cftransaction> --->
            <cfinclude template="ImpresionMFECCO.cfm">
          ...
          <cfelse>
			<cfquery name="q_revertChanges" datasource="#session.dsn#">
            	...
            	<cfthrow message="Error #validaFE.ststimbre# al Timbrar la factura...#validaFE.xmlTimbrado#">
            <cfabort>
          </cfif>
        </cfif>
      </cfif>
	...
	<!--- Si tenemos seleccionada la sustitución, despues de timbrar la factura se lleva a cabo la sustitución --->
	<cfif isDefined("form.FACELE") and form.FACELE eq 'Genera FE'>
	<cfif isDefined("form.sust")>
		<cfif version eq '4.0'>
        			<cfif form.sust neq ''>
    					...
    					<cfif parametro eq "Konesh">
							...
							<cfset OImpresion = LSParseNumber(form.OImpresionID)>
							<cfset rfcEmisor = rsEmpresa.Eidentificacion>
							<cfquery name="rsCod" datasource="#session.dsn#">
								select * from FA_CFDI_Emitido where OImpresionID = <cfqueryparam cfsqltype="cf_sql_integer" value="#OImpresion#">
								and Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">
							</cfquery>
							<cfquery name="rsRelacionado" datasource="#session.dsn#">
										select DocumentoRelacionado from FA_CFDI_Relacionado
										where OImpresionID = <cfqueryparam cfsqltype="cf_sql_integer" value="#OImpresion#">
										and Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">

							</cfquery>
							<cfset codigo= rsRelacionado.DocumentoRelacionado>
    					    <cftransaction>
    					...
    					<cfif parametro eq "Manual">
							...
							<!--- Invoca el componente de sustitucion --->
								...
								<cfset codigo= listToArray(rsRelacionado.DocumentoRelacionado)>
								<cfset sust = 1>
    							    <cftransaction>
        								<cfinvoke component="sif.Componentes.FA_CancelacionFactura"
