...
<cfelseif IsDefined("form.AltaDet") or (IsDefined("form.BOTONSELDET") and #form.BOTONSELDET# eq 'AltaDet')>
	...
	<cftransaction>
		...
		VALUES  (<cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">,
			...
			<cfelse>
				...
				<cfqueryparam cfsqltype="cf_sql_money" value="#LSParseNumber(form.PrecioUnitario)#">,
	...
	<cfquery datasource="#Session.DSN#">
		update FAPreFacturaE 
			set MRetencion = 
			    (select sum( round( ((Cantidad * PrecioUnitario)) * (ir.Iporcentaje / 100) ,6) ) as sumRetencion
	...
	<cfquery name="insertEnc" datasource="#session.dsn#">
		update FAPreFacturaD set
			...
			<cfelse>
				...
				, PrecioUnitario = <cfqueryparam cfsqltype="cf_sql_money" value="#LSParseNumber(form.PrecioUnitario)#">
				, DescuentoLinea = <cfqueryparam cfsqltype="cf_sql_money" value="#LSParseNumber(form.DescuentoLinea)#">
				, TotalLinea = <cfqueryparam cfsqltype="cf_sql_money" value="#LSParseNumber(form.TotalLinea) - Replace(LSParseNumber(Form.IepsLinea),',','','all')#">
				, BMUsucodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Usucodigo#">
	...
	<cfif (isdefined("Form.chk")) or isdefined("Form.Aplicar_Factura") >
		...
		<cfloop from="1" to="#limite#" index="idx">
			...
			<cfquery name="rsPreFact" datasource="#session.dsn#">
            	select PFTcodigo,SNcodigo,Mcodigo,Ocodigo,IDpreFactura,vencimiento,FechaVen,Rcodigo,MRetencion
                from FAPreFacturaE
		...
		<cfquery name="rsPFllave" datasource="#session.dsn#">
        	select distinct PFTcodigo,SNcodigo,Mcodigo,Ocodigo,vencimiento,FechaVen,Rcodigo,MRetencion
            from #PF_aplicacion#
        </cfquery>
				...
				<cfif varGeneraDoc>
            		...
            		values
                		(<cfqueryparam cfsqltype="cf_sql_integer" value="#varNumOI#">,
						 ...
						 0,
						 <cfqueryparam cfsqltype="cf_sql_money"   value="#rsPFllave.MRetencion#">
                     	)
            			<cf_dbidentity1 datasource="#session.DSN#">
				</cfquery>
					...
					select sum(d.OIDdescuento) as sumDescuento, sum(OIDCantidad * OIDPrecioUni) as sumSubTotal, SUM(OIMontoIEPSLinea) as IEPS,
						case i.IEscalonado
							when 1 then
								sum( round( (((OIDCantidad * OIDPrecioUni) - d.OIDdescuento) + d.OIMontoIEPSLinea) * (i.Iporcentaje / 100) ,6) )
							else
								sum( round( ((OIDCantidad * OIDPrecioUni) - d.OIDdescuento) * (i.Iporcentaje / 100) ,6) )
							end as sumImpuesto,
							sum( round( ((OIDCantidad * OIDPrecioUni) - d.OIDdescuento) * (ir.Iporcentaje / 100) ,6) ) as sumRetenciones
							...
							<cfif rsVerificaDet.recordcount GT 0>
								...
								<cfquery datasource="#session.dsn#">
									update DDocumentosCxC
	                                    ...
	                                    DDpreciou 		 = <cfqueryparam cfsqltype="cf_sql_money" value="#rsDatos.PRECIOUNITARIO#">
									where
	...
	<!---ERBG Pone el Centro Funcional Default FIN--->
					...
					<cfquery name="rsTotalesOI" datasource="#session.DSN#">
						select sum(d.DDdesclinea) as sumDescuento, sum(d.DDcantidad * d.DDpreciou) as sumSubTotal, SUM(d.DDMontoIEPS) as IEPS,
							case i.IEscalonado
								when 0 then
									sum(((d.DDCantidad * d.DDpreciou) - d.DDdesclinea) * (i.Iporcentaje / 100))
								when 1 then
									sum((((d.DDCantidad * d.DDpreciou) - d.DDdesclinea) + d.DDMontoIEPS) * (i.Iporcentaje / 100))
								end as sumImpuesto,
								sum(((d.DDCantidad * d.DDpreciou) - d.DDdesclinea) * (ir.Iporcentaje / 100)) as sumRetenciones
...
<cfif isDefined('form.replicarA')>
	...
	<cf_dbtemp name="TT_ReplicaD" returnvariable="varReplicaD" datasource="#session.DSN#">
		...
		<cf_dbtempcol name="PrecioUnitario"		type="money">
	...
	<cftransaction>
		...
		<cfquery name="insertEnc" datasource="#session.dsn#">
			INSERT INTO FAPreFacturaE
				(Ecodigo, FAX04CVD, PFDocumento, Ocodigo, SNcodigo, Mcodigo, FechaCot, FechaVen, PFTcodigo, TipoPago, Estatus,
				 Descuento, Impuesto, Total, NumOrdenCompra,
				 Observaciones, TipoCambio, BMUsucodigo, fechaalta, id_direccion, Fecha_doc, vencimiento, Rcodigo, FAieps)
			VALUES (
...
<cfif cambioTotEnc>
	...
	<cfquery name="rsTotales" datasource="#session.DSN#">
		SELECT SUM(coalesce(sumRetenciones, 0)) AS sumRetenciones, SUM(sumImpuesto) AS sumImpuesto, SUM(sumDescuento) sumDescuento, SUM(sumSubTotal) sumSubTotal, SUM(IEPS) IEPS
		FROM (SELECT SUM(d.DescuentoLinea) as sumDescuento, SUM(Cantidad * PrecioUnitario) as sumSubTotal, SUM(isnull(FAMontoIEPSLinea,0)) as IEPS,
					/*SUM(sumRetenciones) AS sumRetenciones,*/
					case d.afectaIVA
						when 0 then
						...
						SUM(((Cantidad * PrecioUnitario)) * (ir.Iporcentaje / 100)) as sumRetenciones
				from FAPreFacturaD d
					inner join FAPreFacturaE ce
						on ce.IDpreFactura = d.IDpreFactura
							and ce.Ecodigo = d.Ecodigo
