...
<cfif (modo EQ "CAMBIO")>
	...
	<cfquery name="rsDSLinea" datasource="#session.dsn#">
	   select distinct(b.CMPid) from DRemisionesFA a
...
<table align="center" width="99%"  border="0" cellspacing="0" cellpadding="0" summary="Formulario del Encabezado de Ordenes de Compra (ERemisionesFA)">
  <tr>
    <td nowrap>&nbsp;</td>
    <td nowrap>&nbsp;</td>
    <td nowrap>&nbsp;</td>
...
<table align="center">
<td>
<cf_botones modo="#modo#" mododet="#mododet#" nameenc="Remision" generoenc="F" tabindex="5" include="#include#" includevalues="#includevalues#" exclude="#exclude#">
</td>
<cfif modo EQ "CAMBIO">
<!--- <td>
...
<td>
<input type="button" class="btnNormal"  tabindex="1" name="Pedidos" value="Pedidos" onClick="javascript:VentanaPedidos(<cfoutput><cfif isdefined('form.EOidorden') and len(trim(#form.EOidorden#))>#form.EOidorden#</cfif></cfoutput>);">
</td>
<!--- <td>
...
<!--// //poner a código javascript
	...
	/*\sif\fa\operacion\Remisiones*/
	//Llama el conlis
	function VentanaPedidos(EOidorden) {
		var params ="";

		params = "&form=form"+

		popUpWindowIns("/cfmx/sif/fa/operacion/Remisiones/popUp-Pedidos.cfm?EOidorden="+EOidorden+"&Ecodigo="+<cfoutput>#lvarFiltroEcodigo#</cfoutput>+params,50,50,window.screen.width/2+290 ,window.screen.height/2);
	}
