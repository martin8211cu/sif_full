<cfparam name="URLLista" default="/cfmx/sif/fa/pedidos/operacion/listaOrdenCM.cfm">
<cfparam name="URLNuevo" default="/cfmx/sif/fa/pedidos/operacion/ordenCompra.cfm">
...
<!---Permite modificar los pedidos cuando vienen del registro de solicitudes--->
...
<cfif (modo EQ "CAMBIO")>
	...
	<!--- Consulta del encabezado del pedido --->
	...
	<cfquery name="rsDSLinea" datasource="#session.dsn#">
	   select distinct(b.CMPid) from DPedido a
...
<cfif rsModificables.CMTOModCodigoEnc eq '1'>
	<cfif camposEditablesE eq ""><cfset coma =""><cfelse><cfset coma =","></cfif>
	<cfset camposEditablesE  = camposEditablesE &coma& 'Tipo de pedido'>
</cfif>
...
<table align="center" width="99%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td nowrap class="subTitulo"><font size="2">Encabezado de Pedido</font></td>
  </tr>
</table>
<table align="center" width="99%"  border="0" cellspacing="0" cellpadding="0" summary="Formulario del Encabezado de Pedidos (EPedido)">
  <tr>
    <td nowrap>&nbsp;</td>
    <td nowrap>&nbsp;</td>
    <td nowrap>&nbsp;</td>
  ...
  <!---Línea 1--->
	<tr>
    <td nowrap align="right"><strong>N&uacute;mero de Pedido:&nbsp;</strong></td>
    <td nowrap>
			<!---Número de pedido: Este campo se llena con un cálculo en el SQL --->
    ...
    <td nowrap align="right"><strong>Tipo de Pedido:&nbsp;</strong></td>
    <td nowrap>
			<!---Tipo de pedido--->
    ...
    <!---<cfif MODO NEQ "ALTA">
        <tr>
			<cf_pedidoFormaPago TESOPFPtipoId="2" TESOPFPid="#rsOrden.EOidorden#">
        </tr>
...
<!--- Los Pedidos por Solicitud de Compra Directa o cuyos items provienen de un contrato no se puede eliminar ni modificar --->
<cfif modo EQ "CAMBIO" and (rsOrden.EOestado EQ 5 or rsOrden.EOestado EQ 7 or rsOrden.EOestado EQ 8)>
	<cfif rsOrden.EOestado EQ 7 and rsModificaOC.value EQ 0>
		<p align="center" style="color: ##FF0000; font-weight: bold;">El pedido fue generado por solicitud de compra directa. No se puede modificar ni eliminar.</p>
	<cfelseif rsOrden.EOestado EQ 8 and rsModificaOC.value EQ 0>
		<p align="center" style="color: ##FF0000; font-weight: bold;">El pedido fue generado por solicitud de compra con itemes que pertenecen a un contrato. No se puede modificar ni eliminar.</p>
	</cfif>
...
<cfif (modo EQ "CAMBIO") and rsOrden.EOestado eq -7 >
   <p align="center" style="color: ##FF0000; font-weight: bold;">El pedido está en espera de ser aprobada por un encargado.</p>
...
<cfelseif (modo EQ "CAMBIO") and rsOrden.EOestado eq -8 >
    ...
    <p align="center" style="color: ##FF0000; font-weight: bold;">El pedido fue Rechazado.</p>
...
<cfelseif (modo EQ "CAMBIO") and rsOrden.EOestado eq 9>
 	<div align="center">
		<cfif isdefined("form.DOlinea") and len(trim(form.DOlinea))>
            ...
            <input name="botonSel"   type="hidden" />
            <p align="center" style="color: ##FF0000; font-weight: bold;">El pedido sólo puede ser aplicada. No permite modificaciones.</p>
			<p align="center" style="color: ##FF0000;">
			<cfif camposEditablesE neq "">
				(*)Campos editables del Encabezado: #camposEditablesE#<br />
			</cfif>
...
<cfelseif modo EQ "CAMBIO" and rsOrden.EOestado EQ 5>
	<div align="center">
        ...
        <cfelse>
                ...
                <input name="botonSel"   type="hidden"/>
                <input name="Nuevo"  	 type="button" value="Nuevo pedido" onclick="return funcNuevo()" class="btnNuevo"/>
                <input name="Regresar"   type="button" value="Regresar" onclick="return funcRegresar()" class="btnAnterior"/>

             <p align="center" style="color: ##FF0000; font-weight: bold;">El pedido proviene de un proceso de publicación.</p>
			 <p align="center" style="color: ##FF0000;">
			<cfif camposEditablesE neq "">
				(*)Campos editables del Encabezado: #camposEditablesE#<br />
			</cfif>
...
<cfelseif (modo EQ "CAMBIO") and (rsOrden.EOestado EQ 7 or rsOrden.EOestado EQ 8) and rsModificaOC.value EQ 0>
     ...
     <cf_botones modo="#modo#" mododet="#mododet#" nameenc="Pedido" generoenc="F" tabindex="5" include="#include#" includevalues="#includevalues#" exclude="#exclude#"><br>
	<p align="center" style="color: ##FF0000;">
	<cfif camposEditablesE neq "">
		(*)Campos editables del Encabezado: #camposEditablesE#<br />
	</cfif>
...
<cfelseif (modo EQ "CAMBIO") and  (rsOrden.EOestado EQ 5 or rsOrden.EOestado EQ 7 or rsOrden.EOestado EQ 8) and rsModificaOC.value NEQ 0>
	<cf_botones modo="#modo#" mododet="#mododet#" nameenc="Pedido" generoenc="F" tabindex="5" include="#include#" includevalues="#includevalues#" exclude="AltaDet"><br>
<cfelse>
<table align="center">
<td>
<cf_botones modo="#modo#" mododet="#mododet#" nameenc="Pedido" generoenc="F" tabindex="5" include="#include#" includevalues="#includevalues#" exclude="#exclude#">
</td>
<!---<cfif modo EQ "CAMBIO">
<td>
...
<!--// //poner a código javascript
	...
	function VentanaSuficiencia(EOidorden) {
		...
		popUpWindowIns("/cfmx/sif/fa/pedidos/operacion/popUp-suficiencia.cfm?EOidorden="+EOidorden+params,window.screen.width*0.05 ,window.screen.height*0.05,window.screen.width*0.90 ,window.screen.height*0.90);
	}
	...
	function VentanaSolicitudCompra(EOidorden) {
		...
		popUpWindowIns("/cfmx/sif/fa/pedidos/operacion/popUp-solicitudCompra.cfm?EOidorden="+EOidorden+"&Ecodigo="+<cfoutput>#lvarFiltroEcodigo#</cfoutput>+params,50,50,window.screen.width/2+290 ,window.screen.height/2);
	}
	...
	function VentanaContrato(EOidorden) {
		...
		popUpWindowIns("/cfmx/sif/fa/pedidos/operacion/popUp-contrato.cfm?EOidorden="+EOidorden+"&Ecodigo="+<cfoutput>#lvarFiltroEcodigo#</cfoutput>+params,window.screen.width*0.05 ,window.screen.height*0.05,window.screen.width*0.90 ,window.screen.height*0.90);
	}
	...
	objForm.EOnumero.description = "#JSStringFormat('Número de pedido')#";
	objForm.CMTOcodigo.description = "#JSStringFormat('Tipo de pedido')#";
	...
	//FUNCION PARA IR A CREAR NUEVO PEDIDO
	function funcNuevo(){
		deshabilitarValidacion();
		window.location = '<cfoutput>#URLNuevo#</cfoutput>';
	}
	//FUNCION PARA IR A CREAR UNA NUEVA LINEA DEL PEDIDO
