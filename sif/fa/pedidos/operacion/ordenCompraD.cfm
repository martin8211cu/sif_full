...
<!---Permite modificar el pedido cuando vienen del registro de solicitudes--->
...
<script language="javascript" type="text/javascript">
 //poner a código javascript
	...
	function doConlisCFuncional() {
		...
		popUpWindow("/cfmx/sif/fa/pedidos/operacion/ConlisCFuncional.cfm"+params,250,200,650,400);
	}
	...
	function TraeCFuncional(dato) {
		...
		if (dato.value != "") {
			document.getElementById("fr").src="/cfmx/sif/fa/pedidos/operacion/cfuncionalquery.cfm?dato="+dato.value+"&form=form1"+params;
		}
		else{
	...
	function info(){
		open('solicitudes-info.cfm?observaciones=DOobservaciones&descalterna=DOalterna&titulo=Pedidos', 'ordenes', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,copyhistory=yes,width=600,height=470,left=250, top=100,screenX=250,screenY=100');
	}
...
<table align="center" width="99%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="subTitulo"><font size="2">Detalle del pedido</font></td>
  </tr>
</table>
...
<!--- JMRV. Inicio. Check de Distribucion para Articulos. 02/05/2014 --->
	...
	<!---Obtiene la distribucion aplicada en la linea del pedido--->
	<cfif isdefined("form.DOlinea") and len(trim(form.DOlinea))>
	...
	<!---Obtiene la descripcion de la distribucion aplicada en la linea del pedido--->
	...
	function calcular_totales(){
		...
		document.getElementById("frTotal").src="/cfmx/sif/fa/pedidos/operacion/orden-total.cfm" + paramTotal;
		MontoTotal();
	}
