...
<cfset socios = "Socios de Negocios Corporativos">
...
function doConlisSocio<cfoutput>#Attributes.SNcodigo#</cfoutput>() {
	...
	popUpWindow("/cfmx/sif/Utiles/ConlisSociosNegocios2.cfm"+params,250,200,650,400);
}
...
function TraeSocio<cfoutput>#Attributes.SNcodigo#</cfoutput>(codigo) {
	...
	if (codigo!="") {
		document.all["<cfoutput>#Attributes.frame#</cfoutput>"].src="/cfmx/sif/Utiles/sifsocionegociosquery2.cfm?codigo="+codigo+"&formulario="+'<cfoutput>#Attributes.form#</cfoutput>'+params;
	}
	else{
