...
<cfif Not ThisTag.HasEndTag>
	<cf_errorCode	code = "50661" msg = "cf_dbupdate requiere de el tag de cierre">
</cfif>
...
<cfif ArrayLen(ThisTag.set) Is 0>
	<cf_errorCode	code = "50662" msg = "cf_dbupdate debe incluir al menos un cf_dbupdate_set">
</cfif>
...
<cfquery datasource="#Attributes.datasource#">
	...
	<cfelseif dstype is 'db2'>
		update #Attributes.table# 
		<!--- set --->
		<cfif left(ltrim(ThisTag.join[1].text),3) EQ "ON ">
			<cfset ThisTag.join[1].text = " WHERE " & mid(ltrim(ThisTag.join[1].text),4,1000)>
		</cfif>
		<cfloop from="1" to="#ArrayLen(ThisTag.set)#" index="set_index">
			<cfif set_index is 1> set <cfelse> , </cfif>
			#ThisTag.set[set_index].name# = 
			<cfif StructKeyExists(ThisTag.set[set_index],'expr')>
				<cfset sbGeneraSubquery(ThisTag.set[set_index].expr)>
			<cfelse>
				<cfqueryparam cfsqltype="cf_sql_#ThisTag.set[set_index].type#"
					value="#ThisTag.set[set_index].value#" null="#ThisTag.set[set_index].null#">
			</cfif>
		...
		<!--- /where --->
		<cfif ThisTag.join[1].type EQ "inner">
			<cfif ArrayLen(ThisTag.where) EQ 0>WHERE<cfelse>AND</cfif>
			<cfset sbGeneraSubquery("count(1)")> > 0
		</cfif>
	<cfelse>
		<cf_errorCode	code = "50663"
						msg  = "DBMS no soportada para cf_dbupdate: @errorDat_1@"
...
<cffunction name="sbGeneraSubquery" output="yes">
	<cfargument name="campo">
				( select
				<cfset texto_sql = Arguments.campo>
				#PreserveSingleQuotes(texto_sql)# 
					from #ThisTag.join[1].table# 
					<cfloop from="2" to="#ArrayLen(ThisTag.join)#" index="join_index">
					#ThisTag.join[join_index].type# join #ThisTag.join[join_index].table# 
						<cfset tmp_arr = tag_list_to_array(ThisTag.join[join_index].text)>
						<cfloop from="1" to="#ArrayLen(tmp_arr)#" index="i2">
							<cfif i2 gt 1>
						...
						<cfset tmp_arr = tag_list_to_array(ThisTag.join[1].text)>
						<cfloop from="1" to="#ArrayLen(tmp_arr)#" index="i2">
							<cfif i2 gt 1>
								<cfset join_param_index = join_param_index + 1>
								<cfqueryparam cfsqltype="cf_sql_#ThisTag.join_params[join_param_index].type#"
