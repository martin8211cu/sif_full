...
<cfparam name="Attributes.id" default="BTid" type="string"> <!--- Nombre del id --->
<cfparam name="Attributes.name" default="BTcodigo" type="string"> <!--- Nombre del Código --->
<cfparam name="Attributes.desc" default="BTdescripcion" type="string"> <!--- Nombre de la Descripción --->
<cfparam name="Attributes.tipo" default="BTtipo" type="string"> <!--- Nombre del tipo de la transacción --->
<cfparam name="Attributes.frame" default="frMBTransaccionesLibros" type="string"> <!--- Nombre del frame --->
...
<cfparam name="Attributes.BTtce" type="integer" default="0"> <!--- FILTRO TARJETAS DE CREDITO EMPRESARIAL --->
...
<script language="JavaScript" type="text/javascript">
	...
	//Llama el conlis
	function doConlisMBTransaccionesLibros<cfoutput>#Attributes.name#</cfoutput>() {
		var params ="";
		params = "<cfoutput>?form=#Attributes.form#&tipo=#Attributes.tipo#&BTtce=#Attributes.BTtce#&id=#Attributes.id#&name=#Attributes.name#&desc=#Attributes.desc#&conexion=#Attributes.conexion#&empresa=#Attributes.empresa#</cfoutput>";

		popUpWindow<cfoutput>#Attributes.name#</cfoutput>("/cfmx/sif/Utiles/ConlisMBTransaccionesLibros.cfm"+params,250,200,650,400);
	}
	//Obtiene la descripción con base al código
	function TraeMBTransaccionesLibros<cfoutput>#Attributes.name#</cfoutput>(dato) {
		var params ="";
		params = "<cfoutput>&id=#Attributes.id#&tipo=#Attributes.tipo#&BTtce=#Attributes.BTtce#&name=#Attributes.name#&desc=#Attributes.desc#&conexion=#Attributes.conexion#&ecodigo=#Attributes.empresa#</cfoutput>";
		if (dato!="") {
			var fr = document.getElementById("<cfoutput>#Attributes.frame#</cfoutput>");
			fr.src = "/cfmx/sif/Utiles/sifMBTransaccionesLibrosquery.cfm?dato="+dato+"&form="+"<cfoutput>#Attributes.form#</cfoutput>"+params;
		}
		else{
			limpiarMBTransaccionesLibros<cfoutput>#Attributes.name#</cfoutput>();
		}
		return;
	}
	function limpiarMBTransaccionesLibros<cfoutput>#Attributes.name#</cfoutput>(){
	<cfoutput>
	...
	<tr>
		<td>
			...
			<input type="text"
				...
				onblur="javascript: TraeMBTransaccionesLibros#Attributes.name#(document.#Attributes.form#.#Evaluate('Attributes.name')#.value); 
		...
		<td nowrap>
			<input type="text"
				...
				maxlength="80">
			<a href="##" tabindex="-1"><img src="/cfmx/sif/imagenes/Description.gif" alt="Lista Clasificaci&oacute;n Socios de Negocios" name="imagen" width="18" height="14" border="0" align="absmiddle" onclick='javascript: doConlisMBTransaccionesLibros#Attributes.name#();'></a>
		</td>
	</tr>
	</cfoutput>
</table>
