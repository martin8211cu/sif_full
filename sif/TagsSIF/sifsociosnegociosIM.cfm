...
<cfparam name="Attributes.Order" 			default="SNidentificacion" 	type="string"> <!--- Order by --->
<!--- consultas --->
<!--- query --->
<cfif isdefined("Attributes.idquery") and len(trim(Attributes.idquery))>
	...
	<cfquery name="#queryName#" datasource="#Attributes.Conexion#">
		select SNcodigo, SNidentificacion, SNnombre, SNid
		from SNegocios 
		...
		<cfif isdefined("Attributes.SNtiposocio") and len(trim(Attributes.SNtiposocio))>
			...
			<cfelse>
            	...
            	<cfif isdefined("Attributes.Order") and len(trim(Attributes.Order))>
...
function doConlisSocio<cfoutput>#Attributes.SNcodigo#</cfoutput>() {
	<cfif Attributes.modificable>
			...
			var params ="";
			params = "<cfoutput>?formulario=#Attributes.form#&id=#Attributes.SNcodigo#&codigo=#Attributes.SNidentificacion#&desc=#Attributes.SNnombre#&tipo=#Attributes.SNtiposocio#&Ecodigo=#Attributes.Ecodigo#</cfoutput>";
			...
			<cfif Len(Trim("<cfoutput>#Attributes.Order#</cfoutput>")) GT 0> 
				params = params + "<cfoutput>&Order=#Attributes.Order#</cfoutput>";
			</cfif>	
		
			popUpWindow<cfoutput>#Attributes.SNcodigo#</cfoutput>("/cfmx/sif/Utiles/ConlisSociosNegociosIM.cfm"+params,250,200,650,400);
	</cfif>
}
...
function TraeSocio<cfoutput>#Attributes.SNcodigo#</cfoutput>(codigo) {
	<cfif Attributes.modificable>
			var params ="";
			params = "<cfoutput>&id=#Attributes.SNcodigo#&desc=#Attributes.SNnombre#&numero=#Attributes.SNidentificacion#&tipo=#Attributes.SNtiposocio#&Ecodigo=#Attributes.Ecodigo#</cfoutput>";
			...
			fr.src = "/cfmx/sif/Utiles/sifsocionegociosqueryIM.cfm?codigo="+codigo+"&formulario="+'<cfoutput>#Attributes.form#</cfoutput>'+params;
			}
			else{
				...
				<cfoutput>if (window.func#Attributes.SNidentificacion#) {window.func#Attributes.SNidentificacion#()}</cfoutput>
			}
			return;
	</cfif>
}
  ...
  <table width="1%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		<td nowrap width="1%">
			<input 
				type="text" 
				name="#Attributes.SNidentificacion#" 
				id="#Attributes.SNidentificacion#" 
				maxlength="15" 
				size="10" 
				<cfif not Attributes.modificable>
					disabled
				</cfif>
				onblur="javascript:TraeSocio#Attributes.SNcodigo#(document.#Attributes.form#.#Attributes.SNidentificacion#.value); #Attributes.FuncJSalModificar#;" 
			...
			value="<cfif isdefined("Attributes.idquery") and len(trim(Attributes.idquery)) >#Trim(Evaluate(queryName).SNidentificacion)#</cfif>">
		</td>
		<td nowrap width="1%">
