...
<cfparam name="Attributes.path" 		type="String" default="rh/indicadores/rotacion/">
<cfparam name="Attributes.irA" 			type="String">	<!--- si esta definido pinta un boton con un link a otro cfm indicado aqui. Debe tener los parametros incluidos --->
...
<cfset LvarDir	= ExpandPath('')& "/"& Attributes.path><!---GetDirectoryFromPath(GetCurrentTemplatePath())--->
<cfset LvarDir = replace(LvarDir,'\','/', 'ALL')>
...
<!--- PONE UN BOTON PARA MOSTRAR IR A OTRA PANTALLA --->
<cfif isdefined("attributes.irA") and len(trim(attributes.irA))>
	<cf_templatecss>
	<table width="100%" align="center" border="0">
		<tr><td align="center"><input type="button" class="btnNormal" name="Consultar" value="Ver Detalle" onclick="javascript:location.href = '<cfoutput>#trim(attributes.irA)#</cfoutput>';" /></td></tr>
	</table>
</cfif>
...
<script type="text/javascript">
	//AC_FL_RunContent( 'codebase','http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab##version=7,0,19,0',"allowFullScreen", "true", 'width','100%','height','100%','id','#Attributes.name#','src','Rotacion','quality','high','bgcolor','##FFFFFF','name','#Attributes.name#','align','','pluginspage','http://www.macromedia.com/go/getflashplayer','movie','#Attributes.movie#','_xmlurl','#LvarFileName#' ); //end AC code
	AC_FL_RunContent( 'codebase','http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab##version=7,0,19,0','width','100%','height','100%','id','#Attributes.name#','src','Brecha','quality','high','bgcolor','##FFFFFF','name','#Attributes.name#','align','','pluginspage','http://www.macromedia.com/go/getflashplayer','movie','#Attributes.movie#','_xmlurl','#LvarFileName#' ); //end AC code
</script>
<noscript>
