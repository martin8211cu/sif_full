<!--- 
	...
	The general format for using cf_pdfmx is <cf_pdfmx action="action" parameter="paramval" ...>
    The actions outline for a generic 2-page document may be something like:
		...
		A more specific example of usage is as follows...
			...
			<cf_pdfmx action="init" 
			        file="/pathname/pdfout.pdf" 
			        fonts="Arial|Arial,Bold|Arial,Italic">
			<cf_pdfmx action="startpage">
			<cf_pdfmx action="addtext" 
			...
			<cf_pdfmx action="addlink" 
			        ...
			        outlinecolor="0 0 1" 
			        outlinewidth="5">
			<cf_pdfmx action="endpage">
			<cf_pdfmx action="finish">		
        ...
        ADDTEXT        (text="text string" 
				 		x|xinches="pixels(0-612)|inches(0-8.5)" portrait or "pixels(0-792)|inches(0-11)" landscape, 
...
within the PDF specification.	
    ...
    * Useful Notes *
        ...
        - Fontnum refers to the number of the font to be used, as it was given in the fonts=
          line of ACTION="Init".  
            Example: <cf_pdfmx action="init" fonts="TimesNewRoman|Courier|Arial" ...>
                     <cf_pdfmx action="addtext" fontnum="2" ...> refers to Courier
        - Applicable text rendering modes:
            0 : Filled only (default)
            1 : Outlined only
            2 : Filled and outlined
           (there are more that are beyond the scope of cf_pdfmx)
		...
		the pdf is generated. 
			...
			1.  in the tag that calls cf_pdfmx, right before the first call, issue  a 
			...
			4.  when you are done with the structure (after the cf_pdfmx action="finish" call), then issue a
...
<cfif not IsDefined("attributes.action")>
	<em>in cf_pdfmx, no action specified!</em> <!--- must specify action --->
	<cfabort>
</cfif>
...
<cfswitch expression="#attributes.action#">
	 ...
	 <cfcase value="init">
        <cfif not IsDefined("attributes.file")>
            <em>Error: No file given for cf_pdfmx</em> <!--- must specify file --->
            <cfexit>
        </cfif>
        
        <cfif not IsDefined("attributes.fonts")>
            <em>Error: No fonts defined in cf_pdfmx</em> <!--- must have at least one font --->
            <cfexit>
        </cfif>
        
        <cfset pdf=StructNew()>              <!--- entire pdf is stored in a structure --->
        
        <cfset pdf.file=attributes.file>     <!--- filename --->
        <cfset pdf.sections=ArrayNew(1)>     <!--- number of bytes for each obj --->
        <cfset pdf.bytes=0>                  <!--- byte counter during obj construction --->
        <cfset pdf.pagecontents=ArrayNew(1)> <!--- list of all obj's (1 0,2 0,etc..) on page --->
        <cfset pdf.annots=ArrayNew(1)>       <!--- used for hyperlink annotations on endpage --->
		<cfset pdf.curpage=0>                <!--- set on startpage, used in page info arrays --->
		<cfset pdf.fonts=attributes.fonts>   <!--- font list, separate by pipes! --->
		<cfset pdf.streamlen=0>              <!--- used on endpage, length of all text --->
		<cfset pdf.pageappend=''>            <!--- holds annotations for each page --->
		<cfset pdf.hasgraphics=ArrayNew(1)>  <!--- for incomplete ADDIMAGE --->
		<cfset pdf.graphics=ArrayNew(1)>     <!--- for incomplete ADDIMAGE --->
        
        <cffile action="write" file="#pdf.file#" output="" addnewline="no">
        
        <cfparam name="attributes.producer" default="The Producer">
        <cfset creationdate=DateFormat(Now(), "YYYYMMDD") & TimeFormat(Now(), "HHmmss")>
        
		<!--- pdf file marker --->
        <cf_pdfmx action="addtoobject" text="#chr(37)#PDF#chr(45)#1#chr(46)#0">
        
		<!--- producer, creation date obj --->
		<cf_pdfmx action="newobject">
        <cf_pdfmx action="addtoobject" text="<</Producer (#attributes.producer#) /CreationDate (D:#creationdate#) >>">
        <cf_pdfmx action="endobject">
        
		  <!--- obj's for each font --->
        <cfset i=1>
        <cfloop index="font" list="#attributes.fonts#" delimiters="|">
            <cf_pdfmx action="newobject">
            <cf_pdfmx action="addtoobject" text="<</Type /Font">
            <cf_pdfmx action="addtoobject" text="/Name /F#i#">
            <cf_pdfmx action="addtoobject" text="/Subtype /Type1">
            <cf_pdfmx action="addtoobject" text="/BaseFont /#font#">
            <cf_pdfmx action="addtoobject" text="/Encoding /PDFDocEncoding">
            <cf_pdfmx action="addtoobject" text=">>">
            <cf_pdfmx action="endobject">
            <cfset i = i + 1>
        </cfloop>
		  ...
		  <!--- dummy outline obj --->
        <cf_pdfmx action="newobject">
            <cf_pdfmx action="addtoobject" text="<</Type /Outlines">
            <cf_pdfmx action="addtoobject" text="/Count 0">
            <cf_pdfmx action="addtoobject" text=">>">
        <cf_pdfmx action="endobject">
        <cfset pdf.outlines=ArrayLen(pdf.sections)>
        
        <!--- build Font Resource Dictionary object for indirect references --->
        <cf_pdfmx action="newobject">
	        <cf_pdfmx action="addtoobject" text="<<">
	        <cf_pdfmx action="addtoobject" text="/ProcSet [/PDF /Text]">
	        <cf_pdfmx action="addtoobject" text="/Font">
	        <cf_pdfmx action="addtoobject" text="<<">
	            
	        <cfset a=1>
	        <cfloop index="font" list="#attributes.fonts#" delimiters="|">
	            <cfset next_a = a + 1>
	            <cf_pdfmx action="addtoobject" text="/F#a# #next_a# 0 R">
	            <cfset a = a + 1>
	        </cfloop>
	        <cf_pdfmx action="addtoobject" text=">>">
	        <cf_pdfmx action="addtoobject" text=">>">
        ...
        <cfset pdf.resources=ArrayLen(pdf.sections)>
    </cfcase>
	 
	 <!--- done with previous obj, so update pdf.sections, then start new obj --->
    <cfcase value="newobject">
        <cfscript>ArrayAppend(pdf.sections, pdf.bytes);</cfscript>
        <cf_pdfmx action="addtoobject" text="#ArrayLen(pdf.sections)# 0 obj">
    </cfcase>
	 
	 <!--- keep pdf.bytes current, file appendage --->
    <cfcase value="addtoobject">
        <cfset pdf.bytes = pdf.bytes + Len(attributes.text) + 2>
        <cffile action="append" file="#pdf.file#" output="#attributes.text#" addnewline="yes">
    </cfcase>
	 
	 <!--- add endobj --->
    <cfcase value="endobject">
        <cf_pdfmx action="addtoobject" text="endobj">
    </cfcase>
	 
	 <!--- put font number and size into stream --->
    <cfcase value="setfont">
        <cfif IsDefined("attributes.num") and IsDefined("attributes.size")>
            <cf_pdfmx action="addtostream" text="/F#attributes.num# #attributes.size# Tf">
        </cfif>
    </cfcase>
	 ...
	 <cfcase value="startpage">
        <cf_pdfmx action="newobject">      
        <cfset nextsect=ArrayLen(pdf.sections) + 1>
        <cf_pdfmx action="addtoobject" text="<</Length #nextsect# 0 R >>">
        <cf_pdfmx action="addtoobject" text="stream">
        <cfset pdf.streamlen=0>
        <cfset pdf.curpage=pdf.curpage + 1>
        <cfset pdf.pagecontents[pdf.curpage]="#ArrayLen(pdf.sections)# 0 R">
        <cfset pdf.pageappend=''>
        <cfset pdf.annots[pdf.curpage]=''>
		<cfset pdf.hasgraphics[pdf.curpage]=0>
    </cfcase>
    
	 <!--- output endstream, length, and annotations for page --->
	 <cfcase value="endpage">
        <cf_pdfmx action="addtoobject" text="endstream">
        <cf_pdfmx action="endobject">
        
        <cf_pdfmx action="newobject">
            <cf_pdfmx action="addtoobject" text="#numberformat(pdf.streamlen,"0000000000")#">
        <cf_pdfmx action="endobject">

        <cfif pdf.pageappend is not ''>
            
				<cfset appendstring = pdf.pageappend>
				...
				<cfloop condition="nomoreobjects EQ false">
					...
					<cfif nomoreobjects EQ false>
						<cf_pdfmx action="newobject">
						   <cf_pdfmx action="addtoobject" text="#objectstring#">
	            	<cf_pdfmx action="endobject">
						<cfset appendstring = mid(appendstring,endobjposition + 1,len(appendstring))>
						<cfset pdf.annots[pdf.curpage]=pdf.annots[pdf.curpage]&' #ArrayLen(pdf.sections)# 0 R'>
					</cfif>	
				</cfloop>	            
    ...
    <cfcase value="addtostream">
        <cf_pdfmx action="addtoobject" text="#attributes.text#">
        <cfset pdf.streamlen = pdf.streamlen + Len(attributes.text) + 2>
    </cfcase>
	 
	 <!--- finish document --->
    <cfcase value="finish">
        <cfparam name="attributes.pageorientation" default="portrait">
				<cfset pdf.pageorient = attributes.pageorientation>
        <cf_pdfmx action="writepages">
        <cf_pdfmx action="writecatalog">
        <cf_pdfmx action="writecrossref">
        <cf_pdfmx action="writetrailer">
    </cfcase>
	 
	 <!--- output all page-related obj's --->
    <cfcase value="writepages">
        <cfset masterpages=ArrayLen(pdf.pagecontents) + ArrayLen(pdf.sections) + ArrayLen(pdf.graphics) + 1>
        <cfset startpages=ArrayLen(pdf.sections) + ArrayLen(pdf.graphics) + 1>

		<!--- create any necessary graphics objects --->
		<cfloop index="i" from="1" to="#ArrayLen(pdf.graphics)#">
			<cf_pdfmx action="newobject">
				<cf_pdfmx action="addtoobject" text="<<">
				<cf_pdfmx action="addtoobject" text="/Type /XObject">
				<cf_pdfmx action="addtoobject" text="/Subtype /Image">
				<cf_pdfmx action="addtoobject" text="/Width #pdf.graphics[i].width#">
				<cf_pdfmx action="addtoobject" text="/Height #pdf.graphics[i].height#">
				<cf_pdfmx action="addtoobject" text="/BitsPerComponent 8"> 
				<cf_pdfmx action="addtoobject" text="/ColorSpace /DeviceRGB"> 
				<cf_pdfmx action="addtoobject" text="/Length #len(pdf.graphics[i].imagefile)#"> 
				<cf_pdfmx action="addtoobject" text="/Filter DCTDecode">
				<!---	<cf_pdfmx action="addtoobject" text="/F << /FS /URL /F (#pdf.graphics[i].filename#) >> >>"> --->
				<cf_pdfmx action="addtoobject" text="stream">
				<cf_pdfmx action="addtoobject" text="#pdf.graphics[i].imagefile#">
				<cf_pdfmx action="addtoobject" text="endstream">
			<cf_pdfmx action="endobject">
			<cfset pdf.graphics[i].sect = ArrayLen(pdf.sections)>
		</cfloop>
        
        <cfloop index="i" from="1" to="#ArrayLen(pdf.pagecontents)#">
            <cf_pdfmx action="newobject">
                <cf_pdfmx action="addtoobject" text="<</Type /Page">    
                <cf_pdfmx action="addtoobject" text="/Parent #masterpages# 0 R">
				
				<!--- if this page has graphics, build the resource dictionary by hand 
				<cfif pdf.hasgraphics[i] is 1>
		         <cf_pdfmx action="addtoobject" text="/Resources <<">
        		   <cf_pdfmx action="addtoobject" text="/ProcSet [/PDF /Text /ImageB]">
					
					<cfset graphlist=' '>
					<cfloop from="1" to="#ArrayLen(pdf.graphics)#" index="a">
						<cfif pdf.graphics[a].page is i>
							<cfset graphlist=graphlist & pdf.graphics[a].pdfname & " ">
							<cfset graphlist=graphlist & pdf.graphics[a].sect>
							<cfset graphlist=graphlist & " 0 R ">
						</cfif>
					</cfloop>
					<cf_pdfmx action="addtoobject" text="/XObject <<#graphlist#>>">

		         <cf_pdfmx action="addtoobject" text="/Font <<">
                
		          <cfset a=1>
        		    <cfloop index="font" list="#pdf.fonts#" delimiters="|">
		            <cfset next_a = a + 1>
                  <cf_pdfmx action="addtoobject" text="/F#a# #next_a# 0 R">
        		      <cfset a = a + 1>
		          </cfloop>
				...
				<cfelse> --->
	             ...
	             <cf_pdfmx action="addtoobject" text="/Resources #pdf.resources# 0 R">
				<!--- </cfif> --->
				<cfif pdf.pageorient eq 'landscape'> <!--- 8 1/2 x 11 --->
					 <cf_pdfmx action="addtoobject" text="/MediaBox [0 0 612 792]">
				<cfelseif pdf.pageorient eq 'landscape'> <!--- 11 x 8/12 --->				 	 
					 <cf_pdfmx action="addtoobject" text="/MediaBox [0 0 792 612]">
				<cfelseif pdf.pageorient eq 'legal'>  <!--- 11 x 17 --->	 
					 <cf_pdfmx action="addtoobject" text="/MediaBox [0 0 792 1224]">
				</cfif>
            <cf_pdfmx action="addtoobject" text="/Contents [#pdf.pagecontents[i]#]">
            <cfif pdf.annots[i] is not ''>
                <cf_pdfmx action="addtoobject" text="/Annots [#trim(pdf.annots[i])#]">
            </cfif>
            <cf_pdfmx action="addtoobject" text=">>">
            <cf_pdfmx action="endobject">
        </cfloop>
        ...
        <cf_pdfmx action="newobject">
        <cf_pdfmx action="addtoobject" text="<</Type /Pages">
        <cf_pdfmx action="addtoobject" text="/Count #ArrayLen(pdf.pagecontents)#">
            
        <cfset kids="[">
		  <cfset sections=ArrayLen(pdf.sections) - 1>
        <cfloop index="i" from="#startpages#" to="#sections#">
            <cfset kids = kids & " #i# 0 R">
        </cfloop>
            
        <cf_pdfmx action="addtoobject" text="/Kids #kids# ]">
        <cf_pdfmx action="addtoobject" text=">>">
        <cf_pdfmx action="endobject">
    </cfcase>
    ...
    <cfcase value="writecrossref">
        <cfset pdf.xrefstart=pdf.bytes>
        <cf_pdfmx action="addtoobject" text="xref">
        <cfset sects=ArrayLen(pdf.sections) + 1>
        <cf_pdfmx action="addtoobject" text="0 #sects#">
        <cf_pdfmx action="addtoobject" text="0000000000 65535 f">
        
        <cfloop from="1" to="#ArrayLen(pdf.sections)#" index="i">
            <cfset num=NumberFormat(pdf.sections[i], "0000000000")>
            <cf_pdfmx action="addtoobject" text="#num# 00000 n">
        </cfloop>
    </cfcase>
    ...
    <cfcase value="writecatalog">
        <cf_pdfmx action="newobject">
            <cf_pdfmx action="addtoobject" text="<</Type /Catalog">
            <cfset sects=ArrayLen(pdf.sections)-1>
            <cf_pdfmx action="addtoobject" text="/Pages #sects# 0 R">
            <cf_pdfmx action="addtoobject" text="/Outlines #pdf.outlines# 0 R">
            <cf_pdfmx action="addtoobject" text=">>">
        <cf_pdfmx action="endobject">
    </cfcase>
    ...
    <cfcase value="writetrailer">
        <cf_pdfmx action="addtoobject" text="trailer">
        <cf_pdfmx action="addtoobject" text="<<">
        <cfset sects=ArrayLen(pdf.sections)+1>
        <cf_pdfmx action="addtoobject" text="/Size #sects#">
        <cf_pdfmx action="addtoobject" text="/Root #ArrayLen(pdf.sections)# 0 R">
        <cf_pdfmx action="addtoobject" text="/Info 1 0 R">
        <cf_pdfmx action="addtoobject" text=">>">
        <cf_pdfmx action="addtoobject" text="startxref">
        <cf_pdfmx action="addtoobject" text="#numberformat(pdf.xrefstart,"0000000000")#">
        <cf_pdfmx action="addtoobject" text="#chr(37)##chr(37)#EOF">
    </cfcase>
    ...
    <cfcase value="addtext">
        ...
        <cfelse>
            ...
            <cfelse>
                <em>In cf_pdfmx:AddText, x or xInches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cfelse>
            ...
            <cfelse>
                <em>In cf_pdfmx:AddText, y or yInches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cfset attributes.text=Replace(attributes.text, "(", "\(", "ALL")>
        <cfset attributes.text=Replace(attributes.text, ")", "\)", "ALL")>
        <cf_pdfmx action="addtostream" text="BT #attributes.outlinecolor# RG #attributes.fillcolor# rg #attributes.width# w /F#val(attributes.fontnum)# #attributes.fontsize# Tf #attributes.spacing# Tc #attributes.horizScale# Tz #attributes.rendermode# Tr #x# #y# Td (#attributes.text#) Tj ET">
    </cfcase>
    ...
    <cfcase value="drawline">
        ...
        <cfelse>
            ...
            <cfelse>
                <em>In cf_pdfmx:DrawLine, x1 or x1Inches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cfelse>
            ...
            <cfelse>
                <em>In cf_pdfmx:DrawLine, y1 or y1Inches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cfelse>
            ...
            <cfelse>
                <em>In cf_pdfmx:DrawLine, x2 or x2Inches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cfelse>
            ...
            <cfelse>
                <em>In cf_pdfmx:DrawLine, y2 or y2Inches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cf_pdfmx action="addtostream" text="#attributes.width# w">
        <cf_pdfmx action="addtostream" text="[#attributes.dashpattern#] 0 d">
        <cf_pdfmx action="addtostream" text="#x1# #y1# m">
        <cf_pdfmx action="addtostream" text="#x2# #y2# l">
        <cf_pdfmx action="addtostream" text="#attributes.color# RG">
        <cf_pdfmx action="addtostream" text="#attributes.linecap# J">
        <cf_pdfmx action="addtostream" text="S">
    </cfcase>
    ...
    <cfcase value="DrawRect">
        ...
        <cfelse>
            <cfif IsDefined("attributes.x1")>
                <cfset x1=attributes.x1>
            <cfelse>
                <em>In cf_pdfmx:DrawRect, x1 or x1Inches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cfelse>
            <cfif IsDefined("attributes.y1")>
                <cfset y1=attributes.y1>
            <cfelse>
                <em>In cf_pdfmx:DrawRect, y1 or y1Inches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cfelse>
            <cfif IsDefined("attributes.x2")>
                <cfset x2=attributes.x2>
            <cfelse>
                <em>In cf_pdfmx:DrawRect, x2 or x2Inches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cfelse>
            <cfif IsDefined("attributes.y2")>
                <cfset y2=attributes.y2>
            <cfelse>
                <em>In cf_pdfmx:DrawRect, y2 or y2Inches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cf_pdfmx action="addtostream" text="#x1# #y1# #width# #height# re">
        <cf_pdfmx action="addtostream" text="#attributes.width# w">
        <cf_pdfmx action="addtostream" text="#attributes.linecap# J">
        <cf_pdfmx action="addtostream" text="#attributes.outlinecolor# RG">
        <cf_pdfmx action="addtostream" text="#attributes.fillcolor# rg">
        <cf_pdfmx action="addtostream" text="B">
    </cfcase>
    ...
    <cfcase value="addlink">
        ...
        <cfelse>
            <cfif IsDefined("attributes.x")>
                <cfset x=attributes.x>
            <cfelse>
                <em>In cf_pdfmx:AddLink, x or xInches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cfelse>
            <cfif IsDefined("attributes.y")>
                <cfset y=attributes.y>
            <cfelse>
                <em>In cf_pdfmx:AddLink, y or yInches must be specified</em><br>
                <cfabort>
            </cfif>
        </cfif>
        ...
        <cf_pdfmx action="addtext" fontnum="#attributes.fontnum#" fontsize="#attributes.fontsize#"
        ...
        <cfset pdf.pageappend=pdf.pageappend & "<</Type /Annot /Subtype /Link /Rect [#llx# #lly# #urx# #ury#] /Border [ 0 0 #attributes.border# ] /A << /Type /Action /S /URI /URI (#attributes.href#) >> >>">
    </cfcase>
    ...
    <cfcase value="startparagraph">
        ...
        <cf_pdfmx action="addtostream" text="BT">
        <cf_pdfmx action="addtostream" text="#attributes.outlinecolor# RG">
        <cf_pdfmx action="addtostream" text="#attributes.fillcolor# rg">
        <cf_pdfmx action="addtostream" text="#attributes.width# w">
        <cf_pdfmx action="addtostream" text="/F#val(attributes.fontnum)# #attributes.fontsize# Tf">
        <cf_pdfmx action="addtostream" text="#attributes.spacing# Tc">
        <cf_pdfmx action="addtostream" text="#attributes.horizScale# Tz">
        <cf_pdfmx action="addtostream" text="#attributes.rendermode# Tr #attributes.leading# TL">
        <cf_pdfmx action="addtostream" text="#x# #y# Td">
    </cfcase>
    <cfcase value="endparagraph">
        <cf_pdfmx action="addtostream" text="ET">
    </cfcase>
    <cfcase value="addtoparagraph">
        <cfif isdefined("attributes.text")>
            ...
            <cfloop list="#attributes.text#" index="line" delimiters="#chr(10)#">
                <cf_pdfmx action="addtostream" text="(#trim(line)#) '">
            </cfloop>
        </cfif>
    </cfcase>
		...
		<cfif IsDefined("attributes.filename") and IsDefined("attributes.width") and IsDefined("attributes.height")>
    	    ...
    	    <cfelse>
	       	    <cfif IsDefined("attributes.x")>
    	            <cfset x=attributes.x>
	            <cfelse>
            	    <em>In cf_pdfmx:AddImage, x or xInches must be specified</em><br>
        	        <cfabort>
    	        </cfif>
	        </cfif>
	        ...
	        <cfelse>
        	    <cfif IsDefined("attributes.y")>
    	            <cfset y=attributes.y>
	            <cfelse>
            	    <em>In cf_pdfmx:AddImage, y or yInches must be specified</em><br>
        	        <cfabort>
    	        </cfif>
	        </cfif>

			<cfset i=ArrayLen(pdf.graphics) + 1>
			<cfset pdf.graphics[i]=StructNew()>
			<cfset pdf.graphics[i].filename=attributes.filename>
			<cfset pdf.graphics[i].width=attributes.width>
			<cfset pdf.graphics[i].height=attributes.height>
			<cfset pdf.graphics[i].page=pdf.curpage>
			<cfset pdf.graphics[i].pdfname="/Im"&i>
			<cfset pdf.hasgraphics[pdf.curpage]=1>
			
			<cffile action="READ" file="#attributes.filename#" variable="pdf.graphics[i].imagefile">
 			
			<cf_pdfmx action="addtostream" text="q">
			<cf_pdfmx action="addtostream" text="132 0 0 132 #x# #y# cm">
			<cf_pdfmx action="addtostream" text="#pdf.graphics[i].pdfname# Do">
			<cf_pdfmx action="addtostream" text="Q">
 		</cfif>
...
for each array instance of pdf.graphics[i] we need this object	
X 0 obj (where X is the next available object)
<</Type /XObject 
/Subtype /Image 
/Name /#pdf.graphics[i].pdfname# 
/Width #pdf.graphics[i].width# 
...
for each array instance of pdf.graphics, we need a q...Q 
stream...
q
132 0 0 132 #x# #y# cm
/#pdf.graphics[i].pdfname# Do
Q
...endstream	

in the Resource Dictionary object for the page...
for each array instance of pdf.graphics, we need the contents after /XObject
Y 0 obj
<< /ProcSet [/PDF /ImageB]
	/XObject << /#pdf.graphics[i].pdfname# X 0 R >>
				<< /#pdf.graphics[i].pdfname# Z 0 R >>....
>>
endobj
...
...		
	...
	<cfdefaultcase>
        <em>No action="" specified for cf_pdfmx!</em>
        <cfabort>
    </cfdefaultcase>
</cfswitch>
