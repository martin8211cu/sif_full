...
<cfparam name="Attributes.Todas" default="N" type="string"> <!--- Coloca la primera opcion con -- Todas -- --->
<cfparam name="Attributes.habilita" default="S" type="string"> <!--- Habilita o no el tag --->
<cfparam name="Attributes.quitar" default="" type="string"> <!--- elimina la lista de Mcodigos separados por comas -- --->

<cfparam name="Attributes.ExcluirLocal" default="no" type="boolean"> <!--- Excluye la moneda local de la empresa --->
		...
		<cfset vieneFecha = true >
 		<cfquery name="TCsug" datasource="#Attributes.Conexion#">
			...
			where tc.Ecodigo = <cfqueryparam value="#Attributes.Ecodigo#" cfsqltype="cf_sql_integer">
			  and tc.Hfecha  <= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#LSParseDateTime(Attributes.FechaSugTC)#">
			  and tc.Hfechah > <cfqueryparam cfsqltype="cf_sql_timestamp" value="#LSParseDateTime(Attributes.FechaSugTC)#">
		</cfquery>
	</cfif>	
</cfif>

<!--- '#LSDateFormat(Attributes.FechaSugTC,"YYYYMMDD")#' --->
...
<cfif Attributes.Conlis NEQ "S">
	<cf_translatedata name="get" tabla="Monedas" col="Mnombre" conexion="#Attributes.Conexion#" returnvariable="LvarMnombre">
	<cfquery name="rsMonedas" datasource="#Attributes.Conexion#">
		select Mcodigo, #LvarMnombre# as Mnombre, Msimbolo, Miso4217, ts_rversion 
		from Monedas m
		where Ecodigo = <cfqueryparam value="#Attributes.Ecodigo#" cfsqltype="cf_sql_integer">
		<cfif Attributes.ExcluirLocal>
		   	and not exists(select 1 from Empresas where Mcodigo = m.Mcodigo)
		</cfif>
		<cfif isdefined('Attributes.quitar') and len(trim(Attributes.quitar))>
			and Mcodigo not in (#Attributes.quitar#)
		</cfif>

		order by #LvarMnombre#
	</cfquery>
...
function doConlis<cfoutput>#Attributes.Mcodigo#</cfoutput>() {
	...
	<cfif isdefined('Attributes.quitar') and len(trim(Attributes.quitar))>
		params = params + "&quitar=<cfoutput>#Attributes.quitar#</cfoutput>";
	</cfif>
	...
	<cfif Attributes.Conlis NEQ "S">
		<tr>		
			...
			<select name="#Attributes.Mcodigo#" 
				<cfif Attributes.habilita NEQ 'S'>
					disabled
				</cfif>
			 ...
			 <cfelse>
				...
				>
					...
					<!--- Muestra la primera opcion con value -1 --->
					<cfif Attributes.Todas EQ 'S'>
						<option value="-1" <cfif isdefined("Attributes.value") and Attributes.value EQ "-1"> selected</cfif>>-- <cf_translate  key="LB_Todas" xmlFile="/rh/generales.xml">Todas</cf_translate> --</option>
					</cfif> 
                    
					<cfoutput query="rsMonedas"> 
					  <option value="#rsMonedas.Mcodigo#"
					   <cfif isdefined("Attributes.value") and Attributes.value NEQ "">
					   		<cfif trim(rsMonedas.Mcodigo) EQ trim(Attributes.value)>selected</cfif>
 					   <cfelseif isdefined('Attributes.query') and ListLen('Attributes.query.columnList') GT 0 and len(Attributes.query.Mcodigo) and Attributes.query.Mcodigo>
					  		<cfif rsMonedas.Mcodigo EQ Attributes.query.Mcodigo >selected</cfif>
					...
					<cfif Attributes.CrearMoneda >
					  <option value="">-------------------</option>
					  <option value="0"><cf_translate  key="LB_CrearNueva" xmlFile="/rh/generales.xml">Crear Nueva</cf_translate> ...</option>											 				
					</cfif>
				</select>
			</td>
			<td nowrap>&nbsp;</td>
  	...
  	<cfelse>
		<cfoutput>
		<td nowrap>			
			<input type="text" name="#Attributes.Miso4217#"
				<cfif Attributes.habilita NEQ 'S'>
					readonly="true"
				</cfif>
			maxlength="3" size="3" 
			<cfif Attributes.habilita EQ 'S'>
				onBlur="javascript:UpperCase(this);TraeMoneda#Attributes.Mcodigo#(document.#Attributes.form#.#Evaluate('Attributes.Miso4217')#.value);"
			</cfif>
		...
		<td nowrap>
			...
			<cfif Attributes.habilita NEQ 'S'>
				<img src="/cfmx/sif/imagenes/Description.gif" alt="Lista de Monedas" name="imagen" width="18" height="14" border="0" align="absmiddle">
			<cfelse>
				<a href="##" tabindex="-1">
					<img src="/cfmx/sif/imagenes/Description.gif" alt="Lista de Monedas" name="imagen" width="18" height="14" border="0" align="absmiddle" onClick='javascript:doConlis#Attributes.Mcodigo#();'>
				</a>			
			</cfif>
		</td>		
