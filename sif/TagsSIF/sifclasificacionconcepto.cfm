...
<cfparam name="Attributes.id" default="CCid" type="string"> <!--- Nombre del id --->
<cfparam name="Attributes.name" default="CCcodigo" type="string"> <!--- Nombre del Código --->
<cfparam name="Attributes.nivel" default="CCnivel" type="string"> <!--- Nombre del Nivel --->
<cfparam name="Attributes.frame" default="frclasificacion" type="string"> <!--- Nombre del frame --->
<cfparam name="Attributes.desc" default="CCdescripcion" type="string"> <!--- Nombre de la Descripción --->
...
<script language="JavaScript" type="text/javascript">
	...
	function conlis_keyup_<cfoutput>#Attributes.name#</cfoutput>(e) {
		...
		if (keycode == 113) {<!--- El código 113 corresponde a la tecla F2 --->
			doConlisClasificacionConcepto<cfoutput>#Attributes.name#</cfoutput>();
		}
	}
	...
	function doConlisClasificacionConcepto<cfoutput>#Attributes.name#</cfoutput>() {
		var params ="";
		params = "<cfoutput>?formulario=#Attributes.form#&id=#Attributes.id#&name=#Attributes.name#&nivel=#Attributes.nivel#&desc=#Attributes.desc#</cfoutput>";
		popUpWindow<cfoutput>#Attributes.name#</cfoutput>("/cfmx/sif/Utiles/ConlisClasificacionConcepto.cfm"+params,250,200,650,400);
	}
	...
	function TraeClasificacionConcepto<cfoutput>#Attributes.name#</cfoutput>(dato) {
		...
		if (dato!="") {
			...
			fr.src = "/cfmx/sif/Utiles/sifClasificacionConceptoquery.cfm?dato="+dato+"&formulario="+"<cfoutput>#Attributes.form#</cfoutput>"+params;
		}
		else{
