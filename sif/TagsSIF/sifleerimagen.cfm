...
<cfparam name="Attributes.ruta" default="/cfmx/sif/Utiles/sifleerimagencont.cfm" type="string">
<!--- OPARRALES 2018-05-22 clases css o Bootstrap (img-circle) --->
<cfparam name="Attributes.clases" default="" type="string">
...
<cfquery name="rs_Imagen" datasource="#Attributes.Conexion#">
	...
	<cfif IsImageFile("#gettempdirectory()##codigo#.#iif(ucase(ts) EQ 'GIF',de('gif'),de('jpg'))#")>
		<cfset esImagen = true>
	<cfelse>
		<cfset esImagen = false>
	</cfif>
	...
	<cfif Attributes.desplegar>
		<cfif esImagen>
			...
			<!--- OPARRALES 2018-05-22 Se agrego clase Bootstrap para redondear la imagen --->
			<img class="imgUsuario #Attributes.clases#"
		...
		<cfelseif Attributes.tabla NEQ 'Bancos'><!--- Caso en que se llama desde el mantenimiento de bancos, si no existe imagen no debe presentar ninguna imagen --->
			<cfoutput>
			<img
				name="#Attributes.imgname#"
				<!--- OPARRALES 2018-05-22 Se cambia la imagen a mostrar por default --->
				<!--- src="/cfmx/rh/imagenes/not_avail.gif" --->
				src="/cfmx/home/public/foto_Empleado.cfm?DEid=0&ts=0"
				<cfif Attributes.width NEQ -1> width="#Attributes.width#"</cfif><cfif Attributes.height NEQ -1> height="#Attributes.height#"</cfif>
				<cfif attributes.border>border="1" bodercolor="#Attributes.bordercolor#"<cfelse>border="0"</cfif>>
			</cfoutput>
		</cfif>
