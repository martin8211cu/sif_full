...
<cfparam name="Attributes.id" default="GECVid" type="string"> <!--- Nombre del id --->
<cfparam name="Attributes.name" default="GECVcodigo" type="string"> <!--- Nombre del Código --->
<cfparam name="Attributes.nivel" default="GECVnivel" type="string"> <!--- Nombre del Nivel --->
<cfparam name="Attributes.frame" default="frclasificacion" type="string"> <!--- Nombre del frame --->
<cfparam name="Attributes.desc" default="GECVdescripcion" type="string"> <!--- Nombre de la Descripción --->
...
<cfparam name="Attributes.tipoV" default="0" type="string"> <!--- 1 para interior y 2 para exterior --->
...
<script language="JavaScript" type="text/javascript">
	...
	function conlis_keyup_<cfoutput>#Attributes.name#</cfoutput>(e) {
		...
		if (keycode == 113) {<!--- El código 113 corresponde a la tecla F2 --->
			doConlisClasificacionViatico<cfoutput>#Attributes.name#</cfoutput>();
		}
	}

	
	//Llama el conlis
	function doConlisClasificacionViatico<cfoutput>#Attributes.name#</cfoutput>() {
		var params ="";
		params = "<cfoutput>?formulario=#Attributes.form#&id=#Attributes.id#&name=#Attributes.name#&nivel=#Attributes.nivel#&desc=#Attributes.desc#&tipoV=#Attributes.tipoV#</cfoutput>";
		popUpWindow<cfoutput>#Attributes.name#</cfoutput>("/cfmx/sif/Utiles/ConlisClasificacionViatico.cfm"+params,250,200,650,400);
	}
	//Obtiene la descripción con base al código
	function TraeClasificacionViatico<cfoutput>#Attributes.name#</cfoutput>(dato) {
		var params ="";
		...
		if (dato!="") {
			...
			fr.src = "/cfmx/sif/Utiles/sifClasificacionViaticoquery.cfm?dato="+dato+"&formulario="+"<cfoutput>#Attributes.form#</cfoutput>"+params;
		}
		else{
...
<!---		<cfset id = "Trim('#Evaluate('Attributes.query.#Evaluate('Attributes.id')#')#')">
		...
		<cfset nivel = "Trim('#Evaluate('Attributes.query.#Evaluate('Attributes.nivel')#')#')">				--->
	...
	<tr>
		<td>
			...
			<input 	type="text"
					...
					onBlur="javascript: TraeClasificacionViatico#Attributes.name#(document.#Attributes.form#.#Evaluate('Attributes.name')#.value);" 
					onFocus="this.select()"
		...
		<td>
			<a href="##" tabindex="-1"><img src="/cfmx/sif/imagenes/Description.gif" alt="Lista de Viaticos" name="imagen" width="18" height="14" border="0" align="absmiddle" onClick='javascript: doConlisClasificacionViatico#Attributes.name#();'></a>
		</td>
	</tr>
	</cfoutput>
</table>
...
<cfoutput>
<iframe style="display:none;" name="#Attributes.frame#" id="#Attributes.frame#" marginheight="0" marginwidth="0" frameborder="0" height="0" width="0" ></iframe>
</cfoutput>