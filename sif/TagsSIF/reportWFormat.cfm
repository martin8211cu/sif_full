...
<cfif isdefined('Attributes.regresar') and LEN(TRIM(Attributes.regresar))>
  <cfset LvarRegresar = #Attributes.regresar#>
<cfelse>
   <cfset LvarRegresar = 'javascript:history.go(-1)'>
</cfif>
<cfoutput><form action="#LvarRegresar#" method="get" name="form1" style="margin:0">
...
<iframe
	...
	src="/cfmx/sif/Utiles/reportWFormatLayout.cfm?formato=HTML&url=#Attributes.url#&orientacion=#Attributes.orientacion#&pagina=#Attributes.pagina#&#Attributes.params#">
</iframe>
</cfoutput>
<script language="javascript">
	function changeFormat(formato){
		document.getElementById("r").src="/cfmx/sif/Utiles/reportWFormatLayout.cfm?formato="+formato+"&url=<cfoutput>#Attributes.url#</cfoutput>&orientacion=<cfoutput>#Attributes.orientacion#</cfoutput>&pagina=<cfoutput>#Attributes.pagina#</cfoutput>&<cfoutput>#Attributes.params#</cfoutput>";
	}
</script>