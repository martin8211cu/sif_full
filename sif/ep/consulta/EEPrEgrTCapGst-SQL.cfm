...
<cfif isdefined('rsMensaje')>
	<cflocation url="EEPrEgrTCapGst.cfm?rsMensaje=#rsMensaje#">
</cfif>
</cfoutput>
...
<cfinvoke key="MSG_ReporteAnalitico" default="Estado del Ejercicio del Presupuesto de Egresos por Tipo y Objeto de Gasto"	returnvariable="MSG_ReporteAnalitico"	
...
<cfinvoke key="LB_Capitulo" default="TIPO Y OBJETO DEL GASTO"		returnvariable="LB_Capitulo"	component="sif.Componentes.Translate" method="Translate"/>
...
<cfset LvarIrA  = 'EEPrEgrTCapGst.cfm'>
<cfset LvarFile = 'EEPrEgrTCapGst'>
...
<cfquery datasource="#session.dsn#">
	...
	select rep.ID_Estr, 
    epv.ID_Grupo,
	 coalesce(epv.ID_EstrCtaVal, 0) ID_EstrCtaVal,  coalesce(p.PCDcatid, 0),
...
<!---<cfelse>
    <cfquery datasource="#session.DSN#" name="SaldosPos">
        select 
        coalesce(sum(CPCpresup_Anual), 0.00) as IngrEst,
        coalesce(sum(CPCmodificado), 0.00) +  coalesce(sum(CPCmodificacion_Excesos), 0.00) + coalesce(sum(CPCtrasladado), 0.00) as Modific,
        ...
        coalesce(sum(CPCejecutado), 0.00) as IngrDev,
        coalesce(sum(CPCcomprometido), 0.00) + coalesce(sum(CPCejecutado), 0.00) as EgreComp,
        coalesce(sum(CPCejercido), 0.00) as EgreEjer,
        coalesce(sum(CPCpagado), 0.00) as IngrRec
        from #saldoscuenta# s
        inner join #rsEPQuery# r
        on s.PCDcatid = r.PCDcatid
		where r.PCDcatid = 1413 and r.PCDvalorH='001'
    </cfquery>
    <cfquery datasource="#session.DSN#" name="SaldosNeg">
        select 
        coalesce(sum(CPCpresup_Anual), 0.00) as IngrEst,
        coalesce(sum(CPCmodificado), 0.00) +  coalesce(sum(CPCmodificacion_Excesos), 0.00) + coalesce(sum(CPCtrasladado), 0.00) as Modific,
        ...
        inner join #rsEPQuery# r
        on s.PCDcatid = r.PCDcatid
		where r.PCDcatid = 1413 and r.PCDvalorH='002'
    </cfquery>
    
	<cfquery datasource="#session.DSN#">
        update #saldoscuenta#
        set
        EgreApr  = coalesce(#SaldosPos.IngrEst#, 0.00) - coalesce(#SaldosNeg.IngrEst#, 0.00),
<!---        AmpRed   = <cfif #Signo.SaldoInv# eq 1> 0 - coalesce(#Saldos.Modific#, 0.00) <cfelse> coalesce(#Saldos.Modific#, 0.00) </cfif>,
--->        
		AmpRed   = (coalesce(#SaldosPos.IngrMod#, 0.00) - coalesce(#SaldosPos.IngrEst#, 0.00)) - (coalesce(#SaldosNeg.IngrMod#, 0.00)- coalesce(#SaldosNeg.IngrEst#, 0.00)),
        EgreMod  = coalesce(#SaldosPos.IngrMod#, 0.00)  - coalesce(#SaldosNeg.IngrMod#, 0.00),
        EgreComp = coalesce(#SaldosPos.EgreComp#, 0.00) - coalesce(#SaldosNeg.EgreComp#, 0.00),
        EgreDev  = coalesce(#SaldosPos.IngrDev#, 0.00) - coalesce(#SaldosNeg.IngrDev#, 0.00),
        EgreEjer = coalesce(#SaldosPos.EgreEjer#, 0.00) - coalesce(#SaldosNeg.EgreEjer#, 0.00),
        EgrePag  = coalesce(#SaldosPos.IngrRec#, 0.00) - coalesce(#SaldosNeg.IngrRec#, 0.00),
        ModificPer = coalesce(#SaldosPos.ModificPer#, 0.00)- coalesce(#SaldosNeg.ModificPer#, 0.00)
        where #saldoscuenta#.PCDcatid = 1413
    </cfquery>
</cfif>    
--->
</cfloop>
...
<cfquery datasource="#session.DSN#" name="SC">
    select ID_Grupo, EPGcodigo, EPGdescripcion,EPCPcodigoref from dbo.CGGrupoCtasMayor
    where ID_Estr = #rsNumEst.ID_Estr#
    order by ID_Grupo,EPGcodigo
</cfquery>
<cfloop query="SC">    
	<cfquery datasource="#session.DSN#">
        insert into #saldoscuenta#(ID_Estr,Seccion,ID_EstrCtaVal,PCDcatid,CtpGsto,CGEPDescrip,Nota,Mcodigo,EgreApr,AmpRed,EgreMod,EgreComp, 
        EgreDev,EgreEjer,EgrePag,ModificPer)
        
		select ID_Estr,Seccion, 0,0, #SC.EPGcodigo#, '#SC.EPGdescripcion#', '#SC.EPCPcodigoref#', #varMcodigo#,
        	(select coalesce(sum(EgreApr), 0.00) from #saldoscuenta# a where a.Seccion = #SC.ID_Grupo#) as EgreApr,
            (select coalesce(sum(AmpRed), 0.00) from #saldoscuenta# a where a.Seccion = #SC.ID_Grupo#) as AmpRed,
            (select coalesce(sum(EgreMod), 0.00) from #saldoscuenta# a where a.Seccion = #SC.ID_Grupo#) as EgreMod,
            (select coalesce(sum(EgreComp), 0.00) from #saldoscuenta# a where a.Seccion = #SC.ID_Grupo#) as EgreComp,
            (select coalesce(sum(EgreDev), 0.00) from #saldoscuenta# a where a.Seccion = #SC.ID_Grupo#) as EgreDev,
            (select coalesce(sum(EgreEjer), 0.00) from #saldoscuenta# a where a.Seccion = #SC.ID_Grupo#) as EgreEjer,
            (select coalesce(sum(EgrePag), 0.00) from #saldoscuenta# a where a.Seccion = #SC.ID_Grupo#) as EgrePag,
            (select coalesce(sum(ModificPer), 0.00) from #saldoscuenta# a where a.Seccion = #SC.ID_Grupo#) as ModificPer       
        from #saldoscuenta# 
        where #saldoscuenta#.Seccion = #SC.ID_Grupo#
        group by ID_Estr,Seccion
    </cfquery>
</cfloop>
<!---<cf_dumptofile select="select * from #saldoscuenta# order by Seccion">--->

<cfquery name="rsDetalle" datasource="#Session.DSN#">
	select Seccion,CtpGsto,CGEPDescrip,Nota,
...
--->    
   ...
   <!--- (sum(ModificPer)-sum(EgreDev))/#varUnidad#  as EgreSD,--->
    ...
    group by Seccion,CtpGsto,CGEPDescrip,Nota
    order by Seccion,CtpGsto
</cfquery>
...
--->    
    <cfoutput>
    <cfreport format="#form.formato#" template= "EEPrEgrTCapGst.cfr" query="rsDetalle">
        ...
        <cfreportparam name="TotEgreApr"  		value="#rsTotales.EgreApr#">
        <cfreportparam name="TotAmpRed"  		value="#rsTotales.AmpRed#">
        <cfreportparam name="TotEgreMod"  		value="#rsTotales.EgreMod#">
        <cfreportparam name="TotEgreComp"  		value="#rsTotales.EgreComp#">
        <cfreportparam name="TotDispComp"  		value="#rsTotales.DispComp#">
        <cfreportparam name="TotEgreDev"  		value="#rsTotales.EgreDev#">
        <cfreportparam name="TotCompND"  		value="#rsTotales.CompND#">     
        <cfreportparam name="TotEgreSD"  		value="#rsTotales.EgreSD#">        
        <cfreportparam name="TotEgreEjer"  		value="#rsTotales.EgreEjer#">        
        <cfreportparam name="TotEgrePag"  		value="#rsTotales.EgrePag#">        
        <cfreportparam name="TotCuentasPP"  	value="#rsTotales.CuentasPP#"> 
    ...
    <cfoutput>
		...
		<table width="100%">
            ...
            <cfloop query="rsDetalle">
            	<cfif varSeccion neq rsDetalle.Seccion>
					<cfset varSeccion      	= #rsDetalle.Seccion#>
				...
				<tr>
                    ...
                    <cfif rsDetalle.CtpGsto neq '6000'>
