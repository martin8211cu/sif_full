...
<cfinvoke key="MSG_SitucionFDet"    default="Flujo de Efectivo por Actividades de Operaci&oacute;n"	returnvariable="MSG_SitucionFDet" component="sif.Componentes.Translate" method="Translate"/>
...
<cfset LvarIrA = 'RepFlujoEfectivoActOp.cfm'>
<cfset LvarFile = 'RepFlujoEfectivoActOp'>
...
<cfif isdefined("form.botonsel") and len(trim(form.botonsel))>
    <cfoutput>
        ...
        <cfif isdefined('rsMensaje')>
            <cflocation url="RepFlujoEfectivoActOp.cfm?rsMensaje=#rsMensaje#">
        </cfif>
    ...
    <cfset finLoop = form.periodo/form.periodo>
...
<cfquery name="rsUtilP" datasource="#session.dsn#">
	...
	where upper(EPCPcodigoref) = upper('F')
</cfquery>
<cfset ordenU = "#rsUtilP.Orden#/F">
...
<cf_dump var="#rsActivos#"> --->
    ...
    <cfloop from="#iniLoop#" to="#finLoop#" index="mesfin">
        ...
        <!--- <cfswitch expression="#mesfin#">
            <cfcase value="1"><cfset cheque=form.ChequeI1></cfcase>
            <cfcase value="2"><cfset cheque=form.ChequeI2></cfcase>
            <cfdefaultcase>
                <cfset cheque=form.ChequeI1>
            </cfdefaultcase>
        </cfswitch>
		...
		<cfquery name="insUtilP" datasource="#session.dsn#">
			...
			(Ccuenta,CPCpagado)
			select -100, sum(ChequeTranFin-ChequeTranIni)
		...
		<cfquery name="insUtilP" datasource="#session.dsn#">
			insert into #rsSaldoMes#
			(Ccuenta,CPCpagado)
			select -200, sum(FluctuacionCambiaria)
			from CGEstrProgFESaldos
			where Ecodigo = #session.Ecodigo#
				and EPSImes <= #form.mes#
				and EPSIano = #mPeriodo#
...
<!--- <cfquery name="insUtilP" datasource="#session.dsn#">
			select * from  #rsSaldoMes#
		</cfquery>
<cf_dump var="#insUtilP#"> --->
        <cfset arrayappend(qMeses ,#rsSaldoMes#)>
        <cfset menosPeriodo = 1>
    </cfloop>
    ...
    <td nowrap width="70%" align="left"><strong>#LB_Concepto#</strong></td>
        ...
        <cfloop query="rsActivos">
            ...
            <cfif #NIVEL# LTE 4 and #EPCPcodigoref# EQ 'N'>	<cfset pintaL3 = false> </cfif>
...
<!--- <tr>
 ...
 <td nowrap align="right"><strong>#LSNumberFormat(rsTPasivos.Saldo, ',9.00')#</strong></td>
    ...
    <cfelse>
    <cfoutput>
        <form action="RepFlujoEfectivoActOp.cfm" method="post" name="sql">
