...
<cfif isdefined('rsMensaje')>
	<cflocation url="EEPrEgrCapGst.cfm?rsMensaje=#rsMensaje#">
</cfif>
</cfoutput>
...
<cfinvoke key="LB_Capitulo" default="CAPÍTULO DEL GASTO"		returnvariable="LB_Capitulo"	component="sif.Componentes.Translate" method="Translate"/>
...
<cfset LvarIrA  = 'EEPrEgrCapGst.cfm'>
<cfset LvarFile = 'EEPrEgrCapGst'>
...
<cfquery datasource="#session.dsn#">
	...
	select rep.ID_Estr, 
    epv.ID_Grupo,
	 coalesce(epv.ID_EstrCtaVal, 0) as ID_EstrCtaVal, coalesce(p.PCDcatid, 0),
...
<cfquery name="rsNumEst" datasource="#session.dsn#">
 ...
 <cfinvoke returnvariable="rsEPQuery" component="sif.ep.componentes.EP_EstrSaldosPr" method="CG_EstructuraSaldo">
   ...
   <!--- <cfinvokeargument name="LB_NotaPie" 	value="#LB_NotaPie#">--->  
    <cfinvokeargument name="GvarConexion" 	value="#session.dsn#">    
</cfinvoke>

<!---<cf_dumptofile select="select * from #rsEPQuery# order by ID_GpoCat,ID_EstrCtaVal,PCDvalor">--->
...
--->    
    ...
    sum(ModificPer)/#varUnidad# as ModificPer,
...
<cfif form.formato NEQ "HTML">
    ...
    <cfreport format="#form.formato#" template= "EEPrEgrCapGst.cfr" query="rsDetalle">
