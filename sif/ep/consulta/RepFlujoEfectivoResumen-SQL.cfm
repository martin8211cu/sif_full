...
<cfinvoke key="MSG_SitucionFDet"    default="Flujo de Efectivo Resumen"	returnvariable="MSG_SitucionFDet" component="sif.Componentes.Translate" method="Translate"/>
...
<!---
<cfif isdefined("url.chkCeros") and not isdefined("form.chkCeros")>
...
<cfset LvarIrA = 'RepFlujoEfectivoResumen.cfm'>
<cfset LvarFile = 'RepFlujoEfectivoResumen'>
...
<cfif isdefined('rsMensaje')>
	<cflocation url="RepFlujoEfectivoResumen.cfm?rsMensaje=#rsMensaje#">
</cfif>
...
<cfloop from="#iniLoop#" to="#finLoop#" index="mesfin">
	...
	<cfset chequeI ="chequeI#mesfin#">
	<cfset arrayappend(aCheques ,LSParseNumber(form[chequeI]))>
    ...
    <cfoutput>
        ...
        from (
	            ...
	            left join (
	                ...
	                from (select * from #rsArbol# where Nivel > 3) ar
            ...
            union all
	            ...
	            from (select * from #rsArbol# where Nivel = 1 and (upper(ltrim(rtrim(EPCPcodigoref))) <> 'X' or EPCPcodigoref is null )) ar
	            <cfloop from="1" to="#ArrayLen(qMeses)#" index="i">
	            left join (
            ...
            union all
	            ...
	            from (select * from #rsArbol# where Nivel = 2 and EPCPcodigoref <> 'X') ar
	            <cfloop from="1" to="#ArrayLen(qMeses)#" index="i">
	            left join (
        ...
        <table <cfif ArrayLen(qMeses) LT 4> width="70%" </cfif> align="center" cellpadding="0" cellspacing="0">
            ...
            <tr>
                            ...
                            <cfloop query="rsActivos">
                                <cfif #NIVEL# LT 3>	<cfset pintaL3 = true> </cfif>
                                <cfif #NIVEL# LTE 3 and #EPCPcodigoref# EQ 'N'>	<cfset pintaL3 = false> </cfif>
                                <cfif #NIVEL# NEQ 4 or (#NIVEL# EQ 4 and #pintaL3#) or isdefined("form.chkCeros")>
                                    <tr>
                                        <td align="left" nowrap
                                            <cfoutput>
                                                <cfif #NIVEL# EQ 2 and #EPCPcodigoref# EQ 'N'>
                                        ...
                                        >
											...
											<cfelse>
                                                ...
                                                <cfif isdefined("form.chkCeros") and #NIVEL# GT 3>#PCDvalorR#-#PCDvalorSR# &nbsp;</cfif> #descripcion#
...
<cfelse>
    <cfoutput>
        <form action="RepFlujoEfectivoResumen.cfm" method="post" name="sql">
