...
<!---  <cf_dump var="#form#"> --->
<cfinvoke key="MSG_SitucionFDet" default="Situación Financiera Detallada"	returnvariable="MSG_SitucionFDet"
...
<cfquery name="rsActivos" datasource="#session.dsn#">
	...
	left join (
		select 1122 Cmayor, #saldo1122# saldoN, #saldo1122Ant# saldoNAnt
	) saldoNeteo
...
<cfquery name="rsPasivos" datasource="#session.dsn#">
	...
	left join (
		...
		on ar.Cmayor = saldoAnt.Cmayor
	left join (
		select 2112 Cmayor, #saldo1122# saldoN, #saldo1122Ant# saldoNAnt
	) saldoNeteo
		on ar.Cmayor = saldoNeteo.Cmayor
...
<cfquery name="rsTPasivos" datasource="#session.dsn#">
	...
	left join (
		...
		on ar.Cmayor = saldoAnt.Cmayor
	left join (
		select 2112 Cmayor, #saldo2112# saldoN, #saldo2112Ant# saldoNAnt
	) saldoNeteo
		on ar.Cmayor = saldoNeteo.Cmayor
	where left(ar.Orden,1) = '2'
</cfquery>
