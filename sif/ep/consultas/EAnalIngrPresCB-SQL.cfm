...
<cfinvoke key="MSG_ReporteAnalitico" default="Estado de Ejecuci&oacute;n de Ingresos por Rubros"	returnvariable="MSG_ReporteAnalitico"	
...
<cfset LvarIrA = 'EAnalIngrPresCB.cfm'>
<cfset LvarFile = 'EAnalIngrPresCB'>
...
<cfquery name="rsDetalle" datasource="#Session.DSN#">
	select 
    left(p.PCDvalor,1) as PCDvalor,
    case left(p.PCDvalor,1)
    	when '7' then 'INGRESOS POR VENTAS DE BIENES Y SERVICIOS'
        when '8' then 'APORTACIONES'
        when '9' then 'TRANFERENCIAS, ASIGNACIONES, SUBSIDIOS Y SUBVENCIONES RECIBIDOS'
    end 
    as PCDdescripcion,
    sum(CampoA)/#varUnidad# as CampoA,
   	sum(CampoB)/#varUnidad# as CampoB,
    sum(CampoC)*(-1)/#varUnidad# as CampoC,
	sum(CampoD)*(-1)/#varUnidad# as CampoD,
    sum(CampoE)*(-1)/#varUnidad# as CampoE,
    case (sum(CampoA) +sum(CampoB))
        when 0 then 0
        else sum(CampoC)/(sum(CampoA) + sum(CampoB))
	end as Avance
    from #saldoscuenta# s
    ...
    group by left(p.PCDvalor,1)
    order by left(p.PCDvalor,1)
</cfquery>
...
--->		
    ...
    <cfreport format="#form.formato#" template= "EAnalIngrPresCB.cfr" query="rsDetalle">
