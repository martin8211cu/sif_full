...
<cfelse>
    <cfquery name="rsCuentas" datasource="#Session.DSN#"> 
        select distinct c.Cmayor
            ...
            where Cmayor <> 'NULL' and c.CPTMtipoMov not in('D','RA','RC','RP')
            order by c.Cmayor    
   	</cfquery>
...
<cfif isdefined('rsMensaje')>
	<cflocation url="EEPrEgrTOGtCB.cfm?rsMensaje=#rsMensaje#">
</cfif>
</cfoutput>
...
<cfset LvarIrA  = 'EEPrEgrTOGtCB.cfm'>
<cfset LvarFile = 'EEPrEgrTOGtCB'>
...
<cf_dbtemp name="saldoscuenta" returnvariable="saldoscuenta" datasource="#session.dsn#">
    ...
    <cf_dbtempcol name="Seccion"  		type="varchar(4)">
...
<cfquery name="rsDetalle" datasource="#Session.DSN#">
	...
	order by left(s.Seccion,3) desc
</cfquery>
...
<cfif form.formato NEQ "HTML">
    ...
    <cfreport format="#form.formato#" template= "EEPrEgrTOGtCB.cfr" query="rsDetalle">
    ...
    <cfoutput>
		...
		<table width="100%">
            ...
            <cfloop query="rsDetalle">
            	<cfif left(rsDetalle.CtpGsto,3) neq Agrup>
                	<cfset Agrup = '#left(rsDetalle.CtpGsto,3)#'>
                    <tr>
                    	<td><strong>
                    	<cfif Agrup eq 'GCO'>
                        	1 GASTO CORRIENTE
...
--->                    
					...
					<td nowrap align="left" colspan="2">#UCase(rsDetalle.CGEPDescrip)#</td>
            ...
            <tr>
					<td nowrap align="left" colspan="2"><strong>TOTAL</strong></td>
