...
<cf_templateheader title="Estructura Programática">
	<cf_web_portlet_start titulo="Cat&aacute;logo de Firmas">
	<br>
			...
			<td width="60%" valign="top">
				...
				<cfquery name="rsLista" datasource="#session.dsn#">
                 select
					    ID_Firma,
					    Fcodigo EPcodigo,
					    Fdescripcion EPdescripcion
					from CGEstrCatFirma
					where Ecodigo = #session.Ecodigo#
					<cfif isdefined('form.FiltroEPdescripcion')and len(trim(form.FiltroEPdescripcion)) >
						and upper(Fdescripcion) like upper('%#form.FiltroEPdescripcion#%')
					</cfif>
					<cfif isdefined('form.FiltroEPcodigo')and len(trim(form.FiltroEPcodigo)) >
						and upper(Fcodigo) like upper('%#form.FiltroEPcodigo#%')
					</cfif>
				</cfquery>
			<cfoutput>
				<form action="CatFirmas_SQL.cfm" name="form2" method="post">
				...
				isdefined('form.FiltroEPdescripcion')and len(trim(form.FiltroEPdescripcion)) >
					<cfset LvarDireccion = 'CatFirmas.cfm'>
				<cfelse>
					<cfset LvarDireccion = 'CatFirmas.cfm?_'>
				</cfif>
				...
				<cfinvoke
					...
					<cfinclude template="CatFirmas-form.cfm">
					</td>
				</tr>
			</table>
		<br>
...
<script language="javascript" type="text/javascript">
		...
		function funcEliminar(){
			...
			if (confirm("¿Está seguro de que desea Eliminar la(s) estructura(s) seleccionado(s)?")) {
				...
				document.form3.action = "CatFirmas_SQL.cfm";
		...
		function funcFiltrar(){
			document.form2.action='CatFirmas.cfm';
