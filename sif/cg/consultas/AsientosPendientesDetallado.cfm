...
<cfsavecontent variable="queryPendienteDetalle">
	<cfoutput>
		select
			...
			a.ECfechacreacion,
			b.Cdescripcion,
			(select sum(coalesce(Dlocal, 0.00))
					from DContables hd
					where hd.IDcontable = a.IDcontable
							and hd.Dmovimiento = 'D') as Debitos,  
			(select sum(coalesce(Dlocal, 0.00))
					from DContables hd
					where hd.IDcontable = a.IDcontable 
						and hd.Dmovimiento = 'C') as Creditos,
		...
		select
			...
			a.Eperiodo,
			a.Emes,
			<cf_jdbcquery_param cfsqltype="cf_sql_integer" value="null"> as Edocumento,
			a.Edescripcion,
			BMfalta as ECfechacreacion,
			b.Cdescripcion,
			(select sum(coalesce(Dlocal, 0.00))
					from DContablesImportacion hd
					where hd.ECIid = a.ECIid 
							and hd.Dmovimiento = 'D') as Debitos,  
			(select sum(coalesce(Dlocal, 0.00))
					from DContablesImportacion hd
					where hd.ECIid  = a.ECIid  
						and hd.Dmovimiento = 'C') as Creditos,
			'Importacion' as TipoPoliza
		...
		where a.Ecodigo = #Session.Ecodigo#
			...
			<cfif isdefined("form.fechaGeneIni") and len(trim(form.fechaGeneIni)) and isdefined("form.fechaGeneFin") and len(trim(form.fechaGeneFin))>
				...
				<cfelse>
					and <cf_dbfunction name="to_date00" args="a.BMfalta"> between #LSParseDateTime(form.fechaGeneIni)# and #LSParseDateTime(form.fechaGeneFin)#
				</cfif>
			</cfif>
			<cfif isdefined("form.fechaGeneIni") and len(trim(form.fechaGeneIni)) and not ( isdefined("form.fechaGeneFin") and len(trim(form.fechaGeneFin)) )>
				and <cf_dbfunction name="to_date00" args="a.BMfalta"> >= #LSParseDateTime(form.fechaGeneIni)#
...
<table width="99%" align="center"  border="0" cellspacing="0" cellpadding="2">
	...
	<tr><td align="center" colspan="9"><font size="2"><strong>#Title#</strong></font></td></tr>
	<tr><td align="center" colspan="9"><font size="2"><strong>#LB_ConceptoIni#:</strong>&nbsp;&nbsp;#conceptoInicial#&nbsp;&nbsp;<strong>#LB_ConceptoFin#:</strong>&nbsp;&nbsp;#conceptoFinal#</font></td></tr>
	<cfif isdefined("form.EdocumentoI") and len(trim(form.EdocumentoI)) or isdefined("form.EdocumentoF") and len(trim(form.EdocumentoI)) >
		<tr><td align="center" colspan="9"><font size="2"><strong>#LB_PolizaIni#:</strong>&nbsp;&nbsp;#documentoInicial#&nbsp;&nbsp;<strong>#LB_PolizaFin#:</strong>&nbsp;&nbsp;#documentoFinal#</font></td></tr>
	</cfif>
	<tr><td align="center" colspan="9"><font size="2"><strong>#LB_PeriodoMesIni#:</strong>&nbsp;&nbsp;#mesInicial#/#form.periodoini#&nbsp;&nbsp;<strong>#LB_PeriodoMesFin#:</strong>&nbsp;&nbsp;#mesFinal#/#form.periodofin#</font></td></tr>
	<cfif isdefined("form.fechaGeneIni") and len(trim(form.fechaGeneIni)) or isdefined("form.fechaGeneFin") and len(trim(form.fechaGeneFin)) >
		<tr><td align="center" colspan="9"><font size="2"><strong>#LB_FechaGIni#:</strong>&nbsp;&nbsp;#form.fechaGeneIni#&nbsp;&nbsp;<strong>#LB_FechaGFin#:</strong>&nbsp;&nbsp;#form.fechaGeneFin#</font></td></tr>
	</cfif>
	<tr><td align="center" colspan="9"><hr></td></tr>		
</cfoutput>

	<cfif isdefined("queryPendienteDetalle") and len(trim(queryPendienteDetalle)) gt 0>	
		<cftry>
			<cf_jdbcquery_open datasource="#session.dsn#" name="rsPendienteDetalle">
				<cfoutput>#PreserveSingleQuotes(queryPendienteDetalle)#</cfoutput>
			</cf_jdbcquery_open>

			<cfif isdefined("form.toExcel")>
				<cf_exportQueryToFile query="#rsPendienteDetalle#" separador="#chr(9)#" filename="#FileNameTab#.txt" jdbc="true">
			</cfif>

			<cfset Lvar_bx = false>
			<cfoutput query="rsPendienteDetalle">
				<cfif not Lvar_bx>
					<cfset Lvar_bx = true>
					<tr>
						...
						<td nowrap bgcolor="##CCCCCC"><strong>#LB_Documento#</strong></td>
						<td nowrap bgcolor="##CCCCCC"><strong>#LB_Descripcion#</strong></td>
						<td nowrap bgcolor="##CCCCCC" align="right"><strong>#LB_FechaCreacion#</strong></td>
				...
				<tr>
					...
					<td nowrap style="padding-left: 10px;">#Edocumento#</td>
					<td nowrap style="padding-left: 10px;">#Edescripcion#</td>
					<td nowrap align="center">#LSDateFormat(ECfechacreacion,'dd/mm/yyyy')#</td>
...
<cfoutput>
	...
	<tr><td align="center" colspan="9"><strong> --- #Lvar_smsg# ---  </strong></td></tr>
</table> 
</cfoutput>
