...
<cf_dbfunction name="to_sdateDMY"	args="FFECrecursivo" returnvariable="lvarFechaF">
	<cfquery name="rsDocumento" datasource="#Session.DSN#">
		select IDcontable, Ecodigo, Cconcepto, Eperiodo, Emes, Edocumento, Efecha, Edescripcion, Oorigen,
		Edocbase, Ereferencia, ECauxiliar, ECusuario, ECselect, ECreversible, ECestado, ECtipo, ts_rversion, ECrecursivo, ECrecordarForm
		from EContables
		where Ecodigo = #Session.Ecodigo#
			...
			<td width="100%">
				<table width="100%" border="0">
					...
					<tr>
                        ...
                        <td nowrap="nowrap">
                        	<input name="ECrecursivo" type="checkbox" onclick="fnMostrarFF()" value="1" id="ECrecursivo" tabindex="1" <cfif modo EQ 'CAMBIO' and rsDocumento.ECrecursivo EQ 1> checked</cfif>>&nbsp;<label for="ECrecursivo">Recurrente</label>
                            
                        </td>
                        <td id="campoFFECRECURSIVO"  style="display:none" nowrap="nowrap" style="width:150px;">
							<cfif  isdefined("rsDocumento.FFECRECURSIVO") and len(trim(rsDocumento.FFECRECURSIVO))>
                               <label style="font-size:9px">Fin Recurrencia: <cf_sifcalendario name="N_FFECRECURSIVO" value="#LSDateFormat(rsDocumento.FFECRECURSIVO,'dd/mm/yyyy')#"></label>
                            <cfelse>
                               <label style="font-size:9px">Fin Recurrencia: <cf_sifcalendario  name="N_FFECRECURSIVO" value="" ></label>
                            </cfif>
                        </td>
			...
			<td colspan="10" align="center">
				...
				<cfelse>
					<cfif rsTieneLineas.Cantidad GT 0>
						...
						<cfif PermisoAplicar>
							<input type="submit" name="Aplicar" value="Aplicar" class="btnAplicar" onClick="javascript:return Postear();" tabindex="3">
						</cfif>
						<cfoutput>
						...
						<input type="button" name="ConsultarDoc" value="Consultar" tabindex="3"  class="btnNormal" onClick="javascript: verResumido();">
						</cfoutput> 
					</cfif>
					<cfif sufix NEQ 'CC'>
                        	<input type="button" name="Exportar" 	class="btnNormal" value="Exportar Doc." onClick="javascript:exportar();" tabindex="3">
					...
					<cfif sufix NEQ 'CC'>
						<cfif Not ((isdefined("rsDocumento.ECauxiliar") and rsDocumento.ECauxiliar EQ "S") or not PermisoModificar)>
                            <input type="submit" name="BorrarE" 	class="btnEliminar"  tabindex="3" value="Eliminar Doc." onClick="javascript: document.form1.Ccuenta.value = '-1'; document.form1.CFcuenta.value = '-1'; document.form1.Ddescripcion.value = '.'; if (confirm('Desea borrar este documento?')) return true; else return false;">
                        </cfif>					
                        	<input type="submit" name="NuevoE" 		class="btnNuevo"  tabindex="3" value="Nuevo Doc." onClick="javascript: deshabilitarValidacion();">
                        <cfif PermisoModificar>
                            <input type="button" name="btnArchivos" class="btnNormal" tabindex="3" value="Docs. Soporte" onClick="javascript:funcArchivo(1);">									
                        <cfelseif PermisoVisualizar>
                            <input type="button" name="btnArchivos" class="btnNormal"  tabindex="3" value="Docs. Soporte" onClick="javascript:funcArchivo(2);">
                        </cfif>
                    </cfif>
				</cfif>
				<cfif modo EQ "CAMBIO">
					<cfif rsDocumento.ECestado EQ 0>
						<input type="submit" name="btnCerrar" class="btnNormal" value="Cerrar" onClick="javascript: deshabilitarValidacion();" tabindex="3">
					<cfelse>
						<input type="submit" name="btnAbrir"  class="btnNormal" value="Abrir" onClick="javascript: deshabilitarValidacion();" tabindex="3">
					</cfif>
				</cfif>
                <cfif sufix NEQ 'CC'>	
					<cfif isdefined("rsBalanceOfic") and isdefined("rsTotalLineas") and rsTotalLineas.Debitos NEQ rsTotalLineas.Creditos and rsBalanceOfic.Recordcount NEQ 0 and (isdefined("modoDet") and modoDet EQ "ALTA")>
                        <input type="submit" name="btnBalanceaOfic" class="btnNormal" value="Balancear" alt="Balancea por Oficina" title="Balancea por Oficina" onClick="javascript: deshabilitarValidacion();" tabindex="3">
                    </cfif>
                </cfif>
				&nbsp;&nbsp;<input type="button" name="ListaE" class="btnAnterior" value="Ir a Lista" onClick="javascript:Lista();" tabindex="3">					
			</td>		
		</tr>
	</table>
		...
		function valida(boton) {
			...
			<cfif NOT ISDEFINED('rsDocumento.Oorigen') OR NOT LISTFIND('CPCM,CCCP',rsDocumento.Oorigen)>
				if (!dioError && new Number(document.form1.Doriginal.value) == 0 ) {
					alert("El Monto Origen debe ser mayor que cero");
					document.form1.Doriginal.select();
					dioError = true;
				}
				if (!dioError && new Number(document.form1.Dtipocambio.value) == 0 ) {
					alert("El Tipo Cambio debe ser mayor que cero");
					document.form1.Dtipocambio.select();
					dioError = true;
				}
			</cfif>
