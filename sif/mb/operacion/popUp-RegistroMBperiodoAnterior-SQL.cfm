...
<!---SE ESTA CREANDO LA FUNCIONALIDAD PARA BANCOS, PERO SE DEJA LA LOGICA DE TARJETA DE CREDITO POR SI EN UN FUTURO SE OCUPA--->
	<cfset _CBesTCE = 1>
...
<cfif isdefined("form.Aplicar") >
	...
	<cfinvoke component="sif.Componentes.CP_MBPosteoMovimientosB" method="PosteoMovimientos" returnvariable="MLid">
        ...
        <cfinvokeargument name="MovRetroactivo" value="true"/>
		<cfinvokeargument name="ECid" 			value="#form.ECid#"/>
		<cfinvokeargument name="ConciliaML" 			value="true"/>		
	</cfinvoke>
...
<cftransaction>
	...
	<cfelseif isdefined("Form.AltaDet")>	
		...
		<cfquery name="inserted" datasource="#session.DSN#">
			...
			insert into DMovimientos ( EMid, Ecodigo,  Ccuenta, Dcodigo, DMmonto, DMdescripcion,CFid,CFcuenta )
...
<cfif isdefined("form.ECid") and len(trim(form.ECid))>
	<cfset UrlParams = UrlParams & "&ECid="&form.ECid>
</cfif>

<cfif not isdefined("Form.Aplicar") or not isdefined("Form.Baja")>
