...
<cffunction name="write_to_buffer">
	...
	<cfif Arguments.flush Or (buffer.length() GE 16383)>
		<cffile action="append" file="#temporaryFileName#" output="#buffer.toString()#" addnewline="no" charset="#enc.charset#">
...
<cffunction name="exportar" output="yes">
	...
	<cfquery datasource="sifcontrol" name="enc">
		SELECT
			...
			a.BMUsucodigo,
			b.EIsqlexp,
			coalesce(a.charset, 'utf-8') AS CHARSET	<!--- charset varchar(15) null Posibles valores: utf-8,utf-16,iso-8859-1,windows-1252 --->
		FROM EImportador a
		LEFT JOIN EISQL b ON a.EIid = b.EIid
	...
	<cfif html>
			...
			<meta http-equiv="Content-Type" content="text/html; charset=#enc.charset#">
			<cf_templatecss>
			</head>
			<body style="margin:0">
			<style>
	...
	<cfelse>
		...
		<cffile action="write" file="#temporaryFileName#" addnewline="no" output="" charset="#enc.charset#">
		<cfif encabezar>
			<cfloop query="det">
