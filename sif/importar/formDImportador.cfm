<cfif dmodo neq 'ALTA'>
	<cfquery name="rsDForm" datasource="sifcontrol">
		select DItiporeg, DInumero, DInombre, DIdescripcion, DItipo, DIlongitud 
		from DImportador
...
<cfquery name="rsConsecutivo" datasource="sifcontrol">
	select coalesce(max(DInumero),0)+1 as DInumero
	from DImportador
	where EIid=<cfqueryparam cfsqltype="cf_sql_numeric" value="#form.EIid#">
<cfif dmodo neq 'ALTA'>    
	and DItiporeg = <cfqueryparam cfsqltype="cf_sql_char" value="#form.DItiporeg#">
...
<cfquery name="rsConsecutivoEx" datasource="sifcontrol">
	select coalesce(min(DInumero),0)-1 as DInumero
	from DImportador
	where EIid=<cfqueryparam cfsqltype="cf_sql_numeric" value="#form.EIid#">
<cfif dmodo neq 'ALTA'>    
	and DItiporeg = <cfqueryparam cfsqltype="cf_sql_char" value="#form.DItiporeg#">
...
<cfquery name="rsTipoReg" datasource="sifcontrol">
	select DItiporeg, max(DInumero)+1 as siguiente from DImportador
	where EIid=<cfqueryparam cfsqltype="cf_sql_numeric" value="#form.EIid#">
    group by DItiporeg
</cfquery>
...
<cfoutput>
	<table border="0" width="100%" cellpadding="0" cellspacing="0" align="center">
        <tr>
			<td><cf_translate  key="LB_TipoRegistro">Tipo registro</cf_translate>:<br>
				<select name="DItiporeg" onChange="javascript:cambioTipoReg(this);" tabindex="1" >
                	<cfif dmodo eq 'ALTA'>
                    	<option value="-">[Default]</option>
                    </cfif>
                    <cfloop query="rsTipoReg">
                        <option value = "#rsTipoReg.DItiporeg#"
                        <cfif dmodo neq 'ALTA' and rsTipoReg.DItiporeg eq '#form.DItiporeg#'> selected 
                        <cfelse> 
						<cfif dmodo eq 'ALTA' and isdefined("form.TipoReg") and rsTipoReg.DItiporeg eq '#form.TipoReg#'> selected </cfif>
                        </cfif>>
                        
                        <cfif Trim(rsTipoReg.DItiporeg) eq "-">[Default]<cfelse>#rsTipoReg.DItiporeg#</cfif></option>
                    </cfloop>  
                    <option value="***">[Otro]</option>
				</select>
			</td>
<!---        rsConsecutivo.DInumero--->
			<td><cf_translate  key="LB_Numero">N&uacute;mero</cf_translate>:<br>
				<input name="DInumero" type="text" tabindex="1" value="<cfif dmodo neq 'ALTA'>#Trim(rsDForm.DInumero)#<cfelse>#rsTipoReg.siguiente#</cfif>" size="12" maxlength="12" style="text-align: right;" onblur="javascript:fm(this,0); operacion();"  onfocus="javascript:this.value=qf(this); this.select();"  onkeyup="javascript:if(snumber(this,event,0)){ if(Key(event)=='13') {this.blur();}}" alt="El N&uacute;mero" >
                
				<input type="hidden" name="hDInumero" value="<cfif dmodo neq 'ALTA'>#Trim(rsDForm.DInumero)#<cfelse>#rsTipoReg.siguiente#</cfif>">
...
<script language="javascript1.2" type="text/javascript">
	...
	function cambioTipoReg(obj){
		if ( obj.value == "***" ){
			var nuevo = window.prompt('Ingrese el tipo', '');
			if (nuevo != null) {
			...
			document.form1.DInumero.value = '1';
		} else {
			<cfoutput query="rsTipoReg">
			if (obj.selectedIndex == #rsTipoReg.CurrentRow-1#) {
				document.form1.DInumero.value = '#rsTipoReg.siguiente#';
			}
			else {
				document.form1.DInumero.value = '1';
			}
