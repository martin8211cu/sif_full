		...
		<cf_web_portlet_start border="true" skin="#Session.Preferences.Skin#" tituloalign="center" titulo="Importador">
			...
			<table width="100%" cellpadding="0" cellspacing="0">
				...
				<cfif modo NEQ 'ALTA' and dmodo EQ 'ALTA' >
					<tr>
						<td align="center" valign="baseline" >
							...
							<input type="submit" name="AltaD"    	  class="btnGuardar"	value="<cfoutput>#BTN_AgregarDetalle#</cfoutput>"      onClick="javascript: regenera();">
							<input type="submit" name="CambioE"  	  class="btnGuardar" 	value="<cfoutput>#BTN_ModificarEncabezado#</cfoutput>" onClick="javascript:deshabilitarValidacion();" >
							<input type="submit" name="BajaE"    	  class="btnEliminar" 	value="<cfoutput>#BTN_BorrarImportacion#</cfoutput>"   onClick="javascript: if ( confirm('<cfoutput>#MSG_DeseaEliminarEstaImportacionYSusDetalles#</cfoutput>?') ){ deshabilitarValidacion(this); return true; }else{return false;}" >
							<input type="reset"  name="Limpiar"  	  class="btnLimpiar"	value="<cfoutput>#BTN_Limpiar#</cfoutput>" >
							<input type="submit" name="AltaEmpresas"  class="btnNormal"		value="<cfoutput>#BTN_EmpresasPorFormato#</cfoutput>"  onClick="javascript: deshabilitarValidacion(); AsociarEmpresas();">
						</td>	
					</tr>
				</cfif>
				...
				<cfquery datasource="sifcontrol" name="lista_det">
				 select EIid, DInumero, DInombre, DIdescripcion,DItipo, DIlongitud,DItiporeg, CASE DItiporeg WHEN '-' THEN 'Tipo de registro DEFAULT' ELSE 'Tipo de registro '#_Cat#DItiporeg END as DItiporeg1,
			...
			<script language="javascript1.2" type="text/javascript">
				...
				function borrar(value, id){
					if ( confirm('Desea eliminar el registro?') ){
						document.lista.DINUMERO.value = value;
						document.lista.EIID.value = id;
