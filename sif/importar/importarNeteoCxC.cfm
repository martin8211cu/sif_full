<!---
Importador de Neteo de Documentos de CxC
...
--->
<cf_dbtemp name="TempImpNetCxC_v1" returnvariable="TableErr" datasource="#session.DSN#">
...
<cfquery datasource="#session.dsn#">
	...
	where not exists(select 1
					...
					where s.Ecodigo    = #session.Ecodigo#
					  ...
					  and d.CCTcodigo  = #table_name#.TipoTransaccion
...
<cfquery datasource="#session.dsn#">
	...
	from #table_name#
	where not exists(select 1 
	                  from CCTransacciones tt
					where tt.Ecodigo = #session.Ecodigo#
					  and tt.CCTcodigo = #table_name#.TipoTransaccion
					 )
</cfquery>
	...
	select 'Documento con Pago Positivo cuando debe ser Negativo', i.Documento
	  from #table_name# i
	   inner join CCTransacciones tt
	    on tt.CCTcodigo = i.TipoTransaccion
	where tt.Ecodigo = #session.Ecodigo#
	and tt.CCTtipo = 'D'
	and i.Monto > 0
</cfquery>
...
<cfelse>
	<cftransaction>
             ...
             <cfquery name="ENeteoCxC" datasource="#session.dsn#">  	         
				...
				<cf_dbidentity2 datasource="#session.DSN#" name="ENeteoCxC">
				<cfset id = ENeteoCxC.identity> 
