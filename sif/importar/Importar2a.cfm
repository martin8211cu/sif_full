...
<cfquery  dbtype="query" name="lista">
	select 
	EIcodigo,EImodulo, EIdescripcion,EImod_fecha,EImod_login,EIid,upper(EIdescripcion) as descr_sort
	from session.importar_enc
	order by <cfif url.o IS 1>EIcodigo
	     <cfelseif url.o IS 2>EImodulo
	     <cfelseif url.o IS 3>descr_sort
...
<cfif not IsDefined("session.importar_enc")>
...
<cfoutput>
  <tr>
    <td colspan="6" class="subTitulo">Seleccione las definiciones que desea importar</td>
  </tr>
  <tr>
    <td colspan="6">&nbsp;</td>
    </tr>
...
<tr>
    <td colspan="6" align="center"><input type="submit" name="Submit" value="Importar"></td>
    </tr>
</table>
</form>
