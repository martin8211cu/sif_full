...
<cfif isDefined("session.Ecodigo") and isDefined("Form.Dcodigo") and len(trim(#Form.Dcodigo#)) NEQ 0>
	...
	Select Dcodigo, Ddescripcion, timestamp
	from Departamentos
...
<form action="SQLDepartamentos.cfm<cfif isdefined('url.desde') and Trim(url.desde) eq 'rh'>?desde=rh</cfif>" method="post" name="form" onSubmit="document.form.Dcodigo.disabled=false; MM_validateForm('Ddescripcion','','R');return document.MM_returnValue">
  <table width="67%" height="75%" align="center" cellpadding="0" cellspacing="0">
    <tr valign="baseline" bgcolor="#FFFFFF"> 
      ...
      <td>&nbsp;</td>
      <td> <input name="Ddescripcion" type="text"  value="<cfif #modo# NEQ "ALTA"><cfoutput>#rsDepartamentos.Ddescripcion#</cfoutput></cfif>" size="40" maxlength="60"  alt="El campo Descripcin del Tipo de Transaccin">
	  <input type="hidden" name="Dcodigo" value="<cfif modo NEQ "ALTA"><cfoutput>#rsDepartamentos.Dcodigo#</cfoutput></cfif>">
...
<cfset ts = "">
  <cfif modo NEQ "ALTA">
    <cfinvoke component="sif.Componentes.DButils" method="toTimeStamp"returnvariable="ts">
		<cfinvokeargument name="arTimeStamp" value="#rsDepartamentos.timestamp#"/>
	</cfinvoke>
</cfif>  
