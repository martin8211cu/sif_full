	...
	<tr> 
			...
			<td>
				<input name="Rcodigo" tabindex="1" type="text" size="5" maxlength="2"  alt="El Código de la Retención" 
					...
					value="<cfif modo neq 'ALTA'>#htmleditFormat(rsRetenciones.Rcodigo)#</cfif>" >			  <label>
		...
		<tr> 
			...
			<td>
				<input name="Rporcentaje" tabindex="1" type="text" onBlur="javascript: fm(this,2);" 
				<cfif modo neq 'ALTA' and retcomp.cant neq 0>readonly="readonly"</cfif>
				value="<cfif modo NEQ "ALTA">#LSCurrencyFormat(rsRetenciones.Rporcentaje, 'none')#<cfelse>0.00</cfif>" 
				size="8" maxlength="6"  style="text-align: right;" onfocus="javascript:this.value=qf(this); this.select();"  
				onkeyup="javascript:if(snumber(this,event,2)){ if(Key(event)=='13') {this.blur();}}" alt="El Porcentaje de la Retención">			</td>
		</tr>
...
function sum_pct()
{
	...
	if (document.form1.retsimple_#HTMLEditFormat(Trim(retsimple.Rcodigo))#.checked)
		newpct += #NumberFormat(retsimple.Rporcentaje,"0")#;
	</cfoutput>
	document.form1.Rporcentaje.value = newpct;
	fm(document.form1.Rporcentaje,2);
}
