...
<cfif modo NEQ 'ALTA'>
	<cfquery name="rsClasificacion" datasource="#Session.DSN#">		
		...
		b.Ccodigoclas, b.Cdescripcion, b.Cnivel,
				c.CCcodigo, c.CCdescripcion, c.CCnivel
		...
		<!-----
		select id,Ecodigo, SNcodigo, CCid,Ccodigo,nivel,ts_rversion
...
<form action="SQLClasificacionSocioNeg.cfm" method="post" name="form1" onSubmit="javascript: return FuncValidar()">
	...
	<table width="100%" cellpadding="2" cellspacing="0" border="0" align="center">
		...
		<tr>
			...
			<td nowrap>
				<cfif modo neq 'ALTA'>
					<cf_sifsociosnegocios2 idquery="#rsClasificacion.SNcodigo#">
				<cfelse>
		...
		<tr>
			<td align="right"><div id="Clas" style="display: <cfif modo NEQ 'ALTA'>''<cfelse>none</cfif>;"><strong><label>Clasificación:</label></strong></div></td>
			<td>				
...
<table width="100%" cellpadding="2" cellspacing="0" border="0" align="center">
		<cfif modo NEQ 'ALTA'>
			...
			<tr>
				<td colspan="2">
				<!----  onClick=''eliminar();''    onClick=''eliminar();''---->
					<cfquery name="rsLista" datasource="#session.DSN#">							
						select  b.SNnumero, b.SNnombre, 
								case when a.CCid IS null and a.AClaId IS null then 'Tipo: Artículos'
								when a.CCid IS null and a.Ccodigo IS null then 'Tipo:Activo'
										else  'Tipo: Servicios' end as tipo,
								...
								case when a.CCid IS null and a.AClaId IS null then c.Cdescripcion
									  when a.CCid IS null and a.Ccodigo IS null then cl.ACdescripcion
										else  d.CCdescripcion end as descripcion,
...
<!---					<form name="form3" action="SQLClasificacionSocioNeg.cfm" method="post">
--->					<cfinvoke
				 ...
				 </cfinvoke><!---						<cfinvokeargument name="formName" value="form3"/>
...
<script language="JavaScript" type="text/JavaScript">
	...
	/*function eliminar(CodPuesto, Linea){
		if (!confirm('¿Desea eliminar el Dato Variable?')) return false;				
