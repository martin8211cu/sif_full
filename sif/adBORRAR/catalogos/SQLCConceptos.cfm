...
<cfif not isdefined("Form.Nuevo")>
	<cfset nivel = 0 >
	<cfset path  = RepeatString("0", 5-len(trim(form.CCcodigo)) ) & "#trim(form.CCcodigo)#">
			...
			<cf_dbtimestamp
				...
				<cfif (form.CCidpadre neq form._CCidpadre) or (trim(form.CCcodigo) neq trim(form._CCcodigo)) >
					<cfquery name="update_path" datasource="#session.DSN#" >
						  update CConceptos
						  set CCpath =  right( '00000'  #_Cat# ltrim(rtrim(CCcodigo)), 5),
					...
					<cfloop from="0" to="99" index="i">
						<cfquery datasource="#session.DSN#">														  
							...
							set CCpath =    coalesce((select min(p.CCpath)
											...
											and p.CCnivel = #nivel# ), ' ')
											#_Cat# '/' #_Cat# right ( '00000' #_Cat# rtrim(ltrim(CCcodigo)) , 5), 
											CCnivel = #nivel + 1#
