...
<cfif rsExisteConceptoCC.total eq 0 >
	<cfquery name="insertConceptoCC" datasource="#session.DSN#">
		insert into Conceptos(Ecodigo, Ccodigo, Cdescripcion, Ctipo)
		values ( <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">, 
				 'VENTA', 
				 'Ventas', 
				 'I' )
	...
	<cfif rsImpuestos.total eq 0>
		<cfquery name="rsImpuestos" datasource="#session.DSN#">
			...
			values ( <cfqueryparam cfsqltype="cf_sql_integer" value="#session.Ecodigo#">, 
				 ...
				 'Exento de Impuestos', 
				 0, 
				 <cfif form.WTCid eq 0 ><cfif len(trim(rsCcuentaImp.Ccuenta)) ><cfqueryparam cfsqltype="cf_sql_numeric" value="#rsCuentaImp.Ccuenta#"><cfelse>null</cfif><cfelse>null</cfif>,
