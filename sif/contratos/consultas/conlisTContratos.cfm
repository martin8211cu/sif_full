...
function Asignar(id,descripcion){
	if (window.opener != null) {
		window.opener.document.<cfoutput>#form.formulario#</cfoutput>.CTCcodigo.value   = id;
		window.opener.document.<cfoutput>#form.formulario#</cfoutput>.CTCdescripcion.value = descripcion;
		window.close();
	}
}
</script>
...
<cfquery name="rsTContratos" datasource="#session.DSN#">
...
<cfinvoke 
	...
	<cfinvokeargument name="query" value="#rsTContratos#"/> 
	...
	<cfinvokeargument name="irA" value="conlisTContratoSolicitante.cfm"/> 
