...
<cfargument name="CCHid"           type="numeric"          required="yes">
	...
	<cfif totR.disponible gt 0>
		...
		<cfset  CambiaEstadoTP (LvarCCHTid,'EN APROBACION TES','REINTEGRO',id,'Solicitud Pago')>
	</cfif>
</cffunction>
	...
	<cfelse>
		...
		<cfloop query="rsLineas">
			<cfquery name="rsGEL" datasource="#session.dsn#">
				select lg.CFcuenta,lg.GELGtotalOri,lg.CFid, l.CPNAPnum, lg.Linea,lg.PCGDid
		...
		<cfloop query="rsOPs">
			...
			<cfquery name="selectins" datasource="#session.dsn#">
				select
					...
					substring('CCH.Transaccion,'  #LvarCNCT# <cf_dbfunction name="to_char" args="l.CCHcod"> #LvarCNCT# ' : a '  #LvarCNCT# '#rsName.TESBeneficiario#',1,65) as MLdescripcion, 
					'S' as MLconciliado,
