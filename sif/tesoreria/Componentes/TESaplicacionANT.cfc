...
<cffunction name="sbOP_MovimientosCxPOri" output="true" access="private">
  ...
  update BMovimientosCxP set  Dtipocambio= #rsDatosBm.DTipoCambio# , BMmontoref = #rsDatosBm.BMmontoref#, BMfactor = #rsDatosBm.BMfactor# 
			...
			<cfquery datasource="#session.dsn#">
				...
				select 	op.EcodigoPago, 
						...
						<cf_jdbcquery_param cfsqltype="cf_sql_timestamp" value="#LvarFechaContable#">,
                       	substring(rtrim(ltrim('#rsTESOP.ReferenciaOP#')),0,25), 
                        '#rsTESOP.DocumentoOP#', 
...
<cffunction name="sbControl_PresupuestoPago" output="false" access="private" returntype="void">
	...
	<cfquery datasource="#session.DSN#">
		...
		select  '#Arguments.Origen#',					
				'#Arguments.Documento#',
                substring(rtrim(ltrim('#Arguments.Referencia#')),0,25),
				<cf_jdbcquery_param cfsqltype="cf_sql_timestamp" value="#LvarFechaContable#">,
		  ...
		  from #INTARC# i
				inner join TESdetallePago dp
					...
					<cfif LvarAnularEspecial>
						 and nap.CPNAPDlinea	= 0
						 and nap.CPNAPDtipoMov	= '00'
	...
	<cfquery datasource="#session.DSN#">
		...
		select  '#Arguments.Origen#',
				...
				<cfif Arguments.TipoMov NEQ 'P'>-</cfif>
					(
						<cfif LvarLineaIDrepetido>sp.TESSPnumero*100000000.0+</cfif>
						nap.CPNAPnum*10000.0+case when nap.CPNAPDlinea<0 then -nap.CPNAPDlinea else nap.CPNAPDlinea end
					) 
				as NumeroLineaID,
