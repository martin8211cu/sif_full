...
<cffunction name="sbAprobarSP" access="public" output="false"> 
	...
	<cfelse>
		...
		<cfquery datasource="#session.DSN#">
			...
			select  '#arguments.PRES_Origen#',																			<!--- ModuloOrigen --->
					...
					#PreserveSingleQuotes(LvarSignoDB_CR)# * round(abs(dp.TESDPmontoAprobadoOri) * dp.TESDPtipoCambioSP,2) as MontoLocal,
					'RC' as Tipo,
		...
		<cfquery datasource="#Session.DSN#">
			update CPDocumentoD
			   set CPDDsaldo = CPDDsaldo - 
						(
						 select sum(
									round(abs(dp.TESDPmontoAprobadoOri) * dp.TESDPtipoCambioSP,2) 
								)
						   from TESdetallePago dp
						  where TESSPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.SPid#">
...
<cffunction name="InsertaSeguimiento"  access="public">
	...
	<cfargument name="Tipo"		 		type="string" 	defautl="">
	<cfargument name="Motivo"	 		type="string" 	defautl="">
...
<cffunction name="sbAprobarSP_Workflow" access="public" output="false"> 
	...
	<cfreturn sbAprobarSP(
					Arguments.SPid, Arguments.fechaPagoDMY, Arguments.generarOP, Arguments.NAP,
					Arguments.PRES_Origen, Arguments.PRES_Documento, Arguments.PRES_Referencia
				)>
</cffunction>
...
<cffunction name="sbRechazarSPaprobada"  access="public" returntype="string">
	...
	<cftransaction>
		...
		<cfloop query="rsSP">
			...
			<cfelseif rsSP.TESSPtipoDocumento EQ 7>
				<!--- SP Liqui.GE: Obsoleto, compatibilidad con Liquidaciones Viejas --->
				<cfset LvarObjGE.sbOP_MovimientosLiquidacionGE(-1, false, true, rsSP.TESSPid)>
			<cfelseif rsSP.TESSPtipoDocumento EQ 8>
				<!--- SP Fondo CCh --->
				<cf_errorCode	code = "50759" msg = "No se ha implementado Rechazos de Fondeo de Caja Chica">
			<cfelseif rsSP.TESSPtipoDocumento EQ 9>
				<!--- SP Reint.CCh --->
...
<cffunction name="sbReversarOrdenPago" output="false" access="public">
	...
	<cftry>
		<cfset sbAplicarOP (Arguments.TESOPid, true, Arguments.AnularOP, Arguments.AnularSP)>
	<cfcatch type="any">
		<cftransaction action="rollback" />
		<cfrethrow>
	</cfcatch>
		...
		<cfloop query="rsOPs">
			...
			<cfquery datasource="#session.dsn#" name="insert">
				...
				VALUES(
					   ...
					   <cf_jdbcQuery_param cfsqltype="cf_sql_varchar" len="20"  value="#rsOPs.ChequeViejo#"           voidNull>,
			...
			<cfquery datasource="#session.dsn#" name="insert">
				...
				VALUES(
					   ...
					   <cf_jdbcQuery_param cfsqltype="cf_sql_varchar" len="20"  value="#rsOPs.ChequeNuevo#"           voidNull>,
					   <cf_jdbcQuery_param cfsqltype="cf_sql_varchar" len="50"  value="#insert.OPdescripcion#"        voidNull>,
						'N',
						<cf_jdbcquery_param cfsqltype="cf_sql_numeric" value="null">, 
					   <cf_jdbcQuery_param cfsqltype="cf_sql_float"             value="#insert.TESOPtipoCambioPago#"  voidNull>,
...
<cffunction name="sbAplicarSPsinOP" output="true" access="public">
	...
	<!--- DB DetallePago --->
	<cfset sbOP_DB_DetallePagoOri(-1, rsSP)>
	...
	<!--- BALANCEA Moneda Extranjera por Oficina --->
	<cfset sbBAL_MonedaExtranjeraPorOficina(LvarEcodigo)>
	...
	<cfset sbControl_PresupuestoPago(0,LvarEcodigo, Arguments.Oorigen, Arguments.Documento, Arguments.Referencia, 1, 1)>
...
<cffunction name="sbAplicarOP" output="true" access="private">
	...
	<cfloop query="rsAsientos">
		...
		<cfif rsAsientos.EcodigoPago EQ rsAsientos.EcodigoOri>
			...
			<!--- CR Banco --->
			<cfset sbOP_CR_bancoPago(arguments.TESOPid, rsAsientos)>
			<!--- DB CxC intercompany de EcodigoPago hacia EcodigoOri (ignora pagos a la misma empresa) --->
			<cfset sbOP_DB_CxCintercompanyPago(arguments.TESOPid, rsAsientos)>
		<cfelse>
			<!--- CR CXP intercompany de EcodigoOri hacia EcodigoPago --->
			<cfset sbOP_CR_CxPintercompanyOri(arguments.TESOPid, rsAsientos)>
			<cfquery name="rsOficina" datasource="#session.dsn#">
				select i.Ocodigo
				  from CIntercompany i
				 where i.Ecodigo 		= #rsAsientos.EcodigoOri#
		...
		<!--- DB DetallePago --->
		<cfset sbOP_DB_DetallePagoOri(arguments.TESOPid, rsAsientos)>
		<cfset sbOP_trasladoCuenta_IVA(arguments.TESOPid, rsAsientos)>
		<!--- DB Genera DiferencialCambiario para CxP --->
		<cfset sbOP_CxP_DiferencialCambiarioOri(arguments.TESOPid, rsAsientos)>
		...
		<!--- BALANCEA Moneda Extranjera por Oficina --->
		<cfset sbBAL_MonedaExtranjeraPorOficina(rsAsientos.EcodigoOri)>
		...
		<cfset sbControl_PresupuestoPago(arguments.TESOPid, rsAsientos.EcodigoOri,'TEOP', rsTESOP.DocumentoOP, LvarReferencia, rsAsientos.currentRow,rsAsientos.recordCount)>
		<!---- GENERA EL ASIENTO CONTABLE --->
		<cfinvoke component="sif.Componentes.CG_GeneraAsiento" method="GeneraAsiento" returnvariable="LvarIDcontable">
			<cfinvokeargument name="Ecodigo" value="#rsAsientos.EcodigoOri#"/>
			<cfinvokeargument name="Eperiodo" value="#LvarAuxPeriodo#"/>
		...
		<cfif rsTESOP.CFcuentaTraslado EQ "">
			...
			<cfset LvarObjGE.sbOP_MovimientosAnticiposGE(arguments.TESOPid, LvarAnulacion, LvarAnularSP)>
			<!--- Realiza Movimientos en Liquidacion de Gastos a Empleados de la Empresa Ori--->
			<cfset LvarObjGE.sbOP_MovimientosLiquidacionGE(arguments.TESOPid, LvarAnulacion, LvarAnularSP)>
			...
			<cfif rsSQL.cantidad GT 0>		
				...
				<cfset LvarObjCCH.sbOP_MovimientosFondeoCCh(arguments.TESOPid, '', rsAsientos, LvarIDcontable)>
				<cfset LvarObjCCH.sbOP_MovimientosReintegroCCh(arguments.TESOPid)>		
			</cfif>
		</cfif>

		<!--- Realiza Movimientos en Bancos de la Empresa de Pago --->
		<cfset sbOP_MLibrosPago(arguments.TESOPid, rsAsientos, LvarIDcontable)>
	</cfloop>
...
<cffunction name="sbMovimientosIETU" output="false" access="private">
	...
	<cfquery name="rsRegistroDocumento" datasource="#session.dsn#">
		select 	
				cb.Ecodigo,
				<cf_dbfunction name="spart" args="'#rsTESOP.ReferenciaPago#',1,20"> as ReferenciaPago,
...
<cffunction name="sbOP_CR_bancoPago" output="false" access="private">
	...
	<cfquery datasource="#session.dsn#">
		...
		select
				'TEOP', 1, 
				<cf_dbfunction name="to_char" args="op.TESOPnumero">, '#rsTESOP.ReferenciaPago#',
				'#DateFormat(now(),"YYYYMMDD")#', #LvarAuxPeriodo#, #LvarAuxMes#, cb.Ocodigo,
			<cfif LvarAnulacion>
				'D', 
			<cfelse>
...
<cffunction name="sbOP_DB_CxCintercompanyPago" output="false" access="private">
	...
	<cfquery datasource="#session.dsn#">
		...
		group by 
				...
				dp.TESDPtipoCambioOri, op.TESOPtipoCambioPago, dp.TESDPfactorConversion,dp.PCGDid
	</cfquery>
</cffunction>
...
<cftry>
	<cfquery datasource="#session.dsn#">
		...
		select 
				'TEOP', 1, 
				dp.TESDPdocumentoOri, <cf_dbfunction name="sPart"	args="dp.TESDPmoduloOri #_CAT# ' ' #_CAT# dp.TESDPreferenciaOri;1; 25" delimiters=";">, 
			...
			<cfelse>
				...
				<cfif LvarAplicaSPsinOP>
					dp.CFcuentaDB,
				<cfelse>
					coalesce(op.CFcuentaTraslado,dp.CFcuentaDB), 
				</cfif>
				0, 
...
<cffunction name="sbOP_trasladoCuenta_IVA" access="public"  output="false">
	...
	<cfquery name="rsINTARC" datasource="#session.DSN#">
		...
		select
				...
				c.Mcodigo,
				<cfif LvarAnulacion>-</cfif>round( (dp.TESDPmontoAprobadoOri / c.Dtotal) * b.MontoCalculado,2),
				dp.TESDPtipoCambioOri,
				<cfif LvarAnulacion>-</cfif>round(
					round( (dp.TESDPmontoAprobadoOri / c.Dtotal) * b.MontoCalculado,2)
					*
					dp.TESDPtipoCambioOri
				, 2)
		  from TESdetallePago dp
...
<cffunction name="sbOP_CxP_DiferencialCambiarioOri" output="false" access="private">
	...
	<cfquery datasource="#session.dsn#">
		...
		<cfelse>
           ...
           and abs(dp.TESDPtipoCambioOri - cxp.EDtcultrev) <> 0
			<!--- Sin retenciones y Sin multas (Los embargos y cesiones también son las líneas del DocOri, por tanto sí hay que procesarlos) --->
		   and dp.RlineaId is NULL and dp.MlineaId is NULL
	</cfquery>
	...
	<cfquery datasource="#session.dsn#">
		...
		select 
		 ...
		 where p.Ecodigo = #Arguments.Ecodigo#
		   ...
		   and round(INTMON,2) <> round(INTMON2,2)
	</cfquery>

	<cfquery name="rsAsiento" datasource="#session.dsn#">
		select sum(INTMON * case when INTTIP = 'D' then 1 else -1 end) as Diferencia, sum(0.005) as DiferenciaPermitida
		  from #INTARC#
	</cfquery>
	<cfif rsAsiento.DiferenciaPermitida EQ 0>
		<cf_errorCode	code = "51611" msg = "El Asiento Generado está vacío">
	</cfif>
	<cfif rsAsiento.DiferenciaPermitida LT abs(rsAsiento.Diferencia)>
...
<cffunction name="sbOP_MovimientosCxPOri" output="true" access="private">
	...
	<cfquery datasource="#session.dsn#">
		...
		<cfelse>
			...
			dp.TESDPmontoPago - (coalesce(Rmonto,0)+coalesce(Mmonto,0))*TESDPfactorConversion, 
			coalesce(Rmonto,0), 
			coalesce(Mmonto,0), 
...
<cffunction name="sbTransferenciasCB_TFdst" access="private" returntype="numeric">
	...
	<cfquery name="insert" datasource="#session.dsn#">
		...
		values (
				...
				round(#(rsTESTIDs.TESTIDmontoDst)*rsTESTIDs.TESTIDtipoCambioOri#,2), 
...
<cffunction name="sbInicializaCatalogos" output="false" access="public">
	...
	<cftransaction>
		...
		<cfquery datasource="#session.dsn#">
			insert into BTransacciones
				(Ecodigo, BTcodigo, BTdescripcion, BTtipo, BMUsucodigo)
			select Ecodigo, 'P8', 'Pago con Tarjeta Credito Empresarial', 'C', #session.Usucodigo#
			  from Empresas
		...
		<cfquery datasource="#session.dsn#">
			insert into BTransacciones
				(Ecodigo, BTcodigo, BTdescripcion, BTtipo, BMUsucodigo)
			select Ecodigo, 'P9', 'Abono a TCE desde Tesorería', 'D', #session.Usucodigo#
			  from Empresas
		...
		<cfquery datasource="#session.dsn#">
			insert into BTransacciones
				(Ecodigo, BTcodigo, BTdescripcion, BTtipo, BMUsucodigo)
			select Ecodigo, 'X9', 'Anulación de Abono a TCE desde Tesorería', 'C', #session.Usucodigo#
			  from Empresas
...
<cffunction name="sbControl_PresupuestoPago" output="false" access="private" returntype="void">
	...
	<cfquery datasource="#session.DSN#">
		...
		select  '#Arguments.Origen#',					
				...
				-#LvarINTLIN#,																	<!--- NumeroLinea  --->
				dp.CFcuentaDB, cp.CPcuenta,														<!--- CFuenta --->
				dp.OcodigoOri,																	<!--- Oficina --->
				mo.Mcodigo,																		<!--- Mcodigo --->
				<cfif NOT LvarAnulacion>-</cfif>#PreserveSingleQuotes(LvarSignoDB_CR)# * round(abs(dp.TESDPmontoAprobadoOri),2) as MontoOrigen,
				dp.TESDPtipoCambioSP,															<!--- as TipoCambio --->
				<cfif NOT LvarAnulacion>-</cfif>#PreserveSingleQuotes(LvarSignoDB_CR)# * round(abs(dp.TESDPmontoAprobadoOri) * dp.TESDPtipoCambioSP,2) as MontoLocal,
				nap.CPNAPDtipoMov as Tipo,														<!--- as TipoMovimiento --->
				nap.CPNAPnum, nap.CPNAPDlinea,
				dp.PCGDid,
				<cfif NOT LvarAnulacion>-</cfif>#PreserveSingleQuotes(LvarSignoDB_CR)# as Cantidad_1
		  from #INTARC# i
				inner join TESdetallePago dp
					inner join CFinanciera cf
						left join CPresupuesto cp
					...
					<cfif LvarAnularSP>
						 ...
						 and nap.CPNAPDtipoMov	= 'RP'
					<cfelse>
						 and nap.CPNAPDlinea	= dp.TESSPlinea
						 and nap.CPNAPDtipoMov	= 'RC'
					</cfif>
					inner join Monedas mo
						 on mo.Ecodigo  = dp.EcodigoOri
						and mo.Miso4217 = dp.Miso4217Ori
				 on dp.TESDPid 		= i.LIN_IDREF
			where NOT (dp.CFcuentaDB_SP IS NOT NULL AND dp.NAPref_SP IS NOT NULL)
	</cfquery>

	<cfif GvarContaPresupuestaria AND NOT LvarAnularSP>
		<!--- 
			Convierte la RC especial a E en Anticipos de Gasto Empleados
				-RC -lin
				+E	+lin
		--->
		<cfloop list="RC,E" index="LvarMov">
			<cfquery datasource="#session.DSN#">
				insert into #request.intPresupuesto#
					(
						ModuloOrigen,
				...
				select  '#Arguments.Origen#',					
						...
						<cfif LvarMov EQ "RC">-</cfif>(nap.CPNAPnum*10000+case when nap.CPNAPDlinea<0 then -nap.CPNAPDlinea else nap.CPNAPDlinea end),
						dp.CFcuentaDB_SP,
						dp.OcodigoOri,																	<!--- Oficina --->
						mo.Mcodigo,																		<!--- Mcodigo --->
						<cfif LvarMov EQ "RC">-</cfif>#PreserveSingleQuotes(LvarSignoDB_CR)# * round(abs(dp.TESDPmontoAprobadoOri),2) as MontoOrigen,
						nap.CPNAPDtipoCambio,															<!--- as TipoCambio --->
						<cfif LvarMov EQ "RC">-</cfif>#PreserveSingleQuotes(LvarSignoDB_CR)# * round(abs(dp.TESDPmontoAprobadoOri) * nap.CPNAPDtipoCambio,2) as MontoLocal,
						'#LvarMov#' as Tipo,
						<cfif LvarMov EQ "RC">														<!--- as TipoMovimiento --->
							nap.CPNAPnum, nap.CPNAPDlinea,
						</cfif>
						dp.PCGDid,
						<cfif LvarMov EQ "RC">-</cfif>#PreserveSingleQuotes(LvarSignoDB_CR)# as Cantidad_1
				  from #INTARC# i
						inner join TESdetallePago dp
							inner join CFinanciera cf
								left join CPresupuesto cp
							...
							inner join TESsolicitudPago sp
								 on sp.TESSPid = dp.TESSPid
								and sp.TESSPtipoDocumento = 6    <!--- ANTICIPOS GASTO EMPLEADOS --->
								and coalesce(sp.NAP, 0) <> 0
							inner join CPNAPdetalle nap
								  on nap.Ecodigo		= dp.EcodigoOri
								 and nap.CPNAPnum		= sp.NAP
							...
							inner join Monedas mo
								...
								and mo.Miso4217 = dp.Miso4217Ori
						 on dp.TESDPid 		= i.LIN_IDREF
					where dp.CFcuentaDB_SP IS NOT NULL
			</cfquery>
		</cfloop>
	</cfif>
...
<cffunction name="sbPresupuestoMovsEfectivo_CxP_TESGP" access="private">
	...
	<cfquery datasource="#session.DSN#">
		...
		select  '#Arguments.Origen#',
				...
				<cfif Arguments.TipoMov NEQ 'P'>-</cfif>(nap.CPNAPnum*10000+case when nap.CPNAPDlinea<0 then -nap.CPNAPDlinea else nap.CPNAPDlinea end) as NumeroLineaID,
				...
				<cfif Arguments.Anulacion>-</cfif>round(dp.TESDPmontoAprobadoOri / cp.Dtotal * nap.CPNAPDmontoOri * dp.TESDPtipoCambioOri,2) as MontoLocal,
	...
	<cfquery datasource="#session.DSN#">
		...
		select  '#Arguments.Origen#',					
				'#Arguments.Documento#',
				'#Arguments.Referencia#',
				<cf_dbfunction name="concat" args="'TES SP-';#LvarTESSPnumero#" delimiters=";">,
				<cf_jdbcquery_param cfsqltype="cf_sql_timestamp" value="#Arguments.Fecha#">,
				#Arguments.Periodo# as Periodo,
				#Arguments.Mes# as Mes,		
