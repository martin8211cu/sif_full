	...
	<!--- Envía al Repositorio de  CFDI --->
		...
		<cfloop query="rsDetallesBRTP">
        	<cfset varOrigen = #TESDPmoduloOri#>
              	<cfif trim(varOrigen) EQ "TESP" or trim(varOrigen) EQ "CCH" and rsDetallesBRTP.ID_Linea neq "">
					<cfset vOrigen = "TES">
...
<cffunction name="sbOP_trasladoCuenta_IVA" access="public"  output="false">
	...
	<cfquery name="rsINTARC" datasource="#session.DSN#">
		...
		select
				...
				CASE
					WHEN i.Iporcentaje > 0 THEN ((((CAST(b.MontoBaseCalc AS decimal(18, 10))) * CAST(i.Iporcentaje AS decimal(18, 10)) / 100) * (((CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10))) * 100) / (CAST((c.Dtotal-isnull(c.EDretencionVariable,0)-isnull(hc.EDmontoretori,0)) AS decimal(18, 10))))) / 100)
					ELSE 0
				END INTMOE,
				dp.TESDPtipoCambioOri,
				<cfif LvarAnulacion>-</cfif>
				CASE
					WHEN i.Iporcentaje > 0 THEN ((((CAST(b.MontoBaseCalc * dp.TESDPtipoCambioOri  AS decimal(18, 10))) * CAST(i.Iporcentaje AS decimal(18, 10)) / 100) * (((CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10))) * 100) / (CAST((c.Dtotal-isnull(c.EDretencionVariable,0)-isnull(hc.EDmontoretori,0)) AS decimal(18, 10))))) / 100)
					ELSE 0
				END INTMON,
                dp.NumeroEvento,dp.CFid
		  from TESdetallePago dp
			inner join EDocumentosCP c
			inner join HEDocumentosCP hc
					on c.Ecodigo = hc.Ecodigo
					and c.IDdocumento = hc.IDdocumento
	...
	<cftransaction>
		<cftry>
			<cfquery name="rsGetValueImp" datasource="#session.DSN#">
				Update b
				SET MontoCalculado = MontoCalculado - (((CAST(b.MontoBaseCalc AS decimal(18, 10)) * CAST(i.Iporcentaje AS decimal(18, 10)) / 100) * ((CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10)) * 100) / CAST(c.Dtotal AS decimal(18, 10)))) / 100),
					MontoPagado = MontoPagado + (((CAST(b.MontoBaseCalc AS decimal(18, 10)) * CAST(i.Iporcentaje AS decimal(18, 10)) / 100) * ((CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10)) * 100) / CAST(c.Dtotal AS decimal(18, 10)))) / 100)
				<!--- 
	...
	<!--- 0 SIN PAGAR (CUANDO  SE REGISTRA LA FACTURA )
	 ...
	 <cftransaction>
		...
		<cfquery name="rsTotalRet" datasource="#session.dsn#">
			select isnull(abs(sum(TESDPmontoAprobadoLocal)),0) totalRet
			from TESdetallePago
			where TESid	= <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Tesoreria.TESid#">
			and TESOPid	= <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.TESOPid#">
			and TESDPmontoAprobadoLocal < 0
		...
		<cfquery name="rsDATADIOT" datasource="#session.dsn#">
			SELECT dp.TESOPid,
			       ...
			       <cfif rsDetalleSolPag.TESDPmoduloOri EQ 'CPFC'>
					<!--- (sum(d.DDtotallin * (dto.doctotal - #rsTotalRet.totalRet#/*  c.EDmontoretori */)/dto.doctotal) * sum(CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10))) / sum(c.EDsaldo - #rsTotalRet.totalRet#/*  c.EDmontoretori */))  AS MontoBase, --->
					(sum(CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10))))  AS MontoBase,
					...
					<cfelse>
						(sum(d.DDtotallin) * sum(CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10))) / sum(c.EDsaldo -   c.EDmontoretori) * CAST(i.Iporcentaje AS decimal(18, 10))/100) MontoIVA,
						(sum(d.DDtotallin) * sum(CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10))) / sum(c.EDsaldo - c.EDmontoretori ) * CAST(i.Iporcentaje AS decimal(18, 10))/100) * cast(dp.TESDPtipoCambioSP as decimal(18,10)) TESDPmontoPagoLocal,
					</cfif>
					<!--- (sum(d.DDtotallin*(dto.doctotal - #rsTotalRet.totalRet#/*  c.EDmontoretori */)/dto.doctotal) * sum(CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10))) / sum(c.EDsaldo - #rsTotalRet.totalRet#/*  c.EDmontoretori */))  * cast(dp.TESDPtipoCambioSP as decimal(18,10)) AS MontoBaseLocal, --->
					(sum(CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10))))  * cast(dp.TESDPtipoCambioSP as decimal(18,10)) AS MontoBaseLocal,
				   ...
				   <cfif rsDetalleSolPag.TESDPmoduloOri EQ 'CPFC'>
				   		<cfif isDefined("lVarOPIdAnular")>
						   	CASE WHEN i.Iporcentaje > 0 
							THEN (((SUM(CAST(b.MontoBaseCalc AS decimal(18, 10))) * CAST(i.Iporcentaje AS decimal(18, 10)) / 100) 
								* ((SUM(CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10))) * 100) 
								/ SUM(CAST(c.Dtotal AS decimal(18, 10))))) / 100)
				           	ELSE 0
							END AS MontoCalculado,
						<cfelse>
							(sum(d.DDtotallin) * sum(CAST(dp.TESDPmontoAprobadoOri AS decimal(18, 10))) / sum(c.EDsaldo - c.EDmontoretori) * CAST(i.Iporcentaje AS decimal(18, 10))/100) MontoCalculado,
				      	</cfif>
				   <cfelse>
			...
			<cfif rsDetalleSolPag.TESDPmoduloOri EQ 'CPFC'>
				...
				INNER JOIN (select distinct Ecodigo, Icodigo, IDdocumento, DDtotallin from DDocumentosCP) d ON c.IDdocumento = d.IDdocumento
					AND c.Ecodigo = d.Ecodigo
				INNER JOIN (
								select  Ecodigo, IDdocumento, sum(DDtotallin) doctotal 
								from DDocumentosCP
								group by Ecodigo, IDdocumento
				) dto ON c.IDdocumento = dto.IDdocumento
					AND c.Ecodigo = d.Ecodigo
			</cfif>
		...
		<cfif rsDATADIOT.recordCount GT 0 AND NOT isDefined("lVarOPIdAnular")>
			<cfif rsDATADIOT.TESDPmoduloOri EQ 'CPFC'> <!---- SOLICITUDES DE PAGO  DE DOCUMENTOS DE CXP --->
				...
				<cfloop query="rsDATADIOT">
					...
					<!--- <cfdump  var="#rsPagoPar#"> --->
					<cfquery name="rsPago" datasource="#session.dsn#">
						  select 
						  DIOTid,  Ecodigo,	  SNcodigo,	  Oorigen,  TablaOrigen,	
						  CampoId,	  Icodigo,	  IPeriodo,	  IMes,	  OMontoBaseIVA,	
						  isnull(OIVAAcreditable,0) as OIVAAcreditable,	  OIVAPagado,	  LMontoBaseIVA,isnull(LIVAAcreditable,0) as LIVAAcreditable  ,  LIVAPagado,	
						  TipoCambioIVA,	  Usucodigo,	  BMUsucodigo,	  ts_rversion,	  Documento,	
						  DIOTivacodigo,  Pagado, Bid,	MontoRetIVA,DocRefPago,  SaldoLin
						from DIOT_Control
						where CampoId = <cfoutput>'#rsDATADIOT.IDdocumento#'</cfoutput>
						and Ecodigo = #session.ecodigo#
						and Icodigo = <cfoutput>'#rsDATADIOT.IcodigoCP#'</cfoutput>
					...
					<cfif rsPagoPar.recordcount GT 0>
						<cfquery name="XX" datasource="#session.dsn#">
							...
							ts_rversion,DIOTivacodigo,Documento,Pagado,MontoRetIva,DocRefPago)
								...
								getdate(), DIOTivacodigo, Documento = ltrim(rtrim(Documento)), 1 , 
								/* (#rsGetUltimoRegistroDIOT.OIVAAcreditable#-#rsDATADIOT.MontoCalculado#), */
								<cfif rsDATADIOT.DIOTivacodigo eq 1 >
									(select Sum(abs(dp.TESDPmontoAprobadoOri)) 
										from TESdetallePago dp
										inner join Retenciones r
											on dp.Rcodigo = r.Rcodigo
											and dp.EcodigoOri = r.Ecodigo
										where dp.TESDPidDocumento = #rsDATADIOT.IDDocumento#
										and dp.RlineaId is not null
										/* se debe filtrar por las retenciones que afectan IVA */
										and dp.TESOPid = #arguments.TESOPid#
										and UPPER(r.Rdescripcion) like Upper('%IVA%')),
								<cfelse>
									0,
								</cfif>
									/*  */
					...
					<cfif isdefined('rsPagoPar') and rsPagoPar.recordcount EQ 0 and  isdefined('rsPago') and rsPago.recordcount GT 0>
						<cfquery datasource="#session.dsn#">
								UPDATE DIOT_Control
								SET
