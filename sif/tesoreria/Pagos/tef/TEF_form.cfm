						...
						<cfif LvarTESTGdatos NEQ "">
							...
							<tr>
								...
								<cfif rsForm.TESTLestado EQ "0">
                                    <cfif listLen(rsForm.TESTLdatos) LTE rsSQL.currentRow>
	                                    <cfset LvarDato = rsSQL.TESTGvalor>
                                    <cfelse>
	                                    <cfset LvarDato = listGetAt(rsForm.TESTLdatos,rsSQL.currentRow)>
                                    </cfif>
									<td valign="top"><input type="text" name="TESTLdatos" size="20" value="#LvarDato#"></td>
								<cfelse>
									<td valign="top">
                                    <cfif listLen(rsForm.TESTLdatos) LTE rsSQL.currentRow>
	                                    #rsSQL.TESTGvalor#
								...
								<cf_tab text="Parámetros" id=2>
									...
									<div style="width:475px; border:1px solid;">
										<table>
                                            ...
                                            <!---Se comenta el FMT01CODemail ya que en definicion de medio de pago ya no se elige que tipo de formato sino que solamente 
											se indica si envia o no--->
											<!---<tr>
												<td>&nbsp;</td>
											...
											<tr>
												<td>&nbsp;</td>
												<td><strong>Envia Correo al Beneficiario:</strong></td>
												<td><cfif rsForm.TESenviaCorreo eq 1>SI<cfelse>NO</cfif></td>
												<td>&nbsp;</td>
											</tr>
										</table>
									</div>
			...
			<cfquery name="rsSQL" datasource="#session.dsn#">
				select 	mp.TESMPcodigo, 
						case tl.TESTMPtipo
							...
							when 4 then 'Tarjeta Crédito Empresa'
						end as tipoLote
				  from TEStransferenciasL tl
					inner join TESmedioPago mp
						 on mp.TESid		= tl.TESid
...
<script type="text/javascript">
	function funcImprimir(){
		<cfif rsForm.recordcount EQ 0>
		  ...
		  var url = '/cfmx/sif/Utiles/genImpr.cfm?archivo=<cfoutput>#JSStringFormat("/sif/tesoreria/Pagos/tef/imprTRE.cfm")#&imprime=1&TESTLid=#form.TESTLid#<cfif isdefined("url.btnSeleccionarOP")>&btnSeleccionarOP=1</cfif></cfoutput>';
