<cfcomponent output="no">
	...
	<cffunction name="Generar" output="no" access="public" returntype="struct">
		...
		<cfset LvarResult.FileName = "ENV-BNCR-interno" & LvarConsecutivo &".env">
		...
		insert into #Datos# (detalle)
			select 
				...
				'008275' 	#_Cat# 																			<!--- # de cliente siempre es 008275 --->
       ...
       <cfif not isnumeric(rsCuenta.Cuenta)>
        ...
        <cfloop query="rsTESTD">
        <cfset  TESP = replace("#rsTESTD.TESTPcuenta#","-","","All")>
            <cfquery name="cuentas" datasource="#session.dsn#">
                insert into #Datos# (detalle)
                select 	
                    '3'  #_Cat#          																	<!--- 3=Credito    ---->
                    <cf_dbfunction name="sPart"	 args="'#TESP#',6,3" datasource="#session.dsn#">  #_Cat#   	<!----Oficina de apertura de cta  substring(c.DEcuenta,6,3)  ---->
                    <cf_dbfunction name="sPart"	 args="'#TESP#',1,3" datasource="#session.dsn#">  #_Cat#	<!--- 100=Cta Corr   200=Cta Ahorros  300=Cta Electr-- 01=Colones   02=Dolares  ---->
                    <cf_dbfunction name="sPart"	 args="'#TESP#',4,2" datasource="#session.dsn#">  #_Cat# 	<!--- 01=Colones   02=Dolares  ---->
                    <cf_dbfunction name="sPart"	 args="'#TESP#',9,6" datasource="#session.dsn#">  #_Cat# 	<!--- Cta empl. sin digito verificador  ---->
                    <cf_dbfunction name="sPart"	 args="'#TESP#',15,1" datasource="#session.dsn#">  #_Cat#	<!--- Cta empl (solo el digito verificador)   ---->
            ...
            <cfset LvarSumaCuenta= #LvarSumaCuenta# + mid(#TESP#,9,6)>
		</cfloop>	
