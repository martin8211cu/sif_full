<!---------
	...
	Fecha de modificación:	25 de octubre del 2015
	Motivo:			Se cambió el soinPrintDocs.ocx por soinPrintDocs2
----------->

<cfset LvarOCX				= StructNew()>
<cfset LvarOCX.clsid		= "0FC59281-AD36-4FA6-A1AE-525E7D740FCC">
<cfset LvarOCX.version		= "1.0.98">
<cfset LvarSinLoteImpresion	= "N">
		...
		<cfelseif rsForm.TESCFLestado EQ "0">
			...
			<tr>
				...
				<cfif rsTESOP.cantidad GT 0>
					...
					<cfif LvarSinLoteImpresion eq "N">
				...
				<cfif NOT LvarReimpresionEspecial>
                    ...
                    <input type="button" value="Reporte" onClick="funcImprimir()" tabindex="1">
				</td>
			</tr>
			...
			<cfelse>
				<tr>
					<td colspan="2" align="center">
						...
						<cfif rsForm.TESMPcodigo EQ "CHK_NACION">
							<OBJECT ID="Imprime1"
							...
							<cfset LvarEsNacion="&NACION=1">
							<iframe width="0" height="0" id="ifrDatos" src="impresionCheques_datos.cfm?TESCFLid=#form.TESCFLid#&FMT01COD=#rsForm.FMT01COD##LvarEsNacion#<cfif LvarReimpresionEspecial>&Especial=1</cfif>" 
									style="display:none;">
							</iframe>
						...
						<cfelse>
							...
							************** 
							...
							<!--- El idioma se define en la Cuenta Bancaria y es para el monto en letras --->
							<cfquery name="rsIdioma" datasource="#session.dsn#">
								select 
									case CBidioma 
										when 1 then 1 		-- Ingles US:	1,000,000,000 one billion
										when 2 then 2 		-- Ingles UK:	1,000,000,000 one thousand millons
										else 0 				-- Español:		1,000,000,000 mil millones
									end as CBidioma
							...
							<cfif isdefined('Reimpresion')>
								<cfset LvarDES = "Lote de Reimpresion de Cheques N.#form.TESCFLid#">
							<cfelse>
								<cfset LvarDES = "Lote de Impresion de Cheques N.#form.TESCFLid#">
							</cfif>

							<cfif NOT listFind("0,1,2",rsIdioma.CBidioma) >
								<cfset LvarIdioma = "0">
							<cfelse>
								<cfset LvarIdioma = rsIdioma.CBidioma>
							</cfif>

							<!--- Datos a sustuir en el SQL del tipo de formato de impresion --->
							<cfset LvarDatosSQL = structNew()>
							<cfset LvarDatosSQL.TESid	= session.Tesoreria.TESid>
							<cfset LvarDatosSQL.TESCFLid	= url.TESCFLid>
							<cfset LvarDatosSQL.Usucodigo	= session.Usucodigo>

							<cfinvoke 	component="WS.spd2.soinPrintDocs2" 
										method="enviar_spd2"

										DESCRIPCION = "#LvarDES#"
										ARCHIVO		= "CHEQUES"
										FMT01COD	= "#rsForm.FMT01COD#"
										FMT01KEY	= "TESCFDNUMFORMULARIO"
										Cantidad	= "#rsLote.cantidad#"
										Idioma		= "#rsIdioma.CBidioma#"
										Especial	= "#LvarReimpresionEspecial#"
                                        Nacion		= "#rsForm.TESMPcodigo EQ "CHK_NACION"#"
										Datos_SQL	= "#LvarDatosSQL#"
							>
						</cfif>
					</td>
				</tr>		  
...
<script language="javascript" type="text/javascript">
	...
	function funcImprimir(){
		...
		<cfif rsForm.recordcount EQ 0>
		  ...
		  var url = '/cfmx/sif/Utiles/genImpr.cfm?archivo=<cfoutput>#JSStringFormat("/sif/tesoreria/Pagos/cheques/imprCheque.cfm")#&imprime=1&TESCFLid=#form.TESCFLid#<cfif isdefined("Reimpresion")>&Reimpresion=1</cfif></cfoutput>';
