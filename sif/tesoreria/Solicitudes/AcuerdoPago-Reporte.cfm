...
<cfquery name="rsLineasD" datasource="#session.dsn#">
	Select
		...
		<!--- Monto Bruto --->
		, case when dp.TESDPtipoDocumento in (0,5) AND dp.TESDPmontoSolicitadoOri < 0 then 
				0 
		else 
				case when dp.TESDPmontoSolicitadoOri > 0 then
		...
		*case when sp.TESSPtipoDocumento = 5 then 	tessptipocambioorimanual
		...
		<!--- Deducciones de Retencion (2%) --->		
		, (coalesce(dp.Rmonto,0) + coalesce((case when dp.TESDPmontoSolicitadoOri < 0 and dp.Rcodigo = '02' then
																abs(dp.TESDPmontoSolicitadoOri)
															else
																0
															end),0) )
		*case when sp.TESSPtipoDocumento = 5 then 	tessptipocambioorimanual
		...
		*case when sp.TESSPtipoDocumento = 5 then 	tessptipocambioorimanual
		...
		<!--- Deducciones Otras --->
		, coalesce(case when dp.TESDPtipoDocumento in (0,5) AND dp.TESDPmontoSolicitadoOri < 0 then 
						-dp.TESDPmontoSolicitadoOri
					end,0)
		+ coalesce((case when dp.TESDPmontoSolicitadoOri < 0 and dp.Rcodigo <> '02' then
						-dp.TESDPmontoSolicitadoOri
					else
						0
					end),0)
		...
		*case when sp.TESSPtipoDocumento = 5 then 	tessptipocambioorimanual
		...
		when 'E' then 'EMBARGO '	#_CAT# CPCdocumento #_CAT# ' a favor de ' #_CAT# (select SNnombre from SNegocios where SNid = cpc.SNidDestino)
			...
			case when ((sp.TESSPtipoDocumento in (1,5,10) and (sp.TESSPtipocambioOrimanual =1 or dp.TESDPtipocambioori=1)))	 then ''
				 ...
				 then '('#_CAT#  <cf_dbfunction name="to_number" args="sp.TESSPtipocambioOrimanual" dec="2"> #_CAT#')' 	
    				            				else
					case when sp.TESSPtipoDocumento = 5 then 
								'('#_CAT#  <cf_dbfunction name="to_number" args="sp.TESSPtipocambioOrimanual" dec="2"> #_CAT#')' 								
							else 
								'('#_CAT#  <cf_dbfunction name="to_number" args="dp.TESDPtipocambioori" dec="2"> #_CAT#')' 
							end
				end as tipocambio
...
<cfsavecontent variable="printEnc">
	<tr>
		...
		<tr class="fontTTITULO">
			...
			<td align="center" nowrap><strong>FACTURA</strong></td><td>#RepeatString('&nbsp;', CantEsp)#</td>
			<td align="center" nowrap><strong>MONEDA</strong></td><td>#RepeatString('&nbsp;', CantEsp)#</td>
	...
	<cfset counCeesion = 0>		
		<cfloop query="rsLineasD">
			...
			<cfelse>
				...
				<cfset LvarLineasdocumentos = LvarLineasdocumentos+1>
			</tr>
		...
		<tr class="fontTTITULO">
			...
			<td nowrap colspan="4"><strong>#LvarLineasdocumentos# Documentos.        TOTALES:</strong></td>
		...
		<cfif LvarLineasRecorridas GTE 30>	
			<tr class="pageEnd"></tr>	
			<tr><td>#printEnc#</td></tr>
			<tr class="fontTTITULO">
				<td colspan="14" nowrap>&nbsp;</td>
