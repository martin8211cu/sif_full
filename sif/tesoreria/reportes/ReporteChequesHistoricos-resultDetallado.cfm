...
<cfquery name="rsReporte" datasource="#session.DSN#">
	 select b.Bdescripcion,
			...
			sn.SNnumero,
			sn.SNnombre,
			cf.TESCFDnumFormulario as consecutivo, 
			coalesce(( case cf.TESCFDestado when 2 then  cf.TESCFDentregadoFecha when 3 then cf.TESCFDfechaAnulacion else cf.TESCFDfechaEmision  end) , cf.TESCFDfechaEmision)  as fecha,
			cc.Cformato as formato,
			op.TESOPtotalPago as monto,
			case cf.TESCFDestado when 0 then 'En preparación'  when 1 then 'Impreso'  when 2 then 'Entregado' when 3 then 'Anulado' end as estado,
		...
		order by b.Bdescripcion, cb.CBcodigo, sn.SNnumero, cf.TESCFDnumFormulario 
</cfquery>

<cfif rsReporte.recordcount gt 0 >
		<cfif isdefined("url.CBid") and len(trim(url.CBid))>
			<cfquery name="rsCuenta" datasource="#session.DSN#">
				select CBcodigo #LvarCNCT# '-' #LvarCNCT# CBdescripcion as Cuenta
				from CuentasBancos
    ...
    <cfif rsactivatejsreports.valParam EQ 1 AND coldfusionV EQ 2018>
	  ...
	  <cf_js_reports_service_tag queryReport = "#rsReporte#" 
		isLink = False 
		typeReport = #typeRep#
		fileName = "tesoreria.reportes.reporteChequesHistoricos"
		headers = "empresa:#session.Enombre#"/>
	<cfelse>
		<cfreport format="#url.formato#" template= "reporteChequesHistoricos.cfr" query="rsReporte">
