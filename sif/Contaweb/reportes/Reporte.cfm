<cfset contenidohtml = "">
<cfset tempfile_cfm = GetTempFile(GetTempDirectory(),'xxx') >
<cfset tempfile_xls = "#GetTempDirectory()#tmp_#session.usuario#.xls">
<cffile action="write" file="#tempfile_cfm#" output="<!--- Generado #dateformat(now(),"DD/MM/YYYY")# #timeformat(now(),"HH:MM:SS")# --->" nameconflict="overwrite">
<cffile action="write" file="#tempfile_xls#" output="<!--- Generado #dateformat(now(),"DD/MM/YYYY")# #timeformat(now(),"HH:MM:SS")# --->" nameconflict="overwrite">
...
<cfset fnGraba(micontenido, false, false)>
<!--- *********************************************************************************************** --->
<cfsavecontent variable="micontenido">
<style>
H1.Corte_Pagina
...
Reporte Detallado por Consecutivo
...
<!--- *********************************************************************************************** --->
<cfset formatoinicial = "">
<cfif TipoFormato EQ 1>
<cfset registrosxcontext = 1000>
<cfelseif TipoFormato EQ 2>
<cfset registrosxcontext = 600>
<cfelseif TipoFormato EQ 3>
<cfset registrosxcontext = 400>
<cfelse>
<cfset registrosxcontext = 5000>
</cfif>
<!--- 	Cacula la cantidad de ciclos que tiene que recorrer 
cada ciclo esta compuesto de  'registrosxcontext'  registros 	--->
...
<!--- Para no incluir movimientos en cero --->
<cfif isdefined("ID_NoIncluir")>
	<cfset condiciones ="rsCuentasContables.Debitos NEQ 0.00 or rsCuentasContables.Creditos NEQ 0.00">
<cfelse>
	<cfset condiciones ="rsCuentasContables.SaldoIncial NEQ 0.00 or rsCuentasContables.Debitos NEQ 0.00 or rsCuentasContables.Creditos NEQ 0.00">
</cfif>

<cfif #evaluate(condiciones)#>

	<!--- Para no incluir movimientos en cero 
	<cfif rsCuentasContables.SaldoIncial NEQ 0.00 or rsCuentasContables.Debitos NEQ 0.00 or rsCuentasContables.Creditos NEQ 0.00>
	--->
	...
	<td nowrap align="right" class="xl27">#LSCurrencyFormat(rsCuentasContables.SaldoIncial,'none')#</td>
	<td nowrap align="right" class="xl27">#LSCurrencyFormat(rsCuentasContables.Debitos,'none')#</td>
	<td nowrap align="right" class="xl27">#LSCurrencyFormat(rsCuentasContables.Creditos,'none')#</td>
	<td nowrap align="right" class="xl27">#LSCurrencyFormat(rsCuentasContables.SaldoIncial + rsCuentasContables.Debitos - rsCuentasContables.Creditos,'none')#</td>
	<cfelse>
	<td nowrap class="xl28">#rsCuentasContables.FormatoCuenta#</td>
	<td nowrap class="xl28" >#rsCuentasContables.Descripcion#</td>
	<td nowrap align="right" class="xl28">#LSCurrencyFormat(rsCuentasContables.SaldoIncial,'none')#</td>
	<td nowrap align="right" class="xl28">#LSCurrencyFormat(rsCuentasContables.Debitos,'none')#</td>
	<td nowrap align="right" class="xl28">#LSCurrencyFormat(rsCuentasContables.Creditos,'none')#</td>
	<td nowrap align="right" class="xl28">#LSCurrencyFormat(rsCuentasContables.SaldoIncial + rsCuentasContables.Debitos - rsCuentasContables.Creditos,'none')#</td>
	</cfif>
	</tr> 
	</cfsavecontent>
	<cfset fnGraba(micontenido, true, false)>
...
<cfif rsCuentasContables.Detalle EQ 1 and TipoFormato NEQ 4>
	<cfquery name="rsDetalle" datasource="#session.Conta.dsn#">
		...
		<cfcase value="1">
			...
			from 	#archivotbl# c, CGT002 t <cf_dbforceindex name="CGT00202">
		</cfcase>
		<cfcase value="2">
			select
			<cfif incsucursal EQ 1>
			...
			from 	#archivotbl# c, CGM007 t <cf_dbforceindex name="CGM00700">
		</cfcase>
		<cfcase value="3">
			select
			<cfif incsucursal EQ 1>
...
<cfset formatoinicial = rsCuentasContables.FormatoCuenta> 
</cfloop>
</cfloop>
<!--- *********************************************************************************************** --->
<cfsavecontent variable="micontenido">
...
<tr><td>&nbsp;</td></tr>
<cfif isdefined("ID_NoIncluir")>
<tr><td><font size="-2">Nota: El Reporte no incluye cuentas con movimientos mensuales en cero</font></td></tr>
</cfif>
</table>
</cfsavecontent>
