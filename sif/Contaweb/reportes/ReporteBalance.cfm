...
<cfinclude template="ValidaRangosdehora.cfm">
...
<cftry> 
			...
			<cfif primernivel GT 0>
				...
				<cfquery name="rsCuentas1" datasource="#session.Conta.dsn#">
					...
					select 	#llave#,
						...
						c.CGM1ID as CuentaDetalle, 
						1 as Detalle,
						#arreglo[5]# as niveltotal
					from CGM001cubo c1 <cf_dbforceindex name="CGM001cubo02">
						INNER JOIN CGM001 ct <cf_dbforceindex name="CGM00100">
							on  ct.CGM1ID = c1.CGM1ID 
							and ct.CGM1CD like <cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(cDetalle)#%">
						INNER JOIN CGM001cubo c <cf_dbforceindex name="CGM001cubo01">
							on  c.CGM1ID = ct.CGM1ID 
							and c.nivelcubo = #arreglo[4]#
						INNER JOIN CGM001 d <cf_dbforceindex name="CGM00100">
							on d.CGM1ID = c.ctanivel 
			...
			<cfelse>
				...
				<cfquery name="rsCuentas" datasource="#session.Conta.dsn#">
					...
					select 	
						...
						1 as Detalle,
						#arreglo[5]# as niveltotal
					from CGM001cubo c <cf_dbforceindex name="CGM001cubo02">
						INNER JOIN CGM001 ct <cf_dbforceindex name="CGM00100">
	...
	<!--- <cfdump var="#LarrCuentas#">
	<cfset niveltotal = arreglo[5]>
	<cfset niveldetalle = arreglo[4]>	 --->
	<!--- <cfabort>  --->
	...
	<cfif niveltotal LT niveldetalle>
		<cfquery name="rsIncluyeCtasCorte" datasource="#session.Conta.dsn#">
			...
			select 	distinct #llave#,
				...
				0 as Detalle,
				#niveltotal# as niveltotal
			 from #archivotbl# ctas <cf_dbforceindex name="#archivotbl#00">
				inner join CGM001cubo c <cf_dbforceindex name="CGM001cubo01">
					inner join CGM001 cm <cf_dbforceindex name="CGM00100">
	...
	<cfquery name="rsIncluyeCtasMayor" datasource="#session.Conta.dsn#">
		...
		select 	distinct #llave#,
		 ...
		 from #archivotbl# ctas <cf_dbforceindex name="#archivotbl#00">
			...
			inner join CGM001 cm <cf_dbforceindex name="CGM00103">
			...
			select  
					...
					coalesce(sum((
						...
						<cfif len(strAsientos) GT 0>
							from CGM007 s <cf_dbforceindex name="CGM00700">
						<cfelse>
							from CGM004 s <cf_dbforceindex name="CGM00400">
						</cfif>
