		...
		<cf_web_portlet_start titulo="<cfoutput>#nav__SPdescripcion#</cfoutput>"><cfoutput>#pNavegacion#</cfoutput>
			...
			<script language="JavaScript1.2" type="text/javascript">
				...
				function funcNuevo(){
					...
					document.lista.action="criteriosDepto.cfm";
				}

				function filtrarlista(){
					document.lista.action="criteriosDepto-lista.cfm";
					return true;
				}
...
<form style="margin:0" action="criteriosDepto.cfm" method="post" name="lista" id="lista" >
<table width="100%" cellpadding="0" cellspacing="0">
	...
	<cfquery name="rsLista" datasource="#session.DSN#">
		...
		<!---
		<cfif isdefined("form.filtro_DGCDdescripcion") and len(trim(form.filtro_DGCDdescripcion))>
			and upper(b.DGCDdescripcion) like '%#ucase(trim(form.filtro_DGCDdescripcion))#%'
		</cfif>
		--->
	...
	<tr>
		<td>
			...
			query="#rsLista#"
			desplegar="criterio,PCDdescripcion"
			etiquetas="Criterio, Departamento"
			formatos="S,S"
			align="left,left"
			ira="criteriosDepto.cfm"
			nuevo="criteriosDepto.cfm"
