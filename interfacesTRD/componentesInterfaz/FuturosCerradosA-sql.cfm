<!--- Archivo    :  futuroscerradosA-sql.cfm
	  --->
...
<cfquery datasource="sifinterfaces">
	delete from #LvarDsource#futuroscerradosPMI where sessionid = #session.monitoreo.sessionid#
...
create table sif_interfaces..futuroscerradosPMI (fecharegistro date null,sessionid numeric null,
		...
		order_type_code varchar(8) null, realized_profits_detail_num integer null,
...
<!--- Crea query de futuros cerrados  --->
<cfquery datasource="preicts">
	insert #LvarDsource#futuroscerradosPMI (fecharegistro, sessionid, 
		...
		mtm_pl, currency_code, creation_date, market_day, realized_profits_detail_num)
	select getdate(), #session.monitoreo.sessionid# as sessionid, 
		...
		rp.creation_date, rp.market_day, det.realized_profits_detail_num
	...
	and det.mtm_pl <> 0
	and rp.realized_profits_amount <> 0
	order by rp.market_day
</cfquery>
...
<cfquery datasource="#session.dsn#">
	UPDATE #LvarDsource#futuroscerradosPMI SET SNid = a2.SNid
	from #LvarDsource#futuroscerradosPMI a1, SNegocios a2 
...
<!--- subconcepto  --->
<cfquery datasource="preicts">
	UPDATE #LvarDsource#futuroscerradosPMI SET subconcepto = sb.subconcepto_id
	from tesoreria..subconceptos sb, tesoreria..rel_subconceptos_detalles r,
	     tesoreria..subconceptos_detalle s, #LvarDsource#futuroscerradosPMI a1
...
<!--- código de Concepto  --->
<cfquery datasource="#session.dsn#">
	UPDATE #LvarDsource#futuroscerradosPMI SET CodigoConcepto = a2.cuentac
	from #LvarDsource#futuroscerradosPMI a1, Conceptos a2 
	where a2.Ecodigo = #session.ecodigo#
...
<cfquery name="rsFuturos" datasource="sifinterfaces">
	select *
	from  #LvarDsource#futuroscerradosPMI where sessionid = #session.monitoreo.sessionid#
</cfquery>
...
<cfloop query="rsFuturos"> 
	...
	<cfset ws_key = "#rsFuturos.trade_num#FU_C#rsFuturos.fill_num##session.FechaFolio#">
	...
	<cfset ws_folio = "#rsFuturos.trade_num#FU_C#rsFuturos.fill_num##session.FechaFolio#">
	...
	<cfquery datasource="#session.dsn#">
		UPDATE #LvarDsource#futuroscerradosPMI
		...
		from #LvarDsource#futuroscerradosPMI 
		where realized_profits_detail_num = #rsFuturos.realized_profits_detail_num#
