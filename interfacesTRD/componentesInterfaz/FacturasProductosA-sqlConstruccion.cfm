...
<cfquery datasource="sifinterfaces">
	...
	ALTER TABLE sif_interfaces..TECompras add ta_item_num smallint int null
...
<cfquery datasource="preicts">
	select distinct oo.order_type_code, a2.trade_num, a2.order_num, a2.item_num, a2.order_num, a2.item_num,
		 a2.acct_ref_num, a2.acct_num, a1.trade_num as trade_num_CP, a1.order_num as order_num_CP,
		 a1.item_num as item_num_CP
	...
	from allocation_item a1
		...
		inner join trade_item a10
		on a10.trade_num = a2.trade_num
		and a10.order_num = a2.order_num
		and a10.item_num = a2.item_num
		and a10.p_s_ind = 'S'
...
<cfquery datasource="sifinterfaces">
	UPDATE sif_interfaces..TECompras SET ta_order_type_code = a2.order_type_code,
		   ...
		   ta_acct_ref_num = a2.acct_ref_num, ta_acct_num = a2.acct_num
	from sif_interfaces..TECompras a1, sif_interfaces..TETrabajo a2
...
<cfquery datasource="preicts">
	select oo.order_type_code, a2.acct_ref_num, a2.acct_num,a2.trade_num, a2.order_num, a2.item_num,
		   a2.actual_gross_qty, a2.actual_gross_uom_code, ti.cmdty_code,
...
<cfquery datasource="sifinterfaces">
	UPDATE sif_interfaces..TECompras SET tt_order_type_code = a2.order_type_code,
		   actual_gross_qty = a2.actual_gross_qty, actual_gross_uom_code = a2.actual_gross_uom_code,
		   cmdty_code = a2.cmdty_code, tt_acct_ref_num = a2.acct_ref_num, tt_acct_num = a2.acct_num
	from sif_interfaces..TECompras a1, sif_interfaces..TETrabajo a2
	where a1.trade_num = a2.trade_num_CP
</cfquery>
--->
...
<cfloop query="queryCompras"> 
	...
	<cfset LvarOCtipoIC = "C">
	<cfset LvarAlmacen = "">
	<cfset LvarAlm_trade_num = 0>
	<cfset LvarAlm_order_num = 0>
	<cfset LvarAlm_item_num = 0>
	<cfset LvarTransporte = "#queryCompras.c_orden#">
	<cfset LvarSocioAlloc = 0>
	<cfset LvarOrdenAlloc = "">
	<cfset LvarPorcentaje= 1>
	...
	<cfif queryCompras.ta_order_type_code EQ 'TRANSPRT'>
		...
		<cfquery name="rsTipo" datasource="preicts">
			select distinct oo.order_type_code, a2.acct_ref_num, a2.acct_num, a2.trade_num, a2.order_num,
				   a2.item_num, a2.actual_gross_qty, a2.actual_gross_uom_code, ti.cmdty_code
			from allocation_item a1
					inner join allocation_item a2
						on a2.alloc_num = a1.alloc_num
						 and a1.trade_num <> a2.trade_num
					...
					inner join trade_order oo
						on oo.trade_num = a2.trade_num
							 and oo.order_num = a2.order_num
				 where a1.trade_num= <cfqueryparam cfsqltype="cf_sql_integer" value="#queryCompras.ta_trade_num#">
		...
		<cfif rsTipo.recordcount EQ 1>
			...
			<cfif rsTipo.order_type_code EQ 'STORAGE'>
				...
				<cfset LvarAlm_trade_num = "#rsTipo.ta_trade_num#">
				<cfset LvarAlm_order_num = "#rsTipo.ta_order_num#">
				<cfset LvarAlm_item_num = "#rsTipo.ta_item_num#">
		...
		<cfif rsTipo.recordcount GT 1>
			<!--- Determinar si hay almacén --->
			<cfquery query="rsAlmac" dbtype="query">
				select * from rsTipo where order_type_code = 'STORAGE'
			</cfquery>
			<cfif rsAlmac.recordcount GT 0>
				<!--- Determinar si hay blending --->
				<cfquery query="rsBlend" dbtype="query">
					select * from rsTipo where cmdty_code = "#queryCompras.c_producto#"
				</cfquery>
				<cfif rsBlend.recordcount EQ 0>      <!---  Es Blending   --->
					<cfset VolumenTotal = 0>
					<cfset VolumenStorage = 0>
					<cfloop query="rsTipo">
						<!--- Conversión a unidades del Artículo --->
						<cfif #Rtrim(queryCompras.UcodigoArt)# NEQ "#Rtrim(rsTipo.actual_gross_uom_code)#">
							<cfquery name="rsVerifica" datasource="#session.dsn#">
								select Ucodigo, CUAfactor
								from ConversionUnidadesArt
								where Ecodigo  = #session.ecodigo#
								  and Aid = #LvarAid#
								  and Ucodigo = '#Rtrim(rsTipo.actual_gross_uom_code)#'
							</cfquery>
							<cfif rsVerifica.recordcount GT 0>
								<cfset VolumenConv = rsVerifica.CUAfactor * rsTipo.actual_gross_qty>
								<cfset VolumenTotal = VolumenTotal + VolumenConv>
								<cfif rsTipo.order_type_code='STORAGE'>
									<cfset VolumenStorage = VolumenStorage + VolumenConv>
								</cfif>
							</cfif>
						<cfelse>
							<cfset VolumenTotal = VolumenTotal + rsTipo.actual_gross_qty>
							<cfif rsTipo.order_type_code='STORAGE'>
								<cfset VolumenStorage = VolumenStorage + VolumenConv>
							</cfif>
						</cfif> 
					...
					<cfset LvarPorcentaje = (VolumenStorage / VolumenTotal)>
				<cfelse>										<!--- No Es Blending   --->
					<cfquery query="rsAlmac" dbtype="query">
						select * from rsTipo where order_type_code = 'STORAGE' and 
									  cmdty_code = '#queryCompras.c_producto#'
					</cfquery>
					<cfif rsAlmac.recordcount GT 0>							
						<cfset VolumenTotal = 0>
						<cfset VolumenStorage = 0>
						<cfloop query="rsTipo">
							<cfif rs.Tipo.cmdty_code = "#queryCompras.c_producto#">
								<!--- Conversión a unidades del Artículo --->
								<cfif #Rtrim(queryCompras.UcodigoArt)# NEQ "#Rtrim(rsTipo.actual_gross_uom_code)#">
									<cfquery name="rsVerifica" datasource="#session.dsn#">
										select Ucodigo, CUAfactor
						...
						<cfset LvarPorcentaje = (VolumenStorage / VolumenTotal)>
					<cfelse>							
						<cfquery query="rsProd" dbtype="query">
							select * from rsTipo where order_type_code = 'PHYSICAL' and 
										  cmdty_code = '#queryCompras.c_producto#'
						</cfquery>
				...
				</cfif>   <!--- if del rsBlend.recordcount --->
			</cfif>   <!--- if del rsAlmac.recordcount	--->		
		</cfif>
	</cfif>
	...
	<cfif LvarBanderaErrores_registro EQ false>
		...
		<cfelse>
			<cfif queryCompras.c_tipo_folio EQ "NC">
				<cfset LvarCantidad = (rsVerifica.OCTPcantidadTeorica + (ws_f_volumen * -1))>
				<cfif LvarCantidad EQ 0>
					<cfset LvarCantidad = 1>
				</cfif>
