...
create table sif_interfacesser..nofactFletesPMI (fecharegistro date null,sessionid numeric null,
...
<cfquery datasource="preicts">
	...
	select distinct getdate(), #session.monitoreo.sessionid# as sessionid,  'N',
		...
		ti.price_curr_code, a3.p_s_ind, a3.avg_price
	from cost co
...
<cfloop query="rsNoFact"> 
	...
	<cfif rsNoFact.p_s_ind EQ "P">	
		<cfset rsCostos = OGeneralProcA.ConsultaCostosNoFactFletes(rsNoFact.trade_num, rsNoFact.order_num,
					 rsNoFact.item_num,'C',rsNoFact.cmdty_code, rsNoFact.tipoconsulta, vFechaF)>
	<cfelse>
		<cfset rsCostos = OGeneralProcA.ConsultaCostosNoFactFletes(rsNoFact.trade_num, rsNoFact.order_num,
					 rsNoFact.item_num,'V',rsNoFact.cmdty_code, rsNoFact.tipoconsulta, vFechaF)>
	</cfif>
	...
	<cfif rsCostos.recordcount GT 0>
		...
		<cfquery datasource="sifinterfaces">
			UPDATE #LvarDsource#nofactFletesPMI SET montocosto = #rsCostos.monto#, PosNeg = '#ws_pos_neg#'
	...
	<cfset ws_folio = "#rsNoFact.acct_ref_num##session.FechaFolio#">
	...
	<cfquery datasource="sifinterfaces">
		...
		values(getdate(), #session.monitoreo.sessionid#,
			...
			0.00, null, null,
			#numberformat(rsCostos.monto,"9.99")#,
			null, null,
			null, null, null, null
			)
	</cfquery>
