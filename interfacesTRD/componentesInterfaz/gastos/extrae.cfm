...
and exists (select 1 from #preictsdb#..PmiFoliosDetailS c where c.i_folio = a.i_folio and c.i_anio = a.i_anio )
...
a.Orden_Final, case when a.Codigo_Impuesto like 'XXX' then case when exists (select 1 from ##GADventasDis b where a.ID = b.ID and isnull(b.iva,0) > 0) then 'IVA15' else null end else a.Codigo_Impuesto end,null,0, case when a.Movimiento_Destino in ('D','R') then null else a.Orden_Final end,
...
0, null, '0',a.voucher_creation_date,null, 
null, case when a.Codigo_Impuesto like 'XXX' then case when exists (select 1 from ##GADsernoalloc b where a.ID = b.ID and isnull(b.iva,0) > 0) then 'IVA15' else null end else a.Codigo_Impuesto end,null,0, null,
...
null, null,0,'',0,
0, null, '0',a.voucher_creation_date,null, 
a.c_orden, case when a.Codigo_Impuesto like 'XXX' then case when exists (select 1 from ##GADalmacenDis b where a.ID = b.ID and isnull(b.iva,0) > 0) then 'IVA15' else null end else a.Codigo_Impuesto end,null,0, (select min(acct_ref_num) from #preictsdb#..trade b where b.trade_num = a.cost_owner_key6),
null,convert(money,round(a.Amortiza,2)),null,
...
null, null,0,'',0,
0, null, '0',a.voucher_creation_date,null, 
a.Orden_Final, case when a.Codigo_Impuesto like 'XXX' then case when exists (select 1 from ##GADfletes2 b where a.ID = b.ID and isnull(b.iva,0) > 0) then 'IVA15' else null end else a.Codigo_Impuesto end,null,0, case when a.Movimiento_Destino in ('D','R') then null else a.Orden_Final end,
null,convert(money,round(a.Amortiza,2)),null,
a.Cuenta,
case when a.Tipo like 'O' then (select min(OCTtipo) from #minisifdb#..OCtransporte dx, #minisifdb#..OCtransporteProducto cx, #minisifdb#..OCordenComercial ax where a.Orden_Final = ax.OCcontrato and ax.OCid = cx.OCid and cx.OCTid = dx.OCTid
...
null, null,0,'',0, 
0, null, '0',a.voucher_creation_date,null, 
a.c_orden,case when a.Codigo_Impuesto like 'XXX' then case when exists (select 1 from ##GADDis b where a.ID = b.ID and isnull(b.iva,0) > 0) then 'IVA15' else null end else a.Codigo_Impuesto end,null,0, null,
null,convert(money,round(a.Amortiza,2)),null,
		...
		<cfif rsGASTOID10.recordcount GT 0>
			<cfloop query="rsGASTOID10">
				...
				<cfif trim(rsGASTOID10.TipoItem) EQ "S">
					<!---Verifica que el Concepto de Servicio Exista--->
					<cfset ga_Concepto = rsGASTOID10.CodigoItem>
					<cfquery name="rsVerifica" datasource="sifinterfaces">
