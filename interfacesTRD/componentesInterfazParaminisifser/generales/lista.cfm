	...
	<table align="center" border="0" cellspacing="1" cellpadding="2" width="100%">
		...
		onmouseout="this.className='#className#';" style="cursor:default">
			...
			<cf_inputNumber name="factor#CurrentRow#"  value="#NumberFormat(factor, '0.0000')#"
				...
				onchange="multiplicar(this, '#NumberFormat(PrecioTotal, ',0.00')#', #CurrentRow#)" >
			</td>
			</cfif>
...
<cfoutput>
	...
	function guardar() {
		...
		window.open('index.cfm?botonsel=btnGuardarFactor&StartRow=#StartRow#&modo_errores=#modo_errores#&modificado=' + escape(modificado.join()), 'frguardar' );
		return modificado.length;
	}
	
	function navegar(n){
	<cfif modo_errores neq '1'>
		location.href = 'index.cfm?botonsel=btnLista&StartRow=' + n;
	<cfelse>
		location.href = 'index.cfm?botonsel=btnErrores&StartRow=' + n;
	</cfif>
	}
