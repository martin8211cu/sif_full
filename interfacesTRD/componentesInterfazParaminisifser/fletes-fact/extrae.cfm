...
from ##FLETES_IE10 a, #preictsdb#..voucher_cost b, #preictsdb#..cost c
	...
	and c.cost_code in ('CHARHIRE','FREIGHTO','FREIGHTB')	
...
update ##FLETES_IE10
set cost_code = (select min(cost_code) from  #preictsdb#..cost c where 	b.cost_num=c.cost_num and
	c.cost_code in ('CHARHIRE','FREIGHTO','FREIGHTB')	
	and c.cost_status in ('PAID','VOUCHED')
...
where a.cost_owner_key6 is null and b.acct_ref_num = c_orden
...
then 
	Amortiza + round(isnull(abs((select round(sum(Amortiza),2) from ##FLETES_IE104 b where b.ID = a.ID group by ID) - 
(select round(f_importe_total,2) from  ##FLETES_IE10 c where a.ID = c.ID)),0),2)
else
Amortiza
end
from ##FLETES_IE104 a
...
a.Orden_Final, null, null,0, case when a.Movimiento_Destino in ('D','R') then null else a.Orden_Final end,
