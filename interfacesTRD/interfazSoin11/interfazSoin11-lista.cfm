...
<cf_templateheader title="Interfaz Documentos">
		<cf_web_portlet_start border="true" skin="#Session.Preferences.Skin#" tituloalign="center" titulo='Interfaz Pago Documentos'>
			<cfinclude template="../../sif/portlets/pNavegacion.cfm">
			<br><table width="99%" align="center" border="0" cellpadding="0" cellspacing="0"><tr><td>
			<cfset filtro = "">
			<cfinclude template="interfazSoin11-filtro.cfm">
			...
			<cfquery name="rsQuery" datasource="sifinterfaces">
				...
				<cfif isdefined("form.Modulo") and len(form.Modulo)>
					and TipoCobroPago = <cfqueryparam cfsqltype="cf_sql_char" value="#form.Modulo#">
				</cfif>
				<cfif isdefined("form.CodigoMoneda") and len(form.CodigoMoneda)>
					and upper(CodigoMonedaPago) like '%#UCase(Form.CodigoMoneda)#%' 
				</cfif>
				<cfif isdefined("form.Documento") and len(form.Documento)>
					and upper(NumeroDocumento) like '%#UCase(Form.Documento)#%' 
				</cfif>
				order by TipoCobroPago,FechaTransaccion			
			</cfquery>
			

			<form action="interfazSoin11-lista.cfm" method="post" name="lista" style="margin:0">
				...
				<cfinvoke 
				    ...
				    <cfinvokeargument name="desplegar" 	value="ID,NumeroDocumento,CodigoMonedaPago,FechaTransaccion,MontoPago,imagen,estado"/>
					<cfinvokeargument name="etiquetas" 	value="ID,Documento,Moneda,Fecha,Monto,&nbsp;,&nbsp;"/>
					<cfinvokeargument name="formatos" 	value="S,V,V,D,M,V,V"/>
					<cfinvokeargument name="align" 		value="left, left,left,left,right,right,right"/>					
					
					
					<cfinvokeargument name="ajustar" 	value="N"/>
					<cfinvokeargument name="Cortes" 	value="TipoCobroPagoL">
					<cfinvokeargument name="irA" 		value="interfazSoin11-lista.cfm"/>
			...
			<script language="JavaScript1.2" type="text/javascript">
				...
				function funcAplicar() {
						document.lista.action = "interfazSoin11-sql.cfm";
