...
<cfquery datasource="#session.DSN#" name="rsUnidades">
	select Ucodigo, Udescripcion, Uequivalencia, Utipo, ts_rversion
	from Unidades 
...
<form action="SQLUnidades.cfm" method="post" name="form1" >
	...
	<table align="center">
		...
		<tr valign="baseline"> 
      		...
      		<td>
				<input tabindex="1" type="text" name="Ucodigo" size="5" maxlength="5" value="<cfif modo NEQ 'ALTA'>#htmlEditFormat(rsUnidades.Ucodigo)#</cfif>" <cfif modo NEQ 'ALTA'>readonly</cfif> alt="El C&oacute;digo de Unidad" onfocus="javascript:this.select();" />
			</td>
    	</tr>
    	...
    	<tr valign="baseline">
		    ...
		    <td>
				<input tabindex="1" type="text" name="Udescripcion" size="50" maxlength="50" value="<cfif modo NEQ 'ALTA'>#htmlEditFormat(rsUnidades.Udescripcion)#</cfif>" alt="La Descripci&oacute;n de la Unidad" onfocus="javascript:this.select();" />
			</td>
		</tr>
    	...
    	<tr valign="baseline">
		    ...
		    <td>
				<input tabindex="1" type="text" name="Uequivalencia" value="<cfif modo NEQ 'ALTA'>#htmlEditFormat(rsUnidades.Uequivalencia)#</cfif>"  size="8" maxlength="8" style="text-align: right;" onblur="javascript:fm(this,2);"  onfocus="javascript:this.value=qf(this); this.select();"  onkeyup="javascript:if(snumber(this,event,2)){ if(Key(event)=='13') {this.blur();}}" alt="La Equivalencia para la Unidad">
			</td>
		</tr>
