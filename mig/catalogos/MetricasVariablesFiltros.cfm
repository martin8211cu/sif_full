...
<cfoutput>
 ...
 <cfform enctype="multipart/form-data" name="FormFiltrosMetricas" id="FormFiltrosMetricas" method="post" action="MetricasSQL.cfm" onSubmit="return valida(this);">
 
 	<input type="hidden" name="BajaFiltrosMetricas" id="BajaFiltrosMetricas" value="false">
	...
	<input name="pagenum1" value="<cfoutput>#pagenum#</cfoutput>" id="pagenum1"  type="hidden">
	 ...
	 <cf_conlis title="Lista de Metricas"
			campos = "MIGMidderivada , MIGMcodigo, MIGMnombre" 
			desplegables = "N,,S" 
			modificables = "N,S,N"
			tabla="MIGMetricas"
			columnas="MIGMid as MIGMidderivada, MIGMcodigo, MIGMnombre"
			filtro="#filtro#"
	...
	<strong>Filtrar por:</strong><br>
	 ...
	 <cfinput name="valsInMet" value="" id="valsInMet"  type="hidden">
	 <cfinput name="valsOutMet" value="" id="valsOutMet"  type="hidden">
	 <cfparam name="tabChoice" default="1">
 	 <cfinput name="tab" value="#tabChoice#" id="tab"  type="hidden">
 </cfform>
 <cfdiv id="divListasInd" onbinderror="myerror" bind="url:metricasListDeptos.cfm?tipo={FormFiltrosMetricas:rfiltro}&MIGMID={FormFiltrosMetricas:MIGMID}&MOD={FormFiltrosMetricas:modificable}&sub_MIGMid={FormFiltrosMetricas:sub_MIGMid}"/>
		
<!---inicio--->
<cfoutput>
<cfquery datasource="#session.DSN#" name="rsMetricas">
	select distinct c.MIGMid,c.MIGMnombre,c.MIGMcodigo,a.MIGMtipodetalle
	from MIGFiltrosderivadas a  
		inner join MIGMetricas c
		on c.MIGMid = a.MIGMid
		<!---and c.Ecodigo = a.Ecodigo--->
		
	where a.MIGMidderivada = #form.MIGMID#
			...
			<cfif tipodetalle is 'D'>
				<cfquery datasource="#session.DSN#" name="rsDatosSelect">
					...
					from MIGFiltrosderivadas a  
						...
						inner join MIGMetricas c	<!---No debe haber relacion con Ecodigo por que pueden ser diferentes Empresas en el caso de los metricas coorporativas--->
						on c.MIGMid = a.MIGMid
					...
					where MIGMidderivada = #form.MIGMID#
...
<script>
	...
	function refreshThis(){
		var sub_MIGMid=document.FormFiltrosMetricas.MIGMidderivada.value;
		document.getElementById('sub_MIGMid').value = sub_MIGMid;
		var tipo=document.FormFiltrosMetricas.rfiltro.value;
		//alert('metricasListDeptos.cfm?tipo='+tipo+'&MIGMID=#form.MIGMid#&MOD=#rsMod.si#&sub_MIGMid=' + sub_MIGMid)
		ColdFusion.navigate('metricasListDeptos.cfm?tipo='+tipo+'&MIGMID=#form.MIGMid#&MOD=#rsMod.si#&sub_MIGMid=' + sub_MIGMid,'divListasInd'); <!---refresca el cfdiv--->
	}
	function BorrarDetalle(MIGMIDpadre,detalleId,MIGMidhijo){
		if(confirm('Esta seguro que desea borrar el detalle?')){
			document.getElementById('BajaFiltrosMetricas').value = true;
...
function valida(formulario)	{
		...
		if (desp.length==0){
			error_msg += "\n - El nombre de la metrica no puede quedar en blanco.";
	...
	function funcBTNSubmitMet(fd,fsel){
		var sel1 =  fd;
		var sel2 = fsel;
		var msg  = '';
		
		if(document.FormFiltrosMetricas.MIGMidderivada.value == '') msg += '- El campo Métricas es Obligatorio\n';
		...
		document.FormFiltrosMetricas.valsInMet.value = dvalsIn;
		...
		document.FormFiltrosMetricas.valsOutMet.value = dvalsOut;  
		document.FormFiltrosMetricas.submit()	
	}

	function addDeptoMet(){
		
		var sel1 =  document.formMet.Flista;
		var sel2 = document.formMet.Fselected;
