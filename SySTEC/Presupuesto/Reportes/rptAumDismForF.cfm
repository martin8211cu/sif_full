...
<cfoutput>
	<form name="form1" method="post" action="rptAumDismForR.cfm" onSubmit="return validar();"  style="MARGIN:0;">
		<table align="center" border="0">
<!---=================Version Presupuestal=========================--->
			<tr>
				<td width="148" nowrap>Versión de Formulación:</td>
				<td colspan="3">
					<select name="CVid">
...
function crear(obj) 
{
	num++;
		if(ubication)
			alert(ubication[0]);
	var ubication = document.form1.ubicacion.value.split(',');
	GrAntAdd = ubication[0];
	document.getElementById("fiel").style.display="";

	if (ubication[0] == 'go' && document.getElementById(ubication[1]))
	{
		alert('El grupo de Oficinas ya fue agregado a la lista');
		return;
	}
	if (ubication[0] == 'of' && document.getElementById(ubication[1]))
	{
		alert('La Oficina ya fue agregada a la lista');
		return;
	}
