...
<!---=========Versiones de Formulación================--->
<cfquery name="rsVersiones" datasource="#Session.DSN#">
		select CVid, CVdescripcion, CPPid, CVtipo
		from CVersion p
		where p.Ecodigo = #Session.Ecodigo#
		  and p.CVaprobada = <cfif isdefined("form.chkAprobadas")>1<cfelse>0</cfif>
	</cfquery>
<cfoutput>
	<form name="form1" method="post" action="rptApliRecursosForR.cfm" onSubmit="return validar();"  style="MARGIN:0;">
		<table align="center" border="0">
<!---=================Version Presupuestal=========================--->
			<tr>
				<td nowrap>Versión de Formulación:</td>
				<td colspan="3" nowrap>
					<select name="CVid">
						<cfloop query="rsVersiones">
							<option value="#rsVersiones.CVid#">#rsVersiones.CVdescripcion#</option>
						</cfloop>
					</select>
...
<!---=================Catalogo Columnar=======================================--->
			<tr>
				<td>
                	<div id="divTipoCatC">
                        <select name="TipoCat" size="1" id="TipoCat" tabindex="1">
                            <option value="typeCol">Catalogo Columnar</option>
                            <option value="typeTot">Catalogo a Totalizar</option>
                        </select>	
                    </div>	
                    <div id="divTipoCatT">
                    	Catalogo Columnar
	...
	var ubication = document.form1.ubicacion.value.split(',');
	 switch (ubication[0])
	 {
	 	case 'go' :
			...
			document.getElementById("divTipoCatC").style.display="block";
			document.getElementById("divTipoCatT").style.display="none";
		...
		case 'of' :
			...
			document.getElementById("TipoCat").selectedIndex = 0;
			document.getElementById("divTipoCatC").style.display="none";
			document.getElementById("divTipoCatT").style.display="block";
...
function validar() {
		...
		if (document.form1.PCEcatid.value.length == "") {
			if (document.form1.TipoCat.value == 'typeCol')
				errores = errores + '- El Catalogo Columnar no está Definido en la empresa.\n';
			else
				errores = errores + '- El Catalogo Columnar a Totalizar no está Definido en la empresa.\n';
		}
		if (document.form1.CVid.value.length == "") {
			errores = errores + '- La Versión de Formulación es requerida.\n';
		}
		if (document.form1.nivelDet.value.length == "") {
			errores = errores + '- El nivel de detalle es requerido.\n';
		}
		if (errores != "") {
