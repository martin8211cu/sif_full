...
<!---===========Docs Traslados===============--->
<cfquery name="rsDocTraslados" datasource="#Session.DSN#">
...
<cfoutput>
	<form name="form1" method="post" action="rptADVesionFormulacion-imprimir.cfm" onSubmit="return validar();"  style="MARGIN:0;">
		<table align="center" border="0">
<!---=================Docs Traslados=========================--->
			<tr>
				<td nowrap>Grupo de Doc. Traslado Externo:</td>
				<td>
					<select name="CPDAEid">
						<cfloop query="rsDocTraslados">
							<option value="#rsDocTraslados.CPDAEid#">#rsDocTraslados.CPDAEdescripcion#</option>
						</cfloop>
					</select>
				</td>
			</tr>
<!---=================Estado Traslado=========================--->
			<tr>
				<td nowrap>Estado Documento Traslado:</td>
				<td>
					<select name="Estado">
...
/*********AGREGA UNA CUENTA PRESUPUESTAl AL LISTADO, CUANDO ES POR LISTADO O RANGO DE CUENTAS********/
...
function validar() {
		...
		switch (document.form1.ID_REPORTE.value){
			...
			case '2' :
				...
				if(cantidad != 2 ){
					errores = errores + '- Para este tipo de reporte es necesario tener una cuenta inicial y otro final, UNICAMENTE.\n';
				}
				listC= document.form1.CuentaidList;
				for(i = 0; i < listC.length; i++){
					if (fnExisteCaracter(listC[i].value, '_')){
						errores = errores + '- Para este tipo de reporte las cuentas deben de ser de ultimo nivel.\n';
						break;
					}
				}
			break;
		...
		if (document.form1.CPDAEid.value == "") 
			errores = errores + '- Doc. Traslado Externo es requerido.\n';
		if (errores != "") {
	...
	/*********************************************************************************************************/
	function fnExisteCaracter(cuenta, caracter){
		 for(e = 0; e < cuenta.length; e++){
			if(cuenta.charAt(e) == caracter)
				return true;
		}
		return false;
	}
