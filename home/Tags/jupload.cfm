    ...
    <cfloop query="rs">
          <div class="ajax-file-upload-statusbar">
              <div class="ajax-file-upload-filename">
                <div class="row">
                  <div class="col-sm-9" style="word-wrap:break-word;"><cfoutput>#nombre#</cfoutput></div>
                  <div class="col-sm-1">
        ...
        <cfif isdefined("fileTemp")>
           ...
           <cfif isdefined("ts")>
              <cfquery name="ABC_empleadosImagen" datasource="#Session.DSN#">
                insert into #form.tabla# (#form.fk#,#form.nombre#,#form.campo#
                <cfif arraylen(form.addDataColumns)>
                  <cfloop from="1" to="#arraylen(form.addDataColumns)#" index="i">
                    , <cfoutput>#form.addDataColumns[i].name#</cfoutput>
                  </cfloop>
                ...
                values (
                  ...
                  <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.nombreFile#">, 
...
<cfif ThisTag.ExecutionMode is 'start'>
  ...
  <cfparam name="Attributes.tabla"         type="string">
  <cfparam name="Attributes.campo"         type="string">
  <cfparam name="Attributes.nombre"        type="string" default=""><!--- guarda nombre del archivo en el caso que se requiera--->
  <cfparam name="Attributes.pk"            type="string">
  <cfparam name="Attributes.fk"            type="string">
  ...
  <script type="text/javascript">
     ...
     function RefrescarFilePool<cfoutput>#attributes.index#</cfoutput>(){
      <cfoutput> 
        $.ajax({
              ...
              type: "post",
              dataType: "html",
              data: { updateFilePool:1, index:'#attributes.index#', readonly:#attributes.readonly#, pk:'#attributes.pk#', nombre:'#attributes.nombre#', tabla:'#attributes.tabla#', fk:'#attributes.fk#', fkvalor:#attributes.fkvalor#, addDataColumns:'#SerializeJSON(attributes.addDataColumns)#', hideDeleted:'#attributes.hideDeleted#', filter:"#attributes.filter#" }, 
              success:function(data) { 
    ...
    <cfif not attributes.readonly>
      $(document).ready(function(){
        <cfoutput> 
          var settings = {
            url: "/cfmx/commons/Tags/jupload.cfm?JQexe=1&tabla=#attributes.tabla#&campo=#attributes.campo#&fk=#attributes.fk#&fkvalor=#attributes.fkvalor#&nombre=#attributes.nombre#", 
            method: "POST",
            ...
            dynamicFormData: function(){
              var data = { addDataColumns:'#SerializeJSON(attributes.addDataColumns)#', hideDeleted:'#Attributes.hideDeleted#', filter:"#Attributes.filter#" }
              return data;
            },
            showDone: true,
