...
OAT.PivotData = {
	TYPE_BASIC:[0,"Basico - 1234.56"],
	TYPE_PERCENT:[1,"Percentual - 1234.56%"],
	TYPE_SCI:[2,"Cientifico - 1234E+02"],
	TYPE_SPACE:[3,"Con espacio - 1 234.56"],
	TYPE_CUSTOM:[4,"Sin formato"], /* function in options.customType */
	TYPE_COMMA:[5,"Con coma - 1,234.56"],
	TYPE_CURRENCY:[6,"Moneda - $ 1,234.56"] /* currency symbol in options.currencySymbol. $ is default */
}
...
OAT.Pivot = function(div,chartDiv,filterDiv,headerRow,dataRows,headerRowIndexes,headerColIndexes,filterIndexes,dataColumnIndex,optObj) {
	...
	this.div = $Fred(div);
	this.filterDiv = $Fred(filterDiv);
	this.chartDiv = $Fred(chartDiv);
	this.defCArray = ["rgb(213,153,255)","rgb(153,51,205)","rgb(255,255,204)","rgb(204,255,255)","rgb(102,0,102)",
	...
	if (this.chartDiv) { 
        ...
        var c4 = OAT.Dom.create("div",{},"lineaChart");
		var l1 = OAT.Dom.button("");
		var l2 = OAT.Dom.button("");
		var l3 = OAT.Dom.button("");
        var l4 = OAT.Dom.button("");

		
		OAT.Dom.append([self.chartDiv,l1,c1,l3,c3,l2,c2,l4,c4]);/**/
		this.charts = {
			main:new OAT.BarChart(c1,{}),
			row:new OAT.BarChart(c2,{}),
			col:new OAT.BarChart(c3,{}),
            line:new OAT.LineChart(c4,{paddingLeft:30,
										paddingBottom:320,
										paddingTop:10,
										paddingRight:320,
										axes:true,
			...
			colDiv:c3,
            lineDiv:c4,
 			colorOFF: "btn btn-primary btn-xs btnPivot",
        	colorON: "btn btn-info btn-xs btnPivot"
		}
		...
		OAT.Dom.attach(l3,"click",function(){self.options.showColChart = (self.options.showColChart+1) % 2; self.go();}) 
   		OAT.Dom.attach(l4,"click",function(){self.options.showLineChart = (self.options.showLineChart+1) % 2; self.go();}) 
	...
	this.init = function() {
		...
		var aggRef = function() { 
			self.options.agg = parseInt($v("pivot_agg"));
			self.go();
		}
		var aggRefTotals = function() {
			self.options.aggTotals = parseInt($v("pivot_agg_totals"));
			self.go();
		}
		...
		for (var i=0;i< OAT.Statistics.list.length;i++) {
			if (this.options.agg == i) { $Fred("pivot_agg").selectedIndex = i; }
			if (this.options.aggTotals == i) { $Fred("pivot_agg_totals").selectedIndex = i; }
		}
	...
	this.drawFilters = function() {
		...
		if (!self.filterIndexes.length) {
			div.innerHTML = "<cf_translate key='LB_Filtro' xmlFile='/rh/generales.xml'>Filtro</cf_translate>: [<cf_translate key='LB_ArrastreColumnasAqui' xmlFile='/rh/generales.xml'>Arrastre columnas aquí</cf_translate>]";
		}
		for (var i=0;i<self.filterIndexes.length;i++) {
			var index = self.filterIndexes[i];
			var s = OAT.Dom.create("select");
			OAT.Dom.option("[Todos]","",s);
	...
	this.numericalType = function(event) {
		...
		l.innerHTML = "Mostrar datos vac�os";
	...
	this.drawTable = function() { /* this is the crucial part */
		...
		for (var i=0;i<self.colConditions.length;i++) {
			...
			for (var j=0;j<stack.length;j++) { /* column values */
				...
				if (cond.subtotals && i+1 < self.colConditions.length) { /* subtotal columns */
					var th = OAT.Dom.create("th",{},"h2");
					th.innerHTML = "Total de "+item.value;
		...
		for (var i=0;i<self.h;i++) {
			...
			for (var j=0;j<self.w;j++) { /* data */
				...
				/*JC Agrega columna calculada*/
					/*JC Determina el total de la secci�n
				
				var td = OAT.Dom.create("td",{},"gtotal");

				var result =  columna_total[0];
			...
			if (self.options.subtotalrow == 1) { /* JC subtotal rows */
				for (var j=self.rowConditions.length-2;j>=0;j--) { 
					...
					if (cond.subtotals && item.offset+item.spanData-1 == i) { 
						...
						th.innerHTML = "Sub Total -"+item.value;
						tr.appendChild(th);
	...
	this.fillAggStructure = function() { /* mark used branches of aggregation structure */
		function fillPart(struct,arr,row) {
			...
			for (var i=0;i<arr.length;i++) {
				...
				if (!o) { alert("Value not found in distinct!!!"); }
				ptr = o;
			} /* for all conditions */
			ptr.used = true;
		}
	...
	this._drawLineChart = function() {
    

   		var lc = self.charts.line;
		var cArray = [];
		for (var i=0;i<self.h;i++) { cArray.push(self.defCArray[i % self.defCArray.length]); }
		lc.options.colors = cArray;
		var data = [];
		
		for (var i=0;i<self.tabularData.length;i++) { 
			var col = [];
			for (var j=0;j<self.tabularData[0].length;j++) { col.push(self.tabularData[i][j]); }
			data.push(col); 
		}

   		self.charts.lineDiv.innerHTML='';
		...
		console.log(textY);*/
		lc.attachTextX(textX);
		lc.attachTextY(textY);
		lc.draw();
	...
	this._drawCharts = function() {
		...
		if (self.options.showChart) {
			...
			self._drawChart();
			self.charts.mainLink.value = "<cf_translate key='LB_OcultarTabla' xmlFile='/rh/generales.xml'>Ocultar Tabla</cf_translate>";
			self.charts.mainLink.className = self.charts.colorON;
		} else {
			OAT.Dom.hide(self.charts.mainDiv);
			self.charts.mainLink.value = "<cf_translate key='LB_MostrarTabla' xmlFile='/rh/generales.xml'>Mostrar Tabla</cf_translate>";
			self.charts.mainLink.className = self.charts.colorOFF;	
		}
		if (self.options.showRowChart && self.options.totals) {
			OAT.Dom.show(self.charts.rowDiv);
			self._drawRowChart();
			self.charts.rowLink.value = "<cf_translate key='LB_OcultarFilasTotales' xmlFile='/rh/generales.xml'>Ocultar filas de Totales en el Gráfico</cf_translate>";
			self.charts.rowLink.className = self.charts.colorON;
		} else {
			OAT.Dom.hide(self.charts.rowDiv);
			self.charts.rowLink.value = "<cf_translate key='LB_MostrarFilasTotales' xmlFile='/rh/generales.xml'>Mostrar filas de Totales en el Gráfico</cf_translate>";
			self.charts.rowLink.className = self.charts.colorOFF;	
		}
		if (self.options.showColChart && self.options.totals) {
			OAT.Dom.show(self.charts.colDiv);
			self._drawColChart();
			self.charts.colLink.value = "<cf_translate key='LB_OcultarColumnasTotales' xmlFile='/rh/generales.xml'>Ocultar Columnas de Totales en el Gráfico</cf_translate>";
			self.charts.colLink.className = self.charts.colorON;
		} else {
			OAT.Dom.hide(self.charts.colDiv);
			self.charts.colLink.value = "<cf_translate key='LB_MostrarColumnasTotales' xmlFile='/rh/generales.xml'>Mostrar Columnas de Totales en el Gráfico</cf_translate>";
			self.charts.colLink.className = self.charts.colorOFF;	
		}
        
		if (self.options.showLineChart && self.options.totals) {
			OAT.Dom.show(self.charts.lineDiv);
			self._drawLineChart();
			self.charts.lineLink.value = "<cf_translate key='LB_OcultarGrafico' xmlFile='/rh/generales.xml'>Ocultar Gráfico</cf_translate>";
			self.charts.lineLink.className = self.charts.colorON;
		} else {
			OAT.Dom.hide(self.charts.lineDiv);
			self.charts.lineLink.value = "<cf_translate key='LB_MostrarGrafico' xmlFile='/rh/generales.xml'>Mostrar Gráfico</cf_translate>";
			self.charts.lineLink.className = self.charts.colorOFF;	
		}
