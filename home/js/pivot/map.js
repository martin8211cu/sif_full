/*
 *  $Id: map.js,v 1.10 2007/07/31 09:18:38 source Exp $
 *
	...
	this.openWindow = function(marker,something) {
		...
		switch (self.provider) {
			...
			case OAT.MapData.TYPE_MS:
				...
				$Fred(marker.__id).appendChild(marker.__win.div);
	...
	this.addMarker = function(group,lat,lon,file,w,h,clickCallback,overCallback) {
		switch (self.provider) {
			...
			case OAT.MapData.TYPE_MS:
				...
				if (clickCallback) { OAT.Event.attach($Fred(id).firstChild,"click",function(event){clickCallback(marker,event);}); }
				if (overCallback) { OAT.Event.attach($Fred(id).firstChild,"mouseover",function(event){overCallback(marker,event);}); }
			break;
			case OAT.MapData.TYPE_OL: 
				var icon = false;
