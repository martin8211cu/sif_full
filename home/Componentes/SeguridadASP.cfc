...
<cfcomponent extends="Seguridad">
	...
	<cffunction name="_CAMBIA_PASSWORD_BACKEND" access="package" returntype="boolean" output="false">
		...
		<cfquery datasource="#Arguments.Conexion#" name="password_query">
		...
		<cfquery datasource="#Arguments.Conexion#" name="findPassw">
			select count(1) as cantidad
			from UsuarioPassword
			where Usucodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.Usucodigo#">
		</cfquery>
        <cflog file="seguridad" text="query findPassw tiene #findPassw.cantidad# registro(s)">
        ...
        <!---agrego el proceso para que respalde la tabla de Historicos--->
        <cfif isdefined('findPassw') and findPassw.cantidad GT 0 and data.pass.valida.lista gt 0>
            <cfquery datasource="#Arguments.Conexion#" name="findDatosHist">
                select numero, Hash, Usucodigo
                from UsuarioPasswordHist
                where Usucodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.Usucodigo#">
                order by numero desc
            </cfquery>
            
            <cfloop query="findDatosHist">
            	<cfquery datasource="#Arguments.Conexion#" name="UPDATE_HIST">
                    update UsuarioPasswordHist 
                        set numero = #findDatosHist.numero#+1
                    where Usucodigo = #findDatosHist.Usucodigo#
                        and numero = #findDatosHist.numero#
              	</cfquery>
            </cfloop>        
        	<cfquery datasource="#Arguments.Conexion#" name="findDatos">
                select Hash, HashMethod, PasswordSalt,Usucodigo
                from UsuarioPassword
                where Usucodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.Usucodigo#">
            </cfquery>
            
           <cfquery datasource="#Arguments.Conexion#" name="password_HIST">
            insert INTO UsuarioPasswordHist (Usucodigo, Hash, HashMethod, PasswordSalt, numero)
				values (
					<cf_jdbcquery_param cfsqltype="cf_sql_numeric" 	 value="#findDatos.Usucodigo#">,
					<cf_jdbcquery_param cfsqltype="cf_sql_varchar" 	 value="#findDatos.Hash#"	len="128">,
					<cf_jdbcquery_param cfsqltype="cf_sql_varchar" 	 value="#findDatos.HashMethod#" len="8">,
					<cf_jdbcquery_param cfsqltype="cf_sql_varchar" 	 value="#findDatos.PasswordSalt#" len="128">,
					 1)
            </cfquery>
        </cfif>
        
		<cfquery datasource="#Arguments.Conexion#" name="password_query">
			<cfif isdefined('findPassw') and findPassw.cantidad GT 0>
