...
<cfcomponent extends="Seguridad">
	...
	<cfset This.ldapAdminPassword = Politicas.trae_parametro_global("ldap.adminPass")>
	<cfset This.ldapDominio = Politicas.trae_parametro_global("ldap.dominio")> <!--- para windows active directory --->
	...
	<cffunction name="_CAMBIA_PASSWORD_BACKEND" access="package" returntype="boolean" output="false">
		...
		<cfset admin_user = __find_user('admin')>
		<cfset target_user = __find_user(password_query.Usulogin)>
	...
	<cffunction name="_AUTENTICAR_BACKEND" access="package" returntype="boolean" output="false">
		...
		<cfif isdefined("finduser") and finduser.RecordCount>
			<cftry>
				<cfldap 
					 server="#This.ldapServer#"
					 action="query"
