...
<cffunction name="InsertMonProcesos" output="no">
	...
	<cfquery datasource="aspmonitor" name="insert1">
	  ...
	  values (
		...
		<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(session.sitio.ip,1,60)#" maxlength="60">,
		<cfif Len(Arguments.MonPacket.login)>
		<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(Arguments.MonPacket.login,1,40)#" maxlength="40">,
		<cfelse>' ',
		</cfif>
		<cfqueryparam cfsqltype="cf_sql_numeric" value="#Application.srvprocid#">,
		
		<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(Arguments.MonPacket.SScodigo,1,10)#" maxlength="10">,
		<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(Arguments.MonPacket.SMcodigo,1,10)#" maxlength="10">,
		<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(Arguments.MonPacket.SPcodigo,1,10)#" maxlength="10">,
		0, 
		<cfqueryparam cfsqltype="cf_sql_timestamp" value="#Now()#">,
		<cfqueryparam cfsqltype="cf_sql_timestamp" value="#Now()#">,
		<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(CGI.HTTP_USER_AGENT,1,100)#" maxlength="100">,
		<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(Arguments.MonPacket.HttpHost,1,60)#" maxlength="60">,
		...
		<cfquery datasource="aspmonitor" name="ret">
		  insert into MonHistoria (
			...
			values (
				...
				<cfqueryparam cfsqltype="cf_sql_numeric" value="#Application.srvprocid#">,
				<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(Arguments.MonPacket.SScodigo,1,60)#" maxlength="10">,
				<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(Arguments.MonPacket.SMcodigo,1,60)#" maxlength="10">,
				<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(Arguments.MonPacket.SPcodigo,1,60)#" maxlength="10">,
	...
	<cfquery datasource="aspmonitor" name="inserted">
	  insert into MonRequest (
		...
		values (
			...
			<cfqueryparam cfsqltype="cf_sql_numeric" value="#Arguments.MonPacket.sessionid#">,
			<cfqueryparam cfsqltype="cf_sql_numeric" value="#Application.srvprocid#">,
			<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(Arguments.MonPacket.RequestMethod,1,255)#" maxlength="4">,
			<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(Arguments.MonPacket.ScriptName,1,255)#" maxlength="255">,
			<cfqueryparam cfsqltype="cf_sql_varchar" value="#mid(Arguments.MonPacket.QueryString,1,255)#" maxlength="255">,
...
<cffunction name="MonitoreoEnd">
	...
	<cfinvoke method="MonitorearProceso" Estado="1"/><!---- finalizado correctamente---->
...
<cffunction name="ValidarSiSePuedeIniciarSesion">
	...
	<cfif Eactiva_Q.RecordCount is 0>
		<cfif isdefined("session.login_no_interactivo") and session.login_no_interactivo>
			<cfthrow message="aspmonitor: La empresa no está activa">
		</cfif>
	...
	<cfif sesion_multiple is 2 or sesion_multiple is 3 or sesion_multiple is 1>
		<!---
		...
		<cfelseif sesion_multiple is 3>
			...
			<cfloop query="otras_sesiones">
				...
				<cfif expiracion gt Now()>
					<cfif isdefined("session.login_no_interactivo") and session.login_no_interactivo>
						<cfthrow message="aspmonitor: No se permiten más de una sesión activa">
					</cfif>
	...
	<cfif usuario_demo EQ 'true'><!---Si es usuario de demo verifica si no ha expirado---->
		...
		<cfif rsExpira.RecordCount NEQ 0><!----Si ya expiro envia a pantalla con mensaje---->
			<cfif isdefined("session.login_no_interactivo") and session.login_no_interactivo>
				<cfthrow message="aspmonitor: La session ha expirado">
			</cfif>
...
<cffunction name="VerSiLaSesionYaExpiro" output="false">
	...
	<cfif mp.cerrada is 1>
		<cfif isdefined("session.login_no_interactivo") and session.login_no_interactivo>
			<cfthrow message="aspmonitor: La sesión fue cerrada">
		</cfif>
	...
	<cfif mp.RecordCount is 0>
		<cfif isdefined("session.login_no_interactivo") and session.login_no_interactivo>
			<cfthrow message="aspmonitor: La sesión no existe">
		</cfif>
	...
	<cfif Now() GT expiracion>
		<cfif isdefined("session.login_no_interactivo") and session.login_no_interactivo>
			<cfthrow message="aspmonitor: La sesion ha expirado">
		</cfif>
		<cfset This.MonitoreoLogout('T')>
		...
		<cflock name="application_srvprocid" timeout="10" type="exclusive" throwontimeout="yes">
			...
			<cfquery datasource="aspmonitor">
				...
				values (
					...
					<cfqueryparam cfsqltype="cf_sql_varchar"   value="#mid(Server.ColdFusion.AppServer,1,30)#" maxlength="30">,
					<cfqueryparam cfsqltype="cf_sql_varchar"   value="#mid(Server.ColdFusion.ProductVersion,1,30)#" maxlength="30">,

					<cfqueryparam cfsqltype="cf_sql_varchar"   value="#mid(Server.Os.Arch,1,30)#" maxlength="30">,
					<cfqueryparam cfsqltype="cf_sql_varchar"   value="#mid(Server.Os.Name,1,30)#" maxlength="30">,
					<cfqueryparam cfsqltype="cf_sql_varchar"   value="#mid(Server.Os.Version,1,30)#" maxlength="30">,
					
					<cfqueryparam cfsqltype="cf_sql_varchar"   value="#mid(java_version,1,30)#" maxlength="30">,
					<cfqueryparam cfsqltype="cf_sql_varchar"   value="#mid(java_vendor,1,30)#" maxlength="30">,
					<cfqueryparam cfsqltype="cf_sql_varchar"   value="#mid(hostname,1,255)#" maxlength="255">)
			</cfquery>
...
<!----Componente para monitoreo del ERP
fcastro 2015-02-24
Este componente contiene funciones para:
- Monitoreo de procesos para la tabla MMonitoreo de aspmonitor
--->
<cffunction name="MonitorearProceso" output="false" returntype="void">
	<cfargument name="idProceso" 		type="numeric" 	required="true" default="0">
	...
	<cfargument name="InfoProceso" 		type="string" 	required="true" default="">
	<cfargument name="Resolucion" 		type="string" 	required="true" default="">
	<cfargument name="Estado" 			type="numeric" 	required="true" default="0">
	<cfargument name="Error" 			type="any" 		required="false">
	<cfargument name="modoEdicion" 		type="boolean" 	required="true" default="false">


	<!----
		Códigos para los procesos
			101	- Creación Relación Cálculo
			102	- Restaurar Relación Cálculo
			103	- Aplicación Relación Cálculo
			104	- Finalización Relación Cálculo
			105	- Aplicación Relación Aumento
			106	- Cálculo de Nóminas Especiales
			201	- Cierre de Auxiliares
			202	- Cierre de Contabilidad
			203	- Aprobar Presupuesto Ordinario PC
			204	- Aprobar Presupuesto Extraordinario PC
			205	- Aprobar Traslados PC
			206	- Carga Presupuesto Ordinario
			207	- Aprobar Publicación de Presupuesto Ordinario
			208	- Aprobar Publicación de Presupuesto Extraordinario
	------->		

	<!---- si no se encuentra el parametro de politicas globales se apaga el proceso---->
	<cfif !(isDefined("application.politicas_pglobal.monitoreoprocesos") and application.politicas_pglobal.monitoreoprocesos)>
		<cfreturn>
	</cfif>
 
	<cfif arguments.Estado eq 3>
		<cftransaction>
			<!---- guarda el registro en la historia---->
			<cfquery datasource="aspmonitor">
				insert into MMonitoreoH (MMid,MPid,Empresa,BMUsucodigo,FechaHoraInicio,FechaHoraFin,Revisado,Resolucion,Estado,InfoProceso)
				select MMid,MPid,Empresa,BMUsucodigo,FechaHoraInicio,FechaHoraFin,Revisado,Resolucion,Estado,InfoProceso
				from MMonitoreo
				where MMid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.idProceso#">
			</cfquery>
			<!---- borra el registro de proceso----->
			<cfquery datasource="aspmonitor">
				delete from MMonitoreo where MMid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.idProceso#">
			</cfquery>
		</cftransaction>
		<cfreturn>
	</cfif>

	<!--- si se indica un estado se actualiza el registro---->
<cftry>
	

	<cfif structKeyExists(request, "MonitoreoIdCurrentActiveProcess") and arguments.Estado>
		<cf_dbfunction name="now" returnvariable="LvarNow" datasource="aspmonitor">
		<cf_dbfunction name="op_concat" returnvariable="concat">
		<cf_dbfunction name="datediff" args="FechaHoraInicio!#LvarNow#!MI" delimiters="!" datasource="aspmonitor" returnvariable="LvarFechas">

		<cfif arguments.Estado eq 2>
			<cfquery datasource="aspmonitor" name="rsInfoProceso">
				select InfoProceso
				from MMonitoreo
				where MMid =<cfqueryparam cfsqltype="cf_sql_numeric" value="#request.MonitoreoIdCurrentActiveProcess#">
			</cfquery>
			<cfset arguments.InfoProceso&=rsInfoProceso.InfoProceso>
			<cfset arguments.InfoProceso&=', Error: #arguments.Error.RootCause.Message#, Detalle Error: #arguments.Error.RootCause.TagContext[1].template#: #arguments.Error.RootCause.TagContext[1].ID# (line:#arguments.Error.RootCause.TagContext[1].line#)'>
		</cfif>

		
		<cfquery datasource="aspmonitor">
			update MMonitoreo
			set FechaHoraFin	=	<cf_dbfunction name="now">,
			<cfif arguments.Estado eq 2><!---- si existe un error se almacena el error en el sistema---->
				InfoProceso			=   <cfqueryparam cfsqltype="cf_sql_longvarchar" value="#arguments.InfoProceso#">,
			</cfif>
			Estado				= 	<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.Estado#">
			where MMid =<cfqueryparam cfsqltype="cf_sql_numeric" value="#request.MonitoreoIdCurrentActiveProcess#">
		</cfquery>
	
		<cfquery datasource="aspmonitor" name="rsRevisar">
			select count(1) as valor
			from MMonitoreo m
				inner join MProcesos p
					on m.MPid=p.MPid
			where m.MMid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#request.MonitoreoIdCurrentActiveProcess#">
				and not (#preserveSingleQuotes(LvarFechas)# >= p.MPumbral1)
		</cfquery>  

			
		<cfif rsRevisar.valor>
			<cfinvoke method="MonitorearProceso" 
				idProceso="#request.MonitoreoIdCurrentActiveProcess#"
				Resolucion=""
				Estado="3">
		</cfif>
	<!--- siempre que el estado sea 0 se inicia el registro---->	
	<cfelseif arguments.Estado eq 0>
		<cfquery datasource="aspmonitor" name="rsInsertMon">
			insert into MMonitoreo (MPid,Empresa,BMUsucodigo,FechaHoraInicio,FechaHoraFin,InfoProceso,Revisado,Resolucion,Estado)
			values(
					...
					null,
					<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.InfoProceso#">,
					0,<!---- aun no esta revisado---->
					null,<!--- resolucion---->
					0<!---- en proceso--->
					)
				<cf_dbidentity1>
		</cfquery>
		<cf_dbidentity2 name="rsInsertMon">			
		<cfset structInsert(request,"MonitoreoIdCurrentActiveProcess", rsInsertMon.identity)>
	</cfif>
	<cfcatch type="any">
		<cf_dumptofile vaR="#cfcatch#" abort=false>
	</cfcatch>
	</cftry>

</cffunction>

<!---- esta funcion ingresa los procesos que no existen en el sistema
Estos codigos se instalarán en el sistema de ERP por lo cual no podrán modificarse a menos que sean por subversion
--->
<cffunction name="MonitorearProcesoInitProcesos" output="false">
	<cfargument name="idProceso" 		type="numeric" 	required="true" default="0">
	<cfargument name="Ecodigo" 			type="numeric" 	required="true" default="#session.Ecodigo#">
	<cfargument name="Usucodigo" 		type="numeric" 	required="true" default="#session.Usucodigo#">
	<cfargument name="InfoProceso" 		type="string" 	required="true" default="">
	...
	<cfquery datasource="aspmonitor">
		insert into MProcesos( MPid,SScodigo,SMcodigo,MPDescripcion,MPumbral1,MPumbral2)
		 select 101, 'RH', 'NOMINA', 'Creacion Relacion Calculo', 20.00, 30.00 from dual where (select count(1) from MProcesos where MPid=101) =0
		 union
		 select 102, 'RH', 'NOMINA', 'Restaurar Relacion Calculo', 20.00, 30.00 from dual where (select count(1) from MProcesos where MPid=102)=0
		 union
		 select 103, 'RH', 'NOMINA', 'Aplicacion Relacion Calculo', 10.00, 20.00 from dual where (select count(1) from MProcesos where MPid=103)=0
		 union
		 select 104, 'RH', 'NOMINA', 'Finalizacion Relacion Calculo', 10.00, 20.00 from dual where (select count(1) from MProcesos where MPid=104)=0
		 union
		 select 105, 'RH', 'NOMINA', 'Aplicacion Relacion Aumento', 20.00, 30.00 from dual where (select count(1) from MProcesos where MPid=105)=0
		 union
		 select 106, 'RH', 'NOMINA', 'Calculo de Nominas Especiales', 20.00, 30.00 from dual where (select count(1) from MProcesos where MPid=106)=0
		 union
		 select 201, 'SIF', 'CONTABILIDAD', 'Cierre de Auxiliares', 20.00, 30.00 from dual where (select count(1) from MProcesos where MPid=201)=0
		 union
		 select 202, 'SIF', 'CONTABILIDAD', 'Cierre de Contabilidad', 30.00, 40.00 from dual where (select count(1) from MProcesos where MPid=202)=0
		 union
		 select 203, 'SIF', 'ESTIMACION', 'Aprobar Presupuesto Ordinario PC', 30.00, 40.00 from dual where (select count(1) from MProcesos where MPid=203)=0
		 union
		 select 204, 'SIF', 'ESTIMACION', 'Aprobar Presupuesto Extraordinario PC', 30.00, 40.00 from dual where (select count(1) from MProcesos where MPid=204)=0
		 union
		 select 205, 'SIF', 'ESTIMACION', 'Aprobar Traslados PC', 10.00, 20.00 from dual where (select count(1) from MProcesos where MPid=205)=0
		 union
		 select 206, 'SIF', 'PRESUPUESTO', 'Carga Presupuesto Ordinario', 25.00, 35.00 from dual where (select count(1) from MProcesos where MPid=206)=0
		 union
		 select 207, 'SIF', 'PRESUPUESTO', 'Aprobar Publicacion de Presupuesto Ordinario', 30.00, 40.00 from dual where (select count(1) from MProcesos where MPid=207)=0
		 union
		 select 208, 'SIF', 'PRESUPUESTO', 'Aprobar Publicacion de Presupuesto Extraordinario', 30.00, 40.00 from dual where (select count(1) from MProcesos where MPid=208)=0
	</cfquery>
...
<cffunction name="MonitorearProcesoCatalogo" output="false">
	<cfargument name="idProceso" 		type="numeric" 	required="true">
	<cfargument name="SScodigo" 		type="string" 	required="true">
	<cfargument name="SMcodigo" 		type="string" 	required="true">
	<cfargument name="Descripcion" 		type="string" 	required="true">
	<cfargument name="umbral1" 			type="numeric" 	required="true" default="0">
	<cfargument name="umbral2" 			type="numeric" 	required="true" default="0">
	<cfargument name="BMUsucodigo" 		type="numeric" 	required="true" default="#session.Usucodigo#">

	<cfquery datasource="aspmonitor" name="rsvalidar">
		select count(1) as valor from MProcesos
	</cfquery>
	<cfquery datasource="aspmonitor">
		update MProcesos
			set SScodigo=<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.SScodigo#">,
				SMcodigo=<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.SMcodigo#">,
				MPDescripcion=<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.Descripcion#">,
				MPumbral1= <cfqueryparam cfsqltype="cf_sql_numeric" scale="2" value="#arguments.umbral1#">,
				MPumbral2= <cfqueryparam cfsqltype="cf_sql_numeric" scale="2" value="#arguments.umbral2#">,
				BMUsucodigo= <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.BMUsucodigo#">
		where MPid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.idProceso#">
	</cfquery>
 
</cffunction>	
<cffunction name="setMonitoreoProceso" output="false">
	<cfargument name="idProceso" 		type="numeric" 	required="true">
	<cfargument name="Resolucion" 		type="string" 	required="true" default="">
	<cfargument name="Estado" 			type="numeric" 	required="true" default="-1">
	<cfargument name="Revisado" 		type="numeric" 	required="true" default="-1">

	<cfquery datasource="aspmonitor">
		update <cfif arguments.historico>
					MMonitoreoH
				<cfelse>	
					MMonitoreo
				</cfif>
		set BMUsucodigo = BMUsucodigo
		<cfif len(trim(arguments.Resolucion))>
			,Resolucion			=   <cfqueryparam cfsqltype="cf_sql_longvarchar" value="#arguments.Resolucion#">
		</cfif>
		<cfif arguments.Estado neq -1>
			,Estado				= 	<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.Estado#">
		</cfif>
		<cfif arguments.Revisado neq -1>
			,Revisado            = 	<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.Revisado#">
		</cfif>	
		where MMid =<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.idProceso#">
	</cfquery>
...
<cffunction name="getMonitoreoProceso" output="false" returntype="query">
	<cfargument name="idProceso" 	type="numeric" 	required="true" default="0">
	<cfargument name="historico" 	type="boolean" 	required="true" default="false">


	<cf_dbfunction name="now" returnvariable="LvarNow" datasource="aspmonitor">
	<cf_dbfunction name="datediff" args="FechaHoraInicio!coalesce(FechaHoraFin,#LvarNow#)!MI" delimiters="!" datasource="aspmonitor" returnvariable="LvarFechas">
	...
	<cfquery datasource="aspmonitor" name="rsInfoProceso">
		select m.MMid,
		    ...
		    p.SMcodigo,
		    p.MPDescripcion,
		    #preserveSingleQuotes(LvarFechas)# as tiempoT,
		    m.FechaHoraInicio,
		    m.FechaHoraFin,
		    #LvarNow# as fechaConsulta,
		    m.Estado,
		    m.BMUsucodigo,
		    m.Empresa,
		    m.InfoProceso,
		...
		from 	<cfif arguments.historico>
	    	...
	    	inner join MProcesos p
	        on m.MPid=p.MPid
	    where m.MMid =<cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.idProceso#">
	</cfquery>

	<cfreturn rsInfoProceso>
