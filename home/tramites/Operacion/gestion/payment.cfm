<!---
<cfif isdefined("url.id_persona") and len(trim(url.id_persona)) and  not  isdefined("url.id_persona")>
	<cfset url.id_persona=url.id_persona>
</cfif>
<cfif isdefined("url.id_tramite") and len(trim(url.id_tramite))  and  not  isdefined("url.id_tramite")>
	<cfset url.id_tramite=url.id_tramite>
</cfif>
<cfif isdefined("url.id_instancia") and len(trim(url.id_instancia))  and  not  isdefined("url.id_instancia")>
	<cfset url.id_instancia=url.id_instancia>
</cfif>
<cfif isdefined("url.id_requisito") and len(trim(url.id_requisito))  and  not  isdefined("url.id_requisito")>
	<cfset url.id_requisito=url.id_requisito>
</cfif>
--->
...
<table width="100%">
	  <td bgcolor="##ECE9D8" style="padding:3px;">
		<strong>Registro de Pago Realizado</strong>
	</td>
...
<!--
	...
	function change_forma_pago(f){
		...
		(document.all?document.all.detalle_efectivo:document.getElementById('detalle_efectivo')).
			style.display = ( forma_pago == 'E' ) ? 'block' : 'none';
	...
	function validar(f) {
		...
		if (f.forma_pago.value == 'T') {
			...
			if (f.cheque_numero.value  == '') { msg += "\n* Escriba el número de cheque"; ctl = ctl?ctl:f.cheque_numero; }
			if (f.cheque_cuenta.value == '' ) { msg += "\n* Escriba el número de cuenta del cheque"; ctl = ctl?ctl:f.cheque_cuenta; }
			else {
				var msg_cc = fnVerificaCC(f.cheque_cuenta.value);
				if (msg_cc) {
					msg += "\n* " + msg_cc;
					ctl = ctl?ctl:f.cheque_cuenta;
				}
			}
			
			if (f.cheque_Bid.value == '') { msg += "\n* Seleccione el banco emisor del cheque"; ctl = ctl?ctl:f.cheque_Bid; }
			
		}
		else if (f.forma_pago.value == 'E') {
			if (f.num_autorizacion.value == '') { msg += "\n* Indique el No. de Autorización"; ctl = ctl?ctl:f.num_autorizacion; }
		}
		if (msg.length || ctl) {
...
<form action="payment2.cfm" method="get" name="form1" style="margin:0" onSubmit="return validar(this)">
<input type="hidden" name="loc" value="pago2">
...
<tr>
  <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      ...
      <td align="right">&nbsp;</td>
      <td> <input type="text" size="30"  readonly="true" name="importe_pago" id="importe_pago" style="text-align:right;width:250px" value="#NumberFormat(total_header.saldo,',0.00')#" onBlur="pago_blur(this.form)" onFocus="this.select()"> </td>
    </tr>
    <tr>
...
<tr><td>
<div id="detalle_efectivo" style="display:block">
	<table width="100%"  border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td align="right" nowrap>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<strong>No. Autorización:</strong></td>
			<td>&nbsp;&nbsp;</td>
			<td><input name="num_autorizacion" style="width:250px " size="30" type="text" id="num_autorizacion" onfocus="this.select()" maxlength="30"></td>
		</tr>  
	</table>
</div>
<div id="detalle_tarjeta" style="display:block">
	<table width="100%"  border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td colspan="3">
				<cf_tarjeta action="input" title="Medio de pago">
			</td>
		</tr>
	</table>
</div>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td><input name="cheque_cuenta" style="width:250px " size="30" type="text" onfocus="this.select()" maxlength="60"></td>
  </tr>
  <tr>
    <td align="right"><strong>Siglas del banco:</strong></td>
    <td>&nbsp;</td>
    <td><input name="cheque_banco" style="width:250px " size="30" type="text" id="cheque_banco" onfocus="this.select()" maxlength="30"></td>
  </tr>
</table>
</div>
</td>
...
<input type="hidden" name="id_requisito" value="<cfif isdefined("url.id_persona")>#url.id_requisito#</cfif>">  
</form>
</cfoutput>
<script type="text/javascript">
<!--
