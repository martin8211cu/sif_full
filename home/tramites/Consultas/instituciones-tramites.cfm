<cfif isdefined("Form.id_inst") and Len(Trim(Form.id_inst))>
	...
	<cfquery name="rsLista" datasource="#session.tramites.dsn#">
		select a.id_tramite, a.id_inst, a.id_tipotramite, a.id_documento_generado, a.codigo_tramite, a.nombre_tramite, a.descripcion_tramite, a.id_metodo_generado
		from TPTramite a
		where a.id_inst = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.id_inst#">
		order by a.codigo_tramite
	</cfquery>
	...
	<script type="text/javascript" language="javascript1.2">
		function infoTramite(tramite) {
			var params = "?id_tramite="+tramite;
			var width = 650;
			var height = 400;
			var top = (screen.height - height) / 2;
			var left = (screen.width - width) / 2;
			var nuevo = window.open("/cfmx/home/tramites/Operacion/gestion/infoTramite.cfm"+params,'info','menu=no,scrollbars=yes,top='+top+',left='+left+',width='+width+',height='+height);
			nuevo.focus();
		}
	</script>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2">
			...
			<cfif rsLista.recordCount>
			  <cfloop query="rsLista" startrow="#StartRow_lista#" endrow="#StartRow_lista+MaxRows_lista-1#">
				  ...
				  <tr class="#estilo#" onmouseover="javascript: this.style.cursor='pointer'; this.className='listaParSel';" onmouseout="javascript: this.className='#estilo#';" onclick="javascript: infoTramite('#rsLista.id_tramite#');">
					<td>
						#rsLista.codigo_tramite#
					</td>
					<td>
						#rsLista.nombre_tramite#
					</td>
					<td>
						<img title="Ver Detalle del Tramite #trim(rsLista.codigo_tramite)# - #rsLista.nombre_tramite#" src="/cfmx/home/tramites/images/info.gif" border="0">
					</td>
				  </tr>
			  </cfloop>
			  <tr>
			...
			<cfelse>
			  ...
			  <tr>
				<td colspan="3" align="center">
					<strong>No se encontraron tramites en esta institucion</strong>
				</td>
			  </tr>
			</cfif>
			<tr>
