			...
			<cflog file="IS700Times#DateFormat(now(),'YYYYMMDD')#" text="IS700Times, Fin de transaccion  ******, milisegundos: #GetTickCount() - LvarTiempoIni#">
				<cfcatch type = "Any">
					<cfset resultado="#cfcatch.message#, Detalle1: #cfcatch.detail#" & 
					                 "#TRANSACCION.FAX01ECLICOD.XmlText#, #TRANSACCION.CAJA.XmlText#">
...
*********************************************************************************************--->								
			<cfset NewConsecutivo = "">
			<cflog file="IS700Times#DateFormat(now(),'YYYYMMDD')#" text="IS700Times, Luego de , mlisegundos: #GetTickCount() - LvarTiempoIni#">
...
*********************************************************************************************--->								
            ...
            <cfquery datasource="CAJAS" name="rsSucursal">
                select CODSUC as sucursal  
                    from FAM001 
                   where FAM01COD = '#trans.CAJA.XmlText#'
            </cfquery> 
            
            
           	<cfquery datasource="CAJAS" name="rsFAX14DOC">
                select CONSECUTIVO ,   substring(CODSUC,2,2) as serie
                    from FAM039 
                   where CODSUC = '#rsSucursal.sucursal#' 
                   and TIPDOC = 'AD'
            </cfquery> 
            
			
            
            <cfif rsFAX14DOC.recordcount gt 0>
            	<cfset LvarFAX14DOC="#rsFAX14DOC.CONSECUTIVO#-#rsFAX14DOC.serie#">
                
                <cfquery datasource="CAJAS" name="rsLLave">
                    select  Coalesce(max(FAX14CON)+1, 0) as Consecutivo
                        from FAX014 
				...
				<cfif rsLLave.recordcount eq 1>  
                    ...
                    <cfset resultadoPT = insertaFAX014(
                                    #detalleHijo.MONTO.XmlText#,
                                    #LvarFAX14DOC#,
                                    #trans.CAJA.XmlText#,
                                    #trans.MONCOD.XmlText#,
                ...
                <cfquery datasource="CAJAS" >
                    update FAM039
                    set CONSECUTIVO = #rsFAX14DOC.CONSECUTIVO#+1
                    	where CODSUC = '#rsSucursal.sucursal#'
                  		and TIPDOC = 'AD'
                </cfquery>
            
            <cfelse>
            	<cfthrow message="No existe consecutivo en la sucursal:#rsSucursal.sucursal# ">    
            </cfif>
...
*********************************************************************************************--->								
    ...
    <cffunction name="insertarEncabezadoTransaccion" access="private" returnType="string" hint="Funcion para insertar el Encabezado de la transacción(FAX001EX)">
        ...
        <cfif resultadoIET EQ "OK">
            ...
            <cfset sqlIET = sqlIET & procesarArgumento(null,        				0, 0)>
            ...
            <cfset sqlIDT = sqlIDT & procesarArgumento(#ARGUMENTS.USUARIO#,     0, 1)>		--->																												
                ...
                <cfquery  name="InsFAX001" datasource="CAJAS">
				     insert FAX001 (FAM01COD, FAX01NTR,  FAX01FEC,    FAX01TOT,     FAX01TIP,   MONCOD,   CLICOD,
						...
						<cfqueryparam cfsqltype="cf_sql_char" 		value="9">,
...
*********************************************************************************************--->
			...
			<cftry>	
				...
				<cfquery name="InsertaFAX004" datasource="CAJAS">
					 insert FAX004 (FAM01COD,  FAX01NTR, FAX04LIN,  FAX01FEC,  FAX04CAN,  FAX04CVD,
								    ...
								    FAX04DES,  FAX04LIE, FAX04CDE,  FAX04NTE, FAM45COD)
						  values ( <cfqueryparam cfsqltype="cf_sql_char" 		value="#ARGUMENTS.CAJA#">,	
                                   ...
                                   <cfqueryparam cfsqltype="cf_sql_smallint" 	value="2">)
	...
	<cffunction name="ActualizaTrans" acess="private" returntype="string">
		  ...
		  <cftry>
			...
			<cfquery name="ActTrans" datasource="CAJAS">
			    update FAX001
				 ...
				 where FAM01COD = '#ARGUMENTS.CAJA#'
			  	...
			  	insert FAX011 (FAM01COD, FAX01NTR, FAX11LIN, FAM09MAQ, FAX11REC, FAX11STS, FAX09SER,
...
*********************************************************************************************--->
    ...
    <cffunction name="insertaFAX014" acess="private" returntype="string">
		  <cfargument name="MONTO" 			type="string" required="Yes">
          <cfargument name="FAX14DOC" 		type="string" required="Yes">
		  <cfargument name="CAJA" 			type="string" required="Yes">
		  <cfargument name="MONCOD" 		type="string" required="Yes">
          <cfargument name="NumTrans" 		type="string" required="Yes">
		  <cfargument name="FAX14CON" 		type="string" required="Yes">
		  <cfargument name="FAX01ECLICOD"	type="string" required="Yes">
		  ...
		  <cftry>
			...
			<cfquery name="ActTrans" datasource="CAJAS">
				...
				insert FAX014 (FAX14MON, FAX14STS, FAX14DOC, FAM01COD, MONCOD, FAX14MAP, FAX14TDC,FAX14CLA,FAX01NTR, FAX14FEC,
							FAM45COD,FAX14DRE2,FAX14DRE,FAX14NDOC,FAX14CON,CLICOD,INDINTERFAZ)
					 values (<cfqueryparam cfsqltype="cf_sql_float" 	value="#ARGUMENTS.MONTO#">,	
					 		 <cfqueryparam cfsqltype="cf_sql_char" 		value="1">,
	 				 		 <cfqueryparam cfsqltype="cf_sql_varchar" 	value="#ARGUMENTS.FAX14DOC#">, 
	 				 		 <cfqueryparam cfsqltype="cf_sql_char" 		value="#ARGUMENTS.CAJA#">,
							 <cfqueryparam cfsqltype="cf_sql_char" 		value="#ARGUMENTS.MONCOD#">,
							 <cfqueryparam cfsqltype="cf_sql_float" 	value="0">,
							 <cfqueryparam cfsqltype="cf_sql_char" 		value="AD">,
							 ...
							 <cfqueryparam cfsqltype="cf_sql_varchar" 	value="#ARGUMENTS.FAX01ECLICOD#">,
							 <cfqueryparam cfsqltype="cf_sql_char" 		value="S">
                             )
				   
			</cfquery>
		    <cfcatch type = "Any">
				<cfset resultado = "#cfcatch.message#, Detalle: #cfcatch.detail#" >
		    </cfcatch>
		  </cftry>
		  <cfreturn "#resultado#">			
	</cffunction>
