...
<cfquery name="qryDefaults" datasource="#session.DSN#">
	...
	if not exists(select * from ParametrosGenerales where Ecodigo = #session.Ecodigo# and PGnombre='Creditos')
	  insert into ParametrosGenerales (Ecodigo, PGgrupo, PGsecuencia, PGnombre, PGdescripcion, PGtipo, PGvalor) values 	(#session.Ecodigo#, 10,1, 'Creditos','Nombre de la unidad de medida de la carga académica por curso', 'C', 'Creditos')
	if not exists(select * from ParametrosGenerales where Ecodigo = #session.Ecodigo# and PGnombre='Facultad')
	...
	if not exists(select * from ParametrosGenerales where Ecodigo = #session.Ecodigo# and PGnombre='Escuela')
	...
	if not exists(select * from ParametrosGenerales where Ecodigo = #session.Ecodigo# and PGnombre='OfertaAca1')
	...
	if not exists(select * from ParametrosGenerales where Ecodigo = #session.Ecodigo# and PGnombre='OfertaAca2')
	...
	if not exists(select * from ParametrosGenerales where Ecodigo = #session.Ecodigo# and PGnombre='MatriculaLinea')
	...
	if not exists(select * from ParametrosGenerales where Ecodigo = #session.Ecodigo# and PGnombre='MatriculaPago')
	...
	if not exists(select * from Monedas where Ecodigo = #session.Ecodigo#)
	  insert into Monedas (Ecodigo, Mnombre, Msimbolo, Miso4217)
	  select #session.Ecodigo#, Mnombre, Msimbolo, Miso4217
	    from asp..Moneda

	if not exists(select * from TarifasTipo where Ecodigo = #session.Ecodigo#)
	begin
	...
	if not exists(select * from TarifasTipo where Ecodigo = #session.Ecodigo# and TTtipo=3)
	begin
	  insert into TarifasTipo (Ecodigo, TTnombre, TTtipo)
	  values (#session.Ecodigo#, 'Tarifa de Otros Items',3)
	end

	if not exists(select * from GradoAcademico where Ecodigo = #session.Ecodigo#)
	begin
	...
	if not exists(select * from TablaEvaluacion where Ecodigo = #session.Ecodigo#)
	begin
	  insert into TablaEvaluacion (Ecodigo, TEnombre, TEtipo) values (#session.Ecodigo#, 'Tabla A-B-C-D-E-F','1')
	  select @PK = @@identity
	...
	if not exists(select * from ConceptoEvaluacion where Ecodigo = #session.Ecodigo#)
	begin
	  declare @CEcodigo numeric
	  insert into ConceptoEvaluacion (Ecodigo, CEnombre, CEdescripcion, CEorden) values (#session.Ecodigo#, 'Exámenes cortos','',1)
	  select @CEcodigo = @@identity
	...
	if not exists(select * from CicloLectivoTipo where Ecodigo = #session.Ecodigo#)
	begin
	...
	if not exists (select * from TablaResultado where Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.Ecodigo#">)
	BEGIN
		...
		insert into TablaResultadoRango (TRcodigo,TRRtipo,TRRnombre,TRRetiqueta,TRRminimo,TRRmaximo) values(@TRcodigo,'1','Para Ganar el curso','Aprobado',70,100)
		insert into TablaResultadoRango (TRcodigo,TRRtipo,TRRnombre,TRRetiqueta,TRRminimo,TRRmaximo) values(@TRcodigo,'2','Requiere Ampliación','Aplazado',60,69.99)
		insert into TablaResultadoRango (TRcodigo,TRRtipo,TRRnombre,TRRetiqueta,TRRminimo,TRRmaximo) values(@TRcodigo,'3','Para Perder el curso','Reprobado',0,59.99)
	END
</cfquery>
