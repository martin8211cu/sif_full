<cfcomponent>
<script type="text/javascript" language="JavaScript" src="/cfmx/educ/js/pLista1.js"></script>
<cffunction name="pListaEdu" access="public" returntype="string" output="true">
	...
	<cfargument name="formatos" type="string" required="true" default="<!--- Formato para cada columna (M,D,I,N,F,IMG)--->">
	...
	<cfargument name="irA" type="string" required="true" default="_self"><!--- Action del Form --->
	<cfargument name="checkboxes" type="string" required="false" default="N"><!--- Indica si se colocar checkboxes y en que lado se colocan --->
	<cfargument name="debug" type="string" required="false" default="N<!--- Activa el Debug (S,N)--->">
	<cfargument name="MaxRows" type="numeric" required="false" default="20"><!--- Cantidad maxima de filas por pantalla, 0 indica que no hay máximo --->
	<cfargument name="Cortes" type="string" required="false" default=""><!--- columnas que van a servir de corte --->
	<cfargument name="navegacion" type="string" required="false" default=""><!--- Parametros que hay que pasar durante la navegación, formato: key=valor[& key=valor] --->
	<cfargument name="botones" type="string" required="false" default=""><!--- nombre de los botones que se van a colocar --->
	<cfargument name="incluyeForm" type="string" required="false" default="true"><!--- indica si la lista va a contener formulario o es parte de un formulario externo, cada boton va a tener una funcion asociada automaticamente, para implementarla solo hay que crear una funcion cuyo nombre es "func"+nombre del boton --->
	<cfargument name="formName" type="string" required="false" default="lista"><!--- nombre del formulario --->
	<cfargument name="formAttr" type="string" required="false" default=""><!--- Attributos adicionales del formulario --->
	<cfargument name="keys" type="string" required="false" default=""><!--- columnas que constituyen las llaves que se van a enviar a traves del form --->
	<cfargument name="checkedcol" type="string" required="false" default=""><!--- columna que contiene el valor de la suma de las llaves que vienen en keys, utilizado para seleccionar de entrada un checbox --->
	<cfargument name="Conexion" type="string" required="false" default="#Session.DSN#"><!--- Conexion a base de datos --->
	<cfargument name="funcion" type="string" required="false" default=""><!--- función que se quiere agregar a cada uno de los links --->
	<cfargument name="funcionByCols" type="boolean" required="false" default="false"><!--- Indica si se quiere colocar una función por cada columna --->
	<cfargument name="fparams" type="string" required="false" default=""><!--- Columnas del query que van formar parte de los argumentos de la funcion especificada --->
	<cfargument name="showLink" type="boolean" required="false" default="true"><!--- Indica si la lista va a tener link --->
	<cfargument name="showEmptyListMsg" type="boolean" required="false" default="false"><!--- Indica si se muestra el mensaje cuando no hay registros en la lista --->
	<cfargument name="EmptyListMsg" type="string" required="false" default="--- No se encontraron Registros ---"><!--- Mensaje por default cuando la lista no contiene registros --->
	<cfargument name="checkAll" type="boolean" required="false" default="false"><!--- Indica si se coloca el checkbox para checkear todos los elementos de la lista --->
	...
	<!--- Inicio de Tabla --->
	<table id="tbLista" border="0" cellspacing="0" cellpadding="0" width="100%">		
	...
	<cfoutput query="rsLista" startrow="#StartRow_lista#" maxrows="#MaxRows_lista#">
		...
		<td align="left" width="18" height="18" <cfif showLink>onClick="javascript: Procesar('#rsLista.CurrentRow#','#ArrayLen(columnas)#','#formName#'<cfif not incluyeForm>, '#irA#'</cfif>);"</cfif> nowrap>
			<cfif comparenocase(datosBD,datosPost) EQ 0 and not isdefined("Nuevo")>
				<img src="/cfmx/educ/imagenes/lista/addressGo.gif" width="18" height="18"> 
			</cfif>
		</td>
		...
		<cfloop index="cName" list="#desplegar#">
			<cfset func = "">
			<cfif not funcionByCols and Len(Trim(funcion)) NEQ 0>
				<cfset func = funcion>
			<cfelseif funcionByCols and Len(Trim(funcion)) NEQ 0 and col LE ListLen(funcion,',')>
				<cfset func = Trim(ListGetAt(funcion, col, ','))>
			</cfif>
			<!--- Se genera la Salida de la Lista --->
			<td <cfif isdefined("fmt") and Trim(fmt[col]) EQ "IMG">width="20"</cfif> align="#Trim(alin[col])#" <cfif ajustar NEQ "S">nowrap</cfif> <cfif Len(Trim(func)) NEQ 0>onClick="javascript: #func##f#" style="cursor: pointer"<cfelseif showLink>onClick="javascript: Procesar('#rsLista.CurrentRow#','#ArrayLen(columnas)#','#formName#'<cfif not incluyeForm>, '#irA#'</cfif>);" style="cursor: pointer"</cfif>>
				
<!--- 				<cfif not (isdefined("fmt") and Trim(fmt[col]) EQ "IMG")>
					<cfif Len(Trim(func)) NEQ 0>
						<a href="javascript: #func##f#" onmouseover="javascript: window.status = ''; return true;" onmouseout="javascript: window.status = ''; return true;" tabindex="-1">
					<cfelseif showLink>
...
<!---
 				<cfif not (isdefined("fmt") and Trim(fmt[col]) EQ "IMG")>
					<cfif showLink OR Len(Trim(func)) NEQ 0></a></cfif>
				</cfif>
 --->
 			</td>
			<cfset col = col + 1>
...
<tr> 
  <td align="center" colspan="#Iif(ucase(checkboxes) EQ 'S' or ucase(checkboxes) EQ 'D', colsp+2, colsp+1)#">
	<cfif PageNum_lista GT 1>
	  <a href="#CurrentPage#?PageNum_lista=1#QueryString_lista#" tabindex="-1"><img src="/cfmx/educ/imagenes/lista/First.gif" border=0></a> 
	</cfif>
	<cfif PageNum_lista GT 1>
	  <a href="#CurrentPage#?PageNum_lista=#Max(DecrementValue(PageNum_lista),1)##QueryString_lista#" tabindex="-1"><img src="/cfmx/educ/imagenes/lista/Previous.gif" border=0></a> 
	</cfif>
	<cfif PageNum_lista LT TotalPages_lista>
	  <a href="#CurrentPage#?PageNum_lista=#Min(IncrementValue(PageNum_lista),TotalPages_lista)##QueryString_lista#" tabindex="-1"><img src="/cfmx/educ/imagenes/lista/Next.gif" border=0></a> 
	</cfif>
	<cfif PageNum_lista LT TotalPages_lista>
	  <a href="#CurrentPage#?PageNum_lista=#TotalPages_lista##QueryString_lista#" tabindex="-1"><img src="/cfmx/educ/imagenes/lista/Last.gif" border=0></a> 
	</cfif> 
</td>
</tr>
</cfoutput>
...
<input type="hidden" name="_ActionTag" value="">
<input type="hidden" name="_Rows" value="#rows#">
