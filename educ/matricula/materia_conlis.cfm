	...
	<cfif LvarRH>
		...
		--->
		<cfif isdefined("btnFiltroPDI") or (isdefined("url.F4") AND url.F4 EQ "-1")>
...
<cfif rsPeriodo.recordCount EQ 0>
		...
		<cfquery name="rsPlanes" datasource="#Session.DSN#">
			...
			select convert(varchar, a.PEScodigo) as PEScodigo, 
			  ...
			  where b.Ecodigo = #session.Ecodigo#
			    and pea.Apersona=<cfqueryparam cfsqltype="cf_sql_numeric" value="#session.matricula.Apersona#">
	...
	<cfif isdefined("btnFiltroPDI") or isdefined("btnFiltroCompetencias")>
		<cfquery name="rsEmpleado" datasource="#session.dsn#">
			select convert(numeric,a.llave) as DEid, lt.RHPcodigo
			from PersonaDatos p, asp..UsuarioReferencia a, LineaTiempo lt
			...
			and convert(numeric,a.llave) = lt.DEid
			and getdate() between LTdesde and LThasta
			and LTdesde = (
				select max(LTdesde) from LineaTiempo
				where LineaTiempo.DEid = lt.DEid
				and getdate() between LineaTiempo.LTdesde and LineaTiempo.LThasta)
	   </cfquery>
	</cfif>
...
<body>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="##CCCCCC">
        <tr> 
          ...
          <td width="41%" rowspan="15" align="center" valign="middle">
		...
		<tr>
          <td>&nbsp;</td>
          <td><input type="checkbox" onClick="javascript:fnCambiarCheckbox(this);" name="btnFiltroPDI" id="btnFiltroPDI" <cfif isdefined("btnFiltroPDI")>checked</cfif>>&nbsp;Plan de Desarrollo Individual.</td>
		</tr>
		<tr>
          <td>&nbsp;</td>
          <td><input type="checkbox" onClick="javascript:fnCambiarCheckbox(this);" name="btnFiltroCompetencias" id="btnFiltroCompetencias" <cfif isdefined("btnFiltroCompetencias")>checked</cfif>><input type="hidden" name="PEScodigo" value="#form.PEScodigo#">&nbsp;Competencias del Puesto.</td>
		</tr>
		<cfelse>
...
<cfif isdefined("btnFiltroMateria") or isdefined("btnFiltroPDI") or isdefined("btnFiltroCompetencias") or form.MPcodigo NEQ "">
	...
	<!--- INICIA PARA PDI O COMPETENCIAS --->
		...
		and b.EScodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Form.EScodigo#">
		...
		<cfif isdefined("btnFiltroCompetencias")>
			select 1
			  from RHConocimientosPuesto d
			 where d.RHPcodigo = <cfqueryparam value="#rsEmpleado.RHPcodigo#" cfsqltype="cf_sql_char">
			   and d.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Session.Ecodigo#">
			   and d.Ecodigo=e.Ecodigo
			   and d.RHCid=e.RHCid
		<cfelse>
			select 1
			  from RHConocimientosPuesto d, RHEEvaluacionCap f, RHDEvaluacionCap g, RHEvaluadoresCap h
			 where d.RHPcodigo = <cfqueryparam value="#rsEmpleado.RHPcodigo#" cfsqltype="cf_sql_char">
			   and d.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Session.Ecodigo#">
			   and d.Ecodigo = e.Ecodigo
			   and d.RHCid = e.RHCid
				...
				(
					select max(x.RHEECfdesde)
					 ...
					 where x.Ecodigo = b.Ecodigo
					   ...
					   and y.DEid = #rsEmpleado.DEid#
					   and x.RHEECestado = 3
				)
			   and f.RHEECid = g.RHEECid
			   and e.RHCid = g.RHCid
			   and #rsEmpleado.DEid# = g.DEid
	...
	UNION
		...
		<cfelse>
			select 1
			  from RHHabilidadesPuesto d, RHEEvaluacionCap f, RHDEvaluacionCap g, RHEvaluadoresCap h
			 where d.RHPcodigo = <cfqueryparam value="#rsEmpleado.RHPcodigo#" cfsqltype="cf_sql_char">
			   and d.Ecodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Session.Ecodigo#">
			   and d.Ecodigo=e.Ecodigo
			   and d.RHHid=e.RHHid
	...
	<script language="JavaScript" type="text/javascript">
		...
		function fnCambiarCheckbox(obj){
			if (obj.name=='btnFiltroPDI'&&obj.checked==true)
			 	document.formmateria_filtro.btnFiltroCompetencias.checked=false;
			if (obj.name=='btnFiltroCompetencias'&&obj.checked==true)
			 	document.formmateria_filtro.btnFiltroPDI.checked=false;
		}
	</script>
</body>
</html>