...
<table width="100%" border="0" cellpadding="0" cellspacing="0" dwcopytype="CopyTableColumn">
	<tr> 
		<td align="right" class="fileLabel">Bloques:</td>
		...
		<TABLE cellSpacing=0 cellPadding=1 border=0 width="100%">
			...
			<cfelse>
			  ...
			  <cfoutput query="rsPBL_MP">
					...
					<cfif isdefined('LvarModificarBloques') and LvarModificarBloques EQ true>
						<TR>
							<TD colSpan=7 class="BloqueMatricula">
							  <INPUT name="PBLnombre_#rsPBL_MP.currentRow#" value="#rsPBL_MP.PBLnombre#" class="BloqueMatricula" size="35">					
							  <input type="image" name="btnPBLcambiar" src="../../imagenes/iconos/folder_save.gif"
									onclick="javascript: 
										...
										document.formPES.PBLnombre.value = document.formPES.PBLnombre_#rsPBL_MP.currentRow#.value;
				...
				<TR>
					<TD>&nbsp;</TD>
					<TD colSpan=6>No existen Materias definidas en el Bloque</TD>
				</TR>
				<cfelse>
			...
			if(document.formPES.PESbloques.value == ''){
			  ...
			  return false;			
				...
				if (parseInt(document.formPES.PESbloques.value) < <cfoutput>#rsPES.PESbloques#</cfoutput>){
					if (confirm('El número de bloques es menor al actual, \nse van a borrar bolques existentes. \n\n¿Desea continuar?')){
						var bloqueInicioABorrar = <cfoutput>#rsPBL_MAX_conMateria.PBLsecuencia#</cfoutput>
						bloqueInicioABorrar;
						if(document.formPES.PESbloques.value < bloqueInicioABorrar)
							document.formPES.PESbloques.value = bloqueInicioABorrar;
					}else{
						return false;
					}
				}
