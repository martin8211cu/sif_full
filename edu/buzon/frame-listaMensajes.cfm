<cfquery name="rsMensajes" datasource="#Session.Edu.DSN#">
...
<cfsavecontent variable="listaMsg">
	<cfinvoke 
	 ...
	 returnvariable="TotalRegistros">
		...
		<cfinvokeargument name="columnas" value="3 as o, convert(varchar, a.Bcodigo) as Bcodigo, a.Borigen, substring(a.Btitulo,1,50) + (case when datalength(a.Btitulo) > 50 then ' ...' else '' end) as Btitulo, a.Bmensaje, convert(varchar, a.Bfecha, 103) + ' ' + substring(convert(varchar, a.Bfecha, 100), charindex(':', convert(varchar, a.Bfecha, 100))-2, 8) as Fecha, case Bestado when 0 then '/cfmx/edu/Imagenes/email/newmail.gif' when 1 then '/cfmx/edu/Imagenes/email/blank.gif' when 2 then '/cfmx/edu/Imagenes/email/blank.gif' when 3 then '/cfmx/edu/Imagenes/email/blank.gif' else '/cfmx/edu/Imagenes/email/blank.gif' end as Bestado"/>
		...
		<cfinvokeargument name="filtro" value="a.Usucodigo = #Session.Edu.Usucodigo# and a.Ulocalizacion = '#Session.Ulocalizacion#' order by a.Bfecha desc, a.Borigen, a.Btitulo"/>
...
<table width="98%" border="0" cellspacing="0" cellpadding="0" align="center">
  ...
  <tr>
    ...
    <td class="MensajesNuevos" align="right" style="padding-right: 10px; color: black;" nowrap>
		<cfoutput>
			<cfif MaxPages GT 1>
				<b>P&aacute;gina <cfif isdefined("Form.PageNum")>#Form.PageNum#<cfelse>1</cfif>/#MaxPages#</b>
			<cfelse>
				&nbsp;
			</cfif>
		</cfoutput>
    ...
    <td width="5%" class="MensajesNuevos" align="right" nowrap>
		<cfoutput>
			<cfif MaxPages GT 1>
				...
				<select name="callPage" onChange="javascript: location.href = '/cfmx/edu/buzon/index.cfm?o=#tabChoice#&PageNum_lista='+this.value;">
				<cfloop from="1" to="#MaxPages#" index="i">
					<option value="#i#" <cfif isdefined("Form.PageNum") and i eq Form.PageNum>selected</cfif>>#i#</option>
				</cfloop>
				</select>
			<cfelse>
				&nbsp;
