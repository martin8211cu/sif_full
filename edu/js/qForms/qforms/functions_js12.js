function _trim(string){return _rtrim(_ltrim(string));
}
function _ltrim(string){return string.replace(/^\s+/, "");
}
function _rtrim(string){return string.replace(/\s+$/, "");
}
function _listToArray(string,delim){var delim=_param(arguments[1], ",");
tmp=string.split(delim);
for(var i=0;i<tmp.length;i++)tmp[i]=_trim(tmp[i]);
return tmp;
}
function _listSum(string,delim){var delim=_param(arguments[1], ",");
tmp=_listToArray(string,delim);
iValue=0;
for(var i=0;i<tmp.length;i++)iValue+=parseInt(_trim(tmp[i]));
return iValue;
}
function _stripInvalidChars(string, type){var string=_param(arguments[0]);
...
else{var p=new RegExp("[^"+type+"]+", "g");
string=string.replace(p, "");
}
return string;
}
function _isLength(string, len, type){var string=_param(arguments[0]);
...
function _getState(abbr){var abbr=_param(arguments[0]).toLowerCase();
...
if(!_s[abbr]){return null;
} else{return _s[abbr];
}}
qFormAPI.sortOptions=new Object();
qFormAPI.sortOptions.order="asc";
qFormAPI.sortOptions.byText=true;
function _sortOptions(obj, order, byText){var order=_param(arguments[1], qFormAPI.sortOptions.order);
if(order!="asc" && order!="desc")order="asc";
var byText=_param(arguments[2], qFormAPI.sortOptions.byText, "boolean");
var orderAsc=(order=="asc")? true : false;
for(var i=0;i<obj.options.length;i++){for(var j=0;j<obj.options.length-1;j++){if(orderAsc &&(byText && obj.options[j].text>obj.options[j+1].text)||(!byText && obj.options[j].value>obj.options[j+1].value)){_swapOptions(obj.options[j], obj.options[j+1]);
} else if(!orderAsc &&(byText && obj.options[j].text<obj.options[j+1].text)||(!byText && obj.options[j].value<obj.options[j+1].value)){_swapOptions(obj.options[j], obj.options[j+1]);
}}}
return true;
}
function _swapOptions(o1, o2){var sText=o1.text;
var sValue=o1.value;
...
function _transferOptions(field1, field2, sort, type, selectItems, reset){var sort=_param(arguments[2], true, "boolean");
...
for(var i=0;i<field1.length;i++){if(doAll || field1.options[i].selected){field2.options[field2.length]=new Option(field1.options[i].text, field1.options[i].value, false, selectItems);
...
function _getURLParams(){struct=new Object();
var strURL=document.location.href;
var iPOS=strURL.indexOf("?");
if(iPOS!=-1){var strQS=strURL.substring(iPOS+1);
var aryQS=strQS.split("&");
} else{return struct;
}
for(var i=0;i<aryQS.length;i++){iPOS=aryQS[i].indexOf("=");
if(iPOS==-1){struct[aryQS[i]]=null;
} else{var key=aryQS[i].substring(0, iPOS);
...
function _createFields(struct, type){var type=_param(arguments[1], "hidden");
if(this.status==null)return false;
for(key in struct){document.write("<INPUT TYPE=\"" + type + "\" NAME=\"" + key + "\" VALUE=\"" + struct[key] + "\">");
}
return true;
}
function _getEventType(type){var strEvent="onblur";
