...
<cfparam name="Attributes.ruta" default="/cfmx/edu/Utiles/Educleerimagencont.cfm"  type="string">

<cfset sql ="select " & Attributes.Campo & " from " & Attributes.Tabla & " where " & replacenocase(Attributes.Condicion,'where','')>
<cfset codigo = "#Attributes.Tabla##cfid#" & GetFileFromPath(GetTemplatePath())>
...
<cfquery name="rs_Imagen" datasource="#Attributes.Conexion#">
	...
	<img name="#Attributes.imgname#" src="#Attributes.ruta#?tipoimg=#iif(ucase(ts) EQ 'GIF',de('gif'),de('jpg'))#&codigo=#codigo#" <cfif not attributes.autosize><cfif attributes.width NEQ 0>width="#Attributes.width#"</cfif> <cfif attributes.height NEQ 0>height="#Attributes.height#"</cfif></cfif> <cfif attributes.border>border="1" bodercolor="#Attributes.bordercolor#"</cfif>>
	<!--- <img name="#Attributes.imgname#" src="#gettempdirectory()##codigo#.#iif(ucase(ts) EQ 'GIF',de('gif'),de('jpg'))#" width="#Attributes.width#" height="#Attributes.height#"> --->
	</cfoutput>
