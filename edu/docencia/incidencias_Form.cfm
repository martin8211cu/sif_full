      ...
      function fnVerificarDatos()
      {
         ...
         if (LvarTipo == "") 
         {
		   ...
		   else
		   {
			 ...
			 alert ('ERROR: Debe escoger un estudiante'); 
             return false;
		   }
         }
         if (LvarTipo == "A")
		 ...
		 {  
           ...
           {
             alert("ERROR: Digite la Observacin");
             return false;
           }
		 }
         if (LvarTipo == "C")
...
<table border="0" cellpading="0" cellspacing="0" width="100%" style="background-color:#E6E6E6;">
    ...
    <td valign="top">
      <table border="0" cellspacing="0" cellpadding="0">
        <tr> 
          ...
          <td class="LinEnc" rowspan="2">Llamada<br>Atencin</td>
          <td class="LinEnc" rowspan="2">Adver-<br>tencia</td>
          <td class="LinEnc" colspan="2">Ausencia</td>
          <td class="LinEnc" colspan="2">Tard a</td>
        ...
        <cfoutput query="qryEstudiantes">
		  ...
		  <cfif Form.hdnEcodigo eq #Codigo#>
            ...
            <tr class="linDet#LvarPar#">
              <td align="right"><a href="javascript:fnTrabajarConIncidencia('#TipoIncidencia#','#Codigo#')">#lsDateFormat(Fecha,"dd/mm/yy")#&nbsp;</a></td>
			  <cfif TipoIncidencia eq "O">
 			    <cfset LvarTipo="Observacin de ">
				<cfif Tipo eq "P">
				  <cfset LvarTipo=LvarTipo & " Reforzamiento">
				<cfelseif Tipo eq "N">
				  <cfset LvarTipo=LvarTipo & " Llamada de Atenci n">
				<cfelseif Tipo eq "A">
			  ...
			  <cfelse>
				...
				<cfif Tipo eq "T">
				  <cfset LvarTipo=LvarTipo & "Llegada Tarda">
				<cfelseif Tipo eq "A">
  ...
  <cfif qryPeriodoCerrado.Cerrado>
      ...
      <div class="LinEnc" style="font-size:12px; border:0;">PERIODO CERRADO</div></div>
      <p align="center">No se puede modificar las incidencias del curso porque el Per odo est cerrado, para abrirlo utilice la opci n calificar perodo</p>
	  <cfset Form.hdnTipo = "">
  </cfif>
    <cfif Form.hdnTipo eq "" or GvarNombre eq "">
      <cfif not qryPeriodoCerrado.Cerrado>
        <div style="border-top:1px solid ##FFFFFF;">
        <div class="LinEnc" style="font-size:12px; border:0;">Nueva Incidencia</div></div>
	  </cfif>
      <input type="text" size="39" name="txtEstudiante" readonly style="font:bold 14px; border:0; background-color:##C0C0C0" value="#GvarNombre#"><br>
      <br>
	  <cfif GvarNombre neq "Escoger un Estudiante" and not qryPeriodoCerrado.Cerrado>
        <input type="radio" name="optTipo" class="linImpar" value="O" onClick="document.frmIncidencias.hdnTipo.value='O';document.getElementById('divObservacion').style.display='';document.getElementById('divAsistencia').style.display='none'; <!--- document.getElementById('divComunicado').style.display='none'; --->"> 
        Observacion<br>
        <input type="radio" name="optTipo" class="linImpar" value="A" onClick="document.frmIncidencias.hdnTipo.value='A';document.getElementById('divObservacion').style.display='none';document.getElementById('divAsistencia').style.display=''; <!--- document.getElementById('divComunicado').style.display='none'; --->"> 
        Control de Asistencia<br>
        <!--- <input type="radio" name="optTipo" class="linImpar" value="C" onClick="document.frmIncidencias.hdnTipo.value='C';document.getElementById('divObservacion').style.display='none';document.getElementById('divAsistencia').style.display='none';document.getElementById('divComunicado').style.display='';">  --->
	  <cfelse>
        <!--- <input type="radio" name="optTipoComunicado" class="linImpar" value="C" checked>  --->
	  </cfif>
      <!--- Comunicado<br> --->
	  <cfset LvarCodigo = "">
	  <cfset LvarFecha = lsDateFormat(now(), "DD/MM/YYYY")>
	  <cfset LvarTipo = "">
	  <cfset LvarMsg = "">
	  <cfset LvarJustificado = "">
	  <cfset LvarAsunto = "">
    <cfelseif Form.hdnTipo eq "O">
      <div style="border-top:1px solid ##FFFFFF;">
      <div class="LinEnc" style="font-size:12px; border:0;">Trabajar con Incidencia</div></div>
      <input type="text" size="39" name="txtEstudiante" readonly style="font: bold 14px; border:0; background-color:##C0C0C0" value="#GvarNombre#"><br>
	  <br>
      <input type="radio" name="optTipo" class="linImpar" value="O" checked> 
      Observacion<br>
      <cfquery datasource="#Session.Edu.DSN#" name="qryIncidencia">
	    select ACOfecha as Fecha, ACOtipo as Tipo, ACOobservacion as Msg
		  from AlumnoCursoObservacion
		 where ACOcodigo = #Form.hdnCodigo#
	  </cfquery>
	  <cfset LvarCodigo = Form.hdnCodigo>
	  <cfset LvarFecha = lsDateFormat(qryIncidencia.Fecha, "DD/MM/YYYY")>
	  <cfset LvarTipo = qryIncidencia.Tipo>
	  <cfset LvarMsg = qryIncidencia.Msg>
	  <cfset LvarJustificado = "">
	  <cfset LvarAsunto = "">
    <cfelseif Form.hdnTipo eq "A">
      ...
      <input type="text" size="39" name="txtEstudiante" readonly style="font:bold 14px; border:0; background-color:##C0C0C0" value="#GvarNombre#"><br>
	  <br>
      <input type="radio" name="optTipo" class="linImpar" value="A" checked> 
      Control de Asistencia<br>
      <cfquery datasource="#Session.Edu.DSN#" name="qryIncidencia">
	    set nocount on
	    select ACAfecha as Fecha, ACAtipo as Tipo, ACAjustificado as Justificado, 
			   ACAjustificador as Asunto, ACAjustificacion as Msg
		  from AlumnoCursoAsistencia
	  ...
	  <cfset LvarTipo = qryIncidencia.Tipo>
	  <cfset LvarMsg = qryIncidencia.Msg>
	  <cfset LvarJustificado = qryIncidencia.Justificado>
	  <cfset LvarAsunto = qryIncidencia.Asunto>
	</cfif>
    <cfif Form.hdnTipo eq "" or Form.hdnTipo eq "A">
  <div id="divAsistencia" style="<cfif Form.hdnTipo eq "">display: none; </cfif>width: 285">
    <table border="0" width="100%">
      <tr>
        <td width="80">Fecha:</td>
        <td><cf_txtFecha name="txtFechaA" value="#LvarFecha#"></td>
      </tr>
      <tr>
        <td width="80">Tipo:</td>
        <td>
          <input type="radio" name="optTipoA" class="linImpar" value="A" <cfif Form.hdnTipo eq "" or LvarTipo eq "A">checked</cfif>>Ausencia<br>
          <input type="radio" name="optTipoA" class="linImpar" value="T" <cfif LvarTipo eq "T">checked</cfif>>Llegada Tard a<br>
          <input type="radio" name="optTipoA" class="linImpar" value="R" <cfif LvarTipo eq "R">checked</cfif>>Salida Temprano
        </td>
      </tr>
      <tr>
        <td width="80">Justificada:</td>
        <td><input type="checkbox" name="chkJustificado" class="linImpar" value="S" <cfif LvarJustificado eq "S">checked</cfif>></td>
      </tr>
      <tr>
        <td width="80">Quien Justifica?</td>
        <td><input type="text" size="20" name="txtJustifica" value="#LvarAsunto#" onChange="document.frmIncidencias.chkJustificado.checked = (this.value != '');"></td>
      </tr>
    </table>
    Justificacin
    <textarea rows="4" cols="45" style="font:10px Verdana, Arial, Helvetica, sans-serif;" name="txtJustificacion">#LvarMSG#</textarea>
  </div>
	</cfif>
    <cfif Form.hdnTipo eq "" or Form.hdnTipo eq "O">
  <div id="divObservacion" style="<cfif Form.hdnTipo eq "">display: none; </cfif>width: 285">
    <table border="0" width="100%">
      <tr>
        <td width="80">Fecha:</td>
        <td><cf_txtFecha name="txtFechaO" value="#LvarFecha#"></td>
      </tr>
      <tr>
        <td width="80">Tipo:</td>
        <td>
          <input type="radio" name="optTipoO" class="linImpar" value="P" <cfif Form.hdnTipo eq "" or LvarTipo eq "P">checked</cfif>>Reforzamiento<br>
          <input type="radio" name="optTipoO" class="linImpar" value="A" <cfif LvarTipo eq "A">checked</cfif>>Advertencia<br>
          <input type="radio" name="optTipoO" class="linImpar" value="N" <cfif LvarTipo eq "N">checked</cfif>>Llamada de Atenci n
        </td>
      </tr>
    </table>
    
    Observacin<br>
    <textarea rows="4" cols="45" style="font:10px Verdana, Arial, Helvetica, sans-serif;" name="txtObservacion">#LvarMSG#</textarea>
  </div>
	</cfif>
	<!---
    <cfif Form.hdnTipo eq "">
  <div id="divComunicado" style="<cfif GvarNombre neq "Escoger un Estudiante" and not qryPeriodoCerrado.Cerrado>display: none; </cfif>width: 285">
    <table border="0" width="100%">
      <tr>
        <td width="80">De:</td>
        <td>#qryOrigen.Nombre#</td>
      </tr>
      <tr>
        <td width="80">Para:</td>
        <td><select size="1" name="chkTipoC"> 
           <option value="E">El Encargado del Alumno</option>
           <option value="A">El Alumno</option>
           <option value="AE">El Alumno y su Encargado</option>
          </select></td>
      </tr>
      <tr>
        <td width="80">Tipo:</td>
        <td>
        <cfif GvarNombre neq "Escoger un Estudiante">
          <input type="radio" name="optTipoC" class="linImpar" value="A" checked>S lo el Alumno<br>
          <input type="radio" name="optTipoC" class="linImpar" value="G">Todo el Grupo<br>
		<cfelse>
          <input type="radio" name="optTipoC" class="linImpar" value="G" checked>Todo el Grupo<br>
		</cfif>
		<cfif form.cboProfesor neq "0">
          <input type="radio" name="optTipoC" class="linImpar" value="T">Todos los Alumnos del<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Profesor 
                                </cfif>
        </td>
      </tr>
      <tr>
	...
	--->
    <cfif Form.hdnTipo eq "">
      <p align="center"><input type="submit" value="Agregar" name="btnAgregar" class="boxNormal" onClick="return fnVerificarDatos();"></p>
    <cfelse>
      <p align="center">
	    <input type="submit" value="Cambiar" name="btnCambiar" class="txtNormal" onClick="return fnVerificarDatos();">
        <input type="submit" value="Borrar" name="btnBorrar" class="txtNormal" onClick="return fnVerificarDatos();">
	  </p>
	</cfif>
	<br>
</cfoutput>
