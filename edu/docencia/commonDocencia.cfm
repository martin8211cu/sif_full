<!---<cfquery datasource="sdc" name="qryRolActual">
  set nocount on
  select Rolcod=convert(varchar,ur.Rolcod)
    from Usuario u, UsuarioRole ur
   where u.Usucodigo = #Session.Edu.Usucodigo#
    and u.Usucodigo = ur.Usucodigo
  set nocount off
</cfquery>
<cfset Session.userRoles=ArrayNew(1)>--->

<!---<cfset i = 0>
<cfloop query="qryRolActual">
   <cfset i = i + 1>
   <cfset Session.userRoles[i] = #qryRolActual.Rolcod#>
   <cfif qryRolActual.Rolcod IS "11">
      <cfset Session.comoAsistente = true>
   </cfif>
</cfloop>--->
