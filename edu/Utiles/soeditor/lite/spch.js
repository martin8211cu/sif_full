...
var customerid = ""; //get one here http://www.spellchecker.net/
var schema = "7";
...
if (!is_window_opened) {
	...
	else
	{
		s =
		'<title>Spellchecker.net</title>' +                       
		'<frameset rows="'+((bnr==1)?96:30)+',*,'+botf+',0'+(sohide!=''?',0':'')+'" frameborder="No" framespacing=0 border=0>'
	}	
		s +=
...
if (!is_window_opened) {
		var s = 
			...
			'<input type=hidden name=sohide value="">' +
			'<input type=hidden name=saw value="">' +
			'</form>' +
			'<center><h2>Loading...</h2>';
			if ( sohide != '' )
				s = s + '<a href="" onclick="parent.opener.'+fctrlname+'.focus(); parent.close() //document.location = \''+service_host+doc_process+'?cmd=eos&customerid='+customerid+'&sessionid=0&bnr='+bnr+'&schema='+schema+'&hide='+sohide+'\'; return false//parent.close(); return false">Abort SpellChecker</a>&nbsp;&nbsp;' +
					'<a href="" onclick="eval(\'parent.opener.\'+(parent.opener.'+saw+'.send_action)); parent.opener.focus(); document.location = \''+service_host+doc_process+'?cmd=eos&customerid='+customerid+'&sessionid=0&bnr='+bnr+'&schema='+schema+'&hide='+sohide+'\'; return false">Send Without Checking</a>';
		    s =	s + '<center></center>' +
			'</body>';
...
///+++++++++END++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++        
		...
		f_dst.bnr.value = bnr;
		f_dst.sohide.value = sohide;
		f_dst.saw.value = saw;
		f_dst.submit();
		spellWin.focus();

		if (sohide != "")	is_window_opened = true;
	}
}
