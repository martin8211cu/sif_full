...
<!--
** Seguimiento al Profesor
** Hecho por: 		Rodolfo Jiménez Jara. rjimenez@expressmail.net
** Elaborado en:	Soluciones Integrales, S.A., America Central , Costa Rica
** Fecha: 			14-Mayo-2003
** Comentarios: 
** Este es la misma consulta que el rol administrador, solo que aquí no se pueden cambiar 
...
<cffunction name="fnEsFeriado" returntype="boolean">
	...
	<cfscript>
		sbInitFromSession("Ncodigo", "-1",false);
...
<cfif Len(Trim(NivelesDir)) EQ 0>
	<table width="95%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  <tr>
		<td align="center">
			<strong>Usted no tiene ning&uacute;n nivel asignado por el momento</strong>
		</td>
	  </tr>
	  <tr>
		<td align="center">&nbsp;
...
<!--- <cfif not (#form.cboProfesor# eq "-999" or #form.cboCurso# eq "-1" or #form.cboPeriodo# eq "-999")> --->
<cfif Len(Trim(NivelesDir)) NEQ 0>
	<cfif not (#form.cboProfesor# eq "-999" or #form.cboPeriodo# eq "-999")>
		...
		<cfquery datasource="#Session.Edu.DSN#" name="qryLimitesPeriodoActividades">
			select 'E' as Tipo, min(e.ECplaneada) as Inicial, max(e.ECplaneada) as Final
			from EvaluacionCurso e
			where 
			#fnCursoEscogido("e.Ccodigo")#
		...
		<cfif GvarBuscarFeriados or (isDefined("form.btnRecalendarizar") and form.cboTipoRecal eq "Correr")>
			<cfquery datasource="#Session.Edu.DSN#" name="qryCalendarioCod">
				select Ccodigo
		...
		<cfquery datasource="#Session.Edu.DSN#" name="qryActividades">
			select 'Evaluaciones' as tipo, '2' as orden, 'E' as TipoOperacion, 
				ECplaneada as fecha, count(*) as Cantidad
			from EvaluacionCurso e, Curso c, Materia m
			where e.PEcodigo   = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.cboPeriodo#">
				and #fnCursoEscogido("e.Ccodigo")#
				<cfif isdefined("form.Ncodigo") and form.Ncodigo NEQ -1>
					and Ncodigo =  <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.Ncodigo#">
				<cfelseif isdefined("form.Ncodigo") and form.Ncodigo EQ -1>
					and Ncodigo in (#NivelesDir#)
				</cfif> 
				and e.Ccodigo = c.Ccodigo 
	  ...
	  <table border="0" cellspacing="0" cellpadding="3" width="100%">
		  ...
		  <td colspan="3" class="areaFiltro" nowrap>
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			  <tr>
				...
				<td width="34%" align="left" nowrap>Nivel : 
				  <select name="Ncodigo" style="font:10px Verdana, Arial, Helvetica, sans-serif;" 
				   onChange='javascript: fnReLoad();'>
					<cfif isdefined("form.Ncodigo") and #form.Ncodigo# EQ "-1">
					  <option value="-1" selected>* * TODOS LOS NIVELES * *</option>
					  <cfelse>
					  <option value="-1">* * TODOS LOS NIVELES * *</option>
					</cfif>
					<cfoutput query="rsNiveles"> 
					  <option value="#rsNiveles.Ncodigo#"<cfif isdefined("form.Ncodigo") and #form.Ncodigo# EQ #rsNiveles.Ncodigo#>selected</cfif>>#rsNiveles.Ndescripcion#</option>
					</cfoutput> </select></td>
				<td nowrap width="33%">Profesor: 
				  <select name="cboProfesor"
					  style="font:10px Verdana, Arial, Helvetica, sans-serif;"
					  onChange='if (this.value != "-999") fnReLoad();'>
					<!---  <cfif (isdefined("RolActual") and RolActual EQ 11) or (isdefined("RolActual") and RolActual EQ 12)>
					  <option value="-999" selected ></option>
					</cfif> --->
					<cfset LvarSelected="0">
					<cfoutput query="qryProfesores"> 
					  <option value="#qryProfesores.Codigo#"<cfif #form.cboProfesor# eq #qryProfesores.Codigo#> selected<cfset LvarSelected="1"></cfif>>#qryProfesores.Descripcion#</option>
					</cfoutput> 
					<cfif #LvarSelected# eq "0" and form.cboProfesor neq "-999">
					  <cfset form.cboProfesor="-999" >
					</cfif>
					<cfif #qryProfesores.RecordCount# eq "0">
					  <option value="-1">* * NO HAY PROFESORES * *</option>
					</cfif>
				  </select>
			  ...
			  <tr> 
				...
				<td width="34%" align="left" nowrap>Curso: 
				  <select name="cboCurso" 
					...
					<cfoutput query="qryCursos"> 
					  <option value="#qryCursos.Codigo#"<cfif #form.cboCurso# eq #qryCursos.Codigo#> selected<cfset LvarSelected="1"></cfif>>#qryCursos.Descripcion#</option>
					</cfoutput> 
					<cfif #LvarSelected# eq "0" and form.cboCurso neq "-1">
					  <cfset form.cboCurso="-1">
					</cfif>
				  </select></td>
				<td width="33%" nowrap>Período : 
				  <select name="cboPeriodo" 
				style="font:10px Verdana, Arial, Helvetica, sans-serif;"
				 onChange='if (this.value != "-999") fnReLoad();'>
					<cfoutput query="qryPeriodos"> 
					  <option value="#qryPeriodos.Codigo#"<cfif form.cboPeriodo eq qryPeriodos.Codigo > selected<cfset LvarSelected="1"></cfif>>#qryPeriodos.Descripcion#</option>
					</cfoutput> </select> </td>
		...
		<tr> 
		  <td width="10%" rowspan="6" align="right" valign="middle" style="padding-left: 10px; padding-right: 10px" nowrap> 
			<table border="1">
			  <tr> 
				<td align="right"> 
					<!--- <cf_Educleerimagen Tabla="PersonaEducativo" ruta="/cfmx/edu/Utiles/sifleerimagencont.cfm" Campo="Pfoto" Condicion="persona=#LvarPersonaDocente#" Conexion="#Session.Edu.DSN#" autosize="false" width="85" height="113">  --->
					<cf_LoadImage tabla="PersonaEducativo" columnas="Pfoto as contenido, persona as codigo" condicion="persona=#LvarPersonaDocente#" imgname="Foto" width="85" height="113">
				</td>
			  </tr>
			</table>
		  </td>
		  <td width="5%">&nbsp;</td>
		  <td width="95%" rowspan="6" valign="top" nowrap><cfoutput> 
			  <table width="100%" border="0" cellspacing="0" cellpadding="2">
				<tr> 
				  <!--- <td colspan="4" align="center" class="tituloAlterno" style="border-bottom: solid ##000000 1px; border-top: solid ##000000 1px;"><a href="javascript: ligaMensajeria('#trim(form.cboProfesor)#','#trim(form.cboPeriodo)#');">O</a>pciones 
					de Men&uacute;</td> --->
					<td colspan="4" align="center" class="tituloAlterno" style="border-bottom: solid ##000000 1px; border-top: solid ##000000 1px;">Opciones 
					de Men&uacute;</td>
				</tr>
				<tr> 
				  <td style="padding-right: 5px;"><div align="right"> <a href="javascript:a=window.open('ConlisReporteProgresoDIR.cfm?Nivel=#form.Ncodigo#&profesor=#trim(form.cboProfesor)#&Curso=#trim(form.cboCurso)#&Periodo=#trim(form.cboPeriodo)#', 'ReporteProgreso','left=50,top=10,scrollbars=yes,resiable=yes,width=700,height=550,alwaysRaised=yes');a.focus();"><img src="/cfmx/edu/Imagenes/evaluaciones2.gif" border="0" title="Reporte de Progreso del Curso"></a></div> 
				  </td>
				  <td nowrap style="padding-right: 10px;"><font size="2"><a href="javascript:a=window.open('ConlisReporteProgresoDIR.cfm?Nivel=#form.Ncodigo#&profesor=#trim(form.cboProfesor)#&Curso=#trim(form.cboCurso)#&Periodo=#trim(form.cboPeriodo)#', 'ReporteProgreso','left=50,top=10,scrollbars=yes,resiable=yes,width=700,height=550,alwaysRaised=yes');a.focus();">Notas 
					de Progreso</a></font></td>
				  <td  style="padding-right: 5px;"><div align="right"><a href="javascript:fnConsultarObservaciones('O',document.frmActividades.cboCurso.value);"><img src="/cfmx/edu/Imagenes/observaciones.gif" border="0" title="Observaciones y Anotaciones"></a></div></td>
				  <td nowrap style="padding-right: 10px;"><font size="2"><a href="javascript:fnConsultarObservaciones('O',document.frmActividades.cboCurso.value);">Observaciones 
					y Anotaciones</a></font></td>
				</tr>
				<tr> 
				  <td style="padding-right: 5px;"><div align="right"> <a href="javascript:fnConsultarCursoDet('T',document.frmActividades.cboCurso.value);"><img src="/cfmx/edu/Imagenes/tema.gif" border="0" title="Temarios"></a> </div>
				  </td>
				  ...
				  <td align="right" nowrap style="padding-right: 5px;"> 
					<div align="right"><a href="javascript:fnConsultarCursoDet('E',document.frmActividades.cboCurso.value);"><img src="/cfmx/edu/Imagenes/evaluaciones.gif" border="0" title="Plan de Evaluaciones"></a></div></td>
				...
				<tr> 
				  <td style="padding-right: 5px;"><div align="right"> 
					 <!--- <a href="javascript:a=window.open('/cfmx/edu/responsable/comunicados.cfm', 'Comunicados','left=50,top=10,scrollbars=yes,resiable=yes,width=500,height=350,alwaysRaised=yes','Comunicados al Profesor');a.focus();"><img src="/cfmx/edu/Imagenes/comunicado.gif" border="0" title="Comunicados"></a> </div> --->
					 <a href="/cfmx/edu/buzon/index.cfm?a=#LSDateFormat(Now(), 'ddmmyyyy')##LSTimeFormat(Now(),'hhmmss')#"><img src="/cfmx/edu/Imagenes/comunicado.gif" border="0" title="Comunicados"></a> </div>
				  </td>
				  <td nowrap style="padding-right: 10px;"><font size="2">
					 <!--- <a href="javascript:a=window.open('/cfmx/edu/responsable/comunicados.cfm', 'Comunicados','left=50,top=10,scrollbars=yes,resiable=yes,width=500,height=350,alwaysRaised=yes','Comunicados');a.focus();">Comunicados</a> --->
					 <a href="/cfmx/edu/buzon/index.cfm?a=#LSDateFormat(Now(), 'ddmmyyyy')##LSTimeFormat(Now(),'hhmmss')#">Comunicados</a>
				  </font></td>
				  <td align="right" nowrap style="padding-right: 5px;"> 
					<div align="right"><a href="ProgressReportGroup.cfm"><img src="/cfmx/edu/Imagenes/evaluaciones2.gif" border="0" title="Plan de Evaluaciones"></a></div></td>
				  <td nowrap style="padding-right: 10px;"><font size="2"><a href="ProgressReportGroup.cfm">Reporte
				        de Progreso por Grupo</a></font></td>
				</tr>			
			  </table>
			</cfoutput></td>
		</tr>
			  ...
			  <td class="CeldaHdr">
				...
				<cfinvoke 
				 ...
				 returnvariable="wHintRet">
					...
					<cfinvokeargument name="mensaje" value="Haga click aquí para ver actividades de la semana"/>
		...
		<cfif form.hdnFecha neq "">
			...
			<cfquery datasource="#Session.Edu.DSN#" name="qryActividades">
			  ...
			  <cfloop condition="lsDateFormat(LvarFechaI,'YYYYMMDD') lte lsDateFormat(LvarFechaFin,'YYYYMMDD')">
				select convert(datetime,'#lsDateFormat(LvarFechaI,"YYYYMMDD")#') as Fecha,
					   c.Ccodigo     as Curso,
					   case m.Melectiva when 'S' then c.Cnombre else m.Mnombre + ' ' + g.GRnombre end as CursoNombre,
					   'HORARIO'     as Tipo, 
					   'H'           as TipoOperacion, 
					   0             as Codigo,
					   ...
					   ''            as Realizado,
					   1             as Orden
				  from HorarioGuia h, Horario hh, Curso c, Materia m, Grupo g
				 where #fnCursoEscogido("h.Ccodigo")# 
					<cfif isdefined("form.Ncodigo") and form.Ncodigo NEQ -1>
			...
			<cfif form.hdnTipoOperacion eq "" or form.hdnTipoOperacion eq "E">
				select ECplaneada,
					   c.Ccodigo,
					   case m.Melectiva when 'S' then c.Cnombre else m.Mnombre + ' ' + g.GRnombre end,
					   'EVALUACION', 
					   'E', 
					   ECcomponente,
					   ECnombre,
					   ''
						,
					   2
				  from EvaluacionCurso ec, Curso c, Materia m, Grupo g
				 where #fnCursoEscogido("ec.Ccodigo")#
					<cfif isdefined("form.Ncodigo") and form.Ncodigo NEQ -1>
						and m.Ncodigo =  <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.Ncodigo#">
					<cfelseif isdefined("form.Ncodigo") and form.Ncodigo EQ -1>
						and m.Ncodigo in (#NivelesDir#)
				   ...
				   and ECplaneada 
				   <cfif LvarFechaFin eq "">
						= '#lsDateFormat(LvarFecha,"YYYYMMDD")#'
				   <cfelse>
						between '#lsDateFormat(LvarFecha,"YYYYMMDD")#' and '#lsDateFormat(LvarFechaFin,"YYYYMMDD")#'
				   </cfif>
				   and ec.Ccodigo = c.Ccodigo
				   and m.Mconsecutivo = c.Mconsecutivo
				   and c.GRcodigo *= g.GRcodigo
			  </cfif> 
			 <!--- se quito el alumno y ahora se quita este union --->
			  ...
			  <cfif form.hdnTipoOperacion eq "" or form.hdnTipoOperacion eq "T">
				select CPfecha,
					   ...
					   CPcubierto,
					   3
				  from CursoPrograma cp, Curso c, Materia m, Grupo g
				 where #fnCursoEscogido("cp.Ccodigo")#
					<cfif isdefined("form.Ncodigo") and form.Ncodigo NEQ -1>
						and m.Ncodigo =  <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.Ncodigo#">
					<cfelseif isdefined("form.Ncodigo") and form.Ncodigo EQ -1>
						and m.Ncodigo in (#NivelesDir#)
		...
		<!--- documentado por Rodolfo Jimenez, 21/07/2003, el director solo ve las incidencias generales , no el detalle x p sería mucho en pantala	
			  <cfif form.hdnTipoOperacion eq "">
			  UNION
			  </cfif>
			  <cfif form.hdnTipoOperacion eq "" or form.hdnTipoOperacion eq "O">
				select ACOfecha,
					   ...
					   'N',
					   4
				  from AlumnoCursoObservacion ao, Curso c, Materia m, Grupo g
			  ...
			  <cfif form.hdnTipoOperacion eq "" or form.hdnTipoOperacion eq "A">
				select ACAfecha,
					   ...
					   'N',
					   5
				  from AlumnoCursoAsistencia aa, Curso c, Materia m, Grupo g
			...
			<cfoutput query="qryActividades">
						...
						<table width="100%" cellpadding="0" cellspacing="0">
							...
							<cfloop query="rsHorariosDia">
								<cfif LvarLinPar neq "Par">
									<cfset LvarLinPar="Par"><cfelse><cfset LvarLinPar="Impar">
								</cfif>
								<tr>
			  ...
			  <tr>
				...
				<cfinvoke 
				 ...
				 returnvariable="wHintRet">
					...
					<cfinvokeargument name="mensaje" value="Haga click aquí para ver datos del Curso"/>
					<cfinvokeargument name="imagen" value="true"/>
					<cfinvokeargument name="funcion" value="fnConsultarCurso('C',#LvarCurso#);"/>
			  ...
			  <tr>
				...
				<cfif TipoOperacion neq "H">
					...
					<td align="center" class="Lin#LvarLinPar#"><cfif Realizado eq "S"><img src="/cfmx/edu/responsable/realizado.gif"><cfelseif Realizado neq "N">#Realizado#</cfif></td>
					<cfif LvarLinPar neq "Par">
