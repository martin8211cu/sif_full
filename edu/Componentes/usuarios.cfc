<cfcomponent>
	...
	<cffunction name="getEmpresa" access="private" returntype="query">
		...
		<cfquery name="empresa" datasource="#Session.Edu.DSN#">
	...
	<cffunction name="del_usuario" access="private" returntype="boolean">
		...
		<cftry>
			<cfquery name="delete_Usuario" datasource="#Session.Edu.DSN#">
				set nocount on
	...
	<!--- Obtiene los datos de un Usuario segn las referencias que se le pasen --->
	<cffunction name="get_usuario_by_ref" access="public" returntype="query">
		...
		<cfquery name="usuario" datasource="#Session.Edu.DSN#">
			select distinct 
	...
	<!--- Obtiene los datos de un Usuario seg n el Usucodigo y Ulocalizacion --->
	<cffunction name="get_usuario_by_cod" access="public" output="true" returntype="query">
		<cfargument name="corporativo" type="boolean" required="false" default="false">
		<cfargument name="consecutivo" type="numeric" required="true">
		<cfargument name="sistema" type="string" required="true">
		<cfargument name="Usucodigo" type="numeric" required="true">
	...
	<cffunction name="activate_rol" access="public" returntype="boolean">
		...
		<cftry>
			<cfloop index="cont" from="1" to="#ArrayLen(arrRoles)#">
				<cfquery name="activate_UsuarioPermiso#cont#" datasource="#Session.Edu.DSN#">
	...
	<cffunction name="add_rol" access="public" returntype="boolean">
		...
		<cfloop index="cont" from="1" to="#ArrayLen(arrRoles)#">
			<cfquery name="a_UsuarioPermiso" datasource="#Session.Edu.DSN#">
				if not exists(
					select 1
					from UsuarioPermiso
					where Usucodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Usucodigo#">
				...
				insert UsuarioPermiso 
						(Usucodigo, Ulocalizacion, cliente_empresarial, Ecodigo, rol, num_referencia, int_referencia, BMUsucodigo, BMUlocalizacion, BMUsulogin<cfif isdefined("activacion") and Len(Trim(activacion)) NEQ 0>, activo</cfif>)
	...
	<cffunction name="del_rol" access="public" returntype="boolean">
		...
		<cfset arrRoles = ListToArray(Replace(roles, ' ', '', 'all'), ",")>
		<cfloop index="cont" from="1" to="#ArrayLen(arrRoles)#">
			<cfquery name="del_UsuarioPermiso" datasource="#Session.Edu.DSN#">
				delete UsuarioPermiso
				where Usucodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Usucodigo#">
				  and Ulocalizacion = <cfqueryparam cfsqltype="cf_sql_char" value="#Ulocalizacion#">
				  and cliente_empresarial = <cfqueryparam cfsqltype="cf_sql_numeric" value="#cliente_empresarial#">
		...
		<cfquery name="moreRoles" datasource="#Session.Edu.DSN#">
			select 1
			from UsuarioPermiso
			where Usucodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Usucodigo#">
			  and Ulocalizacion = <cfqueryparam cfsqltype="cf_sql_char" value="#Ulocalizacion#">
		...
		<!--- Si no tiene ms roles borrar el usuario --->
		...
		<cfif isdefined('clienteEmpr') and clienteEmpr.recordCount GT 0>
			<cftry>
				<cfif not userExists>
					...
					<cfquery name="a_Usuario" datasource="#Session.Edu.DSN#">
						set nocount on						
					...
					<!--- Ingreso en UsuarioEmpresarial --->
					<cfquery name="a_UsuarioEmpresarial" datasource="#Session.Edu.DSN#">
						insert UsuarioEmpresarial 
							...
							<cfif isdefined("Pfoto") and Pfoto NEQ "">Pfoto, </cfif>BMUsucodigo, BMUlocalizacion, BMUsulogin)
					...
					<cfquery name="a_UsuarioEmpresa" datasource="#Session.Edu.DSN#">
				...
				<cfelse>
					...
					<cfquery name="u_UsuarioEmpresarial" datasource="#Session.Edu.DSN#">
						update UsuarioEmpresarial set
							...
							BMUsucodigo = <cfqueryparam cfsqltype="cf_sql_numeric" value="#Session.Edu.Usucodigo#">, 
				...
				<cfif isdefined('a_Usuario') and a_Usuario.recordCount GT 0>
					...
					<cfif userExists>
						<!--- Aqui debe de sacar cuales son los roles que no estan marcados, para as  borrarlos   --->
						...
						<cfelse>
							...
							<cfquery name="rsRolesEdu" datasource="#Session.Edu.DSN#">
						...
						<!--- Se espera que el Usuario no tenga ms de una referencia por rol --->
