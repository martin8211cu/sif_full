...
function formato(obj){
	if ( trim(obj.value) != '' ){
		var valor = qf(trim(obj.value));
		var entero = '';
		var decimal = '00';
		if (ESNUMERO(valor) ){
			// recupera la parte entera y la parte decimal
			if ( valor.indexOf('.') != -1 ){
				entero  = valor.substr(0,valor.indexOf('.'));
				decimal = valor.substr(valor.indexOf('.')+1, 2);
			}
			else{
				entero = valor;
			}
			...
			if (entero.length > 3){
				...
				for (var i=entero.length-1; i>=0; i--){
					cont += 1;
					if ( (cont % 3) == 0 && i != 0 ){
						tmp    = ',' + entero.substr(i, entero.length) + tmp;
						entero = entero.substr(0,i);
						cont = 0;
					}
				}
				entero += tmp; 
			}
			else if (entero.length == 0){
				entero = '0';
			}
			
			
		}
		obj.value = entero + '.' + decimal
	}
}
