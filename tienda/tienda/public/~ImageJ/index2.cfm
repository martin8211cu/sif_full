...
<cfset WorkingDirectory=GetDirectoryFromPath( GetCurrentTemplatePath() )>
<cfif IsDefined('form.uploadImagePath')>
	<cffile action="Upload" fileField="form.uploadImagePath"
		destination="#WorkingDirectory#" nameConflict="Overwrite" accept="image/*">
	<cfset form.originalImagePath = WorkingDirectory & cffile.serverFile>
</cfif>
...
<cfif structKeyExists(form,"actionSubmit")>
	<cfif form.format is "auto">
		<cfset form.resultImagePath = WorkingDirectory & "imageJ." & listLast(form.originalImagePath,".")>
	<cfelse>
		<cfset form.resultImagePath = WorkingDirectory & "imageJ." & form.format>
	</cfif>
...
<cfoutput>
<table>
<form action="#cgi.SCRIPT_NAME#" method="post" name="uploadForm" enctype="multipart/form-data">
<tr>
	<td>
	<i>1. Select an image</i> : 
	</td>
	<td>
	<input name="uploadImagePath" type="file" size="45"> 
	</td>
	<td>
...
<cfif doDisplayImage>
	...
	<tr>
		<td valign="top">
			<img src="#GetFileFromPath(form.originalImagePath)#" border="1">
		</td>
		<td valign="top">
...
<cfif doDisplayResult>
	...
	<tr>
		<td valign="top">
			<img src="#GetFileFromPath(form.resultImagePath)#" border="1">
		</td>
		<td valign="top">
