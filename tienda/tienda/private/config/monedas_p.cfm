...
<cfif isDefined("Session.Ecodigo") and isDefined("Form.Mcodigo") and Len(Trim(Form.Mcodigo)) NEQ 0 >
	<cfquery name="rsMonedas" datasource="#Session.DSN#" >
		Select convert(varchar,Mcodigo) as Mcodigo, Mnombre, rtrim(Msimbolo) as Msimbolo, rtrim(Miso4217) as Miso4217, ts_rversion
		from Monedas
...
<cfquery name="rsMonedasPortal" datasource="asp" >
	Select convert(varchar,Mcodigo) as Mcodigo, Mnombre, rtrim(Msimbolo) as Msimbolo, rtrim(Miso4217) as Miso4217	
	from Moneda
	order by Mnombre
</cfquery>
...
<form method="post" name="form1" action="monedas_go.cfm">
  ...
  <table align="center">
    <tr valign="baseline"> 
	  ...
	  <cfif modo NEQ "ALTA">
		...
		<cfset longitud = Len(Trim(rsMonedas.Mnombre)) >
	  	<input type="text" name="Mnombre" value="#Trim(rsMonedas.Mnombre)#" size="40" onfocus="this.select()">
    ...
    <tr valign="baseline"> 
      ...
      <td><input type="text" name="Msimbolo" onfocus="this.select()" value="<cfif modo NEQ "ALTA"><cfoutput>#rsMonedas.Msimbolo#</cfoutput><cfelse><cfoutput>#rsMonedasPortal.Msimbolo#</cfoutput></cfif>" size="3"></td>
    </tr>
    <tr valign="baseline"> 
	  ...
	  <cfoutput>
	  <cfif modo EQ "ALTA">
	  <input type="text" name="Miso4217" readonly="1" onfocus="this.select()" value="#rsMonedasPortal.Miso4217#" size="3">
	  <cfelse>
	  <input type="hidden" name="Miso4217" readonly="1" onfocus="this.select()" value="#rsMonedas.Miso4217#" size="3">
	  <strong>#rsMonedas.Miso4217#</strong>
	  </cfif>
	  </cfoutput>
	  </td>
    </tr>
