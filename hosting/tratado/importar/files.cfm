...
<cfparam name="url.c" type="string" default="1"> <!--- Indica que imput invoco este cfm y asi asignar adecuadamente el valor en el imput que corresponda --->
...
<cfloop list="#url.p#" delimiters="/" index="dirpart">
	...
	<cfif DirectoryExists(path & '/' & dirpart)>
		...
		<td width="16">
		<a href="files.cfm?c=#url.c#&p=#URLEncodedFormat(miniurl)#"><img src="foldericon.png" width="16" height="16" border="0"></a></td>
		<td colspan="#maxlevel-level+3#">
		<a href="files.cfm?c=#url.c#&p=#URLEncodedFormat(miniurl)#">#dirpart#&nbsp;&nbsp;</a> </td>
		</tr>
	<cfelse>
		<cfbreak>
	</cfif>
...
<cfloop query="dir2">
	<tr onMouseOver="files_over(this)" onMouseOut="files_out(this)">
	<cfif type is 'Dir'>
	<td colspan="#level#">&nbsp;</td><td><a href="files.cfm?c=#url.c#&p=#URLEncodedFormat(p & '/' & name)#">
		<img src="foldericon.png" width="16" height="16" border="0"></a></td>
		<td width="100%"><a href="files.cfm?c=#url.c#&p=#URLEncodedFormat(p & '/' & name)#">#name#&nbsp;&nbsp;</a></td>
	<cfelse>
...
<!--
	function seleccionar(name){
		...
		<cfelseif url.c eq '2'>
			window.opener.conlisFilesSelectEX('#JSStringFormat(url.p)#/' + name);
		<cfelseif url.c eq '3'>
			window.opener.conlisFilesSelectPARAM('#JSStringFormat(url.p)#/' + name);
		</cfif>
	}
