...
<cfquery name="rsGpoEmpresa" datasource="#Session.DSN#">
    select a.GEid as Ecodigo, a.GEnombre as Edescripcion, b.Ecodigo as EcodigoEmp
    from AnexoGEmpresa a
    inner join AnexoGEmpresaDet b on a.GEid = b.GEid
...
<cfloop query="rsGpoEmpresa">
    <cfquery name="rsCGIC_Empresa" datasource="#Session.DSN#">
        select 
            MonedaConvertida 
        from CGIC_Empresa
        where Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#rsGpoEmpresa.Ecodigo#">
          and CGICMid = <cfqueryparam cfsqltype="cf_sql_numeric" value="#form.CGICMid#">
    </cfquery>	
    <cfif isdefined("rsCGIC_Empresa.MonedaConvertida") and rsCGIC_Empresa.MonedaConvertida NEQ 1>
...
<cffunction name="ValidaSalida" output="true" access="private">
	...
	<cfif rsValida.Cantidad GT 0>
		<cfquery name="rsValida" datasource="#Arguments.datasource#">
			select e.Ecodigo,e.Edescripcion,c.Cformato, c.Cdescripcion
			from CContables c
				inner join CtasMayor m
				on m.Cmayor = c.Cmayor
				and m.Ecodigo = c.Ecodigo
		...
		<form name="form1" method="post" action="<cfoutput>#LvarIrA#</cfoutput>">
			<table width="100%">
				...
				<cfoutput query="rsValida" group="Ecodigo">
					<tr><td>&nbsp;</td></tr>                
                    <tr>
                        <td><strong>#rsValida.Edescripcion#</strong></td>
                    </tr>
                    <tr>
...
<!---	<cfif Arguments.SaldosConv>--->
		...
		<!--- Obtener la moneda de conversión, cuando se solicitan los saldos convertidos --->
			<cfset lvarMonedaConversion = 0>
            <cfquery name="rs_Monconv" datasource="#arguments.datasource#">
                select Pvalor 
                from Parametros
                where Ecodigo = #rsGpoEmpresa.EcodigoEmp#
                  and Pcodigo = 660
            </cfquery>
...
<!---	<cfelse>
		...
		<cfquery datasource="#Arguments.datasour
	...
	<cfif Arguments.IncluirUtil>
    	...
    	<cfloop query="rsGpoEmpresa">
            ...
            <cfquery name="rsCuentaUtilidad" datasource="#session.dsn#">
                ...
                and Ecodigo = <cfqueryparam cfsqltype="cf_sql_integer" value="#rsGpoEmpresa.Ecodigo#">
            </cfquery>
            ...
            <cfquery datasource="#Arguments.datasource#">
                ...
                where c.Ccuenta = #LvarcuentaUtilidad#
                  and c.Ecodigo = #rsGpoEmpresa.Ecodigo#
                  and ((
                        select count(1)
                        from #LvarTablaSaldos# s
                        where s.Ccuenta = c.Ccuenta
...
<!--- <cf_dump var="#rsObtieneColumnas.recordcount#">
--->	
	<cfset LvarSelect = "">
	<cfset LvarGroup  = "">
...
--->	
	<cfquery name="data" datasource="#Arguments.datasource#">
        ...
        from CGIC_Mapeo m
			...
			and em.Ecodigo in(Select Ecodigo from AnexoGEmpresaDet where GEid=#gpoElimina#)
			...
			inner join CContables c 
				on s.Ccuenta=c.Ccuenta and c.Ecodigo=em.Ecodigo
            inner join Monedas mon
            	on s.Mcodigo = mon.Mcodigo and s.Ecodigo = mon.Ecodigo
