...
<cffunction name="load" output="false" returntype="struct">
	<cfargument name="jarfile" type="string">

	<cfscript>
		var ret = StructNew();
		fileSeparator = CreateObject("java", "java.lang.System").getProperty("file.separator");
		if (Not FileExists(Arguments.jarfile)) {
			return ret;
		}
		form.jarfile=Arguments.jarfile;
		...
		while (zipEntries.hasMoreElements()) {
			...
			if (Not zipEntry.isDirectory()) {
				...
				byteOutputStream = CreateObject("java", "java.io.ByteArrayOutputStream")
					.init();
				...
				inpStream.close();
				ret[entryName] = byteOutputStream.toString();
			}
		}
		zipFile.close();
		return ret;
