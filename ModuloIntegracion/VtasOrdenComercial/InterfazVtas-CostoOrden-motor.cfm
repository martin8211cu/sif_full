...
<!---					Costo = (select distinct(Imp_Costo) from UtilidadBruta where Ecodigo = U.Ecodigo and Moneda = U.Moneda                    and Periodo = U.Periodo and Mes = U.Mes and Orden_Comercial = U.Orden_Comercial)--->					
		...
		<cfif rsIDIntermedia.recordcount GT 0>
			<cfloop query="rsIDIntermedia">
			  <cfquery datasource="#GvarConexion#">
				update #sifinterfacesdb#..ESIFLD_HFacturas_Venta set Utilidad_Reg = 1, Mes_Utilidad_Reg = <cfqueryparam                cfsqltype= "cf_sql_integer" value="#form.fltMes#"> where ID_DocumentoV = <cfqueryparam cfsqltype=                "cf_sql_integer" value="#rsIDIntermedia.ID_DocumentoV#">
			  </cfquery>
			</cfloop>
		</cfif>
		...
		<cfquery name="rsIDIntermediaC" datasource="#GvarConexion#">
			...
			from #sifinterfacesdb#..SIFLD_HCosto_Venta C
		...
		<cfif rsIDIntermediaC.recordcount GT 0>
			<cfloop query="rsIDIntermediaC">
			  <cfquery datasource="#GvarConexion#">
				update #sifinterfacesdb#..SIFLD_HCosto_Venta set Costo_Reg = 1, Mes_Costo_Reg = <cfqueryparam                cfsqltype= "cf_sql_integer" value="#form.fltMes#"> where ID_Mov_Costo = <cfqueryparam cfsqltype=                "cf_sql_integer" value="#rsIDIntermediaC.ID_Mov_Costo#">
			  </cfquery>
			</cfloop>
		</cfif>
