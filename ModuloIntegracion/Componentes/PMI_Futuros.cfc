<!--- ABG: Integracion de Futuros FACT Y NO FACT PMI --->
<!--- La Integracion de Futuros, procesa todas las transacciones de Futuros ya sea ganancia o perdida FACT y NOFACT generando una poliza contable.  --->
...
<cfcomponent extends="Interfaz_Base">
	<cffunction name="Ejecuta" access="public" returntype="string" output="no">
        ...
        <cfquery name="rsIDEVentas" datasource="sifinterfaces">
			...
			where 
				Tipo_Documento = 'FU' and Clas_Venta in ('CE','AB') 
				and Estatus = 10
		...
		<cftry>
			...
			<cfelse>
			 ...
			 <!---Usuario --->
<!---			<cfset UsuInterfaz = UInterfaz(varCEcodigo)>	--->
                ...
                <cfquery datasource="sifinterfaces">
                 ...
                 values(<cfqueryparam cfsqltype ="cf_sql_numeric" value="#Maximus#">,
                        ...
                        <cfif rsEVentas.Clas_Venta EQ "AB"> 
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="Posición abierta futuros ganancia #Trim(year(rsEVentas.Fecha_Venta))#-#Trim(month(rsEVentas.Fecha_Venta))#">,
                            <!---<cfqueryparam cfsqltype="cf_sql_varchar" value="FUT-POS-AB">,--->
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="#dateformat(rsEVentas.Fecha_Venta,'yyyy/mm/dd')#">,
                        <cfelse>
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="Posición cerrada futuros ganancia #Trim(year(rsEVentas.Fecha_Venta))#-#Trim(month(rsEVentas.Fecha_Venta))#">,												
                            <!---<cfqueryparam cfsqltype="cf_sql_varchar" value="FUT-POS-CDA">,--->
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="#dateformat(rsEVentas.Fecha_Venta,'yyyy/mm/dd')#">,
                        </cfif>
                        ...
                        <cfif isdefined("rsEVentas.Clas_Venta") and trim(rsEVentas.Clas_Venta) EQ 'AB'>	
                            1,
                        <cfelse>
                            0,
                        </cfif>
			...
			<cfif isdefined("rsDVentas") and rsDVentas.recordcount GT 0>
                ...
                <cfsilent>
				    ...
				    <cfquery name="rsCuenta" datasource="#session.dsn#">
						select CC.CCTcodigo, CC.CFcuenta as CC, S.SNcodigo, CF.CFformato from SNegocios S 
						left join SNCCTcuentas CC on S.SNcodigo = CC.SNcodigo 
						inner join CFinanciera CF on  CF.CFcuenta = CC.CFcuenta
						...
						and CC.CCTcodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsEVentas.Tipo_Documento#">
					</cfquery>
					<cfif isdefined("rsCuenta") and rsCuenta.recordcount EQ 1>
						<cfset varCFformato  = rsCuenta.CFformato>
					<cfelse>
					...
					<!---REGISTRO DEL ASIENTO DE LA CUENTA DEL BROKER---><br>
					 <cfquery datasource="sifinterfaces">
				      ...
				      values (<cfqueryparam cfsqltype ="cf_sql_numeric" value="#Maximus#">,
							 ...
							 <cfif rsEVentas.Clas_Venta EQ "AB">
								 <cfqueryparam cfsqltype="cf_sql_varchar" value="Posición Abierta Futuros #Trim(year(rsEVentas.Fecha_Venta))#-#Trim(month(rsEVentas.Fecha_Venta))#">,	
							 <cfelse>
							 	<cfqueryparam cfsqltype="cf_sql_varchar" value="Posición Cerrada Futuros #Trim(year(rsEVentas.Fecha_Venta))#-#Trim(month(rsEVentas.Fecha_Venta))#">,	
							 </cfif>
				...
				<cfset IDlinea = IDlinea + 1>
				 ...
				 <cfquery datasource="sifinterfaces">
			      ...
			      values (<cfqueryparam cfsqltype ="cf_sql_numeric" value="#Maximus#">,
						 ...
						 <cfqueryparam cfsqltype ="cf_sql_integer" value="#BuscaMes.Pvalor#">,
						 <cfif rsEVentas.Clas_Venta EQ "AB">
						 	<cfqueryparam cfsqltype="cf_sql_varchar" value="Posición Abierta Futuros #Trim(year(rsEVentas.Fecha_Venta))#-#Trim(month(rsEVentas.Fecha_Venta))#-#Trim(rsDVentas.Contrato_Lin)#-#Trim(rsDVentas.Estado_Venta_Lin)#">,												
						 <cfelse>
							<cfqueryparam cfsqltype="cf_sql_varchar" value="Posición Cerrada Futuros #Trim(year(rsEVentas.Fecha_Venta))#-#Trim(month(rsEVentas.Fecha_Venta))#-#Trim(rsDVentas.Contrato_Lin)#-#Trim(rsDVentas.Estado_Venta_Lin)#">,							
						 </cfif>
						 <cfqueryparam cfsqltype="cf_sql_varchar" value="#Trim(left(rsDVentas.Descripcion,20))#">,
						 <cfqueryparam cfsqltype="cf_sql_varchar" value="#Trim(rsDVentas.Cod_Item)#">,
						 'C',
						 null,
						 null,
						 null,
			...
			<!---<cfset disparaInterfaz(18, #Maximus#, #varEcodigo.EQUcodigoSIF#,#varEcodigo.EQUidSIF#, #UsuInterfaz.Usulogin#, 0)>--->
        ...
        <cfquery name="rsIDECompras" datasource="sifinterfaces">
			...
			where 
				Tipo_Documento = 'FU' and Clas_Compra in ('CE','AB') 
				and Estatus = 10
                <cfif Rupdate.recordcount EQ 0>
                    and 1 = 2
                </cfif> 
		...
		<cftry>
			 <cfquery name="rsECompras" datasource="sifinterfaces">
                select Ecodigo, Origen, ID_DocumentoC, Tipo_Documento, Fecha_Compra, Fecha_Arribo, Fecha_Vencimiento, Contrato,
                     Numero_Documento,  Proveedor, Sucursal, Moneda, Fecha_Tipo_Cambio, Tipo_Cambio, Retencion, 
                     Almacen, Clas_Compra, Dest_Compra, Usuario 
                from ESIFLD_Facturas_Compra
                where 
					...
					<cfquery datasource="sifinterfaces">
					 ...
					 values(<cfqueryparam cfsqltype ="cf_sql_numeric" value="#Maximus#">,
							...
							<cfif rsECompras.Clas_Compra EQ "AB"> 
								<cfqueryparam cfsqltype="cf_sql_varchar" value="Posición abierta futuros perdida #Trim(year(rsECompras.Fecha_Compra))#-#Trim(month(rsECompras.Fecha_Compra))#">,
							<!---<cfqueryparam cfsqltype="cf_sql_varchar" value="FUT-POS-AB">,--->
								<cfqueryparam cfsqltype="cf_sql_varchar" 
								value="#dateformat(rsECompras.Fecha_Compra,'yyyy/mm/dd')#">,
							<cfelse>
								<cfqueryparam cfsqltype="cf_sql_varchar" value="Posición cerrada futuros perdida #Trim(year(rsECompras.Fecha_Compra))#-#Trim(month(rsECompras.Fecha_Compra))#">,												
							<!---<cfqueryparam cfsqltype="cf_sql_varchar" value="FUT-POS-CDA">,--->
								<cfqueryparam cfsqltype="cf_sql_varchar" 
								value="#dateformat(rsECompras.Fecha_Compra,'yyyy/mm/dd')#">,
							</cfif>
							...
							getdate(),
							<cfqueryparam cfsqltype="cf_sql_varchar" value="#UsuInterfaz.Usulogin#">,
							<cfif isdefined("rsECompras.Clas_Compra") and trim(rsECompras.Clas_Compra) EQ 'AB'>	
								1,
							<cfelse>
								0,
							</cfif>
			...
			<cfif isdefined("rsDCompras") and rsDCompras.recordcount GT 0>
                ...
                <cfsilent>
					...
					<cfquery name="rsCuenta" datasource="#session.dsn#">
						...
						and CC.CCTcodigo = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsECompras.Tipo_Documento#">
					</cfquery>
					<cfif isdefined("rsCuenta") and rsCuenta.recordcount EQ 1>
						<cfset varCFformato  = rsCuenta.CFformato>
					<cfelse>
					...
					<!---REGISTRO DEL ASIENTO DE LA CUENTA DEL BROKER---><br>
					 <cfquery datasource="sifinterfaces">
				      ...
				      values (<cfqueryparam cfsqltype ="cf_sql_numeric" value="#Maximus#">,
							 ...
							 <cfqueryparam cfsqltype ="cf_sql_integer" value="#Mes#">,
							 <cfif rsECompras.Clas_Compra EQ "AB">
								<cfqueryparam cfsqltype="cf_sql_varchar" value="Posición Abierta Futuros          		                                #Trim(year(rsECompras.Fecha_Compra))#-#Trim(month(rsECompras.Fecha_Compra))#">,												
							 <cfelse>
								<cfqueryparam cfsqltype="cf_sql_varchar" value="Posición Cerrada Futuros                                #Trim(year(rsECompras.Fecha_Compra))#-#Trim(month(rsECompras.Fecha_Compra))#">,												
						 	</cfif>
				...
				<cfset IDlinea = IDlinea + 1>
				 ...
				 <cfquery datasource="sifinterfaces">
			      ...
			      values (<cfqueryparam cfsqltype ="cf_sql_numeric" value="#Maximus#">,
						 ...
						 <cfqueryparam cfsqltype ="cf_sql_integer" value="#BuscaMes.Pvalor#">,
                          <cfif rsECompras.Clas_Compra EQ "AB">
						 	<cfqueryparam cfsqltype="cf_sql_varchar" value="Posición Abierta Futuros #Trim(year(rsECompras.Fecha_Compra))#-#Trim(month(rsECompras.Fecha_Compra))#-#Trim(rsDCompras.Contrato_Lin)#-#Trim(rsDCompras.Estado_Venta_Ref)#">,												
						 <cfelse>
							<cfqueryparam cfsqltype="cf_sql_varchar" value="Posición Cerrada Futuros                            #Trim(year(rsECompras.Fecha_Compra))#-#Trim(month(rsECompras.Fecha_Compra))#-#Trim(rsDCompras.Contrato_Lin)#-#Trim(rsDCompras.Estado_Venta_Ref)#">,							
						 </cfif>
						 <cfqueryparam cfsqltype="cf_sql_varchar" value="#Trim(left(rsDCompras.Descripcion,20))#">, 
						 <cfqueryparam cfsqltype="cf_sql_varchar" value="#Trim(rsDCompras.Cod_Item)#">,
						 'D',
						 null,
						 null,
						 null,
            ...
            <cfif rsVerificaC.Registros LT 1 and rsVerificaV.Registros LT 1 > 
                ...
                <cftry>
                    <cfquery datasource="sifinterfaces">
                        insert into IE18
                         ...
                         from SIFLD_IE18 a 
                         where exists (select 1 from ESIFLD_Facturas_Compra where a.ID = ID18 and Estatus in (92))
                            or exists (select 1 from ESIFLD_Facturas_Venta where a.ID = ID18 and Estatus in (92))
                    </cfquery>
                    <cfquery datasource="sifinterfaces">
                        insert into ID18 
                            (ID, 
                ...
                <cfcatch>
                    ...
                    <cfquery datasource="sifinterfaces">
                        insert into SIFLD_Errores 
                        (Interfaz, Tabla, ID_Documento, MsgError, MsgErrorDet, MsgErrorSQL, MsgErrorParam, MsgErrorPila, Ecodigo, Usuario)
                        select 'G_Futuros', 'ESIFLD_Facturas_Venta', ID_DocumentoV,
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="#Mensaje#">,
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="#Detalle#">,
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="#SQL#">,
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="#PARAM#">,
                    ...
                    <cfquery datasource="sifinterfaces">
                        insert into SIFLD_Errores 
                        (Interfaz, Tabla, ID_Documento, MsgError, MsgErrorDet, MsgErrorSQL, MsgErrorParam, MsgErrorPila, Ecodigo, Usuario)
                        select 'P_Futuros', 'ESIFLD_Facturas_Compra', ID_DocumentoC,
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="#Mensaje#">,
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="#Detalle#">,
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="#SQL#">,
                            <cfqueryparam cfsqltype="cf_sql_varchar" value="#PARAM#">,
