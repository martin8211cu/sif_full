...
<cffunction name="getVentas" access="public" output="false">
	...
	<!--- **************** VENTA CREDITO **************** --->
	<!--- Aqui  empieza el proceso  de extracci�n  de Facturas de CxC de acuerdo  a la fecha de Timbrado---->
	<cfif LABEL EQ 'VENTASCREDITO'>
	<!--- DOCUMENTOS EXISTENTES --->
	<cfquery name="rsGetVtasExist" datasource="sifinterfaces">
		SELECT DISTINCT CONCAT(RTRIM(LTRIM(COALESCE(Numero_Documento, ''))), ':',
		                       RTRIM(LTRIM(COALESCE(Cliente, '')))) AS Numero_Documento,
                        RTRIM(LTRIM(COALESCE(Numero_Documento, ''))) AS Numero_Documento2
		FROM ESIFLD_Facturas_Venta
		WHERE Fecha_Venta BETWEEN <cfqueryparam cfsqltype="cf_sql_timestamp" value="#fechaini#">
								    AND
								   <cfqueryparam cfsqltype="cf_sql_timestamp" value="#fechafin#">
	</cfquery>

		<cfquery name="rsEncCxC" datasource="ldcom">
			SELECT fe.Emp_id,
			       Suc_id,
			       Fiscal_NumeroFactura,
			       Fiscal_Serie + Fiscal_NumeroFactura AS Factura,
			       Fiscal_FechaTimbrado,
			       Fiscal_Fecha,
			       cl.Cliente_CodigoExterno AS Cliente_id,
			       Fiscal_UUID,
			       CASE
			           WHEN Fiscal_Tipo_Factura = 0 THEN 'P'
			           WHEN Fiscal_Tipo_Factura = 1 THEN 'C'
			           ELSE 'C'
			       END AS Tipo_Venta,
			       CASE
			           WHEN Fiscal_Tipo = 1 THEN 'FC'
			           WHEN Fiscal_Tipo = 2 THEN 'NC'
			           ELSE 'FC'
			       END AS Tipo_Documento,
			       Subtotal=(Fiscal_Total-Fiscal_Impuesto+Fiscal_Descuento),
			       Fiscal_Descuento AS Descuento,
			       Fiscal_Impuesto AS Impuesto,
			       Fiscal_IEPS AS IEPS,
			       Fiscal_Total AS Total,
			       coalesce(
			                  (SELECT Cliente_Plazo
			                   FROM Cliente
			                   WHERE Cliente_id= fe.Cliente_id),0) AS DiasCredito,
			       Fiscal_Fecha + coalesce(
			                                 (SELECT Cliente_Plazo
			                                  FROM Cliente
			                                  WHERE Cliente_id= fe.Cliente_id),0) AS Fecha_Vencimiento
			FROM Factura_Fiscal_Encabezado fe
			INNER JOIN Cliente cl ON fe.Cliente_Id = cl.Cliente_Id
			WHERE Fiscal_Estado = 'NO'
			  AND Fiscal_Tipo IN (1,2)
			  AND Fiscal_Tipo_Factura=1
			  <cfif isDefined("form.cbo_Sucursal_Ext") AND #form.cbo_Sucursal_Ext# NEQ -1>
				  AND Suc_id = <cfqueryparam cfsqltype="cf_sql_integer" value='#form.cbo_Sucursal_Ext#'>
			  </cfif>
			  <cfif rsGetVtasExist.RecordCount GT 0>
				 AND CONCAT(RTRIM(LTRIM(COALESCE(CONVERT(varchar,fe.Fiscal_Serie), ''))), RTRIM(LTRIM(COALESCE(CONVERT(varchar,fe.Fiscal_NumeroFactura), ''))), ':',
			                      RTRIM(LTRIM(COALESCE(cl.Cliente_CodigoExterno, cl.Cliente_Id)))) NOT IN (
					<cfloop query="rsGetVtasExist">
						<cfif rsGetVtasExist.CurrentRow NEQ rsGetVtasExist.RecordCount>
							'#rsGetVtasExist.Numero_Documento#',
						<cfelse>
							'#rsGetVtasExist.Numero_Documento#'
						</cfif>
					</cfloop>
			  ...
			  <!--- AND Fiscal_FechaTimbrado  --->
			  AND CONVERT(VARCHAR(10),Fiscal_FechaTimbrado,10) BETWEEN
			  ...
			  <cfelse>
			  	  ...
			  	  <cfqueryparam cfsqltype="cf_sql_date" value='#DateAdd("d", -10, Now())#'>
			  	  AND <cfqueryparam cfsqltype="cf_sql_date" value='#Now()#'>
			  </cfif>
			  <!--- Para Notas de Credito Por Devolucion--->
			  UNION ALL
			SELECT fe.Emp_id,
			       Suc_id,
			       Fiscal_NumeroFactura,
			...
			WHERE Fiscal_Estado = 'NO'
			  AND Fiscal_Tipo IN (1,2)
			  AND Fiscal_Tipo_Factura IN (3,4)
			  <cfif isDefined("form.cbo_Sucursal_Ext") AND #form.cbo_Sucursal_Ext# NEQ -1>
				  AND Suc_id = <cfqueryparam cfsqltype="cf_sql_integer" value='#form.cbo_Sucursal_Ext#'>
			  </cfif>
			  <cfif rsGetVtasExist.RecordCount GT 0>
				 AND CONCAT(RTRIM(LTRIM(COALESCE(CONVERT(varchar,fe.Fiscal_Serie), ''))), RTRIM(LTRIM(COALESCE(CONVERT(varchar,fe.Fiscal_NumeroFactura), ''))), ':') NOT IN (
					<cfloop query="rsGetVtasExist">
						<cfif rsGetVtasExist.CurrentRow NEQ rsGetVtasExist.RecordCount>
							'#rsGetVtasExist.Numero_Documento2#',
						<cfelse>
							'#rsGetVtasExist.Numero_Documento2#'
						</cfif>
					</cfloop>
			...
			<!---  AND (Fiscal_Serie+Fiscal_NumeroFactura) NOT IN
				    (SELECT Numero_Documento
				     FROM #databaseNameInt#.dbo.ESIFLD_Facturas_Venta) --->
			  AND CONVERT(VARCHAR(10),Fiscal_FechaTimbrado,10) BETWEEN
			  <cfif FechaIni GT 0 AND FechaFin GT 0>
			  	  <cfqueryparam cfsqltype="cf_sql_date" value='#FechaIni#'>
			  	  AND <cfqueryparam cfsqltype="cf_sql_date" value='#FechaFin#'>
			  <cfelse>
			  	  ...
			  	  <cfqueryparam cfsqltype="cf_sql_date" value='#DateAdd("d", -10, Now())#'>
			  	  AND <cfqueryparam cfsqltype="cf_sql_date" value='#Now()#'>
			  </cfif> <!--- Para Notas de Credito Por Devolucion--->
		</cfquery>

		<cfif rsEncCxC.RecordCount GT 0>
			<cfloop query="rsEncCxC">
				<!--- CONSULTA DETALLE --->
				<cfquery name="rsDetCxC" datasource="ldcom">
					SELECT Fiscal_NumeroFactura,
					       SUM(Detalle_Impuesto_Monto * Detalle_Cantidad) MontoIVA,
					       CONVERT(int, Detalle_Impuesto_porc) AS Detalle_Impuesto_porc,
					       CASE
					           WHEN Detalle_Precio_Unitario > 0
					           		THEN (Detalle_Total) + SUM(Detalle_Impuesto_Monto * Detalle_Cantidad) + SUM(Detalle_IEPS_Monto * Detalle_Cantidad)
					           ELSE
					           		(Detalle_Total * Detalle_Cantidad)
					       END Total,
					       SUM(Detalle_IEPS_Monto * Detalle_Cantidad) MontoIEPS,
					       CONVERT(int, Detalle_IEPS_Porc) AS Detalle_IEPS_Porc,
					       CASE
					           WHEN Detalle_Precio_Unitario > 0
								    THEN SUM(Detalle_Descuento_Monto)
							   ELSE
									SUM(Detalle_Descuento_Monto * Detalle_Cantidad)
						   END AS Descuento,
					       CASE
					           WHEN Detalle_Precio_Unitario > 0
					           		THEN SUM(Detalle_Total)<!---  - SUM(Detalle_Descuento_Monto) --->
					           ELSE
					           		SUM(Detalle_Total * Detalle_Cantidad) - SUM(Detalle_Impuesto_Monto * Detalle_Cantidad) - SUM(Detalle_IEPS_Monto * Detalle_Cantidad) + SUM(Detalle_Descuento_Monto * Detalle_Cantidad)
					       END Subtotal
					FROM Factura_Fiscal_Detalle
					WHERE Emp_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#rsEncCxC.Emp_id#">
					  AND Suc_Id = <cfqueryparam cfsqltype="cf_sql_integer" value="#rsEncCxC.Suc_Id#">
					  AND Fiscal_NumeroFactura = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsEncCxC.Fiscal_NumeroFactura#">
					GROUP BY Fiscal_NumeroFactura,
					         Detalle_Precio_Unitario,
					         Detalle_Cantidad,
					         Detalle_Impuesto_porc,
					         Detalle_Total,
					         Detalle_IEPS_Porc
				</cfquery>

				<cfquery name="rsDetCxC" dbtype="query">
					SELECT SUM(MontoIVA) AS MontoIVA,
						   Detalle_Impuesto_porc,
						   SUM(Total) AS Total,
						   SUM(MontoIEPS) AS MontoIEPS,
						   Detalle_IEPS_Porc,
						   SUM(Descuento) AS Descuento,
						   SUM(Subtotal) AS Subtotal
					FROM rsDetCxC
					GROUP BY Detalle_Impuesto_porc,
					         Detalle_IEPS_Porc
				</cfquery>
				...
				<cftransaction>
					<cftry>
						...
						<cfquery name="insertVentasCredEnc" datasource="sifinterfaces">
							INSERT INTO ESIFLD_Facturas_Venta
										...
										Retencion, Observaciones, Tipo_CEDI, Estatus, Factura_Cambio, Fecha_Inclusion,
										IEPS, TimbreFiscal,Factura)
							VALUES (<cfqueryparam 	  cfsqltype="cf_sql_numeric"   value="#rsEncCxC.Emp_id#">,
									<cfqueryparam 	  cfsqltype="cf_sql_varchar"   value="LD">,
									<cfqueryparam 	  cfsqltype="cf_sql_numeric"   value="#rsMaxIDECC.MaxID#">,
									<cfif rsEncCxC.Tipo_Documento EQ "FC" OR rsEncCxC.Tipo_Documento EQ "AC">
										<cfqueryparam 	  cfsqltype="cf_sql_varchar"   value="#rsEncCxC.Tipo_Documento#">,
									<cfelse>
										'NC',
									</cfif>
									<cfqueryparam 	  cfsqltype="cf_sql_char" 	   value="#rsEncCxC.Tipo_Venta#">,
									<cfqueryparam 	  cfsqltype="cf_sql_date" 	   value="#dateformat(rsEncCxC.Fiscal_Fecha,"yyyy-mm-dd")#">,
									<cfqueryparam 	  cfsqltype="cf_sql_timestamp" value="#rsEncCxC.Fiscal_Fecha#">,
									<cfqueryparam 	  cfsqltype="cf_sql_timestamp" value="#rsEncCxC.Fecha_Vencimiento#">,
									<cfqueryparam 	  cfsqltype="cf_sql_varchar"   value="#rsEncCxC.Factura#">,
									<cfqueryparam 	  cfsqltype="cf_sql_varchar"   value="#rsEncCxC.Cliente_Id#">,
									<cfqueryparam 	  cfsqltype="cf_sql_null" 	   value="" null="true">,
									abs(<cfqueryparam cfsqltype="cf_sql_money"     value="#numberformat(rsEncCxC.Subtotal,'9.99')#">),
									abs(<cfqueryparam cfsqltype="cf_sql_money"     value="#numberformat(rsEncCxC.Descuento,'9.99')#">),
									abs(<cfqueryparam cfsqltype="cf_sql_money"     value="#numberformat(rsEncCxC.Impuesto,'9.9999')#">),
									abs(<cfqueryparam cfsqltype="cf_sql_money"     value="#numberformat(rsEncCxC.Total,'9.99')#">),
									0,1,
									<cfqueryparam 	  cfsqltype="cf_sql_varchar"   value="#rsEncCxC.Suc_Id#">,
									<cfqueryparam 	  cfsqltype="cf_sql_integer"   value="#rsEncCxC.DiasCredito#">,
									...
									'S',1,0,
									<cfqueryparam cfsqltype="cf_sql_timestamp"    value="#now()#">,
									abs(<cfqueryparam cfsqltype="cf_sql_money"     value="#rsEncCxC.IEPS#">),
									<cfqueryparam 	  cfsqltype="cf_sql_varchar"   value="#rsEncCxC.Fiscal_UUID#">,
									<cfqueryparam 	  cfsqltype="cf_sql_varchar"   value="#rsEncCxC.Factura#">
							)
						</cfquery>
						...
						<cfif rsDetCxC.RecordCount GT 0>
							<cfloop query="rsDetCxC">
								<cfquery name="insertDetCxC" datasource="sifinterfaces">
									INSERT INTO DSIFLD_Facturas_Venta
										(Ecodigo, ID_DocumentoV, ID_linea, Tipo_Lin, Tipo_Item, Clas_Item,
										 ...
										 MontoIEPS, afectaIVA)
									values
										(<cfqueryparam cfsqltype="cf_sql_numeric" value="#rsEncCxC.Emp_id#">,
										 <cfqueryparam cfsqltype="cf_sql_numeric" value="#rsMaxIDECC.MaxID#">,
										 <cfqueryparam cfsqltype="cf_sql_numeric" value="#linDetCxC#">,
										 'S',1,0,
										 <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsDetCxC.Detalle_Impuesto_porc#">,
										 1,
										 abs(convert(decimal(19,2),#rsDetCxC.Total#)),
										 abs(<cfqueryparam cfsqltype="cf_sql_money" value="#numberformat(rsDetCxC.SubTotal,'9.9999')#">),
										 abs(<cfqueryparam cfsqltype="cf_sql_money" value="#numberformat(rsDetCxC.Descuento,'9.9999')#">),
										 0,
										 abs(<cfqueryparam cfsqltype="cf_sql_money" value="#numberformat(rsDetCxC.SubTotal,'9.9999')#">),
										 abs(<cfqueryparam cfsqltype="cf_sql_money" value="#numberformat(rsDetCxC.MontoIVA,'9.9999')#">),
										 0,0,
										 <cfif rsEncCxC.Tipo_Documento EQ "NCD" OR rsEncCxC.Tipo_Documento EQ "NC">
										 	<!--- 'VTASDEVCRE', --->
										 	<cfif #rsDetCxC.MontoIVA# GT 0>
											 	'DEVCRED16',
											<cfelse>
												'DEVCRED0',
											</cfif>
										 ...
										 <cfqueryparam cfsqltype="cf_sql_varchar" value="#rsDetCxC.Detalle_IEPS_Porc#">,
										 abs(<cfqueryparam cfsqltype="cf_sql_money" value="#rsDetCxC.MontoIEPS#">),
										 1
										 )
								</cfquery>
					...
					<cfcatch type="any">
						<cftransaction action="rollback" />
						<cflog file="LOG_Ejecuta_Extraccion_LDCOM" application="no" text="Error al procesar Ventas CXC Fiscales, Error: #cfcatch.Message#">
						<cfthrow message="Error al procesar Ventas CXC Fiscales, Error: #cfcatch.Message#">
					</cfcatch>
					</cftry>
				</cftransaction>
			</cfloop>
