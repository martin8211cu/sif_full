...
<cffunction name="getNotasCreditoDevolucion" access="public" output="false">
...
<cffunction name="getEncabezado" access="private" output="true">
	...
	<cfquery name="rsEncabezado" datasource="#Arguments.DataSource#">
		SELECT s.Cadena_id,
			   ...
			   WHERE ISNULL(fd.TipoDoc_Factura,0) = 4 /*SOLO Devolucion Credito Sucursal*/
				  ...
				  AND fe.tipodoc_id not in (#Arguments.Excluir#) /* solo ventas de contado, devoluciones y servicios */
				   <!---  AND (SELECT COUNT(e.Numero_Documento)
				    	 FROM #databaseNameInt#.dbo.ESIFLD_Facturas_Venta e WHERE e.Tipo_Venta = 'E'
				    	 AND Tipo_Documento = 'NC'
				    	 AND (e.Numero_Documento LIKE 'V-%'
				    	      AND e.Numero_Documento LIKE CONCAT('%', fe.Factura_Id)
				    	      AND e.Numero_Documento LIKE CONCAT('%', fe.Suc_Id, '%'))) = 0 --->
			ORDER BY Cadena_id,
			         Suc_Id,
			         Caja_Id,
			         Factura_Id,
