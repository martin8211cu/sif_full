				...
				<cftry>
					...
					<cfquery name="rsMovBancarios" datasource="sifinterfaces">
						SELECT 	ID_MovimientoB, Ecodigo, Banco_Origen, Cuenta_Origen, Tipo_Operacion, Tipo_Movimiento,
								...
								Documento, Origen, Sucursal, Moneda, coalesce(Tipo_Cambio,1) as Tipo_Cambio, Concepto
						FROM 	SIFLD_Movimientos_Bancarios
					...
					<!--- Agrupamiento --->
					<cfset ParAgrupa = Parametros(Ecodigo=varEcodigo,Pcodigo=9,Sucursal=1,Parametro="Agrupa Documentos",ExtBusqueda=true, Sistema = rsMovBancarios.Origen)>
					...
					<cfif not isdefined("varCuentaB") OR len(varCuentaB) LT 1 >
