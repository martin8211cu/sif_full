<cf_templateheader title="Equivalencias">
	<cf_web_portlet_start titulo="Equivalencias">
		...
		<cfif isdefined('url.EQUidSIF') and not isdefined('form.EQUidSIF')>
			<cfset form.EQUidSIF = url.EQUidSIF>
		</cfif>
		<cfif isdefined('url.SIScodigo') and not isdefined('form.SIScodigo')>
			<cfset form.SIScodigo = url.SIScodigo>
		</cfif>
		<cfif isdefined('url.CATcodigo') and not isdefined('form.CATcodigo')>
			<cfset form.CATcodigo = url.CATcodigo>
		</cfif>
			...
			<tr valign="top">
				<td colspan="4" valign="top" width="50%">
					<cfif len(trim(form.fsistema)) NEQ 0 >
						<cfset ffiltro = ffiltro & " and e.SIScodigo = '" & trim(form.fsistema) & "'">
					...
					<cfset aspbd = Application.dsinfo.asp.schema>
					<cfinvoke component="sif.Componentes.pListas" method="pListaRH" conexion="sifinterfaces"
					 returnvariable="pListaRet">
						<cfinvokeargument name="tabla" value="SIFLD_Equivalencia e inner join #aspbd#..Empresa m on m.Ereferencia = e.EQUempSIF and e.EQUempSIF = #session.Ecodigo# inner join SIFLD_Empresa_Origen o on e.SIScodigo = o.SIScodigo and e.EQUempOrigen = o.EQUempOrigen"/>
						<cfinvokeargument name="columnas" value="e.EQUid, e.SIScodigo, e.CATcodigo, o.EDescripcion, e.EQUcodigoOrigen, e.EQUempSIF, e.EQUcodigoSIF, m.Enombre
															#preservesinglequotes(campos_extra)#"/>
						 <cfinvokeargument name="desplegar" value="EDescripcion, EQUcodigoOrigen, Enombre, EQUcodigoSIF"/>
						...
						<cfinvokeargument name="filtro" value="1=1 #ffiltro# order by e.SIScodigo, CATcodigo"/>
