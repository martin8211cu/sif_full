            ...
            <cfcomponent>
                <cffunction name="getJson" access="remote" returnformat="json">
                    ...
                    <cfquery name="seldata" datasource="#session.dsn#">
                        select distinct a.RHPcodigo,  '<input type=checkbox ' + 
																				   case when (select max(RHPcodigo) from RHOPuestoEquival where rtrim(convert (varchar(255), RHOPid)) = '#session.setFiltroPuesto#') = rtrim(b.RHPcodigo) then
																				   		 ' checked=checked ' 
																				  	else '' 
																				  	end +
																				  	' onclick=fnAgregarQuitar(this,'''+ltrim(rtrim(a.RHPcodigo))+''')>'
																				  	as procesar,
																				   #session.gridTranslatedataCols['gestLvarRHPdescpuesto']# as RHPdescpuesto,
																				   case when (select max(RHPcodigo) from RHOPuestoEquival where rtrim(convert (varchar(255), RHOPid)) = '#session.setFiltroPuesto#') = rtrim(b.RHPcodigo) then
																				   		 1
																				  	else 0 
																				  	end as ordenar 
                        ...
                        <cfif len(trim(lvarDato))>and  upper(rtrim(a.RHPcodigo)) like  <cfqueryparam cfsqltype="cf_sql_varchar" value="%#Ucase(lvarDato)#%"> </cfif>
                        ...
                        <cfif len(trim(lvarDato))><cfif IsNumeric("#lvarDato#")>and  case when '#session.setFiltroPuesto#' = rtrim(convert (varchar(255), b.RHOPid)) then  1 else 0 end =  <cfqueryparam cfsqltype="cf_sql_numeric" value="#lvarDato#"> </cfif></cfif>
                    </cfif>
                    ...
                    <cfset i = 1>
                    <cfset lvarListColumn = "RHPcodigo,RHPdescpuesto,procesar,ORDENAR">
