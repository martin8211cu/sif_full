            ...
            <cfcomponent>
                <cffunction name="getJson" access="remote" returnformat="json">
                    ...
                    <cfquery name="seldata" datasource="#session.dsn#">
                        select distinct a.RHOPid,RHOPDescripcion,case when a.RHOPid = b.RHOPid then '<cf_translate key='LB_Si' xmlfile='/rh/generales.xml'>Si</cf_translate>' else '<cf_translate key='LB_No' xmlfile='/rh/generales.xml'>No</cf_translate>' end as procesar 
                        from RHOPuesto a
                    ...
                    <cfset i = 1>
                    <cfset lvarListColumn = "RHOPDescripcion,procesar,RHOPID">
