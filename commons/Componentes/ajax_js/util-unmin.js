 ...
 * @see http://getahead.ltd.uk/dwr/browser/util/fnExistElement
 */
var fnExistElement;
if (!fnExistElement && document.getElementById) {
  fnExistElement = function() {
...
else if (!fnExistElement && document.all) {
  fnExistElement = function() {
    var elements = new Array();
    for (var i = 0; i < arguments.length; i++) {
      var element = arguments[i];
...
DWRUtil.useLoadingMessage = function(message) {
  ...
  DWREngine.setPreHook(function() {
    var disabledZone = fnExistElement('disabledZone');
    if (!disabledZone) {
      ...
      fnExistElement('messageZone').innerHTML = loadingMessage;
      disabledZone.style.visibility = 'visible';
    }
  });
  DWREngine.setPostHook(function() {
    fnExistElement('disabledZone').style.visibility = 'hidden';
  });
};
...
DWREngine.setPreHook(function() {
var disabledImageZone = fnExistElement('disabledImageZone');
...
fnExistElement('imageZone').src = imageSrc;
disabledImageZone.style.visibility = 'visible';
}
});
DWREngine.setPostHook(function() {
fnExistElement('disabledImageZone').style.visibility = 'hidden';
});
}
...
DWRUtil.highlight = function(ele, options) {
  if (options && options.highlightHandler) {
    options.highlightHandler(fnExistElement(ele));
  }
  else if (DWRUtil._highlightHandler != null) {
    DWRUtil._highlightHandler(fnExistElement(ele));
  }
};
...
DWRUtil._getDataProperties = function(map, prefixes) {
  for (var property in map) {
    if (map[property] != null && typeof map[property] == "object") {
      ...
      if (fnExistElement(nestedProperty) != null || document.getElementsByName(nestedProperty).length >= 1) {
...
DWRUtil.getValues = function(data, options) {
  var ele;
  if (typeof data == "string") ele = fnExistElement(data);
...
/**
 * fnExistElement(ele).className = "X", that we can call from Java easily
 */
...
DWRUtil.debug = function(message) {
  var debug = fnExistElement("dwr-debug");
  if (debug) {
