...
<cfcomponent extends="ajax">
	...
	<cffunction name="fnPreparaHTML1" access="private" output="no"  returntype="boolean" hint="Prepara las instrucciones HTML para el manejo de Listas usando AJAX. Funcion No. 1">
			...
			<!---	Se creo la variable tipmodulo para saber si
					se ingresa al modulo de socios y se agrego un script
					para modificar el tamaño de la tabla para los navegadores
			--->
			<cfset write_to_buffer('
				<script>
				$(document).ready(function(){
				  	if (navigator.userAgent.indexOf("Chrome") !=-1) {
				    	$("table.PlistaTable").removeAttr("width");
				    	$("table.PlistaTable").attr("width", "auto");
					}else{
						$("table.PlistaTable").attr("width", "100%");
					}
				});
				</script>')>
			<!--- Inicio de Tabla --->
				...
				<cfset write_to_buffer(' <tr> ')>
					...
					<cfloop index="i" from="1" to=#ArrayLen(arEtiquetas)#>
						<cfset write_to_buffer('<th class="tituloListas" align="#Trim(alin[i])#" valign="bottom">')>
 						    <!--- Se comenta porque descuadra los filtros, hace falta saber que indica con el filtro de fechas mayores (Eduardo Gonzalez)--->
							...
							<!--- Solo se deja el pintado de las etiquetas --->
							<cfset write_to_buffer(' <strong ')>
								<cfif isdefined("params.fontsize") and len(trim(params.fontsize))>
									<cfset write_to_buffer('style="font-size:#params.fontsize#px"')>
								</cfif>
			...
			<cfloop query="rsLista" startrow="#StartRow_lista#" endrow="#StartRow_lista + MaxRows_lista-1#">
				<!--- FILA DE TOTALES por corte --->
				<cfif lCortes GT 0>
						<cfif datoCorteAnterior eq "">
							<cfset datoCorteAnterior=#Evaluate('rsLista.#Trim(ColsCorte[1])#')# >
						<cfelse>
							<cfif datoCorteAnterior neq #Evaluate('rsLista.#Trim(ColsCorte[1])#')#>
								<cfset x=fnImprimeTotalesCorte()>
							</cfif>
							<cfset datoCorteAnterior=#Evaluate('rsLista.#Trim(ColsCorte[1])#')# >
						</cfif>
				</cfif>
				...
				<!--- Valores para checkbox_function--->
				<cfif Len(Trim(params.keys_checkbox_function))>
					<cfset ListaChkFunc="">
					<cfif Len(Trim(params.keys_checkbox_function))>
						<cfset keyschk2=ListtoArray(ucase(params.keys_checkbox_function),",")>
						<cfloop index="i" from="1" to="#ArrayLen(keyschk2)#">
							<cfset ListaChkFunc = ListaChkFunc & "'"& #Replace(Evaluate('rsLista.#Trim(keyschk2[i])#'),'"',' ','all')# & "',">
						</cfloop>
					</cfif>
					<cfset ListaChkFunc = Trim(Mid(ListaChkFunc,1,len(ListaChkFunc)-1))>
				</cfif>
				...
				<cfif ucase(params.checkboxes) EQ "S" or ucase(params.radios) EQ "S">
					...
					<cfif Len(Trim(params.checkbox_function)) and params.keys_checkbox_function NEQ "" >
						<cfset write_to_buffer('#params.checkbox_function#(#ListaChkFunc#);')>
					<cfelse>
	...
	<cffunction name="fnImprimeTotalesCorte" access="private" output="no" hint="impriem la linea de total del corte">
		 <cfset write_to_buffer('<!-- IAM valor reg #Evaluate('rsLista.#Trim(ColsCorte[1])#')#  -->')>
	...
	<cffunction name="fnPreparaHTMLFin" access="private" output="no" hint="Prepara el final de la pagina ( Area de Botones y Gráficos">
		...
		<cfinvoke component="sif.Componentes.Translate" method="Translate" Key="LB_VistaActual" Default="Vista Actual" XmlFile="/commons/generales.xml" Idioma="" returnvariable="_LvarVistaActual"/>
		<cfinvoke component="sif.Componentes.Translate" method="Translate" Key="LB_de"          Default="de" XmlFile="/commons/generales.xml" Idioma="" returnvariable="_LvarDe"/>

		<cfset _LvarListaLimitada = "">
		<cfif params.maxrowsquery gt 0 and (Desde LTE Hasta)>
			<cfinvoke component="sif.Componentes.Translate" method="Translate" Key="LB_ListaLimitada" Default="Lista Limitada" XmlFile="/commons/generales.xml" Idioma="" returnvariable="_LvarListaLimitada"/>
		...
		<cfif params.usaAJAX>
			<cfif PageNum_lista GT 1>
				<cfset write_to_buffer('<a href="javascript:doQuery_Plista#params.PageIndex#(1)" tabindex="-1"><img src="/cfmx/sif/imagenes/First.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="18" border=0> ')>
			</cfif>
			<cfif PageNum_lista GT 1>
				<cfset write_to_buffer('<a href="javascript:doQuery_Plista#params.PageIndex#(#Max(DecrementValue(PageNum_lista),1)#)" tabindex="-1"><img src="/cfmx/sif/imagenes/Previous.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="14" border=0> ')>
			</cfif>
			<cfif PageNum_lista LT TotalPages_lista>
				<cfset write_to_buffer('<a href="javascript:doQuery_Plista#params.PageIndex#(#Min(IncrementValue(PageNum_lista),TotalPages_lista)#)" tabindex="-1"><img src="/cfmx/sif/imagenes/Next.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="14" border=0> ')>
			</cfif>
			<cfif PageNum_lista LT TotalPages_lista>
				<cfset write_to_buffer('<a href="javascript:doQuery_Plista#params.PageIndex#(#TotalPages_lista#)" tabindex="-1"><img src="/cfmx/sif/imagenes/Last.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="18" border=0> ')>
			</cfif>
		<cfelse>
			<!---Primera pagina--->
			<cfif PageNum_lista GT 1>
				<cfset write_to_buffer('<a href="javascript: if(!window.FuncPlistasChangePage) window.location.href=''#CurrentPage#?PageNum_lista#params.PageIndex#=1#QueryString_lista#''; else FuncPlistasChangePage(''#CurrentPage#'',''#params.PageIndex#'',''1'',''#QueryString_lista#'');" tabindex="-1"><img src="/cfmx/sif/imagenes/First.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="18" border=0> ')>
			</cfif>
			<!---Pagina Anterior--->
			<cfif PageNum_lista GT 1>
				<cfset write_to_buffer('<a href="javascript: if(!window.FuncPlistasChangePage) window.location.href=''#CurrentPage#?PageNum_lista#params.PageIndex#=#Max(DecrementValue(PageNum_lista),1)##QueryString_lista#''; else FuncPlistasChangePage(''#CurrentPage#'',''#params.PageIndex#'',''#Max(DecrementValue(PageNum_lista),1)#'',''#QueryString_lista#'');" tabindex="-1"><img src="/cfmx/sif/imagenes/Previous.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="14" border=0> ')>
			</cfif>
			<!---Pagina Siguiente--->
			<cfif PageNum_lista LT TotalPages_lista>
				<cfset write_to_buffer('<a  href="javascript: if(!window.FuncPlistasChangePage) window.location.href=''#CurrentPage#?PageNum_lista#params.PageIndex#=#Min(IncrementValue(PageNum_lista),TotalPages_lista)##QueryString_lista#''; else FuncPlistasChangePage(''#CurrentPage#'',''#params.PageIndex#'',''#Min(IncrementValue(PageNum_lista),TotalPages_lista)#'',''#QueryString_lista#'');" tabindex="-1"><img src="/cfmx/sif/imagenes/Next.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="14" border=0> ')>
			</cfif>
			<!---Ultima Pagina--->
			<cfif PageNum_lista LT TotalPages_lista>
				<cfset write_to_buffer('<a href="javascript: if(!window.FuncPlistasChangePage) window.location.href=''#CurrentPage#?PageNum_lista#params.PageIndex#=#TotalPages_lista##QueryString_lista#''; else FuncPlistasChangePage(''#CurrentPage#'',''#params.PageIndex#'',''#TotalPages_lista#'',''#QueryString_lista#'');" tabindex="-1"><img src="/cfmx/sif/imagenes/Last.gif" border=0></a> ')>
			<cfelse>
				<cfset write_to_buffer('<img src="/cfmx/sif/imagenes/c2.gif" width="18" border=0> ')>
			</cfif>
		</cfif>
	...
	<cffunction name="getQuery" output="no" access="private">
		...
		<cfelse>
			...
			<!---
			<cfset LvarParametros = createobject("component","commons.Componentes.json").decode(rsSQL.PlistaPRM)>
			--->
