...
function $Fred(something) {
	...
	if (something instanceof Array) {
		var elm = [];
		for (var i=0;i<something.length;i++) { elm.push($Fred(something[i])); }
	}
	if (!elm) return false;
	return elm;
}

function $Fred$(className, root) {
	var e = root || document;
...
OAT.Dom = { /* DOM common object */
	...
	option:function(name,value,parent) {
		...
		opt.innerHTML = name;
		opt.value = value;
		if (parent) { $Fred(parent).appendChild(opt); }
		return opt;
	},
	...
	append:function() {
		for (var i=0;i<arguments.length;i++) {
			...
			var parent = $Fred(arr[0]);
			for (var j=1;j<arr.length;j++) {
				var children = arr[j];
				if (!(children instanceof Array)) { children = [children]; }
				for (var k=0;k<children.length;k++) {
					var child = children[k];
					parent.appendChild($Fred(child));
				}
			}
		}
	},
	...
	isChild:function(child,parent) {
		var c_elm = $Fred(child);
		var p_elm = $Fred(parent);
...
OAT.Loader = { /* first part of loader object */
	...
	include:function(file) {
		...
		for (var i=0;i<value.length;i++) {
			...
			script.src = '/cfmx/commons/js/pivot/'+name; 
