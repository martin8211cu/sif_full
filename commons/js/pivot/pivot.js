...
OAT.Pivot = function(div,chartDiv,filterDiv,headerRow,dataRows,headerRowIndexes,headerColIndexes,filterIndexes,dataColumnIndex,optObj) {
	...
	this.options = {
		...
		aggTotals:3, /* dtto for subtotals & totals */
		showChart:0,
		showRowChart:0,
		showColChart:0,
        showLineChart:1,
	...
	this.defCArray = ["rgb(255,0,0)","rgb(0,120,0)","rgb(0,0,120)","rgb(0,0,250)","rgb(255,255,0)",
						"rgb(0,255,255)","rgb(255,0,255)","rgb(192,192,192)"];
	
	if (this.chartDiv && this.options.mostrarGraficos==1) { 
	...
	this.getClickReference = function(cond) {
		...
		return function(event) {
			...
			alabel.innerHTML = LB_Ascendente;
			...
			dlabel.innerHTML = LB_Descendente;
	...
	this.distinctDivs = function(cond,div) { /* set of distinct values checboxes */
		...
		var all = OAT.Dom.button(LB_Todos);
		OAT.Dom.attach(all,"click",allRef);
		
		var none = OAT.Dom.button(LB_Nada);
		OAT.Dom.attach(none,"click",noneRef);

		var reverse = OAT.Dom.button(LB_Inverso);
	...
	this.drawFilters = function() {
		...
		if (!self.filterIndexes.length) {
			div.innerHTML = LB_Filtro +": ["+LB_ArrastreColumnasAqui+"]";
		}
		for (var i=0;i<self.filterIndexes.length;i++) {
	...
	this.numericalType = function(event) {
		...
		/* contents */
		var type = OAT.Dom.text(LB_TipoNumerico+": ");
		...
		l.innerHTML = LB_MostrarDatosVacios;
	...
	this._drawLineChart = function() {
		...
		for (var i=0;i<self.tabularData.length;i++) { 			
			var col = [];
			for (var j=0;j<self.tabularData[0].length;j++) {
				if(j==0){col.push(0);}
				col.push(self.tabularData[i][j]); 
			}
			data.push(col); 
		}
	...
	this._drawCharts = function() {
		...
		if (self.options.showChart) {
			...
			self._drawChart();
			self.charts.mainLink.value = LB_OcultarTabla;
			self.charts.mainLink.className = self.charts.colorON;
		} else {
			try{
			OAT.Dom.hide(self.charts.mainDiv);
			self.charts.mainLink.value = LB_MostrarTabla;
		...
		if (self.options.showRowChart && self.options.totals) {
			...
			self._drawRowChart();
			self.charts.rowLink.value = LB_OcultarFilasTotales;
			self.charts.rowLink.className = self.charts.colorON;
		} else {
			try{
			OAT.Dom.hide(self.charts.rowDiv);
			self.charts.rowLink.value = LB_MostrarFilasTotales;
		...
		if (self.options.showColChart && self.options.totals) {
			...
			self._drawColChart();
			self.charts.colLink.value = LB_OcultarColumnasTotales;
			self.charts.colLink.className = self.charts.colorON;
		} else {
			try{
			OAT.Dom.hide(self.charts.colDiv);
			self.charts.colLink.value = LB_MostrarColumnasTotales;
		...
		if (self.options.showLineChart && self.options.totals) {
			...
			self._drawLineChart();
			self.charts.lineLink.value = LB_OcultarGrafico;
			self.charts.lineLink.className = self.charts.colorON;
			}catch(err){}
		} else {
			try{
			OAT.Dom.hide(self.charts.lineDiv);
			self.charts.lineLink.value = LB_MostrarGrafico;
