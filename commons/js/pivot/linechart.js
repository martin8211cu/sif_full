/*
 *  $Id: linechart.js,v 1.4 2007/05/18 12:03:38 source Exp $
 *
...
OAT.LineChart = function(div,optObj) {
	...
	this.options = {
		...
		legend:true,
		markerSize:8,
		colors:["#f00","#00f","#0f0","#ff0","#f00","#00f","#0f0","#ff0"],
	...
	this.draw = function() {
		...
		if (self.options.desc) { /* x labels */
			...
			for (var i=0;i<self.textX.length;i++) {
				...
				var t = OAT.SVG.element("text",{"text-anchor":"start",x:x,y:y+self.options.fontSize+2,"font-size":self.options.fontSize,"writing-mode":"tb"}); 
