...
DEMO.pivot = {
	...
	cb:function() {
		...
		for (var i=0;i<OAT.Statistics.list.length;i++) {
			...
			if (pivot.options.agg == i) { $Fred("pivot_agg").selectedIndex = i; }
			if (pivot.options.aggTotals == i) { $Fred("pivot_agg_totals").selectedIndex = i; }
		}
	...
	cb:function() {
		...
		var openRef = function(event) {
			var callback = function(date) {
				$Fred("calendar_value").innerHTML = date[0]+"/"+date[1]+"/"+date[2];
			}
...
DEMO.color = {
	...
	cb:function() {
		...
		var colorRef = function(event) {
			var callback = function(color) { $Fred("color_content").style.backgroundColor = color;}
...
DEMO.json = {
	...
	cb:function() {
		var jsonRef = function(event) {
			...
			data = data.replace(/\n/g,"<br/>");
			data = data.replace(/ /g,"&nbsp;");
			$Fred("json_content").innerHTML = data;
		}
...
DEMO.upload = {
	...
	cb:function() {
		...
		$Fred("upload_content").appendChild(u.div);
	...
	cb:function() {
		...
		var searchReceiveRef = function(data,index) {
			...
			for (var i=0;i<cnt;i++) {
				...
				num.innerHTML = (i+1)+". ";
				$Fred("dock_results").appendChild(num);
				var val = data[i];
				var a = OAT.Dom.create("a");
				a.href = val[0];
				a.innerHTML = val[1];
				$Fred("dock_results").appendChild(a);
				var br = OAT.Dom.create("br");
				$Fred("dock_results").appendChild(br);
			}
		}
