...
<cfquery name="rsCodICTS" datasource="sifinterfaces">
	select CodICTS, b.acct_full_name,EcodigoSDCSoin
...
<!--- Ciclo de Procesamiento de los Registros --->
		...
		<!---
		<cfquery datasource="sifinterfaces">
		update ID12 set Monto=(select sum(MontoOrigen) from icts_soin.IE12 where DocumentoOrigen='#rsRegistrosProcesar.DocumentoOrigen#'
	  	and EcodigoSDC= <cfqueryparam cfsqltype="cf_sql_integer" value="#varCodICTS#">  and Procesado='N' group by DocumentoOrigen ) where 
		Documento ='#rsRegistrosProcesar.DocumentoOrigen#'
		</cfquery
--->
		...
		<cfquery datasource="sifinterfaces">
		update ID12 set Monto=(select sum(MontoOrigen) from icts_soin.IE12 where DocumentoOrigen='#rsRegistrosProcesar.DocumentoOrigen#'
	  	and EcodigoSDC= <cfqueryparam cfsqltype="cf_sql_integer" value="#varCodICTS#">  and Procesado='N' and NumeroSocioDocOrigen = '#rsRegistrosProcesar.NumerosocioDocOrigen#'
		and CodigoMonedaOrigen = '#rsRegistrosProcesar.CodigoMonedaOrigen#' and TipoCambio = #rsRegistrosProcesar.TipoCambio#
		group by DocumentoOrigen ) where 
		Documento ='#rsRegistrosProcesar.DocumentoOrigen#'
		</cfquery>
...
<!--- Inclusión de movimiento en cola de proceso --->
		...
		<cfquery name="rsIE12" datasource="sifinterfaces">
				...
				from IE12
				where EcodigoSDC= #rsCodICTS.EcodigoSDCSoin#
				and ID not in (select IdProceso from InterfazBitacoraProcesos where EcodigoSDC= #rsCodICTS.EcodigoSDCSoin# and NumeroInterfaz=12) 
			  	and ID not in(select IdProceso from InterfazColaProcesos where EcodigoSDC= #rsCodICTS.EcodigoSDCSoin# and NumeroInterfaz=12)
				and Procesado='N'
		</cfquery>
<cfloop query="rsIE12"> 
		<cfquery datasource="sifinterfaces">
