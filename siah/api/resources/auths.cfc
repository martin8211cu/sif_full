...
<cfcomponent extends="taffy.core.resource" taffy_uri="/auths/{empresa}/{filtro}">
		...
		<cftry>
			<cfquery name="rsPermiso" datasource="asp">
				...
				where u.Usucodigo = #arguments.userData.usucodigo#
					...
					<cfif len(trim(arguments.filtro)) gt 0>
						and lower(concat(ltrim(rtrim(p.SScodigo)),'-',ltrim(rtrim(p.SMcodigo)),'-',ltrim(rtrim(p.SPcodigo)))) like lower('#arguments.filtro#%')
					</cfif>
			</cfquery>
